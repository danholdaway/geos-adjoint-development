...
!------------------------------
   ...
   use tp_core_mod,       only: fv_tp_2d, fv_tp_2d_fb
...
! !INPUT PARAMETERS:
  ...
  real(FVPRC), intent(in), dimension(is-ng:ie+ng,js-ng:je+ng):: area
...
!$OMP parallel do default(none) shared(is1,ie1,js1,je1,ws,zs,gz,rdt,km)
  ...
  subroutine update_dz_d(ndif, damp, hord, hord_pert, is, ie, js, je, km, ng, npx, npy, area, rarea,   &
...
!-----------------------------------------------------
! Local array:
  real(FVPRC), dimension(is:   ie+1, js-ng:je+ng,km+1):: crx_adv, xfx_adv
  real(FVPRC), dimension(is-ng:ie+ng,js:   je+1,km+1 ):: cry_adv, yfx_adv
...
!$OMP                          private(ra_x, ra_y, fx, fy)
  do k=1,km+1
     ...
     if (hord == hord_pert) then
        call fv_tp_2d_fb(zh(isd:ied,jsd:jed,k), crx_adv(is:ie+1,jsd:jed,k), cry_adv(isd:ied,js:je+1,k), npx,  npy, hord, &
        ...
        fx_tj = fx
        fy_tj = fy
        !UNCOM  call fv_tp_2d(zh(isd:ied,jsd:jed,k), crx_adv(is:ie+1,jsd:jed,k), cry_adv(isd:ied,js:je+1,k), npx,  npy, hord, &
        !UNCOM                fx, fy, xfx_adv(is:ie+1,jsd:jed,k), yfx_adv(isd:ied,js:je+1,k), gridstruct, bd, ra_x, ra_y,       &
        !UNCOM                nord=ndif(k), damp_c=damp(k))
        call fv_tp_2d(zh(isd:ied,jsd:jed,k), crx_adv(is:ie+1,jsd:jed,k), cry_adv(isd:ied,js:je+1,k), npx,  npy, hord_pert, &
                      fx, fy, xfx_adv(is:ie+1,jsd:jed,k), yfx_adv(isd:ied,js:je+1,k), gridstruct, bd, ra_x, ra_y,       &
                      nord=ndif(k), damp_c=damp(k))  
     endif
     do j=js,je
...
! Enforce monotonicity of height to prevent blowup
  ...
  subroutine Riem_Solver_c(ms,   dt,  is,   ie,   js, je, km,   ng,  &
   ...
   real(FVPRC), intent(in), dimension(is-ng:ie+ng,js-ng:je+ng,km):: pt, delp
   real(FVPRC), intent(in), dimension(is-ng:is-ng,js-ng:js-ng,1):: q_con, cappa
   real(FVPRC), intent(in)::   hs(is-ng:ie+ng,js-ng:je+ng)
   real(FVPRC), intent(in), dimension(is-ng:ie+ng,js-ng:je+ng,km):: w3
! OUTPUT PARAMETERS 
   real(FVPRC), intent(inout), dimension(is-ng:ie+ng,js-ng:je+ng,km+1):: gz
   real(FVPRC), intent(  out), dimension(is-ng:ie+ng,js-ng:je+ng,km+1):: pef
! Local:
  real(FVPRC), dimension(is-1:ie+1,km  ):: dm, dz2, w2, pt2, pm2, gm2, cp2
...
!--------------------------------------------
   ...
   real(FVPRC), intent(in):: dt         ! the BIG horizontal Lagrangian time step
   ...
   real(FVPRC), intent(in), dimension(isd:isd,jsd:jsd,1):: q_con, cappa
   real(FVPRC), intent(in), dimension(isd:ied,jsd:jed,km):: delp, pt
   real(FVPRC), intent(inout), dimension(isd:ied,jsd:jed,km+1):: zh
   real(FVPRC), intent(inout), dimension(isd:ied,jsd:jed,km):: w
   real(FVPRC), intent(inout):: pe(is-1:ie+1,km+1,js-1:je+1)
   real(FVPRC), intent(out):: peln(is:ie,km+1,js:je)          ! ln(pe)
   real(FVPRC), intent(out), dimension(isd:ied,jsd:jed,km+1):: ppe
   real(FVPRC), intent(out):: delz(is-ng:ie+ng,js-ng:je+ng,km)
...
! Local:
  ...
  real(FVPRC) :: peln_cptmp(is:ie,km+1,js:je)          ! ln(pe)
...
2000  continue
  ...
  real(FVPRC), intent(in) :: delz(is-ng:ie+ng, km)  ! delta-height (m)
  real(FVPRC), intent(in) :: w(is:ie, km)  ! vertical vel. (m/s)
...
! Bottom:
  ...
  real(FVPRC),    intent(in), dimension(is:ie,km):: dm2, pm2, gm2
...
! Local:
  ...
  real(FVPRC), dimension(km+1):: m_bot, m_top, r_bot, r_top, pe1, pbar, wbar
...
6000  continue        ! end i-loop
 ...
 subroutine SIM3_solver(dt,  is,  ie, km, rgas, gama, kappa, pe2, dm,   &
   ...
   real(FVPRC), intent(in):: dt, rgas, gama, kappa, alpha, p_fac, scale_m
   real(FVPRC), intent(in   ), dimension(is:ie,km):: dm, pt2
   real(FVPRC), intent(in )::  ws(is:ie)
   real(FVPRC), intent(in ), dimension(is:ie,km+1):: pem
   real(FVPRC), intent(out):: pe2(is:ie,km+1)
   real(FVPRC), intent(inout), dimension(is:ie,km):: dz2, w2
! Local
...
! Recover cell-averaged pressure
 ...
 subroutine SIM1_solver(dt,  is,  ie, km, rgas, gama, gm2, cp2, kappa, pe, dm2,   &
   ...
   real(FVPRC), intent(in), dimension(is:ie,km):: dm2, pt2, pm2, gm2, cp2
   real(FVPRC), intent(in )::  ws(is:ie)
   real(FVPRC), intent(in ), dimension(is:ie,km+1):: pem
...
!#endif
 ...
 subroutine SIM_solver(dt,  is,  ie, km, rgas, gama, gm2, cp2, kappa, pe2, dm2,   &
   ...
   integer, intent(in):: is, ie, km
   real(FVPRC), intent(in):: dt, rgas, gama, kappa, p_fac, alpha, scale_m
   real(FVPRC), intent(in), dimension(is:ie,km):: dm2, pt2, pm2, gm2, cp2
   real(FVPRC), intent(in )::  ws(is:ie)
   real(FVPRC), intent(in ), dimension(is:ie,km+1):: pem
   real(FVPRC), intent(out):: pe2(is:ie,km+1)
...
! Optimized for wind profile reconstruction:
 ...
 real(FVPRC), intent(in ), dimension(i1:i2,km):: q1
 real(FVPRC), intent(out), dimension(i1:i2,km+1):: qe
!-----------------------------------------------------------------------
...
! Optimized for wind profile reconstruction:
 ...
 real(FVPRC), intent(in):: dp0(km)
 real(FVPRC), intent(in),  dimension(i1:i2,j1:j2,km):: q1, q2
 real(FVPRC), intent(out), dimension(i1:i2,j1:j2,km+1):: q1e, q2e
!-----------------------------------------------------------------------
 real(FVPRC), dimension(i1:i2,km+1):: qe1, qe2, gam  ! edge values
 real(FVPRC)  gak(km)
...
! Surface:
 ...
 subroutine geopk_halo_nh(ptop, grav, kappa, cp, delp, delz, pt, phis, pkc, gz, pk3, &
      ...
      real(FVPRC), intent(IN),  dimension(bd%isd:bd%ied,bd%jsd:bd%jed,npz):: pt, delp, delz
      real(FVPRC), intent(INOUT), dimension(bd%isd:bd%ied,bd%jsd:bd%jed,npz+1):: gz, pkc, pk3

      integer :: i,j,k
      real(FVPRC) :: gama !'gamma'
      real(FVPRC) :: rcp
      real(FVPRC) :: ptk, rgrav, rkap
      real(FVPRC) :: peln1

      real(FVPRC), dimension(bd%isd:bd%ied, npz+1, bd%jsd:bd%jed ) :: pe, peln
