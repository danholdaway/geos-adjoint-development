...
& fv_tp_2d_tlm, copy_corners, copy_corners_tlm
  USE FV_MP_MOD_D, ONLY : mp_reduce_max
  USE FV_MP_MOD_D, ONLY : ng, mp_gather, is_master
...
& mpp_update_domains_tlm, cgrid_ne, domain2d, mpp_get_boundary, &
& mpp_get_boundary_tlm
  USE FV_TIMING_MOD, ONLY : timing_on, timing_off
!   use boundary_mod,      only: nested_grid_BC_apply, nested_grid_BC_apply_intT
  USE FV_ARRAYS_MOD_D, ONLY : fv_grid_type, fv_nest_type, fv_atmos_type,&
& fv_grid_bounds_type, real4, real8, fvprc
  USE MPP_MOD_D, ONLY : mpp_error, fatal, mpp_broadcast, mpp_send, &
...
!, offline_tracer_advection
  ...
  PUBLIC tracer_2d_tlm, tracer_2d_nested_tlm
...
&                    , mfx=mfx2, mfy=mfy2)
        ELSE
          ...
          fx_tj = fx
          fy_tj = fy
      call fv_tp_2d( q(isd:ied,jsd:jed,iq), cx2, cy2, npx, npy, hord, fx, fy, &
...
!  Differentiation of tracer_2d in forward (tangent) mode (with options r8):
!   variations   of useful results: q dp1 mfx mfy cx cy
...
! DELP before dyn_core
    ...
    REAL(fvprc), INTENT(INOUT) :: dp1_tl(bd%is:bd%ie, bd%js:bd%je, npz)
! Mass Flux X-Dir
...
! Local Arrays
    ...
    REAL(fvprc) :: dp2_tl(bd%is:bd%ie, bd%js:bd%je, npz)
    REAL(fvprc) :: fx(bd%is:bd%ie+1, bd%js:bd%je, npz, nq)
    REAL(fvprc) :: fx_tl(bd%is:bd%ie+1, bd%js:bd%je, npz, nq)
    REAL(fvprc) :: fy(bd%is:bd%ie, bd%js:bd%je+1, npz, nq)
    REAL(fvprc) :: fy_tl(bd%is:bd%ie, bd%js:bd%je+1, npz, nq)
    REAL(fvprc) :: ra_x(bd%is:bd%ie, bd%jsd:bd%jed, npz)
    REAL(fvprc) :: ra_x_tl(bd%is:bd%ie, bd%jsd:bd%jed, npz)
    REAL(fvprc) :: ra_y(bd%isd:bd%ied, bd%js:bd%je, npz)
    REAL(fvprc) :: ra_y_tl(bd%isd:bd%ied, bd%js:bd%je, npz)
      ...
      DO j=jsd,jed
        DO i=is,ie+1
          IF (cx(i, j, k) .GT. 0.) THEN
            xfx_tl(i, j, k) = gridstruct%dxa(i-1, j)*gridstruct%dy(i, j)&
&             *gridstruct%sin_sg(i-1, j, 3)*cx_tl(i, j, k)
            xfx(i, j, k) = cx(i, j, k)*gridstruct%dxa(i-1, j)*gridstruct&
&             %dy(i, j)*gridstruct%sin_sg(i-1, j, 3)
          ELSE
            xfx_tl(i, j, k) = gridstruct%dxa(i, j)*gridstruct%dy(i, j)*&
&             gridstruct%sin_sg(i, j, 1)*cx_tl(i, j, k)
      ...
      DO j=js,je+1
        DO i=isd,ied
          IF (cy(i, j, k) .GT. 0.) THEN
            yfx_tl(i, j, k) = gridstruct%dya(i, j-1)*gridstruct%dx(i, j)&
&             *gridstruct%sin_sg(i, j-1, 4)*cy_tl(i, j, k)
            yfx(i, j, k) = cy(i, j, k)*gridstruct%dya(i, j-1)*gridstruct&
&             %dx(i, j)*gridstruct%sin_sg(i, j-1, 4)
          ELSE
            yfx_tl(i, j, k) = gridstruct%dya(i, j)*gridstruct%dx(i, j)*&
&             gridstruct%sin_sg(i, j, 2)*cy_tl(i, j, k)
...
!$OMP                          private(dp2, ra_x, ra_y, fx, fy)
      DO k=1,npz
        DO j=js,je
          DO i=is,ie
            dp2_tl(i, j, k) = dp1_tl(i, j, k) + gridstruct%rarea(i, j)*(&
&             mfx_tl(i, j, k)-mfx_tl(i+1, j, k)+mfy_tl(i, j, k)-mfy_tl(i&
&             , j+1, k))
...
&             mfy(i, j, k)-mfy(i, j+1, k))*gridstruct%rarea(i, j)
        ...
        DO j=jsd,jed
          DO i=is,ie
            ra_x_tl(i, j, k) = xfx_tl(i, j, k) - xfx_tl(i+1, j, k)
...
&             +1, j, k)
        ...
        DO j=js,je
          DO i=isd,ied
            ra_y_tl(i, j, k) = yfx_tl(i, j, k) - yfx_tl(i, j+1, k)
...
&                          npy, hord, fx(is:ie+1, js:je, k, iq), fx_tl(&
&                          is:ie+1, js:je, k, iq), fy(is:ie, js:je+1, k&
&                          , iq), fy_tl(is:ie, js:je+1, k, iq), xfx(is:&
&                          ie+1, jsd:jed, k), xfx_tl(is:ie+1, jsd:jed, k&
&                          ), yfx(isd:ied, js:je+1, k), yfx_tl(isd:ied, &
&                          js:je+1, k), gridstruct, bd, ra_x(is:ie, jsd:&
&                          jed, k), ra_x_tl(is:ie, jsd:jed, k), ra_y(isd&
&                          :ied, js:je, k), ra_y_tl(isd:ied, js:je, k), &
&                          mfx=mfx(is:ie+1, js:je, k), mfx_tl=mfx_tl(is:&
&                          ie+1, js:je, k), mfy=mfy(is:ie, js:je+1, k), &
&                          mfy_tl=mfy_tl(is:ie, js:je+1, k))
          ELSE
            q_tj(:, :, k, iq) = q(:, :, k, iq)
            fx_tj(:, :, k, iq) = fx(:, :, k, iq)
            fy_tj(:, :, k, iq) = fy(:, :, k, iq)
...
&                       , fx(is:ie+1, js:je, k, iq), fx_tl(is:ie+1, js:&
&                       je, k, iq), fy(is:ie, js:je+1, k, iq), fy_tl(is:&
&                       ie, js:je+1, k, iq), xfx(is:ie+1, jsd:jed, k), &
&                       xfx_tl(is:ie+1, jsd:jed, k), yfx(isd:ied, js:je+&
&                       1, k), yfx_tl(isd:ied, js:je+1, k), gridstruct, &
&                       bd, ra_x(is:ie, jsd:jed, k), ra_x_tl(is:ie, jsd:&
&                       jed, k), ra_y(isd:ied, js:je, k), ra_y_tl(isd:&
&                       ied, js:je, k), mfx=mfx(is:ie+1, js:je, k), &
&                       mfx_tl=mfx_tl(is:ie+1, js:je, k), mfy=mfy(is:ie&
&                       , js:je+1, k), mfy_tl=mfy_tl(is:ie, js:je+1, k))
          END IF
        END DO
      END DO
! npz
...
&               j, k, iq)*dp1_tl(i, j, k)+gridstruct%rarea(i, j)*(fx_tl(&
&               i, j, k, iq)-fx_tl(i+1, j, k, iq)+fy_tl(i, j, k, iq)-&
&               fy_tl(i, j+1, k, iq)))*dp2(i, j, k)-(q(i, j, k, iq)*dp1(&
&               i, j, k)+(fx(i, j, k, iq)-fx(i+1, j, k, iq)+fy(i, j, k, &
&               iq)-fy(i, j+1, k, iq))*gridstruct%rarea(i, j))*dp2_tl(i&
&               , j, k))/dp2(i, j, k)**2
...
! npz
      ...
      END IF
! npz
      IF (it .NE. nsplt) CALL START_GROUP_HALO_UPDATE_TLM(q_pack, &
&                                                   q_packp, q, q_tl, &
&                                                   domain)
!call timing_on('COMM_TOTAL')
!call timing_on('COMM_TRACER')
...
!$OMP parallel do default(none) shared(is,ie,js,je,npz,dp1,xfx,yfx,rarea,rdt)
      DO k=1,npz
        DO j=js,je
          DO i=is,ie
            dp1_tl(i, j, k) = gridstruct%rarea(i, j)*rdt*(xfx_tl(i+1, j&
&             , k)-xfx_tl(i, j, k)+yfx_tl(i, j+1, k)-yfx_tl(i, j, k))
...
!  Differentiation of tracer_2d_nested in forward (tangent) mode (with options r8):
!   variations   of useful results: q dp1 mfx mfy cx cy
!   with respect to varying inputs: q dp1 mfx mfy cx cy
...
&                       =mfy_tl(is:ie, js:je+1, k))
          ...
          DO j=js,je
            DO i=is,ie
              q_tl(i, j, k, iq) = ((q_tl(i, j, k, iq)*dp1(i, j, k)+q(i, &
&               j, k, iq)*dp1_tl(i, j, k)+gridstruct%rarea(i, j)*(fx_tl(&
&               i, j)-fx_tl(i+1, j)+fy_tl(i, j)-fy_tl(i, j+1)))*dp2(i, j&
&               , k)-(q(i, j, k, iq)*dp1(i, j, k)+(fx(i, j)-fx(i+1, j)+&
&               fy(i, j)-fy(i, j+1))*gridstruct%rarea(i, j))*dp2_tl(i, j&
&               , k))/dp2(i, j, k)**2
...
!call timing_on('COMM_TOTAL')
!call timing_on('COMM_TRACER')
        CALL START_GROUP_HALO_UPDATE_TLM(q_pack, q_packp, q, q_tl, &
&                                  domain)
!call timing_off('COMM_TRACER')
!call timing_off('COMM_TOTAL')
        ...
        DO k=1,npz
          DO j=js,je
            DO i=is,ie
              ...
              dp1(i, j, k) = dp2(i, j, k)
              dp2_tl(i, j, k) = dp1_tl(i, j, k) + gridstruct%rarea(i, j)&
&               *(mfx_tl(i, j, k)-mfx_tl(i+1, j, k)+mfy_tl(i, j, k)-&
&               mfy_tl(i, j+1, k))
              dp2(i, j, k) = dp1(i, j, k) + (mfx(i, j, k)-mfx(i+1, j, k)&
&               +mfy(i, j, k)-mfy(i, j+1, k))*gridstruct%rarea(i, j)
            END DO
