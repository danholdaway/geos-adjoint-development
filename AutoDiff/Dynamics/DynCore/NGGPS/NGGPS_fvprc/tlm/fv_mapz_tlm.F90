...
!#endif
  ...
  USE FV_GRID_UTILS_MOD_D, ONLY : g_sum, g_sum_tlm, ptop_min
...
!  use mpp_mod,           only: FATAL, mpp_error, get_unit, mpp_root_pe, mpp_pe
  USE FV_ARRAYS_MOD_D, ONLY : fv_grid_type
...
& map1_cubic_tlm, map1_q2_tlm

CONTAINS
!  Differentiation of lagrangian_to_eulerian in forward (tangent) mode (with options r8):
!   variations   of useful results: peln q u v w ze0 delp ua delz
!                omga pkz pe pk ps pt te
...
&   kord_wz_pert, kord_tr_pert, kord_tm_pert, peln, peln_tl, te0_2d, &
&   te0_2d_tl, ng, ua, ua_tl, va, omga, omga_tl, te, te_tl, ws, ws_tl, &
&   fill, reproduce_sum, out_dt, dtdt, ptop, ak, bk, gridstruct, domain&
&   , ze0, ze0_tl, gmao_cubic, remap_t, do_sat_adj, hydrostatic, &
...
! surface geopotential
    ...
    REAL(fvprc), INTENT(INOUT) :: te0_2d_tl(is:ie, js:je)
...
! pe to the kappa
    ...
    REAL(fvprc), INTENT(INOUT) :: pk_tl(is:ie, js:je, km+1)
    REAL(fvprc), INTENT(INOUT) :: q(isd:ied, jsd:jed, km, nq)
    REAL(fvprc), INTENT(INOUT) :: q_tl(isd:ied, jsd:jed, km, nq)
! pressure thickness
    REAL(fvprc), INTENT(INOUT) :: delp(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: delp_tl(isd:ied, jsd:jed, km)
...
! u-wind (m/s) on physics grid
    ...
    REAL(fvprc), INTENT(INOUT) :: ua_tl(isd:ied, jsd:jed, km)
...
! in case fillz is not sufficient
    ...
    REAL(fvprc), DIMENSION(is:ie, js:je) :: te_2d_tl, zsum0_tl, zsum1_tl
    REAL(fvprc), DIMENSION(is:ie, km) :: q2, dp2, deng
    REAL(fvprc), DIMENSION(is:ie, km) :: q2_tl, dp2_tl, deng_tl
    REAL(fvprc), DIMENSION(is:ie, km+1) :: ze1, ze2, pe1, pe2, pk1, pk2&
&   , pn2, phis
    REAL(fvprc), DIMENSION(is:ie, km+1) :: ze1_tl, ze2_tl, pe1_tl, &
&   pe2_tl, pk1_tl, pk2_tl, pn2_tl, phis_tl
    REAL(fvprc) :: pe0(is:ie+1, km+1)
    REAL(fvprc) :: pe0_tl(is:ie+1, km+1)
    REAL(fvprc) :: pe3(is:ie+1, km+1)
    REAL(fvprc) :: pe3_tl(is:ie+1, km+1)
    REAL(fvprc) :: gz(is:ie)
    REAL(fvprc) :: gz_tl(is:ie)
    REAL(fvprc) :: dz1(km)
    REAL(fvprc) :: rcp, rg, tmp, tpe, cvm, rgama, rrg, bkh, dtmp, dlnp, &
&   ztop, z_rat
    REAL(fvprc) :: tmp_tl, tpe_tl, dtmp_tl, dlnp_tl
...
&                   delp(i, j, k)*delz_tl(i, j, k))*pt(i, j, k)/delz(i, &
&                   j, k)**2 + kapag*delp(i, j, k)*pt_tl(i, j, k)/delz(i&
&                   , j, k)
...
!#endif
          remap_t = .false.
          CALL PKEZ_TLM(km, is, ie, js, je, j, pe, pk, pk_tl, akap, peln&
&                 , peln_tl, pkz, pkz_tl, ptop)
! Compute cp*T + KE
          DO k=1,km
            DO i=is,ie
              te_tl(i, j, k) = 0.25*gridstruct%rsin2(i, j)*(2*u(i, j, k)&
...
&               v_tl(i, j, k)+v_tl(i+1, j, k)))) + pt_tl(i, j, k)*pkz(i&
&               , j, k) + pt(i, j, k)*pkz_tl(i, j, k)
...
! ="specific volume"/grav
                delz_tl(i, j, k) = -((delz_tl(i, j, k)*delp(i, j, k)-&
&                 delz(i, j, k)*delp_tl(i, j, k))/delp(i, j, k)**2)
...
&               j, k)*delz_tl(i, j, k))/delz(i, j, k)**2)
          ...
          IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
            CALL REMAP_Z_TLM(km, ze1, ze1_tl, km, ze2, ze2_tl, deng, &
&                         deng_tl, is, ie, 1, abs_kord_tm)
          ELSE
            deng_tj = deng
            call remap_z(km, ze1,       km, ze2, deng, is, ie, 1, abs_kord_tm)
            CALL REMAP_Z_TLM(km, ze1, ze1_tl, km, ze2, ze2_tl, deng, &
&                      deng_tl, is, ie, 1, abs_kord_tm_pert)
          END IF
!-------------
! Update delz
!-------------
...
!----------------
        IF (nq .GT. 5) THEN
          IF (kord_tr(1) .EQ. kord_tr_pert(1)) THEN
            CALL MAPN_TRACER_TLM(nq, km, km, pe1, pe1_tl, pe2, pe2_tl&
&                             , q, q_tl, dp2, dp2_tl, kord_tr, j, is, ie&
&                             , isd, ied, jsd, jed, 0.0_FVPRC, fill)
          ELSE
            q_tj(:, j, :, :) = q(:, j, :, :)
            call mapn_tracer(nq, km, km, pe1, pe2, q, dp2, kord_tr, j,     &
                             is, ie, isd, ied, jsd, jed, 0.0_FVPRC, fill)
            CALL MAPN_TRACER_TLM(nq, km, km, pe1, pe1_tl, pe2, pe2_tl, q&
...
&                        pe2, pe2_tl, q2, q2_tl, dp2, dp2_tl, is, ie, 0&
...
&                          , 1, kord_tm)
          ELSE
            ...
            gz_tl = 0.0_FVPRC
            CALL MAP1_PPM_TLM(km, pe1, pe1_tl, gz, gz_tl, km, pe2, &
&                       pe2_tl, te, te_tl, is, ie, j, is, ie, js, je, 1&
&                       , kord_tm_pert)
          END IF
...
&                         ie, 1:km+1, j), gz, km, pn2, pn2_tl, pt, pt_tl&
...
! Map pt using pk
!----------------
          CALL MAP1_CUBIC_TLM(km, pe1, pe1_tl, km, pe2, pe2_tl, pt, &
&                       pt_tl, is, ie, j, isd, ied, jsd, jed, akap, &
&                       t_var=3, conserv=.false.)
        ELSE IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
          gz_tl = 0.0_FVPRC
          CALL MAP1_PPM_TLM(km, pk1, pk1_tl, gz, gz_tl, km, pk2, &
&                        pk2_tl, pt, pt_tl, is, ie, j, isd, ied, jsd, &
&                        jed, 1, abs_kord_tm)
        ELSE
          pt_tj(:, j, :) = pt(:, j, :)
          ...
          gz_tl = 0.0_FVPRC
          CALL MAP1_PPM_TLM(km, pk1, pk1_tl, gz, gz_tl, km, pk2, pk2_tl&
&                     , pt, pt_tl, is, ie, j, isd, ied, jsd, jed, 1, &
&                     abs_kord_tm_pert)
        END IF
        IF (.NOT.hydrostatic) THEN
! Remap vertical wind:
...
&                       , j), km, pe2, pe2_tl, w, w_tl, is, ie, j, isd, &
...
!------
        ...
        IF (kord_mt .EQ. kord_mt_pert) THEN
          gz_tl = 0.0_FVPRC
          CALL MAP1_PPM_TLM(km, pe0(is:ie, 1:km+1), pe0_tl(is:ie, 1:&
&                        km+1), gz, gz_tl, km, pe3(is:ie, 1:km+1), &
&                        pe3_tl(is:ie, 1:km+1), u, u_tl, is, ie, j, isd&
&                        , ied, jsd, jedp1, -1, kord_mt)
        ELSE
          ...
          gz_tl = 0.0_FVPRC
          CALL MAP1_PPM_TLM(km, pe0(is:ie, 1:km+1), pe0_tl(is:ie, 1:km+1&
&                     ), gz, gz_tl, km, pe3(is:ie, 1:km+1), pe3_tl(is:ie&
&                     , 1:km+1), u, u_tl, is, ie, j, isd, ied, jsd, &
&                     jedp1, -1, kord_mt_pert)
        END IF
! (j < je+1)
        IF (j .LT. je + 1) THEN
!------
...
!------- Hybrid_z section ---------------
      IF (gridstruct%square_domain) THEN
        CALL MPP_UPDATE_DOMAINS_TLM(ua, ua_tl, domain, complete=.true., &
...
!$OMP                          private(pe1, pe2)
      DO j=js,je+1
        ...
        IF (kord_mt .EQ. kord_mt_pert) THEN
          gz_tl = 0.0_FVPRC
          CALL MAP1_PPM_TLM(km, pe1, pe1_tl, gz, gz_tl, km, pe2, &
&                        pe2_tl, u, u_tl, is, ie, j, isd, ied, jsd, &
&                        jedp1, -1, kord_mt)
        ELSE
          u_tj(:, j, :) = u(:, j, :)
          call map1_ppm( km, pe1,       gz, &
                         ...
                         is, ie, j, isd, ied, jsd, jedp1, -1, kord_mt)
          gz_tl = 0.0_FVPRC
          CALL MAP1_PPM_TLM(km, pe1, pe1_tl, gz, gz_tl, km, pe2, pe2_tl&
&                     , u, u_tl, is, ie, j, isd, ied, jsd, jedp1, -1, &
&                     kord_mt_pert)
        END IF
      END DO
! v-wind
...
&                     , v, v_tl, is, iep1, j, isd, iedp1, jsd, jed, -1, &
&                     kord_mt_pert)
        END IF
      END DO
    END IF
...
&                 , j, k)*pkz(i, j, k)+0.25*gridstruct%rsin2(i, j)*(u(i&
&                 , j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)&
&                 **2-(u(i, j, k)+u(i, j+1, k))*(v(i, j, k)+v(i+1, j, k)&
&                 )*gridstruct%cosa_s(i, j))) + delp(i, j, k)*(pt_tl(i, &
&                 j, k)*pkz(i, j, k)+pt(i, j, k)*pkz_tl(i, j, k)+0.25*&
&                 gridstruct%rsin2(i, j)*(2*u(i, j, k)*u_tl(i, j, k)+2*u&
&                 (i, j+1, k)*u_tl(i, j+1, k)+2*v(i, j, k)*v_tl(i, j, k)&
&                 +2*v(i+1, j, k)*v_tl(i+1, j, k)-gridstruct%cosa_s(i, j&
&                 )*((u_tl(i, j, k)+u_tl(i, j+1, k))*(v(i, j, k)+v(i+1, &
&                 j, k))+(u(i, j, k)+u(i, j+1, k))*(v_tl(i, j, k)+v_tl(i&
&                 +1, j, k)))))
...
!#endif
                te_2d_tl(i, j) = te_2d_tl(i, j) + delp_tl(i, j, k)*(&
&                 rgama*pt(i, j, k)*pkz(i, j, k)/(1.+r_vir*q(i, j, k, &
&                 sphum))+0.5*(phis(i, k)+phis(i, k+1)+w(i, j, k)**2+0.5&
&                 *gridstruct%rsin2(i, j)*(u(i, j, k)**2+u(i, j+1, k)**2&
&                 +v(i, j, k)**2+v(i+1, j, k)**2-(u(i, j, k)+u(i, j+1, k&
&                 ))*(v(i, j, k)+v(i+1, j, k))*gridstruct%cosa_s(i, j)))&
&                 ) + delp(i, j, k)*((rgama*(pt_tl(i, j, k)*pkz(i, j, k)&
&                 +pt(i, j, k)*pkz_tl(i, j, k))*(1.+r_vir*q(i, j, k, &
&                 sphum))-rgama*pt(i, j, k)*pkz(i, j, k)*r_vir*q_tl(i, j&
&                 , k, sphum))/(1.+r_vir*q(i, j, k, sphum))**2+0.5*(&
&                 phis_tl(i, k)+phis_tl(i, k+1)+2*w(i, j, k)*w_tl(i, j, &
&                 k)+0.5*gridstruct%rsin2(i, j)*(2*u(i, j, k)*u_tl(i, j&
&                 , k)+2*u(i, j+1, k)*u_tl(i, j+1, k)+2*v(i, j, k)*v_tl(&
&                 i, j, k)+2*v(i+1, j, k)*v_tl(i+1, j, k)-gridstruct%&
...
&           zsum1_tl(i, j)
        ...
        END DO
      END DO
      result1_tl = G_SUM_TLM(domain, te_2d, te_2d_tl, is, ie, js, je, ng&
&       , gridstruct%area_64, 0, g_sum=result1)
...
&         ng, gridstruct%area_64, 0, g_sum=result1)
        dtmp_tl = (tpe_tl*cp*result1-tpe*cp*result1_tl)/(cp*result1)**2
        dtmp = tpe/(cp*result1)
      ELSE
        result1_tl = G_SUM_TLM(domain, zsum1, zsum1_tl, is, ie, js, je, &
&         ng, gridstruct%area_64, 0, g_sum=result1)
        dtmp_tl = (tpe_tl*cv_air*result1-tpe*cv_air*result1_tl)/(cv_air*&
&         result1)**2
...
!$OMP                          private(gz, tpe, tmp, dlnp)
      DO j=js,je
        ...
        DO k=km,1,-1
          DO i=is,ie
            tpe_tl = te_tl(i, j, k) - gz_tl(i) - 0.25*gridstruct%rsin2(i&
&             , j)*(2*u(i, j, k)*u_tl(i, j, k)+2*u(i, j+1, k)*u_tl(i, j+&
&             1, k)+2*v(i, j, k)*v_tl(i, j, k)+2*v(i+1, j, k)*v_tl(i+1, &
&             j, k)-gridstruct%cosa_s(i, j)*((u_tl(i, j, k)+u_tl(i, j+1&
&             , k))*(v(i, j, k)+v(i+1, j, k))+(u(i, j, k)+u(i, j+1, k))*&
&             (v_tl(i, j, k)+v_tl(i+1, j, k))))
...
!#else
              pt_tl(i, j, k) = cp*((pt_tl(i, j, k)*pkz(i, j, k)-pt(i, j&
&               , k)*pkz_tl(i, j, k))/pkz(i, j, k)**2+dtmp_tl)
...
!  Differentiation of compute_total_energy in forward (tangent) mode (with options r8):
...
&   va
    ...
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(IN) :: pt_tl, &
&   delp_tl
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km, *), INTENT(IN) :: q
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km, *), INTENT(IN) :: q_tl
    REAL(fvprc), DIMENSION(is:ie, js:je, km), INTENT(IN) :: qc
    REAL(fvprc), DIMENSION(is:ie, js:je, km), INTENT(IN) :: qc_tl
    REAL(fvprc), INTENT(INOUT) :: u(isd:ied, jsd:jed+1, km)
    REAL(fvprc), INTENT(INOUT) :: u_tl(isd:ied, jsd:jed+1, km)
    REAL(fvprc), INTENT(INOUT) :: v(isd:ied+1, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: v_tl(isd:ied+1, jsd:jed, km)
! vertical velocity (m/s)
    ...
    REAL(fvprc), INTENT(IN) :: w_tl(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(IN) :: delz(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(IN) :: delz_tl(isd:ied, jsd:jed, km)
! surface geopotential
    REAL(fvprc), INTENT(IN) :: hs(isd:ied, jsd:jed)
! pressure at layer edges
    REAL(fvprc), INTENT(IN) :: pe(is-1:ie+1, km+1, js-1:je+1)
    REAL(fvprc), INTENT(IN) :: pe_tl(is-1:ie+1, km+1, js-1:je+1)
! log(pe)
    REAL(fvprc), INTENT(IN) :: peln(is:ie, km+1, js:je)
    REAL(fvprc), INTENT(IN) :: peln_tl(is:ie, km+1, js:je)
...
! vertically integrated TE
    ...
    REAL(fvprc), INTENT(OUT) :: te_2d_tl(is:ie, js:je)
! Moist TE
    REAL(fvprc), INTENT(OUT) :: teq(is:ie, js:je)
    REAL(fvprc), INTENT(OUT) :: teq_tl(is:ie, js:je)
! Local
    REAL(fvprc), DIMENSION(is:ie, km) :: tv
    REAL(fvprc), DIMENSION(is:ie, km) :: tv_tl
    REAL(fvprc) :: phiz(is:ie, km+1)
    REAL(fvprc) :: phiz_tl(is:ie, km+1)
...
!  Differentiation of pkez in forward (tangent) mode (with options r8):
...
&   +1)
    ...
    REAL(fvprc), INTENT(IN) :: pk_tl(ifirst:ilast, jfirst:jlast, km+1)
    REAL(fvprc), INTENT(IN) :: ptop
! !OUTPUT
    REAL(fvprc), INTENT(OUT) :: pkz(ifirst:ilast, jfirst:jlast, km)
    REAL(fvprc), INTENT(OUT) :: pkz_tl(ifirst:ilast, jfirst:jlast, km)
! log (pe)
    REAL(fvprc), INTENT(INOUT) :: peln(ifirst:ilast, km+1, jfirst:jlast)
    REAL(fvprc), INTENT(INOUT) :: peln_tl(ifirst:ilast, km+1, jfirst:&
&   jlast)
! Local
    REAL(fvprc) :: pk2(ifirst:ilast, km+1)
    REAL(fvprc) :: pk2_tl(ifirst:ilast, km+1)
    REAL(fvprc) :: pek
    REAL(fvprc) :: pek_tl
    REAL(fvprc) :: lnp
    REAL(fvprc) :: ak1
...
!  Differentiation of map1_cubic in forward (tangent) mode (with options r8):
...
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc), INTENT(IN) :: pe1_tl(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc), INTENT(IN) :: pe2_tl(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
!      real(FVPRC), intent(in) ::    q1(ibeg:iend,jbeg:jend,km) ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_tl(ibeg:iend, jbeg:jend, kn)
! !DESCRIPTION:
!
...
! !LOCAL VARIABLES:
    ...
    REAL(fvprc) :: qx_tl(i1:i2, km)
    REAL(fvprc) :: logpl1(i1:i2, km)
    REAL(fvprc) :: logpl1_tl(i1:i2, km)
    REAL(fvprc) :: logpl2(i1:i2, kn)
    REAL(fvprc) :: logpl2_tl(i1:i2, kn)
    REAL(fvprc) :: dlogp1(i1:i2, km)
    REAL(fvprc) :: dlogp1_tl(i1:i2, km)
    REAL(fvprc) :: vsum1(i1:i2)
    REAL(fvprc) :: vsum1_tl(i1:i2)
    REAL(fvprc) :: vsum2(i1:i2)
    REAL(fvprc) :: vsum2_tl(i1:i2)
    REAL(fvprc) :: am2, am1, ap0, ap1, p, plp1, plp0, plm1, plm2, dlp0, &
&   dlm1, dlm2
    REAL(fvprc) :: am2_tl, am1_tl, ap0_tl, ap1_tl, p_tl, plp1_tl, &
&   plp0_tl, plm1_tl, plm2_tl, dlp0_tl, dlm1_tl, dlm2_tl
    ...
    REAL(fvprc), DIMENSION(i1:i2) :: arg2
    REAL(fvprc), DIMENSION(i1:i2) :: arg2_tl
! Initialization
! --------------
    SELECT CASE  (t_var) 
    CASE (1) 
...
!  Differentiation of scalar_profile in forward (tangent) mode (with options r8):
!   variations   of useful results: a4
!   with respect to varying inputs: delp a4
  SUBROUTINE SCALAR_PROFILE_TLM(qs, a4, a4_tl, delp, delp_tl, km, i1, i2&
&   , iv, kord, qmin)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
...
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
    REAL(fvprc), INTENT(IN) :: delp_tl(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4_tl(4, i1:i2, km)
    REAL(fvprc), INTENT(IN) :: qmin
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: gam_tl(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: q_tl(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: d4_tl(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: bet_tl, a_bot_tl, grat_tl
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    REAL(fvprc) :: pmp_1_tl, lac_1_tl, pmp_2_tl, lac_2_tl
    INTEGER :: i, k, im
    INTRINSIC MAX
    INTRINSIC MIN
    INTRINSIC ABS
...
! Check within the smooth region if subgrid profile is non-monotonic
                  IF (abs6 .GT. abs10) THEN
                    ...
                    ELSE
                      ...
                      a4(3, i, k) = x8
                    END IF
                    a4_tl(4, i, k) = 6.*a4_tl(1, i, k) - 3.*(a4_tl(2, i&
&                     , k)+a4_tl(3, i, k))
...
!  Differentiation of cs_profile in forward (tangent) mode (with options r8):
!   variations   of useful results: a4
!   with respect to varying inputs: qs delp a4
  SUBROUTINE CS_PROFILE_TLM(qs, qs_tl, a4, a4_tl, delp, delp_tl, km, i1&
&   , i2, iv, kord)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
...
! iv = 1: others
    INTEGER, INTENT(IN) :: kord
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
    REAL(fvprc), INTENT(IN) :: qs_tl(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
    REAL(fvprc), INTENT(IN) :: delp_tl(i1:i2, km)
! Interpolated values
...
!  Differentiation of cs_limiters in forward (tangent) mode (with options r8):
!   variations   of useful results: a4
...
! PPM array
    ...
    REAL(fvprc), INTENT(INOUT) :: a4_tl(4, im)
! !LOCAL VARIABLES:
...
!  Differentiation of ppm_profile in forward (tangent) mode (with options r8):
!   variations   of useful results: a4
!   with respect to varying inputs: delp a4
...
! local arrays:
    ...
    REAL(fvprc) :: dc_tl(i1:i2, km)
    REAL(fvprc) :: h2(i1:i2, km)
    REAL(fvprc) :: h2_tl(i1:i2, km)
    REAL(fvprc) :: delq(i1:i2, km)
    REAL(fvprc) :: delq_tl(i1:i2, km)
    REAL(fvprc) :: df2(i1:i2, km)
    REAL(fvprc) :: df2_tl(i1:i2, km)
    REAL(fvprc) :: d4(i1:i2, km)
    REAL(fvprc) :: d4_tl(i1:i2, km)
! local scalars:
    INTEGER :: i, k, km1, lmt, it
    REAL(fvprc) :: fac
    REAL(fvprc) :: a1, a2, c1, c2, c3, d1, d2
    REAL(fvprc) :: a1_tl, a2_tl, c1_tl, c2_tl, c3_tl, d1_tl, d2_tl
    REAL(fvprc) :: qm, dq, lac, qmp, pmp
    REAL(fvprc) :: qm_tl, dq_tl, lac_tl, qmp_tl, pmp_tl
    INTRINSIC ABS
    INTRINSIC MAX
    INTRINSIC MIN
    INTRINSIC SIGN
...
!  Differentiation of ppm_limiters in forward (tangent) mode (with options r8):
!   variations   of useful results: a4
...
! PPM array
    ...
    REAL(fvprc), INTENT(INOUT) :: a4_tl(4, *)
! AA <-- a4(1,i)
! AL <-- a4(2,i)
! AR <-- a4(3,i)
! A6 <-- a4(4,i)
...
!  Differentiation of steepz in forward (tangent) mode (with options r8):
!   variations   of useful results: a4
...
! grid size
    ...
    REAL(fvprc), INTENT(IN) :: dp_tl(i1:i2, km)
! backward diff of q
    REAL(fvprc), INTENT(IN) :: dq(i1:i2, km)
    REAL(fvprc), INTENT(IN) :: dq_tl(i1:i2, km)
! backward sum:  dp(k)+ dp(k-1) 
    REAL(fvprc), INTENT(IN) :: d4(i1:i2, km)
    REAL(fvprc), INTENT(IN) :: d4_tl(i1:i2, km)
! first guess mismatch
    REAL(fvprc), INTENT(IN) :: df2(i1:i2, km)
    REAL(fvprc), INTENT(IN) :: df2_tl(i1:i2, km)
! monotonic mismatch
    REAL(fvprc), INTENT(IN) :: dm(i1:i2, km)
    REAL(fvprc), INTENT(IN) :: dm_tl(i1:i2, km)
! !INPUT/OUTPUT PARAMETERS:
...
! !LOCAL VARIABLES:
    ...
    REAL(fvprc) :: alfa_tl(i1:i2, km)
    REAL(fvprc) :: f(i1:i2, km)
    REAL(fvprc) :: f_tl(i1:i2, km)
    REAL(fvprc) :: rat(i1:i2, km)
    REAL(fvprc) :: rat_tl(i1:i2, km)
    REAL(fvprc) :: dg2
    REAL(fvprc) :: dg2_tl
    INTRINSIC ABS
    INTRINSIC MIN
...
!  Differentiation of remap_z in forward (tangent) mode (with options r8):
!   variations   of useful results: q2
!   with respect to varying inputs: pe1 pe2 q2
  SUBROUTINE REMAP_Z_TLM(km, pe1, pe1_tl, kn, pe2, pe2_tl, q2, q2_tl&
&   , i1, i2, iv, kord)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! Starting longitude
...
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(i1:i2, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_tl(i1:i2, kn)
! !LOCAL VARIABLES:
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: qs_tl(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_tl(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: q4_tl(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, delp, esl
    REAL(fvprc) :: pl_tl, pr_tl, qsum_tl, delp_tl, esl_tl
    INTEGER :: i, k, l, m, k0
    dp1_tl = 0.0_FVPRC
    q4_tl = 0.0_FVPRC
    DO k=1,km
...
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        qs_tl = 0.0_FVPRC
        CALL CS_PROFILE_LINEAR_TLM(qs, qs_tl, q4, q4_tl, dp1, dp1_tl&
&                               , km, i1, i2, iv)
        qsum_tl = 0.0_FVPRC
      ELSE
        qsum_tl = 0.0_FVPRC
...
!  Differentiation of map_scalar in forward (tangent) mode (with options r8):
!   variations   of useful results: q2
!   with respect to varying inputs: pe1 pe2 q2
...
! !LOCAL VARIABLES:
    ...
    REAL(fvprc) :: q4_tl(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    REAL(fvprc) :: pl_tl, pr_tl, qsum_tl, dp_tl, esl_tl
    INTEGER :: i, k, l, m, k0
    dp1_tl = 0.0_FVPRC
    q4_tl = 0.0_FVPRC
    DO k=1,km
...
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL SCALAR_PROFILE_LINEAR_TLM(qs, q4, q4_tl, dp1, dp1_tl, km&
&                                   , i1, i2, iv, q_min)
        qsum_tl = 0.0_FVPRC
      ELSE
        qsum_tl = 0.0_FVPRC
...
!  Differentiation of map1_ppm in forward (tangent) mode (with options r8):
!   variations   of useful results: q2
...
!  Differentiation of mapn_tracer in forward (tangent) mode (with options r8):
!   variations   of useful results: q1
!   with respect to varying inputs: pe1 pe2 dp2 q1
  SUBROUTINE MAPN_TRACER_TLM(nq, km, kn, pe1, pe1_tl, pe2, pe2_tl, q1&
&   , q1_tl, dp2, dp2_tl, kord, j, i1, i2, isd, ied, jsd, jed, q_min, &
&   fill)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! Original vertical dimension
...
! in the new vertical coordinate
    REAL(fvprc), INTENT(IN) :: dp2(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: dp2_tl(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: q_min
    LOGICAL, INTENT(IN) :: fill
! Field input
    REAL(fvprc), INTENT(INOUT) :: q1(isd:ied, jsd:jed, km, nq)
    REAL(fvprc), INTENT(INOUT) :: q1_tl(isd:ied, jsd:jed, km, nq)
! !LOCAL VARIABLES:
    REAL(fvprc) :: q4(4, i1:i2, km, nq)
    REAL(fvprc) :: q4_tl(4, i1:i2, km, nq)
! Field output
    REAL(fvprc) :: q2(i1:i2, kn, nq)
    REAL(fvprc) :: q2_tl(i1:i2, kn, nq)
    REAL(fvprc) :: qsum(nq)
    REAL(fvprc) :: qsum_tl(nq)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_tl(i1:i2, km)
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: pl, pr, dp, esl, fac1, fac2
    REAL(fvprc) :: pl_tl, pr_tl, dp_tl, esl_tl, fac1_tl, fac2_tl
    INTEGER :: i, k, l, m, k0, iq
    dp1_tl = 0.0_FVPRC
    DO k=1,km
      DO i=i1,i2
    ...
    DO iq=1,nq
      DO k=1,km
        ...
        END DO
      END DO
      IF (kord(iq) .EQ. 111) CALL SCALAR_PROFILE_LINEAR_TLM(qs, q4(1&
&                                                        , i1:i2, 1, iq)&
&                                                        , q4_tl(1, i1:&
&                                                        i2, 1, iq), dp1&
&                                                        , dp1_tl, km, &
...
!  Differentiation of map1_q2 in forward (tangent) mode (with options r8):
!   variations   of useful results: q2
...
! Field input
    REAL(fvprc), INTENT(IN) :: q1(ibeg:iend, jbeg:jend, km)
    REAL(fvprc), INTENT(IN) :: q1_tl(ibeg:iend, jbeg:jend, km)
    REAL(fvprc), INTENT(IN) :: dp2(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: dp2_tl(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: q_min
! !INPUT/OUTPUT PARAMETERS:
...
!  Differentiation of cs_profile_linear in forward (tangent) mode (with options r8):
!   variations   of useful results: a4
!   with respect to varying inputs: qs delp a4
  SUBROUTINE CS_PROFILE_LINEAR_TLM(qs, qs_tl, a4, a4_tl, delp, &
&   delp_tl, km, i1, i2, iv)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
...
!  Differentiation of scalar_profile_linear in forward (tangent) mode (with options r8):
!   variations   of useful results: a4
!   with respect to varying inputs: delp a4
  SUBROUTINE SCALAR_PROFILE_LINEAR_TLM(qs, a4, a4_tl, delp, delp_tl, &
&   km, i1, i2, iv, qmin)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
...
&         , k))
      ...
      END DO
    END DO
  END SUBROUTINE SCALAR_PROFILE_LINEAR_TLM
