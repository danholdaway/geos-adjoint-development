...
!----------------------------
  real(FVPRC), parameter:: a1 =  0.5625  !  9/16
  real(FVPRC), parameter:: a2 = -0.0625  ! -1/16
!----------------------
! PPM volume mean form:
!----------------------
  real(FVPRC), parameter:: b1 =  7./12.     ! 0.58333333
  real(FVPRC), parameter:: b2 = -1./12.

  private
  public :: a2b_ord2, a2b_ord4, a2b_ord4_fb
...
! 3-way extrapolation
    ...
    if ( gridstruct%se_corner ) then
        ...
        qout(npx,1) = (extrap_corner(p0, gridstruct%agrid(npx-1,1,1:2), gridstruct%agrid(npx-2, 2,1:2), &
                                     qin(npx-1,1), qin(npx-2, 2)) + &
                       extrap_corner(p0, gridstruct%agrid(npx-1,0,1:2), gridstruct%agrid(npx-2,-1,1:2), &
                                     qin(npx-1,0), qin(npx-2,-1)) + &
                       extrap_corner(p0, gridstruct%agrid(npx  ,1,1:2), gridstruct%agrid(npx+1, 2,1:2), &
                                     qin(npx  ,1), qin(npx+1, 2)))*r3
    endif
    if ( gridstruct%ne_corner ) then
              p0(1:2) = gridstruct%grid(npx,npy,1:2)
        qout(npx,npy) = (extrap_corner(p0, gridstruct%agrid(npx-1,npy-1,1:2), gridstruct%agrid(npx-2,npy-2,1:2), &
                                       qin(npx-1,npy-1), qin(npx-2,npy-2)) + &
                         extrap_corner(p0, gridstruct%agrid(npx  ,npy-1,1:2), gridstruct%agrid(npx+1,npy-2,1:2), &
                                       qin(npx  ,npy-1), qin(npx+1,npy-2)) + &
                         extrap_corner(p0, gridstruct%agrid(npx-1,npy  ,1:2), gridstruct%agrid(npx-2,npy+1,1:2), &
                                       qin(npx-1,npy  ), qin(npx-2,npy+1)))*r3
    endif
    if ( gridstruct%nw_corner ) then
            p0(1:2) = gridstruct%grid(1,npy,1:2)
        qout(1,npy) = (extrap_corner(p0, gridstruct%agrid(1,npy-1,1:2), gridstruct%agrid( 2,npy-2,1:2), &
                                       qin(1,npy-1), qin( 2,npy-2)) + &
                       extrap_corner(p0, gridstruct%agrid(0,npy-1,1:2), gridstruct%agrid(-1,npy-2,1:2), &
                                       qin(0,npy-1), qin(-1,npy-2)) + &
                       extrap_corner(p0, gridstruct%agrid(1,npy,  1:2), gridstruct%agrid( 2,npy+1,1:2), &
                                       qin(1,npy  ), qin( 2,npy+1)))*r3
    endif
...
!------------
    ...
    if ( is==1 ) then
       do j=js1, je1
          q2(j) = (qin(0,j)*gridstruct%dxa(1,j) + qin(1,j)*gridstruct%dxa(0,j))/(gridstruct%dxa(0,j) + gridstruct%dxa(1,j))
       enddo
       do j=js2, je1
          qout(1,j) = gridstruct%edge_w(j)*q2(j-1) + (1.-gridstruct%edge_w(j))*q2(j)
       enddo
!
       do j=max(1,js-2),min(npy-1,je+2)
    ...
    if ( (ie+1)==npx ) then
       do j=js1, je1
          q2(j) = (qin(npx-1,j)*gridstruct%dxa(npx,j) + qin(npx,j)*gridstruct%dxa(npx-1,j))/&
                               (gridstruct%dxa(npx-1,j) + gridstruct%dxa(npx,j))
       enddo
       do j=js2, je1
          qout(npx,j) = gridstruct%edge_e(j)*q2(j-1) + (1.-gridstruct%edge_e(j))*q2(j)
       enddo
!
       do j=max(1,js-2),min(npy-1,je+2)
...
!------------
    ...
    if ( js==1 ) then
       do i=is1, ie1
          q1(i) = (qin(i,0)*gridstruct%dya(i,1) + qin(i,1)*gridstruct%dya(i,0))/(gridstruct%dya(i,0) + gridstruct%dya(i,1))
       enddo
       do i=is2, ie1
          qout(i,1) = gridstruct%edge_s(i)*q1(i-1) + (1.-gridstruct%edge_s(i))*q1(i)
       enddo
!
       do i=max(1,is-2),min(npx-1,ie+2)
    ...
    if ( (je+1)==npy ) then
       do i=is1, ie1
          q1(i) = (qin(i,npy-1)*gridstruct%dya(i,npy) + qin(i,npy)*gridstruct%dya(i,npy-1))/&
                                            (gridstruct%dya(i,npy-1)+gridstruct%dya(i,npy))
       enddo
       do i=is2, ie1
          qout(i,npy) = gridstruct%edge_n(i)*q1(i-1) + (1.-gridstruct%edge_n(i))*q1(i)
       enddo
!
       do i=max(1,is-2),min(npx-1,ie+2)
...
! Y-sweep: PPM
  ...
  subroutine a2b_ord4_fb(qin, qout, gridstruct, npx, npy, is, ie, js, je, ng, replace)
  integer, intent(IN):: npx, npy, is, ie, js, je, ng
  real(FVPRC), intent(INOUT)::  qin(is-ng:ie+ng,js-ng:je+ng)   ! A-grid field
  real(FVPRC), intent(INOUT):: qout(is-ng:ie+ng,js-ng:je+ng)   ! Output  B-grid field
  type(fv_grid_type), intent(IN), target :: gridstruct
...
! 3-way extrapolation
    ...
    if ( gridstruct%sw_corner ) then
          p0(1:2) = gridstruct%grid(1,1,1:2)
        qout(1,1) = (extrap_corner_fb(p0, gridstruct%agrid(1,1,1:2), gridstruct%agrid( 2, 2,1:2), qin(1,1), qin( 2, 2)) + &
                     extrap_corner_fb(p0, gridstruct%agrid(0,1,1:2), gridstruct%agrid(-1, 2,1:2), qin(0,1), qin(-1, 2)) + &
                     extrap_corner_fb(p0, gridstruct%agrid(1,0,1:2), gridstruct%agrid( 2,-1,1:2), qin(1,0), qin( 2,-1)))*r3

    endif
    if ( gridstruct%se_corner ) then
            p0(1:2) = gridstruct%grid(npx,1,1:2)
        qout(npx,1) = (extrap_corner_fb(p0, gridstruct%agrid(npx-1,1,1:2), gridstruct%agrid(npx-2, 2,1:2), &
                                     qin(npx-1,1), qin(npx-2, 2)) + &
                       extrap_corner_fb(p0, gridstruct%agrid(npx-1,0,1:2), gridstruct%agrid(npx-2,-1,1:2), &
                                     qin(npx-1,0), qin(npx-2,-1)) + &
                       extrap_corner_fb(p0, gridstruct%agrid(npx  ,1,1:2), gridstruct%agrid(npx+1, 2,1:2), &
                                     qin(npx  ,1), qin(npx+1, 2)))*r3
    endif
    if ( gridstruct%ne_corner ) then
              p0(1:2) = gridstruct%grid(npx,npy,1:2)
        qout(npx,npy) = (extrap_corner_fb(p0, gridstruct%agrid(npx-1,npy-1,1:2), gridstruct%agrid(npx-2,npy-2,1:2), &
                                       qin(npx-1,npy-1), qin(npx-2,npy-2)) + &
                         extrap_corner_fb(p0, gridstruct%agrid(npx  ,npy-1,1:2), gridstruct%agrid(npx+1,npy-2,1:2), &
                                       qin(npx  ,npy-1), qin(npx+1,npy-2)) + &
                         extrap_corner_fb(p0, gridstruct%agrid(npx-1,npy  ,1:2), gridstruct%agrid(npx-2,npy+1,1:2), &
                                       qin(npx-1,npy  ), qin(npx-2,npy+1)))*r3
    endif
    if ( gridstruct%nw_corner ) then
            p0(1:2) = gridstruct%grid(1,npy,1:2)
        qout(1,npy) = (extrap_corner_fb(p0, gridstruct%agrid(1,npy-1,1:2), gridstruct%agrid( 2,npy-2,1:2), &
                                       qin(1,npy-1), qin( 2,npy-2)) + &
                       extrap_corner_fb(p0, gridstruct%agrid(0,npy-1,1:2), gridstruct%agrid(-1,npy-2,1:2), &
                                       qin(0,npy-1), qin(-1,npy-2)) + &
                       extrap_corner_fb(p0, gridstruct%agrid(1,npy,  1:2), gridstruct%agrid( 2,npy+1,1:2), &
                                       qin(1,npy  ), qin( 2,npy+1)))*r3
    endif
...
!    if ( gridstruct%se_corner ) then
!            p0(1:2) = gridstruct%grid(npx,1,1:2)
!        qout(npx,1) = (extrap_corner(p0, gridstruct%agrid(npx-1,1,1:2), gridstruct%agrid(npx-2, 2,1:2), qin(npx-1,1), qin(npx-2, 2)) + &
!                       extrap_corner(p0, gridstruct%agrid(npx-1,0,1:2), gridstruct%agrid(npx-2,-1,1:2), qin(npx-1,0), qin(npx-2,-1)) + &
!                       extrap_corner(p0, gridstruct%agrid(npx  ,1,1:2), gridstruct%agrid(npx+1, 2,1:2), qin(npx  ,1), qin(npx+1, 2)))*r3
!    endif
!    if ( gridstruct%ne_corner ) then
!              p0(1:2) = gridstruct%grid(npx,npy,1:2)
!        qout(npx,npy) = (extrap_corner(p0, gridstruct%agrid(npx-1,npy-1,1:2), gridstruct%agrid(npx-2,npy-2,1:2), qin(npx-1,npy-1), qin(npx-2,npy-2)) + &
!                         extrap_corner(p0, gridstruct%agrid(npx  ,npy-1,1:2), gridstruct%agrid(npx+1,npy-2,1:2), qin(npx  ,npy-1), qin(npx+1,npy-2)) + &
!                         extrap_corner(p0, gridstruct%agrid(npx-1,npy  ,1:2), gridstruct%agrid(npx-2,npy+1,1:2), qin(npx-1,npy  ), qin(npx-2,npy+1)))*r3
!    endif
!    if ( gridstruct%nw_corner ) then
!            p0(1:2) = gridstruct%grid(1,npy,1:2)
!        qout(1,npy) = (extrap_corner(p0, gridstruct%agrid(1,npy-1,1:2), gridstruct%agrid( 2,npy-2,1:2), qin(1,npy-1), qin( 2,npy-2)) + &
!                       extrap_corner(p0, gridstruct%agrid(0,npy-1,1:2), gridstruct%agrid(-1,npy-2,1:2), qin(0,npy-1), qin(-1,npy-2)) + &
!                       extrap_corner(p0, gridstruct%agrid(1,npy,  1:2), gridstruct%agrid( 2,npy+1,1:2), qin(1,npy  ), qin( 2,npy+1)))*r3
!    endif
!#endif
!#endif
!
...
!#endif
  ...
  subroutine a2b_ord2(qin, qout, gridstruct, npx, npy, is, ie, js, je, ng, replace)
    integer, intent(IN   ) :: npx, npy, is, ie, js, je, ng
    real(FVPRC)   , intent(INOUT) ::  qin(is-ng:ie+ng,js-ng:je+ng)   ! A-grid field
    real(FVPRC)   , intent(  OUT) :: qout(is-ng:ie+ng,js-ng:je+ng)   ! Output  B-grid field
    type(fv_grid_type), intent(IN), target :: gridstruct
    logical, optional, intent(IN) ::  replace
    ! local:
    real(FVPRC) q1(npx), q2(npy)
...
!$AD II-LOOP
  ...
  real(FVPRC) function extrap_corner ( p0, p1, p2, q1, q2 )
    real(FVPRC), intent(in ), dimension(2):: p0, p1, p2
  ...
  real(FVPRC) function extrap_corner_fb ( p0, p1, p2, q1, q2 )
    real(FVPRC), intent(in ), dimension(2):: p0, p1, p2
    real(FVPRC), intent(in ):: q1, q2
    real(FVPRC):: x1, x2
    ...
    extrap_corner_fb = q1 + x1/(x2-x1) * (q1-q2)

  end function extrap_corner_fb
