module dyn_core_mod
  ...
  use fv_arrays_mod,  only: REAL4, REAL8, FVPRC, fv_timing_onoff
  ...
  use a2b_edge_mod,       only: a2b_ord2, a2b_ord4_fb
...
!-----------------------------------------------------------------------
 ...
 subroutine dyn_core(npx, npy, npz, ng, sphum, nq, bdt, n_split, zvir, cp, akap, cappa, grav, hydrostatic,  &
    ...
    real(FVPRC), intent(inout) :: w(   bd%isd:bd%ied,bd%jsd:bd%jed,npz)  ! vertical vel. (m/s)
    real(FVPRC), intent(inout) ::  delz(bd%isd:bd%ied,bd%jsd:bd%jed,npz)  ! delta-height (m)
    real(FVPRC), intent(inout) :: cappa(bd%isd:bd%isd,bd%jsd:bd%jsd,1) ! moist kappa
    ...
    real(FVPRC), intent(inout) :: q(   bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz, nq)  ! 
    real(FVPRC), intent(in), optional:: time_total  ! total time (seconds) since start
...
! Others:
    ...
    real(FVPRC),    parameter:: air_viscosity = 1.E-5   ! [m**2/sec] for T ~ 260 K
!-----------------------------------------------------------------------
    real(FVPRC), intent(out  ):: ws(bd%is:bd%ie,bd%js:bd%je)        ! w at surface
    real(FVPRC), intent(inout):: omga(bd%isd:bd%ied,bd%jsd:bd%jed,npz)    ! Vertical pressure velocity (pa/s)
    real(FVPRC), intent(inout):: uc(bd%isd:bd%ied+1,bd%jsd:bd%jed  ,npz)  ! (uc, vc) are mostly used as the C grid winds
...
! Accumulated Courant number arrays
    ...
    real(FVPRC), intent(inout),dimension(bd%is:bd%ie,bd%js:bd%je,npz):: pkz
    ...
    real(FVPRC) :: pem(bd%is-1:bd%ie+1,npz+1,bd%js-1:bd%je+1), heat_source(bd%isd:bd%ied, bd%jsd:bd%jed, npz)
! Auto 1D & 2D arrays:
    real(FVPRC), dimension(bd%isd:bd%ied,bd%jsd:bd%jed):: ws3, z_rat
    real(FVPRC):: dp_ref(npz)
    real(FVPRC):: zs(bd%isd:bd%ied,bd%jsd:bd%jed)        ! surface height (m)
...
!---------------------------------------
    ...
    real(FVPRC), intent(inout) ::    pkc(bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz+1)
    ...
    real(FVPRC), intent(inout) ::  divgd(bd%isd:bd%ied+1,bd%jsd:bd%jed+1,npz  )
    real(FVPRC), intent(inout) ::  delpc(bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz  )
    ...
    real(FVPRC), intent(inout) ::    pk3(bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz+1)
    real(FVPRC), intent(inout) ::     du(bd%isd:bd%ied  ,bd%jsd:bd%jed+1,npz  )
    real(FVPRC), intent(inout) ::     dv(bd%isd:bd%ied+1,bd%jsd:bd%jed  ,npz  )

    real(FVPRC) :: gz_cptmp(bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz+1)
    real(FVPRC) :: pem_cptmp(bd%is-1:bd%ie+1,npz+1,bd%js-1:bd%je+1)
    real(FVPRC) :: pk_cptmp(bd%is:bd%ie,bd%js:bd%je, npz+1)
    real(FVPRC) ::     zh_cptmp(bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz+1)
    real(FVPRC) :: u_cptmp(bd%isd:bd%ied  ,bd%jsd:bd%jed+1,npz)
    real(FVPRC) :: v_cptmp(bd%isd:bd%ied+1,bd%jsd:bd%jed  ,npz) 
    real(FVPRC) :: pe_cptmp(bd%is-1:bd%ie+1, npz+1,bd%js-1:bd%je+1)
    real(FVPRC) :: ws_cptmp(bd%is:bd%ie,bd%js:bd%je)
    real(FVPRC) :: omga_cptmp(bd%isd:bd%ied,bd%jsd:bd%jed,npz) 
    real(FVPRC) :: om2d_cptmp(bd%is:bd%ie,npz)
    real(FVPRC) :: q_cptmp(   bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz, nq) 
...
! Empty the "flux capacitors"
    ...
    if ( flagstruct%d_con > 1.0E-5 ) then
         heat_source = 0.0 !allocate( heat_source(isd:ied, jsd:jed, npz) )
...
!     endif
     ...
     if ( nq > 0 ) then
         ...
         if ( flagstruct%inline_q ) then
                      call start_group_halo_update(i_pack(10), i_pack(10+12), q, domain)
         else
           if ( it==n_split ) then
                      call start_group_halo_update(i_pack(10), i_pack(10+12), q, domain)
           else
                      call start_group_halo_update(i_pack(10), i_pack(10+12), q_cptmp, domain)
                      call start_group_halo_update(i_pack(10), i_pack(10+12), q, domain)
           endif
         endif
...
!$OMP parallel do default(none) shared(is,ie,js,je,npz,gz,zs,delz)
         ...
         enddo
                             !if (fv_timing_onoff) call timing_on('COMM_TOTAL')
         call start_group_halo_update(i_pack(5), i_pack(5+12), gz,  domain)
                             !if (fv_timing_onoff) call timing_off('COMM_TOTAL')
...
!$OMP parallel do default(none) shared(is,ie,js,je,npz,gz,zs,delz)
         do j=js,je
            do i=is,ie
               gz_cptmp(i,j,npz+1) = zs(i,j)
               gz(i,j,npz+1) = zs(i,j)
            enddo
            do k=npz,1,-1
               do i=is,ie
                  gz_cptmp(i,j,k) = gz_cptmp(i,j,k+1) - delz(i,j,k)
                  gz(i,j,k) = gz(i,j,k+1) - delz(i,j,k)
               enddo
            enddo
         enddo
                             !if (fv_timing_onoff) call timing_on('COMM_TOTAL')
         call start_group_halo_update(i_pack(5), i_pack(5+12), gz_cptmp,  domain)
         call start_group_halo_update(i_pack(5), i_pack(5+12), gz,  domain)
                             !if (fv_timing_onoff) call timing_off('COMM_TOTAL')
...
!#endif
     ...
     if ( it==n_split .and. end_step ) then
       if ( flagstruct%use_old_omega ) then
          pem = 0.0 !allocate ( pem(is-1:ie+1,npz+1,js-1:je+1) )
!$OMP parallel do default(none) shared(is,ie,js,je,npz,pem,delp,ptop)
       ...
       if ( flagstruct%use_old_omega ) then
          pem_cptmp = 0.0 !allocate ( pem(is-1:ie+1,npz+1,js-1:je+1) )
!$OMP parallel do default(none) shared(is,ie,js,je,npz,pem,delp,ptop)
         do j=js-1,je+1
            do i=is-1,ie+1
               pem_cptmp(i,1,j) = ptop
               pem(i,1,j) = ptop
            enddo
            do k=1,npz
               do i=is-1,ie+1
                  pem_cptmp(i,k+1,j) = pem_cptmp(i,k,j) + delp(i,j,k)
                  pem(i,k+1,j) = pem(i,k,j) + delp(i,j,k)
               enddo
            enddo
         enddo
     ...
     if( .not. hydrostatic )  &
          call complete_group_halo_update(i_pack(7), i_pack(7+12), domain)
                                                     !if (fv_timing_onoff) call timing_off('COMM_TOTAL')
...
!$OMP                                  gridstruct)
      do k=1,npz
         call c_sw(delpc(isd:ied,jsd:jed,k), delp(isd:ied  ,jsd:jed  ,k),   ptc(isd:ied  ,jsd:jed  ,k),   &
                      pt(isd:ied,jsd:jed,k),    u(isd:ied  ,jsd:jed+1,k),     v(isd:ied+1,jsd:jed  ,k),   &
                       w(isd:ied,jsd:jed,k),   uc(isd:ied+1,jsd:jed  ,k),    vc(isd:ied  ,jsd:jed+1,k),   &
                      ua(isd:ied,jsd:jed,k),   va(isd:ied  ,jsd:jed  ,k),  omga(isd:ied  ,jsd:jed  ,k),   &
                      ut(isd:ied,jsd:jed,k),   vt(isd:ied  ,jsd:jed  ,k), divgd(isd:ied+1,jsd:jed+1,k),   &
      ...
      if ( flagstruct%nord > 0 ) then
                                                   !if (fv_timing_onoff) call timing_on('COMM_TOTAL')
          call start_group_halo_update(i_pack(3), i_pack(3+12), divgd, domain, position=CORNER)
                                                  !if (fv_timing_onoff) call timing_off('COMM_TOTAL')
      endif
      ...
      if ( hydrostatic ) then
           ...
           if ( it == 1 ) then
                                      ...
                                      !if (fv_timing_onoff) call timing_on('COMM_TOTAL')
              call complete_group_halo_update(i_pack(5), i_pack(5+12), domain)
                                     !if (fv_timing_onoff) call timing_off('COMM_TOTAL')
...
!#endif
      ...
      call p_grad_c(dt2, npz, delpc, pkc, gz, uc, vc, bd, gridstruct%rdxc, gridstruct%rdyc, hydrostatic)
                                                                   ...
                                                                   !if (fv_timing_onoff) call timing_on('COMM_TOTAL')
      call start_group_halo_update(i_pack(9), i_pack(9+12), uc, vc, domain, gridtype=CGRID_NE)
                                                     !if (fv_timing_onoff) call timing_off('COMM_TOTAL')
!#ifdef SW_DYNAMICS
...
!#endif
                                                                   ...
                                                                   !if (fv_timing_onoff) call timing_on('COMM_TOTAL')
    if (flagstruct%inline_q .and. nq>0) call complete_group_halo_update(i_pack(10), i_pack(10+12), domain)
    if (flagstruct%nord > 0) call complete_group_halo_update(i_pack(3), i_pack(3+12), domain)
...
!$OMP                                  hord_m, hord_v, hord_t, hord_p, wk, heat_s, d_con_k, z_rat)
    do k=1,npz
       ...
       nord_v_pert(k) = min(2, flagstruct%nord_pert)
       ...
       d2_divg_pert = min(0.20, flagstruct%d2_bg_pert*(1.-3.*tanh(0.1*log(pfull(k)/pfull(npz)))))
       ...
       if ( flagstruct%do_vort_damp_pert ) then
            damp_vt_pert(k) = flagstruct%vtdm4_pert     ! for delp, delz, and vorticity
       else
            damp_vt_pert(k) = 0.
       endif
       nord_w = nord_v(k)
...
! (no damping of potential temperature in sponge layers)
              if ( k==1 ) then
                   ...
                   d2_divg_pert = max(flagstruct%d2_bg_pert, flagstruct%d2_bg_k1)
                   nord_v(k) = 0   ! for delp, delz, and vorticity, sponger layers
                   nord_v_pert(k) = 0   ! for delp, delz, and vorticity, sponger layers
!#ifndef HIWPP
                   damp_vt(k) = d2_divg
...
!#endif
              ...
              elseif ( k==2 ) then
                   ...
                   d2_divg_pert = max(flagstruct%d2_bg_pert, flagstruct%d2_bg_k2)
                   nord_v(k) = 0
                   nord_v_pert(k) = 0
!#ifndef HIWPP
                   damp_vt(k) = d2_divg
...
!#endif
              ...
              elseif ( k==3 .and. flagstruct%d2_bg_k2>0.05 ) then
                   ...
                   d2_divg_pert = max(flagstruct%d2_bg_pert, 0.2*flagstruct%d2_bg_k2)
                   nord_v(k) = 0
                   nord_v_pert(k) = 0
!#ifndef HIWPP
                   damp_vt(k) = d2_divg
...
!#endif
            ...
            if ( damp_vt_pert(k) < 0.01 .and. nord_k_pert>0 ) d_con_k_pert = 0.
       else
...
! Apply first order scheme for damping the sponge layer
               ...
               d2_divg_pert = min(0.20, flagstruct%d2_bg_k1*flagstruct%d2_bg_pert)   ! 0.25 is the stability limit
               d2_divg = max(flagstruct%d2_divg_max_k1, d2_divg)
               d2_divg_pert = max(flagstruct%d2_divg_max_k1, d2_divg_pert)
           elseif( k == flagstruct%n_sponge+1 .and. npz>24 ) then
               ...
               nord_k_pert = max(0, flagstruct%nord_pert-1)
               d2_divg = min(0.20, flagstruct%d2_bg_k2*flagstruct%d2_bg)
               d2_divg_pert = min(0.20, flagstruct%d2_bg_k2*flagstruct%d2_bg_pert)
               d2_divg = max(flagstruct%d2_divg_max_k2, d2_divg)
               d2_divg_pert = max(flagstruct%d2_divg_max_k2, d2_divg_pert)
               if ( flagstruct%nord > 1 ) then
                    damp_k = 0.
               else
                    damp_k = flagstruct%damp_k_k2
               endif
               if ( flagstruct%nord_pert > 1 ) then
                    damp_k_pert = 0.
               else
       ...
       damp_k_pert = max(damp_k_pert, flagstruct%dddmp_pert)
...
! Average horizontal "convergence" to cell center
            do j=js,je
               ...
               enddo
            enddo
          elseif (.not.last_step .and. end_step) then
            do j=js,je
               do i=is,ie
                  omga(i,j,k) = delp(i,j,k)
                  omga_cptmp(i,j,k) = delp(i,j,k)
               enddo
            enddo
          endif
       endif
...
! Correction factor for 3D Coriolis force
       ...
       call d_sw(   vt(isd:ied  ,jsd:jed  ,k), &
                 vt_tj(isd:ied  ,jsd:jed  ,k), delp(isd:ied  ,jsd:jed  ,k),ptc(isd:ied  ,jsd:jed,k),   pt(isd:ied  ,jsd:jed  ,k),&
                     u(isd:ied  ,jsd:jed+1,k),    v(isd:ied+1,jsd:jed  ,k),  w(isd:ied  ,jsd:jed,k),   uc(isd:ied+1,jsd:jed  ,k),&
                    vc(isd:ied  ,jsd:jed+1,k),   ua(isd:ied  ,jsd:jed  ,k), va(isd:ied  ,jsd:jed,k),divgd(isd:ied+1,jsd:jed+1,k),&
                   mfx(is :ie +1,js :je   ,k),  mfy(is :ie   ,js :je +1,k), cx(is :ie +1,jsd:jed,k),   cy(isd:ied  ,js :je +1,k),&
                   crx(is :ie +1,jsd:jed  ,k),  cry(isd:ied  ,js :je +1,k),xfx(is :ie +1,jsd:jed,k),  yfx(isd:ied  ,js :je +1,k),&
                 ...
                 flagstruct%hord_tr_pert, hord_m_pert, hord_v_pert, hord_t_pert, hord_p_pert,    &
                 nord_k, nord_v(k), nord_w, nord_t, damp_k, d2_divg, flagstruct%d4_bg,  &
                 damp_vt(k), damp_w, damp_t, d_con_k, flagstruct%split_damp, &
                 nord_k_pert, nord_v_pert(k), nord_w_pert, nord_t_pert, damp_k_pert, d2_divg_pert, flagstruct%d4_bg_pert,  &
                 damp_vt_pert(k), damp_w_pert, damp_t_pert, d_con_k_pert, hydrostatic, gridstruct, flagstruct, bd)
...
! Average horizontal "convergence" to cell center
            do j=js,je
               do i=is,ie
                  omga(i,j,k) = omga(i,j,k)*(xfx(i,j,k)-xfx(i+1,j,k)+yfx(i,j,k)-yfx(i,j+1,k))*gridstruct%rarea(i,j)*rdt
                  omga_cptmp(i,j,k) = omga(i,j,k)*(xfx(i,j,k)-xfx(i+1,j,k)+yfx(i,j,k)-yfx(i,j+1,k))*gridstruct%rarea(i,j)*rdt
               enddo
            enddo
          endif
       endif
...
!          end if
        ...
        call update_dz_d(nord_v, damp_vt, flagstruct%hord_tm, flagstruct%hord_tm_pert, is, ie, js, je, npz, ng, &
                         npx, npy, gridstruct%area,  &
...
!        end if
        ...
        if ( gridstruct%square_domain ) then
          call start_group_halo_update(i_pack(6), i_pack(6+12), pkc,  domain, whalo=2, ehalo=2, shalo=2, nhalo=2)
        else
          call start_group_halo_update(i_pack(6), i_pack(6+12), pkc,  domain)
        endif
        if ( remap_step ) then
        call pe_halo(is, ie, js, je, isd, ied, jsd, jed, npz, ptop, pe, delp)
        else
        !TODO call pe_halo(is, ie, js, je, isd, ied, jsd, jed, npz, ptop, pe_cptmp, delp)
        endif
...
!----------------------------
    ...
    if ( hydrostatic ) then
       if ( beta > 0. ) then
          ...
          call one_grad_p(u, v, pkc, gz, divg2, delp, dt, ng, gridstruct, bd, npx, npy, npz, ptop, &
                                    hydrostatic, flagstruct%a2b_ord, flagstruct%d_ext)
       endif
...
!#endif
       ...
       if ( beta > 0. ) then
          call split_p_grad( u, v, du, dv, pkc, gz, delp, pk3, beta_d, dt, ng, gridstruct, bd, npx, npy, npz, flagstruct%use_logp)
...
!-------------------------------------------------------------------------------------------------------
                                                     ...
                                                     !if (fv_timing_onoff) call timing_on('COMM_TOTAL')
    if( gridstruct%grid_type<4 .and. .not. gridstruct%nested) then
      if( it==n_split )then
...
! Prevent accumulation of rounding errors at overlapped domain edges:
          call mpp_get_boundary(u, v, domain, wbuffery=wbuffer, ebuffery=ebuffer,  &
                            sbufferx=sbuffer, nbufferx=nbuffer, gridtype=DGRID_NE )
          call mpp_get_boundary(u_cptmp, v_cptmp, domain, wbuffery=wbuffer, ebuffery=ebuffer,  &
                            sbufferx=sbuffer, nbufferx=nbuffer, gridtype=DGRID_NE )
!$OMP parallel do default(none) shared(is,ie,js,je,npz,u,nbuffer,v,ebuffer)
          do k=1,npz
             do i=is,ie
                u_cptmp(i,je+1,k) = nbuffer(i-is+1,k)
                u(i,je+1,k) = nbuffer(i-is+1,k)
             enddo
             do j=js,je
                v(ie+1,j,k) = ebuffer(j-js+1,k)
                v_cptmp(ie+1,j,k) = ebuffer(j-js+1,k)
             enddo
          enddo
...
!#ifndef ROT3
    if ( it/=n_split) then
         ...
         call start_group_halo_update(i_pack(8), i_pack(8+12), u, v, domain, gridtype=DGRID_NE)
         call start_group_halo_update(i_pack(8), i_pack(8+12), u_cptmp, v_cptmp, domain, gridtype=DGRID_NE)
    endif
!#endif
                                                     !if (fv_timing_onoff) call timing_off('COMM_TOTAL')
...
!          used=send_data(idiag%id_ws, ws, fv_time)
!      endif
    elseif (.not.last_step .and. end_step .and. hydrostatic) then !not last step
      if ( flagstruct%use_old_omega ) then
!$OMP parallel do default(none) shared(is,ie,js,je,npz,omga,pe,pem,rdt)
!$AD II-LOOP
          do k=1,npz
            do j=js,je
               do i=is,ie
                  omga(i,j,k) = (pe(i,k+1,j) - pem(i,k+1,j)) * rdt
                  omga_cptmp(i,j,k) = (pe(i,k+1,j) - pem(i,k+1,j)) * rdt
               enddo
            enddo
         enddo
!------------------------------
! Compute the "advective term"
!------------------------------
         call adv_pe(ua, va, pem, omga_cptmp, gridstruct, bd, npx, npy,  npz, ng)
         call adv_pe(ua, va, pem, omga, gridstruct, bd, npx, npy,  npz, ng)
      else
!$OMP parallel do default(none) shared(is,ie,js,je,npz,omga) private(om2d)
          do j=js,je
            do k=1,npz
               do i=is,ie
                  om2d(i,k) = omga(i,j,k)
                  om2d_cptmp(i,k) = omga(i,j,k)
               enddo
            enddo
            do k=2,npz
               do i=is,ie
                  om2d(i,k) = om2d(i,k-1) + omga(i,j,k)
                  om2d_cptmp(i,k) = om2d(i,k-1) + omga(i,j,k)
               enddo
            enddo
            do k=2,npz
               do i=is,ie
                  omga(i,j,k) = om2d(i,k)
                  omga_cptmp(i,j,k) = om2d(i,k)
               enddo
            enddo
         enddo
      endif
...
!endif
  ...
  subroutine dummy_update

     u(is,js,1) = u_cptmp(is,js,1)*v_cptmp(is,js,1)*gz_cptmp(is,js,1)*&
                  pem_cptmp(is,js,1)*pk_cptmp(is,js,1)*zh_cptmp(is,js,1)*pe_cptmp(is,js,1)*&
                  omga_cptmp(is,js,1) * om2d_cptmp(is,js) * ws_cptmp(is,js) * q_cptmp(is,js,1,1)

  endsubroutine dummy_update
...
real(FVPRC), intent(inout), dimension(isd:ied,jsd:jed,npz+1):: pk3
! Local:
...
real(FVPRC), intent(in):: ptop
real(FVPRC), intent(in   ), dimension(isd:ied,jsd:jed,npz):: delp
real(FVPRC), intent(inout), dimension(is-1:ie+1,npz+1,js-1:je+1):: pe
! Local:
integer:: i,j,k
...
real(FVPRC), intent(in), dimension(bd%isd:bd%ied,bd%jsd:bd%jed,npz):: ua, va
! Pressure at edges:
real(FVPRC), intent(in) :: pem(bd%is-1:bd%ie+1,1:npz+1,bd%js-1:bd%je+1)
...
type(fv_grid_bounds_type), intent(IN) :: bd
real(FVPRC), intent(in), dimension(bd%isd:, bd%jsd: ,:  ):: delpc
! pkc is pe**cappa     if hydrostatic
! pkc is full pressure if non-hydrostatic
real(FVPRC), intent(in), dimension(bd%isd:bd%ied, bd%jsd:bd%jed ,npz+1):: pkc, gz
...
!$AD II-LOOP
   ...
   do j=js,je
      do i=is,ie+1
         uc(i,j,k) = uc(i,j,k) + dt2*rdxc(i,j) / (wk(i-1,j,k)+wk(i,j,k)) *   &
...
enddo
   ...
   do j=js,je+1
      do i=is,ie
         vc(i,j,k) = vc(i,j,k) + dt2*rdyc(i,j) / (wk(i,j-1,k)+wk(i,j,k)) *   &
...
real(FVPRC), intent(inout) ::    pp(bd%isd:bd%ied, bd%jsd:bd%jed, npz+1)  ! perturbation pressure
real(FVPRC), intent(inout) ::    pk(bd%isd:bd%ied, bd%jsd:bd%jed, npz+1)  ! p**kappa
real(FVPRC), intent(inout) ::    gz(bd%isd:bd%ied, bd%jsd:bd%jed, npz+1)  ! g * h
...
do k=1,npz+1
   if ( k/=1 ) then
      call a2b_ord4_fb(pp(isd:ied,jsd:jed,k), wk1, gridstruct, npx, npy, is, ie, js, je, ng, .true.)
      call a2b_ord4_fb(pk(isd:ied,jsd:jed,k), wk1, gridstruct, npx, npy, is, ie, js, je, ng, .true.)
   endif
   call a2b_ord4_fb( gz(isd:ied,jsd:jed,k), wk1, gridstruct, npx, npy, is, ie, js, je, ng, .true.)
enddo
...
do k=1,npz
   call a2b_ord4_fb(delp(isd:ied,jsd:jed,k), wk1, gridstruct, npx, npy, is, ie, js, je, ng)
   do j=js,je+1
      do i=is,ie+1
...
subroutine split_p_grad( u, v, du, dv, pp, gz, delp, pk, beta, dt, ng, gridstruct, bd, npx, npy, npz, use_logp)
integer, intent(IN) :: ng, npx, npy, npz
...
!do k=2,npz+1
   if ( a2b_ord==4 ) then
do k=2,npz+1
      call a2b_ord4_fb(pk(isd:ied,jsd:jed,k), wk(:,:,k), gridstruct, npx, npy, is, ie, js, je, ng, .true.)
enddo
   else
do k=2,npz+1
      call a2b_ord2(pk(isd:ied,jsd:jed,k), wk(:,:,k), gridstruct, npx, npy, is, ie, js, je, ng, .true.)
enddo
   endif
!enddo
...
!do k=1,npz+1
   if ( a2b_ord==4 ) then
do k=1,npz+1
      call a2b_ord4_fb( gz(isd:ied,jsd:jed,k), wk(:,:,k), gridstruct, npx, npy, is, ie, js, je, ng, .true.)
enddo
   else
do k=1,npz+1
      call a2b_ord2( gz(isd:ied,jsd:jed,k), wk(:,:,k), gridstruct, npx, npy, is, ie, js, je, ng, .true.)
enddo
   endif
!enddo
...
!do k=1,npz
      if ( a2b_ord==4 ) then
do k=1,npz
         call a2b_ord4_fb(delp(isd:ied,jsd:jed,k), wk(:,:,k), gridstruct, npx, npy, is, ie, js, je, ng)
      enddo
      else
do k=1,npz
         call a2b_ord2(delp(isd:ied,jsd:jed,k), wk(:,:,k), gridstruct, npx, npy, is, ie, js, je, ng)
      enddo
      endif
!      enddo
   endif
...
do k=1,npz
   do j=js,je+1
      do i=is,ie
         u(i,j,k) = gridstruct%rdx(i,j)*(wk2(i,j)+u(i,j,k) + dt/(wk(i,j,k)+wk(i+1,j,k)) * &
                                 ((gz(i,j,k+1)-gz(i+1,j,k))*(pk(i+1,j,k+1)-pk(i,j,k)) &
                                + (gz(i,j,k)-gz(i+1,j,k+1))*(pk(i,j,k+1)-pk(i+1,j,k))))
      enddo
   enddo
...
do k=1,npz
   do j=js,je
      do i=is,ie+1
         v(i,j,k) = gridstruct%rdy(i,j)*(wk1(i,j)+v(i,j,k) + dt/(wk(i,j,k)+wk(i,j+1,k)) * &
                                 ((gz(i,j,k+1)-gz(i,j+1,k))*(pk(i,j+1,k+1)-pk(i,j,k)) &
                                + (gz(i,j,k)-gz(i,j+1,k+1))*(pk(i,j,k+1)-pk(i,j+1,k))))
      enddo
   enddo
...
subroutine grad1_p_update(divg2, u, v, du, dv, pk, gz, dt, ng, gridstruct, bd, npx, npy, npz, ptop, beta, a2b_ord)
...
type(fv_grid_bounds_type), intent(IN) :: bd
real(FVPRC), intent(INOUT), dimension(bd%isd:bd%ied,bd%jsd:bd%jed,km):: pt, delp
real(FVPRC), intent(INOUT), dimension(bd%isd:,bd%jsd:,1:):: w
...
1000 continue
 ...
 subroutine geopk(ptop, pe, peln, delp, pk, gz, hs, pt, q_con, pkz, km, akap, CG, nested, &
                    computehalo, npx, npy, a2b_ord, bd)

   integer, intent(IN) :: km, npx, npy, a2b_ord
   real(FVPRC)   , intent(IN) :: akap, ptop
   type(fv_grid_bounds_type), intent(IN) :: bd
   real(FVPRC)   , intent(IN) :: hs(bd%isd:bd%ied,bd%jsd:bd%jed)
   real(FVPRC), intent(IN), dimension(bd%isd:bd%ied,bd%jsd:bd%jed,km):: pt, delp
   real(FVPRC), intent(IN), dimension(bd%isd:bd%isd,bd%jsd:bd%jsd,1):: q_con
   logical, intent(IN) :: CG, nested, computehalo
   ! !OUTPUT PARAMETERS
   real(FVPRC), intent(OUT), dimension(bd%isd:bd%ied,bd%jsd:bd%jed,km+1):: gz, pk
   real(FVPRC), intent(OUT) :: pe(bd%is-1:bd%ie+1,km+1,bd%js-1:bd%je+1)
   real(FVPRC), intent(out) :: peln(bd%is:bd%ie,km+1,bd%js:bd%je)          ! ln(pe)
...
2000  continue
 ...
 subroutine del2_cubed(q, cd, gridstruct, domain, npx, npy, km, nmax, bd)
      ...
      real(FVPRC) :: fx(bd%isd:bd%ied+1,bd%jsd:bd%jed), fy(bd%isd:bd%ied,bd%jsd:bd%jed+1)
...
!$OMP                          private(fx, fy)
         do k=1,km
            ...
            if(nt>0) call copy_corners(q(isd:ied,jsd:jed,k), npx, npy, 1, gridstruct%nested, bd, &
            ...
            if(nt>0) call copy_corners(q(isd:ied,jsd:jed,k), npx, npy, 2, gridstruct%nested, bd, &
                 gridstruct%sw_corner, gridstruct%se_corner, gridstruct%nw_corner, gridstruct%ne_corner)
            do j=js-nt,je+1+nt
               do i=is-nt,ie+nt
