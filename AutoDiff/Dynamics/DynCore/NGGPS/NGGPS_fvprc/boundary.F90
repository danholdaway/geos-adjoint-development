module boundary_mod
  ...
  real(FVPRC) :: outflowspd_east  = 3.
  real(FVPRC) :: outflowspd_west  = 3.
  real(FVPRC) :: outflowspd_north = 3.
  real(FVPRC) :: outflowspd_south = 3.
...
contains
  ...
  subroutine outflow_u(u, rdx, dt, npx, cgrid, outflow_east, outflow_west, bd)
    ...
    real(FVPRC), intent(inout), dimension(bd%isd:bd%ied+cgrid  ,bd%jsd:bd%jed+1-cgrid) :: u
    real(FVPRC), intent(in), dimension(bd%isd:bd%ied+cgrid  ,bd%jsd:bd%jed+1-cgrid) :: rdx
  ...
  subroutine outflow_v(v, rdy, dt, npy, cgrid, outflow_north, outflow_south, bd)
    ...
    real(FVPRC), intent(inout), dimension(bd%isd:bd%ied+1-cgrid  ,bd%jsd:bd%jed+cgrid) :: v
    real(FVPRC), intent(in), dimension(bd%isd:bd%ied+1-cgrid  ,bd%jsd:bd%jed+cgrid) :: rdy
    real(FVPRC), intent(in) :: dt
  ...
  subroutine outflow_x(u, q, rdx, dt, npx, istag, jstag, outflow_east, outflow_west, bd)
    ...
    real(FVPRC), intent(in), dimension(bd%isd:bd%ied+istag, bd%jsd:bd%jed+jstag) :: u, rdx
    real(FVPRC), intent(inout), dimension(bd%isd:bd%ied+istag, bd%jsd:bd%jed+jstag) :: q
    real(FVPRC), intent(in) :: dt
    logical, intent(in) :: outflow_east, outflow_west
  ...
  subroutine outflow_y(v, q, rdy, dt, npy, istag, jstag, outflow_north, outflow_south, bd)
    ...
    real(FVPRC), intent(inout), dimension(bd%isd:bd%ied+istag, bd%jsd:bd%jed+jstag) :: q
    real(FVPRC), intent(in), dimension(bd%isd:bd%ied+istag, bd%jsd:bd%jed+jstag) :: v, rdy
    real(FVPRC), intent(in) :: dt
    logical, intent(in) :: outflow_north, outflow_south
  ...
  subroutine fill_nested_grid_2D(var_nest, var_coarse, ind, wt, istag, jstag,  &
   ...
   type(fv_grid_bounds_type), intent(IN) :: bd
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%jed+jstag), intent(INOUT) :: var_nest
   real(FVPRC), dimension(isg:ieg+istag,jsg:jeg+jstag), intent(IN) :: var_coarse 
   integer, dimension(bd%isd:bd%ied+istag,bd%jsd:bd%jed+jstag,2), intent(IN) :: ind
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%jed+jstag,4), intent(IN) :: wt
  ...
  subroutine fill_nested_grid_3D(var_nest, var_coarse, ind, wt, istag, jstag,  &
   ...
   type(fv_grid_bounds_type), intent(IN) :: bd
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%jed+jstag,npz), intent(INOUT) :: var_nest
   real(FVPRC), dimension(isg:ieg+istag,jsg:jeg+jstag,npz), intent(IN) :: var_coarse
   integer, dimension(bd%isd:bd%ied+istag,bd%jsd:bd%jed+jstag,2), intent(IN) :: ind
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%jed+jstag,4), intent(IN) :: wt
...
!!$   endif
 ...
 subroutine nested_grid_BC_mpp_send(var_coarse, nest_domain, istag, jstag)
   ...
   real(FVPRC), dimension(:,:,:), intent(IN) :: var_coarse
   type(nest_domain_type), intent(INOUT) :: nest_domain
...
!!$   endif
 ...
 subroutine nested_grid_BC_save_mpp(nest_domain, ind, wt, istag, jstag, &
   ...
   logical, intent(IN), OPTIONAL :: proc_in, pd_in, debug_in
   ...
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%jed+jstag,npz) :: var_coarse_dummy

   real(FVPRC), dimension(:,:,:), pointer :: var_east, var_west, var_south, var_north
   !real(FVPRC), dimension(bd%ie+1+istag-ns:bd%ied+istag,bd%jsd:bd%jed+jstag,npz), intent(out) :: var_east
   !real(FVPRC), dimension(bd%isd:bd%is-1+ns,bd%jsd:bd%jed+jstag,npz), intent(out) :: var_west
   !real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%js-1+ns,npz), intent(out) :: var_south
   !real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%je+1+jstag-ns:bd%jed+jstag,npz), intent(out) :: var_north
   ...
   integer :: ns = 0
   logical :: process, pd = .false., debug = .false.
...
!!$       print*, mpp_pe, 'CALLING UPDATE_NEST_FINE'
 ...
 subroutine nested_grid_BC_save_2D(var_coarse, ind, wt, istag, jstag, &
   ...
   real(FVPRC), dimension(bd%ie+1+istag-ns:bd%ied+istag,bd%jsd:bd%jed+jstag), intent(out) :: var_east
   real(FVPRC), dimension(bd%isd:bd%is-1+ns,bd%jsd:bd%jed+jstag), intent(out) :: var_west
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%js-1+ns), intent(out) :: var_south
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%je+1+jstag-ns:bd%jed+jstag), intent(out) :: var_north
 ...
 subroutine nested_grid_BC_apply_3D(var_nest, istag, jstag, &
   ...
   real(FVPRC), intent(IN) :: s_weight
   
   real(FVPRC), dimension(bd%ie+1+istag-nsponge:bd%ied+istag,bd%jsd:bd%jed+jstag,npz), intent(in) :: var_east
   real(FVPRC), dimension(bd%isd:bd%is-1+nsponge,bd%jsd:bd%jed+jstag,npz), intent(in) :: var_west
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%js-1+nsponge,npz), intent(in) :: var_south
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%je+1+jstag-nsponge:bd%jed+jstag,npz), intent(in) :: var_north

   integer :: i,j, istart, iend, k
   real(FVPRC) :: denom
   real(FVPRC) :: weights(nsponge)
...
!   end if
 ...
 subroutine nested_grid_BC_apply_2D(var_nest, istag, jstag, &
   ...
   real(FVPRC), dimension(bd%ie+1+istag-nsponge:bd%ied+istag,bd%jsd:bd%jed+jstag), intent(in) :: var_east
   real(FVPRC), dimension(bd%isd:bd%is-1+nsponge,bd%jsd:bd%jed+jstag), intent(in) :: var_west
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%js-1+nsponge), intent(in) :: var_south
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%je+1+jstag-nsponge:bd%jed+jstag), intent(in) :: var_north
...
!   end if
 ...
 subroutine nested_grid_BC_apply_intT_3D(var_nest, istag, jstag, &
   ...
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%jed+jstag, npz), intent(INOUT) :: var_nest
   integer, intent(IN) :: istag, jstag, npx, npy, npz
   real(FVPRC), intent(IN) :: split, step
   integer, intent(IN) :: bctype, nsponge
   real(FVPRC), intent(IN) :: s_weight
   
   real(FVPRC), dimension(bd%ie+1+istag-nsponge:bd%ied+istag,bd%jsd:bd%jed+jstag,npz), intent(in) :: var_east_t0
   real(FVPRC), dimension(bd%isd:bd%is-1+nsponge,bd%jsd:bd%jed+jstag,npz), intent(in) :: var_west_t0
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%js-1+nsponge,npz), intent(in) :: var_south_t0
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%je+1+jstag-nsponge:bd%jed+jstag,npz), intent(in) :: var_north_t0

   real(FVPRC), dimension(bd%ie+1+istag-nsponge:bd%ied+istag,bd%jsd:bd%jed+jstag,npz), intent(in) :: var_east_t1
   real(FVPRC), dimension(bd%isd:bd%is-1+nsponge,bd%jsd:bd%jed+jstag,npz), intent(in) :: var_west_t1
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%js-1+nsponge,npz), intent(in) :: var_south_t1
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%je+1+jstag-nsponge:bd%jed+jstag,npz), intent(in) :: var_north_t1
   ...
   if (is == 1  ) then
      ...
      if (nsponge > 0) then
         ...
         do j=js,je+jstag
         do i=1,nsponge
            var_nest(i,j,k) = var_nest(i,j,k) - weights(i)*( var_nest(i,j,k) - &
                              (var_west_t0(i,j,k)*(split-step) + step*var_west_t1(i,j,k))*denom )
         end do
         end do
         end do
      end if
   ...
   if (js == 1  ) then
      ...
      if (nsponge > 0) then
         ...
         do j=1,nsponge
         do i=istart,iend+istag
            var_nest(i,j,k) = var_nest(i,j,k) - weights(j)*( var_nest(i,j,k) - &
                               (var_south_t0(i,j,k)*(split-step) + step*var_south_t1(i,j,k))*denom )
         end do
         end do
         end do
      end if
   ...
   if (ie == npx-1 ) then
      ...
      if (nsponge > 0) then
         ...
         do j=js,je+jstag
         do i=npx+istag-nsponge,npx-1+istag
            var_nest(i,j,k) = var_nest(i,j,k) - weights(npx+istag-i)*( var_nest(i,j,k) - &
                              (var_east_t0(i,j,k)*(split-step) + step*var_east_t1(i,j,k))*denom )
         end do
         end do
         end do
      end if
   ...
   if (je == npy-1 ) then
      ...
      if (nsponge > 0) then
         ...
         do j=npy+jstag-nsponge,npy-1+jstag
         do i=istart,iend+istag
            var_nest(i,j,k) = var_nest(i,j,k) - weights(npy+jstag-j)*( var_nest(i,j,k) - &
                              (var_north_t0(i,j,k)*(split-step) + step*var_north_t1(i,j,k))*denom )
         end do
         end do
         end do
      end if
 ...
 subroutine nested_grid_BC_apply_intT_2D(var_nest, istag, jstag, &
   ...
   real(FVPRC), dimension(bd%ie+1+istag-nsponge:bd%ied+istag,bd%jsd:bd%jed+jstag), intent(in) :: var_east_t0
   real(FVPRC), dimension(bd%isd:bd%is-1+nsponge,bd%jsd:bd%jed+jstag), intent(in) :: var_west_t0
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%js-1+nsponge), intent(in) :: var_south_t0
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%je+1+jstag-nsponge:bd%jed+jstag), intent(in) :: var_north_t0

   real(FVPRC), dimension(bd%ie+1+istag-nsponge:bd%ied+istag,bd%jsd:bd%jed+jstag), intent(in) :: var_east_t1
   real(FVPRC), dimension(bd%isd:bd%is-1+nsponge,bd%jsd:bd%jed+jstag), intent(in) :: var_west_t1
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%jsd:bd%js-1+nsponge), intent(in) :: var_south_t1
   real(FVPRC), dimension(bd%isd:bd%ied+istag,bd%je+1+jstag-nsponge:bd%jed+jstag), intent(in) :: var_north_t1
   ...
   if (is == 1  ) then
      ...
      if (nsponge > 0) then
         do j=js,je+jstag
         do i=1,nsponge
            var_nest(i,j) = var_nest(i,j) - weights(i)*( var_nest(i,j) - &
                            (var_west_t0(i,j)*(split-step) + step*var_west_t1(i,j))*denom )
         end do
         end do
      end if
   ...
   if (js == 1  ) then
      ...
      if (nsponge > 0) then
         ...
         do j=1,nsponge
         do i=istart,iend+istag
            var_nest(i,j) = var_nest(i,j) - weights(j)*( var_nest(i,j) - &
                            (var_south_t0(i,j)*(split-step) + step*var_south_t1(i,j))*denom )
         end do
         end do
      end if
   ...
   if (ie == npx-1 ) then
      ...
      if (nsponge > 0) then
         do j=js,je+jstag
         do i=npx+istag-nsponge,npx-1+istag
            var_nest(i,j) = var_nest(i,j) - weights(npx+istag-i)*( var_nest(i,j) - &
                                    (var_east_t0(i,j)*(split-step) + step*var_east_t1(i,j))*denom  )
         end do
         end do
      end if
   ...
   if (je == npy-1 ) then
      ...
      if (nsponge > 0) then
         ...
         do j=npy+jstag-nsponge,npy-1+jstag
         do i=istart,iend+istag
            var_nest(i,j) = var_nest(i,j) - weights(npy+jstag-j)*( var_nest(i,j) - &
                               (var_north_t0(i,j)*(split-step) + step*var_north_t1(i,j))*denom )
         end do
         end do
      end if
   end if
 ...
 subroutine nested_grid_sponge_apply(var_nest, istag, jstag, &
   ...
   real(FVPRC), dimension(npx-nsponge:bd%ied+istag,bd%jsd:bd%jed+jstag)        , intent(in) :: var_east
   real(FVPRC), dimension(bd%isd:nsponge          ,bd%jsd:bd%jed+jstag)        , intent(in) :: var_west
   real(FVPRC), dimension(bd%isd:bd%ied+istag        ,bd%jsd:nsponge)          , intent(in) :: var_south
   real(FVPRC), dimension(bd%isd:bd%ied+istag        ,npy-nsponge:bd%jed+jstag), intent(in) :: var_north

   integer :: i,j, istart, iend, bctype
   real(FVPRC), parameter :: s_weight = 0.1
   real(FVPRC) :: denom
   real(FVPRC) :: weights(nsponge)
...
!   end if
 ...
 subroutine gather_grid_2d(parent_grid, var_coarse_this_grid, var_coarse, &
   ...
   real(FVPRC), dimension(isd_p:ied_p+istag,jsd_p:jed_p+jstag), intent(IN) :: var_coarse_this_grid
   real(FVPRC), dimension(isg:ieg+istag,jsg:jeg+jstag,1), intent(out) :: var_coarse
...
!!$   call mpp_sync
 ...
 subroutine gather_grid_3d(parent_grid, var_coarse_this_grid, var_coarse, &
   ...
   type(fv_atmos_type), intent(in) :: parent_grid
   real(FVPRC), dimension(isg:ieg+istag,jsg:jeg+jstag,npz), intent(out) :: var_coarse
   real(FVPRC), dimension(isd_p:ied_p+istag,jsd_p:jed_p+jstag,npz), intent(IN) :: var_coarse_this_grid
   integer, intent(IN) :: isd_p, ied_p, jsd_p, jed_p, isg, ieg, jsg, jeg, &
...
!!$   call mpp_sync
 ...
 subroutine update_coarse_grid_mpp_2d(var_coarse, var_nest, nest_domain, ind_update, dx, dy, area, &
   ...
   real(FVPRC), intent(IN)    :: var_nest(is_n:ie_n+istag,js_n:je_n+jstag)
   real(FVPRC), intent(INOUT) :: var_coarse(isd_p:ied_p+istag,jsd_p:jed_p+jstag)
   ...
   real(FVPRC) :: var_nest_3d(is_n:ie_n+istag,js_n:je_n+jstag,1)
   real(FVPRC) :: var_coarse_3d(isd_p:ied_p+istag,jsd_p:jed_p+jstag,1)

   if (child_proc .and. size(var_nest) > 1) &
   var_nest_3d(is_n:ie_n+istag,js_n:je_n+jstag,1) = var_nest(is_n:ie_n+istag,js_n:je_n+jstag)
   if (parent_proc .and. size(var_coarse) > 1) &
   var_coarse_3d(isd_p:ied_p+istag,jsd_p:jed_p,1) = var_coarse(isd_p:ied_p+istag,jsd_p:jed_p+jstag)
   ...
   if (size(var_coarse) > 1 .and. parent_proc) &
   var_coarse(isd_p:ied_p+istag,jsd_p:jed_p+jstag) = var_coarse_3d(isd_p:ied_p+istag,jsd_p:jed_p,1)
  ...
  subroutine update_coarse_grid_mpp(var_coarse, var_nest, nest_domain, ind_update, dx, dy, area, &
   ...
   integer, intent(IN) :: ind_update(isd_p:ied_p+1,jsd_p:jed_p+1,2)
   real(FVPRC), intent(IN)    :: var_nest(is_n:ie_n+istag,js_n:je_n+jstag,npz)
   real(FVPRC), intent(INOUT) :: var_coarse(isd_p:ied_p+istag,jsd_p:jed_p+jstag,npz)
   real(FVPRC), intent(IN)    :: area(isd:ied,jsd:jed)
   real(FVPRC), intent(IN)    :: dx(isd:ied,jsd:jed+1)
   real(FVPRC), intent(IN)    :: dy(isd:ied+1,jsd:jed)
   logical, intent(IN) :: parent_proc, child_proc
   ...
   real(FVPRC) :: val
   real(FVPRC), allocatable, dimension(:,:,:) :: nest_dat
   real(FVPRC) ::  var_nest_send(is_n:ie_n+istag,js_n:je_n+jstag,npz)
   integer :: position
