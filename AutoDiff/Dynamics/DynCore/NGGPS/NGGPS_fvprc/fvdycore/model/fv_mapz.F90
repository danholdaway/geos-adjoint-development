...
! !INPUT/OUTPUT
  ...
  real(FVPRC), intent(inout):: ze0(is:,js:,1:)    ! Specified height at edges (m)
...
! u-wind will be ghosted one latitude to the north upon exit
  ...
  real(FVPRC), intent(inout)::  w(isd:,jsd:,1:)   ! vertical velocity (m/s)
  real(FVPRC), intent(inout):: pt(isd:ied  ,jsd:jed  ,km)   ! cp*virtual potential temperature 
                                                     ! as input; output: temperature
  real(FVPRC), intent(inout), dimension(isd:,jsd:,1:)::delz, q_con, cappa
...
! !INPUT PARAMETERS:
   ...
   real(FVPRC), intent(in)::  w(isd:,jsd:,1:)   ! vertical velocity (m/s)
   real(FVPRC), intent(in):: delz(isd:,jsd:,1:)
   real(FVPRC), intent(in):: hs(isd:ied,jsd:jed)  ! surface geopotential
