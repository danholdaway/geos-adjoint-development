...
! from the D grid variables.
    real(FVPRC), _ALLOCATABLE :: u(:,:,:)    _NULL  ! D grid zonal wind (m/s)
    real(FVPRC), _ALLOCATABLE :: v(:,:,:)    _NULL  ! D grid meridional wind (m/s)
    real(FVPRC), _ALLOCATABLE :: pt(:,:,:)   _NULL  ! temperature (K)
    real(FVPRC), _ALLOCATABLE :: delp(:,:,:) _NULL  ! pressure thickness (pascal)
    real(FVPRC), _ALLOCATABLE :: q(:,:,:,:)  _NULL  ! specific humidity and constituents

!----------------------
! non-hydrostatic state:
!----------------------------------------------------------------------
    real(FVPRC), _ALLOCATABLE ::     w(:,:,:)  _NULL  ! cell center vertical wind (m/s)
    real(FVPRC), _ALLOCATABLE ::  delz(:,:,:)  _NULL  ! layer thickness (meters)
    real(FVPRC), _ALLOCATABLE ::   ze0(:,:,:)  _NULL  ! height at layer edges for remapping
    real(FVPRC), _ALLOCATABLE ::  q_con(:,:,:) _NULL  ! total condensates
...
!-----------------------------------------------------------------------
! dyn_aux:
    real(FVPRC), _ALLOCATABLE :: ps (:,:)      _NULL  ! Surface pressure (pascal)
    real(FVPRC), _ALLOCATABLE :: pe (:,:,: )   _NULL  ! edge pressure (pascal)
    real(FVPRC), _ALLOCATABLE :: pk  (:,:,:)   _NULL  ! pe**cappa
    real(FVPRC), _ALLOCATABLE :: peln(:,:,:)   _NULL  ! ln(pe)
    real(FVPRC), _ALLOCATABLE :: pkz (:,:,:)   _NULL  ! finite-volume mean pk

! For phys coupling:
    real(FVPRC), _ALLOCATABLE :: u_srf(:,:)    _NULL  ! Surface u-wind
    real(FVPRC), _ALLOCATABLE :: v_srf(:,:)    _NULL  ! Surface v-wind
    real(FVPRC), _ALLOCATABLE :: sgh(:,:)      _NULL  ! Terrain standard deviation
    real(FVPRC), _ALLOCATABLE :: oro(:,:)      _NULL  ! land fraction (1: all land; 0: all water)
    real(FVPRC), _ALLOCATABLE :: ts(:,:)       _NULL  ! skin temperature (sst) from NCEP/GFS (K) -- tile
 
!-----------------------------------------------------------------------
! Others:
!-----------------------------------------------------------------------
    real(FVPRC), _ALLOCATABLE :: phis(:,:)     _NULL  ! Surface geopotential (g*Z_surf)
    ...
    real(FVPRC), _ALLOCATABLE :: ak(:)  _NULL
    real(FVPRC), _ALLOCATABLE :: bk(:)  _NULL
