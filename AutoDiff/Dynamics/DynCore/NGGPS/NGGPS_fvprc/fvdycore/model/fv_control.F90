! $Id: fv_control.F90,v 1.1.2.1.2.2.2.1.22.1.22.4.10.1.48.3.4.4.2.3.2.1.6.1.2.1 2017/02/16 03:47:47 aoloso Exp $
!
!----------------
! FV contro panel
!----------------
...
!CLEANUP module pointers
   ...
   real(FVPRC)    , pointer :: scale_z 
   real(FVPRC)    , pointer :: w_max 
   real(FVPRC)    , pointer :: z_min 

   integer , pointer :: nord
   real(FVPRC)    , pointer :: dddmp 
   real(FVPRC)    , pointer :: d2_bg 
   real(FVPRC)    , pointer :: d4_bg 
   real(FVPRC)    , pointer :: vtdm4 
   real(FVPRC)    , pointer :: d2_bg_k1 
   real(FVPRC)    , pointer :: d2_bg_k2 
   real(FVPRC)    , pointer :: d2_divg_max_k1 
   real(FVPRC)    , pointer :: d2_divg_max_k2 
   real(FVPRC)    , pointer :: damp_k_k1 
   real(FVPRC)    , pointer :: damp_k_k2 
...
! PG off centering:
   real(FVPRC)    , pointer :: beta  
#ifdef SW_DYNAMICS
   integer , pointer :: n_sponge 
   real(FVPRC)    , pointer :: d_ext 
   ...
   real(FVPRC)    , pointer :: p_fac
   real(FVPRC)    , pointer :: a_imp
   ...
   real(FVPRC)    , pointer :: p_ref 
   real(FVPRC)    , pointer :: dry_mass 
   integer , pointer :: nt_prog 
   integer , pointer :: nt_phys 
   real(FVPRC)    , pointer :: tau_h2o 

   real(FVPRC)    , pointer :: d_con 
   real(FVPRC)    , pointer :: consv_te 
   real(FVPRC)    , pointer :: tau 
   real(FVPRC)    , pointer :: rf_cutoff
...
#ifdef MAPL_MODE
   real(FVPRC)   :: dyn_timer, comm_timer
...
!---- version number -----
   character(len=128) :: version = '$Id: fv_control.F90,v 1.1.2.1.2.2.2.1.22.1.22.4.10.1.48.3.4.4.2.3.2.1.6.1.2.1 2017/02/16 03:47:47 aoloso Exp $'
   character(len=128) :: tagname = '$Name: Heracles-UNSTABLE_ncepdyn_Feb222017 $'


   integer dumm
   real(FVPRC) :: umax = 350.           ! max wave speed for grid_type>3
   ...
   namelist /fv_core_nml/npx, npy, ntiles, npz, npz_rst, layout, io_layout, ncnst, nwat,  &
                         ...
                         ioffset, joffset, check_negative, nudge_ic, gmao_cubic

   namelist /test_case_nml/test_case,alpha
 
   public :: fv_atm,grids_on_my_pe
...
!--------------------------------------------------
         ...
         if (grids_on_this_pe(n)) then
            call allocate_fv_atmos_type(Atm(n), Atm(n)%bd%isd, Atm(n)%bd%ied, Atm(n)%bd%jsd, Atm(n)%bd%jed, &
                 ...
                 npx, npy, npz, ndims, ncnst, ng, .false., grids_on_this_pe(n), ngrids)
            ...
            call allocate_fv_atmos_type(Atm(n),  Atm(n)%bd%isd, Atm(n)%bd%ied, Atm(n)%bd%jsd, Atm(n)%bd%jed, &
                 Atm(n)%bd%isc, Atm(n)%bd%iec, Atm(n)%bd%jsc, Atm(n)%bd%jec, &
                 npx, npy, npz, ndims, ncnst, ng, .true., .false., ngrids)
...
!
      ...
      real(FVPRC) :: dim0 = 180.           ! base dimension
      real(FVPRC) :: dt0  = 1800.          ! base time step
      real(FVPRC) :: ns0  = 5.             ! base nsplit for base dimension 
                                    ! For cubed sphere 5 is better
      !real(FVPRC) :: umax = 350.           ! max wave speed for grid_type>3 ! Now defined above
      real(FVPRC) :: dimx, dl, dp, dxmin, dymin, d_fac
...
!!$            if (Atm(n)%neststruct%nestbctype == 3 .or. Atm(n)%neststruct%nestbctype == 4) Atm(n)%parent_grid%neststruct%do_2way_flux_BCs = .true.
  ...
  subroutine setup_pointers(Atm)
    ...
    !This routine associates the MODULE flag pointers with the ARRAY flag variables for the grid active on THIS pe so the flags can be read in from the namelist.
     ...
     gmao_cubic                    => Atm%flagstruct%gmao_cubic
