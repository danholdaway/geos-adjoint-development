...
!-----------------------------------------------------------------------
  ...
  subroutine fv_dynamics(npx, npy, npz, nq_tot,  ng, bdt, consv_te, fill,               &
    ...
    real(FVPRC), intent(inout) ::  ze0(bd%is:bd%is,bd%js:bd%js,1) ! height at edges (m); non-hydrostatic
...
! Local Arrays
      ...
      real(FVPRC), allocatable :: dp1(:,:,:), dtdt_m(:,:,:), cappa(:,:,:)
...
911  call cubed_to_latlon(u, v, ua, va, gridstruct, &
 ...
 subroutine Rayleigh_Super(dt, npx, npy, npz, ks, pm, phis, tau, u, v, w, pt,  &
    ...
    real(FVPRC), intent(inout)::  w(bd%isd:,bd%jsd:,1:) ! cell center vertical wind (m/s)
