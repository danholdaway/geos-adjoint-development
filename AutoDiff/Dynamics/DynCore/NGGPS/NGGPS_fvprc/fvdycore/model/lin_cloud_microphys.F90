...
!==== fms constants ====================
!!! real(FVPRC), parameter :: latv  = hlv             ! = 2.500e6
!!!  real(FVPRC), parameter:: cv_air = 717.56   ! Satoh value
!!! real(FVPRC), parameter :: lati  = hlf             ! = 3.34e5
!!! real(FVPRC), parameter :: lats  = latv+lati       ! = 2.834E6
...
!!! real(FVPRC), parameter :: c_liq = 4190.           ! heat capacity of water at 0C
                                            !
 real(FVPRC), parameter :: eps   = rdgas/rvgas     ! = 0.621971831
 real(FVPRC), parameter :: zvir  = rvgas/rdgas-1.  ! = 0.607789855
 real(FVPRC), parameter :: table_ice  = 273.16  ! freezing point for qs table
 real(FVPRC), parameter :: cv_air =  cp_air - rdgas ! = rdgas * (7/2-1) = 2.5*rdgas=717.68

   real(FVPRC), parameter:: e00 = 610.71  ! saturation vapor pressure at T0
   real(FVPRC), parameter:: c_liq = 4190.       ! heat capacity of water at 0C
   real(FVPRC), parameter:: c_ice = 2106.           ! heat capacity of ice at 0C: c=c_ice+7.3*(T-Tice) 
   real(FVPRC), parameter:: cp_vap = cp_vapor   ! 1846.
!  real(FVPRC), parameter:: cv_vap = 1410.0     ! Emanuel value
! For consistency, cv_vap derived FMS constants:
   real(FVPRC), parameter:: cv_vap = cp_vap - rvgas  ! 1384.5
   real(FVPRC), parameter:: dc_vap = cp_vap - c_liq     ! = -2344.    isobaric heating/cooling
   real(FVPRC), parameter:: dc_ice  =  c_liq - c_ice     ! =  2084

! Values at 0 Deg C
   real(FVPRC), parameter:: hlv0 = 2.501e6   ! Emanuel Appendix-2
   real(FVPRC), parameter:: hlf0 = 3.337e5   ! Emanuel
   real(FVPRC), parameter:: t_ice = 273.16
   real(FVPRC), parameter:: Lv0 =  hlv0 - dc_vap*t_ice   ! = 3.141264e6
   real(FVPRC), parameter:: li00 = hlf0 - dc_ice *t_ice   ! = -2.355446e5

   real(FVPRC), parameter:: d2ice   = cp_vap - c_ice
   real(FVPRC), parameter:: Li2 = hlv0+hlf0 - d2ice*t_ice
!==== fms constants ====================

 real(FVPRC), parameter :: qrmin  = 1.e-9
 real(FVPRC), parameter :: qvmin  = 1.e-22      ! min value for water vapor (treated as zero)
 real(FVPRC), parameter :: qcmin  = 1.e-12      ! min value for cloud condensates
 real(FVPRC), parameter :: sfcrho = 1.20        ! surface air density
 real(FVPRC), parameter :: vmin   = 1.e-3       ! minimum fall speed for rain/graupel
 real(FVPRC), parameter :: rhor   = 1.0e3  ! LFO83
 real(FVPRC), parameter :: dz_min = 1.e-2
 real(FVPRC) :: cracs, csacr, cgacr, cgacs, acco(3,4), csacw,          &
         craci, csaci, cgacw, cgaci, cracw, cssub(5), cgsub(5), &
         crevp(5), cgfr(2), csmlt(5), cgmlt(5)
 real(FVPRC) :: es0, ces0
 real(FVPRC) :: pie, rgrav, fac_rc
 real(FVPRC) :: lcp, icp, tcp
 real(FVPRC) :: lv00, c_air, c_vap
 ...
 real(FVPRC), allocatable:: table(:), table2(:), table3(:), tablew(:), des(:), des2(:), des3(:), desw(:)
 ...
 real(FVPRC) :: lati, latv, lats

 real(FVPRC), parameter :: dt_fr = 8.       ! homogeneous freezing of all cloud water at t_wfr - dt_fr
 ...
 real(FVPRC) :: t_snow_melt = 12.      ! snow melt tempearture scale factor
 real(FVPRC) :: t_grau_melt = 15.      ! graupel melt tempearture scale factor
 real(FVPRC) :: p_min = 100.    ! minimum pressure (Pascal) for MP to operate
...
! namelist  parameters:
!----------------------
 real(FVPRC) :: cld_min = 0.05
 real(FVPRC) :: tice  = 273.16  ! set tice = 165. to trun off ice-phase phys (Kessler emulator)

 real(FVPRC) :: qc_crt  = 5.0e-8  ! minimum condensate mixing ratio to allow partial cloudiness
 real(FVPRC) :: t_min   = 180.  ! Min temp to freeze-dry all water vapor
 real(FVPRC) :: t_sub   = 184.  ! Min temp for sublimation of cloud ice
 real(FVPRC) :: mp_time = 150.  ! maximum micro-physics time step (sec)

 real(FVPRC) :: rh_inc = 0.10   ! rh increment for complete evap of ql and qi
 real(FVPRC) :: rh_inr = 0.25
 real(FVPRC) :: rh_ins = 0.25   ! rh increment for sublimation of snow

 real(FVPRC) :: tau_r  = 120.    ! rain freezing time scale during fast_sat
! The following 3 time scales are for melting during terminal falls
 real(FVPRC) :: tau_s  = 120.    ! snow melt
 real(FVPRC) :: tau_g  = 180.    ! graupel melt
 real(FVPRC) :: tau_mlt = 10.    ! ice melting time-scale

! Fast MP:
 real(FVPRC) :: tau_i2s = 150.   ! 
! cloud water
 real(FVPRC) :: tau_v2l = 600.   ! vapor --> cloud water (condensation)  time scale
 real(FVPRC) :: tau_l2v = 600.   ! cloud water --> vapor (evaporation)  time scale
! Graupel
 real(FVPRC) :: tau_g2v = 900.   ! Grapuel sublimation time scale
 real(FVPRC) :: tau_v2g =21600. ! Grapuel deposition -- make it a slow process

 real(FVPRC) :: dw_land  = 0.20  ! base value for subgrid deviation/variability over land
 real(FVPRC) :: dw_ocean = 0.15  ! base value for ocean
 real(FVPRC) :: ccn_o = 100.
 real(FVPRC) :: ccn_l = 250.
 real(FVPRC) :: rthresh = 10.0e-6     ! critical cloud drop radius (micro m)
...
! WRF/WSM6 ice initiation scheme; qi_crt = qi_gen*min(qi_lim, 0.1*tmp) / den
 real(FVPRC) :: qi_gen  = 1.818E-6
 real(FVPRC) :: qi_lim  = 1.
 real(FVPRC) :: ql_mlt  = 4.0e-3    ! max value of cloud water allowed from melted cloud ice
 real(FVPRC) :: ql_gen  = 1.0e-3    ! max ql generation during remapping step if fast_sat_adj = .T.
 real(FVPRC) :: sat_adj0 = 0.99     ! adjustment factor (0: no, 1: full) during fast_sat_adj

! Cloud condensate upper bounds: "safety valves" for ql & qi
 real(FVPRC) :: ql0_max = 2.0e-3    ! max ql value (converted to rain)
 real(FVPRC) :: qi0_max = 3.0e-6    ! max qi value (by other sources)

 real(FVPRC) :: qi0_crt = 1.0e-4    ! ice  --> snow autocon threshold (was 1.E-4)
                             ! qi0_crt is highly dependent on horizontal resolution
 real(FVPRC) :: qr0_crt = 1.0e-4    ! rain --> snow or graupel/hail threshold
                             ! LFO used *mixing ratio* = 1.E-4 (hail in LFO)
 real(FVPRC) :: c_paut  = 0.55     ! autoconversion ql --> qr  (use 0.5 to reduce autoconversion)
 real(FVPRC) :: c_psaut = 1.0e-3   ! autoconversion rate: cloud_ice -> snow
 real(FVPRC) :: c_psaci = 0.01     ! accretion: cloud ice --> snow (was 0.1 in Zetac)
 real(FVPRC) :: c_piacr = 5.0      ! accretion: rain --> ice:
 real(FVPRC) :: c_cracw = 0.9      ! rain accretion efficiency

! Decreasing  clin to reduce csacw (so as to reduce cloud water ---> snow)
 real(FVPRC) :: alin = 842.0
 real(FVPRC) :: clin = 4.8      ! 4.8 --> 6. (to ehance ql--> qs)

!-----------------
! Graupel control:
!-----------------
 real(FVPRC) :: qs0_crt = 2.0e-3   ! snow --> graupel density threshold (0.6e-3 in Purdue Lin scheme)
 real(FVPRC) :: c_pgacs = 2.0e-3   ! snow --> graupel "accretion" eff. (was 0.1 in Zetac)

! fall velocity tuning constants:
 real(FVPRC) :: den_ref = sfcrho   ! Reference (surface) density for fall speed
 ...
 real(FVPRC) :: global_area = -1.

 real(FVPRC) :: tice0, t_wfr
 real(FVPRC) :: p_crt   = 100.E2   !
 integer:: k_moist = 100
...
!---- version number -----
 character(len=128) :: version = '$Id: lin_cloud_microphys.F90,v 1.2 2017/02/21 20:43:02 bmauer Exp $'
...
! kks == 1; kke == kbot == npz
  ...
  real(FVPRC),            intent(in):: dt_in

  real(FVPRC), intent(in   ), dimension(:,:)  :: area
  real(FVPRC), intent(in   ), dimension(:,:)  :: land  !land fraction
  real(FVPRC), intent(out  ), dimension(:,:)  :: rain, snow, ice, graupel
  real(FVPRC), intent(in   ), dimension(:,:,:):: delp, dz, uin, vin
  real(FVPRC), intent(in   ), dimension(:,:,:):: pt, qv, ql, qr, qg, qa, qn 
  real(FVPRC), intent(inout), dimension(:,:,:):: qi, qs
  real(FVPRC), intent(inout), dimension(:,:,:):: pt_dt,  qa_dt, udt, vdt, w
  real(FVPRC), intent(inout), dimension(:,:,:):: qv_dt, ql_dt, qr_dt, qi_dt,  &
                                          qs_dt, qg_dt


! local:
  logical used
  real(FVPRC) :: mpdt, rdt, dts, convt, tot_prec
  integer :: i,j,k
  integer :: is,ie, js,je  ! physics window
  integer :: ks,ke         ! vertical dimension
  integer :: seconds, days, ntimes

  real(FVPRC), dimension(iie-iis+1,jje-jjs+1)           :: prec1, prec_mp, cond, w_var, rh0, maxdbz
  real(FVPRC), dimension(iie-iis+1,jje-jjs+1,kke-kks+1) :: vt_r, vt_s, vt_g, vt_i, qn2, dbz
  real(FVPRC), dimension(size(pt,1), size(pt,3)) :: m2_rain, m2_sol
...
! 6) qg: graupel     (kg/kg)
  ...
  real(FVPRC), intent(in), dimension(is:):: area1, land
  real(FVPRC), intent(in), dimension(is:,js:,ks:):: uin, vin, delp, pt, qv, ql, qr, qg, qa, qn, dz
  real(FVPRC), intent(inout), dimension(is:,js:,ks:):: qi, qs
  real(FVPRC), intent(inout), dimension(is:,js:,ks:):: u_dt, v_dt, w, pt_dt,  qa_dt,  &
                                            qv_dt, ql_dt, qr_dt, qi_dt, qs_dt, qg_dt
  real(FVPRC), intent(inout), dimension(is:):: rain, snow, ice, graupel, cond
  real(FVPRC), intent(out), dimension(is:,js:)       :: w_var
  real(FVPRC), intent(out), dimension(is:,js:,ks:) :: vt_r, vt_s, vt_g, vt_i, qn2
  real(FVPRC), intent(out), dimension(is:,ks:):: m2_rain,  m2_sol

!----------
! local var
!----------
  real(FVPRC), dimension(ktop:kbot):: qvz, qlz, qrz, qiz, qsz, qgz, qaz, &
                               vtiz, vtsz, vtgz, vtrz, dp0, &
                               dp1, qv0, ql0, qr0, qi0, qs0, qg0, qa0, t0, den, &
                               den0, tz, p1, dz0, dz1, denfac
  real(FVPRC), dimension(ktop:kbot):: ccn, c_praut, m1_rain, m1_sol, m1, u0, v0, u1, v1, w1
  real(FVPRC) :: cpaut,  rh_adj, rh_rain
  real(FVPRC) :: r1, s1, i1, g1, rdt, ccn0
  real(FVPRC) :: dt_rain, dts, fac_sno, fac_gra
  real(FVPRC) :: s_leng, t_land, t_ocean, h_var
  real(FVPRC) :: cvm, tmp, omq
  real(FVPRC) :: dqi, qio, qin
  integer :: i,k,n
...
! Input q fields are dry mixing ratios, and dm is dry air mass
 real(FVPRC), intent(in   ), dimension(ktop:kbot):: dm, m1, dz, qv, ql, qr, qi, qs, qg
 real(FVPRC), intent(inout), dimension(ktop:kbot):: tz
 real(FVPRC), intent(in):: cw
!
 real(FVPRC),dimension(ktop:kbot):: dgz, cvm, cvn
 real(FVPRC) :: tmp
 integer:: k
...
! dm here is dry air mass
 ...
 integer, intent(in):: ktop, kbot
 real(FVPRC),    intent(in):: dt                    ! time step (s)
 real(FVPRC),    intent(in), dimension(ktop:kbot):: dp, p1, dz, pm, den, denfac, ccn, c_praut
 real(FVPRC),    intent(in):: rh_rain, h_var
 real(FVPRC), intent(inout), dimension(ktop:kbot):: tz, qv, ql, qr, qi, qs, qg, vtr
 real(FVPRC), intent(out):: r1
 real(FVPRC), intent(inout), dimension(ktop:kbot):: m1_rain, w1

! local:
 real(FVPRC), parameter:: so3 = 7./3.
 real(FVPRC), dimension(ktop:kbot):: dl, dm
 real(FVPRC), dimension(ktop:kbot+1):: ze, zt
 real(FVPRC) :: sink, dq, qc0, qc
 real(FVPRC) :: rho0, qden
 real(FVPRC) :: zs = 0.
 real(FVPRC) :: dt5
 integer k
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
 real(FVPRC), parameter :: vconr = 2503.23638966667
 real(FVPRC), parameter :: normr = 25132741228.7183
 real(FVPRC), parameter :: thr=1.e-10
 logical no_fall
...
! Revised continuous form: linearly decays (with subgrid dl) to zero at qc == ql + dl
 ...
 integer, intent(in):: ktop, kbot
 real(FVPRC),    intent(in):: dt                 ! time step (s)
 real(FVPRC),    intent(in), dimension(ktop:kbot):: den, denfac
 real(FVPRC),    intent(in)                      :: rh_rain, h_var
 real(FVPRC), intent(inout), dimension(ktop:kbot):: tz, qv, qr, ql, qi, qs, qg
! local:
 real(FVPRC) :: dqv, qsat, dqsdt, evap, t2, qden, q_plus, q_minus, sink
 real(FVPRC) :: qpz, dq, dqh, tin, lcpk
 integer k
...
! Edges: qE == qbar +/- dm
 ...
 real(FVPRC), intent(in ):: p1(km),  q(km), h_var
 real(FVPRC), intent(out):: dm(km)
...
!-------------------------------------------------------
 ...
 integer, intent(in) :: ktop, kbot
 real(FVPRC), intent(in),    dimension(ktop:kbot):: p1, dp1, den, denfac, vts, vtg, vtr
 real(FVPRC), intent(inout), dimension(ktop:kbot):: tzk, qvk, qlk, qrk, qik, qsk, qgk, qak
 real(FVPRC), intent(in) :: rh_adj, rh_rain, dts, fac_sno, fac_gra, h_var
! local:
 real(FVPRC), parameter:: rhos = 0.1e3    ! snow density (1/10 of water)
 real(FVPRC), dimension(2*(kbot-ktop-1)):: p2, den2, tz2, qv2, ql2, qr2, qi2, qs2, qg2, qa2
 real(FVPRC), dimension(ktop:kbot) :: lcpk, icpk, tcpk, di
!!! real(FVPRC) :: rh_sno, rh_gra
 real(FVPRC) :: rdts, fac_g2v, fac_v2g, fac_i2s
 real(FVPRC) :: tz, qv, ql, qr, qi, qs, qg, melt
 real(FVPRC) :: pracw, pracs, psacw, pgacw, pgmlt,   &
        psmlt, prevp, psacr, pgacr, pgfr,  pgacs,   &
        pgaut, pgaci, praci, psaut, psaci, piacr, pgsub
 real(FVPRC) :: tc, tsq, dqs0, qden, qim, qsm, pssub
 real(FVPRC) :: factor, sink
 real(FVPRC) :: tmp1, qsw, qsi, dqsdt, dq
 real(FVPRC) :: dtmp, qc, q_plus, q_minus, cvm
 integer:: km, kn
 integer:: i, j, k, k1
...
! constant distribution if id ==0
 real(FVPRC), intent(in), dimension(ktop:kbot):: q1, dp
 real(FVPRC), intent(out):: q2(km)
! local
 real(FVPRC) :: a4(4,ktop:kbot)
 real(FVPRC) :: tmp
 integer:: k, k1
...
! Temperature sentive high vertical resolution processes:
 ...
 integer, intent(in):: ktop, kbot
 real(FVPRC), intent(in),    dimension(ktop:kbot):: p1, den
 real(FVPRC), intent(in)                         :: dts, rh_adj, h_var
 real(FVPRC), intent(inout), dimension(ktop:kbot):: tz, qv, ql, qr, qi, qs, qg, qa
! local:
 real(FVPRC), dimension(ktop:kbot):: lcpk, icpk, tcpk, tcp3
 real(FVPRC) :: fac_v2l, fac_l2v
 real(FVPRC) :: pidep, qi_crt
! qstar over water may be accurate only down to -80 C with ~10% uncertainty
! must not be too large to allow PSC
 real(FVPRC) :: rh, clouds,  rqi, tin, qsw, qsi, qpz, qstar
 real(FVPRC) :: dqsdt, dwsdt, dq, dq0, cond0, factor, tmp, cvm
 real(FVPRC) :: q_plus, q_minus, dt_pisub
 real(FVPRC) :: evap, sink, tc, pisub, iwt, q_adj, dtmp, lat2
 integer :: k
...
! input pt is T_vir
 real(FVPRC), intent(in):: mdt ! remapping time step
 integer, intent(in):: is, ie, js, je, ng
 logical, intent(in):: hydrostatic, last_step, consv_te, out_dt
 real(FVPRC), intent(in), dimension(is-ng:ie+ng,js-ng:je+ng):: dp, area, delz

 real(FVPRC), intent(in):: dpeln(is:ie,js:je)           ! ln(pe)
 real(FVPRC), intent(inout), dimension(is-ng:ie+ng,js-ng:je+ng):: pt, qv, ql, qi, qr, qs, qg, qa
 real(FVPRC), intent(inout), dimension(is-ng:,js-ng:):: q_con
#ifdef MOIST_CAPPA
 real(FVPRC), intent(inout), dimension(is-ng:,js-ng:):: cappa
#endif
 real(FVPRC), intent(inout)::dtdt(is:ie,js:je)
 real(FVPRC), intent(out):: te0(is:ie,js:je)
! Local:
 real(FVPRC), dimension(is:ie):: cpm, t0, pt1, icp2, lcp2, tcp2, tcp3, den, q_liq, q_sol, hvar, src, p1
 real(FVPRC) :: sink, qsw, rh, fac_v2l, fac_l2v, cond0
 real(FVPRC) :: tc, qsi, dqsdt, dq, dq0, pidep, qi_crt, tmp, dtmp, lat2
 real(FVPRC) :: condensates, qpz, tin, qstar, rqi, q_plus, q_minus
 real(FVPRC) :: sdt, rh_ql, adj_fac, fac_s, fac_r, fac_i2s
 real(FVPRC) :: factor, psacw, qim
 integer i,j
...
! Impose minimum cloudiness if substantial condensates exist
 ...
 real(FVPRC), intent(in):: dt       ! time step (s)
 real(FVPRC), intent(in),    dimension(is:ie):: den, hvar, qi, qs, qg
 real(FVPRC), intent(inout), dimension(is:ie):: tz, qv, qr, ql
! local:
 real(FVPRC), dimension(is:ie):: lcp2, denfac
 real(FVPRC) :: dqv, qsat, dqsdt, evap, qden, q_plus, q_minus, sink
 real(FVPRC) :: tin, t2, qpz, dq, dqh, q_liq, q_sol
 integer i
...
! lagrangian control-volume method:
 ...
 real(FVPRC),    intent(in):: dtm                    ! time step (s)
 integer, intent(in):: ktop, kbot
 real(FVPRC),    intent(in):: fac_sno, fac_gra
 real(FVPRC),    intent(in), dimension(ktop:kbot):: vtg, vts, vti, pm, den, dp, dz
 real(FVPRC),    intent(inout), dimension(ktop:kbot):: qv, ql, qr, qg, qs, qi, tz, m1_sol, w1
 real(FVPRC),    intent(out):: r1, g1, s1, i1
! local:
 real(FVPRC), dimension(ktop:kbot+1):: ze, zt
 real(FVPRC) :: qsat, dqsdt, dt5, melt, evap, dtime
 real(FVPRC) :: factor, frac
 real(FVPRC) :: tmp1, precip, tc, sink
 real(FVPRC), dimension(ktop:kbot):: lcpk, icpk
 real(FVPRC), dimension(ktop:kbot):: m1, dm
 real(FVPRC) :: zs = 0.
 real(FVPRC) :: cvm
 integer k, k0, m
 logical no_fall
...
!----------------------------------------------
 ...
 integer, intent(in):: ktop, kbot
 real(FVPRC),    intent(in):: q(ktop:kbot)
...
! local:
 ...
 real(FVPRC),    intent(in):: zs
 integer, intent(in):: ktop, kbot
 real(FVPRC),    intent(in), dimension(ktop:kbot+1):: ze, zt
 real(FVPRC),    intent(in), dimension(ktop:kbot):: dp
 real(FVPRC),    intent(inout), dimension(ktop:kbot):: q, m1
 real(FVPRC),    intent(out):: precip
! local:
 real(FVPRC), dimension(ktop:kbot):: qm
 integer k, k0, n, m
...
! m1: flux
 ...
 real(FVPRC),    intent(out):: precip
! local:
 real(FVPRC), dimension(ktop:kbot):: qm, dz
 real(FVPRC):: a4(4,ktop:kbot)
 real(FVPRC):: pl, pr, delz, esl
 integer k, k0, n, m
 real(FVPRC), parameter:: r3 = 1./3., r23 = 2./3.
...
! dp must be dry air_mass (because moist air mass will be changed due to terminal fall).
 ...
 real(FVPRC)   , intent(in):: del(km)
 logical, intent(in):: do_mono
 real(FVPRC) , intent(inout):: a4(4,km)
!-----------------------------------------------------------------------
 real(FVPRC), parameter:: qp_min = 1.e-6
 real(FVPRC)  gam(km)
 real(FVPRC)  q(km+1)
 real(FVPRC)   d4, bet, a_bot, grat, pmp, lac
...
! Bottom layer:
 ...
 integer, intent(in) :: km
 real(FVPRC), intent(inout) :: a4(4,km)   ! PPM array
! !LOCAL VARIABLES:
...
! Positive definite constraint
 ...
 integer, intent(in)                     :: ktop, kbot
 real(FVPRC), intent(in ), dimension(ktop:kbot) :: den, qs, qi, qg, ql, tk
 real(FVPRC), intent(out), dimension(ktop:kbot) :: vts, vti, vtg
! fall velocity constants:
 real(FVPRC), parameter :: thi = 1.0e-9   ! cloud ice threshold for terminal fall
 real(FVPRC), parameter :: thg = 1.0e-9
 real(FVPRC), parameter :: ths = 1.0e-9
 real(FVPRC), parameter :: vf_min = 1.0E-5
 real(FVPRC), parameter :: vs_max = 7.        ! max fall speed for snow
!-----------------------------------------------------------------------
! marshall-palmer constants
!-----------------------------------------------------------------------
 real(FVPRC) :: vcons = 6.6280504, vcong = 87.2382675, vconi = 3.29
 real(FVPRC) :: norms = 942477796.076938, &
         normg =  5026548245.74367
 real(FVPRC), dimension(ktop:kbot) :: ri, qden, tc, rhof
 real(FVPRC) :: aa = -4.14122e-5, bb = -0.00538922, cc = -0.0516344, dd = 0.00216078, ee = 1.9714

 real(FVPRC) :: rho0, vconf
 integer:: k
!-----------------------------------------------------------------------
...
! vti = vconi*rhof*(qi*den)**0.16
 ...
 real(FVPRC) :: gcon, cd, scm3, pisq, act(8), acc(3)
 real(FVPRC) :: vdifu, tcond
 real(FVPRC) :: visk
 real(FVPRC) :: ch2o, hltf
 real(FVPRC) ::  hlts, hltc, ri50

 real(FVPRC) :: gam263, gam275, gam290,                                &
...
!
 real(FVPRC) :: rnzr, rnzs, rnzg, rhos, rhog
...
! TESTING the water vapor tables
 ...
 real(FVPRC) function acr3d(v1, v2, q1, q2, c, cac, rho)
 real(FVPRC), intent(in) :: v1, v2, c, rho
 real(FVPRC), intent(in) :: q1, q2    ! mixing ratio!!!
 real(FVPRC), intent(in) :: cac(3)
 real(FVPRC) :: t1, s1, s2
!integer :: k
! real(FVPRC) :: a
!     a=0.0
...
!----------
 ...
 real(FVPRC) function smlt(tc, dqs, qsrho,psacw,psacr,c,rho, rhofac)
 real(FVPRC), intent(in):: tc,dqs,qsrho,psacw,psacr,c(5),rho, rhofac
 ...
 real(FVPRC) function gmlt(tc, dqs,qgrho,pgacw,pgacr,c, rho)
 real(FVPRC), intent(in)::  tc,dqs,qgrho,pgacw,pgacr,c(5),rho
...
! local:
 ...
 real(FVPRC) function wqs2(ta, den, dqdt)
! Pure water phase; universal dry/moist formular using air density
! Input "den" can be either dry or moist air density
  real(FVPRC), intent(in):: ta, den
  real(FVPRC), intent(out):: dqdt
! local:
  real(FVPRC) :: es, ap1
  real(FVPRC), parameter:: tmin=table_ice - 160.
  integer it
...
! Finite diff, del_T = 0.1:
 ...
 real(FVPRC) function wet_bulb(q, t, den)
! Liquid phase only
 real(FVPRC), intent(in):: t, q, den
 real(FVPRC) :: qs, tp, dqdt
...
! tp is negative if super-saturated
 ...
 real(FVPRC) function iqs1(ta, den)
...
! local:
 ...
 real(FVPRC) function iqs2(ta, den, dqdt)
! water-ice phase; universal dry/moist formular using air density
! Input "den" can be either dry or moist air density
  real(FVPRC), intent(in):: ta, den
  real(FVPRC), intent(out):: dqdt
! local:
 ...
 real(FVPRC) function qs1d_moist(ta, qv, pa, dqdt)
! 2-phase tabel
  real(FVPRC), intent(in):: ta, pa, qv
  real(FVPRC), intent(out):: dqdt
! local:
  real(FVPRC) :: es, ap1
  real(FVPRC), parameter:: tmin=table_ice - 160.
  real(FVPRC), parameter:: eps10 = 10.*eps
  integer it
 ...
 real(FVPRC) function wqsat2_moist(ta, qv, pa, dqdt)
! Pure water phase
  real(FVPRC), intent(in):: ta, pa, qv
  real(FVPRC), intent(out):: dqdt
! local:
 ...
 real(FVPRC) function wqsat_moist(ta, qv, pa)
! Pure water phase
  real(FVPRC), intent(in):: ta, pa, qv
! local:
  real(FVPRC) :: es, ap1
 ...
 real(FVPRC) function qs1d_m(ta, qv, pa)
! 2-phase tabel
  real(FVPRC), intent(in):: ta, pa, qv
! local:
  real es, ap1
...
! over Ice:
 ...
 real(FVPRC) function esw_table(ta)
! pure water phase table
 ...
 real(FVPRC) function es2_table(ta)
! two-phase table
  real(FVPRC), intent(in):: ta
  real(FVPRC), parameter:: tmin=table_ice - 160.
  real  ap1
 ...
 subroutine esw_table1d(ta, es, n)
  integer, intent(in):: n
! For waterphase only
  real(FVPRC), intent(in)::  ta(n)
  real(FVPRC), intent(out):: es(n)
  real(FVPRC), parameter:: tmin=table_ice - 160.
  real  ap1
  integer i, it
...
!----------
 ...
 real(FVPRC) function qs_blend(t, p, q)
...
! this is designed for 6-class micro-physics schemes
 integer, intent(in):: ktop, kbot
 real(FVPRC), intent(in):: dp(ktop:kbot), p1(ktop:kbot)
 real(FVPRC), intent(inout), dimension(ktop:kbot)::    &
...
!-------------------------------------------
      ...
      real(FVPRC), intent(in):: phalf(is:ie,js:je,km+1)
      real(FVPRC), intent(in):: pm(is:ie,js:je,km)
      real(FVPRC), intent(in):: zfull(is:ie,js:je,km)
      real(FVPRC), intent(in):: zhalf(is:ie,js:je,km+1)
      real(FVPRC), intent(in):: delp(isd:ied,jsd:jed,km)      ! Delta p at each model level
! Output:
      real(FVPRC), intent(inout), dimension(is:ie,js:je,km)::  ta, ua, va
      real(FVPRC), intent(inout):: qa(is:ie,js:je,km,nq)   ! Specific humidity & tracers
      real(FVPRC), intent(inout):: w(isd:ied,jsd:jed,km)
      real(FVPRC), intent(inout):: u_dt(isd:ied,jsd:jed,km)   ! updated u-wind field
      real(FVPRC), intent(inout):: v_dt(isd:ied,jsd:jed,km)   !         v-wind
      real(FVPRC), intent(inout):: t_dt(is:ie,js:je,km)   !         temperature
      real(FVPRC), intent(inout):: q_dt(is:ie,js:je,km,nq) !
!---------------------------Local variables-----------------------------
      real(FVPRC), dimension(is:ie,km):: tvm, u0, v0, w0, t0, gz, hd, pkz, qcon
      real(FVPRC), dimension(is:ie,km+1):: pk, peln
      real(FVPRC):: q0(is:ie,km,nq)
      real(FVPRC):: gzh(is:ie)
      real(FVPRC):: pbot, ri, pt1, pt2, lf, ratio
      real(FVPRC):: rdt, dh, dh0, dhs, dq, tv, h0, mc, mx,  fra, rk, rz, rcp
      real(FVPRC):: qsw
      integer:: mcond
      integer kcond
      integer i, j, k, n, m, iq, kk
      real(FVPRC), parameter:: ustar2 = 1.E-6
      real(FVPRC), parameter:: dh_min = 1.E-4
...
!     Mark Stoelinga (stoeling@atmos.washington.edu) for a copy.  
   ...
   real(FVPRC),    intent(IN),  dimension(is:, js:, ks:) :: qv, qr, qs, qg, pt, delp, dz
   real(FVPRC),    intent(OUT), dimension(is:, js:, ks:) :: dbz
   real(FVPRC),    intent(OUT), dimension(is:, js:)      :: maxdbz
   logical, intent(IN) :: in0r, in0s, in0g, iliqskin
   real(FVPRC),    intent(OUT) :: allmax

   !Parameters for constant intercepts (in0[rsg] = .false.)
   real(FVPRC), parameter :: rn0_r = 8.e6 ! m^-4
   real(FVPRC), parameter :: rn0_s = 2.e7 ! m^-4
   real(FVPRC), parameter :: rn0_g = 4.e6 ! m^-4

   !Constants for variable intercepts
   real(FVPRC), parameter :: r1=1.e-15
   real(FVPRC), parameter :: ron=8.e6
   real(FVPRC), parameter :: ron2=1.e10
   real(FVPRC), parameter :: son=2.e7
   real(FVPRC), parameter :: gon=5.e7
   real(FVPRC), parameter :: ron_min = 8.e6
   real(FVPRC), parameter :: ron_qr0 = 0.00010
   real(FVPRC), parameter :: ron_delqr0 = 0.25*ron_qr0
   real(FVPRC), parameter :: ron_const1r = (ron2-ron_min)*0.5
   real(FVPRC), parameter :: ron_const2r = (ron2+ron_min)*0.5

   !Other constants
   real(FVPRC), parameter :: gamma_seven = 720.
   real(FVPRC), parameter :: rho_r = rhor ! 1000. kg m^-3
   real(FVPRC), parameter :: rho_s = 100.   ! kg m^-3
   real(FVPRC), parameter :: rho_g = 400.   ! kg m^-3
   real(FVPRC), parameter :: alpha = 0.224
   real(FVPRC), parameter :: factor_r = gamma_seven * 1.e18 * (1./(pi*rho_r))**1.75
   real(FVPRC), parameter :: factor_s = gamma_seven * 1.e18 * (1./(pi*rho_s))**1.75 &
        * (rho_s/rho_r)**2 * alpha
   real(FVPRC), parameter :: factor_g = gamma_seven * 1.e18 * (1./(pi*rho_g))**1.75 &
        * (rho_g/rho_r)**2 * alpha

   integer :: i,j,k
   real(FVPRC) :: factorb_s, factorb_g, rhoair
   real(FVPRC) :: temp_c, pres, sonv, gonv, ronv, z_e
   real(FVPRC) :: q1, q2, q3
 ...
 real(FVPRC) function g_sum(p, ifirst, ilast, jfirst, jlast, area, mode)
!-------------------------
...
!-------------------------
 ...
 real(FVPRC), intent(IN) :: area(ifirst:ilast,jfirst:jlast)
 integer :: i,j
 real gsum
 ...
 real(FVPRC), intent(in):: hght(is:ie,js:je,km+1)  ! hght(k) > hght(k+1)
 real(FVPRC), intent(in):: a3(is:ie,js:je,km)
 real(FVPRC), intent(in):: zl
 real(FVPRC), intent(out):: a2(is:ie,js:je)
! local:
 real zm(km)
 integer i,j,k
