...
!-----------------------------------------------------------------------
 ...
 subroutine dyn_core(npx, npy, npz, ng, sphum, nq, bdt, n_split, zvir, cp, akap, cappa, grav, hydrostatic,  &
    ...
    real(FVPRC), intent(inout) :: w(   bd%isd:,bd%jsd:,1:)  ! vertical vel. (m/s)
    real(FVPRC), intent(inout) ::  delz(bd%isd:,bd%jsd:,1:)  ! delta-height (m)
    real(FVPRC), intent(inout) :: cappa(bd%isd:,bd%jsd:,1:) ! moist kappa
...
! Accumulated Courant number arrays
    ...
    real(FVPRC), allocatable, dimension(:,:,:):: pem, heat_source
! Auto 1D & 2D arrays:
...
1000 continue
 ...
 subroutine geopk(ptop, pe, peln, delp, pk, gz, hs, pt, q_con, pkz, km, akap, CG, nested, computehalo, npx, npy, a2b_ord, bd)
   ...
   real(FVPRC), intent(IN), dimension(bd%isd:,bd%jsd:,1:):: q_con
