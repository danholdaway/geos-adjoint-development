...
#endif
 real(FVPRC), parameter::  big_number=1.d8
 real(FVPRC), parameter:: tiny_number=1.d-8

 real(kind=R_GRID), parameter :: pi = 3.14159265358979323846d0
 real(kind=R_GRID) :: radius=6371.d0 !km

 real(FVPRC), parameter:: ptop_min=1.d-8
...
!---- version number -----
 character(len=128) :: version = '$Id: fv_grid_utils.F90,v 1.1.2.1.2.1.30.1.4.1.100.2.4.1.2.3.2.1.4.1 2017/02/16 03:47:47 aoloso Exp $'
...
!     6---2---7
      ...
      real(FVPRC), pointer, dimension(:,:) :: divg_u, divg_v
      real(FVPRC), pointer, dimension(:,:) :: cosa_u, cosa_v, cosa_s
      real(FVPRC), pointer, dimension(:,:) :: sina_u, sina_v
      real(FVPRC), pointer, dimension(:,:) :: rsin_u, rsin_v
      real(FVPRC), pointer, dimension(:,:) :: rsina, rsin2
...
! bma commented out  cos_sg(i,j,5) = inner_prod( ec1(1:3,i,j), ec2(1:3,i,j) )
         enddo
      enddo
...
!rab      deallocate ( Atm%gridstruct%area_64 )
      ...
      deallocate ( Atm%gridstruct%dx_64 )
      deallocate ( Atm%gridstruct%dy_64 )
...
!
  ...
  real(FVPRC) function inner_prod(v1, v2)
...
!-------------------------------------
 ...
 subroutine intersect_cross(a1,a2,b1,b2,radius,x_inter,local_a,local_b)
  ...
  !Local pointers
  real(FVPRC), pointer, dimension(:,:) :: a11, a12, a21, a22
  real(FVPRC), pointer, dimension(:,:) :: z11, z12, z21, z22
  ...
  if ( grid_type < 4 ) then
     ...
     do j=js-1,je+1
        do i=is-1,ie+1
           z11(i,j) =  inner_prod(ec1(1:3,i,j), vlon(i,j,1:3))    ! e1*e_lon
           z12(i,j) =  inner_prod(ec1(1:3,i,j), vlat(i,j,1:3))    ! e1*e_lat
           z21(i,j) =  inner_prod(ec2(1:3,i,j), vlon(i,j,1:3))    ! e2*e_lon
           z22(i,j) =  inner_prod(ec2(1:3,i,j), vlat(i,j,1:3))    ! e2*e_lat
!---------------------------------------------------------------------------
#ifdef ORIG_ARRAY
           a11(i,j) =  0.5d0*z22(i,j) / sina_s(i,j)
           a12(i,j) = -0.5d0*z12(i,j) / sina_s(i,j)
           a21(i,j) = -0.5d0*z21(i,j) / sina_s(i,j)
           a22(i,j) =  0.5d0*z11(i,j) / sina_s(i,j)
#else
...
!-------------------------
 ...
 real(FVPRC) function global_qsum(p, ifirst, ilast, jfirst, jlast)
...
! For computing global max/min at cell Corners
 ...
 type(fv_grid_bounds_type), intent(IN) :: bd
  ...
  real(FVPRC), intent(inout):: pe(bd%is-1:bd%ie+1,km+1,bd%js-1:bd%je+1)
  real(FVPRC), intent(out):: ak(km+1), bk(km+1)
  type(domain2d), intent(IN) :: domain
! local:
  real(FVPRC) ph(km+1)
  real(FVPRC), allocatable:: pem(:,:)
  real(kind=4) :: p4
  integer k, i, j
  integer :: is,  ie,  js,  je
...
! local:
    ...
    real(FVPRC), pointer, dimension(:,:,:) :: en1, en2, agrid, vlon, vlat
