...
!
      ...
      real(FVPRC)  , allocatable :: grid(:,:,:,:)
      real(FVPRC)  , allocatable :: area(:,:,:)
      real(FVPRC)  , allocatable :: dx(:,:,:)
      real(FVPRC)  , allocatable :: dy(:,:,:)
...
!
      ...
      implicit      none
         ...
         real(FVPRC)  , intent(INOUT) :: grid(nsx, nsy, 3)
         integer, intent(IN) :: region

         real(FVPRC)   :: alpha(nsx)
         real(FVPRC)   :: beta(nsy)
         real(FVPRC)   :: capX, capY, capZ
         integer :: i,j
...
#include "netcdf.inc"
         ...
         real(FVPRC)   , intent(INOUT) :: grid_global(1-ng:npx  +ng,1-ng:npy  +ng,ndims,1:nregions)
         real(FVPRC)   , intent(INOUT) :: area_global(1:npx-1,1:npy-1,1:nregions)
         real(FVPRC)   , intent(INOUT) ::   dx_global(1:npx-1,1:npy  ,1:nregions)
         real(FVPRC)   , intent(INOUT) ::   dy_global(1:npx  ,1:npy-1,1:nregions)
         ...
         real(FVPRC) :: gg_tmp(1-ng:npx  +ng,1-ng:npy  +ng,ndims,1:nregions)
...
!
      ...
      implicit      none
         ...
         real(FVPRC) , intent(IN)    :: x1in, y1in, z1in
         real(FVPRC) , intent(IN)    :: angle        ! angle to rotate in radians
         real(FVPRC) , intent(OUT)   :: x2out, y2out, z2out
         integer, intent(IN), optional :: convert ! if present convert input point
                                                  ! from spherical to cartesian, rotate, 
                                                  ! and convert back
                     
         real(FVPRC)  :: c, s
         real(FVPRC)  :: x1,y1,z1, x2,y2,z2
...
!      
      ...
      implicit      none
         ...
         real(FVPRC) , intent(IN)  :: x, y, z
         real(FVPRC) , intent(OUT) :: lon, lat, r
         real(FVPRC)  :: S
...
! 
      ...
      implicit      none
         ...
         real(FVPRC) , intent(IN)  :: lon, lat, r
         real(FVPRC) , intent(OUT) :: x, y, z
