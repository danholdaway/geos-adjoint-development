...
#  endif WATER_CYCLE
 ...
 use fv_arrays_mod, only: max_step, R_GRID
...
!---- version number -----
 character(len=128) :: version = '$Id: fv_diagnostics.F90,v 1.2.2.1.2.1.30.1.88.1.20.1.2.3.2.1 2017/02/16 03:47:48 aoloso Exp $'
 character(len=128) :: tagname = '$Name: Heracles-UNSTABLE_ncepdyn_Feb222017 $'

 interface prt_maxmin
    module procedure prt_maxmin_r4_2d
    module procedure prt_maxmin_r4_3d
    module procedure prt_maxmin_r8_2d
    module procedure prt_maxmin_r8_3d
 end interface

contains

 subroutine prt_maxmin_r8_3d(qname, q, is, ie, js, je, n_g, km, fac, master_proc)
      character(len=*), intent(in)::  qname
      integer, intent(in):: is, ie, js, je
      integer, intent(in):: n_g, km
      !real(REAL64), intent(in)::    q(is-n_g:ie+n_g, js-n_g:je+n_g, km)
      real(REAL64), intent(in)::    q(is-n_g:, js-n_g:, :)
      real(FVPRC), intent(in)::    fac
      logical, optional, intent(in) :: master_proc

      real(FVPRC) qmin, qmax
      integer i,j,k
      logical :: local_master

      if (present(master_proc)) then
         local_master = master_proc
      else
         local_master = master
      end if
...
!           qmax = max(qmax, q(i,j,k))
 ...
 end subroutine prt_maxmin_r8_3d

 subroutine prt_maxmin_r8_2d(qname, q, is, ie, js, je, n_g, km, fac, master_proc)
      character(len=*), intent(in)::  qname
      integer, intent(in):: is, ie, js, je
      integer, intent(in):: n_g, km
      !real(REAL64), intent(in)::    q(is-n_g:ie+n_g, js-n_g:je+n_g, km)
...
!           qmax = max(qmax, q(i,j,k))
 ...
 end subroutine prt_maxmin_r8_2d

 subroutine prt_maxmin_r4_3d(qname, q, is, ie, js, je, n_g, km, fac, master_proc)
      character(len=*), intent(in)::  qname
      integer, intent(in):: is, ie, js, je
      integer, intent(in):: n_g, km
      !real(REAL32), intent(in)::    q(is-n_g:ie+n_g, js-n_g:je+n_g, km)
      real(REAL32), intent(in)::    q(is-n_g:, js-n_g:, :)
      real(FVPRC), intent(in)::    fac
      logical, optional, intent(in) :: master_proc
...
!           qmax = max(qmax, q(i,j,k))
 ...
 end subroutine prt_maxmin_r4_3d

 subroutine prt_maxmin_r4_2d(qname, q, is, ie, js, je, n_g, km, fac, master_proc)
      character(len=*), intent(in)::  qname
      integer, intent(in):: is, ie, js, je
      integer, intent(in):: n_g, km
      !real(REAL32), intent(in)::    q(is-n_g:ie+n_g, js-n_g:je+n_g, km)
...
!           qmax = max(qmax, q(i,j,k))
 ...
 end subroutine prt_maxmin_r4_2d
 ...
 subroutine fv_diag_init(Atm, axes, Time, npx, npy, npz, p_ref)
    ...
    real(FVPRC), intent(in):: p_ref

    real(FVPRC), allocatable :: grid_xt(:), grid_yt(:), grid_xe(:), grid_ye(:), grid_xn(:), grid_yn(:)
    real(FVPRC), allocatable :: grid_x(:),  grid_y(:)
    real(FVPRC)              :: vrange(2), vsrange(2), wrange(2), trange(2), slprange(2), rhrange(2)
    real(FVPRC), allocatable :: a3(:,:,:)
    real(FVPRC)              :: pfull(npz)
    real(FVPRC)              :: hyam(npz), hybm(npz)
...
!!!  This will need mods for more than 1 tile per pe  !!!
   ...
   rhrange = (/  -10.,  150. /)  ! RH
    ...
    call get_eta_level(Atm(1)%npz, p_ref, pfull, idiag%phalf, Atm(1)%ak, Atm(1)%bk, 0.01_FVPRC)
...
! local:
    real(FVPRC) zs(is-ng:ie+ng, js-ng:je+ng)
    real(FVPRC) zb(is-ng:ie+ng, js-ng:je+ng)
    real(FVPRC) pdx(3,is:ie,js:je+1)
    real(FVPRC) pdy(3,is:ie+1,js:je)
    integer i, j, n

    real(FVPRC), pointer :: rarea(:,:)
    real(FVPRC), pointer, dimension(:,:) :: dx, dy
...
!                                                       ^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 subroutine fv_diag(Atm, zvir, Time, print_freq)
    ...
    real(FVPRC), allocatable :: a2(:,:),a3(:,:,:), wk(:,:,:), wz(:,:,:), ucoor(:,:,:), vcoor(:,:,:)
    real(FVPRC), allocatable :: slp(:,:), depress(:,:), ws_max(:,:), tc_count(:,:)
    real(FVPRC), allocatable :: u2(:,:), v2(:,:), x850(:,:)
    real(FVPRC), allocatable :: dmmr(:,:,:), dvmr(:,:,:)
    real(FVPRC) height(2)
    real(FVPRC) plevs(10)
    real(FVPRC)    :: tot_mq, tmp, sar, slon, slat
    logical :: used
    logical :: bad_range
    integer i,j,k, yr, mon, dd, hr, mn, days, seconds
    character(len=128)   :: tname
    real(FVPRC), parameter:: ws_0 = 16.   ! minimum max_wind_speed within the 7x7 search box
    real(FVPRC), parameter:: ws_1 = 20.
    real(FVPRC), parameter:: vort_c0= 2.2e-5 
    logical, allocatable :: storm(:,:), cat_crt(:,:)

#ifdef MARS_GCM
    real(FVPRC)  ::   atm_mass,  sfc_mass, atm_cloud
    real(FVPRC)  ::   tsfc_dust, tcol_dust
#endif
...
! cat45: SLP< 944; srf_wnd>ws_1; vort>vort_c0
    ...
    if( prt_minmax ) then
        ...
        call prt_mxm('ZS', idiag%zsurf,     isc, iec, jsc, jec, 0,   1, 1.0_FVPRC, Atm(n)%gridstruct%area_64, Atm(n)%domain)
        call prt_maxmin('PS', Atm(n)%ps, isc, iec, jsc, jec, ngc, 1, 0.01_FVPRC)
...
#endif
        call prt_maxmin('UA', Atm(n)%ua, isc, iec, jsc, jec, ngc, npz, 1._FVPRC)
        call prt_maxmin('VA', Atm(n)%va, isc, iec, jsc, jec, ngc, npz, 1._FVPRC)

        if ( .not. Atm(n)%flagstruct%hydrostatic ) then
          call prt_maxmin('W ', Atm(n)%w , isc, iec, jsc, jec, ngc, npz, 1._FVPRC)
          if ( Atm(n)%flagstruct%hybrid_z ) call prt_maxmin('Hybrid_ZTOP (km)', Atm(n)%ze0(isc:iec,jsc:jec,1), &
                                                 isc, iec, jsc, jec, 0, 1, 1.E-3_FVPRC)
          call prt_maxmin('DZ (m)', Atm(n)%delz(isc:iec,jsc:jec,1:npz),    &
                          isc, iec, jsc, jec, 0, npz, 1._FVPRC)
          call prt_maxmin('Bottom DZ (m)', Atm(n)%delz(isc:iec,jsc:jec,npz),    &
                          isc, iec, jsc, jec, 0, 1, 1._FVPRC)
!         call prt_maxmin('Top DZ (m)', Atm(n)%delz(isc:iec,jsc:jec,1),    &
!                         isc, iec, jsc, jec, 0, 1, 1._FVPRC)
        endif

#ifndef SW_DYNAMICS
        call prt_maxmin('TA', Atm(n)%pt,   isc, iec, jsc, jec, ngc, npz, 1._FVPRC)
        call prt_maxmin('OM', Atm(n)%omga, isc, iec, jsc, jec, ngc, npz, 1._FVPRC)
#endif
    ...
    elseif ( Atm(n)%flagstruct%range_warn ) then
         ...
         call range_check('TA', Atm(n)%pt, isc, iec, jsc, jec, ngc, npz, Atm(n)%gridstruct%agrid,   &
                           150._FVPRC, 350._FVPRC, bad_range)
#endif
          ...
          if ( idiag%id_srh > 0 ) then
             ...
             if(prt_minmax) then
                do j=jsc,jec
                   ...
                   enddo
                enddo
                call prt_maxmin('SRH over CONUS', a2, isc, iec, jsc, jec, 0,   1, 1._FVPRC)
             endif
          endif
...
! Compute FV mean pressure
               ...
               if(prt_minmax) then
                  call prt_maxmin('RH_sf (%)', wk(isc:iec,jsc:jec,npz), isc, iec, jsc, jec, 0,   1, 1._FVPRC)
                  call prt_maxmin('RH_3D (%)', wk, isc, iec, jsc, jec, 0, npz, 1._FVPRC)
               endif
          endif
       ...
       if (idiag%id_rh50>0  .or. idiag%id_rh100>0 .or. idiag%id_rh200>0 .or. idiag%id_rh250>0 .or. &
           ...
           if (idiag%id_rh50>0) then
               call interpolate_vertical(isc, iec, jsc, jec, npz, 50.e2_FVPRC, Atm(n)%peln, wk(isc:iec,jsc:jec,:), a2)
               used=send_data(idiag%id_rh50, a2, Time)
           endif
           if (idiag%id_rh100>0) then
               call interpolate_vertical(isc, iec, jsc, jec, npz, 100.e2_FVPRC, Atm(n)%peln, wk(isc:iec,jsc:jec,:), a2)
               used=send_data(idiag%id_rh100, a2, Time)
           endif
           if (idiag%id_rh200>0) then
               call interpolate_vertical(isc, iec, jsc, jec, npz, 200.e2_FVPRC, Atm(n)%peln, wk(isc:iec,jsc:jec,:), a2)
               used=send_data(idiag%id_rh200, a2, Time)
           endif
           if (idiag%id_rh250>0) then
               call interpolate_vertical(isc, iec, jsc, jec, npz, 250.e2_FVPRC, Atm(n)%peln, wk(isc:iec,jsc:jec,:), a2)
               used=send_data(idiag%id_rh250, a2, Time)
           endif
           if (idiag%id_rh500>0) then
               call interpolate_vertical(isc, iec, jsc, jec, npz, 500.e2_FVPRC, Atm(n)%peln, wk(isc:iec,jsc:jec,:), a2)
               used=send_data(idiag%id_rh500, a2, Time)
           endif
           if (idiag%id_rh700>0) then
               call interpolate_vertical(isc, iec, jsc, jec, npz, 700.e2_FVPRC, Atm(n)%peln, wk(isc:iec,jsc:jec,:), a2)
               used=send_data(idiag%id_rh700, a2, Time)
           endif
           if (idiag%id_rh850>0) then
               call interpolate_vertical(isc, iec, jsc, jec, npz, 850.e2_FVPRC, Atm(n)%peln, wk(isc:iec,jsc:jec,:), a2)
               used=send_data(idiag%id_rh850, a2, Time)
           endif
           if (idiag%id_rh1000>0) then
               call interpolate_vertical(isc, iec, jsc, jec, npz, 1000.e2_FVPRC, Atm(n)%peln, wk(isc:iec,jsc:jec,:), a2)
       ...
       if (idiag%id_rh10_cmip>0 .or. idiag%id_rh50_cmip>0  .or. idiag%id_rh100_cmip>0 .or. &
           ...
           if (idiag%id_rh10_cmip>0) then
               call interpolate_vertical(isc, iec, jsc, jec, npz, 10.e2_FVPRC, Atm(n)%peln, wk(isc:iec,jsc:jec,:), a2)
       ...
       if( idiag%id_slp>0 .or. idiag%id_tm>0 .or. idiag%id_hght>0 .or. idiag%id_c15>0 ) then
          ...
          if( prt_minmax )   &
          call prt_maxmin('ZTOP', wz(isc:iec,jsc:jec,1), isc, iec, jsc, jec, 0, 1, 1.E-3_FVPRC)
...
! Cumpute SLP (pressure at height=0)
          ...
          call get_pressure_given_height(isc, iec, jsc, jec, ngc, npz, wz, 1, height(2),   &
                                        Atm(n)%pt(:,:,npz), Atm(n)%peln, slp, 0.01_FVPRC)
          used = send_data (idiag%id_slp, slp, Time)
          if( prt_minmax )   &
             call prt_maxmin('SLP', slp, isc, iec, jsc, jec, 0, 1, 1._FVPRC)
          endif
...
! Compute H3000 and/or H500
          if( idiag%id_tm>0 .or. idiag%id_hght>0 .or. idiag%id_ppt>0) then
             ...
             if(idiag%id_h100>0) then
                ...
                if(prt_minmax)   &
                call prt_mxm('Z100',a3(isc:iec,jsc:jec,3),isc,iec,jsc,jec,0,1,1.E-3_FVPRC,Atm(n)%gridstruct%area_64,Atm(n)%domain)
             endif
...
! Western Pac: negative; positive elsewhere
                   ...
                   enddo
                enddo
                call prt_maxmin('Depress', depress, isc, iec, jsc, jec, 0,   1, 1._FVPRC)
                do j=jsc,jec
                   do i=isc,iec
...
! Excluding the SH cyclones
                   ...
                   enddo
                enddo
                call prt_maxmin('NH Deps', depress, isc, iec, jsc, jec, 0,   1, 1._FVPRC)
...
! ATL basin cyclones
                do j=jsc,jec
                   ...
                   enddo
                enddo
                call prt_maxmin('ATL Deps', depress, isc, iec, jsc, jec, 0,   1, 1._FVPRC)
             endif
            endif
...
! Total 3D condensates
       ...
       if(idiag%id_us > 0 .and. idiag%id_vs > 0) then
          ...
          if(prt_minmax) call prt_maxmin('Surf_wind_speed', a2, isc, iec, jsc, jec, 0, 1, 1._FVPRC)
       endif
       ...
       if(idiag%id_tb > 0) then
          ...
          if( prt_minmax )   &
          call prt_mxm('T_bot:', a2, isc, iec, jsc, jec, 0, 1, 1._FVPRC, Atm(n)%gridstruct%area_64, Atm(n)%domain)
       endif
...
! 10-mb
       if ( idiag%id_u10>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      10.e2_FVPRC, Atm(n)%peln, Atm(n)%ua(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_u10, a2, Time)
       endif
       if ( idiag%id_v10>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      10.e2_FVPRC, Atm(n)%peln, Atm(n)%va(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_v10, a2, Time)
       endif
       if ( idiag%id_t10>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      10.e2_FVPRC, Atm(n)%peln, Atm(n)%pt(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_t10, a2, Time)
       endif
       if ( idiag%id_q10>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      10.e2_FVPRC, Atm(n)%peln, Atm(n)%q(isc:iec,jsc:jec,:,sphum), a2)
            used=send_data(idiag%id_q10, a2, Time)
       endif
       if ( idiag%id_omg10>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      10.e2_FVPRC, Atm(n)%peln, Atm(n)%omga(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_omg10, a2, Time)
       endif
! 50-mb
       if ( idiag%id_u50>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      50.e2_FVPRC, Atm(n)%peln, Atm(n)%ua(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_u50, a2, Time)
       endif
       if ( idiag%id_v50>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      50.e2_FVPRC, Atm(n)%peln, Atm(n)%va(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_v50, a2, Time)
       endif
       if ( idiag%id_t50>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      50.e2_FVPRC, Atm(n)%peln, Atm(n)%pt(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_t50, a2, Time)
       endif
       if ( idiag%id_q50>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      50.e2_FVPRC, Atm(n)%peln, Atm(n)%q(isc:iec,jsc:jec,:,sphum), a2)
            used=send_data(idiag%id_q50, a2, Time)
       endif
       if ( idiag%id_omg50>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      50.e2_FVPRC, Atm(n)%peln, Atm(n)%omga(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_omg50, a2, Time)
       endif
! 100-mb
       if ( idiag%id_u100>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      100.e2_FVPRC, Atm(n)%peln, Atm(n)%ua(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_u100, a2, Time)
       endif
       if ( idiag%id_v100>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      100.e2_FVPRC, Atm(n)%peln, Atm(n)%va(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_v100, a2, Time)
       endif
       if ( idiag%id_t100>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      100.e2_FVPRC, Atm(n)%peln, Atm(n)%pt(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_t100, a2, Time)
            if( prt_minmax ) then
            call prt_mxm('T100:', a2, isc, iec, jsc, jec, 0, 1, 1._FVPRC, Atm(n)%gridstruct%area_64, Atm(n)%domain)
#ifdef WPAC
...
! 200-mb
       ...
       if ( idiag%id_w200>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      200.e2_FVPRC, Atm(n)%peln, Atm(n)%w(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_w200, a2, Time)
       endif
       if ( idiag%id_t200>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      200.e2_FVPRC, Atm(n)%peln, Atm(n)%pt(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_t200, a2, Time)
            if( prt_minmax ) then
            call prt_mxm('T200:', a2, isc, iec, jsc, jec, 0, 1, 1._FVPRC, Atm(n)%gridstruct%area_64, Atm(n)%domain)
...
!$OMP parallel do default(none) shared(isc,iec,jsc,jec,wz,npz,Atm,n)
            do j=jsc,jec
               ...
               enddo
            enddo
            call interpolate_z(isc, iec, jsc, jec, npz, 5.e3_FVPRC, wz, Atm(n)%w(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_w5km, a2, Time)
            if(prt_minmax) call prt_maxmin('W5km', a2, isc, iec, jsc, jec, 0, 1, 1._FVPRC)
           deallocate (wz)
       endif
       if ( idiag%id_q850>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      850.e2_FVPRC, Atm(n)%peln, Atm(n)%q(isc:iec,jsc:jec,:,sphum), a2)
            used=send_data(idiag%id_q850, a2, Time)
       endif
       if ( idiag%id_omg850>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      850.e2_FVPRC, Atm(n)%peln, Atm(n)%omga(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_omg850, a2, Time)
       endif
! 1000-mb
       if ( idiag%id_u1000>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      1000.e2_FVPRC, Atm(n)%peln, Atm(n)%ua(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_u1000, a2, Time)
       endif
       if ( idiag%id_v1000>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      1000.e2_FVPRC, Atm(n)%peln, Atm(n)%va(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_v1000, a2, Time)
       endif
       if ( idiag%id_t1000>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      1000.e2_FVPRC, Atm(n)%peln, Atm(n)%pt(isc:iec,jsc:jec,:), a2)
            used=send_data(idiag%id_t1000, a2, Time)
       endif
       if ( idiag%id_q1000>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      1000.e2_FVPRC, Atm(n)%peln, Atm(n)%q(isc:iec,jsc:jec,:,sphum), a2)
            used=send_data(idiag%id_q1000, a2, Time)
       endif
       if ( idiag%id_omg1000>0 ) then
            call interpolate_vertical(isc, iec, jsc, jec, npz,   &
                                      1000.e2_FVPRC, Atm(n)%peln, Atm(n)%omga(isc:iec,jsc:jec,:), a2)
...
!               wk(i,j,k) =  (Atm(n)%pt(i,j,k)-300.)/Atm(n)%pkz(i,j,k) * pk0
          ...
          if( prt_minmax ) then
              call prt_maxmin('PoTemp', wk, isc, iec, jsc, jec, 0, npz, 1._FVPRC)
          endif
        ...
        do itrac=1, Atm(n)%ncnst
          ...
          if( prt_minmax ) then
              ...
              call prt_maxmin(trim(tname), Atm(n)%q(:,:,1,itrac), &
                              isc, iec, jsc, jec, ngc, npz, 1._FVPRC)
          endif
!-------------------------------
...
! Note: There is a check in fv_pack.F90 that ensures that tracer number one is sphum
 ...
 real(FVPRC), intent(in), dimension(isc:iec,jsc:jec):: us, vs
 real(FVPRC), intent(out) :: ws_max(isc:iec,jsc:jec)
 type(domain2d), intent(INOUT) :: domain
! Local
 real(FVPRC) :: wx(isc:iec,jsd:jed), ws(isd:ied,jsd:jed)
 integer:: i,j
 ...
 integer isc, iec, jsc, jec
 real(FVPRC), intent(in)  :: u(isd:ied, jsd:jed+1, npz), v(isd:ied+1, jsd:jed, npz)
 real(FVPRC), intent(out) :: vort(isc:iec, jsc:jec, npz)
 real(FVPRC), intent(IN) :: dx(isd:ied,jsd:jed+1)
 real(FVPRC), intent(IN) :: dy(isd:ied+1,jsd:jed)
 real(FVPRC), intent(IN) :: rarea(isd:ied,jsd:jed)
! Local
 real(FVPRC) :: utmp(isc:iec, jsc:jec+1), vtmp(isc:iec+1, jsc:jec)
 integer :: i,j,k
 ...
 subroutine get_height_field(is, ie, js, je, ng, km, wz, pt, q, peln, zvir)
  ...
  real(FVPRC), intent(in):: pt(is-ng:ie+ng,js-ng:je+ng,km)
  real(FVPRC), intent(in)::  q(is-ng:ie+ng,js-ng:je+ng,km,*) ! water vapor
  real(FVPRC), intent(in):: zvir
  real(FVPRC), intent(out):: wz(is:ie,js:je,km+1)
!
  integer i,j,k
  real(FVPRC) gg
 ...
 subroutine range_check(qname, q, is, ie, js, je, n_g, km, pos, q_low, q_hi, bad_range)
      ...
      real(FVPRC), intent(in):: pos(is-n_g:ie+n_g, js-n_g:je+n_g,2)
      real(FVPRC), intent(in):: q_low, q_hi
...
!     gmean = g_sum(domain, q(is,js,km), is, ie, js, je, 3, area, 1) 
 ...
 real(FVPRC), intent(in)::   ps(is-n_g:ie+n_g, js-n_g:je+n_g)
 real(FVPRC), intent(in):: delp(is-n_g:ie+n_g, js-n_g:je+n_g, km)
 real(FVPRC), intent(in)::  q(is-n_g:ie+n_g, js-n_g:je+n_g, km, nq)
 real(kind=R_GRID), intent(IN):: area(is-n_g:ie+n_g,js-n_g:je+n_g)
 type(domain2d), intent(INOUT) :: domain
! Local:
 real(FVPRC) psq(is:ie,js:je,nwat), psqv(is:ie,js:je)
 real(FVPRC) q_strat(is:ie,js:je)
 real(FVPRC) qtot(nwat), qwat
 real(FVPRC) psmo, totw, psdry
 integer k, n, kstrat
...
!-------------------
 ...
 real(FVPRC), intent(in):: delp(is-n_g:ie+n_g, js-n_g:je+n_g, km)
 real(FVPRC), intent(in)::    q(is-n_g:ie+n_g, js-n_g:je+n_g, km)
 real(FVPRC), intent(out):: sum2(is:ie,js:je)
 type(domain2d), intent(INOUT) :: domain
 ...
 real(FVPRC) function p_sum(is, ie, js, je, km, n_g, delp, area, domain)
 integer, intent(in):: is, ie, js, je,  n_g, km
 real(FVPRC), intent(in):: delp(is-n_g:ie+n_g, js-n_g:je+n_g, km)
 real(kind=R_GRID), intent(IN) :: area(is-n_g:ie+n_g, js-n_g:je+n_g)
 real(FVPRC) :: sum2(is:ie,js:je)
 integer i,j,k
 type(domain2d), intent(INOUT) :: domain
...
!$OMP parallel do default(none) shared(is,ie,js,je,km,sum2,delp)
 ...
 real(FVPRC), intent(in):: wz(is:ie,js:je,km+1)
 real(FVPRC), intent(in):: ts(is-ng:ie+ng,js-ng:je+ng)
 real(FVPRC), intent(in):: peln(is:ie,km+1,js:je)
 real(FVPRC), intent(in):: height(kd)   ! must be monotonically decreasing with increasing k
 real(FVPRC), intent(out):: a2(is:ie,js:je,kd)      ! pressure (pa)
 real(FVPRC), optional, intent(in):: fac
...
1000   continue
 ...
 integer,  intent(in):: kd           ! vertical dimension of the ouput height
 real(FVPRC), intent(in):: log_p(kd)    ! must be monotonically decreasing with increasing k
                                 ! log (p)
 real(FVPRC), intent(in):: wz(is:ie,js:je,km+1)
 real(FVPRC), intent(in):: peln(is:ie,km+1,js:je)
 real(FVPRC), intent(out):: a2(is:ie,js:je,kd)      ! height (m)
...
! !INPUT PARAMETERS:
   ...
   real(FVPRC), intent(in):: grav, zvir
   real(FVPRC), intent(in), dimension(is-ng:ie+ng,js-ng:je+ng,km):: pt, ua, va
   real(FVPRC), intent(in):: delz(is-ng:ie+ng,js-ng:je+ng,km)
   real(FVPRC), intent(in):: q(is-ng:ie+ng,js-ng:je+ng,km,*)
   real(FVPRC), intent(in):: phis(is-ng:ie+ng,js-ng:je+ng)
   real(FVPRC), intent(in):: peln(is:ie,km+1,js:je) 
   logical, intent(in):: hydrostatic
   real(FVPRC), intent(out):: srh(is:ie,js:je)   ! unit: (m/s)**2
   real(FVPRC), parameter:: z_crit = 3.e3   ! lowest 3-km
!---------------------------------------------------------------------------------
...
!
   real(FVPRC):: rdg
   real(FVPRC), dimension(is:ie):: zh, uc, vc, dz
   integer i, j, k, k0
...
! !INPUT PARAMETERS:
   integer, intent(in)::  is, ie, js, je, ng, km
   real(FVPRC), intent(in):: grav
   real(FVPRC), intent(in):: pt(is-ng:ie+ng,js-ng:je+ng,km) 
   real(FVPRC), intent(in):: pkz(is:ie,js:je,km) 
   real(FVPRC), intent(in):: delp(is-ng:ie+ng,js-ng:je+ng,km)
   real(FVPRC), intent(in):: f_d(is-ng:ie+ng,js-ng:je+ng) 

! vort is relative vorticity as input. Becomes PV on output
      real(FVPRC), intent(inout):: vort(is:ie,js:je,km)
...
!---------------------------------------------------------------------
!BOC
      real(FVPRC) w3d(is:ie,js:je,km)
      real(FVPRC) te(is:ie,js:je,km+1), t2(is:ie,km), delp2(is:ie,km)
      real(FVPRC) te2(is:ie,km+1)
      integer i, j, k
...
! Entropy is the thermodynamic variable in the following form
 ...
 subroutine ppme(p,qe,delp,im,km)
  ...
  real(FVPRC), intent(in)::    p(im,km)
  real(FVPRC), intent(in):: delp(im,km)
  real(FVPRC), intent(out)::qe(im,km+1)

! local arrays.
      real(FVPRC) dc(im,km),delq(im,km), a6(im,km)
      real(FVPRC) c1, c2, c3, tmp, qmax, qmin
      real(FVPRC) a1, a2, s1, s2, s3, s4, ss3, s32, s34, s42
      real(FVPRC) a3, b2, sc, dm, d1, d2, f1, f2, f3, f4
      real(FVPRC) qm, dq
      integer i, k, km1
...
subroutine rh_calc (pfull, t, qv, rh, do_cmip)
   ...
   real(FVPRC), intent (in),  dimension(:,:) :: pfull, t, qv
   real(FVPRC), intent (out), dimension(:,:) :: rh
   real(FVPRC), dimension(size(t,1),size(t,2)) :: esat
   logical, intent(in), optional :: do_cmip

   real(FVPRC), parameter :: d622 = rdgas/rvgas
   real(FVPRC), parameter :: d378 = 1.-d622
