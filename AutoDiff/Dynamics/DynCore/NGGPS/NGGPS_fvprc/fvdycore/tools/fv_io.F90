...
!---- version number -----
  character(len=128) :: version = '$Id: fv_io.F90,v 1.2.2.1.2.1.120.1.20.1.2.1.4.1 2017/02/16 03:47:48 aoloso Exp $'
...
! fix for single tile runs where you need fv_core.res.nc and fv_core.res.tile1.nc
    ...
    do nt = 2, ntracers
       ...
       !This is already registered by fv_io_register_restart, which is always called.
...
!
!-------------------------------------------------------------------------
    real(FVPRC), allocatable:: ak_r(:), bk_r(:)
    real(FVPRC), allocatable:: u_r(:,:,:), v_r(:,:,:), pt_r(:,:,:), delp_r(:,:,:)
    real(FVPRC), allocatable:: w_r(:,:,:), delz_r(:,:,:), ze0_r(:,:,:)
    real(FVPRC), allocatable:: q_r(:,:,:,:)
!-------------------------------------------------------------------------
...
!    n = 1
  ...
  subroutine register_bcs_2d(Atm, BCfile_ne, BCfile_sw, fname_ne, fname_sw, &
    ...
    real(FVPRC), dimension(:,:),     intent(in), optional :: var
