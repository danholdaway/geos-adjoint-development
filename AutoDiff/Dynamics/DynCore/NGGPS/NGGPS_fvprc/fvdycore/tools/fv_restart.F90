...
!---- version number -----
  character(len=128) :: version = '$Id: fv_restart.F90,v 1.2.2.1.2.2.30.1.88.1.20.1.2.1.4.1 2017/02/16 03:47:48 aoloso Exp $'
...
contains 
  ...
  subroutine fv_restart(fv_domain, Atm, dt_atmos, seconds, days, cold_start, grid_type, grids_on_this_pe)
    ...
    integer :: unit
    real(FVPRC), allocatable :: dz1(:)
    real(FVPRC) rgrav, f00, ztop, pertn
    logical :: hybrid
...
!---------------------------------------------------------------------------------------------
    if ( Atm(n)%flagstruct%external_ic ) then
         ...
         call get_external_ic(Atm(n:n), Atm(n)%domain, cold_start) 
...
! Transform the (starting) Eulerian vertical coordinate from sigma-p to hybrid_z
     if ( Atm(n)%flagstruct%hybrid_z ) then
       if ( Atm(n)%flagstruct%make_hybrid_z ) then
          ...
          deallocate ( dz1 )
!         call prt_maxmin('ZE0', Atm(n)%ze0,  isc, iec, jsc, jec, 0, npz, 1.E-3_FVPRC)
!         call prt_maxmin('DZ0', Atm(n)%delz, isc, iec, jsc, jec, 0, npz, 1._FVPRC   )
       endif
...
!     call prt_maxmin('ORO',Atm(n)%oro, isc, iec, jsc, jec,          0, 1, 1._FVPRC)


      if ( (.not.Atm(n)%flagstruct%hydrostatic) ) then
!      if ( (.not.Atm(n)%flagstruct%hydrostatic) .and. (.not.Atm(n)%flagstruct%make_nh) ) then
            call prt_maxmin('DZ', Atm(n)%delz, isc, iec, jsc, jec, Atm(n)%ng, npz, 1._FVPRC)
            if ( Atm(n)%flagstruct%hybrid_z ) then
            call prt_maxmin('ZTOP(km)', Atm(n)%ze0, isc, iec, jsc, jec, 0, 1, 1.E-3_FVPRC)
            call prt_maxmin('DZ_top', Atm(n)%delz, isc, iec, jsc, jec, Atm(n)%ng, 1, 1.E-3_FVPRC)
            endif
      endif

      call prt_maxmin('PS', Atm(n)%ps, isc, iec, jsc, jec, Atm(n)%ng, 1,    0.01_FVPRC)
      call prt_maxmin('T ', Atm(n)%pt, isc, iec, jsc, jec, Atm(n)%ng, npz, 1._FVPRC)

! Check tracers:
      do i=1, ncnst
          call get_tracer_names ( MODEL_ATMOS, i, tname )
          call prt_maxmin(trim(tname), Atm(n)%q(isd:ied,jsd:jed,1:npz,i), isc, iec, jsc, jec, Atm(n)%ng, npz, 1._FVPRC)
      enddo
#endif
      call prt_maxmin('U ', Atm(n)%u(isc:iec,jsc:jec,1:npz), isc, iec, jsc, jec, 0, npz, 1._FVPRC)
      call prt_maxmin('V ', Atm(n)%v(isc:iec,jsc:jec,1:npz), isc, iec, jsc, jec, 0, npz, 1._FVPRC)
      if ( .not.Atm(n)%flagstruct%hydrostatic )   &
      call prt_maxmin('W ', Atm(n)%w, isc, iec, jsc, jec, Atm(n)%ng, npz, 1._FVPRC)
...
!--------------------------------------------
  ...
  subroutine setup_nested_boundary_halo(Atm, proc_in)
    ...
    real(FVPRC), allocatable :: g_dat(:,:,:), g_dat2(:,:,:)
    real(FVPRC), allocatable :: pt_coarse(:,:,:)
       ...
       if (present(setBCs)) then
          if (setBCs) then
             call nested_grid_BC_apply(uc, &
                  ...
                  var_south=neststruct%uc_BC%south_t1, bctype=1, nsponge=0, s_weight=0._FVPRC)
             call nested_grid_BC_apply(vc, &
                  ...
                  var_south=neststruct%vc_BC%south_t1, bctype=1, nsponge=0, s_weight=0._FVPRC)
          end if
       end if
    endif
       ...
       if (Atm%flagstruct%grid_type < 4) then
          if ( .not. Atm%flagstruct%external_ic ) then
             call init_case(Atm%u,Atm%v,Atm%w,Atm%pt,Atm%delp, &
                            ...
                            Atm%ks, Atm%neststruct%npx_global, Atm%ptop, Atm%domain, &
                            Atm%tile, Atm%bd)
          endif
...
!---------------
      ...
      call prt_maxmin('PS', Atm(n)%ps, isc, iec, jsc, jec, Atm(n)%ng, 1, 0.01_FVPRC)
      call prt_maxmin('U ', Atm(n)%u(isd:ied,jsd:jed,1:npz), isc, iec, jsc, jec, Atm(n)%ng, npz, 1._FVPRC)
      call prt_maxmin('V ', Atm(n)%v(isd:ied,jsd:jed,1:npz), isc, iec, jsc, jec, Atm(n)%ng, npz, 1._FVPRC)
      if ( .not. Atm(n)%flagstruct%hydrostatic )    &
      call prt_maxmin('W ', Atm(n)%w , isc, iec, jsc, jec, Atm(n)%ng, npz, 1._FVPRC)
      call prt_maxmin('T ', Atm(n)%pt, isc, iec, jsc, jec, Atm(n)%ng, npz, 1._FVPRC)
...
!     if (use_ncep_sst .or. Atm(1)%flagstruct%nudge) call fv_io_register_nudge_restart(Atm)
 ...
 subroutine d2c_setup(u, v, uc, vc, dord4, &
  ...
  real(FVPRC), intent(in) ::  u(isd:ied,jsd:jed+1)
  real(FVPRC), intent(in) ::  v(isd:ied+1,jsd:jed)
  ...
  real(FVPRC), intent(in) :: rsin_u(isd:ied+1,jsd:jed)
  real(FVPRC), intent(in) :: rsin_v(isd:ied,jsd:jed+1)
  real(FVPRC), intent(in) :: cosa_s(isd:ied,jsd:jed)
  real(FVPRC), intent(in) :: rsin2(isd:ied,jsd:jed)
...
! 4th order interpolation:
 ...
 subroutine d2a_setup(u, v, ua, va, dord4, &
  ...
  real(FVPRC), intent(in) ::  u(isd:ied,jsd:jed+1)
  real(FVPRC), intent(in) ::  v(isd:ied+1,jsd:jed)
  real(FVPRC), intent(out), dimension(isd:ied  ,jsd:jed  ):: ua
  real(FVPRC), intent(out), dimension(isd:ied  ,jsd:jed  ):: va
  integer, intent(in) :: isd,ied,jsd,jed, is,ie,js,je, npx,npy,grid_type
  real(FVPRC), intent(in) :: cosa_s(isd:ied,jsd:jed)
  real(FVPRC), intent(in) :: rsin2(isd:ied,jsd:jed)
