...
#include <netcdf.inc>
  ...
  public :: rotate_winds,globalsum,mirror_grid, get_unit_vector

  INTERFACE rotate_winds
     MODULE PROCEDURE rotate_winds_r4
     MODULE PROCEDURE rotate_winds_r8
  END INTERFACE
  ...
  !INTERFACE ctoa
     !MODULE PROCEDURE ctoa_r4
     !MODULE PROCEDURE ctoa_r8
     !MODULE PROCEDURE ctoa_3d_r4
     !MODULE PROCEDURE ctoa_3d_r8
  !END INTERFACE

  !INTERFACE dtoa
     !MODULE PROCEDURE dtoa_r4
     !MODULE PROCEDURE dtoa_r8
     !MODULE PROCEDURE dtoa_3d_r4
     !MODULE PROCEDURE dtoa_3d_r8
  !END INTERFACE

  !INTERFACE atod
     !MODULE PROCEDURE atod_r4
     !MODULE PROCEDURE atod_r8
     !MODULE PROCEDURE atod_3d_r4
     !MODULE PROCEDURE atod_3d_r8
  !END INTERFACE

  !INTERFACE atoc
     !MODULE PROCEDURE atoc_r4
     !MODULE PROCEDURE atoc_r8
     !MODULE PROCEDURE atoc_3d_r4
     !MODULE PROCEDURE atoc_3d_r8
  !END INTERFACE

  !---- version number -----
  character(len=128) :: version = '$Id: fv_grid_tools.F90,v 1.1.2.1.2.1.2.1.10.1.12.1.4.3.4.5.2.4.2.5.2.1.2.1.2.1 2017/02/16 03:47:48 aoloso Exp $'
...
contains
  ...
  subroutine read_grid(Atm, grid_file, ndims, nregions, ng)
    ...
    real(FVPRC), allocatable, dimension(:,:)  :: tmpx, tmpy
...
!    real(kind=R_GRID) :: grid_global(1-ng:npx  +ng,1-ng:npy  +ng,ndims,1:nregions)
    ...
    real(FVPRC), pointer, dimension(:,:,:) :: e1, e2

    real(FVPRC), pointer, dimension(:,:) :: rarea, rarea_c
    real(FVPRC), pointer, dimension(:,:) :: rdx, rdy, rdxc, rdyc, rdxa, rdya
                   ...
                   do n=1,nregions
                      do j=1,npy
                         do i=1,npx
                            ...
                            if (ABS(grid_global(i,j,1,1)) < 1.d-10) grid_global(i,j,1,1) = 0.0d0
                            if (ABS(grid_global(i,j,2,1)) < 1.d-10) grid_global(i,j,2,1) = 0.0d0
                         enddo
                      enddo
                   enddo
                else
...
! The following is a hack to get pass the am2 phys init:
  ...
  ! Setup an f-plane at LON=deglon LAT=deglat with slight dx/dy so MAPL/Physics is happy
    ...
    subroutine setup_aligned_nest(Atm)
      ...
      real(FVPRC),    pointer, dimension(:,:,:) :: wt_h, wt_u, wt_v
...
!!
      !subroutine atod_r4(uin, vin, uout, vout, npx, npy, ng)


         !integer,      intent(IN) :: npx, npy, ng
         !real*4, intent(IN)    ::  uin(isd:ied  ,jsd:jed  ) ! A-grid u-wind field
         !real*4, intent(IN)    ::  vin(isd:ied  ,jsd:jed  ) ! A-grid v-wind field
         !real*4, intent(OUT)   :: uout(isd:ied  ,jsd:jed+1) ! D-grid u-wind field
         !real*4, intent(OUT)   :: vout(isd:ied+1,jsd:jed  ) ! D-grid v-wind field
         ...
         !do i=isd+1,ied
            ...
            !uout(i,:) = tmp1j(:)/dyc(i,:)
         !enddo
         !call mpp_update_domains( uout, vout, domain, gridtype=DGRID_NE, complete=.true.)
         !call fill_corners(uout, vout, npx, npy, VECTOR=.true., DGRID=.true.)
      !end subroutine atod_r4

!!     atod :: interpolate from the A-Grid to the D-grid
!!
      !subroutine atod_3d_r8(uin, vin, uout, vout, npx, npy, npz, ng)
         !integer,      intent(IN) :: npx, npy, npz, ng
         !real*8, intent(IN)    ::  uin(isd:ied  ,jsd:jed  , npz) ! A-grid u-wind field
         !real*8, intent(IN)    ::  vin(isd:ied  ,jsd:jed  , npz) ! A-grid v-wind field
         !real*8, intent(OUT)   :: uout(isd:ied  ,jsd:jed+1, npz) ! D-grid u-wind field
         !real*8, intent(OUT)   :: vout(isd:ied+1,jsd:jed  , npz) ! D-grid v-wind field
         !integer :: k
         !do k=1,npz
            !call atod(uin(:,:,k), vin(:,:,k), uout(:,:,k), vout(:,:,k), npx, npy, ng)
         !enddo
      !end subroutine atod_3d_r8

!!     atod :: interpolate from the A-Grid to the D-grid
!!
      !subroutine atod_3d_r4(uin, vin, uout, vout, npx, npy, npz, ng)
         !integer,      intent(IN) :: npx, npy, npz, ng
         !real*4, intent(IN)    ::  uin(isd:ied  ,jsd:jed  , npz) ! A-grid u-wind field
         !real*4, intent(IN)    ::  vin(isd:ied  ,jsd:jed  , npz) ! A-grid v-wind field
         !real*4, intent(OUT)   :: uout(isd:ied  ,jsd:jed+1, npz) ! D-grid u-wind field
         !real*4, intent(OUT)   :: vout(isd:ied+1,jsd:jed  , npz) ! D-grid v-wind field
         !integer :: k
         !do k=1,npz
            !call atod(uin(:,:,k), vin(:,:,k), uout(:,:,k), vout(:,:,k), npx, npy, ng)
         !enddo
      !end subroutine atod_3d_r4

      !subroutine atod_r8(uin, vin, uout, vout, npx, npy, ng)


         !integer,      intent(IN) :: npx, npy, ng
         !real*8, intent(IN)    ::  uin(isd:ied  ,jsd:jed  ) ! A-grid u-wind field
         !real*8, intent(IN)    ::  vin(isd:ied  ,jsd:jed  ) ! A-grid v-wind field
         !real*8, intent(OUT)   :: uout(isd:ied  ,jsd:jed+1) ! D-grid u-wind field
         !real*8, intent(OUT)   :: vout(isd:ied+1,jsd:jed  ) ! D-grid v-wind field
         ...
         !call mpp_update_domains( uout, vout, domain, gridtype=DGRID_NE, complete=.true.)
         !call fill_corners(uout, vout, npx, npy, VECTOR=.true., DGRID=.true.)
      !end subroutine atod_r8
!!
!!     dtoa :: interpolate from the D-Grid to the A-grid
!!
      !subroutine dtoa_r4(uin, vin, uout, vout, npx, npy, ng)

         !integer,      intent(IN) :: npx, npy, ng
         !real*4, intent(IN)    ::  uin(isd:ied  ,jsd:jed+1)    ! D-grid u-wind field
         !real*4, intent(IN)    ::  vin(isd:ied+1,jsd:jed  )    ! D-grid v-wind field
         !real*4, intent(OUT)   :: uout(isd:ied  ,jsd:jed  )    ! A-grid u-wind field
         !real*4, intent(OUT)   :: vout(isd:ied  ,jsd:jed  )    ! A-grid v-wind field
...
!#endif
      ...
      !end subroutine dtoa_r4

!!
!!     dtoa :: interpolate from the D-Grid to the A-grid
!!
      !subroutine dtoa_r8(uin, vin, uout, vout, npx, npy, ng)

         !integer,      intent(IN) :: npx, npy, ng
         !real*8, intent(IN)    ::  uin(isd:ied  ,jsd:jed+1)    ! D-grid u-wind field
         !real*8, intent(IN)    ::  vin(isd:ied+1,jsd:jed  )    ! D-grid v-wind field
         !real*8, intent(OUT)   :: uout(isd:ied  ,jsd:jed  )    ! A-grid u-wind field
         !real*8, intent(OUT)   :: vout(isd:ied  ,jsd:jed  )    ! A-grid v-wind field
...
!#endif
      ...
      !end subroutine dtoa_r8

!!
      !subroutine dtoa_3d_r4(uin, vin, uout, vout, npx, npy, npz, ng)
         !integer,      intent(IN) :: npx, npy, npz, ng
         !real*4, intent(IN)    ::  uin(isd:ied  ,jsd:jed+1, npz)    ! D-grid u-wind field
         !real*4, intent(IN)    ::  vin(isd:ied+1,jsd:jed  , npz)    ! D-grid v-wind field
         !real*4, intent(OUT)   :: uout(isd:ied  ,jsd:jed  , npz)    ! A-grid u-wind field
         !real*4, intent(OUT)   :: vout(isd:ied  ,jsd:jed  , npz)    ! A-grid v-wind field
         !integer :: k
         !do k=1,npz
            !call dtoa(uin(:,:,k), vin(:,:,k), uout(:,:,k), vout(:,:,k), npx, npy, ng)
         !enddo
      !end subroutine dtoa_3d_r4
!!
!! ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ !
!!-------------------------------------------------------------------------------
...
!!     dtoa :: interpolate from the D-Grid to the A-grid
!!
      !subroutine dtoa_3d_r8(uin, vin, uout, vout, npx, npy, npz, ng)
         !integer,      intent(IN) :: npx, npy, npz, ng
         !real*8, intent(IN)    ::  uin(isd:ied  ,jsd:jed+1, npz)    ! D-grid u-wind field
         !real*8, intent(IN)    ::  vin(isd:ied+1,jsd:jed  , npz)    ! D-grid v-wind field
         !real*8, intent(OUT)   :: uout(isd:ied  ,jsd:jed  , npz)    ! A-grid u-wind field
         !real*8, intent(OUT)   :: vout(isd:ied  ,jsd:jed  , npz)    ! A-grid v-wind field
         !integer :: k
         !do k=1,npz
            !call dtoa(uin(:,:,k), vin(:,:,k), uout(:,:,k), vout(:,:,k), npx, npy, ng)
         !enddo
      !end subroutine dtoa_3d_r8

!!     atoc :: interpolate from the A-Grid to the C-grid
!!
      !subroutine atoc_r4(uin, vin, uout, vout, npx, npy, ng, noComm)


         !integer,      intent(IN) :: npx, npy, ng
         !real*4, intent(IN)    ::  uin(isd:ied  ,jsd:jed  ) ! A-grid u-wind field
         !real*4, intent(IN)    ::  vin(isd:ied  ,jsd:jed  ) ! A-grid v-wind field
         !real*4, intent(OUT)   :: uout(isd:ied+1,jsd:jed  ) ! C-grid u-wind field
         !real*4, intent(OUT)   :: vout(isd:ied  ,jsd:jed+1) ! C-grid v-wind field
...
!#else
         !do j=jsd,jed
            !tmp2i(:) = uin(:,j)
            !call interp_left_edge_1d(tmp1i, tmp2i, dxa(:,j), isd, ied, interpOrder)
            !uout(:,j) = tmp1i(:)
         !enddo
         !do i=isd,ied
!!!$            tmp1j(:) = vout(i,:)
...
!#endif
      ...
      !end subroutine atoc_r4
!!     atoc :: interpolate from the A-Grid to the C-grid
!!
      !subroutine atoc_r8(uin, vin, uout, vout, npx, npy, ng, noComm)

         !integer,      intent(IN) :: npx, npy, ng
         !real*8, intent(IN)    ::  uin(isd:ied  ,jsd:jed  ) ! A-grid u-wind field
         !real*8, intent(IN)    ::  vin(isd:ied  ,jsd:jed  ) ! A-grid v-wind field
         !real*8, intent(OUT)   :: uout(isd:ied+1,jsd:jed  ) ! C-grid u-wind field
         !real*8, intent(OUT)   :: vout(isd:ied  ,jsd:jed+1) ! C-grid v-wind field
         !logical, OPTIONAL, intent(IN)   :: noComm
...
!#endif
      ...
      !end subroutine atoc_r8

!! vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv !
!!
      !subroutine atoc_3d_r8(uin, vin, uout, vout, npx, npy, npz, ng)
         !integer,      intent(IN) :: npx, npy, npz, ng
         !real*8, intent(IN)    ::  uin(isd:ied  ,jsd:jed  , npz) ! A-grid u-wind field
         !real*8, intent(IN)    ::  vin(isd:ied  ,jsd:jed  , npz) ! A-grid v-wind field
         !real*8, intent(OUT)   :: uout(isd:ied+1,jsd:jed  , npz) ! C-grid u-wind field
         !real*8, intent(OUT)   :: vout(isd:ied  ,jsd:jed+1, npz) ! C-grid v-wind field
         !integer :: k
         !do k=1,npz
            !call atoc(uin(:,:,k), vin(:,:,k), uout(:,:,k), vout(:,:,k), npx, npy, ng)
         !enddo
      !end subroutine atoc_3d_r8
!!
!! ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ !
!!-------------------------------------------------------------------------------
...
!! vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv !
!!
      !subroutine atoc_3d_r4(uin, vin, uout, vout, npx, npy, npz, ng)
         !integer,      intent(IN) :: npx, npy, npz, ng
         !real*4, intent(IN)    ::  uin(isd:ied  ,jsd:jed  , npz) ! A-grid u-wind field
         !real*4, intent(IN)    ::  vin(isd:ied  ,jsd:jed  , npz) ! A-grid v-wind field
         !real*4, intent(OUT)   :: uout(isd:ied+1,jsd:jed  , npz) ! C-grid u-wind field
         !real*4, intent(OUT)   :: vout(isd:ied  ,jsd:jed+1, npz) ! C-grid v-wind field
         !integer :: k
         !do k=1,npz
            !call atoc(uin(:,:,k), vin(:,:,k), uout(:,:,k), vout(:,:,k), npx, npy, ng)
         !enddo
      !end subroutine atoc_3d_r4
!! vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv !
!!
!!     ctoa :: interpolate from the C-Grid to the A-grid
!!
      !subroutine ctoa_r4(uin, vin, uout, vout, npx, npy, ng)


         !integer,      intent(IN) :: npx, npy, ng
         !real*4, intent(IN)    ::  uin(isd:ied+1,jsd:jed  )    ! C-grid u-wind field
         !real*4, intent(IN)    ::  vin(isd:ied  ,jsd:jed+1)    ! C-grid v-wind field
         !real*4, intent(OUT)   :: uout(isd:ied  ,jsd:jed  )    ! A-grid u-wind field
         !real*4, intent(OUT)   :: vout(isd:ied  ,jsd:jed  )    ! A-grid v-wind field
      ...
      !end subroutine ctoa_r4

!!     ctoa :: interpolate from the C-Grid to the A-grid
!!
      !subroutine ctoa_r8(uin, vin, uout, vout, npx, npy, ng)


         !integer,      intent(IN) :: npx, npy, ng
         !real*8, intent(IN)    ::  uin(isd:ied+1,jsd:jed  )    ! C-grid u-wind field
         !real*8, intent(IN)    ::  vin(isd:ied  ,jsd:jed+1)    ! C-grid v-wind field
         !real*8, intent(OUT)   :: uout(isd:ied  ,jsd:jed  )    ! A-grid u-wind field
         !real*8, intent(OUT)   :: vout(isd:ied  ,jsd:jed  )    ! A-grid v-wind field
      ...
      !end subroutine ctoa_r8
!!     ctoa :: interpolate from the C-Grid to the A-grid
!!    
      !subroutine ctoa_3d_r4(uin, vin, uout, vout, npx, npy, npz, ng)
         !integer,      intent(IN) :: npx, npy, npz, ng
         !real*4, intent(IN)    ::  uin(isd:ied+1,jsd:jed  , npz)    ! C-grid u-wind field
         !real*4, intent(IN)    ::  vin(isd:ied  ,jsd:jed+1, npz)    ! C-grid v-wind field
         !real*4, intent(OUT)   :: uout(isd:ied  ,jsd:jed  , npz)    ! A-grid u-wind field 
         !real*4, intent(OUT)   :: vout(isd:ied  ,jsd:jed  , npz)    ! A-grid v-wind field
         !integer :: k
         !do k=1,npz
            !call ctoa(uin(:,:,k), vin(:,:,k), uout(:,:,k), vout(:,:,k), npx, npy, ng)
         !enddo
      !end subroutine ctoa_3d_r4
!!
!! ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ !
!!-------------------------------------------------------------------------------
...
!!     ctoa :: interpolate from the C-Grid to the A-grid
!!    
      !subroutine ctoa_3d_r8(uin, vin, uout, vout, npx, npy, npz, ng)
         !integer,      intent(IN) :: npx, npy, npz, ng
         !real*8, intent(IN)    ::  uin(isd:ied+1,jsd:jed  , npz)    ! C-grid u-wind field
         !real*8, intent(IN)    ::  vin(isd:ied  ,jsd:jed+1, npz)    ! C-grid v-wind field
         !real*8, intent(OUT)   :: uout(isd:ied  ,jsd:jed  , npz)    ! A-grid u-wind field 
         !real*8, intent(OUT)   :: vout(isd:ied  ,jsd:jed  , npz)    ! A-grid v-wind field
         !integer :: k
         !do k=1,npz
            !call ctoa(uin(:,:,k), vin(:,:,k), uout(:,:,k), vout(:,:,k), npx, npy, ng)
         !enddo
      !end subroutine ctoa_3d_r8

!
!     rotate_winds :: rotate winds from the sphere-to-cube || cube-to-sphere
!
      subroutine rotate_winds_r4(myU, myV, p1, p2, p3, p4, t1, ndims, dir)


         integer,      intent(IN) :: ndims
         real*4, intent(INOUT) :: myU    ! u-wind field
         real*4, intent(INOUT) :: myV    ! v-wind field
         real*8, intent(IN)    :: p1(ndims)    !             p4     
         real*8, intent(IN)    :: p2(ndims)    !                    
         real*8, intent(IN)    :: p3(ndims)    !        p1   t1   p3
         real*8, intent(IN)    :: p4(ndims)    !                    
         real*8, intent(IN)    :: t1(ndims)    !             p2     
         integer,   intent(IN)    :: dir   ! Direction ; 1=>sphere-to-cube  2=> cube-to-sphere

         real*8 :: ee1(3), ee2(3), ee3(3), elon(3), elat(3)

         real*8 :: g11, g12, g21, g22
      ...
      end subroutine rotate_winds_r4

      subroutine rotate_winds_r8(myU, myV, p1, p2, p3, p4, t1, ndims, dir)


         integer,      intent(IN) :: ndims
         real*8, intent(INOUT) :: myU    ! u-wind field
         real*8, intent(INOUT) :: myV    ! v-wind field
         real*8, intent(IN)    :: p1(ndims)    !             p4     
         real*8, intent(IN)    :: p2(ndims)    !                    
         real*8, intent(IN)    :: p3(ndims)    !        p1   t1   p3
         real*8, intent(IN)    :: p4(ndims)    !                    
      ...
      end subroutine rotate_winds_r8

!
!     gsum :: get global sum
!
      real(FVPRC)  function globalsum(p, npx, npy, ifirst, ilast, jfirst, jlast, gridstruct, tile) result (gsum)
