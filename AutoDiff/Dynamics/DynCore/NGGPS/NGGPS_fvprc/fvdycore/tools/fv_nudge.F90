...
!use fms_io_mod,        only: field_size
 ...
 integer :: k_breed = 0
 integer :: k_trop = 0
 real(FVPRC)    :: p_trop = 950.E2
 real(FVPRC)    :: dps_min = 50.      ! maximum PS increment (pa; each step) due to inline breeding
 real(FVPRC)    :: del2_cd = 0.16
...
! Namelist variables:
 ...
 real(FVPRC)    :: mask_fac = 0.25        ! [0,1]  0: no mask;  1: full strength
...
! to the use of the time-varying weighting factor
 real(FVPRC) :: tau_ps     = 21600.       ! 1-day
 real(FVPRC) :: tau_q      = 86400.       ! 1-day
 real(FVPRC) :: tau_winds  = 21600.       !  6-hr
 real(FVPRC) :: tau_virt   = 43200. 
 real(FVPRC) :: tau_hght   = 43200.
...
! track dataset: 'INPUT/tropical_cyclones.txt'
  ...
  real(FVPRC) :: grid_size     = 28.E3
  real(FVPRC) :: tau_vt_slp    = 1200.
  real(FVPRC) :: tau_vt_wind   = 1200.
  real(FVPRC) :: tau_vt_rad    = 4.0  

  real(FVPRC) :: pt_lim =  0.2
  real(FVPRC) ::  slp_env = 101010.    ! storm environment pressure (pa)
  real(FVPRC) :: pre0_env = 100000.    ! critical storm environment pressure (pa) for size computation
  real(FVPRC), parameter:: tm_max = 315.
!------------------
  real(FVPRC):: r_lo = 2.0
  real(FVPRC):: r_hi = 5.0    ! try 4.0?
!------------------
  real(FVPRC)::  r_fac = 1.2
  real(FVPRC) :: r_min = 200.E3
  real(FVPRC) :: r_inc =  25.E3
  real(FVPRC), parameter:: del_r = 50.E3
  real(FVPRC):: elapsed_time = 0.0
  real(FVPRC):: nudged_time = 1.E12 ! seconds 
...
! Accumulated tendencies
  ...
  real(FVPRC), intent(out), dimension(is:ie,js:je,npz):: t_dt, q_dt
  real(FVPRC), intent(out), dimension(is:ie,js:je):: ps_dt, ts

  type(fv_grid_type), intent(INOUT), target :: gridstruct
! local:
  real(FVPRC):: h2(is:ie,js:je)
  real(FVPRC):: m_err(is:ie,js:je)         ! height error at specified model interface level
  real(FVPRC):: slp_n(is:ie,js:je)         ! "Observed" SLP
  real(FVPRC):: slp_m(is:ie,js:je)         ! "model" SLP
  real(FVPRC)::   mask(is:ie,js:je)
  real(FVPRC)::     tv(is:ie,js:je)
  real(FVPRC):: peln(is:ie,npz+1)
  real(FVPRC):: pe2(is:ie, npz+1)
  real(FVPRC):: ptmp
  real(FVPRC), allocatable :: ps_obs(:,:)
  real(FVPRC), allocatable, dimension(:,:,:):: u_obs, v_obs, t_obs, q_obs
  real(FVPRC), allocatable, dimension(:,:,:):: du_obs, dv_obs
  real(FVPRC):: ps_fac(is:ie,js:je)
  integer :: seconds, days
  integer :: i,j,k, iq, kht
  real(FVPRC) :: factor, rms, bias, co
  real(FVPRC) :: rdt, press(npz), profile(npz), prof_t(npz), prof_q(npz), du, dv
  logical used


  real(kind=R_GRID), pointer, dimension(:,:,:) :: agrid
  real(FVPRC), pointer, dimension(:,:)   :: rarea, area
  ...
  real(FVPRC), pointer, dimension(:,:) :: dx, dy, rdxc, rdyc
...
!-------------------------------------------
      ...
      if ( nudge_debug ) then
           ...
           call prt_maxmin('PS_o', ps_obs, is, ie, js, je, 0, 1, 0.01_FVPRC)
           ptmp = 0.01*g0_sum(ps_obs, is, ie, js, je, 1, .false., isd, ied, jsd, jed, area)
           if(master) write(*,*) 'Mean PS_o=', ptmp
           call prt_maxmin('SLP_m', slp_m, is, ie, js, je, 0, 1, 0.01_FVPRC)
           call prt_maxmin('SLP_o', slp_n, is, ie, js, je, 0, 1, 0.01_FVPRC)
      endif
...
!$OMP parallel do default(none) shared(is,ie,js,je,phis,gz0,m_err,mask,slp_m,slp_n)
      ...
      call prt_maxmin('SLP Error (mb)=', m_err, is, ie, js, je, 0, 1, 0.01_FVPRC)
...
! Input
      real(FVPRC), intent(in):: dt, factor
      integer, intent(in):: npz, nwat, npx, npy
      real(FVPRC), intent(in), dimension(npz+1):: ak, bk
      type(fv_grid_bounds_type), intent(IN) :: bd
      real(FVPRC), intent(in):: phis(isd:ied,jsd:jed)
      real(FVPRC), intent(in), dimension(is:ie,js:je):: ps_obs, mask, tm
      type(fv_grid_type), intent(IN), target :: gridstruct
      type(domain2d), intent(INOUT) :: domain
! Input/Output
      real(FVPRC), intent(inout), dimension(isd:ied,jsd:jed):: ps
      real(FVPRC), intent(inout), dimension(isd:ied,jsd:jed,npz):: delp, pt, ua, va
      real(FVPRC), intent(inout):: q(isd:ied,jsd:jed,npz,nwat)
! local
      real(FVPRC), dimension(is:ie,js:je):: ps_dt
      integer, parameter:: kmax = 100
      real(FVPRC):: pn0(kmax+1), pk0(kmax+1)
      real(FVPRC), dimension(is:ie,npz+1):: pe2, peln
      real(FVPRC):: pst, dbk, pt0, rdt, bias
      integer i, j, k, iq

      real(FVPRC), pointer, dimension(:,:) :: area
...
! Convert tracer density back to moist mixing ratios
 ...
 real(FVPRC), intent(inout):: ps_dt(is:ie,js:je)
 real(FVPRC), intent(IN), dimension(isd:ied,jsd:jed) :: area
!
 real(FVPRC):: esl, total_area
 real(FVPRC):: bias, psum
 integer:: i, j
 ...
 real(FVPRC) function g0_sum(p, ifirst, ilast, jfirst, jlast, mode, reproduce, isd, ied, jsd, jed, area)
...
!     call mpp_sum(gsum)    ! does this work?
 ...
 real(FVPRC), intent(in), dimension(isc:iec,jsc:jec):: tm, ps, gz
! tm: virtual temperature required as input
 real(FVPRC), intent(out):: slp(isc:iec,jsc:jec)
 integer:: i,j
 ...
 subroutine get_obs(Time, dt, zvir, ak, bk, ps, ts, ps_obs, delp, pt, nwat, q, u_obs, v_obs, t_obs, q_obs,  &
  ...
  type(fv_grid_bounds_type), intent(IN) :: bd
  real(FVPRC), intent(in), dimension(isd:ied,jsd:jed):: phis
  real(FVPRC), intent(in), dimension(is:ie,js:je):: mask
  real(FVPRC), intent(inout), dimension(isd:ied,jsd:jed):: ps
...
! local:
  ...
  real(FVPRC), allocatable,dimension(:,:,:):: uu, vv
  integer :: seconds, days
  integer :: i,j,k
...
! Needs gz3 for ps_nudging
 ...
 character(len=17) :: mod_name = 'fv_nudge'
  ...
  type(fv_grid_bounds_type), intent(IN) :: bd
  real(FVPRC), intent(in), dimension(bd%isd:bd%ied,bd%jsd:bd%jed):: phis
  real(FVPRC), intent(in), dimension(npz+1):: ak, bk
  real(FVPRC), intent(out), dimension(bd%is:bd%ie,bd%js:bd%je):: ts
...
!
  type(fv_grid_bounds_type), intent(IN) :: bd
  real(FVPRC), intent(out), dimension(is:ie,js:je):: ts, ps
...
!-------------------------------------------------------
      ...
      do j=js,je
         ...
         enddo
      enddo
      call prt_maxmin('SST_model', ts, is, ie, js, je, 0, 1, 1._FVPRC)

#ifndef DYCORE_SOLO
! Perform interp to FMS SST format/grid
      call ncep2fms( wk1 )
      if(master) call pmaxmin( 'SST_ncep', sst_ncep, i_sst, j_sst, 1._FVPRC)
!     if(nfile/=1 .and. master) call pmaxmin( 'SST_anom', sst_anom, i_sst, j_sst, 1._FVPRC)
#endif
       deallocate ( wk1 ) 
...
! nfile = nfile + 1
 ...
 subroutine remap_coef(agrid)
  ...
  real(FVPRC), intent(IN), dimension(isd:ied,jsd:jed,2) :: agrid
...
! local:
  real(FVPRC), dimension(is:ie,km+1):: pn0
  integer i,j,k
...
! local
      ...
      real(kind=R_GRID):: pos(2)
      real(FVPRC):: slp_o         ! sea-level pressure (Pa)
      real(FVPRC):: w10_o, p_env
      real(FVPRC):: r_vor
      real(FVPRC):: relx0, relx, f1, pbreed, pbtop, delp0, dp0
      real(FVPRC):: ratio, p_count, p_sum, a_sum, mass_sink, delps
      real(FVPRC):: p_lo, p_hi, tau_vt, mslp0
      real(FVPRC):: split_time, fac, pdep, r2, r3
      integer year, month, day, hour, minute, second
      integer n, i, j, k, iq, k0

      real(FVPRC), pointer :: area(:,:)
...
! Compute lowest layer air temperature:
       ...
       enddo
    enddo
!   call prt_maxmin('TM', tm, is, ie, js, je, 0, 1, 1._FVPRC)
...
123   continue
      ...
      if ( p_env>1015.E2 .or. p_env<920.E2 ) then
         if( nudge_debug ) then
            ...
            call prt_maxmin('SLP_breeding', slp, is, ie, js, je, 0, 1, 0.01_FVPRC)
         endif
         goto 5000
      endif
...
!             if(master) write(*,*) 'Failed to size the Vortex for the weak storm'
         ...
         if ( r_vor < 1250.E3_FVPRC ) goto 123
...
! Under-development:
                     ...
                     delps = relx*(ps(i,j) - p_hi) * min( (tm_max-tm(i,j))/10., 1._FVPRC)
! After mp115
!                    delps = relx*(ps(i,j) - p_hi) * min( (tm_max-tm(i,j))/5., 1._FVPRC)
...
400     continue 
      ...
      if ( abs(mass_sink)<1.E-40_FVPRC ) goto 5000
...
!------------------------------------------------------------------------------------------
      ...
      real(FVPRC), intent(in), dimension(is:ie,js:je):: slp
      type(fv_grid_type), intent(IN), target :: gridstruct
      real(FVPRC), intent(inout):: u(isd:ied,jsd:jed+1,npz)
      real(FVPRC), intent(inout):: v(isd:ied+1,jsd:jed,npz)
! Input/Output
      real(FVPRC), intent(inout), dimension(isd:ied,jsd:jed,npz):: delp
! local
      real(FVPRC), dimension(is:ie,js:je):: us, vs
      real(FVPRC) wu(is:ie,  js:je+1)
      real(FVPRC) wv(is:ie+1,js:je)
      real(FVPRC) u1(is:ie), v1(is:ie)
      real(FVPRC):: dist(is:ie,js:je), wind(is:ie,js:je)
      real(kind=R_GRID):: pos(2)
      real(FVPRC):: slp_o         ! sea-level pressure (Pa)
      real(FVPRC):: w10_o, p_env
      real(FVPRC):: r_vor, pc, p_count
      real(FVPRC):: r_max, speed, ut, vt, speed_local        ! tangent wind speed
      real(FVPRC):: u_bg, v_bg, mass, t_mass
      real(FVPRC):: relx0, relx, f1
      real(FVPRC):: z0, mslp0
      real(FVPRC):: zz = 35.           ! mid-layer height at the lowest model level
      real(FVPRC):: wind_fac           ! Computed ( ~ 1.2)
      integer n, i, j

      ! Pointers
      real(FVPRC), pointer, dimension(:,:) :: dx, dy, rdxa, rdya, a11, a21, a12, a22, area
...
! Input
      ...
      real(FVPRC), intent(in):: zvir
      real(FVPRC), intent(in), dimension(npz+1):: ak, bk
      real(FVPRC), intent(in), dimension(isd:ied,jsd:jed):: phis, ps
      real(FVPRC), intent(in), dimension(is:ie,js:je,npz):: u_obs, v_obs
      type(fv_grid_type), intent(IN), target :: gridstruct
! Input/Output
      real(FVPRC), intent(inout), dimension(isd:ied,jsd:jed,npz):: delp, pt, ua, va, u_dt, v_dt
      real(FVPRC), intent(inout)::q(isd:ied,jsd:jed,npz,nwat)
...
! Output
    ...
    real(FVPRC), intent(out):: w10_o          ! 10-m wind speed
...
! This routine is for filtering the wind tendency
   ...
   type(fv_grid_bounds_type), intent(IN) :: bd
   real(FVPRC), intent(inout), dimension(is:ie,js:je,kmd):: du, dv
   integer, intent(IN) :: npx, npy
   type(fv_grid_type), intent(IN), target :: gridstruct
  type(domain2d), intent(INOUT) :: domain
! local:
...
!$OMP                          private(fx, fy)
 ...
 subroutine rmse_bias(a, rms, bias, area)
   real(FVPRC), intent(in):: a(is:ie,js:je)
   real(FVPRC), intent(IN) :: area(isd:ied,jsd:jed)
   real(FVPRC), intent(out):: rms, bias
   integer:: i,j
   real(FVPRC):: total_area
 ...
 real(FVPRC), intent(in):: a(is:ie,js:je), b(is:ie,js:je)
 real(FVPRC), intent(in):: area(isd:ied,jsd:jed)
 real(FVPRC), intent(out):: co
 real(FVPRC):: m_a, m_b, std_a, std_b
 integer:: i,j
 real(FVPRC):: total_area
...
! Compute correlation: 
 ...
 real(FVPRC),  intent(in):: a(is:ie,js:je)
 real(FVPRC), intent(IN) :: area(isd:ied,jsd:jed)
 real(FVPRC), intent(out):: mean, stdv
 integer:: i,j
 real(FVPRC):: total_area
