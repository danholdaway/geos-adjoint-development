...
#define DEALLOCGLOB_(A) if(associated(A)) then;A=0;call MAPL_DeAllocNodeArray(A,rc=STATUS);if(STATUS==MAPL_NoShm) deallocate(A,stat=STATUS);NULLIFY(A);endif
   ...
   !use fv_grid_tools_mod, only: cubed_sphere,  &
         !dx,dy, dxa,dya, dxc,dyc, area, rarea
   !use fv_mp_mod,         only: gid, masterproc, domain, tile, ng, mp_barrier, mp_gather, mp_bcst, &
         !is,js,ie,je, isd,jsd,ied,jed, fill_corners, YDir, &
         !npes, npes_x, npes_y
   use fv_mp_mod,         only: is_master, gid, masterproc, tile, ng, mp_barrier, mp_gather, mp_bcst, &
         is,js,ie,je, isd,jsd,ied,jed, fill_corners, YDir,npes_x,npes_y
...
!  is Atm%q defined in all cases?
      ...
      call prt_maxmin('T', Atm(1)%pt, is, ie, js, je, ng, Atm(1)%npz, 1.0_FVPRC, master_proc=(gid==0) )
...
! always one at this point
      ...
      call prt_maxmin('ZS', Atm(1)%phis,  is, ie, js, je, ng, 1, 1./grav, master_proc=(gid==0))
...
! Read V
         ...
         call prt_maxmin(' U_geos', u0, is_i, ie_i, js_i, je_i, ng_i, km, 1.0_FVPRC, master_proc=(gid==0))
         call prt_maxmin(' V_geos', v0, is_i, ie_i, js_i, je_i, ng_i, km, 1.0_FVPRC, master_proc=(gid==0))
         allocate ( ua(is:ie,js:je,km) )
         allocate ( va(is:ie,js:je,km) )
...
! Horiz Interp for surface pressure 
         call prt_maxmin('PS_geos', ps0, is_i, ie_i, js_i, je_i, ng_i, 1, 1.0_FVPRC, master_proc=(gid==0))
         do j=js,je
            do i=is,ie
...
! Horiz Interp for surface height
         call prt_maxmin('GZ_geos', gz0, is_i, ie_i, js_i, je_i, ng_i, 1, 1.0/grav, master_proc=(gid==0))
         do j=js,je
            do i=is,ie
               ic=index_c2c(1,i,j,tile)
               jc=index_c2c(2,i,j,tile)
...
! Read in tracers: only sphum at this point
         if( file_exist("moist_internal_restart_in") .and. ntracers(1) > 0 ) then
            ...
            do ivar=iq_moist0,iq_moist1
               ...
               call prt_maxmin( 'Q_geos', q0, is_i, ie_i, js_i, je_i, ng_i, km, 1._FVPRC, master_proc=(gid==0))
            enddo
...
! Horiz Interp for T
         ...
         call prt_maxmin( 'T_geos', t0, is_i, ie_i, js_i, je_i, ng_i, km, 1.0_FVPRC, master_proc=(gid==0))
         allocate (  tp(is:ie,js:je,km) )
         do k=1,km
            do j=js,je
               do i=is,ie
...
! Finished, let's check the results !
      ...
      call prt_maxmin('GZ_model', Atm(1)%phis, is, ie, js, je, ng, 1, 1.0/grav, master_proc=(gid==0))
      call prt_maxmin('PS_model', Atm(1)%ps, is, ie, js, je, ng, 1, 0.01_FVPRC, master_proc=(gid==0))
      call prt_maxmin('DP_model', Atm(1)%delp, is, ie, js, je, ng, npz, 1.0_FVPRC, master_proc=(gid==0))
      call prt_maxmin(' U_model', Atm(1)%u, is, ie, js, je, ng, npz, 1.0_FVPRC, master_proc=(gid==0))
      call prt_maxmin(' V_model', Atm(1)%v, is, ie, js, je, ng, npz, 1.0_FVPRC, master_proc=(gid==0))
      call prt_maxmin('PT_model', Atm(1)%pt, is, ie, js, je, ng, npz, 1.0_FVPRC, master_proc=(gid==0))
! Range check the MOIST tracers
      ...
      do iq=1,nmoist+npchem+ngocart
         call prt_maxmin('QP_model', Atm(1)%q(is:ie,js:je,1:npz,iq), is, ie, js, je, 0, npz, 1._FVPRC, master_proc=(gid==0))
      enddo
...
! Regrid from -180:180 to 0:360
         ...
         if(gid==0) call pmaxmin( 'U_geos',   u0(:,2:jm,:), im*(jm-1), km, 1.0_FVPRC)
         if(gid==0) call pmaxmin( 'V_geos',   v0,           im*jm    , km, 1.0_FVPRC)
         allocate ( ua(is:ie,js:je,km) )
         allocate ( va(is:ie,js:je,km) )
         allocate ( ua0(im,jm) )
         allocate ( va0(im,jm) )
...
! Horiz Interp for surface pressure 
         if(gid==0) call pmaxmin( 'PS_geos', ps0, im,    jm, 0.01_FVPRC)
         do j=js,je
            do i=is,ie
               i1 = id1(i,j)
               i2 = id2(i,j)
         ...
         deallocate ( ps0 )
! Horiz Interp for surface height
         if(gid==0) call pmaxmin( 'ZS_geos', gz0, im,    jm, 1.0/grav)
         do j=js,je
            do i=is,ie
               i1 = id1(i,j)
               i2 = id2(i,j)
...
! Read in tracers: only sphum at this point
         if( file_exist("moist_internal_restart_in") .and. ntracers(1) > 0 ) then
            ...
            do ivar=iq_moist0,iq_moist1
               ...
               if (gid==0) call pmaxmin( 'MOIST_Q_',  q0(:,:,:), im*jm, km, 1.0_FVPRC)
            enddo
            if (filetype == 0) then
               call MAPL_NCIOClose(NCIO,destroy=.true.)
            else
...
! Read in tracers: only sphum at this point
         if( file_exist("gocart_internal_restart_in") .and. ntracers(2) > 0 ) then
            ...
            do ivar=iq_gocart0,iq_gocart1
               ...
               enddo
               if (gid==0) call pmaxmin( 'GOCART_Q_',  q0(:,:,:), im*jm, km, 1.0_FVPRC)
            enddo
...
! Read in tracers: only sphum at this point
         if( file_exist("pchem_internal_restart_in") .and. ntracers(3) > 0 ) then
            ...
            do ivar=iq_pchem0,iq_pchem1
               ...
               enddo
               if (gid==0) call pmaxmin( 'PCHEM_Q_',  q0(:,:,:), im*jm, km, 1.0_FVPRC)
            enddo
...
! Horiz Interp for T
         if(gid==0) call pmaxmin( 'T_geos',   t0, im*jm, km, 1.0_FVPRC) 
         allocate (  tp(is:ie,js:je,km) )
         do k=1,km
            do j=js,je
               do i=is,ie
...
! Range check the MOIST tracers
      ...
      if (gid==0) print*
      do iq=iq_moist0,iq_moist1
         call prt_maxmin('QP_MOIST_Q', Atm(1)%q(is:ie,js:je,1:npz,iq), is, ie, js, je, 0, npz, 1._FVPRC, master_proc=(gid==0))
      enddo
      if (gid==0) print*
      do iq=iq_gocart0,iq_gocart1
         call prt_maxmin('QP_GOCART_Q', Atm(1)%q(is:ie,js:je,1:npz,iq), is, ie, js, je, 0, npz, 1._FVPRC, master_proc=(gid==0))
      enddo
      if (gid==0) print*
      do iq=iq_pchem0,iq_pchem1
         call prt_maxmin('QP_PCHEM_Q', Atm(1)%q(is:ie,js:je,1:npz,iq), is, ie, js, je, 0, npz, 1._FVPRC, master_proc=(gid==0))
      enddo
...
!------------------------------------------------------
         ...
         do j=js,je
            ...
            enddo
         enddo
         call prt_maxmin('SST_model', Atm(1)%ts, is, ie, js, je, 0, 1, 1._FVPRC, master_proc=(gid==0))
...
! Share the load
         ...
         if(gid==1) call pmaxmin( 'U_data',   u0, im*jm, km, 1._FVPRC)
         if(gid==1) call pmaxmin( 'V_data',   v0, im*jm, km, 1._FVPRC)
         if(gid==2) call pmaxmin( 'T_data',   t0, im*jm, km, 1._FVPRC)
         if(gid==3) call pmaxmin( 'DEL-P',   dp0, im*jm, km, 0.01_FVPRC)
...
! D to A transform on lat-lon grid:
      ...
      if(gid==4) call pmaxmin( 'UA', ua, im*jm, km, 1._FVPRC)
      if(gid==4) call pmaxmin( 'VA', va, im*jm, km, 1._FVPRC)
      ...
      if (gid==0) call pmaxmin( 'PS_data (mb)', ps0, im, jm, 0.01_FVPRC)
...
5000 continue
  ...
  call prt_maxmin('PS_model', Atm%ps, is, ie, js, je, ng, 1, 0.01_FVPRC, master_proc=(gid==0))
...
5000              continue
                  ...
                  call prt_maxmin('UT', ut, is, ie, js, je, ng, npz, 1.0_FVPRC, master_proc=(gid==0))
                  call prt_maxmin('VT', vt, is, ie, js, je, ng, npz, 1.0_FVPRC, master_proc=(gid==0))
...
! call prt_maxmin('WZ', wz, is, ie, js, je, mg, npz, 1._FVPRC, gid==0)
...
5000 continue
  ...
  call prt_maxmin('PS_model', Atm%ps, is, ie, js, je, ng, 1, 0.01_FVPRC, master_proc=(gid==0))
  call prt_maxmin('UT', ut, is, ie, js, je, ng, npz, 1._FVPRC, master_proc=(gid==0))
  call prt_maxmin('VT', vt, is, ie, js, je, ng, npz, 1._FVPRC, master_proc=(gid==0))
...
! allocate global arrays (preferable in shared memory)
                        if(MAPL_ShmInitialized) then
                           if (gid==masterproc) write(*,*) 'Using MAPL_Shmem in external_ic: init_cubsph_grid' 
                           call MAPL_AllocNodeArray(grid_in,Shp=(/npts,npts,2,ntiles/),rc=STATUS)
                        else
                           if (gid==masterproc) write(*,*) 'WARNING... in external_ic: Global grid allocate'
