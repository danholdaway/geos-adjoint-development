...
!         nlat = 21600
      ...
      integer           ::  nlon = 21600
      integer           ::  nlat = 10800
      real(FVPRC):: peak_fac = 1.0  ! overshoot factor for the mountain peak
...
!
      real(FVPRC), allocatable:: zs_g(:,:), sgh_g(:,:), oro_g(:,:)
...
!---- version number -----
      character(len=128) :: version = '$Id: fv_surf_map.F90,v 1.1.2.1.2.2.30.1.88.1.20.1.2.1.4.1 2017/02/16 03:47:48 aoloso Exp $'
...
#include <netcdf.inc>
    ...
    ! INPUT arrays
      ...
      real(FVPRC), intent(in):: dx(bd%is-ng:bd%ie+ng, bd%js-ng:bd%je+ng+1)
      real(FVPRC), intent(in):: dy(bd%is-ng:bd%ie+ng+1, bd%js-ng:bd%je+ng)
      real(FVPRC), intent(in)::dxc(bd%is-ng:bd%ie+ng+1, bd%js-ng:bd%je+ng)
      real(FVPRC), intent(in)::dyc(bd%is-ng:bd%ie+ng, bd%js-ng:bd%je+ng+1)
      ...
      real(FVPRC), intent(IN):: sin_sg(9,bd%isd:bd%ied,bd%jsd:bd%jed)
    ...
    ! OUTPUT arrays
      real(FVPRC), intent(out):: phis(bd%is-ng:bd%ie+ng, bd%js-ng:bd%je+ng)
! Local:
      real(FVPRC), allocatable :: z2(:,:)
! Position of edges of the box containing the original data point:
      integer          londim
      integer          latdim
      character(len=80) :: topoflnm
      real(kind=4), allocatable ::  ft(:,:), zs(:,:)
      real(FVPRC), allocatable :: lon1(:),  lat1(:)
      real(FVPRC) dx1, dx2, dy1, dy2, lats, latn, r2d
      real(kind=R_GRID) da_max
      real(kind=R_GRID) cd2
      real(FVPRC) zmean, z2mean, delg
...
!-----------------------------------------------
 ...
 subroutine del2_cubed_sphere(npx, npy, q, area, dx, dy, dxc, dyc, sin_sg, nmax, cd, zero_ocean, oro, nested, domain, bd)
    ...
    ! INPUT arrays
      ...
      real(FVPRC), intent(in):: dxc(bd%isd:bd%ied+1,bd%jsd:bd%jed)
      real(FVPRC), intent(in):: dyc(bd%isd:bd%ied,  bd%jsd:bd%jed+1)
      real(FVPRC), intent(IN):: sin_sg(9,bd%isd:bd%ied,bd%jsd:bd%jed)
      real(FVPRC), intent(in):: oro(bd%isd:bd%ied,  bd%jsd:bd%jed)        ! 0==water, 1==land
      logical, intent(IN) :: nested
      type(domain2d), intent(INOUT) :: domain
    ! OUTPUT arrays
      real(FVPRC), intent(inout):: q(bd%is-ng:bd%ie+ng, bd%js-ng:bd%je+ng)
! Local:
      real(FVPRC) ddx(bd%is:bd%ie+1,bd%js:bd%je), ddy(bd%is:bd%ie,bd%js:bd%je+1)
      integer i,j,n
...
! Limit diffusive flux over ater cells:
 ...
 subroutine del4_cubed_sphere(npx, npy, q, area, dx, dy, dxc, dyc, sin_sg, nmax, zero_ocean, oro, nested, domain, bd)
      ...
      type(domain2d), intent(INOUT) :: domain
! diffusivity
      real(FVPRC) :: diff(bd%is-3:bd%ie+2,bd%js-3:bd%je+2)
! diffusive fluxes: 
      real(FVPRC) :: fx1(bd%is:bd%ie+1,bd%js:bd%je), fy1(bd%is:bd%ie,bd%js:bd%je+1)
      real(FVPRC) :: fx2(bd%is:bd%ie+1,bd%js:bd%je), fy2(bd%is:bd%ie,bd%js:bd%je+1)
      real(FVPRC) :: fx4(bd%is:bd%ie+1,bd%js:bd%je), fy4(bd%is:bd%ie,bd%js:bd%je+1)
      real(FVPRC), dimension(bd%isd:bd%ied,bd%jsd:bd%jed):: d2, win, wou 
      real(FVPRC), dimension(bd%is:bd%ie,bd%js:bd%je):: qlow, q0, qmin, qmax
      real(FVPRC), parameter:: esl = 1.E-20
      integer i,j, n
...
! Input
      ...
      real(FVPRC), intent(in):: lat1(jt+1)       ! original southern edge of the cell [-pi/2:pi/2]
      real(FVPRC), intent(in):: lon1(im+1)       ! original western edge of the cell [0:2*pi]
      ...
      logical, intent(IN) :: nested
! Output
      real(FVPRC), intent(out):: q2(bd%isd:bd%ied,bd%jsd:bd%jed) ! Mapped data at the target resolution
      real(FVPRC), intent(out):: f2(bd%isd:bd%ied,bd%jsd:bd%jed) ! oro
      real(FVPRC), intent(out):: h2(bd%isd:bd%ied,bd%jsd:bd%jed) ! variances of terrain
! Local
      real(FVPRC) :: lon_g(-igh:im+igh)
      real(FVPRC) lat_g(jt), cos_g(jt)
      ...
      real(FVPRC) th1, aa, asum,  qsum, fsum, hsum, lon_w, lon_e, lat_s, lat_n, r2d
      real(FVPRC) qsp, fsp, hsp
      real(FVPRC) qnp, fnp, hnp
      real(FVPRC) delg, th0, tmp1, prod1, prod2, prod3, prod4
      integer ig_lon, jp
      integer:: lat_crit
      real(FVPRC) pi5, pi2
...
4444  continue
 ...
 logical function inside_p4(p1, p2, p3, p4, pp, th0)
      ...
      real(FVPRC), intent(in):: p1(3), p2(3), p3(3), p4(3)
      real(FVPRC), intent(in):: pp(3)
      real(FVPRC), intent(in):: th0
! A * B = |A| |B| cos(angle)
! Local:
...
!--------------------------------- 
      ...
      real(FVPRC), intent(inout) :: lfrac(bd%isd:bd%ied,bd%jsd:bd%jed)
