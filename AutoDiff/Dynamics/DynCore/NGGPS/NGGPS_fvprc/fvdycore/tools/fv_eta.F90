...
!---- version number -----
 character(len=128) :: version = '$Id: fv_eta.F90,v 1.1.2.1.2.1.18.1.12.1.88.1.20.1.2.1.4.1 2017/02/16 03:47:48 aoloso Exp $'
...
! This is the easy to use version of the set_eta
      ...
      real(FVPRC), intent(out):: ak(km+1)
      real(FVPRC), intent(out):: bk(km+1)
      real(FVPRC), intent(out):: ptop         ! model top (Pa)
      real(FVPRC) pint, stretch_fac
      integer  k
...
! Hi-top:
 ...
 subroutine mount_waves(km, ak, bk, ptop, ks, pint)
  ...
  real(FVPRC),    intent(out):: ptop, pint
  integer, intent(out):: ks
! Local
  real(FVPRC), parameter:: p00 = 1.E5
  real(FVPRC), dimension(km+1):: ze, pe1, peln, eta
  real(FVPRC), dimension(km):: dz, dlnp
  real(FVPRC) ztop, t0, dz0, sum1, tmp1
  real(FVPRC) ep, es, alpha, beta, gama, s_fac
  integer  k, k500
...
! Pure pressure:
 ...
 subroutine set_eta(km, ks, ptop, ak, bk)
      ...
      real(FVPRC), intent(out):: ptop         ! model top (Pa)
! local
      real(FVPRC) a24(25),b24(25)            ! GFDL AM2L24
      real(FVPRC) a26(27),b26(27)            ! Jablonowski & Williamson 26-level
      real(FVPRC) a32(33),b32(33)
      real(FVPRC) a32w(33),b32w(33)
      real(FVPRC) a47(48),b47(48)
      real(FVPRC) a48(49),b48(49)
      real(FVPRC) a52(53),b52(53)
      real(FVPRC) a54(55),b54(55)
      real(FVPRC) a56(57),b56(57)            ! For Mars GCM
      real(FVPRC) a60(61),b60(61)
      real(FVPRC) a64(65),b64(65)
      real(FVPRC) a68(69),b68(69)            ! cjg: grid with enhanced PBL resolution
      real(FVPRC) a96(97),b96(97)            ! cjg: grid with enhanced PBL resolution
      real(FVPRC) a100(101),b100(101)
      real(FVPRC) a104(105),b104(105)

#if defined(MARS_GCM)
      real(FVPRC):: p0= 6.0E2
      real(FVPRC):: pc= 1.0e-3
      real(FVPRC) a36(37),b36(37)            ! For Mars GCM
      real(FVPRC) a28(29),b28(29)            !  Mars 28 levels 
      real(FVPRC) a30(31),b30(31)            !  Mars 31 levels
      real(FVPRC) a46(47),b46(47)            !  Mars 46 levels 
#elif defined(VENUS_GCM)
      real(FVPRC):: p0= 92.E5
      real(FVPRC):: pc= 20.E5
      real(FVPRC) a50(51), b50(51)
#else
      real(FVPRC):: p0=1000.E2
      real(FVPRC):: pc=200.E2
#endif

      real(FVPRC) pt, pint, lnpe, dlnp
      real(FVPRC) press(km+1), pt1(km)
      integer  k
...
! Low top:
        case (31)
             ptop = 300.
             pint = 100.E2
             call var_dz(km, ak, bk, ptop, ks, pint, 1.035_FVPRC)
#ifndef TEST_GWAVES
        case (41)
             ptop = 100.
             pint = 100.E2
             call var_hi(km, ak, bk, ptop, ks, pint, 1.035_FVPRC)
#endif
        case (51)
             ptop = 100.
             pint = 100.E2
...
!            call var_dz(km, ak, bk, ptop, ks, pint, 1.035_FVPRC)
        ...
        case (63)
             ptop = 10.
             pint = 100.E2
             call var_hi(km, ak, bk, ptop, ks, pint, 1.03_FVPRC)
! Hi-top:
        case (71)
             ptop = 1.
             pint = 100.E2
...
! Pure sigma-coordinate with uniform spacing in "z"
!--------------------------------------------------
          call gw_1d(km, 1000.E2_FVPRC, ak, bk, ptop, 10.E3_FVPRC, pt1)
            ks = 0
          pint = ak(1)
#else
...
!        pint = pt + 0.5*1.E5/real(km)     ! SJL: 20120327
 ...
 subroutine var_hi(km, ak, bk, ptop, ks, pint, s_rate)
  ...
  real(FVPRC),    intent(in):: s_rate        ! between [1. 1.1]
  real(FVPRC),    intent(out):: ak(km+1), bk(km+1)
  real(FVPRC),    intent(inout):: pint
  integer, intent(out):: ks
! Local
  real(FVPRC), parameter:: p00 = 1.E5
  real(FVPRC), dimension(km+1):: ze, pe1, peln, eta
  real(FVPRC), dimension(km):: dz, s_fac, dlnp
  real(FVPRC) ztop, t0, dz0, sum1, tmp1
  real(FVPRC) ep, es, alpha, beta, gama
!---- Tunable parameters:
  integer:: k_inc = 15   ! # of layers from bottom up to near const dz region
  real(FVPRC):: s0 = 0.10 ! lowest layer stretch factor
!-----------------------
  real(FVPRC):: s_inc
  integer  k
...
! Pure pressure:
 ...
 subroutine hybrid_z_dz(km, dz, ztop, s_rate)
  integer, intent(in):: km
  real(FVPRC),    intent(in):: s_rate        ! between [1. 1.1]
  real(FVPRC),    intent(in):: ztop
  real(FVPRC),    intent(out):: dz(km)
! Local
  real(FVPRC), parameter:: p00 = 1.E5
  real(FVPRC), dimension(km+1):: ze, pe1, peln, eta
  real(FVPRC), dimension(km):: s_fac, dlnp
  real(FVPRC) t0, dz0, sum1, tmp1
  real(FVPRC) ep, es, alpha, beta, gama
  integer  k
 ...
 subroutine get_eta_level(npz, p_s, pf, ph, ak, bk, pscale)
  ...
  real(FVPRC), intent(in)  :: p_s            ! unit: pascal
  real(FVPRC), intent(in)  :: ak(npz+1)
  real(FVPRC), intent(in)  :: bk(npz+1)
  real(FVPRC), intent(in), optional :: pscale
  real(FVPRC), intent(out) :: pf(npz)
  real(FVPRC), intent(out) :: ph(npz+1)
  integer k
 ...
 subroutine compute_dz(km, ztop, dz)
  ...
  integer, intent(in):: km
  real(FVPRC),   intent(in):: ztop        ! try 50.E3
  real(FVPRC),   intent(out):: dz(km)
!------------------------------
  real(FVPRC) ze(km+1), dzt(km)
  integer k
...
! Top:
 ...
 subroutine compute_dz_var(km, ztop, dz)
  ...
  real(FVPRC),   intent(out):: dz(km)
!------------------------------
  real(FVPRC), parameter:: s_rate = 1.0
  real(FVPRC) ze(km+1)
  real(FVPRC) s_fac(km)
  real(FVPRC) sum1, dz0
  integer k
...
! Re-scale dz with the stretched ztop
 ...
 subroutine compute_dz_L32(km, ztop, dz)
  ...
  integer, intent(in):: km
  real(FVPRC),   intent(out):: dz(km)
  real(FVPRC),   intent(out):: ztop        ! try 50.E3
!------------------------------
  real(FVPRC) dzt(km)
  real(FVPRC) ze(km+1)
  real(FVPRC) dz0, dz1, dz2
  real(FVPRC) z0, z1, z2
...
!       endif
 ...
 subroutine compute_dz_L101(km, ztop, dz)
  ...
  real(FVPRC),   intent(out):: dz(km)
  real(FVPRC),   intent(out):: ztop      ! try 30.E3
!------------------------------
  real(FVPRC) ze(km+1)
  real(FVPRC) dz0, dz1
  real(FVPRC):: stretch_f = 1.16
  integer k, k0, k1
...
!  ztop (km) = 20.2859154
 ...
 real(FVPRC), intent(in):: rgrav, ztop
 real(FVPRC), intent(in):: dz(km)       ! Reference vertical resolution for zs=0
 real(FVPRC), intent(in):: hs(is-ng:ie+ng,js-ng:je+ng)
 real(FVPRC), intent(inout)::  ze(is:ie,js:je,km+1)
 real(FVPRC), optional, intent(out):: dz3(is-ng:ie+ng,js-ng:je+ng,km)
! local
 logical:: filter_xy = .false.
 real(FVPRC), allocatable:: delz(:,:,:)
 integer ntimes
 real(FVPRC) zint
 real(FVPRC):: z1(is:ie,js:je)
 real(FVPRC):: z(km+1)
 real(FVPRC) sig, z_rat
...
!--------------------------------------
  ...
  real(FVPRC), intent(inout):: ze(is:ie,js:je,km+1)
! local:
  real(FVPRC), parameter:: df = 0.25
  real(FVPRC) dz(km)
  real(FVPRC) flux(km+1)
  integer k, n, k1, k2
  ...
  integer, intent(in):: km
  real(FVPRC),    intent(in):: p0, ztop
  real(FVPRC),    intent(inout):: ptop
  real(FVPRC),    intent(inout):: ak(km+1), bk(km+1)
  real(FVPRC), intent(out):: pt1(km)
! Local
  logical:: isothermal
  real(FVPRC), dimension(km+1):: ze, pe1, pk1
  real(FVPRC), dimension(km):: dz1
  real(FVPRC) t0, n2, s0
  integer  k
...
! Compute volume mean potential temperature with hydrostatic eqn:
  ...
  real(FVPRC), intent(inout):: q(im,km)
!---
  integer i, k
  real(FVPRC) qtmp
