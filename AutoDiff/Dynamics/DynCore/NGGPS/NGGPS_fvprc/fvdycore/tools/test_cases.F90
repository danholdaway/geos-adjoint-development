 module test_cases_mod
...
#ifndef MAPL_MODE
      use constants_mod,     only: radius, pi, omega, grav, kappa, rdgas, cp_air, rvgas
#else
      use MAPL_MOD
#endif
      ...
      use mpp_mod,           only: mpp_error, FATAL, mpp_root_pe, mpp_broadcast, mpp_sum, mpp_pe
...
!!! END DEBUG CODE
...
#ifdef MAPL_MODE
  real(R_GRID), parameter :: RADIUS       = MAPL_RADIUS
  ...
  real(FVPRC), parameter :: RDGAS        = MAPL_RDRY
...
! Case 0 parameters
      real(FVPRC) :: p0_c0 = 3.0
      real(FVPRC) :: rgamma = 5.0
      real(FVPRC) :: lat0 = pi/2.0 !pi/4.8
      real(FVPRC) :: lon0 = 0.0 !pi-0.8

!  pi_shift moves the initial location of the cosine bell for Case 1
      real(FVPRC), parameter :: pi_shift = 0.0 !3.0*pi/4.
 ...
 ! -1:null_op, 0:All-Grids, 1:C-Grid, 2:D-Grid, 3:A-Grid, 4:A-Grid then Rotate, 5:D-Grid with unit vectors then Rotate
      ...
      real(FVPRC), allocatable, dimension(:) :: pz0, zz0

      integer :: tracer_test, wind_field

     ! Ubar = initial wind speed parameter
     real(FVPRC)   :: Ubar, Vbar
     ! gh0 = initial surface height parameter
     real(FVPRC)   :: gh0

     !  case 9 parameters
     real(FVPRC)  , allocatable :: case9_B(:,:)
     real(FVPRC)   :: AofT(2)


     !  Validating fields used in statistics
     real(FVPRC)  , allocatable :: phi0(:,:,:) ! Validating Field
     real(FVPRC)  , allocatable :: ua0(:,:,:)  ! Validating U-Wind
     real(FVPRC)  , allocatable :: va0(:,:,:)  ! Validating V-Windfms_io_exit, get_tile_string, &

     real(FVPRC)  , allocatable :: gh_table(:), lats_table(:)
     logical :: gh_initialized = .false.

     !  Initial Conservation statistics ; total mass ; enstrophy ; energy
     real(FVPRC)   :: tmass_orig
     real(FVPRC)   :: tvort_orig
     real(FVPRC)   :: tener_orig
 ...
 !---- version number -----
      character(len=128) :: version = '$Id: test_cases.F90,v 1.2.2.1.2.1.30.1.22.1.46.6.4.3.2.1.2.1.2.1.2.1 2017/02/16 03:47:48 aoloso Exp $'
...
!
 ...
 ! defOnGrid = -1:null_op, 0:All-Grids, 1:C-Grid, 2:D-Grid, 3:A-Grid, 4:A-Grid then Rotate, 5:D-Grid with unit vectors then Rotate
      ...
      real(FVPRC)  ,    intent(INOUT) :: UBar
      real(FVPRC) ,      intent(INOUT) ::    u(isd:ied  ,jsd:jed+1)
      real(FVPRC) ,      intent(INOUT) ::    v(isd:ied+1,jsd:jed  )
      real(FVPRC) ,      intent(INOUT) ::   uc(isd:ied+1,jsd:jed  )
      real(FVPRC) ,      intent(INOUT) ::   vc(isd:ied  ,jsd:jed+1)
      real(FVPRC) ,      intent(INOUT) ::   ua(isd:ied  ,jsd:jed  )
      real(FVPRC) ,      intent(INOUT) ::   va(isd:ied  ,jsd:jed  )
      ...
      real(FVPRC)   :: dist, r, r0 
      integer :: i,j,k,n
      real(FVPRC) :: utmp, vtmp

      real(FVPRC) :: psi_b(isd:ied+1,jsd:jed+1), psi(isd:ied,jsd:jed), psi1, psi2 
      integer :: is2, ie2, js2, je2

      real(kind=R_GRID), pointer, dimension(:,:,:)   :: agrid, grid
      real(FVPRC), pointer, dimension(:,:)     :: area, rarea, fC, f0
      real(kind=R_GRID), pointer, dimension(:,:,:)   :: ee1, ee2, en1, en2
      real(kind=R_GRID), pointer, dimension(:,:,:,:) :: ew, es
      real(FVPRC), pointer, dimension(:,:)     :: dx,dy, dxa,dya, rdxa, rdya, dxc,dyc
      ...
      real(FVPRC),    pointer :: acapN, acapS, globalarea
...
!
      ...
      real(FVPRC) ,      intent(inout) :: delz(bd%isd:,bd%jsd:,1:)
      real(FVPRC) ,      intent(inout)   ::  ze0(bd%is:,bd%js:,1:)

      real(FVPRC) ,      intent(inout) ::   ak(npz+1)
      real(FVPRC) ,      intent(inout) ::   bk(npz+1)
      ...
      real(FVPRC), intent(INOUT) :: ptop

      type(domain2d), intent(IN), target :: domain_in

      real(FVPRC)   ::  tmp(1-ng:npx  +ng,1-ng:npy  +ng,1:nregions)
      real(FVPRC)   :: tmp1(1   :npx     ,1   :npy     ,1:nregions)
      ...
      real(FVPRC)   :: dist, r, r1, r2, r0, omg, A, B, C
      integer :: i,j,k,nreg,z,zz
      integer :: i0,j0,n0, nt
      real(FVPRC)   :: utmp,vtmp,ftmp
      real(FVPRC)   :: rk

      integer, parameter :: jm = 5761
      real(FVPRC)   :: ll_phi(jm)
      real(FVPRC)   ::   ll_u(jm)
      real(FVPRC)   ::   ll_j(jm)
      real(FVPRC)   ::   cose(jm)
      real(FVPRC)   ::   sine(jm)
      real(FVPRC)   ::   cosp(jm)
      real(FVPRC)   :: ddeg, deg, DDP, DP, ph5
      real(FVPRC)   :: myB, myC, yy
      integer   :: jj,jm1

      real(FVPRC) :: Vtx, p, w_p
      real(FVPRC) :: x1,y1,z1,x2,y2,z2,ang
      ...
      real(FVPRC) :: L1_norm
      real(FVPRC) :: L2_norm
      real(FVPRC) :: Linf_norm
      real(FVPRC) :: pmin, pmin1
      real(FVPRC) :: pmax, pmax1
      real(FVPRC) :: grad(bd%isd:bd%ied  ,bd%jsd:bd%jed,2)
      real(FVPRC) :: div0(bd%isd:bd%ied  ,bd%jsd:bd%jed  ) 
      real(FVPRC) :: vor0(bd%isd:bd%ied  ,bd%jsd:bd%jed  )
      real(FVPRC) :: divg(bd%isd:bd%ied  ,bd%jsd:bd%jed  )
      real(FVPRC) :: vort(bd%isd:bd%ied  ,bd%jsd:bd%jed  )
      real(FVPRC) :: ztop, rgrav, p00, pturb, zmid, pk0, t00
      real(FVPRC) :: dz1(npz), ppt(npz)
      real(FVPRC) :: ze1(npz+1), pe1(npz+1)

      integer :: nlon,nlat
      character(len=80) :: oflnm, hgtflnm
      integer :: is2, ie2, js2, je2

     real(FVPRC) :: psi(bd%isd:bd%ied,bd%jsd:bd%jed)
     real(FVPRC) :: psi_b(bd%isd:bd%ied+1,bd%jsd:bd%jed+1)
     real(FVPRC) :: psi1, psi2

! Baroclinic Test Case 12
      real(FVPRC) :: eta(npz), eta_0, eta_s, eta_t
      real(FVPRC) :: eta_v(npz), press, anti_rot
      real(FVPRC) :: T_0, T_mean, delta_T, lapse_rate, n2, zeta, s0
      real(FVPRC) :: pt1,pt2,pt3,pt4,pt5,pt6, pt7, pt8, pt9, u1, pt0
      real(FVPRC) :: uu1, uu2, uu3, vv1, vv2, vv3
!     real wbuffer(npx+1,npz)
!     real sbuffer(npy+1,npz)
      real(FVPRC) wbuffer(npy+2,npz)
      real(FVPRC) sbuffer(npx+2,npz)
 
      real(FVPRC) :: gz(bd%isd:bd%ied,bd%jsd:bd%jed,npz+1), zt, zdist
      real(FVPRC) :: zvir

! Super-Cell
      real(FVPRC) :: us0 = 30.
      real(FVPRC), dimension(npz):: pk1, ts1, qs1, uz1, zs1, dudz
      real(FVPRC):: zm, zc
...
!Test case 35
      real(FVPRC):: cs_m3
!Test case 51
      real(FVPRC) :: omega0, k_cell, z0, H, px
      real(FVPRC) :: d1, d2, p1p(2), rt, s
      real(FVPRC) :: wind_alpha, period, h0, rm, zp3(3), dz3(3), k0, lp


!Test case 55
      real(FVPRC), dimension(npz+1) :: pe0, gz0, ue, ve, we, pte, qe
      real(FVPRC) :: d, cor, exppr, exppz, gamma, Ts0, q00, exponent, ztrop, height, zp, rp
      real(FVPRC) :: qtrop, ttrop, zq1, zq2
      real(FVPRC) :: dum, dum1, dum2, dum3, dum4, dum5, dum6, ptmp, uetmp, vetmp
      real(FVPRC) ::   pe_u(bd%is:bd%ie,npz+1,bd%js:bd%je+1)
      real(FVPRC) ::   pe_v(bd%is:bd%ie+1,npz+1,bd%js:bd%je)
      real(FVPRC) ::   ps_u(bd%is:bd%ie,bd%js:bd%je+1)
      real(FVPRC) ::   ps_v(bd%is:bd%ie+1,bd%js:bd%je)
      ...
      real(FVPRC), pointer, dimension(:,:)     :: rarea, fC, f0
      real(kind=R_GRID), pointer, dimension(:,:,:)   :: ee1, ee2, en1, en2
      real(kind=R_GRID), pointer, dimension(:,:,:,:) :: ew, es
      real(FVPRC), pointer, dimension(:,:)     :: dx,dy, dxa,dya, rdxa, rdya, dxc,dyc
...
! Aqua-planet case: mean SLP=1.E5
         phis = 0.0
         call hydro_eq(npz, is, ie, js, je, ps, phis, 1.E5_FVPRC,      &
...
! Center of the mountain:
       ...
       if ( hybrid_z ) then
            ...
            elseif( npz.eq.31 .or. npz.eq.41 .or. npz.eq.51 ) then
                ztop = 16.E3
                call hybrid_z_dz(npz, dz1, ztop, 1.0_FVPRC)
            else
                if ( is_master() ) write(*,*) 'Using const DZ'
                ztop = 15.E3
                dz1(1) = ztop / real(npz) 
...
! *** Add Initial perturbation ***
      if ( test_case == 37 ) then
        ...
        call prt_maxmin('PS', ps(is:ie,js:je), is, ie, js, je, 0, 1, 0.01_FVPRC)
...
!--------------------------------------------------------------------
  ...
  real(FVPRC), intent(in):: nx    ! east-west wave number
  real(FVPRC), intent(in):: ny    ! North-south wave number
  real(FVPRC), intent(in), optional:: rn    ! (optional) magnitude of random perturbation
  real(kind=R_GRID), intent(in), dimension(i0:i1,j0:j1):: lon, lat
  real(FVPRC), intent(out):: q(ifirst:ilast,jfirst:jlast,km,nq)
! Local var:
  real(FVPRC):: qt(i0:i1,j0:j1)
  real(FVPRC):: qtmp, ftmp
  integer:: i,j,k,iq
...
! Rankine vortex
!----------------------------
  real(FVPRC), intent(in):: ubar ! max wind (m/s)
  real(FVPRC), intent(in):: r0   ! Radius of max wind (m)
  real(FVPRC), intent(in):: p1(2)   ! center position (longitude, latitude) in radian
  real(FVPRC), intent(inout):: u(isd:ied,  jsd:jed+1)
  real(FVPRC), intent(inout):: v(isd:ied+1,jsd:jed)
...
! local:
  ...
  real(kind=R_GRID):: e1(3), e2(3), ex(3), ey(3)
  real(FVPRC):: vr, r, d2, cos_p, x1, y1
  real(FVPRC):: utmp, vtmp
  integer i, j
...
! shift:
  ...
  enddo
     ...
     real(FVPRC) function gh_jet(npy, lat_in)
     integer, intent(in):: npy
     real(FVPRC), intent(in):: lat_in
     real(FVPRC) lat, lon, dp, uu
     real(FVPRC) h0, ft
     integer j,jm
...
! Search:
     ...
     real(FVPRC) function u_jet(lat)
      real(FVPRC) lat, lon, dp
      real(FVPRC) umax, en, ph0, ph1
      ...
      real(FVPRC), intent(OUT) :: B(isd:ied,jsd:jed)
      real(FVPRC), intent(IN) :: agrid(isd:ied,jsd:jed,2)
      real(FVPRC) :: myC,yy,myB
      integer :: i,j
...
!     
   ...
   real(FVPRC) , intent(INOUT) :: phis(isd:ied  ,jsd:jed  )
   real(FVPRC) , intent(IN) :: time_since_start
   real(FVPRC) :: tday, amean
   integer :: i,j
!
! Generate B forcing function
!
...
!-------------------------------------------------------------------------------
   ...
   subroutine case51_forcing(delp, uc, vc, u, v, ua, va, pe, time, dt, gridstruct, npx, npy, npz, ptop, domain)
     ...
     real(FVPRC) :: s, l, dt2, V0, phase
     real(FVPRC) :: ull, vll, lonp
     real(FVPRC) :: p0(2), elon(3), elat(3)

     real(FVPRC) :: psi(isd:ied,jsd:jed)
     real(FVPRC) :: psi_b(isd:ied+1,jsd:jed+1)
     real(FVPRC) :: dist, psi1, psi2
     ...
     real(kind=R_GRID), pointer, dimension(:,:,:) :: agrid, grid
     real(FVPRC), pointer, dimension(:,:)   :: dx, dxa, dy, dya, dxc, dyc
...
!                                                in Williamson, 1994 (p.16)
       subroutine get_stats(dt, dtout, nt, maxnt, ndays, u,v,pt,delp,q,phis, ps, &
         ...
         real(FVPRC)  ,    intent(IN) :: dt, dtout, ndays
         real(FVPRC) ,      intent(INOUT) ::    u(isd:ied  ,jsd:jed+1,npz)
         real(FVPRC) ,      intent(INOUT) ::    v(isd:ied+1,jsd:jed  ,npz)
         real(FVPRC) ,      intent(INOUT) ::   pt(isd:ied  ,jsd:jed  ,npz)
         real(FVPRC) ,      intent(INOUT) :: delp(isd:ied  ,jsd:jed  ,npz)
         real(FVPRC) ,      intent(INOUT) ::    q(isd:ied  ,jsd:jed  ,npz, ncnst)
         real(FVPRC) ,      intent(INOUT) :: phis(isd:ied  ,jsd:jed  )
         real(FVPRC) ,      intent(INOUT) ::   ps(isd:ied  ,jsd:jed  )
         real(FVPRC) ,      intent(INOUT) ::   uc(isd:ied+1,jsd:jed  ,npz)
         real(FVPRC) ,      intent(INOUT) ::   vc(isd:ied  ,jsd:jed+1,npz)
         real(FVPRC) ,      intent(INOUT) ::   ua(isd:ied  ,jsd:jed  ,npz)
         real(FVPRC) ,      intent(INOUT) ::   va(isd:ied  ,jsd:jed  ,npz)
         ...
         real(FVPRC)   :: L2_norm
         real(FVPRC)   :: Linf_norm
         real(FVPRC)   :: pmin, pmin1, uamin1, vamin1
         real(FVPRC)   :: pmax, pmax1, uamax1, vamax1
         real(kind=4) :: arr_r4(5)
         real(FVPRC)   :: tmass0, tvort0, tener0, tKE0
         real(FVPRC)   :: tmass, tvort, tener, tKE
         real(FVPRC)   :: temp(is:ie,js:je)
         integer :: i0, j0, k0, n0
         integer :: i, j, k, n, iq

         real(FVPRC) :: psmo, Vtx, p, w_p, p0
         real(FVPRC) :: x1,y1,z1,x2,y2,z2,ang

         real(FVPRC)   :: p1(2), p2(2), p3(2), r, r0, dist, heading

         real(FVPRC) :: uc0(isd:ied+1,jsd:jed  ,npz)
         real(FVPRC) :: vc0(isd:ied  ,jsd:jed+1,npz)

         real(FVPRC) :: myDay
         integer :: myRec

         real(FVPRC), save, allocatable, dimension(:,:,:) :: u0, v0
         real(FVPRC)  ::    up(isd:ied  ,jsd:jed+1,npz)
         real(FVPRC)  ::    vp(isd:ied+1,jsd:jed  ,npz)

         real(FVPRC), dimension(:,:,:), pointer :: grid, agrid
         real(FVPRC), dimension(:,:),   pointer :: area, f0, dx, dy, dxa, dya, dxc, dyc
...
!                                 Arrival Point (Lon/Lat)
         ...
         real(FVPRC) , intent(IN)  :: p1(2), p2(2)
         real(FVPRC) , intent(IN)  :: dist
         real(FVPRC) , intent(IN)  :: heading
         real(FVPRC) , intent(OUT) :: p3(2)
...
!                     for any var
       ...
       subroutine get_scalar_stats(var, varT, npx, npy, ndims, nregions, &
         ...
         real(FVPRC)  ,    intent(IN) ::  var(isd:ied,jsd:jed)
         real(FVPRC)  ,    intent(IN) :: varT(isd:ied,jsd:jed)
         real(FVPRC)  ,   intent(OUT) :: vmin
         real(FVPRC)  ,   intent(OUT) :: vmax
         real(FVPRC)  ,   intent(OUT) :: L1_norm
         real(FVPRC)  ,   intent(OUT) :: L2_norm
         real(FVPRC)  ,   intent(OUT) :: Linf_norm
         ...
         real(FVPRC)   :: varSUM, varSUM2, varMAX
         real(FVPRC)   :: gsum
         real(FVPRC)   :: vminT, vmaxT, vmeanT, vvarT
         integer :: i0, j0, n0

         real(FVPRC), dimension(:,:,:), pointer :: grid, agrid
         real(FVPRC), dimension(:,:),   pointer :: area
...
!                     for any var
       ...
       subroutine get_vector_stats(varU, varUT, varV, varVT, &
         ...
         integer,      intent(IN) :: ndims
         integer,      intent(IN) :: nregions, tile
         real(FVPRC)  ,    intent(IN) ::  varU(isd:ied,jsd:jed)
         real(FVPRC)  ,    intent(IN) :: varUT(isd:ied,jsd:jed)
         real(FVPRC)  ,    intent(IN) ::  varV(isd:ied,jsd:jed)
         real(FVPRC)  ,    intent(IN) :: varVT(isd:ied,jsd:jed)
         real(FVPRC)  ,   intent(OUT) :: vmin
         real(FVPRC)  ,   intent(OUT) :: vmax
         real(FVPRC)  ,   intent(OUT) :: L1_norm
         real(FVPRC)  ,   intent(OUT) :: L2_norm
         ...
         real(FVPRC)   ::  var(isd:ied,jsd:jed)
         real(FVPRC)   :: varT(isd:ied,jsd:jed)
         real(FVPRC)   :: vmean
         real(FVPRC)   :: vvar
         real(FVPRC)   :: vmin1
         real(FVPRC)   :: vmax1
...
!
       ...
       real(FVPRC), intent(IN) :: ndt
       integer, intent(IN) :: n_split
       integer, intent(IN) :: npx, npy, npz, tile
       logical, OPTIONAL, intent(IN) :: noPrint
       real(FVPRC) ,      intent(IN) ::   uc(isd:ied+1,jsd:jed  ,npz)
       real(FVPRC) ,      intent(IN) ::   vc(isd:ied  ,jsd:jed+1,npz)
 
       real(FVPRC) :: ideal_c=0.06
       real(FVPRC) :: tolerance= 1.e-3
       real(FVPRC) :: dt_inc, dt_orig 
       real(FVPRC)   :: meanCy, minCy, maxCy, meanCx, minCx, maxCx
       ...
       type(fv_grid_type), intent(IN), target :: gridstruct
       real(FVPRC), dimension(:,:), pointer :: dxc, dyc
...
!
      subroutine pmxn(p, npx, npy, nregions, tile, gridstruct, pmin, pmax, i0, j0, n0)
         ...
         integer,      intent(IN) :: npy
         integer,      intent(IN) :: nregions, tile
         real(FVPRC)  , intent(IN)  :: p(isd:ied,jsd:jed)
         type(fv_grid_type), intent(IN), target  :: gridstruct
         real(FVPRC)  , intent(OUT) :: pmin
         real(FVPRC)  , intent(OUT) :: pmax
      ...
      real(FVPRC), pointer, dimension(:,:,:)   :: agrid, grid
      real(FVPRC), pointer, dimension(:,:)     :: area, rarea, fC, f0
      real(kind=R_GRID), pointer, dimension(:,:,:)   :: ee1, ee2, en1, en2
      real(kind=R_GRID), pointer, dimension(:,:,:,:) :: ew, es
      real(FVPRC), pointer, dimension(:,:)     :: dx,dy, dxa,dya, rdxa, rdya, dxc,dyc
...
!
      ...
      real(FVPRC) ,      intent(INOUT) ::   ua(isd:ied  ,jsd:jed  ,npz)
      real(FVPRC) ,      intent(INOUT) ::   va(isd:ied  ,jsd:jed  ,npz)
      real(FVPRC) ,      intent(INOUT) :: omga(isd:ied  ,jsd:jed  ,npz)
      ...
      real(FVPRC), allocatable :: tmp(:,:,:)
      real(FVPRC), allocatable :: tmpA(:,:,:)
#if defined(SW_DYNAMICS) 
      real(FVPRC), allocatable :: ut(:,:,:)
      real(FVPRC), allocatable :: vt(:,:,:)
#else       
      real(FVPRC), allocatable :: ut(:,:,:,:)
      real(FVPRC), allocatable :: vt(:,:,:,:)
      real(FVPRC), allocatable :: tmpA_3d(:,:,:,:)
#endif
      real(FVPRC), allocatable :: vort(:,:)

      real(FVPRC)   :: p1(2)      ! Temporary Point
      real(FVPRC)   :: p2(2)      ! Temporary Point
      real(FVPRC)   :: p3(2)      ! Temporary Point
      real(FVPRC)   :: p4(2)      ! Temporary Point
      real(FVPRC)   :: pa(2)      ! Temporary Point
      real(FVPRC)   :: utmp, vtmp, r, r0, dist, heading
...
!
      ...
      real(FVPRC)   ::  tmp(1-ng:npx  +ng,1-ng:npy  +ng,1:nregions)
      real(FVPRC)   :: tmpA(1-ng:npx-1+ng,1-ng:npy-1+ng,1:nregions)
      real(FVPRC)   :: p1(2)      ! Temporary Point
      real(FVPRC)   :: p2(2)      ! Temporary Point
      real(FVPRC)   :: p3(2)      ! Temporary Point
      real(FVPRC)   :: p4(2)      ! Temporary Point
      real(FVPRC)   :: pa(2)      ! Temporary Point
      real(FVPRC)   :: ut(1:npx,1:npy,1:nregions)
      real(FVPRC)   :: vt(1:npx,1:npy,1:nregions)
      real(FVPRC)   :: utmp, vtmp, r, r0, dist, heading
      integer   ::  i,j,k,n,nreg
      real(FVPRC)   :: vort(isd:ied,jsd:jed)
...
#include <netcdf.inc>
         ...
         real(FVPRC)  , intent(IN)  :: p(npx-1,npy-1,npz,ntiles)
         integer,      intent(IN) :: ndims
...
!
      subroutine wrt2d(iout, nrec, i1,i2, j1,j2, npx, npy, nregions, p)
         ...
         integer,      intent(IN) :: npy
         integer,      intent(IN) :: nregions
         real(FVPRC)  , intent(IN)  :: p(npx-1,npy-1,nregions)
...
!
      subroutine init_double_periodic(u,v,w,pt,delp,q,phis, ps,pe,peln,pk,pkz,  uc,vc, ua,va, ak, bk,  &
        ...
        real(FVPRC), dimension(bd%is:bd%ie):: pm, qs
        real(FVPRC), dimension(1:npz):: pk1, ts1, qs1
        real(FVPRC) :: us0 = 30.
        real(FVPRC) :: dist, r0, f0_const, prf, rgrav
        real(FVPRC) :: ptmp, ze, zc, zm, utmp, vtmp
        real(FVPRC) :: t00, p00, xmax, xc, xx, yy, pk0, pturb, ztop
        real(FVPRC) :: ze1(npz+1)
         real(FVPRC):: dz1(npz)
        real(FVPRC):: zvir
        ...
        real(kind=R_GRID), pointer, dimension(:,:)     :: area
        real(FVPRC), pointer, dimension(:,:)     :: rarea, fC, f0
        real(FVPRC), pointer, dimension(:,:,:)   :: ee1, ee2, en1, en2
        real(FVPRC), pointer, dimension(:,:,:,:) :: ew, es
        real(FVPRC), pointer, dimension(:,:)     :: dx,dy, dxa,dya, rdxa, rdya, dxc,dyc
...
! Morris Weisman & J. Klemp 2002 sounding
 ...
 real(FVPRC), intent(in):: p00
 real(FVPRC), intent(inout), dimension(km+1):: pe
 real(FVPRC), intent(in), dimension(km+1):: ze
! pt: potential temperature / pk0
! qz: specific humidity (mixing ratio)
 real(FVPRC), intent(out), dimension(km):: pt, qz
! Local:
 integer, parameter:: nx = 5
 real(FVPRC), parameter:: qst = 1.0e-6
 real(FVPRC), parameter:: qv0 = 1.4e-2
 real(FVPRC), parameter:: ztr = 12.E3
 real(FVPRC), parameter:: ttr = 213.
 real(FVPRC), parameter:: ptr = 343.    ! Tropopause potential temp.
 real(FVPRC), parameter:: pt0 = 300.    ! surface potential temperature
 real(FVPRC), dimension(km):: zs, rh, temp, dp, dp0
 real(FVPRC), dimension(km+1):: peln, pk
 real(FVPRC):: qs, zvir, fac_z, pk0, temp1, pm
 integer:: k, n, kk
...
! NCAR form:
 ...
 if ( is_master() ) then
      ...
      call prt_m1('1D Sounding T0', temp, 1, km, 1, 1, 0, 1, 1._FVPRC)
 endif
 ...
 real(FVPRC), intent(in), dimension(km  ):: ts1, qs1, uz1, dudz
 real(FVPRC), intent(in), dimension(km+1):: ze1
 real(FVPRC), intent(in):: zvir, ps0
 real(FVPRC), intent(inout):: ptop
 real(kind=R_GRID), intent(in):: agrid(is-ng:ie+ng,js-ng:je+ng,2)
 real(FVPRC), intent(inout), dimension(km+1):: ak, bk
 real(FVPRC), intent(inout), dimension(is-ng:ie+ng,js-ng:je+ng,km):: pt, delz
 real(FVPRC), intent(out), dimension(is:ie,js:je,km+1):: pk
! pt is FV's cp*thelta_v
 real(FVPRC), intent(inout), dimension(is-1:ie+1,km+1,js-1:je+1):: pe
! Local
 integer, parameter:: nt=5
 integer, parameter:: nlat=1001
 real(FVPRC), dimension(nlat,km):: pt2, pky, dzc
 real(FVPRC), dimension(nlat,km+1):: pk2, pe2, peln2, pte
 real(FVPRC), dimension(km+1):: pe1
 real(FVPRC):: lat(nlat), latc(nlat-1)
 real(FVPRC):: fac_y, dlat, dz0, pk0, tmp1, tmp2, tmp3, pint
...
! Convert pt2 to temperature
 ...
 if ( is_master() ) then
    ...
    call prt_m1('Super_K pe',   pe2, 1, nlat, 1, km+1, 0, 1, 0.01_FVPRC)
    call prt_m1('Super_K Temp', pt2, 1, nlat, 1, km,   0, 1, 1._FVPRC)
 endif
...
! One pressure layer
 ...
 integer, intent(in):: km
 real(FVPRC), intent(in):: zz(km)
 real(FVPRC), intent(out):: um(km), dudz(km)
! Local
 real(FVPRC), parameter:: zs = 5.e3
 real(FVPRC), parameter:: us = 30.
 real(FVPRC):: uc = 15.
 integer k
...
! Output sounding on pressure levels:
 integer, intent(in):: km
 real(FVPRC), intent(in):: ps     ! surface pressure (Pa)
 real(FVPRC), intent(in), dimension(km):: pk1
 real(FVPRC), intent(out), dimension(km):: tp, qp
! Local:
 integer, parameter:: ns = 401
 integer, parameter:: nx = 3
 real(FVPRC), dimension(ns):: zs, pt, qs, us, rh, pp, pk, dpk, dqdt
 real(FVPRC), parameter:: Tmin = 175.
 real(FVPRC), parameter:: p00 = 1.0e5
 real(FVPRC), parameter:: qst = 3.0e-6
 real(FVPRC), parameter:: qv0 = 1.4e-2
 real(FVPRC), parameter:: ztr = 12.E3
 real(FVPRC), parameter:: ttr = 213.
 real(FVPRC), parameter:: ptr = 343.    ! Tropopause potential temp.
 real(FVPRC), parameter:: pt0 = 300.    ! surface potential temperature
 real(FVPRC):: dz0, zvir, fac_z, pk0, temp1, p2
 integer:: k, n, kk
...
555  continue
 ...
 enddo
      ...
      subroutine init_latlon(u,v,pt,delp,q,phis, ps,pe,peln,pk,pkz,  uc,vc, ua,va, ak, bk,  &
        ...
        real(FVPRC) ,      intent(INOUT) ::   ps(isd:ied  ,jsd:jed  )
        real(FVPRC) ,      intent(INOUT) ::   pe(is-1:ie+1,npz+1,js-1:je+1)
        real(FVPRC) ,      intent(INOUT) ::   pk(is:ie    ,js:je    ,npz+1)
        real(FVPRC) ,      intent(INOUT) :: peln(is :ie   ,npz+1    ,js:je)
        real(FVPRC) ,      intent(INOUT) ::  pkz(is:ie    ,js:je    ,npz  )
        real(FVPRC) ,      intent(INOUT) ::   uc(isd:ied+1,jsd:jed  ,npz)
        real(FVPRC) ,      intent(INOUT) ::   vc(isd:ied  ,jsd:jed+1,npz)
        real(FVPRC) ,      intent(INOUT) ::   ua(isd:ied  ,jsd:jed  ,npz)
        real(FVPRC) ,      intent(INOUT) ::   va(isd:ied  ,jsd:jed  ,npz)
        real(FVPRC) ,      intent(inout) :: delz(isd:,jsd:,1:)
        real(FVPRC) ,      intent(inout)   ::  ze0(is:,js:,1:)
...
!!$           call mpp_update_domains( delp, domain )
      ...
      subroutine init_latlon_winds(UBar, u, v, ua, va, uc, vc, defOnGrid, gridstruct)
        ...
        real(FVPRC)   :: p1(2),p2(2),p3(2),p4(2), pt(2)
        real(FVPRC) :: e1(3), e2(3), ex(3), ey(3)
        ...
        real(FVPRC), dimension(:,:,:), pointer :: grid, agrid
        real(FVPRC), dimension(:,:),   pointer :: area, dx, dy, dxc, dyc
...
! Output
 ...
 ! real   , intent(inout) :: vc(ifirst-ng:ilast+ng,jfirst-ng:jlast+1+ng)
  ...
  real(FVPRC)   , intent(inout) ::  u(isd:ied,jsd:jed+1) !ifirst-ng:ilast+ng,jfirst-ng:jlast+1+ng)
  real(FVPRC)   , intent(inout) ::  v(isd:ied+1,jsd:jed) !ifirst-ng:ilast+1+ng,jfirst-ng:jlast+ng)
  real(FVPRC)   , intent(inout) :: ua(isd:ied,jsd:jed)   !ifirst-ng:ilast+ng,jfirst-ng:jlast+ng)
  real(FVPRC)   , intent(inout) :: va(isd:ied,jsd:jed)   !(ifirst-ng:ilast+ng,jfirst-ng:jlast+ng)
  real(FVPRC)   , intent(inout) :: uc(isd:ied+1,jsd:jed) !(ifirst-ng:ilast+1+ng,jfirst-ng:jlast+ng)
  real(FVPRC)   , intent(inout) :: vc(isd:ied,jsd:jed+1) !(ifirst-ng:ilast+ng,jfirst-ng:jlast+1+ng)

!--------------------------------------------------------------
! Local 

  real(FVPRC)   :: sinlon(im,jm)
  real(FVPRC)   :: coslon(im,jm)
  real(FVPRC)   :: sinl5(im,jm)
  real(FVPRC)   :: cosl5(im,jm)

    real(FVPRC) :: tmp1(jsd:jed+1)
    real(FVPRC) :: tmp2(jsd:jed)
    real(FVPRC) :: tmp3(jsd:jed)

    real(FVPRC)  mag,mag1,mag2, ang,ang1,ang2 
    real(FVPRC)  us, vs, un, vn
    integer i, j, k, im2
    integer js1g1
    integer js2g1
    integer js2g2
...
!
         integer,      intent(IN) :: npx, npy
         real(FVPRC)  , intent(IN)    ::  qin(isd:ied  ,jsd:jed  )    ! A-grid field
         real(FVPRC)  , intent(OUT)   :: qout(isd:ied+1,jsd:jed+1)    ! Output  B-grid field
         integer, OPTIONAL, intent(IN) :: altInterp 
         logical, intent(IN) :: nested, cubed_sphere
         real(FVPRC), intent(IN), dimension(isd:ied,jsd:jed)    :: dxa, dya

         integer :: i,j,n

         real(FVPRC) :: tmp1j(jsd:jed+1)
         real(FVPRC) :: tmp2j(jsd:jed+1)
         real(FVPRC) :: tmp3j(jsd:jed+1)
         real(FVPRC) :: tmp1i(isd:ied+1)
         real(FVPRC) :: tmp2i(isd:ied+1)
         real(FVPRC) :: tmp3i(isd:ied+1)
         real(FVPRC) :: tmpq(isd:ied  ,jsd:jed  )
         real(FVPRC) :: tmpq1(isd:ied+1,jsd:jed+1)
         real(FVPRC) :: tmpq2(isd:ied+1,jsd:jed+1)
...
!
      subroutine atod(uin, vin, uout, vout, dxa, dya, dxc, dyc, npx, npy, ng, nested, domain)
         ...
         real(FVPRC)  , intent(IN)    ::  uin(isd:ied  ,jsd:jed  ) ! A-grid u-wind field
         real(FVPRC)  , intent(IN)    ::  vin(isd:ied  ,jsd:jed  ) ! A-grid v-wind field
         real(FVPRC)  , intent(OUT)   :: uout(isd:ied  ,jsd:jed+1) ! D-grid u-wind field
         real(FVPRC)  , intent(OUT)   :: vout(isd:ied+1,jsd:jed  ) ! D-grid v-wind field
         logical, intent(IN) :: nested
         real(FVPRC)  , intent(IN), dimension(isd:ied,jsd:jed) :: dxa, dya
         real(FVPRC)  , intent(IN), dimension(isd:ied+1,jsd:jed) :: dxc
         real(FVPRC)  , intent(IN), dimension(isd:ied,jsd:jed+1) :: dyc
         type(domain2d), intent(INOUT) :: domain


         integer :: i,j
         real(FVPRC) :: tmp1i(isd:ied+1)
         real(FVPRC) :: tmp2i(isd:ied)
         real(FVPRC) :: tmp3i(isd:ied)
         real(FVPRC) :: tmp1j(jsd:jed+1)
         real(FVPRC) :: tmp2j(jsd:jed)
         real(FVPRC) :: tmp3j(jsd:jed)
...
!
      subroutine dtoa(uin, vin, uout, vout, dx, dy, dxa, dya, dxc, dyc, npx, npy, ng)
         ...
         integer,      intent(IN) :: npx, npy, ng
         real(FVPRC)  , intent(IN)    ::  uin(isd:ied  ,jsd:jed+1)    ! D-grid u-wind field
         real(FVPRC)  , intent(IN)    ::  vin(isd:ied+1,jsd:jed  )    ! D-grid v-wind field
         real(FVPRC)  , intent(OUT)   :: uout(isd:ied  ,jsd:jed  )    ! A-grid u-wind field
         real(FVPRC)  , intent(OUT)   :: vout(isd:ied  ,jsd:jed  )    ! A-grid v-wind field
         real(FVPRC)  , intent(IN), dimension(isd:ied,jsd:jed+1) :: dx, dyc
         real(FVPRC)  , intent(IN), dimension(isd:ied+1,jsd:jed) :: dy, dxc
         real(FVPRC)  , intent(IN), dimension(isd:ied,jsd:jed) :: dxa, dya
...
!
      subroutine atoc(uin, vin, uout, vout, dx, dy, dxa, dya, npx, npy, ng, nested, domain, noComm)
         ...
         real(FVPRC)  , intent(IN)    ::  uin(isd:ied  ,jsd:jed  ) ! A-grid u-wind field
         real(FVPRC)  , intent(IN)    ::  vin(isd:ied  ,jsd:jed  ) ! A-grid v-wind field
         real(FVPRC)  , intent(OUT)   :: uout(isd:ied+1,jsd:jed  ) ! C-grid u-wind field
         real(FVPRC)  , intent(OUT)   :: vout(isd:ied  ,jsd:jed+1) ! C-grid v-wind field
         logical, intent(IN) :: nested
         logical, OPTIONAL, intent(IN)   :: noComm
         real(FVPRC)  , intent(IN), dimension(isd:ied,jsd:jed+1) :: dx
         real(FVPRC)  , intent(IN), dimension(isd:ied+1,jsd:jed) :: dy
         real(FVPRC)  , intent(IN), dimension(isd:ied,jsd:jed) :: dxa, dya
         type(domain2d), intent(INOUT) :: domain
...
!
      subroutine ctoa(uin, vin, uout, vout, dx, dy, dxc, dyc, dxa, dya, npx, npy, ng)
         ...
         integer,      intent(IN) :: npx, npy, ng 
         real(FVPRC)  , intent(IN)    ::  uin(isd:ied+1,jsd:jed  )    ! C-grid u-wind field
         real(FVPRC)  , intent(IN)    ::  vin(isd:ied  ,jsd:jed+1)    ! C-grid v-wind field
         real(FVPRC)  , intent(OUT)   :: uout(isd:ied  ,jsd:jed  )    ! A-grid u-wind field
         real(FVPRC)  , intent(OUT)   :: vout(isd:ied  ,jsd:jed  )    ! A-grid v-wind field
         real(FVPRC)  , intent(IN), dimension(isd:ied+1,jsd:jed) :: dxc, dy
         real(FVPRC)  , intent(IN), dimension(isd:ied,jsd:jed+1) :: dyc, dx
         real(FVPRC)  , intent(IN), dimension(isd:ied,jsd:jed) :: dxa, dya
...
!
      subroutine rotate_winds(myU, myV, p1, p2, p3, p4, t1, ndims, dir)
         ...
         integer,      intent(IN) :: ndims
         real(FVPRC)  , intent(INOUT) :: myU    ! u-wind field
         real(FVPRC)  , intent(INOUT) :: myV    ! v-wind field
         ...
         real(FVPRC) :: g11, g12, g21, g22

         real(FVPRC) :: newu, newv
      ...
      subroutine mp_update_dwinds_2d(u, v, npx, npy, domain)
        use mpp_parameter_mod, only: DGRID_NE
         real(FVPRC)  , intent(INOUT)   :: u(isd:ied  ,jsd:jed+1) ! D-grid u-wind field
         real(FVPRC)  , intent(INOUT)   :: v(isd:ied+1,jsd:jed  ) ! D-grid v-wind field
         integer,      intent(IN) :: npx, npy
         type(domain2d), intent(INOUT) :: domain
...
!
      subroutine mp_update_dwinds_3d(u, v, npx, npy, npz, domain)
        use mpp_parameter_mod, only: DGRID_NE
         real(FVPRC)  , intent(INOUT)   :: u(isd:ied  ,jsd:jed+1,npz) ! D-grid u-wind field
         real(FVPRC)  , intent(INOUT)   :: v(isd:ied+1,jsd:jed  ,npz) ! D-grid v-wind field
         integer,      intent(IN) :: npx, npy, npz
         type(domain2d), intent(INOUT) :: domain
         integer k
...
!     gsum :: get global sum
!
      real(FVPRC)  function globalsum(p, npx, npy, ifirst, ilast, jfirst, jlast, isd, ied, jsd, jed, gridstruct, tile) result (gsum)
...
!
 ...
 integer, intent(in):: ifirst,ilast
 real(FVPRC), intent(out)  :: qout(ifirst:)
 real(FVPRC), intent(in)   ::  qin(ifirst:)
 real(FVPRC), intent(in)   ::   dx(ifirst:)
 integer, intent(in):: order
 integer :: i

 real(FVPRC) :: dm(ifirst:ilast),qmax,qmin
 real(FVPRC) :: r3, da1, da2, a6da, a6, al, ar  
 real(FVPRC) :: qLa, qLb1, qLb2
 real(FVPRC) :: x
...
! !INPUT PARAMETERS:
      ...
      real(FVPRC), intent(in):: coslon(im,jm), sinlon(im,jm)
      real(FVPRC), intent(in):: cosl5(im,jm),sinl5(im,jm)
      real(FVPRC), intent(in):: u(im,jfirst-ng_d:jlast+ng_s)

! !INPUT/OUTPUT PARAMETERS:
      real(FVPRC), intent(inout):: v(im,jfirst-ng_d:jlast+ng_d)
...
! !LOCAL VARIABLES:
      ...
      integer i, imh
      real(FVPRC)  uanp(im), uasp(im), vanp(im), vasp(im)
      real(FVPRC)  un, vn, us, vs, r2im
...
! Single PE version
      ...
      real(FVPRC), intent(in)::    fac

      real(FVPRC) qmin, qmax
      integer i,j,k
