...
#if defined(SPMD)
      use fv_mp_mod, only: mp_barrier, masterproc, mp_reduce_max, mp_reduce_min
#endif
!
...
!
...
#endif
      real(FVPRC) , private       :: etime
      real(kind=8) , private       :: totim
      real(FVPRC) , private       :: tarray(2)
      type tms
           private
           real (kind=8) :: usr, sys
      end type tms
      ...
      real(FVPRC) , private       :: us_tmp1(nblks,2)
      real(FVPRC) , private       :: us_tmp2(nblks,2)

      logical, private :: module_initialized = .false.

!---- version number -----
      character(len=128) :: version = '$Id: fv_timing.F90,v 1.1.2.1.122.1.20.1.2.1.6.1.2.1 2017/02/16 03:47:48 aoloso Exp $'
