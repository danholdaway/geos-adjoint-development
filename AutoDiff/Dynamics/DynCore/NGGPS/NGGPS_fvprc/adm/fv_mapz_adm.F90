!        Generated by TAPENADE     (INRIA, Ecuador team)
!  Tapenade 3.12 (r6213) - 13 Oct 2016 10:30
!
! SJL: Apr 12, 2012
! This revision may actually produce rounding level differences due to the elimination of KS to compute
! pressure level for remapping.
MODULE FV_MAPZ_MOD_B
!#ifndef MAPL_MODE
!  use constants_mod,     only: radius, pi, rvgas, rdgas, grav, hlv, hlf, cp_air
!#else
  USE MAPL_MOD, ONLY : mapl_kappa, mapl_radius, mapl_pi_r8, mapl_rvap, &
& mapl_rgas, mapl_grav
  USE CONSTANTS_MOD, ONLY : hlv, hlf, cp_air
  USE FV_ARRAYS_MOD_B, ONLY : real4, real8, 8
!#endif
  USE FV_ARRAYS_MOD_B, ONLY : real4, real8, 8
  USE TRACER_MANAGER_MOD, ONLY : get_tracer_index
  USE FIELD_MANAGER_MOD, ONLY : model_atmos
  USE FV_GRID_UTILS_MOD_B, ONLY : g_sum, g_sum_adm, ptop_min
!  use fv_fill_mod,       only: fillz
!  use fv_eta_mod ,       only: compute_dz_L32, hybrid_z_dz
  USE MPP_DOMAINS_MOD_B, ONLY : mpp_update_domains, &
& mpp_update_domains_adm, domain2d
!  use mpp_mod,           only: FATAL, mpp_error, get_unit, mpp_root_pe, mpp_pe
  USE FV_ARRAYS_MOD_B, ONLY : fv_grid_type
  USE FV_TIMING_MOD, ONLY : timing_on, timing_off
  IMPLICIT NONE
!#ifdef MAPL_MODE
  REAL(real8), PARAMETER :: radius=mapl_radius
  REAL(real8), PARAMETER :: pi=mapl_pi_r8
  REAL(real8), PARAMETER :: rvgas=mapl_rvap
  REAL(real8), PARAMETER :: rdgas=mapl_rgas
  REAL(real8), PARAMETER :: grav=mapl_grav
!#endif
! below which applies stricter constraint
  REAL(fvprc), PARAMETER :: t_min=184.
  REAL(fvprc), PARAMETER :: r3=1./3., r23=2./3., r12=1./12.
  REAL(real8), PARAMETER :: r2=1./2., r0=0.0
! 1384.5
  REAL(fvprc), PARAMETER :: cv_vap=3.*rvgas
! = rdgas * (7/2-1) = 2.5*rdgas=717.68
  REAL(fvprc), PARAMETER :: cv_air=cp_air-rdgas
! heat capacity of ice at 0.C
  REAL(fvprc), PARAMETER :: c_ice=2106.
  REAL(fvprc), PARAMETER :: c_liq=4190.
  REAL(kind=4) :: e_flux, e_flux_nest
  PRIVATE 
  PUBLIC compute_total_energy, lagrangian_to_eulerian, rst_remap, mappm&
& , e_flux, e_flux_nest, map1_cubic, map1_q2
  PUBLIC compute_total_energy_fwd, compute_total_energy_bwd, &
& lagrangian_to_eulerian_fwd, lagrangian_to_eulerian_bwd, map1_cubic_fwd&
& , map1_cubic_bwd, map1_q2_adm
  EXTERNAL COMPUTE_TOTAL_ENERGY_ADM
  EXTERNAL LAGRANGIAN_TO_EULERIAN_ADM

CONTAINS
!  Differentiation of lagrangian_to_eulerian in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord
!2 a2b_edge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_
!core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_gra
!d_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dy
!namics_mod.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_util
!s_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compu
!te_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_trace
!r_fb fv_mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz
!_mod.steepz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_m
!od.riem_solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_
!solver nh_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_m
!od.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corn
!ers sw_core_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d
!_fb tp_core_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxpp
!m_fb tp_core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: ws peln q u v w ze0 delp ua
!                delz omga te0_2d pkz pe pk ps pt te
!   with respect to varying inputs: ws peln q u v w ze0 delp ua
!                delz omga te0_2d pkz pe pk ps pt te
!---- version number -----
!  character(len=128) :: version = '$Id: fv_mapz.F90,v 1.1.2.2.2.2.30.1.22.2.6.2.34.5.4.4.2.2.2.1.4.2.2.2 2017/02/21 20:43:38 bma
!uer Exp $'
!  character(len=128) :: tagname = '$Name: Heracles-UNSTABLE_ncepdyn_Feb222017 $'
  SUBROUTINE LAGRANGIAN_TO_EULERIAN_FWD(last_step, consv, ps, pe, delp, &
&   pkz, pk, mdt, pdt, km, is, ie, js, je, isd, ied, jsd, jed, nq, nwat&
&   , sphum, q_con, u, v, w, delz, pt, q, hs, r_vir, cp, akap, cappa, &
&   kord_mt, kord_wz, kord_tr, kord_tm, kord_mt_pert, kord_wz_pert, &
&   kord_tr_pert, kord_tm_pert, peln, te0_2d, ng, ua, va, omga, te, ws, &
&   fill, reproduce_sum, out_dt, dtdt, ptop, ak, bk, gridstruct, domain&
&   , ze0, gmao_cubic, remap_t, do_sat_adj, hydrostatic, hybrid_z, &
&   do_omega, do_adiabatic_init)
    IMPLICIT NONE
! call cubed_to_latlon(u, v, ua, va, dx, dy, rdxa, rdya, km, 1, flagstruct%c2l_ord)
!  nullify(cosa_s)
!  nullify(rsin2)
    LOGICAL, INTENT(IN) :: last_step
! remap time step
    REAL(fvprc), INTENT(IN) :: mdt
! phys time step
    REAL(fvprc), INTENT(IN) :: pdt
    INTEGER, INTENT(IN) :: km
! number of tracers (including h2o)
    INTEGER, INTENT(IN) :: nq
    INTEGER, INTENT(IN) :: nwat
! index for water vapor (specific humidity)
    INTEGER, INTENT(IN) :: sphum
    INTEGER, INTENT(IN) :: ng
! starting & ending X-Dir index
    INTEGER, INTENT(IN) :: is, ie, isd, ied
! starting & ending Y-Dir index
    INTEGER, INTENT(IN) :: js, je, jsd, jed
! Mapping order for the vector winds
    INTEGER, INTENT(IN) :: kord_mt
! Mapping order/option for w
    INTEGER, INTENT(IN) :: kord_wz
! Mapping order for tracers
    INTEGER, INTENT(IN) :: kord_tr(nq)
! Mapping order for thermodynamics
    INTEGER, INTENT(IN) :: kord_tm
! Mapping order for the vector winds
    INTEGER, INTENT(IN) :: kord_mt_pert
! Mapping order/option for w
    INTEGER, INTENT(IN) :: kord_wz_pert
! Mapping order for tracers
    INTEGER, INTENT(IN) :: kord_tr_pert(nq)
! Mapping order for thermodynamics
    INTEGER, INTENT(IN) :: kord_tm_pert
! factor for TE conservation
    REAL(fvprc), INTENT(IN) :: consv
    REAL(fvprc), INTENT(IN) :: r_vir
    REAL(fvprc), INTENT(IN) :: cp
    REAL(fvprc), INTENT(IN) :: akap
! surface geopotential
    REAL(fvprc), INTENT(IN) :: hs(isd:ied, jsd:jed)
    REAL(fvprc), INTENT(INOUT) :: te0_2d(is:ie, js:je)
    REAL(fvprc), INTENT(IN) :: ws(is:ie, js:je)
    LOGICAL, INTENT(IN) :: do_sat_adj
! fill negative tracers
    LOGICAL, INTENT(IN) :: fill
    LOGICAL, INTENT(IN) :: reproduce_sum
    LOGICAL, INTENT(IN) :: do_omega, do_adiabatic_init
    REAL(fvprc), INTENT(IN) :: ptop
    REAL(fvprc), INTENT(IN) :: ak(km+1)
    REAL(fvprc), INTENT(IN) :: bk(km+1)
    TYPE(FV_GRID_TYPE), INTENT(IN), TARGET :: gridstruct
    TYPE(DOMAIN2D), INTENT(INOUT) :: domain
! !INPUT/OUTPUT
! pe to the kappa
    REAL(fvprc), INTENT(INOUT) :: pk(is:ie, js:je, km+1)
    REAL(fvprc), INTENT(INOUT) :: q(isd:ied, jsd:jed, km, nq)
! pressure thickness
    REAL(fvprc), INTENT(INOUT) :: delp(isd:ied, jsd:jed, km)
! pressure at layer edges
    REAL(fvprc), INTENT(INOUT) :: pe(is-1:ie+1, km+1, js-1:je+1)
! surface pressure
    REAL(fvprc), INTENT(INOUT) :: ps(isd:ied, jsd:jed)
! Specified height at edges (m)
    REAL(fvprc), INTENT(INOUT) :: ze0(is:ie, js:je, km+1)
! u-wind will be ghosted one latitude to the north upon exit
! u-wind (m/s)
    REAL(fvprc), INTENT(INOUT) :: u(isd:ied, jsd:jed+1, km)
! v-wind (m/s)
    REAL(fvprc), INTENT(INOUT) :: v(isd:ied+1, jsd:jed, km)
! vertical velocity (m/s)
    REAL(fvprc), INTENT(INOUT) :: w(isd:ied, jsd:jed, km)
! cp*virtual potential temperature 
    REAL(fvprc), INTENT(INOUT) :: pt(isd:ied, jsd:jed, km)
! as input; output: temperature
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(INOUT) :: delz
    REAL(fvprc), DIMENSION(isd:isd, jsd:jsd, 1), INTENT(INOUT) :: cappa&
&   , q_con
    LOGICAL, INTENT(INOUT) :: remap_t
    LOGICAL, INTENT(IN) :: gmao_cubic
    LOGICAL, INTENT(IN) :: hydrostatic
    LOGICAL, INTENT(IN) :: hybrid_z
    LOGICAL, INTENT(IN) :: out_dt
! u-wind (m/s) on physics grid
    REAL(fvprc), INTENT(INOUT) :: ua(isd:ied, jsd:jed, km)
! v-wind (m/s) on physics grid
    REAL(fvprc), INTENT(INOUT) :: va(isd:ied, jsd:jed, km)
! vertical press. velocity (pascal/sec)
    REAL(fvprc), INTENT(INOUT) :: omga(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: dtdt(is:ie, js:je, km)
! layer-mean pk for converting t to pt
    REAL(fvprc) :: pkz(is:ie, js:je, km)
    REAL(fvprc) :: te(is:ie, js:je, km)
! log(pe)
    REAL(fvprc), INTENT(INOUT) :: peln(is:ie, km+1, js:je)
! !DESCRIPTION:
!
! !REVISION HISTORY:
! SJL 03.11.04: Initial version for partial remapping
!
!-----------------------------------------------------------------------
    INTEGER :: i, j, k
! real(FVPRC) q_source(is:ie,js:je,nq)    ! numerical tracer source from surface
! in case fillz is not sufficient
    REAL(fvprc), DIMENSION(is:ie, js:je) :: te_2d, zsum0, zsum1, dpeln
    REAL(fvprc), DIMENSION(is:ie, km) :: q2, dp2, deng
    REAL(fvprc), DIMENSION(is:ie, km+1) :: ze1, ze2, pe1, pe2, pk1, pk2&
&   , pn2, phis
    REAL(fvprc) :: pe0(is:ie+1, km+1)
    REAL(fvprc) :: pe3(is:ie+1, km+1)
    REAL(fvprc) :: gz(is:ie)
    REAL(fvprc) :: dz1(km)
    REAL(fvprc) :: rcp, rg, tmp, tpe, cvm, rgama, rrg, bkh, dtmp, dlnp, &
&   ztop, z_rat
    REAL(fvprc) :: k1k, kapag, q_liq, q_sol
    INTEGER :: liq_wat, ice_wat, rainwat, snowwat, cld_amt, graupel, iq&
&   , n, kp, k_next
    LOGICAL :: te_map
!     real(FVPRC), pointer, dimension(:,:) :: cosa_s, rsin2
    INTEGER :: isp1, iep1, jsp1, jep1
    INTEGER :: isdp1, iedp1, jsdp1, jedp1
    INTEGER :: abs_kord_tm, abs_kord_tm_pert
    REAL(fvprc) :: u_tj(isd:ied, jsd:jed+1, km)
    REAL(fvprc) :: v_tj(isd:ied+1, jsd:jed, km)
    REAL(fvprc) :: w_tj(isd:ied, jsd:jed, km)
    REAL(fvprc) :: pt_tj(isd:ied, jsd:jed, km)
    REAL(fvprc) :: q2_tj(is:ie, km)
    REAL(fvprc) :: deng_tj(is:ie, km)
    REAL(fvprc) :: te_tj(is:ie, js:je, km)
    REAL(fvprc) :: q_tj(isd:ied, jsd:jed, km, nq)
    REAL(fvprc) :: delz_tj(isd:ied, jsd:jed, km)
    INTRINSIC ABS
    INTRINSIC LOG
    INTRINSIC EXP
    INTRINSIC REAL
    REAL(fvprc) :: result1
    INTEGER :: ad_count
    iep1 = ie + 1
    iedp1 = ied + 1
    jedp1 = jed + 1
!      cosa_s => gridstruct%cosa_s
!      rsin2 => gridstruct%rsin2
! rg/Cv=0.4
    k1k = akap/(1.-akap)
    kapag = -(akap/grav)
    rg = akap*cp
! cv/cp
    rgama = 1. - akap
    rcp = 1./cp
    rrg = -(rdgas/grav)
    IF (kord_tm .LT. 0) THEN
      te_map = .false.
    ELSE
      te_map = .true.
    END IF
    IF (kord_tm .GE. 0.) THEN
      abs_kord_tm = kord_tm
    ELSE
      abs_kord_tm = -kord_tm
    END IF
    IF (kord_tm_pert .GE. 0.) THEN
      abs_kord_tm_pert = kord_tm_pert
    ELSE
      abs_kord_tm_pert = -kord_tm_pert
    END IF
!$OMP parallel do default(none) shared(is,ie,js,je,km,pe,ptop,kord_tm,remap_t,hydrostatic, &
!$OMP                                  pt,pk,rg,peln,q,liq_wat,rainwat,ice_wat,snowwat,    &
!$OMP                                  graupel,q_con,sphum,cappa,r_vir,rcp,k1k,kapag,delp, &
!$OMP                                  delz,akap,pkz,te,rsin2,u,v,cosa_s,hybrid_z,ztop,ps, &
!$OMP                                  ze0,ak,bk,nq,isd,ied,jsd,jed,kord_tr,fill,te_map,   &
!$OMP                                  hs,gz,w,ws,kord_wz,do_omega,omga,rrg,kord_mt,ua)    &
!$OMP                          private(cvm,dz1,q_liq,q_sol,z_rat,kp,k_next,bkh,deng,dp2,   &
!$OMP                                  pe0,pe1,pe2,pe3,pk1,pk2,pn2,phis,q2,ze1,ze2)
    DO j=js,je+1
      CALL PUSHINTEGER(j)
      CALL PUSHREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
      DO k=1,km+1
        DO i=is,ie
          pe1(i, k) = pe(i, k, j)
        END DO
      END DO
      DO i=is,ie
        CALL PUSHREALARRAY(pe2(i, 1))
        pe2(i, 1) = ptop
        CALL PUSHREALARRAY(pe2(i, km+1))
        pe2(i, km+1) = pe(i, km+1, j)
      END DO
!(j < je+1)
      IF (j .NE. je + 1) THEN
        IF (kord_tm .LT. 0) THEN
! remap_t test
          IF (remap_t) THEN
! hydro test
! Note: pt at this stage is cp*Theta_v
! Transform virtual pt to virtual Temp
            IF (hydrostatic) THEN
              DO k=1,km
                DO i=is,ie
                  CALL PUSHREALARRAY(pt(i, j, k))
                  pt(i, j, k) = pt(i, j, k)*(pk(i, j, k+1)-pk(i, j, k))/&
&                   (rg*(peln(i, k+1, j)-peln(i, k, j)))
                END DO
              END DO
              CALL PUSHCONTROL2B_FV(0)
            ELSE
              DO k=1,km
                DO i=is,ie
!#ifdef MOIST_CAPPA
!#ifdef USE_NWAT3
!                     q_liq = q(i,j,k,liq_wat)
!                     q_sol = q(i,j,k,ice_wat)
!#else
!                     q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!                     q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!!                    cappa(i,j,k) = rdgas/(rdgas + ((1.-(q(i,j,k,sphum)+q_liq+q_sol))*cv_air + q(i,j,k,sphum)*cv_vap +  &
!!                                                        q_liq*c_liq + q_sol*c_ice)/(1.+r_vir*q(i,j,k,sphum)))
!                     q_con(i,j,k) = q_liq + q_sol
!                     cvm = (1.-(q(i,j,k,sphum)+q_Con(i,j,k)))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!                     cappa(i,j,k) = rdgas/(rdgas + cvm*(1.-q_con(i,j,k))/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k)))
!                     pt(i,j,k) = rcp*pt(i,j,k)*exp(cappa(i,j,k)/(1.-cappa(i,j,k))*log(kapag*delp(i,j,k)/delz(i,j,k)*pt(i,j,k)))
!#else
                  CALL PUSHREALARRAY(pt(i, j, k))
                  pt(i, j, k) = rcp*pt(i, j, k)*EXP(k1k*LOG(kapag*delp(i&
&                   , j, k)/delz(i, j, k)*pt(i, j, k)))
                END DO
              END DO
              CALL PUSHCONTROL2B_FV(1)
            END IF
          ELSE
            CALL PUSHCONTROL2B_FV(2)
          END IF
        ELSE
!#endif
          remap_t = .false.
          CALL PKEZ_FWD(km, is, ie, js, je, j, pe, pk, akap, peln, pkz, &
&                 ptop)
! Compute cp*T + KE
          DO k=1,km
            DO i=is,ie
              CALL PUSHREALARRAY(te(i, j, k))
              te(i, j, k) = 0.25*gridstruct%rsin2(i, j)*(u(i, j, k)**2+u&
&               (i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-(u(i, j, k)&
&               +u(i, j+1, k))*(v(i, j, k)+v(i+1, j, k))*gridstruct%&
&               cosa_s(i, j)) + pt(i, j, k)*pkz(i, j, k)
            END DO
          END DO
          CALL PUSHCONTROL2B_FV(3)
        END IF
        IF (.NOT.hydrostatic) THEN
          IF (hybrid_z) THEN
            CALL PUSHCONTROL2B_FV(2)
          ELSE
!call compute_dz_L32(km, ztop, dz1)
!          else
!               ztop = 18.E3
!               call hybrid_z_dz(km, dz1, ztop, 1.)
!               call mpp_error(FATAL,'==> Error from fv_mapz: hybrid_z works only with L32')
            DO k=1,km
              DO i=is,ie
! ="specific volume"/grav
                CALL PUSHREALARRAY(delz(i, j, k))
                delz(i, j, k) = -(delz(i, j, k)/delp(i, j, k))
              END DO
            END DO
            CALL PUSHCONTROL2B_FV(1)
          END IF
        ELSE
          CALL PUSHCONTROL2B_FV(0)
        END IF
! update ps
        DO i=is,ie
          ps(i, j) = pe1(i, km+1)
        END DO
        IF (hybrid_z) THEN
!--------------------------
! hybrid z_p coordinate
!--------------------------
          DO i=is,ie
            CALL PUSHREALARRAY(ze1(i, km+1))
            ze1(i, km+1) = ze0(i, j, km+1)
          END DO
!!$AD II-LOOP
          DO k=km,1,-1
            DO i=is,ie
! current height
              CALL PUSHREALARRAY(ze1(i, k))
              ze1(i, k) = ze1(i, k+1) - delz(i, j, k)
            END DO
          END DO
!
! Copy ztop; the top layer must be thick enough to prevent numerical problems.
!
          DO i=is,ie
            CALL PUSHREALARRAY(ze2(i, 1))
            ze2(i, 1) = ze1(i, 1)
! Note: ze0 (top) updated
            ze0(i, j, 1) = ze1(i, 1)
          END DO
          CALL PUSHREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
          DO k=2,km+1
            DO i=is,ie
! specified height
              ze2(i, k) = ze0(i, j, k)
            END DO
          END DO
          CALL PUSHREALARRAY(deng, 8*(ie-is+1)*km/8)
! Check/fix monotonicity of top 3 layers thickness
!-----------------------------------------------
!       if ( km==32 ) then
!        do i=is,ie
!           do k=1,3
!              z_rat = (ze2(i,1)-ze2(i,4)) / (dz1(1)+dz1(2)+dz1(3))
!              ze2(i,3) = ze2(i,4) + dz1(3)*z_rat
!              ze2(i,2) = ze2(i,3) + dz1(2)*z_rat
!           enddo
!        enddo
!       endif
!-----------------------------------------------
          DO k=1,km
            DO i=is,ie
! density * grav
              deng(i, k) = -(delp(i, j, k)/delz(i, j, k))
            END DO
          END DO
          IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
            CALL REMAP_Z_FWD(km, ze1, km, ze2, deng, is, ie, 1, &
&                         abs_kord_tm)
            CALL PUSHCONTROL1B_FV(1)
          ELSE
            CALL PUSHREALARRAY(deng, 8*(ie-is+1)*km/8)
            CALL REMAP_Z(km, ze1, km, ze2, deng, is, ie, 1, &
&                  abs_kord_tm_pert)
            CALL PUSHCONTROL1B_FV(0)
          END IF
!-------------
! Update delz
!-------------
          DO k=1,km
            DO i=is,ie
              CALL PUSHREALARRAY(delz(i, j, k))
              delz(i, j, k) = ze2(i, k+1) - ze2(i, k)
            END DO
          END DO
!------------
! update delp
!------------
          DO k=1,km-1
            DO i=is,ie
              CALL PUSHREALARRAY(dp2(i, k))
              dp2(i, k) = -(delz(i, j, k)*deng(i, k))
              CALL PUSHREALARRAY(pe2(i, k+1))
              pe2(i, k+1) = pe2(i, k) + dp2(i, k)
            END DO
          END DO
          DO i=is,ie
! to reduce rounding error
            CALL PUSHREALARRAY(dp2(i, km))
            dp2(i, km) = pe2(i, km+1) - pe2(i, km)
          END DO
          CALL PUSHCONTROL1B_FV(1)
        ELSE
!
! Hybrid sigma-P coordinate:
!
          DO k=2,km
            DO i=is,ie
              CALL PUSHREALARRAY(pe2(i, k))
              pe2(i, k) = ak(k) + bk(k)*pe(i, km+1, j)
            END DO
          END DO
          CALL PUSHREALARRAY(dp2, 8*(ie-is+1)*km/8)
          DO k=1,km
            DO i=is,ie
              dp2(i, k) = pe2(i, k+1) - pe2(i, k)
            END DO
          END DO
          CALL PUSHCONTROL1B_FV(0)
        END IF
!------------
! update delp
!------------
        DO k=1,km
          DO i=is,ie
            CALL PUSHREALARRAY(delp(i, j, k))
            delp(i, j, k) = dp2(i, k)
          END DO
        END DO
!----------------
! Map constituents
!----------------
        IF (nq .GT. 5) THEN
          IF (kord_tr(1) .EQ. kord_tr_pert(1)) THEN
            CALL MAPN_TRACER_FWD(nq, km, km, pe1, pe2, q, dp2, &
&                             kord_tr, j, is, ie, isd, ied, jsd, jed, &
&                             0.0_FVPRC, fill)
            CALL PUSHCONTROL2B_FV(3)
          ELSE
            CALL PUSHREALARRAY(q, 8*(ied-isd+1)*(jed-jsd+1)*km*nq/8&
&                        )
            CALL MAPN_TRACER(nq, km, km, pe1, pe2, q, dp2, kord_tr_pert&
&                      , j, is, ie, isd, ied, jsd, jed, 0.0_FVPRC, fill)
            CALL PUSHCONTROL2B_FV(2)
          END IF
        ELSE IF (nq .GT. 0) THEN
! Remap one tracer at a time
          DO iq=1,nq
            IF (kord_tr(iq) .EQ. kord_tr_pert(iq)) THEN
              CALL MAP1_Q2_FWD(km, pe1, q(isd:ied, jsd:jed, 1:km, iq)&
&                           , km, pe2, q2, dp2, is, ie, 0, kord_tr(iq), &
&                           j, isd, ied, jsd, jed, 0.0_FVPRC)
              CALL PUSHCONTROL1B_FV(1)
            ELSE
              CALL MAP1_Q2(km, pe1, q(isd:ied, jsd:jed, 1:km, iq), km, &
&                    pe2, q2, dp2, is, ie, 0, kord_tr_pert(iq), j, isd, &
&                    ied, jsd, jed, 0.0_FVPRC)
              CALL PUSHCONTROL1B_FV(0)
            END IF
!if (fill) call fillz(ie-is+1, km, 1, q2, dp2)
            DO k=1,km
              DO i=is,ie
                CALL PUSHREALARRAY(q(i, j, k, iq))
                q(i, j, k, iq) = q2(i, k)
              END DO
            END DO
          END DO
          CALL PUSHCONTROL2B_FV(1)
        ELSE
          CALL PUSHCONTROL2B_FV(0)
        END IF
        CALL PUSHREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
!------------------
! Compute p**Kappa
!------------------
        DO k=1,km+1
          DO i=is,ie
            pk1(i, k) = pk(i, j, k)
          END DO
        END DO
        DO i=is,ie
          CALL PUSHREALARRAY(pn2(i, 1))
          pn2(i, 1) = peln(i, 1, j)
          CALL PUSHREALARRAY(pn2(i, km+1))
          pn2(i, km+1) = peln(i, km+1, j)
          CALL PUSHREALARRAY(pk2(i, 1))
          pk2(i, 1) = pk1(i, 1)
          CALL PUSHREALARRAY(pk2(i, km+1))
          pk2(i, km+1) = pk1(i, km+1)
        END DO
        DO k=2,km
          DO i=is,ie
            CALL PUSHREALARRAY(pn2(i, k))
            pn2(i, k) = LOG(pe2(i, k))
            CALL PUSHREALARRAY(pk2(i, k))
            pk2(i, k) = EXP(akap*pn2(i, k))
          END DO
        END DO
        IF (te_map) THEN
!---------------------
! Compute Total Energy
!---------------------
          DO i=is,ie
            CALL PUSHREALARRAY(phis(i, km+1))
            phis(i, km+1) = hs(i, j)
          END DO
!!$AD II-LOOP
          DO k=km,1,-1
            DO i=is,ie
              CALL PUSHREALARRAY(phis(i, k))
              phis(i, k) = phis(i, k+1) + pt(i, j, k)*(pk1(i, k+1)-pk1(i&
&               , k))
            END DO
          END DO
          CALL PUSHREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
          DO k=1,km+1
            DO i=is,ie
              phis(i, k) = phis(i, k)*pe1(i, k)
            END DO
          END DO
          DO k=1,km
            DO i=is,ie
              CALL PUSHREALARRAY(te(i, j, k))
              te(i, j, k) = te(i, j, k) + (phis(i, k+1)-phis(i, k))/(pe1&
&               (i, k+1)-pe1(i, k))
            END DO
          END DO
!----------------
! Map Total Energy
!----------------
          IF (gmao_cubic) THEN
            CALL MAP1_CUBIC_FWD(km, pe1, km, pe2, te, is, ie, j, is, ie&
&                         , js, je, akap, t_var=1, conserv=.true.)
            CALL PUSHCONTROL4B(0)
          ELSE IF (kord_tm .EQ. kord_tm_pert) THEN
            CALL MAP1_PPM_FWD(km, pe1, gz, km, pe2, te, is, ie, j, is&
&                          , ie, js, je, 1, kord_tm)
            CALL PUSHCONTROL4B(1)
          ELSE
            CALL PUSHREALARRAY(te, 8*(ie-is+1)*(je-js+1)*km/8)
            CALL MAP1_PPM(km, pe1, gz, km, pe2, te, is, ie, j, is, ie, &
&                   js, je, 1, kord_tm_pert)
            CALL PUSHCONTROL4B(2)
          END IF
        ELSE IF (remap_t) THEN
!----------------------------------
! Map t using ze1 (hybrid_z) or logp 
!Note: the '1' in remap_z and map1_ppm was a '2' in siena_prerelease. It has been changed to be its value in siena -- 19mar12 lmh
!----------------------------------
          IF (hybrid_z) THEN
            CALL PUSHREALARRAY(deng, 8*(ie-is+1)*km/8)
            DO k=1,km
              DO i=is,ie
                deng(i, k) = pt(i, j, k)
              END DO
            END DO
            IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
              CALL REMAP_Z_FWD(km, ze1, km, ze2, deng, is, ie, 1, &
&                           abs_kord_tm)
              CALL PUSHCONTROL1B_FV(1)
            ELSE
              CALL PUSHREALARRAY(deng, 8*(ie-is+1)*km/8)
              CALL REMAP_Z(km, ze1, km, ze2, deng, is, ie, 1, &
&                    abs_kord_tm_pert)
              CALL PUSHCONTROL1B_FV(0)
            END IF
            DO k=1,km
              DO i=is,ie
                CALL PUSHREALARRAY(pt(i, j, k))
                pt(i, j, k) = deng(i, k)
              END DO
            END DO
            CALL PUSHCONTROL4B(3)
          ELSE IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
            CALL MAP_SCALAR_FWD(km, peln(is:ie, 1:km+1, j), gz, km, &
&                            pn2, pt, is, ie, j, isd, ied, jsd, jed, 1, &
&                            abs_kord_tm, t_min)
            CALL PUSHCONTROL4B(4)
          ELSE
            CALL PUSHREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
            CALL MAP_SCALAR(km, peln(is:ie, 1:km+1, j), gz, km, pn2, pt&
&                     , is, ie, j, isd, ied, jsd, jed, 1, &
&                     abs_kord_tm_pert, t_min)
            CALL PUSHCONTROL4B(5)
          END IF
        ELSE IF (gmao_cubic) THEN
!----------------
! Map pt using pk
!----------------
          CALL MAP1_CUBIC_FWD(km, pe1, km, pe2, pt, is, ie, j, isd, ied&
&                       , jsd, jed, akap, t_var=3, conserv=.false.)
          CALL PUSHCONTROL4B(6)
        ELSE IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
          CALL MAP1_PPM_FWD(km, pk1, gz, km, pk2, pt, is, ie, j, isd&
&                        , ied, jsd, jed, 1, abs_kord_tm)
          CALL PUSHCONTROL4B(7)
        ELSE
          CALL PUSHREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
          CALL MAP1_PPM(km, pk1, gz, km, pk2, pt, is, ie, j, isd, ied, &
&                 jsd, jed, 1, abs_kord_tm_pert)
          CALL PUSHCONTROL4B(8)
        END IF
        IF (.NOT.hydrostatic) THEN
! Remap vertical wind:
          IF (kord_wz .EQ. kord_wz_pert) THEN
            CALL MAP1_PPM_FWD(km, pe1, ws(is:ie, j), km, pe2, w, is, &
&                          ie, j, isd, ied, jsd, jed, -2, kord_wz)
            CALL PUSHCONTROL1B_FV(0)
          ELSE
            CALL PUSHREALARRAY(w, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
            CALL MAP1_PPM(km, pe1, ws(is:ie, j), km, pe2, w, is, ie, j, &
&                   isd, ied, jsd, jed, -2, kord_wz_pert)
            CALL PUSHCONTROL1B_FV(1)
          END IF
          IF (.NOT.hybrid_z) THEN
! Remap delz for hybrid sigma-p coordinate
            IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
              CALL MAP1_PPM_FWD(km, pe1, gz, km, pe2, delz, is, ie, j&
&                            , isd, ied, jsd, jed, 1, abs_kord_tm)
              CALL PUSHCONTROL1B_FV(1)
            ELSE
              CALL PUSHREALARRAY(delz, 8*(ied-isd+1)*(jed-jsd+1)*km&
&                           /8)
              CALL MAP1_PPM(km, pe1, gz, km, pe2, delz, is, ie, j, isd, &
&                     ied, jsd, jed, 1, abs_kord_tm_pert)
              CALL PUSHCONTROL1B_FV(0)
            END IF
            DO k=1,km
              DO i=is,ie
                CALL PUSHREALARRAY(delz(i, j, k))
                delz(i, j, k) = -(delz(i, j, k)*dp2(i, k))
              END DO
            END DO
            CALL PUSHCONTROL2B_FV(2)
          ELSE
            CALL PUSHCONTROL2B_FV(1)
          END IF
        ELSE
          CALL PUSHCONTROL2B_FV(0)
        END IF
!----------
! Update pk
!----------
        DO k=2,km
          DO i=is,ie
            CALL PUSHREALARRAY(pk(i, j, k))
            pk(i, j, k) = pk2(i, k)
          END DO
        END DO
!----------------
        IF (do_omega) THEN
          DO i=is,ie
            CALL PUSHREALARRAY(pe3(i, 1))
            pe3(i, 1) = 0.
          END DO
          CALL PUSHREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
          DO k=2,km+1
            DO i=is,ie
              pe3(i, k) = omga(i, j, k-1)
            END DO
          END DO
          CALL PUSHCONTROL1B_FV(1)
        ELSE
          CALL PUSHCONTROL1B_FV(0)
        END IF
        DO k=1,km+1
          DO i=is,ie
            CALL PUSHREALARRAY(pe0(i, k))
            pe0(i, k) = peln(i, k, j)
            CALL PUSHREALARRAY(peln(i, k, j))
            peln(i, k, j) = pn2(i, k)
          END DO
        END DO
!------------
! Compute pkz
!------------
        IF (hydrostatic) THEN
          DO k=1,km
            DO i=is,ie
              CALL PUSHREALARRAY(pkz(i, j, k))
              pkz(i, j, k) = (pk2(i, k+1)-pk2(i, k))/(akap*(peln(i, k+1&
&               , j)-peln(i, k, j)))
            END DO
          END DO
          CALL PUSHCONTROL2B_FV(0)
        ELSE IF (remap_t) THEN
! Note: pt at this stage is T_v or T_m
          DO k=1,km
            DO i=is,ie
!#ifdef MOIST_CAPPA
!#ifdef USE_NWAT3
!            q_liq = q(i,j,k,liq_wat)
!            q_sol = q(i,j,k,ice_wat)
!#else
!            q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!            q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!            q_con(i,j,k) = q_liq + q_sol
!!           cappa(i,j,k) = rdgas/(rdgas+((1.-q(i,j,k,sphum)-q_con(i,j,k))*cv_air + q(i,j,k,sphum)*cv_vap +  &
!!                                            q_liq*c_liq + q_sol*c_ice)/(1.+r_vir*q(i,j,k,sphum)))
!            cvm = (1.-(q(i,j,k,sphum)+q_con(i,j,k)))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!            cappa(i,j,k) = rdgas/(rdgas + cvm*(1.-q_con(i,j,k))/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k)))
!            pkz(i,j,k) = exp(cappa(i,j,k)*log(rrg*delp(i,j,k)/delz(i,j,k)*pt(i,j,k)))
!#else
              CALL PUSHREALARRAY(pkz(i, j, k))
              pkz(i, j, k) = EXP(akap*LOG(rrg*delp(i, j, k)/delz(i, j, k&
&               )*pt(i, j, k)))
            END DO
          END DO
          CALL PUSHCONTROL2B_FV(1)
        ELSE
!#endif
! Note: pt at this stage is cp*Theta_v
          DO k=1,km
            DO i=is,ie
!#ifdef MOIST_CAPPA
!            pkz(i,j,k) = exp(cappa(i,j,k)/(1.-cappa(i,j,k))*log(kapag*delp(i,j,k)/delz(i,j,k)*pt(i,j,k)) )
!#else
              CALL PUSHREALARRAY(pkz(i, j, k))
              pkz(i, j, k) = EXP(k1k*LOG(kapag*delp(i, j, k)/delz(i, j, &
&               k)*pt(i, j, k)))
            END DO
          END DO
          CALL PUSHCONTROL2B_FV(2)
        END IF
!#endif
! end do_omega
! Interpolate omega/pe3 (defined at pe0) to remapped cell center (dp2)
        IF (do_omega) THEN
          CALL PUSHREALARRAY(dp2, 8*(ie-is+1)*km/8)
          DO k=1,km
            DO i=is,ie
              dp2(i, k) = 0.5*(peln(i, k, j)+peln(i, k+1, j))
            END DO
          END DO
          DO i=is,ie
            k_next = 1
            DO 110 n=1,km
              kp = k_next
              CALL PUSHINTEGER(k)
              ad_count = 1
              DO k=kp,km
                IF (dp2(i, n) .LE. pe0(i, k+1) .AND. dp2(i, n) .GE. pe0(&
&                   i, k)) THEN
                  GOTO 100
                ELSE
                  CALL PUSHINTEGER(k)
                  ad_count = ad_count + 1
                END IF
              END DO
              CALL PUSHCONTROL1B_FV(0)
              CALL PUSHINTEGER(ad_count)
              CALL PUSHCONTROL1B_FV(1)
              GOTO 110
 100          CALL PUSHCONTROL1B_FV(1)
              CALL PUSHINTEGER(ad_count)
              CALL PUSHREALARRAY(omga(i, j, n))
              omga(i, j, n) = pe3(i, k) + (pe3(i, k+1)-pe3(i, k))*(dp2(i&
&               , n)-pe0(i, k))/(pe0(i, k+1)-pe0(i, k))
              k_next = k
              CALL PUSHCONTROL1B_FV(0)
 110        CONTINUE
          END DO
          CALL PUSHCONTROL2B_FV(0)
        ELSE
          CALL PUSHCONTROL2B_FV(1)
        END IF
      ELSE
        CALL PUSHCONTROL2B_FV(2)
      END IF
! end hybrid_z check
      IF (.NOT.hybrid_z) THEN
        DO i=is,ie+1
          CALL PUSHREALARRAY(pe0(i, 1))
          pe0(i, 1) = pe(i, 1, j)
        END DO
        CALL PUSHINTEGER(k)
        CALL PUSHREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
!------
! map u
!------
        DO k=2,km+1
          DO i=is,ie
            pe0(i, k) = 0.5*(pe(i, k, j-1)+pe1(i, k))
          END DO
        END DO
        DO k=1,km+1
          CALL PUSHREALARRAY(bkh)
          bkh = 0.5*bk(k)
          DO i=is,ie
            CALL PUSHREALARRAY(pe3(i, k))
            pe3(i, k) = ak(k) + bkh*(pe(i, km+1, j-1)+pe1(i, km+1))
          END DO
        END DO
        IF (kord_mt .EQ. kord_mt_pert) THEN
          CALL MAP1_PPM_FWD(km, pe0(is:ie, 1:km+1), gz, km, pe3(is:ie&
&                        , 1:km+1), u, is, ie, j, isd, ied, jsd, jedp1, &
&                        -1, kord_mt)
          CALL PUSHCONTROL1B_FV(0)
        ELSE
          CALL PUSHREALARRAY(u, 8*(ied-isd+1)*(jed-jsd+2)*km/8)
          CALL MAP1_PPM(km, pe0(is:ie, 1:km+1), gz, km, pe3(is:ie, 1:km+&
&                 1), u, is, ie, j, isd, ied, jsd, jedp1, -1, &
&                 kord_mt_pert)
          CALL PUSHCONTROL1B_FV(1)
        END IF
! (j < je+1)
        IF (j .LT. je + 1) THEN
!------
! map v
!------
          DO i=is,ie+1
            CALL PUSHREALARRAY(pe3(i, 1))
            pe3(i, 1) = ak(1)
          END DO
          DO k=2,km+1
            CALL PUSHREALARRAY(bkh)
            bkh = 0.5*bk(k)
            DO i=is,ie+1
              CALL PUSHREALARRAY(pe0(i, k))
              pe0(i, k) = 0.5*(pe(i-1, k, j)+pe(i, k, j))
              CALL PUSHREALARRAY(pe3(i, k))
              pe3(i, k) = ak(k) + bkh*(pe(i-1, km+1, j)+pe(i, km+1, j))
            END DO
          END DO
          IF (kord_mt .EQ. kord_mt_pert) THEN
            CALL MAP1_PPM_FWD(km, pe0, gz, km, pe3, v, is, iep1, j, &
&                          isd, iedp1, jsd, jed, -1, kord_mt)
            CALL PUSHCONTROL2B_FV(3)
          ELSE
            CALL PUSHREALARRAY(v, 8*(ied-isd+2)*(jed-jsd+1)*km/8)
            CALL MAP1_PPM(km, pe0, gz, km, pe3, v, is, iep1, j, isd, &
&                   iedp1, jsd, jed, -1, kord_mt_pert)
            CALL PUSHCONTROL2B_FV(2)
          END IF
        ELSE
          CALL PUSHCONTROL2B_FV(1)
        END IF
      ELSE
        CALL PUSHCONTROL2B_FV(0)
      END IF
      CALL PUSHINTEGER(k)
      DO k=1,km
        DO i=is,ie
          CALL PUSHREALARRAY(ua(i, j, k))
          ua(i, j, k) = pe2(i, k+1)
        END DO
      END DO
    END DO
    IF (hybrid_z) THEN
!------- Hybrid_z section ---------------
      IF (gridstruct%square_domain) THEN
        CALL PUSHREALARRAY(ua, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
        CALL MPP_UPDATE_DOMAINS(ua, domain, whalo=1, ehalo=1, shalo=1, &
&                         nhalo=1, complete=.true.)
        CALL PUSHCONTROL1B_FV(1)
      ELSE
        CALL PUSHREALARRAY(ua, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
        CALL MPP_UPDATE_DOMAINS(ua, domain, complete=.true.)
        CALL PUSHCONTROL1B_FV(0)
      END IF
! u-wind
!$OMP parallel do default(none) shared(is,ie,js,je,km,ptop,u,gz,pe,ua,isd,ied,jsd,jed,kord_mt) &
!$OMP                          private(pe1, pe2)
      DO j=js,je+1
        DO i=is,ie
          CALL PUSHREALARRAY(pe1(i, 1))
          pe1(i, 1) = ptop
          CALL PUSHREALARRAY(pe2(i, 1))
          pe2(i, 1) = ptop
        END DO
        DO k=2,km+1
          DO i=is,ie
            CALL PUSHREALARRAY(pe1(i, k))
            pe1(i, k) = 0.5*(pe(i, k, j-1)+pe(i, k, j))
            CALL PUSHREALARRAY(pe2(i, k))
            pe2(i, k) = 0.5*(ua(i, j-1, k-1)+ua(i, j, k-1))
          END DO
        END DO
        IF (kord_mt .EQ. kord_mt_pert) THEN
          CALL MAP1_PPM_FWD(km, pe1, gz, km, pe2, u, is, ie, j, isd, &
&                        ied, jsd, jedp1, -1, kord_mt)
          CALL PUSHCONTROL1B_FV(1)
        ELSE
          CALL PUSHREALARRAY(u, 8*(ied-isd+1)*(jed-jsd+2)*km/8)
          CALL MAP1_PPM(km, pe1, gz, km, pe2, u, is, ie, j, isd, ied, &
&                 jsd, jedp1, -1, kord_mt_pert)
          CALL PUSHCONTROL1B_FV(0)
        END IF
      END DO
! v-wind
!$OMP parallel do default(none) shared(is,ie,js,je,ptop,pe,ua,km,v,gz,isd,ied,jsd,jed,kord_mt) &
!$OMP                          private(pe0, pe3)
      DO j=js,je
        DO i=is,ie+1
          CALL PUSHREALARRAY(pe0(i, 1))
          pe0(i, 1) = ptop
          CALL PUSHREALARRAY(pe3(i, 1))
          pe3(i, 1) = ptop
        END DO
        DO k=2,km+1
          DO i=is,ie+1
            CALL PUSHREALARRAY(pe0(i, k))
            pe0(i, k) = 0.5*(pe(i-1, k, j)+pe(i, k, j))
            CALL PUSHREALARRAY(pe3(i, k))
            pe3(i, k) = 0.5*(ua(i-1, j, k-1)+ua(i, j, k-1))
          END DO
        END DO
        IF (kord_mt .EQ. kord_mt_pert) THEN
          CALL MAP1_PPM_FWD(km, pe0, gz, km, pe3, v, is, iep1, j, isd&
&                        , iedp1, jsd, jed, -1, kord_mt)
          CALL PUSHCONTROL1B_FV(1)
        ELSE
          CALL PUSHREALARRAY(v, 8*(ied-isd+2)*(jed-jsd+1)*km/8)
          CALL MAP1_PPM(km, pe0, gz, km, pe3, v, is, iep1, j, isd, iedp1&
&                 , jsd, jed, -1, kord_mt_pert)
          CALL PUSHCONTROL1B_FV(0)
        END IF
      END DO
      CALL PUSHCONTROL1B_FV(1)
    ELSE
      CALL PUSHCONTROL1B_FV(0)
    END IF
    CALL PUSHREALARRAY(pe, 8*(ie-is+3)*(km+1)*(je-js+3)/8)
!------------- Hybrid_z section ----------------------
!$OMP parallel do default(none) shared(is,ie,js,je,km,pe,ua)
    DO k=2,km
      DO j=js,je
        DO i=is,ie
          pe(i, k, j) = ua(i, j, k-1)
        END DO
      END DO
    END DO
! end consv check
!  call cubed_to_latlon(u,  v, ua, va, dx, dy, rdxa, rdya, km, 1, flagstruct%c2l_ord)
    IF (last_step .AND. consv .GT. 0. .AND. (.NOT.do_adiabatic_init)) &
&   THEN
      IF (te_map) THEN
!$OMP parallel do default(none) shared(is,ie,js,je,km,te_2d,te,delp)
        DO j=js,je
          DO i=is,ie
            te_2d(i, j) = te(i, j, 1)*delp(i, j, 1)
          END DO
          DO k=2,km
            DO i=is,ie
              te_2d(i, j) = te_2d(i, j) + te(i, j, k)*delp(i, j, k)
            END DO
          END DO
        END DO
        CALL PUSHCONTROL1B_FV(1)
      ELSE
!$OMP parallel do default(none) shared(is,ie,js,je,km,remap_t,hydrostatic,hs,rg,pt,peln, &
!$OMP                                  te_2d,pe,delp,cp,rsin2,u,v,cosa_s,delz,rainwat,   &
!$OMP                                  liq_wat,ice_wat,snowwat,graupel,q_con,r_vir,      &
!$OMP                                  sphum,w,pk,pkz,rgama)                             &
!$OMP                          private(q_liq, q_sol, cvm, gz, phis)
        DO j=js,je
          IF (remap_t) THEN
            IF (hydrostatic) THEN
              DO i=is,ie
                CALL PUSHREALARRAY(gz(i))
                gz(i) = hs(i, j)
                DO k=1,km
                  gz(i) = gz(i) + rg*pt(i, j, k)*(peln(i, k+1, j)-peln(i&
&                   , k, j))
                END DO
              END DO
              DO i=is,ie
                te_2d(i, j) = pe(i, km+1, j)*hs(i, j) - pe(i, 1, j)*gz(i&
&                 )
              END DO
              DO k=1,km
                DO i=is,ie
                  te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cp*pt(i, j&
&                   , k)+0.25*gridstruct%rsin2(i, j)*(u(i, j, k)**2+u(i&
&                   , j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-(u(i, j, &
&                   k)+u(i, j+1, k))*(v(i, j, k)+v(i+1, j, k))*&
&                   gridstruct%cosa_s(i, j)))
                END DO
              END DO
              CALL PUSHCONTROL2B_FV(3)
            ELSE
! non-hydrostatic & remap_t
              DO i=is,ie
                CALL PUSHREALARRAY(phis(i, km+1))
                phis(i, km+1) = hs(i, j)
                DO k=km,1,-1
                  CALL PUSHREALARRAY(phis(i, k))
                  phis(i, k) = phis(i, k+1) - grav*delz(i, j, k)
                END DO
              END DO
              DO i=is,ie
                te_2d(i, j) = 0.
              END DO
              DO k=1,km
                DO i=is,ie
! KE using 3D winds:
!#ifdef USE_COND
!#ifdef USE_NWAT3
!           q_liq = q(i,j,k,liq_wat)
!           q_sol = q(i,j,k,ice_wat)
!!#else
!           q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!           q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!           q_con(i,j,k) = q_liq + q_sol
!#ifdef MOIST_CAPPA
!           cvm = (1.-(q(i,j,k,sphum)+q_con(i,j,k)))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!#else
!           cvm = cv_air
!#endif
!                 te_2d(i,j) = te_2d(i,j) + delp(i,j,k)*(cvm*pt(i,j,k)/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k)) + &
!#else
!#endif
                  te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cv_air*pt(i&
&                   , j, k)/(1.+r_vir*q(i, j, k, sphum))+0.5*(phis(i, k)&
&                   +phis(i, k+1)+w(i, j, k)**2+0.5*gridstruct%rsin2(i, &
&                   j)*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+&
&                   1, j, k)**2-(u(i, j, k)+u(i, j+1, k))*(v(i, j, k)+v(&
&                   i+1, j, k))*gridstruct%cosa_s(i, j))))
                END DO
              END DO
              CALL PUSHCONTROL2B_FV(2)
            END IF
          ELSE IF (hydrostatic) THEN
            DO i=is,ie
              CALL PUSHREALARRAY(gz(i))
              gz(i) = hs(i, j)
              DO k=1,km
                gz(i) = gz(i) + pt(i, j, k)*(pk(i, j, k+1)-pk(i, j, k))
              END DO
            END DO
            DO i=is,ie
              te_2d(i, j) = pe(i, km+1, j)*hs(i, j) - pe(i, 1, j)*gz(i)
            END DO
            DO k=1,km
              DO i=is,ie
                te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(pt(i, j, k)*&
&                 pkz(i, j, k)+0.25*gridstruct%rsin2(i, j)*(u(i, j, k)**&
&                 2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-(u(i, &
&                 j, k)+u(i, j+1, k))*(v(i, j, k)+v(i+1, j, k))*&
&                 gridstruct%cosa_s(i, j)))
              END DO
            END DO
            CALL PUSHCONTROL2B_FV(1)
          ELSE
!-----------------
! Non-hydrostatic:
!-----------------
            DO i=is,ie
              CALL PUSHREALARRAY(phis(i, km+1))
              phis(i, km+1) = hs(i, j)
              DO k=km,1,-1
                CALL PUSHREALARRAY(phis(i, k))
                phis(i, k) = phis(i, k+1) - grav*delz(i, j, k)
              END DO
            END DO
            DO i=is,ie
              te_2d(i, j) = 0.
            END DO
            DO k=1,km
              DO i=is,ie
! KE using 3D winds:
!#ifdef USE_COND
!#ifdef USE_NWAT3
!                 q_con(i,j,k) = q(i,j,k,liq_wat) + q(i,j,k,ice_wat)
!#else
!                 q_con(i,j,k) = q(i,j,k,liq_wat) + q(i,j,k,rainwat) + q(i,j,k,ice_wat)  &
!                              + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!                 te_2d(i,j) = te_2d(i,j) + delp(i,j,k)*(rgama*pt(i,j,k)*pkz(i,j,k)/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k)) + &
!#else
!#endif
                te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(rgama*pt(i, j&
&                 , k)*pkz(i, j, k)/(1.+r_vir*q(i, j, k, sphum))+0.5*(&
&                 phis(i, k)+phis(i, k+1)+w(i, j, k)**2+0.5*gridstruct%&
&                 rsin2(i, j)*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)&
&                 **2+v(i+1, j, k)**2-(u(i, j, k)+u(i, j+1, k))*(v(i, j&
&                 , k)+v(i+1, j, k))*gridstruct%cosa_s(i, j))))
              END DO
            END DO
            CALL PUSHCONTROL2B_FV(0)
          END IF
        END DO
        CALL PUSHCONTROL1B_FV(0)
      END IF
! openMP j-loop
!$OMP parallel do default(none) shared(is,ie,js,je,zsum1,pkz,delp,km,zsum0,ptop,pk,te_2d,te0_2d)
      DO j=js,je
        DO i=is,ie
          zsum1(i, j) = pkz(i, j, 1)*delp(i, j, 1)
        END DO
        DO k=2,km
          DO i=is,ie
            zsum1(i, j) = zsum1(i, j) + pkz(i, j, k)*delp(i, j, k)
          END DO
        END DO
        DO i=is,ie
          zsum0(i, j) = ptop*(pk(i, j, 1)-pk(i, j, km+1)) + zsum1(i, j)
          te_2d(i, j) = te0_2d(i, j) - te_2d(i, j)
        END DO
      END DO
      result1 = G_SUM(domain, te_2d, is, ie, js, je, ng, gridstruct%&
&       area_64, 0)
      tpe = consv*result1
! unit: W/m**2
! Note pdt is "phys" time step
      IF (hydrostatic) THEN
        result1 = G_SUM(domain, zsum0, is, ie, js, je, ng, gridstruct%&
&         area_64, 0)
        dtmp = tpe/(cp*result1)
        CALL PUSHCONTROL1B_FV(0)
      ELSE
        result1 = G_SUM(domain, zsum1, is, ie, js, je, ng, gridstruct%&
&         area_64, 0)
        dtmp = tpe/(cv_air*result1)
        CALL PUSHCONTROL1B_FV(1)
      END IF
!-------------------------------------------------------------------------------
! One may use this quick fix to ensure reproducibility at the expense of a lower
! floating precision; this is fine for the TE correction
!-------------------------------------------------------------------------------
! convert to 4-byte real
      IF (reproduce_sum) THEN
        CALL PUSHCONTROL1B_FV(0)
        dtmp = REAL(dtmp, 4)
      ELSE
        CALL PUSHCONTROL1B_FV(0)
      END IF
    ELSE
      dtmp = 0.
!#ifdef USE_COND
!!$OMP parallel do default(none) shared(is,ie,js,je,km,q_con,q,liq_wat,ice_wat, &
!!$OMP                                  rainwat,snowwat,graupel)
!      do k=1,km
!         do j=js,je
!         do i=is,ie
!#ifdef USE_NWAT3
!            q_con(i,j,k) = q(i,j,k,liq_wat)+q(i,j,k,ice_wat)
!#else
!            q_con(i,j,k) = q(i,j,k,liq_wat)+q(i,j,k,rainwat)+q(i,j,k,ice_wat)+q(i,j,k,snowwat)+q(i,j,k,graupel)
!#endif
!         enddo
!         enddo
!      enddo
!#endif
      CALL PUSHCONTROL1B_FV(1)
    END IF
    IF (te_map) THEN
!kord_mt >= 0
!$OMP parallel do default(none) shared(is,ie,js,je,km,hs,te,rsin2,u,v,cosa_s,rg,peln, &
!$OMP                                  cp,pe,delp,pt,pkz,dtmp) &
!$OMP                          private(gz, tpe, tmp, dlnp)
      DO j=js,je
        DO i=is,ie
          CALL PUSHREALARRAY(gz(i))
          gz(i) = hs(i, j)
        END DO
        DO k=km,1,-1
          DO i=is,ie
            CALL PUSHREALARRAY(tpe)
            tpe = te(i, j, k) - gz(i) - 0.25*gridstruct%rsin2(i, j)*(u(i&
&             , j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-(&
&             u(i, j, k)+u(i, j+1, k))*(v(i, j, k)+v(i+1, j, k))*&
&             gridstruct%cosa_s(i, j))
            dlnp = rg*(peln(i, k+1, j)-peln(i, k, j))
            tmp = tpe/(cp-pe(i, k, j)*dlnp/delp(i, j, k))
            CALL PUSHREALARRAY(pt(i, j, k))
            pt(i, j, k) = cp*(tmp/pkz(i, j, k)+dtmp)
            CALL PUSHREALARRAY(gz(i))
            gz(i) = gz(i) + dlnp*tmp
          END DO
        END DO
      END DO
      CALL PUSHREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
      CALL PUSHREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(pe2, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
      CALL PUSHREALARRAY(gz, 8*(ie-is+1)/8)
      CALL PUSHREALARRAY(ze1, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(dp2, 8*(ie-is+1)*km/8)
      CALL PUSHREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHINTEGER(iep1)
      CALL PUSHREALARRAY(rg)
      CALL PUSHREALARRAY(tpe)
      CALL PUSHREALARRAY(rgama)
      CALL PUSHINTEGER(abs_kord_tm_pert)
      CALL PUSHREALARRAY(te_2d, 8*(ie-is+1)*(je-js+1)/8)
      CALL PUSHREALARRAY(deng, 8*(ie-is+1)*km/8)
      CALL PUSHREALARRAY(result1)
      CALL PUSHREALARRAY(zsum0, 8*(ie-is+1)*(je-js+1)/8)
      CALL PUSHREALARRAY(pn2, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(zsum1, 8*(ie-is+1)*(je-js+1)/8)
      CALL PUSHREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(bkh)
      CALL PUSHREALARRAY(pk2, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHCONTROL3B_FV(0)
    ELSE IF (remap_t) THEN
! end k-loop
!#ifndef MAPL_MODE
!        ! Note: pt at this stage is T_v
!        if ( .not. do_adiabatic_init ) then
!#ifdef FAST_LIN_MP
!! Notes: must set consv=0
!                                           call timing_on('FAST_LIN_MP')
!      call lin_fast_mp(u, v, w, pt, q, sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, cld_amt,  & 
!                       delp, delz, gridstruct, mdt, lprec, fprec, f_land,      &
!                       hydrostatic, phys_hydrostatic, time)
!                                           call timing_off('FAST_LIN_MP')
!#else
!          if ( do_sat_adj ) then
!                                           call timing_on('sat_adj2')
!!$OMP parallel do default(none) shared(is,ie,js,je,km,peln,ng,mdt,hydrostatic,consv,te, &
!!$OMP                                  q,isd,jsd,sphum,liq_wat,ice_wat,rainwat,snowwat, &
!!$OMP                                  graupel,cld_amt,gridstruct,delz,pt,delp,         &
!!$OMP                                  q_con,cappa,dtdt,out_dt,last_step,pkz,rrg,akap ) &
!!$OMP                          private(dpeln)
!           do k=1,km
!              do j=js,je
!                 do i=is,ie
!                    dpeln(i,j) = peln(i,k+1,j) - peln(i,k,j)
!                 enddo
!              enddo
!              call sat_adj2(mdt, is, ie, js, je, ng, hydrostatic, consv>0., &
!                            te(is,js,k), q(isd,jsd,k,sphum), q(isd,jsd,k,liq_wat),   &
!                            q(isd,jsd,k,ice_wat), q(isd,jsd,k,rainwat),    &
!                            q(isd,jsd,k,snowwat), q(isd,jsd,k,graupel), q(isd,jsd,k,cld_amt), gridstruct%area(isd,jsd), &
!                            dpeln, delz(isd:,jsd:,k), pt(isd,jsd,k), delp(isd,jsd,k),           &
!                            q_con(isd:,jsd:,k), cappa(isd:,jsd:,k), dtdt(is,js,k), out_dt, last_step)
!              if ( .not. hydrostatic  ) then
!                 do j=js,je
!                    do i=is,ie
!#ifdef MOIST_CAPPA
!                       pkz(i,j,k) = exp(cappa(i,j,k)*log(rrg*delp(i,j,k)/delz(i,j,k)*pt(i,j,k)))
!#else
!                       pkz(i,j,k) = exp(akap*log(rrg*delp(i,j,k)/delz(i,j,k)*pt(i,j,k)))
!#endif
!                    enddo
!                 enddo
!              endif
!           enddo    ! OpenMP k-loop
!           if ( consv > 0. ) then
!!$OMP parallel do default(none) shared(is,ie,js,je,km,te_2d,te,te0_2d)
!                do j=js,je
!                   do k=1,km
!                      do i=is,ie
!                         te0_2d(i,j) = te0_2d(i,j) + te(i,j,k)
!                      enddo
!                   enddo
!                enddo
!           endif
!                                           call timing_off('sat_adj2')
!         endif
!#endif
!        endif
!#endif
      IF (last_step) THEN
        IF (hydrostatic) THEN
          CALL PUSHREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
!$OMP parallel do default(none) shared(is,ie,js,je,km,pt,dtmp,pkz,r_vir,q,sphum)
          DO k=1,km
            DO j=js,je
              DO i=is,ie
                pt(i, j, k) = (pt(i, j, k)+dtmp*pkz(i, j, k))/(1.+r_vir*&
&                 q(i, j, k, sphum))
              END DO
            END DO
          END DO
          CALL PUSHREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
          CALL PUSHREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(pe2, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
          CALL PUSHREALARRAY(gz, 8*(ie-is+1)/8)
          CALL PUSHREALARRAY(ze1, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(dp2, 8*(ie-is+1)*km/8)
          CALL PUSHREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHINTEGER(iep1)
          CALL PUSHREALARRAY(tpe)
          CALL PUSHREALARRAY(rgama)
          CALL PUSHREALARRAY(dtmp)
          CALL PUSHINTEGER(abs_kord_tm_pert)
          CALL PUSHREALARRAY(te_2d, 8*(ie-is+1)*(je-js+1)/8)
          CALL PUSHREALARRAY(deng, 8*(ie-is+1)*km/8)
          CALL PUSHREALARRAY(result1)
          CALL PUSHREALARRAY(zsum0, 8*(ie-is+1)*(je-js+1)/8)
          CALL PUSHREALARRAY(pn2, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(zsum1, 8*(ie-is+1)*(je-js+1)/8)
          CALL PUSHREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(bkh)
          CALL PUSHREALARRAY(pk2, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHCONTROL3B_FV(1)
        ELSE
          CALL PUSHREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
!$OMP parallel do default(none) shared(is,ie,js,je,km,q,liq_wat,ice_wat,rainwat,snowwat, &
!$OMP                                  graupel,q_con,pt,dtmp,pkz,r_vir,sphum )           &
!$OMP                          private(q_liq, q_sol, cvm)
          DO k=1,km
            DO j=js,je
              DO i=is,ie
! Output temperature if last_step
!#ifdef USE_COND
!#ifdef MOIST_CAPPA
!#ifdef USE_NWAT3
!           q_liq = q(i,j,k,liq_wat)
!           q_sol = q(i,j,k,ice_wat)
!#else
!           q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!           q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!           cvm = (1.-(q(i,j,k,sphum)+q_con(i,j,k)))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!
!                    pt(i,j,k) = (pt(i,j,k)+dtmp*cv_air/cvm*pkz(i,j,k))/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k))
!#else
!                    pt(i,j,k) = (pt(i,j,k)+dtmp*pkz(i,j,k))/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k))
!#endif
!#else
                pt(i, j, k) = (pt(i, j, k)+dtmp*pkz(i, j, k))/(1.+r_vir*&
&                 q(i, j, k, sphum))
              END DO
            END DO
          END DO
          CALL PUSHREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
          CALL PUSHREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(pe2, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
          CALL PUSHREALARRAY(gz, 8*(ie-is+1)/8)
          CALL PUSHREALARRAY(ze1, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(dp2, 8*(ie-is+1)*km/8)
          CALL PUSHREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHINTEGER(iep1)
          CALL PUSHREALARRAY(tpe)
          CALL PUSHREALARRAY(rgama)
          CALL PUSHREALARRAY(dtmp)
          CALL PUSHINTEGER(abs_kord_tm_pert)
          CALL PUSHREALARRAY(te_2d, 8*(ie-is+1)*(je-js+1)/8)
          CALL PUSHREALARRAY(deng, 8*(ie-is+1)*km/8)
          CALL PUSHREALARRAY(result1)
          CALL PUSHREALARRAY(zsum0, 8*(ie-is+1)*(je-js+1)/8)
          CALL PUSHREALARRAY(pn2, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(zsum1, 8*(ie-is+1)*(je-js+1)/8)
          CALL PUSHREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHREALARRAY(bkh)
          CALL PUSHREALARRAY(pk2, 8*(ie-is+1)*(km+1)/8)
          CALL PUSHCONTROL3B_FV(2)
        END IF
      ELSE
        CALL PUSHREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
!#endif
!$OMP parallel do default(none) shared(is,ie,js,je,km,pt,cp,pkz,dtmp,q,liq_wat,ice_wat, &
!$OMP                                  rainwat,snowwat,graupel,sphum,q_con) &
!$OMP                          private(q_liq, q_sol, cvm)
        DO k=1,km
          DO j=js,je
            DO i=is,ie
!#ifdef MOIST_CAPPA
!#ifdef USE_NWAT3
!           q_liq = q(i,j,k,liq_wat)
!           q_sol = q(i,j,k,ice_wat)
!#else
!           q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!           q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!           cvm = (1.-(q(i,j,k,sphum)+q_con(i,j,k)))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!                    pt(i,j,k) = cp*(pt(i,j,k)/pkz(i,j,k) + dtmp*cv_air/cvm)
!#else
              pt(i, j, k) = cp*(pt(i, j, k)/pkz(i, j, k)+dtmp)
            END DO
          END DO
        END DO
        CALL PUSHREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
        CALL PUSHREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
        CALL PUSHREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
        CALL PUSHREALARRAY(pe2, 8*(ie-is+1)*(km+1)/8)
        CALL PUSHREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
        CALL PUSHREALARRAY(gz, 8*(ie-is+1)/8)
        CALL PUSHREALARRAY(ze1, 8*(ie-is+1)*(km+1)/8)
        CALL PUSHREALARRAY(dp2, 8*(ie-is+1)*km/8)
        CALL PUSHREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
        CALL PUSHINTEGER(iep1)
        CALL PUSHREALARRAY(tpe)
        CALL PUSHREALARRAY(rgama)
        CALL PUSHINTEGER(abs_kord_tm_pert)
        CALL PUSHREALARRAY(te_2d, 8*(ie-is+1)*(je-js+1)/8)
        CALL PUSHREALARRAY(deng, 8*(ie-is+1)*km/8)
        CALL PUSHREALARRAY(result1)
        CALL PUSHREALARRAY(zsum0, 8*(ie-is+1)*(je-js+1)/8)
        CALL PUSHREALARRAY(pn2, 8*(ie-is+1)*(km+1)/8)
        CALL PUSHREALARRAY(zsum1, 8*(ie-is+1)*(je-js+1)/8)
        CALL PUSHREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
        CALL PUSHREALARRAY(bkh)
        CALL PUSHREALARRAY(pk2, 8*(ie-is+1)*(km+1)/8)
        CALL PUSHCONTROL3B_FV(3)
      END IF
    ELSE
      CALL PUSHREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
!#endif
!$OMP parallel do default(none) shared(is,ie,js,je,km,pt,cp,dtmp)
      DO k=1,km
        DO j=js,je
          DO i=is,ie
            pt(i, j, k) = pt(i, j, k) + cp*dtmp
          END DO
        END DO
      END DO
      CALL PUSHREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
      CALL PUSHREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(pe2, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
      CALL PUSHREALARRAY(gz, 8*(ie-is+1)/8)
      CALL PUSHREALARRAY(ze1, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(dp2, 8*(ie-is+1)*km/8)
      CALL PUSHREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHINTEGER(iep1)
      CALL PUSHREALARRAY(tpe)
      CALL PUSHREALARRAY(rgama)
      CALL PUSHINTEGER(abs_kord_tm_pert)
      CALL PUSHREALARRAY(te_2d, 8*(ie-is+1)*(je-js+1)/8)
      CALL PUSHREALARRAY(deng, 8*(ie-is+1)*km/8)
      CALL PUSHREALARRAY(result1)
      CALL PUSHREALARRAY(zsum0, 8*(ie-is+1)*(je-js+1)/8)
      CALL PUSHREALARRAY(pn2, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(zsum1, 8*(ie-is+1)*(je-js+1)/8)
      CALL PUSHREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(bkh)
      CALL PUSHREALARRAY(pk2, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHCONTROL3B_FV(4)
    END IF
  END SUBROUTINE LAGRANGIAN_TO_EULERIAN_FWD
!  Differentiation of lagrangian_to_eulerian in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_or
!d2 a2b_edge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn
!_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_gr
!ad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_d
!ynamics_mod.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_uti
!ls_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.comp
!ute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_trac
!er_fb fv_mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_map
!z_mod.steepz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_
!mod.riem_solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1
!_solver nh_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_
!mod.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4cor
!ners sw_core_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2
!d_fb tp_core_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxp
!pm_fb tp_core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: ws peln q u v w ze0 delp ua
!                delz omga te0_2d pkz pe pk ps pt te
!   with respect to varying inputs: ws peln q u v w ze0 delp ua
!                delz omga te0_2d pkz pe pk ps pt te
!---- version number -----
!  character(len=128) :: version = '$Id: fv_mapz.F90,v 1.1.2.2.2.2.30.1.22.2.6.2.34.5.4.4.2.2.2.1.4.2.2.2 2017/02/21 20:43:38 bma
!uer Exp $'
!  character(len=128) :: tagname = '$Name: Heracles-UNSTABLE_ncepdyn_Feb222017 $'
  SUBROUTINE LAGRANGIAN_TO_EULERIAN_BWD(last_step, consv, ps, ps_ad, pe&
&   , pe_ad, delp, delp_ad, pkz, pkz_ad, pk, pk_ad, mdt, pdt, km, is, ie&
&   , js, je, isd, ied, jsd, jed, nq, nwat, sphum, q_con, u, u_ad, v, &
&   v_ad, w, w_ad, delz, delz_ad, pt, pt_ad, q, q_ad, hs, r_vir, cp, &
&   akap, cappa, kord_mt, kord_wz, kord_tr, kord_tm, kord_mt_pert, &
&   kord_wz_pert, kord_tr_pert, kord_tm_pert, peln, peln_ad, te0_2d, &
&   te0_2d_ad, ng, ua, ua_ad, va, omga, omga_ad, te, te_ad, ws, ws_ad, &
&   fill, reproduce_sum, out_dt, dtdt, ptop, ak, bk, gridstruct, domain&
&   , ze0, ze0_ad, gmao_cubic, remap_t, do_sat_adj, hydrostatic, &
&   hybrid_z, do_omega, do_adiabatic_init)
    IMPLICIT NONE
! call cubed_to_latlon(u, v, ua, va, dx, dy, rdxa, rdya, km, 1, flagstruct%c2l_ord)
!  nullify(cosa_s)
!  nullify(rsin2)
    LOGICAL, INTENT(IN) :: last_step
    REAL(fvprc), INTENT(IN) :: mdt
    REAL(fvprc), INTENT(IN) :: pdt
    INTEGER, INTENT(IN) :: km
    INTEGER, INTENT(IN) :: nq
    INTEGER, INTENT(IN) :: nwat
    INTEGER, INTENT(IN) :: sphum
    INTEGER, INTENT(IN) :: ng
    INTEGER, INTENT(IN) :: is, ie, isd, ied
    INTEGER, INTENT(IN) :: js, je, jsd, jed
    INTEGER, INTENT(IN) :: kord_mt
    INTEGER, INTENT(IN) :: kord_wz
    INTEGER, INTENT(IN) :: kord_tr(nq)
    INTEGER, INTENT(IN) :: kord_tm
    INTEGER, INTENT(IN) :: kord_mt_pert
    INTEGER, INTENT(IN) :: kord_wz_pert
    INTEGER, INTENT(IN) :: kord_tr_pert(nq)
    INTEGER, INTENT(IN) :: kord_tm_pert
    REAL(fvprc), INTENT(IN) :: consv
    REAL(fvprc), INTENT(IN) :: r_vir
    REAL(fvprc), INTENT(IN) :: cp
    REAL(fvprc), INTENT(IN) :: akap
    REAL(fvprc), INTENT(IN) :: hs(isd:ied, jsd:jed)
    REAL(fvprc), INTENT(INOUT) :: te0_2d(is:ie, js:je)
    REAL(fvprc), INTENT(INOUT) :: te0_2d_ad(is:ie, js:je)
    REAL(fvprc), INTENT(IN) :: ws(is:ie, js:je)
    REAL(fvprc) :: ws_ad(is:ie, js:je)
    LOGICAL, INTENT(IN) :: do_sat_adj
    LOGICAL, INTENT(IN) :: fill
    LOGICAL, INTENT(IN) :: reproduce_sum
    LOGICAL, INTENT(IN) :: do_omega, do_adiabatic_init
    REAL(fvprc), INTENT(IN) :: ptop
    REAL(fvprc), INTENT(IN) :: ak(km+1)
    REAL(fvprc), INTENT(IN) :: bk(km+1)
    TYPE(FV_GRID_TYPE), INTENT(IN), TARGET :: gridstruct
    TYPE(DOMAIN2D), INTENT(INOUT) :: domain
    REAL(fvprc), INTENT(INOUT) :: pk(is:ie, js:je, km+1)
    REAL(fvprc), INTENT(INOUT) :: pk_ad(is:ie, js:je, km+1)
    REAL(fvprc), INTENT(INOUT) :: q(isd:ied, jsd:jed, km, nq)
    REAL(fvprc), INTENT(INOUT) :: q_ad(isd:ied, jsd:jed, km, nq)
    REAL(fvprc), INTENT(INOUT) :: delp(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: delp_ad(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: pe(is-1:ie+1, km+1, js-1:je+1)
    REAL(fvprc), INTENT(INOUT) :: pe_ad(is-1:ie+1, km+1, js-1:je+1)
    REAL(fvprc), INTENT(INOUT) :: ps(isd:ied, jsd:jed)
    REAL(fvprc), INTENT(INOUT) :: ps_ad(isd:ied, jsd:jed)
    REAL(fvprc), INTENT(INOUT) :: ze0(is:ie, js:je, km+1)
    REAL(fvprc), INTENT(INOUT) :: ze0_ad(is:ie, js:je, km+1)
    REAL(fvprc), INTENT(INOUT) :: u(isd:ied, jsd:jed+1, km)
    REAL(fvprc), INTENT(INOUT) :: u_ad(isd:ied, jsd:jed+1, km)
    REAL(fvprc), INTENT(INOUT) :: v(isd:ied+1, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: v_ad(isd:ied+1, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: w(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: w_ad(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: pt(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: pt_ad(isd:ied, jsd:jed, km)
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(INOUT) :: delz
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(INOUT) :: &
&   delz_ad
    REAL(fvprc), DIMENSION(isd:isd, jsd:jsd, 1), INTENT(INOUT) :: cappa&
&   , q_con
    LOGICAL, INTENT(INOUT) :: remap_t
    LOGICAL, INTENT(IN) :: gmao_cubic
    LOGICAL, INTENT(IN) :: hydrostatic
    LOGICAL, INTENT(IN) :: hybrid_z
    LOGICAL, INTENT(IN) :: out_dt
    REAL(fvprc), INTENT(INOUT) :: ua(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: ua_ad(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: va(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: omga(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: omga_ad(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: dtdt(is:ie, js:je, km)
    REAL(fvprc) :: pkz(is:ie, js:je, km)
    REAL(fvprc) :: pkz_ad(is:ie, js:je, km)
    REAL(fvprc) :: te(is:ie, js:je, km)
    REAL(fvprc) :: te_ad(is:ie, js:je, km)
    REAL(fvprc), INTENT(INOUT) :: peln(is:ie, km+1, js:je)
    REAL(fvprc), INTENT(INOUT) :: peln_ad(is:ie, km+1, js:je)
    INTEGER :: i, j, k
    REAL(fvprc), DIMENSION(is:ie, js:je) :: te_2d, zsum0, zsum1, dpeln
    REAL(fvprc), DIMENSION(is:ie, js:je) :: te_2d_ad, zsum0_ad, zsum1_ad
    REAL(fvprc), DIMENSION(is:ie, km) :: q2, dp2, deng
    REAL(fvprc), DIMENSION(is:ie, km) :: q2_ad, dp2_ad, deng_ad
    REAL(fvprc), DIMENSION(is:ie, km+1) :: ze1, ze2, pe1, pe2, pk1, pk2&
&   , pn2, phis
    REAL(fvprc), DIMENSION(is:ie, km+1) :: ze1_ad, ze2_ad, pe1_ad, &
&   pe2_ad, pk1_ad, pk2_ad, pn2_ad, phis_ad
    REAL(fvprc) :: pe0(is:ie+1, km+1)
    REAL(fvprc) :: pe0_ad(is:ie+1, km+1)
    REAL(fvprc) :: pe3(is:ie+1, km+1)
    REAL(fvprc) :: pe3_ad(is:ie+1, km+1)
    REAL(fvprc) :: gz(is:ie)
    REAL(fvprc) :: gz_ad(is:ie)
    REAL(fvprc) :: dz1(km)
    REAL(fvprc) :: rcp, rg, tmp, tpe, cvm, rgama, rrg, bkh, dtmp, dlnp, &
&   ztop, z_rat
    REAL(fvprc) :: tmp_ad, tpe_ad, dtmp_ad, dlnp_ad
    REAL(fvprc) :: k1k, kapag, q_liq, q_sol
    INTEGER :: liq_wat, ice_wat, rainwat, snowwat, cld_amt, graupel, iq&
&   , n, kp, k_next
    LOGICAL :: te_map
    INTEGER :: isp1, iep1, jsp1, jep1
    INTEGER :: isdp1, iedp1, jsdp1, jedp1
    INTEGER :: abs_kord_tm, abs_kord_tm_pert
    REAL(fvprc) :: u_tj(isd:ied, jsd:jed+1, km)
    REAL(fvprc) :: v_tj(isd:ied+1, jsd:jed, km)
    REAL(fvprc) :: w_tj(isd:ied, jsd:jed, km)
    REAL(fvprc) :: pt_tj(isd:ied, jsd:jed, km)
    REAL(fvprc) :: q2_tj(is:ie, km)
    REAL(fvprc) :: deng_tj(is:ie, km)
    REAL(fvprc) :: te_tj(is:ie, js:je, km)
    REAL(fvprc) :: q_tj(isd:ied, jsd:jed, km, nq)
    REAL(fvprc) :: delz_tj(isd:ied, jsd:jed, km)
    INTRINSIC ABS
    INTRINSIC LOG
    INTRINSIC EXP
    INTRINSIC REAL
    REAL(fvprc) :: result1
    REAL(fvprc) :: result1_ad
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp2
    REAL(fvprc) :: temp3
    REAL(fvprc) :: temp4
    REAL(fvprc) :: temp5
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp6
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp7
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp_ad11
    REAL(fvprc) :: temp8
    REAL(fvprc) :: temp9
    REAL(fvprc) :: temp10
    REAL(fvprc) :: temp_ad12
    REAL(fvprc) :: temp11
    REAL(fvprc) :: temp12
    REAL(fvprc) :: temp13
    REAL(fvprc) :: temp_ad13
    REAL(fvprc) :: temp14
    REAL(fvprc) :: temp15
    REAL(fvprc) :: temp16
    REAL(fvprc) :: temp_ad14
    REAL(fvprc) :: temp_ad15
    REAL(fvprc) :: temp_ad16
    REAL(fvprc) :: temp17
    REAL(fvprc) :: temp18
    REAL(fvprc) :: temp19
    REAL(fvprc) :: temp_ad17
    REAL(fvprc) :: temp_ad18
    REAL(fvprc) :: temp_ad19
    REAL(fvprc) :: temp20
    REAL(fvprc) :: temp21
    REAL(fvprc) :: temp22
    REAL(fvprc) :: temp23
    REAL(fvprc) :: temp24
    REAL(fvprc) :: temp_ad20
    REAL(fvprc) :: temp_ad21
    REAL(fvprc) :: temp_ad22
    REAL(fvprc) :: temp_ad23
    REAL(fvprc) :: temp_ad24
    REAL(fvprc) :: temp_ad25
    REAL(fvprc) :: temp25
    REAL(fvprc) :: temp26
    REAL(fvprc) :: temp27
    REAL(fvprc) :: temp_ad26
    REAL(fvprc) :: temp_ad27
    REAL(fvprc) :: temp_ad28
    REAL(fvprc) :: temp28
    REAL(fvprc) :: temp29
    REAL(fvprc) :: temp30
    REAL(fvprc) :: temp31
    REAL(fvprc) :: temp32
    REAL(fvprc) :: temp33
    REAL(fvprc) :: temp_ad29
    REAL(fvprc) :: temp_ad30
    REAL(fvprc) :: temp_ad31
    REAL(fvprc) :: temp_ad32
    REAL(fvprc) :: temp_ad33
    REAL(fvprc) :: temp_ad34
    REAL(fvprc) :: temp_ad35
    REAL(fvprc) :: temp34
    REAL(fvprc) :: temp35
    REAL(fvprc) :: temp36
    REAL(fvprc) :: temp_ad36
    REAL(fvprc) :: temp_ad37
    REAL(fvprc) :: temp_ad38
    REAL(fvprc) :: temp_ad39
    REAL(fvprc) :: temp_ad40
    REAL(fvprc) :: temp_ad41
    REAL(fvprc) :: temp_ad42
    REAL(fvprc) :: temp_ad43
    INTEGER :: branch
    INTEGER :: ad_count
    INTEGER :: i0
    REAL(fvprc) :: temp37
    REAL(fvprc) :: temp_ad44
    REAL(fvprc) :: temp38
    REAL(fvprc) :: temp_ad45
    REAL(fvprc) :: temp_ad46
    CALL POPCONTROL3B_FV(branch)
    IF (branch .LT. 2) THEN
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY(pk2, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(bkh)
        CALL POPREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(zsum1, 8*(ie-is+1)*(je-js+1)/8)
        CALL POPREALARRAY(pn2, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(zsum0, 8*(ie-is+1)*(je-js+1)/8)
        CALL POPREALARRAY(result1)
        CALL POPREALARRAY(deng, 8*(ie-is+1)*km/8)
        CALL POPREALARRAY(te_2d, 8*(ie-is+1)*(je-js+1)/8)
        CALL POPINTEGER(abs_kord_tm_pert)
        CALL POPREALARRAY(rgama)
        CALL POPREALARRAY(tpe)
        CALL POPREALARRAY(rg)
        CALL POPINTEGER(iep1)
        CALL POPREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(dp2, 8*(ie-is+1)*km/8)
        CALL POPREALARRAY(ze1, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(gz, 8*(ie-is+1)/8)
        CALL POPREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
        CALL POPREALARRAY(pe2, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
        CALL POPREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
        gz_ad = 0.0_FVPRC
        dtmp_ad = 0.0_FVPRC
        DO j=je,js,-1
          DO k=1,km,1
            DO i=ie,is,-1
              temp_ad37 = cp*pt_ad(i, j, k)/pkz(i, j, k)
              dlnp = rg*(peln(i, k+1, j)-peln(i, k, j))
              tmp = tpe/(cp-pe(i, k, j)*dlnp/delp(i, j, k))
              CALL POPREALARRAY(gz(i))
              tmp_ad = temp_ad37 + dlnp*gz_ad(i)
              CALL POPREALARRAY(pt(i, j, k))
              pkz_ad(i, j, k) = pkz_ad(i, j, k) - tmp*temp_ad37/pkz(i, j&
&               , k)
              dtmp_ad = dtmp_ad + cp*pt_ad(i, j, k)
              pt_ad(i, j, k) = 0.0_FVPRC
              temp36 = delp(i, j, k)
              temp35 = pe(i, k, j)
              temp34 = temp35*dlnp/temp36
              temp_ad38 = tmp_ad/(cp-temp34)
              temp_ad36 = tpe*temp_ad38/((cp-temp34)*temp36)
              dlnp_ad = temp35*temp_ad36 + tmp*gz_ad(i)
              tpe_ad = temp_ad38
              pe_ad(i, k, j) = pe_ad(i, k, j) + dlnp*temp_ad36
              delp_ad(i, j, k) = delp_ad(i, j, k) - temp34*temp_ad36
              peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + rg*dlnp_ad
              peln_ad(i, k, j) = peln_ad(i, k, j) - rg*dlnp_ad
              CALL POPREALARRAY(tpe)
              temp_ad39 = -(gridstruct%rsin2(i, j)*0.25*tpe_ad)
              temp_ad40 = -(gridstruct%cosa_s(i, j)*temp_ad39)
              temp_ad41 = (v(i, j, k)+v(i+1, j, k))*temp_ad40
              temp_ad42 = (u(i, j, k)+u(i, j+1, k))*temp_ad40
              te_ad(i, j, k) = te_ad(i, j, k) + tpe_ad
              gz_ad(i) = gz_ad(i) - tpe_ad
              u_ad(i, j, k) = u_ad(i, j, k) + temp_ad41 + 2*u(i, j, k)*&
&               temp_ad39
              u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp_ad41 + 2*u(i, j+1&
&               , k)*temp_ad39
              v_ad(i, j, k) = v_ad(i, j, k) + temp_ad42 + 2*v(i, j, k)*&
&               temp_ad39
              v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp_ad42 + 2*v(i+1, j&
&               , k)*temp_ad39
            END DO
          END DO
          DO i=ie,is,-1
            CALL POPREALARRAY(gz(i))
            gz_ad(i) = 0.0_FVPRC
          END DO
        END DO
        GOTO 100
      ELSE
        CALL POPREALARRAY(pk2, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(bkh)
        CALL POPREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(zsum1, 8*(ie-is+1)*(je-js+1)/8)
        CALL POPREALARRAY(pn2, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(zsum0, 8*(ie-is+1)*(je-js+1)/8)
        CALL POPREALARRAY(result1)
        CALL POPREALARRAY(deng, 8*(ie-is+1)*km/8)
        CALL POPREALARRAY(te_2d, 8*(ie-is+1)*(je-js+1)/8)
        CALL POPINTEGER(abs_kord_tm_pert)
        CALL POPREALARRAY(dtmp)
        CALL POPREALARRAY(rgama)
        CALL POPREALARRAY(tpe)
        CALL POPINTEGER(iep1)
        CALL POPREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(dp2, 8*(ie-is+1)*km/8)
        CALL POPREALARRAY(ze1, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(gz, 8*(ie-is+1)/8)
        CALL POPREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
        CALL POPREALARRAY(pe2, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
        CALL POPREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
        CALL POPREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
        dtmp_ad = 0.0_FVPRC
        CALL POPREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
        DO k=1,km
          DO j=je,js,-1
            DO i=ie,is,-1
              temp37 = r_vir*q(i, j, k, sphum) + 1.
              temp_ad44 = pt_ad(i, j, k)/temp37
              dtmp_ad = dtmp_ad + pkz(i, j, k)*temp_ad44
              pkz_ad(i, j, k) = pkz_ad(i, j, k) + dtmp*temp_ad44
              q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) - (pt(i, j, k)&
&               +dtmp*pkz(i, j, k))*r_vir*temp_ad44/temp37
              pt_ad(i, j, k) = temp_ad44
            END DO
          END DO
        END DO
      END IF
    ELSE IF (branch .EQ. 2) THEN
      CALL POPREALARRAY(pk2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(bkh)
      CALL POPREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(zsum1, 8*(ie-is+1)*(je-js+1)/8)
      CALL POPREALARRAY(pn2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(zsum0, 8*(ie-is+1)*(je-js+1)/8)
      CALL POPREALARRAY(result1)
      CALL POPREALARRAY(deng, 8*(ie-is+1)*km/8)
      CALL POPREALARRAY(te_2d, 8*(ie-is+1)*(je-js+1)/8)
      CALL POPINTEGER(abs_kord_tm_pert)
      CALL POPREALARRAY(dtmp)
      CALL POPREALARRAY(rgama)
      CALL POPREALARRAY(tpe)
      CALL POPINTEGER(iep1)
      CALL POPREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(dp2, 8*(ie-is+1)*km/8)
      CALL POPREALARRAY(ze1, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(gz, 8*(ie-is+1)/8)
      CALL POPREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
      CALL POPREALARRAY(pe2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
      CALL POPREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
      dtmp_ad = 0.0_FVPRC
      CALL POPREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
      DO k=1,km
        DO j=je,js,-1
          DO i=ie,is,-1
            temp38 = r_vir*q(i, j, k, sphum) + 1.
            temp_ad45 = pt_ad(i, j, k)/temp38
            dtmp_ad = dtmp_ad + pkz(i, j, k)*temp_ad45
            pkz_ad(i, j, k) = pkz_ad(i, j, k) + dtmp*temp_ad45
            q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) - (pt(i, j, k)+&
&             dtmp*pkz(i, j, k))*r_vir*temp_ad45/temp38
            pt_ad(i, j, k) = temp_ad45
          END DO
        END DO
      END DO
    ELSE IF (branch .EQ. 3) THEN
      CALL POPREALARRAY(pk2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(bkh)
      CALL POPREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(zsum1, 8*(ie-is+1)*(je-js+1)/8)
      CALL POPREALARRAY(pn2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(zsum0, 8*(ie-is+1)*(je-js+1)/8)
      CALL POPREALARRAY(result1)
      CALL POPREALARRAY(deng, 8*(ie-is+1)*km/8)
      CALL POPREALARRAY(te_2d, 8*(ie-is+1)*(je-js+1)/8)
      CALL POPINTEGER(abs_kord_tm_pert)
      CALL POPREALARRAY(rgama)
      CALL POPREALARRAY(tpe)
      CALL POPINTEGER(iep1)
      CALL POPREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(dp2, 8*(ie-is+1)*km/8)
      CALL POPREALARRAY(ze1, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(gz, 8*(ie-is+1)/8)
      CALL POPREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
      CALL POPREALARRAY(pe2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
      CALL POPREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
      dtmp_ad = 0.0_FVPRC
      CALL POPREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
      DO k=1,km
        DO j=je,js,-1
          DO i=ie,is,-1
            temp_ad46 = cp*pt_ad(i, j, k)/pkz(i, j, k)
            pkz_ad(i, j, k) = pkz_ad(i, j, k) - pt(i, j, k)*temp_ad46/&
&             pkz(i, j, k)
            dtmp_ad = dtmp_ad + cp*pt_ad(i, j, k)
            pt_ad(i, j, k) = temp_ad46
          END DO
        END DO
      END DO
    ELSE
      CALL POPREALARRAY(pk2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(bkh)
      CALL POPREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(zsum1, 8*(ie-is+1)*(je-js+1)/8)
      CALL POPREALARRAY(pn2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(zsum0, 8*(ie-is+1)*(je-js+1)/8)
      CALL POPREALARRAY(result1)
      CALL POPREALARRAY(deng, 8*(ie-is+1)*km/8)
      CALL POPREALARRAY(te_2d, 8*(ie-is+1)*(je-js+1)/8)
      CALL POPINTEGER(abs_kord_tm_pert)
      CALL POPREALARRAY(rgama)
      CALL POPREALARRAY(tpe)
      CALL POPINTEGER(iep1)
      CALL POPREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(dp2, 8*(ie-is+1)*km/8)
      CALL POPREALARRAY(ze1, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(gz, 8*(ie-is+1)/8)
      CALL POPREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
      CALL POPREALARRAY(pe2, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
      CALL POPREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
      CALL POPREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
      dtmp_ad = 0.0_FVPRC
      CALL POPREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
      DO k=1,km
        DO j=je,js,-1
          DO i=ie,is,-1
            dtmp_ad = dtmp_ad + cp*pt_ad(i, j, k)
          END DO
        END DO
      END DO
    END IF
    rg = akap*cp
    gz_ad = 0.0_FVPRC
 100 CALL POPCONTROL1B_FV(branch)
    IF (branch .EQ. 0) THEN
      CALL POPCONTROL1B_FV(branch)
      IF (branch .EQ. 0) THEN
        temp_ad34 = dtmp_ad/(cp*result1)
        tpe_ad = temp_ad34
        result1_ad = -(tpe*temp_ad34/result1)
        zsum0_ad = 0.0_FVPRC
        CALL G_SUM_ADM(domain, zsum0, zsum0_ad, is, ie, js, je, ng, &
&                gridstruct%area_64, 0, g_sum_ad=result1_ad)
        zsum1_ad = 0.0_FVPRC
      ELSE
        temp_ad35 = dtmp_ad/(cv_air*result1)
        tpe_ad = temp_ad35
        result1_ad = -(tpe*temp_ad35/result1)
        zsum1_ad = 0.0_FVPRC
        CALL G_SUM_ADM(domain, zsum1, zsum1_ad, is, ie, js, je, ng, &
&                gridstruct%area_64, 0, g_sum_ad=result1_ad)
        zsum0_ad = 0.0_FVPRC
      END IF
      result1_ad = consv*tpe_ad
      te_2d_ad = 0.0_FVPRC
      CALL G_SUM_ADM(domain, te_2d, te_2d_ad, is, ie, js, je, ng, &
&              gridstruct%area_64, 0, g_sum_ad=result1_ad)
      DO j=je,js,-1
        DO i=ie,is,-1
          te0_2d_ad(i, j) = te0_2d_ad(i, j) + te_2d_ad(i, j)
          te_2d_ad(i, j) = -te_2d_ad(i, j)
          pk_ad(i, j, 1) = pk_ad(i, j, 1) + ptop*zsum0_ad(i, j)
          pk_ad(i, j, km+1) = pk_ad(i, j, km+1) - ptop*zsum0_ad(i, j)
          zsum1_ad(i, j) = zsum1_ad(i, j) + zsum0_ad(i, j)
          zsum0_ad(i, j) = 0.0_FVPRC
        END DO
        DO k=km,2,-1
          DO i=ie,is,-1
            pkz_ad(i, j, k) = pkz_ad(i, j, k) + delp(i, j, k)*zsum1_ad(i&
&             , j)
            delp_ad(i, j, k) = delp_ad(i, j, k) + pkz(i, j, k)*zsum1_ad(&
&             i, j)
          END DO
        END DO
        DO i=ie,is,-1
          pkz_ad(i, j, 1) = pkz_ad(i, j, 1) + delp(i, j, 1)*zsum1_ad(i, &
&           j)
          delp_ad(i, j, 1) = delp_ad(i, j, 1) + pkz(i, j, 1)*zsum1_ad(i&
&           , j)
          zsum1_ad(i, j) = 0.0_FVPRC
        END DO
      END DO
      CALL POPCONTROL1B_FV(branch)
      IF (branch .EQ. 0) THEN
        phis_ad = 0.0_FVPRC
        DO j=je,js,-1
          CALL POPCONTROL2B_FV(branch)
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
              DO k=km,1,-1
                DO i=ie,is,-1
                  temp33 = v(i, j, k) + v(i+1, j, k)
                  temp32 = u(i, j, k) + u(i, j+1, k)
                  temp31 = 0.5*gridstruct%rsin2(i, j)
                  temp30 = r_vir*q(i, j, k, sphum) + 1.
                  temp29 = pt(i, j, k)*pkz(i, j, k)
                  temp28 = temp29/temp30
                  temp_ad29 = delp(i, j, k)*te_2d_ad(i, j)
                  temp_ad30 = rgama*temp_ad29/temp30
                  temp_ad31 = 0.5*temp_ad29
                  temp_ad32 = temp31*temp_ad31
                  temp_ad33 = -(gridstruct%cosa_s(i, j)*temp_ad32)
                  delp_ad(i, j, k) = delp_ad(i, j, k) + (rgama*temp28+&
&                   0.5*(phis(i, k)+phis(i, k+1)+w(i, j, k)**2+temp31*(u&
&                   (i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j&
&                   , k)**2-gridstruct%cosa_s(i, j)*(temp32*temp33))))*&
&                   te_2d_ad(i, j)
                  pt_ad(i, j, k) = pt_ad(i, j, k) + pkz(i, j, k)*&
&                   temp_ad30
                  pkz_ad(i, j, k) = pkz_ad(i, j, k) + pt(i, j, k)*&
&                   temp_ad30
                  q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) - temp28*&
&                   r_vir*temp_ad30
                  phis_ad(i, k) = phis_ad(i, k) + temp_ad31
                  phis_ad(i, k+1) = phis_ad(i, k+1) + temp_ad31
                  w_ad(i, j, k) = w_ad(i, j, k) + 2*w(i, j, k)*temp_ad31
                  u_ad(i, j, k) = u_ad(i, j, k) + temp33*temp_ad33 + 2*u&
&                   (i, j, k)*temp_ad32
                  u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp33*temp_ad33 +&
&                   2*u(i, j+1, k)*temp_ad32
                  v_ad(i, j, k) = v_ad(i, j, k) + temp32*temp_ad33 + 2*v&
&                   (i, j, k)*temp_ad32
                  v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp32*temp_ad33 +&
&                   2*v(i+1, j, k)*temp_ad32
                END DO
              END DO
              DO i=ie,is,-1
                te_2d_ad(i, j) = 0.0_FVPRC
              END DO
              DO i=ie,is,-1
                DO k=1,km,1
                  CALL POPREALARRAY(phis(i, k))
                  phis_ad(i, k+1) = phis_ad(i, k+1) + phis_ad(i, k)
                  delz_ad(i, j, k) = delz_ad(i, j, k) - grav*phis_ad(i, &
&                   k)
                  phis_ad(i, k) = 0.0_FVPRC
                END DO
                CALL POPREALARRAY(phis(i, km+1))
                phis_ad(i, km+1) = 0.0_FVPRC
              END DO
            ELSE
              DO k=km,1,-1
                DO i=ie,is,-1
                  temp27 = v(i, j, k) + v(i+1, j, k)
                  temp26 = u(i, j, k) + u(i, j+1, k)
                  temp25 = 0.25*gridstruct%rsin2(i, j)
                  temp_ad26 = delp(i, j, k)*te_2d_ad(i, j)
                  temp_ad27 = temp25*temp_ad26
                  temp_ad28 = -(gridstruct%cosa_s(i, j)*temp_ad27)
                  delp_ad(i, j, k) = delp_ad(i, j, k) + (pt(i, j, k)*pkz&
&                   (i, j, k)+temp25*(u(i, j, k)**2+u(i, j+1, k)**2+v(i&
&                   , j, k)**2+v(i+1, j, k)**2-gridstruct%cosa_s(i, j)*(&
&                   temp26*temp27)))*te_2d_ad(i, j)
                  pt_ad(i, j, k) = pt_ad(i, j, k) + pkz(i, j, k)*&
&                   temp_ad26
                  pkz_ad(i, j, k) = pkz_ad(i, j, k) + pt(i, j, k)*&
&                   temp_ad26
                  u_ad(i, j, k) = u_ad(i, j, k) + temp27*temp_ad28 + 2*u&
&                   (i, j, k)*temp_ad27
                  u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp27*temp_ad28 +&
&                   2*u(i, j+1, k)*temp_ad27
                  v_ad(i, j, k) = v_ad(i, j, k) + temp26*temp_ad28 + 2*v&
&                   (i, j, k)*temp_ad27
                  v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp26*temp_ad28 +&
&                   2*v(i+1, j, k)*temp_ad27
                END DO
              END DO
              DO i=ie,is,-1
                pe_ad(i, km+1, j) = pe_ad(i, km+1, j) + hs(i, j)*&
&                 te_2d_ad(i, j)
                pe_ad(i, 1, j) = pe_ad(i, 1, j) - gz(i)*te_2d_ad(i, j)
                gz_ad(i) = gz_ad(i) - pe(i, 1, j)*te_2d_ad(i, j)
                te_2d_ad(i, j) = 0.0_FVPRC
              END DO
              DO i=ie,is,-1
                DO k=km,1,-1
                  temp_ad25 = pt(i, j, k)*gz_ad(i)
                  pt_ad(i, j, k) = pt_ad(i, j, k) + (pk(i, j, k+1)-pk(i&
&                   , j, k))*gz_ad(i)
                  pk_ad(i, j, k+1) = pk_ad(i, j, k+1) + temp_ad25
                  pk_ad(i, j, k) = pk_ad(i, j, k) - temp_ad25
                END DO
                CALL POPREALARRAY(gz(i))
                gz_ad(i) = 0.0_FVPRC
              END DO
            END IF
          ELSE IF (branch .EQ. 2) THEN
            DO k=km,1,-1
              DO i=ie,is,-1
                temp24 = v(i, j, k) + v(i+1, j, k)
                temp23 = u(i, j, k) + u(i, j+1, k)
                temp22 = 0.5*gridstruct%rsin2(i, j)
                temp21 = r_vir*q(i, j, k, sphum) + 1.
                temp20 = pt(i, j, k)/temp21
                temp_ad20 = delp(i, j, k)*te_2d_ad(i, j)
                temp_ad21 = cv_air*temp_ad20/temp21
                temp_ad22 = 0.5*temp_ad20
                temp_ad23 = temp22*temp_ad22
                temp_ad24 = -(gridstruct%cosa_s(i, j)*temp_ad23)
                delp_ad(i, j, k) = delp_ad(i, j, k) + (cv_air*temp20+0.5&
&                 *(phis(i, k)+phis(i, k+1)+w(i, j, k)**2+temp22*(u(i, j&
&                 , k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-&
&                 gridstruct%cosa_s(i, j)*(temp23*temp24))))*te_2d_ad(i&
&                 , j)
                pt_ad(i, j, k) = pt_ad(i, j, k) + temp_ad21
                q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) - temp20*&
&                 r_vir*temp_ad21
                phis_ad(i, k) = phis_ad(i, k) + temp_ad22
                phis_ad(i, k+1) = phis_ad(i, k+1) + temp_ad22
                w_ad(i, j, k) = w_ad(i, j, k) + 2*w(i, j, k)*temp_ad22
                u_ad(i, j, k) = u_ad(i, j, k) + temp24*temp_ad24 + 2*u(i&
&                 , j, k)*temp_ad23
                u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp24*temp_ad24 + 2&
&                 *u(i, j+1, k)*temp_ad23
                v_ad(i, j, k) = v_ad(i, j, k) + temp23*temp_ad24 + 2*v(i&
&                 , j, k)*temp_ad23
                v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp23*temp_ad24 + 2&
&                 *v(i+1, j, k)*temp_ad23
              END DO
            END DO
            DO i=ie,is,-1
              te_2d_ad(i, j) = 0.0_FVPRC
            END DO
            DO i=ie,is,-1
              DO k=1,km,1
                CALL POPREALARRAY(phis(i, k))
                phis_ad(i, k+1) = phis_ad(i, k+1) + phis_ad(i, k)
                delz_ad(i, j, k) = delz_ad(i, j, k) - grav*phis_ad(i, k)
                phis_ad(i, k) = 0.0_FVPRC
              END DO
              CALL POPREALARRAY(phis(i, km+1))
              phis_ad(i, km+1) = 0.0_FVPRC
            END DO
          ELSE
            DO k=km,1,-1
              DO i=ie,is,-1
                temp19 = v(i, j, k) + v(i+1, j, k)
                temp18 = u(i, j, k) + u(i, j+1, k)
                temp17 = 0.25*gridstruct%rsin2(i, j)
                temp_ad17 = delp(i, j, k)*te_2d_ad(i, j)
                temp_ad18 = temp17*temp_ad17
                temp_ad19 = -(gridstruct%cosa_s(i, j)*temp_ad18)
                delp_ad(i, j, k) = delp_ad(i, j, k) + (cp*pt(i, j, k)+&
&                 temp17*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(&
&                 i+1, j, k)**2-gridstruct%cosa_s(i, j)*(temp18*temp19))&
&                 )*te_2d_ad(i, j)
                pt_ad(i, j, k) = pt_ad(i, j, k) + cp*temp_ad17
                u_ad(i, j, k) = u_ad(i, j, k) + temp19*temp_ad19 + 2*u(i&
&                 , j, k)*temp_ad18
                u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp19*temp_ad19 + 2&
&                 *u(i, j+1, k)*temp_ad18
                v_ad(i, j, k) = v_ad(i, j, k) + temp18*temp_ad19 + 2*v(i&
&                 , j, k)*temp_ad18
                v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp18*temp_ad19 + 2&
&                 *v(i+1, j, k)*temp_ad18
              END DO
            END DO
            DO i=ie,is,-1
              pe_ad(i, km+1, j) = pe_ad(i, km+1, j) + hs(i, j)*te_2d_ad(&
&               i, j)
              pe_ad(i, 1, j) = pe_ad(i, 1, j) - gz(i)*te_2d_ad(i, j)
              gz_ad(i) = gz_ad(i) - pe(i, 1, j)*te_2d_ad(i, j)
              te_2d_ad(i, j) = 0.0_FVPRC
            END DO
            DO i=ie,is,-1
              DO k=km,1,-1
                temp_ad16 = rg*pt(i, j, k)*gz_ad(i)
                pt_ad(i, j, k) = pt_ad(i, j, k) + rg*(peln(i, k+1, j)-&
&                 peln(i, k, j))*gz_ad(i)
                peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + temp_ad16
                peln_ad(i, k, j) = peln_ad(i, k, j) - temp_ad16
              END DO
              CALL POPREALARRAY(gz(i))
              gz_ad(i) = 0.0_FVPRC
            END DO
          END IF
        END DO
      ELSE
        DO j=je,js,-1
          DO k=km,2,-1
            DO i=ie,is,-1
              te_ad(i, j, k) = te_ad(i, j, k) + delp(i, j, k)*te_2d_ad(i&
&               , j)
              delp_ad(i, j, k) = delp_ad(i, j, k) + te(i, j, k)*te_2d_ad&
&               (i, j)
            END DO
          END DO
          DO i=ie,is,-1
            te_ad(i, j, 1) = te_ad(i, j, 1) + delp(i, j, 1)*te_2d_ad(i, &
&             j)
            delp_ad(i, j, 1) = delp_ad(i, j, 1) + te(i, j, 1)*te_2d_ad(i&
&             , j)
            te_2d_ad(i, j) = 0.0_FVPRC
          END DO
        END DO
        phis_ad = 0.0_FVPRC
      END IF
    ELSE
      phis_ad = 0.0_FVPRC
    END IF
    CALL POPREALARRAY(pe, 8*(ie-is+3)*(km+1)*(je-js+3)/8)
    DO k=2,km
      DO j=je,js,-1
        DO i=ie,is,-1
          ua_ad(i, j, k-1) = ua_ad(i, j, k-1) + pe_ad(i, k, j)
          pe_ad(i, k, j) = 0.0_FVPRC
        END DO
      END DO
    END DO
    CALL POPCONTROL1B_FV(branch)
    IF (branch .EQ. 0) THEN
      pe0_ad = 0.0_FVPRC
      pe1_ad = 0.0_FVPRC
      pe2_ad = 0.0_FVPRC
      pe3_ad = 0.0_FVPRC
    ELSE
      pe0_ad = 0.0_FVPRC
      pe3_ad = 0.0_FVPRC
      DO j=je,js,-1
        CALL POPCONTROL1B_FV(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREALARRAY(v, 8*(ied-isd+2)*(jed-jsd+1)*km/8)
          gz_ad = 0.0_FVPRC
          CALL MAP1_PPM_ADM(km, pe0, pe0_ad, gz, gz_ad, km, pe3, pe3_ad&
&                     , v, v_ad, is, iep1, j, isd, iedp1, jsd, jed, -1, &
&                     kord_mt_pert)
        ELSE
          gz_ad = 0.0_FVPRC
          CALL MAP1_PPM_BWD(km, pe0, pe0_ad, gz, gz_ad, km, pe3, &
&                        pe3_ad, v, v_ad, is, iep1, j, isd, iedp1, jsd, &
&                        jed, -1, kord_mt)
        END IF
        DO k=km+1,2,-1
          DO i=ie+1,is,-1
            CALL POPREALARRAY(pe3(i, k))
            ua_ad(i-1, j, k-1) = ua_ad(i-1, j, k-1) + 0.5*pe3_ad(i, k)
            ua_ad(i, j, k-1) = ua_ad(i, j, k-1) + 0.5*pe3_ad(i, k)
            pe3_ad(i, k) = 0.0_FVPRC
            CALL POPREALARRAY(pe0(i, k))
            pe_ad(i-1, k, j) = pe_ad(i-1, k, j) + 0.5*pe0_ad(i, k)
            pe_ad(i, k, j) = pe_ad(i, k, j) + 0.5*pe0_ad(i, k)
            pe0_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
        DO i=ie+1,is,-1
          CALL POPREALARRAY(pe3(i, 1))
          pe3_ad(i, 1) = 0.0_FVPRC
          CALL POPREALARRAY(pe0(i, 1))
          pe0_ad(i, 1) = 0.0_FVPRC
        END DO
      END DO
      pe1_ad = 0.0_FVPRC
      pe2_ad = 0.0_FVPRC
      DO j=je+1,js,-1
        CALL POPCONTROL1B_FV(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREALARRAY(u, 8*(ied-isd+1)*(jed-jsd+2)*km/8)
          gz_ad = 0.0_FVPRC
          CALL MAP1_PPM_ADM(km, pe1, pe1_ad, gz, gz_ad, km, pe2, pe2_ad&
&                     , u, u_ad, is, ie, j, isd, ied, jsd, jedp1, -1, &
&                     kord_mt_pert)
        ELSE
          gz_ad = 0.0_FVPRC
          CALL MAP1_PPM_BWD(km, pe1, pe1_ad, gz, gz_ad, km, pe2, &
&                        pe2_ad, u, u_ad, is, ie, j, isd, ied, jsd, &
&                        jedp1, -1, kord_mt)
        END IF
        DO k=km+1,2,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(pe2(i, k))
            ua_ad(i, j-1, k-1) = ua_ad(i, j-1, k-1) + 0.5*pe2_ad(i, k)
            ua_ad(i, j, k-1) = ua_ad(i, j, k-1) + 0.5*pe2_ad(i, k)
            pe2_ad(i, k) = 0.0_FVPRC
            CALL POPREALARRAY(pe1(i, k))
            pe_ad(i, k, j-1) = pe_ad(i, k, j-1) + 0.5*pe1_ad(i, k)
            pe_ad(i, k, j) = pe_ad(i, k, j) + 0.5*pe1_ad(i, k)
            pe1_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
        DO i=ie,is,-1
          CALL POPREALARRAY(pe2(i, 1))
          pe2_ad(i, 1) = 0.0_FVPRC
          CALL POPREALARRAY(pe1(i, 1))
          pe1_ad(i, 1) = 0.0_FVPRC
        END DO
      END DO
      CALL POPCONTROL1B_FV(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY(ua, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
        CALL MPP_UPDATE_DOMAINS_ADM(ua, ua_ad, domain, complete=.true.)
      ELSE
        CALL POPREALARRAY(ua, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
        CALL MPP_UPDATE_DOMAINS_ADM(ua, ua_ad, domain, complete=.true., &
&                             whalo=1, ehalo=1, shalo=1, nhalo=1)
      END IF
    END IF
    kapag = -(akap/grav)
    rcp = 1./cp
    k1k = akap/(1.-akap)
    rrg = -(rdgas/grav)
    ze1_ad = 0.0_FVPRC
    dp2_ad = 0.0_FVPRC
    ze2_ad = 0.0_FVPRC
    q2_ad = 0.0_FVPRC
    deng_ad = 0.0_FVPRC
    pn2_ad = 0.0_FVPRC
    pk1_ad = 0.0_FVPRC
    pk2_ad = 0.0_FVPRC
    DO j=je+1,js,-1
      DO k=km,1,-1
        DO i=ie,is,-1
          CALL POPREALARRAY(ua(i, j, k))
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + ua_ad(i, j, k)
          ua_ad(i, j, k) = 0.0_FVPRC
        END DO
      END DO
      CALL POPINTEGER(k)
      CALL POPCONTROL2B_FV(branch)
      IF (branch .LT. 2) THEN
        IF (branch .EQ. 0) GOTO 110
      ELSE
        IF (branch .EQ. 2) THEN
          CALL POPREALARRAY(v, 8*(ied-isd+2)*(jed-jsd+1)*km/8)
          gz_ad = 0.0_FVPRC
          CALL MAP1_PPM_ADM(km, pe0, pe0_ad, gz, gz_ad, km, pe3, pe3_ad&
&                     , v, v_ad, is, iep1, j, isd, iedp1, jsd, jed, -1, &
&                     kord_mt_pert)
        ELSE
          gz_ad = 0.0_FVPRC
          CALL MAP1_PPM_BWD(km, pe0, pe0_ad, gz, gz_ad, km, pe3, &
&                        pe3_ad, v, v_ad, is, iep1, j, isd, iedp1, jsd, &
&                        jed, -1, kord_mt)
        END IF
        DO k=km+1,2,-1
          DO i=ie+1,is,-1
            CALL POPREALARRAY(pe3(i, k))
            pe_ad(i-1, km+1, j) = pe_ad(i-1, km+1, j) + bkh*pe3_ad(i, k)
            pe_ad(i, km+1, j) = pe_ad(i, km+1, j) + bkh*pe3_ad(i, k)
            pe3_ad(i, k) = 0.0_FVPRC
            CALL POPREALARRAY(pe0(i, k))
            pe_ad(i-1, k, j) = pe_ad(i-1, k, j) + 0.5*pe0_ad(i, k)
            pe_ad(i, k, j) = pe_ad(i, k, j) + 0.5*pe0_ad(i, k)
            pe0_ad(i, k) = 0.0_FVPRC
          END DO
          CALL POPREALARRAY(bkh)
        END DO
        DO i=ie+1,is,-1
          CALL POPREALARRAY(pe3(i, 1))
          pe3_ad(i, 1) = 0.0_FVPRC
        END DO
      END IF
      CALL POPCONTROL1B_FV(branch)
      IF (branch .EQ. 0) THEN
        gz_ad = 0.0_FVPRC
        CALL MAP1_PPM_BWD(km, pe0(is:ie, 1:km+1), pe0_ad(is:ie, 1:km+&
&                      1), gz, gz_ad, km, pe3(is:ie, 1:km+1), pe3_ad(is:&
&                      ie, 1:km+1), u, u_ad, is, ie, j, isd, ied, jsd, &
&                      jedp1, -1, kord_mt)
      ELSE
        CALL POPREALARRAY(u, 8*(ied-isd+1)*(jed-jsd+2)*km/8)
        gz_ad = 0.0_FVPRC
        CALL MAP1_PPM_ADM(km, pe0(is:ie, 1:km+1), pe0_ad(is:ie, 1:km+1)&
&                   , gz, gz_ad, km, pe3(is:ie, 1:km+1), pe3_ad(is:ie, 1&
&                   :km+1), u, u_ad, is, ie, j, isd, ied, jsd, jedp1, -1&
&                   , kord_mt_pert)
      END IF
      DO k=km+1,1,-1
        DO i=ie,is,-1
          CALL POPREALARRAY(pe3(i, k))
          pe_ad(i, km+1, j-1) = pe_ad(i, km+1, j-1) + bkh*pe3_ad(i, k)
          pe1_ad(i, km+1) = pe1_ad(i, km+1) + bkh*pe3_ad(i, k)
          pe3_ad(i, k) = 0.0_FVPRC
        END DO
        CALL POPREALARRAY(bkh)
      END DO
      CALL POPREALARRAY(pe0, 8*(ie-is+2)*(km+1)/8)
      DO k=2,km+1
        DO i=ie,is,-1
          pe_ad(i, k, j-1) = pe_ad(i, k, j-1) + 0.5*pe0_ad(i, k)
          pe1_ad(i, k) = pe1_ad(i, k) + 0.5*pe0_ad(i, k)
          pe0_ad(i, k) = 0.0_FVPRC
        END DO
      END DO
      CALL POPINTEGER(k)
      DO i=ie+1,is,-1
        CALL POPREALARRAY(pe0(i, 1))
        pe_ad(i, 1, j) = pe_ad(i, 1, j) + pe0_ad(i, 1)
        pe0_ad(i, 1) = 0.0_FVPRC
      END DO
 110  CALL POPCONTROL2B_FV(branch)
      IF (branch .EQ. 0) THEN
        DO i=ie,is,-1
          DO n=km,1,-1
            CALL POPCONTROL1B_FV(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREALARRAY(omga(i, j, n))
              temp14 = pe0(i, k+1) - pe0(i, k)
              temp_ad14 = omga_ad(i, j, n)/temp14
              temp16 = dp2(i, n) - pe0(i, k)
              temp15 = pe3(i, k+1) - pe3(i, k)
              temp_ad15 = -(temp15*temp16*temp_ad14/temp14)
              pe3_ad(i, k) = pe3_ad(i, k) + omga_ad(i, j, n) - temp16*&
&               temp_ad14
              pe3_ad(i, k+1) = pe3_ad(i, k+1) + temp16*temp_ad14
              dp2_ad(i, n) = dp2_ad(i, n) + temp15*temp_ad14
              pe0_ad(i, k) = pe0_ad(i, k) - temp_ad15 - temp15*temp_ad14
              pe0_ad(i, k+1) = pe0_ad(i, k+1) + temp_ad15
              omga_ad(i, j, n) = 0.0_FVPRC
            END IF
            CALL POPINTEGER(ad_count)
            DO i0=1,ad_count
              IF (i0 .EQ. 1) CALL POPCONTROL1B_FV(branch)
              CALL POPINTEGER(k)
            END DO
          END DO
        END DO
        CALL POPREALARRAY(dp2, 8*(ie-is+1)*km/8)
        DO k=1,km
          DO i=ie,is,-1
            peln_ad(i, k, j) = peln_ad(i, k, j) + 0.5*dp2_ad(i, k)
            peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + 0.5*dp2_ad(i, k)
            dp2_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
      ELSE IF (branch .NE. 1) THEN
        GOTO 130
      END IF
      CALL POPCONTROL2B_FV(branch)
      IF (branch .EQ. 0) THEN
        DO k=km,1,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(pkz(i, j, k))
            temp7 = akap*(peln(i, k+1, j)-peln(i, k, j))
            temp_ad10 = pkz_ad(i, j, k)/temp7
            temp_ad11 = -((pk2(i, k+1)-pk2(i, k))*akap*temp_ad10/temp7)
            pk2_ad(i, k+1) = pk2_ad(i, k+1) + temp_ad10
            pk2_ad(i, k) = pk2_ad(i, k) - temp_ad10
            peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + temp_ad11
            peln_ad(i, k, j) = peln_ad(i, k, j) - temp_ad11
            pkz_ad(i, j, k) = 0.0_FVPRC
          END DO
        END DO
      ELSE IF (branch .EQ. 1) THEN
        DO k=km,1,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(pkz(i, j, k))
            temp10 = delz(i, j, k)
            temp9 = delp(i, j, k)*pt(i, j, k)
            temp8 = temp9/temp10
            temp_ad12 = akap*EXP(akap*LOG(rrg*temp8))*pkz_ad(i, j, k)/(&
&             temp8*temp10)
            delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*temp_ad12
            pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad12
            delz_ad(i, j, k) = delz_ad(i, j, k) - temp8*temp_ad12
            pkz_ad(i, j, k) = 0.0_FVPRC
          END DO
        END DO
      ELSE
        DO k=km,1,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(pkz(i, j, k))
            temp13 = delz(i, j, k)
            temp12 = delp(i, j, k)*pt(i, j, k)
            temp11 = temp12/temp13
            temp_ad13 = k1k*EXP(k1k*LOG(kapag*temp11))*pkz_ad(i, j, k)/(&
&             temp11*temp13)
            delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*temp_ad13
            pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad13
            delz_ad(i, j, k) = delz_ad(i, j, k) - temp11*temp_ad13
            pkz_ad(i, j, k) = 0.0_FVPRC
          END DO
        END DO
      END IF
      DO k=km+1,1,-1
        DO i=ie,is,-1
          CALL POPREALARRAY(peln(i, k, j))
          pn2_ad(i, k) = pn2_ad(i, k) + peln_ad(i, k, j)
          peln_ad(i, k, j) = pe0_ad(i, k)
          CALL POPREALARRAY(pe0(i, k))
          pe0_ad(i, k) = 0.0_FVPRC
        END DO
      END DO
      CALL POPCONTROL1B_FV(branch)
      IF (branch .NE. 0) THEN
        CALL POPREALARRAY(pe3, 8*(ie-is+2)*(km+1)/8)
        DO k=2,km+1
          DO i=ie,is,-1
            omga_ad(i, j, k-1) = omga_ad(i, j, k-1) + pe3_ad(i, k)
            pe3_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
        DO i=ie,is,-1
          CALL POPREALARRAY(pe3(i, 1))
          pe3_ad(i, 1) = 0.0_FVPRC
        END DO
      END IF
      DO k=km,2,-1
        DO i=ie,is,-1
          CALL POPREALARRAY(pk(i, j, k))
          pk2_ad(i, k) = pk2_ad(i, k) + pk_ad(i, j, k)
          pk_ad(i, j, k) = 0.0_FVPRC
        END DO
      END DO
      CALL POPCONTROL2B_FV(branch)
      IF (branch .NE. 0) THEN
        IF (branch .NE. 1) THEN
          DO k=km,1,-1
            DO i=ie,is,-1
              CALL POPREALARRAY(delz(i, j, k))
              dp2_ad(i, k) = dp2_ad(i, k) - delz(i, j, k)*delz_ad(i, j, &
&               k)
              delz_ad(i, j, k) = -(dp2(i, k)*delz_ad(i, j, k))
            END DO
          END DO
          CALL POPCONTROL1B_FV(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY(delz, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
            gz_ad = 0.0_FVPRC
            CALL MAP1_PPM_ADM(km, pe1, pe1_ad, gz, gz_ad, km, pe2, &
&                       pe2_ad, delz, delz_ad, is, ie, j, isd, ied, jsd&
&                       , jed, 1, abs_kord_tm_pert)
          ELSE
            gz_ad = 0.0_FVPRC
            CALL MAP1_PPM_BWD(km, pe1, pe1_ad, gz, gz_ad, km, pe2, &
&                          pe2_ad, delz, delz_ad, is, ie, j, isd, ied, &
&                          jsd, jed, 1, abs_kord_tm)
          END IF
        END IF
        CALL POPCONTROL1B_FV(branch)
        IF (branch .EQ. 0) THEN
          CALL MAP1_PPM_BWD(km, pe1, pe1_ad, ws(is:ie, j), ws_ad(is:&
&                        ie, j), km, pe2, pe2_ad, w, w_ad, is, ie, j, &
&                        isd, ied, jsd, jed, -2, kord_wz)
        ELSE
          CALL POPREALARRAY(w, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
          CALL MAP1_PPM_ADM(km, pe1, pe1_ad, ws(is:ie, j), ws_ad(is:ie, &
&                     j), km, pe2, pe2_ad, w, w_ad, is, ie, j, isd, ied&
&                     , jsd, jed, -2, kord_wz_pert)
        END IF
      END IF
      CALL POPCONTROL4B(branch)
      IF (branch .LT. 4) THEN
        IF (branch .LT. 2) THEN
          IF (branch .EQ. 0) THEN
            CALL MAP1_CUBIC_BWD(km, pe1, pe1_ad, km, pe2, pe2_ad, te, &
&                         te_ad, is, ie, j, is, ie, js, je, akap, t_var=&
&                         1, conserv=.true.)
          ELSE
            gz_ad = 0.0_FVPRC
            CALL MAP1_PPM_BWD(km, pe1, pe1_ad, gz, gz_ad, km, pe2, &
&                          pe2_ad, te, te_ad, is, ie, j, is, ie, js, je&
&                          , 1, kord_tm)
          END IF
        ELSE IF (branch .EQ. 2) THEN
          CALL POPREALARRAY(te, 8*(ie-is+1)*(je-js+1)*km/8)
          gz_ad = 0.0_FVPRC
          CALL MAP1_PPM_ADM(km, pe1, pe1_ad, gz, gz_ad, km, pe2, pe2_ad&
&                     , te, te_ad, is, ie, j, is, ie, js, je, 1, &
&                     kord_tm_pert)
        ELSE
          DO k=km,1,-1
            DO i=ie,is,-1
              CALL POPREALARRAY(pt(i, j, k))
              deng_ad(i, k) = deng_ad(i, k) + pt_ad(i, j, k)
              pt_ad(i, j, k) = 0.0_FVPRC
            END DO
          END DO
          CALL POPCONTROL1B_FV(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY(deng, 8*(ie-is+1)*km/8)
            CALL REMAP_Z_ADM(km, ze1, ze1_ad, km, ze2, ze2_ad, deng, &
&                      deng_ad, is, ie, 1, abs_kord_tm_pert)
          ELSE
            CALL REMAP_Z_BWD(km, ze1, ze1_ad, km, ze2, ze2_ad, deng, &
&                         deng_ad, is, ie, 1, abs_kord_tm)
          END IF
          CALL POPREALARRAY(deng, 8*(ie-is+1)*km/8)
          DO k=1,km
            DO i=ie,is,-1
              pt_ad(i, j, k) = pt_ad(i, j, k) + deng_ad(i, k)
              deng_ad(i, k) = 0.0_FVPRC
            END DO
          END DO
          GOTO 120
        END IF
        DO k=km,1,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(te(i, j, k))
            temp6 = pe1(i, k+1) - pe1(i, k)
            temp_ad8 = te_ad(i, j, k)/temp6
            temp_ad9 = -((phis(i, k+1)-phis(i, k))*temp_ad8/temp6)
            phis_ad(i, k+1) = phis_ad(i, k+1) + temp_ad8
            phis_ad(i, k) = phis_ad(i, k) - temp_ad8
            pe1_ad(i, k+1) = pe1_ad(i, k+1) + temp_ad9
            pe1_ad(i, k) = pe1_ad(i, k) - temp_ad9
          END DO
        END DO
        CALL POPREALARRAY(phis, 8*(ie-is+1)*(km+1)/8)
        DO k=1,km+1
          DO i=ie,is,-1
            pe1_ad(i, k) = pe1_ad(i, k) + phis(i, k)*phis_ad(i, k)
            phis_ad(i, k) = pe1(i, k)*phis_ad(i, k)
          END DO
        END DO
        DO k=1,km,1
          DO i=ie,is,-1
            CALL POPREALARRAY(phis(i, k))
            temp_ad7 = pt(i, j, k)*phis_ad(i, k)
            phis_ad(i, k+1) = phis_ad(i, k+1) + phis_ad(i, k)
            pt_ad(i, j, k) = pt_ad(i, j, k) + (pk1(i, k+1)-pk1(i, k))*&
&             phis_ad(i, k)
            pk1_ad(i, k+1) = pk1_ad(i, k+1) + temp_ad7
            pk1_ad(i, k) = pk1_ad(i, k) - temp_ad7
            phis_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
        DO i=ie,is,-1
          CALL POPREALARRAY(phis(i, km+1))
          phis_ad(i, km+1) = 0.0_FVPRC
        END DO
      ELSE IF (branch .LT. 6) THEN
        IF (branch .EQ. 4) THEN
          CALL MAP_SCALAR_BWD(km, peln(is:ie, 1:km+1, j), peln_ad(is:&
&                          ie, 1:km+1, j), gz, km, pn2, pn2_ad, pt, &
&                          pt_ad, is, ie, j, isd, ied, jsd, jed, 1, &
&                          abs_kord_tm, t_min)
        ELSE
          CALL POPREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
          CALL MAP_SCALAR_ADM(km, peln(is:ie, 1:km+1, j), peln_ad(is:ie&
&                       , 1:km+1, j), gz, km, pn2, pn2_ad, pt, pt_ad, is&
&                       , ie, j, isd, ied, jsd, jed, 1, abs_kord_tm_pert&
&                       , t_min)
        END IF
      ELSE IF (branch .EQ. 6) THEN
        CALL MAP1_CUBIC_BWD(km, pe1, pe1_ad, km, pe2, pe2_ad, pt, pt_ad&
&                     , is, ie, j, isd, ied, jsd, jed, akap, t_var=3, &
&                     conserv=.false.)
      ELSE IF (branch .EQ. 7) THEN
        gz_ad = 0.0_FVPRC
        CALL MAP1_PPM_BWD(km, pk1, pk1_ad, gz, gz_ad, km, pk2, pk2_ad&
&                      , pt, pt_ad, is, ie, j, isd, ied, jsd, jed, 1, &
&                      abs_kord_tm)
      ELSE
        CALL POPREALARRAY(pt, 8*(ied-isd+1)*(jed-jsd+1)*km/8)
        gz_ad = 0.0_FVPRC
        CALL MAP1_PPM_ADM(km, pk1, pk1_ad, gz, gz_ad, km, pk2, pk2_ad, &
&                   pt, pt_ad, is, ie, j, isd, ied, jsd, jed, 1, &
&                   abs_kord_tm_pert)
      END IF
 120  DO k=km,2,-1
        DO i=ie,is,-1
          CALL POPREALARRAY(pk2(i, k))
          pn2_ad(i, k) = pn2_ad(i, k) + EXP(akap*pn2(i, k))*akap*pk2_ad(&
&           i, k)
          pk2_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY(pn2(i, k))
          pe2_ad(i, k) = pe2_ad(i, k) + pn2_ad(i, k)/pe2(i, k)
          pn2_ad(i, k) = 0.0_FVPRC
        END DO
      END DO
      DO i=ie,is,-1
        CALL POPREALARRAY(pk2(i, km+1))
        pk1_ad(i, km+1) = pk1_ad(i, km+1) + pk2_ad(i, km+1)
        pk2_ad(i, km+1) = 0.0_FVPRC
        CALL POPREALARRAY(pk2(i, 1))
        pk1_ad(i, 1) = pk1_ad(i, 1) + pk2_ad(i, 1)
        pk2_ad(i, 1) = 0.0_FVPRC
        CALL POPREALARRAY(pn2(i, km+1))
        peln_ad(i, km+1, j) = peln_ad(i, km+1, j) + pn2_ad(i, km+1)
        pn2_ad(i, km+1) = 0.0_FVPRC
        CALL POPREALARRAY(pn2(i, 1))
        peln_ad(i, 1, j) = peln_ad(i, 1, j) + pn2_ad(i, 1)
        pn2_ad(i, 1) = 0.0_FVPRC
      END DO
      CALL POPREALARRAY(pk1, 8*(ie-is+1)*(km+1)/8)
      DO k=1,km+1
        DO i=ie,is,-1
          pk_ad(i, j, k) = pk_ad(i, j, k) + pk1_ad(i, k)
          pk1_ad(i, k) = 0.0_FVPRC
        END DO
      END DO
      CALL POPCONTROL2B_FV(branch)
      IF (branch .LT. 2) THEN
        IF (branch .NE. 0) THEN
          DO iq=nq,1,-1
            DO k=km,1,-1
              DO i=ie,is,-1
                CALL POPREALARRAY(q(i, j, k, iq))
                q2_ad(i, k) = q2_ad(i, k) + q_ad(i, j, k, iq)
                q_ad(i, j, k, iq) = 0.0_FVPRC
              END DO
            END DO
            CALL POPCONTROL1B_FV(branch)
            IF (branch .EQ. 0) THEN
              CALL MAP1_Q2_ADM(km, pe1, pe1_ad, q(isd:ied, jsd:jed, 1:km&
&                        , iq), q_ad(isd:ied, jsd:jed, 1:km, iq), km, &
&                        pe2, pe2_ad, q2, q2_ad, dp2, dp2_ad, is, ie, 0&
&                        , kord_tr_pert(iq), j, isd, ied, jsd, jed, &
&                        0.0_FVPRC)
            ELSE
              CALL MAP1_Q2_BWD(km, pe1, pe1_ad, q(isd:ied, jsd:jed, 1&
&                           :km, iq), q_ad(isd:ied, jsd:jed, 1:km, iq), &
&                           km, pe2, pe2_ad, q2, q2_ad, dp2, dp2_ad, is&
&                           , ie, 0, kord_tr(iq), j, isd, ied, jsd, jed&
&                           , 0.0_FVPRC)
            END IF
          END DO
        END IF
      ELSE IF (branch .EQ. 2) THEN
        CALL POPREALARRAY(q, 8*(ied-isd+1)*(jed-jsd+1)*km*nq/8)
        CALL MAPN_TRACER_ADM(nq, km, km, pe1, pe1_ad, pe2, pe2_ad, q, &
&                      q_ad, dp2, dp2_ad, kord_tr_pert, j, is, ie, isd, &
&                      ied, jsd, jed, 0.0_FVPRC, fill)
      ELSE
        CALL MAPN_TRACER_BWD(nq, km, km, pe1, pe1_ad, pe2, pe2_ad, q&
&                         , q_ad, dp2, dp2_ad, kord_tr, j, is, ie, isd, &
&                         ied, jsd, jed, 0.0_FVPRC, fill)
      END IF
      DO k=km,1,-1
        DO i=ie,is,-1
          CALL POPREALARRAY(delp(i, j, k))
          dp2_ad(i, k) = dp2_ad(i, k) + delp_ad(i, j, k)
          delp_ad(i, j, k) = 0.0_FVPRC
        END DO
      END DO
      CALL POPCONTROL1B_FV(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY(dp2, 8*(ie-is+1)*km/8)
        DO k=1,km
          DO i=ie,is,-1
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + dp2_ad(i, k)
            pe2_ad(i, k) = pe2_ad(i, k) - dp2_ad(i, k)
            dp2_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
        DO k=km,2,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(pe2(i, k))
            pe_ad(i, km+1, j) = pe_ad(i, km+1, j) + bk(k)*pe2_ad(i, k)
            pe2_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
      ELSE
        DO i=ie,is,-1
          CALL POPREALARRAY(dp2(i, km))
          pe2_ad(i, km+1) = pe2_ad(i, km+1) + dp2_ad(i, km)
          pe2_ad(i, km) = pe2_ad(i, km) - dp2_ad(i, km)
          dp2_ad(i, km) = 0.0_FVPRC
        END DO
        DO k=km-1,1,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(pe2(i, k+1))
            pe2_ad(i, k) = pe2_ad(i, k) + pe2_ad(i, k+1)
            dp2_ad(i, k) = dp2_ad(i, k) + pe2_ad(i, k+1)
            pe2_ad(i, k+1) = 0.0_FVPRC
            CALL POPREALARRAY(dp2(i, k))
            delz_ad(i, j, k) = delz_ad(i, j, k) - deng(i, k)*dp2_ad(i, k&
&             )
            deng_ad(i, k) = deng_ad(i, k) - delz(i, j, k)*dp2_ad(i, k)
            dp2_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
        DO k=km,1,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(delz(i, j, k))
            ze2_ad(i, k+1) = ze2_ad(i, k+1) + delz_ad(i, j, k)
            ze2_ad(i, k) = ze2_ad(i, k) - delz_ad(i, j, k)
            delz_ad(i, j, k) = 0.0_FVPRC
          END DO
        END DO
        CALL POPCONTROL1B_FV(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREALARRAY(deng, 8*(ie-is+1)*km/8)
          CALL REMAP_Z_ADM(km, ze1, ze1_ad, km, ze2, ze2_ad, deng, &
&                    deng_ad, is, ie, 1, abs_kord_tm_pert)
        ELSE
          CALL REMAP_Z_BWD(km, ze1, ze1_ad, km, ze2, ze2_ad, deng, &
&                       deng_ad, is, ie, 1, abs_kord_tm)
        END IF
        CALL POPREALARRAY(deng, 8*(ie-is+1)*km/8)
        DO k=1,km
          DO i=ie,is,-1
            temp_ad43 = -(deng_ad(i, k)/delz(i, j, k))
            delp_ad(i, j, k) = delp_ad(i, j, k) + temp_ad43
            delz_ad(i, j, k) = delz_ad(i, j, k) - delp(i, j, k)*&
&             temp_ad43/delz(i, j, k)
            deng_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
        CALL POPREALARRAY(ze2, 8*(ie-is+1)*(km+1)/8)
        DO k=2,km+1
          DO i=ie,is,-1
            ze0_ad(i, j, k) = ze0_ad(i, j, k) + ze2_ad(i, k)
            ze2_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
        DO i=ie,is,-1
          ze1_ad(i, 1) = ze1_ad(i, 1) + ze2_ad(i, 1) + ze0_ad(i, j, 1)
          ze0_ad(i, j, 1) = 0.0_FVPRC
          CALL POPREALARRAY(ze2(i, 1))
          ze2_ad(i, 1) = 0.0_FVPRC
        END DO
        DO k=1,km,1
          DO i=ie,is,-1
            CALL POPREALARRAY(ze1(i, k))
            ze1_ad(i, k+1) = ze1_ad(i, k+1) + ze1_ad(i, k)
            delz_ad(i, j, k) = delz_ad(i, j, k) - ze1_ad(i, k)
            ze1_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
        DO i=ie,is,-1
          CALL POPREALARRAY(ze1(i, km+1))
          ze0_ad(i, j, km+1) = ze0_ad(i, j, km+1) + ze1_ad(i, km+1)
          ze1_ad(i, km+1) = 0.0_FVPRC
        END DO
      END IF
      DO i=ie,is,-1
        pe1_ad(i, km+1) = pe1_ad(i, km+1) + ps_ad(i, j)
        ps_ad(i, j) = 0.0_FVPRC
      END DO
      CALL POPCONTROL2B_FV(branch)
      IF (branch .NE. 0) THEN
        IF (branch .EQ. 1) THEN
          DO k=km,1,-1
            DO i=ie,is,-1
              CALL POPREALARRAY(delz(i, j, k))
              temp_ad6 = -(delz_ad(i, j, k)/delp(i, j, k))
              delp_ad(i, j, k) = delp_ad(i, j, k) - delz(i, j, k)*&
&               temp_ad6/delp(i, j, k)
              delz_ad(i, j, k) = temp_ad6
            END DO
          END DO
        END IF
      END IF
      CALL POPCONTROL2B_FV(branch)
      IF (branch .LT. 2) THEN
        IF (branch .EQ. 0) THEN
          DO k=km,1,-1
            DO i=ie,is,-1
              CALL POPREALARRAY(pt(i, j, k))
              temp1 = rg*(peln(i, k+1, j)-peln(i, k, j))
              temp_ad = pt_ad(i, j, k)/temp1
              temp0 = pk(i, j, k+1) - pk(i, j, k)
              temp = pt(i, j, k)
              temp_ad0 = -(temp*temp0*rg*temp_ad/temp1)
              pk_ad(i, j, k+1) = pk_ad(i, j, k+1) + temp*temp_ad
              pk_ad(i, j, k) = pk_ad(i, j, k) - temp*temp_ad
              peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + temp_ad0
              peln_ad(i, k, j) = peln_ad(i, k, j) - temp_ad0
              pt_ad(i, j, k) = temp0*temp_ad
            END DO
          END DO
        ELSE
          DO k=km,1,-1
            DO i=ie,is,-1
              CALL POPREALARRAY(pt(i, j, k))
              temp5 = delz(i, j, k)
              temp4 = delp(i, j, k)*pt(i, j, k)
              temp2 = temp4/temp5
              temp3 = k1k*LOG(kapag*temp2)
              temp_ad1 = k1k*EXP(temp3)*pt(i, j, k)*rcp*pt_ad(i, j, k)/(&
&               temp2*temp5)
              delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*temp_ad1
              delz_ad(i, j, k) = delz_ad(i, j, k) - temp2*temp_ad1
              pt_ad(i, j, k) = delp(i, j, k)*temp_ad1 + rcp*EXP(temp3)*&
&               pt_ad(i, j, k)
            END DO
          END DO
        END IF
      ELSE IF (branch .NE. 2) THEN
        DO k=km,1,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(te(i, j, k))
            temp_ad2 = gridstruct%rsin2(i, j)*0.25*te_ad(i, j, k)
            temp_ad3 = -(gridstruct%cosa_s(i, j)*temp_ad2)
            temp_ad4 = (v(i, j, k)+v(i+1, j, k))*temp_ad3
            temp_ad5 = (u(i, j, k)+u(i, j+1, k))*temp_ad3
            u_ad(i, j, k) = u_ad(i, j, k) + temp_ad4 + 2*u(i, j, k)*&
&             temp_ad2
            u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp_ad4 + 2*u(i, j+1, k&
&             )*temp_ad2
            v_ad(i, j, k) = v_ad(i, j, k) + temp_ad5 + 2*v(i, j, k)*&
&             temp_ad2
            v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp_ad5 + 2*v(i+1, j, k&
&             )*temp_ad2
            pt_ad(i, j, k) = pt_ad(i, j, k) + pkz(i, j, k)*te_ad(i, j, k&
&             )
            pkz_ad(i, j, k) = pkz_ad(i, j, k) + pt(i, j, k)*te_ad(i, j, &
&             k)
            te_ad(i, j, k) = 0.0_FVPRC
          END DO
        END DO
        CALL PKEZ_BWD(km, is, ie, js, je, j, pe, pk, pk_ad, akap, peln, &
&               peln_ad, pkz, pkz_ad, ptop)
      END IF
 130  DO i=ie,is,-1
        CALL POPREALARRAY(pe2(i, km+1))
        pe_ad(i, km+1, j) = pe_ad(i, km+1, j) + pe2_ad(i, km+1)
        pe2_ad(i, km+1) = 0.0_FVPRC
        CALL POPREALARRAY(pe2(i, 1))
        pe2_ad(i, 1) = 0.0_FVPRC
      END DO
      CALL POPREALARRAY(pe1, 8*(ie-is+1)*(km+1)/8)
      CALL POPINTEGER(j)
      DO k=1,km+1
        DO i=ie,is,-1
          pe_ad(i, k, j) = pe_ad(i, k, j) + pe1_ad(i, k)
          pe1_ad(i, k) = 0.0_FVPRC
        END DO
      END DO
    END DO
  END SUBROUTINE LAGRANGIAN_TO_EULERIAN_BWD
!---- version number -----
!  character(len=128) :: version = '$Id: fv_mapz.F90,v 1.1.2.2.2.2.30.1.22.2.6.2.34.5.4.4.2.2.2.1.4.2.2.2 2017/02/21 20:43:38 bma
!uer Exp $'
!  character(len=128) :: tagname = '$Name: Heracles-UNSTABLE_ncepdyn_Feb222017 $'
  SUBROUTINE LAGRANGIAN_TO_EULERIAN(last_step, consv, ps, pe, delp, pkz&
&   , pk, mdt, pdt, km, is, ie, js, je, isd, ied, jsd, jed, nq, nwat, &
&   sphum, q_con, u, v, w, delz, pt, q, hs, r_vir, cp, akap, cappa, &
&   kord_mt, kord_wz, kord_tr, kord_tm, kord_mt_pert, kord_wz_pert, &
&   kord_tr_pert, kord_tm_pert, peln, te0_2d, ng, ua, va, omga, te, ws, &
&   fill, reproduce_sum, out_dt, dtdt, ptop, ak, bk, gridstruct, domain&
&   , ze0, gmao_cubic, remap_t, do_sat_adj, hydrostatic, hybrid_z, &
&   do_omega, do_adiabatic_init)
    IMPLICIT NONE
! call cubed_to_latlon(u, v, ua, va, dx, dy, rdxa, rdya, km, 1, flagstruct%c2l_ord)
!  nullify(cosa_s)
!  nullify(rsin2)
    LOGICAL, INTENT(IN) :: last_step
! remap time step
    REAL(fvprc), INTENT(IN) :: mdt
! phys time step
    REAL(fvprc), INTENT(IN) :: pdt
    INTEGER, INTENT(IN) :: km
! number of tracers (including h2o)
    INTEGER, INTENT(IN) :: nq
    INTEGER, INTENT(IN) :: nwat
! index for water vapor (specific humidity)
    INTEGER, INTENT(IN) :: sphum
    INTEGER, INTENT(IN) :: ng
! starting & ending X-Dir index
    INTEGER, INTENT(IN) :: is, ie, isd, ied
! starting & ending Y-Dir index
    INTEGER, INTENT(IN) :: js, je, jsd, jed
! Mapping order for the vector winds
    INTEGER, INTENT(IN) :: kord_mt
! Mapping order/option for w
    INTEGER, INTENT(IN) :: kord_wz
! Mapping order for tracers
    INTEGER, INTENT(IN) :: kord_tr(nq)
! Mapping order for thermodynamics
    INTEGER, INTENT(IN) :: kord_tm
! Mapping order for the vector winds
    INTEGER, INTENT(IN) :: kord_mt_pert
! Mapping order/option for w
    INTEGER, INTENT(IN) :: kord_wz_pert
! Mapping order for tracers
    INTEGER, INTENT(IN) :: kord_tr_pert(nq)
! Mapping order for thermodynamics
    INTEGER, INTENT(IN) :: kord_tm_pert
! factor for TE conservation
    REAL(fvprc), INTENT(IN) :: consv
    REAL(fvprc), INTENT(IN) :: r_vir
    REAL(fvprc), INTENT(IN) :: cp
    REAL(fvprc), INTENT(IN) :: akap
! surface geopotential
    REAL(fvprc), INTENT(IN) :: hs(isd:ied, jsd:jed)
    REAL(fvprc), INTENT(INOUT) :: te0_2d(is:ie, js:je)
    REAL(fvprc), INTENT(IN) :: ws(is:ie, js:je)
    LOGICAL, INTENT(IN) :: do_sat_adj
! fill negative tracers
    LOGICAL, INTENT(IN) :: fill
    LOGICAL, INTENT(IN) :: reproduce_sum
    LOGICAL, INTENT(IN) :: do_omega, do_adiabatic_init
    REAL(fvprc), INTENT(IN) :: ptop
    REAL(fvprc), INTENT(IN) :: ak(km+1)
    REAL(fvprc), INTENT(IN) :: bk(km+1)
    TYPE(FV_GRID_TYPE), INTENT(IN), TARGET :: gridstruct
    TYPE(DOMAIN2D), INTENT(INOUT) :: domain
! !INPUT/OUTPUT
! pe to the kappa
    REAL(fvprc), INTENT(INOUT) :: pk(is:ie, js:je, km+1)
    REAL(fvprc), INTENT(INOUT) :: q(isd:ied, jsd:jed, km, nq)
! pressure thickness
    REAL(fvprc), INTENT(INOUT) :: delp(isd:ied, jsd:jed, km)
! pressure at layer edges
    REAL(fvprc), INTENT(INOUT) :: pe(is-1:ie+1, km+1, js-1:je+1)
! surface pressure
    REAL(fvprc), INTENT(INOUT) :: ps(isd:ied, jsd:jed)
! Specified height at edges (m)
    REAL(fvprc), INTENT(INOUT) :: ze0(is:ie, js:je, km+1)
! u-wind will be ghosted one latitude to the north upon exit
! u-wind (m/s)
    REAL(fvprc), INTENT(INOUT) :: u(isd:ied, jsd:jed+1, km)
! v-wind (m/s)
    REAL(fvprc), INTENT(INOUT) :: v(isd:ied+1, jsd:jed, km)
! vertical velocity (m/s)
    REAL(fvprc), INTENT(INOUT) :: w(isd:ied, jsd:jed, km)
! cp*virtual potential temperature 
    REAL(fvprc), INTENT(INOUT) :: pt(isd:ied, jsd:jed, km)
! as input; output: temperature
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(INOUT) :: delz
    REAL(fvprc), DIMENSION(isd:isd, jsd:jsd, 1), INTENT(INOUT) :: cappa&
&   , q_con
    LOGICAL, INTENT(INOUT) :: remap_t
    LOGICAL, INTENT(IN) :: gmao_cubic
    LOGICAL, INTENT(IN) :: hydrostatic
    LOGICAL, INTENT(IN) :: hybrid_z
    LOGICAL, INTENT(IN) :: out_dt
! u-wind (m/s) on physics grid
    REAL(fvprc), INTENT(INOUT) :: ua(isd:ied, jsd:jed, km)
! v-wind (m/s) on physics grid
    REAL(fvprc), INTENT(INOUT) :: va(isd:ied, jsd:jed, km)
! vertical press. velocity (pascal/sec)
    REAL(fvprc), INTENT(INOUT) :: omga(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: dtdt(is:ie, js:je, km)
! layer-mean pk for converting t to pt
    REAL(fvprc), INTENT(OUT) :: pkz(is:ie, js:je, km)
    REAL(fvprc), INTENT(OUT) :: te(is:ie, js:je, km)
! log(pe)
    REAL(fvprc), INTENT(INOUT) :: peln(is:ie, km+1, js:je)
! !DESCRIPTION:
!
! !REVISION HISTORY:
! SJL 03.11.04: Initial version for partial remapping
!
!-----------------------------------------------------------------------
    INTEGER :: i, j, k
! real(FVPRC) q_source(is:ie,js:je,nq)    ! numerical tracer source from surface
! in case fillz is not sufficient
    REAL(fvprc), DIMENSION(is:ie, js:je) :: te_2d, zsum0, zsum1, dpeln
    REAL(fvprc), DIMENSION(is:ie, km) :: q2, dp2, deng
    REAL(fvprc), DIMENSION(is:ie, km+1) :: ze1, ze2, pe1, pe2, pk1, pk2&
&   , pn2, phis
    REAL(fvprc) :: pe0(is:ie+1, km+1)
    REAL(fvprc) :: pe3(is:ie+1, km+1)
    REAL(fvprc) :: gz(is:ie)
    REAL(fvprc) :: dz1(km)
    REAL(fvprc) :: rcp, rg, tmp, tpe, cvm, rgama, rrg, bkh, dtmp, dlnp, &
&   ztop, z_rat
    REAL(fvprc) :: k1k, kapag, q_liq, q_sol
    INTEGER :: liq_wat, ice_wat, rainwat, snowwat, cld_amt, graupel, iq&
&   , n, kp, k_next
    LOGICAL :: te_map
!     real(FVPRC), pointer, dimension(:,:) :: cosa_s, rsin2
    INTEGER :: isp1, iep1, jsp1, jep1
    INTEGER :: isdp1, iedp1, jsdp1, jedp1
    INTEGER :: abs_kord_tm, abs_kord_tm_pert
    REAL(fvprc) :: u_tj(isd:ied, jsd:jed+1, km)
    REAL(fvprc) :: v_tj(isd:ied+1, jsd:jed, km)
    REAL(fvprc) :: w_tj(isd:ied, jsd:jed, km)
    REAL(fvprc) :: pt_tj(isd:ied, jsd:jed, km)
    REAL(fvprc) :: q2_tj(is:ie, km)
    REAL(fvprc) :: deng_tj(is:ie, km)
    REAL(fvprc) :: te_tj(is:ie, js:je, km)
    REAL(fvprc) :: q_tj(isd:ied, jsd:jed, km, nq)
    REAL(fvprc) :: delz_tj(isd:ied, jsd:jed, km)
    INTRINSIC ABS
    INTRINSIC LOG
    INTRINSIC EXP
    INTRINSIC REAL
    REAL(fvprc) :: result1
    isp1 = is + 1
    iep1 = ie + 1
    jsp1 = js + 1
    jep1 = je + 1
    isdp1 = isd + 1
    iedp1 = ied + 1
    jsdp1 = jsd + 1
    jedp1 = jed + 1
!      cosa_s => gridstruct%cosa_s
!      rsin2 => gridstruct%rsin2
! rg/Cv=0.4
    k1k = akap/(1.-akap)
    kapag = -(akap/grav)
    rg = akap*cp
! cv/cp
    rgama = 1. - akap
    rcp = 1./cp
    rrg = -(rdgas/grav)
    IF (kord_tm .LT. 0) THEN
      te_map = .false.
    ELSE
      te_map = .true.
    END IF
    IF (kord_tm .GE. 0.) THEN
      abs_kord_tm = kord_tm
    ELSE
      abs_kord_tm = -kord_tm
    END IF
    IF (kord_tm_pert .GE. 0.) THEN
      abs_kord_tm_pert = kord_tm_pert
    ELSE
      abs_kord_tm_pert = -kord_tm_pert
    END IF
    IF (nwat .GE. 3) THEN
      liq_wat = GET_TRACER_INDEX(model_atmos, 'liq_wat')
      ice_wat = GET_TRACER_INDEX(model_atmos, 'ice_wat')
      cld_amt = GET_TRACER_INDEX(model_atmos, 'cld_amt')
    END IF
    IF (nwat .EQ. 6) THEN
      rainwat = GET_TRACER_INDEX(model_atmos, 'rainwat')
      snowwat = GET_TRACER_INDEX(model_atmos, 'snowwat')
      graupel = GET_TRACER_INDEX(model_atmos, 'graupel')
    END IF
!$OMP parallel do default(none) shared(is,ie,js,je,km,pe,ptop,kord_tm,remap_t,hydrostatic, &
!$OMP                                  pt,pk,rg,peln,q,liq_wat,rainwat,ice_wat,snowwat,    &
!$OMP                                  graupel,q_con,sphum,cappa,r_vir,rcp,k1k,kapag,delp, &
!$OMP                                  delz,akap,pkz,te,rsin2,u,v,cosa_s,hybrid_z,ztop,ps, &
!$OMP                                  ze0,ak,bk,nq,isd,ied,jsd,jed,kord_tr,fill,te_map,   &
!$OMP                                  hs,gz,w,ws,kord_wz,do_omega,omga,rrg,kord_mt,ua)    &
!$OMP                          private(cvm,dz1,q_liq,q_sol,z_rat,kp,k_next,bkh,deng,dp2,   &
!$OMP                                  pe0,pe1,pe2,pe3,pk1,pk2,pn2,phis,q2,ze1,ze2)
    DO j=js,je+1
      DO k=1,km+1
        DO i=is,ie
          pe1(i, k) = pe(i, k, j)
        END DO
      END DO
      DO i=is,ie
        pe2(i, 1) = ptop
        pe2(i, km+1) = pe(i, km+1, j)
      END DO
!(j < je+1)
      IF (j .NE. je + 1) THEN
        IF (kord_tm .LT. 0) THEN
! remap_t test
          IF (remap_t) THEN
! hydro test
! Note: pt at this stage is cp*Theta_v
! Transform virtual pt to virtual Temp
            IF (hydrostatic) THEN
              DO k=1,km
                DO i=is,ie
                  pt(i, j, k) = pt(i, j, k)*(pk(i, j, k+1)-pk(i, j, k))/&
&                   (rg*(peln(i, k+1, j)-peln(i, k, j)))
                END DO
              END DO
            ELSE
              DO k=1,km
                DO i=is,ie
!#ifdef MOIST_CAPPA
!#ifdef USE_NWAT3
!                     q_liq = q(i,j,k,liq_wat)
!                     q_sol = q(i,j,k,ice_wat)
!#else
!                     q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!                     q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!!                    cappa(i,j,k) = rdgas/(rdgas + ((1.-(q(i,j,k,sphum)+q_liq+q_sol))*cv_air + q(i,j,k,sphum)*cv_vap +  &
!!                                                        q_liq*c_liq + q_sol*c_ice)/(1.+r_vir*q(i,j,k,sphum)))
!                     q_con(i,j,k) = q_liq + q_sol
!                     cvm = (1.-(q(i,j,k,sphum)+q_Con(i,j,k)))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!                     cappa(i,j,k) = rdgas/(rdgas + cvm*(1.-q_con(i,j,k))/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k)))
!                     pt(i,j,k) = rcp*pt(i,j,k)*exp(cappa(i,j,k)/(1.-cappa(i,j,k))*log(kapag*delp(i,j,k)/delz(i,j,k)*pt(i,j,k)))
!#else
                  pt(i, j, k) = rcp*pt(i, j, k)*EXP(k1k*LOG(kapag*delp(i&
&                   , j, k)/delz(i, j, k)*pt(i, j, k)))
                END DO
              END DO
            END IF
          END IF
        ELSE
!#endif
          remap_t = .false.
          CALL PKEZ(km, is, ie, js, je, j, pe, pk, akap, peln, pkz, ptop&
&            )
! Compute cp*T + KE
          DO k=1,km
            DO i=is,ie
              te(i, j, k) = 0.25*gridstruct%rsin2(i, j)*(u(i, j, k)**2+u&
&               (i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-(u(i, j, k)&
&               +u(i, j+1, k))*(v(i, j, k)+v(i+1, j, k))*gridstruct%&
&               cosa_s(i, j)) + pt(i, j, k)*pkz(i, j, k)
            END DO
          END DO
        END IF
        IF (.NOT.hydrostatic) THEN
          IF (.NOT.hybrid_z) THEN
!call compute_dz_L32(km, ztop, dz1)
!          else
!               ztop = 18.E3
!               call hybrid_z_dz(km, dz1, ztop, 1.)
!               call mpp_error(FATAL,'==> Error from fv_mapz: hybrid_z works only with L32')
            DO k=1,km
              DO i=is,ie
! ="specific volume"/grav
                delz(i, j, k) = -(delz(i, j, k)/delp(i, j, k))
              END DO
            END DO
          END IF
        END IF
! update ps
        DO i=is,ie
          ps(i, j) = pe1(i, km+1)
        END DO
        IF (hybrid_z) THEN
!--------------------------
! hybrid z_p coordinate
!--------------------------
          DO i=is,ie
            ze1(i, km+1) = ze0(i, j, km+1)
          END DO
!!$AD II-LOOP
          DO k=km,1,-1
            DO i=is,ie
! current height
              ze1(i, k) = ze1(i, k+1) - delz(i, j, k)
            END DO
          END DO
!
! Copy ztop; the top layer must be thick enough to prevent numerical problems.
!
          DO i=is,ie
            ze2(i, 1) = ze1(i, 1)
! Note: ze0 (top) updated
            ze0(i, j, 1) = ze1(i, 1)
          END DO
          DO k=2,km+1
            DO i=is,ie
! specified height
              ze2(i, k) = ze0(i, j, k)
            END DO
          END DO
! Check/fix monotonicity of top 3 layers thickness
!-----------------------------------------------
!       if ( km==32 ) then
!        do i=is,ie
!           do k=1,3
!              z_rat = (ze2(i,1)-ze2(i,4)) / (dz1(1)+dz1(2)+dz1(3))
!              ze2(i,3) = ze2(i,4) + dz1(3)*z_rat
!              ze2(i,2) = ze2(i,3) + dz1(2)*z_rat
!           enddo
!        enddo
!       endif
!-----------------------------------------------
          DO k=1,km
            DO i=is,ie
! density * grav
              deng(i, k) = -(delp(i, j, k)/delz(i, j, k))
            END DO
          END DO
          IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
            CALL REMAP_Z(km, ze1, km, ze2, deng, is, ie, 1, &
&                     abs_kord_tm)
          ELSE
            deng_tj = deng
            CALL REMAP_Z(km, ze1, km, ze2, deng, is, ie, 1, &
&                  abs_kord_tm_pert)
          END IF
!-------------
! Update delz
!-------------
          DO k=1,km
            DO i=is,ie
              delz(i, j, k) = ze2(i, k+1) - ze2(i, k)
            END DO
          END DO
!------------
! update delp
!------------
          DO k=1,km-1
            DO i=is,ie
              dp2(i, k) = -(delz(i, j, k)*deng(i, k))
              pe2(i, k+1) = pe2(i, k) + dp2(i, k)
            END DO
          END DO
          DO i=is,ie
! to reduce rounding error
            dp2(i, km) = pe2(i, km+1) - pe2(i, km)
          END DO
        ELSE
!
! Hybrid sigma-P coordinate:
!
          DO k=2,km
            DO i=is,ie
              pe2(i, k) = ak(k) + bk(k)*pe(i, km+1, j)
            END DO
          END DO
          DO k=1,km
            DO i=is,ie
              dp2(i, k) = pe2(i, k+1) - pe2(i, k)
            END DO
          END DO
        END IF
!------------
! update delp
!------------
        DO k=1,km
          DO i=is,ie
            delp(i, j, k) = dp2(i, k)
          END DO
        END DO
!----------------
! Map constituents
!----------------
        IF (nq .GT. 5) THEN
          IF (kord_tr(1) .EQ. kord_tr_pert(1)) THEN
            CALL MAPN_TRACER(nq, km, km, pe1, pe2, q, dp2, kord_tr, j&
&                         , is, ie, isd, ied, jsd, jed, 0.0_FVPRC, fill)
          ELSE
            q_tj(:, j, :, :) = q(:, j, :, :)
            CALL MAPN_TRACER(nq, km, km, pe1, pe2, q, dp2, kord_tr_pert&
&                      , j, is, ie, isd, ied, jsd, jed, 0.0_FVPRC, fill)
          END IF
        ELSE IF (nq .GT. 0) THEN
! Remap one tracer at a time
          DO iq=1,nq
            IF (kord_tr(iq) .EQ. kord_tr_pert(iq)) THEN
              CALL MAP1_Q2(km, pe1, q(isd:ied, jsd:jed, 1:km, iq), km&
&                       , pe2, q2, dp2, is, ie, 0, kord_tr(iq), j, isd, &
&                       ied, jsd, jed, 0.0_FVPRC)
            ELSE
              q2_tj = q2
              CALL MAP1_Q2(km, pe1, q(isd:ied, jsd:jed, 1:km, iq), km, &
&                    pe2, q2, dp2, is, ie, 0, kord_tr_pert(iq), j, isd, &
&                    ied, jsd, jed, 0.0_FVPRC)
            END IF
!if (fill) call fillz(ie-is+1, km, 1, q2, dp2)
            DO k=1,km
              DO i=is,ie
                q(i, j, k, iq) = q2(i, k)
              END DO
            END DO
          END DO
        END IF
!------------------
! Compute p**Kappa
!------------------
        DO k=1,km+1
          DO i=is,ie
            pk1(i, k) = pk(i, j, k)
          END DO
        END DO
        DO i=is,ie
          pn2(i, 1) = peln(i, 1, j)
          pn2(i, km+1) = peln(i, km+1, j)
          pk2(i, 1) = pk1(i, 1)
          pk2(i, km+1) = pk1(i, km+1)
        END DO
        DO k=2,km
          DO i=is,ie
            pn2(i, k) = LOG(pe2(i, k))
            pk2(i, k) = EXP(akap*pn2(i, k))
          END DO
        END DO
        IF (te_map) THEN
!---------------------
! Compute Total Energy
!---------------------
          DO i=is,ie
            phis(i, km+1) = hs(i, j)
          END DO
!!$AD II-LOOP
          DO k=km,1,-1
            DO i=is,ie
              phis(i, k) = phis(i, k+1) + pt(i, j, k)*(pk1(i, k+1)-pk1(i&
&               , k))
            END DO
          END DO
          DO k=1,km+1
            DO i=is,ie
              phis(i, k) = phis(i, k)*pe1(i, k)
            END DO
          END DO
          DO k=1,km
            DO i=is,ie
              te(i, j, k) = te(i, j, k) + (phis(i, k+1)-phis(i, k))/(pe1&
&               (i, k+1)-pe1(i, k))
            END DO
          END DO
!----------------
! Map Total Energy
!----------------
          IF (gmao_cubic) THEN
            CALL MAP1_CUBIC(km, pe1, km, pe2, te, is, ie, j, is, ie, js&
&                     , je, akap, 1, .true.)
          ELSE IF (kord_tm .EQ. kord_tm_pert) THEN
            CALL MAP1_PPM(km, pe1, gz, km, pe2, te, is, ie, j, is, ie&
&                      , js, je, 1, kord_tm)
          ELSE
            te_tj(:, j, :) = te(:, j, :)
            CALL MAP1_PPM(km, pe1, gz, km, pe2, te, is, ie, j, is, ie, &
&                   js, je, 1, kord_tm_pert)
          END IF
        ELSE IF (remap_t) THEN
!----------------------------------
! Map t using ze1 (hybrid_z) or logp 
!Note: the '1' in remap_z and map1_ppm was a '2' in siena_prerelease. It has been changed to be its value in siena -- 19mar12 lmh
!----------------------------------
          IF (hybrid_z) THEN
            DO k=1,km
              DO i=is,ie
                deng(i, k) = pt(i, j, k)
              END DO
            END DO
            IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
              CALL REMAP_Z(km, ze1, km, ze2, deng, is, ie, 1, &
&                       abs_kord_tm)
            ELSE
              deng_tj = deng
              CALL REMAP_Z(km, ze1, km, ze2, deng, is, ie, 1, &
&                    abs_kord_tm_pert)
            END IF
            DO k=1,km
              DO i=is,ie
                pt(i, j, k) = deng(i, k)
              END DO
            END DO
          ELSE IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
            CALL MAP_SCALAR(km, peln(is:ie, 1:km+1, j), gz, km, pn2, &
&                        pt, is, ie, j, isd, ied, jsd, jed, 1, &
&                        abs_kord_tm, t_min)
          ELSE
            pt_tj(:, j, :) = pt(:, j, :)
            CALL MAP_SCALAR(km, peln(is:ie, 1:km+1, j), gz, km, pn2, pt&
&                     , is, ie, j, isd, ied, jsd, jed, 1, &
&                     abs_kord_tm_pert, t_min)
          END IF
        ELSE IF (gmao_cubic) THEN
!----------------
! Map pt using pk
!----------------
          CALL MAP1_CUBIC(km, pe1, km, pe2, pt, is, ie, j, isd, ied, jsd&
&                   , jed, akap, 3, .false.)
        ELSE IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
          CALL MAP1_PPM(km, pk1, gz, km, pk2, pt, is, ie, j, isd, ied&
&                    , jsd, jed, 1, abs_kord_tm)
        ELSE
          pt_tj(:, j, :) = pt(:, j, :)
          CALL MAP1_PPM(km, pk1, gz, km, pk2, pt, is, ie, j, isd, ied, &
&                 jsd, jed, 1, abs_kord_tm_pert)
        END IF
        IF (.NOT.hydrostatic) THEN
! Remap vertical wind:
          IF (kord_wz .EQ. kord_wz_pert) THEN
            CALL MAP1_PPM(km, pe1, ws(is:ie, j), km, pe2, w, is, ie, &
&                      j, isd, ied, jsd, jed, -2, kord_wz)
          ELSE
            w_tj(:, j, :) = w(:, j, :)
            CALL MAP1_PPM(km, pe1, ws(is:ie, j), km, pe2, w, is, ie, j, &
&                   isd, ied, jsd, jed, -2, kord_wz_pert)
          END IF
          IF (.NOT.hybrid_z) THEN
! Remap delz for hybrid sigma-p coordinate
            IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
              CALL MAP1_PPM(km, pe1, gz, km, pe2, delz, is, ie, j, &
&                        isd, ied, jsd, jed, 1, abs_kord_tm)
            ELSE
              delz_tj(:, j, :) = delz(:, j, :)
              CALL MAP1_PPM(km, pe1, gz, km, pe2, delz, is, ie, j, isd, &
&                     ied, jsd, jed, 1, abs_kord_tm_pert)
            END IF
            DO k=1,km
              DO i=is,ie
                delz(i, j, k) = -(delz(i, j, k)*dp2(i, k))
              END DO
            END DO
          END IF
        END IF
!----------
! Update pk
!----------
        DO k=2,km
          DO i=is,ie
            pk(i, j, k) = pk2(i, k)
          END DO
        END DO
!----------------
        IF (do_omega) THEN
          DO i=is,ie
            pe3(i, 1) = 0.
          END DO
          DO k=2,km+1
            DO i=is,ie
              pe3(i, k) = omga(i, j, k-1)
            END DO
          END DO
        END IF
        DO k=1,km+1
          DO i=is,ie
            pe0(i, k) = peln(i, k, j)
            peln(i, k, j) = pn2(i, k)
          END DO
        END DO
!------------
! Compute pkz
!------------
        IF (hydrostatic) THEN
          DO k=1,km
            DO i=is,ie
              pkz(i, j, k) = (pk2(i, k+1)-pk2(i, k))/(akap*(peln(i, k+1&
&               , j)-peln(i, k, j)))
            END DO
          END DO
        ELSE IF (remap_t) THEN
! Note: pt at this stage is T_v or T_m
          DO k=1,km
            DO i=is,ie
!#ifdef MOIST_CAPPA
!#ifdef USE_NWAT3
!            q_liq = q(i,j,k,liq_wat)
!            q_sol = q(i,j,k,ice_wat)
!#else
!            q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!            q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!            q_con(i,j,k) = q_liq + q_sol
!!           cappa(i,j,k) = rdgas/(rdgas+((1.-q(i,j,k,sphum)-q_con(i,j,k))*cv_air + q(i,j,k,sphum)*cv_vap +  &
!!                                            q_liq*c_liq + q_sol*c_ice)/(1.+r_vir*q(i,j,k,sphum)))
!            cvm = (1.-(q(i,j,k,sphum)+q_con(i,j,k)))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!            cappa(i,j,k) = rdgas/(rdgas + cvm*(1.-q_con(i,j,k))/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k)))
!            pkz(i,j,k) = exp(cappa(i,j,k)*log(rrg*delp(i,j,k)/delz(i,j,k)*pt(i,j,k)))
!#else
              pkz(i, j, k) = EXP(akap*LOG(rrg*delp(i, j, k)/delz(i, j, k&
&               )*pt(i, j, k)))
            END DO
          END DO
        ELSE
!#endif
! Note: pt at this stage is cp*Theta_v
          DO k=1,km
            DO i=is,ie
!#ifdef MOIST_CAPPA
!            pkz(i,j,k) = exp(cappa(i,j,k)/(1.-cappa(i,j,k))*log(kapag*delp(i,j,k)/delz(i,j,k)*pt(i,j,k)) )
!#else
              pkz(i, j, k) = EXP(k1k*LOG(kapag*delp(i, j, k)/delz(i, j, &
&               k)*pt(i, j, k)))
            END DO
          END DO
        END IF
!#endif
! end do_omega
! Interpolate omega/pe3 (defined at pe0) to remapped cell center (dp2)
        IF (do_omega) THEN
          DO k=1,km
            DO i=is,ie
              dp2(i, k) = 0.5*(peln(i, k, j)+peln(i, k+1, j))
            END DO
          END DO
          DO i=is,ie
            k_next = 1
            DO 100 n=1,km
              kp = k_next
              DO k=kp,km
                IF (dp2(i, n) .LE. pe0(i, k+1) .AND. dp2(i, n) .GE. pe0(&
&                   i, k)) THEN
                  omga(i, j, n) = pe3(i, k) + (pe3(i, k+1)-pe3(i, k))*(&
&                   dp2(i, n)-pe0(i, k))/(pe0(i, k+1)-pe0(i, k))
                  k_next = k
                  GOTO 100
                END IF
              END DO
 100        CONTINUE
          END DO
        END IF
      END IF
! end hybrid_z check
      IF (.NOT.hybrid_z) THEN
        DO i=is,ie+1
          pe0(i, 1) = pe(i, 1, j)
        END DO
!------
! map u
!------
        DO k=2,km+1
          DO i=is,ie
            pe0(i, k) = 0.5*(pe(i, k, j-1)+pe1(i, k))
          END DO
        END DO
        DO k=1,km+1
          bkh = 0.5*bk(k)
          DO i=is,ie
            pe3(i, k) = ak(k) + bkh*(pe(i, km+1, j-1)+pe1(i, km+1))
          END DO
        END DO
        IF (kord_mt .EQ. kord_mt_pert) THEN
          CALL MAP1_PPM(km, pe0(is:ie, 1:km+1), gz, km, pe3(is:ie, 1:&
&                    km+1), u, is, ie, j, isd, ied, jsd, jedp1, -1, &
&                    kord_mt)
        ELSE
          u_tj(:, j, :) = u(:, j, :)
          CALL MAP1_PPM(km, pe0(is:ie, 1:km+1), gz, km, pe3(is:ie, 1:km+&
&                 1), u, is, ie, j, isd, ied, jsd, jedp1, -1, &
&                 kord_mt_pert)
        END IF
! (j < je+1)
        IF (j .LT. je + 1) THEN
!------
! map v
!------
          DO i=is,ie+1
            pe3(i, 1) = ak(1)
          END DO
          DO k=2,km+1
            bkh = 0.5*bk(k)
            DO i=is,ie+1
              pe0(i, k) = 0.5*(pe(i-1, k, j)+pe(i, k, j))
              pe3(i, k) = ak(k) + bkh*(pe(i-1, km+1, j)+pe(i, km+1, j))
            END DO
          END DO
          IF (kord_mt .EQ. kord_mt_pert) THEN
            CALL MAP1_PPM(km, pe0, gz, km, pe3, v, is, iep1, j, isd, &
&                      iedp1, jsd, jed, -1, kord_mt)
          ELSE
            v_tj(:, j, :) = v(:, j, :)
            CALL MAP1_PPM(km, pe0, gz, km, pe3, v, is, iep1, j, isd, &
&                   iedp1, jsd, jed, -1, kord_mt_pert)
          END IF
        END IF
      END IF
      DO k=1,km
        DO i=is,ie
          ua(i, j, k) = pe2(i, k+1)
        END DO
      END DO
    END DO
    IF (hybrid_z) THEN
!------- Hybrid_z section ---------------
      IF (gridstruct%square_domain) THEN
        CALL MPP_UPDATE_DOMAINS(ua, domain, complete=.true., whalo=1, &
&                         ehalo=1, shalo=1, nhalo=1)
      ELSE
        CALL MPP_UPDATE_DOMAINS(ua, domain, complete=.true.)
      END IF
! u-wind
!$OMP parallel do default(none) shared(is,ie,js,je,km,ptop,u,gz,pe,ua,isd,ied,jsd,jed,kord_mt) &
!$OMP                          private(pe1, pe2)
      DO j=js,je+1
        DO i=is,ie
          pe1(i, 1) = ptop
          pe2(i, 1) = ptop
        END DO
        DO k=2,km+1
          DO i=is,ie
            pe1(i, k) = 0.5*(pe(i, k, j-1)+pe(i, k, j))
            pe2(i, k) = 0.5*(ua(i, j-1, k-1)+ua(i, j, k-1))
          END DO
        END DO
        IF (kord_mt .EQ. kord_mt_pert) THEN
          CALL MAP1_PPM(km, pe1, gz, km, pe2, u, is, ie, j, isd, ied&
&                    , jsd, jedp1, -1, kord_mt)
        ELSE
          u_tj(:, j, :) = u(:, j, :)
          CALL MAP1_PPM(km, pe1, gz, km, pe2, u, is, ie, j, isd, ied, &
&                 jsd, jedp1, -1, kord_mt_pert)
        END IF
      END DO
! v-wind
!$OMP parallel do default(none) shared(is,ie,js,je,ptop,pe,ua,km,v,gz,isd,ied,jsd,jed,kord_mt) &
!$OMP                          private(pe0, pe3)
      DO j=js,je
        DO i=is,ie+1
          pe0(i, 1) = ptop
          pe3(i, 1) = ptop
        END DO
        DO k=2,km+1
          DO i=is,ie+1
            pe0(i, k) = 0.5*(pe(i-1, k, j)+pe(i, k, j))
            pe3(i, k) = 0.5*(ua(i-1, j, k-1)+ua(i, j, k-1))
          END DO
        END DO
        IF (kord_mt .EQ. kord_mt_pert) THEN
          CALL MAP1_PPM(km, pe0, gz, km, pe3, v, is, iep1, j, isd, &
&                    iedp1, jsd, jed, -1, kord_mt)
        ELSE
          v_tj(:, j, :) = v(:, j, :)
          CALL MAP1_PPM(km, pe0, gz, km, pe3, v, is, iep1, j, isd, iedp1&
&                 , jsd, jed, -1, kord_mt_pert)
        END IF
      END DO
    END IF
!------------- Hybrid_z section ----------------------
!$OMP parallel do default(none) shared(is,ie,js,je,km,pe,ua)
    DO k=2,km
      DO j=js,je
        DO i=is,ie
          pe(i, k, j) = ua(i, j, k-1)
        END DO
      END DO
    END DO
! end consv check
!  call cubed_to_latlon(u,  v, ua, va, dx, dy, rdxa, rdya, km, 1, flagstruct%c2l_ord)
    IF (last_step .AND. consv .GT. 0. .AND. (.NOT.do_adiabatic_init)) &
&   THEN
      IF (te_map) THEN
!$OMP parallel do default(none) shared(is,ie,js,je,km,te_2d,te,delp)
        DO j=js,je
          DO i=is,ie
            te_2d(i, j) = te(i, j, 1)*delp(i, j, 1)
          END DO
          DO k=2,km
            DO i=is,ie
              te_2d(i, j) = te_2d(i, j) + te(i, j, k)*delp(i, j, k)
            END DO
          END DO
        END DO
      ELSE
!$OMP parallel do default(none) shared(is,ie,js,je,km,remap_t,hydrostatic,hs,rg,pt,peln, &
!$OMP                                  te_2d,pe,delp,cp,rsin2,u,v,cosa_s,delz,rainwat,   &
!$OMP                                  liq_wat,ice_wat,snowwat,graupel,q_con,r_vir,      &
!$OMP                                  sphum,w,pk,pkz,rgama)                             &
!$OMP                          private(q_liq, q_sol, cvm, gz, phis)
        DO j=js,je
          IF (remap_t) THEN
            IF (hydrostatic) THEN
              DO i=is,ie
                gz(i) = hs(i, j)
                DO k=1,km
                  gz(i) = gz(i) + rg*pt(i, j, k)*(peln(i, k+1, j)-peln(i&
&                   , k, j))
                END DO
              END DO
              DO i=is,ie
                te_2d(i, j) = pe(i, km+1, j)*hs(i, j) - pe(i, 1, j)*gz(i&
&                 )
              END DO
              DO k=1,km
                DO i=is,ie
                  te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cp*pt(i, j&
&                   , k)+0.25*gridstruct%rsin2(i, j)*(u(i, j, k)**2+u(i&
&                   , j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-(u(i, j, &
&                   k)+u(i, j+1, k))*(v(i, j, k)+v(i+1, j, k))*&
&                   gridstruct%cosa_s(i, j)))
                END DO
              END DO
            ELSE
! non-hydrostatic & remap_t
              DO i=is,ie
                phis(i, km+1) = hs(i, j)
                DO k=km,1,-1
                  phis(i, k) = phis(i, k+1) - grav*delz(i, j, k)
                END DO
              END DO
              DO i=is,ie
                te_2d(i, j) = 0.
              END DO
              DO k=1,km
                DO i=is,ie
! KE using 3D winds:
!#ifdef USE_COND
!#ifdef USE_NWAT3
!           q_liq = q(i,j,k,liq_wat)
!           q_sol = q(i,j,k,ice_wat)
!!#else
!           q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!           q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!           q_con(i,j,k) = q_liq + q_sol
!#ifdef MOIST_CAPPA
!           cvm = (1.-(q(i,j,k,sphum)+q_con(i,j,k)))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!#else
!           cvm = cv_air
!#endif
!                 te_2d(i,j) = te_2d(i,j) + delp(i,j,k)*(cvm*pt(i,j,k)/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k)) + &
!#else
!#endif
                  te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cv_air*pt(i&
&                   , j, k)/(1.+r_vir*q(i, j, k, sphum))+0.5*(phis(i, k)&
&                   +phis(i, k+1)+w(i, j, k)**2+0.5*gridstruct%rsin2(i, &
&                   j)*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+&
&                   1, j, k)**2-(u(i, j, k)+u(i, j+1, k))*(v(i, j, k)+v(&
&                   i+1, j, k))*gridstruct%cosa_s(i, j))))
                END DO
              END DO
            END IF
          ELSE IF (hydrostatic) THEN
            DO i=is,ie
              gz(i) = hs(i, j)
              DO k=1,km
                gz(i) = gz(i) + pt(i, j, k)*(pk(i, j, k+1)-pk(i, j, k))
              END DO
            END DO
            DO i=is,ie
              te_2d(i, j) = pe(i, km+1, j)*hs(i, j) - pe(i, 1, j)*gz(i)
            END DO
            DO k=1,km
              DO i=is,ie
                te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(pt(i, j, k)*&
&                 pkz(i, j, k)+0.25*gridstruct%rsin2(i, j)*(u(i, j, k)**&
&                 2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-(u(i, &
&                 j, k)+u(i, j+1, k))*(v(i, j, k)+v(i+1, j, k))*&
&                 gridstruct%cosa_s(i, j)))
              END DO
            END DO
          ELSE
!-----------------
! Non-hydrostatic:
!-----------------
            DO i=is,ie
              phis(i, km+1) = hs(i, j)
              DO k=km,1,-1
                phis(i, k) = phis(i, k+1) - grav*delz(i, j, k)
              END DO
            END DO
            DO i=is,ie
              te_2d(i, j) = 0.
            END DO
            DO k=1,km
              DO i=is,ie
! KE using 3D winds:
!#ifdef USE_COND
!#ifdef USE_NWAT3
!                 q_con(i,j,k) = q(i,j,k,liq_wat) + q(i,j,k,ice_wat)
!#else
!                 q_con(i,j,k) = q(i,j,k,liq_wat) + q(i,j,k,rainwat) + q(i,j,k,ice_wat)  &
!                              + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!                 te_2d(i,j) = te_2d(i,j) + delp(i,j,k)*(rgama*pt(i,j,k)*pkz(i,j,k)/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k)) + &
!#else
!#endif
                te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(rgama*pt(i, j&
&                 , k)*pkz(i, j, k)/(1.+r_vir*q(i, j, k, sphum))+0.5*(&
&                 phis(i, k)+phis(i, k+1)+w(i, j, k)**2+0.5*gridstruct%&
&                 rsin2(i, j)*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)&
&                 **2+v(i+1, j, k)**2-(u(i, j, k)+u(i, j+1, k))*(v(i, j&
&                 , k)+v(i+1, j, k))*gridstruct%cosa_s(i, j))))
              END DO
            END DO
          END IF
        END DO
      END IF
! openMP j-loop
!$OMP parallel do default(none) shared(is,ie,js,je,zsum1,pkz,delp,km,zsum0,ptop,pk,te_2d,te0_2d)
      DO j=js,je
        DO i=is,ie
          zsum1(i, j) = pkz(i, j, 1)*delp(i, j, 1)
        END DO
        DO k=2,km
          DO i=is,ie
            zsum1(i, j) = zsum1(i, j) + pkz(i, j, k)*delp(i, j, k)
          END DO
        END DO
        DO i=is,ie
          zsum0(i, j) = ptop*(pk(i, j, 1)-pk(i, j, km+1)) + zsum1(i, j)
          te_2d(i, j) = te0_2d(i, j) - te_2d(i, j)
        END DO
      END DO
      result1 = G_SUM(domain, te_2d, is, ie, js, je, ng, gridstruct%&
&       area_64, 0)
      tpe = consv*result1
! unit: W/m**2
      e_flux = tpe/(grav*pdt*4.*pi*radius**2)
! Note pdt is "phys" time step
      IF (hydrostatic) THEN
        result1 = G_SUM(domain, zsum0, is, ie, js, je, ng, gridstruct%&
&         area_64, 0)
        dtmp = tpe/(cp*result1)
      ELSE
        result1 = G_SUM(domain, zsum1, is, ie, js, je, ng, gridstruct%&
&         area_64, 0)
        dtmp = tpe/(cv_air*result1)
      END IF
!-------------------------------------------------------------------------------
! One may use this quick fix to ensure reproducibility at the expense of a lower
! floating precision; this is fine for the TE correction
!-------------------------------------------------------------------------------
! convert to 4-byte real
      IF (reproduce_sum) dtmp = REAL(dtmp, 4)
    ELSE
      dtmp = 0.
      e_flux = 0.
!#ifdef USE_COND
!!$OMP parallel do default(none) shared(is,ie,js,je,km,q_con,q,liq_wat,ice_wat, &
!!$OMP                                  rainwat,snowwat,graupel)
!      do k=1,km
!         do j=js,je
!         do i=is,ie
!#ifdef USE_NWAT3
!            q_con(i,j,k) = q(i,j,k,liq_wat)+q(i,j,k,ice_wat)
!#else
!            q_con(i,j,k) = q(i,j,k,liq_wat)+q(i,j,k,rainwat)+q(i,j,k,ice_wat)+q(i,j,k,snowwat)+q(i,j,k,graupel)
!#endif
!         enddo
!         enddo
!      enddo
!#endif
    END IF
    IF (te_map) THEN
!kord_mt >= 0
!$OMP parallel do default(none) shared(is,ie,js,je,km,hs,te,rsin2,u,v,cosa_s,rg,peln, &
!$OMP                                  cp,pe,delp,pt,pkz,dtmp) &
!$OMP                          private(gz, tpe, tmp, dlnp)
      DO j=js,je
        DO i=is,ie
          gz(i) = hs(i, j)
        END DO
        DO k=km,1,-1
          DO i=is,ie
            tpe = te(i, j, k) - gz(i) - 0.25*gridstruct%rsin2(i, j)*(u(i&
&             , j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-(&
&             u(i, j, k)+u(i, j+1, k))*(v(i, j, k)+v(i+1, j, k))*&
&             gridstruct%cosa_s(i, j))
            dlnp = rg*(peln(i, k+1, j)-peln(i, k, j))
            tmp = tpe/(cp-pe(i, k, j)*dlnp/delp(i, j, k))
            pt(i, j, k) = cp*(tmp/pkz(i, j, k)+dtmp)
            gz(i) = gz(i) + dlnp*tmp
          END DO
        END DO
      END DO
    ELSE IF (remap_t) THEN
! end k-loop
!#ifndef MAPL_MODE
!        ! Note: pt at this stage is T_v
!        if ( .not. do_adiabatic_init ) then
!#ifdef FAST_LIN_MP
!! Notes: must set consv=0
!                                           call timing_on('FAST_LIN_MP')
!      call lin_fast_mp(u, v, w, pt, q, sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, cld_amt,  & 
!                       delp, delz, gridstruct, mdt, lprec, fprec, f_land,      &
!                       hydrostatic, phys_hydrostatic, time)
!                                           call timing_off('FAST_LIN_MP')
!#else
!          if ( do_sat_adj ) then
!                                           call timing_on('sat_adj2')
!!$OMP parallel do default(none) shared(is,ie,js,je,km,peln,ng,mdt,hydrostatic,consv,te, &
!!$OMP                                  q,isd,jsd,sphum,liq_wat,ice_wat,rainwat,snowwat, &
!!$OMP                                  graupel,cld_amt,gridstruct,delz,pt,delp,         &
!!$OMP                                  q_con,cappa,dtdt,out_dt,last_step,pkz,rrg,akap ) &
!!$OMP                          private(dpeln)
!           do k=1,km
!              do j=js,je
!                 do i=is,ie
!                    dpeln(i,j) = peln(i,k+1,j) - peln(i,k,j)
!                 enddo
!              enddo
!              call sat_adj2(mdt, is, ie, js, je, ng, hydrostatic, consv>0., &
!                            te(is,js,k), q(isd,jsd,k,sphum), q(isd,jsd,k,liq_wat),   &
!                            q(isd,jsd,k,ice_wat), q(isd,jsd,k,rainwat),    &
!                            q(isd,jsd,k,snowwat), q(isd,jsd,k,graupel), q(isd,jsd,k,cld_amt), gridstruct%area(isd,jsd), &
!                            dpeln, delz(isd:,jsd:,k), pt(isd,jsd,k), delp(isd,jsd,k),           &
!                            q_con(isd:,jsd:,k), cappa(isd:,jsd:,k), dtdt(is,js,k), out_dt, last_step)
!              if ( .not. hydrostatic  ) then
!                 do j=js,je
!                    do i=is,ie
!#ifdef MOIST_CAPPA
!                       pkz(i,j,k) = exp(cappa(i,j,k)*log(rrg*delp(i,j,k)/delz(i,j,k)*pt(i,j,k)))
!#else
!                       pkz(i,j,k) = exp(akap*log(rrg*delp(i,j,k)/delz(i,j,k)*pt(i,j,k)))
!#endif
!                    enddo
!                 enddo
!              endif
!           enddo    ! OpenMP k-loop
!           if ( consv > 0. ) then
!!$OMP parallel do default(none) shared(is,ie,js,je,km,te_2d,te,te0_2d)
!                do j=js,je
!                   do k=1,km
!                      do i=is,ie
!                         te0_2d(i,j) = te0_2d(i,j) + te(i,j,k)
!                      enddo
!                   enddo
!                enddo
!           endif
!                                           call timing_off('sat_adj2')
!         endif
!#endif
!        endif
!#endif
      IF (last_step) THEN
        IF (hydrostatic) THEN
!$OMP parallel do default(none) shared(is,ie,js,je,km,pt,dtmp,pkz,r_vir,q,sphum)
          DO k=1,km
            DO j=js,je
              DO i=is,ie
                pt(i, j, k) = (pt(i, j, k)+dtmp*pkz(i, j, k))/(1.+r_vir*&
&                 q(i, j, k, sphum))
              END DO
            END DO
          END DO
        ELSE
!$OMP parallel do default(none) shared(is,ie,js,je,km,q,liq_wat,ice_wat,rainwat,snowwat, &
!$OMP                                  graupel,q_con,pt,dtmp,pkz,r_vir,sphum )           &
!$OMP                          private(q_liq, q_sol, cvm)
          DO k=1,km
            DO j=js,je
              DO i=is,ie
! Output temperature if last_step
!#ifdef USE_COND
!#ifdef MOIST_CAPPA
!#ifdef USE_NWAT3
!           q_liq = q(i,j,k,liq_wat)
!           q_sol = q(i,j,k,ice_wat)
!#else
!           q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!           q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!           cvm = (1.-(q(i,j,k,sphum)+q_con(i,j,k)))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!
!                    pt(i,j,k) = (pt(i,j,k)+dtmp*cv_air/cvm*pkz(i,j,k))/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k))
!#else
!                    pt(i,j,k) = (pt(i,j,k)+dtmp*pkz(i,j,k))/(1.+r_vir*q(i,j,k,sphum)-q_con(i,j,k))
!#endif
!#else
                pt(i, j, k) = (pt(i, j, k)+dtmp*pkz(i, j, k))/(1.+r_vir*&
&                 q(i, j, k, sphum))
              END DO
            END DO
          END DO
        END IF
      ELSE
!#endif
!$OMP parallel do default(none) shared(is,ie,js,je,km,pt,cp,pkz,dtmp,q,liq_wat,ice_wat, &
!$OMP                                  rainwat,snowwat,graupel,sphum,q_con) &
!$OMP                          private(q_liq, q_sol, cvm)
        DO k=1,km
          DO j=js,je
            DO i=is,ie
!#ifdef MOIST_CAPPA
!#ifdef USE_NWAT3
!           q_liq = q(i,j,k,liq_wat)
!           q_sol = q(i,j,k,ice_wat)
!#else
!           q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!           q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!           cvm = (1.-(q(i,j,k,sphum)+q_con(i,j,k)))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!                    pt(i,j,k) = cp*(pt(i,j,k)/pkz(i,j,k) + dtmp*cv_air/cvm)
!#else
              pt(i, j, k) = cp*(pt(i, j, k)/pkz(i, j, k)+dtmp)
            END DO
          END DO
        END DO
      END IF
    ELSE
!#endif
!$OMP parallel do default(none) shared(is,ie,js,je,km,pt,cp,dtmp)
      DO k=1,km
        DO j=js,je
          DO i=is,ie
            pt(i, j, k) = pt(i, j, k) + cp*dtmp
          END DO
        END DO
      END DO
    END IF
  END SUBROUTINE LAGRANGIAN_TO_EULERIAN
!  Differentiation of compute_total_energy in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 
!a2b_edge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_co
!re_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_
!p dyn_core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dyna
!mics_mod.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_
!mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute
!_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer_
!fb fv_mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_m
!od.steepz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod
!.riem_solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_so
!lver nh_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod
!.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corner
!s sw_core_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d_f
!b tp_core_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm_
!fb tp_core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: qc peln q u v w teq delp delz
!                te_2d pe pt
!   with respect to varying inputs: qc peln q u v w delp delz pe
!                pt
  SUBROUTINE COMPUTE_TOTAL_ENERGY_FWD(is, ie, js, je, isd, ied, jsd, jed&
&   , km, u, v, w, delz, pt, delp, q, qc, pe, peln, hs, rsin2_l, &
&   cosa_s_l, r_vir, cp, rg, hlv, te_2d, ua, va, teq, moist_phys, sphum&
&   , liq_wat, rainwat, ice_wat, snowwat, graupel, hydrostatic, id_te)
    IMPLICIT NONE
!------------------------------------------------------
! Compute vertically integrated total energy per column
!------------------------------------------------------
! !INPUT PARAMETERS:
    INTEGER, INTENT(IN) :: km, is, ie, js, je, isd, ied, jsd, jed, id_te
    INTEGER, INTENT(IN) :: sphum, liq_wat, ice_wat, rainwat, snowwat, &
&   graupel
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(INOUT) :: ua, &
&   va
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(IN) :: pt, delp
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km, *), INTENT(IN) :: q
    REAL(fvprc), DIMENSION(is:ie, js:je, km), INTENT(IN) :: qc
    REAL(fvprc), INTENT(INOUT) :: u(isd:ied, jsd:jed+1, km)
    REAL(fvprc), INTENT(INOUT) :: v(isd:ied+1, jsd:jed, km)
! vertical velocity (m/s)
    REAL(fvprc), INTENT(IN) :: w(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(IN) :: delz(isd:ied, jsd:jed, km)
! surface geopotential
    REAL(fvprc), INTENT(IN) :: hs(isd:ied, jsd:jed)
! pressure at layer edges
    REAL(fvprc), INTENT(IN) :: pe(is-1:ie+1, km+1, js-1:je+1)
! log(pe)
    REAL(fvprc), INTENT(IN) :: peln(is:ie, km+1, js:je)
    REAL(fvprc), INTENT(IN) :: cp, rg, r_vir, hlv
    REAL(fvprc), INTENT(IN) :: rsin2_l(isd:ied, jsd:jed)
    REAL(fvprc), INTENT(IN) :: cosa_s_l(isd:ied, jsd:jed)
    LOGICAL, INTENT(IN) :: moist_phys, hydrostatic
! Output:
! vertically integrated TE
    REAL(fvprc) :: te_2d(is:ie, js:je)
! Moist TE
    REAL(fvprc) :: teq(is:ie, js:je)
! Local
    REAL(fvprc), DIMENSION(is:ie, km) :: tv
    REAL(fvprc) :: phiz(is:ie, km+1)
    REAL(fvprc) :: cvm, q_liq, q_sol
    INTEGER :: i, j, k
!----------------------
! Output lat-lon winds:
!----------------------
!  call cubed_to_latlon(u, v, ua, va, dx, dy, rdxa, rdya, km, flagstruct%c2l_ord)
!$OMP parallel do default(none) shared(is,ie,js,je,km,hydrostatic,hs,pt,qc,rg,peln,te_2d, &
!$OMP                                  q,pe,delp,cp,rsin2_l,u,v,cosa_s_l,delz,moist_phys,w, &
!$OMP                                  liq_wat,rainwat,ice_wat,snowwat,graupel,sphum)   &
!$OMP                          private(phiz, tv, cvm, q_sol, q_liq)
    DO j=js,je
      IF (hydrostatic) THEN
        DO i=is,ie
          CALL PUSHREALARRAY(phiz(i, km+1))
          phiz(i, km+1) = hs(i, j)
        END DO
        DO k=km,1,-1
          DO i=is,ie
            CALL PUSHREALARRAY(tv(i, k))
            tv(i, k) = pt(i, j, k)*(1.+qc(i, j, k))
            CALL PUSHREALARRAY(phiz(i, k))
            phiz(i, k) = phiz(i, k+1) + rg*tv(i, k)*(peln(i, k+1, j)-&
&             peln(i, k, j))
          END DO
        END DO
        DO i=is,ie
          te_2d(i, j) = pe(i, km+1, j)*phiz(i, km+1) - pe(i, 1, j)*phiz(&
&           i, 1)
        END DO
        DO k=1,km
          DO i=is,ie
            te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cp*tv(i, k)+0.25*&
&             rsin2_l(i, j)*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2&
&             +v(i+1, j, k)**2-(u(i, j, k)+u(i, j+1, k))*(v(i, j, k)+v(i&
&             +1, j, k))*cosa_s_l(i, j)))
          END DO
        END DO
        CALL PUSHCONTROL2B_FV(2)
      ELSE
!-----------------
! Non-hydrostatic:
!-----------------
        DO i=is,ie
          CALL PUSHREALARRAY(phiz(i, km+1))
          phiz(i, km+1) = hs(i, j)
          DO k=km,1,-1
            CALL PUSHREALARRAY(phiz(i, k))
            phiz(i, k) = phiz(i, k+1) - grav*delz(i, j, k)
          END DO
        END DO
        DO i=is,ie
          te_2d(i, j) = 0.
        END DO
        IF (moist_phys) THEN
          DO k=1,km
            DO i=is,ie
!#if defined(USE_COND) && defined(MOIST_CAPPA)
!#ifdef USE_NWAT3
!           q_liq = q(i,j,k,liq_wat)
!           q_sol = q(i,j,k,ice_wat)
!#else
!           q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!           q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!           cvm = (1.-(q(i,j,k,sphum)+q_liq+q_sol))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!           te_2d(i,j) = te_2d(i,j) + delp(i,j,k)*( cvm*pt(i,j,k) +  &
!#else
!#endif
              te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cv_air*pt(i, j&
&               , k)+0.5*(phiz(i, k)+phiz(i, k+1)+w(i, j, k)**2+0.5*&
&               rsin2_l(i, j)*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)&
&               **2+v(i+1, j, k)**2-(u(i, j, k)+u(i, j+1, k))*(v(i, j, k&
&               )+v(i+1, j, k))*cosa_s_l(i, j))))
            END DO
          END DO
          CALL PUSHCONTROL2B_FV(1)
        ELSE
          DO k=1,km
            DO i=is,ie
              te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cv_air*pt(i, j&
&               , k)+0.5*(phiz(i, k)+phiz(i, k+1)+w(i, j, k)**2+0.5*&
&               rsin2_l(i, j)*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)&
&               **2+v(i+1, j, k)**2-(u(i, j, k)+u(i, j+1, k))*(v(i, j, k&
&               )+v(i+1, j, k))*cosa_s_l(i, j))))
            END DO
          END DO
          CALL PUSHCONTROL2B_FV(0)
        END IF
      END IF
    END DO
!-------------------------------------
! Doganostics computation for moist TE
!-------------------------------------
    IF (id_te .GT. 0) THEN
!$OMP parallel do default(none) shared(is,ie,js,je,teq,te_2d,moist_phys,km,hlv,sphum,q,delp)
      DO j=js,je
        DO i=is,ie
          teq(i, j) = te_2d(i, j)
        END DO
        IF (moist_phys) THEN
          DO k=1,km
            DO i=is,ie
              teq(i, j) = teq(i, j) + hlv*q(i, j, k, sphum)*delp(i, j, k&
&               )
            END DO
          END DO
          CALL PUSHCONTROL1B_FV(1)
        ELSE
          CALL PUSHCONTROL1B_FV(0)
        END IF
      END DO
      CALL PUSHREALARRAY(phiz, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(tv, 8*(ie-is+1)*km/8)
      CALL PUSHCONTROL1B_FV(1)
    ELSE
      CALL PUSHREALARRAY(phiz, 8*(ie-is+1)*(km+1)/8)
      CALL PUSHREALARRAY(tv, 8*(ie-is+1)*km/8)
      CALL PUSHCONTROL1B_FV(0)
    END IF
  END SUBROUTINE COMPUTE_TOTAL_ENERGY_FWD
!  Differentiation of compute_total_energy in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord2
! a2b_edge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_c
!ore_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad
!_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dyn
!amics_mod.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils
!_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.comput
!e_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer
!_fb fv_mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_
!mod.steepz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mo
!d.riem_solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_s
!olver nh_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mo
!d.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corne
!rs sw_core_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d_
!fb tp_core_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm
!_fb tp_core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: qc peln q u v w teq delp delz
!                te_2d pe pt
!   with respect to varying inputs: qc peln q u v w delp delz pe
!                pt
  SUBROUTINE COMPUTE_TOTAL_ENERGY_BWD(is, ie, js, je, isd, ied, jsd, jed&
&   , km, u, u_ad, v, v_ad, w, w_ad, delz, delz_ad, pt, pt_ad, delp, &
&   delp_ad, q, q_ad, qc, qc_ad, pe, pe_ad, peln, peln_ad, hs, rsin2_l, &
&   cosa_s_l, r_vir, cp, rg, hlv, te_2d, te_2d_ad, ua, va, teq, teq_ad, &
&   moist_phys, sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, &
&   hydrostatic, id_te)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: km, is, ie, js, je, isd, ied, jsd, jed, id_te
    INTEGER, INTENT(IN) :: sphum, liq_wat, ice_wat, rainwat, snowwat, &
&   graupel
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(INOUT) :: ua, &
&   va
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(IN) :: pt, delp
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km) :: pt_ad, delp_ad
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km, *), INTENT(IN) :: q
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km, *) :: q_ad
    REAL(fvprc), DIMENSION(is:ie, js:je, km), INTENT(IN) :: qc
    REAL(fvprc), DIMENSION(is:ie, js:je, km) :: qc_ad
    REAL(fvprc), INTENT(INOUT) :: u(isd:ied, jsd:jed+1, km)
    REAL(fvprc), INTENT(INOUT) :: u_ad(isd:ied, jsd:jed+1, km)
    REAL(fvprc), INTENT(INOUT) :: v(isd:ied+1, jsd:jed, km)
    REAL(fvprc), INTENT(INOUT) :: v_ad(isd:ied+1, jsd:jed, km)
    REAL(fvprc), INTENT(IN) :: w(isd:ied, jsd:jed, km)
    REAL(fvprc) :: w_ad(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(IN) :: delz(isd:ied, jsd:jed, km)
    REAL(fvprc) :: delz_ad(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(IN) :: hs(isd:ied, jsd:jed)
    REAL(fvprc), INTENT(IN) :: pe(is-1:ie+1, km+1, js-1:je+1)
    REAL(fvprc) :: pe_ad(is-1:ie+1, km+1, js-1:je+1)
    REAL(fvprc), INTENT(IN) :: peln(is:ie, km+1, js:je)
    REAL(fvprc) :: peln_ad(is:ie, km+1, js:je)
    REAL(fvprc), INTENT(IN) :: cp, rg, r_vir, hlv
    REAL(fvprc), INTENT(IN) :: rsin2_l(isd:ied, jsd:jed)
    REAL(fvprc), INTENT(IN) :: cosa_s_l(isd:ied, jsd:jed)
    LOGICAL, INTENT(IN) :: moist_phys, hydrostatic
    REAL(fvprc) :: te_2d(is:ie, js:je)
    REAL(fvprc) :: te_2d_ad(is:ie, js:je)
    REAL(fvprc) :: teq(is:ie, js:je)
    REAL(fvprc) :: teq_ad(is:ie, js:je)
    REAL(fvprc), DIMENSION(is:ie, km) :: tv
    REAL(fvprc), DIMENSION(is:ie, km) :: tv_ad
    REAL(fvprc) :: phiz(is:ie, km+1)
    REAL(fvprc) :: phiz_ad(is:ie, km+1)
    REAL(fvprc) :: cvm, q_liq, q_sol
    INTEGER :: i, j, k
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp2
    REAL(fvprc) :: temp3
    REAL(fvprc) :: temp4
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp5
    REAL(fvprc) :: temp6
    REAL(fvprc) :: temp7
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    INTEGER :: branch
    CALL POPCONTROL1B_FV(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREALARRAY(tv, 8*(ie-is+1)*km/8)
      CALL POPREALARRAY(phiz, 8*(ie-is+1)*(km+1)/8)
    ELSE
      CALL POPREALARRAY(tv, 8*(ie-is+1)*km/8)
      CALL POPREALARRAY(phiz, 8*(ie-is+1)*(km+1)/8)
      DO j=je,js,-1
        CALL POPCONTROL1B_FV(branch)
        IF (branch .NE. 0) THEN
          DO k=km,1,-1
            DO i=ie,is,-1
              q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) + hlv*delp(i, &
&               j, k)*teq_ad(i, j)
              delp_ad(i, j, k) = delp_ad(i, j, k) + hlv*q(i, j, k, sphum&
&               )*teq_ad(i, j)
            END DO
          END DO
        END IF
        DO i=ie,is,-1
          te_2d_ad(i, j) = te_2d_ad(i, j) + teq_ad(i, j)
          teq_ad(i, j) = 0.0_FVPRC
        END DO
      END DO
    END IF
    phiz_ad = 0.0_FVPRC
    tv_ad = 0.0_FVPRC
    DO 100 j=je,js,-1
      CALL POPCONTROL2B_FV(branch)
      IF (branch .EQ. 0) THEN
        DO k=km,1,-1
          DO i=ie,is,-1
            temp7 = v(i, j, k) + v(i+1, j, k)
            temp6 = u(i, j, k) + u(i, j+1, k)
            temp5 = 0.5*rsin2_l(i, j)
            temp_ad7 = delp(i, j, k)*te_2d_ad(i, j)
            temp_ad8 = 0.5*temp_ad7
            temp_ad9 = temp5*temp_ad8
            temp_ad10 = -(cosa_s_l(i, j)*temp_ad9)
            delp_ad(i, j, k) = delp_ad(i, j, k) + (cv_air*pt(i, j, k)+&
&             0.5*(phiz(i, k)+phiz(i, k+1)+w(i, j, k)**2+temp5*(u(i, j, &
&             k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-&
&             cosa_s_l(i, j)*(temp6*temp7))))*te_2d_ad(i, j)
            pt_ad(i, j, k) = pt_ad(i, j, k) + cv_air*temp_ad7
            phiz_ad(i, k) = phiz_ad(i, k) + temp_ad8
            phiz_ad(i, k+1) = phiz_ad(i, k+1) + temp_ad8
            w_ad(i, j, k) = w_ad(i, j, k) + 2*w(i, j, k)*temp_ad8
            u_ad(i, j, k) = u_ad(i, j, k) + temp7*temp_ad10 + 2*u(i, j, &
&             k)*temp_ad9
            u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp7*temp_ad10 + 2*u(i&
&             , j+1, k)*temp_ad9
            v_ad(i, j, k) = v_ad(i, j, k) + temp6*temp_ad10 + 2*v(i, j, &
&             k)*temp_ad9
            v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp6*temp_ad10 + 2*v(i+&
&             1, j, k)*temp_ad9
          END DO
        END DO
      ELSE IF (branch .EQ. 1) THEN
        DO k=km,1,-1
          DO i=ie,is,-1
            temp4 = v(i, j, k) + v(i+1, j, k)
            temp3 = u(i, j, k) + u(i, j+1, k)
            temp2 = 0.5*rsin2_l(i, j)
            temp_ad3 = delp(i, j, k)*te_2d_ad(i, j)
            temp_ad4 = 0.5*temp_ad3
            temp_ad5 = temp2*temp_ad4
            temp_ad6 = -(cosa_s_l(i, j)*temp_ad5)
            delp_ad(i, j, k) = delp_ad(i, j, k) + (cv_air*pt(i, j, k)+&
&             0.5*(phiz(i, k)+phiz(i, k+1)+w(i, j, k)**2+temp2*(u(i, j, &
&             k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-&
&             cosa_s_l(i, j)*(temp3*temp4))))*te_2d_ad(i, j)
            pt_ad(i, j, k) = pt_ad(i, j, k) + cv_air*temp_ad3
            phiz_ad(i, k) = phiz_ad(i, k) + temp_ad4
            phiz_ad(i, k+1) = phiz_ad(i, k+1) + temp_ad4
            w_ad(i, j, k) = w_ad(i, j, k) + 2*w(i, j, k)*temp_ad4
            u_ad(i, j, k) = u_ad(i, j, k) + temp4*temp_ad6 + 2*u(i, j, k&
&             )*temp_ad5
            u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp4*temp_ad6 + 2*u(i, &
&             j+1, k)*temp_ad5
            v_ad(i, j, k) = v_ad(i, j, k) + temp3*temp_ad6 + 2*v(i, j, k&
&             )*temp_ad5
            v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp3*temp_ad6 + 2*v(i+1&
&             , j, k)*temp_ad5
          END DO
        END DO
      ELSE
        DO k=km,1,-1
          DO i=ie,is,-1
            temp1 = v(i, j, k) + v(i+1, j, k)
            temp0 = u(i, j, k) + u(i, j+1, k)
            temp = 0.25*rsin2_l(i, j)
            temp_ad0 = delp(i, j, k)*te_2d_ad(i, j)
            temp_ad1 = temp*temp_ad0
            temp_ad2 = -(cosa_s_l(i, j)*temp_ad1)
            delp_ad(i, j, k) = delp_ad(i, j, k) + (cp*tv(i, k)+temp*(u(i&
&             , j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-&
&             cosa_s_l(i, j)*(temp0*temp1)))*te_2d_ad(i, j)
            tv_ad(i, k) = tv_ad(i, k) + cp*temp_ad0
            u_ad(i, j, k) = u_ad(i, j, k) + temp1*temp_ad2 + 2*u(i, j, k&
&             )*temp_ad1
            u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp1*temp_ad2 + 2*u(i, &
&             j+1, k)*temp_ad1
            v_ad(i, j, k) = v_ad(i, j, k) + temp0*temp_ad2 + 2*v(i, j, k&
&             )*temp_ad1
            v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp0*temp_ad2 + 2*v(i+1&
&             , j, k)*temp_ad1
          END DO
        END DO
        DO i=ie,is,-1
          pe_ad(i, km+1, j) = pe_ad(i, km+1, j) + phiz(i, km+1)*te_2d_ad&
&           (i, j)
          phiz_ad(i, km+1) = phiz_ad(i, km+1) + pe(i, km+1, j)*te_2d_ad(&
&           i, j)
          pe_ad(i, 1, j) = pe_ad(i, 1, j) - phiz(i, 1)*te_2d_ad(i, j)
          phiz_ad(i, 1) = phiz_ad(i, 1) - pe(i, 1, j)*te_2d_ad(i, j)
          te_2d_ad(i, j) = 0.0_FVPRC
        END DO
        DO k=1,km,1
          DO i=ie,is,-1
            CALL POPREALARRAY(phiz(i, k))
            temp_ad = rg*tv(i, k)*phiz_ad(i, k)
            phiz_ad(i, k+1) = phiz_ad(i, k+1) + phiz_ad(i, k)
            tv_ad(i, k) = tv_ad(i, k) + rg*(peln(i, k+1, j)-peln(i, k, j&
&             ))*phiz_ad(i, k)
            peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + temp_ad
            peln_ad(i, k, j) = peln_ad(i, k, j) - temp_ad
            phiz_ad(i, k) = 0.0_FVPRC
            CALL POPREALARRAY(tv(i, k))
            pt_ad(i, j, k) = pt_ad(i, j, k) + (qc(i, j, k)+1.)*tv_ad(i, &
&             k)
            qc_ad(i, j, k) = qc_ad(i, j, k) + pt(i, j, k)*tv_ad(i, k)
            tv_ad(i, k) = 0.0_FVPRC
          END DO
        END DO
        DO i=ie,is,-1
          CALL POPREALARRAY(phiz(i, km+1))
          phiz_ad(i, km+1) = 0.0_FVPRC
        END DO
        GOTO 100
      END IF
      DO i=ie,is,-1
        te_2d_ad(i, j) = 0.0_FVPRC
      END DO
      DO i=ie,is,-1
        DO k=1,km,1
          CALL POPREALARRAY(phiz(i, k))
          phiz_ad(i, k+1) = phiz_ad(i, k+1) + phiz_ad(i, k)
          delz_ad(i, j, k) = delz_ad(i, j, k) - grav*phiz_ad(i, k)
          phiz_ad(i, k) = 0.0_FVPRC
        END DO
        CALL POPREALARRAY(phiz(i, km+1))
        phiz_ad(i, km+1) = 0.0_FVPRC
      END DO
 100 CONTINUE
  END SUBROUTINE COMPUTE_TOTAL_ENERGY_BWD
  SUBROUTINE COMPUTE_TOTAL_ENERGY(is, ie, js, je, isd, ied, jsd, jed, km&
&   , u, v, w, delz, pt, delp, q, qc, pe, peln, hs, rsin2_l, cosa_s_l, &
&   r_vir, cp, rg, hlv, te_2d, ua, va, teq, moist_phys, sphum, liq_wat, &
&   rainwat, ice_wat, snowwat, graupel, hydrostatic, id_te)
    IMPLICIT NONE
!------------------------------------------------------
! Compute vertically integrated total energy per column
!------------------------------------------------------
! !INPUT PARAMETERS:
    INTEGER, INTENT(IN) :: km, is, ie, js, je, isd, ied, jsd, jed, id_te
    INTEGER, INTENT(IN) :: sphum, liq_wat, ice_wat, rainwat, snowwat, &
&   graupel
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(INOUT) :: ua, &
&   va
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km), INTENT(IN) :: pt, delp
    REAL(fvprc), DIMENSION(isd:ied, jsd:jed, km, *), INTENT(IN) :: q
    REAL(fvprc), DIMENSION(is:ie, js:je, km), INTENT(IN) :: qc
    REAL(fvprc), INTENT(INOUT) :: u(isd:ied, jsd:jed+1, km)
    REAL(fvprc), INTENT(INOUT) :: v(isd:ied+1, jsd:jed, km)
! vertical velocity (m/s)
    REAL(fvprc), INTENT(IN) :: w(isd:ied, jsd:jed, km)
    REAL(fvprc), INTENT(IN) :: delz(isd:ied, jsd:jed, km)
! surface geopotential
    REAL(fvprc), INTENT(IN) :: hs(isd:ied, jsd:jed)
! pressure at layer edges
    REAL(fvprc), INTENT(IN) :: pe(is-1:ie+1, km+1, js-1:je+1)
! log(pe)
    REAL(fvprc), INTENT(IN) :: peln(is:ie, km+1, js:je)
    REAL(fvprc), INTENT(IN) :: cp, rg, r_vir, hlv
    REAL(fvprc), INTENT(IN) :: rsin2_l(isd:ied, jsd:jed)
    REAL(fvprc), INTENT(IN) :: cosa_s_l(isd:ied, jsd:jed)
    LOGICAL, INTENT(IN) :: moist_phys, hydrostatic
! Output:
! vertically integrated TE
    REAL(fvprc), INTENT(OUT) :: te_2d(is:ie, js:je)
! Moist TE
    REAL(fvprc), INTENT(OUT) :: teq(is:ie, js:je)
! Local
    REAL(fvprc), DIMENSION(is:ie, km) :: tv
    REAL(fvprc) :: phiz(is:ie, km+1)
    REAL(fvprc) :: cvm, q_liq, q_sol
    INTEGER :: i, j, k
!----------------------
! Output lat-lon winds:
!----------------------
!  call cubed_to_latlon(u, v, ua, va, dx, dy, rdxa, rdya, km, flagstruct%c2l_ord)
!$OMP parallel do default(none) shared(is,ie,js,je,km,hydrostatic,hs,pt,qc,rg,peln,te_2d, &
!$OMP                                  q,pe,delp,cp,rsin2_l,u,v,cosa_s_l,delz,moist_phys,w, &
!$OMP                                  liq_wat,rainwat,ice_wat,snowwat,graupel,sphum)   &
!$OMP                          private(phiz, tv, cvm, q_sol, q_liq)
    DO j=js,je
      IF (hydrostatic) THEN
        DO i=is,ie
          phiz(i, km+1) = hs(i, j)
        END DO
        DO k=km,1,-1
          DO i=is,ie
            tv(i, k) = pt(i, j, k)*(1.+qc(i, j, k))
            phiz(i, k) = phiz(i, k+1) + rg*tv(i, k)*(peln(i, k+1, j)-&
&             peln(i, k, j))
          END DO
        END DO
        DO i=is,ie
          te_2d(i, j) = pe(i, km+1, j)*phiz(i, km+1) - pe(i, 1, j)*phiz(&
&           i, 1)
        END DO
        DO k=1,km
          DO i=is,ie
            te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cp*tv(i, k)+0.25*&
&             rsin2_l(i, j)*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2&
&             +v(i+1, j, k)**2-(u(i, j, k)+u(i, j+1, k))*(v(i, j, k)+v(i&
&             +1, j, k))*cosa_s_l(i, j)))
          END DO
        END DO
      ELSE
!-----------------
! Non-hydrostatic:
!-----------------
        DO i=is,ie
          phiz(i, km+1) = hs(i, j)
          DO k=km,1,-1
            phiz(i, k) = phiz(i, k+1) - grav*delz(i, j, k)
          END DO
        END DO
        DO i=is,ie
          te_2d(i, j) = 0.
        END DO
        IF (moist_phys) THEN
          DO k=1,km
            DO i=is,ie
!#if defined(USE_COND) && defined(MOIST_CAPPA)
!#ifdef USE_NWAT3
!           q_liq = q(i,j,k,liq_wat)
!           q_sol = q(i,j,k,ice_wat)
!#else
!           q_liq = q(i,j,k,liq_wat) + q(i,j,k,rainwat)
!           q_sol = q(i,j,k,ice_wat) + q(i,j,k,snowwat) + q(i,j,k,graupel)
!#endif
!           cvm = (1.-(q(i,j,k,sphum)+q_liq+q_sol))*cv_air+q(i,j,k,sphum)*cv_vap+q_liq*c_liq+q_sol*c_ice
!           te_2d(i,j) = te_2d(i,j) + delp(i,j,k)*( cvm*pt(i,j,k) +  &
!#else
!#endif
              te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cv_air*pt(i, j&
&               , k)+0.5*(phiz(i, k)+phiz(i, k+1)+w(i, j, k)**2+0.5*&
&               rsin2_l(i, j)*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)&
&               **2+v(i+1, j, k)**2-(u(i, j, k)+u(i, j+1, k))*(v(i, j, k&
&               )+v(i+1, j, k))*cosa_s_l(i, j))))
            END DO
          END DO
        ELSE
          DO k=1,km
            DO i=is,ie
              te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cv_air*pt(i, j&
&               , k)+0.5*(phiz(i, k)+phiz(i, k+1)+w(i, j, k)**2+0.5*&
&               rsin2_l(i, j)*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)&
&               **2+v(i+1, j, k)**2-(u(i, j, k)+u(i, j+1, k))*(v(i, j, k&
&               )+v(i+1, j, k))*cosa_s_l(i, j))))
            END DO
          END DO
        END IF
      END IF
    END DO
!-------------------------------------
! Doganostics computation for moist TE
!-------------------------------------
    IF (id_te .GT. 0) THEN
!$OMP parallel do default(none) shared(is,ie,js,je,teq,te_2d,moist_phys,km,hlv,sphum,q,delp)
      DO j=js,je
        DO i=is,ie
          teq(i, j) = te_2d(i, j)
        END DO
        IF (moist_phys) THEN
          DO k=1,km
            DO i=is,ie
              teq(i, j) = teq(i, j) + hlv*q(i, j, k, sphum)*delp(i, j, k&
&               )
            END DO
          END DO
        END IF
      END DO
    END IF
  END SUBROUTINE COMPUTE_TOTAL_ENERGY
!  Differentiation of pkez in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b
!_ord4_fb a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo d
!yn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.g
!rad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleig
!h_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_lat
!lon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv
!_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.m
!ap1_q2_fb fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tra
!cer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c n
!h_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod
!.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corn
!er_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.co
!mpute_div_damping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.co
!py_corners_fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.f
!yppm_fb tp_core_mod.deln_flux)):
!   gradient     of useful results: peln pkz pk
!   with respect to varying inputs: peln pkz pk
  SUBROUTINE PKEZ_FWD(km, ifirst, ilast, jfirst, jlast, j, pe, pk, akap&
&   , peln, pkz, ptop)
    IMPLICIT NONE
! !INPUT PARAMETERS:
    INTEGER, INTENT(IN) :: km, j
! Latitude strip
    INTEGER, INTENT(IN) :: ifirst, ilast
! Latitude strip
    INTEGER, INTENT(IN) :: jfirst, jlast
    REAL(fvprc), INTENT(IN) :: akap
    REAL(fvprc), INTENT(IN) :: pe(ifirst-1:ilast+1, km+1, jfirst-1:jlast&
&   +1)
    REAL(fvprc), INTENT(IN) :: pk(ifirst:ilast, jfirst:jlast, km+1)
    REAL(fvprc), INTENT(IN) :: ptop
! !OUTPUT
    REAL(fvprc) :: pkz(ifirst:ilast, jfirst:jlast, km)
! log (pe)
    REAL(fvprc), INTENT(INOUT) :: peln(ifirst:ilast, km+1, jfirst:jlast)
! Local
    REAL(fvprc) :: pk2(ifirst:ilast, km+1)
    REAL(fvprc) :: pek
    REAL(fvprc) :: lnp
    REAL(fvprc) :: ak1
    INTEGER :: i, k
    INTRINSIC LOG
    ak1 = (akap+1.)/akap
    pek = pk(ifirst, j, 1)
    DO i=ifirst,ilast
      pk2(i, 1) = pek
    END DO
    DO k=2,km+1
      DO i=ifirst,ilast
!             peln(i,k,j) =  log(pe(i,k,j))
        pk2(i, k) = pk(i, j, k)
      END DO
    END DO
!---- GFDL modification
    IF (ptop .LT. ptop_min) THEN
      DO i=ifirst,ilast
        CALL PUSHREALARRAY(peln(i, 1, j))
        peln(i, 1, j) = peln(i, 2, j) - ak1
      END DO
      CALL PUSHCONTROL1B_FV(1)
    ELSE
      lnp = LOG(ptop)
      DO i=ifirst,ilast
        CALL PUSHREALARRAY(peln(i, 1, j))
        peln(i, 1, j) = lnp
      END DO
      CALL PUSHCONTROL1B_FV(0)
    END IF
!---- GFDL modification
    DO k=1,km
      DO i=ifirst,ilast
        CALL PUSHREALARRAY(pkz(i, j, k))
        pkz(i, j, k) = (pk2(i, k+1)-pk2(i, k))/(akap*(peln(i, k+1, j)-&
&         peln(i, k, j)))
      END DO
    END DO
    CALL PUSHREALARRAY(pk2, 8*(ilast-ifirst+1)*(km+1)/8)
  END SUBROUTINE PKEZ_FWD
!  Differentiation of pkez in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2
!b_ord4_fb a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo 
!dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.
!grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.raylei
!gh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_la
!tlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy f
!v_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.
!map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tr
!acer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c 
!nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mo
!d.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_cor
!ner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.c
!ompute_div_damping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.c
!opy_corners_fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.
!fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: peln pkz pk
!   with respect to varying inputs: peln pkz pk
  SUBROUTINE PKEZ_BWD(km, ifirst, ilast, jfirst, jlast, j, pe, pk, pk_ad&
&   , akap, peln, peln_ad, pkz, pkz_ad, ptop)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: km, j
    INTEGER, INTENT(IN) :: ifirst, ilast
    INTEGER, INTENT(IN) :: jfirst, jlast
    REAL(fvprc), INTENT(IN) :: akap
    REAL(fvprc), INTENT(IN) :: pe(ifirst-1:ilast+1, km+1, jfirst-1:jlast&
&   +1)
    REAL(fvprc), INTENT(IN) :: pk(ifirst:ilast, jfirst:jlast, km+1)
    REAL(fvprc) :: pk_ad(ifirst:ilast, jfirst:jlast, km+1)
    REAL(fvprc), INTENT(IN) :: ptop
    REAL(fvprc) :: pkz(ifirst:ilast, jfirst:jlast, km)
    REAL(fvprc) :: pkz_ad(ifirst:ilast, jfirst:jlast, km)
    REAL(fvprc), INTENT(INOUT) :: peln(ifirst:ilast, km+1, jfirst:jlast)
    REAL(fvprc), INTENT(INOUT) :: peln_ad(ifirst:ilast, km+1, jfirst:&
&   jlast)
    REAL(fvprc) :: pk2(ifirst:ilast, km+1)
    REAL(fvprc) :: pk2_ad(ifirst:ilast, km+1)
    REAL(fvprc) :: pek
    REAL(fvprc) :: pek_ad
    REAL(fvprc) :: lnp
    REAL(fvprc) :: ak1
    INTEGER :: i, k
    INTRINSIC LOG
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    INTEGER :: branch
    CALL POPREALARRAY(pk2, 8*(ilast-ifirst+1)*(km+1)/8)
    pk2_ad = 0.0_FVPRC
    DO k=km,1,-1
      DO i=ilast,ifirst,-1
        CALL POPREALARRAY(pkz(i, j, k))
        temp = akap*(peln(i, k+1, j)-peln(i, k, j))
        temp_ad = pkz_ad(i, j, k)/temp
        temp_ad0 = -((pk2(i, k+1)-pk2(i, k))*akap*temp_ad/temp)
        pk2_ad(i, k+1) = pk2_ad(i, k+1) + temp_ad
        pk2_ad(i, k) = pk2_ad(i, k) - temp_ad
        peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + temp_ad0
        peln_ad(i, k, j) = peln_ad(i, k, j) - temp_ad0
        pkz_ad(i, j, k) = 0.0_FVPRC
      END DO
    END DO
    CALL POPCONTROL1B_FV(branch)
    IF (branch .EQ. 0) THEN
      DO i=ilast,ifirst,-1
        CALL POPREALARRAY(peln(i, 1, j))
        peln_ad(i, 1, j) = 0.0_FVPRC
      END DO
    ELSE
      DO i=ilast,ifirst,-1
        CALL POPREALARRAY(peln(i, 1, j))
        peln_ad(i, 2, j) = peln_ad(i, 2, j) + peln_ad(i, 1, j)
        peln_ad(i, 1, j) = 0.0_FVPRC
      END DO
    END IF
    DO k=km+1,2,-1
      DO i=ilast,ifirst,-1
        pk_ad(i, j, k) = pk_ad(i, j, k) + pk2_ad(i, k)
        pk2_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
    pek_ad = 0.0_FVPRC
    DO i=ilast,ifirst,-1
      pek_ad = pek_ad + pk2_ad(i, 1)
      pk2_ad(i, 1) = 0.0_FVPRC
    END DO
    pk_ad(ifirst, j, 1) = pk_ad(ifirst, j, 1) + pek_ad
  END SUBROUTINE PKEZ_BWD
  SUBROUTINE PKEZ(km, ifirst, ilast, jfirst, jlast, j, pe, pk, akap, &
&   peln, pkz, ptop)
    IMPLICIT NONE
! !INPUT PARAMETERS:
    INTEGER, INTENT(IN) :: km, j
! Latitude strip
    INTEGER, INTENT(IN) :: ifirst, ilast
! Latitude strip
    INTEGER, INTENT(IN) :: jfirst, jlast
    REAL(fvprc), INTENT(IN) :: akap
    REAL(fvprc), INTENT(IN) :: pe(ifirst-1:ilast+1, km+1, jfirst-1:jlast&
&   +1)
    REAL(fvprc), INTENT(IN) :: pk(ifirst:ilast, jfirst:jlast, km+1)
    REAL(fvprc), INTENT(IN) :: ptop
! !OUTPUT
    REAL(fvprc), INTENT(OUT) :: pkz(ifirst:ilast, jfirst:jlast, km)
! log (pe)
    REAL(fvprc), INTENT(INOUT) :: peln(ifirst:ilast, km+1, jfirst:jlast)
! Local
    REAL(fvprc) :: pk2(ifirst:ilast, km+1)
    REAL(fvprc) :: pek
    REAL(fvprc) :: lnp
    REAL(fvprc) :: ak1
    INTEGER :: i, k
    INTRINSIC LOG
    ak1 = (akap+1.)/akap
    pek = pk(ifirst, j, 1)
    DO i=ifirst,ilast
      pk2(i, 1) = pek
    END DO
    DO k=2,km+1
      DO i=ifirst,ilast
!             peln(i,k,j) =  log(pe(i,k,j))
        pk2(i, k) = pk(i, j, k)
      END DO
    END DO
!---- GFDL modification
    IF (ptop .LT. ptop_min) THEN
      DO i=ifirst,ilast
        peln(i, 1, j) = peln(i, 2, j) - ak1
      END DO
    ELSE
      lnp = LOG(ptop)
      DO i=ifirst,ilast
        peln(i, 1, j) = lnp
      END DO
    END IF
!---- GFDL modification
    DO k=1,km
      DO i=ifirst,ilast
        pkz(i, j, k) = (pk2(i, k+1)-pk2(i, k))/(akap*(peln(i, k+1, j)-&
&         peln(i, k, j)))
      END DO
    END DO
  END SUBROUTINE PKEZ
!  Differentiation of remap_z in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_ord4 a2b
!_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.
!adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_updat
!e dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_super fv_d
!ynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlon fv_grid_
!utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_mapz_mod.pk
!ez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2 fv
!_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tracer2d_mod.tr
!acer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_core_mod.r
!iem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.sim_solver 
!nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest sw_c
!ore_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.compute_div_da
!mping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy_corners_f
!b tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyppm tp_c
!ore_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 q2
!   with respect to varying inputs: pe1 pe2 q2
  SUBROUTINE REMAP_Z_ADM(km, pe1, pe1_ad, kn, pe2, pe2_ad, q2, q2_ad, i1&
&   , i2, iv, kord)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! Method order
    INTEGER, INTENT(IN) :: kord
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
    INTEGER, INTENT(IN) :: iv
! height at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
! (from model top to bottom surface)
! hieght at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
! (from model top to bottom surface)
!      real(FVPRC), intent(in) ::  q1(i1:i2,km)        ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(i1:i2, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_ad(i1:i2, kn)
! !LOCAL VARIABLES:
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: qs_ad(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_ad(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: q4_ad(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, delp, esl
    REAL(fvprc) :: pl_ad, pr_ad, qsum_ad, delp_ad, esl_ad
    INTEGER :: i, k, l, m, k0
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp_ad11
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: branch
    INTEGER :: ad_count0
    INTEGER :: i3
    DO k=1,km
      DO i=i1,i2
! negative
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q2(i, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
        CALL CS_PROFILE_LINEAR(qs, q4, dp1, km, i1, i2, iv)
        CALL PUSHCONTROL2B(2)
      ELSE
        CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
        CALL CS_PROFILE(qs, q4, dp1, km, i1, i2, iv, kord)
        CALL PUSHCONTROL2B(1)
      END IF
    ELSE
      CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL PPM_PROFILE(q4, dp1, km, i1, i2, iv, kord)
      CALL PUSHCONTROL2B(0)
    END IF
! Mapping
    DO i=i1,i2
      k0 = 1
      DO 120 k=1,kn
        CALL PUSHINTEGER4(l)
        ad_count = 1
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .LE. pe1(i, l) .AND. pe2(i, k) .GE. pe1(i, l+1)&
&         ) THEN
            GOTO 100
          ELSE
            CALL PUSHINTEGER4(l)
            ad_count = ad_count + 1
          END IF
        END DO
        CALL PUSHCONTROL1B(0)
        CALL PUSHINTEGER4(ad_count)
        CALL PUSHCONTROL2B(2)
        GOTO 123
 100    CALL PUSHCONTROL1B(1)
        CALL PUSHINTEGER4(ad_count)
        pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
        IF (pe2(i, k+1) .GE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
          k0 = l
          CALL PUSHCONTROL1B(0)
          GOTO 120
        ELSE
! Fractional area...
          CALL PUSHREALARRAY_ADM(qsum)
          qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, l)+&
&           q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+pl*(1.+&
&           pl))))
          CALL PUSHINTEGER4(m)
          ad_count0 = 1
          DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
            IF (pe2(i, k+1) .LT. pe1(i, m+1)) THEN
! Whole layer..
              qsum = qsum + dp1(i, m)*q4(1, i, m)
              CALL PUSHINTEGER4(m)
              ad_count0 = ad_count0 + 1
            ELSE
              GOTO 110
            END IF
          END DO
          CALL PUSHCONTROL1B(0)
          CALL PUSHINTEGER4(ad_count0)
          CALL PUSHCONTROL2B(1)
          GOTO 123
 110      CALL PUSHCONTROL1B(1)
          CALL PUSHINTEGER4(ad_count0)
          delp = pe2(i, k+1) - pe1(i, m)
          esl = delp/dp1(i, m)
          qsum = qsum + delp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(2, i, &
&           m)+q4(4, i, m)*(1.-r23*esl)))
          k0 = m
          CALL PUSHCONTROL2B(0)
        END IF
 123    CALL PUSHCONTROL1B(1)
 120  CONTINUE
    END DO
    dp1_ad = 0.0_FVPRC
    qsum_ad = 0.0_FVPRC
    q4_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          temp_ad0 = 0.5*(pr+pl)*q2_ad(i, k)
          temp_ad1 = 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2, i, l))*q2_ad(i, &
&           k)
          temp_ad2 = -(r3*q4(4, i, l)*q2_ad(i, k))
          q4_ad(2, i, l) = q4_ad(2, i, l) + q2_ad(i, k) - temp_ad0
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad0 - r3*(pr*(pr+pl)+pl&
&           **2)*q2_ad(i, k)
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad0
          pr_ad = (2*pr+pl)*temp_ad2 + temp_ad1
          pl_ad = (2*pl+pr)*temp_ad2 + temp_ad1
          q2_ad(i, k) = 0.0_FVPRC
          temp_ad3 = pr_ad/dp1(i, l)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad3
          pe1_ad(i, l) = pe1_ad(i, l) - temp_ad3
          dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k+1)-pe1(i, l))*temp_ad3&
&           /dp1(i, l)
        ELSE
          temp1 = pe2(i, k+1) - pe2(i, k)
          temp_ad11 = -(qsum*q2_ad(i, k)/temp1**2)
          qsum_ad = qsum_ad + q2_ad(i, k)/temp1
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad11
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad11
          q2_ad(i, k) = 0.0_FVPRC
          CALL POPCONTROL2B(branch)
          IF (branch .EQ. 0) THEN
            delp = pe2(i, k+1) - pe1(i, m)
            esl = delp/dp1(i, m)
            temp0 = q4(3, i, m) - q4(2, i, m) + q4(4, i, m)*(-(r23*esl)+&
&             1.)
            temp_ad8 = delp*qsum_ad
            temp_ad9 = 0.5*esl*temp_ad8
            q4_ad(2, i, m) = q4_ad(2, i, m) + temp_ad8 - temp_ad9
            esl_ad = 0.5*temp0*temp_ad8 - q4(4, i, m)*r23*temp_ad9
            q4_ad(3, i, m) = q4_ad(3, i, m) + temp_ad9
            q4_ad(4, i, m) = q4_ad(4, i, m) + (1.-r23*esl)*temp_ad9
            temp_ad10 = esl_ad/dp1(i, m)
            delp_ad = temp_ad10 + (q4(2, i, m)+0.5*(esl*temp0))*qsum_ad
            dp1_ad(i, m) = dp1_ad(i, m) - delp*temp_ad10/dp1(i, m)
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + delp_ad
            pe1_ad(i, m) = pe1_ad(i, m) - delp_ad
          ELSE IF (branch .NE. 1) THEN
            GOTO 130
          END IF
          CALL POPINTEGER4(ad_count0)
          DO i3=1,ad_count0
            IF (i3 .EQ. 1) THEN
              CALL POPCONTROL1B(branch)
            ELSE
              dp1_ad(i, m) = dp1_ad(i, m) + q4(1, i, m)*qsum_ad
              q4_ad(1, i, m) = q4_ad(1, i, m) + dp1(i, m)*qsum_ad
            END IF
            CALL POPINTEGER4(m)
          END DO
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY_ADM(qsum)
          temp = q4(4, i, l) + q4(3, i, l) - q4(2, i, l)
          temp_ad4 = (q4(2, i, l)+0.5*(temp*(pl+1.))-r3*(q4(4, i, l)*(pl&
&           *(pl+1.)+1.)))*qsum_ad
          temp_ad5 = (pe1(i, l+1)-pe2(i, k))*qsum_ad
          temp_ad6 = 0.5*(pl+1.)*temp_ad5
          temp_ad7 = -(r3*q4(4, i, l)*temp_ad5)
          pe1_ad(i, l+1) = pe1_ad(i, l+1) + temp_ad4
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad4
          q4_ad(2, i, l) = q4_ad(2, i, l) + temp_ad5 - temp_ad6
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad6 - r3*(pl*(pl+1.)+1.&
&           )*temp_ad5
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad6
          pl_ad = (2*pl+1.)*temp_ad7 + 0.5*temp*temp_ad5
          qsum_ad = 0.0_FVPRC
        END IF
        temp_ad = pl_ad/dp1(i, l)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad
        pe1_ad(i, l) = pe1_ad(i, l) - temp_ad
        dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k)-pe1(i, l))*temp_ad/dp1(&
&         i, l)
 130    CALL POPINTEGER4(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B(branch)
          CALL POPINTEGER4(l)
        END DO
      END DO
    END DO
    CALL POPCONTROL2B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL PPM_PROFILE_ADM(q4, q4_ad, dp1, dp1_ad, km, i1, i2, iv, kord)
    ELSE IF (branch .EQ. 1) THEN
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      qs_ad = 0.0_FVPRC
      CALL CS_PROFILE_ADM(qs, qs_ad, q4, q4_ad, dp1, dp1_ad, km, i1, i2&
&                   , iv, kord)
    ELSE
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      qs_ad = 0.0_FVPRC
      CALL CS_PROFILE_LINEAR_ADM(qs, qs_ad, q4, q4_ad, dp1, dp1_ad, km, &
&                          i1, i2, iv)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
        q2_ad(i, k) = q2_ad(i, k) + q4_ad(1, i, k)
        q4_ad(1, i, k) = 0.0_FVPRC
        pe1_ad(i, k+1) = pe1_ad(i, k+1) + dp1_ad(i, k)
        pe1_ad(i, k) = pe1_ad(i, k) - dp1_ad(i, k)
        dp1_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE REMAP_Z_ADM
  SUBROUTINE REMAP_Z(km, pe1, kn, pe2, q2, i1, i2, iv, kord)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! Method order
    INTEGER, INTENT(IN) :: kord
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
    INTEGER, INTENT(IN) :: iv
! height at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! hieght at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
!      real(FVPRC), intent(in) ::  q1(i1:i2,km)        ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(i1:i2, kn)
! !LOCAL VARIABLES:
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, delp, esl
    INTEGER :: i, k, l, m, k0
    DO k=1,km
      DO i=i1,i2
! negative
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q2(i, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL CS_PROFILE_LINEAR(qs, q4, dp1, km, i1, i2, iv)
      ELSE
        CALL CS_PROFILE(qs, q4, dp1, km, i1, i2, iv, kord)
      END IF
    ELSE
      CALL PPM_PROFILE(q4, dp1, km, i1, i2, iv, kord)
    END IF
! Mapping
    DO i=i1,i2
      k0 = 1
      DO k=1,kn
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .LE. pe1(i, l) .AND. pe2(i, k) .GE. pe1(i, l+1)&
&         ) THEN
            pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
            IF (pe2(i, k+1) .GE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
              pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
              q2(i, k) = q4(2, i, l) + 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2&
&               , i, l))*(pr+pl) - q4(4, i, l)*r3*(pr*(pr+pl)+pl**2)
              k0 = l
              GOTO 555
            ELSE
! Fractional area...
              qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, &
&               l)+q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+&
&               pl*(1.+pl))))
              DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
                IF (pe2(i, k+1) .LT. pe1(i, m+1)) THEN
! Whole layer..
                  qsum = qsum + dp1(i, m)*q4(1, i, m)
                ELSE
                  delp = pe2(i, k+1) - pe1(i, m)
                  esl = delp/dp1(i, m)
                  qsum = qsum + delp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-&
&                   q4(2, i, m)+q4(4, i, m)*(1.-r23*esl)))
                  k0 = m
                  GOTO 123
                END IF
              END DO
              GOTO 123
            END IF
          END IF
        END DO
 123    q2(i, k) = qsum/(pe2(i, k+1)-pe2(i, k))
 555    CONTINUE
      END DO
    END DO
  END SUBROUTINE REMAP_Z
!  Differentiation of map_scalar in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_ord4 
!a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_m
!od.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_up
!date dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_super f
!v_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlon fv_gr
!id_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_mapz_mod
!.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2
! fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tracer2d_mod
!.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_core_mo
!d.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.sim_solv
!er nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest s
!w_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.compute_div
!_damping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy_corner
!s_fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyppm t
!p_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 q2
!   with respect to varying inputs: pe1 pe2 q2
  SUBROUTINE MAP_SCALAR_ADM(km, pe1, pe1_ad, qs, kn, pe2, pe2_ad, q2, &
&   q2_ad, i1, i2, j, ibeg, iend, jbeg, jend, iv, kord, q_min)
    IMPLICIT NONE
! iv=1
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! Mode: 0 == constituents  1 == temp
    INTEGER, INTENT(IN) :: iv
!       2 == remap temp with cs scheme
! Method order
    INTEGER, INTENT(IN) :: kord
! Current latitude
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! bottom BC
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
! real(FVPRC), intent(in) ::    q1(ibeg:iend,jbeg:jend,km) ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_ad(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc), INTENT(IN) :: q_min
! !DESCRIPTION:
! IV = 0: constituents
! pe1: pressure at layer edges (from model top to bottom surface)
!      in the original vertical coordinate
! pe2: pressure at layer edges (from model top to bottom surface)
!      in the new vertical coordinate
! !LOCAL VARIABLES:
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_ad(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: q4_ad(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    REAL(fvprc) :: pl_ad, pr_ad, qsum_ad, dp_ad, esl_ad
    INTEGER :: i, k, l, m, k0
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp_ad11
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: branch
    INTEGER :: ad_count0
    INTEGER :: i3
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q2(i, j, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
        CALL SCALAR_PROFILE_LINEAR(qs, q4, dp1, km, i1, i2, iv, q_min)
        CALL PUSHCONTROL2B(2)
      ELSE
        CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
        CALL SCALAR_PROFILE(qs, q4, dp1, km, i1, i2, iv, kord, q_min)
        CALL PUSHCONTROL2B(1)
      END IF
    ELSE
      CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL PPM_PROFILE(q4, dp1, km, i1, i2, iv, kord)
      CALL PUSHCONTROL2B(0)
    END IF
    DO i=i1,i2
      k0 = 1
      DO 120 k=1,kn
        CALL PUSHINTEGER4(l)
        ad_count = 1
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            GOTO 100
          ELSE
            CALL PUSHINTEGER4(l)
            ad_count = ad_count + 1
          END IF
        END DO
        CALL PUSHCONTROL1B(0)
        CALL PUSHINTEGER4(ad_count)
        CALL PUSHCONTROL2B(2)
        GOTO 123
 100    CALL PUSHCONTROL1B(1)
        CALL PUSHINTEGER4(ad_count)
        pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
        IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
          k0 = l
          CALL PUSHCONTROL1B(0)
          GOTO 120
        ELSE
! Fractional area...
          CALL PUSHREALARRAY_ADM(qsum)
          qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, l)+&
&           q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+pl*(1.+&
&           pl))))
          CALL PUSHINTEGER4(m)
          ad_count0 = 1
          DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
            IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer
              qsum = qsum + dp1(i, m)*q4(1, i, m)
              CALL PUSHINTEGER4(m)
              ad_count0 = ad_count0 + 1
            ELSE
              GOTO 110
            END IF
          END DO
          CALL PUSHCONTROL1B(0)
          CALL PUSHINTEGER4(ad_count0)
          CALL PUSHCONTROL2B(1)
          GOTO 123
 110      CALL PUSHCONTROL1B(1)
          CALL PUSHINTEGER4(ad_count0)
          dp = pe2(i, k+1) - pe1(i, m)
          esl = dp/dp1(i, m)
          qsum = qsum + dp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(2, i, m)&
&           +q4(4, i, m)*(1.-r23*esl)))
          k0 = m
          CALL PUSHCONTROL2B(0)
        END IF
 123    CALL PUSHCONTROL1B(1)
 120  CONTINUE
    END DO
    dp1_ad = 0.0_FVPRC
    qsum_ad = 0.0_FVPRC
    q4_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          temp_ad0 = 0.5*(pr+pl)*q2_ad(i, j, k)
          temp_ad1 = 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2, i, l))*q2_ad(i, &
&           j, k)
          temp_ad2 = -(r3*q4(4, i, l)*q2_ad(i, j, k))
          q4_ad(2, i, l) = q4_ad(2, i, l) + q2_ad(i, j, k) - temp_ad0
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad0 - r3*(pr*(pr+pl)+pl&
&           **2)*q2_ad(i, j, k)
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad0
          pr_ad = (2*pr+pl)*temp_ad2 + temp_ad1
          pl_ad = (2*pl+pr)*temp_ad2 + temp_ad1
          q2_ad(i, j, k) = 0.0_FVPRC
          temp_ad3 = pr_ad/dp1(i, l)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad3
          pe1_ad(i, l) = pe1_ad(i, l) - temp_ad3
          dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k+1)-pe1(i, l))*temp_ad3&
&           /dp1(i, l)
        ELSE
          temp1 = pe2(i, k+1) - pe2(i, k)
          temp_ad11 = -(qsum*q2_ad(i, j, k)/temp1**2)
          qsum_ad = qsum_ad + q2_ad(i, j, k)/temp1
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad11
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad11
          q2_ad(i, j, k) = 0.0_FVPRC
          CALL POPCONTROL2B(branch)
          IF (branch .EQ. 0) THEN
            dp = pe2(i, k+1) - pe1(i, m)
            esl = dp/dp1(i, m)
            temp0 = q4(3, i, m) - q4(2, i, m) + q4(4, i, m)*(-(r23*esl)+&
&             1.)
            temp_ad8 = dp*qsum_ad
            temp_ad9 = 0.5*esl*temp_ad8
            q4_ad(2, i, m) = q4_ad(2, i, m) + temp_ad8 - temp_ad9
            esl_ad = 0.5*temp0*temp_ad8 - q4(4, i, m)*r23*temp_ad9
            q4_ad(3, i, m) = q4_ad(3, i, m) + temp_ad9
            q4_ad(4, i, m) = q4_ad(4, i, m) + (1.-r23*esl)*temp_ad9
            temp_ad10 = esl_ad/dp1(i, m)
            dp_ad = temp_ad10 + (q4(2, i, m)+0.5*(esl*temp0))*qsum_ad
            dp1_ad(i, m) = dp1_ad(i, m) - dp*temp_ad10/dp1(i, m)
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + dp_ad
            pe1_ad(i, m) = pe1_ad(i, m) - dp_ad
          ELSE IF (branch .NE. 1) THEN
            GOTO 130
          END IF
          CALL POPINTEGER4(ad_count0)
          DO i3=1,ad_count0
            IF (i3 .EQ. 1) THEN
              CALL POPCONTROL1B(branch)
            ELSE
              dp1_ad(i, m) = dp1_ad(i, m) + q4(1, i, m)*qsum_ad
              q4_ad(1, i, m) = q4_ad(1, i, m) + dp1(i, m)*qsum_ad
            END IF
            CALL POPINTEGER4(m)
          END DO
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY_ADM(qsum)
          temp = q4(4, i, l) + q4(3, i, l) - q4(2, i, l)
          temp_ad4 = (q4(2, i, l)+0.5*(temp*(pl+1.))-r3*(q4(4, i, l)*(pl&
&           *(pl+1.)+1.)))*qsum_ad
          temp_ad5 = (pe1(i, l+1)-pe2(i, k))*qsum_ad
          temp_ad6 = 0.5*(pl+1.)*temp_ad5
          temp_ad7 = -(r3*q4(4, i, l)*temp_ad5)
          pe1_ad(i, l+1) = pe1_ad(i, l+1) + temp_ad4
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad4
          q4_ad(2, i, l) = q4_ad(2, i, l) + temp_ad5 - temp_ad6
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad6 - r3*(pl*(pl+1.)+1.&
&           )*temp_ad5
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad6
          pl_ad = (2*pl+1.)*temp_ad7 + 0.5*temp*temp_ad5
          qsum_ad = 0.0_FVPRC
        END IF
        temp_ad = pl_ad/dp1(i, l)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad
        pe1_ad(i, l) = pe1_ad(i, l) - temp_ad
        dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k)-pe1(i, l))*temp_ad/dp1(&
&         i, l)
 130    CALL POPINTEGER4(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B(branch)
          CALL POPINTEGER4(l)
        END DO
      END DO
    END DO
    CALL POPCONTROL2B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL PPM_PROFILE_ADM(q4, q4_ad, dp1, dp1_ad, km, i1, i2, iv, kord)
    ELSE IF (branch .EQ. 1) THEN
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL SCALAR_PROFILE_ADM(qs, q4, q4_ad, dp1, dp1_ad, km, i1, i2, iv&
&                       , kord, q_min)
    ELSE
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL SCALAR_PROFILE_LINEAR_ADM(qs, q4, q4_ad, dp1, dp1_ad, km, i1&
&                              , i2, iv, q_min)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
        q2_ad(i, j, k) = q2_ad(i, j, k) + q4_ad(1, i, k)
        q4_ad(1, i, k) = 0.0_FVPRC
        pe1_ad(i, k+1) = pe1_ad(i, k+1) + dp1_ad(i, k)
        pe1_ad(i, k) = pe1_ad(i, k) - dp1_ad(i, k)
        dp1_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE MAP_SCALAR_ADM
  SUBROUTINE MAP_SCALAR(km, pe1, qs, kn, pe2, q2, i1, i2, j, ibeg, iend&
&   , jbeg, jend, iv, kord, q_min)
    IMPLICIT NONE
! iv=1
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! Mode: 0 == constituents  1 == temp
    INTEGER, INTENT(IN) :: iv
!       2 == remap temp with cs scheme
! Method order
    INTEGER, INTENT(IN) :: kord
! Current latitude
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! bottom BC
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
! real(FVPRC), intent(in) ::    q1(ibeg:iend,jbeg:jend,km) ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc), INTENT(IN) :: q_min
! !DESCRIPTION:
! IV = 0: constituents
! pe1: pressure at layer edges (from model top to bottom surface)
!      in the original vertical coordinate
! pe2: pressure at layer edges (from model top to bottom surface)
!      in the new vertical coordinate
! !LOCAL VARIABLES:
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    INTEGER :: i, k, l, m, k0
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q2(i, j, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL SCALAR_PROFILE_LINEAR(qs, q4, dp1, km, i1, i2, iv, q_min)
      ELSE
        CALL SCALAR_PROFILE(qs, q4, dp1, km, i1, i2, iv, kord, q_min)
      END IF
    ELSE
      CALL PPM_PROFILE(q4, dp1, km, i1, i2, iv, kord)
    END IF
    DO i=i1,i2
      k0 = 1
      DO k=1,kn
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
            IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
              pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
              q2(i, j, k) = q4(2, i, l) + 0.5*(q4(4, i, l)+q4(3, i, l)-&
&               q4(2, i, l))*(pr+pl) - q4(4, i, l)*r3*(pr*(pr+pl)+pl**2)
              k0 = l
              GOTO 555
            ELSE
! Fractional area...
              qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, &
&               l)+q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+&
&               pl*(1.+pl))))
              DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
                IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer
                  qsum = qsum + dp1(i, m)*q4(1, i, m)
                ELSE
                  dp = pe2(i, k+1) - pe1(i, m)
                  esl = dp/dp1(i, m)
                  qsum = qsum + dp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(&
&                   2, i, m)+q4(4, i, m)*(1.-r23*esl)))
                  k0 = m
                  GOTO 123
                END IF
              END DO
              GOTO 123
            END IF
          END IF
        END DO
 123    q2(i, j, k) = qsum/(pe2(i, k+1)-pe2(i, k))
 555    CONTINUE
      END DO
    END DO
  END SUBROUTINE MAP_SCALAR
!  Differentiation of map1_ppm in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_ord4 a2
!b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod
!.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_upda
!te dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_super fv_
!dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlon fv_grid
!_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_mapz_mod.p
!kez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2 f
!v_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tracer2d_mod.t
!racer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_core_mod.
!riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.sim_solver
! nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest sw_
!core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.compute_div_d
!amping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy_corners_
!fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyppm tp_
!core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 qs q2
!   with respect to varying inputs: pe1 pe2 qs q2
  SUBROUTINE MAP1_PPM_ADM(km, pe1, pe1_ad, qs, qs_ad, kn, pe2, pe2_ad, &
&   q2, q2_ad, i1, i2, j, ibeg, iend, jbeg, jend, iv, kord)
    IMPLICIT NONE
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! Mode: 0 == constituents  1 == ???
    INTEGER, INTENT(IN) :: iv
!       2 == remap temp with cs scheme
! Method order
    INTEGER, INTENT(IN) :: kord
! Current latitude
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! bottom BC
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
    REAL(fvprc) :: qs_ad(i1:i2)
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
! real(FVPRC), intent(in) ::    q1(ibeg:iend,jbeg:jend,km) ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_ad(ibeg:iend, jbeg:jend, kn)
! !DESCRIPTION:
! IV = 0: constituents
! pe1: pressure at layer edges (from model top to bottom surface)
!      in the original vertical coordinate
! pe2: pressure at layer edges (from model top to bottom surface)
!      in the new vertical coordinate
! !LOCAL VARIABLES:
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_ad(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: q4_ad(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    REAL(fvprc) :: pl_ad, pr_ad, qsum_ad, dp_ad, esl_ad
    INTEGER :: i, k, l, m, k0
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp_ad11
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: branch
    INTEGER :: ad_count0
    INTEGER :: i3
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q2(i, j, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
        CALL CS_PROFILE_LINEAR(qs, q4, dp1, km, i1, i2, iv)
        CALL PUSHCONTROL2B(2)
      ELSE
        CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
        CALL CS_PROFILE(qs, q4, dp1, km, i1, i2, iv, kord)
        CALL PUSHCONTROL2B(1)
      END IF
    ELSE
      CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL PPM_PROFILE(q4, dp1, km, i1, i2, iv, kord)
      CALL PUSHCONTROL2B(0)
    END IF
    DO i=i1,i2
      k0 = 1
      DO 120 k=1,kn
        CALL PUSHINTEGER4(l)
        ad_count = 1
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            GOTO 100
          ELSE
            CALL PUSHINTEGER4(l)
            ad_count = ad_count + 1
          END IF
        END DO
        CALL PUSHCONTROL1B(0)
        CALL PUSHINTEGER4(ad_count)
        CALL PUSHCONTROL2B(2)
        GOTO 123
 100    CALL PUSHCONTROL1B(1)
        CALL PUSHINTEGER4(ad_count)
        pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
        IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
          k0 = l
          CALL PUSHCONTROL1B(0)
          GOTO 120
        ELSE
! Fractional area...
          CALL PUSHREALARRAY_ADM(qsum)
          qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, l)+&
&           q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+pl*(1.+&
&           pl))))
          CALL PUSHINTEGER4(m)
          ad_count0 = 1
          DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
            IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer
              qsum = qsum + dp1(i, m)*q4(1, i, m)
              CALL PUSHINTEGER4(m)
              ad_count0 = ad_count0 + 1
            ELSE
              GOTO 110
            END IF
          END DO
          CALL PUSHCONTROL1B(0)
          CALL PUSHINTEGER4(ad_count0)
          CALL PUSHCONTROL2B(1)
          GOTO 123
 110      CALL PUSHCONTROL1B(1)
          CALL PUSHINTEGER4(ad_count0)
          dp = pe2(i, k+1) - pe1(i, m)
          esl = dp/dp1(i, m)
          qsum = qsum + dp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(2, i, m)&
&           +q4(4, i, m)*(1.-r23*esl)))
          k0 = m
          CALL PUSHCONTROL2B(0)
        END IF
 123    CALL PUSHCONTROL1B(1)
 120  CONTINUE
    END DO
    dp1_ad = 0.0_FVPRC
    qsum_ad = 0.0_FVPRC
    q4_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          temp_ad0 = 0.5*(pr+pl)*q2_ad(i, j, k)
          temp_ad1 = 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2, i, l))*q2_ad(i, &
&           j, k)
          temp_ad2 = -(r3*q4(4, i, l)*q2_ad(i, j, k))
          q4_ad(2, i, l) = q4_ad(2, i, l) + q2_ad(i, j, k) - temp_ad0
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad0 - r3*(pr*(pr+pl)+pl&
&           **2)*q2_ad(i, j, k)
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad0
          pr_ad = (2*pr+pl)*temp_ad2 + temp_ad1
          pl_ad = (2*pl+pr)*temp_ad2 + temp_ad1
          q2_ad(i, j, k) = 0.0_FVPRC
          temp_ad3 = pr_ad/dp1(i, l)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad3
          pe1_ad(i, l) = pe1_ad(i, l) - temp_ad3
          dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k+1)-pe1(i, l))*temp_ad3&
&           /dp1(i, l)
        ELSE
          temp1 = pe2(i, k+1) - pe2(i, k)
          temp_ad11 = -(qsum*q2_ad(i, j, k)/temp1**2)
          qsum_ad = qsum_ad + q2_ad(i, j, k)/temp1
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad11
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad11
          q2_ad(i, j, k) = 0.0_FVPRC
          CALL POPCONTROL2B(branch)
          IF (branch .EQ. 0) THEN
            dp = pe2(i, k+1) - pe1(i, m)
            esl = dp/dp1(i, m)
            temp0 = q4(3, i, m) - q4(2, i, m) + q4(4, i, m)*(-(r23*esl)+&
&             1.)
            temp_ad8 = dp*qsum_ad
            temp_ad9 = 0.5*esl*temp_ad8
            q4_ad(2, i, m) = q4_ad(2, i, m) + temp_ad8 - temp_ad9
            esl_ad = 0.5*temp0*temp_ad8 - q4(4, i, m)*r23*temp_ad9
            q4_ad(3, i, m) = q4_ad(3, i, m) + temp_ad9
            q4_ad(4, i, m) = q4_ad(4, i, m) + (1.-r23*esl)*temp_ad9
            temp_ad10 = esl_ad/dp1(i, m)
            dp_ad = temp_ad10 + (q4(2, i, m)+0.5*(esl*temp0))*qsum_ad
            dp1_ad(i, m) = dp1_ad(i, m) - dp*temp_ad10/dp1(i, m)
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + dp_ad
            pe1_ad(i, m) = pe1_ad(i, m) - dp_ad
          ELSE IF (branch .NE. 1) THEN
            GOTO 130
          END IF
          CALL POPINTEGER4(ad_count0)
          DO i3=1,ad_count0
            IF (i3 .EQ. 1) THEN
              CALL POPCONTROL1B(branch)
            ELSE
              dp1_ad(i, m) = dp1_ad(i, m) + q4(1, i, m)*qsum_ad
              q4_ad(1, i, m) = q4_ad(1, i, m) + dp1(i, m)*qsum_ad
            END IF
            CALL POPINTEGER4(m)
          END DO
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY_ADM(qsum)
          temp = q4(4, i, l) + q4(3, i, l) - q4(2, i, l)
          temp_ad4 = (q4(2, i, l)+0.5*(temp*(pl+1.))-r3*(q4(4, i, l)*(pl&
&           *(pl+1.)+1.)))*qsum_ad
          temp_ad5 = (pe1(i, l+1)-pe2(i, k))*qsum_ad
          temp_ad6 = 0.5*(pl+1.)*temp_ad5
          temp_ad7 = -(r3*q4(4, i, l)*temp_ad5)
          pe1_ad(i, l+1) = pe1_ad(i, l+1) + temp_ad4
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad4
          q4_ad(2, i, l) = q4_ad(2, i, l) + temp_ad5 - temp_ad6
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad6 - r3*(pl*(pl+1.)+1.&
&           )*temp_ad5
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad6
          pl_ad = (2*pl+1.)*temp_ad7 + 0.5*temp*temp_ad5
          qsum_ad = 0.0_FVPRC
        END IF
        temp_ad = pl_ad/dp1(i, l)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad
        pe1_ad(i, l) = pe1_ad(i, l) - temp_ad
        dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k)-pe1(i, l))*temp_ad/dp1(&
&         i, l)
 130    CALL POPINTEGER4(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B(branch)
          CALL POPINTEGER4(l)
        END DO
      END DO
    END DO
    CALL POPCONTROL2B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL PPM_PROFILE_ADM(q4, q4_ad, dp1, dp1_ad, km, i1, i2, iv, kord)
    ELSE IF (branch .EQ. 1) THEN
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL CS_PROFILE_ADM(qs, qs_ad, q4, q4_ad, dp1, dp1_ad, km, i1, i2&
&                   , iv, kord)
    ELSE
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL CS_PROFILE_LINEAR_ADM(qs, qs_ad, q4, q4_ad, dp1, dp1_ad, km, &
&                          i1, i2, iv)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
        q2_ad(i, j, k) = q2_ad(i, j, k) + q4_ad(1, i, k)
        q4_ad(1, i, k) = 0.0_FVPRC
        pe1_ad(i, k+1) = pe1_ad(i, k+1) + dp1_ad(i, k)
        pe1_ad(i, k) = pe1_ad(i, k) - dp1_ad(i, k)
        dp1_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE MAP1_PPM_ADM
  SUBROUTINE MAP1_PPM(km, pe1, qs, kn, pe2, q2, i1, i2, j, ibeg, iend, &
&   jbeg, jend, iv, kord)
    IMPLICIT NONE
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! Mode: 0 == constituents  1 == ???
    INTEGER, INTENT(IN) :: iv
!       2 == remap temp with cs scheme
! Method order
    INTEGER, INTENT(IN) :: kord
! Current latitude
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! bottom BC
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
! real(FVPRC), intent(in) ::    q1(ibeg:iend,jbeg:jend,km) ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
! !DESCRIPTION:
! IV = 0: constituents
! pe1: pressure at layer edges (from model top to bottom surface)
!      in the original vertical coordinate
! pe2: pressure at layer edges (from model top to bottom surface)
!      in the new vertical coordinate
! !LOCAL VARIABLES:
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    INTEGER :: i, k, l, m, k0
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q2(i, j, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL CS_PROFILE_LINEAR(qs, q4, dp1, km, i1, i2, iv)
      ELSE
        CALL CS_PROFILE(qs, q4, dp1, km, i1, i2, iv, kord)
      END IF
    ELSE
      CALL PPM_PROFILE(q4, dp1, km, i1, i2, iv, kord)
    END IF
    DO i=i1,i2
      k0 = 1
      DO k=1,kn
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
            IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
              pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
              q2(i, j, k) = q4(2, i, l) + 0.5*(q4(4, i, l)+q4(3, i, l)-&
&               q4(2, i, l))*(pr+pl) - q4(4, i, l)*r3*(pr*(pr+pl)+pl**2)
              k0 = l
              GOTO 555
            ELSE
! Fractional area...
              qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, &
&               l)+q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+&
&               pl*(1.+pl))))
              DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
                IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer
                  qsum = qsum + dp1(i, m)*q4(1, i, m)
                ELSE
                  dp = pe2(i, k+1) - pe1(i, m)
                  esl = dp/dp1(i, m)
                  qsum = qsum + dp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(&
&                   2, i, m)+q4(4, i, m)*(1.-r23*esl)))
                  k0 = m
                  GOTO 123
                END IF
              END DO
              GOTO 123
            END IF
          END IF
        END DO
 123    q2(i, j, k) = qsum/(pe2(i, k+1)-pe2(i, k))
 555    CONTINUE
      END DO
    END DO
  END SUBROUTINE MAP1_PPM
!  Differentiation of mapn_tracer in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_ord4
! a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_
!mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_u
!pdate dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_super 
!fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlon fv_g
!rid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_mapz_mo
!d.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2_f
!b fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tracer2d_mo
!d.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_core_m
!od.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.sim_sol
!ver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest 
!sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.compute_di
!v_damping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy_corne
!rs_fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyppm 
!tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 dp2 q1
!   with respect to varying inputs: pe1 pe2 dp2 q1
  SUBROUTINE MAPN_TRACER_ADM(nq, km, kn, pe1, pe1_ad, pe2, pe2_ad, q1, &
&   q1_ad, dp2, dp2_ad, kord, j, i1, i2, isd, ied, jsd, jed, q_min, fill&
& )
    IMPLICIT NONE
! !INPUT PARAMETERS:
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
    INTEGER, INTENT(IN) :: j, nq, i1, i2
    INTEGER, INTENT(IN) :: isd, ied, jsd, jed
    INTEGER, INTENT(IN) :: kord(nq)
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
    REAL(fvprc), INTENT(IN) :: dp2(i1:i2, kn)
    REAL(fvprc) :: dp2_ad(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: q_min
    LOGICAL, INTENT(IN) :: fill
! Field input
    REAL(fvprc), INTENT(INOUT) :: q1(isd:ied, jsd:jed, km, nq)
    REAL(fvprc), INTENT(INOUT) :: q1_ad(isd:ied, jsd:jed, km, nq)
! !LOCAL VARIABLES:
    REAL(fvprc) :: q4(4, i1:i2, km, nq)
    REAL(fvprc) :: q4_ad(4, i1:i2, km, nq)
! Field output
    REAL(fvprc) :: q2(i1:i2, kn, nq)
    REAL(fvprc) :: q2_ad(i1:i2, kn, nq)
    REAL(fvprc) :: qsum(nq)
    REAL(fvprc) :: qsum_ad(nq)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_ad(i1:i2, km)
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: pl, pr, dp, esl, fac1, fac2
    REAL(fvprc) :: pl_ad, pr_ad, dp_ad, esl_ad, fac1_ad, fac2_ad
    INTEGER :: i, k, l, m, k0, iq
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    INTEGER :: branch
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: ad_count0
    INTEGER :: i3
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
      END DO
    END DO
    DO iq=1,nq
      DO k=1,km
        DO i=i1,i2
          q4(1, i, k, iq) = q1(i, j, k, iq)
        END DO
      END DO
      IF (kord(iq) .EQ. 111) THEN
        CALL PUSHREALARRAY_ADM(q4(1, :, 1, iq), 8*(i2-i1+1)/8)
        CALL SCALAR_PROFILE_LINEAR(qs, q4(1, i1:i2, 1, iq), dp1, km, i1&
&                            , i2, 0, q_min)
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHREALARRAY_ADM(q4(1, :, 1, iq), 8*(i2-i1+1)/8)
        CALL SCALAR_PROFILE(qs, q4(1, i1:i2, 1, iq), dp1, km, i1, i2, 0&
&                     , kord(iq), q_min)
        CALL PUSHCONTROL1B(0)
      END IF
    END DO
! Mapping
    DO i=i1,i2
      k0 = 1
      DO 130 k=1,kn
        CALL PUSHINTEGER4(l)
        ad_count = 1
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            GOTO 100
          ELSE
            CALL PUSHINTEGER4(l)
            ad_count = ad_count + 1
          END IF
        END DO
        CALL PUSHCONTROL1B(0)
        CALL PUSHINTEGER4(ad_count)
        CALL PUSHCONTROL2B(2)
        GOTO 120
 100    CALL PUSHCONTROL1B(1)
        CALL PUSHINTEGER4(ad_count)
        pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
        IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          CALL PUSHREALARRAY_ADM(fac1)
          fac1 = pr + pl
          CALL PUSHREALARRAY_ADM(fac2)
          fac2 = r3*(pr*fac1+pl*pl)
          CALL PUSHREALARRAY_ADM(fac1)
          fac1 = 0.5*fac1
          k0 = l
          CALL PUSHCONTROL1B(0)
          GOTO 130
        ELSE
! Fractional area...
          CALL PUSHREALARRAY_ADM(dp)
          dp = pe1(i, l+1) - pe2(i, k)
          CALL PUSHREALARRAY_ADM(fac1)
          fac1 = 1. + pl
          CALL PUSHREALARRAY_ADM(fac2)
          fac2 = r3*(1.+pl*fac1)
          CALL PUSHREALARRAY_ADM(fac1)
          fac1 = 0.5*fac1
          DO iq=1,nq
            CALL PUSHREALARRAY_ADM(qsum(iq))
            qsum(iq) = dp*(q4(2, i, l, iq)+(q4(4, i, l, iq)+q4(3, i, l, &
&             iq)-q4(2, i, l, iq))*fac1-q4(4, i, l, iq)*fac2)
          END DO
          CALL PUSHINTEGER4(m)
          ad_count0 = 1
          DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
            IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer..
              DO iq=1,nq
                CALL PUSHREALARRAY_ADM(qsum(iq))
                qsum(iq) = qsum(iq) + dp1(i, m)*q4(1, i, m, iq)
              END DO
              CALL PUSHINTEGER4(m)
              ad_count0 = ad_count0 + 1
            ELSE
              GOTO 110
            END IF
          END DO
          CALL PUSHCONTROL1B(0)
          CALL PUSHINTEGER4(ad_count0)
          CALL PUSHCONTROL2B(1)
          GOTO 120
 110      CALL PUSHCONTROL1B(1)
          CALL PUSHINTEGER4(ad_count0)
          CALL PUSHREALARRAY_ADM(dp)
          dp = pe2(i, k+1) - pe1(i, m)
          esl = dp/dp1(i, m)
          CALL PUSHREALARRAY_ADM(fac1)
          fac1 = 0.5*esl
          CALL PUSHREALARRAY_ADM(fac2)
          fac2 = 1. - r23*esl
          DO iq=1,nq
            CALL PUSHREALARRAY_ADM(qsum(iq))
            qsum(iq) = qsum(iq) + dp*(q4(2, i, m, iq)+fac1*(q4(3, i, m, &
&             iq)-q4(2, i, m, iq)+q4(4, i, m, iq)*fac2))
          END DO
          k0 = m
          CALL PUSHCONTROL2B(0)
        END IF
 120    CALL PUSHCONTROL1B(1)
 130  CONTINUE
    END DO
    q2_ad = 0.0_FVPRC
    DO iq=nq,1,-1
      DO k=kn,1,-1
        DO i=i2,i1,-1
          q2_ad(i, k, iq) = q2_ad(i, k, iq) + q1_ad(i, j, k, iq)
          q1_ad(i, j, k, iq) = 0.0_FVPRC
        END DO
      END DO
    END DO
    dp1_ad = 0.0_FVPRC
    qsum_ad = 0.0_FVPRC
    q4_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          fac1_ad = 0.0_FVPRC
          fac2_ad = 0.0_FVPRC
          DO iq=nq,1,-1
            temp_ad2 = fac1*q2_ad(i, k, iq)
            q4_ad(2, i, l, iq) = q4_ad(2, i, l, iq) + q2_ad(i, k, iq) - &
&             temp_ad2
            q4_ad(4, i, l, iq) = q4_ad(4, i, l, iq) + temp_ad2 - fac2*&
&             q2_ad(i, k, iq)
            q4_ad(3, i, l, iq) = q4_ad(3, i, l, iq) + temp_ad2
            fac1_ad = fac1_ad + (q4(4, i, l, iq)+q4(3, i, l, iq)-q4(2, i&
&             , l, iq))*q2_ad(i, k, iq)
            fac2_ad = fac2_ad - q4(4, i, l, iq)*q2_ad(i, k, iq)
            q2_ad(i, k, iq) = 0.0_FVPRC
          END DO
          temp_ad0 = r3*fac2_ad
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY_ADM(fac1)
          fac1_ad = pr*temp_ad0 + 0.5*fac1_ad
          CALL POPREALARRAY_ADM(fac2)
          pr_ad = fac1_ad + fac1*temp_ad0
          pl_ad = fac1_ad + 2*pl*temp_ad0
          CALL POPREALARRAY_ADM(fac1)
          temp_ad1 = pr_ad/dp1(i, l)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad1
          pe1_ad(i, l) = pe1_ad(i, l) - temp_ad1
          dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k+1)-pe1(i, l))*temp_ad1&
&           /dp1(i, l)
        ELSE
          DO iq=nq,1,-1
            temp_ad8 = q2_ad(i, k, iq)/dp2(i, k)
            qsum_ad(iq) = qsum_ad(iq) + temp_ad8
            dp2_ad(i, k) = dp2_ad(i, k) - qsum(iq)*temp_ad8/dp2(i, k)
            q2_ad(i, k, iq) = 0.0_FVPRC
          END DO
          CALL POPCONTROL2B(branch)
          IF (branch .EQ. 0) THEN
            dp_ad = 0.0_FVPRC
            fac1_ad = 0.0_FVPRC
            fac2_ad = 0.0_FVPRC
            DO iq=nq,1,-1
              CALL POPREALARRAY_ADM(qsum(iq))
              temp0 = q4(3, i, m, iq) - q4(2, i, m, iq) + q4(4, i, m, iq&
&               )*fac2
              temp_ad6 = dp*qsum_ad(iq)
              temp_ad7 = fac1*temp_ad6
              dp_ad = dp_ad + (q4(2, i, m, iq)+fac1*temp0)*qsum_ad(iq)
              q4_ad(2, i, m, iq) = q4_ad(2, i, m, iq) + temp_ad6 - &
&               temp_ad7
              fac1_ad = fac1_ad + temp0*temp_ad6
              q4_ad(3, i, m, iq) = q4_ad(3, i, m, iq) + temp_ad7
              q4_ad(4, i, m, iq) = q4_ad(4, i, m, iq) + fac2*temp_ad7
              fac2_ad = fac2_ad + q4(4, i, m, iq)*temp_ad7
            END DO
            CALL POPREALARRAY_ADM(fac2)
            esl_ad = 0.5*fac1_ad - r23*fac2_ad
            CALL POPREALARRAY_ADM(fac1)
            temp_ad5 = esl_ad/dp1(i, m)
            dp_ad = dp_ad + temp_ad5
            dp1_ad(i, m) = dp1_ad(i, m) - dp*temp_ad5/dp1(i, m)
            CALL POPREALARRAY_ADM(dp)
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + dp_ad
            pe1_ad(i, m) = pe1_ad(i, m) - dp_ad
          ELSE IF (branch .NE. 1) THEN
            GOTO 140
          END IF
          CALL POPINTEGER4(ad_count0)
          DO i3=1,ad_count0
            IF (i3 .EQ. 1) THEN
              CALL POPCONTROL1B(branch)
            ELSE
              DO iq=nq,1,-1
                CALL POPREALARRAY_ADM(qsum(iq))
                dp1_ad(i, m) = dp1_ad(i, m) + q4(1, i, m, iq)*qsum_ad(iq&
&                 )
                q4_ad(1, i, m, iq) = q4_ad(1, i, m, iq) + dp1(i, m)*&
&                 qsum_ad(iq)
              END DO
            END IF
            CALL POPINTEGER4(m)
          END DO
          dp_ad = 0.0_FVPRC
          fac1_ad = 0.0_FVPRC
          fac2_ad = 0.0_FVPRC
          DO iq=nq,1,-1
            CALL POPREALARRAY_ADM(qsum(iq))
            temp = q4(4, i, l, iq) + q4(3, i, l, iq) - q4(2, i, l, iq)
            temp_ad3 = dp*qsum_ad(iq)
            temp_ad4 = fac1*temp_ad3
            dp_ad = dp_ad + (q4(2, i, l, iq)+temp*fac1-q4(4, i, l, iq)*&
&             fac2)*qsum_ad(iq)
            q4_ad(2, i, l, iq) = q4_ad(2, i, l, iq) + temp_ad3 - &
&             temp_ad4
            q4_ad(4, i, l, iq) = q4_ad(4, i, l, iq) + temp_ad4 - fac2*&
&             temp_ad3
            q4_ad(3, i, l, iq) = q4_ad(3, i, l, iq) + temp_ad4
            fac1_ad = fac1_ad + temp*temp_ad3
            fac2_ad = fac2_ad - q4(4, i, l, iq)*temp_ad3
            qsum_ad(iq) = 0.0_FVPRC
          END DO
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY_ADM(fac1)
          fac1_ad = r3*pl*fac2_ad + 0.5*fac1_ad
          CALL POPREALARRAY_ADM(fac2)
          pl_ad = fac1_ad + r3*fac1*fac2_ad
          CALL POPREALARRAY_ADM(fac1)
          CALL POPREALARRAY_ADM(dp)
          pe1_ad(i, l+1) = pe1_ad(i, l+1) + dp_ad
          pe2_ad(i, k) = pe2_ad(i, k) - dp_ad
        END IF
        temp_ad = pl_ad/dp1(i, l)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad
        pe1_ad(i, l) = pe1_ad(i, l) - temp_ad
        dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k)-pe1(i, l))*temp_ad/dp1(&
&         i, l)
 140    CALL POPINTEGER4(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B(branch)
          CALL POPINTEGER4(l)
        END DO
      END DO
    END DO
    DO iq=nq,1,-1
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(q4(1, :, 1, iq), 8*(i2-i1+1)/8)
        CALL SCALAR_PROFILE_ADM(qs, q4(1, i1:i2, 1, iq), q4_ad(1, i1:i2&
&                         , 1, iq), dp1, dp1_ad, km, i1, i2, 0, kord(iq)&
&                         , q_min)
      ELSE
        CALL POPREALARRAY_ADM(q4(1, :, 1, iq), 8*(i2-i1+1)/8)
        CALL SCALAR_PROFILE_LINEAR_ADM(qs, q4(1, i1:i2, 1, iq), q4_ad(1&
&                                , i1:i2, 1, iq), dp1, dp1_ad, km, i1, &
&                                i2, 0, q_min)
      END IF
      DO k=km,1,-1
        DO i=i2,i1,-1
          q1_ad(i, j, k, iq) = q1_ad(i, j, k, iq) + q4_ad(1, i, k, iq)
          q4_ad(1, i, k, iq) = 0.0_FVPRC
        END DO
      END DO
    END DO
    DO k=km,1,-1
      DO i=i2,i1,-1
        pe1_ad(i, k+1) = pe1_ad(i, k+1) + dp1_ad(i, k)
        pe1_ad(i, k) = pe1_ad(i, k) - dp1_ad(i, k)
        dp1_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE MAPN_TRACER_ADM
  SUBROUTINE MAPN_TRACER(nq, km, kn, pe1, pe2, q1, dp2, kord, j, i1, i2&
&   , isd, ied, jsd, jed, q_min, fill)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
    INTEGER, INTENT(IN) :: j, nq, i1, i2
    INTEGER, INTENT(IN) :: isd, ied, jsd, jed
    INTEGER, INTENT(IN) :: kord(nq)
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
    REAL(fvprc), INTENT(IN) :: dp2(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: q_min
    LOGICAL, INTENT(IN) :: fill
! Field input
    REAL(fvprc), INTENT(INOUT) :: q1(isd:ied, jsd:jed, km, nq)
! !LOCAL VARIABLES:
    REAL(fvprc) :: q4(4, i1:i2, km, nq)
! Field output
    REAL(fvprc) :: q2(i1:i2, kn, nq)
    REAL(fvprc) :: qsum(nq)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: pl, pr, dp, esl, fac1, fac2
    INTEGER :: i, k, l, m, k0, iq
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
      END DO
    END DO
    DO iq=1,nq
      DO k=1,km
        DO i=i1,i2
          q4(1, i, k, iq) = q1(i, j, k, iq)
        END DO
      END DO
      IF (kord(iq) .EQ. 111) THEN
        CALL SCALAR_PROFILE_LINEAR(qs, q4(1, i1:i2, 1, iq), dp1, km, i1&
&                            , i2, 0, q_min)
      ELSE
        CALL SCALAR_PROFILE(qs, q4(1, i1:i2, 1, iq), dp1, km, i1, i2, 0&
&                     , kord(iq), q_min)
      END IF
    END DO
! Mapping
    DO i=i1,i2
      k0 = 1
      DO k=1,kn
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
            IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
              pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
              fac1 = pr + pl
              fac2 = r3*(pr*fac1+pl*pl)
              fac1 = 0.5*fac1
              DO iq=1,nq
                q2(i, k, iq) = q4(2, i, l, iq) + (q4(4, i, l, iq)+q4(3, &
&                 i, l, iq)-q4(2, i, l, iq))*fac1 - q4(4, i, l, iq)*fac2
              END DO
              k0 = l
              GOTO 555
            ELSE
! Fractional area...
              dp = pe1(i, l+1) - pe2(i, k)
              fac1 = 1. + pl
              fac2 = r3*(1.+pl*fac1)
              fac1 = 0.5*fac1
              DO iq=1,nq
                qsum(iq) = dp*(q4(2, i, l, iq)+(q4(4, i, l, iq)+q4(3, i&
&                 , l, iq)-q4(2, i, l, iq))*fac1-q4(4, i, l, iq)*fac2)
              END DO
              DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
                IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer..
                  DO iq=1,nq
                    qsum(iq) = qsum(iq) + dp1(i, m)*q4(1, i, m, iq)
                  END DO
                ELSE
                  dp = pe2(i, k+1) - pe1(i, m)
                  esl = dp/dp1(i, m)
                  fac1 = 0.5*esl
                  fac2 = 1. - r23*esl
                  DO iq=1,nq
                    qsum(iq) = qsum(iq) + dp*(q4(2, i, m, iq)+fac1*(q4(3&
&                     , i, m, iq)-q4(2, i, m, iq)+q4(4, i, m, iq)*fac2))
                  END DO
                  k0 = m
                  GOTO 123
                END IF
              END DO
              GOTO 123
            END IF
          END IF
        END DO
 123    CONTINUE
        DO iq=1,nq
          q2(i, k, iq) = qsum(iq)/dp2(i, k)
        END DO
 555    CONTINUE
      END DO
    END DO
!if (fill) call fillz(i2-i1+1, kn, nq, q2, dp2)
    DO iq=1,nq
!    if (fill) call fillz(i2-i1+1, kn, 1, q2(i1,1,iq), dp2)
      DO k=1,kn
        DO i=i1,i2
          q1(i, j, k, iq) = q2(i, k, iq)
        END DO
      END DO
    END DO
  END SUBROUTINE MAPN_TRACER
!  Differentiation of map1_q2 in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_ord4 a2b
!_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.
!adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_updat
!e dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_super fv_d
!ynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlon fv_grid_
!utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_mapz_mod.pk
!ez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2 fv
!_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tracer2d_mod.tr
!acer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_core_mod.r
!iem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.sim_solver 
!nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest sw_c
!ore_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.compute_div_da
!mping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy_corners_f
!b tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyppm tp_c
!ore_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 dp2 q1 q2
!   with respect to varying inputs: pe1 pe2 dp2 q1 q2
  SUBROUTINE MAP1_Q2_ADM(km, pe1, pe1_ad, q1, q1_ad, kn, pe2, pe2_ad, q2&
&   , q2_ad, dp2, dp2_ad, i1, i2, iv, kord, j, ibeg, iend, jbeg, jend, &
&   q_min)
    IMPLICIT NONE
! !INPUT PARAMETERS:
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: i1, i2
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Mode: 0 ==  constituents 1 == ???
    INTEGER, INTENT(IN) :: iv
    INTEGER, INTENT(IN) :: kord
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
! Field input
    REAL(fvprc), INTENT(IN) :: q1(ibeg:iend, jbeg:jend, km)
    REAL(fvprc) :: q1_ad(ibeg:iend, jbeg:jend, km)
    REAL(fvprc), INTENT(IN) :: dp2(i1:i2, kn)
    REAL(fvprc) :: dp2_ad(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: q_min
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(i1:i2, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_ad(i1:i2, kn)
! !LOCAL VARIABLES:
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_ad(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: q4_ad(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    REAL(fvprc) :: pl_ad, pr_ad, qsum_ad, dp_ad, esl_ad
    INTEGER :: i, k, l, m, k0
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp_ad11
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: branch
    INTEGER :: ad_count0
    INTEGER :: i3
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q1(i, j, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
        CALL SCALAR_PROFILE_LINEAR(qs, q4, dp1, km, i1, i2, iv, q_min)
        CALL PUSHCONTROL2B(2)
      ELSE
        CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
        CALL SCALAR_PROFILE(qs, q4, dp1, km, i1, i2, iv, kord, q_min)
        CALL PUSHCONTROL2B(1)
      END IF
    ELSE
      CALL PUSHREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL PPM_PROFILE(q4, dp1, km, i1, i2, iv, kord)
      CALL PUSHCONTROL2B(0)
    END IF
! Mapping
    DO i=i1,i2
      k0 = 1
      DO 120 k=1,kn
        CALL PUSHINTEGER4(l)
        ad_count = 1
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            GOTO 100
          ELSE
            CALL PUSHINTEGER4(l)
            ad_count = ad_count + 1
          END IF
        END DO
        CALL PUSHCONTROL1B(0)
        CALL PUSHINTEGER4(ad_count)
        CALL PUSHCONTROL2B(2)
        GOTO 123
 100    CALL PUSHCONTROL1B(1)
        CALL PUSHINTEGER4(ad_count)
        pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
        IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
          k0 = l
          CALL PUSHCONTROL1B(0)
          GOTO 120
        ELSE
! Fractional area...
          CALL PUSHREALARRAY_ADM(qsum)
          qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, l)+&
&           q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+pl*(1.+&
&           pl))))
          CALL PUSHINTEGER4(m)
          ad_count0 = 1
          DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
            IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer..
              qsum = qsum + dp1(i, m)*q4(1, i, m)
              CALL PUSHINTEGER4(m)
              ad_count0 = ad_count0 + 1
            ELSE
              GOTO 110
            END IF
          END DO
          CALL PUSHCONTROL1B(0)
          CALL PUSHINTEGER4(ad_count0)
          CALL PUSHCONTROL2B(1)
          GOTO 123
 110      CALL PUSHCONTROL1B(1)
          CALL PUSHINTEGER4(ad_count0)
          dp = pe2(i, k+1) - pe1(i, m)
          esl = dp/dp1(i, m)
          qsum = qsum + dp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(2, i, m)&
&           +q4(4, i, m)*(1.-r23*esl)))
          k0 = m
          CALL PUSHCONTROL2B(0)
        END IF
 123    CALL PUSHCONTROL1B(1)
 120  CONTINUE
    END DO
    dp1_ad = 0.0_FVPRC
    qsum_ad = 0.0_FVPRC
    q4_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          temp_ad0 = 0.5*(pr+pl)*q2_ad(i, k)
          temp_ad1 = 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2, i, l))*q2_ad(i, &
&           k)
          temp_ad2 = -(r3*q4(4, i, l)*q2_ad(i, k))
          q4_ad(2, i, l) = q4_ad(2, i, l) + q2_ad(i, k) - temp_ad0
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad0 - r3*(pr*(pr+pl)+pl&
&           **2)*q2_ad(i, k)
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad0
          pr_ad = (2*pr+pl)*temp_ad2 + temp_ad1
          pl_ad = (2*pl+pr)*temp_ad2 + temp_ad1
          q2_ad(i, k) = 0.0_FVPRC
          temp_ad3 = pr_ad/dp1(i, l)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad3
          pe1_ad(i, l) = pe1_ad(i, l) - temp_ad3
          dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k+1)-pe1(i, l))*temp_ad3&
&           /dp1(i, l)
        ELSE
          temp_ad11 = q2_ad(i, k)/dp2(i, k)
          qsum_ad = qsum_ad + temp_ad11
          dp2_ad(i, k) = dp2_ad(i, k) - qsum*temp_ad11/dp2(i, k)
          q2_ad(i, k) = 0.0_FVPRC
          CALL POPCONTROL2B(branch)
          IF (branch .EQ. 0) THEN
            dp = pe2(i, k+1) - pe1(i, m)
            esl = dp/dp1(i, m)
            temp0 = q4(3, i, m) - q4(2, i, m) + q4(4, i, m)*(-(r23*esl)+&
&             1.)
            temp_ad8 = dp*qsum_ad
            temp_ad9 = 0.5*esl*temp_ad8
            q4_ad(2, i, m) = q4_ad(2, i, m) + temp_ad8 - temp_ad9
            esl_ad = 0.5*temp0*temp_ad8 - q4(4, i, m)*r23*temp_ad9
            q4_ad(3, i, m) = q4_ad(3, i, m) + temp_ad9
            q4_ad(4, i, m) = q4_ad(4, i, m) + (1.-r23*esl)*temp_ad9
            temp_ad10 = esl_ad/dp1(i, m)
            dp_ad = temp_ad10 + (q4(2, i, m)+0.5*(esl*temp0))*qsum_ad
            dp1_ad(i, m) = dp1_ad(i, m) - dp*temp_ad10/dp1(i, m)
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + dp_ad
            pe1_ad(i, m) = pe1_ad(i, m) - dp_ad
          ELSE IF (branch .NE. 1) THEN
            GOTO 130
          END IF
          CALL POPINTEGER4(ad_count0)
          DO i3=1,ad_count0
            IF (i3 .EQ. 1) THEN
              CALL POPCONTROL1B(branch)
            ELSE
              dp1_ad(i, m) = dp1_ad(i, m) + q4(1, i, m)*qsum_ad
              q4_ad(1, i, m) = q4_ad(1, i, m) + dp1(i, m)*qsum_ad
            END IF
            CALL POPINTEGER4(m)
          END DO
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY_ADM(qsum)
          temp = q4(4, i, l) + q4(3, i, l) - q4(2, i, l)
          temp_ad4 = (q4(2, i, l)+0.5*(temp*(pl+1.))-r3*(q4(4, i, l)*(pl&
&           *(pl+1.)+1.)))*qsum_ad
          temp_ad5 = (pe1(i, l+1)-pe2(i, k))*qsum_ad
          temp_ad6 = 0.5*(pl+1.)*temp_ad5
          temp_ad7 = -(r3*q4(4, i, l)*temp_ad5)
          pe1_ad(i, l+1) = pe1_ad(i, l+1) + temp_ad4
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad4
          q4_ad(2, i, l) = q4_ad(2, i, l) + temp_ad5 - temp_ad6
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad6 - r3*(pl*(pl+1.)+1.&
&           )*temp_ad5
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad6
          pl_ad = (2*pl+1.)*temp_ad7 + 0.5*temp*temp_ad5
          qsum_ad = 0.0_FVPRC
        END IF
        temp_ad = pl_ad/dp1(i, l)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad
        pe1_ad(i, l) = pe1_ad(i, l) - temp_ad
        dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k)-pe1(i, l))*temp_ad/dp1(&
&         i, l)
 130    CALL POPINTEGER4(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B(branch)
          CALL POPINTEGER4(l)
        END DO
      END DO
    END DO
    CALL POPCONTROL2B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL PPM_PROFILE_ADM(q4, q4_ad, dp1, dp1_ad, km, i1, i2, iv, kord)
    ELSE IF (branch .EQ. 1) THEN
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL SCALAR_PROFILE_ADM(qs, q4, q4_ad, dp1, dp1_ad, km, i1, i2, iv&
&                       , kord, q_min)
    ELSE
      CALL POPREALARRAY_ADM(q4, 8*4*(i2-i1+1)*km/8)
      CALL SCALAR_PROFILE_LINEAR_ADM(qs, q4, q4_ad, dp1, dp1_ad, km, i1&
&                              , i2, iv, q_min)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
        q1_ad(i, j, k) = q1_ad(i, j, k) + q4_ad(1, i, k)
        q4_ad(1, i, k) = 0.0_FVPRC
        pe1_ad(i, k+1) = pe1_ad(i, k+1) + dp1_ad(i, k)
        pe1_ad(i, k) = pe1_ad(i, k) - dp1_ad(i, k)
        dp1_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE MAP1_Q2_ADM
  SUBROUTINE MAP1_Q2(km, pe1, q1, kn, pe2, q2, dp2, i1, i2, iv, kord, j&
&   , ibeg, iend, jbeg, jend, q_min)
    IMPLICIT NONE
! !INPUT PARAMETERS:
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: i1, i2
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Mode: 0 ==  constituents 1 == ???
    INTEGER, INTENT(IN) :: iv
    INTEGER, INTENT(IN) :: kord
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
! Field input
    REAL(fvprc), INTENT(IN) :: q1(ibeg:iend, jbeg:jend, km)
    REAL(fvprc), INTENT(IN) :: dp2(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: q_min
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(i1:i2, kn)
! !LOCAL VARIABLES:
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    INTEGER :: i, k, l, m, k0
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q1(i, j, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL SCALAR_PROFILE_LINEAR(qs, q4, dp1, km, i1, i2, iv, q_min)
      ELSE
        CALL SCALAR_PROFILE(qs, q4, dp1, km, i1, i2, iv, kord, q_min)
      END IF
    ELSE
      CALL PPM_PROFILE(q4, dp1, km, i1, i2, iv, kord)
    END IF
! Mapping
    DO i=i1,i2
      k0 = 1
      DO k=1,kn
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
            IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
              pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
              q2(i, k) = q4(2, i, l) + 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2&
&               , i, l))*(pr+pl) - q4(4, i, l)*r3*(pr*(pr+pl)+pl**2)
              k0 = l
              GOTO 555
            ELSE
! Fractional area...
              qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, &
&               l)+q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+&
&               pl*(1.+pl))))
              DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
                IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer..
                  qsum = qsum + dp1(i, m)*q4(1, i, m)
                ELSE
                  dp = pe2(i, k+1) - pe1(i, m)
                  esl = dp/dp1(i, m)
                  qsum = qsum + dp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(&
&                   2, i, m)+q4(4, i, m)*(1.-r23*esl)))
                  k0 = m
                  GOTO 123
                END IF
              END DO
              GOTO 123
            END IF
          END IF
        END DO
 123    q2(i, k) = qsum/dp2(i, k)
 555    CONTINUE
      END DO
    END DO
  END SUBROUTINE MAP1_Q2
!  Differentiation of map1_cubic in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_m
!od.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_
!halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core
!_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.r
!ayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_
!to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_ene
!rgy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz
!_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz 
!fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solv
!er_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_co
!re_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergenc
!e_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_
!mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_
!mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core
!_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 q2
!   with respect to varying inputs: pe1 pe2 q2
!----------------------------------------------------------------------- 
!BOP
! !ROUTINE:  map1_cubic --- Cubic Interpolation for vertical re-mapping
!
! !INTERFACE:
  SUBROUTINE MAP1_CUBIC_FWD(km, pe1, kn, pe2, q2, i1, i2, j, ibeg, iend&
&   , jbeg, jend, akap, t_var, conserv)
    IMPLICIT NONE
!EOC
! !INPUT PARAMETERS:
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
    REAL(fvprc), INTENT(IN) :: akap
! Thermodynamic variable to remap
    INTEGER, INTENT(IN) :: t_var
!     1:TE  2:T  3:PT 
    LOGICAL, INTENT(IN) :: conserv
! Current latitude
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
!      real(FVPRC), intent(in) ::    q1(ibeg:iend,jbeg:jend,km) ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
! !DESCRIPTION:
!
!     Perform Cubic Interpolation a given latitude
! pe1: pressure at layer edges (from model top to bottom surface)
!      in the original vertical coordinate
! pe2: pressure at layer edges (from model top to bottom surface)
!      in the new vertical coordinate
!
! !REVISION HISTORY:
!    2005.11.14   Takacs    Initial Code
!    2016.07.20   Putman    Modified to make genaric for any thermodynamic variable
!
!EOP
!-----------------------------------------------------------------------
!BOC
!
! !LOCAL VARIABLES:
    REAL(fvprc) :: qx(i1:i2, km)
    REAL(fvprc) :: logpl1(i1:i2, km)
    REAL(fvprc) :: logpl2(i1:i2, kn)
    REAL(fvprc) :: dlogp1(i1:i2, km)
    REAL(fvprc) :: vsum1(i1:i2)
    REAL(fvprc) :: vsum2(i1:i2)
    REAL(fvprc) :: am2, am1, ap0, ap1, p, plp1, plp0, plm1, plm2, dlp0, &
&   dlm1, dlm2
    INTEGER :: i, k, lm2, lm1, lp0, lp1
    INTRINSIC LOG
    INTRINSIC EXP
    INTRINSIC MAX
    INTRINSIC MIN
    INTEGER :: ad_count
! Initialization
! --------------
    SELECT CASE  (t_var) 
    CASE (1) 
! Total Energy Remapping in Log(P)
      DO k=1,km
        qx(:, k) = q2(i1:i2, j, k)
        logpl1(:, k) = LOG(r2*(pe1(:, k)+pe1(:, k+1)))
      END DO
      DO k=1,kn
        logpl2(:, k) = LOG(r2*(pe2(:, k)+pe2(:, k+1)))
      END DO
      DO k=1,km-1
        dlogp1(:, k) = logpl1(:, k+1) - logpl1(:, k)
      END DO
      CALL PUSHCONTROL2B_FV(1)
    CASE (2) 
! Temperature Remapping in Log(P)
      DO k=1,km
        qx(:, k) = q2(i1:i2, j, k)
        logpl1(:, k) = LOG(r2*(pe1(:, k)+pe1(:, k+1)))
      END DO
      DO k=1,kn
        logpl2(:, k) = LOG(r2*(pe2(:, k)+pe2(:, k+1)))
      END DO
      DO k=1,km-1
        dlogp1(:, k) = logpl1(:, k+1) - logpl1(:, k)
      END DO
      CALL PUSHCONTROL2B_FV(2)
    CASE (3) 
! Potential Temperature Remapping in P^KAPPA
      DO k=1,km
        qx(:, k) = q2(i1:i2, j, k)
        logpl1(:, k) = EXP(akap*LOG(r2*(pe1(:, k)+pe1(:, k+1))))
      END DO
      DO k=1,kn
        logpl2(:, k) = EXP(akap*LOG(r2*(pe2(:, k)+pe2(:, k+1))))
      END DO
      DO k=1,km-1
        dlogp1(:, k) = logpl1(:, k+1) - logpl1(:, k)
      END DO
      CALL PUSHCONTROL2B_FV(3)
    CASE DEFAULT
      CALL PUSHCONTROL2B_FV(0)
    END SELECT
    IF (conserv) THEN
! Compute vertical integral of Input TE
! -------------------------------------
      vsum1(:) = r0
      DO i=i1,i2
        DO k=1,km
          vsum1(i) = vsum1(i) + qx(i, k)*(pe1(i, k+1)-pe1(i, k))
        END DO
        CALL PUSHREALARRAY(vsum1(i))
        vsum1(i) = vsum1(i)/(pe1(i, km+1)-pe1(i, 1))
      END DO
      CALL PUSHCONTROL1B_FV(1)
    ELSE
      CALL PUSHCONTROL1B_FV(0)
    END IF
! Interpolate TE onto target Pressures
! ------------------------------------
    DO i=i1,i2
      DO k=1,kn
        CALL PUSHINTEGER(lp0)
        lp0 = 1
        ad_count = 1
        DO WHILE (lp0 .LE. km)
          IF (logpl1(i, lp0) .LT. logpl2(i, k)) THEN
            lp0 = lp0 + 1
            ad_count = ad_count + 1
          ELSE
            GOTO 100
          END IF
        END DO
        CALL PUSHCONTROL1B_FV(0)
        CALL PUSHINTEGER(ad_count)
        GOTO 110
 100    CALL PUSHCONTROL1B_FV(1)
        CALL PUSHINTEGER(ad_count)
 110    IF (lp0 - 1 .LT. 1) THEN
          CALL PUSHINTEGER(lm1)
          lm1 = 1
          CALL PUSHCONTROL1B_FV(0)
        ELSE
          CALL PUSHINTEGER(lm1)
          lm1 = lp0 - 1
          CALL PUSHCONTROL1B_FV(1)
        END IF
        IF (lp0 .GT. km) THEN
          lp0 = km
        ELSE
          lp0 = lp0
        END IF
! Extrapolate Linearly in LogP above first model level
! ----------------------------------------------------
        IF (lm1 .EQ. 1 .AND. lp0 .EQ. 1) THEN
          CALL PUSHREALARRAY(q2(i, j, k))
          q2(i, j, k) = qx(i, 1) + (qx(i, 2)-qx(i, 1))*(logpl2(i, k)-&
&           logpl1(i, 1))/(logpl1(i, 2)-logpl1(i, 1))
! Extrapolate Linearly in LogP below last model level
! ---------------------------------------------------
          CALL PUSHCONTROL2B_FV(3)
        ELSE IF (lm1 .EQ. km .AND. lp0 .EQ. km) THEN
          CALL PUSHREALARRAY(q2(i, j, k))
          q2(i, j, k) = qx(i, km) + (qx(i, km)-qx(i, km-1))*(logpl2(i, k&
&           )-logpl1(i, km))/(logpl1(i, km)-logpl1(i, km-1))
! Interpolate Linearly in LogP between levels 1 => 2 and km-1 => km
! -----------------------------------------------------------------
          CALL PUSHCONTROL2B_FV(2)
        ELSE IF (lm1 .EQ. 1 .OR. lp0 .EQ. km) THEN
          CALL PUSHREALARRAY(q2(i, j, k))
          q2(i, j, k) = qx(i, lp0) + (qx(i, lm1)-qx(i, lp0))*(logpl2(i, &
&           k)-logpl1(i, lp0))/(logpl1(i, lm1)-logpl1(i, lp0))
! Interpolate Cubicly in LogP between other model levels
! ------------------------------------------------------
          CALL PUSHCONTROL2B_FV(1)
        ELSE
          CALL PUSHINTEGER(lp1)
          lp1 = lp0 + 1
          CALL PUSHINTEGER(lm2)
          lm2 = lm1 - 1
          CALL PUSHREALARRAY(p)
          p = logpl2(i, k)
          plp1 = logpl1(i, lp1)
          plp0 = logpl1(i, lp0)
          plm1 = logpl1(i, lm1)
          plm2 = logpl1(i, lm2)
          CALL PUSHREALARRAY(dlp0)
          dlp0 = dlogp1(i, lp0)
          CALL PUSHREALARRAY(dlm1)
          dlm1 = dlogp1(i, lm1)
          CALL PUSHREALARRAY(dlm2)
          dlm2 = dlogp1(i, lm2)
          CALL PUSHREALARRAY(ap1)
          ap1 = (p-plp0)*(p-plm1)*(p-plm2)/(dlp0*(dlp0+dlm1)*(dlp0+dlm1+&
&           dlm2))
          CALL PUSHREALARRAY(ap0)
          ap0 = (plp1-p)*(p-plm1)*(p-plm2)/(dlp0*dlm1*(dlm1+dlm2))
          CALL PUSHREALARRAY(am1)
          am1 = (plp1-p)*(plp0-p)*(p-plm2)/(dlm1*dlm2*(dlp0+dlm1))
          CALL PUSHREALARRAY(am2)
          am2 = (plp1-p)*(plp0-p)*(plm1-p)/(dlm2*(dlm1+dlm2)*(dlp0+dlm1+&
&           dlm2))
          CALL PUSHREALARRAY(q2(i, j, k))
          q2(i, j, k) = ap1*qx(i, lp1) + ap0*qx(i, lp0) + am1*qx(i, lm1)&
&           + am2*qx(i, lm2)
          CALL PUSHCONTROL2B_FV(0)
        END IF
      END DO
    END DO
    IF (conserv) THEN
! Compute vertical integral of Output TE
! --------------------------------------
      vsum2(:) = r0
      DO i=i1,i2
        DO k=1,kn
          vsum2(i) = vsum2(i) + q2(i, j, k)*(pe2(i, k+1)-pe2(i, k))
        END DO
        CALL PUSHREALARRAY(vsum2(i))
        vsum2(i) = vsum2(i)/(pe2(i, kn+1)-pe2(i, 1))
      END DO
! Adjust Final TE to conserve
! ---------------------------
      DO i=i1,i2
        DO k=1,kn
          CALL PUSHREALARRAY(q2(i, j, k))
          q2(i, j, k) = q2(i, j, k) + vsum1(i) - vsum2(i)
        END DO
      END DO
      CALL PUSHINTEGER(lp0)
      CALL PUSHREALARRAY(dlp0)
      CALL PUSHINTEGER(lp1)
      CALL PUSHREALARRAY(p)
      CALL PUSHINTEGER(lm1)
      CALL PUSHREALARRAY(dlm1)
      CALL PUSHINTEGER(lm2)
      CALL PUSHREALARRAY(dlm2)
      CALL PUSHREALARRAY(qx, 8*(i2-i1+1)*km/8)
      CALL PUSHREALARRAY(ap0)
      CALL PUSHREALARRAY(ap1)
      CALL PUSHREALARRAY(am1)
      CALL PUSHREALARRAY(logpl1, 8*(i2-i1+1)*km/8)
      CALL PUSHREALARRAY(am2)
      CALL PUSHREALARRAY(logpl2, 8*(i2-i1+1)*kn/8)
      CALL PUSHCONTROL1B_FV(0)
    ELSE
      CALL PUSHINTEGER(lp0)
      CALL PUSHREALARRAY(dlp0)
      CALL PUSHINTEGER(lp1)
      CALL PUSHREALARRAY(p)
      CALL PUSHINTEGER(lm1)
      CALL PUSHREALARRAY(dlm1)
      CALL PUSHINTEGER(lm2)
      CALL PUSHREALARRAY(dlm2)
      CALL PUSHREALARRAY(qx, 8*(i2-i1+1)*km/8)
      CALL PUSHREALARRAY(ap0)
      CALL PUSHREALARRAY(ap1)
      CALL PUSHREALARRAY(am1)
      CALL PUSHREALARRAY(logpl1, 8*(i2-i1+1)*km/8)
      CALL PUSHREALARRAY(am2)
      CALL PUSHREALARRAY(logpl2, 8*(i2-i1+1)*kn/8)
      CALL PUSHCONTROL1B_FV(1)
    END IF
  END SUBROUTINE MAP1_CUBIC_FWD
!  Differentiation of map1_cubic in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_
!mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe
!_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_cor
!e_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.
!rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed
!_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_en
!ergy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_map
!z_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz
! fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_sol
!ver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_c
!ore_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergen
!ce_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core
!_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core
!_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_cor
!e_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 q2
!   with respect to varying inputs: pe1 pe2 q2
!----------------------------------------------------------------------- 
!BOP
! !ROUTINE:  map1_cubic --- Cubic Interpolation for vertical re-mapping
!
! !INTERFACE:
  SUBROUTINE MAP1_CUBIC_BWD(km, pe1, pe1_ad, kn, pe2, pe2_ad, q2, q2_ad&
&   , i1, i2, j, ibeg, iend, jbeg, jend, akap, t_var, conserv)
    IMPLICIT NONE
!EOC
    INTEGER, INTENT(IN) :: i1
    INTEGER, INTENT(IN) :: i2
    REAL(fvprc), INTENT(IN) :: akap
    INTEGER, INTENT(IN) :: t_var
    LOGICAL, INTENT(IN) :: conserv
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
    INTEGER, INTENT(IN) :: km
    INTEGER, INTENT(IN) :: kn
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_ad(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc) :: qx(i1:i2, km)
    REAL(fvprc) :: qx_ad(i1:i2, km)
    REAL(fvprc) :: logpl1(i1:i2, km)
    REAL(fvprc) :: logpl1_ad(i1:i2, km)
    REAL(fvprc) :: logpl2(i1:i2, kn)
    REAL(fvprc) :: logpl2_ad(i1:i2, kn)
    REAL(fvprc) :: dlogp1(i1:i2, km)
    REAL(fvprc) :: dlogp1_ad(i1:i2, km)
    REAL(fvprc) :: vsum1(i1:i2)
    REAL(fvprc) :: vsum1_ad(i1:i2)
    REAL(fvprc) :: vsum2(i1:i2)
    REAL(fvprc) :: vsum2_ad(i1:i2)
    REAL(fvprc) :: am2, am1, ap0, ap1, p, plp1, plp0, plm1, plm2, dlp0, &
&   dlm1, dlm2
    REAL(fvprc) :: am2_ad, am1_ad, ap0_ad, ap1_ad, p_ad, plp1_ad, &
&   plp0_ad, plm1_ad, plm2_ad, dlp0_ad, dlm1_ad, dlm2_ad
    INTEGER :: i, k, lm2, lm1, lp0, lp1
    INTRINSIC LOG
    INTRINSIC EXP
    INTRINSIC MAX
    INTRINSIC MIN
    REAL(fvprc), DIMENSION(i2-i1+1) :: temp_ad
    REAL(fvprc), DIMENSION(i2-i1+1) :: temp_ad0
    REAL(fvprc), DIMENSION(i2-i1+1) :: temp_ad1
    REAL(fvprc), DIMENSION(i2-i1+1) :: temp_ad2
    REAL, DIMENSION(i2-i1+1) :: temp
    REAL(fvprc), DIMENSION(i2-i1+1) :: temp_ad3
    REAL, DIMENSION(i2-i1+1) :: temp0
    REAL(fvprc), DIMENSION(i2-i1+1) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp2
    REAL(fvprc) :: temp3
    REAL(fvprc) :: temp4
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp5
    REAL(fvprc) :: temp6
    REAL(fvprc) :: temp7
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp8
    REAL(fvprc) :: temp9
    REAL(fvprc) :: temp10
    REAL(fvprc) :: temp_ad11
    REAL(fvprc) :: temp_ad12
    REAL(fvprc) :: temp11
    REAL(fvprc) :: temp12
    REAL(fvprc) :: temp13
    REAL(fvprc) :: temp14
    REAL(fvprc) :: temp15
    REAL(fvprc) :: temp16
    REAL(fvprc) :: temp17
    REAL(fvprc) :: temp18
    REAL(fvprc) :: temp19
    REAL(fvprc) :: temp20
    REAL(fvprc) :: temp_ad13
    REAL(fvprc) :: temp_ad14
    REAL(fvprc) :: temp_ad15
    REAL(fvprc) :: temp_ad16
    REAL(fvprc) :: temp_ad17
    REAL(fvprc) :: temp_ad18
    REAL(fvprc) :: temp_ad19
    REAL(fvprc) :: temp_ad20
    REAL(fvprc) :: temp_ad21
    REAL(fvprc) :: temp_ad22
    REAL(fvprc) :: temp_ad23
    REAL(fvprc) :: temp_ad24
    REAL(fvprc) :: temp_ad25
    REAL(fvprc) :: temp_ad26
    REAL(fvprc) :: temp_ad27
    REAL(fvprc) :: temp_ad28
    REAL(fvprc) :: temp_ad29
    REAL(fvprc) :: temp21
    REAL(fvprc) :: temp_ad30
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: branch
    CALL POPCONTROL1B_FV(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREALARRAY(logpl2, 8*(i2-i1+1)*kn/8)
      CALL POPREALARRAY(am2)
      CALL POPREALARRAY(logpl1, 8*(i2-i1+1)*km/8)
      CALL POPREALARRAY(am1)
      CALL POPREALARRAY(ap1)
      CALL POPREALARRAY(ap0)
      CALL POPREALARRAY(qx, 8*(i2-i1+1)*km/8)
      CALL POPREALARRAY(dlm2)
      CALL POPINTEGER(lm2)
      CALL POPREALARRAY(dlm1)
      CALL POPINTEGER(lm1)
      CALL POPREALARRAY(p)
      CALL POPINTEGER(lp1)
      CALL POPREALARRAY(dlp0)
      CALL POPINTEGER(lp0)
      vsum1_ad = 0.0_FVPRC
      vsum2_ad = 0.0_FVPRC
      DO i=i2,i1,-1
        DO k=kn,1,-1
          CALL POPREALARRAY(q2(i, j, k))
          vsum1_ad(i) = vsum1_ad(i) + q2_ad(i, j, k)
          vsum2_ad(i) = vsum2_ad(i) - q2_ad(i, j, k)
        END DO
      END DO
      DO i=i2,i1,-1
        CALL POPREALARRAY(vsum2(i))
        temp21 = pe2(i, kn+1) - pe2(i, 1)
        temp_ad30 = -(vsum2(i)*vsum2_ad(i)/temp21**2)
        pe2_ad(i, kn+1) = pe2_ad(i, kn+1) + temp_ad30
        pe2_ad(i, 1) = pe2_ad(i, 1) - temp_ad30
        vsum2_ad(i) = vsum2_ad(i)/temp21
        DO k=kn,1,-1
          temp_ad29 = q2(i, j, k)*vsum2_ad(i)
          q2_ad(i, j, k) = q2_ad(i, j, k) + (pe2(i, k+1)-pe2(i, k))*&
&           vsum2_ad(i)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad29
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad29
        END DO
      END DO
    ELSE
      CALL POPREALARRAY(logpl2, 8*(i2-i1+1)*kn/8)
      CALL POPREALARRAY(am2)
      CALL POPREALARRAY(logpl1, 8*(i2-i1+1)*km/8)
      CALL POPREALARRAY(am1)
      CALL POPREALARRAY(ap1)
      CALL POPREALARRAY(ap0)
      CALL POPREALARRAY(qx, 8*(i2-i1+1)*km/8)
      CALL POPREALARRAY(dlm2)
      CALL POPINTEGER(lm2)
      CALL POPREALARRAY(dlm1)
      CALL POPINTEGER(lm1)
      CALL POPREALARRAY(p)
      CALL POPINTEGER(lp1)
      CALL POPREALARRAY(dlp0)
      CALL POPINTEGER(lp0)
      vsum1_ad = 0.0_FVPRC
    END IF
    qx_ad = 0.0_FVPRC
    logpl1_ad = 0.0_FVPRC
    logpl2_ad = 0.0_FVPRC
    dlogp1_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL2B_FV(branch)
        IF (branch .LT. 2) THEN
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY(q2(i, j, k))
            ap1_ad = qx(i, lp1)*q2_ad(i, j, k)
            qx_ad(i, lp1) = qx_ad(i, lp1) + ap1*q2_ad(i, j, k)
            ap0_ad = qx(i, lp0)*q2_ad(i, j, k)
            qx_ad(i, lp0) = qx_ad(i, lp0) + ap0*q2_ad(i, j, k)
            am1_ad = qx(i, lm1)*q2_ad(i, j, k)
            qx_ad(i, lm1) = qx_ad(i, lm1) + am1*q2_ad(i, j, k)
            am2_ad = qx(i, lm2)*q2_ad(i, j, k)
            qx_ad(i, lm2) = qx_ad(i, lm2) + am2*q2_ad(i, j, k)
            q2_ad(i, j, k) = 0.0_FVPRC
            plp0 = logpl1(i, lp0)
            plp1 = logpl1(i, lp1)
            plm1 = logpl1(i, lm1)
            CALL POPREALARRAY(am2)
            temp20 = dlm2*(dlm1+dlm2)
            temp19 = temp20*(dlp0+dlm1+dlm2)
            temp_ad13 = am2_ad/temp19
            temp_ad14 = (plm1-p)*temp_ad13
            temp18 = (plp1-p)*(plp0-p)
            temp_ad15 = -(temp18*(plm1-p)*temp_ad13/temp19)
            temp_ad16 = (dlp0+dlm1+dlm2)*temp_ad15
            temp_ad17 = temp20*temp_ad15
            plm2 = logpl1(i, lm2)
            CALL POPREALARRAY(am1)
            temp17 = dlm1*dlm2
            temp_ad20 = am1_ad/(temp17*(dlp0+dlm1))
            temp_ad18 = (p-plm2)*temp_ad20
            temp16 = (plp1-p)*(plp0-p)
            temp_ad24 = -(temp16*(p-plm2)*temp_ad20/(temp17*(dlp0+dlm1))&
&             )
            CALL POPREALARRAY(ap0)
            temp15 = dlp0*dlm1
            temp_ad23 = ap0_ad/(temp15*(dlm1+dlm2))
            temp_ad19 = (p-plm2)*temp_ad23
            plp1_ad = (plp0-p)*temp_ad18 + (p-plm1)*temp_ad19 + (plp0-p)&
&             *temp_ad14
            temp14 = (plp1-p)*(p-plm1)
            temp_ad26 = -(temp14*(p-plm2)*temp_ad23/(temp15*(dlm1+dlm2))&
&             )
            CALL POPREALARRAY(ap1)
            temp13 = dlp0*(dlp0+dlm1)
            temp12 = temp13*(dlp0+dlm1+dlm2)
            temp_ad22 = ap1_ad/temp12
            temp_ad21 = (p-plm2)*temp_ad22
            plp0_ad = (plp1-p)*temp_ad18 - (p-plm1)*temp_ad21 + (plp1-p)&
&             *temp_ad14
            plm1_ad = temp18*temp_ad13 - (p-plp0)*temp_ad21 - (plp1-p)*&
&             temp_ad19
            temp11 = (p-plp0)*(p-plm1)
            p_ad = (2*p-plp1-plp0)*temp_ad18 + temp16*temp_ad20 + (2*p-&
&             plp0-plm1)*temp_ad21 + temp11*temp_ad22 + temp14*temp_ad23&
&             + (plp1-2*p+plm1)*temp_ad19 - temp18*temp_ad13 + (2*p-plp1&
&             -plp0)*temp_ad14
            plm2_ad = -(temp14*temp_ad23) - temp11*temp_ad22 - temp16*&
&             temp_ad20
            temp_ad28 = -(temp11*(p-plm2)*temp_ad22/temp12)
            temp_ad27 = (dlp0+dlm1+dlm2)*temp_ad28
            temp_ad25 = temp13*temp_ad28
            dlm2_ad = (dlp0+dlm1)*dlm1*temp_ad24 + temp_ad25 + temp15*&
&             temp_ad26 + temp_ad17 + (2*dlm2+dlm1)*temp_ad16
            dlm1_ad = (temp17+(dlp0+dlm1)*dlm2)*temp_ad24 + dlp0*&
&             temp_ad27 + temp_ad25 + (temp15+(dlm1+dlm2)*dlp0)*&
&             temp_ad26 + temp_ad17 + dlm2*temp_ad16
            dlp0_ad = temp17*temp_ad24 + (2*dlp0+dlm1)*temp_ad27 + &
&             temp_ad25 + (dlm1+dlm2)*dlm1*temp_ad26 + temp_ad17
            CALL POPREALARRAY(dlm2)
            dlogp1_ad(i, lm2) = dlogp1_ad(i, lm2) + dlm2_ad
            CALL POPREALARRAY(dlm1)
            dlogp1_ad(i, lm1) = dlogp1_ad(i, lm1) + dlm1_ad
            CALL POPREALARRAY(dlp0)
            dlogp1_ad(i, lp0) = dlogp1_ad(i, lp0) + dlp0_ad
            logpl1_ad(i, lm2) = logpl1_ad(i, lm2) + plm2_ad
            logpl1_ad(i, lm1) = logpl1_ad(i, lm1) + plm1_ad
            logpl1_ad(i, lp0) = logpl1_ad(i, lp0) + plp0_ad
            logpl1_ad(i, lp1) = logpl1_ad(i, lp1) + plp1_ad
            CALL POPREALARRAY(p)
            logpl2_ad(i, k) = logpl2_ad(i, k) + p_ad
            CALL POPINTEGER(lm2)
            CALL POPINTEGER(lp1)
          ELSE
            CALL POPREALARRAY(q2(i, j, k))
            temp8 = logpl1(i, lm1) - logpl1(i, lp0)
            temp_ad11 = q2_ad(i, j, k)/temp8
            temp10 = logpl2(i, k) - logpl1(i, lp0)
            temp9 = qx(i, lm1) - qx(i, lp0)
            temp_ad12 = -(temp9*temp10*temp_ad11/temp8)
            qx_ad(i, lp0) = qx_ad(i, lp0) + q2_ad(i, j, k) - temp10*&
&             temp_ad11
            qx_ad(i, lm1) = qx_ad(i, lm1) + temp10*temp_ad11
            logpl2_ad(i, k) = logpl2_ad(i, k) + temp9*temp_ad11
            logpl1_ad(i, lp0) = logpl1_ad(i, lp0) - temp_ad12 - temp9*&
&             temp_ad11
            logpl1_ad(i, lm1) = logpl1_ad(i, lm1) + temp_ad12
            q2_ad(i, j, k) = 0.0_FVPRC
          END IF
        ELSE IF (branch .EQ. 2) THEN
          CALL POPREALARRAY(q2(i, j, k))
          temp5 = logpl1(i, km) - logpl1(i, km-1)
          temp_ad9 = q2_ad(i, j, k)/temp5
          temp7 = logpl2(i, k) - logpl1(i, km)
          temp6 = qx(i, km) - qx(i, km-1)
          temp_ad10 = -(temp6*temp7*temp_ad9/temp5)
          qx_ad(i, km) = qx_ad(i, km) + temp7*temp_ad9 + q2_ad(i, j, k)
          qx_ad(i, km-1) = qx_ad(i, km-1) - temp7*temp_ad9
          logpl2_ad(i, k) = logpl2_ad(i, k) + temp6*temp_ad9
          logpl1_ad(i, km) = logpl1_ad(i, km) + temp_ad10 - temp6*&
&           temp_ad9
          logpl1_ad(i, km-1) = logpl1_ad(i, km-1) - temp_ad10
          q2_ad(i, j, k) = 0.0_FVPRC
        ELSE
          CALL POPREALARRAY(q2(i, j, k))
          temp3 = logpl1(i, 2) - logpl1(i, 1)
          temp4 = qx(i, 2) - qx(i, 1)
          temp2 = temp4/temp3
          temp_ad7 = (logpl2(i, k)-logpl1(i, 1))*q2_ad(i, j, k)/temp3
          temp_ad8 = -(temp2*temp_ad7)
          qx_ad(i, 1) = qx_ad(i, 1) + q2_ad(i, j, k) - temp_ad7
          logpl2_ad(i, k) = logpl2_ad(i, k) + temp2*q2_ad(i, j, k)
          logpl1_ad(i, 1) = logpl1_ad(i, 1) - temp_ad8 - temp2*q2_ad(i, &
&           j, k)
          qx_ad(i, 2) = qx_ad(i, 2) + temp_ad7
          logpl1_ad(i, 2) = logpl1_ad(i, 2) + temp_ad8
          q2_ad(i, j, k) = 0.0_FVPRC
        END IF
        CALL POPCONTROL1B_FV(branch)
        IF (branch .EQ. 0) THEN
          CALL POPINTEGER(lm1)
        ELSE
          CALL POPINTEGER(lm1)
        END IF
        CALL POPINTEGER(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B_FV(branch)
        END DO
        CALL POPINTEGER(lp0)
      END DO
    END DO
    CALL POPCONTROL1B_FV(branch)
    IF (branch .NE. 0) THEN
      DO i=i2,i1,-1
        CALL POPREALARRAY(vsum1(i))
        temp1 = pe1(i, km+1) - pe1(i, 1)
        temp_ad6 = -(vsum1(i)*vsum1_ad(i)/temp1**2)
        pe1_ad(i, km+1) = pe1_ad(i, km+1) + temp_ad6
        pe1_ad(i, 1) = pe1_ad(i, 1) - temp_ad6
        vsum1_ad(i) = vsum1_ad(i)/temp1
        DO k=km,1,-1
          temp_ad5 = qx(i, k)*vsum1_ad(i)
          qx_ad(i, k) = qx_ad(i, k) + (pe1(i, k+1)-pe1(i, k))*vsum1_ad(i&
&           )
          pe1_ad(i, k+1) = pe1_ad(i, k+1) + temp_ad5
          pe1_ad(i, k) = pe1_ad(i, k) - temp_ad5
        END DO
      END DO
    END IF
    CALL POPCONTROL2B_FV(branch)
    IF (branch .LT. 2) THEN
      IF (branch .NE. 0) THEN
        DO k=km-1,1,-1
          logpl1_ad(:, k+1) = logpl1_ad(:, k+1) + dlogp1_ad(:, k)
          logpl1_ad(:, k) = logpl1_ad(:, k) - dlogp1_ad(:, k)
          dlogp1_ad(:, k) = 0.0_FVPRC
        END DO
        DO k=kn,1,-1
          temp_ad0 = logpl2_ad(:, k)/(pe2(:, k)+pe2(:, k+1))
          pe2_ad(:, k) = pe2_ad(:, k) + temp_ad0
          pe2_ad(:, k+1) = pe2_ad(:, k+1) + temp_ad0
          logpl2_ad(:, k) = 0.0_FVPRC
        END DO
        DO k=km,1,-1
          temp_ad = logpl1_ad(:, k)/(pe1(:, k)+pe1(:, k+1))
          pe1_ad(:, k) = pe1_ad(:, k) + temp_ad
          pe1_ad(:, k+1) = pe1_ad(:, k+1) + temp_ad
          logpl1_ad(:, k) = 0.0_FVPRC
          q2_ad(i1:i2, j, k) = q2_ad(i1:i2, j, k) + qx_ad(:, k)
          qx_ad(:, k) = 0.0_FVPRC
        END DO
      END IF
    ELSE IF (branch .EQ. 2) THEN
      DO k=km-1,1,-1
        logpl1_ad(:, k+1) = logpl1_ad(:, k+1) + dlogp1_ad(:, k)
        logpl1_ad(:, k) = logpl1_ad(:, k) - dlogp1_ad(:, k)
        dlogp1_ad(:, k) = 0.0_FVPRC
      END DO
      DO k=kn,1,-1
        temp_ad2 = logpl2_ad(:, k)/(pe2(:, k)+pe2(:, k+1))
        pe2_ad(:, k) = pe2_ad(:, k) + temp_ad2
        pe2_ad(:, k+1) = pe2_ad(:, k+1) + temp_ad2
        logpl2_ad(:, k) = 0.0_FVPRC
      END DO
      DO k=km,1,-1
        temp_ad1 = logpl1_ad(:, k)/(pe1(:, k)+pe1(:, k+1))
        pe1_ad(:, k) = pe1_ad(:, k) + temp_ad1
        pe1_ad(:, k+1) = pe1_ad(:, k+1) + temp_ad1
        logpl1_ad(:, k) = 0.0_FVPRC
        q2_ad(i1:i2, j, k) = q2_ad(i1:i2, j, k) + qx_ad(:, k)
        qx_ad(:, k) = 0.0_FVPRC
      END DO
    ELSE
      DO k=km-1,1,-1
        logpl1_ad(:, k+1) = logpl1_ad(:, k+1) + dlogp1_ad(:, k)
        logpl1_ad(:, k) = logpl1_ad(:, k) - dlogp1_ad(:, k)
        dlogp1_ad(:, k) = 0.0_FVPRC
      END DO
      DO k=kn,1,-1
        temp0 = r2*(pe2(:, k)+pe2(:, k+1))
        temp_ad4 = akap*EXP(akap*LOG(temp0))*r2*logpl2_ad(:, k)/temp0
        pe2_ad(:, k) = pe2_ad(:, k) + temp_ad4
        pe2_ad(:, k+1) = pe2_ad(:, k+1) + temp_ad4
        logpl2_ad(:, k) = 0.0_FVPRC
      END DO
      DO k=km,1,-1
        temp = r2*(pe1(:, k)+pe1(:, k+1))
        temp_ad3 = akap*EXP(akap*LOG(temp))*r2*logpl1_ad(:, k)/temp
        pe1_ad(:, k) = pe1_ad(:, k) + temp_ad3
        pe1_ad(:, k+1) = pe1_ad(:, k+1) + temp_ad3
        logpl1_ad(:, k) = 0.0_FVPRC
        q2_ad(i1:i2, j, k) = q2_ad(i1:i2, j, k) + qx_ad(:, k)
        qx_ad(:, k) = 0.0_FVPRC
      END DO
    END IF
  END SUBROUTINE MAP1_CUBIC_BWD
!----------------------------------------------------------------------- 
!BOP
! !ROUTINE:  map1_cubic --- Cubic Interpolation for vertical re-mapping
!
! !INTERFACE:
  SUBROUTINE MAP1_CUBIC(km, pe1, kn, pe2, q2, i1, i2, j, ibeg, iend, &
&   jbeg, jend, akap, t_var, conserv)
    IMPLICIT NONE
!EOC
! !INPUT PARAMETERS:
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
    REAL(fvprc), INTENT(IN) :: akap
! Thermodynamic variable to remap
    INTEGER, INTENT(IN) :: t_var
!     1:TE  2:T  3:PT 
    LOGICAL, INTENT(IN) :: conserv
! Current latitude
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
!      real(FVPRC), intent(in) ::    q1(ibeg:iend,jbeg:jend,km) ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
! !DESCRIPTION:
!
!     Perform Cubic Interpolation a given latitude
! pe1: pressure at layer edges (from model top to bottom surface)
!      in the original vertical coordinate
! pe2: pressure at layer edges (from model top to bottom surface)
!      in the new vertical coordinate
!
! !REVISION HISTORY:
!    2005.11.14   Takacs    Initial Code
!    2016.07.20   Putman    Modified to make genaric for any thermodynamic variable
!
!EOP
!-----------------------------------------------------------------------
!BOC
!
! !LOCAL VARIABLES:
    REAL(fvprc) :: qx(i1:i2, km)
    REAL(fvprc) :: logpl1(i1:i2, km)
    REAL(fvprc) :: logpl2(i1:i2, kn)
    REAL(fvprc) :: dlogp1(i1:i2, km)
    REAL(fvprc) :: vsum1(i1:i2)
    REAL(fvprc) :: vsum2(i1:i2)
    REAL(fvprc) :: am2, am1, ap0, ap1, p, plp1, plp0, plm1, plm2, dlp0, &
&   dlm1, dlm2
    INTEGER :: i, k, lm2, lm1, lp0, lp1
    INTRINSIC LOG
    INTRINSIC EXP
    INTRINSIC MAX
    INTRINSIC MIN
! Initialization
! --------------
    SELECT CASE  (t_var) 
    CASE (1) 
! Total Energy Remapping in Log(P)
      DO k=1,km
        qx(:, k) = q2(i1:i2, j, k)
        logpl1(:, k) = LOG(r2*(pe1(:, k)+pe1(:, k+1)))
      END DO
      DO k=1,kn
        logpl2(:, k) = LOG(r2*(pe2(:, k)+pe2(:, k+1)))
      END DO
      DO k=1,km-1
        dlogp1(:, k) = logpl1(:, k+1) - logpl1(:, k)
      END DO
    CASE (2) 
! Temperature Remapping in Log(P)
      DO k=1,km
        qx(:, k) = q2(i1:i2, j, k)
        logpl1(:, k) = LOG(r2*(pe1(:, k)+pe1(:, k+1)))
      END DO
      DO k=1,kn
        logpl2(:, k) = LOG(r2*(pe2(:, k)+pe2(:, k+1)))
      END DO
      DO k=1,km-1
        dlogp1(:, k) = logpl1(:, k+1) - logpl1(:, k)
      END DO
    CASE (3) 
! Potential Temperature Remapping in P^KAPPA
      DO k=1,km
        qx(:, k) = q2(i1:i2, j, k)
        logpl1(:, k) = EXP(akap*LOG(r2*(pe1(:, k)+pe1(:, k+1))))
      END DO
      DO k=1,kn
        logpl2(:, k) = EXP(akap*LOG(r2*(pe2(:, k)+pe2(:, k+1))))
      END DO
      DO k=1,km-1
        dlogp1(:, k) = logpl1(:, k+1) - logpl1(:, k)
      END DO
    END SELECT
    IF (conserv) THEN
! Compute vertical integral of Input TE
! -------------------------------------
      vsum1(:) = r0
      DO i=i1,i2
        DO k=1,km
          vsum1(i) = vsum1(i) + qx(i, k)*(pe1(i, k+1)-pe1(i, k))
        END DO
        vsum1(i) = vsum1(i)/(pe1(i, km+1)-pe1(i, 1))
      END DO
    END IF
! Interpolate TE onto target Pressures
! ------------------------------------
    DO i=i1,i2
      DO k=1,kn
        lm1 = 1
        lp0 = 1
        DO WHILE (lp0 .LE. km)
          IF (logpl1(i, lp0) .LT. logpl2(i, k)) THEN
            lp0 = lp0 + 1
          ELSE
            GOTO 100
          END IF
        END DO
 100    IF (lp0 - 1 .LT. 1) THEN
          lm1 = 1
        ELSE
          lm1 = lp0 - 1
        END IF
        IF (lp0 .GT. km) THEN
          lp0 = km
        ELSE
          lp0 = lp0
        END IF
! Extrapolate Linearly in LogP above first model level
! ----------------------------------------------------
        IF (lm1 .EQ. 1 .AND. lp0 .EQ. 1) THEN
          q2(i, j, k) = qx(i, 1) + (qx(i, 2)-qx(i, 1))*(logpl2(i, k)-&
&           logpl1(i, 1))/(logpl1(i, 2)-logpl1(i, 1))
! Extrapolate Linearly in LogP below last model level
! ---------------------------------------------------
        ELSE IF (lm1 .EQ. km .AND. lp0 .EQ. km) THEN
          q2(i, j, k) = qx(i, km) + (qx(i, km)-qx(i, km-1))*(logpl2(i, k&
&           )-logpl1(i, km))/(logpl1(i, km)-logpl1(i, km-1))
! Interpolate Linearly in LogP between levels 1 => 2 and km-1 => km
! -----------------------------------------------------------------
        ELSE IF (lm1 .EQ. 1 .OR. lp0 .EQ. km) THEN
          q2(i, j, k) = qx(i, lp0) + (qx(i, lm1)-qx(i, lp0))*(logpl2(i, &
&           k)-logpl1(i, lp0))/(logpl1(i, lm1)-logpl1(i, lp0))
! Interpolate Cubicly in LogP between other model levels
! ------------------------------------------------------
        ELSE
          lp1 = lp0 + 1
          lm2 = lm1 - 1
          p = logpl2(i, k)
          plp1 = logpl1(i, lp1)
          plp0 = logpl1(i, lp0)
          plm1 = logpl1(i, lm1)
          plm2 = logpl1(i, lm2)
          dlp0 = dlogp1(i, lp0)
          dlm1 = dlogp1(i, lm1)
          dlm2 = dlogp1(i, lm2)
          ap1 = (p-plp0)*(p-plm1)*(p-plm2)/(dlp0*(dlp0+dlm1)*(dlp0+dlm1+&
&           dlm2))
          ap0 = (plp1-p)*(p-plm1)*(p-plm2)/(dlp0*dlm1*(dlm1+dlm2))
          am1 = (plp1-p)*(plp0-p)*(p-plm2)/(dlm1*dlm2*(dlp0+dlm1))
          am2 = (plp1-p)*(plp0-p)*(plm1-p)/(dlm2*(dlm1+dlm2)*(dlp0+dlm1+&
&           dlm2))
          q2(i, j, k) = ap1*qx(i, lp1) + ap0*qx(i, lp0) + am1*qx(i, lm1)&
&           + am2*qx(i, lm2)
        END IF
      END DO
    END DO
    IF (conserv) THEN
! Compute vertical integral of Output TE
! --------------------------------------
      vsum2(:) = r0
      DO i=i1,i2
        DO k=1,kn
          vsum2(i) = vsum2(i) + q2(i, j, k)*(pe2(i, k+1)-pe2(i, k))
        END DO
        vsum2(i) = vsum2(i)/(pe2(i, kn+1)-pe2(i, 1))
      END DO
! Adjust Final TE to conserve
! ---------------------------
      DO i=i1,i2
        DO k=1,kn
          q2(i, j, k) = q2(i, j, k) + vsum1(i) - vsum2(i)
        END DO
      END DO
    END IF
!          q2(i,j,k) = q2(i,j,k) * vsum1(i)/vsum2(i)
    RETURN
  END SUBROUTINE MAP1_CUBIC
!  Differentiation of scalar_profile_linear in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a
!2b_ord4_fb a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo
! dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod
!.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayle
!igh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_l
!atlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy 
!fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod
!.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_t
!racer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c
! nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_m
!od.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_co
!rner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.
!compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.
!copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod
!.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: delp a4
!   with respect to varying inputs: delp a4
  SUBROUTINE SCALAR_PROFILE_LINEAR_ADM(qs, a4, a4_ad, delp, delp_ad, km&
&   , i1, i2, iv, qmin)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
    INTEGER, INTENT(IN) :: i1, i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
    REAL(fvprc) :: delp_ad(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4_ad(4, i1:i2, km)
    REAL(fvprc), INTENT(IN) :: qmin
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: gam_ad(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: q_ad(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: d4_ad(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: bet_ad, a_bot_ad, grat_ad
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    INTEGER :: i, k, im
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp2
    REAL(fvprc) :: temp_ad11
    REAL(fvprc) :: temp_ad12
    REAL(fvprc) :: temp_ad13
    REAL(fvprc) :: temp_ad14
    INTEGER :: branch
    IF (iv .EQ. -2) THEN
      DO i=i1,i2
        gam(i, 2) = 0.5
        q(i, 1) = 1.5*a4(1, i, 1)
      END DO
      DO k=2,km-1
        DO i=i1,i2
          grat = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY_ADM(bet)
          bet = 2. + grat + grat - gam(i, k)
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))/bet
          gam(i, k+1) = grat/bet
        END DO
      END DO
      DO i=i1,i2
        grat = delp(i, km-1)/delp(i, km)
        CALL PUSHREALARRAY_ADM(q(i, km))
        q(i, km) = (3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, km-&
&         1))/(2.+grat+grat-gam(i, km))
        CALL PUSHREALARRAY_ADM(q(i, km+1))
        q(i, km+1) = qs(i)
      END DO
      DO k=km-1,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = q(i, k) - gam(i, k+1)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B(1)
    ELSE
      DO i=i1,i2
! grid ratio
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        q(i, 1) = ((grat+grat)*(grat+1.)*a4(1, i, 1)+a4(1, i, 2))/bet
        gam(i, 1) = (1.+grat*(grat+1.5))/bet
      END DO
      DO k=2,km
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(d4(i))
          d4(i) = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY_ADM(bet)
          bet = 2. + d4(i) + d4(i) - gam(i, k-1)
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))/bet
          gam(i, k) = d4(i)/bet
        END DO
      END DO
      DO i=i1,i2
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL PUSHREALARRAY_ADM(q(i, km+1))
        q(i, km+1) = (2.*d4(i)*(d4(i)+1.)*a4(1, i, km)+a4(1, i, km-1)-&
&         a_bot*q(i, km))/(d4(i)*(d4(i)+0.5)-a_bot*gam(i, km))
      END DO
      DO k=km,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = q(i, k) - gam(i, k)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B(0)
    END IF
    q_ad = 0.0_FVPRC
    DO k=km,1,-1
      DO i=i2,i1,-1
        temp_ad14 = 3.*a4_ad(4, i, k)
        a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad14
        a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad14
        a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad14
        a4_ad(4, i, k) = 0.0_FVPRC
        q_ad(i, k+1) = q_ad(i, k+1) + a4_ad(3, i, k)
        a4_ad(3, i, k) = 0.0_FVPRC
        q_ad(i, k) = q_ad(i, k) + a4_ad(2, i, k)
        a4_ad(2, i, k) = 0.0_FVPRC
      END DO
    END DO
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      gam_ad = 0.0_FVPRC
      DO k=1,km,1
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(q(i, k))
          gam_ad(i, k) = gam_ad(i, k) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k)*q_ad(i, k)
        END DO
      END DO
      d4_ad = 0.0_FVPRC
      DO i=i2,i1,-1
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL POPREALARRAY_ADM(q(i, km+1))
        temp2 = d4(i)*(d4(i)+0.5) - a_bot*gam(i, km)
        temp_ad11 = q_ad(i, km+1)/temp2
        temp1 = d4(i)*(d4(i)+1.)
        temp_ad12 = 2.*a4(1, i, km)*temp_ad11
        temp_ad13 = -((2.*(temp1*a4(1, i, km))+a4(1, i, km-1)-a_bot*q(i&
&         , km))*temp_ad11/temp2)
        a4_ad(1, i, km) = a4_ad(1, i, km) + 2.*temp1*temp_ad11
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + temp_ad11
        a_bot_ad = -(gam(i, km)*temp_ad13) - q(i, km)*temp_ad11
        d4_ad(i) = d4_ad(i) + (2*d4(i)+1.5)*a_bot_ad + (2*d4(i)+0.5)*&
&         temp_ad13 + (2*d4(i)+1.)*temp_ad12
        q_ad(i, km) = q_ad(i, km) - a_bot*temp_ad11
        gam_ad(i, km) = gam_ad(i, km) - a_bot*temp_ad13
        q_ad(i, km+1) = 0.0_FVPRC
      END DO
      DO k=km,2,-1
        DO i=i2,i1,-1
          temp_ad9 = q_ad(i, k)/bet
          temp_ad8 = 3.*temp_ad9
          CALL POPREALARRAY_ADM(q(i, k))
          bet_ad = -((3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))*&
&           temp_ad9/bet) - d4(i)*gam_ad(i, k)/bet**2
          d4_ad(i) = d4_ad(i) + a4(1, i, k)*temp_ad8 + 2*bet_ad + gam_ad&
&           (i, k)/bet
          gam_ad(i, k) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + temp_ad8
          a4_ad(1, i, k) = a4_ad(1, i, k) + d4(i)*temp_ad8
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad9
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY_ADM(bet)
          gam_ad(i, k-1) = gam_ad(i, k-1) - bet_ad
          CALL POPREALARRAY_ADM(d4(i))
          temp_ad10 = d4_ad(i)/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad10
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad10/delp(i&
&           , k)
          d4_ad(i) = 0.0_FVPRC
        END DO
      END DO
      DO i=i2,i1,-1
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        temp_ad4 = gam_ad(i, 1)/bet
        gam_ad(i, 1) = 0.0_FVPRC
        temp_ad6 = q_ad(i, 1)/bet
        temp_ad5 = a4(1, i, 1)*temp_ad6
        temp0 = 2*grat*(grat+1.)
        bet_ad = -((temp0*a4(1, i, 1)+a4(1, i, 2))*temp_ad6/bet) - (grat&
&         *(grat+1.5)+1.)*temp_ad4/bet
        grat_ad = (4*grat+2*1.)*temp_ad5 + (2*grat+0.5)*bet_ad + (2*grat&
&         +1.5)*temp_ad4
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + temp0*temp_ad6
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + temp_ad6
        q_ad(i, 1) = 0.0_FVPRC
        temp_ad7 = grat_ad/delp(i, 1)
        delp_ad(i, 2) = delp_ad(i, 2) + temp_ad7
        delp_ad(i, 1) = delp_ad(i, 1) - delp(i, 2)*temp_ad7/delp(i, 1)
      END DO
    ELSE
      gam_ad = 0.0_FVPRC
      DO k=1,km-1,1
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(q(i, k))
          gam_ad(i, k+1) = gam_ad(i, k+1) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k+1)*q_ad(i, k)
        END DO
      END DO
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(q(i, km+1))
        q_ad(i, km+1) = 0.0_FVPRC
        grat = delp(i, km-1)/delp(i, km)
        CALL POPREALARRAY_ADM(q(i, km))
        temp = 2*grat - gam(i, km) + 2.
        temp_ad1 = q_ad(i, km)/temp
        temp_ad2 = -((3.*(a4(1, i, km-1)+a4(1, i, km))-qs(i)*grat-q(i, &
&         km-1))*temp_ad1/temp)
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + 3.*temp_ad1
        a4_ad(1, i, km) = a4_ad(1, i, km) + 3.*temp_ad1
        grat_ad = 2*temp_ad2 - qs(i)*temp_ad1
        q_ad(i, km-1) = q_ad(i, km-1) - temp_ad1
        gam_ad(i, km) = gam_ad(i, km) - temp_ad2
        q_ad(i, km) = 0.0_FVPRC
        temp_ad3 = grat_ad/delp(i, km)
        delp_ad(i, km-1) = delp_ad(i, km-1) + temp_ad3
        delp_ad(i, km) = delp_ad(i, km) - delp(i, km-1)*temp_ad3/delp(i&
&         , km)
      END DO
      DO k=km-1,2,-1
        DO i=i2,i1,-1
          temp_ad = q_ad(i, k)/bet
          CALL POPREALARRAY_ADM(q(i, k))
          grat = delp(i, k-1)/delp(i, k)
          bet_ad = -((3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))*temp_ad/&
&           bet) - grat*gam_ad(i, k+1)/bet**2
          grat_ad = 2*bet_ad + gam_ad(i, k+1)/bet
          gam_ad(i, k+1) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + 3.*temp_ad
          a4_ad(1, i, k) = a4_ad(1, i, k) + 3.*temp_ad
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY_ADM(bet)
          gam_ad(i, k) = gam_ad(i, k) - bet_ad
          temp_ad0 = grat_ad/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad0
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad0/delp(i, &
&           k)
        END DO
      END DO
      DO i=i2,i1,-1
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + 1.5*q_ad(i, 1)
        q_ad(i, 1) = 0.0_FVPRC
      END DO
    END IF
  END SUBROUTINE SCALAR_PROFILE_LINEAR_ADM
  SUBROUTINE SCALAR_PROFILE_LINEAR(qs, a4, delp, km, i1, i2, iv, qmin)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
    INTEGER, INTENT(IN) :: i1, i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(IN) :: qmin
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    INTEGER :: i, k, im
    IF (iv .EQ. -2) THEN
      DO i=i1,i2
        gam(i, 2) = 0.5
        q(i, 1) = 1.5*a4(1, i, 1)
      END DO
      DO k=2,km-1
        DO i=i1,i2
          grat = delp(i, k-1)/delp(i, k)
          bet = 2. + grat + grat - gam(i, k)
          q(i, k) = (3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))/bet
          gam(i, k+1) = grat/bet
        END DO
      END DO
      DO i=i1,i2
        grat = delp(i, km-1)/delp(i, km)
        q(i, km) = (3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, km-&
&         1))/(2.+grat+grat-gam(i, km))
        q(i, km+1) = qs(i)
      END DO
      DO k=km-1,1,-1
        DO i=i1,i2
          q(i, k) = q(i, k) - gam(i, k+1)*q(i, k+1)
        END DO
      END DO
    ELSE
      DO i=i1,i2
! grid ratio
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        q(i, 1) = ((grat+grat)*(grat+1.)*a4(1, i, 1)+a4(1, i, 2))/bet
        gam(i, 1) = (1.+grat*(grat+1.5))/bet
      END DO
      DO k=2,km
        DO i=i1,i2
          d4(i) = delp(i, k-1)/delp(i, k)
          bet = 2. + d4(i) + d4(i) - gam(i, k-1)
          q(i, k) = (3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))/bet
          gam(i, k) = d4(i)/bet
        END DO
      END DO
      DO i=i1,i2
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        q(i, km+1) = (2.*d4(i)*(d4(i)+1.)*a4(1, i, km)+a4(1, i, km-1)-&
&         a_bot*q(i, km))/(d4(i)*(d4(i)+0.5)-a_bot*gam(i, km))
      END DO
      DO k=km,1,-1
        DO i=i1,i2
          q(i, k) = q(i, k) - gam(i, k)*q(i, k+1)
        END DO
      END DO
    END IF
    DO k=1,km
      DO i=i1,i2
        a4(2, i, k) = q(i, k)
        a4(3, i, k) = q(i, k+1)
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
    END DO
  END SUBROUTINE SCALAR_PROFILE_LINEAR
!  Differentiation of scalar_profile in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_ord4
!_fb a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_co
!re_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_
!p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_sup
!er fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlon f
!v_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_mapz
!_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q
!2_fb fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tracer2d
!_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_cor
!e_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.sim_
!solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner_ne
!st sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.compute
!_div_damping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy_co
!rners_fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyppm_
!fb tp_core_mod.deln_flux)):
!   gradient     of useful results: delp a4
!   with respect to varying inputs: delp a4
  SUBROUTINE SCALAR_PROFILE_ADM(qs, a4, a4_ad, delp, delp_ad, km, i1, i2&
&   , iv, kord, qmin)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
    INTEGER, INTENT(IN) :: i1, i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
    INTEGER, INTENT(IN) :: kord
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
    REAL(fvprc) :: delp_ad(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4_ad(4, i1:i2, km)
    REAL(fvprc), INTENT(IN) :: qmin
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: gam_ad(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: q_ad(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: d4_ad(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: bet_ad, a_bot_ad, grat_ad
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    REAL(fvprc) :: pmp_1_ad, lac_1_ad, pmp_2_ad, lac_2_ad
    INTEGER :: i, k, im
    INTRINSIC MAX
    INTRINSIC MIN
    INTRINSIC ABS
    INTEGER :: abs0
    INTEGER :: abs1
    REAL(fvprc) :: abs2
    INTEGER :: abs3
    REAL(fvprc) :: abs4
    INTEGER :: abs5
    REAL(fvprc) :: abs6
    INTEGER :: abs7
    REAL(fvprc) :: abs8
    REAL(fvprc) :: abs9
    REAL(fvprc) :: abs10
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp2
    REAL(fvprc) :: temp_ad11
    REAL(fvprc) :: temp_ad12
    REAL(fvprc) :: temp_ad13
    REAL(fvprc) :: y1_ad
    REAL(fvprc) :: y2_ad
    REAL(fvprc) :: y3_ad
    REAL(fvprc) :: y4_ad
    REAL(fvprc) :: y5_ad
    REAL(fvprc) :: y6_ad
    REAL(fvprc) :: y7_ad
    REAL(fvprc) :: y8_ad
    REAL(fvprc) :: temp_ad14
    REAL(fvprc) :: temp_ad15
    REAL(fvprc) :: y17_ad
    REAL(fvprc) :: x1_ad
    REAL(fvprc) :: y9_ad
    REAL(fvprc) :: y18_ad
    REAL(fvprc) :: x2_ad
    REAL(fvprc) :: y10_ad
    REAL(fvprc) :: temp_ad16
    REAL(fvprc) :: y19_ad
    REAL(fvprc) :: x3_ad
    REAL(fvprc) :: y11_ad
    REAL(fvprc) :: y20_ad
    REAL(fvprc) :: x4_ad
    REAL(fvprc) :: y12_ad
    REAL(fvprc) :: y21_ad
    REAL(fvprc) :: x5_ad
    REAL(fvprc) :: y13_ad
    REAL(fvprc) :: y22_ad
    REAL(fvprc) :: x6_ad
    REAL(fvprc) :: y14_ad
    REAL(fvprc) :: y23_ad
    REAL(fvprc) :: x7_ad
    REAL(fvprc) :: y15_ad
    REAL(fvprc) :: y24_ad
    REAL(fvprc) :: x8_ad
    REAL(fvprc) :: y16_ad
    REAL(fvprc) :: temp_ad17
    REAL(fvprc) :: temp_ad18
    REAL(fvprc) :: temp_ad19
    INTEGER :: branch
    REAL(fvprc) :: y24
    REAL(fvprc) :: y23
    REAL(fvprc) :: y22
    REAL(fvprc) :: y21
    REAL(fvprc) :: y20
    REAL(fvprc) :: x8
    REAL(fvprc) :: x7
    REAL(fvprc) :: x6
    REAL(fvprc) :: x5
    REAL(fvprc) :: x4
    REAL(fvprc) :: x3
    REAL(fvprc) :: x2
    REAL(fvprc) :: x1
    REAL(fvprc) :: y19
    REAL(fvprc) :: y18
    REAL(fvprc) :: y17
    REAL(fvprc) :: y16
    REAL(fvprc) :: y15
    REAL(fvprc) :: y14
    REAL(fvprc) :: y13
    REAL(fvprc) :: y12
    REAL(fvprc) :: y11
    REAL(fvprc) :: y10
    REAL(fvprc) :: y9
    REAL(fvprc) :: y8
    REAL(fvprc) :: y7
    REAL(fvprc) :: y6
    REAL(fvprc) :: y5
    REAL(fvprc) :: y4
    REAL(fvprc) :: y3
    REAL(fvprc) :: y2
    REAL(fvprc) :: y1
    IF (iv .EQ. -2) THEN
      DO i=i1,i2
        gam(i, 2) = 0.5
        q(i, 1) = 1.5*a4(1, i, 1)
      END DO
      DO k=2,km-1
        DO i=i1,i2
          grat = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY_ADM(bet)
          bet = 2. + grat + grat - gam(i, k)
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))/bet
          gam(i, k+1) = grat/bet
        END DO
      END DO
      DO i=i1,i2
        grat = delp(i, km-1)/delp(i, km)
        CALL PUSHREALARRAY_ADM(q(i, km))
        q(i, km) = (3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, km-&
&         1))/(2.+grat+grat-gam(i, km))
        CALL PUSHREALARRAY_ADM(q(i, km+1))
        q(i, km+1) = qs(i)
      END DO
      DO k=km-1,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = q(i, k) - gam(i, k+1)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B(0)
    ELSE
      DO i=i1,i2
! grid ratio
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        q(i, 1) = ((grat+grat)*(grat+1.)*a4(1, i, 1)+a4(1, i, 2))/bet
        gam(i, 1) = (1.+grat*(grat+1.5))/bet
      END DO
      DO k=2,km
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(d4(i))
          d4(i) = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY_ADM(bet)
          bet = 2. + d4(i) + d4(i) - gam(i, k-1)
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))/bet
          gam(i, k) = d4(i)/bet
        END DO
      END DO
      DO i=i1,i2
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL PUSHREALARRAY_ADM(q(i, km+1))
        q(i, km+1) = (2.*d4(i)*(d4(i)+1.)*a4(1, i, km)+a4(1, i, km-1)-&
&         a_bot*q(i, km))/(d4(i)*(d4(i)+0.5)-a_bot*gam(i, km))
      END DO
      DO k=km,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = q(i, k) - gam(i, k)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B(1)
    END IF
!------------------
! Apply constraints
!------------------
    im = i2 - i1 + 1
! Apply *large-scale* constraints 
    DO i=i1,i2
      IF (a4(1, i, 1) .LT. a4(1, i, 2)) THEN
        y1 = a4(1, i, 2)
        CALL PUSHCONTROL1B(0)
      ELSE
        y1 = a4(1, i, 1)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (q(i, 2) .GT. y1) THEN
        CALL PUSHREALARRAY_ADM(q(i, 2))
        q(i, 2) = y1
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREALARRAY_ADM(q(i, 2))
        q(i, 2) = q(i, 2)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (a4(1, i, 1) .GT. a4(1, i, 2)) THEN
        y2 = a4(1, i, 2)
        CALL PUSHCONTROL1B(0)
      ELSE
        y2 = a4(1, i, 1)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (q(i, 2) .LT. y2) THEN
        q(i, 2) = y2
        CALL PUSHCONTROL1B(0)
      ELSE
        q(i, 2) = q(i, 2)
        CALL PUSHCONTROL1B(1)
      END IF
    END DO
    DO k=2,km
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(gam(i, k))
        gam(i, k) = a4(1, i, k) - a4(1, i, k-1)
      END DO
    END DO
! Interior:
    DO k=3,km-1
      DO i=i1,i2
        IF (gam(i, k-1)*gam(i, k+1) .GT. 0.) THEN
          IF (a4(1, i, k-1) .LT. a4(1, i, k)) THEN
            y3 = a4(1, i, k)
            CALL PUSHCONTROL1B(0)
          ELSE
            y3 = a4(1, i, k-1)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (q(i, k) .GT. y3) THEN
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = y3
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = q(i, k)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (a4(1, i, k-1) .GT. a4(1, i, k)) THEN
            y4 = a4(1, i, k)
            CALL PUSHCONTROL1B(0)
          ELSE
            y4 = a4(1, i, k-1)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (q(i, k) .LT. y4) THEN
            q(i, k) = y4
            CALL PUSHCONTROL3B(5)
          ELSE
            q(i, k) = q(i, k)
            CALL PUSHCONTROL3B(6)
          END IF
        ELSE IF (gam(i, k-1) .GT. 0.) THEN
          IF (a4(1, i, k-1) .GT. a4(1, i, k)) THEN
            y5 = a4(1, i, k)
            CALL PUSHCONTROL1B(0)
          ELSE
            y5 = a4(1, i, k-1)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (q(i, k) .LT. y5) THEN
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = y5
            CALL PUSHCONTROL3B(3)
          ELSE
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = q(i, k)
            CALL PUSHCONTROL3B(4)
          END IF
        ELSE
          IF (a4(1, i, k-1) .LT. a4(1, i, k)) THEN
            y6 = a4(1, i, k)
            CALL PUSHCONTROL1B(0)
          ELSE
            y6 = a4(1, i, k-1)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (q(i, k) .GT. y6) THEN
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = y6
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = q(i, k)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (iv .EQ. 0) THEN
            IF (0. .LT. q(i, k)) THEN
              CALL PUSHCONTROL3B(0)
              q(i, k) = q(i, k)
            ELSE
              q(i, k) = 0.
              CALL PUSHCONTROL3B(2)
            END IF
          ELSE
            CALL PUSHCONTROL3B(1)
          END IF
        END IF
      END DO
    END DO
! Bottom:
    DO i=i1,i2
      IF (a4(1, i, km-1) .LT. a4(1, i, km)) THEN
        y7 = a4(1, i, km)
        CALL PUSHCONTROL1B(0)
      ELSE
        y7 = a4(1, i, km-1)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (q(i, km) .GT. y7) THEN
        CALL PUSHREALARRAY_ADM(q(i, km))
        q(i, km) = y7
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREALARRAY_ADM(q(i, km))
        q(i, km) = q(i, km)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (a4(1, i, km-1) .GT. a4(1, i, km)) THEN
        y8 = a4(1, i, km)
        CALL PUSHCONTROL1B(0)
      ELSE
        y8 = a4(1, i, km-1)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (q(i, km) .LT. y8) THEN
        q(i, km) = y8
        CALL PUSHCONTROL1B(0)
      ELSE
        q(i, km) = q(i, km)
        CALL PUSHCONTROL1B(1)
      END IF
    END DO
    DO k=1,km
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(2, i, k))
        a4(2, i, k) = q(i, k)
        CALL PUSHREALARRAY_ADM(a4(3, i, k))
        a4(3, i, k) = q(i, k+1)
      END DO
    END DO
    DO k=2,km-1
      DO i=i1,i2
        IF (gam(i, k)*gam(i, k+1) .GT. 0.0) THEN
          CALL PUSHCONTROL1B(1)
          extm(i, k) = .false.
        ELSE
          CALL PUSHCONTROL1B(0)
          extm(i, k) = .true.
        END IF
      END DO
    END DO
!---------------------------
! Apply subgrid constraints:
!---------------------------
! f(s) = AL + s*[(AR-AL) + A6*(1-s)]         ( 0 <= s  <= 1 )
! Top 2 and bottom 2 layers always use monotonic mapping
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(2, i, 1)) THEN
          CALL PUSHREALARRAY_ADM(a4(2, i, 1))
          a4(2, i, 1) = a4(2, i, 1)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHREALARRAY_ADM(a4(2, i, 1))
          a4(2, i, 1) = 0.
          CALL PUSHCONTROL1B(1)
        END IF
      END DO
      CALL PUSHCONTROL2B(0)
    ELSE IF (iv .EQ. -1) THEN
      DO i=i1,i2
        IF (a4(2, i, 1)*a4(1, i, 1) .LE. 0.) THEN
          CALL PUSHREALARRAY_ADM(a4(2, i, 1))
          a4(2, i, 1) = 0.
          CALL PUSHCONTROL1B(1)
        ELSE
          CALL PUSHCONTROL1B(0)
        END IF
      END DO
      CALL PUSHCONTROL2B(1)
    ELSE IF (iv .EQ. 2) THEN
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(2, i, 1))
        a4(2, i, 1) = a4(1, i, 1)
        CALL PUSHREALARRAY_ADM(a4(3, i, 1))
        a4(3, i, 1) = a4(1, i, 1)
        CALL PUSHREALARRAY_ADM(a4(4, i, 1))
        a4(4, i, 1) = 0.
      END DO
      CALL PUSHCONTROL2B(2)
    ELSE
      CALL PUSHCONTROL2B(3)
    END IF
    IF (iv .NE. 2) THEN
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(4, i, 1))
        a4(4, i, 1) = 3.*(2.*a4(1, i, 1)-(a4(2, i, 1)+a4(3, i, 1)))
      END DO
      CALL PUSHREALARRAY_ADM(a4(1, i1, 1), 8*4/8)
      CALL CS_LIMITERS(im, extm(i1, 1), a4(1, i1, 1), 1)
      CALL PUSHCONTROL1B(1)
    ELSE
      CALL PUSHCONTROL1B(0)
    END IF
! k=2
    DO i=i1,i2
      CALL PUSHREALARRAY_ADM(a4(4, i, 2))
      a4(4, i, 2) = 3.*(2.*a4(1, i, 2)-(a4(2, i, 2)+a4(3, i, 2)))
    END DO
    CALL PUSHREALARRAY_ADM(a4(1, i1, 2), 8*4/8)
    CALL CS_LIMITERS(im, extm(i1, 2), a4(1, i1, 2), 2)
!-------------------------------------
! Huynh's 2nd constraint for interior:
!-------------------------------------
    DO k=3,km-2
      IF (kord .GE. 0.) THEN
        abs0 = kord
      ELSE
        abs0 = -kord
      END IF
      IF (abs0 .LT. 9) THEN
        DO i=i1,i2
! Left  edges
          pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
          lac_1 = pmp_1 + 1.5*gam(i, k+2)
          IF (a4(1, i, k) .GT. pmp_1) THEN
            IF (pmp_1 .GT. lac_1) THEN
              y17 = lac_1
              CALL PUSHCONTROL2B(0)
            ELSE
              y17 = pmp_1
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .GT. lac_1) THEN
            y17 = lac_1
            CALL PUSHCONTROL2B(2)
          ELSE
            y17 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (a4(2, i, k) .LT. y17) THEN
            x1 = y17
            CALL PUSHCONTROL1B(0)
          ELSE
            x1 = a4(2, i, k)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (a4(1, i, k) .LT. pmp_1) THEN
            IF (pmp_1 .LT. lac_1) THEN
              y9 = lac_1
              CALL PUSHCONTROL2B(0)
            ELSE
              y9 = pmp_1
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .LT. lac_1) THEN
            y9 = lac_1
            CALL PUSHCONTROL2B(2)
          ELSE
            y9 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (x1 .GT. y9) THEN
            CALL PUSHREALARRAY_ADM(a4(2, i, k))
            a4(2, i, k) = y9
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(a4(2, i, k))
            a4(2, i, k) = x1
            CALL PUSHCONTROL1B(1)
          END IF
! Right edges
          pmp_2 = a4(1, i, k) + 2.*gam(i, k)
          lac_2 = pmp_2 - 1.5*gam(i, k-1)
          IF (a4(1, i, k) .GT. pmp_2) THEN
            IF (pmp_2 .GT. lac_2) THEN
              y18 = lac_2
              CALL PUSHCONTROL2B(0)
            ELSE
              y18 = pmp_2
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .GT. lac_2) THEN
            y18 = lac_2
            CALL PUSHCONTROL2B(2)
          ELSE
            y18 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (a4(3, i, k) .LT. y18) THEN
            x2 = y18
            CALL PUSHCONTROL1B(0)
          ELSE
            x2 = a4(3, i, k)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (a4(1, i, k) .LT. pmp_2) THEN
            IF (pmp_2 .LT. lac_2) THEN
              y10 = lac_2
              CALL PUSHCONTROL2B(0)
            ELSE
              y10 = pmp_2
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .LT. lac_2) THEN
            y10 = lac_2
            CALL PUSHCONTROL2B(2)
          ELSE
            y10 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (x2 .GT. y10) THEN
            CALL PUSHREALARRAY_ADM(a4(3, i, k))
            a4(3, i, k) = y10
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(a4(3, i, k))
            a4(3, i, k) = x2
            CALL PUSHCONTROL1B(1)
          END IF
          CALL PUSHREALARRAY_ADM(a4(4, i, k))
          a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
        END DO
        CALL PUSHCONTROL3B(0)
      ELSE
        IF (kord .GE. 0.) THEN
          abs1 = kord
        ELSE
          abs1 = -kord
        END IF
        IF (abs1 .EQ. 9) THEN
          DO i=i1,i2
            IF (extm(i, k) .AND. ((extm(i, k-1) .OR. extm(i, k+1)) .OR. &
&               a4(1, i, k) .LT. qmin)) THEN
! grid-scale 2-delta-z wave detected
              CALL PUSHREALARRAY_ADM(a4(2, i, k))
              a4(2, i, k) = a4(1, i, k)
              CALL PUSHREALARRAY_ADM(a4(3, i, k))
              a4(3, i, k) = a4(1, i, k)
              CALL PUSHREALARRAY_ADM(a4(4, i, k))
              a4(4, i, k) = 0.
              CALL PUSHCONTROL2B(2)
            ELSE
              CALL PUSHREALARRAY_ADM(a4(4, i, k))
              a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, k)&
&               )
              IF (a4(4, i, k) .GE. 0.) THEN
                abs2 = a4(4, i, k)
              ELSE
                abs2 = -a4(4, i, k)
              END IF
              IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                abs8 = a4(2, i, k) - a4(3, i, k)
              ELSE
                abs8 = -(a4(2, i, k)-a4(3, i, k))
              END IF
! Check within the smooth region if subgrid profile is non-monotonic
              IF (abs2 .GT. abs8) THEN
                pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                lac_1 = pmp_1 + 1.5*gam(i, k+2)
                IF (a4(1, i, k) .GT. pmp_1) THEN
                  IF (pmp_1 .GT. lac_1) THEN
                    y19 = lac_1
                    CALL PUSHCONTROL2B(0)
                  ELSE
                    y19 = pmp_1
                    CALL PUSHCONTROL2B(1)
                  END IF
                ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                  y19 = lac_1
                  CALL PUSHCONTROL2B(2)
                ELSE
                  y19 = a4(1, i, k)
                  CALL PUSHCONTROL2B(3)
                END IF
                IF (a4(2, i, k) .LT. y19) THEN
                  x3 = y19
                  CALL PUSHCONTROL1B(0)
                ELSE
                  x3 = a4(2, i, k)
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (a4(1, i, k) .LT. pmp_1) THEN
                  IF (pmp_1 .LT. lac_1) THEN
                    y11 = lac_1
                    CALL PUSHCONTROL2B(0)
                  ELSE
                    y11 = pmp_1
                    CALL PUSHCONTROL2B(1)
                  END IF
                ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                  y11 = lac_1
                  CALL PUSHCONTROL2B(2)
                ELSE
                  y11 = a4(1, i, k)
                  CALL PUSHCONTROL2B(3)
                END IF
                IF (x3 .GT. y11) THEN
                  CALL PUSHREALARRAY_ADM(a4(2, i, k))
                  a4(2, i, k) = y11
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHREALARRAY_ADM(a4(2, i, k))
                  a4(2, i, k) = x3
                  CALL PUSHCONTROL1B(1)
                END IF
                pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                lac_2 = pmp_2 - 1.5*gam(i, k-1)
                IF (a4(1, i, k) .GT. pmp_2) THEN
                  IF (pmp_2 .GT. lac_2) THEN
                    y20 = lac_2
                    CALL PUSHCONTROL2B(0)
                  ELSE
                    y20 = pmp_2
                    CALL PUSHCONTROL2B(1)
                  END IF
                ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                  y20 = lac_2
                  CALL PUSHCONTROL2B(2)
                ELSE
                  y20 = a4(1, i, k)
                  CALL PUSHCONTROL2B(3)
                END IF
                IF (a4(3, i, k) .LT. y20) THEN
                  x4 = y20
                  CALL PUSHCONTROL1B(0)
                ELSE
                  x4 = a4(3, i, k)
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (a4(1, i, k) .LT. pmp_2) THEN
                  IF (pmp_2 .LT. lac_2) THEN
                    y12 = lac_2
                    CALL PUSHCONTROL2B(0)
                  ELSE
                    y12 = pmp_2
                    CALL PUSHCONTROL2B(1)
                  END IF
                ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                  y12 = lac_2
                  CALL PUSHCONTROL2B(2)
                ELSE
                  y12 = a4(1, i, k)
                  CALL PUSHCONTROL2B(3)
                END IF
                IF (x4 .GT. y12) THEN
                  CALL PUSHREALARRAY_ADM(a4(3, i, k))
                  a4(3, i, k) = y12
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHREALARRAY_ADM(a4(3, i, k))
                  a4(3, i, k) = x4
                  CALL PUSHCONTROL1B(1)
                END IF
                CALL PUSHREALARRAY_ADM(a4(4, i, k))
                a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, &
&                 k))
                CALL PUSHCONTROL2B(1)
              ELSE
                CALL PUSHCONTROL2B(0)
              END IF
            END IF
          END DO
          CALL PUSHCONTROL3B(1)
        ELSE
          IF (kord .GE. 0.) THEN
            abs3 = kord
          ELSE
            abs3 = -kord
          END IF
          IF (abs3 .EQ. 10) THEN
            DO i=i1,i2
              IF (extm(i, k)) THEN
                IF ((a4(1, i, k) .LT. qmin .OR. extm(i, k-1)) .OR. extm(&
&                   i, k+1)) THEN
! grid-scale 2-delta-z wave detected; or q is too small -> ehance vertical mixing
                  CALL PUSHREALARRAY_ADM(a4(2, i, k))
                  a4(2, i, k) = a4(1, i, k)
                  CALL PUSHREALARRAY_ADM(a4(3, i, k))
                  a4(3, i, k) = a4(1, i, k)
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 0.
                  CALL PUSHCONTROL2B(3)
                ELSE
! True local extremum
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                  CALL PUSHCONTROL2B(2)
                END IF
              ELSE
! not a local extremum
                CALL PUSHREALARRAY_ADM(a4(4, i, k))
                a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, &
&                 k))
                IF (a4(4, i, k) .GE. 0.) THEN
                  abs4 = a4(4, i, k)
                ELSE
                  abs4 = -a4(4, i, k)
                END IF
                IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                  abs9 = a4(2, i, k) - a4(3, i, k)
                ELSE
                  abs9 = -(a4(2, i, k)-a4(3, i, k))
                END IF
! Check within the smooth region if subgrid profile is non-monotonic
                IF (abs4 .GT. abs9) THEN
                  pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                  lac_1 = pmp_1 + 1.5*gam(i, k+2)
                  IF (a4(1, i, k) .GT. pmp_1) THEN
                    IF (pmp_1 .GT. lac_1) THEN
                      y21 = lac_1
                      CALL PUSHCONTROL2B(0)
                    ELSE
                      y21 = pmp_1
                      CALL PUSHCONTROL2B(1)
                    END IF
                  ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                    y21 = lac_1
                    CALL PUSHCONTROL2B(2)
                  ELSE
                    y21 = a4(1, i, k)
                    CALL PUSHCONTROL2B(3)
                  END IF
                  IF (a4(2, i, k) .LT. y21) THEN
                    x5 = y21
                    CALL PUSHCONTROL1B(0)
                  ELSE
                    x5 = a4(2, i, k)
                    CALL PUSHCONTROL1B(1)
                  END IF
                  IF (a4(1, i, k) .LT. pmp_1) THEN
                    IF (pmp_1 .LT. lac_1) THEN
                      y13 = lac_1
                      CALL PUSHCONTROL2B(0)
                    ELSE
                      y13 = pmp_1
                      CALL PUSHCONTROL2B(1)
                    END IF
                  ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                    y13 = lac_1
                    CALL PUSHCONTROL2B(2)
                  ELSE
                    y13 = a4(1, i, k)
                    CALL PUSHCONTROL2B(3)
                  END IF
                  IF (x5 .GT. y13) THEN
                    CALL PUSHREALARRAY_ADM(a4(2, i, k))
                    a4(2, i, k) = y13
                    CALL PUSHCONTROL1B(0)
                  ELSE
                    CALL PUSHREALARRAY_ADM(a4(2, i, k))
                    a4(2, i, k) = x5
                    CALL PUSHCONTROL1B(1)
                  END IF
                  pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                  lac_2 = pmp_2 - 1.5*gam(i, k-1)
                  IF (a4(1, i, k) .GT. pmp_2) THEN
                    IF (pmp_2 .GT. lac_2) THEN
                      y22 = lac_2
                      CALL PUSHCONTROL2B(0)
                    ELSE
                      y22 = pmp_2
                      CALL PUSHCONTROL2B(1)
                    END IF
                  ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                    y22 = lac_2
                    CALL PUSHCONTROL2B(2)
                  ELSE
                    y22 = a4(1, i, k)
                    CALL PUSHCONTROL2B(3)
                  END IF
                  IF (a4(3, i, k) .LT. y22) THEN
                    x6 = y22
                    CALL PUSHCONTROL1B(0)
                  ELSE
                    x6 = a4(3, i, k)
                    CALL PUSHCONTROL1B(1)
                  END IF
                  IF (a4(1, i, k) .LT. pmp_2) THEN
                    IF (pmp_2 .LT. lac_2) THEN
                      y14 = lac_2
                      CALL PUSHCONTROL2B(0)
                    ELSE
                      y14 = pmp_2
                      CALL PUSHCONTROL2B(1)
                    END IF
                  ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                    y14 = lac_2
                    CALL PUSHCONTROL2B(2)
                  ELSE
                    y14 = a4(1, i, k)
                    CALL PUSHCONTROL2B(3)
                  END IF
                  IF (x6 .GT. y14) THEN
                    CALL PUSHREALARRAY_ADM(a4(3, i, k))
                    a4(3, i, k) = y14
                    CALL PUSHCONTROL1B(0)
                  ELSE
                    CALL PUSHREALARRAY_ADM(a4(3, i, k))
                    a4(3, i, k) = x6
                    CALL PUSHCONTROL1B(1)
                  END IF
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                  CALL PUSHCONTROL2B(1)
                ELSE
                  CALL PUSHCONTROL2B(0)
                END IF
              END IF
            END DO
            CALL PUSHCONTROL3B(2)
          ELSE
            IF (kord .GE. 0.) THEN
              abs5 = kord
            ELSE
              abs5 = -kord
            END IF
            IF (abs5 .EQ. 12) THEN
              DO i=i1,i2
                IF (extm(i, k)) THEN
                  CALL PUSHREALARRAY_ADM(a4(2, i, k))
                  a4(2, i, k) = a4(1, i, k)
                  CALL PUSHREALARRAY_ADM(a4(3, i, k))
                  a4(3, i, k) = a4(1, i, k)
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 0.
                  CALL PUSHCONTROL2B(2)
                ELSE
! not a local extremum
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                  IF (a4(4, i, k) .GE. 0.) THEN
                    abs6 = a4(4, i, k)
                  ELSE
                    abs6 = -a4(4, i, k)
                  END IF
                  IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                    abs10 = a4(2, i, k) - a4(3, i, k)
                  ELSE
                    abs10 = -(a4(2, i, k)-a4(3, i, k))
                  END IF
! Check within the smooth region if subgrid profile is non-monotonic
                  IF (abs6 .GT. abs10) THEN
                    pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                    lac_1 = pmp_1 + 1.5*gam(i, k+2)
                    IF (a4(1, i, k) .GT. pmp_1) THEN
                      IF (pmp_1 .GT. lac_1) THEN
                        y23 = lac_1
                        CALL PUSHCONTROL2B(0)
                      ELSE
                        y23 = pmp_1
                        CALL PUSHCONTROL2B(1)
                      END IF
                    ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                      y23 = lac_1
                      CALL PUSHCONTROL2B(2)
                    ELSE
                      y23 = a4(1, i, k)
                      CALL PUSHCONTROL2B(3)
                    END IF
                    IF (a4(2, i, k) .LT. y23) THEN
                      x7 = y23
                      CALL PUSHCONTROL1B(0)
                    ELSE
                      x7 = a4(2, i, k)
                      CALL PUSHCONTROL1B(1)
                    END IF
                    IF (a4(1, i, k) .LT. pmp_1) THEN
                      IF (pmp_1 .LT. lac_1) THEN
                        y15 = lac_1
                        CALL PUSHCONTROL2B(0)
                      ELSE
                        y15 = pmp_1
                        CALL PUSHCONTROL2B(1)
                      END IF
                    ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                      y15 = lac_1
                      CALL PUSHCONTROL2B(2)
                    ELSE
                      y15 = a4(1, i, k)
                      CALL PUSHCONTROL2B(3)
                    END IF
                    IF (x7 .GT. y15) THEN
                      CALL PUSHREALARRAY_ADM(a4(2, i, k))
                      a4(2, i, k) = y15
                      CALL PUSHCONTROL1B(0)
                    ELSE
                      CALL PUSHREALARRAY_ADM(a4(2, i, k))
                      a4(2, i, k) = x7
                      CALL PUSHCONTROL1B(1)
                    END IF
                    pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                    lac_2 = pmp_2 - 1.5*gam(i, k-1)
                    IF (a4(1, i, k) .GT. pmp_2) THEN
                      IF (pmp_2 .GT. lac_2) THEN
                        y24 = lac_2
                        CALL PUSHCONTROL2B(0)
                      ELSE
                        y24 = pmp_2
                        CALL PUSHCONTROL2B(1)
                      END IF
                    ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                      y24 = lac_2
                      CALL PUSHCONTROL2B(2)
                    ELSE
                      y24 = a4(1, i, k)
                      CALL PUSHCONTROL2B(3)
                    END IF
                    IF (a4(3, i, k) .LT. y24) THEN
                      x8 = y24
                      CALL PUSHCONTROL1B(0)
                    ELSE
                      x8 = a4(3, i, k)
                      CALL PUSHCONTROL1B(1)
                    END IF
                    IF (a4(1, i, k) .LT. pmp_2) THEN
                      IF (pmp_2 .LT. lac_2) THEN
                        y16 = lac_2
                        CALL PUSHCONTROL2B(0)
                      ELSE
                        y16 = pmp_2
                        CALL PUSHCONTROL2B(1)
                      END IF
                    ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                      y16 = lac_2
                      CALL PUSHCONTROL2B(2)
                    ELSE
                      y16 = a4(1, i, k)
                      CALL PUSHCONTROL2B(3)
                    END IF
                    IF (x8 .GT. y16) THEN
                      CALL PUSHREALARRAY_ADM(a4(3, i, k))
                      a4(3, i, k) = y16
                      CALL PUSHCONTROL1B(0)
                    ELSE
                      CALL PUSHREALARRAY_ADM(a4(3, i, k))
                      a4(3, i, k) = x8
                      CALL PUSHCONTROL1B(1)
                    END IF
                    CALL PUSHREALARRAY_ADM(a4(4, i, k))
                    a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3&
&                     , i, k))
                    CALL PUSHCONTROL2B(1)
                  ELSE
                    CALL PUSHCONTROL2B(0)
                  END IF
                END IF
              END DO
              CALL PUSHCONTROL3B(3)
            ELSE
              IF (kord .GE. 0.) THEN
                abs7 = kord
              ELSE
                abs7 = -kord
              END IF
              IF (abs7 .EQ. 14) THEN
                DO i=i1,i2
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i&
&                   , k)))
                END DO
                CALL PUSHCONTROL3B(4)
              ELSE
! kord = 11, 12, 13
                DO i=i1,i2
                  IF (extm(i, k) .AND. (extm(i, k-1) .OR. extm(i, k+1))&
&                 ) THEN
! Noisy region:
                    CALL PUSHREALARRAY_ADM(a4(2, i, k))
                    a4(2, i, k) = a4(1, i, k)
                    CALL PUSHREALARRAY_ADM(a4(3, i, k))
                    a4(3, i, k) = a4(1, i, k)
                    CALL PUSHREALARRAY_ADM(a4(4, i, k))
                    a4(4, i, k) = 0.
                    CALL PUSHCONTROL1B(1)
                  ELSE
                    CALL PUSHREALARRAY_ADM(a4(4, i, k))
                    a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, &
&                     i, k)))
                    CALL PUSHCONTROL1B(0)
                  END IF
                END DO
                CALL PUSHCONTROL3B(5)
              END IF
            END IF
          END IF
        END IF
      END IF
! Additional constraint to ensure positivity
      IF (iv .EQ. 0) THEN
        CALL PUSHREALARRAY_ADM(a4(1, i1, k), 8*4/8)
        CALL CS_LIMITERS(im, extm(i1, k), a4(1, i1, k), 0)
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHCONTROL1B(0)
      END IF
    END DO
! k-loop
!----------------------------------
! Bottom layer subgrid constraints:
!----------------------------------
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(3, i, km)) THEN
          CALL PUSHREALARRAY_ADM(a4(3, i, km))
          a4(3, i, km) = a4(3, i, km)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHREALARRAY_ADM(a4(3, i, km))
          a4(3, i, km) = 0.
          CALL PUSHCONTROL1B(1)
        END IF
      END DO
      CALL PUSHCONTROL2B(2)
    ELSE IF (iv .EQ. -1) THEN
      DO i=i1,i2
        IF (a4(3, i, km)*a4(1, i, km) .LE. 0.) THEN
          CALL PUSHREALARRAY_ADM(a4(3, i, km))
          a4(3, i, km) = 0.
          CALL PUSHCONTROL1B(1)
        ELSE
          CALL PUSHCONTROL1B(0)
        END IF
      END DO
      CALL PUSHCONTROL2B(1)
    ELSE
      CALL PUSHCONTROL2B(0)
    END IF
    DO k=km-1,km
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(4, i, k))
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
      IF (k .EQ. km - 1) THEN
        CALL PUSHREALARRAY_ADM(a4(1, i1, k), 8*4/8)
        CALL CS_LIMITERS(im, extm(i1, k), a4(1, i1, k), 2)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (k .EQ. km) THEN
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHCONTROL1B(0)
      END IF
    END DO
    DO k=km,km-1,-1
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) CALL CS_LIMITERS_ADM(im, extm(i1, k), a4(1, i1&
&                                       , k), a4_ad(1, i1, k), 1)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(a4(1, i1, k), 8*4/8)
        CALL CS_LIMITERS_ADM(im, extm(i1, k), a4(1:, i1, k), a4_ad(1:, &
&                      i1, k), 2)
      END IF
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(4, i, k))
        temp_ad19 = 3.*a4_ad(4, i, k)
        a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad19
        a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad19
        a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad19
        a4_ad(4, i, k) = 0.0_FVPRC
      END DO
    END DO
    CALL POPCONTROL2B(branch)
    IF (branch .NE. 0) THEN
      IF (branch .EQ. 1) THEN
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) THEN
            CALL POPREALARRAY_ADM(a4(3, i, km))
            a4_ad(3, i, km) = 0.0_FVPRC
          END IF
        END DO
      ELSE
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(a4(3, i, km))
          ELSE
            CALL POPREALARRAY_ADM(a4(3, i, km))
            a4_ad(3, i, km) = 0.0_FVPRC
          END IF
        END DO
      END IF
    END IF
    gam_ad = 0.0_FVPRC
    DO k=km-2,3,-1
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) THEN
        CALL POPREALARRAY_ADM(a4(1, i1, k), 8*4/8)
        CALL CS_LIMITERS_ADM(im, extm(i1, k), a4(1:, i1, k), a4_ad(1:, &
&                      i1, k), 0)
      END IF
      CALL POPCONTROL3B(branch)
      IF (branch .LT. 3) THEN
        IF (branch .EQ. 0) THEN
          DO i=i2,i1,-1
            CALL POPREALARRAY_ADM(a4(4, i, k))
            temp_ad16 = 3.*a4_ad(4, i, k)
            a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad16
            a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad16
            a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad16
            a4_ad(4, i, k) = 0.0_FVPRC
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREALARRAY_ADM(a4(3, i, k))
              y10_ad = a4_ad(3, i, k)
              a4_ad(3, i, k) = 0.0_FVPRC
              x2_ad = 0.0_FVPRC
            ELSE
              CALL POPREALARRAY_ADM(a4(3, i, k))
              x2_ad = a4_ad(3, i, k)
              a4_ad(3, i, k) = 0.0_FVPRC
              y10_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL2B(branch)
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                lac_2_ad = y10_ad
                pmp_2_ad = 0.0_FVPRC
              ELSE
                pmp_2_ad = y10_ad
                lac_2_ad = 0.0_FVPRC
              END IF
            ELSE
              IF (branch .EQ. 2) THEN
                lac_2_ad = y10_ad
              ELSE
                a4_ad(1, i, k) = a4_ad(1, i, k) + y10_ad
                lac_2_ad = 0.0_FVPRC
              END IF
              pmp_2_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              y18_ad = x2_ad
            ELSE
              a4_ad(3, i, k) = a4_ad(3, i, k) + x2_ad
              y18_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL2B(branch)
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                lac_2_ad = lac_2_ad + y18_ad
              ELSE
                pmp_2_ad = pmp_2_ad + y18_ad
              END IF
            ELSE IF (branch .EQ. 2) THEN
              lac_2_ad = lac_2_ad + y18_ad
            ELSE
              a4_ad(1, i, k) = a4_ad(1, i, k) + y18_ad
            END IF
            pmp_2_ad = pmp_2_ad + lac_2_ad
            gam_ad(i, k-1) = gam_ad(i, k-1) - 1.5*lac_2_ad
            a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_2_ad
            gam_ad(i, k) = gam_ad(i, k) + 2.*pmp_2_ad
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREALARRAY_ADM(a4(2, i, k))
              y9_ad = a4_ad(2, i, k)
              a4_ad(2, i, k) = 0.0_FVPRC
              x1_ad = 0.0_FVPRC
            ELSE
              CALL POPREALARRAY_ADM(a4(2, i, k))
              x1_ad = a4_ad(2, i, k)
              a4_ad(2, i, k) = 0.0_FVPRC
              y9_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL2B(branch)
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                lac_1_ad = y9_ad
                pmp_1_ad = 0.0_FVPRC
              ELSE
                pmp_1_ad = y9_ad
                lac_1_ad = 0.0_FVPRC
              END IF
            ELSE
              IF (branch .EQ. 2) THEN
                lac_1_ad = y9_ad
              ELSE
                a4_ad(1, i, k) = a4_ad(1, i, k) + y9_ad
                lac_1_ad = 0.0_FVPRC
              END IF
              pmp_1_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              y17_ad = x1_ad
            ELSE
              a4_ad(2, i, k) = a4_ad(2, i, k) + x1_ad
              y17_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL2B(branch)
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                lac_1_ad = lac_1_ad + y17_ad
              ELSE
                pmp_1_ad = pmp_1_ad + y17_ad
              END IF
            ELSE IF (branch .EQ. 2) THEN
              lac_1_ad = lac_1_ad + y17_ad
            ELSE
              a4_ad(1, i, k) = a4_ad(1, i, k) + y17_ad
            END IF
            pmp_1_ad = pmp_1_ad + lac_1_ad
            gam_ad(i, k+2) = gam_ad(i, k+2) + 1.5*lac_1_ad
            a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_1_ad
            gam_ad(i, k+1) = gam_ad(i, k+1) - 2.*pmp_1_ad
          END DO
        ELSE IF (branch .EQ. 1) THEN
          DO 100 i=i2,i1,-1
            CALL POPCONTROL2B(branch)
            IF (branch .NE. 0) THEN
              IF (branch .EQ. 1) THEN
                CALL POPREALARRAY_ADM(a4(4, i, k))
                a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
                a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
                a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
                a4_ad(4, i, k) = 0.0_FVPRC
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY_ADM(a4(3, i, k))
                  y12_ad = a4_ad(3, i, k)
                  a4_ad(3, i, k) = 0.0_FVPRC
                  x4_ad = 0.0_FVPRC
                ELSE
                  CALL POPREALARRAY_ADM(a4(3, i, k))
                  x4_ad = a4_ad(3, i, k)
                  a4_ad(3, i, k) = 0.0_FVPRC
                  y12_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_2_ad = y12_ad
                    pmp_2_ad = 0.0_FVPRC
                  ELSE
                    pmp_2_ad = y12_ad
                    lac_2_ad = 0.0_FVPRC
                  END IF
                ELSE
                  IF (branch .EQ. 2) THEN
                    lac_2_ad = y12_ad
                  ELSE
                    a4_ad(1, i, k) = a4_ad(1, i, k) + y12_ad
                    lac_2_ad = 0.0_FVPRC
                  END IF
                  pmp_2_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  y20_ad = x4_ad
                ELSE
                  a4_ad(3, i, k) = a4_ad(3, i, k) + x4_ad
                  y20_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_2_ad = lac_2_ad + y20_ad
                  ELSE
                    pmp_2_ad = pmp_2_ad + y20_ad
                  END IF
                ELSE IF (branch .EQ. 2) THEN
                  lac_2_ad = lac_2_ad + y20_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y20_ad
                END IF
                pmp_2_ad = pmp_2_ad + lac_2_ad
                gam_ad(i, k-1) = gam_ad(i, k-1) - 1.5*lac_2_ad
                a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_2_ad
                gam_ad(i, k) = gam_ad(i, k) + 2.*pmp_2_ad
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY_ADM(a4(2, i, k))
                  y11_ad = a4_ad(2, i, k)
                  a4_ad(2, i, k) = 0.0_FVPRC
                  x3_ad = 0.0_FVPRC
                ELSE
                  CALL POPREALARRAY_ADM(a4(2, i, k))
                  x3_ad = a4_ad(2, i, k)
                  a4_ad(2, i, k) = 0.0_FVPRC
                  y11_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_1_ad = y11_ad
                    pmp_1_ad = 0.0_FVPRC
                  ELSE
                    pmp_1_ad = y11_ad
                    lac_1_ad = 0.0_FVPRC
                  END IF
                ELSE
                  IF (branch .EQ. 2) THEN
                    lac_1_ad = y11_ad
                  ELSE
                    a4_ad(1, i, k) = a4_ad(1, i, k) + y11_ad
                    lac_1_ad = 0.0_FVPRC
                  END IF
                  pmp_1_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  y19_ad = x3_ad
                ELSE
                  a4_ad(2, i, k) = a4_ad(2, i, k) + x3_ad
                  y19_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_1_ad = lac_1_ad + y19_ad
                  ELSE
                    pmp_1_ad = pmp_1_ad + y19_ad
                  END IF
                ELSE IF (branch .EQ. 2) THEN
                  lac_1_ad = lac_1_ad + y19_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y19_ad
                END IF
                pmp_1_ad = pmp_1_ad + lac_1_ad
                gam_ad(i, k+2) = gam_ad(i, k+2) + 1.5*lac_1_ad
                a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_1_ad
                gam_ad(i, k+1) = gam_ad(i, k+1) - 2.*pmp_1_ad
              ELSE
                CALL POPREALARRAY_ADM(a4(4, i, k))
                a4_ad(4, i, k) = 0.0_FVPRC
                CALL POPREALARRAY_ADM(a4(3, i, k))
                a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(3, i, k)
                a4_ad(3, i, k) = 0.0_FVPRC
                CALL POPREALARRAY_ADM(a4(2, i, k))
                a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(2, i, k)
                a4_ad(2, i, k) = 0.0_FVPRC
                GOTO 100
              END IF
            END IF
            CALL POPREALARRAY_ADM(a4(4, i, k))
            a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
            a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
            a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
            a4_ad(4, i, k) = 0.0_FVPRC
 100      CONTINUE
        ELSE
          DO i=i2,i1,-1
            CALL POPCONTROL2B(branch)
            IF (branch .LT. 2) THEN
              IF (branch .NE. 0) THEN
                CALL POPREALARRAY_ADM(a4(4, i, k))
                a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
                a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
                a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
                a4_ad(4, i, k) = 0.0_FVPRC
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY_ADM(a4(3, i, k))
                  y14_ad = a4_ad(3, i, k)
                  a4_ad(3, i, k) = 0.0_FVPRC
                  x6_ad = 0.0_FVPRC
                ELSE
                  CALL POPREALARRAY_ADM(a4(3, i, k))
                  x6_ad = a4_ad(3, i, k)
                  a4_ad(3, i, k) = 0.0_FVPRC
                  y14_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_2_ad = y14_ad
                    pmp_2_ad = 0.0_FVPRC
                  ELSE
                    pmp_2_ad = y14_ad
                    lac_2_ad = 0.0_FVPRC
                  END IF
                ELSE
                  IF (branch .EQ. 2) THEN
                    lac_2_ad = y14_ad
                  ELSE
                    a4_ad(1, i, k) = a4_ad(1, i, k) + y14_ad
                    lac_2_ad = 0.0_FVPRC
                  END IF
                  pmp_2_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  y22_ad = x6_ad
                ELSE
                  a4_ad(3, i, k) = a4_ad(3, i, k) + x6_ad
                  y22_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_2_ad = lac_2_ad + y22_ad
                  ELSE
                    pmp_2_ad = pmp_2_ad + y22_ad
                  END IF
                ELSE IF (branch .EQ. 2) THEN
                  lac_2_ad = lac_2_ad + y22_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y22_ad
                END IF
                pmp_2_ad = pmp_2_ad + lac_2_ad
                gam_ad(i, k-1) = gam_ad(i, k-1) - 1.5*lac_2_ad
                a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_2_ad
                gam_ad(i, k) = gam_ad(i, k) + 2.*pmp_2_ad
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY_ADM(a4(2, i, k))
                  y13_ad = a4_ad(2, i, k)
                  a4_ad(2, i, k) = 0.0_FVPRC
                  x5_ad = 0.0_FVPRC
                ELSE
                  CALL POPREALARRAY_ADM(a4(2, i, k))
                  x5_ad = a4_ad(2, i, k)
                  a4_ad(2, i, k) = 0.0_FVPRC
                  y13_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_1_ad = y13_ad
                    pmp_1_ad = 0.0_FVPRC
                  ELSE
                    pmp_1_ad = y13_ad
                    lac_1_ad = 0.0_FVPRC
                  END IF
                ELSE
                  IF (branch .EQ. 2) THEN
                    lac_1_ad = y13_ad
                  ELSE
                    a4_ad(1, i, k) = a4_ad(1, i, k) + y13_ad
                    lac_1_ad = 0.0_FVPRC
                  END IF
                  pmp_1_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  y21_ad = x5_ad
                ELSE
                  a4_ad(2, i, k) = a4_ad(2, i, k) + x5_ad
                  y21_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_1_ad = lac_1_ad + y21_ad
                  ELSE
                    pmp_1_ad = pmp_1_ad + y21_ad
                  END IF
                ELSE IF (branch .EQ. 2) THEN
                  lac_1_ad = lac_1_ad + y21_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y21_ad
                END IF
                pmp_1_ad = pmp_1_ad + lac_1_ad
                gam_ad(i, k+2) = gam_ad(i, k+2) + 1.5*lac_1_ad
                a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_1_ad
                gam_ad(i, k+1) = gam_ad(i, k+1) - 2.*pmp_1_ad
              END IF
              CALL POPREALARRAY_ADM(a4(4, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
              a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(4, i, k) = 0.0_FVPRC
            ELSE IF (branch .EQ. 2) THEN
              CALL POPREALARRAY_ADM(a4(4, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
              a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(4, i, k) = 0.0_FVPRC
            ELSE
              CALL POPREALARRAY_ADM(a4(4, i, k))
              a4_ad(4, i, k) = 0.0_FVPRC
              CALL POPREALARRAY_ADM(a4(3, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(3, i, k)
              a4_ad(3, i, k) = 0.0_FVPRC
              CALL POPREALARRAY_ADM(a4(2, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(2, i, k)
              a4_ad(2, i, k) = 0.0_FVPRC
            END IF
          END DO
        END IF
      ELSE IF (branch .EQ. 3) THEN
        DO 110 i=i2,i1,-1
          CALL POPCONTROL2B(branch)
          IF (branch .NE. 0) THEN
            IF (branch .EQ. 1) THEN
              CALL POPREALARRAY_ADM(a4(4, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
              a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(4, i, k) = 0.0_FVPRC
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREALARRAY_ADM(a4(3, i, k))
                y16_ad = a4_ad(3, i, k)
                a4_ad(3, i, k) = 0.0_FVPRC
                x8_ad = 0.0_FVPRC
              ELSE
                CALL POPREALARRAY_ADM(a4(3, i, k))
                x8_ad = a4_ad(3, i, k)
                a4_ad(3, i, k) = 0.0_FVPRC
                y16_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL2B(branch)
              IF (branch .LT. 2) THEN
                IF (branch .EQ. 0) THEN
                  lac_2_ad = y16_ad
                  pmp_2_ad = 0.0_FVPRC
                ELSE
                  pmp_2_ad = y16_ad
                  lac_2_ad = 0.0_FVPRC
                END IF
              ELSE
                IF (branch .EQ. 2) THEN
                  lac_2_ad = y16_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y16_ad
                  lac_2_ad = 0.0_FVPRC
                END IF
                pmp_2_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                y24_ad = x8_ad
              ELSE
                a4_ad(3, i, k) = a4_ad(3, i, k) + x8_ad
                y24_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL2B(branch)
              IF (branch .LT. 2) THEN
                IF (branch .EQ. 0) THEN
                  lac_2_ad = lac_2_ad + y24_ad
                ELSE
                  pmp_2_ad = pmp_2_ad + y24_ad
                END IF
              ELSE IF (branch .EQ. 2) THEN
                lac_2_ad = lac_2_ad + y24_ad
              ELSE
                a4_ad(1, i, k) = a4_ad(1, i, k) + y24_ad
              END IF
              pmp_2_ad = pmp_2_ad + lac_2_ad
              gam_ad(i, k-1) = gam_ad(i, k-1) - 1.5*lac_2_ad
              a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_2_ad
              gam_ad(i, k) = gam_ad(i, k) + 2.*pmp_2_ad
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREALARRAY_ADM(a4(2, i, k))
                y15_ad = a4_ad(2, i, k)
                a4_ad(2, i, k) = 0.0_FVPRC
                x7_ad = 0.0_FVPRC
              ELSE
                CALL POPREALARRAY_ADM(a4(2, i, k))
                x7_ad = a4_ad(2, i, k)
                a4_ad(2, i, k) = 0.0_FVPRC
                y15_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL2B(branch)
              IF (branch .LT. 2) THEN
                IF (branch .EQ. 0) THEN
                  lac_1_ad = y15_ad
                  pmp_1_ad = 0.0_FVPRC
                ELSE
                  pmp_1_ad = y15_ad
                  lac_1_ad = 0.0_FVPRC
                END IF
              ELSE
                IF (branch .EQ. 2) THEN
                  lac_1_ad = y15_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y15_ad
                  lac_1_ad = 0.0_FVPRC
                END IF
                pmp_1_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                y23_ad = x7_ad
              ELSE
                a4_ad(2, i, k) = a4_ad(2, i, k) + x7_ad
                y23_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL2B(branch)
              IF (branch .LT. 2) THEN
                IF (branch .EQ. 0) THEN
                  lac_1_ad = lac_1_ad + y23_ad
                ELSE
                  pmp_1_ad = pmp_1_ad + y23_ad
                END IF
              ELSE IF (branch .EQ. 2) THEN
                lac_1_ad = lac_1_ad + y23_ad
              ELSE
                a4_ad(1, i, k) = a4_ad(1, i, k) + y23_ad
              END IF
              pmp_1_ad = pmp_1_ad + lac_1_ad
              gam_ad(i, k+2) = gam_ad(i, k+2) + 1.5*lac_1_ad
              a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_1_ad
              gam_ad(i, k+1) = gam_ad(i, k+1) - 2.*pmp_1_ad
            ELSE
              CALL POPREALARRAY_ADM(a4(4, i, k))
              a4_ad(4, i, k) = 0.0_FVPRC
              CALL POPREALARRAY_ADM(a4(3, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(3, i, k)
              a4_ad(3, i, k) = 0.0_FVPRC
              CALL POPREALARRAY_ADM(a4(2, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(2, i, k)
              a4_ad(2, i, k) = 0.0_FVPRC
              GOTO 110
            END IF
          END IF
          CALL POPREALARRAY_ADM(a4(4, i, k))
          a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
          a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
          a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
          a4_ad(4, i, k) = 0.0_FVPRC
 110    CONTINUE
      ELSE IF (branch .EQ. 4) THEN
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(a4(4, i, k))
          temp_ad17 = 3.*a4_ad(4, i, k)
          a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad17
          a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad17
          a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad17
          a4_ad(4, i, k) = 0.0_FVPRC
        END DO
      ELSE
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(a4(4, i, k))
            temp_ad18 = 3.*a4_ad(4, i, k)
            a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad18
            a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad18
            a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad18
            a4_ad(4, i, k) = 0.0_FVPRC
          ELSE
            CALL POPREALARRAY_ADM(a4(4, i, k))
            a4_ad(4, i, k) = 0.0_FVPRC
            CALL POPREALARRAY_ADM(a4(3, i, k))
            a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(3, i, k)
            a4_ad(3, i, k) = 0.0_FVPRC
            CALL POPREALARRAY_ADM(a4(2, i, k))
            a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(2, i, k)
            a4_ad(2, i, k) = 0.0_FVPRC
          END IF
        END DO
      END IF
    END DO
    CALL POPREALARRAY_ADM(a4(1, i1, 2), 8*4/8)
    CALL CS_LIMITERS_ADM(im, extm(i1, 2), a4(1:, i1, 2), a4_ad(1:, i1, 2&
&                  ), 2)
    DO i=i2,i1,-1
      CALL POPREALARRAY_ADM(a4(4, i, 2))
      temp_ad15 = 3.*a4_ad(4, i, 2)
      a4_ad(1, i, 2) = a4_ad(1, i, 2) + 2.*temp_ad15
      a4_ad(2, i, 2) = a4_ad(2, i, 2) - temp_ad15
      a4_ad(3, i, 2) = a4_ad(3, i, 2) - temp_ad15
      a4_ad(4, i, 2) = 0.0_FVPRC
    END DO
    CALL POPCONTROL1B(branch)
    IF (branch .NE. 0) THEN
      CALL POPREALARRAY_ADM(a4(1, i1, 1), 8*4/8)
      CALL CS_LIMITERS_ADM(im, extm(i1, 1), a4(1:, i1, 1), a4_ad(1:, i1&
&                    , 1), 1)
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(4, i, 1))
        temp_ad14 = 3.*a4_ad(4, i, 1)
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + 2.*temp_ad14
        a4_ad(2, i, 1) = a4_ad(2, i, 1) - temp_ad14
        a4_ad(3, i, 1) = a4_ad(3, i, 1) - temp_ad14
        a4_ad(4, i, 1) = 0.0_FVPRC
      END DO
    END IF
    CALL POPCONTROL2B(branch)
    IF (branch .LT. 2) THEN
      IF (branch .EQ. 0) THEN
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(a4(2, i, 1))
          ELSE
            CALL POPREALARRAY_ADM(a4(2, i, 1))
            a4_ad(2, i, 1) = 0.0_FVPRC
          END IF
        END DO
      ELSE
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) THEN
            CALL POPREALARRAY_ADM(a4(2, i, 1))
            a4_ad(2, i, 1) = 0.0_FVPRC
          END IF
        END DO
      END IF
    ELSE IF (branch .EQ. 2) THEN
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(4, i, 1))
        a4_ad(4, i, 1) = 0.0_FVPRC
        CALL POPREALARRAY_ADM(a4(3, i, 1))
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + a4_ad(3, i, 1)
        a4_ad(3, i, 1) = 0.0_FVPRC
        CALL POPREALARRAY_ADM(a4(2, i, 1))
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + a4_ad(2, i, 1)
        a4_ad(2, i, 1) = 0.0_FVPRC
      END DO
    END IF
    DO k=km-1,2,-1
      DO i=i2,i1,-1
        CALL POPCONTROL1B(branch)
      END DO
    END DO
    q_ad = 0.0_FVPRC
    DO k=km,1,-1
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(3, i, k))
        q_ad(i, k+1) = q_ad(i, k+1) + a4_ad(3, i, k)
        a4_ad(3, i, k) = 0.0_FVPRC
        CALL POPREALARRAY_ADM(a4(2, i, k))
        q_ad(i, k) = q_ad(i, k) + a4_ad(2, i, k)
        a4_ad(2, i, k) = 0.0_FVPRC
      END DO
    END DO
    DO i=i2,i1,-1
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        y8_ad = q_ad(i, km)
        q_ad(i, km) = 0.0_FVPRC
      ELSE
        y8_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, km) = a4_ad(1, i, km) + y8_ad
      ELSE
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + y8_ad
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(q(i, km))
        y7_ad = q_ad(i, km)
        q_ad(i, km) = 0.0_FVPRC
      ELSE
        CALL POPREALARRAY_ADM(q(i, km))
        y7_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, km) = a4_ad(1, i, km) + y7_ad
      ELSE
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + y7_ad
      END IF
    END DO
    DO k=km-1,3,-1
      DO 130 i=i2,i1,-1
        CALL POPCONTROL3B(branch)
        IF (branch .NE. 0) THEN
          IF (branch .LT. 4) THEN
            IF (branch .EQ. 1) THEN
              GOTO 120
            ELSE IF (branch .EQ. 2) THEN
              q_ad(i, k) = 0.0_FVPRC
              GOTO 120
            ELSE
              CALL POPREALARRAY_ADM(q(i, k))
              y5_ad = q_ad(i, k)
              q_ad(i, k) = 0.0_FVPRC
            END IF
          ELSE IF (branch .EQ. 4) THEN
            CALL POPREALARRAY_ADM(q(i, k))
            y5_ad = 0.0_FVPRC
          ELSE
            IF (branch .EQ. 5) THEN
              y4_ad = q_ad(i, k)
              q_ad(i, k) = 0.0_FVPRC
            ELSE
              y4_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              a4_ad(1, i, k) = a4_ad(1, i, k) + y4_ad
            ELSE
              a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + y4_ad
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREALARRAY_ADM(q(i, k))
              y3_ad = q_ad(i, k)
              q_ad(i, k) = 0.0_FVPRC
            ELSE
              CALL POPREALARRAY_ADM(q(i, k))
              y3_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              a4_ad(1, i, k) = a4_ad(1, i, k) + y3_ad
            ELSE
              a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + y3_ad
            END IF
            GOTO 130
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            a4_ad(1, i, k) = a4_ad(1, i, k) + y5_ad
          ELSE
            a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + y5_ad
          END IF
          GOTO 130
        END IF
 120    CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREALARRAY_ADM(q(i, k))
          y6_ad = q_ad(i, k)
          q_ad(i, k) = 0.0_FVPRC
        ELSE
          CALL POPREALARRAY_ADM(q(i, k))
          y6_ad = 0.0_FVPRC
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          a4_ad(1, i, k) = a4_ad(1, i, k) + y6_ad
        ELSE
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + y6_ad
        END IF
 130  CONTINUE
    END DO
    DO k=km,2,-1
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(gam(i, k))
        a4_ad(1, i, k) = a4_ad(1, i, k) + gam_ad(i, k)
        a4_ad(1, i, k-1) = a4_ad(1, i, k-1) - gam_ad(i, k)
        gam_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
    DO i=i2,i1,-1
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        y2_ad = q_ad(i, 2)
        q_ad(i, 2) = 0.0_FVPRC
      ELSE
        y2_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + y2_ad
      ELSE
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + y2_ad
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(q(i, 2))
        y1_ad = q_ad(i, 2)
        q_ad(i, 2) = 0.0_FVPRC
      ELSE
        CALL POPREALARRAY_ADM(q(i, 2))
        y1_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + y1_ad
      ELSE
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + y1_ad
      END IF
    END DO
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      DO k=1,km-1,1
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(q(i, k))
          gam_ad(i, k+1) = gam_ad(i, k+1) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k+1)*q_ad(i, k)
        END DO
      END DO
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(q(i, km+1))
        q_ad(i, km+1) = 0.0_FVPRC
        grat = delp(i, km-1)/delp(i, km)
        CALL POPREALARRAY_ADM(q(i, km))
        temp = 2*grat - gam(i, km) + 2.
        temp_ad1 = q_ad(i, km)/temp
        temp_ad2 = -((3.*(a4(1, i, km-1)+a4(1, i, km))-qs(i)*grat-q(i, &
&         km-1))*temp_ad1/temp)
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + 3.*temp_ad1
        a4_ad(1, i, km) = a4_ad(1, i, km) + 3.*temp_ad1
        grat_ad = 2*temp_ad2 - qs(i)*temp_ad1
        q_ad(i, km-1) = q_ad(i, km-1) - temp_ad1
        gam_ad(i, km) = gam_ad(i, km) - temp_ad2
        q_ad(i, km) = 0.0_FVPRC
        temp_ad3 = grat_ad/delp(i, km)
        delp_ad(i, km-1) = delp_ad(i, km-1) + temp_ad3
        delp_ad(i, km) = delp_ad(i, km) - delp(i, km-1)*temp_ad3/delp(i&
&         , km)
      END DO
      DO k=km-1,2,-1
        DO i=i2,i1,-1
          temp_ad = q_ad(i, k)/bet
          CALL POPREALARRAY_ADM(q(i, k))
          grat = delp(i, k-1)/delp(i, k)
          bet_ad = -((3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))*temp_ad/&
&           bet) - grat*gam_ad(i, k+1)/bet**2
          grat_ad = 2*bet_ad + gam_ad(i, k+1)/bet
          gam_ad(i, k+1) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + 3.*temp_ad
          a4_ad(1, i, k) = a4_ad(1, i, k) + 3.*temp_ad
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY_ADM(bet)
          gam_ad(i, k) = gam_ad(i, k) - bet_ad
          temp_ad0 = grat_ad/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad0
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad0/delp(i, &
&           k)
        END DO
      END DO
      DO i=i2,i1,-1
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + 1.5*q_ad(i, 1)
        q_ad(i, 1) = 0.0_FVPRC
      END DO
    ELSE
      DO k=1,km,1
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(q(i, k))
          gam_ad(i, k) = gam_ad(i, k) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k)*q_ad(i, k)
        END DO
      END DO
      d4_ad = 0.0_FVPRC
      DO i=i2,i1,-1
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL POPREALARRAY_ADM(q(i, km+1))
        temp2 = d4(i)*(d4(i)+0.5) - a_bot*gam(i, km)
        temp_ad11 = q_ad(i, km+1)/temp2
        temp1 = d4(i)*(d4(i)+1.)
        temp_ad12 = 2.*a4(1, i, km)*temp_ad11
        temp_ad13 = -((2.*(temp1*a4(1, i, km))+a4(1, i, km-1)-a_bot*q(i&
&         , km))*temp_ad11/temp2)
        a4_ad(1, i, km) = a4_ad(1, i, km) + 2.*temp1*temp_ad11
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + temp_ad11
        a_bot_ad = -(gam(i, km)*temp_ad13) - q(i, km)*temp_ad11
        d4_ad(i) = d4_ad(i) + (2*d4(i)+1.5)*a_bot_ad + (2*d4(i)+0.5)*&
&         temp_ad13 + (2*d4(i)+1.)*temp_ad12
        q_ad(i, km) = q_ad(i, km) - a_bot*temp_ad11
        gam_ad(i, km) = gam_ad(i, km) - a_bot*temp_ad13
        q_ad(i, km+1) = 0.0_FVPRC
      END DO
      DO k=km,2,-1
        DO i=i2,i1,-1
          temp_ad9 = q_ad(i, k)/bet
          temp_ad8 = 3.*temp_ad9
          CALL POPREALARRAY_ADM(q(i, k))
          bet_ad = -((3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))*&
&           temp_ad9/bet) - d4(i)*gam_ad(i, k)/bet**2
          d4_ad(i) = d4_ad(i) + a4(1, i, k)*temp_ad8 + 2*bet_ad + gam_ad&
&           (i, k)/bet
          gam_ad(i, k) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + temp_ad8
          a4_ad(1, i, k) = a4_ad(1, i, k) + d4(i)*temp_ad8
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad9
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY_ADM(bet)
          gam_ad(i, k-1) = gam_ad(i, k-1) - bet_ad
          CALL POPREALARRAY_ADM(d4(i))
          temp_ad10 = d4_ad(i)/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad10
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad10/delp(i&
&           , k)
          d4_ad(i) = 0.0_FVPRC
        END DO
      END DO
      DO i=i2,i1,-1
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        temp_ad4 = gam_ad(i, 1)/bet
        gam_ad(i, 1) = 0.0_FVPRC
        temp_ad6 = q_ad(i, 1)/bet
        temp_ad5 = a4(1, i, 1)*temp_ad6
        temp0 = 2*grat*(grat+1.)
        bet_ad = -((temp0*a4(1, i, 1)+a4(1, i, 2))*temp_ad6/bet) - (grat&
&         *(grat+1.5)+1.)*temp_ad4/bet
        grat_ad = (4*grat+2*1.)*temp_ad5 + (2*grat+0.5)*bet_ad + (2*grat&
&         +1.5)*temp_ad4
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + temp0*temp_ad6
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + temp_ad6
        q_ad(i, 1) = 0.0_FVPRC
        temp_ad7 = grat_ad/delp(i, 1)
        delp_ad(i, 2) = delp_ad(i, 2) + temp_ad7
        delp_ad(i, 1) = delp_ad(i, 1) - delp(i, 2)*temp_ad7/delp(i, 1)
      END DO
    END IF
  END SUBROUTINE SCALAR_PROFILE_ADM
  SUBROUTINE SCALAR_PROFILE(qs, a4, delp, km, i1, i2, iv, kord, qmin)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
    INTEGER, INTENT(IN) :: i1, i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
    INTEGER, INTENT(IN) :: kord
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(IN) :: qmin
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    INTEGER :: i, k, im
    INTRINSIC MAX
    INTRINSIC MIN
    INTRINSIC ABS
    INTEGER :: abs0
    INTEGER :: abs1
    REAL(fvprc) :: abs2
    INTEGER :: abs3
    REAL(fvprc) :: abs4
    INTEGER :: abs5
    REAL(fvprc) :: abs6
    INTEGER :: abs7
    REAL(fvprc) :: abs8
    REAL(fvprc) :: abs9
    REAL(fvprc) :: abs10
    REAL(fvprc) :: y24
    REAL(fvprc) :: y23
    REAL(fvprc) :: y22
    REAL(fvprc) :: y21
    REAL(fvprc) :: y20
    REAL(fvprc) :: x8
    REAL(fvprc) :: x7
    REAL(fvprc) :: x6
    REAL(fvprc) :: x5
    REAL(fvprc) :: x4
    REAL(fvprc) :: x3
    REAL(fvprc) :: x2
    REAL(fvprc) :: x1
    REAL(fvprc) :: y19
    REAL(fvprc) :: y18
    REAL(fvprc) :: y17
    REAL(fvprc) :: y16
    REAL(fvprc) :: y15
    REAL(fvprc) :: y14
    REAL(fvprc) :: y13
    REAL(fvprc) :: y12
    REAL(fvprc) :: y11
    REAL(fvprc) :: y10
    REAL(fvprc) :: y9
    REAL(fvprc) :: y8
    REAL(fvprc) :: y7
    REAL(fvprc) :: y6
    REAL(fvprc) :: y5
    REAL(fvprc) :: y4
    REAL(fvprc) :: y3
    REAL(fvprc) :: y2
    REAL(fvprc) :: y1
    IF (iv .EQ. -2) THEN
      DO i=i1,i2
        gam(i, 2) = 0.5
        q(i, 1) = 1.5*a4(1, i, 1)
      END DO
      DO k=2,km-1
        DO i=i1,i2
          grat = delp(i, k-1)/delp(i, k)
          bet = 2. + grat + grat - gam(i, k)
          q(i, k) = (3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))/bet
          gam(i, k+1) = grat/bet
        END DO
      END DO
      DO i=i1,i2
        grat = delp(i, km-1)/delp(i, km)
        q(i, km) = (3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, km-&
&         1))/(2.+grat+grat-gam(i, km))
        q(i, km+1) = qs(i)
      END DO
      DO k=km-1,1,-1
        DO i=i1,i2
          q(i, k) = q(i, k) - gam(i, k+1)*q(i, k+1)
        END DO
      END DO
    ELSE
      DO i=i1,i2
! grid ratio
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        q(i, 1) = ((grat+grat)*(grat+1.)*a4(1, i, 1)+a4(1, i, 2))/bet
        gam(i, 1) = (1.+grat*(grat+1.5))/bet
      END DO
      DO k=2,km
        DO i=i1,i2
          d4(i) = delp(i, k-1)/delp(i, k)
          bet = 2. + d4(i) + d4(i) - gam(i, k-1)
          q(i, k) = (3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))/bet
          gam(i, k) = d4(i)/bet
        END DO
      END DO
      DO i=i1,i2
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        q(i, km+1) = (2.*d4(i)*(d4(i)+1.)*a4(1, i, km)+a4(1, i, km-1)-&
&         a_bot*q(i, km))/(d4(i)*(d4(i)+0.5)-a_bot*gam(i, km))
      END DO
      DO k=km,1,-1
        DO i=i1,i2
          q(i, k) = q(i, k) - gam(i, k)*q(i, k+1)
        END DO
      END DO
    END IF
!------------------
! Apply constraints
!------------------
    im = i2 - i1 + 1
! Apply *large-scale* constraints 
    DO i=i1,i2
      IF (a4(1, i, 1) .LT. a4(1, i, 2)) THEN
        y1 = a4(1, i, 2)
      ELSE
        y1 = a4(1, i, 1)
      END IF
      IF (q(i, 2) .GT. y1) THEN
        q(i, 2) = y1
      ELSE
        q(i, 2) = q(i, 2)
      END IF
      IF (a4(1, i, 1) .GT. a4(1, i, 2)) THEN
        y2 = a4(1, i, 2)
      ELSE
        y2 = a4(1, i, 1)
      END IF
      IF (q(i, 2) .LT. y2) THEN
        q(i, 2) = y2
      ELSE
        q(i, 2) = q(i, 2)
      END IF
    END DO
    DO k=2,km
      DO i=i1,i2
        gam(i, k) = a4(1, i, k) - a4(1, i, k-1)
      END DO
    END DO
! Interior:
    DO k=3,km-1
      DO i=i1,i2
        IF (gam(i, k-1)*gam(i, k+1) .GT. 0.) THEN
          IF (a4(1, i, k-1) .LT. a4(1, i, k)) THEN
            y3 = a4(1, i, k)
          ELSE
            y3 = a4(1, i, k-1)
          END IF
          IF (q(i, k) .GT. y3) THEN
            q(i, k) = y3
          ELSE
            q(i, k) = q(i, k)
          END IF
          IF (a4(1, i, k-1) .GT. a4(1, i, k)) THEN
            y4 = a4(1, i, k)
          ELSE
            y4 = a4(1, i, k-1)
          END IF
          IF (q(i, k) .LT. y4) THEN
            q(i, k) = y4
          ELSE
            q(i, k) = q(i, k)
          END IF
        ELSE IF (gam(i, k-1) .GT. 0.) THEN
          IF (a4(1, i, k-1) .GT. a4(1, i, k)) THEN
            y5 = a4(1, i, k)
          ELSE
            y5 = a4(1, i, k-1)
          END IF
          IF (q(i, k) .LT. y5) THEN
            q(i, k) = y5
          ELSE
            q(i, k) = q(i, k)
          END IF
        ELSE
          IF (a4(1, i, k-1) .LT. a4(1, i, k)) THEN
            y6 = a4(1, i, k)
          ELSE
            y6 = a4(1, i, k-1)
          END IF
          IF (q(i, k) .GT. y6) THEN
            q(i, k) = y6
          ELSE
            q(i, k) = q(i, k)
          END IF
          IF (iv .EQ. 0) THEN
            IF (0. .LT. q(i, k)) THEN
              q(i, k) = q(i, k)
            ELSE
              q(i, k) = 0.
            END IF
          END IF
        END IF
      END DO
    END DO
! Bottom:
    DO i=i1,i2
      IF (a4(1, i, km-1) .LT. a4(1, i, km)) THEN
        y7 = a4(1, i, km)
      ELSE
        y7 = a4(1, i, km-1)
      END IF
      IF (q(i, km) .GT. y7) THEN
        q(i, km) = y7
      ELSE
        q(i, km) = q(i, km)
      END IF
      IF (a4(1, i, km-1) .GT. a4(1, i, km)) THEN
        y8 = a4(1, i, km)
      ELSE
        y8 = a4(1, i, km-1)
      END IF
      IF (q(i, km) .LT. y8) THEN
        q(i, km) = y8
      ELSE
        q(i, km) = q(i, km)
      END IF
    END DO
    DO k=1,km
      DO i=i1,i2
        a4(2, i, k) = q(i, k)
        a4(3, i, k) = q(i, k+1)
      END DO
    END DO
    DO k=2,km-1
      DO i=i1,i2
        IF (gam(i, k)*gam(i, k+1) .GT. 0.0) THEN
          extm(i, k) = .false.
        ELSE
          extm(i, k) = .true.
        END IF
      END DO
    END DO
!---------------------------
! Apply subgrid constraints:
!---------------------------
! f(s) = AL + s*[(AR-AL) + A6*(1-s)]         ( 0 <= s  <= 1 )
! Top 2 and bottom 2 layers always use monotonic mapping
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(2, i, 1)) THEN
          a4(2, i, 1) = a4(2, i, 1)
        ELSE
          a4(2, i, 1) = 0.
        END IF
      END DO
    ELSE IF (iv .EQ. -1) THEN
      DO i=i1,i2
        IF (a4(2, i, 1)*a4(1, i, 1) .LE. 0.) a4(2, i, 1) = 0.
      END DO
    ELSE IF (iv .EQ. 2) THEN
      DO i=i1,i2
        a4(2, i, 1) = a4(1, i, 1)
        a4(3, i, 1) = a4(1, i, 1)
        a4(4, i, 1) = 0.
      END DO
    END IF
    IF (iv .NE. 2) THEN
      DO i=i1,i2
        a4(4, i, 1) = 3.*(2.*a4(1, i, 1)-(a4(2, i, 1)+a4(3, i, 1)))
      END DO
      CALL CS_LIMITERS(im, extm(i1, 1), a4(1, i1, 1), 1)
    END IF
! k=2
    DO i=i1,i2
      a4(4, i, 2) = 3.*(2.*a4(1, i, 2)-(a4(2, i, 2)+a4(3, i, 2)))
    END DO
    CALL CS_LIMITERS(im, extm(i1, 2), a4(1, i1, 2), 2)
!-------------------------------------
! Huynh's 2nd constraint for interior:
!-------------------------------------
    DO k=3,km-2
      IF (kord .GE. 0.) THEN
        abs0 = kord
      ELSE
        abs0 = -kord
      END IF
      IF (abs0 .LT. 9) THEN
        DO i=i1,i2
! Left  edges
          pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
          lac_1 = pmp_1 + 1.5*gam(i, k+2)
          IF (a4(1, i, k) .GT. pmp_1) THEN
            IF (pmp_1 .GT. lac_1) THEN
              y17 = lac_1
            ELSE
              y17 = pmp_1
            END IF
          ELSE IF (a4(1, i, k) .GT. lac_1) THEN
            y17 = lac_1
          ELSE
            y17 = a4(1, i, k)
          END IF
          IF (a4(2, i, k) .LT. y17) THEN
            x1 = y17
          ELSE
            x1 = a4(2, i, k)
          END IF
          IF (a4(1, i, k) .LT. pmp_1) THEN
            IF (pmp_1 .LT. lac_1) THEN
              y9 = lac_1
            ELSE
              y9 = pmp_1
            END IF
          ELSE IF (a4(1, i, k) .LT. lac_1) THEN
            y9 = lac_1
          ELSE
            y9 = a4(1, i, k)
          END IF
          IF (x1 .GT. y9) THEN
            a4(2, i, k) = y9
          ELSE
            a4(2, i, k) = x1
          END IF
! Right edges
          pmp_2 = a4(1, i, k) + 2.*gam(i, k)
          lac_2 = pmp_2 - 1.5*gam(i, k-1)
          IF (a4(1, i, k) .GT. pmp_2) THEN
            IF (pmp_2 .GT. lac_2) THEN
              y18 = lac_2
            ELSE
              y18 = pmp_2
            END IF
          ELSE IF (a4(1, i, k) .GT. lac_2) THEN
            y18 = lac_2
          ELSE
            y18 = a4(1, i, k)
          END IF
          IF (a4(3, i, k) .LT. y18) THEN
            x2 = y18
          ELSE
            x2 = a4(3, i, k)
          END IF
          IF (a4(1, i, k) .LT. pmp_2) THEN
            IF (pmp_2 .LT. lac_2) THEN
              y10 = lac_2
            ELSE
              y10 = pmp_2
            END IF
          ELSE IF (a4(1, i, k) .LT. lac_2) THEN
            y10 = lac_2
          ELSE
            y10 = a4(1, i, k)
          END IF
          IF (x2 .GT. y10) THEN
            a4(3, i, k) = y10
          ELSE
            a4(3, i, k) = x2
          END IF
          a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
        END DO
      ELSE
        IF (kord .GE. 0.) THEN
          abs1 = kord
        ELSE
          abs1 = -kord
        END IF
        IF (abs1 .EQ. 9) THEN
          DO i=i1,i2
            IF (extm(i, k) .AND. ((extm(i, k-1) .OR. extm(i, k+1)) .OR. &
&               a4(1, i, k) .LT. qmin)) THEN
! grid-scale 2-delta-z wave detected
              a4(2, i, k) = a4(1, i, k)
              a4(3, i, k) = a4(1, i, k)
              a4(4, i, k) = 0.
            ELSE
              a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, k)&
&               )
              IF (a4(4, i, k) .GE. 0.) THEN
                abs2 = a4(4, i, k)
              ELSE
                abs2 = -a4(4, i, k)
              END IF
              IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                abs8 = a4(2, i, k) - a4(3, i, k)
              ELSE
                abs8 = -(a4(2, i, k)-a4(3, i, k))
              END IF
! Check within the smooth region if subgrid profile is non-monotonic
              IF (abs2 .GT. abs8) THEN
                pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                lac_1 = pmp_1 + 1.5*gam(i, k+2)
                IF (a4(1, i, k) .GT. pmp_1) THEN
                  IF (pmp_1 .GT. lac_1) THEN
                    y19 = lac_1
                  ELSE
                    y19 = pmp_1
                  END IF
                ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                  y19 = lac_1
                ELSE
                  y19 = a4(1, i, k)
                END IF
                IF (a4(2, i, k) .LT. y19) THEN
                  x3 = y19
                ELSE
                  x3 = a4(2, i, k)
                END IF
                IF (a4(1, i, k) .LT. pmp_1) THEN
                  IF (pmp_1 .LT. lac_1) THEN
                    y11 = lac_1
                  ELSE
                    y11 = pmp_1
                  END IF
                ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                  y11 = lac_1
                ELSE
                  y11 = a4(1, i, k)
                END IF
                IF (x3 .GT. y11) THEN
                  a4(2, i, k) = y11
                ELSE
                  a4(2, i, k) = x3
                END IF
                pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                lac_2 = pmp_2 - 1.5*gam(i, k-1)
                IF (a4(1, i, k) .GT. pmp_2) THEN
                  IF (pmp_2 .GT. lac_2) THEN
                    y20 = lac_2
                  ELSE
                    y20 = pmp_2
                  END IF
                ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                  y20 = lac_2
                ELSE
                  y20 = a4(1, i, k)
                END IF
                IF (a4(3, i, k) .LT. y20) THEN
                  x4 = y20
                ELSE
                  x4 = a4(3, i, k)
                END IF
                IF (a4(1, i, k) .LT. pmp_2) THEN
                  IF (pmp_2 .LT. lac_2) THEN
                    y12 = lac_2
                  ELSE
                    y12 = pmp_2
                  END IF
                ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                  y12 = lac_2
                ELSE
                  y12 = a4(1, i, k)
                END IF
                IF (x4 .GT. y12) THEN
                  a4(3, i, k) = y12
                ELSE
                  a4(3, i, k) = x4
                END IF
                a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, &
&                 k))
              END IF
            END IF
          END DO
        ELSE
          IF (kord .GE. 0.) THEN
            abs3 = kord
          ELSE
            abs3 = -kord
          END IF
          IF (abs3 .EQ. 10) THEN
            DO i=i1,i2
              IF (extm(i, k)) THEN
                IF ((a4(1, i, k) .LT. qmin .OR. extm(i, k-1)) .OR. extm(&
&                   i, k+1)) THEN
! grid-scale 2-delta-z wave detected; or q is too small -> ehance vertical mixing
                  a4(2, i, k) = a4(1, i, k)
                  a4(3, i, k) = a4(1, i, k)
                  a4(4, i, k) = 0.
                ELSE
! True local extremum
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                END IF
              ELSE
! not a local extremum
                a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, &
&                 k))
                IF (a4(4, i, k) .GE. 0.) THEN
                  abs4 = a4(4, i, k)
                ELSE
                  abs4 = -a4(4, i, k)
                END IF
                IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                  abs9 = a4(2, i, k) - a4(3, i, k)
                ELSE
                  abs9 = -(a4(2, i, k)-a4(3, i, k))
                END IF
! Check within the smooth region if subgrid profile is non-monotonic
                IF (abs4 .GT. abs9) THEN
                  pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                  lac_1 = pmp_1 + 1.5*gam(i, k+2)
                  IF (a4(1, i, k) .GT. pmp_1) THEN
                    IF (pmp_1 .GT. lac_1) THEN
                      y21 = lac_1
                    ELSE
                      y21 = pmp_1
                    END IF
                  ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                    y21 = lac_1
                  ELSE
                    y21 = a4(1, i, k)
                  END IF
                  IF (a4(2, i, k) .LT. y21) THEN
                    x5 = y21
                  ELSE
                    x5 = a4(2, i, k)
                  END IF
                  IF (a4(1, i, k) .LT. pmp_1) THEN
                    IF (pmp_1 .LT. lac_1) THEN
                      y13 = lac_1
                    ELSE
                      y13 = pmp_1
                    END IF
                  ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                    y13 = lac_1
                  ELSE
                    y13 = a4(1, i, k)
                  END IF
                  IF (x5 .GT. y13) THEN
                    a4(2, i, k) = y13
                  ELSE
                    a4(2, i, k) = x5
                  END IF
                  pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                  lac_2 = pmp_2 - 1.5*gam(i, k-1)
                  IF (a4(1, i, k) .GT. pmp_2) THEN
                    IF (pmp_2 .GT. lac_2) THEN
                      y22 = lac_2
                    ELSE
                      y22 = pmp_2
                    END IF
                  ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                    y22 = lac_2
                  ELSE
                    y22 = a4(1, i, k)
                  END IF
                  IF (a4(3, i, k) .LT. y22) THEN
                    x6 = y22
                  ELSE
                    x6 = a4(3, i, k)
                  END IF
                  IF (a4(1, i, k) .LT. pmp_2) THEN
                    IF (pmp_2 .LT. lac_2) THEN
                      y14 = lac_2
                    ELSE
                      y14 = pmp_2
                    END IF
                  ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                    y14 = lac_2
                  ELSE
                    y14 = a4(1, i, k)
                  END IF
                  IF (x6 .GT. y14) THEN
                    a4(3, i, k) = y14
                  ELSE
                    a4(3, i, k) = x6
                  END IF
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                END IF
              END IF
            END DO
          ELSE
            IF (kord .GE. 0.) THEN
              abs5 = kord
            ELSE
              abs5 = -kord
            END IF
            IF (abs5 .EQ. 12) THEN
              DO i=i1,i2
                IF (extm(i, k)) THEN
                  a4(2, i, k) = a4(1, i, k)
                  a4(3, i, k) = a4(1, i, k)
                  a4(4, i, k) = 0.
                ELSE
! not a local extremum
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                  IF (a4(4, i, k) .GE. 0.) THEN
                    abs6 = a4(4, i, k)
                  ELSE
                    abs6 = -a4(4, i, k)
                  END IF
                  IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                    abs10 = a4(2, i, k) - a4(3, i, k)
                  ELSE
                    abs10 = -(a4(2, i, k)-a4(3, i, k))
                  END IF
! Check within the smooth region if subgrid profile is non-monotonic
                  IF (abs6 .GT. abs10) THEN
                    pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                    lac_1 = pmp_1 + 1.5*gam(i, k+2)
                    IF (a4(1, i, k) .GT. pmp_1) THEN
                      IF (pmp_1 .GT. lac_1) THEN
                        y23 = lac_1
                      ELSE
                        y23 = pmp_1
                      END IF
                    ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                      y23 = lac_1
                    ELSE
                      y23 = a4(1, i, k)
                    END IF
                    IF (a4(2, i, k) .LT. y23) THEN
                      x7 = y23
                    ELSE
                      x7 = a4(2, i, k)
                    END IF
                    IF (a4(1, i, k) .LT. pmp_1) THEN
                      IF (pmp_1 .LT. lac_1) THEN
                        y15 = lac_1
                      ELSE
                        y15 = pmp_1
                      END IF
                    ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                      y15 = lac_1
                    ELSE
                      y15 = a4(1, i, k)
                    END IF
                    IF (x7 .GT. y15) THEN
                      a4(2, i, k) = y15
                    ELSE
                      a4(2, i, k) = x7
                    END IF
                    pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                    lac_2 = pmp_2 - 1.5*gam(i, k-1)
                    IF (a4(1, i, k) .GT. pmp_2) THEN
                      IF (pmp_2 .GT. lac_2) THEN
                        y24 = lac_2
                      ELSE
                        y24 = pmp_2
                      END IF
                    ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                      y24 = lac_2
                    ELSE
                      y24 = a4(1, i, k)
                    END IF
                    IF (a4(3, i, k) .LT. y24) THEN
                      x8 = y24
                    ELSE
                      x8 = a4(3, i, k)
                    END IF
                    IF (a4(1, i, k) .LT. pmp_2) THEN
                      IF (pmp_2 .LT. lac_2) THEN
                        y16 = lac_2
                      ELSE
                        y16 = pmp_2
                      END IF
                    ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                      y16 = lac_2
                    ELSE
                      y16 = a4(1, i, k)
                    END IF
                    IF (x8 .GT. y16) THEN
                      a4(3, i, k) = y16
                    ELSE
                      a4(3, i, k) = x8
                    END IF
                    a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3&
&                     , i, k))
                  END IF
                END IF
              END DO
            ELSE
              IF (kord .GE. 0.) THEN
                abs7 = kord
              ELSE
                abs7 = -kord
              END IF
              IF (abs7 .EQ. 14) THEN
                DO i=i1,i2
                  a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i&
&                   , k)))
                END DO
              ELSE
! kord = 11, 12, 13
                DO i=i1,i2
                  IF (extm(i, k) .AND. (extm(i, k-1) .OR. extm(i, k+1))&
&                 ) THEN
! Noisy region:
                    a4(2, i, k) = a4(1, i, k)
                    a4(3, i, k) = a4(1, i, k)
                    a4(4, i, k) = 0.
                  ELSE
                    a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, &
&                     i, k)))
                  END IF
                END DO
              END IF
            END IF
          END IF
        END IF
      END IF
! Additional constraint to ensure positivity
      IF (iv .EQ. 0) CALL CS_LIMITERS(im, extm(i1, k), a4(1, i1, k), 0)
    END DO
! k-loop
!----------------------------------
! Bottom layer subgrid constraints:
!----------------------------------
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(3, i, km)) THEN
          a4(3, i, km) = a4(3, i, km)
        ELSE
          a4(3, i, km) = 0.
        END IF
      END DO
    ELSE IF (iv .EQ. -1) THEN
      DO i=i1,i2
        IF (a4(3, i, km)*a4(1, i, km) .LE. 0.) a4(3, i, km) = 0.
      END DO
    END IF
    DO k=km-1,km
      DO i=i1,i2
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
      IF (k .EQ. km - 1) CALL CS_LIMITERS(im, extm(i1, k), a4(1, i1, k)&
&                                   , 2)
      IF (k .EQ. km) CALL CS_LIMITERS(im, extm(i1, k), a4(1, i1, k), 1)
    END DO
  END SUBROUTINE SCALAR_PROFILE
!  Differentiation of cs_profile_linear in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_o
!rd4_fb a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn
!_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.gra
!d1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_
!super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlo
!n fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_m
!apz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map
!1_q2_fb fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_trace
!r2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_
!core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.s
!im_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner
!_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.comp
!ute_div_damping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy
!_corners_fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyp
!pm_fb tp_core_mod.deln_flux)):
!   gradient     of useful results: qs delp a4
!   with respect to varying inputs: qs delp a4
  SUBROUTINE CS_PROFILE_LINEAR_ADM(qs, qs_ad, a4, a4_ad, delp, delp_ad, &
&   km, i1, i2, iv)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
    INTEGER, INTENT(IN) :: i1, i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
    REAL(fvprc) :: qs_ad(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
    REAL(fvprc) :: delp_ad(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4_ad(4, i1:i2, km)
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: gam_ad(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: q_ad(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: d4_ad(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: bet_ad, a_bot_ad, grat_ad
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    INTEGER :: i, k, im
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp2
    REAL(fvprc) :: temp_ad11
    REAL(fvprc) :: temp_ad12
    REAL(fvprc) :: temp_ad13
    REAL(fvprc) :: temp_ad14
    INTEGER :: branch
    IF (iv .EQ. -2) THEN
      DO i=i1,i2
        gam(i, 2) = 0.5
        q(i, 1) = 1.5*a4(1, i, 1)
      END DO
      DO k=2,km-1
        DO i=i1,i2
          grat = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY_ADM(bet)
          bet = 2. + grat + grat - gam(i, k)
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))/bet
          gam(i, k+1) = grat/bet
        END DO
      END DO
      DO i=i1,i2
        grat = delp(i, km-1)/delp(i, km)
        CALL PUSHREALARRAY_ADM(q(i, km))
        q(i, km) = (3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, km-&
&         1))/(2.+grat+grat-gam(i, km))
        CALL PUSHREALARRAY_ADM(q(i, km+1))
        q(i, km+1) = qs(i)
      END DO
      DO k=km-1,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = q(i, k) - gam(i, k+1)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B(1)
    ELSE
      DO i=i1,i2
! grid ratio
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        q(i, 1) = ((grat+grat)*(grat+1.)*a4(1, i, 1)+a4(1, i, 2))/bet
        gam(i, 1) = (1.+grat*(grat+1.5))/bet
      END DO
      DO k=2,km
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(d4(i))
          d4(i) = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY_ADM(bet)
          bet = 2. + d4(i) + d4(i) - gam(i, k-1)
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))/bet
          gam(i, k) = d4(i)/bet
        END DO
      END DO
      DO i=i1,i2
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL PUSHREALARRAY_ADM(q(i, km+1))
        q(i, km+1) = (2.*d4(i)*(d4(i)+1.)*a4(1, i, km)+a4(1, i, km-1)-&
&         a_bot*q(i, km))/(d4(i)*(d4(i)+0.5)-a_bot*gam(i, km))
      END DO
      DO k=km,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = q(i, k) - gam(i, k)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B(0)
    END IF
    q_ad = 0.0_FVPRC
    DO k=km,1,-1
      DO i=i2,i1,-1
        temp_ad14 = 3.*a4_ad(4, i, k)
        a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad14
        a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad14
        a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad14
        a4_ad(4, i, k) = 0.0_FVPRC
        q_ad(i, k+1) = q_ad(i, k+1) + a4_ad(3, i, k)
        a4_ad(3, i, k) = 0.0_FVPRC
        q_ad(i, k) = q_ad(i, k) + a4_ad(2, i, k)
        a4_ad(2, i, k) = 0.0_FVPRC
      END DO
    END DO
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      gam_ad = 0.0_FVPRC
      DO k=1,km,1
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(q(i, k))
          gam_ad(i, k) = gam_ad(i, k) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k)*q_ad(i, k)
        END DO
      END DO
      d4_ad = 0.0_FVPRC
      DO i=i2,i1,-1
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL POPREALARRAY_ADM(q(i, km+1))
        temp2 = d4(i)*(d4(i)+0.5) - a_bot*gam(i, km)
        temp_ad11 = q_ad(i, km+1)/temp2
        temp1 = d4(i)*(d4(i)+1.)
        temp_ad12 = 2.*a4(1, i, km)*temp_ad11
        temp_ad13 = -((2.*(temp1*a4(1, i, km))+a4(1, i, km-1)-a_bot*q(i&
&         , km))*temp_ad11/temp2)
        a4_ad(1, i, km) = a4_ad(1, i, km) + 2.*temp1*temp_ad11
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + temp_ad11
        a_bot_ad = -(gam(i, km)*temp_ad13) - q(i, km)*temp_ad11
        d4_ad(i) = d4_ad(i) + (2*d4(i)+1.5)*a_bot_ad + (2*d4(i)+0.5)*&
&         temp_ad13 + (2*d4(i)+1.)*temp_ad12
        q_ad(i, km) = q_ad(i, km) - a_bot*temp_ad11
        gam_ad(i, km) = gam_ad(i, km) - a_bot*temp_ad13
        q_ad(i, km+1) = 0.0_FVPRC
      END DO
      DO k=km,2,-1
        DO i=i2,i1,-1
          temp_ad9 = q_ad(i, k)/bet
          temp_ad8 = 3.*temp_ad9
          CALL POPREALARRAY_ADM(q(i, k))
          bet_ad = -((3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))*&
&           temp_ad9/bet) - d4(i)*gam_ad(i, k)/bet**2
          d4_ad(i) = d4_ad(i) + a4(1, i, k)*temp_ad8 + 2*bet_ad + gam_ad&
&           (i, k)/bet
          gam_ad(i, k) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + temp_ad8
          a4_ad(1, i, k) = a4_ad(1, i, k) + d4(i)*temp_ad8
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad9
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY_ADM(bet)
          gam_ad(i, k-1) = gam_ad(i, k-1) - bet_ad
          CALL POPREALARRAY_ADM(d4(i))
          temp_ad10 = d4_ad(i)/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad10
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad10/delp(i&
&           , k)
          d4_ad(i) = 0.0_FVPRC
        END DO
      END DO
      DO i=i2,i1,-1
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        temp_ad4 = gam_ad(i, 1)/bet
        gam_ad(i, 1) = 0.0_FVPRC
        temp_ad6 = q_ad(i, 1)/bet
        temp_ad5 = a4(1, i, 1)*temp_ad6
        temp0 = 2*grat*(grat+1.)
        bet_ad = -((temp0*a4(1, i, 1)+a4(1, i, 2))*temp_ad6/bet) - (grat&
&         *(grat+1.5)+1.)*temp_ad4/bet
        grat_ad = (4*grat+2*1.)*temp_ad5 + (2*grat+0.5)*bet_ad + (2*grat&
&         +1.5)*temp_ad4
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + temp0*temp_ad6
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + temp_ad6
        q_ad(i, 1) = 0.0_FVPRC
        temp_ad7 = grat_ad/delp(i, 1)
        delp_ad(i, 2) = delp_ad(i, 2) + temp_ad7
        delp_ad(i, 1) = delp_ad(i, 1) - delp(i, 2)*temp_ad7/delp(i, 1)
      END DO
    ELSE
      gam_ad = 0.0_FVPRC
      DO k=1,km-1,1
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(q(i, k))
          gam_ad(i, k+1) = gam_ad(i, k+1) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k+1)*q_ad(i, k)
        END DO
      END DO
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(q(i, km+1))
        qs_ad(i) = qs_ad(i) + q_ad(i, km+1)
        q_ad(i, km+1) = 0.0_FVPRC
        grat = delp(i, km-1)/delp(i, km)
        CALL POPREALARRAY_ADM(q(i, km))
        temp = 2*grat - gam(i, km) + 2.
        temp_ad1 = q_ad(i, km)/temp
        temp_ad2 = -((3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, &
&         km-1))*temp_ad1/temp)
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + 3.*temp_ad1
        a4_ad(1, i, km) = a4_ad(1, i, km) + 3.*temp_ad1
        grat_ad = 2*temp_ad2 - qs(i)*temp_ad1
        qs_ad(i) = qs_ad(i) - grat*temp_ad1
        q_ad(i, km-1) = q_ad(i, km-1) - temp_ad1
        gam_ad(i, km) = gam_ad(i, km) - temp_ad2
        q_ad(i, km) = 0.0_FVPRC
        temp_ad3 = grat_ad/delp(i, km)
        delp_ad(i, km-1) = delp_ad(i, km-1) + temp_ad3
        delp_ad(i, km) = delp_ad(i, km) - delp(i, km-1)*temp_ad3/delp(i&
&         , km)
      END DO
      DO k=km-1,2,-1
        DO i=i2,i1,-1
          temp_ad = q_ad(i, k)/bet
          CALL POPREALARRAY_ADM(q(i, k))
          grat = delp(i, k-1)/delp(i, k)
          bet_ad = -((3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))*temp_ad/&
&           bet) - grat*gam_ad(i, k+1)/bet**2
          grat_ad = 2*bet_ad + gam_ad(i, k+1)/bet
          gam_ad(i, k+1) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + 3.*temp_ad
          a4_ad(1, i, k) = a4_ad(1, i, k) + 3.*temp_ad
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY_ADM(bet)
          gam_ad(i, k) = gam_ad(i, k) - bet_ad
          temp_ad0 = grat_ad/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad0
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad0/delp(i, &
&           k)
        END DO
      END DO
      DO i=i2,i1,-1
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + 1.5*q_ad(i, 1)
        q_ad(i, 1) = 0.0_FVPRC
      END DO
    END IF
  END SUBROUTINE CS_PROFILE_LINEAR_ADM
!  Differentiation of cs_profile in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_ord4 
!a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_m
!od.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_up
!date dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_super f
!v_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlon fv_gr
!id_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_mapz_mod
!.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2
! fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tracer2d_mod
!.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_core_mo
!d.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.sim_solv
!er nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest s
!w_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.compute_div
!_damping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy_corner
!s_fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyppm t
!p_core_mod.deln_flux)):
!   gradient     of useful results: qs delp a4
!   with respect to varying inputs: qs delp a4
  SUBROUTINE CS_PROFILE_ADM(qs, qs_ad, a4, a4_ad, delp, delp_ad, km, i1&
&   , i2, iv, kord)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
    INTEGER, INTENT(IN) :: i1, i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
    INTEGER, INTENT(IN) :: kord
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
    REAL(fvprc) :: qs_ad(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
    REAL(fvprc) :: delp_ad(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4_ad(4, i1:i2, km)
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: gam_ad(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: q_ad(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: d4_ad(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: bet_ad, a_bot_ad, grat_ad
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    REAL(fvprc) :: pmp_1_ad, lac_1_ad, pmp_2_ad, lac_2_ad
    INTEGER :: i, k, im
    INTRINSIC MAX
    INTRINSIC MIN
    INTRINSIC ABS
    INTEGER :: abs0
    INTEGER :: abs1
    REAL(fvprc) :: abs2
    INTEGER :: abs3
    REAL(fvprc) :: abs4
    INTEGER :: abs5
    REAL(fvprc) :: abs6
    INTEGER :: abs7
    REAL(fvprc) :: abs8
    REAL(fvprc) :: abs9
    REAL(fvprc) :: abs10
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp2
    REAL(fvprc) :: temp_ad11
    REAL(fvprc) :: temp_ad12
    REAL(fvprc) :: temp_ad13
    REAL(fvprc) :: y1_ad
    REAL(fvprc) :: y2_ad
    REAL(fvprc) :: y3_ad
    REAL(fvprc) :: y4_ad
    REAL(fvprc) :: y5_ad
    REAL(fvprc) :: y6_ad
    REAL(fvprc) :: y7_ad
    REAL(fvprc) :: y8_ad
    REAL(fvprc) :: temp_ad14
    REAL(fvprc) :: temp_ad15
    REAL(fvprc) :: y17_ad
    REAL(fvprc) :: x1_ad
    REAL(fvprc) :: y9_ad
    REAL(fvprc) :: y18_ad
    REAL(fvprc) :: x2_ad
    REAL(fvprc) :: y10_ad
    REAL(fvprc) :: temp_ad16
    REAL(fvprc) :: y19_ad
    REAL(fvprc) :: x3_ad
    REAL(fvprc) :: y11_ad
    REAL(fvprc) :: y20_ad
    REAL(fvprc) :: x4_ad
    REAL(fvprc) :: y12_ad
    REAL(fvprc) :: y21_ad
    REAL(fvprc) :: x5_ad
    REAL(fvprc) :: y13_ad
    REAL(fvprc) :: y22_ad
    REAL(fvprc) :: x6_ad
    REAL(fvprc) :: y14_ad
    REAL(fvprc) :: y23_ad
    REAL(fvprc) :: x7_ad
    REAL(fvprc) :: y15_ad
    REAL(fvprc) :: y24_ad
    REAL(fvprc) :: x8_ad
    REAL(fvprc) :: y16_ad
    REAL(fvprc) :: temp_ad17
    REAL(fvprc) :: temp_ad18
    REAL(fvprc) :: temp_ad19
    INTEGER :: branch
    REAL(fvprc) :: y24
    REAL(fvprc) :: y23
    REAL(fvprc) :: y22
    REAL(fvprc) :: y21
    REAL(fvprc) :: y20
    REAL(fvprc) :: x8
    REAL(fvprc) :: x7
    REAL(fvprc) :: x6
    REAL(fvprc) :: x5
    REAL(fvprc) :: x4
    REAL(fvprc) :: x3
    REAL(fvprc) :: x2
    REAL(fvprc) :: x1
    REAL(fvprc) :: y19
    REAL(fvprc) :: y18
    REAL(fvprc) :: y17
    REAL(fvprc) :: y16
    REAL(fvprc) :: y15
    REAL(fvprc) :: y14
    REAL(fvprc) :: y13
    REAL(fvprc) :: y12
    REAL(fvprc) :: y11
    REAL(fvprc) :: y10
    REAL(fvprc) :: y9
    REAL(fvprc) :: y8
    REAL(fvprc) :: y7
    REAL(fvprc) :: y6
    REAL(fvprc) :: y5
    REAL(fvprc) :: y4
    REAL(fvprc) :: y3
    REAL(fvprc) :: y2
    REAL(fvprc) :: y1
    IF (iv .EQ. -2) THEN
      DO i=i1,i2
        gam(i, 2) = 0.5
        q(i, 1) = 1.5*a4(1, i, 1)
      END DO
      DO k=2,km-1
        DO i=i1,i2
          grat = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY_ADM(bet)
          bet = 2. + grat + grat - gam(i, k)
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))/bet
          gam(i, k+1) = grat/bet
        END DO
      END DO
      DO i=i1,i2
        grat = delp(i, km-1)/delp(i, km)
        CALL PUSHREALARRAY_ADM(q(i, km))
        q(i, km) = (3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, km-&
&         1))/(2.+grat+grat-gam(i, km))
        CALL PUSHREALARRAY_ADM(q(i, km+1))
        q(i, km+1) = qs(i)
      END DO
      DO k=km-1,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = q(i, k) - gam(i, k+1)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B(0)
    ELSE
      DO i=i1,i2
! grid ratio
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        q(i, 1) = ((grat+grat)*(grat+1.)*a4(1, i, 1)+a4(1, i, 2))/bet
        gam(i, 1) = (1.+grat*(grat+1.5))/bet
      END DO
      DO k=2,km
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(d4(i))
          d4(i) = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY_ADM(bet)
          bet = 2. + d4(i) + d4(i) - gam(i, k-1)
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))/bet
          gam(i, k) = d4(i)/bet
        END DO
      END DO
      DO i=i1,i2
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL PUSHREALARRAY_ADM(q(i, km+1))
        q(i, km+1) = (2.*d4(i)*(d4(i)+1.)*a4(1, i, km)+a4(1, i, km-1)-&
&         a_bot*q(i, km))/(d4(i)*(d4(i)+0.5)-a_bot*gam(i, km))
      END DO
      DO k=km,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(q(i, k))
          q(i, k) = q(i, k) - gam(i, k)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B(1)
    END IF
!------------------
! Apply constraints
!------------------
    im = i2 - i1 + 1
! Apply *large-scale* constraints 
    DO i=i1,i2
      IF (a4(1, i, 1) .LT. a4(1, i, 2)) THEN
        y1 = a4(1, i, 2)
        CALL PUSHCONTROL1B(0)
      ELSE
        y1 = a4(1, i, 1)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (q(i, 2) .GT. y1) THEN
        CALL PUSHREALARRAY_ADM(q(i, 2))
        q(i, 2) = y1
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREALARRAY_ADM(q(i, 2))
        q(i, 2) = q(i, 2)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (a4(1, i, 1) .GT. a4(1, i, 2)) THEN
        y2 = a4(1, i, 2)
        CALL PUSHCONTROL1B(0)
      ELSE
        y2 = a4(1, i, 1)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (q(i, 2) .LT. y2) THEN
        q(i, 2) = y2
        CALL PUSHCONTROL1B(0)
      ELSE
        q(i, 2) = q(i, 2)
        CALL PUSHCONTROL1B(1)
      END IF
    END DO
    DO k=2,km
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(gam(i, k))
        gam(i, k) = a4(1, i, k) - a4(1, i, k-1)
      END DO
    END DO
! Interior:
    DO k=3,km-1
      DO i=i1,i2
        IF (gam(i, k-1)*gam(i, k+1) .GT. 0.) THEN
          IF (a4(1, i, k-1) .LT. a4(1, i, k)) THEN
            y3 = a4(1, i, k)
            CALL PUSHCONTROL1B(0)
          ELSE
            y3 = a4(1, i, k-1)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (q(i, k) .GT. y3) THEN
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = y3
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = q(i, k)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (a4(1, i, k-1) .GT. a4(1, i, k)) THEN
            y4 = a4(1, i, k)
            CALL PUSHCONTROL1B(0)
          ELSE
            y4 = a4(1, i, k-1)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (q(i, k) .LT. y4) THEN
            q(i, k) = y4
            CALL PUSHCONTROL3B(5)
          ELSE
            q(i, k) = q(i, k)
            CALL PUSHCONTROL3B(6)
          END IF
        ELSE IF (gam(i, k-1) .GT. 0.) THEN
          IF (a4(1, i, k-1) .GT. a4(1, i, k)) THEN
            y5 = a4(1, i, k)
            CALL PUSHCONTROL1B(0)
          ELSE
            y5 = a4(1, i, k-1)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (q(i, k) .LT. y5) THEN
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = y5
            CALL PUSHCONTROL3B(3)
          ELSE
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = q(i, k)
            CALL PUSHCONTROL3B(4)
          END IF
        ELSE
          IF (a4(1, i, k-1) .LT. a4(1, i, k)) THEN
            y6 = a4(1, i, k)
            CALL PUSHCONTROL1B(0)
          ELSE
            y6 = a4(1, i, k-1)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (q(i, k) .GT. y6) THEN
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = y6
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(q(i, k))
            q(i, k) = q(i, k)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (iv .EQ. 0) THEN
            IF (0. .LT. q(i, k)) THEN
              CALL PUSHCONTROL3B(0)
              q(i, k) = q(i, k)
            ELSE
              q(i, k) = 0.
              CALL PUSHCONTROL3B(2)
            END IF
          ELSE
            CALL PUSHCONTROL3B(1)
          END IF
        END IF
      END DO
    END DO
! Bottom:
    DO i=i1,i2
      IF (a4(1, i, km-1) .LT. a4(1, i, km)) THEN
        y7 = a4(1, i, km)
        CALL PUSHCONTROL1B(0)
      ELSE
        y7 = a4(1, i, km-1)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (q(i, km) .GT. y7) THEN
        CALL PUSHREALARRAY_ADM(q(i, km))
        q(i, km) = y7
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREALARRAY_ADM(q(i, km))
        q(i, km) = q(i, km)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (a4(1, i, km-1) .GT. a4(1, i, km)) THEN
        y8 = a4(1, i, km)
        CALL PUSHCONTROL1B(0)
      ELSE
        y8 = a4(1, i, km-1)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (q(i, km) .LT. y8) THEN
        q(i, km) = y8
        CALL PUSHCONTROL1B(0)
      ELSE
        q(i, km) = q(i, km)
        CALL PUSHCONTROL1B(1)
      END IF
    END DO
    DO k=1,km
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(2, i, k))
        a4(2, i, k) = q(i, k)
        CALL PUSHREALARRAY_ADM(a4(3, i, k))
        a4(3, i, k) = q(i, k+1)
      END DO
    END DO
    DO k=2,km-1
      DO i=i1,i2
        IF (gam(i, k)*gam(i, k+1) .GT. 0.0) THEN
          CALL PUSHCONTROL1B(1)
          extm(i, k) = .false.
        ELSE
          CALL PUSHCONTROL1B(0)
          extm(i, k) = .true.
        END IF
      END DO
    END DO
!---------------------------
! Apply subgrid constraints:
!---------------------------
! f(s) = AL + s*[(AR-AL) + A6*(1-s)]         ( 0 <= s  <= 1 )
! Top 2 and bottom 2 layers always use monotonic mapping
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(2, i, 1)) THEN
          CALL PUSHREALARRAY_ADM(a4(2, i, 1))
          a4(2, i, 1) = a4(2, i, 1)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHREALARRAY_ADM(a4(2, i, 1))
          a4(2, i, 1) = 0.
          CALL PUSHCONTROL1B(1)
        END IF
      END DO
      CALL PUSHCONTROL2B(0)
    ELSE IF (iv .EQ. -1) THEN
      DO i=i1,i2
        IF (a4(2, i, 1)*a4(1, i, 1) .LE. 0.) THEN
          CALL PUSHREALARRAY_ADM(a4(2, i, 1))
          a4(2, i, 1) = 0.
          CALL PUSHCONTROL1B(1)
        ELSE
          CALL PUSHCONTROL1B(0)
        END IF
      END DO
      CALL PUSHCONTROL2B(1)
    ELSE IF (iv .EQ. 2) THEN
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(2, i, 1))
        a4(2, i, 1) = a4(1, i, 1)
        CALL PUSHREALARRAY_ADM(a4(3, i, 1))
        a4(3, i, 1) = a4(1, i, 1)
        CALL PUSHREALARRAY_ADM(a4(4, i, 1))
        a4(4, i, 1) = 0.
      END DO
      CALL PUSHCONTROL2B(2)
    ELSE
      CALL PUSHCONTROL2B(3)
    END IF
    IF (iv .NE. 2) THEN
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(4, i, 1))
        a4(4, i, 1) = 3.*(2.*a4(1, i, 1)-(a4(2, i, 1)+a4(3, i, 1)))
      END DO
      CALL PUSHREALARRAY_ADM(a4(1, i1, 1), 8*4/8)
      CALL CS_LIMITERS(im, extm(i1, 1), a4(1, i1, 1), 1)
      CALL PUSHCONTROL1B(1)
    ELSE
      CALL PUSHCONTROL1B(0)
    END IF
! k=2
    DO i=i1,i2
      CALL PUSHREALARRAY_ADM(a4(4, i, 2))
      a4(4, i, 2) = 3.*(2.*a4(1, i, 2)-(a4(2, i, 2)+a4(3, i, 2)))
    END DO
    CALL PUSHREALARRAY_ADM(a4(1, i1, 2), 8*4/8)
    CALL CS_LIMITERS(im, extm(i1, 2), a4(1, i1, 2), 2)
!-------------------------------------
! Huynh's 2nd constraint for interior:
!-------------------------------------
    DO k=3,km-2
      IF (kord .GE. 0.) THEN
        abs0 = kord
      ELSE
        abs0 = -kord
      END IF
      IF (abs0 .LT. 9) THEN
        DO i=i1,i2
! Left  edges
          pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
          lac_1 = pmp_1 + 1.5*gam(i, k+2)
          IF (a4(1, i, k) .GT. pmp_1) THEN
            IF (pmp_1 .GT. lac_1) THEN
              y17 = lac_1
              CALL PUSHCONTROL2B(0)
            ELSE
              y17 = pmp_1
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .GT. lac_1) THEN
            y17 = lac_1
            CALL PUSHCONTROL2B(2)
          ELSE
            y17 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (a4(2, i, k) .LT. y17) THEN
            x1 = y17
            CALL PUSHCONTROL1B(0)
          ELSE
            x1 = a4(2, i, k)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (a4(1, i, k) .LT. pmp_1) THEN
            IF (pmp_1 .LT. lac_1) THEN
              y9 = lac_1
              CALL PUSHCONTROL2B(0)
            ELSE
              y9 = pmp_1
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .LT. lac_1) THEN
            y9 = lac_1
            CALL PUSHCONTROL2B(2)
          ELSE
            y9 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (x1 .GT. y9) THEN
            CALL PUSHREALARRAY_ADM(a4(2, i, k))
            a4(2, i, k) = y9
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(a4(2, i, k))
            a4(2, i, k) = x1
            CALL PUSHCONTROL1B(1)
          END IF
! Right edges
          pmp_2 = a4(1, i, k) + 2.*gam(i, k)
          lac_2 = pmp_2 - 1.5*gam(i, k-1)
          IF (a4(1, i, k) .GT. pmp_2) THEN
            IF (pmp_2 .GT. lac_2) THEN
              y18 = lac_2
              CALL PUSHCONTROL2B(0)
            ELSE
              y18 = pmp_2
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .GT. lac_2) THEN
            y18 = lac_2
            CALL PUSHCONTROL2B(2)
          ELSE
            y18 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (a4(3, i, k) .LT. y18) THEN
            x2 = y18
            CALL PUSHCONTROL1B(0)
          ELSE
            x2 = a4(3, i, k)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (a4(1, i, k) .LT. pmp_2) THEN
            IF (pmp_2 .LT. lac_2) THEN
              y10 = lac_2
              CALL PUSHCONTROL2B(0)
            ELSE
              y10 = pmp_2
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .LT. lac_2) THEN
            y10 = lac_2
            CALL PUSHCONTROL2B(2)
          ELSE
            y10 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (x2 .GT. y10) THEN
            CALL PUSHREALARRAY_ADM(a4(3, i, k))
            a4(3, i, k) = y10
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(a4(3, i, k))
            a4(3, i, k) = x2
            CALL PUSHCONTROL1B(1)
          END IF
          CALL PUSHREALARRAY_ADM(a4(4, i, k))
          a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
        END DO
        CALL PUSHCONTROL3B(0)
      ELSE
        IF (kord .GE. 0.) THEN
          abs1 = kord
        ELSE
          abs1 = -kord
        END IF
        IF (abs1 .EQ. 9) THEN
          DO i=i1,i2
            IF (extm(i, k) .AND. (extm(i, k-1) .OR. extm(i, k+1))) THEN
! c90_mp122
! grid-scale 2-delta-z wave detected
              CALL PUSHREALARRAY_ADM(a4(2, i, k))
              a4(2, i, k) = a4(1, i, k)
              CALL PUSHREALARRAY_ADM(a4(3, i, k))
              a4(3, i, k) = a4(1, i, k)
              CALL PUSHREALARRAY_ADM(a4(4, i, k))
              a4(4, i, k) = 0.
              CALL PUSHCONTROL2B(2)
            ELSE
              CALL PUSHREALARRAY_ADM(a4(4, i, k))
              a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, k)&
&               )
              IF (a4(4, i, k) .GE. 0.) THEN
                abs2 = a4(4, i, k)
              ELSE
                abs2 = -a4(4, i, k)
              END IF
              IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                abs8 = a4(2, i, k) - a4(3, i, k)
              ELSE
                abs8 = -(a4(2, i, k)-a4(3, i, k))
              END IF
! Check within the smooth region if subgrid profile is non-monotonic
              IF (abs2 .GT. abs8) THEN
                pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                lac_1 = pmp_1 + 1.5*gam(i, k+2)
                IF (a4(1, i, k) .GT. pmp_1) THEN
                  IF (pmp_1 .GT. lac_1) THEN
                    y19 = lac_1
                    CALL PUSHCONTROL2B(0)
                  ELSE
                    y19 = pmp_1
                    CALL PUSHCONTROL2B(1)
                  END IF
                ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                  y19 = lac_1
                  CALL PUSHCONTROL2B(2)
                ELSE
                  y19 = a4(1, i, k)
                  CALL PUSHCONTROL2B(3)
                END IF
                IF (a4(2, i, k) .LT. y19) THEN
                  x3 = y19
                  CALL PUSHCONTROL1B(0)
                ELSE
                  x3 = a4(2, i, k)
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (a4(1, i, k) .LT. pmp_1) THEN
                  IF (pmp_1 .LT. lac_1) THEN
                    y11 = lac_1
                    CALL PUSHCONTROL2B(0)
                  ELSE
                    y11 = pmp_1
                    CALL PUSHCONTROL2B(1)
                  END IF
                ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                  y11 = lac_1
                  CALL PUSHCONTROL2B(2)
                ELSE
                  y11 = a4(1, i, k)
                  CALL PUSHCONTROL2B(3)
                END IF
                IF (x3 .GT. y11) THEN
                  CALL PUSHREALARRAY_ADM(a4(2, i, k))
                  a4(2, i, k) = y11
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHREALARRAY_ADM(a4(2, i, k))
                  a4(2, i, k) = x3
                  CALL PUSHCONTROL1B(1)
                END IF
                pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                lac_2 = pmp_2 - 1.5*gam(i, k-1)
                IF (a4(1, i, k) .GT. pmp_2) THEN
                  IF (pmp_2 .GT. lac_2) THEN
                    y20 = lac_2
                    CALL PUSHCONTROL2B(0)
                  ELSE
                    y20 = pmp_2
                    CALL PUSHCONTROL2B(1)
                  END IF
                ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                  y20 = lac_2
                  CALL PUSHCONTROL2B(2)
                ELSE
                  y20 = a4(1, i, k)
                  CALL PUSHCONTROL2B(3)
                END IF
                IF (a4(3, i, k) .LT. y20) THEN
                  x4 = y20
                  CALL PUSHCONTROL1B(0)
                ELSE
                  x4 = a4(3, i, k)
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (a4(1, i, k) .LT. pmp_2) THEN
                  IF (pmp_2 .LT. lac_2) THEN
                    y12 = lac_2
                    CALL PUSHCONTROL2B(0)
                  ELSE
                    y12 = pmp_2
                    CALL PUSHCONTROL2B(1)
                  END IF
                ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                  y12 = lac_2
                  CALL PUSHCONTROL2B(2)
                ELSE
                  y12 = a4(1, i, k)
                  CALL PUSHCONTROL2B(3)
                END IF
                IF (x4 .GT. y12) THEN
                  CALL PUSHREALARRAY_ADM(a4(3, i, k))
                  a4(3, i, k) = y12
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHREALARRAY_ADM(a4(3, i, k))
                  a4(3, i, k) = x4
                  CALL PUSHCONTROL1B(1)
                END IF
                CALL PUSHREALARRAY_ADM(a4(4, i, k))
                a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, &
&                 k))
                CALL PUSHCONTROL2B(1)
              ELSE
                CALL PUSHCONTROL2B(0)
              END IF
            END IF
          END DO
          CALL PUSHCONTROL3B(1)
        ELSE
          IF (kord .GE. 0.) THEN
            abs3 = kord
          ELSE
            abs3 = -kord
          END IF
          IF (abs3 .EQ. 10) THEN
            DO i=i1,i2
              IF (extm(i, k)) THEN
                IF (extm(i, k-1) .OR. extm(i, k+1)) THEN
! grid-scale 2-delta-z wave detected
                  CALL PUSHREALARRAY_ADM(a4(2, i, k))
                  a4(2, i, k) = a4(1, i, k)
                  CALL PUSHREALARRAY_ADM(a4(3, i, k))
                  a4(3, i, k) = a4(1, i, k)
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 0.
                  CALL PUSHCONTROL2B(3)
                ELSE
! True local extremum
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                  CALL PUSHCONTROL2B(2)
                END IF
              ELSE
! not a local extremum
                CALL PUSHREALARRAY_ADM(a4(4, i, k))
                a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, &
&                 k))
                IF (a4(4, i, k) .GE. 0.) THEN
                  abs4 = a4(4, i, k)
                ELSE
                  abs4 = -a4(4, i, k)
                END IF
                IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                  abs9 = a4(2, i, k) - a4(3, i, k)
                ELSE
                  abs9 = -(a4(2, i, k)-a4(3, i, k))
                END IF
! Check within the smooth region if subgrid profile is non-monotonic
                IF (abs4 .GT. abs9) THEN
                  pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                  lac_1 = pmp_1 + 1.5*gam(i, k+2)
                  IF (a4(1, i, k) .GT. pmp_1) THEN
                    IF (pmp_1 .GT. lac_1) THEN
                      y21 = lac_1
                      CALL PUSHCONTROL2B(0)
                    ELSE
                      y21 = pmp_1
                      CALL PUSHCONTROL2B(1)
                    END IF
                  ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                    y21 = lac_1
                    CALL PUSHCONTROL2B(2)
                  ELSE
                    y21 = a4(1, i, k)
                    CALL PUSHCONTROL2B(3)
                  END IF
                  IF (a4(2, i, k) .LT. y21) THEN
                    x5 = y21
                    CALL PUSHCONTROL1B(0)
                  ELSE
                    x5 = a4(2, i, k)
                    CALL PUSHCONTROL1B(1)
                  END IF
                  IF (a4(1, i, k) .LT. pmp_1) THEN
                    IF (pmp_1 .LT. lac_1) THEN
                      y13 = lac_1
                      CALL PUSHCONTROL2B(0)
                    ELSE
                      y13 = pmp_1
                      CALL PUSHCONTROL2B(1)
                    END IF
                  ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                    y13 = lac_1
                    CALL PUSHCONTROL2B(2)
                  ELSE
                    y13 = a4(1, i, k)
                    CALL PUSHCONTROL2B(3)
                  END IF
                  IF (x5 .GT. y13) THEN
                    CALL PUSHREALARRAY_ADM(a4(2, i, k))
                    a4(2, i, k) = y13
                    CALL PUSHCONTROL1B(0)
                  ELSE
                    CALL PUSHREALARRAY_ADM(a4(2, i, k))
                    a4(2, i, k) = x5
                    CALL PUSHCONTROL1B(1)
                  END IF
                  pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                  lac_2 = pmp_2 - 1.5*gam(i, k-1)
                  IF (a4(1, i, k) .GT. pmp_2) THEN
                    IF (pmp_2 .GT. lac_2) THEN
                      y22 = lac_2
                      CALL PUSHCONTROL2B(0)
                    ELSE
                      y22 = pmp_2
                      CALL PUSHCONTROL2B(1)
                    END IF
                  ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                    y22 = lac_2
                    CALL PUSHCONTROL2B(2)
                  ELSE
                    y22 = a4(1, i, k)
                    CALL PUSHCONTROL2B(3)
                  END IF
                  IF (a4(3, i, k) .LT. y22) THEN
                    x6 = y22
                    CALL PUSHCONTROL1B(0)
                  ELSE
                    x6 = a4(3, i, k)
                    CALL PUSHCONTROL1B(1)
                  END IF
                  IF (a4(1, i, k) .LT. pmp_2) THEN
                    IF (pmp_2 .LT. lac_2) THEN
                      y14 = lac_2
                      CALL PUSHCONTROL2B(0)
                    ELSE
                      y14 = pmp_2
                      CALL PUSHCONTROL2B(1)
                    END IF
                  ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                    y14 = lac_2
                    CALL PUSHCONTROL2B(2)
                  ELSE
                    y14 = a4(1, i, k)
                    CALL PUSHCONTROL2B(3)
                  END IF
                  IF (x6 .GT. y14) THEN
                    CALL PUSHREALARRAY_ADM(a4(3, i, k))
                    a4(3, i, k) = y14
                    CALL PUSHCONTROL1B(0)
                  ELSE
                    CALL PUSHREALARRAY_ADM(a4(3, i, k))
                    a4(3, i, k) = x6
                    CALL PUSHCONTROL1B(1)
                  END IF
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                  CALL PUSHCONTROL2B(1)
                ELSE
                  CALL PUSHCONTROL2B(0)
                END IF
              END IF
            END DO
            CALL PUSHCONTROL3B(2)
          ELSE
            IF (kord .GE. 0.) THEN
              abs5 = kord
            ELSE
              abs5 = -kord
            END IF
            IF (abs5 .EQ. 12) THEN
              DO i=i1,i2
                IF (extm(i, k)) THEN
! grid-scale 2-delta-z wave detected
                  CALL PUSHREALARRAY_ADM(a4(2, i, k))
                  a4(2, i, k) = a4(1, i, k)
                  CALL PUSHREALARRAY_ADM(a4(3, i, k))
                  a4(3, i, k) = a4(1, i, k)
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 0.
                  CALL PUSHCONTROL2B(2)
                ELSE
! not a local extremum
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                  IF (a4(4, i, k) .GE. 0.) THEN
                    abs6 = a4(4, i, k)
                  ELSE
                    abs6 = -a4(4, i, k)
                  END IF
                  IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                    abs10 = a4(2, i, k) - a4(3, i, k)
                  ELSE
                    abs10 = -(a4(2, i, k)-a4(3, i, k))
                  END IF
! Check within the smooth region if subgrid profile is non-monotonic
                  IF (abs6 .GT. abs10) THEN
                    pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                    lac_1 = pmp_1 + 1.5*gam(i, k+2)
                    IF (a4(1, i, k) .GT. pmp_1) THEN
                      IF (pmp_1 .GT. lac_1) THEN
                        y23 = lac_1
                        CALL PUSHCONTROL2B(0)
                      ELSE
                        y23 = pmp_1
                        CALL PUSHCONTROL2B(1)
                      END IF
                    ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                      y23 = lac_1
                      CALL PUSHCONTROL2B(2)
                    ELSE
                      y23 = a4(1, i, k)
                      CALL PUSHCONTROL2B(3)
                    END IF
                    IF (a4(2, i, k) .LT. y23) THEN
                      x7 = y23
                      CALL PUSHCONTROL1B(0)
                    ELSE
                      x7 = a4(2, i, k)
                      CALL PUSHCONTROL1B(1)
                    END IF
                    IF (a4(1, i, k) .LT. pmp_1) THEN
                      IF (pmp_1 .LT. lac_1) THEN
                        y15 = lac_1
                        CALL PUSHCONTROL2B(0)
                      ELSE
                        y15 = pmp_1
                        CALL PUSHCONTROL2B(1)
                      END IF
                    ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                      y15 = lac_1
                      CALL PUSHCONTROL2B(2)
                    ELSE
                      y15 = a4(1, i, k)
                      CALL PUSHCONTROL2B(3)
                    END IF
                    IF (x7 .GT. y15) THEN
                      CALL PUSHREALARRAY_ADM(a4(2, i, k))
                      a4(2, i, k) = y15
                      CALL PUSHCONTROL1B(0)
                    ELSE
                      CALL PUSHREALARRAY_ADM(a4(2, i, k))
                      a4(2, i, k) = x7
                      CALL PUSHCONTROL1B(1)
                    END IF
                    pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                    lac_2 = pmp_2 - 1.5*gam(i, k-1)
                    IF (a4(1, i, k) .GT. pmp_2) THEN
                      IF (pmp_2 .GT. lac_2) THEN
                        y24 = lac_2
                        CALL PUSHCONTROL2B(0)
                      ELSE
                        y24 = pmp_2
                        CALL PUSHCONTROL2B(1)
                      END IF
                    ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                      y24 = lac_2
                      CALL PUSHCONTROL2B(2)
                    ELSE
                      y24 = a4(1, i, k)
                      CALL PUSHCONTROL2B(3)
                    END IF
                    IF (a4(3, i, k) .LT. y24) THEN
                      x8 = y24
                      CALL PUSHCONTROL1B(0)
                    ELSE
                      x8 = a4(3, i, k)
                      CALL PUSHCONTROL1B(1)
                    END IF
                    IF (a4(1, i, k) .LT. pmp_2) THEN
                      IF (pmp_2 .LT. lac_2) THEN
                        y16 = lac_2
                        CALL PUSHCONTROL2B(0)
                      ELSE
                        y16 = pmp_2
                        CALL PUSHCONTROL2B(1)
                      END IF
                    ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                      y16 = lac_2
                      CALL PUSHCONTROL2B(2)
                    ELSE
                      y16 = a4(1, i, k)
                      CALL PUSHCONTROL2B(3)
                    END IF
                    IF (x8 .GT. y16) THEN
                      CALL PUSHREALARRAY_ADM(a4(3, i, k))
                      a4(3, i, k) = y16
                      CALL PUSHCONTROL1B(0)
                    ELSE
                      CALL PUSHREALARRAY_ADM(a4(3, i, k))
                      a4(3, i, k) = x8
                      CALL PUSHCONTROL1B(1)
                    END IF
                    CALL PUSHREALARRAY_ADM(a4(4, i, k))
                    a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3&
&                     , i, k))
                    CALL PUSHCONTROL2B(1)
                  ELSE
                    CALL PUSHCONTROL2B(0)
                  END IF
                END IF
              END DO
              CALL PUSHCONTROL3B(3)
            ELSE
              IF (kord .GE. 0.) THEN
                abs7 = kord
              ELSE
                abs7 = -kord
              END IF
              IF (abs7 .EQ. 14) THEN
                DO i=i1,i2
                  CALL PUSHREALARRAY_ADM(a4(4, i, k))
                  a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i&
&                   , k)))
                END DO
                CALL PUSHCONTROL3B(4)
              ELSE
! kord = 11, 12, 13
                DO i=i1,i2
                  IF (extm(i, k) .AND. (extm(i, k-1) .OR. extm(i, k+1))&
&                 ) THEN
! Noisy region:
                    CALL PUSHREALARRAY_ADM(a4(2, i, k))
                    a4(2, i, k) = a4(1, i, k)
                    CALL PUSHREALARRAY_ADM(a4(3, i, k))
                    a4(3, i, k) = a4(1, i, k)
                    CALL PUSHREALARRAY_ADM(a4(4, i, k))
                    a4(4, i, k) = 0.
                    CALL PUSHCONTROL1B(1)
                  ELSE
                    CALL PUSHREALARRAY_ADM(a4(4, i, k))
                    a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, &
&                     i, k)))
                    CALL PUSHCONTROL1B(0)
                  END IF
                END DO
                CALL PUSHCONTROL3B(5)
              END IF
            END IF
          END IF
        END IF
      END IF
! Additional constraint to ensure positivity
      IF (iv .EQ. 0) THEN
        CALL PUSHREALARRAY_ADM(a4(1, i1, k), 8*4/8)
        CALL CS_LIMITERS(im, extm(i1, k), a4(1, i1, k), 0)
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHCONTROL1B(0)
      END IF
    END DO
! k-loop
!----------------------------------
! Bottom layer subgrid constraints:
!----------------------------------
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(3, i, km)) THEN
          CALL PUSHREALARRAY_ADM(a4(3, i, km))
          a4(3, i, km) = a4(3, i, km)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHREALARRAY_ADM(a4(3, i, km))
          a4(3, i, km) = 0.
          CALL PUSHCONTROL1B(1)
        END IF
      END DO
      CALL PUSHCONTROL2B(2)
    ELSE IF (iv .EQ. -1) THEN
      DO i=i1,i2
        IF (a4(3, i, km)*a4(1, i, km) .LE. 0.) THEN
          CALL PUSHREALARRAY_ADM(a4(3, i, km))
          a4(3, i, km) = 0.
          CALL PUSHCONTROL1B(1)
        ELSE
          CALL PUSHCONTROL1B(0)
        END IF
      END DO
      CALL PUSHCONTROL2B(1)
    ELSE
      CALL PUSHCONTROL2B(0)
    END IF
    DO k=km-1,km
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(4, i, k))
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
      IF (k .EQ. km - 1) THEN
        CALL PUSHREALARRAY_ADM(a4(1, i1, k), 8*4/8)
        CALL CS_LIMITERS(im, extm(i1, k), a4(1, i1, k), 2)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (k .EQ. km) THEN
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHCONTROL1B(0)
      END IF
    END DO
    DO k=km,km-1,-1
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) CALL CS_LIMITERS_ADM(im, extm(i1, k), a4(1, i1&
&                                       , k), a4_ad(1, i1, k), 1)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(a4(1, i1, k), 8*4/8)
        CALL CS_LIMITERS_ADM(im, extm(i1, k), a4(1:, i1, k), a4_ad(1:, &
&                      i1, k), 2)
      END IF
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(4, i, k))
        temp_ad19 = 3.*a4_ad(4, i, k)
        a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad19
        a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad19
        a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad19
        a4_ad(4, i, k) = 0.0_FVPRC
      END DO
    END DO
    CALL POPCONTROL2B(branch)
    IF (branch .NE. 0) THEN
      IF (branch .EQ. 1) THEN
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) THEN
            CALL POPREALARRAY_ADM(a4(3, i, km))
            a4_ad(3, i, km) = 0.0_FVPRC
          END IF
        END DO
      ELSE
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(a4(3, i, km))
          ELSE
            CALL POPREALARRAY_ADM(a4(3, i, km))
            a4_ad(3, i, km) = 0.0_FVPRC
          END IF
        END DO
      END IF
    END IF
    gam_ad = 0.0_FVPRC
    DO k=km-2,3,-1
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) THEN
        CALL POPREALARRAY_ADM(a4(1, i1, k), 8*4/8)
        CALL CS_LIMITERS_ADM(im, extm(i1, k), a4(1:, i1, k), a4_ad(1:, &
&                      i1, k), 0)
      END IF
      CALL POPCONTROL3B(branch)
      IF (branch .LT. 3) THEN
        IF (branch .EQ. 0) THEN
          DO i=i2,i1,-1
            CALL POPREALARRAY_ADM(a4(4, i, k))
            temp_ad16 = 3.*a4_ad(4, i, k)
            a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad16
            a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad16
            a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad16
            a4_ad(4, i, k) = 0.0_FVPRC
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREALARRAY_ADM(a4(3, i, k))
              y10_ad = a4_ad(3, i, k)
              a4_ad(3, i, k) = 0.0_FVPRC
              x2_ad = 0.0_FVPRC
            ELSE
              CALL POPREALARRAY_ADM(a4(3, i, k))
              x2_ad = a4_ad(3, i, k)
              a4_ad(3, i, k) = 0.0_FVPRC
              y10_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL2B(branch)
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                lac_2_ad = y10_ad
                pmp_2_ad = 0.0_FVPRC
              ELSE
                pmp_2_ad = y10_ad
                lac_2_ad = 0.0_FVPRC
              END IF
            ELSE
              IF (branch .EQ. 2) THEN
                lac_2_ad = y10_ad
              ELSE
                a4_ad(1, i, k) = a4_ad(1, i, k) + y10_ad
                lac_2_ad = 0.0_FVPRC
              END IF
              pmp_2_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              y18_ad = x2_ad
            ELSE
              a4_ad(3, i, k) = a4_ad(3, i, k) + x2_ad
              y18_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL2B(branch)
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                lac_2_ad = lac_2_ad + y18_ad
              ELSE
                pmp_2_ad = pmp_2_ad + y18_ad
              END IF
            ELSE IF (branch .EQ. 2) THEN
              lac_2_ad = lac_2_ad + y18_ad
            ELSE
              a4_ad(1, i, k) = a4_ad(1, i, k) + y18_ad
            END IF
            pmp_2_ad = pmp_2_ad + lac_2_ad
            gam_ad(i, k-1) = gam_ad(i, k-1) - 1.5*lac_2_ad
            a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_2_ad
            gam_ad(i, k) = gam_ad(i, k) + 2.*pmp_2_ad
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREALARRAY_ADM(a4(2, i, k))
              y9_ad = a4_ad(2, i, k)
              a4_ad(2, i, k) = 0.0_FVPRC
              x1_ad = 0.0_FVPRC
            ELSE
              CALL POPREALARRAY_ADM(a4(2, i, k))
              x1_ad = a4_ad(2, i, k)
              a4_ad(2, i, k) = 0.0_FVPRC
              y9_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL2B(branch)
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                lac_1_ad = y9_ad
                pmp_1_ad = 0.0_FVPRC
              ELSE
                pmp_1_ad = y9_ad
                lac_1_ad = 0.0_FVPRC
              END IF
            ELSE
              IF (branch .EQ. 2) THEN
                lac_1_ad = y9_ad
              ELSE
                a4_ad(1, i, k) = a4_ad(1, i, k) + y9_ad
                lac_1_ad = 0.0_FVPRC
              END IF
              pmp_1_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              y17_ad = x1_ad
            ELSE
              a4_ad(2, i, k) = a4_ad(2, i, k) + x1_ad
              y17_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL2B(branch)
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                lac_1_ad = lac_1_ad + y17_ad
              ELSE
                pmp_1_ad = pmp_1_ad + y17_ad
              END IF
            ELSE IF (branch .EQ. 2) THEN
              lac_1_ad = lac_1_ad + y17_ad
            ELSE
              a4_ad(1, i, k) = a4_ad(1, i, k) + y17_ad
            END IF
            pmp_1_ad = pmp_1_ad + lac_1_ad
            gam_ad(i, k+2) = gam_ad(i, k+2) + 1.5*lac_1_ad
            a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_1_ad
            gam_ad(i, k+1) = gam_ad(i, k+1) - 2.*pmp_1_ad
          END DO
        ELSE IF (branch .EQ. 1) THEN
          DO 100 i=i2,i1,-1
            CALL POPCONTROL2B(branch)
            IF (branch .NE. 0) THEN
              IF (branch .EQ. 1) THEN
                CALL POPREALARRAY_ADM(a4(4, i, k))
                a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
                a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
                a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
                a4_ad(4, i, k) = 0.0_FVPRC
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY_ADM(a4(3, i, k))
                  y12_ad = a4_ad(3, i, k)
                  a4_ad(3, i, k) = 0.0_FVPRC
                  x4_ad = 0.0_FVPRC
                ELSE
                  CALL POPREALARRAY_ADM(a4(3, i, k))
                  x4_ad = a4_ad(3, i, k)
                  a4_ad(3, i, k) = 0.0_FVPRC
                  y12_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_2_ad = y12_ad
                    pmp_2_ad = 0.0_FVPRC
                  ELSE
                    pmp_2_ad = y12_ad
                    lac_2_ad = 0.0_FVPRC
                  END IF
                ELSE
                  IF (branch .EQ. 2) THEN
                    lac_2_ad = y12_ad
                  ELSE
                    a4_ad(1, i, k) = a4_ad(1, i, k) + y12_ad
                    lac_2_ad = 0.0_FVPRC
                  END IF
                  pmp_2_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  y20_ad = x4_ad
                ELSE
                  a4_ad(3, i, k) = a4_ad(3, i, k) + x4_ad
                  y20_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_2_ad = lac_2_ad + y20_ad
                  ELSE
                    pmp_2_ad = pmp_2_ad + y20_ad
                  END IF
                ELSE IF (branch .EQ. 2) THEN
                  lac_2_ad = lac_2_ad + y20_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y20_ad
                END IF
                pmp_2_ad = pmp_2_ad + lac_2_ad
                gam_ad(i, k-1) = gam_ad(i, k-1) - 1.5*lac_2_ad
                a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_2_ad
                gam_ad(i, k) = gam_ad(i, k) + 2.*pmp_2_ad
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY_ADM(a4(2, i, k))
                  y11_ad = a4_ad(2, i, k)
                  a4_ad(2, i, k) = 0.0_FVPRC
                  x3_ad = 0.0_FVPRC
                ELSE
                  CALL POPREALARRAY_ADM(a4(2, i, k))
                  x3_ad = a4_ad(2, i, k)
                  a4_ad(2, i, k) = 0.0_FVPRC
                  y11_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_1_ad = y11_ad
                    pmp_1_ad = 0.0_FVPRC
                  ELSE
                    pmp_1_ad = y11_ad
                    lac_1_ad = 0.0_FVPRC
                  END IF
                ELSE
                  IF (branch .EQ. 2) THEN
                    lac_1_ad = y11_ad
                  ELSE
                    a4_ad(1, i, k) = a4_ad(1, i, k) + y11_ad
                    lac_1_ad = 0.0_FVPRC
                  END IF
                  pmp_1_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  y19_ad = x3_ad
                ELSE
                  a4_ad(2, i, k) = a4_ad(2, i, k) + x3_ad
                  y19_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_1_ad = lac_1_ad + y19_ad
                  ELSE
                    pmp_1_ad = pmp_1_ad + y19_ad
                  END IF
                ELSE IF (branch .EQ. 2) THEN
                  lac_1_ad = lac_1_ad + y19_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y19_ad
                END IF
                pmp_1_ad = pmp_1_ad + lac_1_ad
                gam_ad(i, k+2) = gam_ad(i, k+2) + 1.5*lac_1_ad
                a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_1_ad
                gam_ad(i, k+1) = gam_ad(i, k+1) - 2.*pmp_1_ad
              ELSE
                CALL POPREALARRAY_ADM(a4(4, i, k))
                a4_ad(4, i, k) = 0.0_FVPRC
                CALL POPREALARRAY_ADM(a4(3, i, k))
                a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(3, i, k)
                a4_ad(3, i, k) = 0.0_FVPRC
                CALL POPREALARRAY_ADM(a4(2, i, k))
                a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(2, i, k)
                a4_ad(2, i, k) = 0.0_FVPRC
                GOTO 100
              END IF
            END IF
            CALL POPREALARRAY_ADM(a4(4, i, k))
            a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
            a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
            a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
            a4_ad(4, i, k) = 0.0_FVPRC
 100      CONTINUE
        ELSE
          DO i=i2,i1,-1
            CALL POPCONTROL2B(branch)
            IF (branch .LT. 2) THEN
              IF (branch .NE. 0) THEN
                CALL POPREALARRAY_ADM(a4(4, i, k))
                a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
                a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
                a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
                a4_ad(4, i, k) = 0.0_FVPRC
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY_ADM(a4(3, i, k))
                  y14_ad = a4_ad(3, i, k)
                  a4_ad(3, i, k) = 0.0_FVPRC
                  x6_ad = 0.0_FVPRC
                ELSE
                  CALL POPREALARRAY_ADM(a4(3, i, k))
                  x6_ad = a4_ad(3, i, k)
                  a4_ad(3, i, k) = 0.0_FVPRC
                  y14_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_2_ad = y14_ad
                    pmp_2_ad = 0.0_FVPRC
                  ELSE
                    pmp_2_ad = y14_ad
                    lac_2_ad = 0.0_FVPRC
                  END IF
                ELSE
                  IF (branch .EQ. 2) THEN
                    lac_2_ad = y14_ad
                  ELSE
                    a4_ad(1, i, k) = a4_ad(1, i, k) + y14_ad
                    lac_2_ad = 0.0_FVPRC
                  END IF
                  pmp_2_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  y22_ad = x6_ad
                ELSE
                  a4_ad(3, i, k) = a4_ad(3, i, k) + x6_ad
                  y22_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_2_ad = lac_2_ad + y22_ad
                  ELSE
                    pmp_2_ad = pmp_2_ad + y22_ad
                  END IF
                ELSE IF (branch .EQ. 2) THEN
                  lac_2_ad = lac_2_ad + y22_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y22_ad
                END IF
                pmp_2_ad = pmp_2_ad + lac_2_ad
                gam_ad(i, k-1) = gam_ad(i, k-1) - 1.5*lac_2_ad
                a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_2_ad
                gam_ad(i, k) = gam_ad(i, k) + 2.*pmp_2_ad
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY_ADM(a4(2, i, k))
                  y13_ad = a4_ad(2, i, k)
                  a4_ad(2, i, k) = 0.0_FVPRC
                  x5_ad = 0.0_FVPRC
                ELSE
                  CALL POPREALARRAY_ADM(a4(2, i, k))
                  x5_ad = a4_ad(2, i, k)
                  a4_ad(2, i, k) = 0.0_FVPRC
                  y13_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_1_ad = y13_ad
                    pmp_1_ad = 0.0_FVPRC
                  ELSE
                    pmp_1_ad = y13_ad
                    lac_1_ad = 0.0_FVPRC
                  END IF
                ELSE
                  IF (branch .EQ. 2) THEN
                    lac_1_ad = y13_ad
                  ELSE
                    a4_ad(1, i, k) = a4_ad(1, i, k) + y13_ad
                    lac_1_ad = 0.0_FVPRC
                  END IF
                  pmp_1_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  y21_ad = x5_ad
                ELSE
                  a4_ad(2, i, k) = a4_ad(2, i, k) + x5_ad
                  y21_ad = 0.0_FVPRC
                END IF
                CALL POPCONTROL2B(branch)
                IF (branch .LT. 2) THEN
                  IF (branch .EQ. 0) THEN
                    lac_1_ad = lac_1_ad + y21_ad
                  ELSE
                    pmp_1_ad = pmp_1_ad + y21_ad
                  END IF
                ELSE IF (branch .EQ. 2) THEN
                  lac_1_ad = lac_1_ad + y21_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y21_ad
                END IF
                pmp_1_ad = pmp_1_ad + lac_1_ad
                gam_ad(i, k+2) = gam_ad(i, k+2) + 1.5*lac_1_ad
                a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_1_ad
                gam_ad(i, k+1) = gam_ad(i, k+1) - 2.*pmp_1_ad
              END IF
              CALL POPREALARRAY_ADM(a4(4, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
              a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(4, i, k) = 0.0_FVPRC
            ELSE IF (branch .EQ. 2) THEN
              CALL POPREALARRAY_ADM(a4(4, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
              a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(4, i, k) = 0.0_FVPRC
            ELSE
              CALL POPREALARRAY_ADM(a4(4, i, k))
              a4_ad(4, i, k) = 0.0_FVPRC
              CALL POPREALARRAY_ADM(a4(3, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(3, i, k)
              a4_ad(3, i, k) = 0.0_FVPRC
              CALL POPREALARRAY_ADM(a4(2, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(2, i, k)
              a4_ad(2, i, k) = 0.0_FVPRC
            END IF
          END DO
        END IF
      ELSE IF (branch .EQ. 3) THEN
        DO 110 i=i2,i1,-1
          CALL POPCONTROL2B(branch)
          IF (branch .NE. 0) THEN
            IF (branch .EQ. 1) THEN
              CALL POPREALARRAY_ADM(a4(4, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
              a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
              a4_ad(4, i, k) = 0.0_FVPRC
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREALARRAY_ADM(a4(3, i, k))
                y16_ad = a4_ad(3, i, k)
                a4_ad(3, i, k) = 0.0_FVPRC
                x8_ad = 0.0_FVPRC
              ELSE
                CALL POPREALARRAY_ADM(a4(3, i, k))
                x8_ad = a4_ad(3, i, k)
                a4_ad(3, i, k) = 0.0_FVPRC
                y16_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL2B(branch)
              IF (branch .LT. 2) THEN
                IF (branch .EQ. 0) THEN
                  lac_2_ad = y16_ad
                  pmp_2_ad = 0.0_FVPRC
                ELSE
                  pmp_2_ad = y16_ad
                  lac_2_ad = 0.0_FVPRC
                END IF
              ELSE
                IF (branch .EQ. 2) THEN
                  lac_2_ad = y16_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y16_ad
                  lac_2_ad = 0.0_FVPRC
                END IF
                pmp_2_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                y24_ad = x8_ad
              ELSE
                a4_ad(3, i, k) = a4_ad(3, i, k) + x8_ad
                y24_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL2B(branch)
              IF (branch .LT. 2) THEN
                IF (branch .EQ. 0) THEN
                  lac_2_ad = lac_2_ad + y24_ad
                ELSE
                  pmp_2_ad = pmp_2_ad + y24_ad
                END IF
              ELSE IF (branch .EQ. 2) THEN
                lac_2_ad = lac_2_ad + y24_ad
              ELSE
                a4_ad(1, i, k) = a4_ad(1, i, k) + y24_ad
              END IF
              pmp_2_ad = pmp_2_ad + lac_2_ad
              gam_ad(i, k-1) = gam_ad(i, k-1) - 1.5*lac_2_ad
              a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_2_ad
              gam_ad(i, k) = gam_ad(i, k) + 2.*pmp_2_ad
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREALARRAY_ADM(a4(2, i, k))
                y15_ad = a4_ad(2, i, k)
                a4_ad(2, i, k) = 0.0_FVPRC
                x7_ad = 0.0_FVPRC
              ELSE
                CALL POPREALARRAY_ADM(a4(2, i, k))
                x7_ad = a4_ad(2, i, k)
                a4_ad(2, i, k) = 0.0_FVPRC
                y15_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL2B(branch)
              IF (branch .LT. 2) THEN
                IF (branch .EQ. 0) THEN
                  lac_1_ad = y15_ad
                  pmp_1_ad = 0.0_FVPRC
                ELSE
                  pmp_1_ad = y15_ad
                  lac_1_ad = 0.0_FVPRC
                END IF
              ELSE
                IF (branch .EQ. 2) THEN
                  lac_1_ad = y15_ad
                ELSE
                  a4_ad(1, i, k) = a4_ad(1, i, k) + y15_ad
                  lac_1_ad = 0.0_FVPRC
                END IF
                pmp_1_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                y23_ad = x7_ad
              ELSE
                a4_ad(2, i, k) = a4_ad(2, i, k) + x7_ad
                y23_ad = 0.0_FVPRC
              END IF
              CALL POPCONTROL2B(branch)
              IF (branch .LT. 2) THEN
                IF (branch .EQ. 0) THEN
                  lac_1_ad = lac_1_ad + y23_ad
                ELSE
                  pmp_1_ad = pmp_1_ad + y23_ad
                END IF
              ELSE IF (branch .EQ. 2) THEN
                lac_1_ad = lac_1_ad + y23_ad
              ELSE
                a4_ad(1, i, k) = a4_ad(1, i, k) + y23_ad
              END IF
              pmp_1_ad = pmp_1_ad + lac_1_ad
              gam_ad(i, k+2) = gam_ad(i, k+2) + 1.5*lac_1_ad
              a4_ad(1, i, k) = a4_ad(1, i, k) + pmp_1_ad
              gam_ad(i, k+1) = gam_ad(i, k+1) - 2.*pmp_1_ad
            ELSE
              CALL POPREALARRAY_ADM(a4(4, i, k))
              a4_ad(4, i, k) = 0.0_FVPRC
              CALL POPREALARRAY_ADM(a4(3, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(3, i, k)
              a4_ad(3, i, k) = 0.0_FVPRC
              CALL POPREALARRAY_ADM(a4(2, i, k))
              a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(2, i, k)
              a4_ad(2, i, k) = 0.0_FVPRC
              GOTO 110
            END IF
          END IF
          CALL POPREALARRAY_ADM(a4(4, i, k))
          a4_ad(1, i, k) = a4_ad(1, i, k) + 6.*a4_ad(4, i, k)
          a4_ad(2, i, k) = a4_ad(2, i, k) - 3.*a4_ad(4, i, k)
          a4_ad(3, i, k) = a4_ad(3, i, k) - 3.*a4_ad(4, i, k)
          a4_ad(4, i, k) = 0.0_FVPRC
 110    CONTINUE
      ELSE IF (branch .EQ. 4) THEN
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(a4(4, i, k))
          temp_ad17 = 3.*a4_ad(4, i, k)
          a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad17
          a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad17
          a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad17
          a4_ad(4, i, k) = 0.0_FVPRC
        END DO
      ELSE
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(a4(4, i, k))
            temp_ad18 = 3.*a4_ad(4, i, k)
            a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad18
            a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad18
            a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad18
            a4_ad(4, i, k) = 0.0_FVPRC
          ELSE
            CALL POPREALARRAY_ADM(a4(4, i, k))
            a4_ad(4, i, k) = 0.0_FVPRC
            CALL POPREALARRAY_ADM(a4(3, i, k))
            a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(3, i, k)
            a4_ad(3, i, k) = 0.0_FVPRC
            CALL POPREALARRAY_ADM(a4(2, i, k))
            a4_ad(1, i, k) = a4_ad(1, i, k) + a4_ad(2, i, k)
            a4_ad(2, i, k) = 0.0_FVPRC
          END IF
        END DO
      END IF
    END DO
    CALL POPREALARRAY_ADM(a4(1, i1, 2), 8*4/8)
    CALL CS_LIMITERS_ADM(im, extm(i1, 2), a4(1:, i1, 2), a4_ad(1:, i1, 2&
&                  ), 2)
    DO i=i2,i1,-1
      CALL POPREALARRAY_ADM(a4(4, i, 2))
      temp_ad15 = 3.*a4_ad(4, i, 2)
      a4_ad(1, i, 2) = a4_ad(1, i, 2) + 2.*temp_ad15
      a4_ad(2, i, 2) = a4_ad(2, i, 2) - temp_ad15
      a4_ad(3, i, 2) = a4_ad(3, i, 2) - temp_ad15
      a4_ad(4, i, 2) = 0.0_FVPRC
    END DO
    CALL POPCONTROL1B(branch)
    IF (branch .NE. 0) THEN
      CALL POPREALARRAY_ADM(a4(1, i1, 1), 8*4/8)
      CALL CS_LIMITERS_ADM(im, extm(i1, 1), a4(1:, i1, 1), a4_ad(1:, i1&
&                    , 1), 1)
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(4, i, 1))
        temp_ad14 = 3.*a4_ad(4, i, 1)
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + 2.*temp_ad14
        a4_ad(2, i, 1) = a4_ad(2, i, 1) - temp_ad14
        a4_ad(3, i, 1) = a4_ad(3, i, 1) - temp_ad14
        a4_ad(4, i, 1) = 0.0_FVPRC
      END DO
    END IF
    CALL POPCONTROL2B(branch)
    IF (branch .LT. 2) THEN
      IF (branch .EQ. 0) THEN
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(a4(2, i, 1))
          ELSE
            CALL POPREALARRAY_ADM(a4(2, i, 1))
            a4_ad(2, i, 1) = 0.0_FVPRC
          END IF
        END DO
      ELSE
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) THEN
            CALL POPREALARRAY_ADM(a4(2, i, 1))
            a4_ad(2, i, 1) = 0.0_FVPRC
          END IF
        END DO
      END IF
    ELSE IF (branch .EQ. 2) THEN
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(4, i, 1))
        a4_ad(4, i, 1) = 0.0_FVPRC
        CALL POPREALARRAY_ADM(a4(3, i, 1))
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + a4_ad(3, i, 1)
        a4_ad(3, i, 1) = 0.0_FVPRC
        CALL POPREALARRAY_ADM(a4(2, i, 1))
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + a4_ad(2, i, 1)
        a4_ad(2, i, 1) = 0.0_FVPRC
      END DO
    END IF
    DO k=km-1,2,-1
      DO i=i2,i1,-1
        CALL POPCONTROL1B(branch)
      END DO
    END DO
    q_ad = 0.0_FVPRC
    DO k=km,1,-1
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(3, i, k))
        q_ad(i, k+1) = q_ad(i, k+1) + a4_ad(3, i, k)
        a4_ad(3, i, k) = 0.0_FVPRC
        CALL POPREALARRAY_ADM(a4(2, i, k))
        q_ad(i, k) = q_ad(i, k) + a4_ad(2, i, k)
        a4_ad(2, i, k) = 0.0_FVPRC
      END DO
    END DO
    DO i=i2,i1,-1
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        y8_ad = q_ad(i, km)
        q_ad(i, km) = 0.0_FVPRC
      ELSE
        y8_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, km) = a4_ad(1, i, km) + y8_ad
      ELSE
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + y8_ad
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(q(i, km))
        y7_ad = q_ad(i, km)
        q_ad(i, km) = 0.0_FVPRC
      ELSE
        CALL POPREALARRAY_ADM(q(i, km))
        y7_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, km) = a4_ad(1, i, km) + y7_ad
      ELSE
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + y7_ad
      END IF
    END DO
    DO k=km-1,3,-1
      DO 130 i=i2,i1,-1
        CALL POPCONTROL3B(branch)
        IF (branch .NE. 0) THEN
          IF (branch .LT. 4) THEN
            IF (branch .EQ. 1) THEN
              GOTO 120
            ELSE IF (branch .EQ. 2) THEN
              q_ad(i, k) = 0.0_FVPRC
              GOTO 120
            ELSE
              CALL POPREALARRAY_ADM(q(i, k))
              y5_ad = q_ad(i, k)
              q_ad(i, k) = 0.0_FVPRC
            END IF
          ELSE IF (branch .EQ. 4) THEN
            CALL POPREALARRAY_ADM(q(i, k))
            y5_ad = 0.0_FVPRC
          ELSE
            IF (branch .EQ. 5) THEN
              y4_ad = q_ad(i, k)
              q_ad(i, k) = 0.0_FVPRC
            ELSE
              y4_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              a4_ad(1, i, k) = a4_ad(1, i, k) + y4_ad
            ELSE
              a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + y4_ad
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREALARRAY_ADM(q(i, k))
              y3_ad = q_ad(i, k)
              q_ad(i, k) = 0.0_FVPRC
            ELSE
              CALL POPREALARRAY_ADM(q(i, k))
              y3_ad = 0.0_FVPRC
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              a4_ad(1, i, k) = a4_ad(1, i, k) + y3_ad
            ELSE
              a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + y3_ad
            END IF
            GOTO 130
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            a4_ad(1, i, k) = a4_ad(1, i, k) + y5_ad
          ELSE
            a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + y5_ad
          END IF
          GOTO 130
        END IF
 120    CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREALARRAY_ADM(q(i, k))
          y6_ad = q_ad(i, k)
          q_ad(i, k) = 0.0_FVPRC
        ELSE
          CALL POPREALARRAY_ADM(q(i, k))
          y6_ad = 0.0_FVPRC
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          a4_ad(1, i, k) = a4_ad(1, i, k) + y6_ad
        ELSE
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + y6_ad
        END IF
 130  CONTINUE
    END DO
    DO k=km,2,-1
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(gam(i, k))
        a4_ad(1, i, k) = a4_ad(1, i, k) + gam_ad(i, k)
        a4_ad(1, i, k-1) = a4_ad(1, i, k-1) - gam_ad(i, k)
        gam_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
    DO i=i2,i1,-1
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        y2_ad = q_ad(i, 2)
        q_ad(i, 2) = 0.0_FVPRC
      ELSE
        y2_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + y2_ad
      ELSE
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + y2_ad
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(q(i, 2))
        y1_ad = q_ad(i, 2)
        q_ad(i, 2) = 0.0_FVPRC
      ELSE
        CALL POPREALARRAY_ADM(q(i, 2))
        y1_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + y1_ad
      ELSE
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + y1_ad
      END IF
    END DO
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      DO k=1,km-1,1
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(q(i, k))
          gam_ad(i, k+1) = gam_ad(i, k+1) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k+1)*q_ad(i, k)
        END DO
      END DO
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(q(i, km+1))
        qs_ad(i) = qs_ad(i) + q_ad(i, km+1)
        q_ad(i, km+1) = 0.0_FVPRC
        grat = delp(i, km-1)/delp(i, km)
        CALL POPREALARRAY_ADM(q(i, km))
        temp = 2*grat - gam(i, km) + 2.
        temp_ad1 = q_ad(i, km)/temp
        temp_ad2 = -((3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, &
&         km-1))*temp_ad1/temp)
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + 3.*temp_ad1
        a4_ad(1, i, km) = a4_ad(1, i, km) + 3.*temp_ad1
        grat_ad = 2*temp_ad2 - qs(i)*temp_ad1
        qs_ad(i) = qs_ad(i) - grat*temp_ad1
        q_ad(i, km-1) = q_ad(i, km-1) - temp_ad1
        gam_ad(i, km) = gam_ad(i, km) - temp_ad2
        q_ad(i, km) = 0.0_FVPRC
        temp_ad3 = grat_ad/delp(i, km)
        delp_ad(i, km-1) = delp_ad(i, km-1) + temp_ad3
        delp_ad(i, km) = delp_ad(i, km) - delp(i, km-1)*temp_ad3/delp(i&
&         , km)
      END DO
      DO k=km-1,2,-1
        DO i=i2,i1,-1
          temp_ad = q_ad(i, k)/bet
          CALL POPREALARRAY_ADM(q(i, k))
          grat = delp(i, k-1)/delp(i, k)
          bet_ad = -((3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))*temp_ad/&
&           bet) - grat*gam_ad(i, k+1)/bet**2
          grat_ad = 2*bet_ad + gam_ad(i, k+1)/bet
          gam_ad(i, k+1) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + 3.*temp_ad
          a4_ad(1, i, k) = a4_ad(1, i, k) + 3.*temp_ad
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY_ADM(bet)
          gam_ad(i, k) = gam_ad(i, k) - bet_ad
          temp_ad0 = grat_ad/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad0
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad0/delp(i, &
&           k)
        END DO
      END DO
      DO i=i2,i1,-1
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + 1.5*q_ad(i, 1)
        q_ad(i, 1) = 0.0_FVPRC
      END DO
    ELSE
      DO k=1,km,1
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(q(i, k))
          gam_ad(i, k) = gam_ad(i, k) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k)*q_ad(i, k)
        END DO
      END DO
      d4_ad = 0.0_FVPRC
      DO i=i2,i1,-1
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL POPREALARRAY_ADM(q(i, km+1))
        temp2 = d4(i)*(d4(i)+0.5) - a_bot*gam(i, km)
        temp_ad11 = q_ad(i, km+1)/temp2
        temp1 = d4(i)*(d4(i)+1.)
        temp_ad12 = 2.*a4(1, i, km)*temp_ad11
        temp_ad13 = -((2.*(temp1*a4(1, i, km))+a4(1, i, km-1)-a_bot*q(i&
&         , km))*temp_ad11/temp2)
        a4_ad(1, i, km) = a4_ad(1, i, km) + 2.*temp1*temp_ad11
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + temp_ad11
        a_bot_ad = -(gam(i, km)*temp_ad13) - q(i, km)*temp_ad11
        d4_ad(i) = d4_ad(i) + (2*d4(i)+1.5)*a_bot_ad + (2*d4(i)+0.5)*&
&         temp_ad13 + (2*d4(i)+1.)*temp_ad12
        q_ad(i, km) = q_ad(i, km) - a_bot*temp_ad11
        gam_ad(i, km) = gam_ad(i, km) - a_bot*temp_ad13
        q_ad(i, km+1) = 0.0_FVPRC
      END DO
      DO k=km,2,-1
        DO i=i2,i1,-1
          temp_ad9 = q_ad(i, k)/bet
          temp_ad8 = 3.*temp_ad9
          CALL POPREALARRAY_ADM(q(i, k))
          bet_ad = -((3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))*&
&           temp_ad9/bet) - d4(i)*gam_ad(i, k)/bet**2
          d4_ad(i) = d4_ad(i) + a4(1, i, k)*temp_ad8 + 2*bet_ad + gam_ad&
&           (i, k)/bet
          gam_ad(i, k) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + temp_ad8
          a4_ad(1, i, k) = a4_ad(1, i, k) + d4(i)*temp_ad8
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad9
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY_ADM(bet)
          gam_ad(i, k-1) = gam_ad(i, k-1) - bet_ad
          CALL POPREALARRAY_ADM(d4(i))
          temp_ad10 = d4_ad(i)/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad10
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad10/delp(i&
&           , k)
          d4_ad(i) = 0.0_FVPRC
        END DO
      END DO
      DO i=i2,i1,-1
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        temp_ad4 = gam_ad(i, 1)/bet
        gam_ad(i, 1) = 0.0_FVPRC
        temp_ad6 = q_ad(i, 1)/bet
        temp_ad5 = a4(1, i, 1)*temp_ad6
        temp0 = 2*grat*(grat+1.)
        bet_ad = -((temp0*a4(1, i, 1)+a4(1, i, 2))*temp_ad6/bet) - (grat&
&         *(grat+1.5)+1.)*temp_ad4/bet
        grat_ad = (4*grat+2*1.)*temp_ad5 + (2*grat+0.5)*bet_ad + (2*grat&
&         +1.5)*temp_ad4
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + temp0*temp_ad6
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + temp_ad6
        q_ad(i, 1) = 0.0_FVPRC
        temp_ad7 = grat_ad/delp(i, 1)
        delp_ad(i, 2) = delp_ad(i, 2) + temp_ad7
        delp_ad(i, 1) = delp_ad(i, 1) - delp(i, 2)*temp_ad7/delp(i, 1)
      END DO
    END IF
  END SUBROUTINE CS_PROFILE_ADM
!  Differentiation of cs_limiters in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_ord4
! a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_
!mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_u
!pdate dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_super 
!fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlon fv_g
!rid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_mapz_mo
!d.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2_f
!b fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tracer2d_mo
!d.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_core_m
!od.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.sim_sol
!ver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest 
!sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.compute_di
!v_damping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy_corne
!rs_fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyppm 
!tp_core_mod.deln_flux)):
!   gradient     of useful results: a4
!   with respect to varying inputs: a4
  SUBROUTINE CS_LIMITERS_ADM(im, extm, a4, a4_ad, iv)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: im
    INTEGER, INTENT(IN) :: iv
    LOGICAL, INTENT(IN) :: extm(im)
! PPM array
    REAL(fvprc), INTENT(INOUT) :: a4(4, im)
    REAL(fvprc), INTENT(INOUT) :: a4_ad(4, im)
! !LOCAL VARIABLES:
    REAL(fvprc) :: da1, da2, a6da
    INTEGER :: i
    INTRINSIC ABS
    REAL(fvprc) :: abs0
    INTEGER :: branch
    IF (iv .EQ. 0) THEN
! Positive definite constraint
      DO i=1,im
        IF (a4(1, i) .LE. 0.) THEN
          CALL PUSHCONTROL3B(5)
        ELSE
          IF (a4(3, i) - a4(2, i) .GE. 0.) THEN
            abs0 = a4(3, i) - a4(2, i)
          ELSE
            abs0 = -(a4(3, i)-a4(2, i))
          END IF
          IF (abs0 .LT. -a4(4, i)) THEN
            IF (a4(1, i) + 0.25*(a4(3, i)-a4(2, i))**2/a4(4, i) + a4(4, &
&               i)*r12 .LT. 0.) THEN
! local minimum is negative
              IF (a4(1, i) .LT. a4(3, i) .AND. a4(1, i) .LT. a4(2, i)) &
&             THEN
                CALL PUSHCONTROL3B(4)
              ELSE IF (a4(3, i) .GT. a4(2, i)) THEN
                CALL PUSHCONTROL3B(3)
              ELSE
                CALL PUSHCONTROL3B(2)
              END IF
            ELSE
              CALL PUSHCONTROL3B(1)
            END IF
          ELSE
            CALL PUSHCONTROL3B(0)
          END IF
        END IF
      END DO
      DO i=im,1,-1
        CALL POPCONTROL3B(branch)
        IF (branch .LT. 3) THEN
          IF (branch .NE. 0) THEN
            IF (branch .NE. 1) THEN
              a4_ad(3, i) = a4_ad(3, i) + a4_ad(2, i)
              a4_ad(4, i) = a4_ad(4, i) - a4_ad(2, i)
              a4_ad(2, i) = 0.0_FVPRC
              a4_ad(3, i) = a4_ad(3, i) + 3.*a4_ad(4, i)
              a4_ad(1, i) = a4_ad(1, i) - 3.*a4_ad(4, i)
              a4_ad(4, i) = 0.0_FVPRC
            END IF
          END IF
        ELSE IF (branch .EQ. 3) THEN
          a4_ad(2, i) = a4_ad(2, i) + a4_ad(3, i)
          a4_ad(4, i) = a4_ad(4, i) - a4_ad(3, i)
          a4_ad(3, i) = 0.0_FVPRC
          a4_ad(2, i) = a4_ad(2, i) + 3.*a4_ad(4, i)
          a4_ad(1, i) = a4_ad(1, i) - 3.*a4_ad(4, i)
          a4_ad(4, i) = 0.0_FVPRC
        ELSE IF (branch .EQ. 4) THEN
          a4_ad(4, i) = 0.0_FVPRC
          a4_ad(1, i) = a4_ad(1, i) + a4_ad(2, i)
          a4_ad(2, i) = 0.0_FVPRC
          a4_ad(1, i) = a4_ad(1, i) + a4_ad(3, i)
          a4_ad(3, i) = 0.0_FVPRC
        ELSE
          a4_ad(4, i) = 0.0_FVPRC
          a4_ad(1, i) = a4_ad(1, i) + a4_ad(3, i)
          a4_ad(3, i) = 0.0_FVPRC
          a4_ad(1, i) = a4_ad(1, i) + a4_ad(2, i)
          a4_ad(2, i) = 0.0_FVPRC
        END IF
      END DO
    ELSE IF (iv .EQ. 1) THEN
      DO i=1,im
        IF ((a4(1, i)-a4(2, i))*(a4(1, i)-a4(3, i)) .GE. 0.) THEN
          CALL PUSHCONTROL2B(3)
        ELSE
          da1 = a4(3, i) - a4(2, i)
          da2 = da1**2
          a6da = a4(4, i)*da1
          IF (a6da .LT. -da2) THEN
            CALL PUSHCONTROL2B(2)
          ELSE IF (a6da .GT. da2) THEN
            CALL PUSHCONTROL2B(1)
          ELSE
            CALL PUSHCONTROL2B(0)
          END IF
        END IF
      END DO
      DO i=im,1,-1
        CALL POPCONTROL2B(branch)
        IF (branch .LT. 2) THEN
          IF (branch .NE. 0) THEN
            a4_ad(3, i) = a4_ad(3, i) + a4_ad(2, i)
            a4_ad(4, i) = a4_ad(4, i) - a4_ad(2, i)
            a4_ad(2, i) = 0.0_FVPRC
            a4_ad(3, i) = a4_ad(3, i) + 3.*a4_ad(4, i)
            a4_ad(1, i) = a4_ad(1, i) - 3.*a4_ad(4, i)
            a4_ad(4, i) = 0.0_FVPRC
          END IF
        ELSE IF (branch .EQ. 2) THEN
          a4_ad(2, i) = a4_ad(2, i) + a4_ad(3, i)
          a4_ad(4, i) = a4_ad(4, i) - a4_ad(3, i)
          a4_ad(3, i) = 0.0_FVPRC
          a4_ad(2, i) = a4_ad(2, i) + 3.*a4_ad(4, i)
          a4_ad(1, i) = a4_ad(1, i) - 3.*a4_ad(4, i)
          a4_ad(4, i) = 0.0_FVPRC
        ELSE
          a4_ad(4, i) = 0.0_FVPRC
          a4_ad(1, i) = a4_ad(1, i) + a4_ad(3, i)
          a4_ad(3, i) = 0.0_FVPRC
          a4_ad(1, i) = a4_ad(1, i) + a4_ad(2, i)
          a4_ad(2, i) = 0.0_FVPRC
        END IF
      END DO
    ELSE
! Standard PPM constraint
      DO i=1,im
        IF (extm(i)) THEN
          CALL PUSHCONTROL2B(3)
        ELSE
          da1 = a4(3, i) - a4(2, i)
          da2 = da1**2
          a6da = a4(4, i)*da1
          IF (a6da .LT. -da2) THEN
            CALL PUSHCONTROL2B(2)
          ELSE IF (a6da .GT. da2) THEN
            CALL PUSHCONTROL2B(1)
          ELSE
            CALL PUSHCONTROL2B(0)
          END IF
        END IF
      END DO
      DO i=im,1,-1
        CALL POPCONTROL2B(branch)
        IF (branch .LT. 2) THEN
          IF (branch .NE. 0) THEN
            a4_ad(3, i) = a4_ad(3, i) + a4_ad(2, i)
            a4_ad(4, i) = a4_ad(4, i) - a4_ad(2, i)
            a4_ad(2, i) = 0.0_FVPRC
            a4_ad(3, i) = a4_ad(3, i) + 3.*a4_ad(4, i)
            a4_ad(1, i) = a4_ad(1, i) - 3.*a4_ad(4, i)
            a4_ad(4, i) = 0.0_FVPRC
          END IF
        ELSE IF (branch .EQ. 2) THEN
          a4_ad(2, i) = a4_ad(2, i) + a4_ad(3, i)
          a4_ad(4, i) = a4_ad(4, i) - a4_ad(3, i)
          a4_ad(3, i) = 0.0_FVPRC
          a4_ad(2, i) = a4_ad(2, i) + 3.*a4_ad(4, i)
          a4_ad(1, i) = a4_ad(1, i) - 3.*a4_ad(4, i)
          a4_ad(4, i) = 0.0_FVPRC
        ELSE
          a4_ad(4, i) = 0.0_FVPRC
          a4_ad(1, i) = a4_ad(1, i) + a4_ad(3, i)
          a4_ad(3, i) = 0.0_FVPRC
          a4_ad(1, i) = a4_ad(1, i) + a4_ad(2, i)
          a4_ad(2, i) = 0.0_FVPRC
        END IF
      END DO
    END IF
  END SUBROUTINE CS_LIMITERS_ADM
!  Differentiation of ppm_profile in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_ord4
! a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_
!mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_u
!pdate dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_super 
!fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlon fv_g
!rid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_mapz_mo
!d.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2_f
!b fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tracer2d_mo
!d.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_core_m
!od.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.sim_sol
!ver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest 
!sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.compute_di
!v_damping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy_corne
!rs_fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyppm 
!tp_core_mod.deln_flux)):
!   gradient     of useful results: delp a4
!   with respect to varying inputs: delp a4
  SUBROUTINE PPM_PROFILE_ADM(a4, a4_ad, delp, delp_ad, km, i1, i2, iv, &
&   kord)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
! iv = 2: temp (if remap_t) and w (iv=-2)
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! Order (or more accurately method no.):
    INTEGER, INTENT(IN) :: kord
! 
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
    REAL(fvprc) :: delp_ad(i1:i2, km)
! !INPUT/OUTPUT PARAMETERS:
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4_ad(4, i1:i2, km)
! DESCRIPTION:
!
!   Perform the piecewise parabolic reconstruction
! 
! !REVISION HISTORY: 
! S.-J. Lin   revised at GFDL 2007
!-----------------------------------------------------------------------
! local arrays:
    REAL(fvprc) :: dc(i1:i2, km)
    REAL(fvprc) :: dc_ad(i1:i2, km)
    REAL(fvprc) :: h2(i1:i2, km)
    REAL(fvprc) :: h2_ad(i1:i2, km)
    REAL(fvprc) :: delq(i1:i2, km)
    REAL(fvprc) :: delq_ad(i1:i2, km)
    REAL(fvprc) :: df2(i1:i2, km)
    REAL(fvprc) :: df2_ad(i1:i2, km)
    REAL(fvprc) :: d4(i1:i2, km)
    REAL(fvprc) :: d4_ad(i1:i2, km)
! local scalars:
    INTEGER :: i, k, km1, lmt, it
    REAL(fvprc) :: fac
    REAL(fvprc) :: a1, a2, c1, c2, c3, d1, d2
    REAL(fvprc) :: a1_ad, a2_ad, c1_ad, c2_ad, c3_ad, d1_ad, d2_ad
    REAL(fvprc) :: qm, dq, lac, qmp, pmp
    REAL(fvprc) :: qm_ad, dq_ad, lac_ad, qmp_ad, pmp_ad
    INTRINSIC ABS
    INTRINSIC MAX
    INTRINSIC MIN
    INTRINSIC SIGN
    REAL(fvprc) :: min1
    REAL(fvprc) :: min1_ad
    INTEGER :: abs0
    REAL(fvprc) :: max1
    REAL(fvprc) :: max1_ad
    REAL(fvprc) :: min2
    REAL(fvprc) :: min2_ad
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: x1_ad
    REAL(fvprc) :: y1_ad
    REAL(fvprc) :: z1_ad
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp2
    REAL(fvprc) :: temp3
    REAL(fvprc) :: temp4
    REAL(fvprc) :: temp5
    REAL(fvprc) :: temp6
    REAL(fvprc) :: temp7
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp8
    REAL(fvprc) :: temp_ad11
    REAL(fvprc) :: temp_ad12
    REAL(fvprc) :: temp_ad13
    REAL(fvprc) :: temp_ad14
    REAL(fvprc) :: temp_ad15
    REAL(fvprc) :: temp_ad16
    REAL(fvprc) :: temp_ad17
    REAL(fvprc) :: temp_ad18
    REAL(fvprc) :: temp_ad19
    REAL(fvprc) :: temp_ad20
    REAL(fvprc) :: temp_ad21
    REAL(fvprc) :: y2_ad
    REAL(fvprc) :: y3_ad
    REAL(fvprc) :: temp9
    REAL(fvprc) :: temp_ad22
    REAL(fvprc) :: temp_ad23
    REAL(fvprc) :: temp_ad24
    REAL(fvprc) :: temp_ad25
    REAL(fvprc) :: temp_ad26
    REAL(fvprc) :: temp_ad27
    REAL(fvprc) :: temp_ad28
    REAL(fvprc) :: temp_ad29
    REAL(fvprc) :: temp_ad30
    REAL(fvprc) :: temp_ad31
    REAL(fvprc) :: temp_ad32
    REAL(fvprc) :: y4_ad
    REAL(fvprc) :: y5_ad
    REAL(fvprc) :: temp_ad33
    REAL(fvprc) :: temp10
    REAL(fvprc) :: temp11
    REAL(fvprc) :: temp12
    REAL(fvprc) :: temp13
    REAL(fvprc) :: temp14
    REAL(fvprc) :: temp15
    REAL(fvprc) :: temp16
    REAL(fvprc) :: temp_ad34
    REAL(fvprc) :: temp_ad35
    REAL(fvprc) :: temp_ad36
    REAL(fvprc) :: y8_ad
    REAL(fvprc) :: x2_ad
    REAL(fvprc) :: y6_ad
    REAL(fvprc) :: y9_ad
    REAL(fvprc) :: x3_ad
    REAL(fvprc) :: y7_ad
    REAL(fvprc) :: temp_ad37
    REAL(fvprc) :: temp_ad38
    REAL(fvprc) :: temp_ad39
    INTEGER :: branch
    REAL(fvprc) :: x3
    REAL(fvprc) :: x2
    REAL(fvprc) :: x1
    REAL(fvprc) :: z1
    REAL(fvprc) :: y9
    REAL(fvprc) :: y8
    REAL(fvprc) :: y7
    REAL(fvprc) :: y6
    REAL(fvprc) :: y5
    REAL(fvprc) :: y4
    REAL(fvprc) :: y3
    REAL(fvprc) :: y2
    REAL(fvprc) :: y1
    km1 = km - 1
    it = i2 - i1 + 1
    DO k=2,km
      DO i=i1,i2
        delq(i, k-1) = a4(1, i, k) - a4(1, i, k-1)
        d4(i, k) = delp(i, k-1) + delp(i, k)
      END DO
    END DO
    DO k=2,km1
      DO i=i1,i2
        c1 = (delp(i, k-1)+0.5*delp(i, k))/d4(i, k+1)
        c2 = (delp(i, k+1)+0.5*delp(i, k))/d4(i, k)
        df2(i, k) = delp(i, k)*(c1*delq(i, k)+c2*delq(i, k-1))/(d4(i, k)&
&         +delp(i, k+1))
        IF (df2(i, k) .GE. 0.) THEN
          x1 = df2(i, k)
          CALL PUSHCONTROL1B(0)
        ELSE
          x1 = -df2(i, k)
          CALL PUSHCONTROL1B(1)
        END IF
        IF (a4(1, i, k-1) .LT. a4(1, i, k)) THEN
          IF (a4(1, i, k) .LT. a4(1, i, k+1)) THEN
            max1 = a4(1, i, k+1)
            CALL PUSHCONTROL2B(0)
          ELSE
            max1 = a4(1, i, k)
            CALL PUSHCONTROL2B(1)
          END IF
        ELSE IF (a4(1, i, k-1) .LT. a4(1, i, k+1)) THEN
          max1 = a4(1, i, k+1)
          CALL PUSHCONTROL2B(2)
        ELSE
          max1 = a4(1, i, k-1)
          CALL PUSHCONTROL2B(3)
        END IF
        y1 = max1 - a4(1, i, k)
        IF (a4(1, i, k-1) .GT. a4(1, i, k)) THEN
          IF (a4(1, i, k) .GT. a4(1, i, k+1)) THEN
            min2 = a4(1, i, k+1)
            CALL PUSHCONTROL2B(0)
          ELSE
            min2 = a4(1, i, k)
            CALL PUSHCONTROL2B(1)
          END IF
        ELSE IF (a4(1, i, k-1) .GT. a4(1, i, k+1)) THEN
          min2 = a4(1, i, k+1)
          CALL PUSHCONTROL2B(2)
        ELSE
          min2 = a4(1, i, k-1)
          CALL PUSHCONTROL2B(3)
        END IF
        z1 = a4(1, i, k) - min2
        IF (x1 .GT. y1) THEN
          IF (y1 .GT. z1) THEN
            CALL PUSHREALARRAY_ADM(min1)
            min1 = z1
            CALL PUSHCONTROL2B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(min1)
            min1 = y1
            CALL PUSHCONTROL2B(1)
          END IF
        ELSE IF (x1 .GT. z1) THEN
          CALL PUSHREALARRAY_ADM(min1)
          min1 = z1
          CALL PUSHCONTROL2B(2)
        ELSE
          CALL PUSHREALARRAY_ADM(min1)
          min1 = x1
          CALL PUSHCONTROL2B(3)
        END IF
        dc(i, k) = SIGN(min1, df2(i, k))
      END DO
    END DO
!-----------------------------------------------------------
! 4th order interpolation of the provisional cell edge value
!-----------------------------------------------------------
    DO k=3,km1
      DO i=i1,i2
        c1 = delq(i, k-1)*delp(i, k-1)/d4(i, k)
        a1 = d4(i, k-1)/(d4(i, k)+delp(i, k-1))
        a2 = d4(i, k+1)/(d4(i, k)+delp(i, k))
        a4(2, i, k) = a4(1, i, k-1) + c1 + 2./(d4(i, k-1)+d4(i, k+1))*(&
&         delp(i, k)*(c1*(a1-a2)+a2*dc(i, k-1))-delp(i, k-1)*a1*dc(i, k)&
&         )
      END DO
    END DO
    IF (km .GT. 8 .AND. kord .GT. 4) THEN
      CALL STEEPZ_FWD(i1, i2, km, a4, df2, dc, delq, delp, d4)
      CALL PUSHCONTROL1B(1)
    ELSE
      CALL PUSHCONTROL1B(0)
    END IF
! Area preserving cubic with 2nd deriv. = 0 at the boundaries
! Top
    DO i=i1,i2
      d1 = delp(i, 1)
      d2 = delp(i, 2)
      CALL PUSHREALARRAY_ADM(qm)
      qm = (d2*a4(1, i, 1)+d1*a4(1, i, 2))/(d1+d2)
      CALL PUSHREALARRAY_ADM(dq)
      dq = 2.*(a4(1, i, 2)-a4(1, i, 1))/(d1+d2)
      CALL PUSHREALARRAY_ADM(c1)
      c1 = 4.*(a4(2, i, 3)-qm-d2*dq)/(d2*(2.*d2*d2+d1*(d2+3.*d1)))
      c3 = dq - 0.5*c1*(d2*(5.*d1+d2)-3.*d1*d1)
      CALL PUSHREALARRAY_ADM(a4(2, i, 2))
      a4(2, i, 2) = qm - 0.25*c1*d1*d2*(d2+3.*d1)
! Top edge:
!-------------------------------------------------------
      CALL PUSHREALARRAY_ADM(a4(2, i, 1))
      a4(2, i, 1) = d1*(2.*c1*d1**2-c3) + a4(2, i, 2)
      IF (a4(1, i, 1) .GT. a4(1, i, 2)) THEN
        y2 = a4(1, i, 2)
        CALL PUSHCONTROL1B(0)
      ELSE
        y2 = a4(1, i, 1)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (a4(2, i, 2) .LT. y2) THEN
        CALL PUSHREALARRAY_ADM(a4(2, i, 2))
        a4(2, i, 2) = y2
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREALARRAY_ADM(a4(2, i, 2))
        a4(2, i, 2) = a4(2, i, 2)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (a4(1, i, 1) .LT. a4(1, i, 2)) THEN
        y3 = a4(1, i, 2)
        CALL PUSHCONTROL1B(0)
      ELSE
        y3 = a4(1, i, 1)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (a4(2, i, 2) .GT. y3) THEN
        CALL PUSHREALARRAY_ADM(a4(2, i, 2))
        a4(2, i, 2) = y3
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREALARRAY_ADM(a4(2, i, 2))
        a4(2, i, 2) = a4(2, i, 2)
        CALL PUSHCONTROL1B(1)
      END IF
      CALL PUSHREALARRAY_ADM(dc(i, 1))
      dc(i, 1) = 0.5*(a4(2, i, 2)-a4(1, i, 1))
    END DO
! Enforce monotonicity  within the top layer
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(2, i, 1)) THEN
          CALL PUSHREALARRAY_ADM(a4(2, i, 1))
          a4(2, i, 1) = a4(2, i, 1)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHREALARRAY_ADM(a4(2, i, 1))
          a4(2, i, 1) = 0.
          CALL PUSHCONTROL1B(1)
        END IF
        IF (0. .LT. a4(2, i, 2)) THEN
          CALL PUSHREALARRAY_ADM(a4(2, i, 2))
          a4(2, i, 2) = a4(2, i, 2)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHREALARRAY_ADM(a4(2, i, 2))
          a4(2, i, 2) = 0.
          CALL PUSHCONTROL1B(1)
        END IF
      END DO
      CALL PUSHCONTROL2B(3)
    ELSE IF (iv .EQ. -1) THEN
      DO i=i1,i2
        IF (a4(2, i, 1)*a4(1, i, 1) .LE. 0.) THEN
          CALL PUSHREALARRAY_ADM(a4(2, i, 1))
          a4(2, i, 1) = 0.
          CALL PUSHCONTROL1B(1)
        ELSE
          CALL PUSHCONTROL1B(0)
        END IF
      END DO
      CALL PUSHCONTROL2B(2)
    ELSE
      IF (iv .GE. 0.) THEN
        abs0 = iv
      ELSE
        abs0 = -iv
      END IF
      IF (abs0 .EQ. 2) THEN
        DO i=i1,i2
          CALL PUSHREALARRAY_ADM(a4(2, i, 1))
          a4(2, i, 1) = a4(1, i, 1)
          CALL PUSHREALARRAY_ADM(a4(3, i, 1))
          a4(3, i, 1) = a4(1, i, 1)
        END DO
        CALL PUSHCONTROL2B(1)
      ELSE
        CALL PUSHCONTROL2B(0)
      END IF
    END IF
! Bottom
! Area preserving cubic with 2nd deriv. = 0 at the surface
    DO i=i1,i2
      d1 = delp(i, km)
      d2 = delp(i, km1)
      CALL PUSHREALARRAY_ADM(qm)
      qm = (d2*a4(1, i, km)+d1*a4(1, i, km1))/(d1+d2)
      CALL PUSHREALARRAY_ADM(dq)
      dq = 2.*(a4(1, i, km1)-a4(1, i, km))/(d1+d2)
      CALL PUSHREALARRAY_ADM(c1)
      c1 = (a4(2, i, km1)-qm-d2*dq)/(d2*(2.*d2*d2+d1*(d2+3.*d1)))
      c3 = dq - 2.0*c1*(d2*(5.*d1+d2)-3.*d1*d1)
      CALL PUSHREALARRAY_ADM(a4(2, i, km))
      a4(2, i, km) = qm - c1*d1*d2*(d2+3.*d1)
! Bottom edge:
!-----------------------------------------------------
      CALL PUSHREALARRAY_ADM(a4(3, i, km))
      a4(3, i, km) = d1*(8.*c1*d1**2-c3) + a4(2, i, km)
      IF (a4(1, i, km) .GT. a4(1, i, km1)) THEN
        y4 = a4(1, i, km1)
        CALL PUSHCONTROL1B(0)
      ELSE
        y4 = a4(1, i, km)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (a4(2, i, km) .LT. y4) THEN
        CALL PUSHREALARRAY_ADM(a4(2, i, km))
        a4(2, i, km) = y4
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREALARRAY_ADM(a4(2, i, km))
        a4(2, i, km) = a4(2, i, km)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (a4(1, i, km) .LT. a4(1, i, km1)) THEN
        y5 = a4(1, i, km1)
        CALL PUSHCONTROL1B(0)
      ELSE
        y5 = a4(1, i, km)
        CALL PUSHCONTROL1B(1)
      END IF
      IF (a4(2, i, km) .GT. y5) THEN
        CALL PUSHREALARRAY_ADM(a4(2, i, km))
        a4(2, i, km) = y5
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREALARRAY_ADM(a4(2, i, km))
        a4(2, i, km) = a4(2, i, km)
        CALL PUSHCONTROL1B(1)
      END IF
      CALL PUSHREALARRAY_ADM(dc(i, km))
      dc(i, km) = 0.5*(a4(1, i, km)-a4(2, i, km))
    END DO
! Enforce constraint on the "slope" at the surface
!#ifdef BOT_MONO
!      do i=i1,i2
!         a4(4,i,km) = 0
!         if( a4(3,i,km) * a4(1,i,km) <= 0. ) a4(3,i,km) = 0.
!         d1 = a4(1,i,km) - a4(2,i,km)
!         d2 = a4(3,i,km) - a4(1,i,km)
!         if ( d1*d2 < 0. ) then
!              a4(2,i,km) = a4(1,i,km)
!              a4(3,i,km) = a4(1,i,km)
!         else
!              dq = sign(min(abs(d1),abs(d2),0.5*abs(delq(i,km-1))), d1)
!              a4(2,i,km) = a4(1,i,km) - dq
!              a4(3,i,km) = a4(1,i,km) + dq
!         endif
!      enddo
!#else
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(2, i, km)) THEN
          CALL PUSHREALARRAY_ADM(a4(2, i, km))
          a4(2, i, km) = a4(2, i, km)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHREALARRAY_ADM(a4(2, i, km))
          a4(2, i, km) = 0.
          CALL PUSHCONTROL1B(1)
        END IF
        IF (0. .LT. a4(3, i, km)) THEN
          CALL PUSHREALARRAY_ADM(a4(3, i, km))
          a4(3, i, km) = a4(3, i, km)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHREALARRAY_ADM(a4(3, i, km))
          a4(3, i, km) = 0.
          CALL PUSHCONTROL1B(1)
        END IF
      END DO
      CALL PUSHCONTROL2B(2)
    ELSE IF (iv .LT. 0) THEN
      DO i=i1,i2
        IF (a4(1, i, km)*a4(3, i, km) .LE. 0.) THEN
          CALL PUSHREALARRAY_ADM(a4(3, i, km))
          a4(3, i, km) = 0.
          CALL PUSHCONTROL1B(1)
        ELSE
          CALL PUSHCONTROL1B(0)
        END IF
      END DO
      CALL PUSHCONTROL2B(1)
    ELSE
      CALL PUSHCONTROL2B(0)
    END IF
!#endif
    DO k=1,km1
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(3, i, k))
        a4(3, i, k) = a4(2, i, k+1)
      END DO
    END DO
!-----------------------------------------------------------
! f(s) = AL + s*[(AR-AL) + A6*(1-s)]         ( 0 <= s  <= 1 )
!-----------------------------------------------------------
! Top 2 and bottom 2 layers always use monotonic mapping
    DO k=1,2
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(4, i, k))
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
      CALL PUSHREALARRAY_ADM(a4(1, i1, k), 8*4/8)
      CALL PPM_LIMITERS(dc(i1, k), a4(1, i1, k), it, 0)
    END DO
    IF (kord .GE. 7) THEN
!-----------------------
! Huynh's 2nd constraint
!-----------------------
      DO k=2,km1
        DO i=i1,i2
! Method#1
!           h2(i,k) = delq(i,k) - delq(i,k-1)
! Method#2 - better
          h2(i, k) = 2.*(dc(i, k+1)/delp(i, k+1)-dc(i, k-1)/delp(i, k-1)&
&           )/(delp(i, k)+0.5*(delp(i, k-1)+delp(i, k+1)))*delp(i, k)**2
        END DO
      END DO
! Method#3
!!!            h2(i,k) = dc(i,k+1) - dc(i,k-1)
! original quasi-monotone
      fac = 1.5
      DO k=3,km-2
        DO i=i1,i2
! Right edges
!        qmp   = a4(1,i,k) + 2.0*delq(i,k-1)
!        lac   = a4(1,i,k) + fac*h2(i,k-1) + 0.5*delq(i,k-1)
!
          pmp = 2.*dc(i, k)
          qmp = a4(1, i, k) + pmp
          lac = a4(1, i, k) + fac*h2(i, k-1) + dc(i, k)
          IF (a4(1, i, k) .GT. qmp) THEN
            IF (qmp .GT. lac) THEN
              y8 = lac
              CALL PUSHCONTROL2B(0)
            ELSE
              y8 = qmp
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .GT. lac) THEN
            y8 = lac
            CALL PUSHCONTROL2B(2)
          ELSE
            y8 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (a4(3, i, k) .LT. y8) THEN
            x2 = y8
            CALL PUSHCONTROL1B(0)
          ELSE
            x2 = a4(3, i, k)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (a4(1, i, k) .LT. qmp) THEN
            IF (qmp .LT. lac) THEN
              y6 = lac
              CALL PUSHCONTROL2B(0)
            ELSE
              y6 = qmp
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .LT. lac) THEN
            y6 = lac
            CALL PUSHCONTROL2B(2)
          ELSE
            y6 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (x2 .GT. y6) THEN
            CALL PUSHREALARRAY_ADM(a4(3, i, k))
            a4(3, i, k) = y6
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(a4(3, i, k))
            a4(3, i, k) = x2
            CALL PUSHCONTROL1B(1)
          END IF
! Left  edges
!        qmp   = a4(1,i,k) - 2.0*delq(i,k)
!        lac   = a4(1,i,k) + fac*h2(i,k+1) - 0.5*delq(i,k)
!
          qmp = a4(1, i, k) - pmp
          lac = a4(1, i, k) + fac*h2(i, k+1) - dc(i, k)
          IF (a4(1, i, k) .GT. qmp) THEN
            IF (qmp .GT. lac) THEN
              y9 = lac
              CALL PUSHCONTROL2B(0)
            ELSE
              y9 = qmp
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .GT. lac) THEN
            y9 = lac
            CALL PUSHCONTROL2B(2)
          ELSE
            y9 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (a4(2, i, k) .LT. y9) THEN
            x3 = y9
            CALL PUSHCONTROL1B(0)
          ELSE
            x3 = a4(2, i, k)
            CALL PUSHCONTROL1B(1)
          END IF
          IF (a4(1, i, k) .LT. qmp) THEN
            IF (qmp .LT. lac) THEN
              y7 = lac
              CALL PUSHCONTROL2B(0)
            ELSE
              y7 = qmp
              CALL PUSHCONTROL2B(1)
            END IF
          ELSE IF (a4(1, i, k) .LT. lac) THEN
            y7 = lac
            CALL PUSHCONTROL2B(2)
          ELSE
            y7 = a4(1, i, k)
            CALL PUSHCONTROL2B(3)
          END IF
          IF (x3 .GT. y7) THEN
            CALL PUSHREALARRAY_ADM(a4(2, i, k))
            a4(2, i, k) = y7
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(a4(2, i, k))
            a4(2, i, k) = x3
            CALL PUSHCONTROL1B(1)
          END IF
!-------------
! Recompute A6
!-------------
          CALL PUSHREALARRAY_ADM(a4(4, i, k))
          a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
        END DO
! Additional constraint to ensure positivity when kord=7
        IF (iv .EQ. 0 .AND. kord .GE. 6) THEN
          CALL PUSHREALARRAY_ADM(a4(1, i1, k), 8*4/8)
          CALL PPM_LIMITERS(dc(i1, k), a4(1, i1, k), it, 2)
          CALL PUSHCONTROL1B(1)
        ELSE
          CALL PUSHCONTROL1B(0)
        END IF
      END DO
      CALL PUSHCONTROL1B(1)
    ELSE
      lmt = kord - 3
      IF (0 .LT. lmt) THEN
        lmt = lmt
      ELSE
        lmt = 0
      END IF
      IF (iv .EQ. 0) THEN
        IF (2 .GT. lmt) THEN
          CALL PUSHCONTROL1B(1)
          lmt = lmt
        ELSE
          CALL PUSHCONTROL1B(1)
          lmt = 2
        END IF
      ELSE
        CALL PUSHCONTROL1B(0)
      END IF
      DO k=3,km-2
        IF (kord .NE. 4) THEN
          DO i=i1,i2
            CALL PUSHREALARRAY_ADM(a4(4, i, k))
            a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
          END DO
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHCONTROL1B(1)
        END IF
        IF (kord .NE. 6) THEN
          CALL PUSHREALARRAY_ADM(a4(1, i1, k), 8*4/8)
          CALL PPM_LIMITERS(dc(i1, k), a4(1, i1, k), it, lmt)
          CALL PUSHCONTROL1B(1)
        ELSE
          CALL PUSHCONTROL1B(0)
        END IF
      END DO
      CALL PUSHCONTROL1B(0)
    END IF
    DO k=km1,km
      DO i=i1,i2
        CALL PUSHREALARRAY_ADM(a4(4, i, k))
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
    END DO
    dc_ad = 0.0_FVPRC
    DO k=km,km1,-1
      CALL PPM_LIMITERS_ADM(dc(i1, k), dc_ad(i1, k), a4(1, i1, k), a4_ad&
&                     (1, i1, k), it, 0)
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(4, i, k))
        temp_ad39 = 3.*a4_ad(4, i, k)
        a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad39
        a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad39
        a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad39
        a4_ad(4, i, k) = 0.0_FVPRC
      END DO
    END DO
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      DO k=km-2,3,-1
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) THEN
          CALL POPREALARRAY_ADM(a4(1, i1, k), 8*4/8)
          CALL PPM_LIMITERS_ADM(dc(i1, k), dc_ad(i1, k), a4(1:, i1, k), &
&                         a4_ad(1:, i1, k), it, lmt)
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          DO i=i2,i1,-1
            CALL POPREALARRAY_ADM(a4(4, i, k))
            temp_ad38 = 3.*a4_ad(4, i, k)
            a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad38
            a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad38
            a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad38
            a4_ad(4, i, k) = 0.0_FVPRC
          END DO
        END IF
      END DO
      CALL POPCONTROL1B(branch)
    ELSE
      h2_ad = 0.0_FVPRC
      DO k=km-2,3,-1
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) THEN
          CALL POPREALARRAY_ADM(a4(1, i1, k), 8*4/8)
          CALL PPM_LIMITERS_ADM(dc(i1, k), dc_ad(i1, k), a4(1:, i1, k), &
&                         a4_ad(1:, i1, k), it, 2)
        END IF
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(a4(4, i, k))
          temp_ad37 = 3.*a4_ad(4, i, k)
          a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad37
          a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad37
          a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad37
          a4_ad(4, i, k) = 0.0_FVPRC
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(a4(2, i, k))
            y7_ad = a4_ad(2, i, k)
            a4_ad(2, i, k) = 0.0_FVPRC
            x3_ad = 0.0_FVPRC
          ELSE
            CALL POPREALARRAY_ADM(a4(2, i, k))
            x3_ad = a4_ad(2, i, k)
            a4_ad(2, i, k) = 0.0_FVPRC
            y7_ad = 0.0_FVPRC
          END IF
          CALL POPCONTROL2B(branch)
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
              lac_ad = y7_ad
              qmp_ad = 0.0_FVPRC
            ELSE
              qmp_ad = y7_ad
              lac_ad = 0.0_FVPRC
            END IF
          ELSE
            IF (branch .EQ. 2) THEN
              lac_ad = y7_ad
            ELSE
              a4_ad(1, i, k) = a4_ad(1, i, k) + y7_ad
              lac_ad = 0.0_FVPRC
            END IF
            qmp_ad = 0.0_FVPRC
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            y9_ad = x3_ad
          ELSE
            a4_ad(2, i, k) = a4_ad(2, i, k) + x3_ad
            y9_ad = 0.0_FVPRC
          END IF
          CALL POPCONTROL2B(branch)
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
              lac_ad = lac_ad + y9_ad
            ELSE
              qmp_ad = qmp_ad + y9_ad
            END IF
          ELSE IF (branch .EQ. 2) THEN
            lac_ad = lac_ad + y9_ad
          ELSE
            a4_ad(1, i, k) = a4_ad(1, i, k) + y9_ad
          END IF
          a4_ad(1, i, k) = a4_ad(1, i, k) + qmp_ad + lac_ad
          h2_ad(i, k+1) = h2_ad(i, k+1) + fac*lac_ad
          dc_ad(i, k) = dc_ad(i, k) - lac_ad
          pmp_ad = -qmp_ad
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(a4(3, i, k))
            y6_ad = a4_ad(3, i, k)
            a4_ad(3, i, k) = 0.0_FVPRC
            x2_ad = 0.0_FVPRC
          ELSE
            CALL POPREALARRAY_ADM(a4(3, i, k))
            x2_ad = a4_ad(3, i, k)
            a4_ad(3, i, k) = 0.0_FVPRC
            y6_ad = 0.0_FVPRC
          END IF
          CALL POPCONTROL2B(branch)
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
              lac_ad = y6_ad
              qmp_ad = 0.0_FVPRC
            ELSE
              qmp_ad = y6_ad
              lac_ad = 0.0_FVPRC
            END IF
          ELSE
            IF (branch .EQ. 2) THEN
              lac_ad = y6_ad
            ELSE
              a4_ad(1, i, k) = a4_ad(1, i, k) + y6_ad
              lac_ad = 0.0_FVPRC
            END IF
            qmp_ad = 0.0_FVPRC
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            y8_ad = x2_ad
          ELSE
            a4_ad(3, i, k) = a4_ad(3, i, k) + x2_ad
            y8_ad = 0.0_FVPRC
          END IF
          CALL POPCONTROL2B(branch)
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
              lac_ad = lac_ad + y8_ad
            ELSE
              qmp_ad = qmp_ad + y8_ad
            END IF
          ELSE IF (branch .EQ. 2) THEN
            lac_ad = lac_ad + y8_ad
          ELSE
            a4_ad(1, i, k) = a4_ad(1, i, k) + y8_ad
          END IF
          pmp_ad = pmp_ad + qmp_ad
          a4_ad(1, i, k) = a4_ad(1, i, k) + qmp_ad + lac_ad
          h2_ad(i, k-1) = h2_ad(i, k-1) + fac*lac_ad
          dc_ad(i, k) = dc_ad(i, k) + 2.*pmp_ad + lac_ad
        END DO
      END DO
      DO k=km1,2,-1
        DO i=i2,i1,-1
          temp11 = delp(i, k) + 0.5*(delp(i, k-1)+delp(i, k+1))
          temp16 = delp(i, k)**2
          temp10 = temp16/temp11
          temp15 = delp(i, k-1)
          temp14 = dc(i, k-1)/temp15
          temp13 = delp(i, k+1)
          temp12 = dc(i, k+1)/temp13
          temp_ad34 = 2.*temp10*h2_ad(i, k)
          temp_ad35 = (temp12-temp14)*2.*h2_ad(i, k)/temp11
          temp_ad36 = -(temp10*temp_ad35)
          dc_ad(i, k+1) = dc_ad(i, k+1) + temp_ad34/temp13
          delp_ad(i, k+1) = delp_ad(i, k+1) + 0.5*temp_ad36 - temp12*&
&           temp_ad34/temp13
          dc_ad(i, k-1) = dc_ad(i, k-1) - temp_ad34/temp15
          delp_ad(i, k-1) = delp_ad(i, k-1) + 0.5*temp_ad36 + temp14*&
&           temp_ad34/temp15
          delp_ad(i, k) = delp_ad(i, k) + temp_ad36 + 2*delp(i, k)*&
&           temp_ad35
          h2_ad(i, k) = 0.0_FVPRC
        END DO
      END DO
    END IF
    DO k=2,1,-1
      CALL POPREALARRAY_ADM(a4(1, i1, k), 8*4/8)
      CALL PPM_LIMITERS_ADM(dc(i1, k), dc_ad(i1, k), a4(1:, i1, k), &
&                     a4_ad(1:, i1, k), it, 0)
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(4, i, k))
        temp_ad33 = 3.*a4_ad(4, i, k)
        a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad33
        a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad33
        a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad33
        a4_ad(4, i, k) = 0.0_FVPRC
      END DO
    END DO
    DO k=km1,1,-1
      DO i=i2,i1,-1
        CALL POPREALARRAY_ADM(a4(3, i, k))
        a4_ad(2, i, k+1) = a4_ad(2, i, k+1) + a4_ad(3, i, k)
        a4_ad(3, i, k) = 0.0_FVPRC
      END DO
    END DO
    CALL POPCONTROL2B(branch)
    IF (branch .NE. 0) THEN
      IF (branch .EQ. 1) THEN
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) THEN
            CALL POPREALARRAY_ADM(a4(3, i, km))
            a4_ad(3, i, km) = 0.0_FVPRC
          END IF
        END DO
      ELSE
        DO i=i2,i1,-1
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(a4(3, i, km))
          ELSE
            CALL POPREALARRAY_ADM(a4(3, i, km))
            a4_ad(3, i, km) = 0.0_FVPRC
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(a4(2, i, km))
          ELSE
            CALL POPREALARRAY_ADM(a4(2, i, km))
            a4_ad(2, i, km) = 0.0_FVPRC
          END IF
        END DO
      END IF
    END IF
    DO i=i2,i1,-1
      CALL POPREALARRAY_ADM(dc(i, km))
      a4_ad(1, i, km) = a4_ad(1, i, km) + 0.5*dc_ad(i, km)
      a4_ad(2, i, km) = a4_ad(2, i, km) - 0.5*dc_ad(i, km)
      dc_ad(i, km) = 0.0_FVPRC
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(a4(2, i, km))
        y5_ad = a4_ad(2, i, km)
        a4_ad(2, i, km) = 0.0_FVPRC
      ELSE
        CALL POPREALARRAY_ADM(a4(2, i, km))
        y5_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, km1) = a4_ad(1, i, km1) + y5_ad
      ELSE
        a4_ad(1, i, km) = a4_ad(1, i, km) + y5_ad
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(a4(2, i, km))
        y4_ad = a4_ad(2, i, km)
        a4_ad(2, i, km) = 0.0_FVPRC
      ELSE
        CALL POPREALARRAY_ADM(a4(2, i, km))
        y4_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, km1) = a4_ad(1, i, km1) + y4_ad
      ELSE
        a4_ad(1, i, km) = a4_ad(1, i, km) + y4_ad
      END IF
      d1 = delp(i, km)
      d2 = delp(i, km1)
      c3 = dq - 2.0*c1*(d2*(5.*d1+d2)-3.*d1*d1)
      CALL POPREALARRAY_ADM(a4(3, i, km))
      temp_ad22 = d1*a4_ad(3, i, km)
      d1_ad = c1*8.*2*d1*temp_ad22 + (8.*(c1*d1**2)-c3)*a4_ad(3, i, km)
      c3_ad = -temp_ad22
      a4_ad(2, i, km) = a4_ad(2, i, km) + a4_ad(3, i, km)
      a4_ad(3, i, km) = 0.0_FVPRC
      CALL POPREALARRAY_ADM(a4(2, i, km))
      temp_ad23 = -((d2+3.*d1)*a4_ad(2, i, km))
      c1_ad = d2*d1*temp_ad23 - 2.0*(d2*(5.*d1+d2)-3.*d1**2)*c3_ad + 8.*&
&       d1**2*temp_ad22
      temp_ad24 = -(c1*d1*d2*a4_ad(2, i, km))
      temp_ad26 = -(2.0*c1*c3_ad)
      temp9 = 2.*d2**2 + d1*(d2+3.*d1)
      temp_ad25 = c1_ad/(d2*temp9)
      qm_ad = a4_ad(2, i, km) - temp_ad25
      a4_ad(2, i, km) = 0.0_FVPRC
      dq_ad = c3_ad - d2*temp_ad25
      temp_ad31 = -((a4(2, i, km1)-qm-d2*dq)*temp_ad25/(d2*temp9))
      temp_ad30 = d2*temp_ad31
      a4_ad(2, i, km1) = a4_ad(2, i, km1) + temp_ad25
      temp_ad32 = 2.*dq_ad/(d1+d2)
      temp_ad27 = -((a4(1, i, km1)-a4(1, i, km))*temp_ad32/(d1+d2))
      a4_ad(1, i, km1) = a4_ad(1, i, km1) + temp_ad32
      temp_ad29 = qm_ad/(d1+d2)
      a4_ad(1, i, km) = a4_ad(1, i, km) + d2*temp_ad29 - temp_ad32
      temp_ad28 = -((d2*a4(1, i, km)+d1*a4(1, i, km1))*temp_ad29/(d1+d2)&
&       )
      d1_ad = d1_ad + (d2*5.-3.*2*d1)*temp_ad26 + temp_ad27 + temp_ad28 &
&       + a4(1, i, km1)*temp_ad29 + (d1*3.+d2+3.*d1)*temp_ad30 + 3.*&
&       temp_ad24 + d2*c1*temp_ad23
      d2_ad = (2*d2+5.*d1)*temp_ad26 + temp_ad27 + temp_ad28 + a4(1, i, &
&       km)*temp_ad29 + (d1+2.*2*d2)*temp_ad30 + temp9*temp_ad31 - dq*&
&       temp_ad25 + temp_ad24 + c1*d1*temp_ad23
      CALL POPREALARRAY_ADM(c1)
      CALL POPREALARRAY_ADM(dq)
      CALL POPREALARRAY_ADM(qm)
      a4_ad(1, i, km1) = a4_ad(1, i, km1) + d1*temp_ad29
      delp_ad(i, km1) = delp_ad(i, km1) + d2_ad
      delp_ad(i, km) = delp_ad(i, km) + d1_ad
    END DO
    CALL POPCONTROL2B(branch)
    IF (branch .LT. 2) THEN
      IF (branch .NE. 0) THEN
        DO i=i2,i1,-1
          CALL POPREALARRAY_ADM(a4(3, i, 1))
          a4_ad(1, i, 1) = a4_ad(1, i, 1) + a4_ad(3, i, 1)
          a4_ad(3, i, 1) = 0.0_FVPRC
          CALL POPREALARRAY_ADM(a4(2, i, 1))
          a4_ad(1, i, 1) = a4_ad(1, i, 1) + a4_ad(2, i, 1)
          a4_ad(2, i, 1) = 0.0_FVPRC
        END DO
      END IF
    ELSE IF (branch .EQ. 2) THEN
      DO i=i2,i1,-1
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) THEN
          CALL POPREALARRAY_ADM(a4(2, i, 1))
          a4_ad(2, i, 1) = 0.0_FVPRC
        END IF
      END DO
    ELSE
      DO i=i2,i1,-1
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREALARRAY_ADM(a4(2, i, 2))
        ELSE
          CALL POPREALARRAY_ADM(a4(2, i, 2))
          a4_ad(2, i, 2) = 0.0_FVPRC
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREALARRAY_ADM(a4(2, i, 1))
        ELSE
          CALL POPREALARRAY_ADM(a4(2, i, 1))
          a4_ad(2, i, 1) = 0.0_FVPRC
        END IF
      END DO
    END IF
    DO i=i2,i1,-1
      CALL POPREALARRAY_ADM(dc(i, 1))
      a4_ad(2, i, 2) = a4_ad(2, i, 2) + 0.5*dc_ad(i, 1)
      a4_ad(1, i, 1) = a4_ad(1, i, 1) - 0.5*dc_ad(i, 1)
      dc_ad(i, 1) = 0.0_FVPRC
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(a4(2, i, 2))
        y3_ad = a4_ad(2, i, 2)
        a4_ad(2, i, 2) = 0.0_FVPRC
      ELSE
        CALL POPREALARRAY_ADM(a4(2, i, 2))
        y3_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + y3_ad
      ELSE
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + y3_ad
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY_ADM(a4(2, i, 2))
        y2_ad = a4_ad(2, i, 2)
        a4_ad(2, i, 2) = 0.0_FVPRC
      ELSE
        CALL POPREALARRAY_ADM(a4(2, i, 2))
        y2_ad = 0.0_FVPRC
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + y2_ad
      ELSE
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + y2_ad
      END IF
      d1 = delp(i, 1)
      d2 = delp(i, 2)
      c3 = dq - 0.5*c1*(d2*(5.*d1+d2)-3.*d1*d1)
      CALL POPREALARRAY_ADM(a4(2, i, 1))
      temp_ad11 = d1*a4_ad(2, i, 1)
      d1_ad = c1*2.*2*d1*temp_ad11 + (2.*(c1*d1**2)-c3)*a4_ad(2, i, 1)
      c3_ad = -temp_ad11
      a4_ad(2, i, 2) = a4_ad(2, i, 2) + a4_ad(2, i, 1)
      a4_ad(2, i, 1) = 0.0_FVPRC
      CALL POPREALARRAY_ADM(a4(2, i, 2))
      temp_ad12 = -(0.25*(d2+3.*d1)*a4_ad(2, i, 2))
      c1_ad = d2*d1*temp_ad12 - 0.5*(d2*(5.*d1+d2)-3.*d1**2)*c3_ad + 2.*&
&       d1**2*temp_ad11
      temp_ad13 = -(0.25*c1*d1*d2*a4_ad(2, i, 2))
      temp_ad15 = -(0.5*c1*c3_ad)
      temp8 = 2.*d2**2 + d1*(d2+3.*d1)
      temp_ad14 = 4.*c1_ad/(d2*temp8)
      qm_ad = a4_ad(2, i, 2) - temp_ad14
      a4_ad(2, i, 2) = 0.0_FVPRC
      dq_ad = c3_ad - d2*temp_ad14
      temp_ad20 = -((a4(2, i, 3)-qm-d2*dq)*temp_ad14/(d2*temp8))
      temp_ad19 = d2*temp_ad20
      a4_ad(2, i, 3) = a4_ad(2, i, 3) + temp_ad14
      temp_ad21 = 2.*dq_ad/(d1+d2)
      temp_ad16 = -((a4(1, i, 2)-a4(1, i, 1))*temp_ad21/(d1+d2))
      a4_ad(1, i, 2) = a4_ad(1, i, 2) + temp_ad21
      temp_ad18 = qm_ad/(d1+d2)
      a4_ad(1, i, 1) = a4_ad(1, i, 1) + d2*temp_ad18 - temp_ad21
      temp_ad17 = -((d2*a4(1, i, 1)+d1*a4(1, i, 2))*temp_ad18/(d1+d2))
      d1_ad = d1_ad + (d2*5.-3.*2*d1)*temp_ad15 + temp_ad16 + temp_ad17 &
&       + a4(1, i, 2)*temp_ad18 + (d1*3.+d2+3.*d1)*temp_ad19 + 3.*&
&       temp_ad13 + d2*c1*temp_ad12
      d2_ad = (2*d2+5.*d1)*temp_ad15 + temp_ad16 + temp_ad17 + a4(1, i, &
&       1)*temp_ad18 + (d1+2.*2*d2)*temp_ad19 + temp8*temp_ad20 - dq*&
&       temp_ad14 + temp_ad13 + c1*d1*temp_ad12
      CALL POPREALARRAY_ADM(c1)
      CALL POPREALARRAY_ADM(dq)
      CALL POPREALARRAY_ADM(qm)
      a4_ad(1, i, 2) = a4_ad(1, i, 2) + d1*temp_ad18
      delp_ad(i, 2) = delp_ad(i, 2) + d2_ad
      delp_ad(i, 1) = delp_ad(i, 1) + d1_ad
    END DO
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      df2_ad = 0.0_FVPRC
      delq_ad = 0.0_FVPRC
      d4_ad = 0.0_FVPRC
    ELSE
      CALL STEEPZ_BWD(i1, i2, km, a4, a4_ad, df2, df2_ad, dc, dc_ad, &
&               delq, delq_ad, delp, delp_ad, d4, d4_ad)
    END IF
    DO k=km1,3,-1
      DO i=i2,i1,-1
        temp2 = d4(i, k)
        temp1 = delq(i, k-1)/temp2
        temp4 = d4(i, k) + delp(i, k)
        c1 = delq(i, k-1)*delp(i, k-1)/d4(i, k)
        a1 = d4(i, k-1)/(d4(i, k)+delp(i, k-1))
        a2 = d4(i, k+1)/(d4(i, k)+delp(i, k))
        temp7 = d4(i, k-1) + d4(i, k+1)
        temp6 = a1*dc(i, k)
        temp5 = c1*(a1-a2) + a2*dc(i, k-1)
        temp_ad4 = 2.*a4_ad(2, i, k)/temp7
        temp_ad5 = delp(i, k)*temp_ad4
        temp_ad6 = -(delp(i, k-1)*temp_ad4)
        temp_ad7 = -((delp(i, k)*temp5-delp(i, k-1)*temp6)*temp_ad4/&
&         temp7)
        a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + a4_ad(2, i, k)
        c1_ad = (a1-a2)*temp_ad5 + a4_ad(2, i, k)
        delp_ad(i, k) = delp_ad(i, k) + temp5*temp_ad4
        a1_ad = dc(i, k)*temp_ad6 + c1*temp_ad5
        a2_ad = (dc(i, k-1)-c1)*temp_ad5
        dc_ad(i, k-1) = dc_ad(i, k-1) + a2*temp_ad5
        delp_ad(i, k-1) = delp_ad(i, k-1) - temp6*temp_ad4
        dc_ad(i, k) = dc_ad(i, k) + a1*temp_ad6
        d4_ad(i, k-1) = d4_ad(i, k-1) + temp_ad7
        d4_ad(i, k+1) = d4_ad(i, k+1) + a2_ad/temp4 + temp_ad7
        a4_ad(2, i, k) = 0.0_FVPRC
        temp_ad8 = -(d4(i, k+1)*a2_ad/temp4**2)
        d4_ad(i, k) = d4_ad(i, k) + temp_ad8
        delp_ad(i, k) = delp_ad(i, k) + temp_ad8
        temp3 = d4(i, k) + delp(i, k-1)
        temp_ad9 = -(d4(i, k-1)*a1_ad/temp3**2)
        d4_ad(i, k-1) = d4_ad(i, k-1) + a1_ad/temp3
        delp_ad(i, k-1) = delp_ad(i, k-1) + temp1*c1_ad + temp_ad9
        temp_ad10 = delp(i, k-1)*c1_ad/temp2
        d4_ad(i, k) = d4_ad(i, k) + temp_ad9 - temp1*temp_ad10
        delq_ad(i, k-1) = delq_ad(i, k-1) + temp_ad10
      END DO
    END DO
    DO k=km1,2,-1
      DO i=i2,i1,-1
        min1_ad = SIGN(1.d0, min1*df2(i, k))*dc_ad(i, k)
        dc_ad(i, k) = 0.0_FVPRC
        CALL POPCONTROL2B(branch)
        IF (branch .LT. 2) THEN
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(min1)
            z1_ad = min1_ad
            y1_ad = 0.0_FVPRC
          ELSE
            CALL POPREALARRAY_ADM(min1)
            y1_ad = min1_ad
            z1_ad = 0.0_FVPRC
          END IF
          x1_ad = 0.0_FVPRC
        ELSE
          IF (branch .EQ. 2) THEN
            CALL POPREALARRAY_ADM(min1)
            z1_ad = min1_ad
            x1_ad = 0.0_FVPRC
          ELSE
            CALL POPREALARRAY_ADM(min1)
            x1_ad = min1_ad
            z1_ad = 0.0_FVPRC
          END IF
          y1_ad = 0.0_FVPRC
        END IF
        a4_ad(1, i, k) = a4_ad(1, i, k) + z1_ad
        min2_ad = -z1_ad
        CALL POPCONTROL2B(branch)
        IF (branch .LT. 2) THEN
          IF (branch .EQ. 0) THEN
            a4_ad(1, i, k+1) = a4_ad(1, i, k+1) + min2_ad
          ELSE
            a4_ad(1, i, k) = a4_ad(1, i, k) + min2_ad
          END IF
        ELSE IF (branch .EQ. 2) THEN
          a4_ad(1, i, k+1) = a4_ad(1, i, k+1) + min2_ad
        ELSE
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + min2_ad
        END IF
        max1_ad = y1_ad
        a4_ad(1, i, k) = a4_ad(1, i, k) - y1_ad
        CALL POPCONTROL2B(branch)
        IF (branch .LT. 2) THEN
          IF (branch .EQ. 0) THEN
            a4_ad(1, i, k+1) = a4_ad(1, i, k+1) + max1_ad
          ELSE
            a4_ad(1, i, k) = a4_ad(1, i, k) + max1_ad
          END IF
        ELSE IF (branch .EQ. 2) THEN
          a4_ad(1, i, k+1) = a4_ad(1, i, k+1) + max1_ad
        ELSE
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + max1_ad
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          df2_ad(i, k) = df2_ad(i, k) + x1_ad
        ELSE
          df2_ad(i, k) = df2_ad(i, k) - x1_ad
        END IF
        c1 = (delp(i, k-1)+0.5*delp(i, k))/d4(i, k+1)
        c2 = (delp(i, k+1)+0.5*delp(i, k))/d4(i, k)
        temp0 = d4(i, k) + delp(i, k+1)
        temp = delp(i, k)/temp0
        temp_ad = temp*df2_ad(i, k)
        temp_ad0 = (c1*delq(i, k)+c2*delq(i, k-1))*df2_ad(i, k)/temp0
        temp_ad1 = -(temp*temp_ad0)
        c1_ad = delq(i, k)*temp_ad
        delq_ad(i, k) = delq_ad(i, k) + c1*temp_ad
        c2_ad = delq(i, k-1)*temp_ad
        delq_ad(i, k-1) = delq_ad(i, k-1) + c2*temp_ad
        delp_ad(i, k) = delp_ad(i, k) + temp_ad0
        df2_ad(i, k) = 0.0_FVPRC
        temp_ad2 = c2_ad/d4(i, k)
        d4_ad(i, k) = d4_ad(i, k) + temp_ad1 - (delp(i, k+1)+0.5*delp(i&
&         , k))*temp_ad2/d4(i, k)
        delp_ad(i, k+1) = delp_ad(i, k+1) + temp_ad2 + temp_ad1
        delp_ad(i, k) = delp_ad(i, k) + 0.5*temp_ad2
        temp_ad3 = c1_ad/d4(i, k+1)
        delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad3
        delp_ad(i, k) = delp_ad(i, k) + 0.5*temp_ad3
        d4_ad(i, k+1) = d4_ad(i, k+1) - (delp(i, k-1)+0.5*delp(i, k))*&
&         temp_ad3/d4(i, k+1)
      END DO
    END DO
    DO k=km,2,-1
      DO i=i2,i1,-1
        delp_ad(i, k-1) = delp_ad(i, k-1) + d4_ad(i, k)
        delp_ad(i, k) = delp_ad(i, k) + d4_ad(i, k)
        d4_ad(i, k) = 0.0_FVPRC
        a4_ad(1, i, k) = a4_ad(1, i, k) + delq_ad(i, k-1)
        a4_ad(1, i, k-1) = a4_ad(1, i, k-1) - delq_ad(i, k-1)
        delq_ad(i, k-1) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE PPM_PROFILE_ADM
!  Differentiation of ppm_limiters in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a2b_ord4_f
!b a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core
!_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_
!update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayleigh_super
! fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_latlon fv_
!grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy fv_mapz_m
!od.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2_
!fb fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_tracer2d_m
!od.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c nh_core_
!mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_mod.sim_so
!lver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest
! sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.compute_d
!iv_damping_fb sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.copy_corn
!ers_fb tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod.fyppm
! tp_core_mod.deln_flux)):
!   gradient     of useful results: dm a4
!   with respect to varying inputs: dm a4
  SUBROUTINE PPM_LIMITERS_ADM(dm, dm_ad, a4, a4_ad, itot, lmt)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! the linear slope
    REAL(fvprc), INTENT(IN) :: dm(*)
    REAL(fvprc) :: dm_ad(*)
! Total Longitudes
    INTEGER, INTENT(IN) :: itot
! 0: Standard PPM constraint
    INTEGER, INTENT(IN) :: lmt
! 1: Improved full monotonicity constraint (Lin)
! 2: Positive definite constraint
! 3: do nothing (return immediately)
! !INPUT/OUTPUT PARAMETERS:
! PPM array
    REAL(fvprc), INTENT(INOUT) :: a4(4, *)
    REAL(fvprc), INTENT(INOUT) :: a4_ad(4, *)
! AA <-- a4(1,i)
! AL <-- a4(2,i)
! AR <-- a4(3,i)
! A6 <-- a4(4,i)
! !LOCAL VARIABLES:
    REAL(fvprc) :: qmp
    REAL(fvprc) :: qmp_ad
    REAL(fvprc) :: da1, da2, a6da
    REAL(fvprc) :: fmin
    INTEGER :: i
    INTRINSIC ABS
    INTRINSIC MIN
    INTRINSIC SIGN
    REAL(fvprc) :: min1
    REAL(fvprc) :: min1_ad
    REAL(fvprc) :: min2
    REAL(fvprc) :: min2_ad
    REAL(fvprc) :: abs0
    REAL(fvprc) :: x1_ad
    REAL(fvprc) :: y1_ad
    REAL(fvprc) :: x2_ad
    REAL(fvprc) :: y2_ad
    REAL(fvprc) :: temp_ad
    INTEGER :: branch
    REAL(fvprc) :: x2
    REAL(fvprc) :: x1
    REAL(fvprc) :: y2
    REAL(fvprc) :: y1
! Developer: S.-J. Lin
    IF (lmt .NE. 3) THEN
      IF (lmt .EQ. 0) THEN
! Standard PPM constraint
        DO i=1,itot
          IF (dm(i) .LT. 10.e-10) THEN
            CALL PUSHCONTROL2B(3)
          ELSE
            da1 = a4(3, i) - a4(2, i)
            da2 = da1**2
            a6da = a4(4, i)*da1
            IF (a6da .LT. -da2) THEN
              CALL PUSHCONTROL2B(2)
            ELSE IF (a6da .GT. da2) THEN
              CALL PUSHCONTROL2B(1)
            ELSE
              CALL PUSHCONTROL2B(0)
            END IF
          END IF
        END DO
        DO i=itot,1,-1
          CALL POPCONTROL2B(branch)
          IF (branch .LT. 2) THEN
            IF (branch .NE. 0) THEN
              a4_ad(3, i) = a4_ad(3, i) + a4_ad(2, i)
              a4_ad(4, i) = a4_ad(4, i) - a4_ad(2, i)
              a4_ad(2, i) = 0.0_FVPRC
              a4_ad(3, i) = a4_ad(3, i) + 3.*a4_ad(4, i)
              a4_ad(1, i) = a4_ad(1, i) - 3.*a4_ad(4, i)
              a4_ad(4, i) = 0.0_FVPRC
            END IF
          ELSE IF (branch .EQ. 2) THEN
            a4_ad(2, i) = a4_ad(2, i) + a4_ad(3, i)
            a4_ad(4, i) = a4_ad(4, i) - a4_ad(3, i)
            a4_ad(3, i) = 0.0_FVPRC
            a4_ad(2, i) = a4_ad(2, i) + 3.*a4_ad(4, i)
            a4_ad(1, i) = a4_ad(1, i) - 3.*a4_ad(4, i)
            a4_ad(4, i) = 0.0_FVPRC
          ELSE
            a4_ad(4, i) = 0.0_FVPRC
            a4_ad(1, i) = a4_ad(1, i) + a4_ad(3, i)
            a4_ad(3, i) = 0.0_FVPRC
            a4_ad(1, i) = a4_ad(1, i) + a4_ad(2, i)
            a4_ad(2, i) = 0.0_FVPRC
          END IF
        END DO
      ELSE IF (lmt .EQ. 1) THEN
! Improved full monotonicity constraint (Lin 2004)
! Note: no need to provide first guess of A6 <-- a4(4,i)
        DO i=1,itot
          qmp = 2.*dm(i)
          IF (qmp .GE. 0.) THEN
            x1 = qmp
            CALL PUSHCONTROL1B(0)
          ELSE
            x1 = -qmp
            CALL PUSHCONTROL1B(1)
          END IF
          IF (a4(2, i) - a4(1, i) .GE. 0.) THEN
            y1 = a4(2, i) - a4(1, i)
            CALL PUSHCONTROL1B(0)
          ELSE
            y1 = -(a4(2, i)-a4(1, i))
            CALL PUSHCONTROL1B(1)
          END IF
          IF (x1 .GT. y1) THEN
            CALL PUSHREALARRAY_ADM(min1)
            min1 = y1
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(min1)
            min1 = x1
            CALL PUSHCONTROL1B(1)
          END IF
          CALL PUSHREALARRAY_ADM(a4(2, i))
          a4(2, i) = a4(1, i) - SIGN(min1, qmp)
          IF (qmp .GE. 0.) THEN
            x2 = qmp
            CALL PUSHCONTROL1B(0)
          ELSE
            x2 = -qmp
            CALL PUSHCONTROL1B(1)
          END IF
          IF (a4(3, i) - a4(1, i) .GE. 0.) THEN
            y2 = a4(3, i) - a4(1, i)
            CALL PUSHCONTROL1B(0)
          ELSE
            y2 = -(a4(3, i)-a4(1, i))
            CALL PUSHCONTROL1B(1)
          END IF
          IF (x2 .GT. y2) THEN
            CALL PUSHREALARRAY_ADM(min2)
            min2 = y2
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREALARRAY_ADM(min2)
            min2 = x2
            CALL PUSHCONTROL1B(1)
          END IF
        END DO
        DO i=itot,1,-1
          temp_ad = 3.*a4_ad(4, i)
          a4_ad(1, i) = a4_ad(1, i) + 2.*temp_ad
          a4_ad(2, i) = a4_ad(2, i) - temp_ad
          a4_ad(3, i) = a4_ad(3, i) - temp_ad
          a4_ad(4, i) = 0.0_FVPRC
          qmp = 2.*dm(i)
          a4_ad(1, i) = a4_ad(1, i) + a4_ad(3, i)
          min2_ad = SIGN(1.d0, min2*qmp)*a4_ad(3, i)
          a4_ad(3, i) = 0.0_FVPRC
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(min2)
            y2_ad = min2_ad
            x2_ad = 0.0_FVPRC
          ELSE
            CALL POPREALARRAY_ADM(min2)
            x2_ad = min2_ad
            y2_ad = 0.0_FVPRC
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            a4_ad(3, i) = a4_ad(3, i) + y2_ad
            a4_ad(1, i) = a4_ad(1, i) - y2_ad
          ELSE
            a4_ad(1, i) = a4_ad(1, i) + y2_ad
            a4_ad(3, i) = a4_ad(3, i) - y2_ad
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            qmp_ad = x2_ad
          ELSE
            qmp_ad = -x2_ad
          END IF
          CALL POPREALARRAY_ADM(a4(2, i))
          a4_ad(1, i) = a4_ad(1, i) + a4_ad(2, i)
          min1_ad = -(SIGN(1.d0, min1*qmp)*a4_ad(2, i))
          a4_ad(2, i) = 0.0_FVPRC
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY_ADM(min1)
            y1_ad = min1_ad
            x1_ad = 0.0_FVPRC
          ELSE
            CALL POPREALARRAY_ADM(min1)
            x1_ad = min1_ad
            y1_ad = 0.0_FVPRC
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            a4_ad(2, i) = a4_ad(2, i) + y1_ad
            a4_ad(1, i) = a4_ad(1, i) - y1_ad
          ELSE
            a4_ad(1, i) = a4_ad(1, i) + y1_ad
            a4_ad(2, i) = a4_ad(2, i) - y1_ad
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            qmp_ad = qmp_ad + x1_ad
          ELSE
            qmp_ad = qmp_ad - x1_ad
          END IF
          dm_ad(i) = dm_ad(i) + 2.*qmp_ad
        END DO
      ELSE IF (lmt .EQ. 2) THEN
! Positive definite constraint
        DO i=1,itot
          IF (a4(3, i) - a4(2, i) .GE. 0.) THEN
            abs0 = a4(3, i) - a4(2, i)
          ELSE
            abs0 = -(a4(3, i)-a4(2, i))
          END IF
          IF (abs0 .LT. -a4(4, i)) THEN
            fmin = a4(1, i) + 0.25*(a4(3, i)-a4(2, i))**2/a4(4, i) + a4(&
&             4, i)*r12
            IF (fmin .LT. 0.) THEN
              IF (a4(1, i) .LT. a4(3, i) .AND. a4(1, i) .LT. a4(2, i)) &
&             THEN
                CALL PUSHCONTROL3B(4)
              ELSE IF (a4(3, i) .GT. a4(2, i)) THEN
                CALL PUSHCONTROL3B(3)
              ELSE
                CALL PUSHCONTROL3B(2)
              END IF
            ELSE
              CALL PUSHCONTROL3B(1)
            END IF
          ELSE
            CALL PUSHCONTROL3B(0)
          END IF
        END DO
        DO i=itot,1,-1
          CALL POPCONTROL3B(branch)
          IF (branch .GE. 2) THEN
            IF (branch .EQ. 2) THEN
              a4_ad(3, i) = a4_ad(3, i) + a4_ad(2, i)
              a4_ad(4, i) = a4_ad(4, i) - a4_ad(2, i)
              a4_ad(2, i) = 0.0_FVPRC
              a4_ad(3, i) = a4_ad(3, i) + 3.*a4_ad(4, i)
              a4_ad(1, i) = a4_ad(1, i) - 3.*a4_ad(4, i)
              a4_ad(4, i) = 0.0_FVPRC
            ELSE IF (branch .EQ. 3) THEN
              a4_ad(2, i) = a4_ad(2, i) + a4_ad(3, i)
              a4_ad(4, i) = a4_ad(4, i) - a4_ad(3, i)
              a4_ad(3, i) = 0.0_FVPRC
              a4_ad(2, i) = a4_ad(2, i) + 3.*a4_ad(4, i)
              a4_ad(1, i) = a4_ad(1, i) - 3.*a4_ad(4, i)
              a4_ad(4, i) = 0.0_FVPRC
            ELSE
              a4_ad(4, i) = 0.0_FVPRC
              a4_ad(1, i) = a4_ad(1, i) + a4_ad(2, i)
              a4_ad(2, i) = 0.0_FVPRC
              a4_ad(1, i) = a4_ad(1, i) + a4_ad(3, i)
              a4_ad(3, i) = 0.0_FVPRC
            END IF
          END IF
        END DO
      END IF
    END IF
  END SUBROUTINE PPM_LIMITERS_ADM
!  Differentiation of steepz in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.a
!2b_ord4_fb a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo
! dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod
!.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayle
!igh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_l
!atlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy 
!fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod
!.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_t
!racer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_c
! nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_m
!od.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_co
!rner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.
!compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod.
!copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mod
!.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: dm dp a4
!   with respect to varying inputs: dm dp df2 dq d4 a4
  SUBROUTINE STEEPZ_FWD(i1, i2, km, a4, df2, dm, dq, dp, d4)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: km, i1, i2
! grid size
    REAL(fvprc), INTENT(IN) :: dp(i1:i2, km)
! backward diff of q
    REAL(fvprc), INTENT(IN) :: dq(i1:i2, km)
! backward sum:  dp(k)+ dp(k-1) 
    REAL(fvprc), INTENT(IN) :: d4(i1:i2, km)
! first guess mismatch
    REAL(fvprc), INTENT(IN) :: df2(i1:i2, km)
! monotonic mismatch
    REAL(fvprc), INTENT(IN) :: dm(i1:i2, km)
! !INPUT/OUTPUT PARAMETERS:
! first guess/steepened
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
! !LOCAL VARIABLES:
    INTEGER :: i, k
    REAL(fvprc) :: alfa(i1:i2, km)
    REAL(fvprc) :: f(i1:i2, km)
    REAL(fvprc) :: rat(i1:i2, km)
    REAL(fvprc) :: dg2
    INTRINSIC ABS
    INTRINSIC MIN
    INTRINSIC MAX
    REAL(fvprc) :: abs0
    REAL(fvprc) :: y1
! Compute ratio of dq/dp
    DO k=2,km
      DO i=i1,i2
        rat(i, k) = dq(i, k-1)/d4(i, k)
      END DO
    END DO
! Compute F
    DO k=2,km-1
      DO i=i1,i2
        f(i, k) = (rat(i, k+1)-rat(i, k))/(dp(i, k-1)+dp(i, k)+dp(i, k+1&
&         ))
      END DO
    END DO
    DO k=3,km-2
      DO i=i1,i2
        IF (df2(i, k) .GE. 0.) THEN
          abs0 = df2(i, k)
        ELSE
          abs0 = -df2(i, k)
        END IF
        IF (f(i, k+1)*f(i, k-1) .LT. 0. .AND. abs0 .GT. 10.e-10) THEN
!dh fix to prevenet tap complaint on real equality test
          CALL PUSHREALARRAY(dg2)
          dg2 = (f(i, k+1)-f(i, k-1))*((dp(i, k+1)-dp(i, k-1))**2+d4(i, &
&           k)*d4(i, k+1))
          IF (0.5 .GT. -(0.1875*dg2/df2(i, k))) THEN
            y1 = -(0.1875*dg2/df2(i, k))
            CALL PUSHCONTROL1B_FV(0)
          ELSE
            y1 = 0.5
            CALL PUSHCONTROL1B_FV(1)
          END IF
          IF (0. .LT. y1) THEN
            alfa(i, k) = y1
            CALL PUSHCONTROL2B_FV(1)
          ELSE
            alfa(i, k) = 0.
            CALL PUSHCONTROL2B_FV(2)
          END IF
        ELSE
          alfa(i, k) = 0.
          CALL PUSHCONTROL2B_FV(0)
        END IF
      END DO
    END DO
    DO k=4,km-2
      DO i=i1,i2
        CALL PUSHREALARRAY(a4(2, i, k))
        a4(2, i, k) = (1.-alfa(i, k-1)-alfa(i, k))*a4(2, i, k) + alfa(i&
&         , k-1)*(a4(1, i, k)-dm(i, k)) + alfa(i, k)*(a4(1, i, k-1)+dm(i&
&         , k-1))
      END DO
    END DO
    CALL PUSHREALARRAY(f, 8*(i2-i1+1)*km/8)
    CALL PUSHREALARRAY(dg2)
    CALL PUSHREALARRAY(alfa, 8*(i2-i1+1)*km/8)
    CALL PUSHREALARRAY(rat, 8*(i2-i1+1)*km/8)
  END SUBROUTINE STEEPZ_FWD
!  Differentiation of steepz in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_mod.
!a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_hal
!o dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mo
!d.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.rayl
!eigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_to_
!latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_energy
! fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mo
!d.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz fv_
!tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solver_
!c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_core_
!mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergence_c
!orner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod
!.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_mod
!.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core_mo
!d.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: dm dp a4
!   with respect to varying inputs: dm dp df2 dq d4 a4
  SUBROUTINE STEEPZ_BWD(i1, i2, km, a4, a4_ad, df2, df2_ad, dm, dm_ad, &
&   dq, dq_ad, dp, dp_ad, d4, d4_ad)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: km, i1, i2
    REAL(fvprc), INTENT(IN) :: dp(i1:i2, km)
    REAL(fvprc) :: dp_ad(i1:i2, km)
    REAL(fvprc), INTENT(IN) :: dq(i1:i2, km)
    REAL(fvprc) :: dq_ad(i1:i2, km)
    REAL(fvprc), INTENT(IN) :: d4(i1:i2, km)
    REAL(fvprc) :: d4_ad(i1:i2, km)
    REAL(fvprc), INTENT(IN) :: df2(i1:i2, km)
    REAL(fvprc) :: df2_ad(i1:i2, km)
    REAL(fvprc), INTENT(IN) :: dm(i1:i2, km)
    REAL(fvprc) :: dm_ad(i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4_ad(4, i1:i2, km)
    INTEGER :: i, k
    REAL(fvprc) :: alfa(i1:i2, km)
    REAL(fvprc) :: alfa_ad(i1:i2, km)
    REAL(fvprc) :: f(i1:i2, km)
    REAL(fvprc) :: f_ad(i1:i2, km)
    REAL(fvprc) :: rat(i1:i2, km)
    REAL(fvprc) :: rat_ad(i1:i2, km)
    REAL(fvprc) :: dg2
    REAL(fvprc) :: dg2_ad
    INTRINSIC ABS
    INTRINSIC MIN
    INTRINSIC MAX
    REAL(fvprc) :: abs0
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: y1_ad
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    INTEGER :: branch
    REAL(fvprc) :: y1
    CALL POPREALARRAY(rat, 8*(i2-i1+1)*km/8)
    CALL POPREALARRAY(alfa, 8*(i2-i1+1)*km/8)
    CALL POPREALARRAY(dg2)
    CALL POPREALARRAY(f, 8*(i2-i1+1)*km/8)
    alfa_ad = 0.0_FVPRC
    DO k=km-2,4,-1
      DO i=i2,i1,-1
        CALL POPREALARRAY(a4(2, i, k))
        temp_ad6 = a4(2, i, k)*a4_ad(2, i, k)
        temp_ad7 = alfa(i, k-1)*a4_ad(2, i, k)
        temp_ad8 = alfa(i, k)*a4_ad(2, i, k)
        alfa_ad(i, k) = alfa_ad(i, k) + (a4(1, i, k-1)+dm(i, k-1))*a4_ad&
&         (2, i, k) - temp_ad6
        alfa_ad(i, k-1) = alfa_ad(i, k-1) + (a4(1, i, k)-dm(i, k))*a4_ad&
&         (2, i, k) - temp_ad6
        a4_ad(1, i, k) = a4_ad(1, i, k) + temp_ad7
        dm_ad(i, k) = dm_ad(i, k) - temp_ad7
        a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + temp_ad8
        dm_ad(i, k-1) = dm_ad(i, k-1) + temp_ad8
        a4_ad(2, i, k) = (1.-alfa(i, k)-alfa(i, k-1))*a4_ad(2, i, k)
      END DO
    END DO
    df2_ad = 0.0_FVPRC
    d4_ad = 0.0_FVPRC
    f_ad = 0.0_FVPRC
    DO k=km-2,3,-1
      DO i=i2,i1,-1
        CALL POPCONTROL2B_FV(branch)
        IF (branch .EQ. 0) THEN
          alfa_ad(i, k) = 0.0_FVPRC
        ELSE
          IF (branch .EQ. 1) THEN
            y1_ad = alfa_ad(i, k)
            alfa_ad(i, k) = 0.0_FVPRC
          ELSE
            alfa_ad(i, k) = 0.0_FVPRC
            y1_ad = 0.0_FVPRC
          END IF
          CALL POPCONTROL1B_FV(branch)
          IF (branch .EQ. 0) THEN
            temp_ad5 = -(0.1875*y1_ad/df2(i, k))
            dg2_ad = temp_ad5
            df2_ad(i, k) = df2_ad(i, k) - dg2*temp_ad5/df2(i, k)
          ELSE
            dg2_ad = 0.0_FVPRC
          END IF
          CALL POPREALARRAY(dg2)
          temp0 = dp(i, k+1) - dp(i, k-1)
          temp_ad2 = (temp0**2+d4(i, k)*d4(i, k+1))*dg2_ad
          temp_ad3 = (f(i, k+1)-f(i, k-1))*dg2_ad
          temp_ad4 = 2*temp0*temp_ad3
          f_ad(i, k+1) = f_ad(i, k+1) + temp_ad2
          f_ad(i, k-1) = f_ad(i, k-1) - temp_ad2
          dp_ad(i, k+1) = dp_ad(i, k+1) + temp_ad4
          dp_ad(i, k-1) = dp_ad(i, k-1) - temp_ad4
          d4_ad(i, k) = d4_ad(i, k) + d4(i, k+1)*temp_ad3
          d4_ad(i, k+1) = d4_ad(i, k+1) + d4(i, k)*temp_ad3
        END IF
      END DO
    END DO
    rat_ad = 0.0_FVPRC
    DO k=km-1,2,-1
      DO i=i2,i1,-1
        temp = dp(i, k-1) + dp(i, k) + dp(i, k+1)
        temp_ad0 = f_ad(i, k)/temp
        temp_ad1 = -((rat(i, k+1)-rat(i, k))*temp_ad0/temp)
        rat_ad(i, k+1) = rat_ad(i, k+1) + temp_ad0
        rat_ad(i, k) = rat_ad(i, k) - temp_ad0
        dp_ad(i, k-1) = dp_ad(i, k-1) + temp_ad1
        dp_ad(i, k) = dp_ad(i, k) + temp_ad1
        dp_ad(i, k+1) = dp_ad(i, k+1) + temp_ad1
        f_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
    dq_ad = 0.0_FVPRC
    DO k=km,2,-1
      DO i=i2,i1,-1
        temp_ad = rat_ad(i, k)/d4(i, k)
        dq_ad(i, k-1) = dq_ad(i, k-1) + temp_ad
        d4_ad(i, k) = d4_ad(i, k) - dq(i, k-1)*temp_ad/d4(i, k)
        rat_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE STEEPZ_BWD
  SUBROUTINE RST_REMAP(km, kn, is, ie, js, je, isd, ied, jsd, jed, nq, &
&   delp_r, u_r, v_r, w_r, delz_r, pt_r, q_r, delp, u, v, w, delz, pt, q&
&   , ak_r, bk_r, ptop, ak, bk, hydrostatic, make_nh, domain, &
&   square_domain)
    IMPLICIT NONE
!------------------------------------
! Assuming hybrid sigma-P coordinate:
!------------------------------------
! !INPUT PARAMETERS:
! Restart z-dimension
    INTEGER, INTENT(IN) :: km
! Run time dimension
    INTEGER, INTENT(IN) :: kn
! number of tracers (including h2o)
    INTEGER, INTENT(IN) :: nq
! starting & ending X-Dir index
    INTEGER, INTENT(IN) :: is, ie, isd, ied
! starting & ending Y-Dir index
    INTEGER, INTENT(IN) :: js, je, jsd, jed
    LOGICAL, INTENT(IN) :: hydrostatic, make_nh, square_domain
    REAL(fvprc), INTENT(IN) :: ptop
    REAL(fvprc), INTENT(IN) :: ak_r(km+1)
    REAL(fvprc), INTENT(IN) :: bk_r(km+1)
    REAL(fvprc), INTENT(IN) :: ak(kn+1)
    REAL(fvprc), INTENT(IN) :: bk(kn+1)
! pressure thickness
    REAL(fvprc), INTENT(IN) :: delp_r(is:ie, js:je, km)
! u-wind (m/s)
    REAL(fvprc), INTENT(IN) :: u_r(is:ie, js:je+1, km)
! v-wind (m/s)
    REAL(fvprc), INTENT(IN) :: v_r(is:ie+1, js:je, km)
    REAL(fvprc), INTENT(INOUT) :: pt_r(is:ie, js:je, km)
    REAL(fvprc), INTENT(IN) :: w_r(is:ie, js:je, km)
    REAL(fvprc), INTENT(IN) :: q_r(is:ie, js:je, km, *)
    REAL(fvprc), INTENT(INOUT) :: delz_r(is:ie, js:je, km)
    TYPE(DOMAIN2D), INTENT(INOUT) :: domain
! Output:
! pressure thickness
    REAL(fvprc), INTENT(OUT) :: delp(isd:ied, jsd:jed, kn)
! u-wind (m/s)
    REAL(fvprc), INTENT(OUT) :: u(isd:ied, jsd:jed+1, kn)
! v-wind (m/s)
    REAL(fvprc), INTENT(OUT) :: v(isd:ied+1, jsd:jed, kn)
! vertical velocity (m/s)
    REAL(fvprc), INTENT(OUT) :: w(isd:, jsd:, :)
! temperature
    REAL(fvprc), INTENT(OUT) :: pt(isd:ied, jsd:jed, kn)
    REAL(fvprc), INTENT(OUT) :: q(isd:ied, jsd:jed, kn, *)
! delta-height (m)
    REAL(fvprc), INTENT(OUT) :: delz(isd:, jsd:, :)
!-----------------------------------------------------------------------
    REAL(fvprc) :: r_vir
! surface pressure
    REAL(fvprc) :: ps(isd:ied, jsd:jed)
    REAL(fvprc) :: pe1(is:ie, km+1)
    REAL(fvprc) :: pe2(is:ie, kn+1)
    REAL(fvprc) :: pv1(is:ie+1, km+1)
    REAL(fvprc) :: pv2(is:ie+1, kn+1)
    INTEGER :: i, j, k, iq
    INTEGER, PARAMETER :: kord=4
    INTRINSIC LOG
    INTEGER :: arg1
    r_vir = rvgas/rdgas - 1.
!$OMP parallel do default(none) shared(is,ie,js,je,ps,ak_r)
    DO j=js,je
      DO i=is,ie
        ps(i, j) = ak_r(1)
      END DO
    END DO
! this OpenMP do-loop setup cannot work in it's current form....
!$OMP parallel do default(none) shared(is,ie,js,je,km,ps,delp_r)
    DO j=js,je
      DO k=1,km
        DO i=is,ie
          ps(i, j) = ps(i, j) + delp_r(i, j, k)
        END DO
      END DO
    END DO
! only one cell is needed
    IF (square_domain) THEN
      CALL MPP_UPDATE_DOMAINS(ps, domain, complete=.true., whalo=1, &
&                       ehalo=1, shalo=1, nhalo=1)
    ELSE
      CALL MPP_UPDATE_DOMAINS(ps, domain, complete=.true.)
    END IF
! Compute virtual Temp
!$OMP parallel do default(none) shared(is,ie,js,je,km,pt_r,r_vir,q_r)
    DO k=1,km
      DO j=js,je
        DO i=is,ie
          pt_r(i, j, k) = pt_r(i, j, k)*(1.+r_vir*q_r(i, j, k, 1))
        END DO
      END DO
    END DO
!$OMP parallel do default(none) shared(is,ie,js,je,km,ak_r,bk_r,ps,kn,ak,bk,u_r,u,delp, &
!$OMP                                  nq,hydrostatic,make_nh,w_r,w,delz_r,delp_r,delz, &
!$OMP                                  pt_r,pt,v_r,v) &
!$OMP                          private(pe1,  pe2, pv1, pv2)
    DO j=js,je+1
!------
! map u
!------
      DO k=1,km+1
        DO i=is,ie
          pe1(i, k) = ak_r(k) + 0.5*bk_r(k)*(ps(i, j-1)+ps(i, j))
        END DO
      END DO
      DO k=1,kn+1
        DO i=is,ie
          pe2(i, k) = ak(k) + 0.5*bk(k)*(ps(i, j-1)+ps(i, j))
        END DO
      END DO
      CALL REMAP_2D(km, pe1, u_r(is:ie, j:j, 1:km), kn, pe2, u(is:ie, j:&
&             j, 1:kn), is, ie, -1, kord)
!(j < je+1)
      IF (j .NE. je + 1) THEN
!---------------
! Hybrid sigma-p
!---------------
        DO k=1,km+1
          DO i=is,ie
            pe1(i, k) = ak_r(k) + bk_r(k)*ps(i, j)
          END DO
        END DO
        DO k=1,kn+1
          DO i=is,ie
            pe2(i, k) = ak(k) + bk(k)*ps(i, j)
          END DO
        END DO
!-------------
! Compute delp
!-------------
        DO k=1,kn
          DO i=is,ie
            delp(i, j, k) = pe2(i, k+1) - pe2(i, k)
          END DO
        END DO
!----------------
! Map constituents
!----------------
        IF (nq .NE. 0) THEN
          DO iq=1,nq
            CALL REMAP_2D(km, pe1, q_r(is:ie, j:j, 1:km, iq:iq), kn, pe2&
&                   , q(is:ie, j:j, 1:kn, iq:iq), is, ie, 0, kord)
          END DO
        END IF
        IF (.NOT.hydrostatic .AND. (.NOT.make_nh)) THEN
! Remap vertical wind:
          CALL REMAP_2D(km, pe1, w_r(is:ie, j:j, 1:km), kn, pe2, w(is:ie&
&                 , j:j, 1:kn), is, ie, -1, kord)
! Remap delz for hybrid sigma-p coordinate
          DO k=1,km
            DO i=is,ie
! ="specific volume"/grav
              delz_r(i, j, k) = -(delz_r(i, j, k)/delp_r(i, j, k))
            END DO
          END DO
          CALL REMAP_2D(km, pe1, delz_r(is:ie, j:j, 1:km), kn, pe2, delz&
&                 (is:ie, j:j, 1:kn), is, ie, 1, kord)
          DO k=1,kn
            DO i=is,ie
              delz(i, j, k) = -(delz(i, j, k)*delp(i, j, k))
            END DO
          END DO
        END IF
! Geopotential conserving remap of virtual temperature:
        DO k=1,km+1
          DO i=is,ie
            pe1(i, k) = LOG(pe1(i, k))
          END DO
        END DO
        DO k=1,kn+1
          DO i=is,ie
            pe2(i, k) = LOG(pe2(i, k))
          END DO
        END DO
        CALL REMAP_2D(km, pe1, pt_r(is:ie, j:j, 1:km), kn, pe2, pt(is:ie&
&               , j:j, 1:kn), is, ie, 1, kord)
!------
! map v
!------
        DO k=1,km+1
          DO i=is,ie+1
            pv1(i, k) = ak_r(k) + 0.5*bk_r(k)*(ps(i-1, j)+ps(i, j))
          END DO
        END DO
        DO k=1,kn+1
          DO i=is,ie+1
            pv2(i, k) = ak(k) + 0.5*bk(k)*(ps(i-1, j)+ps(i, j))
          END DO
        END DO
        arg1 = ie + 1
        CALL REMAP_2D(km, pv1, v_r(is:ie+1, j:j, 1:km), kn, pv2, v(is:ie&
&               +1, j:j, 1:kn), is, arg1, -1, kord)
      END IF
    END DO
!$OMP parallel do default(none) shared(is,ie,js,je,kn,pt,r_vir,q)
    DO k=1,kn
      DO j=js,je
        DO i=is,ie
          pt(i, j, k) = pt(i, j, k)/(1.+r_vir*q(i, j, k, 1))
        END DO
      END DO
    END DO
  END SUBROUTINE RST_REMAP
!-----------------------------------------------------------------------
  SUBROUTINE REMAP_2D(km, pe1, q1, kn, pe2, q2, i1, i2, iv, kord)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i1, i2
! Mode: 0 ==  constituents 1 ==others
    INTEGER, INTENT(IN) :: iv
    INTEGER, INTENT(IN) :: kord
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
! Field input
    REAL(fvprc), INTENT(IN) :: q1(i1:i2, km)
! Field output
    REAL(fvprc), INTENT(OUT) :: q2(i1:i2, kn)
! !LOCAL VARIABLES:
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    INTEGER :: i, k, l, m, k0
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q1(i, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL CS_PROFILE_LINEAR(qs, q4, dp1, km, i1, i2, iv)
      ELSE
        CALL CS_PROFILE(qs, q4, dp1, km, i1, i2, iv, kord)
      END IF
    ELSE
      CALL PPM_PROFILE(q4, dp1, km, i1, i2, iv, kord)
    END IF
    DO i=i1,i2
      k0 = 1
      DO k=1,kn
!#ifdef OLD_TOP_EDGE
!         if( pe2(i,k+1) <= pe1(i,1) ) then
!! Entire grid above old ptop
!             q2(i,k) = q4(2,i,1)
!         elseif( pe2(i,k) < pe1(i,1) .and. pe2(i,k+1)>pe1(i,1) ) then
!! Partially above old ptop:
!             q2(i,k) = q1(i,1)
!#else
        IF (pe2(i, k) .LE. pe1(i, 1)) THEN
! above old ptop:
          q2(i, k) = q1(i, 1)
!#endif
        ELSE
          DO l=k0,km
! locate the top edge: pe2(i,k)
            IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1&
&               )) THEN
              pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
              IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
                pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
                q2(i, k) = q4(2, i, l) + 0.5*(q4(4, i, l)+q4(3, i, l)-q4&
&                 (2, i, l))*(pr+pl) - q4(4, i, l)*r3*(pr*(pr+pl)+pl**2)
                k0 = l
                GOTO 555
              ELSE
! Fractional area...
                qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i&
&                 , l)+q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*&
&                 (1.+pl*(1.+pl))))
                DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
                  IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer..
                    qsum = qsum + dp1(i, m)*q4(1, i, m)
                  ELSE
                    dp = pe2(i, k+1) - pe1(i, m)
                    esl = dp/dp1(i, m)
                    qsum = qsum + dp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-&
&                     q4(2, i, m)+q4(4, i, m)*(1.-r23*esl)))
                    k0 = m
                    GOTO 123
                  END IF
                END DO
                GOTO 123
              END IF
            END IF
          END DO
 123      q2(i, k) = qsum/(pe2(i, k+1)-pe2(i, k))
        END IF
 555    CONTINUE
      END DO
    END DO
  END SUBROUTINE REMAP_2D
!#ifdef ANA_REMAP
! subroutine ana_remap_(km, kn, is,ie,js,je, isd,ied,jsd,jed, nq,  &
!                       delp_r, u_r, v_r, ptv_r, q_r,      &
!                       delp,   u  , v  , ptv  , q  ,      &
!                       ak, bk, istatus, phis_r, phis)
!  implicit none
!
!! Todling: Stripped from original S.J.Lin's routine;
!!          remove non-hydrostatic support
!!------------------------------------
!! Assuming hybrid sigma-P coordinate:
!!------------------------------------
!! !INPUT PARAMETERS:
!  integer, intent(in):: km                    ! Restart z-dimension
!  integer, intent(in):: kn                    ! Run time dimension
!  integer, intent(in):: nq                    ! number of tracers (including h2o)
!  integer, intent(in):: is,ie,isd,ied         ! starting & ending X-Dir index
!  integer, intent(in):: js,je,jsd,jed         ! starting & ending Y-Dir index
!  real(REAL8), intent(in) :: ak(kn+1)
!  real(REAL8), intent(in) :: bk(kn+1)
!  real(REAL8), intent(in), optional:: phis_r(is:ie,js:je)   ! topography
!  real(REAL8), intent(in):: delp_r(is:ie  ,js:je  ,km)      ! pressure thickness
!  real(REAL8), intent(in)::    u_r(is:ie  ,js:je+1,km)      ! u-wind (m/s)
!  real(REAL8), intent(in)::    v_r(is:ie+1,js:je  ,km)      ! v-wind (m/s)
!  real(REAL8), intent(in)::  ptv_r(is:ie  ,js:je  ,km)      ! virtual-potential temperature  
!  real(REAL8), intent(in)::    q_r(is:ie  ,js:je  ,km,nq)
!! Input related to desired remapped output:
!  real(REAL8), intent(in), optional:: phis(is:ie,js:je)       ! topography (unghosted)
!! Output:
!  integer, intent(out) :: istatus
!  real(REAL8), intent(inout):: delp(isd:ied  ,jsd:jed  ,kn)   ! pressure thickness
!  real(REAL8), intent(out)::      u(isd:ied  ,jsd:jed+1,kn)   ! u-wind (m/s)
!  real(REAL8), intent(out)::      v(isd:ied+1,jsd:jed  ,kn)   ! v-wind (m/s)
!  real(REAL8), intent(out)::    ptv(isd:ied  ,jsd:jed  ,kn)   ! virtual-potential temperature
!  real(REAL8), intent(out)::      q(isd:ied  ,jsd:jed  ,kn,nq)
!!-----------------------------------------------------------------------
!  real(REAL8) r_vir,cp,kappa
!  real(REAL8) ps_r(isd:ied,jsd:jed)  ! surface pressure
!  real(REAL8) ps  (isd:ied,jsd:jed)  ! surface pressure
!  real(REAL8)  pe1(is:ie,km+1)
!  real(REAL8)  pe2(is:ie,kn+1)
!  real(REAL8)  pv1(is:ie+1,km+1)
!  real(REAL8)  pv2(is:ie+1,kn+1)
!  real(REAL8),allocatable:: ple(:,:,:)
!  real(REAL8),allocatable:: pke(:,:,:)
!  real(REAL8),allocatable:: phi(:,:,:)
!
!  integer i,j,k , iq
!  integer, parameter:: kord=4
!  kappa = MAPL_KAPPA
!  r_vir = rvgas/rdgas - 1.
!  cp    = rvgas/kappa
!
!  istatus=0
!! For now, take only same eta levels between input and output
!  if(km/=kn) then
!     istatus=1
!     return
!  endif
!
!!$omp parallel do default(shared)
!  do j=js,je
!     do i=is,ie
!        ps_r(i,j) = ak(1)
!     enddo
!  enddo
!
!!$omp parallel do default(shared)
!  do k=1,km
!     do j=js,je
!        do i=is,ie
!           ps_r(i,j) = ps_r(i,j) + delp_r(i,j,k)
!        enddo
!     enddo
!  enddo
!  if ( present(phis) .and. present(phis_r) ) then
!
!!    Construct Input Heights
!!    -----------------------
!     allocate(ple(is:ie,js:je,km+1))
!     allocate(pke(is:ie,js:je,km+1))
!     allocate(phi(is:ie,js:je,km+1))
!     ple(:,:,1) = ak(1)
!     do k=2,km+1
!        ple(:,:,k) = ple(:,:,k-1) + delp_r(:,:,k-1)
!     enddo
!     pke = ple**kappa
!
!     phi(:,:,km+1) = phis_r
!     do k=km,1,-1
!        phi(:,:,k) = phi(:,:,k+1) + cp*ptv_r(:,:,k)*( pke(:,:,k+1)-pke(:,:,k) )
!     enddo
!
!!    Compute new surface pressure consistent with output topography
!!    --------------------------------------------------------------
!     do j=js,je
!        do i=is,ie
!           k = km
!           do while ( phi(i,j,k).lt.phis(i,j) )
!              k = k-1
!           enddo
!           ps(i,j) = ple(i,j,k+1)*( 1+(phi(i,j,k+1)-phis(i,j))/(cp*ptv_r(i,j,k)*pke(i,j,k+1)) )**(1.0/kappa)
!        enddo
!     enddo
!     deallocate(phi)
!     deallocate(pke)
!     deallocate(ple)
!
!  else ! when no phis available ...
!!$omp parallel do default(shared)
!     do j=js,je
!        do i=is,ie
!           ps(i,j) = ak(1)
!        enddo
!     enddo
!
!!$omp parallel do default(shared)
!     do k=1,kn
!        do j=js,je
!           do i=is,ie
!              ps(i,j) = ps(i,j) + delp(i,j,k)
!           enddo
!        enddo
!     enddo
!
!  endif ! <phis check>
!
!! only one cell is needed
!  call mpp_update_domains(ps  , domain, complete=.true.)
!  call mpp_update_domains(ps_r, domain, complete=.true.)
!
!!$omp parallel do default(shared) private(pe1,  pe2, pv1, pv2)
!  do 1000 j=js,je+1
!!------
!! map u
!!------
!     do k=1,km+1
!        do i=is,ie
!           pe1(i,k) = ak(k) + 0.5*bk(k)*(ps_r(i,j-1)+ps_r(i,j))
!        enddo
!     enddo
!     do k=1,kn+1
!        do i=is,ie
!           pe2(i,k) = ak(k) + 0.5*bk(k)*(ps(i,j-1)+ps(i,j))
!        enddo
!     enddo
!
!     call remap_2d(km, pe1, u_r(is:ie,j:j,1:km),       &
!                   kn, pe2,   u(is:ie,j:j,1:kn),       &
!                   is, ie, -1, kord)
!
!  if ( j /= (je+1) )  then
!!---------------
!! Hybrid sigma-p
!!---------------
!     do k=1,km+1
!        do i=is,ie
!           pe1(i,k) = ak(k) + bk(k)*ps_r(i,j)
!        enddo
!     enddo
!
!     do k=1,kn+1
!        do i=is,ie
!           pe2(i,k) =   ak(k) + bk(k)*ps(i,j)
!        enddo
!     enddo
!
!!-------------
!! Compute delp
!!-------------
!      do k=1,kn
!         do i=is,ie
!            delp(i,j,k) = pe2(i,k+1) - pe2(i,k)
!         enddo
!      enddo
!
!!----------------
!! Map constituents
!!----------------
!      if( nq /= 0 ) then
!          do iq=1,nq
!             call remap_2d(km, pe1, q_r(is:ie,j:j,1:km,iq:iq),  &
!                           kn, pe2,   q(is:ie,j:j,1:kn,iq:iq),  &
!                           is, ie, 0, kord)
!          enddo
!      endif
!
!! Geopotential conserving remap of virtual temperature:
!       do k=1,km+1
!          do i=is,ie
!             pe1(i,k) = log(pe1(i,k))
!          enddo
!       enddo
!       do k=1,kn+1
!          do i=is,ie
!             pe2(i,k) = log(pe2(i,k))
!          enddo
!       enddo
!
!       call remap_2d(km, pe1,ptv_r(is:ie,j:j,1:km),       &
!                     kn, pe2,  ptv(is:ie,j:j,1:kn),       &
!                     is, ie, 1, kord)
!!------
!! map v
!!------
!       do k=1,km+1
!          do i=is,ie+1
!             pv1(i,k) = ak(k) + 0.5*bk(k)*(ps_r(i-1,j)+ps_r(i,j))
!          enddo
!       enddo
!       do k=1,kn+1
!          do i=is,ie+1
!             pv2(i,k) = ak(k) + 0.5*bk(k)*(ps(i-1,j)+ps(i,j))
!          enddo
!       enddo
!
!       call remap_2d(km, pv1, v_r(is:ie+1,j:j,1:km),       &
!                     kn, pv2,   v(is:ie+1,j:j,1:kn),       &
!                     is, ie+1, -1, kord)
!
!  endif !(j < je+1)
!1000  continue
!
! end subroutine ana_remap_
!#endif
  SUBROUTINE MAPPM(km, pe1, q1, kn, pe2, q2, i1, i2, iv, kord, ptop)
    IMPLICIT NONE
! IV = 0: constituents
! IV = 1: potential temp
! IV =-1: winds
! Mass flux preserving mapping: q1(im,km) -> q2(im,kn)
! pe1: pressure at layer edges (from model top to bottom surface)
!      in the original vertical coordinate
! pe2: pressure at layer edges (from model top to bottom surface)
!      in the new vertical coordinate
    INTEGER, INTENT(IN) :: i1, i2, km, kn, kord, iv
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1), pe2(i1:i2, kn+1)
    REAL(fvprc), INTENT(IN) :: q1(i1:i2, km)
    REAL(fvprc), INTENT(OUT) :: q2(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: ptop
! local
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: a4(4, i1:i2, km)
    INTEGER :: i, k, l
    INTEGER :: k0, k1
    REAL(fvprc) :: pl, pr, tt, delp, qsum, dpsum, esl
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        a4(1, i, k) = q1(i, k)
      END DO
    END DO
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL CS_PROFILE_LINEAR(qs, a4, dp1, km, i1, i2, iv)
      ELSE
        CALL CS_PROFILE(qs, a4, dp1, km, i1, i2, iv, kord)
      END IF
    ELSE
      CALL PPM_PROFILE(a4, dp1, km, i1, i2, iv, kord)
    END IF
!------------------------------------
! Lowest layer: constant distribution
!------------------------------------
    DO i=i1,i2
      a4(2, i, km) = q1(i, km)
      a4(3, i, km) = q1(i, km)
      a4(4, i, km) = 0.
    END DO
    DO i=i1,i2
      k0 = 1
      DO k=1,kn
        IF (pe2(i, k+1) .LE. pe1(i, 1)) THEN
! Entire grid above old ptop
          q2(i, k) = a4(2, i, 1)
        ELSE IF (pe2(i, k) .GE. pe1(i, km+1)) THEN
! Entire grid below old ps
          q2(i, k) = a4(3, i, km)
        ELSE IF (pe2(i, k) .LT. pe1(i, 1) .AND. pe2(i, k+1) .GT. pe1(i, &
&           1)) THEN
! Part of the grid above ptop
          q2(i, k) = a4(1, i, 1)
        ELSE
          DO l=k0,km
! locate the top edge at pe2(i,k)
            IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1&
&               )) THEN
              k0 = l
              pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
              IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
                pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
                tt = r3*(pr*(pr+pl)+pl**2)
                q2(i, k) = a4(2, i, l) + 0.5*(a4(4, i, l)+a4(3, i, l)-a4&
&                 (2, i, l))*(pr+pl) - a4(4, i, l)*tt
                GOTO 555
              ELSE
! Fractional area...
                delp = pe1(i, l+1) - pe2(i, k)
                tt = r3*(1.+pl*(1.+pl))
                qsum = delp*(a4(2, i, l)+0.5*(a4(4, i, l)+a4(3, i, l)-a4&
&                 (2, i, l))*(1.+pl)-a4(4, i, l)*tt)
                dpsum = delp
                k1 = l + 1
                GOTO 111
              END IF
            END IF
          END DO
 111      CONTINUE
          DO l=k1,km
            IF (pe2(i, k+1) .GT. pe1(i, l+1)) THEN
! Whole layer..
              qsum = qsum + dp1(i, l)*q1(i, l)
              dpsum = dpsum + dp1(i, l)
            ELSE
              delp = pe2(i, k+1) - pe1(i, l)
              esl = delp/dp1(i, l)
              qsum = qsum + delp*(a4(2, i, l)+0.5*esl*(a4(3, i, l)-a4(2&
&               , i, l)+a4(4, i, l)*(1.-r23*esl)))
              dpsum = dpsum + delp
              k0 = l
              GOTO 123
            END IF
          END DO
          delp = pe2(i, k+1) - pe1(i, km+1)
          IF (delp .GT. 0.) THEN
! Extended below old ps
            qsum = qsum + delp*a4(3, i, km)
            dpsum = dpsum + delp
          END IF
 123      q2(i, k) = qsum/dpsum
        END IF
 555    CONTINUE
      END DO
    END DO
  END SUBROUTINE MAPPM
  SUBROUTINE CS_PROFILE_LINEAR(qs, a4, delp, km, i1, i2, iv)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
    INTEGER, INTENT(IN) :: i1, i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    INTEGER :: i, k, im
    IF (iv .EQ. -2) THEN
      DO i=i1,i2
        gam(i, 2) = 0.5
        q(i, 1) = 1.5*a4(1, i, 1)
      END DO
      DO k=2,km-1
        DO i=i1,i2
          grat = delp(i, k-1)/delp(i, k)
          bet = 2. + grat + grat - gam(i, k)
          q(i, k) = (3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))/bet
          gam(i, k+1) = grat/bet
        END DO
      END DO
      DO i=i1,i2
        grat = delp(i, km-1)/delp(i, km)
        q(i, km) = (3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, km-&
&         1))/(2.+grat+grat-gam(i, km))
        q(i, km+1) = qs(i)
      END DO
      DO k=km-1,1,-1
        DO i=i1,i2
          q(i, k) = q(i, k) - gam(i, k+1)*q(i, k+1)
        END DO
      END DO
    ELSE
      DO i=i1,i2
! grid ratio
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        q(i, 1) = ((grat+grat)*(grat+1.)*a4(1, i, 1)+a4(1, i, 2))/bet
        gam(i, 1) = (1.+grat*(grat+1.5))/bet
      END DO
      DO k=2,km
        DO i=i1,i2
          d4(i) = delp(i, k-1)/delp(i, k)
          bet = 2. + d4(i) + d4(i) - gam(i, k-1)
          q(i, k) = (3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))/bet
          gam(i, k) = d4(i)/bet
        END DO
      END DO
      DO i=i1,i2
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        q(i, km+1) = (2.*d4(i)*(d4(i)+1.)*a4(1, i, km)+a4(1, i, km-1)-&
&         a_bot*q(i, km))/(d4(i)*(d4(i)+0.5)-a_bot*gam(i, km))
      END DO
      DO k=km,1,-1
        DO i=i1,i2
          q(i, k) = q(i, k) - gam(i, k)*q(i, k+1)
        END DO
      END DO
    END IF
    DO k=1,km
      DO i=i1,i2
        a4(2, i, k) = q(i, k)
        a4(3, i, k) = q(i, k+1)
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
    END DO
  END SUBROUTINE CS_PROFILE_LINEAR
  SUBROUTINE CS_PROFILE(qs, a4, delp, km, i1, i2, iv, kord)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
    INTEGER, INTENT(IN) :: i1, i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
    INTEGER, INTENT(IN) :: kord
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    INTEGER :: i, k, im
    INTRINSIC MAX
    INTRINSIC MIN
    INTRINSIC ABS
    INTEGER :: abs0
    INTEGER :: abs1
    REAL(fvprc) :: abs2
    INTEGER :: abs3
    REAL(fvprc) :: abs4
    INTEGER :: abs5
    REAL(fvprc) :: abs6
    INTEGER :: abs7
    REAL(fvprc) :: abs8
    REAL(fvprc) :: abs9
    REAL(fvprc) :: abs10
    REAL(fvprc) :: y24
    REAL(fvprc) :: y23
    REAL(fvprc) :: y22
    REAL(fvprc) :: y21
    REAL(fvprc) :: y20
    REAL(fvprc) :: x8
    REAL(fvprc) :: x7
    REAL(fvprc) :: x6
    REAL(fvprc) :: x5
    REAL(fvprc) :: x4
    REAL(fvprc) :: x3
    REAL(fvprc) :: x2
    REAL(fvprc) :: x1
    REAL(fvprc) :: y19
    REAL(fvprc) :: y18
    REAL(fvprc) :: y17
    REAL(fvprc) :: y16
    REAL(fvprc) :: y15
    REAL(fvprc) :: y14
    REAL(fvprc) :: y13
    REAL(fvprc) :: y12
    REAL(fvprc) :: y11
    REAL(fvprc) :: y10
    REAL(fvprc) :: y9
    REAL(fvprc) :: y8
    REAL(fvprc) :: y7
    REAL(fvprc) :: y6
    REAL(fvprc) :: y5
    REAL(fvprc) :: y4
    REAL(fvprc) :: y3
    REAL(fvprc) :: y2
    REAL(fvprc) :: y1
    IF (iv .EQ. -2) THEN
      DO i=i1,i2
        gam(i, 2) = 0.5
        q(i, 1) = 1.5*a4(1, i, 1)
      END DO
      DO k=2,km-1
        DO i=i1,i2
          grat = delp(i, k-1)/delp(i, k)
          bet = 2. + grat + grat - gam(i, k)
          q(i, k) = (3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))/bet
          gam(i, k+1) = grat/bet
        END DO
      END DO
      DO i=i1,i2
        grat = delp(i, km-1)/delp(i, km)
        q(i, km) = (3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, km-&
&         1))/(2.+grat+grat-gam(i, km))
        q(i, km+1) = qs(i)
      END DO
      DO k=km-1,1,-1
        DO i=i1,i2
          q(i, k) = q(i, k) - gam(i, k+1)*q(i, k+1)
        END DO
      END DO
    ELSE
      DO i=i1,i2
! grid ratio
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        q(i, 1) = ((grat+grat)*(grat+1.)*a4(1, i, 1)+a4(1, i, 2))/bet
        gam(i, 1) = (1.+grat*(grat+1.5))/bet
      END DO
      DO k=2,km
        DO i=i1,i2
          d4(i) = delp(i, k-1)/delp(i, k)
          bet = 2. + d4(i) + d4(i) - gam(i, k-1)
          q(i, k) = (3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))/bet
          gam(i, k) = d4(i)/bet
        END DO
      END DO
      DO i=i1,i2
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        q(i, km+1) = (2.*d4(i)*(d4(i)+1.)*a4(1, i, km)+a4(1, i, km-1)-&
&         a_bot*q(i, km))/(d4(i)*(d4(i)+0.5)-a_bot*gam(i, km))
      END DO
      DO k=km,1,-1
        DO i=i1,i2
          q(i, k) = q(i, k) - gam(i, k)*q(i, k+1)
        END DO
      END DO
    END IF
!------------------
! Apply constraints
!------------------
    im = i2 - i1 + 1
! Apply *large-scale* constraints 
    DO i=i1,i2
      IF (a4(1, i, 1) .LT. a4(1, i, 2)) THEN
        y1 = a4(1, i, 2)
      ELSE
        y1 = a4(1, i, 1)
      END IF
      IF (q(i, 2) .GT. y1) THEN
        q(i, 2) = y1
      ELSE
        q(i, 2) = q(i, 2)
      END IF
      IF (a4(1, i, 1) .GT. a4(1, i, 2)) THEN
        y2 = a4(1, i, 2)
      ELSE
        y2 = a4(1, i, 1)
      END IF
      IF (q(i, 2) .LT. y2) THEN
        q(i, 2) = y2
      ELSE
        q(i, 2) = q(i, 2)
      END IF
    END DO
    DO k=2,km
      DO i=i1,i2
        gam(i, k) = a4(1, i, k) - a4(1, i, k-1)
      END DO
    END DO
! Interior:
    DO k=3,km-1
      DO i=i1,i2
        IF (gam(i, k-1)*gam(i, k+1) .GT. 0.) THEN
          IF (a4(1, i, k-1) .LT. a4(1, i, k)) THEN
            y3 = a4(1, i, k)
          ELSE
            y3 = a4(1, i, k-1)
          END IF
          IF (q(i, k) .GT. y3) THEN
            q(i, k) = y3
          ELSE
            q(i, k) = q(i, k)
          END IF
          IF (a4(1, i, k-1) .GT. a4(1, i, k)) THEN
            y4 = a4(1, i, k)
          ELSE
            y4 = a4(1, i, k-1)
          END IF
          IF (q(i, k) .LT. y4) THEN
            q(i, k) = y4
          ELSE
            q(i, k) = q(i, k)
          END IF
        ELSE IF (gam(i, k-1) .GT. 0.) THEN
          IF (a4(1, i, k-1) .GT. a4(1, i, k)) THEN
            y5 = a4(1, i, k)
          ELSE
            y5 = a4(1, i, k-1)
          END IF
          IF (q(i, k) .LT. y5) THEN
            q(i, k) = y5
          ELSE
            q(i, k) = q(i, k)
          END IF
        ELSE
          IF (a4(1, i, k-1) .LT. a4(1, i, k)) THEN
            y6 = a4(1, i, k)
          ELSE
            y6 = a4(1, i, k-1)
          END IF
          IF (q(i, k) .GT. y6) THEN
            q(i, k) = y6
          ELSE
            q(i, k) = q(i, k)
          END IF
          IF (iv .EQ. 0) THEN
            IF (0. .LT. q(i, k)) THEN
              q(i, k) = q(i, k)
            ELSE
              q(i, k) = 0.
            END IF
          END IF
        END IF
      END DO
    END DO
! Bottom:
    DO i=i1,i2
      IF (a4(1, i, km-1) .LT. a4(1, i, km)) THEN
        y7 = a4(1, i, km)
      ELSE
        y7 = a4(1, i, km-1)
      END IF
      IF (q(i, km) .GT. y7) THEN
        q(i, km) = y7
      ELSE
        q(i, km) = q(i, km)
      END IF
      IF (a4(1, i, km-1) .GT. a4(1, i, km)) THEN
        y8 = a4(1, i, km)
      ELSE
        y8 = a4(1, i, km-1)
      END IF
      IF (q(i, km) .LT. y8) THEN
        q(i, km) = y8
      ELSE
        q(i, km) = q(i, km)
      END IF
    END DO
    DO k=1,km
      DO i=i1,i2
        a4(2, i, k) = q(i, k)
        a4(3, i, k) = q(i, k+1)
      END DO
    END DO
    DO k=2,km-1
      DO i=i1,i2
        IF (gam(i, k)*gam(i, k+1) .GT. 0.0) THEN
          extm(i, k) = .false.
        ELSE
          extm(i, k) = .true.
        END IF
      END DO
    END DO
!---------------------------
! Apply subgrid constraints:
!---------------------------
! f(s) = AL + s*[(AR-AL) + A6*(1-s)]         ( 0 <= s  <= 1 )
! Top 2 and bottom 2 layers always use monotonic mapping
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(2, i, 1)) THEN
          a4(2, i, 1) = a4(2, i, 1)
        ELSE
          a4(2, i, 1) = 0.
        END IF
      END DO
    ELSE IF (iv .EQ. -1) THEN
      DO i=i1,i2
        IF (a4(2, i, 1)*a4(1, i, 1) .LE. 0.) a4(2, i, 1) = 0.
      END DO
    ELSE IF (iv .EQ. 2) THEN
      DO i=i1,i2
        a4(2, i, 1) = a4(1, i, 1)
        a4(3, i, 1) = a4(1, i, 1)
        a4(4, i, 1) = 0.
      END DO
    END IF
    IF (iv .NE. 2) THEN
      DO i=i1,i2
        a4(4, i, 1) = 3.*(2.*a4(1, i, 1)-(a4(2, i, 1)+a4(3, i, 1)))
      END DO
      CALL CS_LIMITERS(im, extm(i1, 1), a4(1, i1, 1), 1)
    END IF
! k=2
    DO i=i1,i2
      a4(4, i, 2) = 3.*(2.*a4(1, i, 2)-(a4(2, i, 2)+a4(3, i, 2)))
    END DO
    CALL CS_LIMITERS(im, extm(i1, 2), a4(1, i1, 2), 2)
!-------------------------------------
! Huynh's 2nd constraint for interior:
!-------------------------------------
    DO k=3,km-2
      IF (kord .GE. 0.) THEN
        abs0 = kord
      ELSE
        abs0 = -kord
      END IF
      IF (abs0 .LT. 9) THEN
        DO i=i1,i2
! Left  edges
          pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
          lac_1 = pmp_1 + 1.5*gam(i, k+2)
          IF (a4(1, i, k) .GT. pmp_1) THEN
            IF (pmp_1 .GT. lac_1) THEN
              y17 = lac_1
            ELSE
              y17 = pmp_1
            END IF
          ELSE IF (a4(1, i, k) .GT. lac_1) THEN
            y17 = lac_1
          ELSE
            y17 = a4(1, i, k)
          END IF
          IF (a4(2, i, k) .LT. y17) THEN
            x1 = y17
          ELSE
            x1 = a4(2, i, k)
          END IF
          IF (a4(1, i, k) .LT. pmp_1) THEN
            IF (pmp_1 .LT. lac_1) THEN
              y9 = lac_1
            ELSE
              y9 = pmp_1
            END IF
          ELSE IF (a4(1, i, k) .LT. lac_1) THEN
            y9 = lac_1
          ELSE
            y9 = a4(1, i, k)
          END IF
          IF (x1 .GT. y9) THEN
            a4(2, i, k) = y9
          ELSE
            a4(2, i, k) = x1
          END IF
! Right edges
          pmp_2 = a4(1, i, k) + 2.*gam(i, k)
          lac_2 = pmp_2 - 1.5*gam(i, k-1)
          IF (a4(1, i, k) .GT. pmp_2) THEN
            IF (pmp_2 .GT. lac_2) THEN
              y18 = lac_2
            ELSE
              y18 = pmp_2
            END IF
          ELSE IF (a4(1, i, k) .GT. lac_2) THEN
            y18 = lac_2
          ELSE
            y18 = a4(1, i, k)
          END IF
          IF (a4(3, i, k) .LT. y18) THEN
            x2 = y18
          ELSE
            x2 = a4(3, i, k)
          END IF
          IF (a4(1, i, k) .LT. pmp_2) THEN
            IF (pmp_2 .LT. lac_2) THEN
              y10 = lac_2
            ELSE
              y10 = pmp_2
            END IF
          ELSE IF (a4(1, i, k) .LT. lac_2) THEN
            y10 = lac_2
          ELSE
            y10 = a4(1, i, k)
          END IF
          IF (x2 .GT. y10) THEN
            a4(3, i, k) = y10
          ELSE
            a4(3, i, k) = x2
          END IF
          a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
        END DO
      ELSE
        IF (kord .GE. 0.) THEN
          abs1 = kord
        ELSE
          abs1 = -kord
        END IF
        IF (abs1 .EQ. 9) THEN
          DO i=i1,i2
            IF (extm(i, k) .AND. (extm(i, k-1) .OR. extm(i, k+1))) THEN
! c90_mp122
! grid-scale 2-delta-z wave detected
              a4(2, i, k) = a4(1, i, k)
              a4(3, i, k) = a4(1, i, k)
              a4(4, i, k) = 0.
            ELSE
              a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, k)&
&               )
              IF (a4(4, i, k) .GE. 0.) THEN
                abs2 = a4(4, i, k)
              ELSE
                abs2 = -a4(4, i, k)
              END IF
              IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                abs8 = a4(2, i, k) - a4(3, i, k)
              ELSE
                abs8 = -(a4(2, i, k)-a4(3, i, k))
              END IF
! Check within the smooth region if subgrid profile is non-monotonic
              IF (abs2 .GT. abs8) THEN
                pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                lac_1 = pmp_1 + 1.5*gam(i, k+2)
                IF (a4(1, i, k) .GT. pmp_1) THEN
                  IF (pmp_1 .GT. lac_1) THEN
                    y19 = lac_1
                  ELSE
                    y19 = pmp_1
                  END IF
                ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                  y19 = lac_1
                ELSE
                  y19 = a4(1, i, k)
                END IF
                IF (a4(2, i, k) .LT. y19) THEN
                  x3 = y19
                ELSE
                  x3 = a4(2, i, k)
                END IF
                IF (a4(1, i, k) .LT. pmp_1) THEN
                  IF (pmp_1 .LT. lac_1) THEN
                    y11 = lac_1
                  ELSE
                    y11 = pmp_1
                  END IF
                ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                  y11 = lac_1
                ELSE
                  y11 = a4(1, i, k)
                END IF
                IF (x3 .GT. y11) THEN
                  a4(2, i, k) = y11
                ELSE
                  a4(2, i, k) = x3
                END IF
                pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                lac_2 = pmp_2 - 1.5*gam(i, k-1)
                IF (a4(1, i, k) .GT. pmp_2) THEN
                  IF (pmp_2 .GT. lac_2) THEN
                    y20 = lac_2
                  ELSE
                    y20 = pmp_2
                  END IF
                ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                  y20 = lac_2
                ELSE
                  y20 = a4(1, i, k)
                END IF
                IF (a4(3, i, k) .LT. y20) THEN
                  x4 = y20
                ELSE
                  x4 = a4(3, i, k)
                END IF
                IF (a4(1, i, k) .LT. pmp_2) THEN
                  IF (pmp_2 .LT. lac_2) THEN
                    y12 = lac_2
                  ELSE
                    y12 = pmp_2
                  END IF
                ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                  y12 = lac_2
                ELSE
                  y12 = a4(1, i, k)
                END IF
                IF (x4 .GT. y12) THEN
                  a4(3, i, k) = y12
                ELSE
                  a4(3, i, k) = x4
                END IF
                a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, &
&                 k))
              END IF
            END IF
          END DO
        ELSE
          IF (kord .GE. 0.) THEN
            abs3 = kord
          ELSE
            abs3 = -kord
          END IF
          IF (abs3 .EQ. 10) THEN
            DO i=i1,i2
              IF (extm(i, k)) THEN
                IF (extm(i, k-1) .OR. extm(i, k+1)) THEN
! grid-scale 2-delta-z wave detected
                  a4(2, i, k) = a4(1, i, k)
                  a4(3, i, k) = a4(1, i, k)
                  a4(4, i, k) = 0.
                ELSE
! True local extremum
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                END IF
              ELSE
! not a local extremum
                a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i, &
&                 k))
                IF (a4(4, i, k) .GE. 0.) THEN
                  abs4 = a4(4, i, k)
                ELSE
                  abs4 = -a4(4, i, k)
                END IF
                IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                  abs9 = a4(2, i, k) - a4(3, i, k)
                ELSE
                  abs9 = -(a4(2, i, k)-a4(3, i, k))
                END IF
! Check within the smooth region if subgrid profile is non-monotonic
                IF (abs4 .GT. abs9) THEN
                  pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                  lac_1 = pmp_1 + 1.5*gam(i, k+2)
                  IF (a4(1, i, k) .GT. pmp_1) THEN
                    IF (pmp_1 .GT. lac_1) THEN
                      y21 = lac_1
                    ELSE
                      y21 = pmp_1
                    END IF
                  ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                    y21 = lac_1
                  ELSE
                    y21 = a4(1, i, k)
                  END IF
                  IF (a4(2, i, k) .LT. y21) THEN
                    x5 = y21
                  ELSE
                    x5 = a4(2, i, k)
                  END IF
                  IF (a4(1, i, k) .LT. pmp_1) THEN
                    IF (pmp_1 .LT. lac_1) THEN
                      y13 = lac_1
                    ELSE
                      y13 = pmp_1
                    END IF
                  ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                    y13 = lac_1
                  ELSE
                    y13 = a4(1, i, k)
                  END IF
                  IF (x5 .GT. y13) THEN
                    a4(2, i, k) = y13
                  ELSE
                    a4(2, i, k) = x5
                  END IF
                  pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                  lac_2 = pmp_2 - 1.5*gam(i, k-1)
                  IF (a4(1, i, k) .GT. pmp_2) THEN
                    IF (pmp_2 .GT. lac_2) THEN
                      y22 = lac_2
                    ELSE
                      y22 = pmp_2
                    END IF
                  ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                    y22 = lac_2
                  ELSE
                    y22 = a4(1, i, k)
                  END IF
                  IF (a4(3, i, k) .LT. y22) THEN
                    x6 = y22
                  ELSE
                    x6 = a4(3, i, k)
                  END IF
                  IF (a4(1, i, k) .LT. pmp_2) THEN
                    IF (pmp_2 .LT. lac_2) THEN
                      y14 = lac_2
                    ELSE
                      y14 = pmp_2
                    END IF
                  ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                    y14 = lac_2
                  ELSE
                    y14 = a4(1, i, k)
                  END IF
                  IF (x6 .GT. y14) THEN
                    a4(3, i, k) = y14
                  ELSE
                    a4(3, i, k) = x6
                  END IF
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                END IF
              END IF
            END DO
          ELSE
            IF (kord .GE. 0.) THEN
              abs5 = kord
            ELSE
              abs5 = -kord
            END IF
            IF (abs5 .EQ. 12) THEN
              DO i=i1,i2
                IF (extm(i, k)) THEN
! grid-scale 2-delta-z wave detected
                  a4(2, i, k) = a4(1, i, k)
                  a4(3, i, k) = a4(1, i, k)
                  a4(4, i, k) = 0.
                ELSE
! not a local extremum
                  a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3, i&
&                   , k))
                  IF (a4(4, i, k) .GE. 0.) THEN
                    abs6 = a4(4, i, k)
                  ELSE
                    abs6 = -a4(4, i, k)
                  END IF
                  IF (a4(2, i, k) - a4(3, i, k) .GE. 0.) THEN
                    abs10 = a4(2, i, k) - a4(3, i, k)
                  ELSE
                    abs10 = -(a4(2, i, k)-a4(3, i, k))
                  END IF
! Check within the smooth region if subgrid profile is non-monotonic
                  IF (abs6 .GT. abs10) THEN
                    pmp_1 = a4(1, i, k) - 2.*gam(i, k+1)
                    lac_1 = pmp_1 + 1.5*gam(i, k+2)
                    IF (a4(1, i, k) .GT. pmp_1) THEN
                      IF (pmp_1 .GT. lac_1) THEN
                        y23 = lac_1
                      ELSE
                        y23 = pmp_1
                      END IF
                    ELSE IF (a4(1, i, k) .GT. lac_1) THEN
                      y23 = lac_1
                    ELSE
                      y23 = a4(1, i, k)
                    END IF
                    IF (a4(2, i, k) .LT. y23) THEN
                      x7 = y23
                    ELSE
                      x7 = a4(2, i, k)
                    END IF
                    IF (a4(1, i, k) .LT. pmp_1) THEN
                      IF (pmp_1 .LT. lac_1) THEN
                        y15 = lac_1
                      ELSE
                        y15 = pmp_1
                      END IF
                    ELSE IF (a4(1, i, k) .LT. lac_1) THEN
                      y15 = lac_1
                    ELSE
                      y15 = a4(1, i, k)
                    END IF
                    IF (x7 .GT. y15) THEN
                      a4(2, i, k) = y15
                    ELSE
                      a4(2, i, k) = x7
                    END IF
                    pmp_2 = a4(1, i, k) + 2.*gam(i, k)
                    lac_2 = pmp_2 - 1.5*gam(i, k-1)
                    IF (a4(1, i, k) .GT. pmp_2) THEN
                      IF (pmp_2 .GT. lac_2) THEN
                        y24 = lac_2
                      ELSE
                        y24 = pmp_2
                      END IF
                    ELSE IF (a4(1, i, k) .GT. lac_2) THEN
                      y24 = lac_2
                    ELSE
                      y24 = a4(1, i, k)
                    END IF
                    IF (a4(3, i, k) .LT. y24) THEN
                      x8 = y24
                    ELSE
                      x8 = a4(3, i, k)
                    END IF
                    IF (a4(1, i, k) .LT. pmp_2) THEN
                      IF (pmp_2 .LT. lac_2) THEN
                        y16 = lac_2
                      ELSE
                        y16 = pmp_2
                      END IF
                    ELSE IF (a4(1, i, k) .LT. lac_2) THEN
                      y16 = lac_2
                    ELSE
                      y16 = a4(1, i, k)
                    END IF
                    IF (x8 .GT. y16) THEN
                      a4(3, i, k) = y16
                    ELSE
                      a4(3, i, k) = x8
                    END IF
                    a4(4, i, k) = 6.*a4(1, i, k) - 3.*(a4(2, i, k)+a4(3&
&                     , i, k))
                  END IF
                END IF
              END DO
            ELSE
              IF (kord .GE. 0.) THEN
                abs7 = kord
              ELSE
                abs7 = -kord
              END IF
              IF (abs7 .EQ. 14) THEN
                DO i=i1,i2
                  a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i&
&                   , k)))
                END DO
              ELSE
! kord = 11, 12, 13
                DO i=i1,i2
                  IF (extm(i, k) .AND. (extm(i, k-1) .OR. extm(i, k+1))&
&                 ) THEN
! Noisy region:
                    a4(2, i, k) = a4(1, i, k)
                    a4(3, i, k) = a4(1, i, k)
                    a4(4, i, k) = 0.
                  ELSE
                    a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, &
&                     i, k)))
                  END IF
                END DO
              END IF
            END IF
          END IF
        END IF
      END IF
! Additional constraint to ensure positivity
      IF (iv .EQ. 0) CALL CS_LIMITERS(im, extm(i1, k), a4(1, i1, k), 0)
    END DO
! k-loop
!----------------------------------
! Bottom layer subgrid constraints:
!----------------------------------
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(3, i, km)) THEN
          a4(3, i, km) = a4(3, i, km)
        ELSE
          a4(3, i, km) = 0.
        END IF
      END DO
    ELSE IF (iv .EQ. -1) THEN
      DO i=i1,i2
        IF (a4(3, i, km)*a4(1, i, km) .LE. 0.) a4(3, i, km) = 0.
      END DO
    END IF
    DO k=km-1,km
      DO i=i1,i2
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
      IF (k .EQ. km - 1) CALL CS_LIMITERS(im, extm(i1, k), a4(1, i1, k)&
&                                   , 2)
      IF (k .EQ. km) CALL CS_LIMITERS(im, extm(i1, k), a4(1, i1, k), 1)
    END DO
  END SUBROUTINE CS_PROFILE
  SUBROUTINE CS_LIMITERS(im, extm, a4, iv)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: im
    INTEGER, INTENT(IN) :: iv
    LOGICAL, INTENT(IN) :: extm(im)
! PPM array
    REAL(fvprc), INTENT(INOUT) :: a4(4, im)
! !LOCAL VARIABLES:
    REAL(fvprc) :: da1, da2, a6da
    INTEGER :: i
    INTRINSIC ABS
    REAL(fvprc) :: abs0
    IF (iv .EQ. 0) THEN
! Positive definite constraint
      DO i=1,im
        IF (a4(1, i) .LE. 0.) THEN
          a4(2, i) = a4(1, i)
          a4(3, i) = a4(1, i)
          a4(4, i) = 0.
        ELSE
          IF (a4(3, i) - a4(2, i) .GE. 0.) THEN
            abs0 = a4(3, i) - a4(2, i)
          ELSE
            abs0 = -(a4(3, i)-a4(2, i))
          END IF
          IF (abs0 .LT. -a4(4, i)) THEN
            IF (a4(1, i) + 0.25*(a4(3, i)-a4(2, i))**2/a4(4, i) + a4(4, &
&               i)*r12 .LT. 0.) THEN
! local minimum is negative
              IF (a4(1, i) .LT. a4(3, i) .AND. a4(1, i) .LT. a4(2, i)) &
&             THEN
                a4(3, i) = a4(1, i)
                a4(2, i) = a4(1, i)
                a4(4, i) = 0.
              ELSE IF (a4(3, i) .GT. a4(2, i)) THEN
                a4(4, i) = 3.*(a4(2, i)-a4(1, i))
                a4(3, i) = a4(2, i) - a4(4, i)
              ELSE
                a4(4, i) = 3.*(a4(3, i)-a4(1, i))
                a4(2, i) = a4(3, i) - a4(4, i)
              END IF
            END IF
          END IF
        END IF
      END DO
    ELSE IF (iv .EQ. 1) THEN
      DO i=1,im
        IF ((a4(1, i)-a4(2, i))*(a4(1, i)-a4(3, i)) .GE. 0.) THEN
          a4(2, i) = a4(1, i)
          a4(3, i) = a4(1, i)
          a4(4, i) = 0.
        ELSE
          da1 = a4(3, i) - a4(2, i)
          da2 = da1**2
          a6da = a4(4, i)*da1
          IF (a6da .LT. -da2) THEN
            a4(4, i) = 3.*(a4(2, i)-a4(1, i))
            a4(3, i) = a4(2, i) - a4(4, i)
          ELSE IF (a6da .GT. da2) THEN
            a4(4, i) = 3.*(a4(3, i)-a4(1, i))
            a4(2, i) = a4(3, i) - a4(4, i)
          END IF
        END IF
      END DO
    ELSE
! Standard PPM constraint
      DO i=1,im
        IF (extm(i)) THEN
          a4(2, i) = a4(1, i)
          a4(3, i) = a4(1, i)
          a4(4, i) = 0.
        ELSE
          da1 = a4(3, i) - a4(2, i)
          da2 = da1**2
          a6da = a4(4, i)*da1
          IF (a6da .LT. -da2) THEN
            a4(4, i) = 3.*(a4(2, i)-a4(1, i))
            a4(3, i) = a4(2, i) - a4(4, i)
          ELSE IF (a6da .GT. da2) THEN
            a4(4, i) = 3.*(a4(3, i)-a4(1, i))
            a4(2, i) = a4(3, i) - a4(4, i)
          END IF
        END IF
      END DO
    END IF
  END SUBROUTINE CS_LIMITERS
  SUBROUTINE PPM_PROFILE(a4, delp, km, i1, i2, iv, kord)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
! iv = 2: temp (if remap_t) and w (iv=-2)
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! Order (or more accurately method no.):
    INTEGER, INTENT(IN) :: kord
! 
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
! !INPUT/OUTPUT PARAMETERS:
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
! DESCRIPTION:
!
!   Perform the piecewise parabolic reconstruction
! 
! !REVISION HISTORY: 
! S.-J. Lin   revised at GFDL 2007
!-----------------------------------------------------------------------
! local arrays:
    REAL(fvprc) :: dc(i1:i2, km)
    REAL(fvprc) :: h2(i1:i2, km)
    REAL(fvprc) :: delq(i1:i2, km)
    REAL(fvprc) :: df2(i1:i2, km)
    REAL(fvprc) :: d4(i1:i2, km)
! local scalars:
    INTEGER :: i, k, km1, lmt, it
    REAL(fvprc) :: fac
    REAL(fvprc) :: a1, a2, c1, c2, c3, d1, d2
    REAL(fvprc) :: qm, dq, lac, qmp, pmp
    INTRINSIC ABS
    INTRINSIC MAX
    INTRINSIC MIN
    INTRINSIC SIGN
    REAL(fvprc) :: min1
    INTEGER :: abs0
    REAL(fvprc) :: max1
    REAL(fvprc) :: min2
    REAL(fvprc) :: x3
    REAL(fvprc) :: x2
    REAL(fvprc) :: x1
    REAL(fvprc) :: z1
    REAL(fvprc) :: y9
    REAL(fvprc) :: y8
    REAL(fvprc) :: y7
    REAL(fvprc) :: y6
    REAL(fvprc) :: y5
    REAL(fvprc) :: y4
    REAL(fvprc) :: y3
    REAL(fvprc) :: y2
    REAL(fvprc) :: y1
    km1 = km - 1
    it = i2 - i1 + 1
    DO k=2,km
      DO i=i1,i2
        delq(i, k-1) = a4(1, i, k) - a4(1, i, k-1)
        d4(i, k) = delp(i, k-1) + delp(i, k)
      END DO
    END DO
    DO k=2,km1
      DO i=i1,i2
        c1 = (delp(i, k-1)+0.5*delp(i, k))/d4(i, k+1)
        c2 = (delp(i, k+1)+0.5*delp(i, k))/d4(i, k)
        df2(i, k) = delp(i, k)*(c1*delq(i, k)+c2*delq(i, k-1))/(d4(i, k)&
&         +delp(i, k+1))
        IF (df2(i, k) .GE. 0.) THEN
          x1 = df2(i, k)
        ELSE
          x1 = -df2(i, k)
        END IF
        IF (a4(1, i, k-1) .LT. a4(1, i, k)) THEN
          IF (a4(1, i, k) .LT. a4(1, i, k+1)) THEN
            max1 = a4(1, i, k+1)
          ELSE
            max1 = a4(1, i, k)
          END IF
        ELSE IF (a4(1, i, k-1) .LT. a4(1, i, k+1)) THEN
          max1 = a4(1, i, k+1)
        ELSE
          max1 = a4(1, i, k-1)
        END IF
        y1 = max1 - a4(1, i, k)
        IF (a4(1, i, k-1) .GT. a4(1, i, k)) THEN
          IF (a4(1, i, k) .GT. a4(1, i, k+1)) THEN
            min2 = a4(1, i, k+1)
          ELSE
            min2 = a4(1, i, k)
          END IF
        ELSE IF (a4(1, i, k-1) .GT. a4(1, i, k+1)) THEN
          min2 = a4(1, i, k+1)
        ELSE
          min2 = a4(1, i, k-1)
        END IF
        z1 = a4(1, i, k) - min2
        IF (x1 .GT. y1) THEN
          IF (y1 .GT. z1) THEN
            min1 = z1
          ELSE
            min1 = y1
          END IF
        ELSE IF (x1 .GT. z1) THEN
          min1 = z1
        ELSE
          min1 = x1
        END IF
        dc(i, k) = SIGN(min1, df2(i, k))
      END DO
    END DO
!-----------------------------------------------------------
! 4th order interpolation of the provisional cell edge value
!-----------------------------------------------------------
    DO k=3,km1
      DO i=i1,i2
        c1 = delq(i, k-1)*delp(i, k-1)/d4(i, k)
        a1 = d4(i, k-1)/(d4(i, k)+delp(i, k-1))
        a2 = d4(i, k+1)/(d4(i, k)+delp(i, k))
        a4(2, i, k) = a4(1, i, k-1) + c1 + 2./(d4(i, k-1)+d4(i, k+1))*(&
&         delp(i, k)*(c1*(a1-a2)+a2*dc(i, k-1))-delp(i, k-1)*a1*dc(i, k)&
&         )
      END DO
    END DO
    IF (km .GT. 8 .AND. kord .GT. 4) CALL STEEPZ(i1, i2, km, a4, df2, dc&
&                                          , delq, delp, d4)
! Area preserving cubic with 2nd deriv. = 0 at the boundaries
! Top
    DO i=i1,i2
      d1 = delp(i, 1)
      d2 = delp(i, 2)
      qm = (d2*a4(1, i, 1)+d1*a4(1, i, 2))/(d1+d2)
      dq = 2.*(a4(1, i, 2)-a4(1, i, 1))/(d1+d2)
      c1 = 4.*(a4(2, i, 3)-qm-d2*dq)/(d2*(2.*d2*d2+d1*(d2+3.*d1)))
      c3 = dq - 0.5*c1*(d2*(5.*d1+d2)-3.*d1*d1)
      a4(2, i, 2) = qm - 0.25*c1*d1*d2*(d2+3.*d1)
! Top edge:
!-------------------------------------------------------
      a4(2, i, 1) = d1*(2.*c1*d1**2-c3) + a4(2, i, 2)
      IF (a4(1, i, 1) .GT. a4(1, i, 2)) THEN
        y2 = a4(1, i, 2)
      ELSE
        y2 = a4(1, i, 1)
      END IF
      IF (a4(2, i, 2) .LT. y2) THEN
        a4(2, i, 2) = y2
      ELSE
        a4(2, i, 2) = a4(2, i, 2)
      END IF
      IF (a4(1, i, 1) .LT. a4(1, i, 2)) THEN
        y3 = a4(1, i, 2)
      ELSE
        y3 = a4(1, i, 1)
      END IF
      IF (a4(2, i, 2) .GT. y3) THEN
        a4(2, i, 2) = y3
      ELSE
        a4(2, i, 2) = a4(2, i, 2)
      END IF
      dc(i, 1) = 0.5*(a4(2, i, 2)-a4(1, i, 1))
    END DO
! Enforce monotonicity  within the top layer
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(2, i, 1)) THEN
          a4(2, i, 1) = a4(2, i, 1)
        ELSE
          a4(2, i, 1) = 0.
        END IF
        IF (0. .LT. a4(2, i, 2)) THEN
          a4(2, i, 2) = a4(2, i, 2)
        ELSE
          a4(2, i, 2) = 0.
        END IF
      END DO
    ELSE IF (iv .EQ. -1) THEN
      DO i=i1,i2
        IF (a4(2, i, 1)*a4(1, i, 1) .LE. 0.) a4(2, i, 1) = 0.
      END DO
    ELSE
      IF (iv .GE. 0.) THEN
        abs0 = iv
      ELSE
        abs0 = -iv
      END IF
      IF (abs0 .EQ. 2) THEN
        DO i=i1,i2
          a4(2, i, 1) = a4(1, i, 1)
          a4(3, i, 1) = a4(1, i, 1)
        END DO
      END IF
    END IF
! Bottom
! Area preserving cubic with 2nd deriv. = 0 at the surface
    DO i=i1,i2
      d1 = delp(i, km)
      d2 = delp(i, km1)
      qm = (d2*a4(1, i, km)+d1*a4(1, i, km1))/(d1+d2)
      dq = 2.*(a4(1, i, km1)-a4(1, i, km))/(d1+d2)
      c1 = (a4(2, i, km1)-qm-d2*dq)/(d2*(2.*d2*d2+d1*(d2+3.*d1)))
      c3 = dq - 2.0*c1*(d2*(5.*d1+d2)-3.*d1*d1)
      a4(2, i, km) = qm - c1*d1*d2*(d2+3.*d1)
! Bottom edge:
!-----------------------------------------------------
      a4(3, i, km) = d1*(8.*c1*d1**2-c3) + a4(2, i, km)
      IF (a4(1, i, km) .GT. a4(1, i, km1)) THEN
        y4 = a4(1, i, km1)
      ELSE
        y4 = a4(1, i, km)
      END IF
      IF (a4(2, i, km) .LT. y4) THEN
        a4(2, i, km) = y4
      ELSE
        a4(2, i, km) = a4(2, i, km)
      END IF
      IF (a4(1, i, km) .LT. a4(1, i, km1)) THEN
        y5 = a4(1, i, km1)
      ELSE
        y5 = a4(1, i, km)
      END IF
      IF (a4(2, i, km) .GT. y5) THEN
        a4(2, i, km) = y5
      ELSE
        a4(2, i, km) = a4(2, i, km)
      END IF
      dc(i, km) = 0.5*(a4(1, i, km)-a4(2, i, km))
    END DO
! Enforce constraint on the "slope" at the surface
!#ifdef BOT_MONO
!      do i=i1,i2
!         a4(4,i,km) = 0
!         if( a4(3,i,km) * a4(1,i,km) <= 0. ) a4(3,i,km) = 0.
!         d1 = a4(1,i,km) - a4(2,i,km)
!         d2 = a4(3,i,km) - a4(1,i,km)
!         if ( d1*d2 < 0. ) then
!              a4(2,i,km) = a4(1,i,km)
!              a4(3,i,km) = a4(1,i,km)
!         else
!              dq = sign(min(abs(d1),abs(d2),0.5*abs(delq(i,km-1))), d1)
!              a4(2,i,km) = a4(1,i,km) - dq
!              a4(3,i,km) = a4(1,i,km) + dq
!         endif
!      enddo
!#else
    IF (iv .EQ. 0) THEN
      DO i=i1,i2
        IF (0. .LT. a4(2, i, km)) THEN
          a4(2, i, km) = a4(2, i, km)
        ELSE
          a4(2, i, km) = 0.
        END IF
        IF (0. .LT. a4(3, i, km)) THEN
          a4(3, i, km) = a4(3, i, km)
        ELSE
          a4(3, i, km) = 0.
        END IF
      END DO
    ELSE IF (iv .LT. 0) THEN
      DO i=i1,i2
        IF (a4(1, i, km)*a4(3, i, km) .LE. 0.) a4(3, i, km) = 0.
      END DO
    END IF
!#endif
    DO k=1,km1
      DO i=i1,i2
        a4(3, i, k) = a4(2, i, k+1)
      END DO
    END DO
!-----------------------------------------------------------
! f(s) = AL + s*[(AR-AL) + A6*(1-s)]         ( 0 <= s  <= 1 )
!-----------------------------------------------------------
! Top 2 and bottom 2 layers always use monotonic mapping
    DO k=1,2
      DO i=i1,i2
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
      CALL PPM_LIMITERS(dc(i1, k), a4(1, i1, k), it, 0)
    END DO
    IF (kord .GE. 7) THEN
!-----------------------
! Huynh's 2nd constraint
!-----------------------
      DO k=2,km1
        DO i=i1,i2
! Method#1
!           h2(i,k) = delq(i,k) - delq(i,k-1)
! Method#2 - better
          h2(i, k) = 2.*(dc(i, k+1)/delp(i, k+1)-dc(i, k-1)/delp(i, k-1)&
&           )/(delp(i, k)+0.5*(delp(i, k-1)+delp(i, k+1)))*delp(i, k)**2
        END DO
      END DO
! Method#3
!!!            h2(i,k) = dc(i,k+1) - dc(i,k-1)
! original quasi-monotone
      fac = 1.5
      DO k=3,km-2
        DO i=i1,i2
! Right edges
!        qmp   = a4(1,i,k) + 2.0*delq(i,k-1)
!        lac   = a4(1,i,k) + fac*h2(i,k-1) + 0.5*delq(i,k-1)
!
          pmp = 2.*dc(i, k)
          qmp = a4(1, i, k) + pmp
          lac = a4(1, i, k) + fac*h2(i, k-1) + dc(i, k)
          IF (a4(1, i, k) .GT. qmp) THEN
            IF (qmp .GT. lac) THEN
              y8 = lac
            ELSE
              y8 = qmp
            END IF
          ELSE IF (a4(1, i, k) .GT. lac) THEN
            y8 = lac
          ELSE
            y8 = a4(1, i, k)
          END IF
          IF (a4(3, i, k) .LT. y8) THEN
            x2 = y8
          ELSE
            x2 = a4(3, i, k)
          END IF
          IF (a4(1, i, k) .LT. qmp) THEN
            IF (qmp .LT. lac) THEN
              y6 = lac
            ELSE
              y6 = qmp
            END IF
          ELSE IF (a4(1, i, k) .LT. lac) THEN
            y6 = lac
          ELSE
            y6 = a4(1, i, k)
          END IF
          IF (x2 .GT. y6) THEN
            a4(3, i, k) = y6
          ELSE
            a4(3, i, k) = x2
          END IF
! Left  edges
!        qmp   = a4(1,i,k) - 2.0*delq(i,k)
!        lac   = a4(1,i,k) + fac*h2(i,k+1) - 0.5*delq(i,k)
!
          qmp = a4(1, i, k) - pmp
          lac = a4(1, i, k) + fac*h2(i, k+1) - dc(i, k)
          IF (a4(1, i, k) .GT. qmp) THEN
            IF (qmp .GT. lac) THEN
              y9 = lac
            ELSE
              y9 = qmp
            END IF
          ELSE IF (a4(1, i, k) .GT. lac) THEN
            y9 = lac
          ELSE
            y9 = a4(1, i, k)
          END IF
          IF (a4(2, i, k) .LT. y9) THEN
            x3 = y9
          ELSE
            x3 = a4(2, i, k)
          END IF
          IF (a4(1, i, k) .LT. qmp) THEN
            IF (qmp .LT. lac) THEN
              y7 = lac
            ELSE
              y7 = qmp
            END IF
          ELSE IF (a4(1, i, k) .LT. lac) THEN
            y7 = lac
          ELSE
            y7 = a4(1, i, k)
          END IF
          IF (x3 .GT. y7) THEN
            a4(2, i, k) = y7
          ELSE
            a4(2, i, k) = x3
          END IF
!-------------
! Recompute A6
!-------------
          a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
        END DO
! Additional constraint to ensure positivity when kord=7
        IF (iv .EQ. 0 .AND. kord .GE. 6) CALL PPM_LIMITERS(dc(i1, k), a4&
&                                                    (1, i1, k), it, 2)
      END DO
    ELSE
      lmt = kord - 3
      IF (0 .LT. lmt) THEN
        lmt = lmt
      ELSE
        lmt = 0
      END IF
      IF (iv .EQ. 0) THEN
        IF (2 .GT. lmt) THEN
          lmt = lmt
        ELSE
          lmt = 2
        END IF
      END IF
      DO k=3,km-2
        IF (kord .NE. 4) THEN
          DO i=i1,i2
            a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
          END DO
        END IF
        IF (kord .NE. 6) CALL PPM_LIMITERS(dc(i1, k), a4(1, i1, k), it, &
&                                    lmt)
      END DO
    END IF
    DO k=km1,km
      DO i=i1,i2
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
      CALL PPM_LIMITERS(dc(i1, k), a4(1, i1, k), it, 0)
    END DO
  END SUBROUTINE PPM_PROFILE
  SUBROUTINE PPM_LIMITERS(dm, a4, itot, lmt)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! the linear slope
    REAL(fvprc), INTENT(IN) :: dm(*)
! Total Longitudes
    INTEGER, INTENT(IN) :: itot
! 0: Standard PPM constraint
    INTEGER, INTENT(IN) :: lmt
! 1: Improved full monotonicity constraint (Lin)
! 2: Positive definite constraint
! 3: do nothing (return immediately)
! !INPUT/OUTPUT PARAMETERS:
! PPM array
    REAL(fvprc), INTENT(INOUT) :: a4(4, *)
! AA <-- a4(1,i)
! AL <-- a4(2,i)
! AR <-- a4(3,i)
! A6 <-- a4(4,i)
! !LOCAL VARIABLES:
    REAL(fvprc) :: qmp
    REAL(fvprc) :: da1, da2, a6da
    REAL(fvprc) :: fmin
    INTEGER :: i
    INTRINSIC ABS
    INTRINSIC MIN
    INTRINSIC SIGN
    REAL(fvprc) :: min1
    REAL(fvprc) :: min2
    REAL(fvprc) :: abs0
    REAL(fvprc) :: x2
    REAL(fvprc) :: x1
    REAL(fvprc) :: y2
    REAL(fvprc) :: y1
! Developer: S.-J. Lin
    IF (lmt .EQ. 3) THEN
      RETURN
    ELSE IF (lmt .EQ. 0) THEN
! Standard PPM constraint
      DO i=1,itot
        IF (dm(i) .LT. 10.e-10) THEN
!dh fix to prevent tap complaint about real equality test
          a4(2, i) = a4(1, i)
          a4(3, i) = a4(1, i)
          a4(4, i) = 0.
        ELSE
          da1 = a4(3, i) - a4(2, i)
          da2 = da1**2
          a6da = a4(4, i)*da1
          IF (a6da .LT. -da2) THEN
            a4(4, i) = 3.*(a4(2, i)-a4(1, i))
            a4(3, i) = a4(2, i) - a4(4, i)
          ELSE IF (a6da .GT. da2) THEN
            a4(4, i) = 3.*(a4(3, i)-a4(1, i))
            a4(2, i) = a4(3, i) - a4(4, i)
          END IF
        END IF
      END DO
    ELSE IF (lmt .EQ. 1) THEN
! Improved full monotonicity constraint (Lin 2004)
! Note: no need to provide first guess of A6 <-- a4(4,i)
      DO i=1,itot
        qmp = 2.*dm(i)
        IF (qmp .GE. 0.) THEN
          x1 = qmp
        ELSE
          x1 = -qmp
        END IF
        IF (a4(2, i) - a4(1, i) .GE. 0.) THEN
          y1 = a4(2, i) - a4(1, i)
        ELSE
          y1 = -(a4(2, i)-a4(1, i))
        END IF
        IF (x1 .GT. y1) THEN
          min1 = y1
        ELSE
          min1 = x1
        END IF
        a4(2, i) = a4(1, i) - SIGN(min1, qmp)
        IF (qmp .GE. 0.) THEN
          x2 = qmp
        ELSE
          x2 = -qmp
        END IF
        IF (a4(3, i) - a4(1, i) .GE. 0.) THEN
          y2 = a4(3, i) - a4(1, i)
        ELSE
          y2 = -(a4(3, i)-a4(1, i))
        END IF
        IF (x2 .GT. y2) THEN
          min2 = y2
        ELSE
          min2 = x2
        END IF
        a4(3, i) = a4(1, i) + SIGN(min2, qmp)
        a4(4, i) = 3.*(2.*a4(1, i)-(a4(2, i)+a4(3, i)))
      END DO
    ELSE IF (lmt .EQ. 2) THEN
! Positive definite constraint
      DO i=1,itot
        IF (a4(3, i) - a4(2, i) .GE. 0.) THEN
          abs0 = a4(3, i) - a4(2, i)
        ELSE
          abs0 = -(a4(3, i)-a4(2, i))
        END IF
        IF (abs0 .LT. -a4(4, i)) THEN
          fmin = a4(1, i) + 0.25*(a4(3, i)-a4(2, i))**2/a4(4, i) + a4(4&
&           , i)*r12
          IF (fmin .LT. 0.) THEN
            IF (a4(1, i) .LT. a4(3, i) .AND. a4(1, i) .LT. a4(2, i)) &
&           THEN
              a4(3, i) = a4(1, i)
              a4(2, i) = a4(1, i)
              a4(4, i) = 0.
            ELSE IF (a4(3, i) .GT. a4(2, i)) THEN
              a4(4, i) = 3.*(a4(2, i)-a4(1, i))
              a4(3, i) = a4(2, i) - a4(4, i)
            ELSE
              a4(4, i) = 3.*(a4(3, i)-a4(1, i))
              a4(2, i) = a4(3, i) - a4(4, i)
            END IF
          END IF
        END IF
      END DO
    END IF
  END SUBROUTINE PPM_LIMITERS
  SUBROUTINE STEEPZ(i1, i2, km, a4, df2, dm, dq, dp, d4)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: km, i1, i2
! grid size
    REAL(fvprc), INTENT(IN) :: dp(i1:i2, km)
! backward diff of q
    REAL(fvprc), INTENT(IN) :: dq(i1:i2, km)
! backward sum:  dp(k)+ dp(k-1) 
    REAL(fvprc), INTENT(IN) :: d4(i1:i2, km)
! first guess mismatch
    REAL(fvprc), INTENT(IN) :: df2(i1:i2, km)
! monotonic mismatch
    REAL(fvprc), INTENT(IN) :: dm(i1:i2, km)
! !INPUT/OUTPUT PARAMETERS:
! first guess/steepened
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
! !LOCAL VARIABLES:
    INTEGER :: i, k
    REAL(fvprc) :: alfa(i1:i2, km)
    REAL(fvprc) :: f(i1:i2, km)
    REAL(fvprc) :: rat(i1:i2, km)
    REAL(fvprc) :: dg2
    INTRINSIC ABS
    INTRINSIC MIN
    INTRINSIC MAX
    REAL(fvprc) :: abs0
    REAL(fvprc) :: y1
! Compute ratio of dq/dp
    DO k=2,km
      DO i=i1,i2
        rat(i, k) = dq(i, k-1)/d4(i, k)
      END DO
    END DO
! Compute F
    DO k=2,km-1
      DO i=i1,i2
        f(i, k) = (rat(i, k+1)-rat(i, k))/(dp(i, k-1)+dp(i, k)+dp(i, k+1&
&         ))
      END DO
    END DO
    DO k=3,km-2
      DO i=i1,i2
        IF (df2(i, k) .GE. 0.) THEN
          abs0 = df2(i, k)
        ELSE
          abs0 = -df2(i, k)
        END IF
        IF (f(i, k+1)*f(i, k-1) .LT. 0. .AND. abs0 .GT. 10.e-10) THEN
!dh fix to prevenet tap complaint on real equality test
          dg2 = (f(i, k+1)-f(i, k-1))*((dp(i, k+1)-dp(i, k-1))**2+d4(i, &
&           k)*d4(i, k+1))
          IF (0.5 .GT. -(0.1875*dg2/df2(i, k))) THEN
            y1 = -(0.1875*dg2/df2(i, k))
          ELSE
            y1 = 0.5
          END IF
          IF (0. .LT. y1) THEN
            alfa(i, k) = y1
          ELSE
            alfa(i, k) = 0.
          END IF
        ELSE
          alfa(i, k) = 0.
        END IF
      END DO
    END DO
    DO k=4,km-2
      DO i=i1,i2
        a4(2, i, k) = (1.-alfa(i, k-1)-alfa(i, k))*a4(2, i, k) + alfa(i&
&         , k-1)*(a4(1, i, k)-dm(i, k)) + alfa(i, k)*(a4(1, i, k-1)+dm(i&
&         , k-1))
      END DO
    END DO
  END SUBROUTINE STEEPZ
!  Differentiation of remap_z in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_m
!od.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_
!halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core
!_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.r
!ayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_
!to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_ene
!rgy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz
!_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz 
!fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solv
!er_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_co
!re_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergenc
!e_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_
!mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_
!mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core
!_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 q2
!   with respect to varying inputs: pe1 pe2 q2
  SUBROUTINE REMAP_Z_FWD(km, pe1, kn, pe2, q2, i1, i2, iv, kord)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! Method order
    INTEGER, INTENT(IN) :: kord
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
    INTEGER, INTENT(IN) :: iv
! height at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! hieght at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
!      real(FVPRC), intent(in) ::  q1(i1:i2,km)        ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(i1:i2, kn)
! !LOCAL VARIABLES:
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, delp, esl
    INTEGER :: i, k, l, m, k0
    INTEGER :: ad_count
    INTEGER :: ad_count0
    DO k=1,km
      DO i=i1,i2
! negative
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q2(i, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL CS_PROFILE_LINEAR_FWD(qs, q4, dp1, km, i1, i2, iv)
        CALL PUSHCONTROL2B_FV(2)
      ELSE
        CALL PUSHCONTROL2B_FV(1)
      END IF
    ELSE
      CALL PUSHCONTROL2B_FV(0)
    END IF
!call  cs_profile( qs, q4, dp1, km, i1, i2, iv, kord )
!call ppm_profile( q4, dp1, km, i1, i2, iv, kord )
! Mapping
    DO i=i1,i2
      k0 = 1
      DO 120 k=1,kn
        CALL PUSHINTEGER(l)
        ad_count = 1
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .LE. pe1(i, l) .AND. pe2(i, k) .GE. pe1(i, l+1)&
&         ) THEN
            GOTO 100
          ELSE
            CALL PUSHINTEGER(l)
            ad_count = ad_count + 1
          END IF
        END DO
        CALL PUSHCONTROL1B_FV(0)
        CALL PUSHINTEGER(ad_count)
        CALL PUSHCONTROL2B_FV(2)
        GOTO 123
 100    CALL PUSHCONTROL1B_FV(1)
        CALL PUSHINTEGER(ad_count)
        pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
        IF (pe2(i, k+1) .GE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          CALL PUSHREALARRAY(q2(i, k))
          q2(i, k) = q4(2, i, l) + 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2, i&
&           , l))*(pr+pl) - q4(4, i, l)*r3*(pr*(pr+pl)+pl**2)
          k0 = l
          CALL PUSHCONTROL1B_FV(0)
          GOTO 120
        ELSE
! Fractional area...
          CALL PUSHREALARRAY(qsum)
          qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, l)+&
&           q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+pl*(1.+&
&           pl))))
          CALL PUSHINTEGER(m)
          ad_count0 = 1
          DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
            IF (pe2(i, k+1) .LT. pe1(i, m+1)) THEN
! Whole layer..
              qsum = qsum + dp1(i, m)*q4(1, i, m)
              CALL PUSHINTEGER(m)
              ad_count0 = ad_count0 + 1
            ELSE
              GOTO 110
            END IF
          END DO
          CALL PUSHCONTROL1B_FV(0)
          CALL PUSHINTEGER(ad_count0)
          CALL PUSHCONTROL2B_FV(1)
          GOTO 123
 110      CALL PUSHCONTROL1B_FV(1)
          CALL PUSHINTEGER(ad_count0)
          delp = pe2(i, k+1) - pe1(i, m)
          esl = delp/dp1(i, m)
          qsum = qsum + delp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(2, i, &
&           m)+q4(4, i, m)*(1.-r23*esl)))
          k0 = m
          CALL PUSHCONTROL2B_FV(0)
        END IF
 123    CALL PUSHREALARRAY(q2(i, k))
        q2(i, k) = qsum/(pe2(i, k+1)-pe2(i, k))
        CALL PUSHCONTROL1B_FV(1)
 120  CONTINUE
    END DO
    CALL PUSHINTEGER(l)
    CALL PUSHINTEGER(m)
    CALL PUSHREALARRAY(qs, 8*(i2-i1+1)/8)
    CALL PUSHREALARRAY(dp1, 8*(i2-i1+1)*km/8)
    CALL PUSHREALARRAY(qsum)
    CALL PUSHREALARRAY(q4, 8*4*(i2-i1+1)*km/8)
  END SUBROUTINE REMAP_Z_FWD
!  Differentiation of remap_z in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_
!mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe
!_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_cor
!e_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.
!rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed
!_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_en
!ergy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_map
!z_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz
! fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_sol
!ver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_c
!ore_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergen
!ce_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core
!_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core
!_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_cor
!e_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 q2
!   with respect to varying inputs: pe1 pe2 q2
  SUBROUTINE REMAP_Z_BWD(km, pe1, pe1_ad, kn, pe2, pe2_ad, q2, q2_ad&
&   , i1, i2, iv, kord)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i1
    INTEGER, INTENT(IN) :: i2
    INTEGER, INTENT(IN) :: kord
    INTEGER, INTENT(IN) :: km
    INTEGER, INTENT(IN) :: kn
    INTEGER, INTENT(IN) :: iv
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
    REAL(fvprc), INTENT(INOUT) :: q2(i1:i2, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_ad(i1:i2, kn)
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: qs_ad(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_ad(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: q4_ad(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, delp, esl
    REAL(fvprc) :: pl_ad, pr_ad, qsum_ad, delp_ad, esl_ad
    INTEGER :: i, k, l, m, k0
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp_ad11
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: branch
    INTEGER :: ad_count0
    INTEGER :: i3
    CALL POPREALARRAY(q4, 8*4*(i2-i1+1)*km/8)
    CALL POPREALARRAY(qsum)
    CALL POPREALARRAY(dp1, 8*(i2-i1+1)*km/8)
    CALL POPREALARRAY(qs, 8*(i2-i1+1)/8)
    CALL POPINTEGER(m)
    CALL POPINTEGER(l)
    dp1_ad = 0.0_FVPRC
    qsum_ad = 0.0_FVPRC
    q4_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL1B_FV(branch)
        IF (branch .EQ. 0) THEN
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY(q2(i, k))
          temp_ad0 = 0.5*(pr+pl)*q2_ad(i, k)
          temp_ad1 = 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2, i, l))*q2_ad(i, &
&           k)
          temp_ad2 = -(r3*q4(4, i, l)*q2_ad(i, k))
          q4_ad(2, i, l) = q4_ad(2, i, l) + q2_ad(i, k) - temp_ad0
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad0 - r3*(pr*(pr+pl)+pl&
&           **2)*q2_ad(i, k)
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad0
          pr_ad = (2*pr+pl)*temp_ad2 + temp_ad1
          pl_ad = (2*pl+pr)*temp_ad2 + temp_ad1
          q2_ad(i, k) = 0.0_FVPRC
          temp_ad3 = pr_ad/dp1(i, l)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad3
          pe1_ad(i, l) = pe1_ad(i, l) - temp_ad3
          dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k+1)-pe1(i, l))*temp_ad3&
&           /dp1(i, l)
        ELSE
          CALL POPREALARRAY(q2(i, k))
          temp1 = pe2(i, k+1) - pe2(i, k)
          temp_ad11 = -(qsum*q2_ad(i, k)/temp1**2)
          qsum_ad = qsum_ad + q2_ad(i, k)/temp1
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad11
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad11
          q2_ad(i, k) = 0.0_FVPRC
          CALL POPCONTROL2B_FV(branch)
          IF (branch .EQ. 0) THEN
            delp = pe2(i, k+1) - pe1(i, m)
            esl = delp/dp1(i, m)
            temp0 = q4(3, i, m) - q4(2, i, m) + q4(4, i, m)*(-(r23*esl)+&
&             1.)
            temp_ad8 = delp*qsum_ad
            temp_ad9 = 0.5*esl*temp_ad8
            q4_ad(2, i, m) = q4_ad(2, i, m) + temp_ad8 - temp_ad9
            esl_ad = 0.5*temp0*temp_ad8 - q4(4, i, m)*r23*temp_ad9
            q4_ad(3, i, m) = q4_ad(3, i, m) + temp_ad9
            q4_ad(4, i, m) = q4_ad(4, i, m) + (1.-r23*esl)*temp_ad9
            temp_ad10 = esl_ad/dp1(i, m)
            delp_ad = temp_ad10 + (q4(2, i, m)+0.5*(esl*temp0))*qsum_ad
            dp1_ad(i, m) = dp1_ad(i, m) - delp*temp_ad10/dp1(i, m)
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + delp_ad
            pe1_ad(i, m) = pe1_ad(i, m) - delp_ad
          ELSE IF (branch .NE. 1) THEN
            GOTO 100
          END IF
          CALL POPINTEGER(ad_count0)
          DO i3=1,ad_count0
            IF (i3 .EQ. 1) THEN
              CALL POPCONTROL1B_FV(branch)
            ELSE
              dp1_ad(i, m) = dp1_ad(i, m) + q4(1, i, m)*qsum_ad
              q4_ad(1, i, m) = q4_ad(1, i, m) + dp1(i, m)*qsum_ad
            END IF
            CALL POPINTEGER(m)
          END DO
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY(qsum)
          temp = q4(4, i, l) + q4(3, i, l) - q4(2, i, l)
          temp_ad4 = (q4(2, i, l)+0.5*(temp*(pl+1.))-r3*(q4(4, i, l)*(pl&
&           *(pl+1.)+1.)))*qsum_ad
          temp_ad5 = (pe1(i, l+1)-pe2(i, k))*qsum_ad
          temp_ad6 = 0.5*(pl+1.)*temp_ad5
          temp_ad7 = -(r3*q4(4, i, l)*temp_ad5)
          pe1_ad(i, l+1) = pe1_ad(i, l+1) + temp_ad4
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad4
          q4_ad(2, i, l) = q4_ad(2, i, l) + temp_ad5 - temp_ad6
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad6 - r3*(pl*(pl+1.)+1.&
&           )*temp_ad5
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad6
          pl_ad = (2*pl+1.)*temp_ad7 + 0.5*temp*temp_ad5
          qsum_ad = 0.0_FVPRC
        END IF
        temp_ad = pl_ad/dp1(i, l)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad
        pe1_ad(i, l) = pe1_ad(i, l) - temp_ad
        dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k)-pe1(i, l))*temp_ad/dp1(&
&         i, l)
 100    CALL POPINTEGER(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B_FV(branch)
          CALL POPINTEGER(l)
        END DO
      END DO
    END DO
    CALL POPCONTROL2B_FV(branch)
    IF (branch .NE. 0) THEN
      IF (branch .NE. 1) THEN
        qs_ad = 0.0_FVPRC
        CALL CS_PROFILE_LINEAR_BWD(qs, qs_ad, q4, q4_ad, dp1, dp1_ad&
&                               , km, i1, i2, iv)
      END IF
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
        q2_ad(i, k) = q2_ad(i, k) + q4_ad(1, i, k)
        q4_ad(1, i, k) = 0.0_FVPRC
        pe1_ad(i, k+1) = pe1_ad(i, k+1) + dp1_ad(i, k)
        pe1_ad(i, k) = pe1_ad(i, k) - dp1_ad(i, k)
        dp1_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE REMAP_Z_BWD
!  Differentiation of map_scalar in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edg
!e_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.
!pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_c
!ore_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mo
!d.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cub
!ed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_
!energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_m
!apz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.stee
!pz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_s
!olver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh
!_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.diverg
!ence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_co
!re_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_co
!re_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_c
!ore_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 q2
!   with respect to varying inputs: pe1 pe2 q2
  SUBROUTINE MAP_SCALAR_FWD(km, pe1, qs, kn, pe2, q2, i1, i2, j, ibeg&
&   , iend, jbeg, jend, iv, kord, q_min)
    IMPLICIT NONE
! iv=1
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! Mode: 0 == constituents  1 == temp
    INTEGER, INTENT(IN) :: iv
!       2 == remap temp with cs scheme
! Method order
    INTEGER, INTENT(IN) :: kord
! Current latitude
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! bottom BC
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
! real(FVPRC), intent(in) ::    q1(ibeg:iend,jbeg:jend,km) ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc), INTENT(IN) :: q_min
! !DESCRIPTION:
! IV = 0: constituents
! pe1: pressure at layer edges (from model top to bottom surface)
!      in the original vertical coordinate
! pe2: pressure at layer edges (from model top to bottom surface)
!      in the new vertical coordinate
! !LOCAL VARIABLES:
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    INTEGER :: i, k, l, m, k0
    INTEGER :: ad_count
    INTEGER :: ad_count0
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q2(i, j, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL SCALAR_PROFILE_LINEAR_FWD(qs, q4, dp1, km, i1, i2, iv, &
&                                   q_min)
        CALL PUSHCONTROL2B_FV(2)
      ELSE
        CALL PUSHCONTROL2B_FV(1)
      END IF
    ELSE
      CALL PUSHCONTROL2B_FV(0)
    END IF
!call scalar_profile( qs, q4, dp1, km, i1, i2, iv, kord, q_min )
!call ppm_profile( q4, dp1, km, i1, i2, iv, kord )
    DO i=i1,i2
      k0 = 1
      DO 120 k=1,kn
        CALL PUSHINTEGER(l)
        ad_count = 1
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            GOTO 100
          ELSE
            CALL PUSHINTEGER(l)
            ad_count = ad_count + 1
          END IF
        END DO
        CALL PUSHCONTROL1B_FV(0)
        CALL PUSHINTEGER(ad_count)
        CALL PUSHCONTROL2B_FV(2)
        GOTO 123
 100    CALL PUSHCONTROL1B_FV(1)
        CALL PUSHINTEGER(ad_count)
        pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
        IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          CALL PUSHREALARRAY(q2(i, j, k))
          q2(i, j, k) = q4(2, i, l) + 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2&
&           , i, l))*(pr+pl) - q4(4, i, l)*r3*(pr*(pr+pl)+pl**2)
          k0 = l
          CALL PUSHCONTROL1B_FV(0)
          GOTO 120
        ELSE
! Fractional area...
          CALL PUSHREALARRAY(qsum)
          qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, l)+&
&           q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+pl*(1.+&
&           pl))))
          CALL PUSHINTEGER(m)
          ad_count0 = 1
          DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
            IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer
              qsum = qsum + dp1(i, m)*q4(1, i, m)
              CALL PUSHINTEGER(m)
              ad_count0 = ad_count0 + 1
            ELSE
              GOTO 110
            END IF
          END DO
          CALL PUSHCONTROL1B_FV(0)
          CALL PUSHINTEGER(ad_count0)
          CALL PUSHCONTROL2B_FV(1)
          GOTO 123
 110      CALL PUSHCONTROL1B_FV(1)
          CALL PUSHINTEGER(ad_count0)
          dp = pe2(i, k+1) - pe1(i, m)
          esl = dp/dp1(i, m)
          qsum = qsum + dp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(2, i, m)&
&           +q4(4, i, m)*(1.-r23*esl)))
          k0 = m
          CALL PUSHCONTROL2B_FV(0)
        END IF
 123    CALL PUSHREALARRAY(q2(i, j, k))
        q2(i, j, k) = qsum/(pe2(i, k+1)-pe2(i, k))
        CALL PUSHCONTROL1B_FV(1)
 120  CONTINUE
    END DO
    CALL PUSHINTEGER(l)
    CALL PUSHINTEGER(m)
    CALL PUSHREALARRAY(dp1, 8*(i2-i1+1)*km/8)
    CALL PUSHREALARRAY(qsum)
    CALL PUSHREALARRAY(q4, 8*4*(i2-i1+1)*km/8)
  END SUBROUTINE MAP_SCALAR_FWD
!  Differentiation of map_scalar in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_ed
!ge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod
!.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_
!core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_m
!od.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cu
!bed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total
!_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_
!mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.ste
!epz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_
!solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver n
!h_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.diver
!gence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_c
!ore_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_c
!ore_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_
!core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 q2
!   with respect to varying inputs: pe1 pe2 q2
  SUBROUTINE MAP_SCALAR_BWD(km, pe1, pe1_ad, qs, kn, pe2, pe2_ad, q2&
&   , q2_ad, i1, i2, j, ibeg, iend, jbeg, jend, iv, kord, q_min)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i1
    INTEGER, INTENT(IN) :: i2
    INTEGER, INTENT(IN) :: iv
    INTEGER, INTENT(IN) :: kord
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
    INTEGER, INTENT(IN) :: km
    INTEGER, INTENT(IN) :: kn
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_ad(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc), INTENT(IN) :: q_min
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_ad(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: q4_ad(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    REAL(fvprc) :: pl_ad, pr_ad, qsum_ad, dp_ad, esl_ad
    INTEGER :: i, k, l, m, k0
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp_ad11
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: branch
    INTEGER :: ad_count0
    INTEGER :: i3
    CALL POPREALARRAY(q4, 8*4*(i2-i1+1)*km/8)
    CALL POPREALARRAY(qsum)
    CALL POPREALARRAY(dp1, 8*(i2-i1+1)*km/8)
    CALL POPINTEGER(m)
    CALL POPINTEGER(l)
    dp1_ad = 0.0_FVPRC
    qsum_ad = 0.0_FVPRC
    q4_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL1B_FV(branch)
        IF (branch .EQ. 0) THEN
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY(q2(i, j, k))
          temp_ad0 = 0.5*(pr+pl)*q2_ad(i, j, k)
          temp_ad1 = 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2, i, l))*q2_ad(i, &
&           j, k)
          temp_ad2 = -(r3*q4(4, i, l)*q2_ad(i, j, k))
          q4_ad(2, i, l) = q4_ad(2, i, l) + q2_ad(i, j, k) - temp_ad0
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad0 - r3*(pr*(pr+pl)+pl&
&           **2)*q2_ad(i, j, k)
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad0
          pr_ad = (2*pr+pl)*temp_ad2 + temp_ad1
          pl_ad = (2*pl+pr)*temp_ad2 + temp_ad1
          q2_ad(i, j, k) = 0.0_FVPRC
          temp_ad3 = pr_ad/dp1(i, l)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad3
          pe1_ad(i, l) = pe1_ad(i, l) - temp_ad3
          dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k+1)-pe1(i, l))*temp_ad3&
&           /dp1(i, l)
        ELSE
          CALL POPREALARRAY(q2(i, j, k))
          temp1 = pe2(i, k+1) - pe2(i, k)
          temp_ad11 = -(qsum*q2_ad(i, j, k)/temp1**2)
          qsum_ad = qsum_ad + q2_ad(i, j, k)/temp1
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad11
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad11
          q2_ad(i, j, k) = 0.0_FVPRC
          CALL POPCONTROL2B_FV(branch)
          IF (branch .EQ. 0) THEN
            dp = pe2(i, k+1) - pe1(i, m)
            esl = dp/dp1(i, m)
            temp0 = q4(3, i, m) - q4(2, i, m) + q4(4, i, m)*(-(r23*esl)+&
&             1.)
            temp_ad8 = dp*qsum_ad
            temp_ad9 = 0.5*esl*temp_ad8
            q4_ad(2, i, m) = q4_ad(2, i, m) + temp_ad8 - temp_ad9
            esl_ad = 0.5*temp0*temp_ad8 - q4(4, i, m)*r23*temp_ad9
            q4_ad(3, i, m) = q4_ad(3, i, m) + temp_ad9
            q4_ad(4, i, m) = q4_ad(4, i, m) + (1.-r23*esl)*temp_ad9
            temp_ad10 = esl_ad/dp1(i, m)
            dp_ad = temp_ad10 + (q4(2, i, m)+0.5*(esl*temp0))*qsum_ad
            dp1_ad(i, m) = dp1_ad(i, m) - dp*temp_ad10/dp1(i, m)
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + dp_ad
            pe1_ad(i, m) = pe1_ad(i, m) - dp_ad
          ELSE IF (branch .NE. 1) THEN
            GOTO 100
          END IF
          CALL POPINTEGER(ad_count0)
          DO i3=1,ad_count0
            IF (i3 .EQ. 1) THEN
              CALL POPCONTROL1B_FV(branch)
            ELSE
              dp1_ad(i, m) = dp1_ad(i, m) + q4(1, i, m)*qsum_ad
              q4_ad(1, i, m) = q4_ad(1, i, m) + dp1(i, m)*qsum_ad
            END IF
            CALL POPINTEGER(m)
          END DO
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY(qsum)
          temp = q4(4, i, l) + q4(3, i, l) - q4(2, i, l)
          temp_ad4 = (q4(2, i, l)+0.5*(temp*(pl+1.))-r3*(q4(4, i, l)*(pl&
&           *(pl+1.)+1.)))*qsum_ad
          temp_ad5 = (pe1(i, l+1)-pe2(i, k))*qsum_ad
          temp_ad6 = 0.5*(pl+1.)*temp_ad5
          temp_ad7 = -(r3*q4(4, i, l)*temp_ad5)
          pe1_ad(i, l+1) = pe1_ad(i, l+1) + temp_ad4
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad4
          q4_ad(2, i, l) = q4_ad(2, i, l) + temp_ad5 - temp_ad6
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad6 - r3*(pl*(pl+1.)+1.&
&           )*temp_ad5
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad6
          pl_ad = (2*pl+1.)*temp_ad7 + 0.5*temp*temp_ad5
          qsum_ad = 0.0_FVPRC
        END IF
        temp_ad = pl_ad/dp1(i, l)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad
        pe1_ad(i, l) = pe1_ad(i, l) - temp_ad
        dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k)-pe1(i, l))*temp_ad/dp1(&
&         i, l)
 100    CALL POPINTEGER(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B_FV(branch)
          CALL POPINTEGER(l)
        END DO
      END DO
    END DO
    CALL POPCONTROL2B_FV(branch)
    IF (branch .NE. 0) THEN
      IF (branch .NE. 1) CALL SCALAR_PROFILE_LINEAR_BWD(qs, q4, q4_ad&
&                                                    , dp1, dp1_ad, km, &
&                                                    i1, i2, iv, q_min)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
        q2_ad(i, j, k) = q2_ad(i, j, k) + q4_ad(1, i, k)
        q4_ad(1, i, k) = 0.0_FVPRC
        pe1_ad(i, k+1) = pe1_ad(i, k+1) + dp1_ad(i, k)
        pe1_ad(i, k) = pe1_ad(i, k) - dp1_ad(i, k)
        dp1_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE MAP_SCALAR_BWD
!  Differentiation of map1_ppm in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_
!mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe
!_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_cor
!e_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.
!rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed
!_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_en
!ergy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_map
!z_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz
! fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_sol
!ver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_c
!ore_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergen
!ce_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core
!_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core
!_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_cor
!e_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 qs q2
!   with respect to varying inputs: pe1 pe2 qs q2
  SUBROUTINE MAP1_PPM_FWD(km, pe1, qs, kn, pe2, q2, i1, i2, j, ibeg, &
&   iend, jbeg, jend, iv, kord)
    IMPLICIT NONE
! Starting longitude
    INTEGER, INTENT(IN) :: i1
! Finishing longitude
    INTEGER, INTENT(IN) :: i2
! Mode: 0 == constituents  1 == ???
    INTEGER, INTENT(IN) :: iv
!       2 == remap temp with cs scheme
! Method order
    INTEGER, INTENT(IN) :: kord
! Current latitude
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! bottom BC
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
! real(FVPRC), intent(in) ::    q1(ibeg:iend,jbeg:jend,km) ! Field input
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
! !DESCRIPTION:
! IV = 0: constituents
! pe1: pressure at layer edges (from model top to bottom surface)
!      in the original vertical coordinate
! pe2: pressure at layer edges (from model top to bottom surface)
!      in the new vertical coordinate
! !LOCAL VARIABLES:
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    INTEGER :: i, k, l, m, k0
    INTEGER :: ad_count
    INTEGER :: ad_count0
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q2(i, j, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL CS_PROFILE_LINEAR_FWD(qs, q4, dp1, km, i1, i2, iv)
        CALL PUSHCONTROL2B_FV(2)
      ELSE
        CALL PUSHCONTROL2B_FV(1)
      END IF
    ELSE
      CALL PUSHCONTROL2B_FV(0)
    END IF
!call  cs_profile( qs, q4, dp1, km, i1, i2, iv, kord )
!call ppm_profile( q4, dp1, km, i1, i2, iv, kord )
    DO i=i1,i2
      k0 = 1
      DO 120 k=1,kn
        CALL PUSHINTEGER(l)
        ad_count = 1
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            GOTO 100
          ELSE
            CALL PUSHINTEGER(l)
            ad_count = ad_count + 1
          END IF
        END DO
        CALL PUSHCONTROL1B_FV(0)
        CALL PUSHINTEGER(ad_count)
        CALL PUSHCONTROL2B_FV(2)
        GOTO 123
 100    CALL PUSHCONTROL1B_FV(1)
        CALL PUSHINTEGER(ad_count)
        pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
        IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          CALL PUSHREALARRAY(q2(i, j, k))
          q2(i, j, k) = q4(2, i, l) + 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2&
&           , i, l))*(pr+pl) - q4(4, i, l)*r3*(pr*(pr+pl)+pl**2)
          k0 = l
          CALL PUSHCONTROL1B_FV(0)
          GOTO 120
        ELSE
! Fractional area...
          CALL PUSHREALARRAY(qsum)
          qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, l)+&
&           q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+pl*(1.+&
&           pl))))
          CALL PUSHINTEGER(m)
          ad_count0 = 1
          DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
            IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer
              qsum = qsum + dp1(i, m)*q4(1, i, m)
              CALL PUSHINTEGER(m)
              ad_count0 = ad_count0 + 1
            ELSE
              GOTO 110
            END IF
          END DO
          CALL PUSHCONTROL1B_FV(0)
          CALL PUSHINTEGER(ad_count0)
          CALL PUSHCONTROL2B_FV(1)
          GOTO 123
 110      CALL PUSHCONTROL1B_FV(1)
          CALL PUSHINTEGER(ad_count0)
          dp = pe2(i, k+1) - pe1(i, m)
          esl = dp/dp1(i, m)
          qsum = qsum + dp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(2, i, m)&
&           +q4(4, i, m)*(1.-r23*esl)))
          k0 = m
          CALL PUSHCONTROL2B_FV(0)
        END IF
 123    CALL PUSHREALARRAY(q2(i, j, k))
        q2(i, j, k) = qsum/(pe2(i, k+1)-pe2(i, k))
        CALL PUSHCONTROL1B_FV(1)
 120  CONTINUE
    END DO
    CALL PUSHINTEGER(l)
    CALL PUSHINTEGER(m)
    CALL PUSHREALARRAY(dp1, 8*(i2-i1+1)*km/8)
    CALL PUSHREALARRAY(qsum)
    CALL PUSHREALARRAY(q4, 8*4*(i2-i1+1)*km/8)
  END SUBROUTINE MAP1_PPM_FWD
!  Differentiation of map1_ppm in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge
!_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.p
!e_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_co
!re_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod
!.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cube
!d_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_e
!nergy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_ma
!pz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steep
!z fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_so
!lver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_
!core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.diverge
!nce_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_cor
!e_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_cor
!e_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_co
!re_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 qs q2
!   with respect to varying inputs: pe1 pe2 qs q2
  SUBROUTINE MAP1_PPM_BWD(km, pe1, pe1_ad, qs, qs_ad, kn, pe2, pe2_ad&
&   , q2, q2_ad, i1, i2, j, ibeg, iend, jbeg, jend, iv, kord)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i1
    INTEGER, INTENT(IN) :: i2
    INTEGER, INTENT(IN) :: iv
    INTEGER, INTENT(IN) :: kord
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
    INTEGER, INTENT(IN) :: km
    INTEGER, INTENT(IN) :: kn
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
    REAL(fvprc) :: qs_ad(i1:i2)
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
    REAL(fvprc), INTENT(INOUT) :: q2(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_ad(ibeg:iend, jbeg:jend, kn)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_ad(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: q4_ad(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    REAL(fvprc) :: pl_ad, pr_ad, qsum_ad, dp_ad, esl_ad
    INTEGER :: i, k, l, m, k0
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp_ad11
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: branch
    INTEGER :: ad_count0
    INTEGER :: i3
    CALL POPREALARRAY(q4, 8*4*(i2-i1+1)*km/8)
    CALL POPREALARRAY(qsum)
    CALL POPREALARRAY(dp1, 8*(i2-i1+1)*km/8)
    CALL POPINTEGER(m)
    CALL POPINTEGER(l)
    dp1_ad = 0.0_FVPRC
    qsum_ad = 0.0_FVPRC
    q4_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL1B_FV(branch)
        IF (branch .EQ. 0) THEN
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY(q2(i, j, k))
          temp_ad0 = 0.5*(pr+pl)*q2_ad(i, j, k)
          temp_ad1 = 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2, i, l))*q2_ad(i, &
&           j, k)
          temp_ad2 = -(r3*q4(4, i, l)*q2_ad(i, j, k))
          q4_ad(2, i, l) = q4_ad(2, i, l) + q2_ad(i, j, k) - temp_ad0
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad0 - r3*(pr*(pr+pl)+pl&
&           **2)*q2_ad(i, j, k)
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad0
          pr_ad = (2*pr+pl)*temp_ad2 + temp_ad1
          pl_ad = (2*pl+pr)*temp_ad2 + temp_ad1
          q2_ad(i, j, k) = 0.0_FVPRC
          temp_ad3 = pr_ad/dp1(i, l)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad3
          pe1_ad(i, l) = pe1_ad(i, l) - temp_ad3
          dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k+1)-pe1(i, l))*temp_ad3&
&           /dp1(i, l)
        ELSE
          CALL POPREALARRAY(q2(i, j, k))
          temp1 = pe2(i, k+1) - pe2(i, k)
          temp_ad11 = -(qsum*q2_ad(i, j, k)/temp1**2)
          qsum_ad = qsum_ad + q2_ad(i, j, k)/temp1
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad11
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad11
          q2_ad(i, j, k) = 0.0_FVPRC
          CALL POPCONTROL2B_FV(branch)
          IF (branch .EQ. 0) THEN
            dp = pe2(i, k+1) - pe1(i, m)
            esl = dp/dp1(i, m)
            temp0 = q4(3, i, m) - q4(2, i, m) + q4(4, i, m)*(-(r23*esl)+&
&             1.)
            temp_ad8 = dp*qsum_ad
            temp_ad9 = 0.5*esl*temp_ad8
            q4_ad(2, i, m) = q4_ad(2, i, m) + temp_ad8 - temp_ad9
            esl_ad = 0.5*temp0*temp_ad8 - q4(4, i, m)*r23*temp_ad9
            q4_ad(3, i, m) = q4_ad(3, i, m) + temp_ad9
            q4_ad(4, i, m) = q4_ad(4, i, m) + (1.-r23*esl)*temp_ad9
            temp_ad10 = esl_ad/dp1(i, m)
            dp_ad = temp_ad10 + (q4(2, i, m)+0.5*(esl*temp0))*qsum_ad
            dp1_ad(i, m) = dp1_ad(i, m) - dp*temp_ad10/dp1(i, m)
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + dp_ad
            pe1_ad(i, m) = pe1_ad(i, m) - dp_ad
          ELSE IF (branch .NE. 1) THEN
            GOTO 100
          END IF
          CALL POPINTEGER(ad_count0)
          DO i3=1,ad_count0
            IF (i3 .EQ. 1) THEN
              CALL POPCONTROL1B_FV(branch)
            ELSE
              dp1_ad(i, m) = dp1_ad(i, m) + q4(1, i, m)*qsum_ad
              q4_ad(1, i, m) = q4_ad(1, i, m) + dp1(i, m)*qsum_ad
            END IF
            CALL POPINTEGER(m)
          END DO
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY(qsum)
          temp = q4(4, i, l) + q4(3, i, l) - q4(2, i, l)
          temp_ad4 = (q4(2, i, l)+0.5*(temp*(pl+1.))-r3*(q4(4, i, l)*(pl&
&           *(pl+1.)+1.)))*qsum_ad
          temp_ad5 = (pe1(i, l+1)-pe2(i, k))*qsum_ad
          temp_ad6 = 0.5*(pl+1.)*temp_ad5
          temp_ad7 = -(r3*q4(4, i, l)*temp_ad5)
          pe1_ad(i, l+1) = pe1_ad(i, l+1) + temp_ad4
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad4
          q4_ad(2, i, l) = q4_ad(2, i, l) + temp_ad5 - temp_ad6
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad6 - r3*(pl*(pl+1.)+1.&
&           )*temp_ad5
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad6
          pl_ad = (2*pl+1.)*temp_ad7 + 0.5*temp*temp_ad5
          qsum_ad = 0.0_FVPRC
        END IF
        temp_ad = pl_ad/dp1(i, l)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad
        pe1_ad(i, l) = pe1_ad(i, l) - temp_ad
        dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k)-pe1(i, l))*temp_ad/dp1(&
&         i, l)
 100    CALL POPINTEGER(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B_FV(branch)
          CALL POPINTEGER(l)
        END DO
      END DO
    END DO
    CALL POPCONTROL2B_FV(branch)
    IF (branch .NE. 0) THEN
      IF (branch .NE. 1) CALL CS_PROFILE_LINEAR_BWD(qs, qs_ad, q4, &
&                                                q4_ad, dp1, dp1_ad, km&
&                                                , i1, i2, iv)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
        q2_ad(i, j, k) = q2_ad(i, j, k) + q4_ad(1, i, k)
        q4_ad(1, i, k) = 0.0_FVPRC
        pe1_ad(i, k+1) = pe1_ad(i, k+1) + dp1_ad(i, k)
        pe1_ad(i, k) = pe1_ad(i, k) - dp1_ad(i, k)
        dp1_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE MAP1_PPM_BWD
!  Differentiation of mapn_tracer in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_ed
!ge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod
!.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_
!core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_m
!od.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cu
!bed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total
!_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_
!mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.ste
!epz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_
!solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver n
!h_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.diver
!gence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_c
!ore_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_c
!ore_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_
!core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 dp2 q1
!   with respect to varying inputs: pe1 pe2 dp2 q1
  SUBROUTINE MAPN_TRACER_FWD(nq, km, kn, pe1, pe2, q1, dp2, kord, j, &
&   i1, i2, isd, ied, jsd, jed, q_min, fill)
    IMPLICIT NONE
! !INPUT PARAMETERS:
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
    INTEGER, INTENT(IN) :: j, nq, i1, i2
    INTEGER, INTENT(IN) :: isd, ied, jsd, jed
    INTEGER, INTENT(IN) :: kord(nq)
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
    REAL(fvprc), INTENT(IN) :: dp2(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: q_min
    LOGICAL, INTENT(IN) :: fill
! Field input
    REAL(fvprc), INTENT(INOUT) :: q1(isd:ied, jsd:jed, km, nq)
! !LOCAL VARIABLES:
    REAL(fvprc) :: q4(4, i1:i2, km, nq)
! Field output
    REAL(fvprc) :: q2(i1:i2, kn, nq)
    REAL(fvprc) :: qsum(nq)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: pl, pr, dp, esl, fac1, fac2
    INTEGER :: i, k, l, m, k0, iq
    INTEGER :: ad_count
    INTEGER :: ad_count0
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
      END DO
    END DO
    DO iq=1,nq
      DO k=1,km
        DO i=i1,i2
          q4(1, i, k, iq) = q1(i, j, k, iq)
        END DO
      END DO
      IF (kord(iq) .EQ. 111) THEN
        CALL SCALAR_PROFILE_LINEAR_FWD(qs, q4(1, i1:i2, 1, iq), dp1, &
&                                   km, i1, i2, 0, q_min)
        CALL PUSHCONTROL1B_FV(1)
      ELSE
        CALL PUSHCONTROL1B_FV(0)
      END IF
    END DO
!call scalar_profile( qs, q4(1,i1:i2,1,iq), dp1, km, i1, i2, 0, kord(iq), q_min )
! Mapping
    DO i=i1,i2
      k0 = 1
      DO 130 k=1,kn
        CALL PUSHINTEGER(l)
        ad_count = 1
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            GOTO 100
          ELSE
            CALL PUSHINTEGER(l)
            ad_count = ad_count + 1
          END IF
        END DO
        CALL PUSHCONTROL1B_FV(0)
        CALL PUSHINTEGER(ad_count)
        CALL PUSHCONTROL2B_FV(2)
        GOTO 120
 100    CALL PUSHCONTROL1B_FV(1)
        CALL PUSHINTEGER(ad_count)
        pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
        IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          CALL PUSHREALARRAY(fac1)
          fac1 = pr + pl
          CALL PUSHREALARRAY(fac2)
          fac2 = r3*(pr*fac1+pl*pl)
          CALL PUSHREALARRAY(fac1)
          fac1 = 0.5*fac1
          DO iq=1,nq
            q2(i, k, iq) = q4(2, i, l, iq) + (q4(4, i, l, iq)+q4(3, i, l&
&             , iq)-q4(2, i, l, iq))*fac1 - q4(4, i, l, iq)*fac2
          END DO
          k0 = l
          CALL PUSHCONTROL1B_FV(0)
          GOTO 130
        ELSE
! Fractional area...
          CALL PUSHREALARRAY(dp)
          dp = pe1(i, l+1) - pe2(i, k)
          CALL PUSHREALARRAY(fac1)
          fac1 = 1. + pl
          CALL PUSHREALARRAY(fac2)
          fac2 = r3*(1.+pl*fac1)
          CALL PUSHREALARRAY(fac1)
          fac1 = 0.5*fac1
          DO iq=1,nq
            CALL PUSHREALARRAY(qsum(iq))
            qsum(iq) = dp*(q4(2, i, l, iq)+(q4(4, i, l, iq)+q4(3, i, l, &
&             iq)-q4(2, i, l, iq))*fac1-q4(4, i, l, iq)*fac2)
          END DO
          CALL PUSHINTEGER(m)
          ad_count0 = 1
          DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
            IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer..
              DO iq=1,nq
                CALL PUSHREALARRAY(qsum(iq))
                qsum(iq) = qsum(iq) + dp1(i, m)*q4(1, i, m, iq)
              END DO
              CALL PUSHINTEGER(m)
              ad_count0 = ad_count0 + 1
            ELSE
              GOTO 110
            END IF
          END DO
          CALL PUSHCONTROL1B_FV(0)
          CALL PUSHINTEGER(ad_count0)
          CALL PUSHCONTROL2B_FV(1)
          GOTO 120
 110      CALL PUSHCONTROL1B_FV(1)
          CALL PUSHINTEGER(ad_count0)
          CALL PUSHREALARRAY(dp)
          dp = pe2(i, k+1) - pe1(i, m)
          esl = dp/dp1(i, m)
          CALL PUSHREALARRAY(fac1)
          fac1 = 0.5*esl
          CALL PUSHREALARRAY(fac2)
          fac2 = 1. - r23*esl
          DO iq=1,nq
            CALL PUSHREALARRAY(qsum(iq))
            qsum(iq) = qsum(iq) + dp*(q4(2, i, m, iq)+fac1*(q4(3, i, m, &
&             iq)-q4(2, i, m, iq)+q4(4, i, m, iq)*fac2))
          END DO
          k0 = m
          CALL PUSHCONTROL2B_FV(0)
        END IF
 120    DO iq=1,nq
          q2(i, k, iq) = qsum(iq)/dp2(i, k)
        END DO
        CALL PUSHCONTROL1B_FV(1)
 130  CONTINUE
    END DO
!if (fill) call fillz(i2-i1+1, kn, nq, q2, dp2)
    DO iq=1,nq
!    if (fill) call fillz(i2-i1+1, kn, 1, q2(i1,1,iq), dp2)
      DO k=1,kn
        DO i=i1,i2
          CALL PUSHREALARRAY(q1(i, j, k, iq))
          q1(i, j, k, iq) = q2(i, k, iq)
        END DO
      END DO
    END DO
    CALL PUSHINTEGER(l)
    CALL PUSHINTEGER(m)
    CALL PUSHREALARRAY(dp)
    CALL PUSHREALARRAY(fac1)
    CALL PUSHREALARRAY(fac2)
    CALL PUSHREALARRAY(qs, 8*(i2-i1+1)/8)
    CALL PUSHREALARRAY(dp1, 8*(i2-i1+1)*km/8)
    CALL PUSHREALARRAY(qsum, 8*nq/8)
    CALL PUSHREALARRAY(q4, 8*4*(i2-i1+1)*km*nq/8)
  END SUBROUTINE MAPN_TRACER_FWD
!  Differentiation of mapn_tracer in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_e
!dge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mo
!d.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn
!_core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_
!mod.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.c
!ubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_tota
!l_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv
!_mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.st
!eepz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem
!_solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver 
!nh_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.dive
!rgence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_
!core_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_
!core_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp
!_core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 dp2 q1
!   with respect to varying inputs: pe1 pe2 dp2 q1
  SUBROUTINE MAPN_TRACER_BWD(nq, km, kn, pe1, pe1_ad, pe2, pe2_ad, q1&
&   , q1_ad, dp2, dp2_ad, kord, j, i1, i2, isd, ied, jsd, jed, q_min, &
&   fill)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: km
    INTEGER, INTENT(IN) :: kn
    INTEGER, INTENT(IN) :: j, nq, i1, i2
    INTEGER, INTENT(IN) :: isd, ied, jsd, jed
    INTEGER, INTENT(IN) :: kord(nq)
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
    REAL(fvprc), INTENT(IN) :: dp2(i1:i2, kn)
    REAL(fvprc) :: dp2_ad(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: q_min
    LOGICAL, INTENT(IN) :: fill
    REAL(fvprc), INTENT(INOUT) :: q1(isd:ied, jsd:jed, km, nq)
    REAL(fvprc), INTENT(INOUT) :: q1_ad(isd:ied, jsd:jed, km, nq)
    REAL(fvprc) :: q4(4, i1:i2, km, nq)
    REAL(fvprc) :: q4_ad(4, i1:i2, km, nq)
    REAL(fvprc) :: q2(i1:i2, kn, nq)
    REAL(fvprc) :: q2_ad(i1:i2, kn, nq)
    REAL(fvprc) :: qsum(nq)
    REAL(fvprc) :: qsum_ad(nq)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_ad(i1:i2, km)
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: pl, pr, dp, esl, fac1, fac2
    REAL(fvprc) :: pl_ad, pr_ad, dp_ad, esl_ad, fac1_ad, fac2_ad
    INTEGER :: i, k, l, m, k0, iq
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    INTEGER :: branch
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: ad_count0
    INTEGER :: i3
    CALL POPREALARRAY(q4, 8*4*(i2-i1+1)*km*nq/8)
    CALL POPREALARRAY(qsum, 8*nq/8)
    CALL POPREALARRAY(dp1, 8*(i2-i1+1)*km/8)
    CALL POPREALARRAY(qs, 8*(i2-i1+1)/8)
    CALL POPREALARRAY(fac2)
    CALL POPREALARRAY(fac1)
    CALL POPREALARRAY(dp)
    CALL POPINTEGER(m)
    CALL POPINTEGER(l)
    q2_ad = 0.0_FVPRC
    DO iq=nq,1,-1
      DO k=kn,1,-1
        DO i=i2,i1,-1
          CALL POPREALARRAY(q1(i, j, k, iq))
          q2_ad(i, k, iq) = q2_ad(i, k, iq) + q1_ad(i, j, k, iq)
          q1_ad(i, j, k, iq) = 0.0_FVPRC
        END DO
      END DO
    END DO
    dp1_ad = 0.0_FVPRC
    qsum_ad = 0.0_FVPRC
    q4_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL1B_FV(branch)
        IF (branch .EQ. 0) THEN
          fac1_ad = 0.0_FVPRC
          fac2_ad = 0.0_FVPRC
          DO iq=nq,1,-1
            temp_ad2 = fac1*q2_ad(i, k, iq)
            q4_ad(2, i, l, iq) = q4_ad(2, i, l, iq) + q2_ad(i, k, iq) - &
&             temp_ad2
            q4_ad(4, i, l, iq) = q4_ad(4, i, l, iq) + temp_ad2 - fac2*&
&             q2_ad(i, k, iq)
            q4_ad(3, i, l, iq) = q4_ad(3, i, l, iq) + temp_ad2
            fac1_ad = fac1_ad + (q4(4, i, l, iq)+q4(3, i, l, iq)-q4(2, i&
&             , l, iq))*q2_ad(i, k, iq)
            fac2_ad = fac2_ad - q4(4, i, l, iq)*q2_ad(i, k, iq)
            q2_ad(i, k, iq) = 0.0_FVPRC
          END DO
          temp_ad0 = r3*fac2_ad
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY(fac1)
          fac1_ad = pr*temp_ad0 + 0.5*fac1_ad
          CALL POPREALARRAY(fac2)
          pr_ad = fac1_ad + fac1*temp_ad0
          pl_ad = fac1_ad + 2*pl*temp_ad0
          CALL POPREALARRAY(fac1)
          temp_ad1 = pr_ad/dp1(i, l)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad1
          pe1_ad(i, l) = pe1_ad(i, l) - temp_ad1
          dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k+1)-pe1(i, l))*temp_ad1&
&           /dp1(i, l)
        ELSE
          DO iq=nq,1,-1
            temp_ad8 = q2_ad(i, k, iq)/dp2(i, k)
            qsum_ad(iq) = qsum_ad(iq) + temp_ad8
            dp2_ad(i, k) = dp2_ad(i, k) - qsum(iq)*temp_ad8/dp2(i, k)
            q2_ad(i, k, iq) = 0.0_FVPRC
          END DO
          CALL POPCONTROL2B_FV(branch)
          IF (branch .EQ. 0) THEN
            dp_ad = 0.0_FVPRC
            fac1_ad = 0.0_FVPRC
            fac2_ad = 0.0_FVPRC
            DO iq=nq,1,-1
              CALL POPREALARRAY(qsum(iq))
              temp0 = q4(3, i, m, iq) - q4(2, i, m, iq) + q4(4, i, m, iq&
&               )*fac2
              temp_ad6 = dp*qsum_ad(iq)
              temp_ad7 = fac1*temp_ad6
              dp_ad = dp_ad + (q4(2, i, m, iq)+fac1*temp0)*qsum_ad(iq)
              q4_ad(2, i, m, iq) = q4_ad(2, i, m, iq) + temp_ad6 - &
&               temp_ad7
              fac1_ad = fac1_ad + temp0*temp_ad6
              q4_ad(3, i, m, iq) = q4_ad(3, i, m, iq) + temp_ad7
              q4_ad(4, i, m, iq) = q4_ad(4, i, m, iq) + fac2*temp_ad7
              fac2_ad = fac2_ad + q4(4, i, m, iq)*temp_ad7
            END DO
            CALL POPREALARRAY(fac2)
            esl_ad = 0.5*fac1_ad - r23*fac2_ad
            CALL POPREALARRAY(fac1)
            temp_ad5 = esl_ad/dp1(i, m)
            dp_ad = dp_ad + temp_ad5
            dp1_ad(i, m) = dp1_ad(i, m) - dp*temp_ad5/dp1(i, m)
            CALL POPREALARRAY(dp)
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + dp_ad
            pe1_ad(i, m) = pe1_ad(i, m) - dp_ad
          ELSE IF (branch .NE. 1) THEN
            GOTO 100
          END IF
          CALL POPINTEGER(ad_count0)
          DO i3=1,ad_count0
            IF (i3 .EQ. 1) THEN
              CALL POPCONTROL1B_FV(branch)
            ELSE
              DO iq=nq,1,-1
                CALL POPREALARRAY(qsum(iq))
                dp1_ad(i, m) = dp1_ad(i, m) + q4(1, i, m, iq)*qsum_ad(iq&
&                 )
                q4_ad(1, i, m, iq) = q4_ad(1, i, m, iq) + dp1(i, m)*&
&                 qsum_ad(iq)
              END DO
            END IF
            CALL POPINTEGER(m)
          END DO
          dp_ad = 0.0_FVPRC
          fac1_ad = 0.0_FVPRC
          fac2_ad = 0.0_FVPRC
          DO iq=nq,1,-1
            CALL POPREALARRAY(qsum(iq))
            temp = q4(4, i, l, iq) + q4(3, i, l, iq) - q4(2, i, l, iq)
            temp_ad3 = dp*qsum_ad(iq)
            temp_ad4 = fac1*temp_ad3
            dp_ad = dp_ad + (q4(2, i, l, iq)+temp*fac1-q4(4, i, l, iq)*&
&             fac2)*qsum_ad(iq)
            q4_ad(2, i, l, iq) = q4_ad(2, i, l, iq) + temp_ad3 - &
&             temp_ad4
            q4_ad(4, i, l, iq) = q4_ad(4, i, l, iq) + temp_ad4 - fac2*&
&             temp_ad3
            q4_ad(3, i, l, iq) = q4_ad(3, i, l, iq) + temp_ad4
            fac1_ad = fac1_ad + temp*temp_ad3
            fac2_ad = fac2_ad - q4(4, i, l, iq)*temp_ad3
            qsum_ad(iq) = 0.0_FVPRC
          END DO
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY(fac1)
          fac1_ad = r3*pl*fac2_ad + 0.5*fac1_ad
          CALL POPREALARRAY(fac2)
          pl_ad = fac1_ad + r3*fac1*fac2_ad
          CALL POPREALARRAY(fac1)
          CALL POPREALARRAY(dp)
          pe1_ad(i, l+1) = pe1_ad(i, l+1) + dp_ad
          pe2_ad(i, k) = pe2_ad(i, k) - dp_ad
        END IF
        temp_ad = pl_ad/dp1(i, l)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad
        pe1_ad(i, l) = pe1_ad(i, l) - temp_ad
        dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k)-pe1(i, l))*temp_ad/dp1(&
&         i, l)
 100    CALL POPINTEGER(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B_FV(branch)
          CALL POPINTEGER(l)
        END DO
      END DO
    END DO
    DO iq=nq,1,-1
      CALL POPCONTROL1B_FV(branch)
      IF (branch .NE. 0) CALL SCALAR_PROFILE_LINEAR_BWD(qs, q4(1, i1:&
&                                                    i2, 1, iq), q4_ad(1&
&                                                    , i1:i2, 1, iq), &
&                                                    dp1, dp1_ad, km, i1&
&                                                    , i2, 0, q_min)
      DO k=km,1,-1
        DO i=i2,i1,-1
          q1_ad(i, j, k, iq) = q1_ad(i, j, k, iq) + q4_ad(1, i, k, iq)
          q4_ad(1, i, k, iq) = 0.0_FVPRC
        END DO
      END DO
    END DO
    DO k=km,1,-1
      DO i=i2,i1,-1
        pe1_ad(i, k+1) = pe1_ad(i, k+1) + dp1_ad(i, k)
        pe1_ad(i, k) = pe1_ad(i, k) - dp1_ad(i, k)
        dp1_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE MAPN_TRACER_BWD
!  Differentiation of map1_q2 in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_m
!od.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_
!halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core
!_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.r
!ayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed_
!to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_ene
!rgy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz
!_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz 
!fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_solv
!er_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_co
!re_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergenc
!e_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_
!mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core_
!mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_core
!_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 dp2 q1 q2
!   with respect to varying inputs: pe1 pe2 dp2 q1 q2
  SUBROUTINE MAP1_Q2_FWD(km, pe1, q1, kn, pe2, q2, dp2, i1, i2, iv, &
&   kord, j, ibeg, iend, jbeg, jend, q_min)
    IMPLICIT NONE
! !INPUT PARAMETERS:
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: i1, i2
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
! Mode: 0 ==  constituents 1 == ???
    INTEGER, INTENT(IN) :: iv
    INTEGER, INTENT(IN) :: kord
! Original vertical dimension
    INTEGER, INTENT(IN) :: km
! Target vertical dimension
    INTEGER, INTENT(IN) :: kn
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
! (from model top to bottom surface)
! in the original vertical coordinate
! pressure at layer edges 
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
! (from model top to bottom surface)
! in the new vertical coordinate
! Field input
    REAL(fvprc), INTENT(IN) :: q1(ibeg:iend, jbeg:jend, km)
    REAL(fvprc), INTENT(IN) :: dp2(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: q_min
! !INPUT/OUTPUT PARAMETERS:
! Field output
    REAL(fvprc), INTENT(INOUT) :: q2(i1:i2, kn)
! !LOCAL VARIABLES:
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    INTEGER :: i, k, l, m, k0
    INTEGER :: ad_count
    INTEGER :: ad_count0
    DO k=1,km
      DO i=i1,i2
        dp1(i, k) = pe1(i, k+1) - pe1(i, k)
        q4(1, i, k) = q1(i, j, k)
      END DO
    END DO
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      IF (kord .EQ. 111) THEN
        CALL SCALAR_PROFILE_LINEAR_FWD(qs, q4, dp1, km, i1, i2, iv, &
&                                   q_min)
        CALL PUSHCONTROL2B_FV(2)
      ELSE
        CALL PUSHCONTROL2B_FV(1)
      END IF
    ELSE
      CALL PUSHCONTROL2B_FV(0)
    END IF
!call  scalar_profile( qs, q4, dp1, km, i1, i2, iv, kord, q_min )
!call ppm_profile( q4, dp1, km, i1, i2, iv, kord )
! Mapping
    DO i=i1,i2
      k0 = 1
      DO 120 k=1,kn
        CALL PUSHINTEGER(l)
        ad_count = 1
        DO l=k0,km
! locate the top edge: pe2(i,k)
          IF (pe2(i, k) .GE. pe1(i, l) .AND. pe2(i, k) .LE. pe1(i, l+1)&
&         ) THEN
            GOTO 100
          ELSE
            CALL PUSHINTEGER(l)
            ad_count = ad_count + 1
          END IF
        END DO
        CALL PUSHCONTROL1B_FV(0)
        CALL PUSHINTEGER(ad_count)
        CALL PUSHCONTROL2B_FV(2)
        GOTO 123
 100    CALL PUSHCONTROL1B_FV(1)
        CALL PUSHINTEGER(ad_count)
        pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
        IF (pe2(i, k+1) .LE. pe1(i, l+1)) THEN
! entire new grid is within the original grid
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          q2(i, k) = q4(2, i, l) + 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2, i&
&           , l))*(pr+pl) - q4(4, i, l)*r3*(pr*(pr+pl)+pl**2)
          k0 = l
          CALL PUSHCONTROL1B_FV(0)
          GOTO 120
        ELSE
! Fractional area...
          CALL PUSHREALARRAY(qsum)
          qsum = (pe1(i, l+1)-pe2(i, k))*(q4(2, i, l)+0.5*(q4(4, i, l)+&
&           q4(3, i, l)-q4(2, i, l))*(1.+pl)-q4(4, i, l)*(r3*(1.+pl*(1.+&
&           pl))))
          CALL PUSHINTEGER(m)
          ad_count0 = 1
          DO m=l+1,km
! locate the bottom edge: pe2(i,k+1)
            IF (pe2(i, k+1) .GT. pe1(i, m+1)) THEN
! Whole layer..
              qsum = qsum + dp1(i, m)*q4(1, i, m)
              CALL PUSHINTEGER(m)
              ad_count0 = ad_count0 + 1
            ELSE
              GOTO 110
            END IF
          END DO
          CALL PUSHCONTROL1B_FV(0)
          CALL PUSHINTEGER(ad_count0)
          CALL PUSHCONTROL2B_FV(1)
          GOTO 123
 110      CALL PUSHCONTROL1B_FV(1)
          CALL PUSHINTEGER(ad_count0)
          dp = pe2(i, k+1) - pe1(i, m)
          esl = dp/dp1(i, m)
          qsum = qsum + dp*(q4(2, i, m)+0.5*esl*(q4(3, i, m)-q4(2, i, m)&
&           +q4(4, i, m)*(1.-r23*esl)))
          k0 = m
          CALL PUSHCONTROL2B_FV(0)
        END IF
 123    q2(i, k) = qsum/dp2(i, k)
        CALL PUSHCONTROL1B_FV(1)
 120  CONTINUE
    END DO
    CALL PUSHINTEGER(l)
    CALL PUSHINTEGER(m)
    CALL PUSHREALARRAY(qs, 8*(i2-i1+1)/8)
    CALL PUSHREALARRAY(dp1, 8*(i2-i1+1)*km/8)
    CALL PUSHREALARRAY(qsum)
    CALL PUSHREALARRAY(q4, 8*4*(i2-i1+1)*km/8)
  END SUBROUTINE MAP1_Q2_FWD
!  Differentiation of map1_q2 in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 a2b_edge_
!mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe
!_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_cor
!e_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dynamics_mod.
!rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_mod.cubed
!_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute_total_en
!ergy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_map
!z_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_mod.steepz
! fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod.riem_sol
!ver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_solver nh_c
!ore_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod.divergen
!ce_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core
!_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d tp_core
!_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm tp_cor
!e_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: pe1 pe2 dp2 q1 q2
!   with respect to varying inputs: pe1 pe2 dp2 q1 q2
  SUBROUTINE MAP1_Q2_BWD(km, pe1, pe1_ad, q1, q1_ad, kn, pe2, pe2_ad&
&   , q2, q2_ad, dp2, dp2_ad, i1, i2, iv, kord, j, ibeg, iend, jbeg, &
&   jend, q_min)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: j
    INTEGER, INTENT(IN) :: i1, i2
    INTEGER, INTENT(IN) :: ibeg, iend, jbeg, jend
    INTEGER, INTENT(IN) :: iv
    INTEGER, INTENT(IN) :: kord
    INTEGER, INTENT(IN) :: km
    INTEGER, INTENT(IN) :: kn
    REAL(fvprc), INTENT(IN) :: pe1(i1:i2, km+1)
    REAL(fvprc) :: pe1_ad(i1:i2, km+1)
    REAL(fvprc), INTENT(IN) :: pe2(i1:i2, kn+1)
    REAL(fvprc) :: pe2_ad(i1:i2, kn+1)
    REAL(fvprc), INTENT(IN) :: q1(ibeg:iend, jbeg:jend, km)
    REAL(fvprc) :: q1_ad(ibeg:iend, jbeg:jend, km)
    REAL(fvprc), INTENT(IN) :: dp2(i1:i2, kn)
    REAL(fvprc) :: dp2_ad(i1:i2, kn)
    REAL(fvprc), INTENT(IN) :: q_min
    REAL(fvprc), INTENT(INOUT) :: q2(i1:i2, kn)
    REAL(fvprc), INTENT(INOUT) :: q2_ad(i1:i2, kn)
    REAL(fvprc) :: qs(i1:i2)
    REAL(fvprc) :: dp1(i1:i2, km)
    REAL(fvprc) :: dp1_ad(i1:i2, km)
    REAL(fvprc) :: q4(4, i1:i2, km)
    REAL(fvprc) :: q4_ad(4, i1:i2, km)
    REAL(fvprc) :: pl, pr, qsum, dp, esl
    REAL(fvprc) :: pl_ad, pr_ad, qsum_ad, dp_ad, esl_ad
    INTEGER :: i, k, l, m, k0
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp_ad11
    INTEGER :: ad_count
    INTEGER :: i0
    INTEGER :: branch
    INTEGER :: ad_count0
    INTEGER :: i3
    CALL POPREALARRAY(q4, 8*4*(i2-i1+1)*km/8)
    CALL POPREALARRAY(qsum)
    CALL POPREALARRAY(dp1, 8*(i2-i1+1)*km/8)
    CALL POPREALARRAY(qs, 8*(i2-i1+1)/8)
    CALL POPINTEGER(m)
    CALL POPINTEGER(l)
    dp1_ad = 0.0_FVPRC
    qsum_ad = 0.0_FVPRC
    q4_ad = 0.0_FVPRC
    DO i=i2,i1,-1
      DO k=kn,1,-1
        CALL POPCONTROL1B_FV(branch)
        IF (branch .EQ. 0) THEN
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          pr = (pe2(i, k+1)-pe1(i, l))/dp1(i, l)
          temp_ad0 = 0.5*(pr+pl)*q2_ad(i, k)
          temp_ad1 = 0.5*(q4(4, i, l)+q4(3, i, l)-q4(2, i, l))*q2_ad(i, &
&           k)
          temp_ad2 = -(r3*q4(4, i, l)*q2_ad(i, k))
          q4_ad(2, i, l) = q4_ad(2, i, l) + q2_ad(i, k) - temp_ad0
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad0 - r3*(pr*(pr+pl)+pl&
&           **2)*q2_ad(i, k)
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad0
          pr_ad = (2*pr+pl)*temp_ad2 + temp_ad1
          pl_ad = (2*pl+pr)*temp_ad2 + temp_ad1
          q2_ad(i, k) = 0.0_FVPRC
          temp_ad3 = pr_ad/dp1(i, l)
          pe2_ad(i, k+1) = pe2_ad(i, k+1) + temp_ad3
          pe1_ad(i, l) = pe1_ad(i, l) - temp_ad3
          dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k+1)-pe1(i, l))*temp_ad3&
&           /dp1(i, l)
        ELSE
          temp_ad11 = q2_ad(i, k)/dp2(i, k)
          qsum_ad = qsum_ad + temp_ad11
          dp2_ad(i, k) = dp2_ad(i, k) - qsum*temp_ad11/dp2(i, k)
          q2_ad(i, k) = 0.0_FVPRC
          CALL POPCONTROL2B_FV(branch)
          IF (branch .EQ. 0) THEN
            dp = pe2(i, k+1) - pe1(i, m)
            esl = dp/dp1(i, m)
            temp0 = q4(3, i, m) - q4(2, i, m) + q4(4, i, m)*(-(r23*esl)+&
&             1.)
            temp_ad8 = dp*qsum_ad
            temp_ad9 = 0.5*esl*temp_ad8
            q4_ad(2, i, m) = q4_ad(2, i, m) + temp_ad8 - temp_ad9
            esl_ad = 0.5*temp0*temp_ad8 - q4(4, i, m)*r23*temp_ad9
            q4_ad(3, i, m) = q4_ad(3, i, m) + temp_ad9
            q4_ad(4, i, m) = q4_ad(4, i, m) + (1.-r23*esl)*temp_ad9
            temp_ad10 = esl_ad/dp1(i, m)
            dp_ad = temp_ad10 + (q4(2, i, m)+0.5*(esl*temp0))*qsum_ad
            dp1_ad(i, m) = dp1_ad(i, m) - dp*temp_ad10/dp1(i, m)
            pe2_ad(i, k+1) = pe2_ad(i, k+1) + dp_ad
            pe1_ad(i, m) = pe1_ad(i, m) - dp_ad
          ELSE IF (branch .NE. 1) THEN
            GOTO 100
          END IF
          CALL POPINTEGER(ad_count0)
          DO i3=1,ad_count0
            IF (i3 .EQ. 1) THEN
              CALL POPCONTROL1B_FV(branch)
            ELSE
              dp1_ad(i, m) = dp1_ad(i, m) + q4(1, i, m)*qsum_ad
              q4_ad(1, i, m) = q4_ad(1, i, m) + dp1(i, m)*qsum_ad
            END IF
            CALL POPINTEGER(m)
          END DO
          pl = (pe2(i, k)-pe1(i, l))/dp1(i, l)
          CALL POPREALARRAY(qsum)
          temp = q4(4, i, l) + q4(3, i, l) - q4(2, i, l)
          temp_ad4 = (q4(2, i, l)+0.5*(temp*(pl+1.))-r3*(q4(4, i, l)*(pl&
&           *(pl+1.)+1.)))*qsum_ad
          temp_ad5 = (pe1(i, l+1)-pe2(i, k))*qsum_ad
          temp_ad6 = 0.5*(pl+1.)*temp_ad5
          temp_ad7 = -(r3*q4(4, i, l)*temp_ad5)
          pe1_ad(i, l+1) = pe1_ad(i, l+1) + temp_ad4
          pe2_ad(i, k) = pe2_ad(i, k) - temp_ad4
          q4_ad(2, i, l) = q4_ad(2, i, l) + temp_ad5 - temp_ad6
          q4_ad(4, i, l) = q4_ad(4, i, l) + temp_ad6 - r3*(pl*(pl+1.)+1.&
&           )*temp_ad5
          q4_ad(3, i, l) = q4_ad(3, i, l) + temp_ad6
          pl_ad = (2*pl+1.)*temp_ad7 + 0.5*temp*temp_ad5
          qsum_ad = 0.0_FVPRC
        END IF
        temp_ad = pl_ad/dp1(i, l)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad
        pe1_ad(i, l) = pe1_ad(i, l) - temp_ad
        dp1_ad(i, l) = dp1_ad(i, l) - (pe2(i, k)-pe1(i, l))*temp_ad/dp1(&
&         i, l)
 100    CALL POPINTEGER(ad_count)
        DO i0=1,ad_count
          IF (i0 .EQ. 1) CALL POPCONTROL1B_FV(branch)
          CALL POPINTEGER(l)
        END DO
      END DO
    END DO
    CALL POPCONTROL2B_FV(branch)
    IF (branch .NE. 0) THEN
      IF (branch .NE. 1) CALL SCALAR_PROFILE_LINEAR_BWD(qs, q4, q4_ad&
&                                                    , dp1, dp1_ad, km, &
&                                                    i1, i2, iv, q_min)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
        q1_ad(i, j, k) = q1_ad(i, j, k) + q4_ad(1, i, k)
        q4_ad(1, i, k) = 0.0_FVPRC
        pe1_ad(i, k+1) = pe1_ad(i, k+1) + dp1_ad(i, k)
        pe1_ad(i, k) = pe1_ad(i, k) - dp1_ad(i, k)
        dp1_ad(i, k) = 0.0_FVPRC
      END DO
    END DO
  END SUBROUTINE MAP1_Q2_BWD
!  Differentiation of cs_profile_linear in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord2 
!a2b_edge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_co
!re_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_
!p dyn_core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dyna
!mics_mod.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils_
!mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.compute
!_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer_
!fb fv_mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_m
!od.steepz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mod
!.riem_solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_so
!lver nh_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mod
!.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corner
!s sw_core_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d_f
!b tp_core_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm_
!fb tp_core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: qs delp a4
!   with respect to varying inputs: qs delp a4
  SUBROUTINE CS_PROFILE_LINEAR_FWD(qs, a4, delp, km, i1, i2, iv)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
    INTEGER, INTENT(IN) :: i1, i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    INTEGER :: i, k, im
    IF (iv .EQ. -2) THEN
      DO i=i1,i2
        gam(i, 2) = 0.5
        q(i, 1) = 1.5*a4(1, i, 1)
      END DO
      DO k=2,km-1
        DO i=i1,i2
          grat = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY(bet)
          bet = 2. + grat + grat - gam(i, k)
          CALL PUSHREALARRAY(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))/bet
          gam(i, k+1) = grat/bet
        END DO
      END DO
      DO i=i1,i2
        grat = delp(i, km-1)/delp(i, km)
        CALL PUSHREALARRAY(q(i, km))
        q(i, km) = (3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, km-&
&         1))/(2.+grat+grat-gam(i, km))
        CALL PUSHREALARRAY(q(i, km+1))
        q(i, km+1) = qs(i)
      END DO
      DO k=km-1,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY(q(i, k))
          q(i, k) = q(i, k) - gam(i, k+1)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B_FV(1)
    ELSE
      DO i=i1,i2
! grid ratio
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        q(i, 1) = ((grat+grat)*(grat+1.)*a4(1, i, 1)+a4(1, i, 2))/bet
        gam(i, 1) = (1.+grat*(grat+1.5))/bet
      END DO
      DO k=2,km
        DO i=i1,i2
          CALL PUSHREALARRAY(d4(i))
          d4(i) = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY(bet)
          bet = 2. + d4(i) + d4(i) - gam(i, k-1)
          CALL PUSHREALARRAY(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))/bet
          gam(i, k) = d4(i)/bet
        END DO
      END DO
      DO i=i1,i2
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL PUSHREALARRAY(q(i, km+1))
        q(i, km+1) = (2.*d4(i)*(d4(i)+1.)*a4(1, i, km)+a4(1, i, km-1)-&
&         a_bot*q(i, km))/(d4(i)*(d4(i)+0.5)-a_bot*gam(i, km))
      END DO
      DO k=km,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY(q(i, k))
          q(i, k) = q(i, k) - gam(i, k)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B_FV(0)
    END IF
    DO k=1,km
      DO i=i1,i2
        CALL PUSHREALARRAY(a4(2, i, k))
        a4(2, i, k) = q(i, k)
        CALL PUSHREALARRAY(a4(3, i, k))
        a4(3, i, k) = q(i, k+1)
        CALL PUSHREALARRAY(a4(4, i, k))
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
    END DO
    CALL PUSHREALARRAY(q, 8*(i2-i1+1)*(km+1)/8)
    CALL PUSHREALARRAY(bet)
    CALL PUSHREALARRAY(d4, 8*(i2-i1+1)/8)
    CALL PUSHREALARRAY(gam, 8*(i2-i1+1)*km/8)
  END SUBROUTINE CS_PROFILE_LINEAR_FWD
!  Differentiation of cs_profile_linear in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord2
! a2b_edge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_c
!ore_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad
!_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_dyn
!amics_mod.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_utils
!_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.comput
!e_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer
!_fb fv_mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_mapz_
!mod.steepz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core_mo
!d.riem_solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim1_s
!olver nh_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core_mo
!d.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4corne
!rs sw_core_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_2d_
!fb tp_core_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fxppm
!_fb tp_core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: qs delp a4
!   with respect to varying inputs: qs delp a4
  SUBROUTINE CS_PROFILE_LINEAR_BWD(qs, qs_ad, a4, a4_ad, delp, &
&   delp_ad, km, i1, i2, iv)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i1, i2
    INTEGER, INTENT(IN) :: km
    INTEGER, INTENT(IN) :: iv
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
    REAL(fvprc) :: qs_ad(i1:i2)
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
    REAL(fvprc) :: delp_ad(i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4_ad(4, i1:i2, km)
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: gam_ad(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: q_ad(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: d4_ad(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: bet_ad, a_bot_ad, grat_ad
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    INTEGER :: i, k, im
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp2
    REAL(fvprc) :: temp_ad11
    REAL(fvprc) :: temp_ad12
    REAL(fvprc) :: temp_ad13
    REAL(fvprc) :: temp_ad14
    INTEGER :: branch
    CALL POPREALARRAY(gam, 8*(i2-i1+1)*km/8)
    CALL POPREALARRAY(d4, 8*(i2-i1+1)/8)
    CALL POPREALARRAY(bet)
    CALL POPREALARRAY(q, 8*(i2-i1+1)*(km+1)/8)
    q_ad = 0.0_FVPRC
    DO k=km,1,-1
      DO i=i2,i1,-1
        CALL POPREALARRAY(a4(4, i, k))
        temp_ad14 = 3.*a4_ad(4, i, k)
        a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad14
        a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad14
        a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad14
        a4_ad(4, i, k) = 0.0_FVPRC
        CALL POPREALARRAY(a4(3, i, k))
        q_ad(i, k+1) = q_ad(i, k+1) + a4_ad(3, i, k)
        a4_ad(3, i, k) = 0.0_FVPRC
        CALL POPREALARRAY(a4(2, i, k))
        q_ad(i, k) = q_ad(i, k) + a4_ad(2, i, k)
        a4_ad(2, i, k) = 0.0_FVPRC
      END DO
    END DO
    CALL POPCONTROL1B_FV(branch)
    IF (branch .EQ. 0) THEN
      gam_ad = 0.0_FVPRC
      DO k=1,km,1
        DO i=i2,i1,-1
          CALL POPREALARRAY(q(i, k))
          gam_ad(i, k) = gam_ad(i, k) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k)*q_ad(i, k)
        END DO
      END DO
      d4_ad = 0.0_FVPRC
      DO i=i2,i1,-1
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL POPREALARRAY(q(i, km+1))
        temp2 = d4(i)*(d4(i)+0.5) - a_bot*gam(i, km)
        temp_ad11 = q_ad(i, km+1)/temp2
        temp1 = d4(i)*(d4(i)+1.)
        temp_ad12 = 2.*a4(1, i, km)*temp_ad11
        temp_ad13 = -((2.*(temp1*a4(1, i, km))+a4(1, i, km-1)-a_bot*q(i&
&         , km))*temp_ad11/temp2)
        a4_ad(1, i, km) = a4_ad(1, i, km) + 2.*temp1*temp_ad11
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + temp_ad11
        a_bot_ad = -(gam(i, km)*temp_ad13) - q(i, km)*temp_ad11
        d4_ad(i) = d4_ad(i) + (2*d4(i)+1.5)*a_bot_ad + (2*d4(i)+0.5)*&
&         temp_ad13 + (2*d4(i)+1.)*temp_ad12
        q_ad(i, km) = q_ad(i, km) - a_bot*temp_ad11
        gam_ad(i, km) = gam_ad(i, km) - a_bot*temp_ad13
        q_ad(i, km+1) = 0.0_FVPRC
      END DO
      DO k=km,2,-1
        DO i=i2,i1,-1
          temp_ad9 = q_ad(i, k)/bet
          temp_ad8 = 3.*temp_ad9
          CALL POPREALARRAY(q(i, k))
          bet_ad = -((3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))*&
&           temp_ad9/bet) - d4(i)*gam_ad(i, k)/bet**2
          d4_ad(i) = d4_ad(i) + a4(1, i, k)*temp_ad8 + 2*bet_ad + gam_ad&
&           (i, k)/bet
          gam_ad(i, k) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + temp_ad8
          a4_ad(1, i, k) = a4_ad(1, i, k) + d4(i)*temp_ad8
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad9
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY(bet)
          gam_ad(i, k-1) = gam_ad(i, k-1) - bet_ad
          CALL POPREALARRAY(d4(i))
          temp_ad10 = d4_ad(i)/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad10
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad10/delp(i&
&           , k)
          d4_ad(i) = 0.0_FVPRC
        END DO
      END DO
      DO i=i2,i1,-1
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        temp_ad4 = gam_ad(i, 1)/bet
        gam_ad(i, 1) = 0.0_FVPRC
        temp_ad6 = q_ad(i, 1)/bet
        temp_ad5 = a4(1, i, 1)*temp_ad6
        temp0 = 2*grat*(grat+1.)
        bet_ad = -((temp0*a4(1, i, 1)+a4(1, i, 2))*temp_ad6/bet) - (grat&
&         *(grat+1.5)+1.)*temp_ad4/bet
        grat_ad = (4*grat+2*1.)*temp_ad5 + (2*grat+0.5)*bet_ad + (2*grat&
&         +1.5)*temp_ad4
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + temp0*temp_ad6
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + temp_ad6
        q_ad(i, 1) = 0.0_FVPRC
        temp_ad7 = grat_ad/delp(i, 1)
        delp_ad(i, 2) = delp_ad(i, 2) + temp_ad7
        delp_ad(i, 1) = delp_ad(i, 1) - delp(i, 2)*temp_ad7/delp(i, 1)
      END DO
    ELSE
      gam_ad = 0.0_FVPRC
      DO k=1,km-1,1
        DO i=i2,i1,-1
          CALL POPREALARRAY(q(i, k))
          gam_ad(i, k+1) = gam_ad(i, k+1) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k+1)*q_ad(i, k)
        END DO
      END DO
      DO i=i2,i1,-1
        CALL POPREALARRAY(q(i, km+1))
        qs_ad(i) = qs_ad(i) + q_ad(i, km+1)
        q_ad(i, km+1) = 0.0_FVPRC
        grat = delp(i, km-1)/delp(i, km)
        CALL POPREALARRAY(q(i, km))
        temp = 2*grat - gam(i, km) + 2.
        temp_ad1 = q_ad(i, km)/temp
        temp_ad2 = -((3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, &
&         km-1))*temp_ad1/temp)
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + 3.*temp_ad1
        a4_ad(1, i, km) = a4_ad(1, i, km) + 3.*temp_ad1
        grat_ad = 2*temp_ad2 - qs(i)*temp_ad1
        qs_ad(i) = qs_ad(i) - grat*temp_ad1
        q_ad(i, km-1) = q_ad(i, km-1) - temp_ad1
        gam_ad(i, km) = gam_ad(i, km) - temp_ad2
        q_ad(i, km) = 0.0_FVPRC
        temp_ad3 = grat_ad/delp(i, km)
        delp_ad(i, km-1) = delp_ad(i, km-1) + temp_ad3
        delp_ad(i, km) = delp_ad(i, km) - delp(i, km-1)*temp_ad3/delp(i&
&         , km)
      END DO
      DO k=km-1,2,-1
        DO i=i2,i1,-1
          temp_ad = q_ad(i, k)/bet
          CALL POPREALARRAY(q(i, k))
          grat = delp(i, k-1)/delp(i, k)
          bet_ad = -((3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))*temp_ad/&
&           bet) - grat*gam_ad(i, k+1)/bet**2
          grat_ad = 2*bet_ad + gam_ad(i, k+1)/bet
          gam_ad(i, k+1) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + 3.*temp_ad
          a4_ad(1, i, k) = a4_ad(1, i, k) + 3.*temp_ad
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY(bet)
          gam_ad(i, k) = gam_ad(i, k) - bet_ad
          temp_ad0 = grat_ad/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad0
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad0/delp(i, &
&           k)
        END DO
      END DO
      DO i=i2,i1,-1
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + 1.5*q_ad(i, 1)
        q_ad(i, 1) = 0.0_FVPRC
      END DO
    END IF
  END SUBROUTINE CS_PROFILE_LINEAR_BWD
!  Differentiation of scalar_profile_linear in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_o
!rd2 a2b_edge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dy
!n_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_g
!rad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv_
!dynamics_mod.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_ut
!ils_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.com
!pute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tra
!cer_fb fv_mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_ma
!pz_mod.steepz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_core
!_mod.riem_solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.sim
!1_solver nh_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_core
!_mod.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4co
!rners sw_core_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp_
!2d_fb tp_core_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.fx
!ppm_fb tp_core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: delp a4
!   with respect to varying inputs: delp a4
  SUBROUTINE SCALAR_PROFILE_LINEAR_FWD(qs, a4, delp, km, i1, i2, iv, &
&   qmin)
    IMPLICIT NONE
! Optimized vertical profile reconstruction:
! Latest: Apr 2008 S.-J. Lin, NOAA/GFDL
    INTEGER, INTENT(IN) :: i1, i2
! vertical dimension
    INTEGER, INTENT(IN) :: km
! iv =-1: winds
    INTEGER, INTENT(IN) :: iv
! iv = 0: positive definite scalars
! iv = 1: others
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
! layer pressure thickness
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
! Interpolated values
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(IN) :: qmin
!-----------------------------------------------------------------------
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    INTEGER :: i, k, im
    IF (iv .EQ. -2) THEN
      DO i=i1,i2
        gam(i, 2) = 0.5
        q(i, 1) = 1.5*a4(1, i, 1)
      END DO
      DO k=2,km-1
        DO i=i1,i2
          grat = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY(bet)
          bet = 2. + grat + grat - gam(i, k)
          CALL PUSHREALARRAY(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))/bet
          gam(i, k+1) = grat/bet
        END DO
      END DO
      DO i=i1,i2
        grat = delp(i, km-1)/delp(i, km)
        CALL PUSHREALARRAY(q(i, km))
        q(i, km) = (3.*(a4(1, i, km-1)+a4(1, i, km))-grat*qs(i)-q(i, km-&
&         1))/(2.+grat+grat-gam(i, km))
        CALL PUSHREALARRAY(q(i, km+1))
        q(i, km+1) = qs(i)
      END DO
      DO k=km-1,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY(q(i, k))
          q(i, k) = q(i, k) - gam(i, k+1)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B_FV(1)
    ELSE
      DO i=i1,i2
! grid ratio
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        q(i, 1) = ((grat+grat)*(grat+1.)*a4(1, i, 1)+a4(1, i, 2))/bet
        gam(i, 1) = (1.+grat*(grat+1.5))/bet
      END DO
      DO k=2,km
        DO i=i1,i2
          CALL PUSHREALARRAY(d4(i))
          d4(i) = delp(i, k-1)/delp(i, k)
          CALL PUSHREALARRAY(bet)
          bet = 2. + d4(i) + d4(i) - gam(i, k-1)
          CALL PUSHREALARRAY(q(i, k))
          q(i, k) = (3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))/bet
          gam(i, k) = d4(i)/bet
        END DO
      END DO
      DO i=i1,i2
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL PUSHREALARRAY(q(i, km+1))
        q(i, km+1) = (2.*d4(i)*(d4(i)+1.)*a4(1, i, km)+a4(1, i, km-1)-&
&         a_bot*q(i, km))/(d4(i)*(d4(i)+0.5)-a_bot*gam(i, km))
      END DO
      DO k=km,1,-1
        DO i=i1,i2
          CALL PUSHREALARRAY(q(i, k))
          q(i, k) = q(i, k) - gam(i, k)*q(i, k+1)
        END DO
      END DO
      CALL PUSHCONTROL1B_FV(0)
    END IF
    DO k=1,km
      DO i=i1,i2
        CALL PUSHREALARRAY(a4(2, i, k))
        a4(2, i, k) = q(i, k)
        CALL PUSHREALARRAY(a4(3, i, k))
        a4(3, i, k) = q(i, k+1)
        CALL PUSHREALARRAY(a4(4, i, k))
        a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
      END DO
    END DO
    CALL PUSHREALARRAY(q, 8*(i2-i1+1)*(km+1)/8)
    CALL PUSHREALARRAY(bet)
    CALL PUSHREALARRAY(d4, 8*(i2-i1+1)/8)
    CALL PUSHREALARRAY(gam, 8*(i2-i1+1)*km/8)
  END SUBROUTINE SCALAR_PROFILE_LINEAR_FWD
!  Differentiation of scalar_profile_linear in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_
!ord2 a2b_edge_mod.a2b_ord4 a2b_edge_mod.extrap_corner  dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo d
!yn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_
!grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed fv_dynamics_mod.fv_dynamics fv
!_dynamics_mod.rayleigh_super fv_dynamics_mod.rayleigh_friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_grid_u
!tils_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.lagrangian_to_eulerian fv_mapz_mod.co
!mpute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tr
!acer_fb fv_mapz_mod.map1_q2 fv_mapz_mod.map1_cubic fv_mapz_mod.scalar_profile_linear fv_mapz_mod.cs_profile_linear fv_m
!apz_mod.steepz fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.update_dz_c nh_core_mod.update_dz_d nh_cor
!e_mod.riem_solver_c nh_core_mod.riem_solver3 nh_core_mod.rim_2d nh_core_mod.sim3_solver nh_core_mod.sim3p0_solver nh_core_mod.si
!m1_solver nh_core_mod.sim_solver nh_core_mod.edge_profile sw_core_mod.c_sw sw_core_mod.d_sw sw_core_mod.divergence_corner sw_cor
!e_mod.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.edge_interpolate4 sw_core_mod.fill2_4corners sw_core_mod.fill_4c
!orners sw_core_mod.compute_div_damping sw_core_mod.smag_corner sw_core_mod.ytp_v sw_core_mod.xtp_u tp_core_mod.fv_tp
!_2d_fb tp_core_mod.copy_corners tp_core_mod.xtp tp_core_mod.ytp tp_core_mod.xppm0 tp_core_mod.yppm0 tp_core_mod.f
!xppm_fb tp_core_mod.fyppm tp_core_mod.deln_flux)):
!   gradient     of useful results: delp a4
!   with respect to varying inputs: delp a4
  SUBROUTINE SCALAR_PROFILE_LINEAR_BWD(qs, a4, a4_ad, delp, delp_ad, &
&   km, i1, i2, iv, qmin)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i1, i2
    INTEGER, INTENT(IN) :: km
    INTEGER, INTENT(IN) :: iv
    REAL(fvprc), INTENT(IN) :: qs(i1:i2)
    REAL(fvprc), INTENT(IN) :: delp(i1:i2, km)
    REAL(fvprc) :: delp_ad(i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4(4, i1:i2, km)
    REAL(fvprc), INTENT(INOUT) :: a4_ad(4, i1:i2, km)
    REAL(fvprc), INTENT(IN) :: qmin
    LOGICAL :: extm(i1:i2, km)
    REAL(fvprc) :: gam(i1:i2, km)
    REAL(fvprc) :: gam_ad(i1:i2, km)
    REAL(fvprc) :: q(i1:i2, km+1)
    REAL(fvprc) :: q_ad(i1:i2, km+1)
    REAL(fvprc) :: d4(i1:i2)
    REAL(fvprc) :: d4_ad(i1:i2)
    REAL(fvprc) :: bet, a_bot, grat
    REAL(fvprc) :: bet_ad, a_bot_ad, grat_ad
    REAL(fvprc) :: pmp_1, lac_1, pmp_2, lac_2
    INTEGER :: i, k, im
    REAL(fvprc) :: temp_ad
    REAL(fvprc) :: temp_ad0
    REAL(fvprc) :: temp
    REAL(fvprc) :: temp_ad1
    REAL(fvprc) :: temp_ad2
    REAL(fvprc) :: temp_ad3
    REAL(fvprc) :: temp0
    REAL(fvprc) :: temp_ad4
    REAL(fvprc) :: temp_ad5
    REAL(fvprc) :: temp_ad6
    REAL(fvprc) :: temp_ad7
    REAL(fvprc) :: temp_ad8
    REAL(fvprc) :: temp_ad9
    REAL(fvprc) :: temp_ad10
    REAL(fvprc) :: temp1
    REAL(fvprc) :: temp2
    REAL(fvprc) :: temp_ad11
    REAL(fvprc) :: temp_ad12
    REAL(fvprc) :: temp_ad13
    REAL(fvprc) :: temp_ad14
    INTEGER :: branch
    CALL POPREALARRAY(gam, 8*(i2-i1+1)*km/8)
    CALL POPREALARRAY(d4, 8*(i2-i1+1)/8)
    CALL POPREALARRAY(bet)
    CALL POPREALARRAY(q, 8*(i2-i1+1)*(km+1)/8)
    q_ad = 0.0_FVPRC
    DO k=km,1,-1
      DO i=i2,i1,-1
        CALL POPREALARRAY(a4(4, i, k))
        temp_ad14 = 3.*a4_ad(4, i, k)
        a4_ad(1, i, k) = a4_ad(1, i, k) + 2.*temp_ad14
        a4_ad(2, i, k) = a4_ad(2, i, k) - temp_ad14
        a4_ad(3, i, k) = a4_ad(3, i, k) - temp_ad14
        a4_ad(4, i, k) = 0.0_FVPRC
        CALL POPREALARRAY(a4(3, i, k))
        q_ad(i, k+1) = q_ad(i, k+1) + a4_ad(3, i, k)
        a4_ad(3, i, k) = 0.0_FVPRC
        CALL POPREALARRAY(a4(2, i, k))
        q_ad(i, k) = q_ad(i, k) + a4_ad(2, i, k)
        a4_ad(2, i, k) = 0.0_FVPRC
      END DO
    END DO
    CALL POPCONTROL1B_FV(branch)
    IF (branch .EQ. 0) THEN
      gam_ad = 0.0_FVPRC
      DO k=1,km,1
        DO i=i2,i1,-1
          CALL POPREALARRAY(q(i, k))
          gam_ad(i, k) = gam_ad(i, k) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k)*q_ad(i, k)
        END DO
      END DO
      d4_ad = 0.0_FVPRC
      DO i=i2,i1,-1
        a_bot = 1. + d4(i)*(d4(i)+1.5)
        CALL POPREALARRAY(q(i, km+1))
        temp2 = d4(i)*(d4(i)+0.5) - a_bot*gam(i, km)
        temp_ad11 = q_ad(i, km+1)/temp2
        temp1 = d4(i)*(d4(i)+1.)
        temp_ad12 = 2.*a4(1, i, km)*temp_ad11
        temp_ad13 = -((2.*(temp1*a4(1, i, km))+a4(1, i, km-1)-a_bot*q(i&
&         , km))*temp_ad11/temp2)
        a4_ad(1, i, km) = a4_ad(1, i, km) + 2.*temp1*temp_ad11
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + temp_ad11
        a_bot_ad = -(gam(i, km)*temp_ad13) - q(i, km)*temp_ad11
        d4_ad(i) = d4_ad(i) + (2*d4(i)+1.5)*a_bot_ad + (2*d4(i)+0.5)*&
&         temp_ad13 + (2*d4(i)+1.)*temp_ad12
        q_ad(i, km) = q_ad(i, km) - a_bot*temp_ad11
        gam_ad(i, km) = gam_ad(i, km) - a_bot*temp_ad13
        q_ad(i, km+1) = 0.0_FVPRC
      END DO
      DO k=km,2,-1
        DO i=i2,i1,-1
          temp_ad9 = q_ad(i, k)/bet
          temp_ad8 = 3.*temp_ad9
          CALL POPREALARRAY(q(i, k))
          bet_ad = -((3.*(a4(1, i, k-1)+d4(i)*a4(1, i, k))-q(i, k-1))*&
&           temp_ad9/bet) - d4(i)*gam_ad(i, k)/bet**2
          d4_ad(i) = d4_ad(i) + a4(1, i, k)*temp_ad8 + 2*bet_ad + gam_ad&
&           (i, k)/bet
          gam_ad(i, k) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + temp_ad8
          a4_ad(1, i, k) = a4_ad(1, i, k) + d4(i)*temp_ad8
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad9
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY(bet)
          gam_ad(i, k-1) = gam_ad(i, k-1) - bet_ad
          CALL POPREALARRAY(d4(i))
          temp_ad10 = d4_ad(i)/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad10
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad10/delp(i&
&           , k)
          d4_ad(i) = 0.0_FVPRC
        END DO
      END DO
      DO i=i2,i1,-1
        grat = delp(i, 2)/delp(i, 1)
        bet = grat*(grat+0.5)
        temp_ad4 = gam_ad(i, 1)/bet
        gam_ad(i, 1) = 0.0_FVPRC
        temp_ad6 = q_ad(i, 1)/bet
        temp_ad5 = a4(1, i, 1)*temp_ad6
        temp0 = 2*grat*(grat+1.)
        bet_ad = -((temp0*a4(1, i, 1)+a4(1, i, 2))*temp_ad6/bet) - (grat&
&         *(grat+1.5)+1.)*temp_ad4/bet
        grat_ad = (4*grat+2*1.)*temp_ad5 + (2*grat+0.5)*bet_ad + (2*grat&
&         +1.5)*temp_ad4
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + temp0*temp_ad6
        a4_ad(1, i, 2) = a4_ad(1, i, 2) + temp_ad6
        q_ad(i, 1) = 0.0_FVPRC
        temp_ad7 = grat_ad/delp(i, 1)
        delp_ad(i, 2) = delp_ad(i, 2) + temp_ad7
        delp_ad(i, 1) = delp_ad(i, 1) - delp(i, 2)*temp_ad7/delp(i, 1)
      END DO
    ELSE
      gam_ad = 0.0_FVPRC
      DO k=1,km-1,1
        DO i=i2,i1,-1
          CALL POPREALARRAY(q(i, k))
          gam_ad(i, k+1) = gam_ad(i, k+1) - q(i, k+1)*q_ad(i, k)
          q_ad(i, k+1) = q_ad(i, k+1) - gam(i, k+1)*q_ad(i, k)
        END DO
      END DO
      DO i=i2,i1,-1
        CALL POPREALARRAY(q(i, km+1))
        q_ad(i, km+1) = 0.0_FVPRC
        grat = delp(i, km-1)/delp(i, km)
        CALL POPREALARRAY(q(i, km))
        temp = 2*grat - gam(i, km) + 2.
        temp_ad1 = q_ad(i, km)/temp
        temp_ad2 = -((3.*(a4(1, i, km-1)+a4(1, i, km))-qs(i)*grat-q(i, &
&         km-1))*temp_ad1/temp)
        a4_ad(1, i, km-1) = a4_ad(1, i, km-1) + 3.*temp_ad1
        a4_ad(1, i, km) = a4_ad(1, i, km) + 3.*temp_ad1
        grat_ad = 2*temp_ad2 - qs(i)*temp_ad1
        q_ad(i, km-1) = q_ad(i, km-1) - temp_ad1
        gam_ad(i, km) = gam_ad(i, km) - temp_ad2
        q_ad(i, km) = 0.0_FVPRC
        temp_ad3 = grat_ad/delp(i, km)
        delp_ad(i, km-1) = delp_ad(i, km-1) + temp_ad3
        delp_ad(i, km) = delp_ad(i, km) - delp(i, km-1)*temp_ad3/delp(i&
&         , km)
      END DO
      DO k=km-1,2,-1
        DO i=i2,i1,-1
          temp_ad = q_ad(i, k)/bet
          CALL POPREALARRAY(q(i, k))
          grat = delp(i, k-1)/delp(i, k)
          bet_ad = -((3.*(a4(1, i, k-1)+a4(1, i, k))-q(i, k-1))*temp_ad/&
&           bet) - grat*gam_ad(i, k+1)/bet**2
          grat_ad = 2*bet_ad + gam_ad(i, k+1)/bet
          gam_ad(i, k+1) = 0.0_FVPRC
          a4_ad(1, i, k-1) = a4_ad(1, i, k-1) + 3.*temp_ad
          a4_ad(1, i, k) = a4_ad(1, i, k) + 3.*temp_ad
          q_ad(i, k-1) = q_ad(i, k-1) - temp_ad
          q_ad(i, k) = 0.0_FVPRC
          CALL POPREALARRAY(bet)
          gam_ad(i, k) = gam_ad(i, k) - bet_ad
          temp_ad0 = grat_ad/delp(i, k)
          delp_ad(i, k-1) = delp_ad(i, k-1) + temp_ad0
          delp_ad(i, k) = delp_ad(i, k) - delp(i, k-1)*temp_ad0/delp(i, &
&           k)
        END DO
      END DO
      DO i=i2,i1,-1
        a4_ad(1, i, 1) = a4_ad(1, i, 1) + 1.5*q_ad(i, 1)
        q_ad(i, 1) = 0.0_FVPRC
      END DO
    END IF
  END SUBROUTINE SCALAR_PROFILE_LINEAR_BWD
END MODULE FV_MAPZ_MOD_B
