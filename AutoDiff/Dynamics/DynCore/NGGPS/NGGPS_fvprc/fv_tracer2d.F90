module fv_tracer2d_mod
   use tp_core_mod,       only: fv_tp_2d, fv_tp_2d_fb, copy_corners
...
!   use boundary_mod,      only: nested_grid_BC_apply, nested_grid_BC_apply_intT
   use fv_arrays_mod,     only: fv_grid_type, fv_nest_type, fv_atmos_type, fv_grid_bounds_type, REAL4, REAL8, FVPRC
...
real(FVPRC), allocatable, dimension(:,:,:) :: nest_fx_west_accum, nest_fx_east_accum, nest_fx_south_accum, nest_fx_north_accum

!---- version number -----
!   character(len=128) :: version = '$Id: fv_tracer2d.F90,v 1.1.2.2.2.1.30.1.90.6.4.1.2.1.4.3.2.2.2.1.2.1 2017/02/16 03:47:47 aoloso Exp $'
...
subroutine tracer_2d_1L(q, dp0, mfx, mfy, cx, cy, gridstruct, neststruct, bd, domain, npx, npy, npz, Qs, nq, hord, hord_pert, &
     q_split, k, q3, dt, id_divg, k_split, dpA)
      ...
      real(FVPRC)   , intent(INOUT) :: q(bd%isd:bd%ied,bd%jsd:bd%jed,nq)       ! 2D Tracers
      real(FVPRC)   , intent(INOUT) ::q3(bd%isd:bd%ied,bd%jsd:bd%jed,npz,nq)   ! Tracers
      real(FVPRC)   , intent(INOUT) :: dp0(bd%is:bd%ie,bd%js:bd%je)        ! DELP before dyn_core
      real(FVPRC)   , intent(IN) :: mfx(bd%is:bd%ie+1,bd%js:bd%je)    ! Mass Flux X-Dir
      real(FVPRC)   , intent(IN) :: mfy(bd%is:bd%ie  ,bd%js:bd%je+1)    ! Mass Flux Y-Dir
      real(FVPRC)   , intent(IN) ::  cx(bd%is:bd%ie+1,bd%jsd:bd%jed)  ! Courant Number X-Dir
      real(FVPRC)   , intent(IN) ::  cy(bd%isd:bd%ied,bd%js :bd%je +1)  ! Courant Number Y-Dir
...
! Local Arrays
      ...
      real(FVPRC) :: q_tj(bd%isd:bd%ied,bd%jsd:bd%jed,nq)       ! 2D Tracers
...
!      dy     => gridstruct%dy  
      ...
      do j=jsd,jed
         do i=is,ie+1
            if (cx(i,j) > 0.) then
                xfx(i,j) = cx(i,j)*gridstruct%dxa(i-1,j)*gridstruct%dy(i,j)*gridstruct%sin_sg(i-1,j,3)
            else
                xfx(i,j) = cx(i,j)*gridstruct%dxa(i,j)*gridstruct%dy(i,j)*gridstruct%sin_sg(i,j,1)
            endif
         enddo
      enddo
      ...
      do j=js,je+1
         do i=isd,ied
            if (cy(i,j) > 0.) then
                yfx(i,j) = cy(i,j)*gridstruct%dya(i,j-1)*gridstruct%dx(i,j)*gridstruct%sin_sg(i,j-1,4)
            else
                yfx(i,j) = cy(i,j)*gridstruct%dya(i,j)*gridstruct%dx(i,j)*gridstruct%sin_sg(i,j,2)
            endif
         enddo
      enddo
...
! Determine nsplt for tracer advection
         ...
         do j=js,je
            do i=is,ie
               cmax = max(abs(cx(i,j))+(1.-gridstruct%sina_u(i,j)),     &
                          abs(cy(i,j))+(1.-gridstruct%sina_v(i,j)), cmax)
            enddo
         enddo
...
! Start time split ...
      do it=1,nsplt
         ...
         do j=js,je
            do i=is,ie
               dp2(i,j) = dp1(i,j) + (mfx2(i,j) - mfx2(i+1,j) +  &
                          mfy2(i,j) - mfy2(i,j+1)) * gridstruct%rarea(i,j)
            enddo
         enddo
         ...
         do iq=Qs,nq
            if (hord == hord_pert) then
               call fv_tp_2d_fb( q(isd:ied,jsd:jed,iq), cx2, cy2, npx, npy, hord, fx, fy, &
                                 xfx, yfx, gridstruct, bd, ra_x, ra_y, mfx=mfx2, mfy=mfy2 )
            else
               q_tj(:,:,iq) = q(:,:,iq)
               fx_tj = fx
               fy_tj = fy 
               !UNCOMcall fv_tp_2d( q(isd:ied,jsd:jed,iq), cx2, cy2, npx, npy, hord, fx, fy, &
               !UNCOM               xfx, yfx, gridstruct, bd, ra_x, ra_y, mfx=mfx2, mfy=mfy2 )
               call fv_tp_2d( q(isd:ied,jsd:jed,iq), cx2, cy2, npx, npy, hord_pert, fx, fy, &
                              xfx, yfx, gridstruct, bd, ra_x, ra_y, mfx=mfx2, mfy=mfy2 )
            endif
            if( it==nsplt ) then
            do j=js,je
...
!           end if
     ...
     if ( id_divg > 0 ) then
         ...
         do j=js,je
            do i=is,ie
               dp0(i,j) = (xfx(i+1,j)-xfx(i,j) + yfx(i,j+1)-yfx(i,j))*gridstruct%rarea(i,j)*rdt
            enddo
         enddo
     endif
...
subroutine tracer_2d(q, dp1, mfx, mfy, cx, cy, gridstruct, bd, domain, npx, npy, npz,   &
                     Qs, nq,  hord, hord_pert, q_split, dt, id_divg, q_pack, q_packp, z_tracer, k_split, dpA)
      ...
      real(FVPRC)   , intent(INOUT) :: q(bd%isd:bd%ied,bd%jsd:bd%jed,npz,nq)   ! Tracers
      real(FVPRC)   , intent(INOUT) :: dp1(bd%is:bd%ie,bd%js:bd%je,npz)        ! DELP before dyn_core
      real(FVPRC)   , intent(INOUT) :: mfx(bd%is:bd%ie+1,bd%js:bd%je,  npz)    ! Mass Flux X-Dir
      real(FVPRC)   , intent(INOUT) :: mfy(bd%is:bd%ie  ,bd%js:bd%je+1,npz)    ! Mass Flux Y-Dir
      real(FVPRC)   , intent(INOUT) ::  cx(bd%is:bd%ie+1,bd%jsd:bd%jed  ,npz)  ! Courant Number X-Dir
      real(FVPRC)   , intent(INOUT) ::  cy(bd%isd:bd%ied,bd%js :bd%je +1,npz)  ! Courant Number Y-Dir
      type(fv_grid_type), intent(IN), target :: gridstruct
      type(domain2d), intent(INOUT) :: domain
      real(FVPRC), optional, intent(OUT) :: dpA(bd%is:bd%ie,bd%js:bd%je,npz)! DELP after advection
...
! Local Arrays
      ...
      real(FVPRC) :: q_tj(bd%isd:bd%ied,bd%jsd:bd%jed,npz,nq)   ! Tracers
...
!$OMP                                  sin_sg,cy,yfx,dya,dx)
      do k=1,npz
         do j=jsd,jed
            do i=is,ie+1
               if (cx(i,j,k) > 0.) then
                  xfx(i,j,k) = cx(i,j,k)*gridstruct%dxa(i-1,j)*gridstruct%dy(i,j)*gridstruct%sin_sg(i-1,j,3)
               else
                  xfx(i,j,k) = cx(i,j,k)*gridstruct%dxa(i,j)*gridstruct%dy(i,j)*gridstruct%sin_sg(i,j,1)
               endif
            enddo
         enddo
         do j=js,je+1
            do i=isd,ied
               if (cy(i,j,k) > 0.) then
                  yfx(i,j,k) = cy(i,j,k)*gridstruct%dya(i,j-1)*gridstruct%dx(i,j)*gridstruct%sin_sg(i,j-1,4)
               else
                  yfx(i,j,k) = cy(i,j,k)*gridstruct%dya(i,j)*gridstruct%dx(i,j)*gridstruct%sin_sg(i,j,2)
               endif
            enddo
         enddo
      enddo
...
! Top layers: C < max( abs(c_x), abs(c_y) )
            ...
            do j=js,je
               do i=is,ie
                  cmax_t  = max(abs(cx(i,j,k)), abs(cy(i,j,k))) + 1.-gridstruct%sin_sg(i,j,5)
                  cmax(k) = max( cmax_t, cmax(k) )
               enddo
            enddo
         endif
...
!$AD II-LOOP
      do k=1,npz
         do j=js,je
            do i=is,ie
               dp2(i,j,k) = dp1(i,j,k) + (mfx(i,j,k)-mfx(i+1,j,k)+mfy(i,j,k)-mfy(i,j+1,k))*gridstruct%rarea(i,j)
            enddo
         enddo

         do j=jsd,jed
            do i=is,ie
               ra_x(i,j,k) = gridstruct%area(i,j) + xfx(i,j,k) - xfx(i+1,j,k)
            enddo
         enddo
         do j=js,je
            do i=isd,ied
               ra_y(i,j,k) = gridstruct%area(i,j) + yfx(i,j,k) - yfx(i,j+1,k)
            enddo
         enddo
      enddo ! npz
      ...
      do k=1,npz
         do iq=Qs,nq
            if (hord == hord_pert) then
            call fv_tp_2d_fb(q(isd:ied,jsd:jed,k,iq), cx(is:ie+1,jsd:jed,k), cy(isd:ied,js:je+1,k), &
                          npx, npy, hord, fx(is:ie+1,js:je,k,iq ), fy(is:ie, js:je+1,k,iq ),&
                          xfx(is:ie+1,jsd:jed,k), yfx(isd:ied,js:je+1,k), &
                          gridstruct, bd, ra_x(is:ie,jsd:jed,k), ra_y(isd:ied,js:je,k), &
                          mfx=mfx(is:ie+1,js:je,k), mfy=mfy(is:ie,js:je+1,k))
            else
            q_tj(:,:,k,iq) = q(:,:,k,iq)
            fx_tj(:,:,k,iq) = fx(:,:,k,iq)
            fy_tj(:,:,k,iq) = fy(:,:,k,iq)
            !UNCOM      call fv_tp_2d(q(isd:ied,jsd:jed,k,iq), cx(is:ie+1,jsd:jed,k), cy(isd:ied,js:je+1,k), &
            !UNCOM                    npx, npy, hord, fx(is:ie+1,js:je,k,iq ), fy(is:ie, js:je+1,k,iq ),&
            !UNCOM                    xfx(is:ie+1,jsd:jed,k), yfx(isd:ied,js:je+1,k), &
            !UNCOM                    gridstruct, bd, ra_x(is:ie,jsd:jed,k), ra_y(isd:ied,js:je,k), &
            !UNCOM                    mfx=mfx(is:ie+1,js:je,k), mfy=mfy(is:ie,js:je+1,k))
            call fv_tp_2d(q(isd:ied,jsd:jed,k,iq), cx(is:ie+1,jsd:jed,k), cy(isd:ied,js:je+1,k), &
                          npx, npy, hord_pert, fx(is:ie+1,js:je,k,iq ), fy(is:ie, js:je+1,k,iq ),&
                          xfx(is:ie+1,jsd:jed,k), yfx(isd:ied,js:je+1,k), &
                          gridstruct, bd, ra_x(is:ie,jsd:jed,k), ra_y(isd:ied,js:je,k), &
                          mfx=mfx(is:ie+1,js:je,k), mfy=mfy(is:ie,js:je+1,k))
...
!$AD II-LOOP
        ...
        do j=js,je
           do i=is,ie
              dp1(i,j,k) = (xfx(i+1,j,k)-xfx(i,j,k) + yfx(i,j+1,k)-yfx(i,j,k))*gridstruct%rarea(i,j)*rdt
           enddo
        enddo
        enddo
   endif
...
subroutine tracer_2d_nested(q, dp1, mfx, mfy, cx, cy, gridstruct, bd, domain, npx, npy, npz,   &
                     nq,  hord, hord_pert, q_split, dt, id_divg, q_pack, q_packp, z_tracer, k_split, neststruct, parent_grid)
...
!$OMP                            private(ra_x, ra_y, fx, fy)
      do k=1,npz
         ...
         do iq=1,nq
            ...
            fy_tj = fy
            !UNCOM      call fv_tp_2d(q(isd:ied,jsd:jed,k,iq), cx(is:ie+1,jsd:jed,k), cy(isd:ied,js:je+1,k), &
            !UNCOM                    npx, npy, hord, fx, fy, xfx(is:ie+1,jsd:jed,k), yfx(isd:ied,js:je+1,k), &
            !UNCOM                    gridstruct, bd, ra_x, ra_y, mfx=mfx(is:ie+1,js:je,k), mfy=mfy(is:ie,js:je+1,k))
            call fv_tp_2d(q(isd:ied,jsd:jed,k,iq), cx(is:ie+1,jsd:jed,k), cy(isd:ied,js:je+1,k), &
                          npx, npy, hord_pert, fx, fy, xfx(is:ie+1,jsd:jed,k), yfx(isd:ied,js:je+1,k), &
                          gridstruct, bd, ra_x, ra_y, mfx=mfx(is:ie+1,js:je,k), mfy=mfy(is:ie,js:je+1,k))
            endif
            do j=js,je
               do i=is,ie
                  q(i,j,k,iq) = ( q(i,j,k,iq)*dp1(i,j,k) + &
                                (fx(i,j)-fx(i+1,j)+fy(i,j)-fy(i,j+1))*gridstruct%rarea(i,j) )/dp2(i,j,k)
               enddo
            enddo
...
!  type(fv_atmos_type), intent(INOUT) :: parent_grid
!
!  real(FVPRC), save, allocatable, dimension(:,:,:) :: coarse_fluxes_west, coarse_fluxes_east, coarse_fluxes_south, coarse_fluxes_north
