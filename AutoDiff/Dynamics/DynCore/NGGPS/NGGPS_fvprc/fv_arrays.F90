...
!#include <fms_platform.h>
  ...
  !use mpp_mod, only: mpp_broadcast !circular compile
  public
...
!---variable for strong typing grid parameters
  ...
  real(FVPRC), parameter:: real_big = 1.e30   ! big enough to cause blowup if used
...
! ESM requested diagnostics  -  dry mass/volume mixing ratios
 ...
 real(FVPRC),    allocatable :: w_mr(:)

     real(FVPRC), allocatable :: phalf(:)
     real(FVPRC), allocatable :: zsurf(:,:)
     real(FVPRC), allocatable :: zxg(:,:)
     real(FVPRC), allocatable :: pt1(:)


     logical :: initialized = .false.
     real(FVPRC)  sphum, liq_wat, ice_wat       ! GFDL physics
     real(FVPRC)  rainwat, snowwat, graupel

     real(FVPRC) :: efx(max_step), efx_sum, efx_nest(max_step), efx_sum_nest, mtq(max_step), mtq_sum
     integer :: steps
  ...
  type fv_grid_type
     ...
     real(FVPRC), allocatable, dimension(:,:,:) :: grid, agrid
     real(FVPRC), allocatable, dimension(:,:) :: area, area_c
     real(FVPRC), allocatable, dimension(:,:) :: rarea, rarea_c     

     real(FVPRC), allocatable, dimension(:,:) :: sina, cosa
     real(FVPRC), allocatable, dimension(:,:,:) :: e1,e2
     real(FVPRC), allocatable, dimension(:,:) :: dx, dy
     real(FVPRC), allocatable, dimension(:,:) :: dxc, dyc
     real(FVPRC), allocatable, dimension(:,:) :: dxa, dya
     real(FVPRC), allocatable, dimension(:,:) :: rdx, rdy
     real(FVPRC), allocatable, dimension(:,:) :: rdxc, rdyc
     real(FVPRC), allocatable, dimension(:,:) :: rdxa, rdya
     ...
     real(FVPRC), allocatable :: l2c_u(:,:), l2c_v(:,:)
     ! divergence Damping:
     real(FVPRC), allocatable :: divg_u(:,:), divg_v(:,:)    !
     ! Cubed_2_latlon:
     real(FVPRC), allocatable :: a11(:,:)
     real(FVPRC), allocatable :: a12(:,:)
     real(FVPRC), allocatable :: a21(:,:)
     real(FVPRC), allocatable :: a22(:,:)
     ! latlon_2_cubed:
     real(FVPRC), allocatable :: z11(:,:)
     real(FVPRC), allocatable :: z12(:,:)
     real(FVPRC), allocatable :: z21(:,:)
     real(FVPRC), allocatable :: z22(:,:)

!    real(FVPRC), allocatable :: w00(:,:)

     real(FVPRC), allocatable :: cosa_u(:,:)
     real(FVPRC), allocatable :: cosa_v(:,:)
     real(FVPRC), allocatable :: cosa_s(:,:)
     real(FVPRC), allocatable :: sina_u(:,:)
     real(FVPRC), allocatable :: sina_v(:,:)
     real(FVPRC), allocatable :: rsin_u(:,:)
     real(FVPRC), allocatable :: rsin_v(:,:)
     real(FVPRC), allocatable ::  rsina(:,:)
     real(FVPRC), allocatable ::  rsin2(:,:)
     ...
     real(FVPRC), allocatable :: sin_sg(:,:,:)
     real(FVPRC), allocatable :: cos_sg(:,:,:)
     !--------------------------------------------------
     ...
     real(FVPRC), allocatable :: eww(:,:)
     real(FVPRC), allocatable :: ess(:,:)

     ! Unit vectors for lat-lon grid
     real(kind=R_GRID), allocatable :: vlon(:,:,:), vlat(:,:,:)
     real(FVPRC), allocatable :: fC(:,:), f0(:,:)
     ...
     real(FVPRC)  :: acapN, acapS
     real(FVPRC)  :: globalarea  ! total Global Area
...
! TLM/ADM options for horizontal transport
   ...
   integer :: hord_vt_pert = -5    !   1 (all) first order scheme
   integer :: hord_tm_pert = -5    !   6 (mt)
   integer :: hord_dp_pert = -5    !  -5 (vt,tm,dp,tr) linear scheme based on PPM 4th order interpolation
   integer :: hord_tr_pert = 333   ! 333 (vt,tm,dp,tr) linear third order scheme

! TLM/ADM options for vertical remapping
   integer :: kord_mt_pert = 111   ! vertical mapping option for (u,v)
   integer :: kord_wz_pert = 111   ! vertical mapping option for w
   integer :: kord_tm_pert = 111   !
   integer :: kord_tr_pert = 111 

! TLM/ADM options for damping
   logical     :: split_damp = .true.
   integer     ::  nord_pert = 2
   real(FVPRC) :: dddmp_pert = 0.2
   real(FVPRC) :: d2_bg_pert = 0.015
   real(FVPRC) :: d4_bg_pert = 0.150
   real(FVPRC) :: d_ext_pert = 0.02
   logical     :: do_vort_damp_pert = .true.
   real(FVPRC) :: vtdm4_pert = 0.0005
   real(FVPRC) :: d_con_pert = 0.
   ...
   real(FVPRC)    :: dddmp = 0.0    ! coefficient for del-2 divergence damping (0.2)
                             ! for C90 or lower: 0.2
   real(FVPRC)    :: d2_bg = 0.0    ! coefficient for background del-2 divergence damping
   real(FVPRC)    :: d4_bg = 0.16   ! coefficient for background del-4(6) divergence damping
                             ! for stability, d4_bg must be <=0.16 if nord=3
   real(FVPRC)    :: vtdm4 = 0.0    ! coefficient for del-4 vorticity damping
   real(FVPRC)    :: d2_bg_k1 = 4.         ! factor for d2_bg (k=1)
   real(FVPRC)    :: d2_bg_k2 = 2.         ! factor for d2_bg (k=2)
   real(FVPRC)    :: d2_divg_max_k1 = 0.15 ! d2_divg max value (k=1)
   real(FVPRC)    :: d2_divg_max_k2 = 0.08 ! d2_divg max value (k=2)
   real(FVPRC)    :: damp_k_k1 = 0.2       ! damp_k value (k=1)
   real(FVPRC)    :: damp_k_k2 = 0.12      ! damp_k value (k=2)
...
! PG off centering:
   real(FVPRC)    :: beta  = 0.0    ! 0.5 is "neutral" but it may not be stable
!#ifdef SW_DYNAMICS
...
!#else
   ...
   real(FVPRC)    :: d_ext = 0.02   ! External model damping (was 0.02)
...
!-----------------------------------------------------------------------------------------------
   ...
   real(FVPRC)    :: p_fac  = 0.05
   real(FVPRC)    :: a_imp  = 0.75  ! Off center parameter for the implicit solver [0.5,1.0]
...
!#ifdef MARS_GCM
!   real(FVPRC)    :: p_ref = 600.
!   real(FVPRC)    :: reference_sfc_pres = 7.7E2
!   real(FVPRC)    :: sponge_damp=   1.0
!   real(FVPRC)    :: dry_mass = 7.7E2
!#else
...
!#endif
   ...
   real(FVPRC)    :: d_con = 0.
   real(FVPRC)    :: consv_te = 0.
   real(FVPRC)    :: tau = 0.                ! Time scale (days) for Rayleigh friction
   real(FVPRC)    :: rf_cutoff = 30.E2       ! cutoff pressure level for RF
...
!------------------------------------------------
  ...
  type fv_nest_BC_type_3D
     ...
     real(FVPRC), allocatable, dimension(:,:,:) :: west_t1, east_t1, south_t1, north_t1
     real(FVPRC), allocatable, dimension(:,:,:) :: west_t0, east_t0, south_t0, north_t0
...
!nested grid flags:
     ...
     real(FVPRC)    :: s_weight = 1.e-6 !sponge weight
     ...
     real(FVPRC), allocatable, dimension(:,:,:) :: wt_h, wt_u, wt_v
...
! from the D grid variables.
    real(FVPRC), ALLOCATABLE :: u(:,:,:)    !_NULL  ! D grid zonal wind (m/s)
    real(FVPRC), ALLOCATABLE :: v(:,:,:)    !_NULL  ! D grid meridional wind (m/s)
    real(FVPRC), ALLOCATABLE :: pt(:,:,:)   !_NULL  ! temperature (K)
    real(FVPRC), ALLOCATABLE :: delp(:,:,:) !_NULL  ! pressure thickness (pascal)
    real(FVPRC), ALLOCATABLE :: q(:,:,:,:)  !_NULL  ! specific humidity and constituents

!----------------------
! non-hydrostatic state:
!----------------------------------------------------------------------
    real(FVPRC), ALLOCATABLE ::     w(:,:,:)  !_NULL  ! cell center vertical wind (m/s)
    real(FVPRC), ALLOCATABLE ::  delz(:,:,:)  !_NULL  ! layer thickness (meters)
    real(FVPRC), ALLOCATABLE ::   ze0(:,:,:)  !_NULL  ! height at layer edges for remapping
    real(FVPRC), ALLOCATABLE ::  q_con(:,:,:) !_NULL  ! total condensates
...
!-----------------------------------------------------------------------
! dyn_aux:
    real(FVPRC), ALLOCATABLE :: ps (:,:)      !_NULL  ! Surface pressure (pascal)
    real(FVPRC), ALLOCATABLE :: pe (:,:,: )   !_NULL  ! edge pressure (pascal)
    real(FVPRC), ALLOCATABLE :: pk  (:,:,:)   !_NULL  ! pe**cappa
    real(FVPRC), ALLOCATABLE :: peln(:,:,:)   !_NULL  ! ln(pe)
    real(FVPRC), ALLOCATABLE :: pkz (:,:,:)   !_NULL  ! finite-volume mean pk

! For phys coupling:
    real(FVPRC), ALLOCATABLE :: u_srf(:,:)    !_NULL  ! Surface u-wind
    real(FVPRC), ALLOCATABLE :: v_srf(:,:)    !_NULL  ! Surface v-wind
    real(FVPRC), ALLOCATABLE :: sgh(:,:)      !_NULL  ! Terrain standard deviation
    real(FVPRC), ALLOCATABLE :: oro(:,:)      !_NULL  ! land fraction (1: all land; 0: all water)
    real(FVPRC), ALLOCATABLE :: ts(:,:)       !_NULL  ! skin temperature (sst) from NCEP/GFS (K) -- tile
 
!-----------------------------------------------------------------------
! Others:
!-----------------------------------------------------------------------
    real(FVPRC), ALLOCATABLE :: phis(:,:)     !_NULL  ! Surface geopotential (g*Z_surf)
    real(FVPRC), ALLOCATABLE :: omga(:,:,:)   !_NULL  ! Vertical pressure velocity (pa/s)
    real(FVPRC), ALLOCATABLE :: ua(:,:,:)     !_NULL  ! (ua, va) are mostly used as the A grid winds
    real(FVPRC), ALLOCATABLE :: va(:,:,:)     !_NULL
    real(FVPRC), ALLOCATABLE :: uc(:,:,:)     !_NULL  ! (uc, vc) are mostly used as the C grid winds
    real(FVPRC), ALLOCATABLE :: vc(:,:,:)     !_NULL

    real(FVPRC), ALLOCATABLE :: ak(:)  !_NULL
    real(FVPRC), ALLOCATABLE :: bk(:)  !_NULL

   integer :: ks

! Accumulated Mass flux arrays
    real(FVPRC), ALLOCATABLE ::  mfx(:,:,:)  !_NULL
    real(FVPRC), ALLOCATABLE ::  mfy(:,:,:)  !_NULL
! Accumulated Courant number arrays
    real(FVPRC), ALLOCATABLE ::  cx(:,:,:)  !_NULL
    real(FVPRC), ALLOCATABLE ::  cy(:,:,:)  !_NULL
...
!---- version number -----
  !character(len=128) :: version = '$Id: fv_arrays.F90,v 1.2.2.1.2.1.30.1.4.1.22.1.46.1.20.1.2.2.2.1.2.1.2.2 2017/02/21 20:43:38 bmauer Exp $'
...
contains
  ...
  subroutine allocate_fv_atmos_type(Atm, isd_in, ied_in, jsd_in, jed_in, is_in, ie_in, js_in, je_in, &
       npx_in, npy_in, npz_in, ndims_in, ncnst_in, ng_in, dummy, alloc_2d, ngrids_in)
...
!!$    Atm%npz = npz_in
    ...
    !--------------------------
    ! bma commented out lines because of but if hydrostatic when calling c_sw in dyn_core.F90
...
!$OMP parallel do default(none) shared(isd,ied,jsd,jed,npz,Atm,ncnst)
     do k=1, npz
        do j=jsd, jed
           do i=isd, ied
