...
implicit none
   ...
   logical :: bad_range
   real(FVPRC), allocatable ::  rf(:), rw(:)
   integer :: kmax=1
   !Arrays for global grid total energy, used for grid nesting
   real(FVPRC), allocatable :: te_2d_coarse(:,:)
   real(FVPRC), allocatable :: dp1_coarse(:,:,:)
...
contains
 ...
 subroutine setup_nested_grid_BCs(npx, npy, npz, cp_air, zvir, ncnst, sphum,     &
    ...
    real(FVPRC), intent(inout) :: delz(bd%isd:        ,bd%jsd:        ,1:)  ! height thickness (m)
    ...
    real(FVPRC), allocatable :: g_dat(:,:,:), pt_coarse(:,:,:), pkz_coarse(:,:,:)
!!$    real(FVPRC) :: pkz_south(isd:ied,jsd:0,  npz)
!!$    real(FVPRC) :: pkz_north(isd:ied,npy:jed,npz)
!!$    real(FVPRC) :: pkz_west (isd:0,  jsd:jed,npz)
!!$    real(FVPRC) :: pkz_east (npx:ied,jsd:jed,npz)
    integer :: i,j,k,n,p
    !integer :: isc, iec, jsc, jec, isd, ied, jsd, jed
    integer :: istart, iend
    logical :: process, do_pd
...
!! Nested grid: receive from parent grid
    if (neststruct%nested) then
       call nested_grid_BC_save(neststruct%nest_domain, &
            ...
            neststruct%delp_BC, proc_in=.true., pd_in=do_pd, debug_in=.false.)
       do n=1,ncnst
          call nested_grid_BC_save(neststruct%nest_domain, &
               neststruct%ind_h, neststruct%wt_h, 0, 0,  npx,  npy,  npz, bd, &
               neststruct%q_BC(n), proc_in=.true., pd_in=do_pd, debug_in=.false.)
       enddo
!#ifndef SW_DYNAMICS
       call nested_grid_BC_save(neststruct%nest_domain, &
            neststruct%ind_h, neststruct%wt_h, 0, 0,  npx,  npy,  npz, bd, &
       ...
       if (.not. flagstruct%hydrostatic) then
          ...
          call nested_grid_BC_save(neststruct%nest_domain, &
               neststruct%ind_h, neststruct%wt_h, 0, 0,  npx,  npy,  npz, bd, &
               neststruct%delz_BC, proc_in=.true.) !Need a negative-definite method? 
       endif
!#endif
       call nested_grid_BC_save(neststruct%nest_domain, &
...
!#endif
 ...
 subroutine setup_pt_BC(pt_BC, pkz_BC, sphum_BC, npx, npy, npz, cp_air, zvir, bd)
   ...
   integer, intent(IN) :: npx, npy, npz
   real(FVPRC), intent(IN) :: cp_air, zvir

   real(FVPRC), dimension(:,:,:), pointer :: ptBC, pkzBC, sphumBC
...
!!$
!!$   real(FVPRC), dimension(:,:,:) :: var, parent_var
...
subroutine twoway_nesting(Atm, ngrids, grids_on_this_pe, kappa, cp_air, zvir, dt_atmos)
   ...
   real(FVPRC), intent(IN) :: kappa, cp_air, zvir, dt_atmos
...
!!!not potential temperature; which may cause problems when updating if this is not the case.
 subroutine twoway_nest_update(npx, npy, npz, cp_air, zvir, ncnst, sphum,     &
    ...
    real(FVPRC), intent(inout) :: delp(bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz)  ! pressure thickness (pascal)
    real(FVPRC), intent(inout) :: q(   bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz, ncnst) ! specific humidity and constituents
    real(FVPRC), intent(inout) :: uc(bd%isd:bd%ied+1,bd%jsd:bd%jed  ,npz) ! (uc,vc) C grid winds
    real(FVPRC), intent(inout) :: vc(bd%isd:bd%ied  ,bd%jsd:bd%jed+1,npz)
    ...
    real(FVPRC), allocatable :: g_dat(:,:,:)
    real(FVPRC), allocatable :: pt_coarse(:,:,:), pkz_coarse(:,:,:)
    real(FVPRC), allocatable :: t_nest(:,:,:), ps0(:,:)
    integer :: i,j,k,n, r, s
    ...
    real(FVPRC) :: qdp(   bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz)
    real(FVPRC), allocatable :: qdp_coarse(:,:,:)
      ...
      do n=1,ncnst
         ...
         if (neststruct%parent_proc) then
            ...
            call update_coarse_grid(qdp_coarse, qdp, neststruct%nest_domain, &
                 neststruct%ind_update_h, gridstruct%dx, gridstruct%dy, gridstruct%area, &
                 isd_p, ied_p, jsd_p, jed_p, isd, ied, jsd, jed, npx, npy, npz, 0, 0, &
                 neststruct%refinement, neststruct%nestupdate, upoff, 0, neststruct%parent_proc, &
...
! dyn_aux:
    ...
    real(FVPRC), intent(inout) :: pk  (bd%is:bd%ie,bd%js:bd%je, npz+1)          ! pe**cappa
...
!#ifndef SW_DYNAMICS
   ...
   call compute_total_energy(is, ie, js, je, isd, ied, jsd, jed, npz,  &
        ...
        ua, va, teq, flagstruct%moist_phys, sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, &
        flagstruct%hydrostatic,idiag%id_te)
...
!#endif
 ...
 subroutine after_twoway_nest_update(npx, npy, npz, ng, bdt,               &
   ...
   type(fv_grid_bounds_type), intent(IN) :: bd
    ...
    real(FVPRC), intent(IN) :: kappa, cp_air, ptop
    real(FVPRC), intent(IN) :: zvir
    ...
    real(FVPRC), intent(inout) :: q(   bd%isd:bd%ied  ,bd%jsd:bd%jed  ,npz, ncnst) ! specific humidity and constituents
    real(FVPRC), intent(inout) :: delz(bd%isd:        ,bd%jsd:        ,1: )   ! delta-height (m); non-hydrostatic only
    real(FVPRC), intent(inout) ::  ze0(bd%is:         ,bd%js:         ,1:   ) ! height at edges (m); non-hydrostatic
...
!-----------------------------------------------------------------------
    ...
    real(FVPRC) :: akap, tpe, rg
    integer:: kord_tracer(ncnst), cld_amt, iq
    real(FVPRC) te_2d_coarse_after(bd%is:bd%ie,bd%js:bd%je)
...
!#ifndef SW_DYNAMICS
   ...
   call p_var(npz, is, ie, js, je, ptop, ptop_min,  &
        ...
        pe, peln,   &
        pk,   pkz, kappa, &
        q, ng, flagstruct%ncnst,  gridstruct%area_64, 0._FVPRC,  &
   ...
   call compute_total_energy(is, ie, js, je, isd, ied, jsd, jed, npz,  &
        ...
        ua, va, teq, flagstruct%moist_phys,  sphum, liq_wat, rainwat, ice_wat, snowwat, &
        graupel,flagstruct%hydrostatic,idiag%id_te)
...
!#endif
 ...
 subroutine update_remap_tq( npz, ak,  bk,  ps, delp,  t,  q,  &
  ...
  real(FVPRC),    intent(in), dimension(isd:ied,jsd:jed):: ps0
  real(FVPRC),    intent(in), dimension(isd:ied,jsd:jed):: ps
  real(FVPRC), intent(in), dimension(isd:ied,jsd:jed,npz):: delp
  real(FVPRC),    intent(inout), dimension(isd:ied,jsd:jed,npz):: t
  real(FVPRC),    intent(in), dimension(isd:ied,jsd:jed,npz,nq):: q
...
5000 continue
 ...
 subroutine update_remap_uv(npz, ak, bk, ps, u, v, kmd, ps0, &
  ...
  real(FVPRC),    intent(inout), dimension(isd:ied,jsd:jed+1,npz):: u
  real(FVPRC),    intent(inout), dimension(isd:ied+1,jsd:jed,npz):: v
!
  integer, intent(in):: kmd
  real(FVPRC),    intent(IN) :: ptop
  real(FVPRC),    intent(in):: ps0(isd:ied,jsd:jed)
  integer,  intent(in) ::  is, ie, js, je, isd, ied, jsd, jed
!
! local:
  real(FVPRC), dimension(is:ie+1,kmd+1):: pe0
  real(FVPRC), dimension(is:ie+1,npz+1):: pe1
  real(FVPRC), dimension(is:ie+1,kmd):: qt
  real(FVPRC), dimension(is:ie+1,npz):: qn1
  integer i,j,k
