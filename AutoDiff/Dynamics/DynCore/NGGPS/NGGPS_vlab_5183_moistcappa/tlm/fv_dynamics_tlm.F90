...
& rvgas, cp_vapor
  USE DYN_CORE_MOD_DIFF, ONLY : dyn_core, dyn_core_tlm, del2_cubed, &
& del2_cubed_tlm, init_ijk_mem
  USE FV_MAPZ_MOD_DIFF, ONLY : compute_total_energy, &
& compute_total_energy_tlm, lagrangian_to_eulerian, &
& lagrangian_to_eulerian_tlm, moist_cv, moist_cv_tlm, moist_cp, &
& moist_cp_tlm
  USE FV_TRACER2D_MOD_DIFF, ONLY : tracer_2d, tracer_2d_tlm, &
& tracer_2d_1l, tracer_2d_1l_tlm, tracer_2d_nested, tracer_2d_nested_tlm
  USE FV_GRID_UTILS_MOD_DIFF, ONLY : cubed_to_latlon, c2l_ord2, &
& c2l_ord2_tlm, g_sum, g_sum_tlm
...
&   , peln_tl, pkz, pkz_tl, phis, q_con, q_con_tl, omga, omga_tl, ua, &
&   ua_tl, va, va_tl, uc, uc_tl, vc, vc_tl, ak, bk, mfx, mfx_tl, mfy, &
&   mfy_tl, cx, cx_tl, cy, cy_tl, ze0, hybrid_z, gridstruct, flagstruct&
...
!$OMP      rainwat,ice_wat,snowwat,graupel) private(cvm)
      DO k=1,npz
        DO j=js,je
          CALL MOIST_CP_TLM(is, ie, isd, ied, jsd, jed, npz, j, k, nwat&
&                     , sphum, liq_wat, rainwat, ice_wat, snowwat, &
&                     graupel, q, q_tl, q_con(is:ie, j, k), q_con_tl(is:&
&                     ie, j, k), cvm)
          DO i=is,ie
...
!$OMP                          private(cvm)
      DO k=1,npz
        IF (flagstruct%moist_phys) THEN
          DO j=js,je
            CALL MOIST_CV_TLM(is, ie, isd, ied, jsd, jed, npz, j, k, &
&                       nwat, sphum, liq_wat, rainwat, ice_wat, snowwat&
&                       , graupel, q, q_tl, q_con(is:ie, j, k), q_con_tl&
&                       (is:ie, j, k), cvm, cvm_tl)
            DO i=is,ie
              dp1_tl(i, j, k) = zvir*q_tl(i, j, k, sphum)
              dp1(i, j, k) = zvir*q(i, j, k, sphum)
              cappa_tl(i, j, k) = -(rdgas*(cvm_tl(i)*(1.+dp1(i, j, k))-&
&               cvm(i)*dp1_tl(i, j, k))/(1.+dp1(i, j, k))**2/(rdgas+cvm(&
&               i)/(1.+dp1(i, j, k)))**2)
              cappa(i, j, k) = rdgas/(rdgas+cvm(i)/(1.+dp1(i, j, k)))
              arg1_tl = (rdg*((delp_tl(i, j, k)*pt(i, j, k)+delp(i, j, k&
&               )*pt_tl(i, j, k))*(1.+dp1(i, j, k))*(1.-q_con(i, j, k))+&
&               delp(i, j, k)*pt(i, j, k)*(dp1_tl(i, j, k)*(1.-q_con(i, &
&               j, k))-(1.+dp1(i, j, k))*q_con_tl(i, j, k)))*delz(i, j, &
&               k)-rdg*delp(i, j, k)*pt(i, j, k)*(1.+dp1(i, j, k))*(1.-&
&               q_con(i, j, k))*delz_tl(i, j, k))/delz(i, j, k)**2
              arg1 = rdg*delp(i, j, k)*pt(i, j, k)*(1.+dp1(i, j, k))*(1.&
&               -q_con(i, j, k))/delz(i, j, k)
...
&             , k)*dp1_tl(i, j, k))*(1.-q_con(i, j, k))-pt(i, j, k)*(1.+&
&             dp1(i, j, k))*q_con_tl(i, j, k))*pkz(i, j, k)-pt(i, j, k)*&
&             (1.+dp1(i, j, k))*(1.-q_con(i, j, k))*pkz_tl(i, j, k))/pkz&
&             (i, j, k)**2
...
&                                domain, complete=.true.)
      CALL START_GROUP_HALO_UPDATE_TLM(i_pack(12), cappa, cappa_tl, &
&                                domain, complete=.true.)
...
&                 , cp_air, akap, cappa, cappa_tl, grav, hydrostatic, u&
&                 , u_tl, v, v_tl, w, w_tl, delz, delz_tl, pt, pt_tl, q&
&                 , q_tl, delp, delp_tl, pe, pe_tl, pk, pk_tl, phis, ws&
&                 , ws_tl, omga, omga_tl, ptop, pfull, ua, ua_tl, va, &
&                 va_tl, uc, uc_tl, vc, vc_tl, mfx, mfx_tl, mfy, mfy_tl&
&                 , cx, cx_tl, cy, cy_tl, pkz, pkz_tl, peln, peln_tl, &
&                 q_con, q_con_tl, ak, bk, dpx, dpx_tl, ks, gridstruct, &
&                 flagstruct, flagstructp, neststruct, idiag, bd, domain&
&                 , n_map .EQ. 1, i_pack, last_step, gz, gz_tl, pkc, &
&                 pkc_tl, ptc, ptc_tl, crx, crx_tl, xfx, xfx_tl, cry, &
&                 cry_tl, yfx, yfx_tl, divgd, divgd_tl, delpc, delpc_tl&
&                 , ut, ut_tl, vt, vt_tl, zh, zh_tl, pk3, pk3_tl, du, &
...
&                                 sphum, q_con, q_con_tl, u, u_tl, v, &
&                                 v_tl, w, w_tl, delz, delz_tl, pt, &
&                                 pt_tl, q, q_tl, phis, zvir, cp_air, &
&                                 akap, cappa, cappa_tl, kord_mt, &
&                                 kord_wz, kord_tracer, kord_tm, peln, &
&                                 peln_tl, te_2d, te_2d_tl, ng, ua, &
&                                 ua_tl, va, omga, omga_tl, dp1, dp1_tl&
&                                 , ws, ws_tl, fill, reproduce_sum, &
&                                 idiag%id_mdt .GT. 0, dtdt_m, ptop, ak&
&                                 , bk, pfull, flagstruct, gridstruct, &
&                                 domain, flagstruct%do_sat_adj, &
&                                 hydrostatic, hybrid_z, do_omega, &
&                                 flagstruct%adiabatic, &
&                                 do_adiabatic_init, mfx, mfy, &
&                                 flagstruct%remap_option, kord_mt_pert&
&                                 , kord_wz_pert, kord_tracer_pert, &
&                                 kord_tm_pert)
        CALL TIMING_OFF('Remapping')
        IF (neststruct%nested .AND. (.NOT.last_step)) CALL &
&         NESTED_GRID_BC_APPLY_INTT_TLM(cappa, cappa_tl, 0, 0, npx, npy&
...
&                             kord_tracer_pert, kord_tm_pert)
        CALL TIMING_OFF('Remapping')
        IF (neststruct%nested .AND. (.NOT.last_step)) CALL &
&         NESTED_GRID_BC_APPLY_INTT(cappa, 0, 0, npx, npy, npz, bd, REAL&
&                             (n_map + 1), REAL(k_split), neststruct%&
&                             cappa_bc, neststruct%nestbctype)
        IF (last_step) THEN
          IF (.NOT.hydrostatic) THEN
!$OMP parallel do default(none) shared(is,ie,js,je,npz,omga,delp,delz,w)
            DO k=1,npz
