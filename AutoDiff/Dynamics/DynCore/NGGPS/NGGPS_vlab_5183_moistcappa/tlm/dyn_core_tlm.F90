...
& start_group_halo_update_tlm, complete_group_halo_update
  ...
  USE SW_CORE_MOD_DIFF, ONLY : c_sw, c_sw_tlm, d_sw, d_sw_tlm
  USE A2B_EDGE_MOD_DIFF, ONLY : a2b_ord2, a2b_ord2_tlm, a2b_ord4, &
& a2b_ord4_tlm
  USE NH_CORE_MOD_DIFF, ONLY : riem_solver3, riem_solver3_tlm
  USE NH_UTILS_MOD_DIFF, ONLY : riem_solver_c, riem_solver_c_tlm, &
& update_dz_c, update_dz_c_tlm, update_dz_d, update_dz_d_tlm, &
& nest_halo_nh, nest_halo_nh_tlm
  USE TP_CORE_MOD_DIFF, ONLY : copy_corners, copy_corners_tlm
...
& nested_grid_bc_apply_intt, nested_grid_bc_apply_intt_tlm
...
!   variations   of useful results: pk3 xfx ws q_con peln q gz
!                du u dv v w delp ua uc ptc mfx delz mfy omga ut
!                divgd pkc delpc va vc yfx pkz pe vt pk zh pt cx
!                cy dpx crx cry
...
&   zvir, cp, akap, cappa, cappa_tl, grav, hydrostatic, u, u_tl, v, v_tl&
&   , w, w_tl, delz, delz_tl, pt, pt_tl, q, q_tl, delp, delp_tl, pe, &
&   pe_tl, pk, pk_tl, phis, ws, ws_tl, omga, omga_tl, ptop, pfull, ua, &
&   ua_tl, va, va_tl, uc, uc_tl, vc, vc_tl, mfx, mfx_tl, mfy, mfy_tl, cx&
&   , cx_tl, cy, cy_tl, pkz, pkz_tl, peln, peln_tl, q_con, q_con_tl, ak&
&   , bk, dpx, dpx_tl, ks, gridstruct, flagstruct, flagstructp, &
&   neststruct, idiag, bd, domain, init_step, i_pack, end_step, gz, &
&   gz_tl, pkc, pkc_tl, ptc, ptc_tl, crx, crx_tl, xfx, xfx_tl, cry, &
&   cry_tl, yfx, yfx_tl, divgd, divgd_tl, delpc, delpc_tl, ut, ut_tl, vt&
&   , vt_tl, zh, zh_tl, pk3, pk3_tl, du, du_tl, dv, dv_tl, time_total)
    IMPLICIT NONE
! end init_step
...
! moist kappa
    ...
    REAL, INTENT(INOUT) :: cappa_tl(bd%isd:bd%ied, bd%jsd:bd%jed, npz)
! temperature (K)
...
&   ua_tl, va_tl
    ...
    REAL, INTENT(INOUT) :: q_con_tl(bd%isd:bd%ied, bd%jsd:bd%jed, npz)
...
&                pkc, pkc_tl, gz, gz_tl, phis, ptc, ptc_tl, q_con, &
&                q_con_tl, pkz, pkz_tl, npz, akap, .true., gridstruct%&
...
&                        cappa, cappa_tl, cp, ptop, phis, omga, omga_tl&
&                        , ptc, ptc_tl, q_con, q_con_tl, delpc, delpc_tl&
&                        , gz, gz_tl, pkc, pkc_tl, ws3, ws3_tl, &
...
&                         delz, delz_tl, ptc, ptc_tl, phis, q_con, &
&                         q_con_tl, cappa, cappa_tl, pkc, pkc_tl, gz, &
&                         gz_tl, pk3, pk3_tl, npx, npy, npz, gridstruct%&
...
&               , jsd:jed, 1), q_con_tl(isd:ied, jsd:jed, 1), z_rat(isd:&
&               ied, jsd:jed), z_rat_tl(isd:ied, jsd:jed), kgb, heat_s, &
&               heat_s_tl, dpx, dpx_tl, zvir, sphum, nq, q, q_tl, k, npz&
...
&                                complete=.true.)
      CALL START_GROUP_HALO_UPDATE_TLM(i_pack(11), q_con, q_con_tl, &
...
&                                    neststruct%pt_bc, bctype=neststruct&
&                                    %nestbctype)
        CALL NESTED_GRID_BC_APPLY_INTT_TLM(q_con, q_con_tl, 0, 0, npx, &
&                                    npy, npz, bd, split_timestep_bc + 1&
&                                    , REAL(n_split*flagstruct%k_split)&
&                                    , neststruct%q_con_bc, bctype=&
&                                    neststruct%nestbctype)
      END IF
! end hydro check
      IF (hydrostatic) THEN
        CALL GEOPK_TLM(ptop, pe, pe_tl, peln, peln_tl, delp, delp_tl, &
&                pkc, pkc_tl, gz, gz_tl, phis, pt, pt_tl, q_con, &
&                q_con_tl, pkz, pkz_tl, npz, akap, .false., gridstruct%&
...
&                       cappa_tl, cp, ptop, zs, q_con, q_con_tl, w, w_tl&
&                       , delz, delz_tl, pt, pt_tl, delp, delp_tl, zh, &
&                       zh_tl, pe, pe_tl, pkc, pkc_tl, pk3, pk3_tl, pk, &
&                       pk_tl, peln, peln_tl, ws, ws_tl, flagstruct%&
&                       scale_z, flagstruct%p_fac, flagstruct%a_imp, &
&                       flagstruct%use_logp, remap_step, beta .LT. -0.1)
        CALL TIMING_OFF('Riem_Solver')
        CALL TIMING_ON('COMM_TOTAL')
...
&                         delz, delz_tl, pt, pt_tl, phis, q_con, &
&                         q_con_tl, cappa, cappa_tl, pkc, pkc_tl, gz, &
&                         gz_tl, pk3, pk3_tl, npx, npy, npz, gridstruct%&
...
&                 + rdg*delp(i, j, k)*pt_tl(i, j, k)/delz(i, j, k)
                ...
                arg2_tl = (cappa_tl(i, j, k)*(1.-cappa(i, j, k))+cappa(i&
&                 , j, k)*cappa_tl(i, j, k))*LOG(arg1)/(1.-cappa(i, j, k&
&                 ))**2 + cappa(i, j, k)*arg1_tl/((1.-cappa(i, j, k))*&
&                 arg1)
...
&               rdg*delp(i, j, k)*pt_tl(i, j, k)/delz(i, j, k)
              ...
              arg2_tl = (cappa_tl(i, j, k)*(1.-cappa(i, j, k))+cappa(i, &
&               j, k)*cappa_tl(i, j, k))*LOG(arg1)/(1.-cappa(i, j, k))**&
&               2 + cappa(i, j, k)*arg1_tl/((1.-cappa(i, j, k))*arg1)
              arg2 = cappa(i, j, k)/(1.-cappa(i, j, k))*LOG(arg1)
              pkz_tl(i, j, k) = arg2_tl*EXP(arg2)
              pkz(i, j, k) = EXP(arg2)
...
&                                split_timestep_bc + 1, REAL(n_split*&
&                                flagstruct%k_split), neststruct%&
&                                q_con_bc, neststruct%nestbctype)
      END IF
! end hydro check
      IF (hydrostatic) THEN
...
&   , pk_tl, gz, gz_tl, hs, pt, pt_tl, q_con, q_con_tl, pkz, pkz_tl, km&
...
! Local:
    ...
    REAL :: peg_tl(bd%isd:bd%ied, km+1)
    REAL :: pkg(bd%isd:bd%ied, km+1)
    REAL :: pkg_tl(bd%isd:bd%ied, km+1)
...
! Top down
      DO k=2,km+1
        DO i=ifirst,ilast
          ...
          peg_tl(i, k) = peg_tl(i, k-1) + delp_tl(i, j, k-1)*(1.-q_con(i&
&           , j, k-1)) - delp(i, j, k-1)*q_con_tl(i, j, k-1)
          peg(i, k) = peg(i, k-1) + delp(i, j, k-1)*(1.-q_con(i, j, k-1)&
&           )
          arg1_tl = akap*peg_tl(i, k)/peg(i, k)
          arg1 = akap*LOG(peg(i, k))
          pkg_tl(i, k) = arg1_tl*EXP(arg1)
          pkg(i, k) = EXP(arg1)
        END DO
        IF (j .GT. js - 2 .AND. j .LT. je + 2) THEN
          IF (ifirst .LT. is - 1) THEN
...
! Bottom up
      DO k=km,1,-1
        DO i=ifirst,ilast
          g1d_tl(i) = g1d_tl(i) + cp_air*(pt_tl(i, j, k)*(pkg(i, k+1)-&
&           pkg(i, k))+pt(i, j, k)*(pkg_tl(i, k+1)-pkg_tl(i, k)))
