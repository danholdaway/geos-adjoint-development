...
!***********************************************************************
 ...
 use fv_arrays_mod, only: fv_grid_type, fv_grid_bounds_type, fv_flags_type, fvprc
...
! scheme 2.1: perturbation form
  ...
  contains
   ...
   subroutine c_sw(delpc, delp, ptc, pt, u,v, w, uc,vc, ua,va, wc,  &
      ...
      real, intent(INOUT), dimension(bd%isd:bd%ied,  bd%jsd:bd%jed  ) :: w
...
!----------------
! Xdir:
      if (flagstruct%grid_type < 3 .and. .not. nested) call fill2_4corners(delp, pt, 1, bd, npx, npy, &
...
! Ydir:
      if (flagstruct%grid_type < 3 .and. .not. nested) call fill2_4corners(delp, pt, 2, bd, npx, npy, &
                                                            sw_corner, se_corner, ne_corner, nw_corner)
      if ( hydrostatic ) then
           do j=js-1,jep1+1
              do i=is-1,iep1      
...
!     d_sw :: D-Grid Shallow Water Routine
   ...
   subroutine d_sw(delpc, delp,  ptc,   pt, u,  v, w, uc,vc, &
      ...
      integer, intent(IN):: hord_tr_pert, hord_mt_pert, hord_vt_pert, hord_tm_pert, hord_dp_pert, &
                            nord_pert, nord_v_pert, nord_w_pert, nord_t_pert
      logical, intent(IN):: split_damp
      real   , intent(IN):: dddmp_pert, d2_bg_pert, d4_bg_pert, damp_v_pert, damp_w_pert, &
                            damp_t_pert
      ...
      real, intent(INOUT), dimension(bd%isd:bd%ied,  bd%jsd:bd%jed):: w
      real, intent(INOUT), dimension(bd%isd:bd%ied,  bd%jsd:bd%jed):: q_con
      real, intent(INOUT), dimension(bd%isd:bd%ied  ,bd%jsd:bd%jed+1):: u, vc
      real, intent(INOUT), dimension(bd%isd:bd%ied+1,bd%jsd:bd%jed  ):: v, uc
...
!                   heat_source(i,j) = -d_con*dw(i,j)*(w(i,j)+0.5*dw(i,j))
            ...
            call fv_tp_2d(w, crx_adv,cry_adv, npx, npy, hord_vt_pert, gx, gy, xfx_adv, yfx_adv, &
                          gridstruct, bd, ra_x, ra_y, mfx=fx, mfy=fy)
            !UNCOMcall fv_tp_2d(w, crx_adv,cry_adv, npx, npy, hord_vt, gx, gy, xfx_adv, yfx_adv, &
           ...
           if (hord_dp == hord_dp_pert  .and. .not. split_damp) then
           call fv_tp_2d_fb(q_con, crx_adv,cry_adv, npx, npy, hord_dp, gx, gy,  &
                xfx_adv,yfx_adv, gridstruct, bd, ra_x, ra_y, mfx=fx, mfy=fy, mass=delp, nord=nord_t, damp_c=damp_t)
           else
           call fv_tp_2d(q_con, crx_adv,cry_adv, npx, npy, hord_dp_pert, gx, gy,  &
                xfx_adv,yfx_adv, gridstruct, bd, ra_x, ra_y, mfx=fx, mfy=fy, mass=delp, nord=nord_t_pert, damp_c=damp_t_pert)
           !UNCOMcall fv_tp_2d(q_con, crx_adv,cry_adv, npx, npy, hord_dp, gx, gy,  &
           !UNCOM    xfx_adv,yfx_adv, gridstruct, bd, ra_x, ra_y, mfx=fx, mfy=fy, mass=delp, nord=nord_t, damp_c=damp_t)
...
!    endif
     ...
     if ( inline_q ) then
        ...
        do iq=1,nq
           ...
           call fv_tp_2d(q(isd:ied,jsd:jed,k,iq), crx_adv,cry_adv, npx, npy, hord_tr_pert, gx, gy,  &
                         xfx_adv,yfx_adv, gridstruct, bd, ra_x, ra_y,     &
                         mfx=fx, mfy=fy, mass=delp, nord=nord_t_pert, damp_c=damp_t_pert)
           !UNCOMcall fv_tp_2d(q(isd:ied,jsd:jed,k,iq), crx_adv,cry_adv, npx, npy, hord_tr, gx, gy,  &
           !UNCOM              xfx_adv,yfx_adv, gridstruct, bd, ra_x, ra_y,     &
           !UNCOM              mfx=fx, mfy=fy, mass=delp, nord=nord_t, damp_c=damp_t)
           endif
           do j=js,je
...
! 2-pt extrapolation from both sides:
      ...
      call ytp_v_fb(is,ie,js,je,isd,ied,jsd,jed, vb, u, v, ub, hord_mt, gridstruct%dy, gridstruct%rdy, &
                 npx, npy, flagstruct%grid_type, nested)
      else
      call ytp_v(is,ie,js,je,isd,ied,jsd,jed, vb, u, v, ub, hord_mt_pert, gridstruct%dy, gridstruct%rdy, &
                 npx, npy, flagstruct%grid_type, nested)
      !UNCOMcall ytp_v(is,ie,js,je,isd,ied,jsd,jed, vb, u, v, ub, hord_mt, gridstruct%dy, gridstruct%rdy, &
      !UNCOM           npx, npy, flagstruct%grid_type, nested)
      endif
...
! 2-pt extrapolation from both sides:
      ...
      if (hord_mt == hord_mt_pert) then
      call xtp_u_fb(is,ie,js,je, isd,ied,jsd,jed, ub, u, v, vb, hord_mt, gridstruct%dx, gridstruct%rdx, &
                 npx, npy, flagstruct%grid_type, nested)
      else
      call xtp_u(is,ie,js,je, isd,ied,jsd,jed, ub, u, v, vb, hord_mt_pert, gridstruct%dx, gridstruct%rdx, &
                 npx, npy, flagstruct%grid_type, nested)
      !UNCOMcall xtp_u(is,ie,js,je, isd,ied,jsd,jed, ub, u, v, vb, hord_mt, gridstruct%dx, gridstruct%rdx, &
      !UNCOM           npx, npy, flagstruct%grid_type, nested)
      endif
...
!-----------------------------------------
    if (.not. nested) then
      ...
      if ( se_corner ) then
           !i = npx
           ke(npx,1) = dt6*( (ut(npx,1) + ut(npx,  0)) * u(npx-1,1) + &
                             (vt(npx,1) + vt(npx-1,1)) * v(npx,  1) + &
                             (ut(npx,1) - vt(npx-1,1)) * u(npx,  1) )
      endif
      if ( ne_corner ) then
           !i = npx;      j = npy
           ke(npx,npy) = dt6*( (ut(npx,npy  ) + ut(npx,npy-1)) * u(npx-1,npy) +  &
                               (vt(npx,npy  ) + vt(npx-1,npy)) * v(npx,npy-1) +  &
                               (ut(npx,npy-1) + vt(npx-1,npy)) * u(npx,npy  )  )
      endif
      if ( nw_corner ) then
           !j = npy
           ke(1,npy) = dt6*( (ut(1,  npy) + ut(1,npy-1)) * u(1,npy  ) +  &
                             (vt(1,  npy) + vt(0,  npy)) * v(1,npy-1) +  &
                             (ut(1,npy-1) - vt(1,  npy)) * u(0,npy  )  )
      endif
    end if
...
!UNCOMvt_tj = vt
        ...
        call del6_vt_flux(nord_v_pert, npx, npy, damp4, wk, vort, ut, vt, gridstruct, bd)
   endif
...
!!! TO DO: separate versions for nested and for cubed-sphere
           ...
           if ( (ie+1)==npx  .and. .not. nested) then
              ...
              call pert_ppm(1, u(npx-2:npx-2,j), bl(npx-2:npx-2), br(npx-2:npx-2), -1)
           endif
...
!--------------
      if( js==1 .and. .not. nested) then
         ...
         j=2
         call pert_ppm(ie-is+2, v(is:ie+1,j), bl(is:ie+1,j), br(is:ie+1,j), -1)
      endif
...
! This routine fill the 4 corners of the scalar fileds only as needed by c_core
 ...
 subroutine xtp_u_fb(is,ie,js,je,isd,ied,jsd,jed,c, u, v, flux, iord, dx, rdx, npx, npy, grid_type, nested)
...
!DEC$ VECTOR ALWAYS
 ...
 subroutine ytp_v_fb(is,ie,js,je,isd,ied,jsd,jed, c, u, v, flux, jord, dy, rdy, npx, npy, grid_type, nested)
 integer, intent(in):: is,ie,js,je, isd,ied,jsd,jed
 integer, intent(IN):: jord
 real, INTENT(IN)  ::   u(isd:ied,jsd:jed+1)
 real, INTENT(IN)  ::   v(isd:ied+1,jsd:jed)
...
!InOut Arguments
 ...
 real, intent(in), dimension(bd%isd:bd%ied  ,bd%jsd:bd%jed+1) :: u
 real, intent(in), dimension(bd%isd:bd%ied+1,bd%jsd:bd%jed  ) :: v
 real, intent(inout), dimension(bd%isd:bd%ied,bd%jsd:bd%jed)  :: wk    !Intent is really in


 real, intent(inout), dimension(bd%isd:bd%ied,bd%jsd:bd%jed)     :: vort
 real, intent(inout), dimension(bd%isd:bd%ied,bd%jsd:bd%jed)     :: delpc, ptc
 real, intent(inout), dimension(bd%isd:bd%ied+1,bd%jsd:bd%jed+1) :: ke
 real, intent(inout), dimension(bd%isd:bd%ied  ,bd%jsd:bd%jed+1) :: vc
 real, intent(inout), dimension(bd%isd:bd%ied+1,bd%jsd:bd%jed  ) :: uc
 real, intent(inout), dimension(bd%isd:bd%ied+1,bd%jsd:bd%jed+1) :: divg_d
...
! Stretched grid with variable damping ~ area
 ...
 subroutine compute_divergence_damping_fb( nord,d2_bg,d4_bg,dddmp,dt, &
                                vort,ptc,delpc,ke,u,v,uc,vc,ua,va,divg_d,wk, &
                                gridstruct, flagstruct, bd)
...
! The following is an approxi form of Smagorinsky diffusion
          ...
          call smag_corner_fb(abs(dt), u, v, ua, va, vort, bd, npx, npy, gridstruct, ng)
      endif
     endif
...
! Stretched grid with variable damping ~ area
 ...
 end subroutine compute_divergence_damping_fb
