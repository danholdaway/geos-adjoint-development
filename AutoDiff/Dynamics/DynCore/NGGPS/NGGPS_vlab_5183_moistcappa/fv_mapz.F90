...
! as input; output: temperature
  real, intent(inout), dimension(isd:ied,jsd:jed,km)::delz
  real, intent(inout), dimension(isd:ied,jsd:jed,km)::q_con, cappa
...
!----------------------------------
         ...
         !UNCOMcall map_scalar(km,  peln(is:ie,1:km+1,j),      gz,   &
         !UNCOM                km,  pn2,           pt,              &
...
!----------------
      if( nq > 5 ) then
           ...
           call mapn_tracer_fb(nq, km, pe1, pe2, q, dp2, kord_tr, j,     &
                            is, ie, isd, ied, jsd, jed, 0., fill)
           else
           call mapn_tracer(nq, km, pe1, pe2, q, dp2, kord_tr_pert, j,     &
                            is, ie, isd, ied, jsd, jed, 0., fill)
           !UNCOMcall mapn_tracer(nq, km, pe1, pe2, q, dp2, kord_tr, j,     &
           !UNCOM                 is, ie, isd, ied, jsd, jed, 0., fill)
           endif
      elseif ( nq > 0 ) then
! Remap one tracer at a time
         do iq=1,nq
             ...
             call map1_q2(km, pe1, q(isd:ied,jsd:jed,1:km,iq),     &
                          km, pe2, q2, dp2,             &
                          is, ie, 0, kord_tr_pert(iq), j, isd, ied, jsd, jed, 0.)
             !UNCOMcall map1_q2(km, pe1, q(isd:ied,jsd:jed,1:km,iq),     &
             !UNCOM             km, pe2, q2, dp2,             &
             !UNCOM             is, ie, 0, kord_tr(iq), j, isd, ied, jsd, jed, 0.)
            endif
...
! Remap vertical wind:
        ...
        !UNCOMcall map1_ppm (km,   pe1,      ws(is:ie,j),   &
        !UNCOM               km,   pe2,  w,              &
...
!------
      ...
      call map1_ppm_fb( km, pe0(is:ie,:),        gz,   &
                     km, pe3(is:ie,:),   u,               &
      ...
      !UNCOMcall map1_ppm( km, pe0(is:ie,:),        gz,   &
      !UNCOM               km, pe3(is:ie,:),   u,               &
      !UNCOM               is, ie, j, isd, ied, jsd, jedp1, -1, kord_mt)
      endif
      if (present(mfy)) then
         call map1_ppm( km, pe0(is:ie,:),       gz,  &
                        km, pe3(is:ie,:), mfy,       &
                        is, ie, j, is, ie, js, jep1, -1, kord_mt)
      endif
...
!------
       ...
       !UNCOMcall map1_ppm (km, pe0,     gz,    &
       !UNCOM               km, pe3,  v, is, ie+1,    &
       !UNCOM               j, isd, iedp1, jsd, jed, -1, kord_mt)
       endif
       if (present(mfx)) then
          call map1_ppm (km, pe0,       gz,         &
                         km, pe3, mfx, is, iep1,    &
                         j, is, iep1, js, je, -1, kord_mt)
       endif
   endif ! (j < je+1)
...
!$OMP do
           do k=kmp,km
              ...
              call fv_sat_adj(abs(mdt), r_vir, is, ie, js, je, ng, hydrostatic, fast_mp_consv, &
                             te(isd:ied,jsd:jed,k), q(isd:ied,jsd:jed,k,sphum), q(isd:ied,jsd:jed,k,liq_wat),   &
                             q(isd:ied,jsd:jed,k,ice_wat), q(isd:ied,jsd:jed,k,rainwat),    &
                             q(isd:ied,jsd:jed,k,snowwat), q(isd:ied,jsd:jed,k,graupel),    &
                      dpln, delz(isd:ied,jsd:jed,k), pt(isd:ied,jsd:jed,k), delp(isd:ied,jsd:jed,k), q_con(isd:ied,jsd:jed,k), &
   cappa(isd:ied,jsd:jed,k), gridstruct%area_64, dtdt(is:ie,js:je,k), out_dt, last_step, cld_amt>0, q(isd:ied,jsd:jed,k,cld_amt))
              if ( .not. hydrostatic  ) then
                 do j=js,je
                    do i=is,ie
...
! !LOCAL VARIABLES:
      ...
      do iq=1,nq
         do k=1,km
            ...
            enddo
         enddo
         call scalar_profile( qs, q4(1:4,i1:i2,1:km,iq), dp1, km, i1, i2, 0, kord(iq), q_min )
      enddo
...
! Compute vertical subgrid distribution
   if ( kord >7 ) then
        call  cs_profile_fb( qs, q4, dp1, km, i1, i2, iv, kord )
   !else
   !     call ppm_profile( q4, dp1, km, i1, i2, iv, kord )
   endif
...
555   continue
 ...
 subroutine mapn_tracer_fb(nq, km, pe1, pe2, q1, dp2, kord, j,     &
                        i1, i2, isd, ied, jsd, jed, q_min, fill)
! !INPUT PARAMETERS:
      integer, intent(in):: km                ! vertical dimension
      integer, intent(in):: j, nq, i1, i2
...
! !LOCAL VARIABLES:
      ...
      do iq=1,nq
         do k=1,km
            ...
            enddo
         enddo
         call scalar_profile_fb( qs, q4(1:4,i1:i2,1:km,iq), dp1, km, i1, i2, 0, kord(iq), q_min )
      enddo
