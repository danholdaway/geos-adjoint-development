...
& hlf, cp_air, cp_vapor
  ...
  USE FV_GRID_UTILS_MOD_DIFF, ONLY : g_sum, g_sum_tlm, ptop_min
...
& moist_cv_tlm, moist_cp_tlm, map1_q2_tlm

CONTAINS
!  Differentiation of lagrangian_to_eulerian in forward (tangent) mode:
!   variations   of useful results: cappa q_con peln q u v w delp
!                ua delz omga te0_2d pkz pe pk ps pt te
...
&   , js, je, isd, ied, jsd, jed, nq, nwat, sphum, q_con, q_con_tl, u, &
&   u_tl, v, v_tl, w, w_tl, delz, delz_tl, pt, pt_tl, q, q_tl, hs, r_vir&
&   , cp, akap, cappa, cappa_tl, kord_mt, kord_wz, kord_tr, kord_tm, &
&   peln, peln_tl, te0_2d, te0_2d_tl, ng, ua, ua_tl, va, omga, omga_tl, &
&   te, te_tl, ws, ws_tl, fill, reproduce_sum, out_dt, dtdt, ptop, ak, &
...
! as input; output: temperature
    ...
    REAL, DIMENSION(isd:ied, jsd:jed, km), INTENT(INOUT) :: q_con_tl, &
&   cappa_tl
...
&   pn2_tl, phis_tl
    ...
    REAL, DIMENSION(is:ie) :: gz_tl, cvm_tl, qv_tl
              ...
              IF (nwat .EQ. 2) THEN
                DO i=is,ie
                  ...
                  IF (0. .LT. q(i, j, k, liq_wat)) THEN
                    q_con_tl(i, j, k) = q_tl(i, j, k, liq_wat)
                    q_con(i, j, k) = q(i, j, k, liq_wat)
                  ELSE
                    q_con_tl(i, j, k) = 0.0
                    q_con(i, j, k) = 0.
                  END IF
                  cvm_tl(i) = cv_vap*qv_tl(i) - cv_air*qv_tl(i)
                  cvm(i) = (1.-qv(i))*cv_air + qv(i)*cv_vap
                  cappa_tl(i, j, k) = -(rdgas*(cvm_tl(i)*(1.+r_vir*qv(i)&
&                   )-cvm(i)*r_vir*qv_tl(i))/(1.+r_vir*qv(i))**2/(rdgas+&
&                   cvm(i)/(1.+r_vir*qv(i)))**2)
                  cappa(i, j, k) = rdgas/(rdgas+cvm(i)/(1.+r_vir*qv(i)))
                  arg1_tl = (rrg*delp_tl(i, j, k)*delz(i, j, k)-rrg*delp&
&                   (i, j, k)*delz_tl(i, j, k))*pt(i, j, k)/delz(i, j, k&
&                   )**2 + rrg*delp(i, j, k)*pt_tl(i, j, k)/delz(i, j, k&
&                   )
                  arg1 = rrg*delp(i, j, k)/delz(i, j, k)*pt(i, j, k)
                  arg2_tl = (cappa_tl(i, j, k)*(1.-cappa(i, j, k))+cappa&
&                   (i, j, k)*cappa_tl(i, j, k))*LOG(arg1)/(1.-cappa(i, &
&                   j, k))**2 + cappa(i, j, k)*arg1_tl/((1.-cappa(i, j, &
&                   k))*arg1)
                  arg2 = cappa(i, j, k)/(1.-cappa(i, j, k))*LOG(arg1)
                  pt_tl(i, j, k) = pt_tl(i, j, k)*EXP(arg2) + pt(i, j, k&
&                   )*arg2_tl*EXP(arg2)
                  pt(i, j, k) = pt(i, j, k)*EXP(arg2)
                END DO
              ELSE
                CALL MOIST_CV_TLM(is, ie, isd, ied, jsd, jed, km, j, k, &
&                           nwat, sphum, liq_wat, rainwat, ice_wat, &
&                           snowwat, graupel, q, q_tl, gz, gz_tl, cvm, &
&                           cvm_tl)
                DO i=is,ie
                  q_con_tl(i, j, k) = gz_tl(i)
                  q_con(i, j, k) = gz(i)
                  cappa_tl(i, j, k) = -(rdgas*(cvm_tl(i)*(1.+r_vir*q(i, &
&                   j, k, sphum))-cvm(i)*r_vir*q_tl(i, j, k, sphum))/(1.&
&                   +r_vir*q(i, j, k, sphum))**2/(rdgas+cvm(i)/(1.+r_vir&
&                   *q(i, j, k, sphum)))**2)
...
&                            is:ie, 1:km+1, j), gz, gz_tl, km, pn2, &
&                            pn2_tl, pt, pt_tl, is, ie, j, isd, ied, jsd&
&                            , jed, 1, abs_kord_tm, t_min)
          ELSE
            CALL MAP_SCALAR_TLM(km, peln(is:ie, 1:km+1, j), peln_tl(is:&
&                         ie, 1:km+1, j), gz, gz_tl, km, pn2, pn2_tl, pt&
...
&                          jed, 1, abs_kord_tm)
          ELSE
            CALL MAP1_PPM_TLM(km, pe1, pe1_tl, gz, gz_tl, km, pe2, &
&                       pe2_tl, pt, pt_tl, is, ie, j, isd, ied, jsd, jed&
&                       , 1, abs_kord_tm_pert)
...
&                   , km, pe3(is:ie, :), pe3_tl(is:ie, :), u, u_tl, is, &
...
&                   cvm(i)*pt(i, j, k)/((1.+r_vir*q(i, j, k, sphum))*(1.&
&                   -gz(i)))+0.5*(phis(i, k)+phis(i, k+1)+w(i, j, k)**2+&
&                   0.5*gridstruct%rsin2(i, j)*(u(i, j, k)**2+u(i, j+1, &
&                   k)**2+v(i, j, k)**2+v(i+1, j, k)**2-(u(i, j, k)+u(i&
&                   , j+1, k))*(v(i, j, k)+v(i+1, j, k))*gridstruct%&
&                   cosa_s(i, j)))) + delp(i, j, k)*(((cvm_tl(i)*pt(i, j&
&                   , k)+cvm(i)*pt_tl(i, j, k))*(1.+r_vir*q(i, j, k, &
&                   sphum))*(1.-gz(i))-cvm(i)*pt(i, j, k)*(r_vir*q_tl(i&
&                   , j, k, sphum)*(1.-gz(i))-(1.+r_vir*q(i, j, k, sphum&
&                   ))*gz_tl(i)))/((1.+r_vir*q(i, j, k, sphum))*(1.-gz(i&
&                   )))**2+0.5*(phis_tl(i, k)+phis_tl(i, k+1)+2*w(i, j, &
&                   k)*w_tl(i, j, k)+0.5*gridstruct%rsin2(i, j)*(2*u(i, &
&                   j, k)*u_tl(i, j, k)+2*u(i, j+1, k)*u_tl(i, j+1, k)+2&
&                   *v(i, j, k)*v_tl(i, j, k)+2*v(i+1, j, k)*v_tl(i+1, j&
&                   , k)-gridstruct%cosa_s(i, j)*((u_tl(i, j, k)+u_tl(i&
...
&               rrg*delp(i, j, k)*pt_tl(i, j, k)/delz(i, j, k)
              arg1 = rrg*delp(i, j, k)/delz(i, j, k)*pt(i, j, k)
              arg2_tl = cappa_tl(i, j, k)*LOG(arg1) + cappa(i, j, k)*&
&               arg1_tl/arg1
              arg2 = cappa(i, j, k)*LOG(arg1)
              pkz_tl(i, j, k) = arg2_tl*EXP(arg2)
              pkz(i, j, k) = EXP(arg2)
            END DO
          END DO
...
&                 dtmp*pkz_tl(i, j, k))*(1.+r_vir*qv(i))*(1.-gz(i))-(pt(&
&                 i, j, k)+dtmp*pkz(i, j, k))*(r_vir*qv_tl(i)*(1.-gz(i))&
&                 -(1.+r_vir*qv(i))*gz_tl(i)))/((1.+r_vir*qv(i))*(1.-gz(&
&                 i)))**2
...
&                 *qv(i))*(1.-gz(i)))
            ...
            ELSE IF (nwat .EQ. 6) THEN
              DO i=is,ie
                gz_tl(i) = q_tl(i, j, k, liq_wat) + q_tl(i, j, k, &
&                 rainwat) + q_tl(i, j, k, ice_wat) + q_tl(i, j, k, &
&                 snowwat) + q_tl(i, j, k, graupel)
                gz(i) = q(i, j, k, liq_wat) + q(i, j, k, rainwat) + q(i&
&                 , j, k, ice_wat) + q(i, j, k, snowwat) + q(i, j, k, &
&                 graupel)
                pt_tl(i, j, k) = ((pt_tl(i, j, k)+dtmp_tl*pkz(i, j, k)+&
&                 dtmp*pkz_tl(i, j, k))*(1.+r_vir*q(i, j, k, sphum))*(1.&
&                 -gz(i))-(pt(i, j, k)+dtmp*pkz(i, j, k))*(r_vir*q_tl(i&
&                 , j, k, sphum)*(1.-gz(i))-(1.+r_vir*q(i, j, k, sphum))&
&                 *gz_tl(i)))/((1.+r_vir*q(i, j, k, sphum))*(1.-gz(i)))&
&                 **2
                pt(i, j, k) = (pt(i, j, k)+dtmp*pkz(i, j, k))/((1.+r_vir&
...
!-----------------
        ...
        IF (moist_phys) THEN
          DO k=1,km
            CALL MOIST_CV_TLM(is, ie, isd, ied, jsd, jed, km, j, k, nwat&
&                       , sphum, liq_wat, rainwat, ice_wat, snowwat, &
&                       graupel, q, q_tl, qd, qd_tl, cvm, cvm_tl)
            DO i=is,ie
              te_2d_tl(i, j) = te_2d_tl(i, j) + delp_tl(i, j, k)*(cvm(i)&
...
&               cvm_tl(i)*pt(i, j, k)+cvm(i)*pt_tl(i, j, k)+0.5*(phiz_tl&
&               (i, k)+phiz_tl(i, k+1)+2*w(i, j, k)*w_tl(i, j, k)+0.5*&
&               rsin2_l(i, j)*(2*u(i, j, k)*u_tl(i, j, k)+2*u(i, j+1, k)&
&               *u_tl(i, j+1, k)+2*v(i, j, k)*v_tl(i, j, k)+2*v(i+1, j, &
&               k)*v_tl(i+1, j, k)-cosa_s_l(i, j)*((u_tl(i, j, k)+u_tl(i&
&               , j+1, k))*(v(i, j, k)+v(i+1, j, k))+(u(i, j, k)+u(i, j+&
&               1, k))*(v_tl(i, j, k)+v_tl(i+1, j, k))))))
...
!  Differentiation of moist_cv in forward (tangent) mode:
!   variations   of useful results: qd cvm
!   with respect to varying inputs: qd cvm q
  SUBROUTINE MOIST_CV_TLM(is, ie, isd, ied, jsd, jed, km, j, k, nwat, &
&   sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, q, q_tl, qd, &
&   qd_tl, cvm, cvm_tl, t1)
    IMPLICIT NONE
...
&   graupel
    ...
    REAL, DIMENSION(isd:ied, jsd:jed, km, nwat), INTENT(IN) :: q_tl
    REAL, DIMENSION(is:ie), INTENT(OUT) :: cvm, qd
    REAL, DIMENSION(is:ie), INTENT(OUT) :: cvm_tl, qd_tl
    REAL, INTENT(IN), OPTIONAL :: t1(is:ie)
!
    REAL, PARAMETER :: t_i0=15.
    REAL, DIMENSION(is:ie) :: qv, ql, qs
    REAL, DIMENSION(is:ie) :: qv_tl, ql_tl, qs_tl
    INTEGER :: i
    INTRINSIC PRESENT
    INTRINSIC MAX
    SELECT CASE  (nwat) 
...
! Special case for GFS physics
        DO i=is,ie
          ...
          ELSE IF (t1(i) .LT. tice - t_i0) THEN
            ...
            qs(i) = qd(i)
          ELSE
            qs_tl(i) = (tice-t1(i))*qd_tl(i)/t_i0
            qs(i) = qd(i)*(tice-t1(i))/t_i0
          END IF
          ql_tl(i) = qd_tl(i) - qs_tl(i)
          ql(i) = qd(i) - qs(i)
          IF (0. .LT. q(i, j, k, sphum)) THEN
            qv_tl(i) = q_tl(i, j, k, sphum)
            qv(i) = q(i, j, k, sphum)
          ELSE
            ...
            qv(i) = 0.
          END IF
          cvm_tl(i) = cv_air*(-qv_tl(i)-qd_tl(i)) + cv_vap*qv_tl(i) + &
&           c_liq*ql_tl(i) + c_ice*qs_tl(i)
...
&           c_liq + qs(i)*c_ice
    ...
    CASE (3) 
      ...
      DO i=is,ie
        qv_tl(i) = q_tl(i, j, k, sphum)
        qv(i) = q(i, j, k, sphum)
        ql_tl(i) = q_tl(i, j, k, liq_wat)
        ql(i) = q(i, j, k, liq_wat)
        qs_tl(i) = q_tl(i, j, k, ice_wat)
        qs(i) = q(i, j, k, ice_wat)
        qd_tl(i) = ql_tl(i) + qs_tl(i)
        qd(i) = ql(i) + qs(i)
        cvm_tl(i) = cv_air*(-qv_tl(i)-qd_tl(i)) + cv_vap*qv_tl(i) + &
&         c_liq*ql_tl(i) + c_ice*qs_tl(i)
...
! K_warm_rain with fake ice
      DO i=is,ie
        qv_tl(i) = q_tl(i, j, k, sphum)
        qv(i) = q(i, j, k, sphum)
        qd_tl(i) = q_tl(i, j, k, liq_wat) + q_tl(i, j, k, rainwat)
...
&         c_liq*qd_tl(i)
    ...
    CASE (6) 
      ...
      DO i=is,ie
        qv_tl(i) = q_tl(i, j, k, sphum)
        qv(i) = q(i, j, k, sphum)
        ql_tl(i) = q_tl(i, j, k, liq_wat) + q_tl(i, j, k, rainwat)
        ql(i) = q(i, j, k, liq_wat) + q(i, j, k, rainwat)
        qs_tl(i) = q_tl(i, j, k, ice_wat) + q_tl(i, j, k, snowwat) + &
&         q_tl(i, j, k, graupel)
...
!  Differentiation of moist_cp in forward (tangent) mode:
!   variations   of useful results: qd
!   with respect to varying inputs: qd q
  SUBROUTINE MOIST_CP_TLM(is, ie, isd, ied, jsd, jed, km, j, k, nwat, &
&   sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, q, q_tl, qd, &
&   qd_tl, cpm, t1)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: is, ie, isd, ied, jsd, jed, km, nwat, j, k
...
&   graupel
    ...
    REAL, DIMENSION(is:ie), INTENT(OUT) :: qd_tl
    REAL, INTENT(IN), OPTIONAL :: t1(is:ie)
!
    REAL, PARAMETER :: t_i0=15.
    REAL, DIMENSION(is:ie) :: qv, ql, qs
    REAL, DIMENSION(is:ie) :: ql_tl, qs_tl
    INTEGER :: i
    INTRINSIC PRESENT
    INTRINSIC MAX
    SELECT CASE  (nwat) 
...
!-----------------------------------------------------------------------
  SUBROUTINE MAP_SCALAR_TLM(km, pe1, pe1_tl, qs, qs_tl, kn, pe2, &
&   pe2_tl, q2, q2_tl, i1, i2, j, ibeg, iend, jbeg, jend, iv, kord, &
&   q_min)
    IMPLICIT NONE
! iv=1
! Starting longitude
...
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      CALL SCALAR_PROFILE_TLM(qs, qs_tl, q4, q4_tl, dp1, dp1_tl, km, &
&                          i1, i2, iv, kord, q_min)
!else
!     call ppm_profile( q4, dp1, km, i1, i2, iv, kord )
      qsum_tl = 0.0
...
! Field output
    ...
    DO iq=1,nq
      ...
      END DO
      qs_tl = 0.0
      CALL SCALAR_PROFILE_TLM(qs, qs_tl, q4(1:4, i1:i2, 1:km, iq), &
&                          q4_tl(1:4, i1:i2, 1:km, iq), dp1, dp1_tl, km&
...
!   with respect to varying inputs: qs delp a4
  SUBROUTINE SCALAR_PROFILE_TLM(qs, qs_tl, a4, a4_tl, delp, delp_tl, &
