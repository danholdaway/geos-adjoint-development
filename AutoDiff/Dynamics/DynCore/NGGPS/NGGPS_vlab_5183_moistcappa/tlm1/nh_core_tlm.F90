...
!------------------------------
  ...
  USE TP_CORE_MOD_DIFF, ONLY : fv_tp_2d, fv_tp_2d_tlm
  USE NH_UTILS_MOD_DIFF, ONLY : update_dz_c, update_dz_c_tlm, &
& update_dz_d, update_dz_d_tlm, nest_halo_nh, nest_halo_nh_tlm
  USE NH_UTILS_MOD_DIFF, ONLY : sim_solver, sim_solver_tlm, sim1_solver,&
& sim1_solver_tlm, sim3_solver, sim3_solver_tlm
  USE NH_UTILS_MOD_DIFF, ONLY : sim3p0_solver, sim3p0_solver_tlm, rim_2d&
& , rim_2d_tlm
...
&   jsd, jed, akap, cappa, cappa_tl, cp, ptop, zs, q_con, q_con_tl, w, &
&   w_tl, delz, delz_tl, pt, pt_tl, delp, delp_tl, zh, zh_tl, pe, pe_tl&
&   , ppe, ppe_tl, pk3, pk3_tl, pk, pk_tl, peln, peln_tl, ws, ws_tl, &
...
! the BIG horizontal Lagrangian time step
    ...
    REAL, DIMENSION(isd:, jsd:, :), INTENT(IN) :: q_con_tl, cappa_tl
...
! Local:
    ...
    REAL, DIMENSION(is:ie, km) :: dm_tl, dz2_tl, pm2_tl, w2_tl, gm2_tl, &
&   cp2_tl
    REAL, DIMENSION(is:ie, km+1) :: pem, pe2, peln2, peg, pelng
    REAL, DIMENSION(is:ie, km+1) :: pem_tl, pe2_tl, peln2_tl, peg_tl, &
&   pelng_tl
...
&           k-1)) - dm(i, k-1)*q_con_tl(i, j, k-1)
      ...
      DO k=1,km
        DO i=is,ie
          pm2_tl(i, k) = ((peg_tl(i, k+1)-peg_tl(i, k))*(pelng(i, k+1)-&
...
&                      scale_m)
      ELSE IF (a_imp .LE. 0.5) THEN
        CALL RIM_2D_TLM(ms, dt, is, ie, km, rdgas, gama, gm2, gm2_tl, &
&                 pe2, pe2_tl, dm, dm_tl, pm2, pm2_tl, w2, w2_tl, dz2, &
&                 dz2_tl, pt(is:ie, j, 1:km), pt_tl(is:ie, j, 1:km), ws(&
&                 is:ie, j), ws_tl(is:ie, j), .false.)
      ELSE IF (a_imp .GT. 0.999) THEN
        CALL SIM1_SOLVER_TLM(dt, is, ie, km, rdgas, gama, gm2, gm2_tl, &
&                      cp2, cp2_tl, akap, pe2, pe2_tl, dm, dm_tl, pm2, &
&                      pm2_tl, pem, pem_tl, w2, w2_tl, dz2, dz2_tl, pt(&
&                      is:ie, j, 1:km), pt_tl(is:ie, j, 1:km), ws(is:ie&
&                      , j), ws_tl(is:ie, j), p_fac)
      ELSE
        CALL SIM_SOLVER_TLM(dt, is, ie, km, rdgas, gama, gm2, gm2_tl, &
&                     cp2, cp2_tl, akap, pe2, pe2_tl, dm, dm_tl, pm2, &
&                     pm2_tl, pem, pem_tl, w2, w2_tl, dz2, dz2_tl, pt(is&
&                     :ie, j, 1:km), pt_tl(is:ie, j, 1:km), ws(is:ie, j)&
&                     , ws_tl(is:ie, j), a_imp, p_fac, scale_m)
      END IF
      DO k=1,km
        DO i=is,ie
