...
MODULE FV_MAPZ_MOD_DIFF
  USE CONSTANTS_MOD, ONLY : radius, pi => pi_8, rvgas, rdgas, grav, hlv,&
& hlf, cp_air, cp_vapor
  USE TRACER_MANAGER_MOD, ONLY : get_tracer_index
  USE FIELD_MANAGER_MOD, ONLY : model_atmos
  USE FV_GRID_UTILS_MOD_DIFF, ONLY : g_sum, g_sum_adm, ptop_min
  USE FV_FILL_MOD_DIFF, ONLY : fillz
...
& mpp_update_domains_adm, domain2d
  ...
  USE FV_MP_MOD_DIFF, ONLY : is_master
  USE FV_CMP_MOD_DIFF, ONLY : qs_init, fv_sat_adj
  USE FV_DIAGNOSTICS_MOD_DIFF, ONLY : prt_mxm
...
! 1846.
  ...
  REAL :: e_flux=0.
  PRIVATE 
  PUBLIC compute_total_energy, lagrangian_to_eulerian, moist_cv, &
& moist_cp, rst_remap, mappm, e_flux, map1_q2
  PUBLIC compute_total_energy_fwd, compute_total_energy_bwd, &
& lagrangian_to_eulerian_fwd, lagrangian_to_eulerian_bwd, moist_cv_fwd, &
& moist_cv_bwd, moist_cp_fwd, moist_cp_bwd, map1_q2_adm
  EXTERNAL COMPUTE_TOTAL_ENERGY_ADM
  EXTERNAL MOIST_CV_ADM
  EXTERNAL MOIST_CP_ADM
...
!mod.Rayleigh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3
!_to_geos fv_grid_utils_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.Lagrangian_to_Euler
!ian fv_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_m
!apz_mod.mapn_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_ma
!pz_mod.cs_limiters fv_mapz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm f
!v_mapz_mod.moist_cv fv_mapz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer2
!d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_u
...
!   gradient     of useful results: ws cappa q_con peln q u v w
!                delp ua delz omga te0_2d pkz pe pk ps pt te
!   with respect to varying inputs: ws cappa q_con peln q u v w
!                delp ua delz omga te0_2d pkz pe pk ps pt te
              ...
              IF (nwat .EQ. 2) THEN
                ...
                DO i=ad_from2,ie
                  ...
                  IF (0. .LT. q(i, j, k, liq_wat)) THEN
                    CALL PUSHREALARRAY(q_con(i, j, k))
                    q_con(i, j, k) = q(i, j, k, liq_wat)
                    CALL PUSHCONTROL(1,0)
                  ELSE
                    CALL PUSHREALARRAY(q_con(i, j, k))
                    q_con(i, j, k) = 0.
                  ...
                  CALL PUSHREALARRAY(cappa(i, j, k))
                  cappa(i, j, k) = rdgas/(rdgas+cvm(i)/(1.+r_vir*qv(i)))
                  CALL PUSHREALARRAY(pt(i, j, k))
                  pt(i, j, k) = pt(i, j, k)*EXP(cappa(i, j, k)/(1.-cappa&
&                   (i, j, k))*LOG(rrg*delp(i, j, k)/delz(i, j, k)*pt(i&
&                   , j, k)))
                END DO
                CALL PUSHINTEGER(i - 1)
                ...
                CALL PUSHCONTROL(1,1)
              ELSE
                CALL MOIST_CV_FWD(is, ie, isd, ied, jsd, jed, km, j, k, &
&                           nwat, sphum, liq_wat, rainwat, ice_wat, &
&                           snowwat, graupel, q, gz, cvm)
                ad_from3 = is
                DO i=ad_from3,ie
                  CALL PUSHREALARRAY(q_con(i, j, k))
                  q_con(i, j, k) = gz(i)
                  CALL PUSHREALARRAY(cappa(i, j, k))
                  cappa(i, j, k) = rdgas/(rdgas+cvm(i)/(1.+r_vir*q(i, j&
&                   , k, sphum)))
                  CALL PUSHREALARRAY(pt(i, j, k))
                  pt(i, j, k) = pt(i, j, k)*EXP(cappa(i, j, k)/(1.-cappa&
&                   (i, j, k))*LOG(rrg*delp(i, j, k)/delz(i, j, k)*pt(i&
...
! KE using 3D winds:
                  CALL PUSHREALARRAY(q_con(i, j, k))
                  q_con(i, j, k) = gz(i)
                  te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cvm(i)*pt(i&
&                   , j, k)/((1.+r_vir*q(i, j, k, sphum))*(1.-gz(i)))+&
      ...
      DO k=kmp,km
        IF (.NOT.hydrostatic) THEN
          DO j=js,je
            DO i=is,ie
              CALL PUSHREALARRAY(pkz(i, j, k))
              pkz(i, j, k) = EXP(cappa(i, j, k)*LOG(rrg*delp(i, j, k)/&
&               delz(i, j, k)*pt(i, j, k)))
            END DO
          END DO
          CALL PUSHCONTROL(1,1)
...
&                 *qv(i))*(1.-gz(i)))
            ...
            ELSE IF (nwat .EQ. 6) THEN
              DO i=is,ie
                CALL PUSHREALARRAY(gz(i))
                gz(i) = q(i, j, k, liq_wat) + q(i, j, k, rainwat) + q(i&
&                 , j, k, ice_wat) + q(i, j, k, snowwat) + q(i, j, k, &
&                 graupel)
                CALL PUSHREALARRAY(pt(i, j, k))
                pt(i, j, k) = (pt(i, j, k)+dtmp*pkz(i, j, k))/((1.+r_vir&
&                 *q(i, j, k, sphum))*(1.-gz(i)))
              END DO
              CALL PUSHCONTROL(2,1)
            ELSE
              CALL MOIST_CV_FWD(is, ie, isd, ied, jsd, jed, km, j, k, &
...
!_mod.Rayleigh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv
!3_to_geos fv_grid_utils_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.Lagrangian_to_Eule
!rian fv_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_
!mapz_mod.mapn_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_m
!apz_mod.cs_limiters fv_mapz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm 
!fv_mapz_mod.moist_cv fv_mapz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer
!2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_
...
&   , js, je, isd, ied, jsd, jed, nq, nwat, sphum, q_con, q_con_ad, u, &
&   u_ad, v, v_ad, w, w_ad, delz, delz_ad, pt, pt_ad, q, q_ad, hs, r_vir&
&   , cp, akap, cappa, cappa_ad, kord_mt, kord_wz, kord_tr, kord_tm, &
&   peln, peln_ad, te0_2d, te0_2d_ad, ng, ua, ua_ad, va, omga, omga_ad, &
&   te, te_ad, ws, ws_ad, fill, reproduce_sum, out_dt, dtdt, ptop, ak, &
&   bk, pfull, flagstruct, gridstruct, domain, do_sat_adj, hydrostatic, &
&   hybrid_z, do_omega, adiabatic, do_adiabatic_init, mfx, mfy, &
&   remap_option, kord_mt_pert, kord_wz_pert, kord_tr_pert, kord_tm_pert&
& )
    IMPLICIT NONE
!$OMP end parallel
    LOGICAL, INTENT(IN) :: last_step
    ...
    REAL, DIMENSION(isd:ied, jsd:jed, km), INTENT(INOUT) :: q_con, cappa
    REAL, DIMENSION(isd:ied, jsd:jed, km), INTENT(INOUT) :: q_con_ad, &
&   cappa_ad
    LOGICAL, INTENT(IN) :: hydrostatic
    LOGICAL, INTENT(IN) :: hybrid_z
    LOGICAL, INTENT(IN) :: out_dt
...
&   pn2_ad, phis_ad
    ...
    REAL, DIMENSION(is:ie) :: gz, cvm, qv
    REAL, DIMENSION(is:ie) :: gz_ad, cvm_ad, qv_ad
    REAL :: rcp, rg, tmp, tpe, rrg, bkh, dtmp, k1k, dlnp
          ...
          DO j=je,js,-1
            ...
            IF (branch .EQ. 0) THEN
              DO i=ie,is,-1
                ...
                temp58 = r_vir*q(i, j, k, sphum) + 1.
                temp57 = temp58*(-gz(i)+1.)
                temp_ad50 = pt_ad(i, j, k)/temp57
                temp_ad51 = -((pt(i, j, k)+dtmp*pkz(i, j, k))*temp_ad50/&
&                 temp57)
                dtmp_ad = dtmp_ad + pkz(i, j, k)*temp_ad50
                pkz_ad(i, j, k) = pkz_ad(i, j, k) + dtmp*temp_ad50
                q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) + (1.-gz(i))&
&                 *r_vir*temp_ad51
                gz_ad(i) = gz_ad(i) - temp58*temp_ad51
                pt_ad(i, j, k) = temp_ad50
              END DO
              cvm_ad = 0.0
              CALL MOIST_CV_BWD(is, ie, isd, ied, jsd, jed, km, j, k, &
&                         nwat, sphum, liq_wat, rainwat, ice_wat, &
&                         snowwat, graupel, q, q_ad, gz, gz_ad, cvm, &
&                         cvm_ad)
            ELSE IF (branch .EQ. 1) THEN
              DO i=ie,is,-1
                CALL POPREALARRAY(pt(i, j, k))
                temp56 = r_vir*q(i, j, k, sphum) + 1.
                temp55 = temp56*(-gz(i)+1.)
                temp_ad48 = pt_ad(i, j, k)/temp55
                temp_ad49 = -((pt(i, j, k)+dtmp*pkz(i, j, k))*temp_ad48/&
&                 temp55)
                dtmp_ad = dtmp_ad + pkz(i, j, k)*temp_ad48
                pkz_ad(i, j, k) = pkz_ad(i, j, k) + dtmp*temp_ad48
                q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) + (1.-gz(i))&
&                 *r_vir*temp_ad49
                gz_ad(i) = gz_ad(i) - temp56*temp_ad49
                pt_ad(i, j, k) = temp_ad48
                CALL POPREALARRAY(gz(i))
                q_ad(i, j, k, liq_wat) = q_ad(i, j, k, liq_wat) + gz_ad(&
&                 i)
                q_ad(i, j, k, rainwat) = q_ad(i, j, k, rainwat) + gz_ad(&
&                 i)
                q_ad(i, j, k, ice_wat) = q_ad(i, j, k, ice_wat) + gz_ad(&
&                 i)
                q_ad(i, j, k, snowwat) = q_ad(i, j, k, snowwat) + gz_ad(&
&                 i)
                q_ad(i, j, k, graupel) = q_ad(i, j, k, graupel) + gz_ad(&
&                 i)
                gz_ad(i) = 0.0
              END DO
            ELSE
              DO i=ie,is,-1
                CALL POPREALARRAY(pt(i, j, k))
                temp54 = r_vir*qv(i) + 1.
                temp53 = temp54*(-gz(i)+1.)
                temp_ad46 = pt_ad(i, j, k)/temp53
                temp_ad47 = -((pt(i, j, k)+dtmp*pkz(i, j, k))*temp_ad46/&
&                 temp53)
                dtmp_ad = dtmp_ad + pkz(i, j, k)*temp_ad46
                pkz_ad(i, j, k) = pkz_ad(i, j, k) + dtmp*temp_ad46
                qv_ad(i) = qv_ad(i) + (1.-gz(i))*r_vir*temp_ad47
                gz_ad(i) = gz_ad(i) - temp54*temp_ad47
                pt_ad(i, j, k) = temp_ad46
                CALL POPCONTROL(1,branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY(qv(i))
                  q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) + qv_ad(i)
                  qv_ad(i) = 0.0
                ELSE
                  CALL POPREALARRAY(qv(i))
                  qv_ad(i) = 0.0
                ...
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY(gz(i))
                  q_ad(i, j, k, liq_wat) = q_ad(i, j, k, liq_wat) + &
&                   gz_ad(i)
                  gz_ad(i) = 0.0
                ELSE
                  CALL POPREALARRAY(gz(i))
      ...
      ELSE IF (branch .EQ. 1) THEN
        ...
        DO k=km,1,-1
          DO j=je,js,-1
            DO i=ie,is,-1
              CALL POPREALARRAY(pt(i, j, k))
              temp61 = r_vir*q(i, j, k, sphum) + 1.
              temp_ad52 = pt_ad(i, j, k)/temp61
              temp60 = pkz(i, j, k)
              temp59 = pt(i, j, k) + dtmp
              dtmp_ad = dtmp_ad + temp60*temp_ad52
              pkz_ad(i, j, k) = pkz_ad(i, j, k) + temp59*temp_ad52
              q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) - temp59*&
&               temp60*r_vir*temp_ad52/temp61
              pt_ad(i, j, k) = temp60*temp_ad52
            END DO
          END DO
        END DO
        gz_ad = 0.0
        ...
        DO j=je,js,-1
          DO k=1,km,1
            DO i=ie,is,-1
              dlnp = rg*(peln(i, k+1, j)-peln(i, k, j))
              CALL POPREALARRAY(gz(i))
              temp_ad53 = (r_vir*q(i, j, k, sphum)+1.)*gz_ad(i)
              tmp_ad = pt_ad(i, j, k) + dlnp*temp_ad53
              CALL POPREALARRAY(pt(i, j, k))
              temp67 = r_vir*q(i, j, k, sphum) + 1.
              temp_ad55 = pt_ad(i, j, k)/temp67
              dtmp_ad = dtmp_ad + pkz(i, j, k)*temp_ad55
              pkz_ad(i, j, k) = pkz_ad(i, j, k) + dtmp*temp_ad55
              pt_ad(i, j, k) = 0.0
              temp66 = r_vir*q(i, j, k, sphum) + 1.
              temp65 = delp(i, j, k)
              temp64 = pe(i, k, j)
              temp63 = temp64*dlnp/temp65
              temp62 = (cp-temp63)*temp66
              temp_ad56 = -(tpe*tmp_ad/temp62**2)
              q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) + (cp-temp63)*&
&               r_vir*temp_ad56 - dtmp*pkz(i, j, k)*r_vir*temp_ad55/&
&               temp67 + dlnp*tmp*r_vir*gz_ad(i)
              temp_ad54 = -(temp66*temp_ad56/temp65)
              dlnp_ad = temp64*temp_ad54 + tmp*temp_ad53
              CALL POPREALARRAY(tmp)
              tpe_ad = tmp_ad/temp62
              pe_ad(i, k, j) = pe_ad(i, k, j) + dlnp*temp_ad54
              delp_ad(i, j, k) = delp_ad(i, j, k) - temp63*temp_ad54
              peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + rg*dlnp_ad
              peln_ad(i, k, j) = peln_ad(i, k, j) - rg*dlnp_ad
              CALL POPREALARRAY(tpe)
              temp_ad57 = -(gridstruct%rsin2(i, j)*0.25*tpe_ad)
              temp_ad58 = -(gridstruct%cosa_s(i, j)*temp_ad57)
              temp_ad59 = (v(i, j, k)+v(i+1, j, k))*temp_ad58
              temp_ad60 = (u(i, j, k)+u(i, j+1, k))*temp_ad58
              te_ad(i, j, k) = te_ad(i, j, k) + tpe_ad
              gz_ad(i) = gz_ad(i) - tpe_ad
              u_ad(i, j, k) = u_ad(i, j, k) + temp_ad59 + 2*u(i, j, k)*&
&               temp_ad57
              u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp_ad59 + 2*u(i, j+1&
&               , k)*temp_ad57
              v_ad(i, j, k) = v_ad(i, j, k) + temp_ad60 + 2*v(i, j, k)*&
&               temp_ad57
              v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp_ad60 + 2*v(i+1, j&
&               , k)*temp_ad57
            END DO
          END DO
          DO i=ie,is,-1
        ...
        ELSE
          ...
          DO k=km,1,-1
            DO j=je,js,-1
              DO i=ie,is,-1
                CALL POPREALARRAY(pt(i, j, k))
                temp_ad61 = pt_ad(i, j, k)/pkz(i, j, k)
                pkz_ad(i, j, k) = pkz_ad(i, j, k) - pt(i, j, k)*&
&                 temp_ad61/pkz(i, j, k)
                pt_ad(i, j, k) = temp_ad61
              END DO
            END DO
          END DO
          gz_ad = 0.0
      ...
      ELSE IF (branch .EQ. 5) THEN
        ...
        DO j=je,js,-1
          DO k=1,km,1
            DO i=ie,is,-1
              temp_ad63 = pt_ad(i, j, k)/pkz(i, j, k)
              dlnp = rg*(peln(i, k+1, j)-peln(i, k, j))
              tmp = tpe/(cp-pe(i, k, j)*dlnp/delp(i, j, k))
              CALL POPREALARRAY(gz(i))
              tmp_ad = temp_ad63 + dlnp*gz_ad(i)
              CALL POPREALARRAY(pt(i, j, k))
              pkz_ad(i, j, k) = pkz_ad(i, j, k) - tmp*temp_ad63/pkz(i, j&
&               , k)
              dtmp_ad = dtmp_ad + pt_ad(i, j, k)
              pt_ad(i, j, k) = 0.0
              temp70 = delp(i, j, k)
              temp69 = pe(i, k, j)
              temp68 = temp69*dlnp/temp70
              temp_ad64 = tmp_ad/(cp-temp68)
              temp_ad62 = tpe*temp_ad64/((cp-temp68)*temp70)
              dlnp_ad = temp69*temp_ad62 + tmp*gz_ad(i)
              tpe_ad = temp_ad64
              pe_ad(i, k, j) = pe_ad(i, k, j) + dlnp*temp_ad62
              delp_ad(i, j, k) = delp_ad(i, j, k) - temp68*temp_ad62
              peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + rg*dlnp_ad
              peln_ad(i, k, j) = peln_ad(i, k, j) - rg*dlnp_ad
              CALL POPREALARRAY(tpe)
              temp_ad65 = -(gridstruct%rsin2(i, j)*0.25*tpe_ad)
              temp_ad66 = -(gridstruct%cosa_s(i, j)*temp_ad65)
              temp_ad67 = (v(i, j, k)+v(i+1, j, k))*temp_ad66
              temp_ad68 = (u(i, j, k)+u(i, j+1, k))*temp_ad66
              te_ad(i, j, k) = te_ad(i, j, k) + tpe_ad
              gz_ad(i) = gz_ad(i) - tpe_ad
              u_ad(i, j, k) = u_ad(i, j, k) + temp_ad67 + 2*u(i, j, k)*&
&               temp_ad65
              u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp_ad67 + 2*u(i, j+1&
&               , k)*temp_ad65
              v_ad(i, j, k) = v_ad(i, j, k) + temp_ad68 + 2*v(i, j, k)*&
&               temp_ad65
              v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp_ad68 + 2*v(i+1, j&
&               , k)*temp_ad65
            END DO
          END DO
          DO i=ie,is,-1
      ...
      IF (branch .NE. 0) THEN
        DO j=je,js,-1
          DO i=ie,is,-1
            ...
            temp52 = delz(i, j, k)
            temp51 = delp(i, j, k)*pt(i, j, k)
            temp49 = temp51/temp52
            temp50 = LOG(rrg*temp49)
            temp_ad44 = EXP(cappa(i, j, k)*temp50)*pkz_ad(i, j, k)
            temp_ad45 = cappa(i, j, k)*temp_ad44/(temp49*temp52)
            cappa_ad(i, j, k) = cappa_ad(i, j, k) + temp50*temp_ad44
            delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*temp_ad45
            pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad45
            delz_ad(i, j, k) = delz_ad(i, j, k) - temp49*temp_ad45
            pkz_ad(i, j, k) = 0.0
          END DO
        END DO
      END IF
    END DO
 120 CALL POPCONTROL(3,branch)
    IF (branch .LT. 3) THEN
      IF (branch .EQ. 0) THEN
        temp_ad42 = dtmp_ad/(cp*result1)
        tpe_ad = temp_ad42
        result1_ad = -(tpe*temp_ad42/result1)
        CALL G_SUM_ADM(domain, zsum0, zsum0_ad, is, ie, js, je, ng, &
&                gridstruct%area_64, 0, reproduce=.true., g_sum_ad=&
&                result1_ad)
        zsum1_ad = 0.0
      ELSE IF (branch .EQ. 1) THEN
        temp_ad43 = dtmp_ad/(cv_air*result1)
        tpe_ad = temp_ad43
        result1_ad = -(tpe*temp_ad43/result1)
...
&                 te_2d_ad(i, j)
            ...
            ELSE
              DO k=km,1,-1
                DO i=ie,is,-1
                  temp48 = v(i, j, k) + v(i+1, j, k)
                  temp47 = u(i, j, k) + u(i, j+1, k)
                  temp46 = 0.5*gridstruct%rsin2(i, j)
                  temp45 = r_vir*q(i, j, k, sphum) + 1.
                  temp44 = pt(i, j, k)/temp45
                  temp_ad37 = delp(i, j, k)*te_2d_ad(i, j)
                  temp_ad38 = cv_air*temp_ad37/temp45
                  temp_ad39 = 0.5*temp_ad37
                  temp_ad40 = temp46*temp_ad39
                  temp_ad41 = -(gridstruct%cosa_s(i, j)*temp_ad40)
                  delp_ad(i, j, k) = delp_ad(i, j, k) + (cv_air*temp44+&
&                   0.5*(phis(i, k)+phis(i, k+1)+w(i, j, k)**2+temp46*(u&
&                   (i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j&
&                   , k)**2-gridstruct%cosa_s(i, j)*(temp47*temp48))))*&
&                   te_2d_ad(i, j)
                  pt_ad(i, j, k) = pt_ad(i, j, k) + temp_ad38
                  q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) - temp44*&
&                   r_vir*temp_ad38
                  phis_ad(i, k) = phis_ad(i, k) + temp_ad39
                  phis_ad(i, k+1) = phis_ad(i, k+1) + temp_ad39
                  w_ad(i, j, k) = w_ad(i, j, k) + 2*w(i, j, k)*temp_ad39
                  u_ad(i, j, k) = u_ad(i, j, k) + temp48*temp_ad41 + 2*u&
&                   (i, j, k)*temp_ad40
                  u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp48*temp_ad41 +&
&                   2*u(i, j+1, k)*temp_ad40
                  v_ad(i, j, k) = v_ad(i, j, k) + temp47*temp_ad41 + 2*v&
&                   (i, j, k)*temp_ad40
                  v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp47*temp_ad41 +&
&                   2*v(i+1, j, k)*temp_ad40
                END DO
              END DO
              DO i=ie,is,-1
                te_2d_ad(i, j) = 0.0
...
&                   k)
        ...
        ELSE IF (branch .EQ. 3) THEN
          DO k=km,1,-1
            DO i=ie,is,-1
              temp43 = v(i, j, k) + v(i+1, j, k)
              temp42 = u(i, j, k) + u(i, j+1, k)
              temp41 = 0.25*gridstruct%rsin2(i, j)
              temp_ad34 = delp(i, j, k)*te_2d_ad(i, j)
              temp_ad35 = temp41*temp_ad34
              temp_ad36 = -(gridstruct%cosa_s(i, j)*temp_ad35)
              delp_ad(i, j, k) = delp_ad(i, j, k) + (cp_air*(pt(i, j, k)&
&               *pkz(i, j, k))+temp41*(u(i, j, k)**2+u(i, j+1, k)**2+v(i&
&               , j, k)**2+v(i+1, j, k)**2-gridstruct%cosa_s(i, j)*(&
&               temp42*temp43)))*te_2d_ad(i, j)
              pt_ad(i, j, k) = pt_ad(i, j, k) + cp_air*pkz(i, j, k)*&
&               temp_ad34
              pkz_ad(i, j, k) = pkz_ad(i, j, k) + cp_air*pt(i, j, k)*&
&               temp_ad34
              u_ad(i, j, k) = u_ad(i, j, k) + temp43*temp_ad36 + 2*u(i, &
&               j, k)*temp_ad35
              u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp43*temp_ad36 + 2*u&
&               (i, j+1, k)*temp_ad35
              v_ad(i, j, k) = v_ad(i, j, k) + temp42*temp_ad36 + 2*v(i, &
&               j, k)*temp_ad35
              v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp42*temp_ad36 + 2*v&
&               (i+1, j, k)*temp_ad35
            END DO
          END DO
          DO i=ie,is,-1
...
&             , j)
          ...
          DO i=ie,is,-1
            DO k=km,1,-1
              temp_ad33 = cp_air*pt(i, j, k)*gz_ad(i)
              pt_ad(i, j, k) = pt_ad(i, j, k) + cp_air*(pk(i, j, k+1)-pk&
&               (i, j, k))*gz_ad(i)
              pk_ad(i, j, k+1) = pk_ad(i, j, k+1) + temp_ad33
              pk_ad(i, j, k) = pk_ad(i, j, k) - temp_ad33
            END DO
            CALL POPREALARRAY(gz(i))
            gz_ad(i) = 0.0
          END DO
        ELSE IF (branch .EQ. 4) THEN
          DO k=km,1,-1
            DO i=ie,is,-1
              temp40 = v(i, j, k) + v(i+1, j, k)
              temp39 = u(i, j, k) + u(i, j+1, k)
              temp38 = 0.5*gridstruct%rsin2(i, j)
              temp37 = r_vir*q(i, j, k, sphum) + 1.
              temp35 = temp37*(-gz(i)+1.)
              temp36 = cvm(i)*pt(i, j, k)
              temp34 = temp36/temp35
              temp_ad27 = delp(i, j, k)*te_2d_ad(i, j)
              temp_ad28 = temp_ad27/temp35
              temp_ad29 = -(temp34*temp_ad28)
              temp_ad30 = 0.5*temp_ad27
              temp_ad31 = temp38*temp_ad30
              temp_ad32 = -(gridstruct%cosa_s(i, j)*temp_ad31)
              delp_ad(i, j, k) = delp_ad(i, j, k) + (temp34+0.5*(phis(i&
&               , k)+phis(i, k+1)+w(i, j, k)**2+temp38*(u(i, j, k)**2+u(&
&               i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-gridstruct%&
&               cosa_s(i, j)*(temp39*temp40))))*te_2d_ad(i, j)
              cvm_ad(i) = cvm_ad(i) + pt(i, j, k)*temp_ad28
              pt_ad(i, j, k) = pt_ad(i, j, k) + cvm(i)*temp_ad28
              q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) + (1.-gz(i))*&
&               r_vir*temp_ad29
              gz_ad(i) = gz_ad(i) + q_con_ad(i, j, k) - temp37*temp_ad29
              phis_ad(i, k) = phis_ad(i, k) + temp_ad30
              phis_ad(i, k+1) = phis_ad(i, k+1) + temp_ad30
              w_ad(i, j, k) = w_ad(i, j, k) + 2*w(i, j, k)*temp_ad30
              u_ad(i, j, k) = u_ad(i, j, k) + temp40*temp_ad32 + 2*u(i, &
&               j, k)*temp_ad31
              u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp40*temp_ad32 + 2*u&
&               (i, j+1, k)*temp_ad31
              v_ad(i, j, k) = v_ad(i, j, k) + temp39*temp_ad32 + 2*v(i, &
&               j, k)*temp_ad31
              v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp39*temp_ad32 + 2*v&
&               (i+1, j, k)*temp_ad31
              CALL POPREALARRAY(q_con(i, j, k))
              q_con_ad(i, j, k) = 0.0
            END DO
            CALL POPCONTROL(1,branch)
            IF (branch .EQ. 0) THEN
...
&                         cvm_ad)
            ELSE
              DO i=ie,is,-1
                ...
                qv_ad(i) = qv_ad(i) + (cv_vap-cv_air)*cvm_ad(i)
                cvm_ad(i) = 0.0
                CALL POPCONTROL(1,branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREALARRAY(gz(i))
        ...
        ELSE
          DO k=km,1,-1
            DO i=ie,is,-1
              ...
              temp31 = 0.25*gridstruct%rsin2(i, j)
              temp_ad24 = delp(i, j, k)*te_2d_ad(i, j)
              temp_ad25 = temp31*temp_ad24
              temp_ad26 = -(gridstruct%cosa_s(i, j)*temp_ad25)
              delp_ad(i, j, k) = delp_ad(i, j, k) + (cp*pt(i, j, k)+&
&               temp31*(u(i, j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+&
&               1, j, k)**2-gridstruct%cosa_s(i, j)*(temp32*temp33)))*&
&               te_2d_ad(i, j)
              pt_ad(i, j, k) = pt_ad(i, j, k) + cp*temp_ad24
              u_ad(i, j, k) = u_ad(i, j, k) + temp33*temp_ad26 + 2*u(i, &
&               j, k)*temp_ad25
              u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp33*temp_ad26 + 2*u&
&               (i, j+1, k)*temp_ad25
              v_ad(i, j, k) = v_ad(i, j, k) + temp32*temp_ad26 + 2*v(i, &
&               j, k)*temp_ad25
              v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp32*temp_ad26 + 2*v&
&               (i+1, j, k)*temp_ad25
            END DO
          END DO
          DO i=ie,is,-1
...
&             , j)
          ...
          DO i=ie,is,-1
            DO k=km,1,-1
              temp_ad23 = rg*pt(i, j, k)*gz_ad(i)
              pt_ad(i, j, k) = pt_ad(i, j, k) + rg*(peln(i, k+1, j)-peln&
&               (i, k, j))*gz_ad(i)
              peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + temp_ad23
              peln_ad(i, k, j) = peln_ad(i, k, j) - temp_ad23
            END DO
            CALL POPREALARRAY(gz(i))
            gz_ad(i) = 0.0
          END DO
 ...
 150 DO k=km,2,-1
    ...
    DO i2=1,ad_count1
      IF (i2 .EQ. 1) THEN
        ...
        END IF
      ELSE
        CALL POPINTEGER(ad_to60)
        DO k=ad_to60,1,-1
          CALL POPINTEGER(ad_from34)
          CALL POPINTEGER(ad_to59)
          DO i=ad_to59,ad_from34,-1
            ...
            DO i=ad_to50,ad_from28,-1
              ...
              DO n=ad_to49,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  ...
                  temp28 = pe0(i, k+1) - pe0(i, k)
                  temp_ad21 = omga_ad(i, j, n)/temp28
                  temp30 = dp2(i, n) - pe0(i, k)
                  temp29 = pe3(i, k+1) - pe3(i, k)
                  temp_ad22 = -(temp29*temp30*temp_ad21/temp28)
                  pe3_ad(i, k) = pe3_ad(i, k) + omga_ad(i, j, n) - &
&                   temp30*temp_ad21
                  pe3_ad(i, k+1) = pe3_ad(i, k+1) + temp30*temp_ad21
                  dp2_ad(i, n) = dp2_ad(i, n) + temp29*temp_ad21
                  pe0_ad(i, k) = pe0_ad(i, k) - temp_ad22 - temp29*&
&                   temp_ad21
                  pe0_ad(i, k+1) = pe0_ad(i, k+1) + temp_ad22
                  omga_ad(i, j, n) = 0.0
                END IF
...
&                 k)
          ...
          IF (branch .EQ. 0) THEN
            ...
            DO k=ad_to42,1,-1
              ...
              DO i=ad_to41,ad_from24,-1
                CALL POPREALARRAY(pkz(i, j, k))
                temp21 = akap*(peln(i, k+1, j)-peln(i, k, j))
                temp_ad17 = pkz_ad(i, j, k)/temp21
                temp_ad18 = -((pk2(i, k+1)-pk2(i, k))*akap*temp_ad17/&
&                 temp21)
                pk2_ad(i, k+1) = pk2_ad(i, k+1) + temp_ad17
                pk2_ad(i, k) = pk2_ad(i, k) - temp_ad17
                peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + temp_ad18
                peln_ad(i, k, j) = peln_ad(i, k, j) - temp_ad18
                pkz_ad(i, j, k) = 0.0
              END DO
            END DO
          ELSE IF (branch .EQ. 1) THEN
            ...
            DO k=ad_to44,1,-1
              ...
              DO i=ad_to43,ad_from25,-1
                CALL POPREALARRAY(pkz(i, j, k))
                temp24 = delz(i, j, k)
                temp23 = delp(i, j, k)*pt(i, j, k)
                temp22 = temp23/temp24
                temp_ad19 = akap*EXP(akap*LOG(rrg*temp22))*pkz_ad(i, j, &
&                 k)/(temp22*temp24)
                delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*&
&                 temp_ad19
                pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*&
&                 temp_ad19
                delz_ad(i, j, k) = delz_ad(i, j, k) - temp22*temp_ad19
                pkz_ad(i, j, k) = 0.0
              END DO
            END DO
          ELSE
            ...
            DO k=ad_to46,1,-1
              ...
              DO i=ad_to45,ad_from26,-1
                CALL POPREALARRAY(pkz(i, j, k))
                temp27 = delz(i, j, k)
                temp26 = delp(i, j, k)*pt(i, j, k)
                temp25 = temp26/temp27
                temp_ad20 = k1k*EXP(k1k*LOG(rrg*temp25))*pkz_ad(i, j, k)&
&                 /(temp25*temp27)
                delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*&
&                 temp_ad20
                pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*&
&                 temp_ad20
                delz_ad(i, j, k) = delz_ad(i, j, k) - temp25*temp_ad20
                pkz_ad(i, j, k) = 0.0
              END DO
            END DO
          END IF
...
&                          ie, 1:km+1, j), gz, gz_ad, km, pn2, pn2_ad, &
...
&                       , 1:km+1, j), gz, gz_ad, km, pn2, pn2_ad, pt, &
...
&                     conserv=.true.)
        ...
        DO k=ad_to28,1,-1
          ...
          DO i=ad_to27,ad_from17,-1
            CALL POPREALARRAY(te(i, j, k))
            temp20 = pe1(i, k+1) - pe1(i, k)
            temp_ad15 = te_ad(i, j, k)/temp20
            temp_ad16 = -((phis(i, k+1)-phis(i, k))*temp_ad15/temp20)
            phis_ad(i, k+1) = phis_ad(i, k+1) + temp_ad15
            phis_ad(i, k) = phis_ad(i, k) - temp_ad15
            pe1_ad(i, k+1) = pe1_ad(i, k+1) + temp_ad16
            pe1_ad(i, k) = pe1_ad(i, k) - temp_ad16
          END DO
        END DO
        ...
        DO k=1,ad_from15,1
          ...
          DO i=ad_to24,ad_from14,-1
            CALL POPREALARRAY(phis(i, k))
            temp_ad14 = cp_air*pt(i, j, k)*phis_ad(i, k)
            phis_ad(i, k+1) = phis_ad(i, k+1) + phis_ad(i, k)
            pt_ad(i, j, k) = pt_ad(i, j, k) + cp_air*(pk1(i, k+1)-pk1(i&
&             , k))*phis_ad(i, k)
            pk1_ad(i, k+1) = pk1_ad(i, k+1) + temp_ad14
            pk1_ad(i, k) = pk1_ad(i, k) - temp_ad14
            phis_ad(i, k) = 0.0
          END DO
        END DO
...
&           i, k)
      ...
      IF (branch .NE. 0) THEN
        ...
        DO k=ad_to10,1,-1
          ...
          DO i=ad_to9,ad_from5,-1
            CALL POPREALARRAY(delz(i, j, k))
            temp_ad13 = -(delz_ad(i, j, k)/delp(i, j, k))
            delp_ad(i, j, k) = delp_ad(i, j, k) - delz(i, j, k)*&
&             temp_ad13/delp(i, j, k)
            delz_ad(i, j, k) = temp_ad13
          END DO
        END DO
      END IF
      CALL POPCONTROL(3,branch)
          ...
          DO k=ad_to6,1,-1
            ...
            IF (branch .EQ. 0) THEN
              ...
              DO i=ad_to5,ad_from3,-1
                CALL POPREALARRAY(pt(i, j, k))
                temp19 = -cappa(i, j, k) + 1.
                temp14 = cappa(i, j, k)/temp19
                temp18 = delz(i, j, k)
                temp17 = delp(i, j, k)*pt(i, j, k)
                temp13 = temp17/temp18
                temp16 = rrg*temp13
                temp15 = LOG(temp16)
                temp_ad5 = pt(i, j, k)*EXP(temp15*temp14)*pt_ad(i, j, k)
                temp_ad6 = rrg*temp14*temp_ad5/(temp16*temp18)
                temp_ad7 = temp15*temp_ad5/temp19
                delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*&
&                 temp_ad6
                delz_ad(i, j, k) = delz_ad(i, j, k) - temp13*temp_ad6
                cappa_ad(i, j, k) = cappa_ad(i, j, k) + (temp14+1.0)*&
&                 temp_ad7
                pt_ad(i, j, k) = delp(i, j, k)*temp_ad6 + EXP(temp15*&
&                 temp14)*pt_ad(i, j, k)
                CALL POPREALARRAY(cappa(i, j, k))
                temp12 = r_vir*q(i, j, k, sphum) + 1.
                temp11 = cvm(i)/temp12
                temp_ad8 = -(rdgas*cappa_ad(i, j, k)/((rdgas+temp11)**2*&
&                 temp12))
                cvm_ad(i) = cvm_ad(i) + temp_ad8
                q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) - temp11*&
&                 r_vir*temp_ad8
                cappa_ad(i, j, k) = 0.0
                CALL POPREALARRAY(q_con(i, j, k))
                gz_ad(i) = gz_ad(i) + q_con_ad(i, j, k)
                q_con_ad(i, j, k) = 0.0
              END DO
              CALL MOIST_CV_BWD(is, ie, isd, ied, jsd, jed, km, j, k, &
&                         nwat, sphum, liq_wat, rainwat, ice_wat, &
...
&                         cvm_ad)
            ELSE
              ...
              DO i=ad_to4,ad_from2,-1
                CALL POPREALARRAY(pt(i, j, k))
                temp10 = -cappa(i, j, k) + 1.
                temp5 = cappa(i, j, k)/temp10
                temp9 = delz(i, j, k)
                temp8 = delp(i, j, k)*pt(i, j, k)
                temp4 = temp8/temp9
                temp7 = rrg*temp4
                temp6 = LOG(temp7)
                temp_ad1 = pt(i, j, k)*EXP(temp6*temp5)*pt_ad(i, j, k)
                temp_ad2 = rrg*temp5*temp_ad1/(temp7*temp9)
                temp_ad3 = temp6*temp_ad1/temp10
                delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*&
&                 temp_ad2
                delz_ad(i, j, k) = delz_ad(i, j, k) - temp4*temp_ad2
                cappa_ad(i, j, k) = cappa_ad(i, j, k) + (temp5+1.0)*&
&                 temp_ad3
                pt_ad(i, j, k) = delp(i, j, k)*temp_ad2 + EXP(temp6*&
&                 temp5)*pt_ad(i, j, k)
                CALL POPREALARRAY(cappa(i, j, k))
                temp3 = r_vir*qv(i) + 1.
                temp2 = cvm(i)/temp3
                temp_ad4 = -(rdgas*cappa_ad(i, j, k)/((rdgas+temp2)**2*&
&                 temp3))
                cvm_ad(i) = cvm_ad(i) + temp_ad4
                qv_ad(i) = qv_ad(i) + (cv_vap-cv_air)*cvm_ad(i) - temp2*&
&                 r_vir*temp_ad4
                cappa_ad(i, j, k) = 0.0
                CALL POPREALARRAY(cvm(i))
                cvm_ad(i) = 0.0
      ...
      ELSE IF (branch .NE. 2) THEN
        IF (branch .EQ. 3) THEN
          ...
          DO k=ad_to8,1,-1
            ...
            DO i=ad_to7,ad_from4,-1
              CALL POPREALARRAY(te(i, j, k))
              temp_ad9 = gridstruct%rsin2(i, j)*0.25*te_ad(i, j, k)
              temp_ad10 = -(gridstruct%cosa_s(i, j)*temp_ad9)
              temp_ad11 = (v(i, j, k)+v(i+1, j, k))*temp_ad10
              temp_ad12 = (u(i, j, k)+u(i, j+1, k))*temp_ad10
              u_ad(i, j, k) = u_ad(i, j, k) + temp_ad11 + 2*u(i, j, k)*&
&               temp_ad9
              u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp_ad11 + 2*u(i, j+1&
&               , k)*temp_ad9
              v_ad(i, j, k) = v_ad(i, j, k) + temp_ad12 + 2*v(i, j, k)*&
&               temp_ad9
              v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp_ad12 + 2*v(i+1, j&
&               , k)*temp_ad9
              pt_ad(i, j, k) = pt_ad(i, j, k) + cp_air*pkz(i, j, k)*&
&               te_ad(i, j, k)
              pkz_ad(i, j, k) = pkz_ad(i, j, k) + cp_air*pt(i, j, k)*&
...
&                   , j, k)))
                END DO
              ELSE
                CALL MOIST_CV(is, ie, isd, ied, jsd, jed, km, j, k, nwat&
            ...
            ELSE
              ...
              DO k=1,km
                IF (nwat .EQ. 2) THEN
                  ...
                  END DO
                ELSE
                  CALL MOIST_CV(is, ie, isd, ied, jsd, jed, km, j, k, &
&                         nwat, sphum, liq_wat, rainwat, ice_wat, &
&                         snowwat, graupel, q, gz, cvm)
                END IF
                DO i=is,ie
...
&                 *q(i, j, k, sphum))*(1.-gz(i)))
            ...
            ELSE
              CALL MOIST_CV(is, ie, isd, ied, jsd, jed, km, j, k, nwat, &
&                     sphum, liq_wat, rainwat, ice_wat, snowwat, graupel&
&                     , q, gz, cvm)
              DO i=is,ie
                pt(i, j, k) = (pt(i, j, k)+dtmp*pkz(i, j, k))/((1.+r_vir&
&                 *q(i, j, k, sphum))*(1.-gz(i)))
...
!d.Rayleigh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_t
!o_geos fv_grid_utils_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.Lagrangian_to_Euleria
!n fv_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_map
!z_mod.mapn_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz
!_mod.cs_limiters fv_mapz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm fv_
!mapz_mod.moist_cv fv_mapz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer2d_
!mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_uti
...
!-----------------
        ...
        IF (moist_phys) THEN
          DO k=1,km
            CALL MOIST_CV_FWD(is, ie, isd, ied, jsd, jed, km, j, k, nwat&
&                       , sphum, liq_wat, rainwat, ice_wat, snowwat, &
&                       graupel, q, qd, cvm)
            DO i=is,ie
              te_2d(i, j) = te_2d(i, j) + delp(i, j, k)*(cvm(i)*pt(i, j&
...
!od.Rayleigh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_
!to_geos fv_grid_utils_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.Lagrangian_to_Euleri
!an fv_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_ma
!pz_mod.mapn_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_map
!z_mod.cs_limiters fv_mapz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm fv
!_mapz_mod.moist_cv fv_mapz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer2d
!_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_ut
      ...
      ELSE IF (branch .EQ. 1) THEN
        DO k=km,1,-1
          DO i=ie,is,-1
            ...
            delp_ad(i, j, k) = delp_ad(i, j, k) + (cvm(i)*pt(i, j, k)+&
&             0.5*(phiz(i, k)+phiz(i, k+1)+w(i, j, k)**2+temp2*(u(i, j, &
&             k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-&
&             cosa_s_l(i, j)*(temp3*temp4))))*te_2d_ad(i, j)
            cvm_ad(i) = cvm_ad(i) + pt(i, j, k)*temp_ad3
            pt_ad(i, j, k) = pt_ad(i, j, k) + cvm(i)*temp_ad3
...
&             , j, k)*temp_ad5
          END DO
          CALL MOIST_CV_BWD(is, ie, isd, ied, jsd, jed, km, j, k, nwat, &
&                     sphum, liq_wat, rainwat, ice_wat, snowwat, graupel&
&                     , q, q_ad, qd, qd_ad, cvm, cvm_ad)
        END DO
      ELSE
        DO k=km,1,-1
          DO i=ie,is,-1
...
!mpute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_trace
!r_fb fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters
! fv_mapz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm fv_mapz_mod.moist_c
!v fv_mapz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer2d_mod.tracer_2d fv
!_tracer2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_utils_mod.Riem_Solv
...
!ompute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_trac
!er_fb fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiter
!s fv_mapz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm fv_mapz_mod.moist_
!cv fv_mapz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer2d_mod.tracer_2d f
!v_tracer2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_utils_mod.Riem_Sol
...
!ics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid_utils_mod.
...
!al_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_m
!apz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_mapz_
...
!ill3_4corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.compute
!_divergence_damping_fb sw_core_mod.smag_corner tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mod.copy_corners tp
!_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_dist
! sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: pe1 pe2 qs q2
!   with respect to varying inputs: pe1 pe2 qs q2
  SUBROUTINE MAP_SCALAR_ADM(km, pe1, pe1_ad, qs, qs_ad, kn, pe2, pe2_ad&
&   , q2, q2_ad, i1, i2, j, ibeg, iend, jbeg, jend, iv, kord, q_min)
    IMPLICIT NONE
! iv=1
! Starting longitude
    INTEGER, INTENT(IN) :: i1
...
&         i, l)
 130    CALL POPINTEGER4(ad_count)
    ...
    ELSE
      ...
      CALL SCALAR_PROFILE_ADM(qs, qs_ad, q4, q4_ad, dp1, dp1_ad, km, i1&
&                       , i2, iv, kord, q_min)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
...
!s_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid_utils_mod.cu
...
!_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_map
!z_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_mapz_mo
...
!l3_4corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.compute_d
!ivergence_damping_fb sw_core_mod.smag_corner tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mod.copy_corners tp_c
!ore_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_dist s
!w_core_mod.edge_interpolate4)):
!   gradient     of useful results: pe1 pe2 qs q2
!   with respect to varying inputs: pe1 pe2 qs q2
...
!mics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid_utils_mod
...
!tal_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_
!mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_mapz
...
&         i, l)
 140    CALL POPINTEGER4(ad_count)
    ...
    DO iq=nq,1,-1
      ...
      qs_ad = 0.0
      CALL SCALAR_PROFILE_ADM(qs, qs_ad, q4(1:4, i1:i2, 1:km, iq), q4_ad&
&                       (1:4, i1:i2, 1:km, iq), dp1, dp1_ad, km, i1, i2&
&                       , 0, kord(iq), q_min)
      DO k=km,1,-1
        DO i=i2,i1,-1
...
!_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid_utils_mod.cub
...
!energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz
!_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_mapz_mod
...
!3_4corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.compute_di
!vergence_damping_fb sw_core_mod.smag_corner tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mod.copy_corners tp_co
!re_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_dist sw
...
!ynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid_utils_
...
!_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer 
!fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_m
...
!   with respect to varying inputs: qs delp a4
  SUBROUTINE SCALAR_PROFILE_ADM(qs, qs_ad, a4, a4_ad, delp, delp_ad, km&
&   , i1, i2, iv, kord, qmin)
    IMPLICIT NONE
...
!  Differentiation of moist_cv in reverse (adjoint) mode, forward sweep (with options split(a2b_edge_mod.a2b_ord4 a2b_edge_mod
...
!   gradient     of useful results: qd cvm q
!   with respect to varying inputs: qd cvm q
  SUBROUTINE MOIST_CV_FWD(is, ie, isd, ied, jsd, jed, km, j, k, nwat, &
&   sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, q, qd, cvm, t1)
    IMPLICIT NONE
...
&         + qs(i)*c_ice
    ...
    CASE DEFAULT
      ...
      CALL PUSHCONTROL(3,5)
    END SELECT
  END SUBROUTINE MOIST_CV_FWD
!  Differentiation of moist_cv in reverse (adjoint) mode, backward sweep (with options split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
...
!   gradient     of useful results: qd cvm q
!   with respect to varying inputs: qd cvm q
  SUBROUTINE MOIST_CV_BWD(is, ie, isd, ied, jsd, jed, km, j, k, nwat, &
&   sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, q, q_ad, qd, &
&   qd_ad, cvm, cvm_ad, t1)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: is, ie, isd, ied, jsd, jed, km, nwat, j, k
    INTEGER, INTENT(IN) :: sphum, liq_wat, rainwat, ice_wat, snowwat, &
&   graupel
    REAL, DIMENSION(isd:ied, jsd:jed, km, nwat), INTENT(IN) :: q
    REAL, DIMENSION(isd:ied, jsd:jed, km, nwat) :: q_ad
    REAL, DIMENSION(is:ie) :: cvm, qd
    REAL, DIMENSION(is:ie) :: cvm_ad, qd_ad
    REAL, INTENT(IN), OPTIONAL :: t1(is:ie)
    REAL, PARAMETER :: t_i0=15.
    REAL, DIMENSION(is:ie) :: qv, ql, qs
    REAL, DIMENSION(is:ie) :: qv_ad, ql_ad, qs_ad
    INTEGER :: i
    INTRINSIC PRESENT
    INTRINSIC MAX
    INTEGER :: branch
    ...
    IF (branch .LT. 3) THEN
      IF (branch .EQ. 0) THEN
        ...
        DO i=ie,is,-1
          CALL POPREALARRAY(cvm(i))
          qd_ad(i) = qd_ad(i) - cv_air*cvm_ad(i)
          qv_ad(i) = qv_ad(i) + (cv_vap-cv_air)*cvm_ad(i)
          ql_ad(i) = ql_ad(i) + c_liq*cvm_ad(i)
          qs_ad(i) = qs_ad(i) + c_ice*cvm_ad(i)
          cvm_ad(i) = 0.0
          CALL POPCONTROL(1,branch)
          IF (branch .EQ. 0) THEN
            q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) + qv_ad(i)
          ...
          ELSE
            qv_ad(i) = 0.0
          END IF
          qd_ad(i) = qd_ad(i) + ql_ad(i)
          qs_ad(i) = qs_ad(i) - ql_ad(i)
          ql_ad(i) = 0.0
          CALL POPCONTROL(2,branch)
          IF (branch .EQ. 0) THEN
            qd_ad(i) = qd_ad(i) + (tice-t1(i))*qs_ad(i)/t_i0
            qs_ad(i) = 0.0
          ELSE IF (branch .EQ. 1) THEN
            qd_ad(i) = qd_ad(i) + qs_ad(i)
            qs_ad(i) = 0.0
          ELSE
            qs_ad(i) = 0.0
          END IF
          CALL POPCONTROL(1,branch)
          IF (branch .EQ. 0) THEN
            CALL POPREALARRAY(qd(i))
            q_ad(i, j, k, liq_wat) = q_ad(i, j, k, liq_wat) + qd_ad(i)
            qd_ad(i) = 0.0
          ELSE
            CALL POPREALARRAY(qd(i))
            qd_ad(i) = 0.0
      ...
      ELSE IF (branch .EQ. 1) THEN
        ...
        DO i=ie,is,-1
          ...
          cvm_ad(i) = 0.0
          CALL POPREALARRAY(qd(i))
          qs_ad(i) = qs_ad(i) + qd_ad(i)
          qd_ad(i) = 0.0
          CALL POPCONTROL(1,branch)
          IF (branch .EQ. 0) THEN
            q_ad(i, j, k, liq_wat) = q_ad(i, j, k, liq_wat) + qs_ad(i)
            qs_ad(i) = 0.0
          ELSE
            qs_ad(i) = 0.0
          END IF
      ...
      ELSE
        ...
        DO i=ie,is,-1
          ...
          qd_ad(i) = qd_ad(i) - cv_air*cvm_ad(i)
          qv_ad(i) = qv_ad(i) + (cv_vap-cv_air)*cvm_ad(i)
          ql_ad(i) = ql_ad(i) + qd_ad(i) + c_liq*cvm_ad(i)
          qs_ad(i) = qs_ad(i) + qd_ad(i) + c_ice*cvm_ad(i)
          cvm_ad(i) = 0.0
          CALL POPREALARRAY(qd(i))
          qd_ad(i) = 0.0
          q_ad(i, j, k, ice_wat) = q_ad(i, j, k, ice_wat) + qs_ad(i)
          qs_ad(i) = 0.0
          q_ad(i, j, k, liq_wat) = q_ad(i, j, k, liq_wat) + ql_ad(i)
          ql_ad(i) = 0.0
          q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) + qv_ad(i)
          qv_ad(i) = 0.0
        END DO
    ...
    ELSE IF (branch .EQ. 3) THEN
      ...
      DO i=ie,is,-1
        CALL POPREALARRAY(cvm(i))
        qd_ad(i) = qd_ad(i) + (c_liq-cv_air)*cvm_ad(i)
        qv_ad(i) = qv_ad(i) + (cv_vap-cv_air)*cvm_ad(i)
        cvm_ad(i) = 0.0
        CALL POPREALARRAY(qd(i))
        q_ad(i, j, k, liq_wat) = q_ad(i, j, k, liq_wat) + qd_ad(i)
        q_ad(i, j, k, rainwat) = q_ad(i, j, k, rainwat) + qd_ad(i)
        qd_ad(i) = 0.0
        q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) + qv_ad(i)
        qv_ad(i) = 0.0
      END DO
    ELSE IF (branch .EQ. 4) THEN
      ...
      DO i=ie,is,-1
        ...
        qd_ad(i) = 0.0
        q_ad(i, j, k, ice_wat) = q_ad(i, j, k, ice_wat) + qs_ad(i)
        q_ad(i, j, k, snowwat) = q_ad(i, j, k, snowwat) + qs_ad(i)
        q_ad(i, j, k, graupel) = q_ad(i, j, k, graupel) + qs_ad(i)
        qs_ad(i) = 0.0
        q_ad(i, j, k, liq_wat) = q_ad(i, j, k, liq_wat) + ql_ad(i)
        q_ad(i, j, k, rainwat) = q_ad(i, j, k, rainwat) + ql_ad(i)
        ql_ad(i) = 0.0
        q_ad(i, j, k, sphum) = q_ad(i, j, k, sphum) + qv_ad(i)
        qv_ad(i) = 0.0
      END DO
...
!  Differentiation of moist_cp in reverse (adjoint) mode, forward sweep (with options split(a2b_edge_mod.a2b_ord4 a2b_edge_mod
!.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mo
!d.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix
!_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_S
!uper fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_gr
...
!corners_fb tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.grea
!t_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: qd q
!   with respect to varying inputs: qd q
  SUBROUTINE MOIST_CP_FWD(is, ie, isd, ied, jsd, jed, km, j, k, nwat, &
&   sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, q, qd, cpm, t1)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: is, ie, isd, ied, jsd, jed, km, nwat, j, k
    INTEGER, INTENT(IN) :: sphum, liq_wat, rainwat, ice_wat, snowwat, &
&   graupel
    REAL, DIMENSION(isd:ied, jsd:jed, km, nwat), INTENT(IN) :: q
    REAL, DIMENSION(is:ie) :: cpm, qd
    REAL, INTENT(IN), OPTIONAL :: t1(is:ie)
!
    REAL, PARAMETER :: t_i0=15.
    REAL, DIMENSION(is:ie) :: qv, ql, qs
...
&         graupel)
    ...
    CASE DEFAULT
      ...
      CALL PUSHCONTROL(3,5)
    END SELECT
  END SUBROUTINE MOIST_CP_FWD
!  Differentiation of moist_cp in reverse (adjoint) mode, backward sweep (with options split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
!d.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_m
!od.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mi
!x_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_
!Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_g
...
!   gradient     of useful results: qd q
!   with respect to varying inputs: qd q
  SUBROUTINE MOIST_CP_BWD(is, ie, isd, ied, jsd, jed, km, j, k, nwat, &
&   sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, q, q_ad, qd, &
&   qd_ad, cpm, t1)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: is, ie, isd, ied, jsd, jed, km, nwat, j, k
...
&   graupel
    ...
    REAL, DIMENSION(isd:ied, jsd:jed, km, nwat) :: q_ad
    REAL, DIMENSION(is:ie) :: cpm, qd
    REAL, DIMENSION(is:ie) :: qd_ad
    REAL, INTENT(IN), OPTIONAL :: t1(is:ie)
    REAL, PARAMETER :: t_i0=15.
    REAL, DIMENSION(is:ie) :: qv, ql, qs
    REAL, DIMENSION(is:ie) :: ql_ad, qs_ad
    INTEGER :: i
    INTRINSIC PRESENT
    INTRINSIC MAX
    INTEGER :: branch
...
!   with respect to varying inputs: pe1 pe2 qs q2
!-----------------------------------------------------------------------
  SUBROUTINE MAP_SCALAR_BWD(km, pe1, pe1_ad, qs, qs_ad, kn, pe2, &
&   pe2_ad, q2, q2_ad, i1, i2, j, ibeg, iend, jbeg, jend, iv, kord, &
&   q_min)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i1
    INTEGER, INTENT(IN) :: i2
...
&         i, l)
 100    CALL POPINTEGER(ad_count)
    ...
    END DO
    CALL POPCONTROL(1,branch)
    IF (branch .NE. 0) CALL SCALAR_PROFILE_BWD(qs, qs_ad, q4, q4_ad, &
&                                           dp1, dp1_ad, km, i1, i2, iv&
&                                           , kord, q_min)
    DO k=km,1,-1
      DO i=i2,i1,-1
        q2_ad(i, j, k) = q2_ad(i, j, k) + q4_ad(1, i, k)
...
&         i, l)
 100    CALL POPINTEGER(ad_count)
    ...
    DO iq=nq,1,-1
      qs_ad = 0.0
      CALL SCALAR_PROFILE_BWD(qs, qs_ad, q4(1:4, i1:i2, 1:km, iq), &
&                          q4_ad(1:4, i1:i2, 1:km, iq), dp1, dp1_ad, km&
&                          , i1, i2, 0, kord(iq), q_min)
      DO k=km,1,-1
        DO i=i2,i1,-1
          q1_ad(i, j, k, iq) = q1_ad(i, j, k, iq) + q4_ad(1, i, k, iq)
...
&         i, l)
 100    CALL POPINTEGER(ad_count)
    ...
    IF (branch .NE. 0) THEN
      qs_ad = 0.0
      CALL SCALAR_PROFILE_BWD(qs, qs_ad, q4, q4_ad, dp1, dp1_ad, km, &
&                          i1, i2, iv, kord, q_min)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
...
!   gradient     of useful results: qs delp a4
!   with respect to varying inputs: qs delp a4
  SUBROUTINE SCALAR_PROFILE_BWD(qs, qs_ad, a4, a4_ad, delp, delp_ad, &
&   km, i1, i2, iv, kord, qmin)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i1, i2
    INTEGER, INTENT(IN) :: km
    INTEGER, INTENT(IN) :: iv
