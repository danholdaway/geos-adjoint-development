...
!***********************************************************************
MODULE SW_CORE_MOD_DIFF
  USE FV_MP_MOD_DIFF, ONLY : ng
...
& copy_corners_adm
  ...
  USE FV_ARRAYS_MOD_DIFF, ONLY : fv_grid_type, fv_grid_bounds_type, &
& fv_flags_type, fvprc
  USE A2B_EDGE_MOD_DIFF, ONLY : a2b_ord4, a2b_ord4_adm, a2b_ord4, &
& a2b_ord4_adm
...
!ers_fb tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_ci
!rcle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: yfx_adv q_con q crx_adv u v
!                w delp ua xfx_adv uc ptc xflux cry_adv delpc va
!                vc yflux divg_d z_rat heat_source pt cx cy dpx
!   with respect to varying inputs: yfx_adv q_con q crx_adv u v
!                w delp ua xfx_adv uc ptc xflux cry_adv delpc va
!                vc yflux divg_d z_rat heat_source pt cx cy dpx
...
&           gy(i, j+1)))*rarea(i, j)
    ...
    END IF
    IF (hord_dp .EQ. hord_dp_pert .AND. (.NOT.split_damp)) THEN
      CALL FV_TP_2D_FWD(q_con, crx_adv, cry_adv, npx, npy, hord_dp, &
&                    gx, gy, xfx_adv, yfx_adv, gridstruct, bd, ra_x, &
&                    ra_y, mfx=fx, mfy=fy, mass=delp, nord=nord_t, &
&                    damp_c=damp_t)
      CALL PUSHCONTROL(1,1)
    ELSE
      CALL PUSHREALARRAY(gy, (bd%ie-bd%is+1)*(bd%je-bd%js+2))
      CALL PUSHREALARRAY(gx, (bd%ie-bd%is+2)*(bd%je-bd%js+1))
      CALL PUSHREALARRAY(q_con, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL FV_TP_2D(q_con, crx_adv, cry_adv, npx, npy, hord_dp_pert, gx&
&             , gy, xfx_adv, yfx_adv, gridstruct, bd, ra_x, ra_y, mfx=fx&
&             , mfy=fy, mass=delp, nord=nord_t_pert, damp_c=damp_t_pert)
    ...
    DO j=js,je
      DO i=is,ie
        CALL PUSHREALARRAY(q_con(i, j))
        q_con(i, j) = delp(i, j)*q_con(i, j) + (gx(i, j)-gx(i+1, j)+(gy(&
&         i, j)-gy(i, j+1)))*rarea(i, j)
      END DO
    END DO
...
&   , xfx_adv_ad, yfx_adv, yfx_adv_ad, q_con, q_con_ad, z_rat, z_rat_ad&
&   , kgb, heat_source, heat_source_ad, dpx, dpx_ad, zvir, sphum, nq, q&
&   , q_ad, k, km, inline_q, dt, hord_tr, hord_mt, hord_vt, hord_tm, &
&   hord_dp, nord, nord_v, nord_w, nord_t, dddmp, d2_bg, d4_bg, damp_v, &
&   damp_w, damp_t, d_con, hydrostatic, gridstruct, flagstruct, bd, &
    ...
    IF (branch .EQ. 0) THEN
      ...
      DO j=je,js,-1
        DO i=ie,is,-1
          ...
          temp_ad90 = delp(i, j)*heat_source_ad(i, j)
          temp_ad91 = -(temp0*temp_ad90)
          temp_ad92 = 2.*temp_ad91
          temp_ad93 = -(cosa_s(i, j)*temp_ad91)
...
&           +du2*dv2)))*heat_source_ad(i, j)
          ub_ad(i, j) = ub_ad(i, j) + 2*ub(i, j)*temp_ad91
          ub_ad(i, j+1) = ub_ad(i, j+1) + 2*ub(i, j+1)*temp_ad91
          vb_ad(i, j) = vb_ad(i, j) + 2*vb(i, j)*temp_ad91
          vb_ad(i+1, j) = vb_ad(i+1, j) + 2*vb(i+1, j)*temp_ad91
          gy_ad(i, j) = gy_ad(i, j) + temp_ad92
          gy_ad(i, j+1) = gy_ad(i, j+1) + temp_ad92
          gx_ad(i, j) = gx_ad(i, j) + temp_ad92
          gx_ad(i+1, j) = gx_ad(i+1, j) + temp_ad92
          u2_ad = dv2*temp_ad93
          dv2_ad = (du2+u2)*temp_ad93
          v2_ad = du2*temp_ad93
          du2_ad = (dv2+v2)*temp_ad93
          heat_source_ad(i, j) = temp_ad90
      ...
      DO j=je,js,-1
        DO i=ie+1,is,-1
          ...
          temp_ad89 = rdy(i, j)*vb_ad(i, j)
          ut_ad(i, j) = ut_ad(i, j) - temp_ad89
          vb_ad(i, j) = temp_ad89
        END DO
      END DO
      DO j=je+1,js,-1
...
&                                   , bd)
    ...
    DO j=je,js,-1
      DO i=ie,is,-1
        CALL POPREALARRAY(q_con(i, j))
        temp_ad87 = q_con_ad(i, j)/delp(i, j)
        delp_ad(i, j) = delp_ad(i, j) - q_con(i, j)*temp_ad87/delp(i, j)
        q_con_ad(i, j) = temp_ad87
      END DO
    END DO
    CALL POPCONTROL(2,branch)
    IF (branch .EQ. 0) THEN
    ...
    ELSE
      ...
      IF (branch .NE. 0) THEN
        DO j=je,js,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(w(i, j))
            temp_ad86 = w_ad(i, j)/delp(i, j)
            delp_ad(i, j) = delp_ad(i, j) - w(i, j)*temp_ad86/delp(i, j)
            w_ad(i, j) = temp_ad86
          END DO
        END DO
      END IF
    ...
    DO j=jed,jsd,-1
      DO i=ied,isd,-1
        ...
        temp_ad85 = rarea(i, j)*wk_ad(i, j)
        vt_ad(i, j) = vt_ad(i, j) + temp_ad85
        vt_ad(i, j+1) = vt_ad(i, j+1) - temp_ad85
        ut_ad(i+1, j) = ut_ad(i+1, j) + temp_ad85
        ut_ad(i, j) = ut_ad(i, j) - temp_ad85
        wk_ad(i, j) = 0.0
      END DO
    END DO
    DO j=jed,jsd,-1
    ...
    IF (branch .NE. 0) THEN
      IF (branch .NE. 1) THEN
        dt6 = dt/6.
        temp_ad81 = dt6*ke_ad(1, npy)
        temp_ad82 = u(1, npy)*temp_ad81
        temp_ad83 = v(1, npy-1)*temp_ad81
        temp_ad84 = u(0, npy)*temp_ad81
        ut_ad(1, npy) = ut_ad(1, npy) + temp_ad82
        ut_ad(1, npy-1) = ut_ad(1, npy-1) + temp_ad84 + temp_ad82
        u_ad(1, npy) = u_ad(1, npy) + (ut(1, npy)+ut(1, npy-1))*&
&         temp_ad81
        vt_ad(1, npy) = vt_ad(1, npy) + temp_ad83 - temp_ad84
        vt_ad(0, npy) = vt_ad(0, npy) + temp_ad83
...
&         temp_ad81
      ...
      CALL POPCONTROL(1,branch)
      IF (branch .EQ. 0) THEN
        temp_ad77 = dt6*ke_ad(npx, npy)
        temp_ad78 = u(npx-1, npy)*temp_ad77
        temp_ad79 = v(npx, npy-1)*temp_ad77
        temp_ad80 = u(npx, npy)*temp_ad77
        ut_ad(npx, npy) = ut_ad(npx, npy) + temp_ad78
        ut_ad(npx, npy-1) = ut_ad(npx, npy-1) + temp_ad80 + temp_ad78
        u_ad(npx-1, npy) = u_ad(npx-1, npy) + (ut(npx, npy)+ut(npx, npy-&
&         1))*temp_ad77
        vt_ad(npx, npy) = vt_ad(npx, npy) + temp_ad79
        vt_ad(npx-1, npy) = vt_ad(npx-1, npy) + temp_ad80 + temp_ad79
...
&         )*temp_ad77
      ...
      CALL POPCONTROL(1,branch)
      IF (branch .EQ. 0) THEN
        temp_ad73 = dt6*ke_ad(npx, 1)
        temp_ad74 = u(npx-1, 1)*temp_ad73
        temp_ad75 = v(npx, 1)*temp_ad73
        temp_ad76 = u(npx, 1)*temp_ad73
        ut_ad(npx, 1) = ut_ad(npx, 1) + temp_ad76 + temp_ad74
        ut_ad(npx, 0) = ut_ad(npx, 0) + temp_ad74
        u_ad(npx-1, 1) = u_ad(npx-1, 1) + (ut(npx, 1)+ut(npx, 0))*&
&         temp_ad73
        vt_ad(npx, 1) = vt_ad(npx, 1) + temp_ad75
        vt_ad(npx-1, 1) = vt_ad(npx-1, 1) + temp_ad75 - temp_ad76
...
&         temp_ad73
      ...
      CALL POPCONTROL(1,branch)
      IF (branch .EQ. 0) THEN
        temp_ad72 = dt6*ke_ad(1, 1)
        ut_ad(1, 1) = ut_ad(1, 1) + (u(0, 1)+u(1, 1))*temp_ad72
        ut_ad(1, 0) = ut_ad(1, 0) + u(1, 1)*temp_ad72
        u_ad(1, 1) = u_ad(1, 1) + (ut(1, 1)+ut(1, 0))*temp_ad72
        vt_ad(1, 1) = vt_ad(1, 1) + (u(0, 1)+v(1, 1))*temp_ad72
        vt_ad(0, 1) = vt_ad(0, 1) + v(1, 1)*temp_ad72
        v_ad(1, 1) = v_ad(1, 1) + (vt(1, 1)+vt(0, 1))*temp_ad72
        u_ad(0, 1) = u_ad(0, 1) + (ut(1, 1)+vt(1, 1))*temp_ad72
        ke_ad(1, 1) = 0.0
      END IF
    END IF
    nested = gridstruct%nested
    DO j=je+1,js,-1
      DO i=ie+1,is,-1
        temp_ad71 = 0.5*ke_ad(i, j)
        ub_ad(i, j) = ub_ad(i, j) + vb(i, j)*temp_ad71
        vb_ad(i, j) = vb_ad(i, j) + ub(i, j)*temp_ad71
        ke_ad(i, j) = temp_ad71
      END DO
    END DO
    CALL POPCONTROL(1,branch)
...
&                )
    ...
    IF (branch .LT. 2) THEN
      IF (branch .EQ. 0) THEN
        DO j=je+1,js,-1
          DO i=ie1,is2,-1
            CALL POPREALARRAY(ub(i, j))
            temp_ad66 = dt5*rsina(i, j)*ub_ad(i, j)
            temp_ad67 = -(cosa(i, j)*temp_ad66)
            uc_ad(i, j-1) = uc_ad(i, j-1) + temp_ad66
            uc_ad(i, j) = uc_ad(i, j) + temp_ad66
            vc_ad(i-1, j) = vc_ad(i-1, j) + temp_ad67
            vc_ad(i, j) = vc_ad(i, j) + temp_ad67
            ub_ad(i, j) = 0.0
          END DO
        END DO
        GOTO 100
    ...
    DO j=je+1,js,-1
      ...
      IF (branch .EQ. 0) THEN
        DO i=ie1,is2,-1
          CALL POPREALARRAY(ub(i, j))
          temp_ad69 = dt5*rsina(i, j)*ub_ad(i, j)
          temp_ad70 = -(cosa(i, j)*temp_ad69)
          uc_ad(i, j-1) = uc_ad(i, j-1) + temp_ad69
          uc_ad(i, j) = uc_ad(i, j) + temp_ad69
          vc_ad(i-1, j) = vc_ad(i-1, j) + temp_ad70
          vc_ad(i, j) = vc_ad(i, j) + temp_ad70
          ub_ad(i, j) = 0.0
        END DO
      ELSE
        DO i=ie1,is2,-1
          CALL POPREALARRAY(ub(i, j))
          temp_ad68 = dt4*ub_ad(i, j)
          ut_ad(i, j-1) = ut_ad(i, j-1) + 3.*temp_ad68
          ut_ad(i, j) = ut_ad(i, j) + 3.*temp_ad68
          ut_ad(i, j-2) = ut_ad(i, j-2) - temp_ad68
          ut_ad(i, j+1) = ut_ad(i, j+1) - temp_ad68
          ub_ad(i, j) = 0.0
        END DO
      END IF
    END DO
...
&                )
    ...
    DO j=je1,js2,-1
      CALL POPCONTROL(1,branch)
      IF (branch .NE. 0) THEN
        temp_ad65 = dt4*vb_ad(npx, j)
        vt_ad(npx-1, j) = vt_ad(npx-1, j) + 3.*temp_ad65
        vt_ad(npx, j) = vt_ad(npx, j) + 3.*temp_ad65
        vt_ad(npx-2, j) = vt_ad(npx-2, j) - temp_ad65
        vt_ad(npx+1, j) = vt_ad(npx+1, j) - temp_ad65
        vb_ad(npx, j) = 0.0
      END IF
      CALL POPCONTROL(1,branch)
      IF (branch .EQ. 0) THEN
        temp_ad64 = dt4*vb_ad(1, j)
        vt_ad(0, j) = vt_ad(0, j) + 3.*temp_ad64
        vt_ad(1, j) = vt_ad(1, j) + 3.*temp_ad64
        vt_ad(-1, j) = vt_ad(-1, j) - temp_ad64
        vt_ad(2, j) = vt_ad(2, j) - temp_ad64
        vb_ad(1, j) = 0.0
      END IF
      DO i=ie1,is2,-1
        temp_ad62 = dt5*rsina(i, j)*vb_ad(i, j)
        temp_ad63 = -(cosa(i, j)*temp_ad62)
        vc_ad(i-1, j) = vc_ad(i-1, j) + temp_ad62
        vc_ad(i, j) = vc_ad(i, j) + temp_ad62
        uc_ad(i, j-1) = uc_ad(i, j-1) + temp_ad63
        uc_ad(i, j) = uc_ad(i, j) + temp_ad63
        vb_ad(i, j) = 0.0
      END DO
    END DO
    CALL POPCONTROL(1,branch)
 ...
 110 CALL POPCONTROL(1,branch)
    ...
    IF (branch .EQ. 0) THEN
      DO j=je,js,-1
        DO i=ie,is,-1
          temp_ad59 = rarea(i, j)*dpx_ad(i, j)
          fx_ad(i, j) = fx_ad(i, j) + temp_ad59
          fx_ad(i+1, j) = fx_ad(i+1, j) - temp_ad59
          fy_ad(i, j) = fy_ad(i, j) + temp_ad59
          fy_ad(i, j+1) = fy_ad(i, j+1) - temp_ad59
        END DO
      END DO
    END IF
    CALL POPCONTROL(1,branch)
      ...
      DO iq=nq,1,-1
        DO j=je,js,-1
          DO i=ie,is,-1
            ...
            temp_ad54 = q_ad(i, j, k, iq)/delp(i, j)
            temp = q(i, j, k, iq)
            temp_ad55 = rarea(i, j)*temp_ad54
            wk_ad(i, j) = wk_ad(i, j) + temp*temp_ad54
            gx_ad(i, j) = gx_ad(i, j) + temp_ad55
            gx_ad(i+1, j) = gx_ad(i+1, j) - temp_ad55
            gy_ad(i, j) = gy_ad(i, j) + temp_ad55
            gy_ad(i, j+1) = gy_ad(i, j+1) - temp_ad55
            delp_ad(i, j) = delp_ad(i, j) - (temp*wk(i, j)+rarea(i, j)*(&
&             gx(i, j)-gx(i+1, j)+gy(i, j)-gy(i, j+1)))*temp_ad54/delp(i&
&             , j)
            q_ad(i, j, k, iq) = wk(i, j)*temp_ad54
          END DO
        END DO
        CALL POPCONTROL(1,branch)
        IF (branch .EQ. 0) THEN
...
&                        damp_t)
      ...
      DO j=je,js,-1
        DO i=ie,is,-1
          ...
          temp_ad51 = pt_ad(i, j)/delp(i, j)
          temp_ad52 = rarea(i, j)*temp_ad51
          gx_ad(i, j) = gx_ad(i, j) + temp_ad52
          gx_ad(i+1, j) = gx_ad(i+1, j) - temp_ad52
          gy_ad(i, j) = gy_ad(i, j) + temp_ad52
          gy_ad(i, j+1) = gy_ad(i, j+1) - temp_ad52
          delp_ad(i, j) = delp_ad(i, j) - (pt(i, j)*wk(i, j)+rarea(i, j)&
&           *(gx(i, j)-gx(i+1, j)+gy(i, j)-gy(i, j+1)))*temp_ad51/delp(i&
&           , j)
          wk_ad(i, j) = wk_ad(i, j) + delp_ad(i, j) + pt(i, j)*temp_ad51
          pt_ad(i, j) = wk(i, j)*temp_ad51
          CALL POPREALARRAY(delp(i, j))
          temp_ad53 = rarea(i, j)*delp_ad(i, j)
          fx_ad(i, j) = fx_ad(i, j) + temp_ad53
          fx_ad(i+1, j) = fx_ad(i+1, j) - temp_ad53
          fy_ad(i, j) = fy_ad(i, j) + temp_ad53
          fy_ad(i, j+1) = fy_ad(i, j+1) - temp_ad53
    ...
    ELSE
      DO j=je,js,-1
        DO i=ie,is,-1
          CALL POPREALARRAY(pt(i, j))
          temp_ad56 = pt_ad(i, j)/delp(i, j)
          delp_ad(i, j) = delp_ad(i, j) - pt(i, j)*temp_ad56/delp(i, j)
          pt_ad(i, j) = temp_ad56
          CALL POPREALARRAY(delp(i, j))
          temp_ad57 = rarea(i, j)*delp_ad(i, j)
          fx_ad(i, j) = fx_ad(i, j) + temp_ad57
          fx_ad(i+1, j) = fx_ad(i+1, j) - temp_ad57
          fy_ad(i, j) = fy_ad(i, j) + temp_ad57
          fy_ad(i, j+1) = fy_ad(i, j+1) - temp_ad57
          CALL POPREALARRAY(pt(i, j))
          temp_ad58 = rarea(i, j)*pt_ad(i, j)
          delp_ad(i, j) = delp_ad(i, j) + pt(i, j)*pt_ad(i, j)
          gx_ad(i, j) = gx_ad(i, j) + temp_ad58
          gx_ad(i+1, j) = gx_ad(i+1, j) - temp_ad58
          gy_ad(i, j) = gy_ad(i, j) + temp_ad58
          gy_ad(i, j+1) = gy_ad(i, j+1) - temp_ad58
...
&                 =delp_ad, nord=nord_t_pert, damp_c=damp_t_pert)
    ...
    DO j=je,js,-1
      DO i=ie,is,-1
        CALL POPREALARRAY(q_con(i, j))
        temp_ad50 = rarea(i, j)*q_con_ad(i, j)
        delp_ad(i, j) = delp_ad(i, j) + q_con(i, j)*q_con_ad(i, j)
        gx_ad(i, j) = gx_ad(i, j) + temp_ad50
        gx_ad(i+1, j) = gx_ad(i+1, j) - temp_ad50
        gy_ad(i, j) = gy_ad(i, j) + temp_ad50
        gy_ad(i, j+1) = gy_ad(i, j+1) - temp_ad50
        q_con_ad(i, j) = delp(i, j)*q_con_ad(i, j)
      END DO
    END DO
    CALL POPCONTROL(1,branch)
    IF (branch .EQ. 0) THEN
      CALL POPREALARRAY(q_con, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL POPREALARRAY(gx, (bd%ie-bd%is+2)*(bd%je-bd%js+1))
      CALL POPREALARRAY(gy, (bd%ie-bd%is+1)*(bd%je-bd%js+2))
      CALL FV_TP_2D_ADM(q_con, q_con_ad, crx_adv, crx_adv_ad, cry_adv, &
&                 cry_adv_ad, npx, npy, hord_dp_pert, gx, gx_ad, gy, &
&                 gy_ad, xfx_adv, xfx_adv_ad, yfx_adv, yfx_adv_ad, &
&                 gridstruct, bd, ra_x, ra_x_ad, ra_y, ra_y_ad, mfx=fx, &
&                 mfx_ad=fx_ad, mfy=fy, mfy_ad=fy_ad, mass=delp, mass_ad&
&                 =delp_ad, nord=nord_t_pert, damp_c=damp_t_pert)
    ELSE
      CALL FV_TP_2D_BWD(q_con, q_con_ad, crx_adv, crx_adv_ad, cry_adv&
&                    , cry_adv_ad, npx, npy, hord_dp, gx, gx_ad, gy, &
&                    gy_ad, xfx_adv, xfx_adv_ad, yfx_adv, yfx_adv_ad, &
&                    gridstruct, bd, ra_x, ra_x_ad, ra_y, ra_y_ad, mfx=&
&                    fx, mfx=fx_ad, mfy=fy, mfy=fy_ad, mass=delp, mass=&
&                    delp_ad, nord=nord_t, damp_c=damp_t)
...
&           gy(i, j+1)))*rarea(i, j)
    ...
    END IF
    IF (hord_dp .EQ. hord_dp_pert .AND. (.NOT.split_damp)) THEN
      CALL FV_TP_2D(q_con, crx_adv, cry_adv, npx, npy, hord_dp, gx, &
&                gy, xfx_adv, yfx_adv, gridstruct, bd, ra_x, ra_y, fx, &
&                fy, delp, nord_t, damp_t)
    ELSE
      CALL FV_TP_2D(q_con, crx_adv, cry_adv, npx, npy, hord_dp_pert, gx&
&             , gy, xfx_adv, yfx_adv, gridstruct, bd, ra_x, ra_y, fx, fy&
