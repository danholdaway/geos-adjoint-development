...
!***********************************************************************
MODULE DYN_CORE_MOD_DIFF
  USE CONSTANTS_MOD, ONLY : rdgas, radius, cp_air, pi
  USE MPP_MOD, ONLY : mpp_pe
  USE MPP_DOMAINS_MOD_DIFF, ONLY : cgrid_ne, dgrid_ne, mpp_get_boundary,&
...
& domain2d
  ...
  USE FV_MP_MOD_DIFF, ONLY : start_group_halo_update, &
& start_group_halo_update_adm, complete_group_halo_update
  USE FV_MP_MOD_DIFF, ONLY : group_halo_update_type
  USE SW_CORE_MOD_DIFF, ONLY : c_sw, c_sw_adm, d_sw, d_sw_adm
  USE A2B_EDGE_MOD_DIFF, ONLY : a2b_ord2, a2b_ord2_adm, a2b_ord4, &
& a2b_ord4_adm
  USE NH_CORE_MOD_DIFF, ONLY : riem_solver3, riem_solver3_adm
  USE NH_UTILS_MOD_DIFF, ONLY : riem_solver_c, riem_solver_c_adm, &
& update_dz_c, update_dz_c_adm, update_dz_d, update_dz_d_adm, &
& nest_halo_nh, nest_halo_nh_adm
  USE TP_CORE_MOD_DIFF, ONLY : copy_corners, copy_corners_adm
  USE FV_TIMING_MOD, ONLY : timing_on, timing_off
  USE FV_DIAGNOSTICS_MOD_DIFF, ONLY : prt_maxmin, fv_time, prt_mxm
  USE FV_NWP_NUDGE_MOD_DIFF, ONLY : breed_slp_inline, do_adiabatic_init
  USE DIAG_MANAGER_MOD, ONLY : send_data
  USE FV_ARRAYS_MOD_DIFF, ONLY : fv_grid_type, fv_flags_type, &
& fv_nest_type, fv_diag_type, fv_grid_bounds_type, r_grid
  USE BOUNDARY_MOD_DIFF, ONLY : extrapolation_bc, &
& nested_grid_bc_apply_intt, nested_grid_bc_apply_intt_adm
...
!   gradient     of useful results: pk3 xfx ws cappa q_con peln
!                q gz du u dv v w delp ua uc ptc mfx delz mfy omga
!                ut divgd pkc delpc va vc yfx pkz pe vt pk zh pt
!                cx cy dpx crx cry
!   with respect to varying inputs: pk3 xfx ws cappa q_con peln
!                q gz du u dv v w delp ua uc ptc delz omga ut divgd
!                pkc delpc va vc yfx pkz pe vt pk zh pt dpx crx
!                cry
!-----------------------------------------------------------------------
...
&                         phis, q_con, cappa, pkc, gz, pk3, npx, npy, &
&                         npz, gridstruct%nested, .false., .false., &
&                         .false., bd)
          CALL PUSHCONTROL(2,1)
        ELSE
          CALL PUSHCONTROL(2,2)
...
&           THEN
        ...
        ELSE
          IF (2 .LT. flagstruct%n_sponge - 1) THEN
            max3 = flagstruct%n_sponge - 1
          ELSE
            max3 = 2
          END IF
          IF (k .EQ. max3 .AND. flagstructp%d2_bg_k2_pert .GT. 0.01) &
&         THEN
            nord_k_pert = 0
...
&           THEN
          ...
          ELSE
            IF (3 .LT. flagstruct%n_sponge) THEN
              max4 = flagstruct%n_sponge
            ELSE
              max4 = 3
            END IF
            IF (k .EQ. max4 .AND. flagstructp%d2_bg_k2_pert .GT. 0.05) &
&           THEN
              nord_k_pert = 0
              IF (flagstructp%d2_bg_pert .LT. 0.2*flagstructp%&
&                 d2_bg_k2_pert) THEN
                CALL PUSHCONTROL(3,5)
                d2_divg_pert = 0.2*flagstructp%d2_bg_k2_pert
              ELSE
                CALL PUSHCONTROL(3,5)
                d2_divg_pert = flagstructp%d2_bg_pert
              END IF
...
&                            .true.)
      CALL PUSHREALARRAY(q_con, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz&
&                  )
      CALL START_GROUP_HALO_UPDATE(i_pack(11), q_con, domain, complete=&
&                            .true.)
      IF (flagstruct%d_ext .GT. 0.) THEN
        CALL PUSHREALARRAY(d2_divg)
...
&                                bctype=neststruct%nestbctype)
        arg1 = split_timestep_bc + 1
        arg2 = REAL(n_split*flagstruct%k_split)
        CALL NESTED_GRID_BC_APPLY_INTT(q_con, 0, 0, npx, npy, npz, bd, &
&                                split_timestep_bc + 1, REAL(n_split*&
&                                flagstruct%k_split), neststruct%&
&                                q_con_bc, bctype=neststruct%nestbctype)
        CALL PUSHCONTROL(1,0)
      ELSE
        CALL PUSHCONTROL(1,1)
      END IF
...
&                         phis, q_con, cappa, pkc, gz, pk3, npx, npy, &
&                         npz, gridstruct%nested, .true., .true., .true.&
&                         , bd)
          CALL PUSHCONTROL(1,0)
        ELSE
          CALL PUSHCONTROL(1,1)
...
! Note: pt at this stage is Theta_m
                ...
                pkz(i, j, k) = EXP(cappa(i, j, k)/(1.-cappa(i, j, k))*&
&                 LOG(rdg*delp(i, j, k)/delz(i, j, k)*pt(i, j, k)))
              END DO
            END DO
          END DO
          CALL PUSHCONTROL(2,0)
...
!         if ( k == 2 ) delt = 1.5*delt
          DO j=js,je
            DO i=is,ie
              CALL PUSHREALARRAY(pkz(i, j, k))
              pkz(i, j, k) = EXP(cappa(i, j, k)/(1.-cappa(i, j, k))*LOG(&
&               rdg*delp(i, j, k)/delz(i, j, k)*pt(i, j, k)))
...
&   zvir, cp, akap, cappa, cappa_ad, grav, hydrostatic, u, u_ad, v, v_ad&
&   , w, w_ad, delz, delz_ad, pt, pt_ad, q, q_ad, delp, delp_ad, pe, &
&   pe_ad, pk, pk_ad, phis, ws, ws_ad, omga, omga_ad, ptop, pfull, ua, &
&   ua_ad, va, va_ad, uc, uc_ad, vc, vc_ad, mfx, mfx_ad, mfy, mfy_ad, cx&
&   , cx_ad, cy, cy_ad, pkz, pkz_ad, peln, peln_ad, q_con, q_con_ad, ak&
&   , bk, dpx, dpx_ad, ks, gridstruct, flagstruct, flagstructp, &
&   neststruct, idiag, bd, domain, init_step, i_pack, end_step, gz, &
&   gz_ad, pkc, pkc_ad, ptc, ptc_ad, crx, crx_ad, xfx, xfx_ad, cry, &
&   cry_ad, yfx, yfx_ad, divgd, divgd_ad, delpc, delpc_ad, ut, ut_ad, vt&
&   , vt_ad, zh, zh_ad, pk3, pk3_ad, du, du_ad, dv, dv_ad, time_total)
    IMPLICIT NONE
! end init_step
! Start of the big dynamic time stepping
!allocate(    gz(isd:ied, jsd:jed ,npz+1) )
...
&   :: v_ad
    ...
    REAL, INTENT(INOUT) :: cappa(bd%isd:bd%ied, bd%jsd:bd%jed, npz)
    REAL, INTENT(INOUT) :: cappa_ad(bd%isd:bd%ied, bd%jsd:bd%jed, npz)
    REAL, INTENT(INOUT) :: pt(bd%isd:bd%ied, bd%jsd:bd%jed, npz)
...
&   ua_ad, va_ad
    REAL, INTENT(INOUT) :: q_con(bd%isd:bd%ied, bd%jsd:bd%jed, npz)
    REAL, INTENT(INOUT) :: q_con_ad(bd%isd:bd%ied, bd%jsd:bd%jed, npz)
    REAL, INTENT(INOUT) :: mfx(bd%is:bd%ie+1, bd%js:bd%je, npz)
          ...
          DO k=n_con,1,-1
            ...
            IF (branch .EQ. 0) THEN
              DO i=ie,is,-1
                ...
                min1_ad = SIGN(1.d0, min1*dtmp)*temp_ad7
...
&                 temp_ad7/pkz(i, j, k)
                ...
                ELSE
                  dtmp_ad = -y1_ad
                END IF
                temp7 = cp_air*delp(i, j, k)
                heat_source_ad(i, j, k) = heat_source_ad(i, j, k) + &
&                 dtmp_ad/temp7
                delp_ad(i, j, k) = delp_ad(i, j, k) - heat_source(i, j, &
&                 k)*cp_air*dtmp_ad/temp7**2
              END DO
            ELSE
              DO i=ie,is,-1
                CALL POPREALARRAY(pt(i, j, k))
                temp6 = cp_air*delp(i, j, k)
                temp5 = temp6*pkz(i, j, k)
                temp_ad6 = -(heat_source(i, j, k)*pt_ad(i, j, k)/temp5**&
&                 2)
                heat_source_ad(i, j, k) = heat_source_ad(i, j, k) + &
&                 pt_ad(i, j, k)/temp5
                delp_ad(i, j, k) = delp_ad(i, j, k) + pkz(i, j, k)*&
&                 cp_air*temp_ad6
                pkz_ad(i, j, k) = pkz_ad(i, j, k) + temp6*temp_ad6
              END DO
            END IF
          END DO
        END DO
        ...
        DO k=n_con,1,-1
          DO j=je,js,-1
            DO i=ie,is,-1
              dtmp = heat_source(i, j, k)/(cv_air*delp(i, j, k))
              CALL POPREALARRAY(pt(i, j, k))
              temp_ad11 = pt_ad(i, j, k)/pkz(i, j, k)
              min2_ad = SIGN(1.d0, min2*dtmp)*temp_ad11
...
&               temp_ad11/pkz(i, j, k)
              ...
              ELSE
                dtmp_ad = -y2_ad
              END IF
              temp13 = -cappa(i, j, k) + 1.
              temp9 = cappa(i, j, k)/temp13
              temp12 = delz(i, j, k)
              temp11 = delp(i, j, k)*pt(i, j, k)
              temp8 = temp11/temp12
              temp10 = LOG(rdg*temp8)
              temp_ad9 = EXP(temp10*temp9)*pkz_ad(i, j, k)
              temp_ad8 = temp9*temp_ad9/(temp8*temp12)
              temp14 = cv_air*delp(i, j, k)
              heat_source_ad(i, j, k) = heat_source_ad(i, j, k) + &
&               dtmp_ad/temp14
              delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*temp_ad8&
&               - heat_source(i, j, k)*cv_air*dtmp_ad/temp14**2
              CALL POPREALARRAY(pkz(i, j, k))
              temp_ad10 = temp10*temp_ad9/temp13
              pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad8
              delz_ad(i, j, k) = delz_ad(i, j, k) - temp8*temp_ad8
              cappa_ad(i, j, k) = cappa_ad(i, j, k) + (temp9+1.0)*&
&               temp_ad10
              pkz_ad(i, j, k) = 0.0
            END DO
          END DO
      ...
      IF (branch .EQ. 0) THEN
        DO j=je,js,-1
          DO i=ie,is,-1
            ...
            temp_ad5 = ws_ad(i, j)/delp(i, j, npz)
...
&             omga(i, j, npz)*temp_ad5/delp(i, j, npz)
            ws_ad(i, j) = 0.0
          END DO
        END DO
        ...
        DO k=npz,1,-1
          DO j=je,js,-1
            DO i=ie,is,-1
              CALL POPREALARRAY(pkz(i, j, k))
              temp4 = -cappa(i, j, k) + 1.
              temp0 = cappa(i, j, k)/temp4
              temp3 = delz(i, j, k)
              temp2 = delp(i, j, k)*pt(i, j, k)
              temp = temp2/temp3
              temp1 = LOG(rdg*temp)
              temp_ad2 = EXP(temp1*temp0)*pkz_ad(i, j, k)
              temp_ad3 = temp0*temp_ad2/(temp*temp3)
              temp_ad4 = temp1*temp_ad2/temp4
              delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*temp_ad3
              pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad3
              delz_ad(i, j, k) = delz_ad(i, j, k) - temp*temp_ad3
              cappa_ad(i, j, k) = cappa_ad(i, j, k) + (temp0+1.0)*&
&               temp_ad4
              pkz_ad(i, j, k) = 0.0
            END DO
          END DO
...
&                pkc, pkc_ad, gz, gz_ad, phis, pt, pt_ad, q_con, &
&                q_con_ad, pkz, pkz_ad, npz, akap, .false., gridstruct%&
&                nested, .true., npx, npy, flagstruct%a2b_ord, bd)
      ELSE
        DO k=npz+1,1,-1
          DO j=je+2,js-2,-1
            DO i=ie+2,is-2,-1
...
&                         delz, delz_ad, pt, pt_ad, phis, q_con, &
&                         q_con_ad, cappa, cappa_ad, pkc, pkc_ad, gz, &
&                         gz_ad, pk3, pk3_ad, npx, npy, npz, gridstruct%&
...
&                       npz, ng, isd, ied, jsd, jed, akap, cappa, &
&                       cappa_ad, cp, ptop, zs, q_con, q_con_ad, w, w_ad&
&                       , delz, delz_ad, pt, pt_ad, delp, delp_ad, zh, &
&                       zh_ad, pe, pe_ad, pkc, pkc_ad, pk3, pk3_ad, pk, &
&                       pk_ad, peln, peln_ad, ws, ws_ad, flagstruct%&
&                       scale_z, flagstruct%p_fac, flagstruct%a_imp, &
&                       flagstruct%use_logp, remap_step, arg10)
...
&                      %hord_tm_pert)
      ...
      CALL POPCONTROL(1,branch)
      IF (branch .EQ. 0) THEN
        CALL NESTED_GRID_BC_APPLY_INTT_ADM(q_con, q_con_ad, 0, 0, npx, &
&                                    npy, npz, bd, arg1, arg2, &
&                                    neststruct%q_con_bc, neststruct%&
&                                    nestbctype)
...
&               , j)
      ...
      ELSE
        divg2_ad = 0.0
      END IF
      CALL POPREALARRAY(q_con, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL START_GROUP_HALO_UPDATE_ADM(i_pack(11), q_con, q_con_ad, &
&                                domain, complete=.true.)
...
&               , jsd:jed, 1), q_con_ad(isd:ied, jsd:jed, 1), z_rat(isd:&
&               ied, jsd:jed), z_rat_ad(isd:ied, jsd:jed), kgb, heat_s, &
&               heat_s_ad, dpx, dpx_ad, zvir, sphum, nq, q, q_ad, k, npz&
&               , flagstruct%inline_q, dt, flagstruct%hord_tr, hord_m, &
&               hord_v, hord_t, hord_p, nord_k, nord_v(k), nord_w, &
&               nord_t, flagstruct%dddmp, d2_divg, flagstruct%d4_bg, &
&               damp_vt(k), damp_w, damp_t, d_con_k, hydrostatic, &
&               gridstruct, flagstruct, bd, flagstructp%hord_tr_pert, &
&               hord_m_pert, hord_v_pert, hord_t_pert, hord_p_pert, &
&               flagstructp%split_damp, nord_k_pert, nord_v_pert(k), &
&               nord_w_pert, nord_t_pert, flagstructp%dddmp_pert, &
&               d2_divg_pert, flagstructp%d4_bg_pert, damp_vt_pert(k), &
...
&                pkc, pkc_ad, gz, gz_ad, phis, ptc, ptc_ad, q_con, &
&                q_con_ad, pkz, pkz_ad, npz, akap, .true., gridstruct%&
&                nested, .false., npx, npy, flagstruct%a2b_ord, bd)
      ELSE
        IF (branch .EQ. 1) THEN
          CALL NEST_HALO_NH_BWD(ptop, grav, akap, cp, delpc, delpc_ad, &
&                         delz, delz_ad, ptc, ptc_ad, phis, q_con, &
&                         q_con_ad, cappa, cappa_ad, pkc, pkc_ad, gz, &
&                         gz_ad, pk3, pk3_ad, npx, npy, npz, gridstruct%&
...
&                        cappa, cappa_ad, cp, ptop, phis, omga, omga_ad&
&                        , ptc, ptc_ad, q_con, q_con_ad, delpc, delpc_ad&
&                        , gz, gz_ad, pkc, pkc_ad, ws3, ws3_ad, &
&                        flagstruct%p_fac, flagstruct%a_imp, flagstruct%&
&                        scale_z)
...
&                     , q_con, cappa, pkc, gz, pk3, npx, npy, npz, &
&                     gridstruct%nested, .false., .false., .false., bd)
        END IF
      END IF
...
!   gradient     of useful results: q_con peln gz delp pkz pe pk
!                pt
!   with respect to varying inputs: q_con peln gz delp pkz pe pk
!                pt
...
&             1, j)-peln(i, k, j)))
    ...
    CALL PUSHINTEGER(ifirst)
    CALL PUSHINTEGER(is)
    CALL PUSHREALARRAY(peg, (bd%ied-bd%isd+1)*(km+1))
    CALL PUSHINTEGER(ie)
    CALL PUSHREALARRAY(pkg, (bd%ied-bd%isd+1)*(km+1))
    CALL PUSHINTEGER(jlast)
...
&   , pk_ad, gz, gz_ad, hs, pt, pt_ad, q_con, q_con_ad, pkz, pkz_ad, km&
...
&   , delp
    ...
    REAL, DIMENSION(bd%isd:bd%ied, bd%jsd:bd%jed, km), INTENT(IN) :: &
&   q_con
    REAL, DIMENSION(bd%isd:bd%ied, bd%jsd:bd%jed, km) :: q_con_ad
    LOGICAL, INTENT(IN) :: cg, nested, computehalo
    REAL, DIMENSION(bd%isd:bd%ied, bd%jsd:bd%jed, km+1) :: gz, pk
    ...
    REAL :: peg(bd%isd:bd%ied, km+1)
    REAL :: peg_ad(bd%isd:bd%ied, km+1)
    REAL :: pkg(bd%isd:bd%ied, km+1)
    REAL :: pkg_ad(bd%isd:bd%ied, km+1)
    REAL(kind=8) :: p1d(bd%isd:bd%ied)
    ...
    CALL POPINTEGER(jlast)
    CALL POPREALARRAY(pkg, (bd%ied-bd%isd+1)*(km+1))
    CALL POPINTEGER(ie)
    CALL POPREALARRAY(peg, (bd%ied-bd%isd+1)*(km+1))
    CALL POPINTEGER(is)
    CALL POPINTEGER(ifirst)
    g1d_ad = 0.0_8
    logp_ad = 0.0
    ...
    DO j=jlast,jfirst,-1
      ...
      DO k=1,km,1
        DO i=ilast,ifirst,-1
          ...
          pt_ad(i, j, k) = pt_ad(i, j, k) + cp_air*(pkg(i, k+1)-pkg(i, k&
&           ))*g1d_ad(i)
          pkg_ad(i, k+1) = pkg_ad(i, k+1) + temp_ad
          pkg_ad(i, k) = pkg_ad(i, k) - temp_ad
        END DO
      END DO
      DO k=km+1,2,-1
        CALL POPCONTROL(2,branch)
...
&           )
          ...
          CALL POPREALARRAY(pkg(i, k))
          peg_ad(i, k) = peg_ad(i, k) + akap*EXP(akap*LOG(peg(i, k)))*&
&           pkg_ad(i, k)/peg(i, k)
          pkg_ad(i, k) = 0.0
          CALL POPREALARRAY(peg(i, k))
          peg_ad(i, k-1) = peg_ad(i, k-1) + peg_ad(i, k)
          delp_ad(i, j, k-1) = delp_ad(i, j, k-1) + p1d_ad(i) + (1.-&
&           q_con(i, j, k-1))*peg_ad(i, k)
