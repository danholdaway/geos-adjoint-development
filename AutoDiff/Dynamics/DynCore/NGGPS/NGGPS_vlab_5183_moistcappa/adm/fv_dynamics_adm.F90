...
!***********************************************************************
MODULE FV_DYNAMICS_MOD_DIFF
  USE CONSTANTS_MOD, ONLY : grav, pi => pi_8, radius, hlv, rdgas, omega,&
& rvgas, cp_vapor
  USE DYN_CORE_MOD_DIFF, ONLY : dyn_core, dyn_core_adm, del2_cubed, &
& del2_cubed_adm, init_ijk_mem
  USE FV_MAPZ_MOD_DIFF, ONLY : compute_total_energy, &
& compute_total_energy_adm, lagrangian_to_eulerian, &
& lagrangian_to_eulerian_adm, moist_cv, moist_cv_adm, moist_cp, &
& moist_cp_adm
  USE FV_TRACER2D_MOD_DIFF, ONLY : tracer_2d, tracer_2d_adm, &
& tracer_2d_1l, tracer_2d_1l_adm, tracer_2d_nested, tracer_2d_nested_adm
  USE FV_GRID_UTILS_MOD_DIFF, ONLY : cubed_to_latlon, c2l_ord2, &
& c2l_ord2_adm, g_sum, g_sum_adm
  USE FV_FILL_MOD_DIFF, ONLY : fill2d
...
& start_group_halo_update_adm, complete_group_halo_update
  ...
  USE FV_DIAGNOSTICS_MOD_DIFF, ONLY : fv_time, prt_mxm, range_check, &
& prt_minmax
  USE MPP_DOMAINS_MOD_DIFF, ONLY : dgrid_ne, cgrid_ne, &
& mpp_update_domains, mpp_update_domains_adm, domain2d
  USE MPP_MOD, ONLY : mpp_pe
  USE FIELD_MANAGER_MOD, ONLY : model_atmos
  USE TRACER_MANAGER_MOD, ONLY : get_tracer_index
  USE FV_SG_MOD, ONLY : neg_adj3
  USE FV_NESTING_MOD_DIFF, ONLY : setup_nested_grid_bcs, &
& setup_nested_grid_bcs_adm
  USE BOUNDARY_MOD_DIFF, ONLY : nested_grid_bc_apply_intt, &
& nested_grid_bc_apply_intt_adm
  USE FV_ARRAYS_MOD_DIFF, ONLY : fv_grid_type, fv_flags_type, &
& fv_atmos_type, fv_nest_type, fv_diag_type, fv_grid_bounds_type
  USE FV_NWP_NUDGE_MOD_DIFF, ONLY : do_adiabatic_init
...
! GFDL physics
    ...
    REAL :: arg2
    REAL*8 :: arg12
    REAL, DIMENSION(bd%is:bd%ie, bd%js:bd%je) :: arg13
    REAL :: result1
    LOGICAL :: res
    gz = 0.0
    pkc = 0.0
...
&                                1., 1., neststruct%q_con_bc, bctype=&
&                                neststruct%nestbctype)
        CALL NESTED_GRID_BC_APPLY_INTT(cappa, 0, 0, npx, npy, npz, bd, &
&                                1., 1., neststruct%cappa_bc, bctype=&
&                                neststruct%nestbctype)
...
!$OMP      rainwat,ice_wat,snowwat,graupel) private(cvm)
      DO k=1,npz
        DO j=js,je
          CALL MOIST_CP_FWD(is, ie, isd, ied, jsd, jed, npz, j, k, nwat&
&                     , sphum, liq_wat, rainwat, ice_wat, snowwat, &
&                     graupel, q, q_con(is:ie, j, k), cvm)
          DO i=is,ie
...
!$OMP                          private(cvm)
      DO k=1,npz
        IF (flagstruct%moist_phys) THEN
          DO j=js,je
            CALL MOIST_CV_FWD(is, ie, isd, ied, jsd, jed, npz, j, k, &
&                       nwat, sphum, liq_wat, rainwat, ice_wat, snowwat&
&                       , graupel, q, q_con(is:ie, j, k), cvm)
            DO i=is,ie
              ...
              pkz(i, j, k) = EXP(cappa(i, j, k)*LOG(rdg*delp(i, j, k)*pt&
&               (i, j, k)*(1.+dp1(i, j, k))*(1.-q_con(i, j, k))/delz(i, &
&               j, k)))
            END DO
          END DO
          CALL PUSHCONTROL(1,1)
...
!$OMP parallel do default(none) shared(is,ie,js,je,npz,pt,dp1,pkz,q_con)
      DO k=1,npz
        DO j=js,je
          DO i=is,ie
            CALL PUSHREALARRAY(pt(i, j, k))
            pt(i, j, k) = pt(i, j, k)*(1.+dp1(i, j, k))*(1.-q_con(i, j, &
&             k))/pkz(i, j, k)
          END DO
        END DO
      END DO
...
&                            .true.)
      CALL PUSHREALARRAY(cappa, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz&
&                  )
      CALL START_GROUP_HALO_UPDATE(i_pack(12), cappa, domain, complete=&
&                            .true.)
...
&                                 kord_tm_pert)
        IF (neststruct%nested .AND. (.NOT.last_step)) THEN
          arg11 = REAL(n_map + 1)
          arg2 = REAL(k_split)
          CALL PUSHREALARRAY(cappa, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)&
&                       *npz)
          CALL NESTED_GRID_BC_APPLY_INTT(cappa, 0, 0, npx, npy, npz, bd&
&                                  , REAL(n_map + 1), REAL(k_split), &
&                                  neststruct%cappa_bc, bctype=&
&                                  neststruct%nestbctype)
          CALL PUSHCONTROL(1,0)
        ELSE
          CALL PUSHCONTROL(1,1)
...
!--------------------------
          IF (flagstruct%nf_omega .GT. 0) THEN
            arg12 = 0.18*gridstruct%da_min
            CALL DEL2_CUBED_FWD(omga, arg12, gridstruct, domain, npx, &
...
&                   ))
      CALL PUSHREALARRAY(cappa, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz&
&                  )
      CALL PUSHREALARRAY(cvm, bd%ie - bd%is + 1)
...
!   gradient     of useful results: q u v w delp delz pt
!   with respect to varying inputs: q u v w delp delz pt
!   RW status of diff variables: q_con:(loc) peln:(loc) q:in-out
!                u:in-out v:in-out w:in-out delp:in-out ua:(loc)
!                uc:(loc) mfx:(loc) delz:in-out mfy:(loc) omga:(loc)
!                va:(loc) vc:(loc) pkz:(loc) pe:(loc) pk:(loc)
!                ps:(loc) pt:in-out cx:(loc) cy:(loc)
!-----------------------------------------------------------------------
!     fv_dynamics :: FV dynamical core driver
!-----------------------------------------------------------------------
...
&   , peln_ad, pkz, pkz_ad, phis, q_con, q_con_ad, omga, omga_ad, ua, &
&   ua_ad, va, va_ad, uc, uc_ad, vc, vc_ad, ak, bk, mfx, mfx_ad, mfy, &
&   mfy_ad, cx, cx_ad, cy, cy_ad, ze0, hybrid_z, gridstruct, flagstruct&
&   , flagstructp, neststruct, idiag, bd, parent_grid, domain, &
&   time_total)
    IMPLICIT NONE
    REAL, INTENT(IN) :: bdt
    REAL, INTENT(IN) :: consv_te
...
&   ua_ad, va_ad
    ...
    REAL :: pfull(npz)
    REAL, DIMENSION(bd%is:bd%ie) :: cvm
    REAL, DIMENSION(bd%is:bd%ie) :: cvm_ad
    REAL :: dp1(bd%isd:bd%ied, bd%jsd:bd%jed, npz), dtdt_m(bd%is:bd%ie, &
&   bd%js:bd%je, npz), cappa(bd%isd:bd%ied, bd%jsd:bd%jed, npz)
    REAL :: dp1_ad(bd%isd:bd%ied, bd%jsd:bd%jed, npz), cappa_ad(bd%isd:&
&   bd%ied, bd%jsd:bd%jed, npz)
    REAL(kind=8) :: psx(bd%isd:bd%ied, bd%jsd:bd%jed)
...
&                  ))
      ...
      CALL POPREALARRAY(cvm, bd%ie - bd%is + 1)
      CALL POPREALARRAY(cappa, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
...
&                  ))
    ...
    CALL POPCONTROL(1,branch)
    IF (branch .EQ. 0) THEN
      temp_ad9 = -(radius*u0_ad/result1)
      amdt_ad = temp_ad9
      result1_ad = -(amdt*temp_ad9/result1)
...
&             )
    ...
    DO j=je,js,-1
      DO i=ie,is,-1
        temp_ad8 = dt2*idiag%zxg(i, j)*te_2d_ad(i, j)
        teq_ad(i, j) = teq_ad(i, j) - te_2d_ad(i, j)
        ps2_ad(i, j) = ps2_ad(i, j) + temp_ad8
        ps_ad(i, j) = ps_ad(i, j) + temp_ad8
      END DO
    END DO
...
&                    ps_ad, m_fac, m_fac_ad)
    ...
    DO n_map=k_split,1,-1
      ...
      ELSE
        IF (branch .NE. 2) THEN
          arg12 = 0.18*gridstruct%da_min
          CALL DEL2_CUBED_BWD(omga, omga_ad, arg12, gridstruct, domain, &
          ...
          DO k=npz,1,-1
            DO j=je,js,-1
              DO i=ie,is,-1
                ...
                temp_ad7 = omga_ad(i, j, k)/delz(i, j, k)
                delp_ad(i, j, k) = delp_ad(i, j, k) + w(i, j, k)*&
&                 temp_ad7
                w_ad(i, j, k) = w_ad(i, j, k) + delp(i, j, k)*temp_ad7
                delz_ad(i, j, k) = delz_ad(i, j, k) - delp(i, j, k)*w(i&
&                 , j, k)*temp_ad7/delz(i, j, k)
                omga_ad(i, j, k) = 0.0
              END DO
            END DO
          END DO
      ...
      CALL POPCONTROL(1,branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY(cappa, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*&
&                    npz)
        CALL NESTED_GRID_BC_APPLY_INTT_ADM(cappa, cappa_ad, 0, 0, npx, &
&                                    npy, npz, bd, arg11, arg2, &
&                                    neststruct%cappa_bc, neststruct%&
&                                    nestbctype)
      END IF
      kord_mt_pert = flagstructp%kord_mt_pert
      kord_wz_pert = flagstructp%kord_wz_pert
...
&                               q_con, q_con_ad, u, u_ad, v, v_ad, w, &
&                               w_ad, delz, delz_ad, pt, pt_ad, q, q_ad&
&                               , phis, zvir, cp_air, akap, cappa, &
&                               cappa_ad, kord_mt, kord_wz, kord_tracer&
&                               , kord_tm, peln, peln_ad, te_2d, &
&                               te_2d_ad, ng, ua, ua_ad, va, omga, &
...
&                 , cp_air, akap, cappa, cappa_ad, grav, hydrostatic, u&
&                 , u_ad, v, v_ad, w, w_ad, delz, delz_ad, pt, pt_ad, q&
&                 , q_ad, delp, delp_ad, pe, pe_ad, pk, pk_ad, phis, ws&
&                 , ws_ad, omga, omga_ad, ptop, pfull, ua, ua_ad, va, &
&                 va_ad, uc, uc_ad, vc, vc_ad, mfx, mfx_ad, mfy, mfy_ad&
&                 , cx, cx_ad, cy, cy_ad, pkz, pkz_ad, peln, peln_ad, &
&                 q_con, q_con_ad, ak, bk, dpx, dpx_ad, ks, gridstruct, &
&                 flagstruct, flagstructp, neststruct, idiag, bd, domain&
&                 , arg10, i_pack, last_step, gz, gz_ad, pkc, pkc_ad, &
&                 ptc, ptc_ad, crx, crx_ad, xfx, xfx_ad, cry, cry_ad, &
&                 yfx, yfx_ad, divgd, divgd_ad, delpc, delpc_ad, ut, &
&                 ut_ad, vt, vt_ad, zh, zh_ad, pk3, pk3_ad, du, du_ad, &
&                 dv, dv_ad, time_total)
      DO k=npz,1,-1
        DO j=jed,jsd,-1
          DO i=ied,isd,-1
...
&                                , complete=.true.)
      CALL POPREALARRAY(cappa, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL START_GROUP_HALO_UPDATE_ADM(i_pack(12), cappa, cappa_ad, &
      ...
      DO k=npz,1,-1
        ...
        DO j=je,js,-1
          DO i=ie,is,-1
            ...
            pkz_ad(i, j, k) = pkz_ad(i, j, k) - pt(i, j, k)*temp_ad5/pkz&
&             (i, j, k)
            pt_ad(i, j, k) = temp_ad5
          END DO
        END DO
      END DO
    ELSE
      DO k=npz,1,-1
        DO j=je,js,-1
          DO i=ie,is,-1
            CALL POPREALARRAY(pt(i, j, k))
            temp11 = pkz(i, j, k)
            temp10 = pt(i, j, k)/temp11
            temp_ad6 = (dp1(i, j, k)+1.)*(1.-q_con(i, j, k))*pt_ad(i, j&
&             , k)/temp11
            dp1_ad(i, j, k) = dp1_ad(i, j, k) + temp10*(1.-q_con(i, j, k&
&             ))*pt_ad(i, j, k)
            q_con_ad(i, j, k) = q_con_ad(i, j, k) - (dp1(i, j, k)+1.)*&
&             temp10*pt_ad(i, j, k)
            pkz_ad(i, j, k) = pkz_ad(i, j, k) - temp10*temp_ad6
            pt_ad(i, j, k) = temp_ad6
          END DO
        END DO
      END DO
    END IF
...
&             , j, k)
            dp1_ad(i, j, k) = 0.0
          END DO
          CALL MOIST_CP_BWD(is, ie, isd, ied, jsd, jed, npz, j, k, nwat&
&                     , sphum, liq_wat, rainwat, ice_wat, snowwat, &
&                     graupel, q, q_ad, q_con(is:ie, j, k), q_con_ad(is:&
&                     ie, j, k), cvm)
        END DO
      END DO
    ELSE
        ...
        IF (branch .EQ. 0) THEN
          DO j=je,js,-1
            DO i=ie,is,-1
              ...
              temp7 = temp8/temp9
              temp_ad4 = kappa*EXP(kappa*LOG(rdg*temp7))*pkz_ad(i, j, k)&
&               /(temp7*temp9)
              delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*temp_ad4
              pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad4
              delz_ad(i, j, k) = delz_ad(i, j, k) - temp7*temp_ad4
              pkz_ad(i, j, k) = 0.0
              CALL POPREALARRAY(dp1(i, j, k))
              dp1_ad(i, j, k) = 0.0
            END DO
        ...
        ELSE
          DO j=je,js,-1
            DO i=ie,is,-1
              CALL POPREALARRAY(pkz(i, j, k))
              temp6 = delz(i, j, k)
              temp2 = (dp1(i, j, k)+1.)/temp6
              temp1 = delp(i, j, k)*pt(i, j, k)
              temp5 = temp1*(-q_con(i, j, k)+1.)
              temp4 = rdg*temp5*temp2
              temp3 = LOG(temp4)
              temp_ad = EXP(cappa(i, j, k)*temp3)*pkz_ad(i, j, k)
              temp_ad0 = cappa(i, j, k)*rdg*temp_ad/temp4
              temp_ad1 = temp2*(1.-q_con(i, j, k))*temp_ad0
              temp_ad2 = temp5*temp_ad0/temp6
              cappa_ad(i, j, k) = cappa_ad(i, j, k) + temp3*temp_ad
              delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*temp_ad1
              pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad1
              q_con_ad(i, j, k) = q_con_ad(i, j, k) - temp1*temp2*&
&               temp_ad0
              delz_ad(i, j, k) = delz_ad(i, j, k) - temp2*temp_ad2
              pkz_ad(i, j, k) = 0.0
              CALL POPREALARRAY(cappa(i, j, k))
              temp0 = dp1(i, j, k) + 1.
              temp = cvm(i)/temp0
              temp_ad3 = -(rdgas*cappa_ad(i, j, k)/((rdgas+temp)**2*&
&               temp0))
              dp1_ad(i, j, k) = dp1_ad(i, j, k) + temp_ad2 - temp*&
&               temp_ad3
              cvm_ad(i) = cvm_ad(i) + temp_ad3
              cappa_ad(i, j, k) = 0.0
              CALL POPREALARRAY(dp1(i, j, k))
...
&               i, j, k)
              dp1_ad(i, j, k) = 0.0
            END DO
            CALL MOIST_CV_BWD(is, ie, isd, ied, jsd, jed, npz, j, k, &
&                       nwat, sphum, liq_wat, rainwat, ice_wat, snowwat&
&                       , graupel, q, q_ad, q_con(is:ie, j, k), q_con_ad&
&                       (is:ie, j, k), cvm, cvm_ad)
          END DO
        END IF
      END DO
    END IF
...
!Correct halo values have now been set up for BCs; we can go ahead and apply them too...
        CALL NESTED_GRID_BC_APPLY_INTT(pt, 0, 0, npx, npy, npz, bd, 1., &
&                                1., neststruct%pt_bc, neststruct%&
&                                nestbctype)
        CALL NESTED_GRID_BC_APPLY_INTT(q_con, 0, 0, npx, npy, npz, bd, &
&                                1., 1., neststruct%q_con_bc, neststruct&
&                                %nestbctype)
        CALL NESTED_GRID_BC_APPLY_INTT(cappa, 0, 0, npx, npy, npz, bd, &
&                                1., 1., neststruct%cappa_bc, neststruct&
&                                %nestbctype)
      END IF
...
!$OMP      rainwat,ice_wat,snowwat,graupel) private(cvm)
      DO k=1,npz
        DO j=js,je
          CALL MOIST_CP(is, ie, isd, ied, jsd, jed, npz, j, k, nwat, &
&                 sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, q&
&                 , q_con(is:ie, j, k), cvm)
          DO i=is,ie
            dp1(i, j, k) = zvir*q(i, j, k, sphum)
          END DO
...
!$OMP                          private(cvm)
      DO k=1,npz
        IF (flagstruct%moist_phys) THEN
          DO j=js,je
            CALL MOIST_CV(is, ie, isd, ied, jsd, jed, npz, j, k, nwat, &
&                   sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, &
&                   q, q_con(is:ie, j, k), cvm)
            DO i=is,ie
              dp1(i, j, k) = zvir*q(i, j, k, sphum)
              cappa(i, j, k) = rdgas/(rdgas+cvm(i)/(1.+dp1(i, j, k)))
              pkz(i, j, k) = EXP(cappa(i, j, k)*LOG(rdg*delp(i, j, k)*pt&
...
&               /delz(i, j, k)))
    ...
    END IF
    IF (flagstruct%fv_debug) THEN
      CALL PRT_MXM('cappa', cappa, is, ie, js, je, ng, npz, 1., &
...
&                             kord_tracer_pert, kord_tm_pert)
        ...
        IF (neststruct%nested .AND. (.NOT.last_step)) THEN
          ...
          arg2 = REAL(k_split)
          CALL NESTED_GRID_BC_APPLY_INTT(cappa, 0, 0, npx, npy, npz, bd&
&                                  , arg11, arg2, neststruct%cappa_bc, &
&                                  neststruct%nestbctype)
        END IF
        IF (last_step) THEN
          IF (.NOT.hydrostatic) THEN
...
!--------------------------
          IF (flagstruct%nf_omega .GT. 0) THEN
            arg12 = 0.18*gridstruct%da_min
            CALL DEL2_CUBED(omga, arg12, gridstruct, domain, npx, npy, &
...
&           ))*idiag%zxg(i, j)
      ...
      IF (idiag%id_amdt .GT. 0) THEN
        arg13(:, :) = te_2d/bdt
        used = SEND_DATA(idiag%id_amdt, arg13(:, :), fv_time)
      END IF
      IF (flagstruct%consv_am .OR. prt_minmax) THEN
        amdt = G_SUM(domain, te_2d, is, ie, js, je, ng, gridstruct%&
&         area_64, 0, .true.)
...
END MODULE FV_DYNAMICS_MOD_DIFF
