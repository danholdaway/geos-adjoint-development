...
!***********************************************************************
MODULE FV_NESTING_MOD_DIFF
  USE MPP_DOMAINS_MOD_DIFF, ONLY : mpp_update_domains, &
& mpp_update_domains_adm
  USE MPP_DOMAINS_MOD_DIFF, ONLY : mpp_global_field
  USE FIELD_MANAGER_MOD, ONLY : model_atmos
  USE TRACER_MANAGER_MOD, ONLY : get_tracer_index
  USE FV_SG_MOD, ONLY : neg_adj3
  USE MPP_DOMAINS_MOD_DIFF, ONLY : mpp_get_data_domain, &
& mpp_get_compute_domain, mpp_get_global_domain
  USE MPP_DOMAINS_MOD_DIFF, ONLY : dgrid_ne, mpp_update_domains, &
& mpp_update_domains_adm, domain2d
  USE FV_RESTART_MOD_DIFF, ONLY : d2a_setup, d2c_setup, d2c_setup_adm
  USE MPP_MOD, ONLY : mpp_sync_self, mpp_sync, mpp_send, mpp_recv, &
& mpp_error, fatal
  USE MPP_DOMAINS_MOD_DIFF, ONLY : mpp_global_sum, bitwise_efp_sum, &
& bitwise_exact_sum
  USE BOUNDARY_MOD_DIFF, ONLY : update_coarse_grid
  USE BOUNDARY_MOD_DIFF, ONLY : nested_grid_bc_send, nested_grid_bc_recv&
& , nested_grid_bc_save_proc
  USE FV_MP_MOD_DIFF, ONLY : is, ie, js, je, isd, ied, jsd, jed, isc, &
& iec, jsc, jec
  USE FV_ARRAYS_MOD_DIFF, ONLY : fv_grid_type, fv_flags_type, &
& fv_atmos_type, fv_nest_type, fv_diag_type, fv_nest_bc_type_3d
  USE FV_ARRAYS_MOD_DIFF, ONLY : allocate_fv_nest_bc_type, fv_atmos_type&
& , fv_grid_bounds_type
  USE FV_GRID_UTILS_MOD_DIFF, ONLY : ptop_min, g_sum, g_sum_adm, &
& cubed_to_latlon, f_p
  USE INIT_HYDRO_MOD_DIFF, ONLY : p_var
  USE CONSTANTS_MOD, ONLY : grav, pi => pi_8, radius, hlv, rdgas, cp_air&
& , rvgas, cp_vapor, kappa
  USE FV_MAPZ_MOD_DIFF, ONLY : mappm
  USE FV_TIMING_MOD, ONLY : timing_on, timing_off
  USE FV_MP_MOD_DIFF, ONLY : is_master
  USE FV_MP_MOD_DIFF, ONLY : mp_reduce_sum
  USE FV_DIAGNOSTICS_MOD_DIFF, ONLY : sphum_ll_fix, range_check
  USE SW_CORE_MOD_DIFF, ONLY : divergence_corner, divergence_corner_adm,&
& divergence_corner_nest, divergence_corner_nest_adm
  IMPLICIT NONE
...
!Individual structures are allocated by nested_grid_BC_recv
  TYPE(FV_NEST_BC_TYPE_3D) :: u_buf, v_buf, uc_buf, vc_buf, delp_buf, &
& delz_buf, pt_buf, pkz_buf, w_buf, divg_buf
  TYPE(FV_NEST_BC_TYPE_3D), ALLOCATABLE :: q_buf(:)
!#ifdef 1
  REAL, DIMENSION(:, :, :), ALLOCATABLE, TARGET :: dum_west, dum_east, &
& dum_north, dum_south
!#endif
  PRIVATE 
...
&                     neststruct%q_bc, ncnst, neststruct%q_con_bc, &
&                     neststruct%cappa_bc, npx, npy, npz, zvir, bd)
      END IF
      CALL NESTED_GRID_BC_SAVE_PROC(neststruct%nest_domain, neststruct%&
...
&   , q_con_bc, cappa_bc, npx, npy, npz, zvir, bd)
    IMPLICIT NONE
    TYPE(FV_GRID_BOUNDS_TYPE), INTENT(IN) :: bd
      ...
      DO k=1,npz
        DO j=jsd,jed
          DO i=isd,0
            ...
            q_sol = ice_watbc_west(i, j, k) + snowwatbc_west(i, j, k) + &
&             graupelbc_west(i, j, k)
            q_con = q_liq + q_sol
            qconbc(i, j, k) = q_con
            cvm = (1.-(sphumbc(i, j, k)+q_con))*cv_air + sphumbc(i, j, k&
&             )*cv_vap + q_liq*c_liq + q_sol*c_ice
            cappabc(i, j, k) = rdgas/(rdgas+cvm/(1.+dp1))
            pkz = EXP(cappabc(i, j, k)*LOG(rdg*delpbc(i, j, k)*ptbc(i, j&
&             , k)*(1.+dp1)*(1.-q_con)/delzbc(i, j, k)))
      ...
      DO k=1,npz
        DO j=jsd,0
          DO i=istart,iend
            ...
            q_sol = ice_watbc_south(i, j, k) + snowwatbc_south(i, j, k) &
&             + graupelbc_south(i, j, k)
            q_con = q_liq + q_sol
            qconbc(i, j, k) = q_con
            cvm = (1.-(sphumbc(i, j, k)+q_con))*cv_air + sphumbc(i, j, k&
&             )*cv_vap + q_liq*c_liq + q_sol*c_ice
      ...
      DO k=1,npz
        DO j=jsd,jed
          DO i=npx,ied
            ...
            q_sol = ice_watbc_east(i, j, k) + snowwatbc_east(i, j, k) + &
&             graupelbc_east(i, j, k)
            q_con = q_liq + q_sol
            qconbc(i, j, k) = q_con
            cvm = (1.-(sphumbc(i, j, k)+q_con))*cv_air + sphumbc(i, j, k&
&             )*cv_vap + q_liq*c_liq + q_sol*c_ice
      ...
      DO k=1,npz
        DO j=npy,jed
          DO i=istart,iend
            ...
            q_sol = ice_watbc_north(i, j, k) + snowwatbc_north(i, j, k) &
&             + graupelbc_north(i, j, k)
            q_con = q_liq + q_sol
            qconbc(i, j, k) = q_con
            cvm = (1.-(sphumbc(i, j, k)+q_con))*cv_air + sphumbc(i, j, k&
&             )*cv_vap + q_liq*c_liq + q_sol*c_ice
...
END MODULE FV_NESTING_MOD_DIFF
