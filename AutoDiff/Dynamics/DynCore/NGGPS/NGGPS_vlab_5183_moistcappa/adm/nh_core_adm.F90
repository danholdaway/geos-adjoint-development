...
!***********************************************************************
MODULE NH_CORE_MOD_DIFF
! Developer: S.-J. Lin, NOAA/GFDL
! To do list:
! include moisture effect in pt
!------------------------------
  USE CONSTANTS_MOD, ONLY : rdgas, cp_air, grav
  USE TP_CORE_MOD_DIFF, ONLY : fv_tp_2d, fv_tp_2d_adm
  USE NH_UTILS_MOD_DIFF, ONLY : update_dz_c, update_dz_c_adm, &
& update_dz_d, update_dz_d_adm, nest_halo_nh, nest_halo_nh_adm
  USE NH_UTILS_MOD_DIFF, ONLY : sim_solver, sim_solver_adm, sim1_solver,&
& sim1_solver_adm, sim3_solver, sim3_solver_adm
  USE NH_UTILS_MOD_DIFF, ONLY : sim3p0_solver, sim3p0_solver_adm, rim_2d&
& , rim_2d_adm
...
!   gradient     of useful results: pk3 ws ppe cappa q_con peln
!                w delp delz pe pk zh pt
!   with respect to varying inputs: pk3 ws ppe cappa q_con peln
!                w delp delz pe pk zh pt
...
&   jsd, jed, akap, cappa, cappa_ad, cp, ptop, zs, q_con, q_con_ad, w, &
&   w_ad, delz, delz_ad, pt, pt_ad, delp, delp_ad, zh, zh_ad, pe, pe_ad&
&   , ppe, ppe_ad, pk3, pk3_ad, pk, pk_ad, peln, peln_ad, ws, ws_ad, &
&   scale_m, p_fac, a_imp, use_logp, last_call, fp_out)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: ms, is, ie, js, je, km, ng
    INTEGER, INTENT(IN) :: isd, ied, jsd, jed
    REAL, INTENT(IN) :: dt
    ...
    REAL, DIMENSION(isd:, jsd:, :), INTENT(IN) :: q_con, cappa
    REAL, DIMENSION(isd:, jsd:, :) :: q_con_ad, cappa_ad
    REAL, DIMENSION(isd:ied, jsd:jed, km), INTENT(IN) :: delp, pt
    ...
    REAL, DIMENSION(is:ie, km) :: dm, dz2, pm2, w2, gm2, cp2
    REAL, DIMENSION(is:ie, km) :: dm_ad, dz2_ad, pm2_ad, w2_ad, gm2_ad, &
&   cp2_ad
    REAL, DIMENSION(is:ie, km+1) :: pem, pe2, peln2, peg, pelng
    REAL, DIMENSION(is:ie, km+1) :: pem_ad, pe2_ad, peln2_ad, peg_ad, &
&   pelng_ad
    REAL :: gama, rgrav, ptk, peln1
    INTEGER :: i, j, k
    INTRINSIC LOG
    ...
    DO j=je,js,-1
      ...
      IF (branch .LT. 2) THEN
        IF (branch .EQ. 0) THEN
          CALL SIM_SOLVER_BWD(dt, is, ie, km, rdgas, gama, gm2, gm2_ad, &
&                       cp2, cp2_ad, akap, pe2, pe2_ad, dm, dm_ad, pm2, &
&                       pm2_ad, pem, pem_ad, w2, w2_ad, dz2, dz2_ad, pt(&
&                       is:ie, j, 1:km), pt_ad(is:ie, j, 1:km), ws(is:ie&
&                       , j), ws_ad(is:ie, j), a_imp, p_fac, scale_m)
        ELSE
          CALL SIM1_SOLVER_BWD(dt, is, ie, km, rdgas, gama, gm2, gm2_ad&
&                        , cp2, cp2_ad, akap, pe2, pe2_ad, dm, dm_ad, &
&                        pm2, pm2_ad, pem, pem_ad, w2, w2_ad, dz2, &
&                        dz2_ad, pt(is:ie, j, 1:km), pt_ad(is:ie, j, 1:&
&                        km), ws(is:ie, j), ws_ad(is:ie, j), p_fac)
        END IF
      ELSE IF (branch .EQ. 2) THEN
        CALL RIM_2D_BWD(ms, dt, is, ie, km, rdgas, gama, gm2, gm2_ad, &
&                 pe2, pe2_ad, dm, dm_ad, pm2, pm2_ad, w2, w2_ad, dz2, &
&                 dz2_ad, pt(is:ie, j, 1:km), pt_ad(is:ie, j, 1:km), ws(&
&                 is:ie, j), ws_ad(is:ie, j), .false.)
...
&                        scale_m)
      ...
      DO k=km,1,-1
        DO i=ie,is,-1
          ...
          temp = pelng(i, k+1) - pelng(i, k)
          temp_ad = pm2_ad(i, k)/temp
          temp_ad0 = -((peg(i, k+1)-peg(i, k))*temp_ad/temp)
