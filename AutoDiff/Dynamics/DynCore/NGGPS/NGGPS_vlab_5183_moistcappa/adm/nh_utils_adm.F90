...
!***********************************************************************
MODULE NH_UTILS_MOD_DIFF
! Developer: S.-J. Lin, NOAA/GFDL
! To do list:
! include moisture effect in pt
!------------------------------
  USE CONSTANTS_MOD, ONLY : rdgas, cp_air, grav
  USE TP_CORE_MOD_DIFF, ONLY : fv_tp_2d, fv_tp_2d_adm, fv_tp_2d, &
& fv_tp_2d_adm
  USE SW_CORE_MOD_DIFF, ONLY : fill_4corners, fill_4corners_adm, &
& del6_vt_flux, del6_vt_flux_adm
  USE FV_ARRAYS_MOD_DIFF, ONLY : fv_grid_bounds_type, fv_grid_type
  IMPLICIT NONE
  PRIVATE 
...
!.great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: ws cappa q_con gz delp w3 pef
!                pt
!   with respect to varying inputs: ws cappa q_con gz delp w3 pef
!                pt
...
! Excluding contribution from condensates:
      ...
      DO k=1,km
        DO i=is1,ie1
          ...
          CALL PUSHREALARRAY(cp2(i, k))
          cp2(i, k) = cappa(i, j, k)
          CALL PUSHREALARRAY(gm2(i, k))
          gm2(i, k) = 1./(1.-cp2(i, k))
          CALL PUSHREALARRAY(dm(i, k))
          dm(i, k) = dm(i, k)*rgrav
...
! Compute Height * grav (for p-gradient computation)
    ...
    CALL PUSHREALARRAY(peg, (ie-is+3)*(km+1))
    CALL PUSHREALARRAY(gama)
    CALL PUSHREALARRAY(pm2, (ie-is+3)*km)
    CALL PUSHREALARRAY(cp2, (ie-is+3)*km)
    ...
    CALL PUSHREALARRAY(gm2, (ie-is+3)*km)
...
&   cappa, cappa_ad, cp, ptop, hs, w3, w3_ad, pt, pt_ad, q_con, q_con_ad&
&   , delp, delp_ad, gz, gz_ad, pef, pef_ad, ws, ws_ad, p_fac, a_imp, &
&   scale_m)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: is, ie, js, je, ng, km
    INTEGER, INTENT(IN) :: ms
...
&   delp
    ...
    REAL, DIMENSION(is-ng:ie+ng, js-ng:je+ng, km), INTENT(IN) :: q_con, &
&   cappa
    REAL, DIMENSION(is-ng:ie+ng, js-ng:je+ng, km) :: q_con_ad, cappa_ad
    REAL, INTENT(IN) :: hs(is-ng:ie+ng, js-ng:je+ng)
    REAL, DIMENSION(is-ng:ie+ng, js-ng:je+ng, km), INTENT(IN) :: w3
...
&   gm2_ad, cp2_ad
    REAL, DIMENSION(is-1:ie+1, km+1) :: pem, pe2, peg
    REAL, DIMENSION(is-1:ie+1, km+1) :: pem_ad, pe2_ad, peg_ad
    REAL :: gama, rgrav
    INTEGER :: i, j, k
    INTEGER :: is1, ie1
    INTRINSIC LOG
    ...
    DO j=je+1,js-1,-1
      ...
      CALL POPCONTROL(2,branch)
      IF (branch .EQ. 0) THEN
        CALL SIM1_SOLVER_BWD(dt, is1, ie1, km, rdgas, gama, gm2, gm2_ad&
&                      , cp2, cp2_ad, akap, pe2, pe2_ad, dm, dm_ad, pm2&
&                      , pm2_ad, pem, pem_ad, w2, w2_ad, dz2, dz2_ad, pt&
&                      (is1:ie1, j, 1:km), pt_ad(is1:ie1, j, 1:km), ws(&
&                      is1:ie1, j), ws_ad(is1:ie1, j), p_fac)
      ELSE IF (branch .EQ. 1) THEN
        CALL RIM_2D_BWD(ms, dt, is1, ie1, km, rdgas, gama, gm2, gm2_ad, &
&                 pe2, pe2_ad, dm, dm_ad, pm2, pm2_ad, w2, w2_ad, dz2, &
&                 dz2_ad, pt(is1:ie1, j, 1:km), pt_ad(is1:ie1, j, 1:km)&
&                 , ws(is1:ie1, j), ws_ad(is1:ie1, j), .true.)
      ELSE
...
&                        ), p_fac, scale_m)
      ...
      DO k=km,1,-1
        DO i=ie1,is1,-1
          ...
          dm_ad(i, k) = rgrav*dm_ad(i, k)
          CALL POPREALARRAY(gm2(i, k))
          cp2_ad(i, k) = cp2_ad(i, k) + gm2_ad(i, k)/(1.-cp2(i, k))**2
          gm2_ad(i, k) = 0.0
          CALL POPREALARRAY(cp2(i, k))
          cappa_ad(i, j, k) = cappa_ad(i, j, k) + cp2_ad(i, k)
          cp2_ad(i, k) = 0.0
          CALL POPREALARRAY(pm2(i, k))
          temp1 = peg(i, k)
          temp = peg(i, k+1)/temp1
          temp0 = LOG(temp)
          temp_ad = pm2_ad(i, k)/temp0
          temp_ad0 = -((peg(i, k+1)-peg(i, k))*temp_ad/(temp*temp0*temp1&
&           ))
          peg_ad(i, k+1) = peg_ad(i, k+1) + temp_ad0 + temp_ad
          peg_ad(i, k) = peg_ad(i, k) - temp*temp_ad0 - temp_ad
          pm2_ad(i, k) = 0.0
      ...
      DO k=km+1,2,-1
        DO i=ie1,is1,-1
          ...
          CALL POPREALARRAY(peg(i, k))
          peg_ad(i, k-1) = peg_ad(i, k-1) + peg_ad(i, k)
          dm_ad(i, k-1) = dm_ad(i, k-1) + pem_ad(i, k) + (1.-q_con(i, j&
&           , k-1))*peg_ad(i, k)
          q_con_ad(i, j, k-1) = q_con_ad(i, j, k-1) - dm(i, k-1)*peg_ad(&
&           i, k)
          peg_ad(i, k) = 0.0
...
!   gradient     of useful results: ws gm2 pe2 dm2 dz2 w2 pm2 pt2
!   with respect to varying inputs: ws gm2 pe2 dm2 dz2 w2 pm2 pt2
...
!.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tr
!acer_fb fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limit
!ers fv_mapz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm fv_mapz_mod.mois
!t_cv fv_mapz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer2d_mod.tracer_2d
! fv_tracer2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_utils_mod.Riem_S
!olver_c nh_utils_mod.Riem_Solver3test nh_utils_mod.imp_diff_w nh_utils_mod.RIM_2D nh_utils_mod.SIM3_solver nh_utils_mod.SIM3p0_s
!olver nh_utils_mod.SIM1_solver nh_utils_mod.SIM_solver nh_utils_mod.edge_scalar nh_utils_mod.edge_profile nh_utils_mod.nest_halo
!_nh sw_core_mod.c_sw sw_core_mod.d_sw  sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest sw_core_mod.d2a2c_vect s
!w_core_mod.fill3_4corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core
!_mod.compute_divergence_damping sw_core_mod.smag_corner tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mod.copy_c
!orners_fb tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great
!_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: ws gm2 pe2 dm2 dz2 w2 pm2 pt2
!   with respect to varying inputs: ws gm2 pe2 dm2 dz2 w2 pm2 pt2
  SUBROUTINE RIM_2D_BWD(ms, bdt, is, ie, km, rgas, gama, gm2, gm2_ad, &
&   pe2, pe2_ad, dm2, dm2_ad, pm2, pm2_ad, w2, w2_ad, dz2, dz2_ad, pt2, &
&   pt2_ad, ws, ws_ad, c_core)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: ms, is, ie, km
    REAL, INTENT(IN) :: bdt, gama, rgas
    REAL, DIMENSION(is:ie, km), INTENT(IN) :: dm2, pm2, gm2
    REAL, DIMENSION(is:ie, km) :: dm2_ad, pm2_ad, gm2_ad
    LOGICAL, INTENT(IN) :: c_core
    REAL, INTENT(IN) :: pt2(is:ie, km)
      ...
      IF (branch .EQ. 0) THEN
        ...
        ELSE
          DO k=km,1,-1
            ...
            dm_ad(k) = dm_ad(k) - (wm(k)+pbar(k+1)-pbar(k))*temp_ad10/dm&
&             (k)
            w2_ad(i, k) = 0.0
            CALL POPREALARRAY(dz2(i, k))
            dz_ad(k) = dz_ad(k) + dz2_ad(i, k)
        ...
        END IF
        CALL POPREALARRAY(pbar(km+1))
        temp_ad9 = bdt*pbar_ad(km+1)
        cm_ad(km) = cm_ad(km) + wbar(km+1)*temp_ad9
        wbar_ad(km+1) = wbar_ad(km+1) + cm(km)*temp_ad9
        wc_ad(km) = wc_ad(km) - temp_ad9
        pp_ad(km) = pp_ad(km) + temp_ad9
        pbar_ad(km+1) = 0.0
        pe1_ad = 0.0
      ELSE
        pe1_ad = 0.0
        ...
        DO n=ms,1,-1
          ...
          ELSE IF (branch .EQ. 1) THEN
            ...
            DO k=km,ad_from7,-1
              CALL POPREALARRAY(w2(i, k))
              temp_ad22 = w2_ad(i, k)/dm(k)
              wm_ad(k) = wm_ad(k) + temp_ad22
              pbar_ad(k+1) = pbar_ad(k+1) + temp_ad22
              pbar_ad(k) = pbar_ad(k) - temp_ad22
              dm_ad(k) = dm_ad(k) - (wm(k)+pbar(k+1)-pbar(k))*temp_ad22/&
&               dm(k)
              w2_ad(i, k) = 0.0
              CALL POPREALARRAY(dz2(i, k))
              dz_ad(k) = dz_ad(k) + dz2_ad(i, k)
          ...
          DO k=km,ad_from4,-1
            ...
            temp_ad20 = wbar_ad(k)/(m_top(k)+m_bot(k))
            temp_ad21 = -((r_bot(k)+r_top(k))*temp_ad20/(m_top(k)+m_bot(&
&             k)))
            r_bot_ad(k) = r_bot_ad(k) + temp_ad20
            r_top_ad(k) = r_top_ad(k) + temp_ad20
            m_top_ad(k) = m_top_ad(k) + temp_ad21
            m_bot_ad(k) = m_bot_ad(k) + temp_ad21
            wbar_ad(k) = 0.0
          END DO
          CALL POPINTEGER(k)
          CALL POPCONTROL(1,branch)
          IF (branch .NE. 0) THEN
            ...
            temp_ad19 = wbar_ad(1)/m_bot(1)
            r_bot_ad(1) = r_bot_ad(1) + temp_ad19
            m_bot_ad(1) = m_bot_ad(1) - r_bot(1)*temp_ad19/m_bot(1)
            wbar_ad(1) = 0.0
          END IF
          CALL POPCONTROL(1,branch)
          IF (branch .NE. 0) THEN
            ...
            DO ke=km,ad_from3,-1
              ...
              IF (branch .EQ. 0) THEN
                ...
                temp_ad18 = z_frac_ad/dts(k)
                time_left_ad = temp_ad18
                dts_ad(k) = dts_ad(k) - time_left*temp_ad18/dts(k)
              ELSE
                IF (branch .EQ. 1) THEN
...
&                   )
                  CALL POPREALARRAY(m_bot(ke))
                  dm_ad(k) = dm_ad(k) + z_frac*m_bot_ad(ke)
                  temp_ad17 = z_frac_ad/dts(k)
                  time_left_ad = temp_ad17
                  dts_ad(k) = dts_ad(k) - time_left*temp_ad17/dts(k)
                ELSE
                  time_left_ad = 0.0
                  m_surf_ad = 0.0
                END IF
...
&             (k)*m_bot_ad(k)
 ...
 100      CALL POPINTEGER(ad_count0)
          ...
          DO k=km,ad_from,-1
            ...
            rden = -(rgas*dm(k)/dz(k))
            temp3 = pf/rden
            temp4 = SQRT(grg*temp3)
            IF (grg*temp3 .EQ. 0.0) THEN
              temp_ad12 = 0.0
            ELSE
              temp_ad12 = grg*dz(k)*dts_ad(k)/(2.0*temp4**3*rden)
            END IF
            pf_ad = temp_ad12 + dts(k)*ptmp1_ad
            CALL POPREALARRAY(dts(k))
            CALL POPREALARRAY(pf)
            temp2 = LOG(rden*pt1(k))
            temp_ad15 = EXP(gm2(i, k)*temp2)*pf_ad
            temp_ad14 = gm2(i, k)*temp_ad15/(rden*pt1(k))
            rden_ad = pt1(k)*temp_ad14 - temp3*temp_ad12
            gm2_ad(i, k) = gm2_ad(i, k) + temp2*temp_ad15
            pt1_ad(k) = pt1_ad(k) + rden*temp_ad14
            temp_ad13 = -(rgas*rden_ad/dz(k))
            dz_ad(k) = dz_ad(k) - dm(k)*temp_ad13/dz(k) - dts_ad(k)/&
&             temp4
            dts_ad(k) = 0.0
            dm_ad(k) = dm_ad(k) + temp_ad13
          END DO
          CALL POPINTEGER(k)
        END DO
        CALL POPCONTROL(2,branch)
        ...
        ELSE IF (branch .EQ. 1) THEN
          ...
          ELSE
            ...
            DO k=ad_to2,1,-1
              CALL POPREALARRAY(w2(i, k))
              temp_ad11 = w2_ad(i, k)/dm(k)
              wm_ad(k) = wm_ad(k) + temp_ad11
              pbar_ad(k+1) = pbar_ad(k+1) + temp_ad11
              pbar_ad(k) = pbar_ad(k) - temp_ad11
              dm_ad(k) = dm_ad(k) - (wm(k)+pbar(k+1)-pbar(k))*temp_ad11/&
&               dm(k)
              w2_ad(i, k) = 0.0
              CALL POPREALARRAY(dz2(i, k))
              dz_ad(k) = dz_ad(k) + dz2_ad(i, k)
      ...
      DO k=ad_to0,2,-1
        ...
        pe1_ad(k) = 0.0
        CALL POPREALARRAY(pbar(k))
        temp_ad6 = bdt*pbar_ad(k)
        wbar_ad(k) = wbar_ad(k) + cm(k-1)*temp_ad6
        pp_ad(k-1) = pp_ad(k-1) + temp_ad6
        pbar_ad(k) = 0.0
        temp_ad8 = wbar_ad(k)/(cm(k-1)+cm(k))
        wc_ad(k-1) = wc_ad(k-1) + temp_ad8 - temp_ad6
        temp_ad7 = -((wc(k-1)+wc(k)+pp(k)-pp(k-1))*temp_ad8/(cm(k-1)+cm(&
&         k)))
        cm_ad(k-1) = cm_ad(k-1) + temp_ad7 + wbar(k)*temp_ad6
        CALL POPREALARRAY(wbar(k))
        wc_ad(k) = wc_ad(k) + temp_ad8
        pp_ad(k) = pp_ad(k) + temp_ad8
        pp_ad(k-1) = pp_ad(k-1) - temp_ad8
        cm_ad(k) = cm_ad(k) + temp_ad7
        wbar_ad(k) = 0.0
      END DO
      CALL POPREALARRAY(wbar(1))
      temp_ad5 = wbar_ad(1)/cm(1)
      wc_ad(1) = wc_ad(1) + temp_ad5
      pp_ad(1) = pp_ad(1) + temp_ad5
      cm_ad(1) = cm_ad(1) - (wc(1)+pp(1))*temp_ad5/cm(1)
      wbar_ad(1) = 0.0
      CALL POPINTEGER(ad_to)
      DO k=ad_to,1,-1
        temp_ad4 = cm_ad(k)/dts(k)
        CALL POPREALARRAY(pp(k))
        pf1_ad(k) = pf1_ad(k) + pp_ad(k)
        pm2_ad(i, k) = pm2_ad(i, k) - pp_ad(k)
        pp_ad(k) = 0.0
        CALL POPREALARRAY(wc(k))
        temp_ad3 = wc_ad(k)/dts(k)
        wm_ad(k) = wm_ad(k) + temp_ad3
        dts_ad(k) = dts_ad(k) - dm(k)*temp_ad4/dts(k) - wm(k)*temp_ad3/&
&         dts(k)
        wc_ad(k) = 0.0
        CALL POPREALARRAY(cm(k))
        dm_ad(k) = dm_ad(k) + temp_ad4
        cm_ad(k) = 0.0
      END DO
      CALL POPINTEGER(k)
 110  CALL POPINTEGER(ad_count)
      DO i0=1,ad_count
        ...
        CALL POPREALARRAY(dts(k))
        temp0 = pf1(k)/rden
        temp1 = SQRT(grg*temp0)
        IF (grg*temp0 .EQ. 0.0) THEN
          temp_ad = 0.0
        ELSE
          temp_ad = grg*dz(k)*dts_ad(k)/(2.0*temp1**3*rden)
        END IF
        pf1_ad(k) = pf1_ad(k) + temp_ad
        CALL POPREALARRAY(pf1(k))
        temp = LOG(rden*pt1(k))
        temp_ad2 = EXP(gm2(i, k)*temp)*pf1_ad(k)
        temp_ad1 = gm2(i, k)*temp_ad2/(rden*pt1(k))
        rden_ad = pt1(k)*temp_ad1 - temp0*temp_ad
        gm2_ad(i, k) = gm2_ad(i, k) + temp*temp_ad2
        pt1_ad(k) = pt1_ad(k) + rden*temp_ad1
        pf1_ad(k) = 0.0
        temp_ad0 = -(rgas*rden_ad/dz(k))
        dz_ad(k) = dz_ad(k) - dm(k)*temp_ad0/dz(k) - dts_ad(k)/temp1
        dts_ad(k) = 0.0
...
!py_corners_fb tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.g
!reat_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: ws gm2 dm2 dz2 w2 pm2 cp2 pem
!                pe pt2
!   with respect to varying inputs: ws gm2 dm2 dz2 w2 pm2 cp2 pem
!                pe pt2
...
! Local
    ...
    DO k=1,km
      DO i=is,ie
        ...
        pe(i, k) = EXP(gm2(i, k)*LOG(-(dm2(i, k)/dz2(i, k)*rgas*pt2(i, k&
&         )))) - pm2(i, k)
      END DO
    END DO
    DO k=1,km-1
...
! Start the w-solver
    DO k=2,km
      DO i=is,ie
        aa(i, k) = t1g*0.5*(gm2(i, k-1)+gm2(i, k))/(dz2(i, k-1)+dz2(i, k&
&         ))*(pem(i, k)+pp(i, k))
      END DO
    END DO
    DO i=is,ie
...
&       *ws(i)-aa(i, km)*w2(i, km-1))/bet(i)
    ...
    DO i=is,ie
      ...
      END IF
      CALL PUSHREALARRAY(dz2(i, km))
      dz2(i, km) = -(dm2(i, km)*rgas*pt2(i, km)*EXP((cp2(i, km)-1.)*LOG(&
&       max1)))
    END DO
    DO k=km-1,1,-1
      DO i=is,ie
...
&         - g_rat(i, k)*p1(i)
        ...
        END IF
        CALL PUSHREALARRAY(dz2(i, k))
        dz2(i, k) = -(dm2(i, k)*rgas*pt2(i, k)*EXP((cp2(i, k)-1.)*LOG(&
&         max2)))
      END DO
    END DO
    CALL PUSHREALARRAY(gam, (ie-is+1)*km)
...
!   with respect to varying inputs: ws gm2 dm2 dz2 w2 pm2 cp2 pem
!                pe pt2
  SUBROUTINE SIM1_SOLVER_BWD(dt, is, ie, km, rgas, gama, gm2, gm2_ad, &
&   cp2, cp2_ad, kappa, pe, pe_ad, dm2, dm2_ad, pm2, pm2_ad, pem, pem_ad&
&   , w2, w2_ad, dz2, dz2_ad, pt2, pt2_ad, ws, ws_ad, p_fac)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: is, ie, km
    REAL, INTENT(IN) :: dt, rgas, gama, kappa, p_fac
    REAL, DIMENSION(is:ie, km), INTENT(IN) :: dm2, pt2, pm2, gm2, cp2
    REAL, DIMENSION(is:ie, km) :: dm2_ad, pt2_ad, pm2_ad, gm2_ad, cp2_ad
    REAL, INTENT(IN) :: ws(is:ie)
    REAL :: ws_ad(is:ie)
    REAL, DIMENSION(is:ie, km+1), INTENT(IN) :: pem
    REAL, DIMENSION(is:ie, km+1) :: pem_ad
...
&   gam_ad
    ...
    DO k=1,km-1,1
      DO i=ie,is,-1
        CALL POPREALARRAY(dz2(i, k))
        temp11 = LOG(max2)
        temp10 = (cp2(i, k)-1.)*temp11
        temp_ad19 = -(rgas*EXP(temp10)*dz2_ad(i, k))
        temp_ad20 = -(dm2(i, k)*pt2(i, k)*rgas*EXP(temp10)*dz2_ad(i, k))
        dm2_ad(i, k) = dm2_ad(i, k) + pt2(i, k)*temp_ad19
        pt2_ad(i, k) = pt2_ad(i, k) + dm2(i, k)*temp_ad19
        cp2_ad(i, k) = cp2_ad(i, k) + temp11*temp_ad20
        max2_ad = (cp2(i, k)-1.)*temp_ad20/max2
        dz2_ad(i, k) = 0.0
        CALL POPCONTROL(1,branch)
        IF (branch .EQ. 0) THEN
          CALL POPREALARRAY(max2)
        ...
        CALL POPREALARRAY(p1(i))
        temp_ad18 = r3*p1_ad(i)
        pe_ad(i, k) = pe_ad(i, k) + temp_ad18
        bb_ad(i, k) = bb_ad(i, k) + pe(i, k+1)*temp_ad18
        pe_ad(i, k+1) = pe_ad(i, k+1) + bb(i, k)*temp_ad18
        g_rat_ad(i, k) = g_rat_ad(i, k) + pe(i, k+2)*temp_ad18 - p1(i)*&
&         p1_ad(i)
        pe_ad(i, k+2) = pe_ad(i, k+2) + g_rat(i, k)*temp_ad18
        p1_ad(i) = -(g_rat(i, k)*p1_ad(i))
      END DO
    END DO
    DO i=ie,is,-1
      CALL POPREALARRAY(dz2(i, km))
      temp9 = LOG(max1)
      temp8 = (cp2(i, km)-1.)*temp9
      temp_ad16 = -(rgas*EXP(temp8)*dz2_ad(i, km))
      temp_ad17 = -(dm2(i, km)*pt2(i, km)*rgas*EXP(temp8)*dz2_ad(i, km))
      dm2_ad(i, km) = dm2_ad(i, km) + pt2(i, km)*temp_ad16
      pt2_ad(i, km) = pt2_ad(i, km) + dm2(i, km)*temp_ad16
      cp2_ad(i, km) = cp2_ad(i, km) + temp9*temp_ad17
      max1_ad = (cp2(i, km)-1.)*temp_ad17/max1
      dz2_ad(i, km) = 0.0
      CALL POPCONTROL(1,branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY(max1)
    ...
    DO k=km,1,-1
      DO i=ie,is,-1
        ...
        temp_ad15 = rdt*dm2(i, k)*pe_ad(i, k+1)
...
&         +1)
    ...
    DO i=ie,is,-1
      CALL POPREALARRAY(w2(i, km))
      temp_ad11 = w2_ad(i, km)/bet(i)
      w1_ad(i, km) = w1_ad(i, km) + dm2(i, km)*temp_ad11
      pp_ad(i, km+1) = pp_ad(i, km+1) + dt*temp_ad11
      pp_ad(i, km) = pp_ad(i, km) - dt*temp_ad11
      ws_ad(i) = ws_ad(i) - p1(i)*temp_ad11
      w2_ad(i, km-1) = w2_ad(i, km-1) - aa(i, km)*temp_ad11
      bet_ad(i) = bet_ad(i) - (dm2(i, km)*w1(i, km)+dt*(pp(i, km+1)-pp(i&
&       , km))-p1(i)*ws(i)-aa(i, km)*w2(i, km-1))*temp_ad11/bet(i)
      dm2_ad(i, km) = dm2_ad(i, km) + bet_ad(i) + w1(i, km)*temp_ad11
      p1_ad(i) = p1_ad(i) - bet_ad(i) - ws(i)*temp_ad11
      w2_ad(i, km) = 0.0
      CALL POPREALARRAY(bet(i))
      gam_ad(i, km) = gam_ad(i, km) - aa(i, km)*bet_ad(i)
      temp_ad12 = gam_ad(i, km)/bet(i)
...
&       temp_ad12 - w2(i, km-1)*temp_ad11
      ...
      gam_ad(i, km) = 0.0
      temp7 = dz2(i, km)
      temp6 = gm2(i, km)/temp7
      temp_ad13 = t1g*p1_ad(i)
      temp_ad14 = (pem(i, km+1)+pp(i, km+1))*temp_ad13/temp7
      pem_ad(i, km+1) = pem_ad(i, km+1) + temp6*temp_ad13
      pp_ad(i, km+1) = pp_ad(i, km+1) + temp6*temp_ad13
      gm2_ad(i, km) = gm2_ad(i, km) + temp_ad14
      dz2_ad(i, km) = dz2_ad(i, km) - temp6*temp_ad14
      p1_ad(i) = 0.0
    END DO
    DO k=km-1,2,-1
      DO i=ie,is,-1
...
&         , k))-aa(i, k)*w2(i, k-1))*temp_ad9/bet(i)
...
&         , k-1)*temp_ad9
    ...
    DO i=ie,is,-1
      ...
      pp_ad(i, 2) = pp_ad(i, 2) + dt*temp_ad8
      bet_ad(i) = bet_ad(i) - (dm2(i, 1)*w1(i, 1)+dt*pp(i, 2))*temp_ad8/&
&       bet(i)
    ...
    DO k=km,2,-1
      DO i=ie,is,-1
        temp3 = dz2(i, k-1) + dz2(i, k)
        temp5 = pem(i, k) + pp(i, k)
        temp4 = gm2(i, k-1) + gm2(i, k)
        temp_ad6 = t1g*0.5*aa_ad(i, k)/temp3
        temp_ad7 = -(temp4*temp5*temp_ad6/temp3)
        gm2_ad(i, k-1) = gm2_ad(i, k-1) + temp5*temp_ad6
        gm2_ad(i, k) = gm2_ad(i, k) + temp5*temp_ad6
        pem_ad(i, k) = pem_ad(i, k) + temp4*temp_ad6
        pp_ad(i, k) = pp_ad(i, k) + temp4*temp_ad6
        dz2_ad(i, k-1) = dz2_ad(i, k-1) + temp_ad7
        dz2_ad(i, k) = dz2_ad(i, k) + temp_ad7
        aa_ad(i, k) = 0.0
      END DO
    ...
    DO k=km,2,-1
      DO i=ie,is,-1
        ...
        temp_ad4 = pp_ad(i, k+1)/bet(i)
        dd_ad(i, k) = dd_ad(i, k) + temp_ad4
        pp_ad(i, k) = pp_ad(i, k) - temp_ad4
        bet_ad(i) = bet_ad(i) - (dd(i, k)-pp(i, k))*temp_ad4/bet(i)
        pp_ad(i, k+1) = 0.0
        CALL POPREALARRAY(bet(i))
        bb_ad(i, k) = bb_ad(i, k) + bet_ad(i)
        gam_ad(i, k) = gam_ad(i, k) - bet_ad(i)
    ...
    DO k=km-1,1,-1
      DO i=ie,is,-1
        ...
        pe_ad(i, k) = pe_ad(i, k) + temp_ad1
        g_rat_ad(i, k) = g_rat_ad(i, k) + 2.*bb_ad(i, k) + pe(i, k+1)*&
&         temp_ad1
        pe_ad(i, k+1) = pe_ad(i, k+1) + g_rat(i, k)*temp_ad1
        dd_ad(i, k) = 0.0
        bb_ad(i, k) = 0.0
        temp_ad2 = g_rat_ad(i, k)/dm2(i, k+1)
        dm2_ad(i, k) = dm2_ad(i, k) + temp_ad2
        dm2_ad(i, k+1) = dm2_ad(i, k+1) - dm2(i, k)*temp_ad2/dm2(i, k+1)
        g_rat_ad(i, k) = 0.0
      END DO
    END DO
    DO k=km,1,-1
      DO i=ie,is,-1
        ...
        temp2 = dz2(i, k)
        temp1 = dm2(i, k)*pt2(i, k)
        temp = temp1/temp2
        temp0 = LOG(-(rgas*temp))
        temp_ad = EXP(gm2(i, k)*temp0)*pe_ad(i, k)
        temp_ad0 = gm2(i, k)*temp_ad/(temp*temp2)
        gm2_ad(i, k) = gm2_ad(i, k) + temp0*temp_ad
        dm2_ad(i, k) = dm2_ad(i, k) + pt2(i, k)*temp_ad0
        pt2_ad(i, k) = pt2_ad(i, k) + dm2(i, k)*temp_ad0
        dz2_ad(i, k) = dz2_ad(i, k) - temp*temp_ad0
...
!   gradient     of useful results: ws gm2 pe2 dm2 dz2 w2 pm2 cp2
!                pem pt2
!   with respect to varying inputs: ws gm2 pe2 dm2 dz2 w2 pm2 cp2
!                pem pt2
...
! P_g perturbation
        CALL PUSHREALARRAY(pe2(i, k))
        pe2(i, k) = EXP(gm2(i, k)*LOG(-(dm2(i, k)/dz2(i, k)*rgas*pt2(i, &
&         k)))) - pm2(i, k)
      END DO
    END DO
    DO k=1,km-1
...
!   with respect to varying inputs: ws gm2 pe2 dm2 dz2 w2 pm2 cp2
!                pem pt2
  SUBROUTINE SIM_SOLVER_BWD(dt, is, ie, km, rgas, gama, gm2, gm2_ad, cp2&
&   , cp2_ad, kappa, pe2, pe2_ad, dm2, dm2_ad, pm2, pm2_ad, pem, pem_ad&
&   , w2, w2_ad, dz2, dz2_ad, pt2, pt2_ad, ws, ws_ad, alpha, p_fac, &
&   scale_m)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: is, ie, km
    REAL, INTENT(IN) :: dt, rgas, gama, kappa, p_fac, alpha, scale_m
...
&   g_rat_ad, gam_ad
    ...
    DO k=1,km-1,1
      DO i=ie,is,-1
        ...
        temp11 = LOG(max2)
        temp10 = (cp2(i, k)-1.)*temp11
        temp_ad21 = -(rgas*EXP(temp10)*dz2_ad(i, k))
        temp_ad22 = -(dm2(i, k)*pt2(i, k)*rgas*EXP(temp10)*dz2_ad(i, k))
        dm2_ad(i, k) = dm2_ad(i, k) + pt2(i, k)*temp_ad21
        pt2_ad(i, k) = pt2_ad(i, k) + dm2(i, k)*temp_ad21
        cp2_ad(i, k) = cp2_ad(i, k) + temp11*temp_ad22
        max2_ad = (cp2(i, k)-1.)*temp_ad22/max2
        dz2_ad(i, k) = 0.0
        CALL POPCONTROL(1,branch)
        IF (branch .EQ. 0) THEN
          CALL POPREALARRAY(max2)
        ...
        END IF
        CALL POPREALARRAY(p1(i))
        temp_ad20 = r3*p1_ad(i)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad20
        bb_ad(i, k) = bb_ad(i, k) + pe2(i, k+1)*temp_ad20
        pe2_ad(i, k+1) = pe2_ad(i, k+1) + bb(i, k)*temp_ad20
        g_rat_ad(i, k) = g_rat_ad(i, k) + pe2(i, k+2)*temp_ad20 - p1(i)*&
&         p1_ad(i)
        pe2_ad(i, k+2) = pe2_ad(i, k+2) + g_rat(i, k)*temp_ad20
        p1_ad(i) = -(g_rat(i, k)*p1_ad(i))
      END DO
    END DO
    DO i=ie,is,-1
      ...
      temp9 = LOG(max1)
      temp8 = (cp2(i, km)-1.)*temp9
      temp_ad18 = -(rgas*EXP(temp8)*dz2_ad(i, km))
      temp_ad19 = -(dm2(i, km)*pt2(i, km)*rgas*EXP(temp8)*dz2_ad(i, km))
      dm2_ad(i, km) = dm2_ad(i, km) + pt2(i, km)*temp_ad18
      pt2_ad(i, km) = pt2_ad(i, km) + dm2(i, km)*temp_ad18
      cp2_ad(i, km) = cp2_ad(i, km) + temp9*temp_ad19
      max1_ad = (cp2(i, km)-1.)*temp_ad19/max1
      dz2_ad(i, km) = 0.0
      CALL POPCONTROL(1,branch)
      IF (branch .EQ. 0) THEN
        CALL POPREALARRAY(max1)
    ...
    DO k=km,1,-1
      DO i=ie,is,-1
        CALL POPREALARRAY(pe2(i, k+1))
        temp_ad16 = ra*pe2_ad(i, k+1)
        temp_ad17 = rdt*dm2(i, k)*temp_ad16
        pe2_ad(i, k) = pe2_ad(i, k) + pe2_ad(i, k+1)
        dm2_ad(i, k) = dm2_ad(i, k) + rdt*(w2(i, k)-w1(i, k))*temp_ad16
        w2_ad(i, k) = w2_ad(i, k) + temp_ad17
        w1_ad(i, k) = w1_ad(i, k) - temp_ad17
        pp_ad(i, k+1) = pp_ad(i, k+1) - beta*temp_ad16
        pp_ad(i, k) = pp_ad(i, k) + beta*temp_ad16
        pe2_ad(i, k+1) = 0.0
      END DO
    END DO
    DO i=ie,is,-1
    ...
    DO i=ie,is,-1
      CALL POPREALARRAY(w2(i, km))
      temp_ad13 = w2_ad(i, km)/bet(i)
      temp7 = t2*w1(i, km) - ra*ws(i)
      w1_ad(i, km) = w1_ad(i, km) + (wk1(i)*t2+dm2(i, km))*temp_ad13
      pp_ad(i, km+1) = pp_ad(i, km+1) + dt*temp_ad13
      pp_ad(i, km) = pp_ad(i, km) - dt*temp_ad13
      wk_ad(i, km) = wk_ad(i, km) - temp_ad13
      ws_ad(i) = ws_ad(i) - wk1(i)*ra*temp_ad13
      w2_ad(i, km-1) = w2_ad(i, km-1) - aa(i, km)*temp_ad13
      bet_ad(i) = bet_ad(i) - (dm2(i, km)*w1(i, km)+dt*(pp(i, km+1)-pp(i&
&       , km))-wk(i, km)+wk1(i)*temp7-aa(i, km)*w2(i, km-1))*temp_ad13/&
&       bet(i)
      dm2_ad(i, km) = dm2_ad(i, km) + bet_ad(i) + w1(i, km)*temp_ad13
      wk1_ad(i) = wk1_ad(i) + temp7*temp_ad13 - bet_ad(i)
      w2_ad(i, km) = 0.0
      CALL POPREALARRAY(bet(i))
      gam_ad(i, km) = gam_ad(i, km) - aa(i, km)*bet_ad(i)
      temp_ad14 = gam_ad(i, km)/bet(i)
      aa_ad(i, km) = aa_ad(i, km) + ((-1.0)-gam(i, km))*bet_ad(i) + &
&       temp_ad14 - w2(i, km-1)*temp_ad13
      bet_ad(i) = -(aa(i, km)*temp_ad14/bet(i))
      CALL POPREALARRAY(gam(i, km))
      gam_ad(i, km) = 0.0
      temp6 = dz2(i, km)
      temp5 = gm2(i, km)/temp6
      temp_ad15 = pe2(i, km+1)*t1g*wk1_ad(i)/temp6
      pe2_ad(i, km+1) = pe2_ad(i, km+1) + t1g*temp5*wk1_ad(i)
      gm2_ad(i, km) = gm2_ad(i, km) + temp_ad15
      dz2_ad(i, km) = dz2_ad(i, km) - temp5*temp_ad15
      wk1_ad(i) = 0.0
    END DO
    DO k=km-1,2,-1
      DO i=ie,is,-1
        CALL POPREALARRAY(w2(i, k))
        temp_ad11 = w2_ad(i, k)/bet(i)
        w1_ad(i, k) = w1_ad(i, k) + dm2(i, k)*temp_ad11
        pp_ad(i, k+1) = pp_ad(i, k+1) + dt*temp_ad11
        pp_ad(i, k) = pp_ad(i, k) - dt*temp_ad11
        wk_ad(i, k+1) = wk_ad(i, k+1) + temp_ad11
        wk_ad(i, k) = wk_ad(i, k) - temp_ad11
        w2_ad(i, k-1) = w2_ad(i, k-1) - aa(i, k)*temp_ad11
        bet_ad(i) = bet_ad(i) - (dm2(i, k)*w1(i, k)+dt*(pp(i, k+1)-pp(i&
&         , k))+wk(i, k+1)-wk(i, k)-aa(i, k)*w2(i, k-1))*temp_ad11/bet(i&
&         )
        dm2_ad(i, k) = dm2_ad(i, k) + bet_ad(i) + w1(i, k)*temp_ad11
        aa_ad(i, k) = aa_ad(i, k) + ((-1.0)-gam(i, k))*bet_ad(i) - w2(i&
&         , k-1)*temp_ad11
        w2_ad(i, k) = 0.0
        CALL POPREALARRAY(bet(i))
        ...
        bet_ad(i) = -(aa(i, k)*temp_ad12/bet(i))
        aa_ad(i, k) = aa_ad(i, k) + temp_ad12
        gam_ad(i, k) = 0.0
      END DO
    END DO
    DO i=ie,is,-1
      CALL POPREALARRAY(w2(i, 1))
      temp_ad10 = w2_ad(i, 1)/bet(i)
      w1_ad(i, 1) = w1_ad(i, 1) + dm2(i, 1)*temp_ad10
      pp_ad(i, 2) = pp_ad(i, 2) + dt*temp_ad10
      wk_ad(i, 2) = wk_ad(i, 2) + temp_ad10
      bet_ad(i) = bet_ad(i) - (dm2(i, 1)*w1(i, 1)+dt*pp(i, 2)+wk(i, 2))*&
&       temp_ad10/bet(i)
      dm2_ad(i, 1) = dm2_ad(i, 1) + bet_ad(i) + w1(i, 1)*temp_ad10
      w2_ad(i, 1) = 0.0
      CALL POPREALARRAY(bet(i))
      aa_ad(i, 2) = aa_ad(i, 2) - bet_ad(i)
      bet_ad(i) = 0.0
    ...
    DO k=km,2,-1
      DO i=ie,is,-1
        ...
        temp_ad6 = t2*aa(i, k)*wk_ad(i, k)
        aa_ad(i, k) = aa_ad(i, k) + t2*(w1(i, k-1)-w1(i, k))*wk_ad(i, k)
        w1_ad(i, k-1) = w1_ad(i, k-1) + temp_ad6
        w1_ad(i, k) = w1_ad(i, k) - temp_ad6
        wk_ad(i, k) = 0.0
        temp4 = dz2(i, k-1) + dz2(i, k)
        temp3 = gm2(i, k-1) + gm2(i, k)
        temp_ad7 = t1g*0.5*aa_ad(i, k)/temp4
        temp_ad8 = pe2(i, k)*temp_ad7
        temp_ad9 = -(temp3*pe2(i, k)*temp_ad7/temp4)
        gm2_ad(i, k-1) = gm2_ad(i, k-1) + temp_ad8
        gm2_ad(i, k) = gm2_ad(i, k) + temp_ad8
        pe2_ad(i, k) = pe2_ad(i, k) + temp3*temp_ad7
        dz2_ad(i, k-1) = dz2_ad(i, k-1) + temp_ad9
        dz2_ad(i, k) = dz2_ad(i, k) + temp_ad9
        aa_ad(i, k) = 0.0
      END DO
    END DO
    DO k=km+1,1,-1
    ...
    DO k=km-1,1,-1
      DO i=ie,is,-1
        temp_ad1 = 3.*dd_ad(i, k)
        pe2_ad(i, k) = pe2_ad(i, k) + temp_ad1
        g_rat_ad(i, k) = g_rat_ad(i, k) + 2.*bb_ad(i, k) + pe2(i, k+1)*&
&         temp_ad1
        pe2_ad(i, k+1) = pe2_ad(i, k+1) + g_rat(i, k)*temp_ad1
        dd_ad(i, k) = 0.0
        bb_ad(i, k) = 0.0
        temp_ad2 = g_rat_ad(i, k)/dm2(i, k+1)
        dm2_ad(i, k) = dm2_ad(i, k) + temp_ad2
    ...
    DO k=km,1,-1
      DO i=ie,is,-1
        ...
        temp = temp1/temp2
        temp0 = LOG(-(rgas*temp))
        temp_ad = EXP(gm2(i, k)*temp0)*pe2_ad(i, k)
        temp_ad0 = gm2(i, k)*temp_ad/(temp*temp2)
        gm2_ad(i, k) = gm2_ad(i, k) + temp0*temp_ad
        dm2_ad(i, k) = dm2_ad(i, k) + pt2(i, k)*temp_ad0
        pt2_ad(i, k) = pt2_ad(i, k) + dm2(i, k)*temp_ad0
...
!opy_corners_fb tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.
!great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: pk3 cappa q_con gz delp delz
!                pkc pt
!   with respect to varying inputs: pk3 cappa q_con gz delp delz
!                pkc pt
  SUBROUTINE NEST_HALO_NH_FWD(ptop, grav, kappa, cp, delp, delz, pt, &
&   phis, q_con, cappa, pkc, gz, pk3, npx, npy, npz, nested, pkc_pertn, &
&   computepk3, fullhalo, bd)
    IMPLICIT NONE
!INPUT: delp, delz, pt
...
!Hydrostatic interface pressure
          ...
          DO k=2,npz+1
            DO i=ifirst,0
              ...
              CALL PUSHREALARRAY(peg(i, k))
              peg(i, k) = peg(i, k-1) + delp(i, j, k-1)*(1.-q_con(i, j, &
&               k-1))
              CALL PUSHREALARRAY(pelng(i, k))
              pelng(i, k) = LOG(peg(i, k))
            END DO
          END DO
...
!Full p
              ...
              pkz(i, k) = EXP(1./(1.-cappa(i, j, k))*LOG(rdg*delp(i, j, &
&               k)/delz(i, j, k)*pt(i, j, k)))
!hydro
...
&                     +1))
        ...
        CALL PUSHREALARRAY(peg, (bd%ied-bd%isd+1)*(npz+1))
        CALL PUSHREALARRAY(rdg)
        CALL PUSHINTEGER(jlast)
        CALL PUSHREALARRAY(pelng, (bd%ied-bd%isd+1)*(npz+1))
...
&   , delz_ad, pt, pt_ad, phis, q_con, q_con_ad, cappa, cappa_ad, pkc, &
&   pkc_ad, gz, gz_ad, pk3, pk3_ad, npx, npy, npz, nested, pkc_pertn, &
&   computepk3, fullhalo, bd)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: npx, npy, npz
    LOGICAL, INTENT(IN) :: pkc_pertn, computepk3, fullhalo, nested
...
&   , delz_ad
    REAL, DIMENSION(bd%isd:bd%ied, bd%jsd:bd%jed, npz), INTENT(IN) :: &
&   q_con
    REAL, DIMENSION(bd%isd:bd%ied, bd%jsd:bd%jed, npz) :: q_con_ad
    REAL, DIMENSION(bd%isd:bd%ied, bd%jsd:bd%jed, npz), INTENT(INOUT) ::&
&   cappa
    REAL, DIMENSION(bd%isd:bd%ied, bd%jsd:bd%jed, npz), INTENT(INOUT) ::&
&   cappa_ad
...
&   peln_ad
    REAL, DIMENSION(bd%isd:bd%ied, npz+1) :: peg, pelng
    REAL, DIMENSION(bd%isd:bd%ied, npz+1) :: peg_ad, pelng_ad
    REAL, DIMENSION(bd%isd:bd%ied, npz) :: gam, bb, dd, pkz
...
&                    jsd+1))
        ...
        CALL POPREALARRAY(pelng, (bd%ied-bd%isd+1)*(npz+1))
        CALL POPINTEGER(jlast)
        CALL POPREALARRAY(rdg)
        CALL POPREALARRAY(peg, (bd%ied-bd%isd+1)*(npz+1))
...
&               , j, k)
          ...
          DO k=npz,2,-1
            DO i=ilast,ifirst,-1
              ...
              temp_ad31 = pkc_ad(i, j, k+1)/bet(i)
              dd_ad(i, k) = dd_ad(i, k) + temp_ad31
              pkc_ad(i, j, k) = pkc_ad(i, j, k) - temp_ad31
              bet_ad(i) = bet_ad(i) - (dd(i, k)-pkc(i, j, k))*temp_ad31/&
&               bet(i)
              pkc_ad(i, j, k+1) = 0.0
              CALL POPREALARRAY(bet(i))
              bb_ad(i, k) = bb_ad(i, k) + bet_ad(i)
              gam_ad(i, k) = gam_ad(i, k) - bet_ad(i)
              CALL POPREALARRAY(gam(i, k))
              temp_ad32 = gam_ad(i, k)/bet(i)
              bet_ad(i) = -(g_rat(i, k-1)*temp_ad32/bet(i))
              g_rat_ad(i, k-1) = g_rat_ad(i, k-1) + temp_ad32
              gam_ad(i, k) = 0.0
            END DO
          END DO
          DO i=ilast,ifirst,-1
            ...
            temp_ad30 = pkc_ad(i, j, 2)/bet(i)
            dd_ad(i, 1) = dd_ad(i, 1) + temp_ad30
            bet_ad(i) = bet_ad(i) - dd(i, 1)*temp_ad30/bet(i)
            pkc_ad(i, j, 2) = 0.0
          ...
          DO k=npz-1,1,-1
            DO i=ilast,ifirst,-1
              ...
              temp_ad28 = 3.*dd_ad(i, k)
              pkz_ad(i, k) = pkz_ad(i, k) + temp_ad28
              g_rat_ad(i, k) = g_rat_ad(i, k) + 2.*bb_ad(i, k) + pkz(i, &
&               k+1)*temp_ad28
              pkz_ad(i, k+1) = pkz_ad(i, k+1) + g_rat(i, k)*temp_ad28
              dd_ad(i, k) = 0.0
              bb_ad(i, k) = 0.0
              CALL POPREALARRAY(g_rat(i, k))
              temp_ad29 = g_rat_ad(i, k)/delp(i, j, k+1)
              delp_ad(i, j, k) = delp_ad(i, j, k) + temp_ad29
              delp_ad(i, j, k+1) = delp_ad(i, j, k+1) - delp(i, j, k)*&
&               temp_ad29/delp(i, j, k+1)
              g_rat_ad(i, k) = 0.0
            END DO
          END DO
          DO k=npz,1,-1
            DO i=ilast,ifirst,-1
              temp25 = -cappa(i, j, k) + 1.
              temp24 = delz(i, j, k)
              temp23 = delp(i, j, k)*pt(i, j, k)
              temp21 = temp23/temp24
              temp22 = LOG(rdg*temp21)
              temp20 = temp22/temp25
              temp_ad27 = EXP(temp20)*pkz_ad(i, k)/temp25
              temp_ad26 = temp_ad27/(temp21*temp24)
              pm_ad = -pkz_ad(i, k)
              temp26 = peln(i, k+1, j) - peln(i, k, j)
              temp_ad25 = -(delp(i, j, k)*pm_ad/temp26**2)
              delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*&
&               temp_ad26 + pm_ad/temp26
              peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + temp_ad25
              peln_ad(i, k, j) = peln_ad(i, k, j) - temp_ad25
              CALL POPREALARRAY(pkz(i, k))
              pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad26
              delz_ad(i, j, k) = delz_ad(i, j, k) - temp21*temp_ad26
              cappa_ad(i, j, k) = cappa_ad(i, j, k) + temp20*temp_ad27
              pkz_ad(i, k) = 0.0
            END DO
          END DO
          DO k=npz+1,2,-1
...
&               , j, k)
          ...
          DO k=npz,2,-1
            DO i=ilast,ifirst,-1
              CALL POPREALARRAY(pkc(i, j, k+1))
              temp_ad23 = pkc_ad(i, j, k+1)/bet(i)
              dd_ad(i, k) = dd_ad(i, k) + temp_ad23
              pkc_ad(i, j, k) = pkc_ad(i, j, k) - temp_ad23
              bet_ad(i) = bet_ad(i) - (dd(i, k)-pkc(i, j, k))*temp_ad23/&
&               bet(i)
              pkc_ad(i, j, k+1) = 0.0
              CALL POPREALARRAY(bet(i))
              bb_ad(i, k) = bb_ad(i, k) + bet_ad(i)
              gam_ad(i, k) = gam_ad(i, k) - bet_ad(i)
              CALL POPREALARRAY(gam(i, k))
              temp_ad24 = gam_ad(i, k)/bet(i)
              bet_ad(i) = -(g_rat(i, k-1)*temp_ad24/bet(i))
              g_rat_ad(i, k-1) = g_rat_ad(i, k-1) + temp_ad24
              gam_ad(i, k) = 0.0
            END DO
          END DO
          DO i=ilast,ifirst,-1
            ...
            bb_ad(i, npz) = 0.0
            CALL POPREALARRAY(pkc(i, j, 2))
            temp_ad22 = pkc_ad(i, j, 2)/bet(i)
            dd_ad(i, 1) = dd_ad(i, 1) + temp_ad22
            bet_ad(i) = bet_ad(i) - dd(i, 1)*temp_ad22/bet(i)
            pkc_ad(i, j, 2) = 0.0
            CALL POPREALARRAY(pkc(i, j, 1))
            pkc_ad(i, j, 1) = 0.0
            CALL POPREALARRAY(bet(i))
          ...
          DO k=npz-1,1,-1
            DO i=ilast,ifirst,-1
              CALL POPREALARRAY(dd(i, k))
              temp_ad20 = 3.*dd_ad(i, k)
              pkz_ad(i, k) = pkz_ad(i, k) + temp_ad20
              g_rat_ad(i, k) = g_rat_ad(i, k) + 2.*bb_ad(i, k) + pkz(i, &
&               k+1)*temp_ad20
              pkz_ad(i, k+1) = pkz_ad(i, k+1) + g_rat(i, k)*temp_ad20
              dd_ad(i, k) = 0.0
              bb_ad(i, k) = 0.0
              CALL POPREALARRAY(g_rat(i, k))
              temp_ad21 = g_rat_ad(i, k)/delp(i, j, k+1)
              delp_ad(i, j, k) = delp_ad(i, j, k) + temp_ad21
              delp_ad(i, j, k+1) = delp_ad(i, j, k+1) - delp(i, j, k)*&
&               temp_ad21/delp(i, j, k+1)
              g_rat_ad(i, k) = 0.0
            END DO
          END DO
          DO k=npz,1,-1
            DO i=ilast,ifirst,-1
              temp18 = -cappa(i, j, k) + 1.
              temp17 = delz(i, j, k)
              temp16 = delp(i, j, k)*pt(i, j, k)
              temp14 = temp16/temp17
              temp15 = LOG(rdg*temp14)
              temp13 = temp15/temp18
              temp_ad19 = EXP(temp13)*pkz_ad(i, k)/temp18
              temp_ad18 = temp_ad19/(temp14*temp17)
              pm_ad = -pkz_ad(i, k)
              temp19 = peln(i, k+1, j) - peln(i, k, j)
              temp_ad17 = -(delp(i, j, k)*pm_ad/temp19**2)
              delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*&
&               temp_ad18 + pm_ad/temp19
              peln_ad(i, k+1, j) = peln_ad(i, k+1, j) + temp_ad17
              peln_ad(i, k, j) = peln_ad(i, k, j) - temp_ad17
              CALL POPREALARRAY(pkz(i, k))
              pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad18
              delz_ad(i, j, k) = delz_ad(i, j, k) - temp14*temp_ad18
              cappa_ad(i, j, k) = cappa_ad(i, j, k) + temp13*temp_ad19
              pkz_ad(i, k) = 0.0
            END DO
          END DO
          DO k=npz+1,2,-1
...
&               , j, k)
          ...
          DO k=npz,2,-1
            DO i=ilast,npx,-1
              CALL POPREALARRAY(pkc(i, j, k+1))
              temp_ad15 = pkc_ad(i, j, k+1)/bet(i)
              dd_ad(i, k) = dd_ad(i, k) + temp_ad15
              pkc_ad(i, j, k) = pkc_ad(i, j, k) - temp_ad15
              bet_ad(i) = bet_ad(i) - (dd(i, k)-pkc(i, j, k))*temp_ad15/&
&               bet(i)
              pkc_ad(i, j, k+1) = 0.0
              CALL POPREALARRAY(bet(i))
              bb_ad(i, k) = bb_ad(i, k) + bet_ad(i)
              gam_ad(i, k) = gam_ad(i, k) - bet_ad(i)
              CALL POPREALARRAY(gam(i, k))
              temp_ad16 = gam_ad(i, k)/bet(i)
              bet_ad(i) = -(g_rat(i, k-1)*temp_ad16/bet(i))
              g_rat_ad(i, k-1) = g_rat_ad(i, k-1) + temp_ad16
              gam_ad(i, k) = 0.0
            END DO
          END DO
          DO i=ilast,npx,-1
            ...
            bb_ad(i, npz) = 0.0
            CALL POPREALARRAY(pkc(i, j, 2))
            temp_ad14 = pkc_ad(i, j, 2)/bet(i)
            dd_ad(i, 1) = dd_ad(i, 1) + temp_ad14
            bet_ad(i) = bet_ad(i) - dd(i, 1)*temp_ad14/bet(i)
            pkc_ad(i, j, 2) = 0.0
            CALL POPREALARRAY(pkc(i, j, 1))
            pkc_ad(i, j, 1) = 0.0
            CALL POPREALARRAY(bet(i))
          ...
          DO k=npz-1,1,-1
            DO i=ilast,npx,-1
              CALL POPREALARRAY(dd(i, k))
              temp_ad12 = 3.*dd_ad(i, k)
              pkz_ad(i, k) = pkz_ad(i, k) + temp_ad12
              g_rat_ad(i, k) = g_rat_ad(i, k) + 2.*bb_ad(i, k) + pkz(i, &
&               k+1)*temp_ad12
              pkz_ad(i, k+1) = pkz_ad(i, k+1) + g_rat(i, k)*temp_ad12
              dd_ad(i, k) = 0.0
              bb_ad(i, k) = 0.0
              CALL POPREALARRAY(g_rat(i, k))
              temp_ad13 = g_rat_ad(i, k)/delp(i, j, k+1)
              delp_ad(i, j, k) = delp_ad(i, j, k) + temp_ad13
              delp_ad(i, j, k+1) = delp_ad(i, j, k+1) - delp(i, j, k)*&
&               temp_ad13/delp(i, j, k+1)
              g_rat_ad(i, k) = 0.0
            END DO
          END DO
          DO k=npz,1,-1
            DO i=ilast,npx,-1
              pm_ad = -pkz_ad(i, k)
              temp12 = pelng(i, k+1) - pelng(i, k)
              temp_ad8 = pm_ad/temp12
              temp_ad9 = -((peg(i, k+1)-peg(i, k))*temp_ad8/temp12)
              peg_ad(i, k+1) = peg_ad(i, k+1) + temp_ad8
              peg_ad(i, k) = peg_ad(i, k) - temp_ad8
              pelng_ad(i, k+1) = pelng_ad(i, k+1) + temp_ad9
              pelng_ad(i, k) = pelng_ad(i, k) - temp_ad9
              CALL POPREALARRAY(pkz(i, k))
              temp11 = -cappa(i, j, k) + 1.
              temp10 = delz(i, j, k)
              temp9 = delp(i, j, k)*pt(i, j, k)
              temp7 = temp9/temp10
              temp8 = LOG(rdg*temp7)
              temp6 = temp8/temp11
              temp_ad10 = EXP(temp6)*pkz_ad(i, k)/temp11
              temp_ad11 = temp_ad10/(temp7*temp10)
              delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*&
&               temp_ad11
              pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad11
              delz_ad(i, j, k) = delz_ad(i, j, k) - temp7*temp_ad11
              cappa_ad(i, j, k) = cappa_ad(i, j, k) + temp6*temp_ad10
              pkz_ad(i, k) = 0.0
            END DO
          END DO
          DO k=npz+1,2,-1
...
&               , j)
              pe_ad(i, k-1, j) = pe_ad(i, k-1, j) + pe_ad(i, k, j)
              CALL POPREALARRAY(pelng(i, k))
              peg_ad(i, k) = peg_ad(i, k) + pelng_ad(i, k)/peg(i, k)
              pelng_ad(i, k) = 0.0
              CALL POPREALARRAY(peg(i, k))
              peg_ad(i, k-1) = peg_ad(i, k-1) + peg_ad(i, k)
              delp_ad(i, j, k-1) = delp_ad(i, j, k-1) + pe_ad(i, k, j) +&
&               (1.-q_con(i, j, k-1))*peg_ad(i, k)
              q_con_ad(i, j, k-1) = q_con_ad(i, j, k-1) - delp(i, j, k-1&
&               )*peg_ad(i, k)
              peg_ad(i, k) = 0.0
              peln_ad(i, k, j) = 0.0
              CALL POPREALARRAY(pe(i, k, j))
...
&               , j, k)
          ...
          DO k=npz,2,-1
            DO i=0,ifirst,-1
              CALL POPREALARRAY(pkc(i, j, k+1))
              temp_ad6 = pkc_ad(i, j, k+1)/bet(i)
              dd_ad(i, k) = dd_ad(i, k) + temp_ad6
              pkc_ad(i, j, k) = pkc_ad(i, j, k) - temp_ad6
              bet_ad(i) = bet_ad(i) - (dd(i, k)-pkc(i, j, k))*temp_ad6/&
&               bet(i)
              pkc_ad(i, j, k+1) = 0.0
              CALL POPREALARRAY(bet(i))
              bb_ad(i, k) = bb_ad(i, k) + bet_ad(i)
              gam_ad(i, k) = gam_ad(i, k) - bet_ad(i)
              CALL POPREALARRAY(gam(i, k))
              temp_ad7 = gam_ad(i, k)/bet(i)
              bet_ad(i) = -(g_rat(i, k-1)*temp_ad7/bet(i))
              g_rat_ad(i, k-1) = g_rat_ad(i, k-1) + temp_ad7
              gam_ad(i, k) = 0.0
            END DO
          END DO
          DO i=0,ifirst,-1
            ...
            bb_ad(i, npz) = 0.0
            CALL POPREALARRAY(pkc(i, j, 2))
            temp_ad5 = pkc_ad(i, j, 2)/bet(i)
            dd_ad(i, 1) = dd_ad(i, 1) + temp_ad5
            bet_ad(i) = bet_ad(i) - dd(i, 1)*temp_ad5/bet(i)
            pkc_ad(i, j, 2) = 0.0
            CALL POPREALARRAY(pkc(i, j, 1))
            pkc_ad(i, j, 1) = 0.0
            CALL POPREALARRAY(bet(i))
          ...
          DO k=npz-1,1,-1
            DO i=0,ifirst,-1
              CALL POPREALARRAY(dd(i, k))
              temp_ad3 = 3.*dd_ad(i, k)
              pkz_ad(i, k) = pkz_ad(i, k) + temp_ad3
              g_rat_ad(i, k) = g_rat_ad(i, k) + 2.*bb_ad(i, k) + pkz(i, &
&               k+1)*temp_ad3
              pkz_ad(i, k+1) = pkz_ad(i, k+1) + g_rat(i, k)*temp_ad3
              dd_ad(i, k) = 0.0
              bb_ad(i, k) = 0.0
              CALL POPREALARRAY(g_rat(i, k))
              temp_ad4 = g_rat_ad(i, k)/delp(i, j, k+1)
              delp_ad(i, j, k) = delp_ad(i, j, k) + temp_ad4
              delp_ad(i, j, k+1) = delp_ad(i, j, k+1) - delp(i, j, k)*&
&               temp_ad4/delp(i, j, k+1)
              g_rat_ad(i, k) = 0.0
            END DO
          END DO
          DO k=npz,1,-1
            DO i=0,ifirst,-1
              pm_ad = -pkz_ad(i, k)
              temp5 = pelng(i, k+1) - pelng(i, k)
              temp_ad = pm_ad/temp5
              temp_ad0 = -((peg(i, k+1)-peg(i, k))*temp_ad/temp5)
              peg_ad(i, k+1) = peg_ad(i, k+1) + temp_ad
              peg_ad(i, k) = peg_ad(i, k) - temp_ad
              pelng_ad(i, k+1) = pelng_ad(i, k+1) + temp_ad0
              pelng_ad(i, k) = pelng_ad(i, k) - temp_ad0
              CALL POPREALARRAY(pkz(i, k))
              temp4 = -cappa(i, j, k) + 1.
              temp3 = delz(i, j, k)
              temp2 = delp(i, j, k)*pt(i, j, k)
              temp0 = temp2/temp3
              temp1 = LOG(rdg*temp0)
              temp = temp1/temp4
              temp_ad1 = EXP(temp)*pkz_ad(i, k)/temp4
              temp_ad2 = temp_ad1/(temp0*temp3)
              delp_ad(i, j, k) = delp_ad(i, j, k) + pt(i, j, k)*temp_ad2
              pt_ad(i, j, k) = pt_ad(i, j, k) + delp(i, j, k)*temp_ad2
              delz_ad(i, j, k) = delz_ad(i, j, k) - temp0*temp_ad2
              cappa_ad(i, j, k) = cappa_ad(i, j, k) + temp*temp_ad1
              pkz_ad(i, k) = 0.0
            END DO
          END DO
          DO k=npz+1,2,-1
...
&   q_con, cappa, pkc, gz, pk3, npx, npy, npz, nested, pkc_pertn, &
&   computepk3, fullhalo, bd)
    IMPLICIT NONE
!INPUT: delp, delz, pt
!OUTPUT: gz, pkc, pk3 (optional)
