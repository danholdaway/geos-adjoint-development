...
module dyn_core_mod
  ...
  use nh_core_mod,        only: Riem_Solver3
  use nh_utils_mod,       only: Riem_Solver_C, update_dz_c, update_dz_d, nest_halo_nh
...
!-----------------------------------------------------------------------
 ...
 subroutine dyn_core(npx, npy, npz, ng, sphum, nq, bdt, n_split, zvir, cp, akap, cappa, grav, hydrostatic,  &
                     ...
                     ks, gridstruct, flagstruct, flagstructp, neststruct, idiag, bd, domain, &
    ...
    real, intent(inout) ::  delz(bd%isd:bd%ied,bd%jsd:bd%jed,npz)  ! delta-height (m, negative)
    real, intent(inout) :: cappa(bd%isd:bd%ied,bd%jsd:bd%jed,npz) ! moist kappa
...
! Accumulated Courant number arrays
    ...
    real :: pem(bd%is-1:bd%ie+1,npz+1,bd%js-1:bd%je+1), heat_source(bd%isd:bd%ied, bd%jsd:bd%jed, npz)
! Auto 1D & 2D arrays:
...
! Empty the "flux capacitors"
    mfx = 0.0!call init_ijk_mem(is, ie+1, js,  je,   npz, mfx, 0.)
    mfy = 0.0!call init_ijk_mem(is, ie  , js,  je+1, npz, mfy, 0.)
    cx = 0.0!call init_ijk_mem(is, ie+1, jsd, jed,  npz, cx, 0.)
    cy = 0.0!call init_ijk_mem(isd, ied, js,  je+1, npz, cy, 0.)
...
! damping on delp and vorticity:
       ...
       nord_v_pert(k) = min(2, flagstructp%nord_pert)
       d2_divg_pert = min(0.20, flagstructp%d2_bg_pert)

       if ( flagstructp%do_vort_damp_pert ) then
            damp_vt_pert(k) = flagstructp%vtdm4_pert     ! for delp, delz, and vorticity
       else
            damp_vt_pert(k) = 0.
       endif
       ...
       if ( npz==1 .or. flagstruct%n_sponge<0 ) then
           d2_divg_pert = flagstructp%d2_bg_pert
              ...
              if ( k==1 ) then
                   nord_k_pert=0; d2_divg_pert = max(0.01, flagstructp%d2_bg_pert, flagstructp%d2_bg_k1_pert)
                   nord_w_pert=0; damp_w_pert = d2_divg_pert
                   if ( flagstructp%do_vort_damp_pert ) then
                        nord_v_pert(k)=0; 
                        damp_vt_pert(k) = 0.5*d2_divg_pert
                   endif
              elseif ( k==MAX(2,flagstruct%n_sponge-1) .and. flagstructp%d2_bg_k2_pert>0.01 ) then
                   nord_k_pert=0; d2_divg_pert = max(flagstructp%d2_bg_pert, flagstructp%d2_bg_k2_pert)
                   nord_w_pert=0; damp_w_pert = d2_divg_pert
                   if ( flagstructp%do_vort_damp_pert ) then
                        nord_v_pert(k)=0; 
                        damp_vt_pert(k) = 0.5*d2_divg_pert
                   endif
              elseif ( k==MAX(3,flagstruct%n_sponge) .and. flagstructp%d2_bg_k2_pert>0.05 ) then
                   nord_k_pert=0;  d2_divg_pert = max(flagstructp%d2_bg_pert, 0.2*flagstructp%d2_bg_k2_pert)
                   nord_w_pert=0;  damp_w_pert = d2_divg_pert
              endif
       endif
...
!--- external mode divergence damping ---
       if ( flagstruct%d_ext > 0. )  &
            call a2b_ord2(delp(isd:ied,jsd:jed,k), wk, gridstruct, npx, npy, is,    &
                          ie, js, je, ng, .false.)
...
! Correction factor for 3D Coriolis force
       ...
       call d_sw(vt(isd:ied,jsd:jed,k), delp(isd:ied,jsd:jed,k), ptc(isd:ied,jsd:jed,k),  pt(isd:ied,jsd:jed,k),      &
                  ...
                  q_con(isd:ied,jsd:jed,1),  z_rat(isd:ied,jsd:jed),  &
                  ...
                  flagstructp%hord_tr_pert, hord_m_pert, hord_v_pert, hord_t_pert, hord_p_pert,    &
                  flagstructp%split_damp, nord_k_pert, nord_v_pert(k), nord_w_pert, nord_t_pert, &
                  flagstructp%dddmp_pert, d2_divg_pert, flagstructp%d4_bg_pert, damp_vt_pert(k), damp_w_pert, &
                  damp_t_pert)
...
! Average horizontal "convergence" to cell center
    ...
    if( flagstruct%fill_dp ) call mix_dp(hydrostatic, w, delp, pt, npz, ak, bk, .false., flagstruct%fv_debug, bd)
                                                             ...
                                                             call timing_on('COMM_TOTAL')
    call start_group_halo_update(i_pack(1), delp, domain, complete=.true.)
    call start_group_halo_update(i_pack(1), pt,   domain, complete=.true.)

    call start_group_halo_update(i_pack(11), q_con, domain, complete=.true.)
...
!           call prt_maxmin('WS', ws, is, ie, js, je, 0, 1, 1., master)
        ...
        if ( gridstruct%square_domain ) then
          ...
          call start_group_halo_update(i_pack(4), zh ,  domain, complete=.true.)
...
!----------------------------
    ...
    if ( hydrostatic ) then
       if ( beta > 0. ) then
          call grad1_p_update(divg2, u, v, pkc, gz, du, dv, dt, ng, gridstruct, bd, npx, npy, npz, ptop, beta_d, flagstruct%a2b_ord)
       else
          call one_grad_p(u, v, pkc, gz, divg2, delp, dt, ng, gridstruct, bd, npx, npy, npz, ptop, hydrostatic, &
       ...
       if ( beta > 0. ) then
          call split_p_grad( u, v, pkc, gz, du, dv, delp, pk3, beta_d, dt, ng, gridstruct, bd, npx, npy, npz, flagstruct%use_logp)
...
1000 continue
 ...
 subroutine geopk(ptop, pe, peln, delp, pk, gz, hs, pt, q_con, pkz, km, akap, CG, nested, computehalo, npx, npy, a2b_ord, bd)
   ...
   real, intent(IN), dimension(bd%isd:bd%ied,bd%jsd:bd%jed,km):: q_con
