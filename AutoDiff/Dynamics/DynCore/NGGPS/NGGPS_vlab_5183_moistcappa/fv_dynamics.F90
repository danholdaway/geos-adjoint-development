...
! Local Arrays
      ...
      real :: dp1(bd%isd:bd%ied, bd%jsd:bd%jed, npz), dtdt_m(bd%is:bd%ie,bd%js:bd%je,npz), cappa(bd%isd:bd%ied,bd%jsd:bd%jed,npz)
      ...
      integer:: kord_tracer_pert(ncnst), kord_mt_pert, kord_wz_pert, kord_tm_pert
      ...
      if( flagstruct%tau > 0. ) then
        if ( gridstruct%grid_type<4 ) then
             call Rayleigh_Super(abs(bdt), npx, npy, npz, ks, pfull, phis, flagstruct%tau, u, v, w, pt,  &
                  ua, va, delz, gridstruct%agrid, cp_air, rdgas, ptop, hydrostatic, (.not. neststruct%nested), &
                  flagstruct%rf_cutoff, rf, gridstruct, domain, bd)
        else
             call Rayleigh_Friction(abs(bdt), npx, npy, npz, ks, pfull, flagstruct%tau, u, v, w, pt,  &
                  ua, va, delz, cp_air, rdgas, ptop, hydrostatic, .true., flagstruct%rf_cutoff, rf, gridstruct, domain, bd)
        endif
      endif
...
!allocate(psx(isd:ied,jsd:jed),dpx(is:ie,js:je))
  ...
  do n_map=1, k_split   ! first level of time-split
      ...
      call start_group_halo_update(i_pack(12), cappa, domain, complete=.true.)
...
!$OMP parallel do default(none) shared(isd,ied,jsd,jed,npz,dp1,delp)
     ...
     call complete_group_halo_update(i_pack(12), domain)
      ...
      call dyn_core(npx, npy, npz, ng, sphum, nq, mdt, n_split, zvir, cp_air, akap, cappa, grav, hydrostatic, &
                    ...
                    gz,pkc,ptc,crx,xfx,cry,yfx,divgd,delpc,ut,vt,zh,pk3,du,dv,&
                    time_total)
...
!!! CLEANUP: merge these two calls?
       if (gridstruct%nested) then
         call tracer_2d_nested(q, dp1, mfx, mfy, cx, cy, gridstruct, bd, domain, npx, npy, npz, nq,    &
                        ...
                        flagstructp%hord_tr_pert, flagstructp%nord_tr_pert, flagstructp%trdm2_pert, flagstructp%split_damp_tr )
       else
     ...
     if ( flagstruct%hord_tr<8 .and. flagstruct%moist_phys ) then
       ...
       if ( liq_wat > 0 )  &
        call fill2D(is, ie, js, je, ng, npz, q(isd:ied,jsd:jed,1,liq_wat), delp, gridstruct%area, domain, &
                    neststruct%nested, npx, npy)
       if ( rainwat > 0 )  &
        call fill2D(is, ie, js, je, ng, npz, q(isd:ied,jsd:jed,1,rainwat), delp, gridstruct%area, domain, &
                    neststruct%nested, npx, npy)
       if ( ice_wat > 0  )  &
        call fill2D(is, ie, js, je, ng, npz, q(isd:ied,jsd:jed,1,ice_wat), delp, gridstruct%area, domain, &
                    neststruct%nested, npx, npy)
       if ( snowwat > 0 )  &
        call fill2D(is, ie, js, je, ng, npz, q(isd:ied,jsd:jed,1,snowwat), delp, gridstruct%area, domain, &
                    neststruct%nested, npx, npy)
       if ( graupel > 0 )  &
        call fill2D(is, ie, js, je, ng, npz, q(isd:ied,jsd:jed,1,graupel), delp, gridstruct%area, domain, &
                    neststruct%nested, npx, npy)
...
!------------------------------------------------------------------------
         ...
         do iq=1,nq
            ...
            if ( iq==cld_amt )  kord_tracer_pert(iq) = 17      ! linear
         enddo
         ...
         call Lagrangian_to_Eulerian(last_step, consv_te, ps, pe, delp,          &
                     ...
                     kord_tracer, kord_tm, peln, te_2d,               &
                     ...
                     kord_mt_pert, kord_wz_pert, kord_tracer_pert, kord_tm_pert)
  ...
  if( nwat==6 ) then
     if (cld_amt > 0) then
      call neg_adj3(is, ie, js, je, ng, npz,        &
                    ...
                    peln, delz,                     &
                    pt, delp, q(isd:ied,jsd:jed,1,sphum),   &
                              q(isd:ied,jsd:jed,1,liq_wat), &
                              q(isd:ied,jsd:jed,1,rainwat), &
                              q(isd:ied,jsd:jed,1,ice_wat), &
                              q(isd:ied,jsd:jed,1,snowwat), &
                              q(isd:ied,jsd:jed,1,graupel), &
                              q(isd:ied,jsd:jed,1,cld_amt), flagstruct%check_negative)
     else
        call neg_adj3(is, ie, js, je, ng, npz,        &
                      flagstruct%hydrostatic,         &
                      peln, delz,                     &
                      pt, delp, q(isd:ied,jsd:jed,1,sphum),   &
                                ...
                                q(isd:ied,jsd:jed,1,ice_wat), &
                                q(isd:ied,jsd:jed,1,snowwat), &
                                q(isd:ied,jsd:jed,1,graupel), check_negative=flagstruct%check_negative)
     endif
     if ( flagstruct%fv_debug ) then
       call prt_mxm('T_dyn_a3',    pt, is, ie, js, je, ng, npz, 1., gridstruct%area_64, domain)
       call prt_mxm('SPHUM_dyn',   q(isd:ied,jsd:jed,1,sphum  ), is, ie, js, je, ng, npz, 1.,gridstruct%area_64, domain)
       call prt_mxm('liq_wat_dyn', q(isd:ied,jsd:jed,1,liq_wat), is, ie, js, je, ng, npz, 1.,gridstruct%area_64, domain)
       call prt_mxm('rainwat_dyn', q(isd:ied,jsd:jed,1,rainwat), is, ie, js, je, ng, npz, 1.,gridstruct%area_64, domain)
       call prt_mxm('ice_wat_dyn', q(isd:ied,jsd:jed,1,ice_wat), is, ie, js, je, ng, npz, 1.,gridstruct%area_64, domain)
       call prt_mxm('snowwat_dyn', q(isd:ied,jsd:jed,1,snowwat), is, ie, js, je, ng, npz, 1.,gridstruct%area_64, domain)
       call prt_mxm('graupel_dyn', q(isd:ied,jsd:jed,1,graupel), is, ie, js, je, ng, npz, 1.,gridstruct%area_64, domain)
     endif
  endif
...
if (fpp%FPP_MAPL_MODE) then
 ...
 subroutine Rayleigh_Super(dt, npx, npy, npz, ks, pm, phis, tau, u, v, w, pt,  &
                           ua, va, delz, agrid, cp, rg, ptop, hydrostatic, conserve, rf_cutoff, rf, gridstruct, domain, bd)
    real, intent(in):: dt
...
!
    ...
    u2f = 0.0!allocate( u2f(isd:ied,jsd:jed,kmax) )
