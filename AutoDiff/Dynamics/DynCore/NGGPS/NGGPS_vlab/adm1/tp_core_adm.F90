...
! Local:
    ...
    IF (.NOT.gridstruct%nested) THEN
      CALL PUSHREAL4ARRAY(q, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
...
&           gridstruct%nested, gridstruct%grid_type)
    ...
    IF (branch .EQ. 0) THEN
      CALL POPREAL4ARRAY(q, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
...
!d.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid_utils_mod.cubed_
...
!rgy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mo
!d.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_mapz_mod.pp
...
!corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.compute_diver
!gence_damping_fb sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_
...
&             -dxa(1, j)*q1(2))/(dxa(1, j)+dxa(2, j)))
          ...
          END IF
          IF (ie + 1 .EQ. npx) THEN
            CALL PUSHREAL4(al(npx-1))
            al(npx-1) = c1*q1(npx-3) + c2*q1(npx-2) + c3*q1(npx-1)
            CALL PUSHREAL4(al(npx))
...
! Positive definite constraint:
        IF (iord .EQ. 9 .OR. iord .EQ. 13) THEN
          arg1 = ie1 - is1 + 1
          CALL PUSHREAL4ARRAY(br(is1), ie - is1 + 2)
          CALL PUSHREAL4ARRAY(bl(is1), ie - is1 + 2)
...
!        endif
            ...
            CALL PUSHREAL4(br(2))
            br(2) = al(3) - q1(2)
            CALL PUSHREAL4ARRAY(br(0), ie + 2)
            CALL PUSHREAL4ARRAY(bl(0), ie + 2)
...
&             temp_ad20
          ...
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL4ARRAY(bl(0), ie + 2)
            CALL POPREAL4ARRAY(br(0), ie + 2)
...
&                       (0:), 1)
        ...
        IF (branch .EQ. 0) THEN
          arg1 = ie1 - is1 + 1
          CALL POPREAL4ARRAY(bl(is1), ie - is1 + 2)
          CALL POPREAL4ARRAY(br(is1), ie - is1 + 2)
...
! Positive definite constraint:
        DO j=js1,je1
          arg1 = ilast - ifirst + 1
          CALL PUSHREAL4ARRAY(br(ifirst, j), ilast - ifirst + 1)
          CALL PUSHREAL4ARRAY(bl(ifirst, j), ilast - ifirst + 1)
...
!        endif
          ...
          CALL PUSHREAL4ARRAY(br(ifirst, 0), ilast - ifirst + 1)
          CALL PUSHREAL4ARRAY(bl(ifirst, 0), ilast - ifirst + 1)
...
!        endif
          ...
          END DO
          arg1 = 3*(ilast-ifirst+1)
          CALL PUSHREAL4ARRAY(br(ifirst, npy-2), ilast - ifirst + 1)
          CALL PUSHREAL4ARRAY(bl(ifirst, npy-2), ilast - ifirst + 1)
...
&             , j)*temp1)*flux_ad(i, j)
      ...
      ELSE
        IF (branch .EQ. 1) THEN
          ...
          al_ad = 0.0
        ELSE
          CALL POPREAL4ARRAY(bl(ifirst, npy-2), ilast - ifirst + 1)
          CALL POPREAL4ARRAY(br(ifirst, npy-2), ilast - ifirst + 1)
...
&             )
        ...
        IF (branch .EQ. 0) THEN
          arg1 = 3*(ilast-ifirst+1)
          CALL POPREAL4ARRAY(bl(ifirst, 0), ilast - ifirst + 1)
          CALL POPREAL4ARRAY(br(ifirst, 0), ilast - ifirst + 1)
...
&             temp_ad17
      ...
      IF (branch .EQ. 0) THEN
        DO j=je1,js1,-1
          arg1 = ilast - ifirst + 1
          CALL POPREAL4ARRAY(bl(ifirst, j), ilast - ifirst + 1)
          CALL POPREAL4ARRAY(br(ifirst, j), ilast - ifirst + 1)
...
!cs_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid_utils_mod.c
...
!l_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_ma
!pz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_mapz_m
...
!ll3_4corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.compute_
!divergence_damping_fb sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_
...
&                         , fx, fy, gridstruct, bd, mass)
          CALL PUSHREAL4ARRAY(fy2, (bd%ied-bd%isd+1)*(bd%je-bd%js+2))
          CALL PUSHREAL4ARRAY(q_j, (bd%ie-bd%is+1)*(bd%jed-bd%jsd+1))
          CALL PUSHREAL4ARRAY(fx1, bd%ie - bd%is + 2)
          CALL PUSHREAL4ARRAY(fx2, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1))
          CALL PUSHREAL4ARRAY(fyy, (bd%ied-bd%isd+1)*(bd%je-bd%js+2))
          CALL PUSHCONTROL3B(2)
        ELSE
...
&   mass_ad, nord, damp_c)
    ...
    IF (branch .LT. 3) THEN
      IF (branch .EQ. 0) THEN
        CALL POPREAL4ARRAY(fyy, (bd%ied-bd%isd+1)*(bd%je-bd%js+2))
        CALL POPREAL4ARRAY(fx2, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1))
        CALL POPREAL4ARRAY(fx1, bd%ie - bd%is + 2)
        CALL POPINTEGER4(je)
        CALL POPINTEGER4(is)
        CALL POPINTEGER4(ie)
        CALL POPREAL4ARRAY(q_j, (bd%ie-bd%is+1)*(bd%jed-bd%jsd+1))
        CALL POPREAL4ARRAY(fy2, (bd%ied-bd%isd+1)*(bd%je-bd%js+2))
        CALL POPINTEGER4(js)
      ELSE IF (branch .EQ. 1) THEN
        CALL POPREAL4ARRAY(fyy, (bd%ied-bd%isd+1)*(bd%je-bd%js+2))
        CALL POPREAL4ARRAY(fx2, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1))
...
&               (q1(i)-q1(i-1)) + xt*xt/6.0*(q1(i+1)-2.0*q1(i)+q1(i-1))
      ...
      END IF
    END DO
    CALL PUSHREAL4ARRAY(al, ie - is + 4)
    CALL PUSHINTEGER4(is1)
    CALL PUSHREAL4(qtmp)
    CALL PUSHREAL4ARRAY(q1, ied - isd + 1)
...
&   isd, ied, jfirst, jlast, jsd, jed, npx, npy, dxa, nested, grid_type)
    ...
    CALL POPREAL4ARRAY(q1, ied - isd + 1)
    CALL POPREAL4(qtmp)
    CALL POPINTEGER4(is1)
    CALL POPREAL4ARRAY(al, ie - is + 4)
    al_ad = 0.0
    q1_ad = 0.0
    DO j=jlast,jfirst,-1
      CALL POPCONTROL2B(branch)
...
&               (i, j+1)-(qtmp+qtmp)))
          ...
          END DO
        END DO
        CALL PUSHREAL4ARRAY(al, (ilast-ifirst+1)*(je-js+4))
...
&   ilast, isd, ied, js, je, jsd, jed, npx, npy, dya, nested, grid_type)
    ...
    IF (branch .LT. 2) THEN
      ...
      ELSE
        ...
        CALL POPREAL4ARRAY(al, (ilast-ifirst+1)*(je-js+4))
        al_ad = 0.0
        DO j=je+1,js,-1
          DO i=ilast,ifirst,-1
            CALL POPCONTROL1B(branch)
...
&           )
      ...
      CALL PUSHREAL4ARRAY(fy2, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2))
      CALL PUSHREAL4ARRAY(fx2, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1))
...
&   , bd%jsd:bd%jed+1)
    ...
    IF (branch .EQ. 0) THEN
      CALL POPREAL4(damp2)
      CALL POPREAL4ARRAY(fx2, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1))
      CALL POPREAL4ARRAY(fy2, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2))
      CALL POPINTEGER4(i1)
      fy2_ad = 0.0
      DO j=je+1,js,-1
        DO i=ie,is,-1
