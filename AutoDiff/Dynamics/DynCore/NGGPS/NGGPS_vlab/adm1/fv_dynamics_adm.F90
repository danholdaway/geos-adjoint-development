...
!$OMP parallel do default(none) shared(theta_d,is,ie,js,je,npz,pt,pkz,q)
        DO k=1,npz
          ...
          IF (theta_d .GT. 0) THEN
            DO j=js,je
              DO i=is,ie
                CALL PUSHREAL4(q(i, j, k, theta_d))
...
! monotonic
          ...
          CALL PUSHINTEGER4(kord_tracer_pert(iq))
...
&                    , m_fac)
      IF (idiag%id_aam .GT. 0) THEN
        IF (prt_minmax) THEN
          res0 = G_SUM(domain, te_2d, is, ie, js, je, ng, gridstruct%&
&           area_64, 0)
...
!Convert back to potential temperature
    IF (hydrostatic .AND. (.NOT.idealtest)) THEN
      ...
      CALL PUSHREAL4ARRAY(pk3, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+&
&                   1))
      CALL PUSHINTEGER4(js)
      CALL PUSHREAL4ARRAY(xfx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHREAL4ARRAY(ws, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL PUSHREAL4ARRAY(gz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+1&
&                   ))
      CALL PUSHREAL4ARRAY(dp1, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHINTEGER4(nq)
      CALL PUSHREAL4ARRAY(ptc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHREAL4ARRAY(pfull, npz)
      CALL PUSHINTEGER4(jsd)
      CALL PUSHREAL4ARRAY(rf, npz)
      CALL PUSHINTEGER4(ied)
      CALL PUSHREAL4ARRAY(ut, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHREAL4(dt2)
      CALL PUSHREAL4ARRAY(pkc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+&
&                   1))
      CALL PUSHREAL4ARRAY(delpc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz&
&                  )
      CALL PUSHINTEGER4ARRAY(kord_tracer_pert, ncnst)
      CALL PUSHINTEGER4(ie)
      CALL PUSHREAL4(rdg)
      CALL PUSHREAL4ARRAY(m_fac, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      ...
      CALL PUSHREAL4ARRAY(te_2d, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL PUSHREAL4(akap)
      CALL PUSHREAL4ARRAY(vt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHREAL4(result1)
      CALL PUSHINTEGER4(je)
      CALL PUSHINTEGER4(sphum)
      CALL PUSHREAL4(amdt)
      CALL PUSHREAL4ARRAY(crx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHREAL4ARRAY(cry, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
...
&   kord_tm_pert
    ...
    IF (branch .EQ. 0) THEN
      ...
      CALL POPREAL4ARRAY(cry, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
      CALL POPREAL4ARRAY(crx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
      CALL POPREAL4(amdt)
      CALL POPINTEGER4(sphum)
      CALL POPINTEGER4(je)
      CALL POPREAL4(result1)
      CALL POPREAL4ARRAY(vt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL POPREAL4(akap)
      CALL POPREAL4ARRAY(te_2d, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL POPINTEGER4(k_split)
      CALL POPINTEGER4(is)
      CALL POPINTEGER4(isd)
      CALL POPREAL4ARRAY(yfx, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
      CALL POPREAL4ARRAY(m_fac, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL POPREAL4(rdg)
      CALL POPINTEGER4(ie)
      CALL POPINTEGER4ARRAY(kord_tracer_pert, ncnst)
      CALL POPREAL4ARRAY(delpc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL POPREAL4ARRAY(pkc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+1&
&                  ))
      CALL POPREAL4(dt2)
      CALL POPREAL4ARRAY(ut, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL POPINTEGER4(ied)
      CALL POPREAL4ARRAY(rf, npz)
      CALL POPINTEGER4(jsd)
      CALL POPREAL4ARRAY(pfull, npz)
      CALL POPREAL4ARRAY(ptc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL POPINTEGER4(nq)
      CALL POPREAL4ARRAY(dp1, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL POPREAL4ARRAY(gz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+1)&
&                 )
      CALL POPREAL4ARRAY(ws, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL POPREAL4ARRAY(xfx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
      CALL POPINTEGER4(js)
      CALL POPREAL4ARRAY(pk3, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+1&
&                  ))
      pkz_ad = 0.0
    ELSE
      CALL POPINTEGER4(jed)
...
&                  v, v_ad, delp, delp_ad, te_2d, te_2d_ad, ps, ps_ad, &
&                  m_fac, m_fac_ad)
...
&                               kord_tracer_pert, kord_tm_pert)
      DO iq=nq,1,-1
        CALL POPCONTROL1B(branch)
        CALL POPINTEGER4(kord_tracer_pert(iq))
      END DO
...
&                 , k)**2+w(i, j, k)**2)*(1.-u2f(i, j, k)**2)*rcv
    ...
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(je)
    CALL PUSHREAL4ARRAY(u2f, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
...
!deallocate ( u2f )
    ...
    INTEGER :: i0
    INTEGER :: branch
    CALL POPREAL4ARRAY(u2f, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
    CALL POPINTEGER4(je)
    CALL POPINTEGER4(is)
    CALL POPINTEGER4(ie)
    CALL POPREAL4(rcv)
...
!$OMP                          private(r1, r2, dm)
    DO j=js,je
      ...
      END DO
    END DO
    CALL PUSHREAL4ARRAY(dm, ie - is + 1)
    CALL PUSHREAL4ARRAY(r1, ie - is + 1)
    CALL PUSHREAL4ARRAY(r2, ie - is + 1)
...
&   va_ad
    ...
    INTEGER :: i, j, k
    INTRINSIC COS
    CALL POPREAL4ARRAY(r2, ie - is + 1)
    CALL POPREAL4ARRAY(r1, ie - is + 1)
    CALL POPREAL4ARRAY(dm, ie - is + 1)
    dm_ad = 0.0
    DO j=je,js,-1
      DO k=npz,1,-1
        DO i=ie,is,-1
...
&           , j)-peln(i, k, j)))
      ...
      END DO
    END DO
    CALL PUSHREAL4ARRAY(pt(is:ie, js:je, :), (ie-is+1)*(je-js+1)*npz)
...
&   )
    ...
    is = bd%is
    je = bd%je
    CALL POPREAL4ARRAY(pt(is:ie, js:je, :), (ie-is+1)*(je-js+1)*npz)
