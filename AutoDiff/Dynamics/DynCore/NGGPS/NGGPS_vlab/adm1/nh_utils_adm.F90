...
& del6_vt_flux, del6_vt_flux_adm, del6_vt_flux
...
!$OMP parallel do default(none) shared(is1,ie1,js1,je1,ws,zs,gz,rdt,km)
    DO j=js1,je1
      ...
      END DO
    END DO
    CALL PUSHREAL4ARRAY(xfx, (ie-is+4)*(je-js+3))
    CALL PUSHREAL4ARRAY(gz2, (ie+2*ng-is+1)*(je+2*ng-js+1))
    CALL PUSHINTEGER4(is1)
    CALL PUSHREAL4(bot_ratio)
    CALL PUSHREAL4(top_ratio)
    CALL PUSHREAL4(int_ratio)
    CALL PUSHINTEGER4(je1)
    CALL PUSHINTEGER4(je2)
    CALL PUSHREAL4ARRAY(yfx, (ie-is+3)*(je-js+4))
    CALL PUSHREAL4(rdt)
    CALL PUSHINTEGER4(js1)
    CALL PUSHREAL4ARRAY(fx, (ie-is+4)*(je-js+3))
    CALL PUSHREAL4ARRAY(fy, (ie-is+3)*(je-js+4))
...
&   se_corner, ne_corner, nw_corner, bd, grid_type)
    ...
    CALL POPREAL4ARRAY(fy, (ie-is+3)*(je-js+4))
    CALL POPREAL4ARRAY(fx, (ie-is+4)*(je-js+3))
    CALL POPINTEGER4(js1)
    CALL POPREAL4(rdt)
    CALL POPREAL4ARRAY(yfx, (ie-is+3)*(je-js+4))
    CALL POPINTEGER4(je2)
    CALL POPINTEGER4(je1)
    CALL POPREAL4(int_ratio)
    CALL POPREAL4(top_ratio)
    CALL POPREAL4(bot_ratio)
    CALL POPINTEGER4(is1)
    CALL POPREAL4ARRAY(gz2, (ie+2*ng-is+1)*(je+2*ng-js+1))
    CALL POPREAL4ARRAY(xfx, (ie-is+4)*(je-js+3))
    DO j=je1,js1,-1
      DO k=1,km,1
        DO i=ie1,is1,-1
...
&                        js:je+1, k), gridstruct, bd, ra_x, ra_y)
        ...
        ELSE
          ...
          CALL PUSHREAL4ARRAY(z2, (ie+2*ng-is+1)*(je+2*ng-js+1))
...
&                        gridstruct, bd, ra_x, ra_y)
        ...
        ELSE
          CALL PUSHREAL4ARRAY(fy, (ie-is+1)*(je-js+2))
          CALL PUSHREAL4ARRAY(fx, (ie-is+2)*(je-js+1))
          CALL PUSHREAL4ARRAY(zh(isd:ied, jsd:jed, k), (ied-isd+1)*(jed-&
&                       jsd+1))
...
!$OMP parallel do default(none) shared(is,ie,js,je,km,ws,zs,zh,rdt)
    DO j=js,je
      ...
      END DO
    END DO
    CALL PUSHREAL4ARRAY(yfx_adv, (ie+2*ng-is+1)*(je-js+2)*(km+1))
    CALL PUSHREAL4ARRAY(crx_adv, (ie-is+2)*(je+2*ng-js+1)*(km+1))
    CALL PUSHREAL4ARRAY(xfx_adv, (ie-is+2)*(je+2*ng-js+1)*(km+1))
    CALL PUSHINTEGER4(jsd)
    CALL PUSHREAL4ARRAY(z2, (ie+2*ng-is+1)*(je+2*ng-js+1))
    CALL PUSHINTEGER4(ied)
    CALL PUSHREAL4ARRAY(cry_adv, (ie+2*ng-is+1)*(je-js+2)*(km+1))
    CALL PUSHREAL4ARRAY(ra_x, (ie-is+1)*(je+2*ng-js+1))
    CALL PUSHREAL4ARRAY(ra_y, (ie+2*ng-is+1)*(je-js+1))
...
&   yfx_adv_ad
    ...
    CALL POPREAL4ARRAY(ra_y, (ie+2*ng-is+1)*(je-js+1))
    CALL POPREAL4ARRAY(ra_x, (ie-is+1)*(je+2*ng-js+1))
    CALL POPREAL4ARRAY(cry_adv, (ie+2*ng-is+1)*(je-js+2)*(km+1))
    CALL POPINTEGER4(ied)
    CALL POPREAL4ARRAY(z2, (ie+2*ng-is+1)*(je+2*ng-js+1))
    CALL POPINTEGER4(jsd)
    CALL POPREAL4ARRAY(xfx_adv, (ie-is+2)*(je+2*ng-js+1)*(km+1))
    CALL POPREAL4ARRAY(crx_adv, (ie-is+2)*(je+2*ng-js+1)*(km+1))
    CALL POPREAL4ARRAY(yfx_adv, (ie+2*ng-is+1)*(je-js+2)*(km+1))
    DO j=je,js,-1
      DO k=1,km,1
        DO i=ie,is,-1
          CALL POPCONTROL1B(branch)
...
&             i, j)-fy(i, j+1))*temp_ad2/temp0)
        ...
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREAL4ARRAY(zh(isd:ied, jsd:jed, k), (ied-isd+1)*(jed-&
&                      jsd+1))
          CALL POPREAL4ARRAY(fx, (ie-is+2)*(je-js+1))
          CALL POPREAL4ARRAY(fy, (ie-is+1)*(je-js+2))
...
! Compute Height * grav (for p-gradient computation)
      ...
      END DO
    END DO
    CALL PUSHREAL4ARRAY(dm, (ie-is+3)*km)
    CALL PUSHREAL4ARRAY(pe2, (ie-is+3)*(km+1))
    CALL PUSHINTEGER4(is1)
    CALL PUSHREAL4ARRAY(dz2, (ie-is+3)*km)
    CALL PUSHREAL4ARRAY(w2, (ie-is+3)*km)
    CALL PUSHREAL4(rgrav)
    CALL PUSHREAL4ARRAY(pm2, (ie-is+3)*km)
    CALL PUSHREAL4(gama)
    CALL PUSHREAL4ARRAY(pem, (ie-is+3)*(km+1))
    CALL PUSHINTEGER4(ie1)
...
&   gz_ad
    ...
    INTEGER :: branch
    CALL POPINTEGER4(ie1)
    CALL POPREAL4ARRAY(pem, (ie-is+3)*(km+1))
    CALL POPREAL4(gama)
    CALL POPREAL4ARRAY(pm2, (ie-is+3)*km)
    CALL POPREAL4(rgrav)
    CALL POPREAL4ARRAY(w2, (ie-is+3)*km)
    CALL POPREAL4ARRAY(dz2, (ie-is+3)*km)
    CALL POPINTEGER4(is1)
    CALL POPREAL4ARRAY(pe2, (ie-is+3)*(km+1))
    CALL POPREAL4ARRAY(dm, (ie-is+3)*km)
    dm_ad = 0.0
    pe2_ad = 0.0
    dz2_ad = 0.0
    w2_ad = 0.0
...
! end i-loop
    DO 170 i=is,ie
      DO k=1,km
        ...
        CALL PUSHREAL4(wm(k))
        wm(k) = w2(i, k)*dm(k)
        CALL PUSHREAL4(pt1(k))
        pt1(k) = pt2(i, k)
      END DO
      pe1(:) = 0.
      CALL PUSHREAL4(wbar(km+1))
      wbar(km+1) = ws(i)
...
! Continuity of (pbar, wbar) is maintained
        DO k=1,km
          rden = -(rgas*dm(k)/dz(k))
          CALL PUSHREAL4(pf1(k))
          pf1(k) = EXP(gama*LOG(rden*pt1(k)))
          CALL PUSHREAL4(dts(k))
...
! next i
      ...
      DO n=1,ms
        ...
        DO k=ad_from,km
          ...
          CALL PUSHREAL4(r_lo(k))
          r_lo(k) = wm(k) + ptmp1
          CALL PUSHREAL4(r_hi(k))
          r_hi(k) = wm(k) - ptmp1
        END DO
 ...
 333    IF (ktop .GE. ks1) THEN
          ...
          DO k=ad_from0,ktop
            z_frac = dt/dts(k)
            CALL PUSHREAL4(r_bot(k))
            r_bot(k) = z_frac*r_lo(k)
            CALL PUSHREAL4(r_top(k+1))
            r_top(k+1) = z_frac*r_hi(k)
            CALL PUSHREAL4(m_bot(k))
            m_bot(k) = z_frac*dm(k)
            CALL PUSHREAL4(m_top(k+1))
            m_top(k+1) = m_bot(k)
          END DO
          CALL PUSHINTEGER4(k - 1)
        ...
        ad_from1 = ktop + 2
        DO k=ad_from1,km+1
          CALL PUSHREAL4(m_top(k))
          m_top(k) = 0.
          CALL PUSHREAL4(r_top(k))
          r_top(k) = 0.
        END DO
        ...
        DO 130 ke=km+1,ktop+2,-1
          ...
          DO k=ke-1,kt1,-1
            IF (time_left .GT. dts(k)) THEN
              ...
              CALL PUSHREAL4(m_top(ke))
              m_top(ke) = m_top(ke) + dm(k)
              CALL PUSHREAL4(r_top(ke))
...
! next level
        ...
        DO 160 ke=ad_from3,km
          ...
          DO k=ke,km
            IF (time_left .GT. dts(k)) THEN
              time_left = time_left - dts(k)
              CALL PUSHREAL4(m_bot(ke))
              m_bot(ke) = m_bot(ke) + dm(k)
              CALL PUSHREAL4(r_bot(ke))
...
! pbar here is actually dt*pbar
      ...
      CALL PUSHCONTROL1B(1)
 170 CONTINUE
    CALL PUSHREAL4ARRAY(dm, km)
    CALL PUSHREAL4ARRAY(dts, km)
    CALL PUSHREAL4(dt)
    CALL PUSHREAL4(grg)
    CALL PUSHREAL4ARRAY(dz, km)
    CALL PUSHREAL4ARRAY(r_lo, km)
    CALL PUSHREAL4ARRAY(wbar, km + 1)
    CALL PUSHINTEGER4(ks0)
    CALL PUSHREAL4ARRAY(pf1, km)
    CALL PUSHREAL4ARRAY(m_top, km + 1)
    CALL PUSHREAL4ARRAY(r_top, km + 1)
    CALL PUSHREAL4ARRAY(m_bot, km + 1)
    CALL PUSHREAL4(rdt)
    CALL PUSHREAL4(pf)
    CALL PUSHREAL4ARRAY(r_bot, km + 1)
    CALL PUSHREAL4ARRAY(ws2, ie - is + 1)
    CALL PUSHREAL4(time_left)
    CALL PUSHREAL4ARRAY(pt1, km)
    CALL PUSHREAL4ARRAY(cm, km)
    CALL PUSHREAL4ARRAY(pp, km)
    CALL PUSHREAL4ARRAY(wc, km)
    CALL PUSHREAL4ARRAY(pbar, km + 1)
    CALL PUSHREAL4(m_surf)
    CALL PUSHREAL4ARRAY(wm, km)
    CALL PUSHREAL4ARRAY(r_hi, km)
...
&   pe1_ad, pbar_ad, wbar_ad
    ...
    INTEGER :: ad_from7
    INTEGER :: ad_from8
    CALL POPREAL4ARRAY(r_hi, km)
    CALL POPREAL4ARRAY(wm, km)
    CALL POPREAL4(m_surf)
    CALL POPREAL4ARRAY(pbar, km + 1)
    CALL POPREAL4ARRAY(wc, km)
    CALL POPREAL4ARRAY(pp, km)
    CALL POPREAL4ARRAY(cm, km)
    CALL POPREAL4ARRAY(pt1, km)
    CALL POPREAL4(time_left)
    CALL POPREAL4ARRAY(ws2, ie - is + 1)
    CALL POPREAL4ARRAY(r_bot, km + 1)
    CALL POPREAL4(pf)
    CALL POPREAL4(rdt)
    CALL POPREAL4ARRAY(m_bot, km + 1)
    CALL POPREAL4ARRAY(r_top, km + 1)
    CALL POPREAL4ARRAY(m_top, km + 1)
    CALL POPREAL4ARRAY(pf1, km)
    CALL POPINTEGER4(ks0)
    CALL POPREAL4ARRAY(wbar, km + 1)
    CALL POPREAL4ARRAY(r_lo, km)
    CALL POPREAL4ARRAY(dz, km)
    CALL POPREAL4(grg)
    CALL POPREAL4(dt)
    CALL POPREAL4ARRAY(dts, km)
    CALL POPREAL4ARRAY(dm, km)
    dm_ad = 0.0
    dts_ad = 0.0
    dz_ad = 0.0
    r_lo_ad = 0.0
...
&                   )
            ...
            DO k=km,ad_from2,-1
              CALL POPREAL4(r_bot(k))
              r_bot_ad(k) = 0.0
              CALL POPREAL4(m_bot(k))
              m_bot_ad(k) = 0.0
            END DO
            CALL POPINTEGER4(k)
            ...
            DO k=km+1,ad_from1,-1
              CALL POPREAL4(r_top(k))
              r_top_ad(k) = 0.0
              CALL POPREAL4(m_top(k))
              m_top_ad(k) = 0.0
            END DO
            CALL POPCONTROL1B(branch)
...
&         *r3 - g_rat(i, k)*p1(i)
    ...
    DO k=1,km+1
      ...
      END DO
    END DO
    CALL PUSHREAL4ARRAY(dd, (ie-is+1)*km)
    CALL PUSHREAL4ARRAY(aa, (ie-is+1)*km)
    CALL PUSHREAL4(r2g)
    CALL PUSHREAL4ARRAY(p1, ie - is + 1)
    CALL PUSHREAL4ARRAY(bet, ie - is + 1)
    CALL PUSHREAL4ARRAY(wk1, ie - is + 1)
    CALL PUSHREAL4(r6g)
    CALL PUSHREAL4(ra)
    CALL PUSHREAL4ARRAY(bb, (ie-is+1)*km)
    CALL PUSHREAL4ARRAY(w1, (ie-is+1)*km)
    CALL PUSHREAL4(t2)
    CALL PUSHREAL4(rdt)
    CALL PUSHREAL4(beta)
    CALL PUSHREAL4ARRAY(pp, (ie-is+1)*(km+1))
    CALL PUSHREAL4ARRAY(g_rat, (ie-is+1)*km)
    CALL PUSHREAL4(capa1)
    CALL PUSHREAL4ARRAY(wk, (ie-is+1)*km)
    CALL PUSHREAL4(t1g)
    CALL PUSHREAL4ARRAY(gam, (ie-is+1)*km)
...
&   g_rat_ad, gam_ad
    ...
    REAL :: temp_ad18
    INTEGER :: branch
    CALL POPREAL4ARRAY(gam, (ie-is+1)*km)
    CALL POPREAL4(t1g)
    CALL POPREAL4ARRAY(wk, (ie-is+1)*km)
    CALL POPREAL4(capa1)
    CALL POPREAL4ARRAY(g_rat, (ie-is+1)*km)
    CALL POPREAL4ARRAY(pp, (ie-is+1)*(km+1))
    CALL POPREAL4(beta)
    CALL POPREAL4(rdt)
    CALL POPREAL4(t2)
    CALL POPREAL4ARRAY(w1, (ie-is+1)*km)
    CALL POPREAL4ARRAY(bb, (ie-is+1)*km)
    CALL POPREAL4(ra)
    CALL POPREAL4(r6g)
    CALL POPREAL4ARRAY(wk1, ie - is + 1)
    CALL POPREAL4ARRAY(bet, ie - is + 1)
    CALL POPREAL4ARRAY(p1, ie - is + 1)
    CALL POPREAL4(r2g)
    CALL POPREAL4ARRAY(aa, (ie-is+1)*km)
    CALL POPREAL4ARRAY(dd, (ie-is+1)*km)
    pp_ad = 0.0
    DO k=km+1,1,-1
      DO i=ie,is,-1
...
&         - pm2(i, k)
    ...
    DO k=2,km
      DO i=is,ie
        ...
        CALL PUSHREAL4(bet(i))
        bet(i) = bb(i, k) - gam(i, k)
        CALL PUSHREAL4(pp(i, k+1))
        pp(i, k+1) = (dd(i, k)-pp(i, k))/bet(i)
      END DO
    END DO
    DO k=km,2,-1
      DO i=is,ie
        CALL PUSHREAL4(pp(i, k))
...
&       *ws(i)-aa(i, km)*w2(i, km-1))/bet(i)
    ...
    END DO
    DO i=is,ie
      CALL PUSHREAL4(pe(i, 1))
      pe(i, 1) = 0.
    END DO
    DO k=1,km
      DO i=is,ie
        CALL PUSHREAL4(pe(i, k+1))
...
! Surface:
    ...
    END DO
    CALL PUSHREAL4(gk)
    CALL PUSHREAL4(a_bot)
    CALL PUSHREAL4(bet)
    CALL PUSHREAL4(xt1)
    CALL PUSHREAL4(xt2)
    CALL PUSHREAL4ARRAY(gak, km)
    CALL PUSHREAL4ARRAY(gam, (i2-i1+1)*(km+1))
...
&   q2e_ad, i1, i2, j1, j2, j, km, dp0, uniform_grid, limiter)
    ...
    REAL :: temp_ad6
    INTEGER :: branch
    CALL POPREAL4ARRAY(gam, (i2-i1+1)*(km+1))
    CALL POPREAL4ARRAY(gak, km)
    CALL POPREAL4(xt2)
    CALL POPREAL4(xt1)
    CALL POPREAL4(bet)
    CALL POPREAL4(a_bot)
...
!pressure solver
          DO k=1,npz-1
            DO i=ifirst,0
              CALL PUSHREAL4(g_rat(i, k))
              g_rat(i, k) = delp(i, j, k)/delp(i, j, k+1)
              bb(i, k) = 2.*(1.+g_rat(i, k))
              CALL PUSHREAL4(dd(i, k))
          ...
          DO i=ifirst,0
            CALL PUSHREAL4(bet(i))
            bet(i) = bb(i, 1)
            CALL PUSHREAL4(pkc(i, j, 1))
            pkc(i, j, 1) = 0.
            CALL PUSHREAL4(pkc(i, j, 2))
            pkc(i, j, 2) = dd(i, 1)/bet(i)
            bb(i, npz) = 2.
            CALL PUSHREAL4(dd(i, npz))
          ...
          DO k=2,npz
            DO i=ifirst,0
              ...
              CALL PUSHREAL4(bet(i))
              bet(i) = bb(i, k) - gam(i, k)
              CALL PUSHREAL4(pkc(i, j, k+1))
              pkc(i, j, k+1) = (dd(i, k)-pkc(i, j, k))/bet(i)
            END DO
          END DO
          DO k=npz,2,-1
            DO i=ifirst,0
              CALL PUSHREAL4(pkc(i, j, k))
...
!pk3 if necessary
          ...
          END IF
        END DO
        CALL PUSHREAL4ARRAY(dd, (bd%ied-bd%isd+1)*npz)
        CALL PUSHREAL4ARRAY(peln, (bd%ied-bd%isd+1)*(npz+1)*(bd%jed-bd%&
&                     jsd+1))
        CALL PUSHREAL4ARRAY(bet, bd%ied - bd%isd + 1)
        CALL PUSHINTEGER4(ilast)
        CALL PUSHREAL4(rgrav)
        CALL PUSHINTEGER4(jlast)
        CALL PUSHREAL4(gama)
        CALL PUSHREAL4ARRAY(pkz, (bd%ied-bd%isd+1)*npz)
        CALL PUSHREAL4ARRAY(pe, (bd%ied-bd%isd+1)*(npz+1)*(bd%jed-bd%jsd&
&                     +1))
        CALL PUSHREAL4ARRAY(g_rat, (bd%ied-bd%isd+1)*(npz-1))
        CALL PUSHINTEGER4(ifirst)
        CALL PUSHREAL4ARRAY(gam, (bd%ied-bd%isd+1)*npz)
        CALL PUSHCONTROL2B(2)
      ELSE
        CALL PUSHREAL4ARRAY(dd, (bd%ied-bd%isd+1)*npz)
        CALL PUSHREAL4ARRAY(peln, (bd%ied-bd%isd+1)*(npz+1)*(bd%jed-bd%&
...
&   peln_ad
    ...
    IF (branch .NE. 0) THEN
      IF (branch .EQ. 1) THEN
        CALL POPREAL4ARRAY(gam, (bd%ied-bd%isd+1)*npz)
        CALL POPINTEGER4(ifirst)
        CALL POPREAL4ARRAY(g_rat, (bd%ied-bd%isd+1)*(npz-1))
        CALL POPREAL4ARRAY(pe, (bd%ied-bd%isd+1)*(npz+1)*(bd%jed-bd%jsd+&
&                    1))
        CALL POPREAL4ARRAY(pkz, (bd%ied-bd%isd+1)*npz)
        CALL POPREAL4(gama)
        CALL POPINTEGER4(jlast)
        CALL POPREAL4(rgrav)
        CALL POPINTEGER4(ilast)
        CALL POPREAL4ARRAY(bet, bd%ied - bd%isd + 1)
        CALL POPREAL4ARRAY(peln, (bd%ied-bd%isd+1)*(npz+1)*(bd%jed-bd%&
&                    jsd+1))
        CALL POPREAL4ARRAY(dd, (bd%ied-bd%isd+1)*npz)
        dd_ad = 0.0
        peln_ad = 0.0
        bet_ad = 0.0
        bb_ad = 0.0
