...
& mpp_update_domains_adm, dgrid_ne, mpp_global_sum, mpp_global_sum_adm
...
&   ngc:jlast+ngc)
    ...
    INTRINSIC PRESENT
    REAL(kind=r_grid), DIMENSION(ilast-ifirst+1, jlast-jfirst+1) :: arg1
    REAL(kind=r_grid), DIMENSION(ilast-ifirst+1, jlast-jfirst+1) :: &
&   arg1_ad
    LOGICAL :: res
    INTEGER :: branch
    REAL :: g_sum
...
&                       bitwise_efp_sum, mpp_global_sum_2d_ad=gsum_ad)
      p_ad = 0.0
      p_ad(:, :) = area(ifirst:ilast, jfirst:jlast)*arg1_ad(:, :)
...
&                       mpp_global_sum_2d_ad=gsum_ad)
      ...
      p_ad(:, :) = area(ifirst:ilast, jfirst:jlast)*arg1_ad(:, :)
    ELSE
      CALL MP_REDUCE_SUM_ADM(gsum, gsum_ad)
      p_ad = 0.0
      DO j=jlast,jfirst,-1
        DO i=ilast,ifirst,-1
...
!-------------------------
    IF (PRESENT(reproduce)) THEN
      IF (reproduce) THEN
        arg1(:, :) = p(:, :)*area(ifirst:ilast, jfirst:jlast)
        gsum = MPP_GLOBAL_SUM(domain, arg1(:, :), flags=bitwise_efp_sum)
      ELSE
        arg1(:, :) = p(:, :)*area(ifirst:ilast, jfirst:jlast)
