...
!call init_ijk_mem(is, ie+1, js,  je,   npz, mfx, 0.)
    CALL PUSHREAL4ARRAY(mfx, (bd%ie-bd%is+2)*(bd%je-bd%js+1)*npz)
    mfx = 0.0
!call init_ijk_mem(is, ie  , js,  je+1, npz, mfy, 0.)
    CALL PUSHREAL4ARRAY(mfy, (bd%ie-bd%is+1)*(bd%je-bd%js+2)*npz)
    mfy = 0.0
!call init_ijk_mem(is, ie+1, jsd, jed,  npz, cx, 0.)
    CALL PUSHREAL4ARRAY(cx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
    cx = 0.0
!call init_ijk_mem(isd, ied, js,  je+1, npz, cy, 0.)
    CALL PUSHREAL4ARRAY(cy, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
    cy = 0.0
...
&                       *nq)
      ...
      END IF
      IF (.NOT.hydrostatic) THEN
        CALL PUSHREAL4ARRAY(w, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
...
!$OMP parallel do default(none) shared(isd,ied,jsd,jed,npz,gz,zs,delz)
            DO j=jsd,jed
              DO i=isd,ied
                CALL PUSHREAL4(gz(i, j, npz+1))
                gz(i, j, npz+1) = zs(i, j)
              END DO
              DO k=npz,1,-1
                DO i=isd,ied
                  CALL PUSHREAL4(gz(i, j, k))
...
!$OMP parallel do default(none) shared(is,ie,js,je,npz,gz,zs,delz)
            ...
            CALL PUSHCONTROL1B(1)
          END IF
          CALL PUSHREAL4ARRAY(gz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(&
&                       npz+1))
...
&                                , bctype=neststruct%nestbctype)
        arg1 = split_timestep_bc + 0.5
        arg2 = REAL(n_split*flagstruct%k_split)
        CALL PUSHREAL4ARRAY(ptc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz&
&                    )
...
&                        a_imp, flagstruct%scale_z)
        IF (gridstruct%nested) THEN
          arg1 = split_timestep_bc + 0.5
          arg2 = REAL(n_split*flagstruct%k_split)
          CALL PUSHREAL4ARRAY(delz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*&
&                       npz)
...
&                 flagstruct%fv_debug, bd)
      ...
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      CALL PUSHREAL4ARRAY(delp, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL START_GROUP_HALO_UPDATE(i_pack(1), delp, domain, complete=&
&                            .true.)
      CALL PUSHREAL4ARRAY(pt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
...
&                       use_logp, remap_step, arg11)
        IF (gridstruct%square_domain) THEN
          CALL PUSHREAL4ARRAY(zh, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(&
&                       npz+1))
          CALL START_GROUP_HALO_UPDATE(i_pack(4), zh, domain)
          CALL PUSHREAL4ARRAY(pkc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(&
&                       npz+1))
...
! Prevent accumulation of rounding errors at overlapped domain edges:
        CALL PUSHREAL4ARRAY(v, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1)*npz)
        CALL PUSHREAL4ARRAY(u, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2)*npz)
...
!-----------------------------------------------------
    IF (nq .GT. 0 .AND. (.NOT.flagstruct%inline_q)) THEN
      CALL PUSHREAL4ARRAY(q, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz*nq)
      CALL START_GROUP_HALO_UPDATE(i_pack(10), q, domain)
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHCONTROL1B(1)
...
&                 )
        ...
        CALL PUSHREAL4(rdg)
        CALL PUSHREAL4ARRAY(damp_vt_pert, npz + 1)
        ...
        CALL PUSHINTEGER4(nord_k)
        CALL PUSHREAL4(damp_t_pert)
        CALL PUSHREAL4ARRAY(ws3, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
        CALL PUSHINTEGER4(nord_t)
        CALL PUSHREAL4ARRAY(damp_vt, npz + 1)
        CALL PUSHINTEGER4ARRAY(nord_v, npz + 1)
        CALL PUSHREAL4(min1)
        CALL PUSHINTEGER4(nord_w)
        CALL PUSHREAL4ARRAY(heat_source, (bd%ied-bd%isd+1)*(bd%jed-bd%&
&                     jsd+1)*npz)
        CALL PUSHINTEGER4(je)
        CALL PUSHINTEGER4(hord_p_pert)
        CALL PUSHREAL4ARRAY(dp_ref, npz)
        CALL PUSHINTEGER4(hord_t_pert)
        CALL PUSHINTEGER4(hord_v_pert)
...
&   npz)
    ...
    IF (branch .EQ. 0) THEN
      ...
      CALL POPREAL4(d2_divg)
      CALL POPINTEGER4(ms)
      CALL POPINTEGER4(hord_v_pert)
      CALL POPINTEGER4(hord_t_pert)
      CALL POPREAL4ARRAY(dp_ref, npz)
      CALL POPINTEGER4(hord_p_pert)
      CALL POPINTEGER4(je)
      CALL POPINTEGER4(nord_w)
      CALL POPINTEGER4ARRAY(nord_v, npz + 1)
      CALL POPREAL4ARRAY(damp_vt, npz + 1)
      CALL POPINTEGER4(nord_t)
      CALL POPREAL4ARRAY(ws3, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL POPREAL4(damp_t_pert)
      CALL POPINTEGER4(nord_k)
      CALL POPINTEGER4(nord_k_pert)
      CALL POPREAL4(d2_divg_pert)
      CALL POPREAL4(rdt)
      CALL POPINTEGER4(is)
      CALL POPREAL4ARRAY(damp_vt_pert, npz + 1)
      CALL POPREAL4(rdg)
      CALL POPINTEGER4(ie)
      CALL POPREAL4(k1k)
      CALL POPREAL4(dt2)
      CALL POPREAL4(beta_d)
    ...
    ELSE
      IF (branch .EQ. 1) THEN
        ...
        CALL POPINTEGER4(hord_p_pert)
        CALL POPINTEGER4(je)
        CALL POPREAL4ARRAY(heat_source, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd&
&                    +1)*npz)
        CALL POPINTEGER4(nord_w)
        CALL POPREAL4(min1)
        CALL POPINTEGER4ARRAY(nord_v, npz + 1)
...
&                         gridstruct%nested, .true., .true., .true., bd)
          CALL POPREAL4ARRAY(delz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*&
&                      npz)
...
&                                     delp, delp_ad)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREAL4ARRAY(pkc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(&
&                      npz+1))
          CALL START_GROUP_HALO_UPDATE_ADM(i_pack(5), pkc, pkc_ad, &
&                                    domain, whalo=2, ehalo=2, shalo=2, &
&                                    nhalo=2)
          CALL POPREAL4ARRAY(zh, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(&
&                      npz+1))
...
&               , j)
      ...
      ELSE
        divg2_ad = 0.0
      END IF
      CALL POPREAL4ARRAY(pt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL START_GROUP_HALO_UPDATE_ADM(i_pack(1), pt, pt_ad, domain, &
&                                complete=.true.)
      CALL POPREAL4ARRAY(delp, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
...
&                                      , ie, js, je, ng, .false.)
        ...
        IF (branch .LT. 6) THEN
          IF (branch .LT. 3) THEN
            IF (branch .NE. 0) THEN
              IF (branch .EQ. 1) THEN
                CALL POPREAL4(damp_vt_pert(k))
                CALL POPINTEGER4(nord_v_pert(k))
              END IF
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL4(damp_vt(k))
...
&                      gridstruct%nw_corner, bd, gridstruct%grid_type)
      ...
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREAL4ARRAY(ptc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
...
&               , gridstruct, flagstruct)
      ...
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREAL4ARRAY(gz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+&
&                    1))
...
&                   )
    ...
    END DO
    CALL POPCONTROL2B(branch)
    CALL POPREAL4ARRAY(cy, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
    CALL POPREAL4ARRAY(cx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
    CALL POPREAL4ARRAY(mfy, (bd%ie-bd%is+1)*(bd%je-bd%js+2)*npz)
    CALL POPREAL4ARRAY(mfx, (bd%ie-bd%is+2)*(bd%je-bd%js+1)*npz)
...
!$OMP                          private(pei)
    DO j=js,je
      CALL PUSHREAL4(pei(is-2))
      pei(is-2) = ptop
      CALL PUSHREAL4(pei(is-1))
      pei(is-1) = ptop
      DO k=1,npz
        CALL PUSHREAL4(pei(is-2))
        ...
        CALL PUSHREAL4(pk3(is-2, j, k+1))
        pk3(is-2, j, k+1) = EXP(akap*LOG(pei(is-2)))
        CALL PUSHREAL4(pk3(is-1, j, k+1))
        pk3(is-1, j, k+1) = EXP(akap*LOG(pei(is-1)))
      END DO
      CALL PUSHREAL4(pei(ie+1))
      pei(ie+1) = ptop
      CALL PUSHREAL4(pei(ie+2))
      pei(ie+2) = ptop
      DO k=1,npz
        CALL PUSHREAL4(pei(ie+1))
        ...
        CALL PUSHREAL4(pk3(ie+1, j, k+1))
        pk3(ie+1, j, k+1) = EXP(akap*LOG(pei(ie+1)))
        CALL PUSHREAL4(pk3(ie+2, j, k+1))
...
!$OMP                          private(pej)
    DO i=is-2,ie+2
      CALL PUSHREAL4(pej(js-2))
      pej(js-2) = ptop
      CALL PUSHREAL4(pej(js-1))
      pej(js-1) = ptop
      DO k=1,npz
        CALL PUSHREAL4(pej(js-2))
        ...
        CALL PUSHREAL4(pk3(i, js-2, k+1))
        pk3(i, js-2, k+1) = EXP(akap*LOG(pej(js-2)))
        CALL PUSHREAL4(pk3(i, js-1, k+1))
        pk3(i, js-1, k+1) = EXP(akap*LOG(pej(js-1)))
      END DO
      CALL PUSHREAL4(pej(je+1))
      pej(je+1) = ptop
      CALL PUSHREAL4(pej(je+2))
      pej(je+2) = ptop
      DO k=1,npz
        CALL PUSHREAL4(pej(je+1))
        ...
        CALL PUSHREAL4(pk3(i, je+1, k+1))
        pk3(i, je+1, k+1) = EXP(akap*LOG(pej(je+1)))
        CALL PUSHREAL4(pk3(i, je+2, k+1))
        pk3(i, je+2, k+1) = EXP(akap*LOG(pej(je+2)))
      END DO
    END DO
    CALL PUSHREAL4ARRAY(pei, ied - isd + 1)
    CALL PUSHREAL4ARRAY(pej, jed - jsd + 1)
...
!_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv
!_tp_2d_fb tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corne
!r_fb fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
...
!$OMP parallel do default(none) shared(is,ie,js,je,npz,pe,delp,ptop)
    DO j=js,je
      CALL PUSHREAL4(pe(is-1, 1, j))
      pe(is-1, 1, j) = ptop
      CALL PUSHREAL4(pe(ie+1, 1, j))
      pe(ie+1, 1, j) = ptop
      DO k=1,npz
        CALL PUSHREAL4(pe(is-1, k+1, j))
        pe(is-1, k+1, j) = pe(is-1, k, j) + delp(is-1, j, k)
        CALL PUSHREAL4(pe(ie+1, k+1, j))
        pe(ie+1, k+1, j) = pe(ie+1, k, j) + delp(ie+1, j, k)
      END DO
    END DO
!$OMP parallel do default(none) shared(is,ie,js,je,npz,pe,delp,ptop)
    DO i=is-1,ie+1
      CALL PUSHREAL4(pe(i, 1, js-1))
      pe(i, 1, js-1) = ptop
      CALL PUSHREAL4(pe(i, 1, je+1))
      pe(i, 1, je+1) = ptop
      DO k=1,npz
        CALL PUSHREAL4(pe(i, k+1, js-1))
        pe(i, k+1, js-1) = pe(i, k, js-1) + delp(i, js-1, k)
        CALL PUSHREAL4(pe(i, k+1, je+1))
...
&           grad(3, i, j))
    ...
    END DO
    CALL PUSHINTEGER4(js)
    CALL PUSHREAL4ARRAY(v3, 3*(bd%ie-bd%is+1)*(bd%je-bd%js+1))
    CALL PUSHREAL4ARRAY(grad, 3*(bd%ie-bd%is+1)*(bd%je-bd%js+1))
    CALL PUSHINTEGER4(ie)
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(je)
...
&   , va
    ...
    CALL POPINTEGER4(is)
    CALL POPINTEGER4(ie)
    CALL POPREAL4ARRAY(grad, 3*(bd%ie-bd%is+1)*(bd%je-bd%js+1))
    CALL POPREAL4ARRAY(v3, 3*(bd%ie-bd%is+1)*(bd%je-bd%js+1))
    CALL POPINTEGER4(js)
    v3_ad = 0.0
    grad_ad = 0.0
    up_ad = 0.0
...
&           , j, k)))
    ...
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(je)
    CALL PUSHREAL4ARRAY(wk, (bd%ie-bd%is+3)*(bd%je-bd%js+3))
...
&   gz_ad
    ...
    REAL :: temp_ad2
    INTEGER :: branch
    CALL POPREAL4ARRAY(wk, (bd%ie-bd%is+3)*(bd%je-bd%js+3))
    CALL POPINTEGER4(je)
    CALL POPINTEGER4(is)
    CALL POPINTEGER4(ie)
    CALL POPINTEGER4(js)
...
!$OMP parallel do default(none) shared(is,ie,js,je,pp,pk,top_value)
    DO j=js,je+1
      DO i=is,ie+1
        CALL PUSHREAL4(pp(i, j, 1))
        pp(i, j, 1) = 0.
        CALL PUSHREAL4(pk(i, j, 1))
        pk(i, j, 1) = top_value
      END DO
    END DO
...
&           gridstruct%rdy(i, j)
    ...
    END DO
    CALL PUSHINTEGER4(js)
    CALL PUSHREAL4ARRAY(wk1, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
    ...
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(je)
    CALL PUSHREAL4ARRAY(wk, (bd%ie-bd%is+2)*(bd%je-bd%js+2))
...
! end k-loop
    ...
    CALL POPREAL4ARRAY(wk, (bd%ie-bd%is+2)*(bd%je-bd%js+2))
    CALL POPINTEGER4(je)
    CALL POPINTEGER4(is)
    ...
    CALL POPREAL4ARRAY(wk1, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
    CALL POPINTEGER4(js)
    wk1_ad = 0.0
    wk_ad = 0.0
    DO k=npz,1,-1
...
&                      ie, js, je, ng, .true.)
    ...
    DO j=je+1,js,-1
      DO i=ie+1,is,-1
        CALL POPREAL4(pk(i, j, 1))
        pk_ad(i, j, 1) = 0.0
        CALL POPREAL4(pp(i, j, 1))
        pp_ad(i, j, 1) = 0.0
      END DO
    END DO
...
! Remap from above and mix pt
          ...
          CALL PUSHREAL4(pt(i, j, km))
          pt(i, j, km) = (pt(i, j, km)*delp(i, j, km)+pt(i, j, km-1)*dp)&
&           /dpmin
          IF (.NOT.hydrostatic) THEN
            CALL PUSHREAL4(w(i, j, km))
...
&             dpmin
          ...
          ELSE
            CALL PUSHCONTROL1B(1)
          END IF
          CALL PUSHREAL4(delp(i, j, km))
          delp(i, j, km) = dpmin
          CALL PUSHREAL4(delp(i, j, km-1))
...
&   w_ad
    ...
    DO j=jlast,jfirst,-1
      ...
      DO i=ilast,ifirst,-1
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) THEN
          CALL POPREAL4(delp(i, j, km-1))
          dp_ad = -delp_ad(i, j, km-1)
          CALL POPREAL4(delp(i, j, km))
          delp_ad(i, j, km) = 0.0
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL4(w(i, j, km))
...
!$OMP                          private(peg, pkg, p1d, logp)
    DO j=jfirst,jlast
      DO i=ifirst,ilast
        CALL PUSHREAL4(p1d(i))
        p1d(i) = ptop
        CALL PUSHREAL4(pk(i, j, 1))
        pk(i, j, 1) = ptk
        CALL PUSHREAL4(gz(i, j, km+1))
        gz(i, j, km+1) = hs(i, j)
      END DO
...
&             1, j)-peln(i, k, j)))
      ...
      END IF
    END DO
    CALL PUSHREAL4ARRAY(logp, bd%ied - bd%isd + 1)
    CALL PUSHINTEGER4(ilast)
    CALL PUSHINTEGER4(jfirst)
    CALL PUSHREAL4ARRAY(p1d, bd%ied - bd%isd + 1)
    CALL PUSHINTEGER4(jlast)
    CALL PUSHINTEGER4(ie)
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(ifirst)
...
&   q_con
    ...
    CALL POPINTEGER4(ie)
    CALL POPINTEGER4(jlast)
    CALL POPREAL4ARRAY(p1d, bd%ied - bd%isd + 1)
    CALL POPINTEGER4(jfirst)
    CALL POPINTEGER4(ilast)
    CALL POPREAL4ARRAY(logp, bd%ied - bd%isd + 1)
    logp_ad = 0.0
    p1d_ad = 0.0
    DO j=jlast,jfirst,-1
      CALL POPCONTROL1B(branch)
...
!     logical, pointer :: sw_corner, se_corner, ne_corner, nw_corner
    ...
    ELSE
      ntimes = 3
    END IF
    CALL PUSHREAL4ARRAY(q, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*km)
...
&                                          ne_corner)
      ...
      END DO
    END DO
    CALL POPREAL4ARRAY(q, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*km)
