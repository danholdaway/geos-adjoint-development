...
& lagrangian_to_eulerian_fwd, lagrangian_to_eulerian_bwd, map1_q2_adm
...
!$OMP                                  pe0,pe1,pe2,pe3,pk1,pk2,pn2,phis,q2)
    DO j=js,je+1
      ...
      ad_from0 = is
      DO i=ad_from0,ie
        CALL PUSHREAL4(pe2(i, 1))
        pe2(i, 1) = ptop
        CALL PUSHREAL4(pe2(i, km+1))
...
!------------------
        ...
        ad_from10 = is
        DO i=ad_from10,ie
          CALL PUSHREAL4(pn2(i, 1))
          pn2(i, 1) = peln(i, 1, j)
          CALL PUSHREAL4(pn2(i, km+1))
          pn2(i, km+1) = peln(i, km+1, j)
          CALL PUSHREAL4(pk2(i, 1))
          pk2(i, 1) = pk1(i, 1)
          CALL PUSHREAL4(pk2(i, km+1))
        ...
        DO k=2,km
          ad_from11 = is
          DO i=ad_from11,ie
            CALL PUSHREAL4(pn2(i, k))
            pn2(i, k) = LOG(pe2(i, k))
            CALL PUSHREAL4(pk2(i, k))
...
&                            abs_kord_tm, t_min)
            CALL PUSHCONTROL3B(0)
          ELSE
            CALL PUSHREAL4ARRAY(pt, (ied-isd+1)*(jed-jsd+1)*km)
...
&                             , is, ie, isd, ied, jsd, jed, 0., fill)
          ...
          ELSE
            CALL PUSHREAL4ARRAY(q, (ied-isd+1)*(jed-jsd+1)*km*nq)
...
&                          ie, j, isd, ied, jsd, jed, -2, kord_wz)
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHREAL4ARRAY(w, (ied-isd+1)*(jed-jsd+1)*km)
...
&                          isd, ied, jsd, jed, 1, abs_kord_tm)
            CALL PUSHCONTROL1B(1)
          ELSE
            CALL PUSHREAL4ARRAY(delz, (ied-isd+1)*(jed-jsd+1)*km)
...
&                      , is, ie, j, isd, ied, jsd, jedp1, -1, kord_mt)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHREAL4ARRAY(u, (ied-isd+1)*(jed-jsd+2)*km)
...
&                        , iedp1, jsd, jed, -1, kord_mt)
          CALL PUSHCONTROL2B(1)
        ELSE
          CALL PUSHREAL4ARRAY(v, (ied-isd+2)*(jed-jsd+1)*km)
...
&                 q(i, j, k, sphum))
        ...
        CALL PUSHREAL4ARRAY(phis, (ie-is+1)*(km+1))
        CALL PUSHREAL4ARRAY(pe0, (ie-is+2)*(km+1))
        CALL PUSHREAL4ARRAY(pe1, (ie-is+1)*(km+1))
        CALL PUSHREAL4ARRAY(pe2, (ie-is+1)*(km+1))
        CALL PUSHREAL4ARRAY(pe3, (ie-is+2)*(km+1))
        CALL PUSHREAL4ARRAY(gz, ie - is + 1)
        CALL PUSHINTEGER4(kmp)
        CALL PUSHREAL4ARRAY(dp2, (ie-is+1)*km)
        ...
        CALL PUSHINTEGER4(abs_kord_tm_pert)
        CALL PUSHREAL4ARRAY(te_2d, (ie-is+1)*(je-js+1))
        CALL PUSHINTEGER4(rainwat)
        CALL PUSHREAL4(result1)
        CALL PUSHINTEGER4(ice_wat)
        CALL PUSHREAL4(rrg)
        CALL PUSHREAL4ARRAY(zsum0, (ie-is+1)*(je-js+1))
        CALL PUSHREAL4ARRAY(pn2, (ie-is+1)*(km+1))
        CALL PUSHREAL4ARRAY(zsum1, (ie-is+1)*(je-js+1))
        CALL PUSHINTEGER4(liq_wat)
        CALL PUSHREAL4ARRAY(pk1, (ie-is+1)*(km+1))
        CALL PUSHREAL4(bkh)
        CALL PUSHREAL4ARRAY(pk2, (ie-is+1)*(km+1))
        CALL PUSHCONTROL3B(0)
      ELSE IF (remap_pt) THEN
! j-loop
! k-loop
...
&   , iq, n, kmp, kp, k_next
    ...
    IF (branch .LT. 3) THEN
      IF (branch .EQ. 0) THEN
        CALL POPREAL4ARRAY(pk2, (ie-is+1)*(km+1))
        CALL POPREAL4(bkh)
        CALL POPREAL4ARRAY(pk1, (ie-is+1)*(km+1))
        CALL POPINTEGER4(liq_wat)
        CALL POPREAL4ARRAY(zsum1, (ie-is+1)*(je-js+1))
        CALL POPREAL4ARRAY(pn2, (ie-is+1)*(km+1))
        CALL POPREAL4ARRAY(zsum0, (ie-is+1)*(je-js+1))
        CALL POPREAL4(rrg)
        CALL POPINTEGER4(ice_wat)
        CALL POPREAL4(result1)
        CALL POPINTEGER4(rainwat)
        CALL POPREAL4ARRAY(te_2d, (ie-is+1)*(je-js+1))
        CALL POPINTEGER4(abs_kord_tm_pert)
        ...
        CALL POPREAL4ARRAY(dp2, (ie-is+1)*km)
        CALL POPINTEGER4(kmp)
        CALL POPREAL4ARRAY(gz, ie - is + 1)
        CALL POPREAL4ARRAY(pe3, (ie-is+2)*(km+1))
        CALL POPREAL4ARRAY(pe2, (ie-is+1)*(km+1))
        CALL POPREAL4ARRAY(pe1, (ie-is+1)*(km+1))
        CALL POPREAL4ARRAY(pe0, (ie-is+2)*(km+1))
        CALL POPREAL4ARRAY(phis, (ie-is+1)*(km+1))
...
&                          , jed, -1, kord_mt)
          ELSE
            CALL POPREAL4ARRAY(v, (ied-isd+2)*(jed-jsd+1)*km)
            gz_ad = 0.0
...
&                        kord_mt)
        ELSE
          CALL POPREAL4ARRAY(u, (ied-isd+1)*(jed-jsd+2)*km)
          gz_ad = 0.0
...
&                 temp_ad13
      ...
      IF (branch .NE. 0) THEN
        ...
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREAL4ARRAY(delz, (ied-isd+1)*(jed-jsd+1)*km)
          gz_ad = 0.0
...
&                        isd, ied, jsd, jed, -2, kord_wz)
        ELSE
          CALL POPREAL4ARRAY(w, (ied-isd+1)*(jed-jsd+1)*km)
...
&                           ied, jsd, jed, 0., fill)
        ELSE
          CALL POPREAL4ARRAY(q, (ied-isd+1)*(jed-jsd+1)*km*nq)
...
&                          abs_kord_tm, t_min)
        ELSE IF (branch .EQ. 1) THEN
          CALL POPREAL4ARRAY(pt, (ied-isd+1)*(jed-jsd+1)*km)
...
&                 , peln_ad, pkz, pkz_ad, ptop)
 ...
 140  CALL POPINTEGER4(ad_from0)
      ...
      DO i=ad_to1,ad_from0,-1
        CALL POPREAL4(pe2(i, km+1))
        pe_ad(i, km+1, j) = pe_ad(i, km+1, j) + pe2_ad(i, km+1)
        pe2_ad(i, km+1) = 0.0
        CALL POPREAL4(pe2(i, 1))
        pe2_ad(i, 1) = 0.0
      END DO
...
!_v_fb sw_core_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew t
!p_core_mod.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod
!.extrap_corner fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
...
!$OMP                          private(phiz, tv, cvm, qd)
    DO j=js,je
      IF (hydrostatic) THEN
        DO i=is,ie
          CALL PUSHREAL4(phiz(i, km+1))
          phiz(i, km+1) = hs(i, j)
        END DO
        DO k=km,1,-1
          DO i=is,ie
            CALL PUSHREAL4(tv(i, k))
            tv(i, k) = pt(i, j, k)*(1.+qc(i, j, k))
            CALL PUSHREAL4(phiz(i, k))
...
&               )
        ...
        END IF
      END DO
      CALL PUSHREAL4ARRAY(phiz, (ie-is+1)*(km+1))
      CALL PUSHREAL4ARRAY(tv, (ie-is+1)*km)
      CALL PUSHCONTROL1B(1)
    ELSE
      CALL PUSHREAL4ARRAY(phiz, (ie-is+1)*(km+1))
      CALL PUSHREAL4ARRAY(tv, (ie-is+1)*km)
...
!p_v_fb sw_core_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew 
!tp_core_mod.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mo
!d.extrap_corner fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: qc peln q u v w teq delp delz
!                te_2d pe pt
!   with respect to varying inputs: qc peln q u v w delp delz pe
!                pt
...
&   graupel, nwat
    ...
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREAL4ARRAY(tv, (ie-is+1)*km)
      CALL POPREAL4ARRAY(phiz, (ie-is+1)*(km+1))
    ELSE
      CALL POPREAL4ARRAY(tv, (ie-is+1)*km)
      CALL POPREAL4ARRAY(phiz, (ie-is+1)*(km+1))
      DO j=je,js,-1
...
&         peln(i, k, j)))
      END DO
    END DO
    CALL PUSHREAL4ARRAY(pk2, (ilast-ifirst+1)*(km+1))
...
&   , akap, peln, peln_ad, pkz, pkz_ad, ptop)
    ...
    REAL :: temp_ad0
    INTEGER :: branch
    CALL POPREAL4ARRAY(pk2, (ilast-ifirst+1)*(km+1))
    pk2_ad = 0.0
    DO k=km,1,-1
      DO i=ilast,ifirst,-1
        CALL POPREAL4(pkz(i, j, k))
...
!_divergence_damping_fb sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp
...
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      CALL PUSHREAL4ARRAY(q4, 4*(i2-i1+1)*km)
...
&         i, l)
 130    CALL POPINTEGER4(ad_count)
    ...
    ELSE
      CALL POPREAL4ARRAY(q4, 4*(i2-i1+1)*km)
...
!ivergence_damping_fb sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_c
...
! Field output
    ...
    DO iq=1,nq
      DO k=1,km
        ...
        END DO
      END DO
      CALL PUSHREAL4ARRAY(q4(:, :, :, iq), 4*(i2-i1+1)*km)
...
&         i, l)
 140    CALL POPINTEGER4(ad_count)
    ...
    END DO
    DO iq=nq,1,-1
      CALL POPREAL4ARRAY(q4(:, :, :, iq), 4*(i2-i1+1)*km)
...
!vergence_damping_fb sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_co
...
!pute_divergence_damping_fb sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d_f
...
! Bottom:
      ...
      DO k=1,km
        DO i=i1,i2
          CALL PUSHREAL4(a4(2, i, k))
          a4(2, i, k) = q(i, k)
          CALL PUSHREAL4(a4(3, i, k))
          a4(3, i, k) = q(i, k+1)
        END DO
      END DO
      DO k=1,km
...
! Top 2 and bottom 2 layers always use monotonic mapping
      ...
      ELSE IF (iv .EQ. 2) THEN
        DO i=i1,i2
          CALL PUSHREAL4(a4(2, i, 1))
          a4(2, i, 1) = a4(1, i, 1)
          CALL PUSHREAL4(a4(3, i, 1))
          a4(3, i, 1) = a4(1, i, 1)
          CALL PUSHREAL4(a4(4, i, 1))
          a4(4, i, 1) = 0.
        END DO
        CALL PUSHCONTROL2B(2)
      ELSE
...
! Positive definite constraint
      DO i=1,im
        IF (a4(1, i) .LE. 0.) THEN
          CALL PUSHREAL4(a4(2, i))
          a4(2, i) = a4(1, i)
          CALL PUSHREAL4(a4(3, i))
          a4(3, i) = a4(1, i)
          CALL PUSHREAL4(a4(4, i))
          a4(4, i) = 0.
          CALL PUSHCONTROL3B(5)
        ELSE
...
! Additional constraint to ensure positivity when kord=7
    ...
    END DO
    CALL PUSHREAL4(qm)
    CALL PUSHREAL4ARRAY(df2, (i2-i1+1)*km)
    CALL PUSHREAL4(dq)
    CALL PUSHREAL4(c1)
    CALL PUSHREAL4ARRAY(delq, (i2-i1+1)*km)
    CALL PUSHREAL4(fac)
    CALL PUSHREAL4ARRAY(d4, (i2-i1+1)*km)
    CALL PUSHINTEGER4(km1)
    CALL PUSHINTEGER4(it)
    CALL PUSHREAL4(min1)
    CALL PUSHREAL4ARRAY(dc, (i2-i1+1)*km)
...
&   kord)
    ...
    REAL :: y2
    REAL :: y1
    CALL POPREAL4ARRAY(dc, (i2-i1+1)*km)
    CALL POPREAL4(min1)
    CALL POPINTEGER4(it)
    CALL POPINTEGER4(km1)
    CALL POPREAL4ARRAY(d4, (i2-i1+1)*km)
    CALL POPREAL4(fac)
    CALL POPREAL4ARRAY(delq, (i2-i1+1)*km)
    CALL POPREAL4(c1)
    CALL POPREAL4(dq)
    CALL POPREAL4ARRAY(df2, (i2-i1+1)*km)
    CALL POPREAL4(qm)
    dc_ad = 0.0
    DO k=km,km1,-1
...
! ---------------------------
      ...
      CALL PUSHINTEGER4(lm2)
      CALL PUSHREAL4(dlm2)
      CALL PUSHREAL4ARRAY(qx, (i2-i1+1)*km)
      CALL PUSHREAL4(ap0)
      CALL PUSHREAL4(ap1)
      CALL PUSHREAL4(am1)
      CALL PUSHREAL4ARRAY(logpl1, (i2-i1+1)*km)
      CALL PUSHREAL4(am2)
      CALL PUSHREAL4ARRAY(logpl2, (i2-i1+1)*kn)
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHINTEGER4(lp0)
      CALL PUSHREAL4(dlp0)
...
&   plm1_ad, plm2_ad, dlp0_ad, dlm1_ad, dlm2_ad
    ...
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREAL4ARRAY(logpl2, (i2-i1+1)*kn)
      CALL POPREAL4(am2)
      CALL POPREAL4ARRAY(logpl1, (i2-i1+1)*km)
      CALL POPREAL4(am1)
      CALL POPREAL4(ap1)
      CALL POPREAL4(ap0)
      CALL POPREAL4ARRAY(qx, (i2-i1+1)*km)
      CALL POPREAL4(dlm2)
...
&           +q4(4, i, m)*(1.-r23*esl)))
 ...
 120  CONTINUE
    ...
    CALL PUSHINTEGER4(l)
    CALL PUSHINTEGER4(m)
    CALL PUSHREAL4ARRAY(dp1, (i2-i1+1)*km)
    CALL PUSHREAL4(qsum)
    CALL PUSHREAL4ARRAY(q4, 4*(i2-i1+1)*km)
...
&   , q2_ad, i1, i2, j, ibeg, iend, jbeg, jend, iv, kord, q_min)
    ...
    CALL POPREAL4ARRAY(q4, 4*(i2-i1+1)*km)
    CALL POPREAL4(qsum)
    CALL POPREAL4ARRAY(dp1, (i2-i1+1)*km)
    CALL POPINTEGER4(m)
    CALL POPINTEGER4(l)
    dp1_ad = 0.0
    qsum_ad = 0.0
...
!    if (fill) call fillz(i2-i1+1, km, 1, q2(i1,1,iq), dp2)
    ...
    CALL PUSHREAL4(fac1)
    CALL PUSHREAL4(fac2)
    CALL PUSHREAL4ARRAY(qs, i2 - i1 + 1)
    CALL PUSHREAL4ARRAY(dp1, (i2-i1+1)*km)
    CALL PUSHREAL4ARRAY(qsum, nq)
    CALL PUSHREAL4ARRAY(q4, 4*(i2-i1+1)*km*nq)
...
& )
    ...
    INTEGER :: ad_count0
    INTEGER :: i3
    CALL POPREAL4ARRAY(q4, 4*(i2-i1+1)*km*nq)
    CALL POPREAL4ARRAY(qsum, nq)
    CALL POPREAL4ARRAY(dp1, (i2-i1+1)*km)
    CALL POPREAL4ARRAY(qs, i2 - i1 + 1)
    CALL POPREAL4(fac2)
    CALL POPREAL4(fac1)
    CALL POPREAL4(dp)
    CALL POPINTEGER4(m)
...
!----- Perfectly linear scheme --------------------------------
    IF (abs0 .GT. 16) THEN
      DO k=1,km
        ...
        END DO
      END DO
      CALL PUSHREAL4ARRAY(q, (i2-i1+1)*(km+1))
      CALL PUSHREAL4(bet)
      CALL PUSHREAL4ARRAY(d4, i2 - i1 + 1)
      CALL PUSHREAL4ARRAY(gam, (i2-i1+1)*km)
      CALL PUSHCONTROL1B(1)
    ELSE
      CALL PUSHREAL4ARRAY(q, (i2-i1+1)*(km+1))
      CALL PUSHREAL4(bet)
...
&   , i2, iv, kord, qmin)
    ...
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREAL4ARRAY(gam, (i2-i1+1)*km)
      CALL POPREAL4ARRAY(d4, i2 - i1 + 1)
      CALL POPREAL4(bet)
      CALL POPREAL4ARRAY(q, (i2-i1+1)*(km+1))
      q_ad = 0.0
    ELSE
      CALL POPREAL4ARRAY(gam, (i2-i1+1)*km)
      CALL POPREAL4ARRAY(d4, i2 - i1 + 1)
