...
! scheme 2.1: perturbation form
  ...
  PRIVATE 
  PUBLIC c_sw, d_sw, fill_4corners, del6_vt_flux, del6_vt_flux, &
...
!d.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp
!_2d_fb tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner_f
!b fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
      ...
      DO j=js-1,jep1
        DO i=is-1,ie+2
          ...
          ELSE
            ...
            CALL PUSHCONTROL1B(1)
          END IF
          CALL PUSHREAL4(fx1(i, j))
          fx1(i, j) = ut(i, j)*fx1(i, j)
          CALL PUSHREAL4(fx(i, j))
...
&         *(ke(i, j-1)-ke(i, j))
    ...
    CALL PUSHREAL4ARRAY(fy1, (bd%ie-bd%is+3)*(bd%je-bd%js+4))
    CALL PUSHREAL4ARRAY(fy2, (bd%ie-bd%is+3)*(bd%je-bd%js+4))
    ...
    CALL PUSHREAL4ARRAY(vort, (bd%ie-bd%is+3)*(bd%je-bd%js+3))
    !CALL PUSHPOINTER8(sina_u)
    !CALL PUSHPOINTER8(sina_v)
    CALL PUSHINTEGER4(is)
    CALL PUSHREAL4ARRAY(fx, (bd%ie-bd%is+4)*(bd%je-bd%js+3))
    CALL PUSHREAL4ARRAY(fy, (bd%ie-bd%is+3)*(bd%je-bd%js+4))
    CALL PUSHINTEGER4(je)
    CALL PUSHREAL4ARRAY(fx1, (bd%ie-bd%is+4)*(bd%je-bd%js+3))
    CALL PUSHREAL4ARRAY(fx2, (bd%ie-bd%is+4)*(bd%je-bd%js+3))
...
!od.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_t
!p_2d_fb tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner_
!fb fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: u v w delp ua uc ptc ut delpc
!                va vc vt divg_d wc pt
!   with respect to varying inputs: u v w delp ua uc ptc ut delpc
!                va vc vt divg_d wc pt
...
&   fy2_ad
    ...
    CALL POPREAL4ARRAY(fx2, (bd%ie-bd%is+4)*(bd%je-bd%js+3))
    CALL POPREAL4ARRAY(fx1, (bd%ie-bd%is+4)*(bd%je-bd%js+3))
    CALL POPINTEGER4(je)
    CALL POPREAL4ARRAY(fy, (bd%ie-bd%is+3)*(bd%je-bd%js+4))
    CALL POPREAL4ARRAY(fx, (bd%ie-bd%is+4)*(bd%je-bd%js+3))
    CALL POPINTEGER4(is)
    sina_v => gridstruct%sina_v
    !CALL ADMM_REBASE(sina_v)
    sina_u => gridstruct%sina_u
    !CALL ADMM_REBASE(sina_u)
    CALL POPREAL4ARRAY(vort, (bd%ie-bd%is+3)*(bd%je-bd%js+3))
    CALL POPINTEGER4(ie)
    dyc => gridstruct%dyc
    ...
    CALL POPREAL4ARRAY(fy2, (bd%ie-bd%is+3)*(bd%je-bd%js+4))
    CALL POPREAL4ARRAY(fy1, (bd%ie-bd%is+3)*(bd%je-bd%js+4))
    dy => gridstruct%dy
    !CALL ADMM_REBASE(dy)
    dxc => gridstruct%dxc
...
! flagstruct%grid_type >= 3
    ...
    DO j=jsd,jed
      DO i=is,ie+1
        CALL PUSHREAL4(xfx_adv(i, j))
        xfx_adv(i, j) = dt*ut(i, j)
      END DO
    END DO
    DO j=js,je+1
      DO i=isd,ied
        CALL PUSHREAL4(yfx_adv(i, j))
...
&                    , nord=nord_v, damp_c=damp_v)
    ...
    ELSE
      ...
      CALL PUSHREAL4ARRAY(delp, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
...
&                      ra_y, mfx=fx, mfy=fy)
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHREAL4ARRAY(gy, (bd%ie-bd%is+1)*(bd%je-bd%js+2))
        CALL PUSHREAL4ARRAY(gx, (bd%ie-bd%is+2)*(bd%je-bd%js+1))
        CALL PUSHREAL4ARRAY(w, (bd%ied-bd%isd+1)*(bd%ied-bd%jsd+1))
...
&                    damp_t)
    ...
    ELSE
      CALL PUSHREAL4ARRAY(gy, (bd%ie-bd%is+1)*(bd%je-bd%js+2))
      CALL PUSHREAL4ARRAY(gx, (bd%ie-bd%is+2)*(bd%je-bd%js+1))
      CALL PUSHREAL4ARRAY(pt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
...
&                 flagstruct%grid_type, nested)
      CALL PUSHCONTROL1B(1)
    ELSE
      CALL PUSHREAL4ARRAY(vb, (bd%ie-bd%is+2)*(bd%je-bd%js+2))
...
&                                      gridstruct, flagstruct, bd)
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHREAL4ARRAY(wk, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL PUSHREAL4ARRAY(divg_d, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+2))
      CALL PUSHREAL4ARRAY(vc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2))
      CALL PUSHREAL4ARRAY(uc, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1))
      CALL PUSHREAL4ARRAY(delpc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL PUSHREAL4ARRAY(ptc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL PUSHREAL4ARRAY(vort, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
...
&             fy, xfx_adv, yfx_adv, gridstruct, bd, ra_x, ra_y)
    ...
    DO j=js,je+1
      DO i=is,ie
        CALL PUSHREAL4(u(i, j))
        u(i, j) = vt(i, j) + ke(i, j) - ke(i+1, j) + fy(i, j)
      END DO
    END DO
    DO j=js,je
      DO i=is,ie+1
        CALL PUSHREAL4(v(i, j))
...
! damping applied to relative vorticity (wk):
    IF (damp_v .GT. 1.e-5) THEN
      ...
      CALL PUSHREAL4ARRAY(vt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2))
      CALL PUSHREAL4ARRAY(ut, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1))
      CALL PUSHREAL4ARRAY(vort, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
...
! Convert lost KE due to divergence damping to "heat"
          CALL PUSHREAL4(heat_source(i, j))
...
! Add diffusive fluxes to the momentum equation:
    IF (damp_v .GT. 1.e-5) THEN
      ...
      CALL PUSHREAL4ARRAY(gx, (bd%ie-bd%is+2)*(bd%je-bd%js+1))
      CALL PUSHREAL4ARRAY(gy, (bd%ie-bd%is+1)*(bd%je-bd%js+2))
      CALL PUSHREAL4ARRAY(dw, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      !CALL PUSHPOINTER8(dx)
      CALL PUSHINTEGER4(is2)
      !CALL PUSHPOINTER8(dy)
      !CALL PUSHPOINTER8(rdya)
      CALL PUSHREAL4ARRAY(ub, (bd%ie-bd%is+2)*(bd%je-bd%js+2))
      !CALL PUSHPOINTER8(cosa_u)
      !CALL PUSHPOINTER8(cosa_v)
...
&   , rdx, rdy
    ...
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREAL4ARRAY(wk, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      rdxa => gridstruct%rdxa
      ...
      CALL POPREAL4ARRAY(vt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2))
      rdx => gridstruct%rdx
      ...
      CALL POPREAL4ARRAY(vort, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL POPREAL4ARRAY(ra_y, (bd%ied-bd%isd+1)*(bd%je-bd%js+1))
      CALL POPREAL4ARRAY(vb, (bd%ie-bd%is+2)*(bd%je-bd%js+2))
      ...
      CALL POPREAL4ARRAY(ut, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1))
      cosa => gridstruct%cosa
      ...
      cosa_u => gridstruct%cosa_u
      !CALL ADMM_REBASE(cosa_u)
      CALL POPREAL4ARRAY(ub, (bd%ie-bd%is+2)*(bd%je-bd%js+2))
      rdya => gridstruct%rdya
      ...
      dx => gridstruct%dx
      !CALL ADMM_REBASE(dx)
      CALL POPREAL4ARRAY(dw, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL POPREAL4ARRAY(gy, (bd%ie-bd%is+1)*(bd%je-bd%js+2))
      CALL POPREAL4ARRAY(gx, (bd%ie-bd%is+2)*(bd%je-bd%js+1))
      CALL POPREAL4(damp)
...
&                                      gridstruct, flagstruct, bd)
    ELSE
      CALL POPREAL4ARRAY(vort, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL POPREAL4ARRAY(ptc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL POPREAL4ARRAY(delpc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL POPREAL4ARRAY(uc, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1))
      CALL POPREAL4ARRAY(vc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2))
      CALL POPREAL4ARRAY(divg_d, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+2))
      CALL POPREAL4ARRAY(wk, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
...
&                        , mass=delp, mass=delp_ad, nord=nord_t, damp_c=&
&                        damp_t)
        END IF
      END DO
      DO j=je,js,-1
        DO i=ie,is,-1
          CALL POPREAL4(pt(i, j))
...
&                    delp_ad, nord=nord_t, damp_c=damp_t)
    ELSE
      CALL POPREAL4ARRAY(pt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL POPREAL4ARRAY(gx, (bd%ie-bd%is+2)*(bd%je-bd%js+1))
      CALL POPREAL4ARRAY(gy, (bd%ie-bd%is+1)*(bd%je-bd%js+2))
...
&                 =delp_ad, nord=nord_t_pert, damp_c=damp_t_pert)
    ...
    IF (branch .EQ. 0) THEN
      ...
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREAL4ARRAY(w, (bd%ied-bd%isd+1)*(bd%ied-bd%jsd+1))
        CALL POPREAL4ARRAY(gx, (bd%ie-bd%is+2)*(bd%je-bd%js+1))
        CALL POPREAL4ARRAY(gy, (bd%ie-bd%is+1)*(bd%je-bd%js+2))
...
&                       wk_ad, fx2, fx2_ad, fy2, fy2_ad, gridstruct, bd)
    ...
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREAL4ARRAY(delp, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL POPREAL4ARRAY(fx, (bd%ie-bd%is+2)*(bd%je-bd%js+1))
      CALL POPREAL4ARRAY(fy, (bd%ie-bd%is+1)*(bd%je-bd%js+2))
...
!amics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid_utils_mo
...
!otal_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv
!_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_map
...
!te_divergence_damping_fb sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d 
...
!_fb sw_core_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_
!core_mod.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.e
!xtrap_corner_fb fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: u v ua va divg_d
!   with respect to varying inputs: u v ua va divg_d
...
!tp_v_fb sw_core_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew
! tp_core_mod.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_m
!od.extrap_corner fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: u v ua va divg_d
...
!ytp_v sw_core_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_e
!w tp_core_mod.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_
!mod.extrap_corner fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: u v ua va divg_d
!   with respect to varying inputs: u v ua va
...
!rgence_damping_fb sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core
...
! in
              ...
              bl(npx-1) = 0.
! edge
              CALL PUSHREAL4(br(npx-1))
              br(npx-1) = 0.
! edge
              CALL PUSHREAL4(bl(npx))
              bl(npx) = 0.
! out
              CALL PUSHREAL4(br(npx))
              br(npx) = 0.
              CALL PUSHCONTROL2B(3)
            ELSE
...
&               )/(dx(1, j)+dx(2, j))
              ...
              CALL PUSHCONTROL1B(1)
            END IF
            CALL PUSHREAL4ARRAY(br(2), ie)
            CALL PUSHREAL4ARRAY(bl(2), ie)
            CALL PERT_PPM(1, u(2, j), bl(2), br(2), -1)
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHCONTROL1B(1)
...
&               j)*u(npx+1, j))/(dx(npx, j)+dx(npx+1, j))
              ...
              CALL PUSHCONTROL1B(1)
            END IF
            CALL PUSHREAL4ARRAY(br(npx-2), ie - npx + 4)
            CALL PUSHREAL4ARRAY(bl(npx-2), ie - npx + 4)
            CALL PERT_PPM(1, u(npx-2, j), bl(npx-2), br(npx-2), -1)
            CALL PUSHCONTROL2B(2)
          ELSE
            CALL PUSHCONTROL2B(1)
...
&             )
        ...
        ELSE
          IF (branch .NE. 1) THEN
            CALL POPREAL4ARRAY(bl(npx-2), ie - npx + 4)
            CALL POPREAL4ARRAY(br(npx-2), ie - npx + 4)
...
&               2, j))*temp_ad12
          ...
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL4ARRAY(bl(2), ie)
            CALL POPREAL4ARRAY(br(2), ie)
...
! in
          ...
          j = 2
          arg1 = ie - is + 2
          CALL PUSHREAL4ARRAY(br(is, j), ie - is + 2)
          CALL PUSHREAL4ARRAY(bl(is, j), ie - is + 2)
...
&             , j)
      ...
      ELSE
        ...
        ELSE
          j = npy - 2
          CALL POPREAL4ARRAY(bl(is, j), ie - is + 2)
          CALL POPREAL4ARRAY(br(is, j), ie - is + 2)
...
! Xdir:
      IF (gridstruct%sw_corner) THEN
        CALL PUSHREAL4(ua(-1, 0))
        ua(-1, 0) = -va(0, 2)
        CALL PUSHREAL4(ua(0, 0))
        ua(0, 0) = -va(0, 1)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (gridstruct%se_corner) THEN
        CALL PUSHREAL4(ua(npx, 0))
        ua(npx, 0) = va(npx, 1)
        CALL PUSHREAL4(ua(npx+1, 0))
        ua(npx+1, 0) = va(npx, 2)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (gridstruct%ne_corner) THEN
        CALL PUSHREAL4(ua(npx, npy))
        ua(npx, npy) = -va(npx, npy-1)
        CALL PUSHREAL4(ua(npx+1, npy))
      ...
      END IF
      IF (gridstruct%nw_corner) THEN
        CALL PUSHREAL4(ua(-1, npy))
        ua(-1, npy) = va(0, npy-2)
        CALL PUSHREAL4(ua(0, npy))
        ua(0, npy) = va(0, npy-1)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
...
!Want to use the UPSTREAM value
          ...
          CALL PUSHREAL4(ut(0, j))
          ut(0, j) = (uc(0, j)-v(0, j)*cosa_u(0, j))*rsin_u(0, j)
          CALL PUSHREAL4(ut(2, j))
...
!------
    ...
    END IF
    IF (gridstruct%sw_corner) THEN
      CALL PUSHREAL4(va(0, -1))
      va(0, -1) = -ua(2, 0)
      CALL PUSHREAL4(va(0, 0))
      va(0, 0) = -ua(1, 0)
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHCONTROL1B(1)
    END IF
    IF (gridstruct%se_corner) THEN
      CALL PUSHREAL4(va(npx, 0))
      va(npx, 0) = ua(npx-1, 0)
      CALL PUSHREAL4(va(npx, -1))
      va(npx, -1) = ua(npx-2, 0)
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHCONTROL1B(1)
    END IF
    IF (gridstruct%ne_corner) THEN
      CALL PUSHREAL4(va(npx, npy))
      va(npx, npy) = -ua(npx-1, npy)
      CALL PUSHREAL4(va(npx, npy+1))
    ...
    END IF
    IF (gridstruct%nw_corner) THEN
      CALL PUSHREAL4(va(0, npy))
      va(0, npy) = ua(1, npy)
      CALL PUSHREAL4(va(0, npy+1))
      va(0, npy+1) = ua(2, npy)
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHCONTROL1B(1)
...
!core_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mo
!d.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_c
!orner_fb fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: u v ua uc ut va vc vt
!   with respect to varying inputs: u v ua uc ut va vc vt
!There is a limit to how far this routine can fill uc and vc in the
! halo, and so either mpp_update_domains or some sort of boundary
...
!sw_core_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core
!_mod.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extra
!p_corner_fb fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
...
! 1: x-dir; 2: y-dir
    ...
    CASE (1) 
      IF (sw_corner) THEN
        CALL PUSHREAL4(q1(-1, 0))
        q1(-1, 0) = q1(0, 2)
        CALL PUSHREAL4(q1(0, 0))
        q1(0, 0) = q1(0, 1)
        CALL PUSHREAL4(q2(-1, 0))
        q2(-1, 0) = q2(0, 2)
        CALL PUSHREAL4(q2(0, 0))
        q2(0, 0) = q2(0, 1)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (se_corner) THEN
        CALL PUSHREAL4(q1(npx+1, 0))
        q1(npx+1, 0) = q1(npx, 2)
        CALL PUSHREAL4(q1(npx, 0))
        q1(npx, 0) = q1(npx, 1)
        CALL PUSHREAL4(q2(npx+1, 0))
        q2(npx+1, 0) = q2(npx, 2)
        CALL PUSHREAL4(q2(npx, 0))
        q2(npx, 0) = q2(npx, 1)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (nw_corner) THEN
        CALL PUSHREAL4(q1(0, npy))
        q1(0, npy) = q1(0, npy-1)
        CALL PUSHREAL4(q1(-1, npy))
        q1(-1, npy) = q1(0, npy-2)
        CALL PUSHREAL4(q2(0, npy))
        q2(0, npy) = q2(0, npy-1)
        CALL PUSHREAL4(q2(-1, npy))
        q2(-1, npy) = q2(0, npy-2)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (ne_corner) THEN
        CALL PUSHREAL4(q1(npx, npy))
        q1(npx, npy) = q1(npx, npy-1)
        CALL PUSHREAL4(q1(npx+1, npy))
        q1(npx+1, npy) = q1(npx, npy-2)
        CALL PUSHREAL4(q2(npx, npy))
        q2(npx, npy) = q2(npx, npy-1)
        CALL PUSHREAL4(q2(npx+1, npy))
        q2(npx+1, npy) = q2(npx, npy-2)
        CALL PUSHCONTROL3B(2)
      ELSE
        CALL PUSHCONTROL3B(1)
    ...
    CASE (2) 
      IF (sw_corner) THEN
        CALL PUSHREAL4(q1(0, 0))
        q1(0, 0) = q1(1, 0)
        CALL PUSHREAL4(q1(0, -1))
        q1(0, -1) = q1(2, 0)
        CALL PUSHREAL4(q2(0, 0))
        q2(0, 0) = q2(1, 0)
        CALL PUSHREAL4(q2(0, -1))
        q2(0, -1) = q2(2, 0)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      ...
      IF (se_corner) THEN
        CALL PUSHREAL4(q1(npx, 0))
        q1(npx, 0) = q1(npx-1, 0)
        CALL PUSHREAL4(q1(npx, -1))
        q1(npx, -1) = q1(npx-2, 0)
        CALL PUSHREAL4(q2(npx, 0))
        q2(npx, 0) = q2(npx-1, 0)
        CALL PUSHREAL4(q2(npx, -1))
        q2(npx, -1) = q2(npx-2, 0)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      ...
      IF (nw_corner) THEN
        CALL PUSHREAL4(q1(0, npy))
        q1(0, npy) = q1(1, npy)
        CALL PUSHREAL4(q1(0, npy+1))
        q1(0, npy+1) = q1(2, npy)
        CALL PUSHREAL4(q2(0, npy))
        q2(0, npy) = q2(1, npy)
        CALL PUSHREAL4(q2(0, npy+1))
        q2(0, npy+1) = q2(2, npy)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      ...
      IF (ne_corner) THEN
        CALL PUSHREAL4(q1(npx, npy))
        q1(npx, npy) = q1(npx-1, npy)
        CALL PUSHREAL4(q1(npx, npy+1))
        q1(npx, npy+1) = q1(npx-2, npy)
        CALL PUSHREAL4(q2(npx, npy))
        q2(npx, npy) = q2(npx-1, npy)
        CALL PUSHREAL4(q2(npx, npy+1))
        q2(npx, npy+1) = q2(npx-2, npy)
        CALL PUSHCONTROL3B(4)
      ELSE
        CALL PUSHCONTROL3B(3)
...
! sw_core_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_cor
!e_mod.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extr
!ap_corner_fb fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: q1 q2
!   with respect to varying inputs: q1 q2
...
! 1: x-dir; 2: y-dir
    ...
    CASE (1) 
      IF (sw_corner) THEN
        CALL PUSHREAL4(q(-1, 0))
        q(-1, 0) = q(0, 2)
        CALL PUSHREAL4(q(0, 0))
        q(0, 0) = q(0, 1)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (se_corner) THEN
        CALL PUSHREAL4(q(npx+1, 0))
        q(npx+1, 0) = q(npx, 2)
        CALL PUSHREAL4(q(npx, 0))
        q(npx, 0) = q(npx, 1)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (nw_corner) THEN
        CALL PUSHREAL4(q(0, npy))
        q(0, npy) = q(0, npy-1)
        CALL PUSHREAL4(q(-1, npy))
        q(-1, npy) = q(0, npy-2)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (ne_corner) THEN
        CALL PUSHREAL4(q(npx, npy))
        q(npx, npy) = q(npx, npy-1)
        CALL PUSHREAL4(q(npx+1, npy))
...
!e_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.f
...
! Local
    ...
    ELSE IF (iord .EQ. 333) THEN
      CALL PUSHREAL4ARRAY(flux, (ie-is+2)*(je-js+2))
      DO j=js,je+1
        DO i=is,ie+1
          IF (c(i, j) .GT. 0.) THEN
...
!DEC$ VECTOR ALWAYS
      ...
      CALL PUSHREAL4ARRAY(bl, ie - is + 3)
      CALL PUSHREAL4ARRAY(br, ie - is + 3)
      CALL PUSHREAL4ARRAY(b0, ie - is + 3)
...
& )
    ...
    IF (branch .LT. 2) THEN
      IF (branch .EQ. 0) THEN
        ...
        END DO
      ELSE
        CALL POPREAL4ARRAY(flux, (ie-is+2)*(je-js+2))
        DO j=js,je+1
          DO i=is,ie+1
            IF (c(i, j) .GT. 0.) THEN
              CALL PUSHCONTROL1B(1)
...
&               )*2*c(i, j)*temp_ad8/6.0 + (u(i, j)-u(i-1, j))*temp_ad6
    ...
    ELSE IF (branch .NE. 2) THEN
      ...
      CALL POPREAL4ARRAY(b0, ie - is + 3)
      CALL POPREAL4ARRAY(br, ie - is + 3)
      CALL POPREAL4ARRAY(bl, ie - is + 3)
...
&               1))
          ...
          END DO
        END DO
        CALL PUSHREAL4ARRAY(bl, (ie-is+2)*(je-js+3))
        CALL PUSHINTEGER4(je3)
        CALL PUSHREAL4ARRAY(br, (ie-is+2)*(je-js+3))
        CALL PUSHINTEGER4(js3)
        CALL PUSHREAL4ARRAY(b0, (ie-is+2)*(je-js+3))
        CALL PUSHCONTROL3B(4)
      ELSE
        CALL PUSHINTEGER4(je3)
        CALL PUSHINTEGER4(js3)
...
&               )*2*c(i, j)*temp_ad8/6.0 + (v(i, j)-v(i, j-1))*temp_ad6
    ...
    ELSE IF (branch .NE. 2) THEN
      IF (branch .EQ. 3) THEN
        ...
        b0_ad = 0.0
      ELSE
        CALL POPREAL4ARRAY(b0, (ie-is+2)*(je-js+3))
        CALL POPINTEGER4(js3)
        CALL POPREAL4ARRAY(br, (ie-is+2)*(je-js+3))
        CALL POPINTEGER4(je3)
        CALL POPREAL4ARRAY(bl, (ie-is+2)*(je-js+3))
        bl_ad = 0.0
        br_ad = 0.0
        b0_ad = 0.0
        DO j=je+1,js,-1
...
!e_divergence_damping_fb sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d t
...
!namics_mod.Rayleigh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_
!mod.fv3_to_geos fv_grid_utils_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.Lagrangian_t
!o_Eulerian fv_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm_
!fb fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile_f
!b fv_mapz_mod.cs_limiters fv_mapz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.
!mappm fv_mapz_mod.moist_cv fv_mapz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_
!tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz
...
!e_mod.ytp_v sw_core_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_g
!host_ew tp_core_mod.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b
!_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: u v ke ua uc ptc delpc va vc
!                vort divg_d wk
!   with respect to varying inputs: u v ke ua uc ptc delpc va vc
!                divg_d wk
...
!ynamics_mod.Rayleigh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics
!_mod.fv3_to_geos fv_grid_utils_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.Lagrangian_
!to_Eulerian fv_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm
!_fb fv_mapz_mod.mapn_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile_
!fb fv_mapz_mod.cs_limiters fv_mapz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod
!.mappm fv_mapz_mod.moist_cv fv_mapz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv
!_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_d
...
!re_mod.ytp_v sw_core_mod.compute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_
!ghost_ew tp_core_mod.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2
!b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: u v ke ua uc ptc delpc va vc
!                vort divg_d wk
!   with respect to varying inputs: u v ke ua uc ptc delpc va vc
!                divg_d wk
...
&                  ng, .false.)
    ...
    !CALL PUSHPOINTER8(rarea_c)
    !CALL PUSHPOINTER8(dyc)
    CALL PUSHREAL4ARRAY(sh, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(jed)
...
&   smag_c_ad, bd, npx, npy, gridstruct, ng)
    ...
    CALL POPINTEGER4(jed)
    CALL POPINTEGER4(is)
    CALL POPREAL4ARRAY(sh, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
    dyc => gridstruct%dyc
    !CALL ADMM_REBASE(dyc)
    rarea_c => gridstruct%rarea_c
    !CALL ADMM_REBASE(rarea_c)
