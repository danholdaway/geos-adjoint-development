...
! to a flux BC on the tracers ONLY, which is implemented in fv_tracer.
 ...
 subroutine update_coarse_grid_mpp_2d(var_coarse, var_nest, nest_domain, ind_update, dx, dy, area, &
   ...
   if (child_proc .and. size(var_nest) > 1) var_nest_3d(is_n:ie_n+istag,js_n:je_n+jstag,1) = &
                                            var_nest(is_n:ie_n+istag,js_n:je_n+jstag)
   if (parent_proc .and. size(var_coarse) > 1) var_coarse_3d(isd_p:ied_p+istag,jsd_p:jed_p,1) = &
                                               var_coarse(isd_p:ied_p+istag,jsd_p:jed_p+jstag)
   ...
   if (size(var_coarse) > 1 .and. parent_proc) var_coarse(isd_p:ied_p+istag,jsd_p:jed_p+jstag) = &
                                               var_coarse_3d(isd_p:ied_p+istag,jsd_p:jed_p,1)
