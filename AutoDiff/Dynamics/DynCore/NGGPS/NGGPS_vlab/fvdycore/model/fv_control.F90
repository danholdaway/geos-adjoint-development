...
!***********************************************************************
! $Id: fv_control.F90,v 1.2.2.1.2.1 2017/09/29 20:23:47 drholdaw Exp $
!
!----------------
! FV contro panel
!----------------
...
!--------------------------------------------------
         ...
         if (grids_on_this_pe(n)) then
            ...
            if ( is_master() ) then
               ...
               write(*,*) 'beta=', beta
               write(*,*) ' '
               write(*,*) 'Tracer damping'
               write(*,*) 'Order', nord_tr
               write(*,*) 'Coefficient', trdm2
            endif
            if ( is_master() ) then
               if (n_split*k_split*p_split/=0) then
                  sdt =  dt_atmos/real(n_split*k_split*p_split)
               else
                  sdt =  1.0
               end if
               if (.not. split_damp) write(*,*) 'SPLIT DAMPING OFF - PERTURBATION Divergence damping Coefficients'
               if (split_damp) write(*,*) 'SPLIT DAMPING ON - PERTURBATION Divergence damping Coefficients'
               write(*,*) 'For small dt=', sdt
               write(*,*) 'Internal mode del-2 SMAG dimensionless coeff=',  dddmp_pert
               write(*,*) 'Internal mode del-2 background diff=', d2_bg_pert*Atm(n)%gridstruct%da_min_c/sdt

               if (nord_pert==1) then
                   write(*,*) 'Internal mode del-4 background diff=', d4_bg_pert
                   write(*,*) 'Vorticity del-4 (m**4/s)=', (vtdm4_pert*Atm(n)%gridstruct%da_min)**2/sdt*1.E-6
               endif
               if (nord_pert==2) write(*,*) 'Internal mode del-6 background diff=', d4_bg_pert
               if (nord_pert==3) write(*,*) 'Internal mode del-8 background diff=', d4_bg_pert

               write(*,*) 'Vorticity del-4 (m**4/s)=', (vtdm4_pert*Atm(n)%gridstruct%da_min)**2/sdt*1.E-6
               write(*,*) 'beta=', beta
               write(*,*) ' '
               write(*,*) 'Tracer perturbation damping'
               write(*,*) 'Order', nord_tr_pert
               write(*,*) 'Coefficient', trdm2_pert
            endif
            if ( is_master() ) then
               write(*,*) 'TRAJECTORY advection options:'
               write(*,*) '  hord_mt =',  hord_mt
               write(*,*) '  hord_vt = ',  hord_vt
               write(*,*) '  hord_tm = ',  hord_tm
               write(*,*) '  hord_dp = ',  hord_dp
               write(*,*) '  hord_tr = ',  hord_tr
               write(*,*) ''
               write(*,*) 'PERTURBATION advection options:'
               write(*,*) '  hord_mt_pert = ',  hord_mt_pert
               write(*,*) '  hord_vt_pert = ',  hord_vt_pert
               write(*,*) '  hord_tm_pert = ',  hord_tm_pert
               write(*,*) '  hord_dp_pert = ',  hord_dp_pert
               write(*,*) '  hord_tr_pert = ',  hord_tr_pert
               write(*,*) ''
               write(*,*) 'TRAJECTORY remapping options:'
               write(*,*) '  kord_mt = ',  kord_mt
               write(*,*) '  kord_wz = ',  kord_wz
               write(*,*) '  kord_tm = ',  kord_tm
               write(*,*) '  kord_tr = ',  kord_tr
               write(*,*) ''
               write(*,*) 'PERTURBATION remapping options:'
               write(*,*) '  kord_mt_pert = ',  kord_mt_pert
               write(*,*) '  kord_wz_pert = ',  kord_wz_pert
               write(*,*) '  kord_tm_pert = ',  kord_tm_pert
               write(*,*) '  kord_tr_pert = ',  kord_tr_pert
               write(*,*)      ''
            endif
...
!     local version of these variables to allow PGI compiler to compile
      ...
      namelist /fv_core_pert_nml/split_hord, hord_mt_pert, hord_vt_pert, hord_tm_pert, hord_dp_pert, hord_tr_pert, &
                                 ...
                                 nord_pert, dddmp_pert, d2_bg_pert, d4_bg_pert, d_con_pert, vtdm4_pert, &
...
!      rewind (f_unit)
   ...
   ! Read linearized FVCORE namelist 
      read (input_nml_file,fv_core_pert_nml,iostat=ios)
...
! Unless specfied the trajectory uses the coeffs suitable for the perts
      if (.not. Atm(n)%flagstruct%split_damp) then
         Atm(n)%flagstruct%nord = Atm(n)%flagstruct%nord_pert
         Atm(n)%flagstruct%dddmp = Atm(n)%flagstruct%dddmp_pert
         Atm(n)%flagstruct%d2_bg = Atm(n)%flagstruct%d2_bg_pert
         Atm(n)%flagstruct%d4_bg = Atm(n)%flagstruct%d4_bg_pert
         Atm(n)%flagstruct%d_con = Atm(n)%flagstruct%d_con_pert
         Atm(n)%flagstruct%do_vort_damp = Atm(n)%flagstruct%do_vort_damp_pert
         Atm(n)%flagstruct%vtdm4 = Atm(n)%flagstruct%vtdm4_pert
      endif

! Unless specfied the trajectory uses the coeffs suitable for the perts
      if (.not. Atm(n)%flagstruct%split_damp_tr) then
         Atm(n)%flagstruct%nord_tr = Atm(n)%flagstruct%nord_tr_pert
         Atm(n)%flagstruct%trdm2 = Atm(n)%flagstruct%trdm2_pert
      endif

! Unless specfied the trajectory uses hord suitable for the perts
      if (.not. Atm(n)%flagstruct%split_hord) then
         Atm(n)%flagstruct%hord_mt = Atm(n)%flagstruct%hord_mt_pert
         Atm(n)%flagstruct%hord_vt = Atm(n)%flagstruct%hord_vt_pert
         Atm(n)%flagstruct%hord_tm = Atm(n)%flagstruct%hord_tm_pert
         Atm(n)%flagstruct%hord_dp = Atm(n)%flagstruct%hord_dp_pert
         Atm(n)%flagstruct%hord_tr = Atm(n)%flagstruct%hord_tr_pert
      endif

! Unless specfied the trajectory uses hord suitable for the perts
      if (.not. Atm(n)%flagstruct%split_kord) then
         Atm(n)%flagstruct%kord_mt = Atm(n)%flagstruct%kord_mt_pert
         Atm(n)%flagstruct%kord_wz = Atm(n)%flagstruct%kord_wz_pert
         Atm(n)%flagstruct%kord_tm = Atm(n)%flagstruct%kord_tm_pert
         Atm(n)%flagstruct%kord_tr = Atm(n)%flagstruct%kord_tr_pert
      endif
...
!Linearized model pointers
     split_hord                    => Atm%flagstruct%split_hord
     hord_mt_pert                  => Atm%flagstruct%hord_mt_pert
     hord_vt_pert                  => Atm%flagstruct%hord_vt_pert
     hord_tm_pert                  => Atm%flagstruct%hord_tm_pert
     hord_dp_pert                  => Atm%flagstruct%hord_dp_pert
     hord_tr_pert                  => Atm%flagstruct%hord_tr_pert
     split_kord                    => Atm%flagstruct%split_kord
     kord_mt_pert                  => Atm%flagstruct%kord_mt_pert
     kord_wz_pert                  => Atm%flagstruct%kord_wz_pert
     kord_tm_pert                  => Atm%flagstruct%kord_tm_pert
     kord_tr_pert                  => Atm%flagstruct%kord_tr_pert
     split_damp                    => Atm%flagstruct%split_damp
     nord_pert                     => Atm%flagstruct%nord_pert
     dddmp_pert                    => Atm%flagstruct%dddmp_pert
     d2_bg_pert                    => Atm%flagstruct%d2_bg_pert
     d4_bg_pert                    => Atm%flagstruct%d4_bg_pert
     d_con_pert                    => Atm%flagstruct%d_con_pert
     do_vort_damp_pert             => Atm%flagstruct%do_vort_damp_pert
     vtdm4_pert                    => Atm%flagstruct%vtdm4_pert
     split_damp_tr                 => Atm%flagstruct%split_damp_tr
     nord_tr_pert                  => Atm%flagstruct%nord_tr_pert
     trdm2_pert                    => Atm%flagstruct%trdm2_pert
