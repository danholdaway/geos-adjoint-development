...
! as input; output: temperature
  ...
  real, intent(inout), dimension(isd:isd,jsd:jsd,1)::q_con, cappa
...
!$OMP do
           do k=kmp,km
              ...
              call fv_sat_adj(abs(mdt), r_vir, is, ie, js, je, ng, hydrostatic, fast_mp_consv, &
                      ...
                      dpln, delz(isd:ied,jsd:jed,k), pt(isd:ied,jsd:jed,k), delp(isd:ied,jsd:jed,k), q_con(isd:isd,jsd:jsd,k), &
   cappa(isd:,jsd:,k), gridstruct%area_64, dtdt(is:ie,js:je,k), out_dt, last_step, cld_amt>0, q(isd:ied,jsd:jed,k,cld_amt))
              if ( .not. hydrostatic  ) then
                 do j=js,je
                    do i=is,ie
