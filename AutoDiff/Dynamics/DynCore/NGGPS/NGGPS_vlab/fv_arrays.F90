...
!  integer, parameter :: REAL8 = REAL64
  ...
  integer, parameter :: FVPRC = 8  ! Full Build Precision for the model
...
!   integer :: kord_mt_pert = 111    ! 111 is linear remapping, not shape preserving or
!   integer :: kord_wz_pert = 111    !     positive definite, anything else not recommended
!   integer :: kord_tm_pert = 111    !     for perturbation quantities
...
!   integer :: nord_tr_pert=0      ! 0: del-2, 1: del-4, 2: del-6
!   real    :: trdm2_pert = 0.0    ! coefficient for del-2 tracer damping
...
!----------------------------------------------------------------------
    ...
    real, ALLOCATABLE ::   ze0(:,:,:)    ! height at layer edges for remapping
    real, ALLOCATABLE ::  q_con(:,:,:)   ! total condensates
...
!-----------------------------------------------------------------------
    ...
    real, ALLOCATABLE :: va(:,:,:)     
    real, ALLOCATABLE :: uc(:,:,:)       ! (uc, vc) are mostly used as the C grid winds
    real, ALLOCATABLE :: vc(:,:,:)     
...
! Accumulated Mass flux arrays
    real, ALLOCATABLE ::  mfx(:,:,:)  
    real, ALLOCATABLE ::  mfy(:,:,:)  
...
! Allocate the perturbation variables
    allocate (    Atm%up(isd:ied  ,jsd:jed+1,npz) )
    Atm%up = 0.0
    allocate (    Atm%vp(isd:ied+1,jsd:jed  ,npz) )
    Atm%vp = 0.0
    allocate (   Atm%ptp(isd:ied  ,jsd:jed  ,npz) )
    Atm%ptp = 0.0
    allocate ( Atm%delpp(isd:ied  ,jsd:jed  ,npz) )
    Atm%delpp = 0.0
    allocate (    Atm%qp(isd:ied  ,jsd:jed  ,npz, ncnst) )
    Atm%qp = 0.0

    allocate (    Atm%wp(isd:ied, jsd:jed  ,npz  ) )
    Atm%wp = 0.0
    allocate ( Atm%delzp(isd:ied, jsd:jed  ,npz) )
    Atm%delzp = 0.0
!if( Atm%flagstruct%hybrid_z ) then
       allocate (  Atm%ze0p(is:ie, js:je ,npz+1) )
!else
!   allocate (  Atm%ze0p(is:is, js:js  ,1) )
!endif
    Atm%ze0p = 0.0

    allocate (   Atm%psp(isd:ied  ,jsd:jed) )
    Atm%psp = 0.0
    allocate (   Atm%pep(is-1:ie+1, npz+1,js-1:je+1) )
    Atm%pep = 0.0
    allocate (   Atm%pkp(is:ie    ,js:je  , npz+1) )
    Atm%pkp = 0.0
    allocate ( Atm%pelnp(is:ie,npz+1,js:je) )
    Atm%pelnp = 0.0
    allocate (  Atm%pkzp(is:ie,js:je,npz) )
    Atm%pkzp = 0.0

    allocate ( Atm%omgap(isd:ied  ,jsd:jed  ,npz) )
    Atm%omgap=0.0

    allocate (   Atm%uap(isd:ied  ,jsd:jed  ,npz) )
    allocate (   Atm%vap(isd:ied  ,jsd:jed  ,npz) )
    allocate (   Atm%ucp(isd:ied+1,jsd:jed  ,npz) )
    allocate (   Atm%vcp(isd:ied  ,jsd:jed+1,npz) )

    allocate ( Atm%mfxp(is:ie+1, js:je,  npz) )
    allocate ( Atm%mfyp(is:ie  , js:je+1,npz) )

    allocate (  Atm%cxp(is:ie+1, jsd:jed, npz) )
    allocate (  Atm%cyp(isd:ied ,js:je+1, npz) )
...
!     6---2---7
    ...
    deallocate ( Atm%vp    )
    deallocate ( Atm%ptp   )
    deallocate ( Atm%delpp )
    deallocate ( Atm%qp    )
    deallocate ( Atm%wp    )
    deallocate ( Atm%delzp )
    deallocate ( Atm%ze0p  )
    deallocate ( Atm%psp   )
    deallocate ( Atm%pep   )
    deallocate ( Atm%pkp   )
    deallocate ( Atm%pelnp )
    deallocate ( Atm%pkzp  )
    deallocate ( Atm%omgap )
    deallocate ( Atm%uap   )
    deallocate ( Atm%vap   )
    deallocate ( Atm%ucp   )
    deallocate ( Atm%vcp   )
