...
!   gradient     of useful results: qc q u v w teq delp delz te_2d
!                pt
!   with respect to varying inputs: qc q u v w delp delz pt
...
&   delp_ad, q, q_ad, qc, qc_ad, pe, peln, hs, rsin2_l, cosa_s_l, r_vir&
&   , cp, rg, hlv, te_2d, te_2d_ad, ua, va, teq, teq_ad, moist_phys, &
&   nwat, sphum, liq_wat, rainwat, ice_wat, snowwat, graupel, &
&   hydrostatic, id_te)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: km, is, ie, js, je, isd, ied, jsd, jed, id_te
    INTEGER, INTENT(IN) :: sphum, liq_wat, ice_wat, rainwat, snowwat, &
      ...
      IF (branch .EQ. 0) THEN
        DO k=km,1,-1
          DO i=ie,is,-1
            ...
            temp_ad6 = delp(i, j, k)*te_2d_ad(i, j)
            temp_ad7 = 0.5*temp_ad6
            temp_ad8 = temp5*temp_ad7
            temp_ad9 = -(cosa_s_l(i, j)*temp_ad8)
...
&             cosa_s_l(i, j)*(temp6*temp7))))*te_2d_ad(i, j)
            pt_ad(i, j, k) = pt_ad(i, j, k) + cv_air*temp_ad6
            phiz_ad(i, k) = phiz_ad(i, k) + temp_ad7
            phiz_ad(i, k+1) = phiz_ad(i, k+1) + temp_ad7
            w_ad(i, j, k) = w_ad(i, j, k) + 2*w(i, j, k)*temp_ad7
            u_ad(i, j, k) = u_ad(i, j, k) + temp7*temp_ad9 + 2*u(i, j, k&
&             )*temp_ad8
            u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp7*temp_ad9 + 2*u(i, &
&             j+1, k)*temp_ad8
            v_ad(i, j, k) = v_ad(i, j, k) + temp6*temp_ad9 + 2*v(i, j, k&
&             )*temp_ad8
            v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp6*temp_ad9 + 2*v(i+1&
&             , j, k)*temp_ad8
          END DO
        END DO
      ELSE IF (branch .EQ. 1) THEN
        DO k=km,1,-1
          DO i=ie,is,-1
            ...
            temp_ad2 = delp(i, j, k)*te_2d_ad(i, j)
            temp_ad3 = 0.5*temp_ad2
            temp_ad4 = temp2*temp_ad3
            temp_ad5 = -(cosa_s_l(i, j)*temp_ad4)
            delp_ad(i, j, k) = delp_ad(i, j, k) + (cv_air*pt(i, j, k)+&
&             0.5*(phiz(i, k)+phiz(i, k+1)+w(i, j, k)**2+temp2*(u(i, j, &
&             k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-&
&             cosa_s_l(i, j)*(temp3*temp4))))*te_2d_ad(i, j)
            pt_ad(i, j, k) = pt_ad(i, j, k) + cv_air*temp_ad2
            phiz_ad(i, k) = phiz_ad(i, k) + temp_ad3
            phiz_ad(i, k+1) = phiz_ad(i, k+1) + temp_ad3
            w_ad(i, j, k) = w_ad(i, j, k) + 2*w(i, j, k)*temp_ad3
            u_ad(i, j, k) = u_ad(i, j, k) + temp4*temp_ad5 + 2*u(i, j, k&
&             )*temp_ad4
            u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp4*temp_ad5 + 2*u(i, &
&             j+1, k)*temp_ad4
            v_ad(i, j, k) = v_ad(i, j, k) + temp3*temp_ad5 + 2*v(i, j, k&
&             )*temp_ad4
            v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp3*temp_ad5 + 2*v(i+1&
&             , j, k)*temp_ad4
          END DO
        END DO
      ELSE
        DO k=km,1,-1
          DO i=ie,is,-1
            ...
            temp_ad = delp(i, j, k)*te_2d_ad(i, j)
            temp_ad0 = temp*temp_ad
            temp_ad1 = -(cosa_s_l(i, j)*temp_ad0)
            delp_ad(i, j, k) = delp_ad(i, j, k) + (cp*tv(i, k)+temp*(u(i&
&             , j, k)**2+u(i, j+1, k)**2+v(i, j, k)**2+v(i+1, j, k)**2-&
&             cosa_s_l(i, j)*(temp0*temp1)))*te_2d_ad(i, j)
            tv_ad(i, k) = tv_ad(i, k) + cp*temp_ad
            u_ad(i, j, k) = u_ad(i, j, k) + temp1*temp_ad1 + 2*u(i, j, k&
&             )*temp_ad0
            u_ad(i, j+1, k) = u_ad(i, j+1, k) + temp1*temp_ad1 + 2*u(i, &
&             j+1, k)*temp_ad0
            v_ad(i, j, k) = v_ad(i, j, k) + temp0*temp_ad1 + 2*v(i, j, k&
&             )*temp_ad0
            v_ad(i+1, j, k) = v_ad(i+1, j, k) + temp0*temp_ad1 + 2*v(i+1&
&             , j, k)*temp_ad0
          END DO
        END DO
        DO i=ie,is,-1
...
&           i, j)
        ...
        DO k=1,km,1
          DO i=ie,is,-1
            ...
            tv_ad(i, k) = tv_ad(i, k) + (peln(i, k+1, j)-peln(i, k, j))*&
&             rg*phiz_ad(i, k)
            phiz_ad(i, k) = 0.0
