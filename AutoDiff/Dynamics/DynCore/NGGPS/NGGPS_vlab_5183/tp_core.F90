...
! Local:
   ...
   call xppm(fx, q_i, crx(is:ie+1,js:je), ord_ou, is,ie,isd,ied, js,je,jsd,jed, npx,npy, gridstruct%dxa, gridstruct%nested, &
...
!$AD II-LOOP
  ...
  do j=jsd,jed
     do i=is,ie
        q_j(i,j) = (q(i,j)*gridstruct%area(i,j) + fx1(i,j)-fx1(i+1,j))/ra_x(i,j)
     enddo
  enddo
...
!                  + x1*(q1(i)  +(1.+xt)*(al(i)-qtmp+xt*(al(i)+al(i+1)-(qtmp+qtmp))))
   ...
   elseif ( iord==333 ) then ! Perfectly linear scheme, more diffusive than ord=2 (HoldawayKent-2015-TellusA)

    !DEC$ VECTOR ALWAYS
          do i=is,ie+1
             xt = c(i,j)
             if ( xt > 0. ) then
                  flux(i,j) = (2.0*q1(i)+5.0*q1(i-1)-q1(i-2) )/6.0 &
                           - 0.5*xt*(q1(i)-q1(i-1)) &
                           + (xt*xt/6.0)*(q1(i)-2.0*q1(i-1)+q1(i-2) )
             else
                  flux(i,j) = (2.0*q1(i-1)+5.0*q1(i)-q1(i+1) )/6.0 &
                           - 0.5*xt*(q1(i)-q1(i-1)) &
                           + (xt*xt/6.0)*(q1(i+1)-2.0*q1(i)+q1(i-1) )
             endif
          enddo
...
! Positive definite constraint:
    if(iord==9 .or. iord==13) call pert_ppm(ie1-is1+1, q1(is1:ie1), bl(is1:ie1), br(is1:ie1), 0)
...
!        if ( iord==8 .or. iord==10 ) then
            xt = max(xt, min(q1(-1),q1(0),min(q1(1),q1(2))))
            xt = min(xt, max(q1(-1),q1(0),max(q1(1),q1(2))))
!        endif
         br(0) = xt - q1(0)
         bl(1) = xt - q1(1)
      ...
      if ( (ie+1)==npx ) then
         ...
         xt = 0.5*(((2.*dxa(npx-1,j)+dxa(npx-2,j))*q1(npx-1)-dxa(npx-1,j)*q1(npx-2))/(dxa(npx-2,j)+dxa(npx-1,j)) &
            +      ((2.*dxa(npx,  j)+dxa(npx+1,j))*q1(npx  )-dxa(npx,  j)*q1(npx+1))/(dxa(npx,  j)+dxa(npx+1,j)))
!        if ( iord==8 .or. iord==10 ) then
            xt = max(xt, min(q1(npx-2),q1(npx-1),min(q1(npx),q1(npx+1))))
            xt = min(xt, max(q1(npx-2),q1(npx-1),max(q1(npx),q1(npx+1))))
!        endif
         br(npx-1) = xt - q1(npx-1)
         bl(npx  ) = xt - q1(npx  )

         br(npx) = s11*(q1(npx+1)-q1(npx)) - s14*dm(npx+1)
         call pert_ppm(3, q1(npx-2:npx), bl(npx-2:npx), br(npx-2:npx), 1)
      endif
    endif
...
!DEC$ VECTOR ALWAYS
   ...
   elseif (jord==333) then ! Perfectly linear scheme, more diffusive than ord=2 (HoldawayKent-2015-TellusA)
         ...
         do i=ifirst,ilast             
             ...
             if ( xt > 0. ) then
                  flux(i,j) = (2.0*q(i,j)+5.0*q(i,j-1)-q(i,j-2) )/6.0 &
                           - 0.5*xt*(q(i,j)-q(i,j-1)) &
                           + (xt*xt/6.0)*(q(i,j)-2.0*q(i,j-1)+q(i,j-2) )
             else
                  flux(i,j) = (2.0*q(i,j-1)+5.0*q(i,j)-q(i,j+1) )/6.0 &
                           - 0.5*xt*(q(i,j)-q(i,j-1)) &
                           + (xt*xt/6.0)*(q(i,j+1)-2.0*q(i,j)+q(i,j-1) )
             endif
          enddo 
       enddo
...
! Positive definite constraint:
     do j=js1,je1
        call pert_ppm(ilast-ifirst+1, q(ifirst:ilast,j), bl(ifirst:ilast,j), br(ifirst:ilast,j), 0)
     enddo
  endif
...
!        if ( jord==8 .or. jord==10 ) then
            xt = max(xt, min(q(i,-1),q(i,0),min(q(i,1),q(i,2))))
            xt = min(xt, max(q(i,-1),q(i,0),max(q(i,1),q(i,2))))
!        endif
         br(i,0) = xt - q(i,0)
         bl(i,1) = xt - q(i,1)
         ...
         br(i,2) = al(i,3) - q(i,2)
      enddo
      call pert_ppm(3*(ilast-ifirst+1), q(ifirst:ilast,0:2), bl(ifirst:ilast,0:2), br(ifirst:ilast,0:2), 1)
    endif
    if( (je+1)==npy ) then
      do i=ifirst,ilast
         ...
         xt = 0.5*(((2.*dya(i,npy-1)+dya(i,npy-2))*q(i,npy-1)-dya(i,npy-1)*q(i,npy-2))/(dya(i,npy-2)+dya(i,npy-1))  &
            +      ((2.*dya(i,npy)+dya(i,npy+1))*q(i,npy)-dya(i,npy)*q(i,npy+1))/(dya(i,npy)+dya(i,npy+1)))
!        if ( jord==8 .or. jord==10 ) then
            xt = max(xt, min(q(i,npy-2),q(i,npy-1),min(q(i,npy),q(i,npy+1))))
            xt = min(xt, max(q(i,npy-2),q(i,npy-1),max(q(i,npy),q(i,npy+1))))
!        endif
     ...
     enddo
     call pert_ppm(3*(ilast-ifirst+1), q(ifirst:ilast,npy-2:npy), bl(ifirst:ilast,npy-2:npy), br(ifirst:ilast,npy-2:npy), 1)
    endif
 end if
...
! Local:
   ...
   call yppm_fb(fy2, q, cry, ord_in, isd,ied,isd,ied, js,je,jsd,jed, npx,npy, &
   ...
   call xppm_fb(fx, q_i, crx(is:ie+1,js:je), ord_ou, is,ie,isd,ied, js,je,jsd,jed, npx,npy, gridstruct%dxa, gridstruct%nested, &
             gridstruct%grid_type)

  if (.not. gridstruct%nested) call copy_corners_fb(q, npx, npy, 1, gridstruct%nested, bd, &
                               gridstruct%sw_corner, gridstruct%se_corner, gridstruct%nw_corner, gridstruct%ne_corner)

  call xppm_fb(fx2, q, crx, ord_in, is,ie,isd,ied, jsd,jed,jsd,jed, npx,npy, &
...
!$AD II-LOOP
  ...
  call yppm_fb(fy, q_j, cry, ord_ou, is,ie,isd,ied, js,je,jsd,jed, npx, npy, &
                gridstruct%dya, gridstruct%nested, gridstruct%grid_type)
...
!$AD II-LOOP
      ...
      if ( present(nord) .and. present(damp_c) .and. present(mass) ) then
        if ( damp_c > 1.e-4 ) then
           damp = (damp_c * gridstruct%da_min)**(nord+1)
           call deln_flux_fb(nord, is,ie,js,je, npx, npy, damp, q, fx, fy, gridstruct, bd, mass )
        endif
      endif
   else
!---------------------------------
...
!---------------------------------
      ...
      if ( present(nord) .and. present(damp_c) ) then
           if ( damp_c > 1.E-4 ) then
                damp = (damp_c * gridstruct%da_min)**(nord+1)
                call deln_flux_fb(nord, is,ie,js,je, npx, npy, damp, q, fx, fy, gridstruct, bd)
           endif
      endif
   endif
...
! YDir:
 ...
 subroutine xppm_fb(flux, q, c, iord, is,ie,isd,ied, jfirst,jlast,jsd,jed, npx, npy, dxa, nested, grid_type)
 integer, INTENT(IN) :: is, ie, isd, ied, jsd, jed
 integer, INTENT(IN) :: jfirst, jlast  ! compute domain
 integer, INTENT(IN) :: iord
 integer, INTENT(IN) :: npx, npy
...
666   continue
 ...
 end subroutine xppm_fb


 subroutine yppm_fb(flux, q, c, jord, ifirst,ilast, isd,ied, js,je,jsd,jed, npx, npy, dya, nested, grid_type)
 integer, INTENT(IN) :: ifirst,ilast    ! Compute domain
 integer, INTENT(IN) :: isd,ied, js,je,jsd,jed
 integer, INTENT(IN) :: jord
 integer, INTENT(IN) :: npx, npy
...
!DEC$ VECTOR ALWAYS
 ...
 end subroutine yppm_fb

 subroutine deln_flux_fb(nord,is,ie,js,je, npx, npy, damp, q, fx, fy, gridstruct, bd, mass )
! Del-n damping for the cell-mean values (A grid)
!------------------
! nord = 0:   del-2
! nord = 1:   del-4
