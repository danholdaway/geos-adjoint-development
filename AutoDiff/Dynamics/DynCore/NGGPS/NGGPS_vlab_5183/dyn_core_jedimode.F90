...
module dyn_core_mod
  ...
  use fv_arrays_mod,      only: fv_flags_pert_type
...
!-----------------------------------------------------------------------
 ...
 subroutine dyn_core(npx, npy, npz, ng, sphum, nq, bdt, n_split, zvir, cp, akap, cappa, grav, hydrostatic,  &
    ...
    real, intent(inout) :: cappa(bd%isd:bd%isd,bd%jsd:bd%jsd,1) ! moist kappa
...
! Divergence damping:
                   ...
                   nord_k_pert=0; d2_divg_pert = max(0.01, flagstructp%d2_bg_pert, flagstruct%d2_bg_k1)
! Vertical velocity:
...
! damping on delp and vorticity:
              ...
              elseif ( k==2 .and. flagstruct%d2_bg_k2>0.01 ) then
                   ...
                   nord_k_pert=0; d2_divg_pert = max(flagstructp%d2_bg_pert, flagstruct%d2_bg_k2)
                   nord_w=0; damp_w = d2_divg
                   nord_w_pert=0; damp_w_pert = d2_divg_pert
                   if ( flagstruct%do_vort_damp ) then
                        nord_v(k)=0; 
              ...
              elseif ( k==3 .and. flagstruct%d2_bg_k2>0.05 ) then
                   ...
                   nord_k_pert=0;  d2_divg_pert = max(flagstructp%d2_bg_pert, 0.2*flagstruct%d2_bg_k2)
                   nord_w=0;  damp_w = d2_divg
                   nord_w_pert=0;  damp_w_pert = d2_divg_pert
                   d_con_k = 0.
              endif
...
! damping on delp and vorticity:
                   ...
                   d_con_k = 0.
              elseif ( k<=flagstruct%n_sponge .and. flagstruct%d2_bg_k2>0.01 ) then
! Apply first order scheme for damping the sponge layer
                   hord_m = 1
                   hord_v = 6
                   hord_t = 6
...
! damping on delp and vorticity:
                   ...
                   d_con_k = 0.
              elseif ( k==flagstruct%n_sponge+1 .and. flagstruct%d2_bg_k2>0.05 ) then
...
1000 continue
 ...
 subroutine geopk(ptop, pe, peln, delp, pk, gz, hs, pt, q_con, pkz, km, akap, CG, nested, computehalo, npx, npy, a2b_ord, bd)
   ...
   real, intent(IN), dimension(bd%isd:bd%isd,bd%jsd:bd%jsd,1):: q_con
