...
!------------------------------
  ...
  USE TP_CORE_MOD_DIFF, ONLY : fv_tp_2d, fv_tp_2d_tlm, fv_tp_2d, &
& fv_tp_2d_tlm
  USE SW_CORE_MOD_DIFF, ONLY : fill_4corners, fill_4corners_tlm, &
& del6_vt_flux, del6_vt_flux_tlm
...
&   cappa, cappa_tl, cp, ptop, hs, w3, w3_tl, pt, pt_tl, q_con, q_con_tl&
&   , delp, delp_tl, gz, gz_tl, pef, pef_tl, ws, ws_tl, p_fac, a_imp, &
&   scale_m)
    IMPLICIT NONE
...
&   cappa
    REAL, DIMENSION(is-ng:ie+ng, js-ng:je+ng, km), INTENT(IN) :: &
&   q_con_tl, cappa_tl
...
&   gm2_tl, cp2_tl
    ...
    REAL, DIMENSION(is-1:ie+1, km+1) :: pem_tl, pe2_tl, peg_tl
    REAL :: gama, rgrav
    INTEGER :: i, j, k
    INTEGER :: is1, ie1
    INTRINSIC LOG
...
! Excluding contribution from condensates:
          peg_tl(i, k) = peg_tl(i, k-1) + dm_tl(i, k-1)*(1.-q_con(i, j, &
&           k-1)) - dm(i, k-1)*q_con_tl(i, j, k-1)
      ...
      DO k=1,km
        DO i=is1,ie1
          ...
          arg1_tl = (peg_tl(i, k+1)*peg(i, k)-peg(i, k+1)*peg_tl(i, k))/&
&           peg(i, k)**2
          arg1 = peg(i, k+1)/peg(i, k)
          pm2_tl(i, k) = ((peg_tl(i, k+1)-peg_tl(i, k))*LOG(arg1)-(peg(i&
&           , k+1)-peg(i, k))*arg1_tl/arg1)/LOG(arg1)**2
          pm2(i, k) = (peg(i, k+1)-peg(i, k))/LOG(arg1)
          cp2_tl(i, k) = cappa_tl(i, j, k)
          cp2(i, k) = cappa(i, j, k)
          gm2_tl(i, k) = -((-cp2_tl(i, k))/(1.-cp2(i, k))**2)
          gm2(i, k) = 1./(1.-cp2(i, k))
...
&                        ), p_fac, scale_m)
      ELSE IF (a_imp .LE. 0.5) THEN
        CALL RIM_2D_TLM(ms, dt, is1, ie1, km, rdgas, gama, gm2, gm2_tl, &
&                 pe2, pe2_tl, dm, dm_tl, pm2, pm2_tl, w2, w2_tl, dz2, &
&                 dz2_tl, pt(is1:ie1, j, 1:km), pt_tl(is1:ie1, j, 1:km)&
&                 , ws(is1:ie1, j), ws_tl(is1:ie1, j), .true.)
      ELSE
        CALL SIM1_SOLVER_TLM(dt, is1, ie1, km, rdgas, gama, gm2, gm2_tl&
&                      , cp2, cp2_tl, akap, pe2, pe2_tl, dm, dm_tl, pm2&
&                      , pm2_tl, pem, pem_tl, w2, w2_tl, dz2, dz2_tl, pt&
&                      (is1:ie1, j, 1:km), pt_tl(is1:ie1, j, 1:km), ws(&
&                      is1:ie1, j), ws_tl(is1:ie1, j), p_fac)
      END IF
      DO k=2,km+1
        DO i=is1,ie1
...
!   with respect to varying inputs: ws gm2 pe2 dm2 dz2 w2 pm2 pt2
  SUBROUTINE RIM_2D_TLM(ms, bdt, is, ie, km, rgas, gama, gm2, gm2_tl, &
&   pe2, pe2_tl, dm2, dm2_tl, pm2, pm2_tl, w2, w2_tl, dz2, dz2_tl, pt2, &
&   pt2_tl, ws, ws_tl, c_core)
    ...
    REAL, DIMENSION(is:ie, km), INTENT(IN) :: dm2_tl, pm2_tl, gm2_tl
...
&           )
          rden = -(rgas*dm(k)/dz(k))
          arg1_tl = gm2_tl(i, k)*LOG(rden*pt1(k)) + gm2(i, k)*(rden_tl*&
&           pt1(k)+rden*pt1_tl(k))/(rden*pt1(k))
          arg1 = gm2(i, k)*LOG(rden*pt1(k))
...
!                pe pt2
  SUBROUTINE SIM1_SOLVER_TLM(dt, is, ie, km, rgas, gama, gm2, gm2_tl, &
&   cp2, cp2_tl, kappa, pe, pe_tl, dm2, dm2_tl, pm2, pm2_tl, pem, pem_tl&
&   , w2, w2_tl, dz2, dz2_tl, pt2, pt2_tl, ws, ws_tl, p_fac)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: is, ie, km
...
&         )*pt2(i, k)/dz2(i, k)**2+dm2(i, k)*pt2_tl(i, k)/dz2(i, k)))
        ...
        arg2_tl = gm2_tl(i, k)*LOG(arg1) + gm2(i, k)*arg1_tl/arg1
        arg2 = gm2(i, k)*LOG(arg1)
...
! Start the w-solver
    DO k=2,km
      DO i=is,ie
        aa_tl(i, k) = (t1g*0.5*(gm2_tl(i, k-1)+gm2_tl(i, k))*(dz2(i, k-1&
&         )+dz2(i, k))-t1g*0.5*(gm2(i, k-1)+gm2(i, k))*(dz2_tl(i, k-1)+&
&         dz2_tl(i, k)))*(pem(i, k)+pp(i, k))/(dz2(i, k-1)+dz2(i, k))**2&
&         + t1g*0.5*(gm2(i, k-1)+gm2(i, k))*(pem_tl(i, k)+pp_tl(i, k))/(&
&         dz2(i, k-1)+dz2(i, k))
...
&         *w2(i, k-1))/bet(i)
    ...
    p1_tl = 0.0
    DO i=is,ie
      p1_tl(i) = (t1g*gm2_tl(i, km)*dz2(i, km)-t1g*gm2(i, km)*dz2_tl(i, &
&       km))*(pem(i, km+1)+pp(i, km+1))/dz2(i, km)**2 + t1g*gm2(i, km)*(&
&       pem_tl(i, km+1)+pp_tl(i, km+1))/dz2(i, km)
...
&         k))+dm2(i, k)*(w2_tl(i, k)-w1_tl(i, k)))
    ...
    DO i=is,ie
      ...
      ELSE
        ...
        max1 = p_fac*pm2(i, km)
      END IF
      arg1_tl = cp2_tl(i, km)*LOG(max1) + (cp2(i, km)-1.)*max1_tl/max1
      arg1 = (cp2(i, km)-1.)*LOG(max1)
...
&         - g_rat(i, k)*p1(i)
        ...
        ELSE
          ...
          max2 = p_fac*pm2(i, k)
        END IF
        arg1_tl = cp2_tl(i, k)*LOG(max2) + (cp2(i, k)-1.)*max2_tl/max2
        arg1 = (cp2(i, k)-1.)*LOG(max2)
...
!                pem pt2
  SUBROUTINE SIM_SOLVER_TLM(dt, is, ie, km, rgas, gama, gm2, gm2_tl, cp2&
&   , cp2_tl, kappa, pe2, pe2_tl, dm2, dm2_tl, pm2, pm2_tl, pem, pem_tl&
&   , w2, w2_tl, dz2, dz2_tl, pt2, pt2_tl, ws, ws_tl, alpha, p_fac, &
&   scale_m)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: is, ie, km
...
! pe2 is Full p
    ...
    DO k=2,km
      DO i=is,ie
        aa_tl(i, k) = (t1g*0.5*(gm2_tl(i, k-1)+gm2_tl(i, k))*(dz2(i, k-1&
&         )+dz2(i, k))-t1g*0.5*(gm2(i, k-1)+gm2(i, k))*(dz2_tl(i, k-1)+&
&         dz2_tl(i, k)))*pe2(i, k)/(dz2(i, k-1)+dz2(i, k))**2 + t1g*0.5*&
&         (gm2(i, k-1)+gm2(i, k))*pe2_tl(i, k)/(dz2(i, k-1)+dz2(i, k))
        aa(i, k) = t1g*0.5*(gm2(i, k-1)+gm2(i, k))/(dz2(i, k-1)+dz2(i, k&
&         ))*pe2(i, k)
        wk_tl(i, k) = t2*(aa_tl(i, k)*(w1(i, k-1)-w1(i, k))+aa(i, k)*(&
&         w1_tl(i, k-1)-w1_tl(i, k)))
...
! Bottom: k=km
    DO i=is,ie
      wk1_tl(i) = (t1g*gm2_tl(i, km)*dz2(i, km)-t1g*gm2(i, km)*dz2_tl(i&
&       , km))*pe2(i, km+1)/dz2(i, km)**2 + t1g*gm2(i, km)*pe2_tl(i, km+&
&       1)/dz2(i, km)
...
&   , delz_tl, pt, pt_tl, phis, q_con, q_con_tl, cappa, cappa_tl, pkc, &
&   pkc_tl, gz, gz_tl, pk3, pk3_tl, npx, npy, npz, nested, pkc_pertn, &
...
&   peln_tl
    ...
    REAL, DIMENSION(bd%isd:bd%ied, npz+1) :: peg_tl, pelng_tl
...
!Hydrostatic interface pressure
          ...
          DO k=2,npz+1
            DO i=ifirst,0
              ...
              peg_tl(i, k) = peg_tl(i, k-1) + delp_tl(i, j, k-1)*(1.-&
&               q_con(i, j, k-1)) - delp(i, j, k-1)*q_con_tl(i, j, k-1)
              peg(i, k) = peg(i, k-1) + delp(i, j, k-1)*(1.-q_con(i, j, &
&               k-1))
              pelng_tl(i, k) = peg_tl(i, k)/peg(i, k)
              pelng(i, k) = LOG(peg(i, k))
            END DO
          END DO
...
&               rdg*delp(i, j, k)*pt_tl(i, j, k)/delz(i, j, k)
              ...
              arg2_tl = cappa_tl(i, j, k)*LOG(arg1)/(1.-cappa(i, j, k))&
&               **2 + arg1_tl/((1.-cappa(i, j, k))*arg1)
              arg2 = 1./(1.-cappa(i, j, k))*LOG(arg1)
              pkz_tl(i, k) = arg2_tl*EXP(arg2)
              pkz(i, k) = EXP(arg2)
!hydro
              pm_tl = ((peg_tl(i, k+1)-peg_tl(i, k))*(pelng(i, k+1)-&
&               pelng(i, k))-(peg(i, k+1)-peg(i, k))*(pelng_tl(i, k+1)-&
&               pelng_tl(i, k)))/(pelng(i, k+1)-pelng(i, k))**2
