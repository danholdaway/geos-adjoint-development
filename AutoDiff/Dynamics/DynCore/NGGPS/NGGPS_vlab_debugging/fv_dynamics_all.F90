...
! Local Arrays
      ...
      real :: dp1(bd%isd:bd%ied, bd%jsd:bd%jed, npz), dtdt_m(bd%is:bd%ie,bd%js:bd%je,npz), cappa(bd%isd:bd%isd,bd%jsd:bd%jsd,1)
...
!since we interpolate on (regular) temperature rather than theta.
      if (gridstruct%nested .or. ANY(neststruct%child_grids)) then
         ...
         !     domain, bd, nwat)
         !call setup_nested_grid_BCs(npx, npy, npz, zvir, ncnst, &
         !     u, v, w, pt, delp, delz, q, uc, vc, pkz, ng, &
         !     gridstruct, flagstruct, neststruct, domain, bd, nwat)
...
!!! CLEANUP: merge these two calls?
       if (gridstruct%nested) then
         call tracer_2d_nested(q, dp1, mfx, mfy, cx, cy, gridstruct, bd, domain, npx, npy, npz, nq,    &
                        ...
                        flagstruct%hord_tr_pert, flagstruct%nord_tr_pert, flagstruct%trdm2_pert, flagstruct%split_damp_tr )
       else
