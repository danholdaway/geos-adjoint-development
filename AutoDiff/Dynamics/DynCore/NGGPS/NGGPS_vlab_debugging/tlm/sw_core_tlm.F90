...
MODULE SW_CORE_MOD_D
  ...
  USE TP_CORE_MOD_D, ONLY : fv_tp_2d, fv_tp_2d_tlm, pert_ppm, &
& pert_ppm_tlm, copy_corners, copy_corners_tlm
  USE FV_MP_MOD_D, ONLY : fill_corners, fill_corners_tlm, xdir, ydir
  USE FV_ARRAYS_MOD_D, ONLY : fv_grid_type, fv_grid_bounds_type, &
& fv_flags_type
  USE A2B_EDGE_MOD_D, ONLY : a2b_ord4, a2b_ord4_tlm
  IMPLICIT NONE
...
&   va, va_tl, wc, wc_tl, ut, ut_tl, vt, vt_tl, divg_d, nord, dt2, &
...
! Local:
    ...
    REAL, DIMENSION(bd%is-1:bd%ie+1, bd%js-1:bd%je+1) :: vort_tl
...
&                 xfx_adv, xfx_adv_tl, yfx_adv, yfx_adv_tl, gridstruct, &
&                 bd, ra_x, ra_x_tl, ra_y, ra_y_tl, nord=nord_v, damp_c=&
&                 damp_v)
    ELSE
      fy_tl = 0.0_8
      fx_tl = 0.0_8
...
&                   cry_adv_tl, npx, npy, hord_vt, gx, gx_tl, gy, gy_tl&
...
&                 xfx_adv, xfx_adv_tl, yfx_adv, yfx_adv_tl, gridstruct, &
&                 bd, ra_x, ra_x_tl, ra_y, ra_y_tl, mfx=fx, mfx_tl=fx_tl&
&                 , mfy=fy, mfy_tl=fy_tl, mass=delp, mass_tl=delp_tl, &
...
&              , v_tl, ub, ub_tl, hord_mt, gridstruct%dy, gridstruct%rdy&
...
&              u_tl, v, vb, vb_tl, hord_mt, gridstruct%dx, gridstruct%&
...
!   endif
    IF (.NOT.split_damp) THEN
      CALL COMPUTE_DIVERGENCE_DAMPING_TLM(nord, d2_bg, d4_bg, dddmp, dt&
&                                   , vort, vort_tl, ptc, ptc_tl, delpc&
&                                   , delpc_tl, ke, ke_tl, u, u_tl, v, &
&                                   v_tl, uc, uc_tl, vc, vc_tl, ua, &
&                                   ua_tl, va, va_tl, divg_d, divg_d_tl&
&                                   , wk, wk_tl, gridstruct, flagstruct&
&                                   , bd)
    ELSE
      CALL COMPUTE_DIVERGENCE_DAMPING_TLM(nord_pert, d2_bg_pert, &
&                                   d4_bg_pert, dddmp_pert, dt, vort, &
&                                   vort_tl, ptc, ptc_tl, delpc, &
&                                   delpc_tl, ke, ke_tl, u, u_tl, v, &
&                                   v_tl, uc, uc_tl, vc, vc_tl, ua, &
&                                   ua_tl, va, va_tl, divg_d, divg_d_tl&
&                                   , wk, wk_tl, gridstruct, flagstruct&
...
! Local
    ...
    REAL :: al_tl(is-1:ie+2), dm_tl(is-2:ie+2)
    REAL :: dq(is-3:ie+2)
    REAL :: dq_tl(is-3:ie+2)
    REAL :: dl, dr, xt, pmp, lac, cfl
    REAL :: xt_tl, pmp_tl, lac_tl, cfl_tl
    REAL :: pmp_1, lac_1, pmp_2, lac_2
    REAL :: pmp_1_tl, lac_1_tl, pmp_2_tl, lac_2_tl
    REAL :: x0, x1, x0l, x0r
    REAL :: x0l_tl, x0r_tl
    INTEGER :: i, j
    INTEGER :: is3, ie3
          ...
          DO i=is,ie+1
            IF (c(i, j) .GT. 0.) THEN
              ...
              IF (smt6(i-1) .OR. smt5(i)) THEN
                fx0_tl(i) = br_tl(i-1) - cfl_tl*b0(i-1) - cfl*b0_tl(i-1)
              ...
              ELSE IF (smt5(i-1)) THEN
                ...
                ELSE
                  ...
                  min1 = x2
                END IF
                fx0_tl(i) = min1_tl*SIGN(1.d0, min1*br(i-1))
                fx0(i) = SIGN(min1, br(i-1))
              END IF
              flux_tl(i, j) = u_tl(i-1, j) + (1.-cfl)*fx0_tl(i) - cfl_tl&
&               *fx0(i)
            ...
            ELSE
              ...
              IF (smt6(i) .OR. smt5(i-1)) THEN
                fx0_tl(i) = bl_tl(i) + cfl_tl*b0(i) + cfl*b0_tl(i)
                fx0(i) = bl(i) + cfl*b0(i)
              ELSE IF (smt5(i)) THEN
                IF (bl(i) .GE. 0.) THEN
                  x3_tl = bl_tl(i)
                ...
                ELSE
                  ...
                  min2 = x3
                END IF
                fx0_tl(i) = min2_tl*SIGN(1.d0, min2*bl(i))
                fx0(i) = SIGN(min2, bl(i))
              END IF
              flux_tl(i, j) = u_tl(i, j) + cfl_tl*fx0(i) + (1.+cfl)*&
&               fx0_tl(i)
          ...
          DO i=is,ie+1
            IF (c(i, j) .GT. 0.) THEN
              IF (smt6(i-1) .OR. smt5(i)) THEN
                cfl_tl = rdx(i-1, j)*c_tl(i, j)
                cfl = c(i, j)*rdx(i-1, j)
                flux_tl(i, j) = u_tl(i-1, j) + (1.-cfl)*(br_tl(i-1)-&
&                 cfl_tl*b0(i-1)-cfl*b0_tl(i-1)) - cfl_tl*(br(i-1)-cfl*&
&                 b0(i-1))
                flux(i, j) = u(i-1, j) + (1.-cfl)*(br(i-1)-cfl*b0(i-1))
              ELSE
...
!  iord=5,6,7
          ...
          DO i=is,ie+1
            IF (c(i, j) .GT. 0.) THEN
              cfl_tl = rdx(i-1, j)*c_tl(i, j)
              cfl = c(i, j)*rdx(i-1, j)
              fx0_tl(i) = (1.-cfl)*(br_tl(i-1)-cfl_tl*b0(i-1)-cfl*b0_tl(&
&               i-1)) - cfl_tl*(br(i-1)-cfl*b0(i-1))
            ...
            ELSE
              cfl_tl = rdx(i, j)*c_tl(i, j)
              cfl = c(i, j)*rdx(i, j)
              fx0_tl(i) = cfl_tl*(bl(i)+cfl*b0(i)) + (1.+cfl)*(bl_tl(i)+&
&               cfl_tl*b0(i)+cfl*b0_tl(i))
              fx0(i) = (1.+cfl)*(bl(i)+cfl*b0(i))
              flux_tl(i, j) = u_tl(i, j)
              flux(i, j) = u(i, j)
            END IF
            IF (smt5(i-1) .OR. smt5(i)) THEN
              flux_tl(i, j) = flux_tl(i, j) + fx0_tl(i)
...
! Perturbation form:
          ...
          ELSE IF (iord .EQ. 9) THEN
            DO i=is3,ie3
              pmp_1_tl = -(2.*dq_tl(i))
              pmp_1 = -(2.*dq(i))
              lac_1_tl = pmp_1_tl + 1.5*dq_tl(i+1)
              ...
              ELSE
                ...
                bl(i) = x7
              END IF
              pmp_2_tl = 2.*dq_tl(i-1)
              pmp_2 = 2.*dq(i-1)
              lac_2_tl = pmp_2_tl - 1.5*dq_tl(i-2)
...
! in
            ...
            ELSE
              ...
              x0l_tl = 0.5*((2.*dx(0, j)+dx(-1, j))*u_tl(0, j)-dx(0, j)*&
&               u_tl(-1, j))/(dx(0, j)+dx(-1, j))
              x0l = 0.5*((2.*dx(0, j)+dx(-1, j))*u(0, j)-dx(0, j)*u(-1, &
&               j))/(dx(0, j)+dx(-1, j))
              x0r_tl = 0.5*((2.*dx(1, j)+dx(2, j))*u_tl(1, j)-dx(1, j)*&
&               u_tl(2, j))/(dx(1, j)+dx(2, j))
...
&               )/(dx(1, j)+dx(2, j))
              ...
              bl(1) = xt - u(1, j)
            END IF
            CALL PERT_PPM_TLM(1, u(2, j), bl(2), bl_tl(2), br(2), br_tl(&
&                       2), -1)
          END IF
...
! out
            ...
            ELSE
              ...
              x0l_tl = 0.5*((2.*dx(npx-1, j)+dx(npx-2, j))*u_tl(npx-1, j&
&               )-dx(npx-1, j)*u_tl(npx-2, j))/(dx(npx-1, j)+dx(npx-2, j&
&               ))
              x0l = 0.5*((2.*dx(npx-1, j)+dx(npx-2, j))*u(npx-1, j)-dx(&
&               npx-1, j)*u(npx-2, j))/(dx(npx-1, j)+dx(npx-2, j))
              x0r_tl = 0.5*((2.*dx(npx, j)+dx(npx+1, j))*u_tl(npx, j)-dx&
&               (npx, j)*u_tl(npx+1, j))/(dx(npx, j)+dx(npx+1, j))
...
&               j)*u(npx+1, j))/(dx(npx, j)+dx(npx+1, j))
              ...
              bl(npx) = xt - u(npx, j)
            END IF
            CALL PERT_PPM_TLM(1, u(npx-2, j), bl(npx-2), bl_tl(npx-2), &
...
!   with respect to varying inputs: v c
  SUBROUTINE YTP_V_TLM(is, ie, js, je, isd, ied, jsd, jed, c, c_tl, u, v&
&   , v_tl, flux, flux_tl, jord, dy, rdy, npx, npy, grid_type, nested)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: is, ie, js, je, isd, ied, jsd, jed
          ...
          DO i=is,ie+1
            IF (c(i, j) .GT. 0.) THEN
              ...
              IF (smt6(i, j-1) .OR. smt5(i, j)) THEN
                fx0_tl(i) = br_tl(i, j-1) - cfl_tl*b0(i, j-1) - cfl*&
&                 b0_tl(i, j-1)
              ...
              ELSE IF (smt5(i, j-1)) THEN
                ...
                END IF
! piece-wise linear
                fx0_tl(i) = min1_tl*SIGN(1.d0, min1*br(i, j-1))
                fx0(i) = SIGN(min1, br(i, j-1))
              END IF
              flux_tl(i, j) = v_tl(i, j-1) + (1.-cfl)*fx0_tl(i) - cfl_tl&
&               *fx0(i)
            ...
            ELSE
              ...
              IF (smt6(i, j) .OR. smt5(i, j-1)) THEN
                fx0_tl(i) = bl_tl(i, j) + cfl_tl*b0(i, j) + cfl*b0_tl(i&
&                 , j)
              ...
              ELSE IF (smt5(i, j)) THEN
                ...
                ELSE
                  ...
                  min2 = x3
                END IF
                fx0_tl(i) = min2_tl*SIGN(1.d0, min2*bl(i, j))
                fx0(i) = SIGN(min2, bl(i, j))
              END IF
              flux_tl(i, j) = v_tl(i, j) + cfl_tl*fx0(i) + (1.+cfl)*&
&               fx0_tl(i)
          ...
          DO i=is,ie+1
            IF (c(i, j) .GT. 0.) THEN
              IF (smt6(i, j-1) .OR. smt5(i, j)) THEN
                ...
                cfl = c(i, j)*rdy(i, j-1)
                flux_tl(i, j) = v_tl(i, j-1) + (1.-cfl)*(br_tl(i, j-1)-&
&                 cfl_tl*b0(i, j-1)-cfl*b0_tl(i, j-1)) - cfl_tl*(br(i, j&
&                 -1)-cfl*b0(i, j-1))
        ...
        DO j=js,je+1
          DO i=is,ie+1
            IF (c(i, j) .GT. 0.) THEN
              cfl_tl = rdy(i, j-1)*c_tl(i, j)
              cfl = c(i, j)*rdy(i, j-1)
              fx0_tl(i) = (1.-cfl)*(br_tl(i, j-1)-cfl_tl*b0(i, j-1)-cfl*&
&               b0_tl(i, j-1)) - cfl_tl*(br(i, j-1)-cfl*b0(i, j-1))
            ...
            ELSE
              cfl_tl = rdy(i, j)*c_tl(i, j)
              cfl = c(i, j)*rdy(i, j)
              fx0_tl(i) = cfl_tl*(bl(i, j)+cfl*b0(i, j)) + (1.+cfl)*(&
&               bl_tl(i, j)+cfl_tl*b0(i, j)+cfl*b0_tl(i, j))
...
&             )
        ...
        ELSE IF (jord .EQ. 9) THEN
          ...
          DO j=js3,je3
            DO i=is,ie+1
              pmp_1_tl = -(2.*dq_tl(i, j))
              pmp_1 = -(2.*dq(i, j))
              lac_1_tl = pmp_1_tl + 1.5*dq_tl(i, j+1)
              ...
              ELSE
                ...
                bl(i, j) = x7
              END IF
              pmp_2_tl = 2.*dq_tl(i, j-1)
              pmp_2 = 2.*dq(i, j-1)
              lac_2_tl = pmp_2_tl - 1.5*dq_tl(i, j-2)
...
!--------------
        IF (js .EQ. 1 .AND. (.NOT.nested)) THEN
          DO i=is,ie+1
            ...
            x0l_tl = 0.5*((2.*dy(i, 0)+dy(i, -1))*v_tl(i, 0)-dy(i, 0)*&
&             v_tl(i, -1))/(dy(i, 0)+dy(i, -1))
            x0l = 0.5*((2.*dy(i, 0)+dy(i, -1))*v(i, 0)-dy(i, 0)*v(i, -1)&
&             )/(dy(i, 0)+dy(i, -1))
            x0r_tl = 0.5*((2.*dy(i, 1)+dy(i, 2))*v_tl(i, 1)-dy(i, 1)*&
&             v_tl(i, 2))/(dy(i, 1)+dy(i, 2))
...
! in
          ...
          END IF
          j = 2
          CALL PERT_PPM_TLM(ie - is + 2, v(is, j), bl(is, j), bl_tl(is, &
&                     j), br(is, j), br_tl(is, j), -1)
        END IF
...
&             i, npy-2)
            ...
            x0l_tl = 0.5*((2.*dy(i, npy-1)+dy(i, npy-2))*v_tl(i, npy-1)-&
&             dy(i, npy-1)*v_tl(i, npy-2))/(dy(i, npy-1)+dy(i, npy-2))
            x0l = 0.5*((2.*dy(i, npy-1)+dy(i, npy-2))*v(i, npy-1)-dy(i, &
&             npy-1)*v(i, npy-2))/(dy(i, npy-1)+dy(i, npy-2))
            x0r_tl = 0.5*((2.*dy(i, npy)+dy(i, npy+1))*v_tl(i, npy)-dy(i&
&             , npy)*v_tl(i, npy+1))/(dy(i, npy)+dy(i, npy+1))
...
!  Differentiation of compute_divergence_damping in forward (tangent) mode (with options r8):
...
&   dt, vort, vort_tl, ptc, ptc_tl, delpc, delpc_tl, ke, ke_tl, u, u_tl&
&   , v, v_tl, uc, uc_tl, vc, vc_tl, ua, ua_tl, va, va_tl, divg_d, &
...
! Interpolate relative vort to cell corners
        vort_tl = 0.0_8
        CALL A2B_ORD4_TLM(wk, wk_tl, vort, vort_tl, gridstruct, npx, npy&
...
&         -ut_tl(i+1, j))
    ...
    END DO
    sh_tl = 0.0_8
    CALL A2B_ORD4_TLM(wk, wk_tl, sh, sh_tl, gridstruct, npx, npy, is, ie&
&               , js, je, ng, .false.)
    DO j=js,je+1
      DO i=is,ie+1
