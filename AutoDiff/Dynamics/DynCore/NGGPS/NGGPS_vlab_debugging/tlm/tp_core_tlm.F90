...
&   hord, fx, fx_tl, fy, fy_tl, xfx, xfx_tl, yfx, yfx_tl, gridstruct, bd&
&   , ra_x, ra_x_tl, ra_y, ra_y_tl, mfx, mfx_tl, mfy, mfy_tl, mass, &
&   mass_tl, nord, damp_c)
    IMPLICIT NONE
...
! Local:
    ...
    END IF
    ord_ou = hord
    IF (.NOT.gridstruct%nested) CALL COPY_CORNERS_TLM(q, q_tl, npx, npy&
&                                               , 2, gridstruct%nested, &
&                                               bd, gridstruct%sw_corner&
...
&         )/ra_y(i, j)
      END DO
    END DO
    CALL XPPM_TLM(fx, fx_tl, q_i, q_i_tl, crx(is:ie+1, js:je), crx_tl(is&
...
&                                               gridstruct%ne_corner)
    fx2_tl = 0.0_8
    CALL XPPM_TLM(fx2, fx2_tl, q, q_tl, crx, crx_tl, ord_in, is, ie, isd&
...
&                      q_tl, fx, fx_tl, fy, fy_tl, gridstruct, bd, mass&
&                      , mass_tl)
        END IF
      END IF
    ELSE
...
!  Differentiation of xppm in forward (tangent) mode (with options r8):
!   variations   of useful results: flux
!   with respect to varying inputs: q flux c
  SUBROUTINE XPPM_TLM(flux, flux_tl, q, q_tl, c, c_tl, iord, is, ie, isd&
...
! Local
    ...
    REAL, DIMENSION(is:ie+1) :: fx0_tl, fx1_tl
    ...
    REAL :: dq(is-3:ie+2)
    REAL :: dq_tl(is-3:ie+2)
    INTEGER :: i, j, ie3, is1, ie1
    REAL :: x0, x1, xt, qtmp, pmp_1, lac_1, pmp_2, lac_2
    REAL :: xt_tl, qtmp_tl, pmp_1_tl, lac_1_tl, pmp_2_tl, lac_2_tl
    INTRINSIC MAX
    INTRINSIC MIN
    INTRINSIC ABS
    INTRINSIC SIGN
          ...
          DO i=is,ie+1
            ...
            IF (xt .GT. 0.) THEN
              ...
              IF (smt6(i-1) .OR. smt5(i)) THEN
                fx1_tl(i) = br_tl(i-1) - xt_tl*b0(i-1) - xt*b0_tl(i-1)
...
! 2nd order, piece-wise linear
                fx1_tl(i) = min1_tl*SIGN(1.d0, min1*br(i-1))
            ...
            ELSE
              ...
              IF (smt6(i) .OR. smt5(i-1)) THEN
                fx1_tl(i) = bl_tl(i) + xt_tl*b0(i) + xt*b0_tl(i)
                fx1(i) = bl(i) + xt*b0(i)
              ELSE IF (smt5(i)) THEN
                IF (bl(i) .GE. 0.) THEN
                  x3_tl = bl_tl(i)
                ...
                ELSE
                  ...
                  min2 = x3
                END IF
                fx1_tl(i) = min2_tl*SIGN(1.d0, min2*bl(i))
            ...
            ELSE
              ...
              abs0 = -xt
            END IF
            flux_tl(i, j) = fx0_tl(i) + (1.-abs0)*fx1_tl(i) - abs0_tl*&
&             fx1(i)
        ...
        ELSE IF (iord .EQ. 4) THEN
          ...
          DO i=is,ie+1
            IF (c(i, j) .GT. 0.) THEN
              ...
              fx0(i) = q1(i-1)
              IF (smt6(i-1) .OR. smt5(i)) THEN
                fx1_tl(i) = (1.-c(i, j))*(br_tl(i-1)-c_tl(i, j)*b0(i-1)-&
&                 c(i, j)*b0_tl(i-1)) - c_tl(i, j)*(br(i-1)-c(i, j)*b0(i&
&                 -1))
            ...
            ELSE
              ...
              fx0(i) = q1(i)
              IF (smt6(i) .OR. smt5(i-1)) THEN
                fx1_tl(i) = c_tl(i, j)*(bl(i)+c(i, j)*b0(i)) + (1.+c(i, &
&                 j))*(bl_tl(i)+c_tl(i, j)*b0(i)+c(i, j)*b0_tl(i))
                fx1(i) = (1.+c(i, j))*(bl(i)+c(i, j)*b0(i))
              END IF
            END IF
            flux_tl(i, j) = fx0_tl(i) + fx1_tl(i)
...
! iord = 5 & 6
          ...
          DO i=is,ie+1
            IF (c(i, j) .GT. 0.) THEN
              fx1_tl(i) = (1.-c(i, j))*(br_tl(i-1)-c_tl(i, j)*b0(i-1)-c(&
&               i, j)*b0_tl(i-1)) - c_tl(i, j)*(br(i-1)-c(i, j)*b0(i-1))
              fx1(i) = (1.-c(i, j))*(br(i-1)-c(i, j)*b0(i-1))
              flux_tl(i, j) = q1_tl(i-1)
              flux(i, j) = q1(i-1)
            ELSE
              fx1_tl(i) = c_tl(i, j)*(bl(i)+c(i, j)*b0(i)) + (1.+c(i, j)&
&               )*(bl_tl(i)+c_tl(i, j)*b0(i)+c(i, j)*b0_tl(i))
              fx1(i) = (1.+c(i, j))*(bl(i)+c(i, j)*b0(i))
              flux_tl(i, j) = q1_tl(i)
              flux(i, j) = q1(i)
            END IF
            IF (smt5(i-1) .OR. smt5(i)) THEN
              flux_tl(i, j) = flux_tl(i, j) + fx1_tl(i)
...
! ord = 13: 10 plus positive definite constraint
        DO i=is-2,ie+2
          xt_tl = 0.25*(q1_tl(i+1)-q1_tl(i-1))
          ...
          ELSE
            ...
            min3 = x4
          END IF
          dm_tl(i) = min3_tl*SIGN(1.d0, min3*xt)
          dm(i) = SIGN(min3, xt)
        END DO
        DO i=is1,ie1+1
          al_tl(i) = 0.5*(q1_tl(i-1)+q1_tl(i)) + r3*(dm_tl(i-1)-dm_tl(i)&
&           )
        ...
        IF (iord .EQ. 8) THEN
          DO i=is1,ie1
            ...
            ELSE
              ...
              min4 = x5
            END IF
            bl_tl(i) = -(min4_tl*SIGN(1.d0, min4*xt))
            bl(i) = -SIGN(min4, xt)
            IF (xt .GE. 0.) THEN
              x6_tl = xt_tl
              x6 = xt
...
! This is emulation of 2nd van Leer scheme using PPM codes
          DO i=is1,ie1
            ...
            ELSE
              ...
              min6 = x7
            END IF
            bl_tl(i) = -(min6_tl*SIGN(1.d0, min6*xt))
            bl(i) = -SIGN(min6, xt)
            IF (xt .GE. 0.) THEN
              x8_tl = xt_tl
              x8 = xt
            ...
            ELSE
              ...
              min7 = x8
            END IF
            br_tl(i) = min7_tl*SIGN(1.d0, min7*xt)
            br(i) = SIGN(min7, xt)
          END DO
        ELSE
          DO i=is1-2,ie1+1
            dq_tl(i) = 2.*(q1_tl(i+1)-q1_tl(i))
            dq(i) = 2.*(q1(i+1)-q1(i))
          END DO
          DO i=is1,ie1
            bl_tl(i) = al_tl(i) - q1_tl(i)
            ...
            ELSE
              ...
              IF (abs3 .GT. abs6) THEN
                pmp_2_tl = dq_tl(i-1)
                pmp_2 = dq(i-1)
                lac_2_tl = pmp_2_tl - 0.75*dq_tl(i-2)
                ...
                pmp_1_tl = -dq_tl(i)
                pmp_1 = -dq(i)
                lac_1_tl = pmp_1_tl + 0.75*dq_tl(i+1)
...
! Positive definite constraint:
        IF (iord .EQ. 9 .OR. iord .EQ. 13) CALL PERT_PPM_TLM(ie1 - is1 +&
&                                                      1, q1(is1), bl(&
&                                                      is1), bl_tl(is1)&
&                                                      , br(is1), br_tl(&
&                                                      is1), 0)
        IF (.NOT.nested .AND. grid_type .LT. 3) THEN
          IF (is .EQ. 1) THEN
            bl_tl(0) = s14*dm_tl(-1) + s11*(q1_tl(-1)-q1_tl(0))
            bl(0) = s14*dm(-1) + s11*(q1(-1)-q1(0))
            xt_tl = 0.5*(((2.*dxa(0, j)+dxa(-1, j))*q1_tl(0)-dxa(0, j)*&
&             q1_tl(-1))/(dxa(-1, j)+dxa(0, j))+((2.*dxa(1, j)+dxa(2, j)&
&             )*q1_tl(1)-dxa(1, j)*q1_tl(2))/(dxa(1, j)+dxa(2, j)))
...
&             dxa(1, j)*q1(2))/(dxa(1, j)+dxa(2, j)))
            ...
            END IF
!        endif
            br_tl(0) = xt_tl - q1_tl(0)
            br(0) = xt - q1(0)
            bl_tl(1) = xt_tl - q1_tl(1)
            bl(1) = xt - q1(1)
            xt_tl = s15*q1_tl(1) + s11*q1_tl(2) - s14*dm_tl(2)
            xt = s15*q1(1) + s11*q1(2) - s14*dm(2)
            br_tl(1) = xt_tl - q1_tl(1)
            br(1) = xt - q1(1)
            bl_tl(2) = xt_tl - q1_tl(2)
            bl(2) = xt - q1(2)
            br_tl(2) = al_tl(3) - q1_tl(2)
            br(2) = al(3) - q1(2)
            CALL PERT_PPM_TLM(3, q1(0), bl(0), bl_tl(0), br(0), br_tl(0)&
&                       , 1)
          END IF
          IF (ie + 1 .EQ. npx) THEN
            bl_tl(npx-2) = al_tl(npx-2) - q1_tl(npx-2)
            bl(npx-2) = al(npx-2) - q1(npx-2)
            xt_tl = s15*q1_tl(npx-1) + s11*q1_tl(npx-2) + s14*dm_tl(npx-&
&             2)
            xt = s15*q1(npx-1) + s11*q1(npx-2) + s14*dm(npx-2)
            br_tl(npx-2) = xt_tl - q1_tl(npx-2)
            br(npx-2) = xt - q1(npx-2)
            bl_tl(npx-1) = xt_tl - q1_tl(npx-1)
            bl(npx-1) = xt - q1(npx-1)
            xt_tl = 0.5*(((2.*dxa(npx-1, j)+dxa(npx-2, j))*q1_tl(npx-1)-&
&             dxa(npx-1, j)*q1_tl(npx-2))/(dxa(npx-2, j)+dxa(npx-1, j))+&
&             ((2.*dxa(npx, j)+dxa(npx+1, j))*q1_tl(npx)-dxa(npx, j)*&
&             q1_tl(npx+1))/(dxa(npx, j)+dxa(npx+1, j)))
...
&             (dxa(npx, j)+dxa(npx+1, j)))
            ...
            END IF
!        endif
            br_tl(npx-1) = xt_tl - q1_tl(npx-1)
            br(npx-1) = xt - q1(npx-1)
            bl_tl(npx) = xt_tl - q1_tl(npx)
            bl(npx) = xt - q1(npx)
            br_tl(npx) = s11*(q1_tl(npx+1)-q1_tl(npx)) - s14*dm_tl(npx+1&
&             )
            br(npx) = s11*(q1(npx+1)-q1(npx)) - s14*dm(npx+1)
            CALL PERT_PPM_TLM(3, q1(npx-2), bl(npx-2), bl_tl(npx-2), br(&
&                       npx-2), br_tl(npx-2), 1)
          END IF
        END IF
        DO i=is,ie+1
...
&             , j)*(bl(i-1)+br(i-1))-c(i, j)*(bl_tl(i-1)+br_tl(i-1))) - &
&             c_tl(i, j)*(br(i-1)-c(i, j)*(bl(i-1)+br(i-1)))
            flux(i, j) = q1(i-1) + (1.-c(i, j))*(br(i-1)-c(i, j)*(bl(i-1&
&             )+br(i-1)))
          ELSE
            flux_tl(i, j) = q1_tl(i) + c_tl(i, j)*(bl(i)+c(i, j)*(bl(i)+&
...
&             )))
    ...
    END DO
  END SUBROUTINE XPPM
!  Differentiation of yppm in forward (tangent) mode (with options r8):
!   variations   of useful results: flux
!   with respect to varying inputs: q flux c
...
&           q_tl(i, j+1))
      ...
      IF (jord .EQ. 7) THEN
        DO j=js1,je3
          DO i=ifirst,ilast
            IF (al(i, j) .LT. 0.) THEN
              al_tl(i, j) = 0.5*(q_tl(i, j)+q_tl(i, j+1))
          ...
          DO i=ifirst,ilast
            ...
            IF (xt .GT. 0.) THEN
              ...
              IF (smt6(i, j-1) .OR. smt5(i, j)) THEN
                fx1_tl(i) = br_tl(i, j-1) - xt_tl*b0(i, j-1) - xt*b0_tl(&
&                 i, j-1)
...
! both up-downwind sides are noisy; 2nd order, piece-wise linear
                fx1_tl(i) = min1_tl*SIGN(1.d0, min1*br(i, j-1))
            ...
            ELSE
              ...
              IF (smt6(i, j) .OR. smt5(i, j-1)) THEN
                fx1_tl(i) = bl_tl(i, j) + xt_tl*b0(i, j) + xt*b0_tl(i, j&
&                 )
              ...
              ELSE IF (smt5(i, j)) THEN
                ...
                ELSE
                  ...
                  min2 = x2
                END IF
                fx1_tl(i) = min2_tl*SIGN(1.d0, min2*bl(i, j))
        ...
        DO j=js,je+1
          ...
          DO i=ifirst,ilast
            IF (c(i, j) .GT. 0.) THEN
              ...
              fx0(i) = q(i, j-1)
              IF (smt6(i, j-1) .OR. smt5(i, j)) THEN
                fx1_tl(i) = (1.-c(i, j))*(br_tl(i, j-1)-c_tl(i, j)*b0(i&
&                 , j-1)-c(i, j)*b0_tl(i, j-1)) - c_tl(i, j)*(br(i, j-1)&
&                 -c(i, j)*b0(i, j-1))
            ...
            ELSE
              ...
              fx0(i) = q(i, j)
              IF (smt6(i, j) .OR. smt5(i, j-1)) THEN
                fx1_tl(i) = c_tl(i, j)*(bl(i, j)+c(i, j)*b0(i, j)) + (1.&
&                 +c(i, j))*(bl_tl(i, j)+c_tl(i, j)*b0(i, j)+c(i, j)*&
&                 b0_tl(i, j))
...
! jord=5,6,7
        ...
        DO j=js,je+1
          DO i=ifirst,ilast
            IF (c(i, j) .GT. 0.) THEN
              fx1_tl(i) = (1.-c(i, j))*(br_tl(i, j-1)-c_tl(i, j)*b0(i, j&
&               -1)-c(i, j)*b0_tl(i, j-1)) - c_tl(i, j)*(br(i, j-1)-c(i&
&               , j)*b0(i, j-1))
              fx1(i) = (1.-c(i, j))*(br(i, j-1)-c(i, j)*b0(i, j-1))
              flux_tl(i, j) = q_tl(i, j-1)
              flux(i, j) = q(i, j-1)
            ELSE
              fx1_tl(i) = c_tl(i, j)*(bl(i, j)+c(i, j)*b0(i, j)) + (1.+c&
&               (i, j))*(bl_tl(i, j)+c_tl(i, j)*b0(i, j)+c(i, j)*b0_tl(i&
&               , j))
              fx1(i) = (1.+c(i, j))*(bl(i, j)+c(i, j)*b0(i, j))
      ...
      DO j=js-2,je+2
        DO i=ifirst,ilast
          ...
          ELSE
            ...
            min3 = x3
          END IF
          dm_tl(i, j) = min3_tl*SIGN(1.d0, min3*xt)
          dm(i, j) = SIGN(min3, xt)
        END DO
      END DO
      al_tl = 0.0_8
        ...
        DO j=js1,je1
          DO i=ifirst,ilast
            ...
            ELSE
              ...
              min4 = x4
            END IF
            bl_tl(i, j) = -(min4_tl*SIGN(1.d0, min4*xt))
            bl(i, j) = -SIGN(min4, xt)
            IF (xt .GE. 0.) THEN
              x5_tl = xt_tl
              x5 = xt
            ...
            ELSE
              ...
              min5 = x5
            END IF
            br_tl(i, j) = min5_tl*SIGN(1.d0, min5*xt)
            br(i, j) = SIGN(min5, xt)
          END DO
        END DO
        ...
        DO j=js1,je1
          DO i=ifirst,ilast
            ...
            ELSE
              ...
              min6 = x6
            END IF
            bl_tl(i, j) = -(min6_tl*SIGN(1.d0, min6*xt))
            bl(i, j) = -SIGN(min6, xt)
            IF (xt .GE. 0.) THEN
              x7_tl = xt_tl
              x7 = xt
            ...
            ELSE
              ...
              min7 = x7
            END IF
            br_tl(i, j) = min7_tl*SIGN(1.d0, min7*xt)
            br(i, j) = SIGN(min7, xt)
          END DO
        END DO
      ELSE
        ...
        DO j=js1-2,je1+1
          DO i=ifirst,ilast
            dq_tl(i, j) = 2.*(q_tl(i, j+1)-q_tl(i, j))
        ...
        DO j=js1,je1
          DO i=ifirst,ilast
            ...
            ELSE
              ...
              END IF
              IF (abs3 .GT. abs6) THEN
                pmp_2_tl = dq_tl(i, j-1)
                pmp_2 = dq(i, j-1)
                lac_2_tl = pmp_2_tl - 0.75*dq_tl(i, j-2)
                ...
                pmp_1_tl = -dq_tl(i, j)
                pmp_1 = -dq(i, j)
                lac_1_tl = pmp_1_tl + 0.75*dq_tl(i, j+1)
...
&                     ifirst, j), 0)
      ...
      IF (.NOT.nested .AND. grid_type .LT. 3) THEN
        IF (js .EQ. 1) THEN
          DO i=ifirst,ilast
            bl_tl(i, 0) = s14*dm_tl(i, -1) + s11*(q_tl(i, -1)-q_tl(i, 0)&
&             )
            bl(i, 0) = s14*dm(i, -1) + s11*(q(i, -1)-q(i, 0))
            xt_tl = 0.5*(((2.*dya(i, 0)+dya(i, -1))*q_tl(i, 0)-dya(i, 0)&
&             *q_tl(i, -1))/(dya(i, -1)+dya(i, 0))+((2.*dya(i, 1)+dya(i&
&             , 2))*q_tl(i, 1)-dya(i, 1)*q_tl(i, 2))/(dya(i, 1)+dya(i, 2&
&             )))
...
!        endif
            ...
            br(i, 2) = al(i, 3) - q(i, 2)
          END DO
          CALL PERT_PPM_TLM(3*(ilast-ifirst+1), q(ifirst, 0), bl(ifirst&
&                     , 0), bl_tl(ifirst, 0), br(ifirst, 0), br_tl(&
&                     ifirst, 0), 1)
        END IF
        IF (je + 1 .EQ. npy) THEN
          DO i=ifirst,ilast
...
&             )-dya(i, npy-1)*q_tl(i, npy-2))/(dya(i, npy-2)+dya(i, npy-&
&             1))+((2.*dya(i, npy)+dya(i, npy+1))*q_tl(i, npy)-dya(i, &
&             npy)*q_tl(i, npy+1))/(dya(i, npy)+dya(i, npy+1)))
...
!        endif
            ...
            bl(i, npy) = xt - q(i, npy)
            br_tl(i, npy) = s11*(q_tl(i, npy+1)-q_tl(i, npy)) - s14*&
&             dm_tl(i, npy+1)
            br(i, npy) = s11*(q(i, npy+1)-q(i, npy)) - s14*dm(i, npy+1)
          END DO
          CALL PERT_PPM_TLM(3*(ilast-ifirst+1), q(ifirst, npy-2), bl(&
&                     ifirst, npy-2), bl_tl(ifirst, npy-2), br(ifirst, &
&                     npy-2), br_tl(ifirst, npy-2), 1)
        END IF
      END IF
      DO j=js,je+1
        DO i=ifirst,ilast
...
!   with respect to varying inputs: q mass fx fy
  SUBROUTINE DELN_FLUX_TLM(nord, is, ie, js, je, npx, npy, damp, q, q_tl&
&   , fx, fx_tl, fy, fy_tl, gridstruct, bd, mass, mass_tl)
    IMPLICIT NONE
