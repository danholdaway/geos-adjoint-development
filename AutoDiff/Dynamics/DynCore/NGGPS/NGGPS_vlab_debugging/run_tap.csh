...
$TAPENADE_HOME/bin/tapenade ${opts} -d -O tlm/ -head "fv_dynamics_mod.fv_dynamics (u v pt delp q w delz)/(u v pt delp q w delz)" fv_dynamics.F90 a2b_edge.F90 boundary.F90 dyn_core.F90 fv_arrays.F90 fv_cmp.F90 fv_control.F90 fv_fill.F90 fv_grid_utils.F90 fv_mapz.F90 fv_nesting.F90 fv_sg.F90 fv_tracer2d.F90 nh_core.F90 nh_utils.F90 sw_core.F90 tp_core.F90 MAPL/*.F90 tools/*.F90 fms/*.F90
...
rm -f mpp* init_hydro_tlm.F90 fv_arrays_tlm.F90 fms_io_tlm.F90
...
sed -i -- 's/CALL ADMM_REBASE/\!CALL ADMM_REBASE/g' *0
grep -i 'CALL POPPOINTER8' *0 | cut -d '(' -f 2- | cut -d ')' -f 1 | awk '\!seen[$0]++' > pointers.txt

foreach line ( "`cat pointers.txt`" )
 set pointto = `grep -i "=>" *0 | grep -i -m 1 "$line " | cut -d ' ' -f 7-`
 set replace = `echo "CALL POPPOINTER8("$line")"`
