...
!      d2_divg = min(0.20, flagstruct%d2_bg*(1.-3.*tanh(0.1*log(pfull(k)/pfull(npz)))))
       ...
       d2_divg_pert = min(0.20, flagstruct%d2_bg_pert)
...
! Divergence damping:
                   ...
                   nord_k_pert=0; d2_divg_pert = max(0.01, flagstruct%d2_bg_pert, flagstruct%d2_bg_k1)
! Vertical velocity:
...
! damping on delp and vorticity:
              ...
              elseif ( k==2 .and. flagstruct%d2_bg_k2>0.01 ) then
                   ...
                   nord_k_pert=0; d2_divg_pert = max(flagstruct%d2_bg_pert, flagstruct%d2_bg_k2)
                   nord_w=0; damp_w = d2_divg
                   nord_w_pert=0; damp_w_pert = d2_divg_pert
                   if ( flagstruct%do_vort_damp ) then
                        nord_v(k)=0; 
              ...
              elseif ( k==3 .and. flagstruct%d2_bg_k2>0.05 ) then
                   ...
                   nord_k_pert=0;  d2_divg_pert = max(flagstruct%d2_bg_pert, 0.2*flagstruct%d2_bg_k2)
                   nord_w=0;  damp_w = d2_divg
                   nord_w_pert=0;  damp_w_pert = d2_divg_pert
                   d_con_k = 0.
              endif
...
! Apply first order scheme for damping the sponge layer
                   ...
                   hord_t_pert = 2
                   hord_p_pert = 2
                   if (.not.flagstruct%split_hord) then
                      hord_v = hord_v_pert
                      hord_t = hord_t_pert
                      hord_p = hord_p_pert
                   endif
...
! Correction factor for 3D Coriolis force
       ...
       call d_sw(vt(isd:ied,jsd:jed,k), delp(isd:ied,jsd:jed,k), ptc(isd:ied,jsd:jed,k),  pt(isd:ied,jsd:jed,k),      &
                  ...
                  flagstruct%split_damp, nord_k_pert, nord_v_pert(k), nord_w_pert, nord_t_pert, &
                  flagstruct%dddmp_pert, d2_divg_pert, flagstruct%d4_bg_pert, damp_vt_pert(k), damp_w_pert, &
                  damp_t_pert)
...
do k=1,npz+1
   if ( k/=1 ) then
      call a2b_ord4(pp(isd:ied,jsd:jed,k), wk1, gridstruct, npx, npy, is, ie, js, je, ng, .true.)
      call a2b_ord4(pk(isd:ied,jsd:jed,k), wk1, gridstruct, npx, npy, is, ie, js, je, ng, .true.)
   endif
   call a2b_ord4( gz(isd:ied,jsd:jed,k), wk1, gridstruct, npx, npy, is, ie, js, je, ng, .true.)
enddo
...
do k=1,npz
   call a2b_ord4(delp(isd:ied,jsd:jed,k), wk1, gridstruct, npx, npy, is, ie, js, je, ng)
   do j=js,je+1
      do i=is,ie+1
...
do k=2,npz+1
   if ( a2b_ord==4 ) then
      call a2b_ord4(pk(isd:ied,jsd:jed,k), wk, gridstruct, npx, npy, is, ie, js, je, ng, .true.)
   else
...
do k=1,npz+1
   if ( a2b_ord==4 ) then
      call a2b_ord4( gz(isd:ied,jsd:jed,k), wk, gridstruct, npx, npy, is, ie, js, je, ng, .true.)
   else
...
do k=1,npz
   ...
   if ( hydrostatic ) then
      ...
      if ( a2b_ord==4 ) then
         call a2b_ord4(delp(isd:ied,jsd:jed,k), wk, gridstruct, npx, npy, is, ie, js, je, ng)
      else
...
! Re-FACTOR rf
                  ...
                  kmax = k
                  rff(k) = 1.0_R_GRID / (1.0_R_GRID+rff(k))
                   rf(k) = rff(k)
             else
                  exit
             endif
