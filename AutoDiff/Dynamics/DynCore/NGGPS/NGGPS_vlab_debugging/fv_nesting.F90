...
!local pointers
    logical, pointer :: child_grids(:)
      ...
      logical :: inline_q, make_nh, nested
      integer, pointer :: nest_timestep, tracer_nest_timestep

      inline_q = flagstruct%inline_q
      make_nh = flagstruct%make_nh
      nested = neststruct%nested
      nest_timestep => neststruct%nest_timestep
      tracer_nest_timestep => neststruct%tracer_nest_timestep
...
!!! update_domains call takes care of the problem.
   ...
   if (neststruct%parent_proc) then
     call mpp_update_domains(parent_grid%ps, parent_grid%domain, complete=.true.)
