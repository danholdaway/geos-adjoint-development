...
!
  ...
  PUBLIC fv_tp_2d_fwd, fv_tp_2d_bwd, pert_ppm_adm, copy_corners_fwd, &
& copy_corners_bwd
...
! 0.58333333
  ...
  EXTERNAL FV_TP_2D_ADM
  EXTERNAL COPY_CORNERS_ADM

CONTAINS
!  Differentiation of fv_tp_2d in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod
...
!EOP
!-----------------------------------------------------------------------
  SUBROUTINE FV_TP_2D_FWD(q, crx, cry, npx, npy, hord, fx, fy, xfx, yfx&
...
&                     gridstruct%nw_corner, gridstruct%ne_corner)
    ...
    END IF
    CALL YPPM_FWD(fy2, q, cry, ord_in, isd, ied, isd, ied, js, je, jsd, &
&           jed, npx, npy, gridstruct%dya, gridstruct%nested, gridstruct&
&           %grid_type)
    DO j=js,je+1
      DO i=isd,ied
...
&         )/ra_y(i, j)
      END DO
    END DO
    CALL XPPM_FWD(fx, q_i, crx(is:ie+1, js:je), ord_ou, is, ie, isd, ied&
&           , js, je, jsd, jed, npx, npy, gridstruct%dxa, gridstruct%&
&           nested, gridstruct%grid_type)
    IF (.NOT.gridstruct%nested) THEN
...
&                     gridstruct%nw_corner, gridstruct%ne_corner)
    ...
    ELSE
      CALL PUSHCONTROL1B(1)
    END IF
    CALL XPPM_FWD(fx2, q, crx, ord_in, is, ie, isd, ied, jsd, jed, jsd, &
&           jed, npx, npy, gridstruct%dxa, gridstruct%nested, gridstruct&
&           %grid_type)
    DO j=jsd,jed
      DO i=is,ie+1
...
!---------------------------------
      ...
      IF (PRESENT(nord) .AND. PRESENT(damp_c) .AND. PRESENT(mass)) THEN
        IF (damp_c .GT. 1.e-4) THEN
          ...
          CALL DELN_FLUX_FWD(nord, is, ie, js, je, npx, npy, damp, q, fx&
&                      , fy, gridstruct, bd, mass)
          CALL PUSHREAL8ARRAY(fy2, (bd%ied-bd%isd+1)*(bd%je-bd%js+2))
          CALL PUSHREAL8ARRAY(q_j, (bd%ie-bd%is+1)*(bd%jed-bd%jsd+1))
          CALL PUSHREAL8ARRAY(fx1, bd%ie - bd%is + 2)
          CALL PUSHREAL8ARRAY(fx2, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1))
          CALL PUSHREAL8ARRAY(fyy, (bd%ied-bd%isd+1)*(bd%je-bd%js+2))
...
!  Differentiation of fv_tp_2d in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
...
&   mass_ad, nord, damp_c)
    ...
    IF (branch .LT. 3) THEN
      IF (branch .EQ. 0) THEN
        CALL POPREAL8ARRAY(fyy, (bd%ied-bd%isd+1)*(bd%je-bd%js+2))
        CALL POPREAL8ARRAY(fx2, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1))
        CALL POPREAL8ARRAY(fx1, bd%ie - bd%is + 2)
        CALL POPINTEGER4(je)
        CALL POPINTEGER4(is)
        CALL POPINTEGER4(ie)
        CALL POPREAL8ARRAY(q_j, (bd%ie-bd%is+1)*(bd%jed-bd%jsd+1))
        CALL POPREAL8ARRAY(fy2, (bd%ied-bd%isd+1)*(bd%je-bd%js+2))
        CALL POPINTEGER4(js)
      ...
      ELSE
        ...
        is = bd%is
        je = bd%je
        CALL DELN_FLUX_BWD(nord, is, ie, js, je, npx, npy, damp, q, q_ad&
&                    , fx, fx_ad, fy, fy_ad, gridstruct, bd, mass, &
&                    mass_ad)
      END IF
      fy2_ad = 0.0_8
      DO j=je+1,js,-1
...
&           , j)
    ...
    ELSE
      ...
      ELSE
        ...
        je = bd%je
        CALL DELN_FLUX_BWD(nord, is, ie, js, je, npx, npy, damp, q, q_ad&
&                    , fx, fx_ad, fy, fy_ad, gridstruct, bd)
      END IF
      fy2_ad = 0.0_8
      DO j=je+1,js,-1
        DO i=ie,is,-1
...
&         fx1(i)-fx1(i+1))*temp_ad0/ra_x(i, j)
      ...
      END DO
    END DO
    CALL XPPM_BWD(fx2, fx2_ad, q, q_ad, crx, crx_ad, ord_in, is, ie, isd&
...
&                                      gridstruct%ne_corner)
    q_i_ad = 0.0_8
    CALL XPPM_BWD(fx, fx_ad, q_i, q_i_ad, crx(is:ie+1, js:je), crx_ad(is&
...
!  Differentiation of xppm in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a2b
...
!   with respect to varying inputs: q flux c
  SUBROUTINE XPPM_FWD(flux, q, c, iord, is, ie, isd, ied, jfirst, jlast&
&   , jsd, jed, npx, npy, dxa, nested, grid_type)
    IMPLICIT NONE
...
&             -dxa(1, j)*q1(2))/(dxa(1, j)+dxa(2, j)))
          ...
          END IF
          IF (ie + 1 .EQ. npx) THEN
            CALL PUSHREAL8(al(npx-1))
            al(npx-1) = c1*q1(npx-3) + c2*q1(npx-2) + c3*q1(npx-1)
            CALL PUSHREAL8(al(npx))
...
! Positive definite constraint:
        IF (iord .EQ. 9 .OR. iord .EQ. 13) THEN
          arg1 = ie1 - is1 + 1
          CALL PUSHREAL8ARRAY(br(is1), ie - is1 + 2)
          CALL PUSHREAL8ARRAY(bl(is1), ie - is1 + 2)
...
!        endif
            ...
            CALL PUSHREAL8(br(2))
            br(2) = al(3) - q1(2)
            CALL PUSHREAL8ARRAY(br(0), ie + 2)
            CALL PUSHREAL8ARRAY(bl(0), ie + 2)
...
&             )))
      ...
      END IF
    END DO
    CALL PUSHREAL8ARRAY(al, ie - is + 4)
    CALL PUSHREAL8(abs0)
    CALL PUSHINTEGER4(is1)
    CALL PUSHREAL8(xt)
    CALL PUSHREAL8ARRAY(bl, ie - is + 3)
    CALL PUSHREAL8(qtmp)
    CALL PUSHREAL8ARRAY(q1, ied - isd + 1)
    ...
    CALL PUSHREAL8(min6)
    CALL PUSHREAL8(min7)
    CALL PUSHREAL8ARRAY(fx1, ie - is + 2)
  END SUBROUTINE XPPM_FWD
!  Differentiation of xppm in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a2
...
!   gradient     of useful results: q flux c
!   with respect to varying inputs: q flux c
  SUBROUTINE XPPM_BWD(flux, flux_ad, q, q_ad, c, c_ad, iord, is, ie, isd&
&   , ied, jfirst, jlast, jsd, jed, npx, npy, dxa, nested, grid_type)
    ...
    REAL :: y2
    REAL :: y1
    CALL POPREAL8ARRAY(fx1, ie - is + 2)
    CALL POPREAL8(min7)
    CALL POPREAL8(min6)
    ...
    CALL POPREAL8ARRAY(q1, ied - isd + 1)
    CALL POPREAL8(qtmp)
    CALL POPREAL8ARRAY(bl, ie - is + 3)
    CALL POPREAL8(xt)
    CALL POPINTEGER4(is1)
    CALL POPREAL8(abs0)
    CALL POPREAL8ARRAY(al, ie - is + 4)
    dm_ad = 0.0_8
    dq_ad = 0.0_8
    al_ad = 0.0_8
    bl_ad = 0.0_8
        ...
        DO i=ie+1,is,-1
          ...
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
              ...
              c_ad(i, j) = c_ad(i, j) - b0(i-1)*temp_ad13 - (br(i-1)-c(i&
&               , j)*b0(i-1))*fx1_ad(i)
          ...
          ELSE
            IF (branch .EQ. 2) THEN
              ...
              c_ad(i, j) = c_ad(i, j) + b0(i)*temp_ad14 + (bl(i)+c(i, j)&
&               *b0(i))*fx1_ad(i)
...
&             temp_ad20
          ...
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8ARRAY(bl(0), ie + 2)
            CALL POPREAL8ARRAY(br(0), ie + 2)
...
&                       (0:), 1)
        ...
        IF (branch .EQ. 0) THEN
          arg1 = ie1 - is1 + 1
          CALL POPREAL8ARRAY(bl(is1), ie - is1 + 2)
          CALL POPREAL8ARRAY(br(is1), ie - is1 + 2)
...
&             )))
    ...
    END DO
  END SUBROUTINE XPPM
!  Differentiation of yppm in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a2b
!_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p_
!grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_dp 
!dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_Super
! fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid_u
...
!   gradient     of useful results: q flux c
!   with respect to varying inputs: q flux c
  SUBROUTINE YPPM_FWD(flux, q, c, jord, ifirst, ilast, isd, ied, js, je&
&   , jsd, jed, npx, npy, dya, nested, grid_type)
    IMPLICIT NONE
! Compute domain
...
&               (i, j+1)-(qtmp+qtmp)))
          ...
          END DO
        END DO
        CALL PUSHREAL8ARRAY(al, (ilast-ifirst+1)*(je-js+4))
...
! both up-downwind sides are noisy; 2nd order, piece-wise linear
        ...
        END DO
        CALL PUSHREAL8(abs0)
        CALL PUSHREAL8ARRAY(bl, (ilast-ifirst+1)*(je-js+3))
        CALL PUSHINTEGER4(je3)
        CALL PUSHREAL8ARRAY(br, (ilast-ifirst+1)*(je-js+3))
        CALL PUSHINTEGER4(js1)
        CALL PUSHREAL8(min1)
        CALL PUSHREAL8(min2)
        CALL PUSHREAL8ARRAY(b0, (ilast-ifirst+1)*(je-js+3))
        CALL PUSHREAL8ARRAY(fx1, ilast - ifirst + 1)
        CALL PUSHCONTROL3B(2)
...
! Positive definite constraint:
        DO j=js1,je1
          arg1 = ilast - ifirst + 1
          CALL PUSHREAL8ARRAY(br(ifirst, j), ilast - ifirst + 1)
          CALL PUSHREAL8ARRAY(bl(ifirst, j), ilast - ifirst + 1)
...
!        endif
          ...
          CALL PUSHREAL8ARRAY(br(ifirst, 0), ilast - ifirst + 1)
          CALL PUSHREAL8ARRAY(bl(ifirst, 0), ilast - ifirst + 1)
...
!        endif
          ...
          END DO
          arg1 = 3*(ilast-ifirst+1)
          CALL PUSHREAL8ARRAY(br(ifirst, npy-2), ilast - ifirst + 1)
          CALL PUSHREAL8ARRAY(bl(ifirst, npy-2), ilast - ifirst + 1)
...
!  Differentiation of yppm in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a2
!b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.p
!_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_dp
! dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_Supe
!r fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid_
...
&   ilast, isd, ied, js, je, jsd, jed, npx, npy, dya, nested, grid_type)
    ...
    IF (branch .LT. 3) THEN
      IF (branch .EQ. 0) THEN
        ...
        CALL POPREAL8ARRAY(al, (ilast-ifirst+1)*(je-js+4))
        al_ad = 0.0_8
        DO j=je+1,js,-1
          DO i=ilast,ifirst,-1
            CALL POPCONTROL1B(branch)
...
&               (i, j)-q(i, j-1)))*flux_ad(i, j)
        ...
        al_ad = 0.0_8
      ELSE
        CALL POPREAL8ARRAY(fx1, ilast - ifirst + 1)
        CALL POPREAL8ARRAY(b0, (ilast-ifirst+1)*(je-js+3))
        CALL POPREAL8(min2)
        CALL POPREAL8(min1)
        CALL POPINTEGER4(js1)
        CALL POPREAL8ARRAY(br, (ilast-ifirst+1)*(je-js+3))
        CALL POPINTEGER4(je3)
        CALL POPREAL8ARRAY(bl, (ilast-ifirst+1)*(je-js+3))
        CALL POPREAL8(abs0)
        bl_ad = 0.0_8
        br_ad = 0.0_8
        b0_ad = 0.0_8
      ...
      DO j=je+1,js,-1
        DO i=ilast,ifirst,-1
          ...
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
              temp_ad13 = (1.-c(i, j))*fx1_ad(i)
              c_ad(i, j) = c_ad(i, j) - b0(i, j-1)*temp_ad13 - (br(i, j-&
&               1)-c(i, j)*b0(i, j-1))*fx1_ad(i)
          ...
          ELSE
            IF (branch .EQ. 2) THEN
              temp_ad14 = (c(i, j)+1.)*fx1_ad(i)
              c_ad(i, j) = c_ad(i, j) + b0(i, j)*temp_ad14 + (bl(i, j)+c&
&               (i, j)*b0(i, j))*fx1_ad(i)
      ...
      DO j=je+1,js,-1
        DO i=ilast,ifirst,-1
          ...
          IF (branch .EQ. 0) THEN
            ...
            flux_ad(i, j) = 0.0_8
            temp_ad15 = (1.-c(i, j))*fx1_ad(i)
            c_ad(i, j) = c_ad(i, j) - b0(i, j-1)*temp_ad15 - (br(i, j-1)&
&             -c(i, j)*b0(i, j-1))*fx1_ad(i)
          ...
          ELSE
            ...
            flux_ad(i, j) = 0.0_8
            temp_ad16 = (c(i, j)+1.)*fx1_ad(i)
            c_ad(i, j) = c_ad(i, j) + b0(i, j)*temp_ad16 + (bl(i, j)+c(i&
&             , j)*b0(i, j))*fx1_ad(i)
...
&             , j)*temp1)*flux_ad(i, j)
      ...
      ELSE
        ...
        ELSE
          arg1 = 3*(ilast-ifirst+1)
          CALL POPREAL8ARRAY(bl(ifirst, npy-2), ilast - ifirst + 1)
          CALL POPREAL8ARRAY(br(ifirst, npy-2), ilast - ifirst + 1)
...
&             )
        ...
        IF (branch .EQ. 0) THEN
          arg1 = 3*(ilast-ifirst+1)
          CALL POPREAL8ARRAY(bl(ifirst, 0), ilast - ifirst + 1)
          CALL POPREAL8ARRAY(br(ifirst, 0), ilast - ifirst + 1)
...
&             temp_ad17
      ...
      IF (branch .EQ. 0) THEN
        DO j=je1,js1,-1
          arg1 = ilast - ifirst + 1
          CALL POPREAL8ARRAY(bl(ifirst, j), ilast - ifirst + 1)
          CALL POPREAL8ARRAY(br(ifirst, j), ilast - ifirst + 1)
 ...
 100        al_ad(i, j+1) = al_ad(i, j+1) + br_ad(i, j)
    ...
    IF (branch .LT. 2) THEN
      IF (branch .EQ. 0) THEN
        DO i=ilast,ifirst,-1
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) al_ad(i, npy+1) = 0.0_8
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) al_ad(i, npy) = 0.0_8
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) al_ad(i, npy-1) = 0.0_8
        END DO
      END IF
...
&         temp_ad2
    ...
    IF (branch .EQ. 0) THEN
      DO i=ilast,ifirst,-1
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) al_ad(i, 2) = 0.0_8
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) al_ad(i, 1) = 0.0_8
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) al_ad(i, 0) = 0.0_8
      END DO
...
!  Differentiation of pert_ppm in reverse (adjoint) mode (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a2b_ord2 dyn_c
...
!_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer fv_mapz_mod.map
!1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_mapz_mod.ppm_profile fv_m
!apz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm fv_mapz_mod.moist_cv fv_mapz_mod.moist_cp fv_map
!z_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.tracer_2d_neste
!d nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_utils_mod.Riem_Solver_c nh_utils_mod.Riem_Solver
!3test nh_utils_mod.imp_diff_w nh_utils_mod.RIM_2D nh_utils_mod.SIM3_solver nh_utils_mod.SIM3p0_solver nh_utils_mod.SIM1_solver n
!h_utils_mod.SIM_solver nh_utils_mod.edge_scalar nh_utils_mod.edge_profile nh_utils_mod.nest_halo_nh sw_core_mod.c_sw sw_core_mod
!.d_sw  sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.fill3_4corners sw_cor
!e_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.compute_divergence_damping sw_cor
!e_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mod.copy_corners tp_core_mod.xpp
!m tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_dist sw_core_mod.edge_interpo
!late4)):
...
!  Differentiation of deln_flux in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
!d.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_m
!od.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mi
!x_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_
!Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_g
...
!   with respect to varying inputs: q mass fx fy
  SUBROUTINE DELN_FLUX_FWD(nord, is, ie, js, je, npx, npy, damp, q, fx, &
&   fy, gridstruct, bd, mass)
    IMPLICIT NONE
...
&           )
      ...
      CALL PUSHREAL8ARRAY(fy2, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2))
      CALL PUSHREAL8ARRAY(fx2, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1))
...
!  Differentiation of deln_flux in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_m
...
!   gradient     of useful results: q mass fx fy
!   with respect to varying inputs: q mass fx fy
  SUBROUTINE DELN_FLUX_BWD(nord, is, ie, js, je, npx, npy, damp, q, q_ad&
...
&   , bd%jsd:bd%jed+1)
    ...
    IF (branch .EQ. 0) THEN
      CALL POPREAL8(damp2)
      CALL POPREAL8ARRAY(fx2, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1))
      CALL POPREAL8ARRAY(fy2, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2))
      CALL POPINTEGER4(i1)
      fy2_ad = 0.0_8
      DO j=je+1,js,-1
        DO i=ie,is,-1
    ...
    IF (branch .EQ. 0) THEN
      ...
      DO n=nord,1,-1
        ...
        DO j=ad_to4,ad_from4,-1
          ...
          END DO
        END DO
        CALL COPY_CORNERS_BWD(d2, d2_ad, npx, npy, 2, gridstruct%nested&
...
&                       , gridstruct%nw_corner, gridstruct%ne_corner)
        ...
        DO j=ad_to2,ad_from2,-1
          ...
          END DO
        END DO
        CALL COPY_CORNERS_BWD(d2, d2_ad, npx, npy, 1, gridstruct%nested&
&                       , bd, gridstruct%sw_corner, gridstruct%se_corner&
&                       , gridstruct%nw_corner, gridstruct%ne_corner)
...
!  Differentiation of copy_corners in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
...
!  Differentiation of copy_corners in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edg
