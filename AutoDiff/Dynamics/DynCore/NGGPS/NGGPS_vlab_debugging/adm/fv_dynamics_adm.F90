...
MODULE FV_DYNAMICS_MOD_B
  USE CONSTANTS_MOD, ONLY : grav, pi => pi_8, radius, hlv, rdgas, omega,&
& rvgas, cp_vapor
  USE DYN_CORE_MOD_B, ONLY : dyn_core, dyn_core_adm, del2_cubed, &
& del2_cubed_adm, init_ijk_mem
  USE FV_MAPZ_MOD_B, ONLY : compute_total_energy, &
& compute_total_energy_adm, lagrangian_to_eulerian, &
& lagrangian_to_eulerian_adm, moist_cv, moist_cp
  USE FV_TRACER2D_MOD_B, ONLY : tracer_2d, tracer_2d_adm, tracer_2d_1l, &
& tracer_2d_1l_adm, tracer_2d_nested, tracer_2d_nested_adm
  USE FV_GRID_UTILS_MOD_B, ONLY : cubed_to_latlon, c2l_ord2, &
& c2l_ord2_adm, g_sum, g_sum_adm
  USE FV_FILL_MOD_B, ONLY : fill2d
...
& start_group_halo_update_adm, complete_group_halo_update
  ...
  USE FV_DIAGNOSTICS_MOD_B, ONLY : fv_time, prt_mxm, range_check, &
& prt_minmax
...
& mpp_update_domains_adm, domain2d
  ...
  USE FV_NESTING_MOD_B, ONLY : setup_nested_grid_bcs
...
& , fv_nest_type, fv_diag_type, fv_grid_bounds_type
  USE FV_NWP_NUDGE_MOD_B, ONLY : do_adiabatic_init
...
CONTAINS
!  Differentiation of fv_dynamics in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
...
&   hybrid_z, gridstruct, flagstruct, neststruct, idiag, bd, parent_grid&
&   , domain, time_total)
    IMPLICIT NONE
!t2 = MPI_Wtime(status)
...
!$OMP parallel do default(none) shared(theta_d,is,ie,js,je,npz,pt,pkz,q)
        DO k=1,npz
          ...
          IF (theta_d .GT. 0) THEN
            DO j=js,je
              DO i=is,ie
                CALL PUSHREAL8(q(i, j, k, theta_d))
...
&                 ak, bk, ks, gridstruct, flagstruct, neststruct, idiag&
&                 , bd, domain, arg10, i_pack, last_step, gz, pkc, ptc, &
...
&                             parent_grid, flagstruct%hord_tr_pert, &
&                             flagstruct%nord_tr_pert, flagstruct%&
&                             trdm2_pert, flagstruct%split_damp_tr)
          CALL PUSHCONTROL2B(0)
...
&                         flagstruct%hord_tr_pert, flagstruct%&
&                         nord_tr_pert, flagstruct%trdm2_pert, &
&                         flagstruct%split_damp_tr)
          CALL PUSHCONTROL2B(1)
        ELSE
...
&                      flagstruct%nord_tr, flagstruct%trdm2, flagstruct%&
&                      hord_tr_pert, flagstruct%nord_tr_pert, flagstruct&
&                      %trdm2_pert, flagstruct%split_damp_tr)
          CALL PUSHCONTROL2B(2)
        END IF
      ELSE
...
!Convert back to potential temperature
    IF (hydrostatic .AND. (.NOT.idealtest)) THEN
      ...
      CALL PUSHREAL8ARRAY(pk3, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+&
&                   1))
      CALL PUSHINTEGER4(js)
      CALL PUSHREAL8ARRAY(xfx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHREAL8ARRAY(ws, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL PUSHREAL8ARRAY(gz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+1&
&                   ))
      CALL PUSHREAL8ARRAY(dp1, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHINTEGER4(nq)
      CALL PUSHREAL8ARRAY(ptc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHREAL8ARRAY(pfull, npz)
      CALL PUSHINTEGER4(jsd)
      CALL PUSHREAL8ARRAY(rf, npz)
      CALL PUSHINTEGER4(ied)
      CALL PUSHREAL8ARRAY(ut, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHREAL8(dt2)
      CALL PUSHREAL8ARRAY(pkc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+&
&                   1))
      CALL PUSHREAL8ARRAY(delpc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz&
&                  )
      CALL PUSHINTEGER4(ie)
      CALL PUSHREAL8(rdg)
      CALL PUSHREAL8ARRAY(m_fac, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL PUSHREAL8ARRAY(yfx, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
      CALL PUSHINTEGER4(isd)
      CALL PUSHINTEGER4(is)
      CALL PUSHINTEGER4(k_split)
      CALL PUSHREAL8ARRAY(te_2d, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL PUSHREAL8(akap)
      CALL PUSHREAL8ARRAY(vt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHREAL8(result1)
      CALL PUSHINTEGER4(je)
      CALL PUSHINTEGER4(sphum)
      CALL PUSHREAL8(amdt)
      CALL PUSHREAL8ARRAY(crx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
      CALL PUSHREAL8ARRAY(cry, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
...
!  Differentiation of fv_dynamics in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
...
&   cx_ad, cy, cy_ad, hybrid_z, gridstruct, flagstruct, neststruct, &
...
&   kord_tm_pert
    ...
    IF (branch .EQ. 0) THEN
      ...
      CALL POPREAL8ARRAY(cry, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
      CALL POPREAL8ARRAY(crx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
      CALL POPREAL8(amdt)
      CALL POPINTEGER4(sphum)
      CALL POPINTEGER4(je)
      CALL POPREAL8(result1)
      CALL POPREAL8ARRAY(vt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL POPREAL8(akap)
      CALL POPREAL8ARRAY(te_2d, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL POPINTEGER4(k_split)
      CALL POPINTEGER4(is)
      CALL POPINTEGER4(isd)
      CALL POPREAL8ARRAY(yfx, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
      CALL POPREAL8ARRAY(m_fac, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL POPREAL8(rdg)
      CALL POPINTEGER4(ie)
      CALL POPREAL8ARRAY(delpc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL POPREAL8ARRAY(pkc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+1&
&                  ))
      CALL POPREAL8(dt2)
      CALL POPREAL8ARRAY(ut, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL POPINTEGER4(ied)
      CALL POPREAL8ARRAY(rf, npz)
      CALL POPINTEGER4(jsd)
      CALL POPREAL8ARRAY(pfull, npz)
      CALL POPREAL8ARRAY(ptc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL POPINTEGER4(nq)
      CALL POPREAL8ARRAY(dp1, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL POPREAL8ARRAY(gz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+1)&
&                 )
      CALL POPREAL8ARRAY(ws, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
      CALL POPREAL8ARRAY(xfx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
      CALL POPINTEGER4(js)
      CALL POPREAL8ARRAY(pk3, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+1&
&                  ))
      pkz_ad = 0.0_8
    ELSE
      CALL POPINTEGER4(jed)
...
&                    %nord_tr, flagstruct%trdm2, flagstruct%hord_tr_pert&
&                    , flagstruct%nord_tr_pert, flagstruct%trdm2_pert, &
&                    flagstruct%split_damp_tr)
      ELSE
        mfx_ad = 0.0_8
        mfy_ad = 0.0_8
...
&                 domain, arg10, i_pack, last_step, gz, gz_ad, pkc, &
    ...
    IF (branch .EQ. 0) THEN
      DO k=npz,1,-1
        ...
        IF (branch .NE. 0) THEN
          DO j=je,js,-1
            DO i=ie,is,-1
              CALL POPREAL8(q(i, j, k, theta_d))
              pt_ad(i, j, k) = pt_ad(i, j, k) + q_ad(i, j, k, theta_d)
              q_ad(i, j, k, theta_d) = 0.0_8
            END DO
          END DO
        END IF
        DO j=je,js,-1
...
&             arg10, i_pack, last_step, gz, pkc, ptc, crx, xfx, cry, yfx&
&             , divgd, delpc, ut, vt, zh, pk3, du, dv, time_total)
...
&                         hord_tr, q_split, mdt, idiag%id_divg, i_pack(&
&                         10), flagstruct%nord_tr, flagstruct%trdm2, &
&                         k_split, neststruct, parent_grid, flagstruct%&
&                         hord_tr_pert, flagstruct%nord_tr_pert, &
&                         flagstruct%trdm2_pert, flagstruct%&
&                         split_damp_tr)
        ELSE IF (flagstruct%z_tracer) THEN
...
!  Differentiation of rayleigh_super in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_ed
...
!if( is_master() ) write(6,*) k, 0.01*pm(k), dt/(rf(k)*sday)
 ...
 100  CALL PUSHCONTROL1B(1)
    ...
    ELSE
      CALL PUSHCONTROL1B(1)
    END IF
 110 CALL C2L_ORD2_FWD(u, v, ua, va, gridstruct, npz, gridstruct%&
...
&                 , k)**2+w(i, j, k)**2)*(1.-u2f(i, j, k)**2)*rcv
    ...
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(je)
    CALL PUSHREAL8ARRAY(u2f, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
  END SUBROUTINE RAYLEIGH_SUPER_FWD
!  Differentiation of rayleigh_super in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_e
...
!deallocate ( u2f )
    ...
    INTEGER :: i0
    INTEGER :: branch
    CALL POPREAL8ARRAY(u2f, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
    CALL POPINTEGER4(je)
    CALL POPINTEGER4(is)
    CALL POPINTEGER4(ie)
    CALL POPREAL8(rcv)
...
!  Differentiation of rayleigh_friction in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b
...
!  Differentiation of rayleigh_friction in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2
...
!  Differentiation of compute_aam in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
!$OMP                          private(r1, r2, dm)
    DO j=js,je
      ...
      END DO
    END DO
    CALL PUSHREAL8ARRAY(dm, ie - is + 1)
    CALL PUSHREAL8ARRAY(r1, ie - is + 1)
    CALL PUSHREAL8ARRAY(r2, ie - is + 1)
  END SUBROUTINE COMPUTE_AAM_FWD
!  Differentiation of compute_aam in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
...
&   va_ad
    ...
    INTEGER :: i, j, k
    INTRINSIC COS
    CALL POPREAL8ARRAY(r2, ie - is + 1)
    CALL POPREAL8ARRAY(r1, ie - is + 1)
    CALL POPREAL8ARRAY(dm, ie - is + 1)
    dm_ad = 0.0_8
    DO j=je,js,-1
      DO k=npz,1,-1
        DO i=ie,is,-1
...
!  Differentiation of geos_to_fv3 in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
&           , j)-peln(i, k, j)))
      ...
      END DO
    END DO
    CALL PUSHREAL8ARRAY(pt(is:ie, js:je, :), (ie-is+1)*(je-js+1)*npz)
    pt(is:ie, js:je, :) = pt(is:ie, js:je, :)*pkz(is:ie, js:je, :)
  END SUBROUTINE GEOS_TO_FV3_FWD
!  Differentiation of geos_to_fv3 in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
...
&   )
    ...
    is = bd%is
    je = bd%je
    CALL POPREAL8ARRAY(pt(is:ie, js:je, :), (ie-is+1)*(je-js+1)*npz)
...
!  Differentiation of fv3_to_geos in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
!  Differentiation of fv3_to_geos in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
