...
& del6_vt_flux, del6_vt_flux_adm
  USE FV_ARRAYS_MOD_B, ONLY : fv_grid_bounds_type, fv_grid_type
  IMPLICIT NONE
  PRIVATE 
  PUBLIC update_dz_c, update_dz_d, nest_halo_nh
  PUBLIC update_dz_c_fwd, update_dz_c_bwd, update_dz_d_fwd, &
& update_dz_d_bwd, nest_halo_nh_fwd, nest_halo_nh_bwd
  PUBLIC sim_solver, sim1_solver, sim3_solver
  PUBLIC sim_solver_fwd, sim_solver_bwd, sim1_solver_fwd, &
& sim1_solver_bwd, sim3_solver_fwd, sim3_solver_bwd
  PUBLIC sim3p0_solver, rim_2d
  PUBLIC sim3p0_solver_fwd, sim3p0_solver_bwd, rim_2d_fwd, rim_2d_bwd
  PUBLIC riem_solver_c
  PUBLIC riem_solver_c_fwd, riem_solver_c_bwd
  REAL, PARAMETER :: dz_min=2.
  REAL, PARAMETER :: r3=1./3.
  EXTERNAL SIM3_SOLVER_ADM
  EXTERNAL SIM3P0_SOLVER_ADM
  EXTERNAL SIM_SOLVER_ADM
  EXTERNAL NEST_HALO_NH_ADM
  EXTERNAL SIM1_SOLVER_ADM
...
CONTAINS
!  Differentiation of update_dz_c in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
...
!$OMP parallel do default(none) shared(is1,ie1,js1,je1,ws,zs,gz,rdt,km)
    DO j=js1,je1
      ...
      END DO
    END DO
    CALL PUSHREAL8ARRAY(xfx, (ie-is+4)*(je-js+3))
    CALL PUSHREAL8ARRAY(gz2, (ie+2*ng-is+1)*(je+2*ng-js+1))
    CALL PUSHINTEGER4(is1)
    CALL PUSHREAL8(bot_ratio)
    CALL PUSHREAL8(top_ratio)
    CALL PUSHREAL8(int_ratio)
    CALL PUSHINTEGER4(je1)
    CALL PUSHINTEGER4(je2)
    CALL PUSHREAL8ARRAY(yfx, (ie-is+3)*(je-js+4))
    CALL PUSHREAL8(rdt)
    CALL PUSHINTEGER4(js1)
    CALL PUSHREAL8ARRAY(fx, (ie-is+4)*(je-js+3))
    CALL PUSHREAL8ARRAY(fy, (ie-is+3)*(je-js+4))
    CALL PUSHINTEGER4(ie1)
    CALL PUSHINTEGER4(ie2)
  END SUBROUTINE UPDATE_DZ_C_FWD
!  Differentiation of update_dz_c in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
...
&   se_corner, ne_corner, nw_corner, bd, grid_type)
    ...
    REAL :: temp_ad0
    INTEGER :: branch
    CALL POPINTEGER4(ie2)
    CALL POPINTEGER4(ie1)
    CALL POPREAL8ARRAY(fy, (ie-is+3)*(je-js+4))
    CALL POPREAL8ARRAY(fx, (ie-is+4)*(je-js+3))
    CALL POPINTEGER4(js1)
    CALL POPREAL8(rdt)
    CALL POPREAL8ARRAY(yfx, (ie-is+3)*(je-js+4))
    CALL POPINTEGER4(je2)
    CALL POPINTEGER4(je1)
    CALL POPREAL8(int_ratio)
    CALL POPREAL8(top_ratio)
    CALL POPREAL8(bot_ratio)
    CALL POPINTEGER4(is1)
    CALL POPREAL8ARRAY(gz2, (ie+2*ng-is+1)*(je+2*ng-js+1))
    CALL POPREAL8ARRAY(xfx, (ie-is+4)*(je-js+3))
    DO j=je1,js1,-1
      DO k=1,km,1
        DO i=ie1,is1,-1
...
!  Differentiation of update_dz_d in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
!--------------------------------------------------------------------
    ...
    INTEGER :: arg1
    uniform_grid = .false.
    CALL PUSHREAL8(damp(km+1))
    damp(km+1) = damp(km)
    CALL PUSHINTEGER4(ndif(km+1))
    ndif(km+1) = ndif(km)
    isd = is - ng
    ied = ie + ng
    jsd = js - ng
...
&                     xfx_adv(is:ie+1, jsd:jed, k), yfx_adv(isd:ied, js:&
&                     je+1, k), gridstruct, bd, ra_x, ra_y)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL FV_TP_2D_FWD(z2, crx_adv(is:ie+1, jsd:jed, k), cry_adv(&
&                     isd:ied, js:je+1, k), npx, npy, hord_pert, fx, fy&
&                     , xfx_adv(is:ie+1, jsd:jed, k), yfx_adv(isd:ied, &
&                     js:je+1, k), gridstruct, bd, ra_x, ra_y)
          CALL PUSHCONTROL1B(1)
        END IF
        CALL DEL6_VT_FLUX_FWD(ndif(k), npx, npy, damp(k), z2, wk2, fx2, &
&                       fy2, gridstruct, bd)
        DO j=js,je
          DO i=is,ie
...
&                     , ra_y)
        ...
        ELSE
          CALL FV_TP_2D_FWD(zh(isd:ied, jsd:jed, k), crx_adv(is:ie+1, &
&                     jsd:jed, k), cry_adv(isd:ied, js:je+1, k), npx, &
&                     npy, hord_pert, fx, fy, xfx_adv(is:ie+1, jsd:jed, &
&                     k), yfx_adv(isd:ied, js:je+1, k), gridstruct, bd, &
&                     ra_x, ra_y)
          CALL PUSHCONTROL1B(0)
        END IF
        DO j=js,je
          DO i=is,ie
            CALL PUSHREAL8(zh(i, j, k))
...
!$OMP parallel do default(none) shared(is,ie,js,je,km,ws,zs,zh,rdt)
    DO j=js,je
      ...
      END DO
    END DO
    CALL PUSHREAL8ARRAY(yfx_adv, (ie+2*ng-is+1)*(je-js+2)*(km+1))
    CALL PUSHREAL8ARRAY(crx_adv, (ie-is+2)*(je+2*ng-js+1)*(km+1))
    CALL PUSHREAL8ARRAY(xfx_adv, (ie-is+2)*(je+2*ng-js+1)*(km+1))
    CALL PUSHINTEGER4(jsd)
    CALL PUSHREAL8ARRAY(z2, (ie+2*ng-is+1)*(je+2*ng-js+1))
    CALL PUSHINTEGER4(ied)
    CALL PUSHREAL8ARRAY(cry_adv, (ie+2*ng-is+1)*(je-js+2)*(km+1))
    CALL PUSHREAL8ARRAY(ra_x, (ie-is+1)*(je+2*ng-js+1))
    CALL PUSHREAL8ARRAY(ra_y, (ie+2*ng-is+1)*(je-js+1))
...
!  Differentiation of update_dz_d in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
...
&   yfx_adv_ad
    ...
    CALL POPREAL8ARRAY(ra_y, (ie+2*ng-is+1)*(je-js+1))
    CALL POPREAL8ARRAY(ra_x, (ie-is+1)*(je+2*ng-js+1))
    CALL POPREAL8ARRAY(cry_adv, (ie+2*ng-is+1)*(je-js+2)*(km+1))
    CALL POPINTEGER4(ied)
    CALL POPREAL8ARRAY(z2, (ie+2*ng-is+1)*(je+2*ng-js+1))
    CALL POPINTEGER4(jsd)
    CALL POPREAL8ARRAY(xfx_adv, (ie-is+2)*(je+2*ng-js+1)*(km+1))
    CALL POPREAL8ARRAY(crx_adv, (ie-is+2)*(je+2*ng-js+1)*(km+1))
    CALL POPREAL8ARRAY(yfx_adv, (ie+2*ng-is+1)*(je-js+2)*(km+1))
    DO j=je,js,-1
      DO k=1,km,1
        DO i=ie,is,-1
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(zh(i, j, k))
...
&                     jed, k), crx_adv(is:ie+1, jsd:jed, k), crx_adv_ad(&
&                     is:ie+1, jsd:jed, k), cry_adv(isd:ied, js:je+1, k)&
&                     , cry_adv_ad(isd:ied, js:je+1, k), npx, npy, hord&
&                     , fx, fx_ad, fy, fy_ad, xfx_adv(is:ie+1, jsd:jed, &
&                     k), xfx_adv_ad(is:ie+1, jsd:jed, k), yfx_adv(isd:&
&                     ied, js:je+1, k), yfx_adv_ad(isd:ied, js:je+1, k)&
...
&             j)-fy(i, j+1))*temp_ad/temp)
          ...
          END DO
        END DO
        CALL DEL6_VT_FLUX_BWD(ndif(k), npx, npy, damp(k), z2, z2_ad, wk2&
...
&                     , npy, hord, fx, fx_ad, fy, fy_ad, xfx_adv(is:ie+1&
&                     , jsd:jed, k), xfx_adv_ad(is:ie+1, jsd:jed, k), &
&                     yfx_adv(isd:ied, js:je+1, k), yfx_adv_ad(isd:ied, &
&                     js:je+1, k), gridstruct, bd, ra_x, ra_x_ad, ra_y, &
&                     ra_y_ad)
...
&                     jed, j, km, dp0, uniform_grid, 0)
    END DO
    CALL POPINTEGER4(ndif(km+1))
    CALL POPREAL8(damp(km+1))
...
&                 ied, js:je+1, k), npx, npy, hord, fx, fy, xfx_adv(is:&
&                 ie+1, jsd:jed, k), yfx_adv(isd:ied, js:je+1, k), &
...
&                 ied, js:je+1, k), gridstruct, bd, ra_x, ra_y)
        ELSE
          CALL FV_TP_2D(zh(isd:ied, jsd:jed, k), crx_adv(is:ie+1, jsd:&
...
!  Differentiation of riem_solver_c in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edg
...
!$OMP                          private(cp2,gm2, dm, dz2, w2, pm2, pe2, pem, peg)
    DO j=js-1,je+1
      DO k=1,km
        DO i=is1,ie1
          CALL PUSHREAL8(dm(i, k))
          dm(i, k) = delp(i, j, k)
        END DO
      END DO
      DO i=is1,ie1
! full pressure at top
        CALL PUSHREAL8(pef(i, j, 1))
        pef(i, j, 1) = ptop
        CALL PUSHREAL8(pem(i, 1))
        pem(i, 1) = ptop
      END DO
      DO k=2,km+1
        DO i=is1,ie1
          CALL PUSHREAL8(pem(i, k))
          pem(i, k) = pem(i, k-1) + dm(i, k-1)
        END DO
      END DO
      DO k=1,km
        DO i=is1,ie1
          CALL PUSHREAL8(dz2(i, k))
          dz2(i, k) = gz(i, j, k+1) - gz(i, j, k)
          CALL PUSHREAL8(pm2(i, k))
          pm2(i, k) = dm(i, k)/LOG(pem(i, k+1)/pem(i, k))
          CALL PUSHREAL8(dm(i, k))
          dm(i, k) = dm(i, k)*rgrav
          CALL PUSHREAL8(w2(i, k))
          w2(i, k) = w3(i, j, k)
        END DO
      END DO
...
! Compute Height * grav (for p-gradient computation)
      ...
      END DO
    END DO
    CALL PUSHREAL8ARRAY(dm, (ie-is+3)*km)
    CALL PUSHREAL8ARRAY(pe2, (ie-is+3)*(km+1))
    CALL PUSHINTEGER4(is1)
    CALL PUSHREAL8ARRAY(dz2, (ie-is+3)*km)
    CALL PUSHREAL8ARRAY(w2, (ie-is+3)*km)
    CALL PUSHREAL8(rgrav)
    CALL PUSHREAL8ARRAY(pm2, (ie-is+3)*km)
    CALL PUSHREAL8(gama)
    CALL PUSHREAL8ARRAY(pem, (ie-is+3)*(km+1))
    CALL PUSHINTEGER4(ie1)
  END SUBROUTINE RIEM_SOLVER_C_FWD
!  Differentiation of riem_solver_c in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_ed
...
&   gz_ad
    ...
    INTEGER :: branch
    CALL POPINTEGER4(ie1)
    CALL POPREAL8ARRAY(pem, (ie-is+3)*(km+1))
    CALL POPREAL8(gama)
    CALL POPREAL8ARRAY(pm2, (ie-is+3)*km)
    CALL POPREAL8(rgrav)
    CALL POPREAL8ARRAY(w2, (ie-is+3)*km)
    CALL POPREAL8ARRAY(dz2, (ie-is+3)*km)
    CALL POPINTEGER4(is1)
    CALL POPREAL8ARRAY(pe2, (ie-is+3)*(km+1))
    CALL POPREAL8ARRAY(dm, (ie-is+3)*km)
    dm_ad = 0.0_8
    pe2_ad = 0.0_8
    dz2_ad = 0.0_8
    w2_ad = 0.0_8
...
&                        ), p_fac, scale_m)
      ...
      END DO
      DO i=ie1,is1,-1
        CALL POPREAL8(pem(i, 1))
        pem_ad(i, 1) = 0.0_8
        CALL POPREAL8(pef(i, j, 1))
        pef_ad(i, j, 1) = 0.0_8
      END DO
      DO k=km,1,-1
        DO i=ie1,is1,-1
...
!  Differentiation of rim_2d in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a
...
! end i-loop
    DO 170 i=is,ie
      DO k=1,km
        ...
        CALL PUSHREAL8(wm(k))
        wm(k) = w2(i, k)*dm(k)
        CALL PUSHREAL8(pt1(k))
        pt1(k) = pt2(i, k)
      END DO
      pe1(:) = 0.
      CALL PUSHREAL8(wbar(km+1))
      wbar(km+1) = ws(i)
      CALL PUSHINTEGER4(ks0)
      ks0 = 1
...
! Continuity of (pbar, wbar) is maintained
        DO k=1,km
          rden = -(rgas*dm(k)/dz(k))
          CALL PUSHREAL8(pf1(k))
          pf1(k) = EXP(gama*LOG(rden*pt1(k)))
          CALL PUSHREAL8(dts(k))
...
! next i
      ...
      DO n=1,ms
        ...
        DO k=ad_from,km
          ...
          CALL PUSHREAL8(r_lo(k))
          r_lo(k) = wm(k) + ptmp1
          CALL PUSHREAL8(r_hi(k))
          r_hi(k) = wm(k) - ptmp1
        END DO
 ...
 333    IF (ktop .GE. ks1) THEN
          ...
          DO k=ad_from0,ktop
            z_frac = dt/dts(k)
            CALL PUSHREAL8(r_bot(k))
            r_bot(k) = z_frac*r_lo(k)
            CALL PUSHREAL8(r_top(k+1))
            r_top(k+1) = z_frac*r_hi(k)
            CALL PUSHREAL8(m_bot(k))
            m_bot(k) = z_frac*dm(k)
            CALL PUSHREAL8(m_top(k+1))
            m_top(k+1) = m_bot(k)
          END DO
          CALL PUSHINTEGER4(k - 1)
        ...
        ad_from1 = ktop + 2
        DO k=ad_from1,km+1
          CALL PUSHREAL8(m_top(k))
          m_top(k) = 0.
          CALL PUSHREAL8(r_top(k))
          r_top(k) = 0.
        END DO
        ...
        DO 130 ke=km+1,ktop+2,-1
          ...
          DO k=ke-1,kt1,-1
            IF (time_left .GT. dts(k)) THEN
              ...
              CALL PUSHREAL8(m_top(ke))
              m_top(ke) = m_top(ke) + dm(k)
              CALL PUSHREAL8(r_top(ke))
...
! next level
        ...
        DO 160 ke=ad_from3,km
          ...
          DO k=ke,km
            IF (time_left .GT. dts(k)) THEN
              time_left = time_left - dts(k)
              CALL PUSHREAL8(m_bot(ke))
              m_bot(ke) = m_bot(ke) + dm(k)
              CALL PUSHREAL8(r_bot(ke))
...
! pbar here is actually dt*pbar
      ...
      CALL PUSHCONTROL1B(1)
 170 CONTINUE
    CALL PUSHREAL8ARRAY(dm, km)
    CALL PUSHREAL8ARRAY(dts, km)
    CALL PUSHREAL8(dt)
    CALL PUSHREAL8(grg)
    CALL PUSHREAL8ARRAY(dz, km)
    CALL PUSHREAL8ARRAY(r_lo, km)
    CALL PUSHREAL8ARRAY(wbar, km + 1)
    CALL PUSHINTEGER4(ks0)
    CALL PUSHREAL8ARRAY(pf1, km)
    CALL PUSHREAL8ARRAY(m_top, km + 1)
    CALL PUSHREAL8ARRAY(r_top, km + 1)
    CALL PUSHREAL8ARRAY(m_bot, km + 1)
    CALL PUSHREAL8(rdt)
    CALL PUSHREAL8(pf)
    CALL PUSHREAL8ARRAY(r_bot, km + 1)
    CALL PUSHREAL8ARRAY(ws2, ie - is + 1)
    CALL PUSHREAL8(time_left)
    CALL PUSHREAL8ARRAY(pt1, km)
    CALL PUSHREAL8ARRAY(cm, km)
    CALL PUSHREAL8ARRAY(pp, km)
    CALL PUSHREAL8ARRAY(wc, km)
    CALL PUSHREAL8ARRAY(pbar, km + 1)
    CALL PUSHREAL8(m_surf)
    CALL PUSHREAL8ARRAY(wm, km)
    CALL PUSHREAL8ARRAY(r_hi, km)
  END SUBROUTINE RIM_2D_FWD
!  Differentiation of rim_2d in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.
...
&   pe1_ad, pbar_ad, wbar_ad
    ...
    INTEGER :: ad_from7
    INTEGER :: ad_from8
    CALL POPREAL8ARRAY(r_hi, km)
    CALL POPREAL8ARRAY(wm, km)
    CALL POPREAL8(m_surf)
    CALL POPREAL8ARRAY(pbar, km + 1)
    CALL POPREAL8ARRAY(wc, km)
    CALL POPREAL8ARRAY(pp, km)
    CALL POPREAL8ARRAY(cm, km)
    CALL POPREAL8ARRAY(pt1, km)
    CALL POPREAL8(time_left)
    CALL POPREAL8ARRAY(ws2, ie - is + 1)
    CALL POPREAL8ARRAY(r_bot, km + 1)
    CALL POPREAL8(pf)
    CALL POPREAL8(rdt)
    CALL POPREAL8ARRAY(m_bot, km + 1)
    CALL POPREAL8ARRAY(r_top, km + 1)
    CALL POPREAL8ARRAY(m_top, km + 1)
    CALL POPREAL8ARRAY(pf1, km)
    CALL POPINTEGER4(ks0)
    CALL POPREAL8ARRAY(wbar, km + 1)
    CALL POPREAL8ARRAY(r_lo, km)
    CALL POPREAL8ARRAY(dz, km)
    CALL POPREAL8(grg)
    CALL POPREAL8(dt)
    CALL POPREAL8ARRAY(dts, km)
    CALL POPREAL8ARRAY(dm, km)
    dm_ad = 0.0_8
    dts_ad = 0.0_8
    dz_ad = 0.0_8
    r_lo_ad = 0.0_8
...
&                   )
            ...
            DO k=km,ad_from2,-1
              CALL POPREAL8(r_bot(k))
              r_bot_ad(k) = 0.0_8
              CALL POPREAL8(m_bot(k))
              m_bot_ad(k) = 0.0_8
            END DO
            CALL POPINTEGER4(k)
            ...
            DO ke=ad_to4,km+1,1
              ...
              DO i2=1,ad_count1
                IF (i2 .EQ. 1) THEN
                  CALL POPCONTROL1B(branch)
                  IF (branch .EQ. 0) time_left_ad = 0.0_8
                ELSE
                  CALL POPREAL8(r_top(ke))
            ...
            DO k=km+1,ad_from1,-1
              CALL POPREAL8(r_top(k))
              r_top_ad(k) = 0.0_8
              CALL POPREAL8(m_top(k))
              m_top_ad(k) = 0.0_8
            END DO
            CALL POPCONTROL1B(branch)
...
&         ))
 ...
 120    CALL POPINTEGER4(k)
      END DO
 130  CALL POPINTEGER4(ks0)
      CALL POPREAL8(wbar(km+1))
...
!  Differentiation of sim3_solver in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
&         *r3 - g_rat(i, k)*p1(i)
    ...
    DO k=1,km+1
      ...
      END DO
    END DO
    CALL PUSHREAL8ARRAY(dd, (ie-is+1)*km)
    CALL PUSHREAL8ARRAY(aa, (ie-is+1)*km)
    CALL PUSHREAL8(r2g)
    CALL PUSHREAL8ARRAY(p1, ie - is + 1)
    CALL PUSHREAL8ARRAY(bet, ie - is + 1)
    CALL PUSHREAL8ARRAY(wk1, ie - is + 1)
    CALL PUSHREAL8(r6g)
    CALL PUSHREAL8(ra)
    CALL PUSHREAL8ARRAY(bb, (ie-is+1)*km)
    CALL PUSHREAL8ARRAY(w1, (ie-is+1)*km)
    CALL PUSHREAL8(t2)
    CALL PUSHREAL8(rdt)
    CALL PUSHREAL8(beta)
    CALL PUSHREAL8ARRAY(pp, (ie-is+1)*(km+1))
    CALL PUSHREAL8ARRAY(g_rat, (ie-is+1)*km)
    CALL PUSHREAL8(capa1)
    CALL PUSHREAL8ARRAY(wk, (ie-is+1)*km)
    CALL PUSHREAL8(t1g)
    CALL PUSHREAL8ARRAY(gam, (ie-is+1)*km)
  END SUBROUTINE SIM3_SOLVER_FWD
!  Differentiation of sim3_solver in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
...
&   g_rat_ad, gam_ad
    ...
    REAL :: temp_ad18
    INTEGER :: branch
    CALL POPREAL8ARRAY(gam, (ie-is+1)*km)
    CALL POPREAL8(t1g)
    CALL POPREAL8ARRAY(wk, (ie-is+1)*km)
    CALL POPREAL8(capa1)
    CALL POPREAL8ARRAY(g_rat, (ie-is+1)*km)
    CALL POPREAL8ARRAY(pp, (ie-is+1)*(km+1))
    CALL POPREAL8(beta)
    CALL POPREAL8(rdt)
    CALL POPREAL8(t2)
    CALL POPREAL8ARRAY(w1, (ie-is+1)*km)
    CALL POPREAL8ARRAY(bb, (ie-is+1)*km)
    CALL POPREAL8(ra)
    CALL POPREAL8(r6g)
    CALL POPREAL8ARRAY(wk1, ie - is + 1)
    CALL POPREAL8ARRAY(bet, ie - is + 1)
    CALL POPREAL8ARRAY(p1, ie - is + 1)
    CALL POPREAL8(r2g)
    CALL POPREAL8ARRAY(aa, (ie-is+1)*km)
    CALL POPREAL8ARRAY(dd, (ie-is+1)*km)
    pp_ad = 0.0_8
    DO k=km+1,1,-1
      DO i=ie,is,-1
...
!  Differentiation of sim3p0_solver in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edg
!e_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_co
!re_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mo
!d.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayle
!igh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos 
...
!  Differentiation of sim3p0_solver in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_ed
!ge_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_c
!ore_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_m
!od.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayl
!eigh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos
...
!  Differentiation of sim1_solver in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
&         - pm2(i, k)
    ...
    DO k=2,km
      DO i=is,ie
        ...
        CALL PUSHREAL8(bet(i))
        bet(i) = bb(i, k) - gam(i, k)
        CALL PUSHREAL8(pp(i, k+1))
        pp(i, k+1) = (dd(i, k)-pp(i, k))/bet(i)
      END DO
    END DO
    DO k=km,2,-1
      DO i=is,ie
        CALL PUSHREAL8(pp(i, k))
...
&       *ws(i)-aa(i, km)*w2(i, km-1))/bet(i)
    ...
    END DO
    DO i=is,ie
      CALL PUSHREAL8(pe(i, 1))
      pe(i, 1) = 0.
    END DO
    DO k=1,km
      DO i=is,ie
        CALL PUSHREAL8(pe(i, k+1))
...
!  Differentiation of sim1_solver in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
...
!  Differentiation of sim_solver in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_m
...
!  Differentiation of sim_solver in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
!  Differentiation of edge_profile in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
...
! Surface:
    ...
    END DO
    CALL PUSHREAL8(gk)
    CALL PUSHREAL8(a_bot)
    CALL PUSHREAL8(bet)
    CALL PUSHREAL8(xt1)
    CALL PUSHREAL8(xt2)
    CALL PUSHREAL8ARRAY(gak, km)
    CALL PUSHREAL8ARRAY(gam, (i2-i1+1)*(km+1))
  END SUBROUTINE EDGE_PROFILE_FWD
!  Differentiation of edge_profile in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edg
!e_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_co
!re_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mo
!d.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayle
!igh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos 
...
&   q2e_ad, i1, i2, j1, j2, j, km, dp0, uniform_grid, limiter)
    ...
    REAL :: temp_ad6
    INTEGER :: branch
    CALL POPREAL8ARRAY(gam, (i2-i1+1)*(km+1))
    CALL POPREAL8ARRAY(gak, km)
    CALL POPREAL8(xt2)
    CALL POPREAL8(xt1)
    CALL POPREAL8(bet)
    CALL POPREAL8(a_bot)
    ...
    IF (branch .NE. 0) THEN
      DO i=i2,i1,-1
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) qe2_ad(i, km+1) = 0.0_8
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) qe1_ad(i, km+1) = 0.0_8
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) qe2_ad(i, 1) = 0.0_8
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) qe1_ad(i, 1) = 0.0_8
      END DO
    END IF
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
...
!  Differentiation of nest_halo_nh in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
...
!pressure solver
          DO k=1,npz-1
            DO i=ifirst,0
              CALL PUSHREAL8(g_rat(i, k))
              g_rat(i, k) = delp(i, j, k)/delp(i, j, k+1)
              bb(i, k) = 2.*(1.+g_rat(i, k))
              CALL PUSHREAL8(dd(i, k))
          ...
          DO i=ifirst,0
            CALL PUSHREAL8(bet(i))
            bet(i) = bb(i, 1)
            CALL PUSHREAL8(pkc(i, j, 1))
            pkc(i, j, 1) = 0.
            CALL PUSHREAL8(pkc(i, j, 2))
            pkc(i, j, 2) = dd(i, 1)/bet(i)
            bb(i, npz) = 2.
            CALL PUSHREAL8(dd(i, npz))
          ...
          DO k=2,npz
            DO i=ifirst,0
              ...
              CALL PUSHREAL8(bet(i))
              bet(i) = bb(i, k) - gam(i, k)
              CALL PUSHREAL8(pkc(i, j, k+1))
              pkc(i, j, k+1) = (dd(i, k)-pkc(i, j, k))/bet(i)
            END DO
          END DO
          DO k=npz,2,-1
            DO i=ifirst,0
              CALL PUSHREAL8(pkc(i, j, k))
...
!pk3 if necessary; doesn't require condenstate loading calculation
          IF (computepk3) THEN
            DO i=ifirst,0
              CALL PUSHREAL8(pk3(i, j, 1))
              pk3(i, j, 1) = ptk
            END DO
            DO k=2,npz+1
              DO i=ifirst,0
                CALL PUSHREAL8(pk3(i, j, k))
...
!pk3 if necessary
          ...
          END IF
        END DO
        CALL PUSHREAL8ARRAY(dd, (bd%ied-bd%isd+1)*npz)
        CALL PUSHREAL8ARRAY(peln, (bd%ied-bd%isd+1)*(npz+1)*(bd%jed-bd%&
&                     jsd+1))
        CALL PUSHREAL8ARRAY(bet, bd%ied - bd%isd + 1)
        CALL PUSHINTEGER4(ilast)
        CALL PUSHREAL8(rgrav)
        CALL PUSHINTEGER4(jlast)
        CALL PUSHREAL8(gama)
        CALL PUSHREAL8ARRAY(pkz, (bd%ied-bd%isd+1)*npz)
        CALL PUSHREAL8ARRAY(pe, (bd%ied-bd%isd+1)*(npz+1)*(bd%jed-bd%jsd&
&                     +1))
        CALL PUSHREAL8ARRAY(g_rat, (bd%ied-bd%isd+1)*(npz-1))
        CALL PUSHINTEGER4(ifirst)
        CALL PUSHREAL8ARRAY(gam, (bd%ied-bd%isd+1)*npz)
        CALL PUSHCONTROL2B(2)
      ELSE
        CALL PUSHREAL8ARRAY(dd, (bd%ied-bd%isd+1)*npz)
        CALL PUSHREAL8ARRAY(peln, (bd%ied-bd%isd+1)*(npz+1)*(bd%jed-bd%&
...
!  Differentiation of nest_halo_nh in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edg
!e_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_co
!re_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mo
!d.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayle
!igh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos 
...
&   peln_ad
    ...
    IF (branch .NE. 0) THEN
      IF (branch .EQ. 1) THEN
        CALL POPREAL8ARRAY(gam, (bd%ied-bd%isd+1)*npz)
        CALL POPINTEGER4(ifirst)
        CALL POPREAL8ARRAY(g_rat, (bd%ied-bd%isd+1)*(npz-1))
        CALL POPREAL8ARRAY(pe, (bd%ied-bd%isd+1)*(npz+1)*(bd%jed-bd%jsd+&
&                    1))
        CALL POPREAL8ARRAY(pkz, (bd%ied-bd%isd+1)*npz)
        CALL POPREAL8(gama)
        CALL POPINTEGER4(jlast)
        CALL POPREAL8(rgrav)
        CALL POPINTEGER4(ilast)
        CALL POPREAL8ARRAY(bet, bd%ied - bd%isd + 1)
        CALL POPREAL8ARRAY(peln, (bd%ied-bd%isd+1)*(npz+1)*(bd%jed-bd%&
&                    jsd+1))
        CALL POPREAL8ARRAY(dd, (bd%ied-bd%isd+1)*npz)
        dd_ad = 0.0_8
        peln_ad = 0.0_8
        bet_ad = 0.0_8
        bb_ad = 0.0_8
