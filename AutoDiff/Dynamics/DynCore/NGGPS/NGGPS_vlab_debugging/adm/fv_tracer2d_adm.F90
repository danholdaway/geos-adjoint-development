...
MODULE FV_TRACER2D_MOD_B
  USE TP_CORE_MOD_B, ONLY : fv_tp_2d, fv_tp_2d_adm, copy_corners, &
& copy_corners_adm
...
CONTAINS
!  Differentiation of tracer_2d_1l in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
...
!$OMP                          private(nsplt, frac)
    DO k=1,npz
      ...
      IF (nsplt .GT. 1) THEN
        ...
        DO j=js,je
          DO i=is,ie+1
            CALL PUSHREAL8(mfx(i, j, k))
            mfx(i, j, k) = mfx(i, j, k)*frac
          END DO
        END DO
        DO j=js,je+1
          DO i=isd,ied
            CALL PUSHREAL8(cy(i, j, k))
...
&                         ra_y, mfx=mfx(is:ie+1, js:je, k), mfy=mfy(is:&
&                         ie, js:je+1, k))
              CALL PUSHCONTROL1B(0)
            ELSE
              CALL FV_TP_2D_FWD(qn2(isd:ied, jsd:jed, iq), cx(is:ie+1, &
&                         jsd:jed, k), cy(isd:ied, js:je+1, k), npx, npy&
&                         , hord_pert, fx, fy, xfx(is:ie+1, jsd:jed, k)&
...
! not last call
          DO j=js,je
            ...
            END DO
          END DO
          CALL PUSHREAL8ARRAY(qn2, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*&
&                       nq)
    ...
    CALL PUSHREAL8ARRAY(dp2, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
    CALL PUSHREAL8(frac)
    CALL PUSHREAL8ARRAY(qn2, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*nq)
...
!  Differentiation of tracer_2d_1l in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edg
...
&   nord_tr_pert, trdm_pert, split_damp_tr, dpa)
    ...
    CALL POPREAL8ARRAY(qn2, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*nq)
    CALL POPREAL8(frac)
    CALL POPREAL8ARRAY(dp2, (bd%ie-bd%is+1)*(bd%je-bd%js+1))
...
&                         cx_ad(is:ie+1, jsd:jed, k), cy(isd:ied, js:je+&
&                         1, k), cy_ad(isd:ied, js:je+1, k), npx, npy, &
&                         hord, fx, fx_ad, fy, fy_ad, xfx(is:ie+1, jsd:&
&                         jed, k), xfx_ad(is:ie+1, jsd:jed, k), yfx(isd:&
&                         ied, js:je+1, k), yfx_ad(isd:ied, js:je+1, k)&
&                         , gridstruct, bd, ra_x, ra_x_ad, ra_y, ra_y_ad&
&                         , mfx(is:ie+1, js:je, k), mfx_ad(is:ie+1, js:&
&                         je, k), mfy(is:ie, js:je+1, k), mfy_ad(is:ie, &
&                         js:je+1, k))
            END IF
          ELSE
            IF (branch .EQ. 1) THEN
      ...
      IF (branch .NE. 0) THEN
        ...
        DO j=je+1,js,-1
          DO i=ied,isd,-1
            ...
            CALL POPREAL8(cy(i, j, k))
            cy_ad(i, j, k) = frac*cy_ad(i, j, k)
          END DO
        END DO
        DO j=je,js,-1
          DO i=ie+1,is,-1
            CALL POPREAL8(mfx(i, j, k))
...
&                     fx, fy, xfx(is:ie+1, jsd:jed, k), yfx(isd:ied, js:&
&                     je+1, k), gridstruct, bd, ra_x, ra_y, mfx=mfx(is:&
&                     ie+1, js:je, k), mfy=mfy(is:ie, js:je+1, k))
            ELSE
              CALL FV_TP_2D(qn2(isd:ied, jsd:jed, iq), cx(is:ie+1, jsd:&
...
!  Differentiation of tracer_2d in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
...
&                           ), yfx(isd:ied, js:je+1, k), gridstruct, bd&
&                           , ra_x, ra_y, mfx=mfx(is:ie+1, js:je, k), &
&                           mfy=mfy(is:ie, js:je+1, k), mass=dp1(isd:ied&
...
&                         , yfx(isd:ied, js:je+1, k), gridstruct, bd, &
&                         ra_x, ra_y, mfx=mfx(is:ie+1, js:je, k), mfy=&
&                         mfy(is:ie, js:je+1, k), mass=dp1(isd:ied, jsd:&
...
!  Differentiation of tracer_2d in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_m
...
&                         , mfx(is:ie+1, js:je, k), mfx_ad(is:ie+1, js:&
&                         je, k), mfy(is:ie, js:je+1, k), mfy_ad(is:ie, &
&                         js:je+1, k), dp1(isd:ied, jsd:jed, k), dp1_ad(&
&                         isd:ied, jsd:jed, k), nord=nord_tr, damp_c=&
&                         trdm)
            END IF
          END DO
          DO j=je,js,-1
...
&                       hord, fx, fy, xfx(is:ie+1, jsd:jed, k), yfx(isd:&
&                       ied, js:je+1, k), gridstruct, bd, ra_x, ra_y, &
&                       mfx=mfx(is:ie+1, js:je, k), mfy=mfy(is:ie, js:je&
&                       +1, k))
              END IF
            ELSE IF (it .EQ. 1 .AND. trdm_pert .GT. 1.e-4) THEN
              CALL FV_TP_2D(q(isd:ied, jsd:jed, k, iq), cx(is:ie+1, jsd:&
...
!  Differentiation of tracer_2d_nested in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_
...
!_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn
!_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_ma
!pz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm fv_mapz_mod.moist_cv fv_m
!apz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer2d_mod.tracer_2d fv_trace
!r2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_utils_mod.Riem_Solver_c n
!h_utils_mod.Riem_Solver3test nh_utils_mod.imp_diff_w nh_utils_mod.RIM_2D nh_utils_mod.SIM3_solver nh_utils_mod.SIM3p0_solver nh_
!utils_mod.SIM1_solver nh_utils_mod.SIM_solver nh_utils_mod.edge_scalar nh_utils_mod.edge_profile nh_utils_mod.nest_halo_nh sw_co
!re_mod.c_sw sw_core_mod.d_sw  sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mo
!d.fill3_4corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.compute_di
!vergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mod.copy_
!corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_dist s
...
&                         isd:ied, js:je+1, k), gridstruct, bd, ra_x, &
&                         ra_y, mfx=mfx(is:ie+1, js:je, k), mfy=mfy(is:&
&                         ie, js:je+1, k), mass=dp1(isd:ied, jsd:jed, k)&
...
!  Differentiation of tracer_2d_nested in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b
...
&                       cy_ad(isd:ied, js:je+1, k), npx, npy, hord, fx, &
&                       fx_ad, fy, fy_ad, xfx(is:ie+1, jsd:jed, k), &
&                       xfx_ad(is:ie+1, jsd:jed, k), yfx(isd:ied, js:je+&
&                       1, k), yfx_ad(isd:ied, js:je+1, k), gridstruct, &
&                       bd, ra_x, ra_x_ad, ra_y, ra_y_ad, mfx(is:ie+1, &
&                       js:je, k), mfx_ad(is:ie+1, js:je, k), mfy(is:ie&
&                       , js:je+1, k), mfy_ad(is:ie, js:je+1, k))
          ELSE
            CALL FV_TP_2D_BWD(q(isd:ied, jsd:jed, k, iq), q_ad(isd:ied, &
&                       jsd:jed, k, iq), cx(is:ie+1, jsd:jed, k), cx_ad(&
&                       is:ie+1, jsd:jed, k), cy(isd:ied, js:je+1, k), &
...
&                       bd, ra_x, ra_x_ad, ra_y, ra_y_ad, mfx(is:ie+1, &
&                       js:je, k), mfx_ad(is:ie+1, js:je, k), mfy(is:ie&
&                       , js:je+1, k), mfy_ad(is:ie, js:je+1, k), dp1(&
&                       isd:ied, jsd:jed, k), dp1_ad(isd:ied, jsd:jed, k&
&                       ), nord=nord_tr, damp_c=trdm)
          END IF
        END DO
        DO j=je,js,-1
...
&                     jed, k), cy(isd:ied, js:je+1, k), npx, npy, hord, &
&                     fx, fy, xfx(is:ie+1, jsd:jed, k), yfx(isd:ied, js:&
&                     je+1, k), gridstruct, bd, ra_x, ra_y, mfx(is:ie+1&
&                     , js:je, k), mfy(is:ie, js:je+1, k), dp1(isd:ied, &
&                     jsd:jed, k), nord_tr, trdm)
            ELSE
              CALL FV_TP_2D(q(isd:ied, jsd:jed, k, iq), cx(is:ie+1, jsd:&
&                     jed, k), cy(isd:ied, js:je+1, k), npx, npy, hord, &
&                     fx, fy, xfx(is:ie+1, jsd:jed, k), yfx(isd:ied, js:&
