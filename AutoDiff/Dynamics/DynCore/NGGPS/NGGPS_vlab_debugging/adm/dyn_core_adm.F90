...
& update_dz_c, update_dz_c_adm, update_dz_d, update_dz_d_adm, &
& nest_halo_nh, nest_halo_nh_adm
  USE TP_CORE_MOD_B, ONLY : copy_corners, copy_corners_adm
  USE FV_TIMING_MOD, ONLY : timing_on, timing_off
  USE FV_DIAGNOSTICS_MOD_B, ONLY : prt_maxmin, fv_time, prt_mxm
  USE FV_NWP_NUDGE_MOD_B, ONLY : breed_slp_inline, do_adiabatic_init
  USE DIAG_MANAGER_MOD, ONLY : send_data
  USE FV_ARRAYS_MOD_B, ONLY : fv_grid_type, fv_flags_type, fv_nest_type,&
& fv_diag_type, fv_grid_bounds_type, r_grid
  USE BOUNDARY_MOD_B, ONLY : extrapolation_bc, nested_grid_bc_apply_intt&
...
CONTAINS
!  Differentiation of dyn_core in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod
...
!call init_ijk_mem(is, ie+1, js,  je,   npz, mfx, 0.)
    CALL PUSHREAL8ARRAY(mfx, (bd%ie-bd%is+2)*(bd%je-bd%js+1)*npz)
    mfx = 0.0
!call init_ijk_mem(is, ie  , js,  je+1, npz, mfy, 0.)
    CALL PUSHREAL8ARRAY(mfy, (bd%ie-bd%is+1)*(bd%je-bd%js+2)*npz)
    mfy = 0.0
!call init_ijk_mem(is, ie+1, jsd, jed,  npz, cx, 0.)
    CALL PUSHREAL8ARRAY(cx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
    cx = 0.0
!call init_ijk_mem(isd, ied, js,  je+1, npz, cy, 0.)
    CALL PUSHREAL8ARRAY(cy, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
    cy = 0.0
...
!   to do time-extrapolation on BCs.
      IF (nq .GT. 0) THEN
        IF (flagstruct%inline_q) THEN
          CALL PUSHREAL8ARRAY(q, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz&
&                       *nq)
      ...
      END IF
      IF (.NOT.hydrostatic) THEN
        CALL PUSHREAL8ARRAY(w, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
...
!$OMP parallel do default(none) shared(isd,ied,jsd,jed,npz,gz,zs,delz)
            DO j=jsd,jed
              DO i=isd,ied
                CALL PUSHREAL8(gz(i, j, npz+1))
                gz(i, j, npz+1) = zs(i, j)
              END DO
              DO k=npz,1,-1
                DO i=isd,ied
                  CALL PUSHREAL8(gz(i, j, k))
...
!$OMP parallel do default(none) shared(is,ie,js,je,npz,gz,zs,delz)
          ...
          END IF
          CALL PUSHREAL8ARRAY(gz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(&
&                       npz+1))
...
&                              corner)
      ...
      IF (gridstruct%nested) THEN
        ...
        CALL PUSHREAL8ARRAY(delpc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*&
&                     npz)
...
&                                , bctype=neststruct%nestbctype)
        arg1 = split_timestep_bc + 0.5
        arg2 = REAL(n_split*flagstruct%k_split)
        CALL PUSHREAL8ARRAY(ptc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz&
&                    )
...
&                        a_imp, flagstruct%scale_z)
        IF (gridstruct%nested) THEN
          arg1 = split_timestep_bc + 0.5
          arg2 = REAL(n_split*flagstruct%k_split)
          CALL PUSHREAL8ARRAY(delz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*&
&                       npz)
...
!QUESTION: What to do with divgd in nested halo?
        arg10 = REAL(n_split*flagstruct%k_split)
...
!!$            endif
      ...
      IF (gridstruct%nested .AND. flagstruct%inline_q) THEN
        DO iq=1,nq
          arg10 = split_timestep_bc + 1
          arg2 = REAL(n_split*flagstruct%k_split)
          CALL PUSHREAL8ARRAY(q(isd:ied, jsd:jed, :, iq), (ied-isd+1)*(&
&                       jed-jsd+1)*npz)
...
!      if ( k==npz ) then
        ...
        IF (2 .GT. flagstruct%nord_pert) THEN
          CALL PUSHINTEGER4(nord_v_pert(k))
        ...
        ELSE
          ...
          CALL PUSHCONTROL1B(1)
        END IF
        IF (0.20 .GT. flagstruct%d2_bg_pert) THEN
...
! for delp, delz, and vorticity
        ...
        ELSE
          damp_vt_pert(k) = 0.
        END IF
        CALL PUSHINTEGER4(nord_w)
        nord_w = nord_v(k)
        CALL PUSHINTEGER4(nord_t)
        nord_t = nord_v(k)
        CALL PUSHINTEGER4(nord_t_pert)
...
! Divergence damping:
            ...
            END IF
            nord_k_pert = 0
            IF (0.01 .LT. flagstruct%d2_bg_pert) THEN
...
&               yfx(isd:ied, js:je+1, k), q_con(isd:isd, jsd:jsd, 1), &
&               z_rat(isd:ied, jsd:jed), kgb, heat_s, zvir, sphum, nq, q&
&               , k, npz, flagstruct%inline_q, dt, flagstruct%hord_tr, &
&               hord_m, hord_v, hord_t, hord_p, nord_k, nord_v(k), &
&               nord_w, nord_t, flagstruct%dddmp, d2_divg, flagstruct%&
&               d4_bg, damp_vt(k), damp_w, damp_t, d_con_k, hydrostatic&
&               , gridstruct, flagstruct, bd, flagstruct%hord_tr_pert, &
&               hord_m_pert, hord_v_pert, hord_t_pert, hord_p_pert, &
&               flagstruct%split_damp, nord_k_pert, nord_v_pert(k), &
&               nord_w_pert, nord_t_pert, flagstruct%dddmp_pert, &
...
&                 flagstruct%fv_debug, bd)
      ...
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      CALL PUSHREAL8ARRAY(delp, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL START_GROUP_HALO_UPDATE(i_pack(1), delp, domain, complete=&
&                            .true.)
      CALL PUSHREAL8ARRAY(pt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
...
&                      yfx, delz, ws, rdt, gridstruct, bd, flagstruct%&
&                      hord_tm_pert)
        arg11 = beta .LT. -0.1
...
&                       use_logp, remap_step, arg11)
        IF (gridstruct%square_domain) THEN
          CALL PUSHREAL8ARRAY(zh, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(&
&                       npz+1))
          CALL START_GROUP_HALO_UPDATE(i_pack(4), zh, domain)
          CALL PUSHREAL8ARRAY(pkc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(&
&                       npz+1))
...
! Prevent accumulation of rounding errors at overlapped domain edges:
        CALL PUSHREAL8ARRAY(v, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1)*npz)
        CALL PUSHREAL8ARRAY(u, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2)*npz)
...
!-----------------------------------------------------
    IF (nq .GT. 0 .AND. (.NOT.flagstruct%inline_q)) THEN
      CALL PUSHREAL8ARRAY(q, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz*nq)
      CALL START_GROUP_HALO_UPDATE(i_pack(10), q, domain)
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHCONTROL1B(1)
    ...
    IF (n_con .NE. 0 .AND. flagstruct%d_con .GT. 1.e-5) THEN
      ...
      ELSE
        nf_ke = 3
      END IF
      arg12 = cnst_0p20*gridstruct%da_min
      CALL DEL2_CUBED_FWD(heat_source, arg12, gridstruct, domain, npx, &
&                   npy, npz, nf_ke, bd)
...
&                 )
        ...
        CALL PUSHREAL8(dt)
        CALL PUSHINTEGER4(nord_t_pert)
        CALL PUSHINTEGER4ARRAY(nord_v_pert, npz + 1)
        CALL PUSHREAL8(beta_d)
        CALL PUSHINTEGER4(n_con)
        CALL PUSHREAL8(dt2)
        CALL PUSHREAL8(k1k)
        ...
        CALL PUSHINTEGER4(nord_k_pert)
        CALL PUSHINTEGER4(nord_k)
        CALL PUSHREAL8ARRAY(ws3, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
        CALL PUSHINTEGER4(nord_t)
        CALL PUSHREAL8ARRAY(damp_vt, npz + 1)
        CALL PUSHINTEGER4ARRAY(nord_v, npz + 1)
        CALL PUSHREAL8(min1)
        CALL PUSHINTEGER4(nord_w)
        CALL PUSHREAL8ARRAY(heat_source, (bd%ied-bd%isd+1)*(bd%jed-bd%&
&                     jsd+1)*npz)
        CALL PUSHINTEGER4(je)
        CALL PUSHREAL8ARRAY(dp_ref, npz)
        CALL PUSHINTEGER4(ms)
        CALL PUSHREAL8(d2_divg)
...
!  Differentiation of dyn_core in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
...
&   npz)
    ...
    IF (branch .EQ. 0) THEN
      ...
      CALL POPINTEGER4(ms)
      CALL POPREAL8ARRAY(dp_ref, npz)
      CALL POPINTEGER4(je)
      CALL POPINTEGER4(nord_w)
      CALL POPINTEGER4ARRAY(nord_v, npz + 1)
      CALL POPREAL8ARRAY(damp_vt, npz + 1)
      CALL POPINTEGER4(nord_t)
      CALL POPREAL8ARRAY(ws3, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
      CALL POPINTEGER4(nord_k)
      CALL POPINTEGER4(nord_k_pert)
      CALL POPREAL8(rdt)
      CALL POPINTEGER4(is)
      CALL POPREAL8(rdg)
      CALL POPINTEGER4(ie)
      CALL POPREAL8(k1k)
      CALL POPREAL8(dt2)
      CALL POPREAL8(beta_d)
      CALL POPINTEGER4ARRAY(nord_v_pert, npz + 1)
      CALL POPINTEGER4(nord_t_pert)
      CALL POPREAL8(dt)
      CALL POPINTEGER4(js)
      heat_source_ad = 0.0_8
    ELSE
      IF (branch .EQ. 1) THEN
        CALL POPREAL8ARRAY(wk, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
        CALL POPREAL8(d2_divg)
        ...
        CALL POPREAL8ARRAY(dp_ref, npz)
        CALL POPINTEGER4(je)
        CALL POPREAL8ARRAY(heat_source, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd&
&                    +1)*npz)
        CALL POPINTEGER4(nord_w)
        CALL POPREAL8(min1)
        CALL POPINTEGER4ARRAY(nord_v, npz + 1)
...
&               - heat_source(i, j, k)*cv_air*dtmp_ad/temp8**2
      ...
      END IF
      arg12 = cnst_0p20*gridstruct%da_min
      CALL DEL2_CUBED_BWD(heat_source, heat_source_ad, arg12, gridstruct&
&                   , domain, npx, npy, npz, nf_ke, bd)
    END IF
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREAL8ARRAY(q, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz*nq)
    ...
    DO it=n_split,1,-1
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) THEN
        CALL POPREAL8ARRAY(v, (bd%ied-bd%isd+2)*(bd%jed-bd%jsd+1)*npz)
        CALL NESTED_GRID_BC_APPLY_INTT_ADM(v, v_ad, 1, 0, npx, npy, npz&
&                                    , bd, arg10, arg2, neststruct%v_bc&
&                                    , neststruct%nestbctype)
        CALL POPREAL8ARRAY(u, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+2)*npz)
        CALL NESTED_GRID_BC_APPLY_INTT_ADM(u, u_ad, 0, 1, npx, npy, npz&
&                                    , bd, arg10, arg2, neststruct%u_bc&
&                                    , neststruct%nestbctype)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREAL8ARRAY(w, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
          CALL NESTED_GRID_BC_APPLY_INTT_ADM(w, w_ad, 0, 0, npx, npy, &
&                                      npz, bd, arg10, arg2, neststruct%&
...
&                         gridstruct%nested, .true., .true., .true., bd)
          CALL POPREAL8ARRAY(delz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*&
&                      npz)
...
&                                     delp, delp_ad)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREAL8ARRAY(pkc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(&
&                      npz+1))
          CALL START_GROUP_HALO_UPDATE_ADM(i_pack(5), pkc, pkc_ad, &
&                                    domain, whalo=2, ehalo=2, shalo=2, &
&                                    nhalo=2)
          CALL POPREAL8ARRAY(zh, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(&
&                      npz+1))
...
&                      delz, ws, ws_ad, rdt, gridstruct, bd, flagstruct%&
&                      hord_tm_pert)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
...
&               , j)
      ...
      ELSE
        divg2_ad = 0.0_8
      END IF
      CALL POPREAL8ARRAY(pt, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
      CALL START_GROUP_HALO_UPDATE_ADM(i_pack(1), pt, pt_ad, domain, &
&                                complete=.true.)
      CALL POPREAL8ARRAY(delp, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
...
&               js:je+1, k), yfx_ad(isd:ied, js:je+1, k), q_con(isd:isd&
&               , jsd:jsd, 1), z_rat(isd:ied, jsd:jed), z_rat_ad(isd:ied&
&               , jsd:jed), kgb, heat_s, heat_s_ad, zvir, sphum, nq, q, &
&               q_ad, k, npz, flagstruct%inline_q, dt, flagstruct%&
...
&               hydrostatic, gridstruct, flagstruct, bd, flagstruct%&
&               hord_tr_pert, hord_m_pert, hord_v_pert, hord_t_pert, &
&               hord_p_pert, flagstruct%split_damp, nord_k_pert, &
&               nord_v_pert(k), nord_w_pert, nord_t_pert, flagstruct%&
&               dddmp_pert, d2_divg_pert, flagstruct%d4_bg_pert, &
...
&                                      , ie, js, je, ng, .false.)
        ...
        IF (branch .LT. 6) THEN
          IF (branch .LT. 3) THEN
            IF (branch .NE. 0) THEN
              IF (branch .EQ. 1) CALL POPINTEGER4(nord_v_pert(k))
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(damp_vt(k))
                CALL POPINTEGER4(nord_v(k))
              END IF
              CALL POPCONTROL1B(branch)
              CALL POPCONTROL1B(branch)
            END IF
          ELSE
            IF (branch .EQ. 3) THEN
              CALL POPINTEGER4(nord_v_pert(k))
        ...
        ELSE IF (branch .LT. 9) THEN
          IF (branch .NE. 6) THEN
            IF (branch .EQ. 7) CALL POPINTEGER4(nord_v_pert(k))
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(damp_vt(k))
              CALL POPINTEGER4(nord_v(k))
        ...
        ELSE IF (branch .LT. 11) THEN
          IF (branch .EQ. 9) CALL POPINTEGER4(nord_v_pert(k))
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(damp_vt(k))
            CALL POPINTEGER4(nord_v(k))
          END IF
        END IF
 110    CALL POPINTEGER4(nord_t_pert)
        CALL POPINTEGER4(nord_t)
        CALL POPINTEGER4(nord_w)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
...
&                      gridstruct%nw_corner, bd, gridstruct%grid_type)
      ...
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREAL8ARRAY(ptc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*npz)
        CALL NESTED_GRID_BC_APPLY_INTT_ADM(ptc, ptc_ad, 0, 0, npx, npy, &
&                                    npz, bd, arg1, arg2, neststruct%&
&                                    pt_bc, neststruct%nestbctype)
        CALL POPREAL8ARRAY(delpc, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*&
&                    npz)
...
&               divgd(isd:ied+1, jsd:jed+1, k), flagstruct%nord, dt2, &
&               hydrostatic, .true., bd, gridstruct, flagstruct)
        uc_ad(isd:ied+1, jsd:jed, k) = 0.0_8
        vc_ad(isd:ied, jsd:jed+1, k) = 0.0_8
        ua_ad(isd:ied, jsd:jed, k) = 0.0_8
        va_ad(isd:ied, jsd:jed, k) = 0.0_8
        ut_ad(isd:ied, jsd:jed, k) = 0.0_8
        vt_ad(isd:ied, jsd:jed, k) = 0.0_8
      END DO
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        DO j=je+1,js-1,-1
      ...
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREAL8ARRAY(gz, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*(npz+&
&                    1))
        ...
        IF (branch .EQ. 0) THEN
          DO j=jed,jsd,-1
            ...
            END DO
            DO i=ied,isd,-1
              CALL POPREAL8(gz(i, j, npz+1))
              gz_ad(i, j, npz+1) = 0.0_8
            END DO
          END DO
        ELSE
          DO j=je,js,-1
...
&                   )
    ...
    END DO
    CALL POPCONTROL2B(branch)
    CALL POPREAL8ARRAY(cy, (bd%ied-bd%isd+1)*(bd%je-bd%js+2)*npz)
    CALL POPREAL8ARRAY(cx, (bd%ie-bd%is+2)*(bd%jed-bd%jsd+1)*npz)
    CALL POPREAL8ARRAY(mfy, (bd%ie-bd%is+1)*(bd%je-bd%js+2)*npz)
    CALL POPREAL8ARRAY(mfx, (bd%ie-bd%is+2)*(bd%je-bd%js+1)*npz)
...
!QUESTION: What to do with divgd in nested halo?
        arg10 = REAL(n_split*flagstruct%k_split)
        CALL NESTED_GRID_BC_APPLY_INTT(divgd, 1, 1, npx, npy, npz, bd, &
&                                split_timestep_bc, arg10, neststruct%&
...
&           hydrostatic, gridstruct, flagstruct, bd, flagstruct%&
&           hord_tr_pert, hord_m_pert, hord_v_pert, hord_t_pert, &
&           hord_p_pert, flagstruct%split_damp, nord_k_pert, nord_v_pert&
&           (k), nord_w_pert, nord_t_pert, flagstruct%dddmp_pert, &
&           d2_divg_pert, flagstruct%d4_bg_pert, damp_vt_pert(k), &
&           damp_w_pert, damp_t_pert)
        IF (hydrostatic .AND. (.NOT.flagstruct%use_old_omega) .AND. &
&           last_step) THEN
...
!Want to move this block into the hydro/nonhydro branch above and merge the two if structures
      IF (gridstruct%nested) THEN
        ...
        arg2 = REAL(n_split*flagstruct%k_split)
        CALL NESTED_GRID_BC_APPLY_INTT(delp, 0, 0, npx, npy, npz, bd, &
&                                arg10, arg2, neststruct%delp_bc, &
&                                neststruct%nestbctype)
        arg10 = split_timestep_bc + 1
        arg2 = REAL(n_split*flagstruct%k_split)
        CALL NESTED_GRID_BC_APPLY_INTT(pt, 0, 0, npx, npy, npz, bd, &
&                                arg10, arg2, neststruct%pt_bc, &
&                                neststruct%nestbctype)
      END IF
! end hydro check
      IF (hydrostatic) THEN
...
&                  rdt, gridstruct, bd, flagstruct%hord_tm_pert)
...
!$OMP parallel do default(none) shared(is,ie,js,je,npz,ws,delz,delp,omga)
      ...
      IF (gridstruct%nested) THEN
        IF (.NOT.hydrostatic) THEN
          ...
          arg2 = REAL(n_split*flagstruct%k_split)
          CALL NESTED_GRID_BC_APPLY_INTT(w, 0, 0, npx, npy, npz, bd, &
&                                  arg10, arg2, neststruct%w_bc, &
&                                  neststruct%nestbctype)
        END IF
        arg10 = split_timestep_bc + 1
        arg2 = REAL(n_split*flagstruct%k_split)
        CALL NESTED_GRID_BC_APPLY_INTT(u, 0, 1, npx, npy, npz, bd, arg10&
&                                , arg2, neststruct%u_bc, neststruct%&
&                                nestbctype)
        arg10 = split_timestep_bc + 1
        arg2 = REAL(n_split*flagstruct%k_split)
        CALL NESTED_GRID_BC_APPLY_INTT(v, 1, 0, npx, npy, npz, bd, arg10&
...
!-----------------------------------------------------
    ...
    IF (n_con .NE. 0 .AND. flagstruct%d_con .GT. 1.e-5) THEN
      ...
      END IF
      arg12 = cnst_0p20*gridstruct%da_min
      CALL DEL2_CUBED(heat_source, arg12, gridstruct, domain, npx, npy, &
&               npz, nf_ke, bd)
! Note: pt here is cp*(Virtual_Temperature/pkz)
      IF (hydrostatic) THEN
!
...
!  Differentiation of pk3_halo in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod
!.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mo
!d.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix
!_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_S
!uper fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_gr
...
!$OMP                          private(pei)
    DO j=js,je
      CALL PUSHREAL8(pei(is-2))
      pei(is-2) = ptop
      CALL PUSHREAL8(pei(is-1))
      pei(is-1) = ptop
      DO k=1,npz
        CALL PUSHREAL8(pei(is-2))
        ...
        CALL PUSHREAL8(pk3(is-2, j, k+1))
        pk3(is-2, j, k+1) = EXP(akap*LOG(pei(is-2)))
        CALL PUSHREAL8(pk3(is-1, j, k+1))
        pk3(is-1, j, k+1) = EXP(akap*LOG(pei(is-1)))
      END DO
      CALL PUSHREAL8(pei(ie+1))
      pei(ie+1) = ptop
      CALL PUSHREAL8(pei(ie+2))
      pei(ie+2) = ptop
      DO k=1,npz
        CALL PUSHREAL8(pei(ie+1))
        ...
        CALL PUSHREAL8(pk3(ie+1, j, k+1))
        pk3(ie+1, j, k+1) = EXP(akap*LOG(pei(ie+1)))
        CALL PUSHREAL8(pk3(ie+2, j, k+1))
...
!$OMP                          private(pej)
    DO i=is-2,ie+2
      CALL PUSHREAL8(pej(js-2))
      pej(js-2) = ptop
      CALL PUSHREAL8(pej(js-1))
      pej(js-1) = ptop
      DO k=1,npz
        CALL PUSHREAL8(pej(js-2))
        ...
        CALL PUSHREAL8(pk3(i, js-2, k+1))
        pk3(i, js-2, k+1) = EXP(akap*LOG(pej(js-2)))
        CALL PUSHREAL8(pk3(i, js-1, k+1))
        pk3(i, js-1, k+1) = EXP(akap*LOG(pej(js-1)))
      END DO
      CALL PUSHREAL8(pej(je+1))
      pej(je+1) = ptop
      CALL PUSHREAL8(pej(je+2))
      pej(je+2) = ptop
      DO k=1,npz
        CALL PUSHREAL8(pej(je+1))
        ...
        CALL PUSHREAL8(pk3(i, je+1, k+1))
        pk3(i, je+1, k+1) = EXP(akap*LOG(pej(je+1)))
        CALL PUSHREAL8(pk3(i, je+2, k+1))
        pk3(i, je+2, k+1) = EXP(akap*LOG(pej(je+2)))
      END DO
    END DO
    CALL PUSHREAL8ARRAY(pei, ied - isd + 1)
    CALL PUSHREAL8ARRAY(pej, jed - jsd + 1)
  END SUBROUTINE PK3_HALO_FWD
!  Differentiation of pk3_halo in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
!d.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_m
!od.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mi
!x_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_
!Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_g
...
&   , akap, pk3, pk3_ad, delp, delp_ad)
    ...
    INTRINSIC LOG
    INTRINSIC EXP
    CALL POPREAL8ARRAY(pej, jed - jsd + 1)
    CALL POPREAL8ARRAY(pei, ied - isd + 1)
    pej_ad = 0.0_8
    DO i=ie+2,is-2,-1
      DO k=npz,1,-1
        CALL POPREAL8(pk3(i, je+2, k+1))
        pej_ad(je+2) = pej_ad(je+2) + akap*EXP(akap*LOG(pej(je+2)))*&
&         pk3_ad(i, je+2, k+1)/pej(je+2)
        pk3_ad(i, je+2, k+1) = 0.0_8
        CALL POPREAL8(pk3(i, je+1, k+1))
        pej_ad(je+1) = pej_ad(je+1) + akap*EXP(akap*LOG(pej(je+1)))*&
&         pk3_ad(i, je+1, k+1)/pej(je+1)
        pk3_ad(i, je+1, k+1) = 0.0_8
        CALL POPREAL8(pej(je+2))
        delp_ad(i, je+2, k) = delp_ad(i, je+2, k) + pej_ad(je+2)
        CALL POPREAL8(pej(je+1))
...
!  Differentiation of pln_halo in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod
!.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mo
!d.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix
!_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_S
!uper fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_gr
...
!  Differentiation of pln_halo in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
!d.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_m
!od.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mi
!x_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_
!Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_g
...
&   , pk3, pk3_ad, delp, delp_ad)
    ...
    DO i=ie+2,is-2,-1
      DO j=je+2,je+1,-1
        pet_ad = 0.0_8
        DO k=npz,1,-1
          CALL POPREAL8(pk3(i, j, k+1))
          pet_ad = pet_ad + pk3_ad(i, j, k+1)/pet
          pk3_ad(i, j, k+1) = 0.0_8
          CALL POPREAL8(pet)
...
!  Differentiation of pe_halo in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.
...
!$OMP parallel do default(none) shared(is,ie,js,je,npz,pe,delp,ptop)
    DO j=js,je
      CALL PUSHREAL8(pe(is-1, 1, j))
      pe(is-1, 1, j) = ptop
      CALL PUSHREAL8(pe(ie+1, 1, j))
      pe(ie+1, 1, j) = ptop
      DO k=1,npz
        CALL PUSHREAL8(pe(is-1, k+1, j))
        pe(is-1, k+1, j) = pe(is-1, k, j) + delp(is-1, j, k)
        CALL PUSHREAL8(pe(ie+1, k+1, j))
        pe(ie+1, k+1, j) = pe(ie+1, k, j) + delp(ie+1, j, k)
      END DO
    END DO
!$OMP parallel do default(none) shared(is,ie,js,je,npz,pe,delp,ptop)
    DO i=is-1,ie+1
      CALL PUSHREAL8(pe(i, 1, js-1))
      pe(i, 1, js-1) = ptop
      CALL PUSHREAL8(pe(i, 1, je+1))
      pe(i, 1, je+1) = ptop
      DO k=1,npz
        CALL PUSHREAL8(pe(i, k+1, js-1))
        pe(i, k+1, js-1) = pe(i, k, js-1) + delp(i, js-1, k)
        CALL PUSHREAL8(pe(i, k+1, je+1))
        pe(i, k+1, je+1) = pe(i, k, je+1) + delp(i, je+1, k)
      END DO
    END DO
  END SUBROUTINE PE_HALO_FWD
!  Differentiation of pe_halo in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod
!.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mo
!d.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix
!_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_S
!uper fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_gr
...
!  Differentiation of adv_pe in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a
...
&           grad(3, i, j))
    ...
    END DO
    CALL PUSHINTEGER4(js)
    CALL PUSHREAL8ARRAY(v3, 3*(bd%ie-bd%is+1)*(bd%je-bd%js+1))
    CALL PUSHREAL8ARRAY(grad, 3*(bd%ie-bd%is+1)*(bd%je-bd%js+1))
    CALL PUSHINTEGER4(ie)
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(je)
  END SUBROUTINE ADV_PE_FWD
!  Differentiation of adv_pe in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.
!a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod
!.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_
!dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_Su
!per fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_gri
...
&   , va
    ...
    CALL POPINTEGER4(is)
    CALL POPINTEGER4(ie)
    CALL POPREAL8ARRAY(grad, 3*(bd%ie-bd%is+1)*(bd%je-bd%js+1))
    CALL POPREAL8ARRAY(v3, 3*(bd%ie-bd%is+1)*(bd%je-bd%js+1))
    CALL POPINTEGER4(js)
    v3_ad = 0.0_8
    grad_ad = 0.0_8
    up_ad = 0.0_8
...
!  Differentiation of p_grad_c in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod
!.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mo
!d.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix
!_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_S
!uper fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_gr
...
&           , j, k)))
    ...
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(je)
    CALL PUSHREAL8ARRAY(wk, (bd%ie-bd%is+3)*(bd%je-bd%js+3))
  END SUBROUTINE P_GRAD_C_FWD
!  Differentiation of p_grad_c in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
!d.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_m
!od.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mi
!x_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_
!Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_g
...
&   gz_ad
    ...
    REAL :: temp_ad2
    INTEGER :: branch
    CALL POPREAL8ARRAY(wk, (bd%ie-bd%is+3)*(bd%je-bd%js+3))
    CALL POPINTEGER4(je)
    CALL POPINTEGER4(is)
    CALL POPINTEGER4(ie)
    CALL POPINTEGER4(js)
...
!  Differentiation of nh_p_grad in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
!d.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_m
!od.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mi
!x_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_
!Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_g
...
!$OMP parallel do default(none) shared(is,ie,js,je,pp,pk,top_value)
    DO j=js,je+1
      DO i=is,ie+1
        CALL PUSHREAL8(pp(i, j, 1))
        pp(i, j, 1) = 0.
        CALL PUSHREAL8(pk(i, j, 1))
        pk(i, j, 1) = top_value
      END DO
    END DO
...
!$OMP                          private(wk1)
    DO k=1,npz+1
      IF (k .NE. 1) THEN
        CALL A2B_ORD4_FWD(pp(isd:ied, jsd:jed, k), wk1, gridstruct, npx&
&                   , npy, is, ie, js, je, ng, .true.)
        CALL A2B_ORD4_FWD(pk(isd:ied, jsd:jed, k), wk1, gridstruct, npx&
&                   , npy, is, ie, js, je, ng, .true.)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
...
!$OMP                          private(wk1, wk, du1, dv1)
    DO k=1,npz
      CALL A2B_ORD4_FWD(delp(isd:ied, jsd:jed, k), wk1, gridstruct, npx&
&                 , npy, is, ie, js, je, ng)
      DO j=js,je+1
        DO i=is,ie+1
          CALL PUSHREAL8(wk(i, j))
...
&           gridstruct%rdy(i, j)
    ...
    END DO
    CALL PUSHINTEGER4(js)
    CALL PUSHREAL8ARRAY(wk1, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
    ...
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(je)
    CALL PUSHREAL8ARRAY(wk, (bd%ie-bd%is+2)*(bd%je-bd%js+2))
    CALL PUSHINTEGER4(jed)
  END SUBROUTINE NH_P_GRAD_FWD
!  Differentiation of nh_p_grad in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_m
...
! end k-loop
    ...
    CALL POPREAL8ARRAY(wk, (bd%ie-bd%is+2)*(bd%je-bd%js+2))
    CALL POPINTEGER4(je)
    CALL POPINTEGER4(is)
    ...
    CALL POPREAL8ARRAY(wk1, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1))
    CALL POPINTEGER4(js)
    wk1_ad = 0.0_8
    wk_ad = 0.0_8
    DO k=npz,1,-1
      ...
      DO j=je+1,js,-1
        ...
        END DO
      END DO
      CALL A2B_ORD4_BWD(delp(isd:ied, jsd:jed, k), delp_ad(isd:ied, jsd:&
&                 jed, k), wk1, wk1_ad, gridstruct, npx, npy, is, ie, js&
&                 , je, ng)
    END DO
    DO k=npz+1,1,-1
      CALL A2B_ORD4_BWD(gz(isd:ied, jsd:jed, k), gz_ad(isd:ied, jsd:jed&
&                 , k), wk1, wk1_ad, gridstruct, npx, npy, is, ie, js, &
&                 je, ng, .true.)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
...
&                   js, je, ng, .true.)
    ...
    DO j=je+1,js,-1
      DO i=ie+1,is,-1
        CALL POPREAL8(pk(i, j, 1))
        pk_ad(i, j, 1) = 0.0_8
        CALL POPREAL8(pp(i, j, 1))
        pp_ad(i, j, 1) = 0.0_8
      END DO
    END DO
...
!$OMP                          private(wk1)
    DO k=1,npz+1
      IF (k .NE. 1) THEN
        CALL A2B_ORD4(pp(isd:ied, jsd:jed, k), wk1, gridstruct, npx, npy&
&               , is, ie, js, je, ng, .true.)
        CALL A2B_ORD4(pk(isd:ied, jsd:jed, k), wk1, gridstruct, npx, npy&
&               , is, ie, js, je, ng, .true.)
      END IF
      CALL A2B_ORD4(gz(isd:ied, jsd:jed, k), wk1, gridstruct, npx, npy, &
&             is, ie, js, je, ng, .true.)
    END DO
!$OMP parallel do default(none) shared(is,ie,js,je,npz,delp,gridstruct,npx,npy,ng,isd,jsd, &
!$OMP                                  pk,dt,gz,u,pp,v) &
!$OMP                          private(wk1, wk, du1, dv1)
    DO k=1,npz
      CALL A2B_ORD4(delp(isd:ied, jsd:jed, k), wk1, gridstruct, npx, npy&
&             , is, ie, js, je, ng)
      DO j=js,je+1
        DO i=is,ie+1
          wk(i, j) = pk(i, j, k+1) - pk(i, j, k)
...
!  Differentiation of split_p_grad in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
...
!  Differentiation of split_p_grad in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edg
...
!  Differentiation of one_grad_p in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_m
!od.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_
!mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.m
!ix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh
!_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_
...
!$OMP                          private(wk)
    DO k=2,npz+1
      IF (a2b_ord .EQ. 4) THEN
        CALL A2B_ORD4_FWD(pk(isd:ied, jsd:jed, k), wk, gridstruct, npx, &
&                   npy, is, ie, js, je, ng, .true.)
        CALL PUSHCONTROL1B(1)
      ELSE
...
!$OMP                          private(wk)
    DO k=1,npz+1
      IF (a2b_ord .EQ. 4) THEN
        CALL A2B_ORD4_FWD(gz(isd:ied, jsd:jed, k), wk, gridstruct, npx, &
&                   npy, is, ie, js, je, ng, .true.)
        CALL PUSHCONTROL1B(1)
      ELSE
...
!$OMP                          private(wk)
    DO k=1,npz
      ...
      ELSE IF (a2b_ord .EQ. 4) THEN
        CALL A2B_ORD4_FWD(delp(isd:ied, jsd:jed, k), wk, gridstruct, npx&
&                   , npy, is, ie, js, je, ng)
        CALL PUSHCONTROL2B(1)
      ELSE
...
!  Differentiation of one_grad_p in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
...
!$OMP                          private(wk)
    DO k=2,npz+1
      IF (a2b_ord .EQ. 4) THEN
        CALL A2B_ORD4(pk(isd:ied, jsd:jed, k), wk, gridstruct, npx, npy&
&               , is, ie, js, je, ng, .true.)
      ELSE
...
!$OMP                          private(wk)
    DO k=1,npz+1
      IF (a2b_ord .EQ. 4) THEN
        CALL A2B_ORD4(gz(isd:ied, jsd:jed, k), wk, gridstruct, npx, npy&
&               , is, ie, js, je, ng, .true.)
      ELSE
...
!  Differentiation of grad1_p_update in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_ed
...
!  Differentiation of grad1_p_update in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_e
...
!  Differentiation of mix_dp in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a
!2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.
!p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_d
!p dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_Sup
!er fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid
...
! Remap from above and mix pt
          ...
          CALL PUSHREAL8(pt(i, j, km))
          pt(i, j, km) = (pt(i, j, km)*delp(i, j, km)+pt(i, j, km-1)*dp)&
&           /dpmin
          IF (.NOT.hydrostatic) THEN
            CALL PUSHREAL8(w(i, j, km))
...
&             dpmin
          ...
          ELSE
            CALL PUSHCONTROL1B(1)
          END IF
          CALL PUSHREAL8(delp(i, j, km))
          delp(i, j, km) = dpmin
          CALL PUSHREAL8(delp(i, j, km-1))
    ...
    CALL PUSHINTEGER4(jfirst)
    CALL PUSHINTEGER4(jlast)
    CALL PUSHINTEGER4(ifirst)
  END SUBROUTINE MIX_DP_FWD
!  Differentiation of mix_dp in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.
!a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod
!.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_
!dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_Su
!per fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_gri
...
&   w_ad
    ...
    CALL POPINTEGER4(jlast)
    CALL POPINTEGER4(jfirst)
    ...
    DO j=jlast,jfirst,-1
      ...
      DO i=ilast,ifirst,-1
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) THEN
          CALL POPREAL8(delp(i, j, km-1))
          dp_ad = -delp_ad(i, j, km-1)
          CALL POPREAL8(delp(i, j, km))
          delp_ad(i, j, km) = 0.0_8
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(w(i, j, km))
...
!  Differentiation of geopk in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a2
...
!$OMP                          private(peg, pkg, p1d, logp)
    DO j=jfirst,jlast
      DO i=ifirst,ilast
        CALL PUSHREAL8(p1d(i))
        p1d(i) = ptop
        CALL PUSHREAL8(pk(i, j, 1))
        pk(i, j, 1) = ptk
        CALL PUSHREAL8(gz(i, j, km+1))
        gz(i, j, km+1) = hs(i, j)
      END DO
...
&             1, j)-peln(i, k, j)))
      ...
      END IF
    END DO
    CALL PUSHREAL8ARRAY(logp, bd%ied - bd%isd + 1)
    CALL PUSHINTEGER4(ilast)
    CALL PUSHINTEGER4(jfirst)
    CALL PUSHREAL8ARRAY(p1d, bd%ied - bd%isd + 1)
    CALL PUSHINTEGER4(jlast)
    CALL PUSHINTEGER4(ie)
    CALL PUSHINTEGER4(is)
    CALL PUSHINTEGER4(ifirst)
  END SUBROUTINE GEOPK_FWD
!  Differentiation of geopk in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a
!2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mod.
!p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix_d
!p dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_Sup
!er fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_grid
...
&   q_con
    ...
    CALL POPINTEGER4(ie)
    CALL POPINTEGER4(jlast)
    CALL POPREAL8ARRAY(p1d, bd%ied - bd%isd + 1)
    CALL POPINTEGER4(jfirst)
    CALL POPINTEGER4(ilast)
    CALL POPREAL8ARRAY(logp, bd%ied - bd%isd + 1)
    logp_ad = 0.0_8
    p1d_ad = 0.0_8
    DO j=jlast,jfirst,-1
      CALL POPCONTROL1B(branch)
      ...
      DO k=1,km,1
        DO i=ilast,ifirst,-1
          CALL POPREAL8(gz(i, j, k))
          temp_ad = cp_air*pt(i, j, k)*gz_ad(i, j, k)
          gz_ad(i, j, k+1) = gz_ad(i, j, k+1) + gz_ad(i, j, k)
...
&           )
      ...
      END IF
      DO i=ilast,ifirst,-1
        CALL POPREAL8(gz(i, j, km+1))
        gz_ad(i, j, km+1) = 0.0_8
        CALL POPREAL8(pk(i, j, 1))
        pk_ad(i, j, 1) = 0.0_8
        CALL POPREAL8(p1d(i))
        p1d_ad(i) = 0.0_8
...
!  Differentiation of del2_cubed in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_m
!od.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_
!mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.m
!ix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh
!_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_
...
!     logical, pointer :: sw_corner, se_corner, ne_corner, nw_corner
    ...
    ELSE
      ntimes = 3
    END IF
    CALL PUSHREAL8ARRAY(q, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*km)
...
!$OMP                          private(fx, fy)
      DO k=1,km
        ...
        END IF
        IF (nt .GT. 0) THEN
          CALL COPY_CORNERS_FWD(q(isd:ied, jsd:jed, k), npx, npy, 1, &
...
&                         gridstruct%ne_corner)
        ...
        CALL PUSHINTEGER4(ad_from0)
        IF (nt .GT. 0) THEN
          CALL COPY_CORNERS_FWD(q(isd:ied, jsd:jed, k), npx, npy, 2, &
&                         gridstruct%nested, bd, gridstruct%sw_corner, &
&                         gridstruct%se_corner, gridstruct%nw_corner, &
&                         gridstruct%ne_corner)
          CALL PUSHCONTROL1B(1)
...
!  Differentiation of del2_cubed in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
&   bd%jsd:bd%jed+1)
    ...
    DO n=ntimes,1,-1
      DO k=km,1,-1
        ...
        END DO
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) CALL COPY_CORNERS_BWD(q(isd:ied, jsd:jed, k)&
...
&                                          ne_corner)
      ...
      END DO
    END DO
    CALL POPREAL8ARRAY(q, (bd%ied-bd%isd+1)*(bd%jed-bd%jsd+1)*km)
