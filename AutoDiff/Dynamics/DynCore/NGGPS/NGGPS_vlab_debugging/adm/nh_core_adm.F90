...
CONTAINS
!  Differentiation of riem_solver3 in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
...
&                     ws(is:ie, j), a_imp, p_fac, scale_m)
      ...
      END DO
    END DO
    CALL PUSHREAL8ARRAY(dm, (ie-is+1)*km)
    CALL PUSHREAL8ARRAY(pe2, (ie-is+1)*(km+1))
    CALL PUSHREAL8ARRAY(dz2, (ie-is+1)*km)
    CALL PUSHREAL8ARRAY(w2, (ie-is+1)*km)
    CALL PUSHREAL8(rgrav)
    CALL PUSHREAL8ARRAY(pm2, (ie-is+1)*km)
    CALL PUSHREAL8(gama)
    CALL PUSHREAL8ARRAY(pem, (ie-is+1)*(km+1))
    CALL PUSHREAL8ARRAY(peln2, (ie-is+1)*(km+1))
  END SUBROUTINE RIEM_SOLVER3_FWD
!  Differentiation of riem_solver3 in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edg
...
&   use_logp, last_call, fp_out)
    ...
    REAL :: temp_ad
    INTEGER :: branch
    CALL POPREAL8ARRAY(peln2, (ie-is+1)*(km+1))
    CALL POPREAL8ARRAY(pem, (ie-is+1)*(km+1))
    CALL POPREAL8(gama)
    CALL POPREAL8ARRAY(pm2, (ie-is+1)*km)
    CALL POPREAL8(rgrav)
    CALL POPREAL8ARRAY(w2, (ie-is+1)*km)
    CALL POPREAL8ARRAY(dz2, (ie-is+1)*km)
    CALL POPREAL8ARRAY(pe2, (ie-is+1)*(km+1))
    CALL POPREAL8ARRAY(dm, (ie-is+1)*km)
    dm_ad = 0.0_8
    pe2_ad = 0.0_8
    dz2_ad = 0.0_8
    w2_ad = 0.0_8
    ...
    DO j=je,js,-1
      ...
      END DO
      DO i=ie,is,-1
        CALL POPREAL8(zh(i, j, km+1))
        zh_ad(i, j, km+1) = 0.0_8
      END DO
