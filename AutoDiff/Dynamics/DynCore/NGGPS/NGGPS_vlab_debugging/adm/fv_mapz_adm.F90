...
& mpp_update_domains_adm, domain2d
  ...
  USE FV_ARRAYS_MOD_B, ONLY : fv_grid_type, fv_flags_type
  USE FV_TIMING_MOD, ONLY : timing_on, timing_off
  USE FV_MP_MOD_B, ONLY : is_master
  USE FV_CMP_MOD_B, ONLY : qs_init, fv_sat_adj
  USE FV_DIAGNOSTICS_MOD_B, ONLY : prt_mxm
  IMPLICIT NONE
!-----------------------------------------------------------------------
...
& lagrangian_to_eulerian_fwd, lagrangian_to_eulerian_bwd, map1_q2_fwd, &
& map1_q2_bwd
...
!4 a2b_edge_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_
...
!ian fv_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mo
!d.mapn_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters
...
!ore_mod.fill3_4corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.comp
!ute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mod
!.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_
...
!----------------------------------
          IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
            CALL MAP_SCALAR_FWD(km, peln(is:ie, 1:km+1, j), gz, km, pn2&
...
!----------------------------------
          IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
            CALL MAP1_PPM_FWD(km, pe1, gz, km, pe2, pt, is, ie, j, isd, &
...
&                          is, ie, isd, ied, jsd, jed, 0., fill)
          ...
          ELSE
            CALL MAPN_TRACER_FWD(nq, km, pe1, pe2, q, dp2, kord_tr_pert&
&                          , j, is, ie, isd, ied, jsd, jed, 0., fill)
...
&                        km, pe2, q2, dp2, is, ie, 0, kord_tr(iq), j, &
&                        isd, ied, jsd, jed, 0.)
              CALL PUSHCONTROL1B(0)
            ELSE
              CALL MAP1_Q2_FWD(km, pe1, q(isd:ied, jsd:jed, 1:km, iq), &
&                        km, pe2, q2, dp2, is, ie, 0, kord_tr_pert(iq), &
&                        j, isd, ied, jsd, jed, 0.)
              CALL PUSHCONTROL1B(1)
            END IF
            DO k=1,km
...
! Remap vertical wind:
          IF (kord_wz .EQ. kord_wz_pert) THEN
            CALL MAP1_PPM_FWD(km, pe1, ws(is:ie, j), km, pe2, w, is, ie&
&                       , j, isd, ied, jsd, jed, -2, kord_wz)
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL MAP1_PPM_FWD(km, pe1, ws(is:ie, j), km, pe2, w, is, ie&
&                       , j, isd, ied, jsd, jed, -2, kord_wz_pert)
            CALL PUSHCONTROL1B(1)
          END IF
! Remap delz for hybrid sigma-p coordinate
          IF (abs_kord_tm .EQ. abs_kord_tm_pert) THEN
            CALL MAP1_PPM_FWD(km, pe1, gz, km, pe2, delz, is, ie, j, isd&
&                       , ied, jsd, jed, 1, abs_kord_tm)
            CALL PUSHCONTROL1B(1)
          ELSE
            CALL MAP1_PPM_FWD(km, pe1, gz, km, pe2, delz, is, ie, j, isd&
...
&                 q(i, j, k, sphum))
          ...
          END DO
        END DO
        CALL PUSHINTEGER4(graupel)
        CALL PUSHREAL8ARRAY(phis, (ie-is+1)*(km+1))
        CALL PUSHREAL8ARRAY(pe0, (ie-is+2)*(km+1))
        CALL PUSHREAL8ARRAY(pe1, (ie-is+1)*(km+1))
        CALL PUSHREAL8ARRAY(pe2, (ie-is+1)*(km+1))
        CALL PUSHREAL8ARRAY(pe3, (ie-is+2)*(km+1))
        CALL PUSHREAL8ARRAY(gz, ie - is + 1)
        CALL PUSHINTEGER4(kmp)
        CALL PUSHREAL8ARRAY(dp2, (ie-is+1)*km)
        CALL PUSHINTEGER4(iep1)
        CALL PUSHREAL8(rg)
        CALL PUSHREAL8(tpe)
        CALL PUSHREAL8(k1k)
        CALL PUSHINTEGER4(snowwat)
        CALL PUSHREAL8(dtmp)
        CALL PUSHINTEGER4(cld_amt)
        CALL PUSHREAL8ARRAY(te_2d, (ie-is+1)*(je-js+1))
        CALL PUSHINTEGER4(rainwat)
        CALL PUSHREAL8(result1)
        CALL PUSHINTEGER4(ice_wat)
        CALL PUSHREAL8(rrg)
        CALL PUSHREAL8ARRAY(zsum0, (ie-is+1)*(je-js+1))
        CALL PUSHREAL8ARRAY(pn2, (ie-is+1)*(km+1))
        CALL PUSHREAL8ARRAY(zsum1, (ie-is+1)*(je-js+1))
        CALL PUSHINTEGER4(liq_wat)
        CALL PUSHREAL8ARRAY(pk1, (ie-is+1)*(km+1))
        CALL PUSHREAL8(bkh)
        CALL PUSHREAL8ARRAY(pk2, (ie-is+1)*(km+1))
        CALL PUSHCONTROL3B(0)
      ELSE IF (remap_pt) THEN
! j-loop
! k-loop
...
!d4 a2b_edge_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv
...
!rian fv_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_m
!od.mapn_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiter
...
!core_mod.fill3_4corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.com
!pute_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mo
!d.copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle
...
&   , iq, n, kmp, kp, k_next
    ...
    IF (branch .LT. 3) THEN
      IF (branch .EQ. 0) THEN
        CALL POPREAL8ARRAY(pk2, (ie-is+1)*(km+1))
        CALL POPREAL8(bkh)
        CALL POPREAL8ARRAY(pk1, (ie-is+1)*(km+1))
        CALL POPINTEGER4(liq_wat)
        CALL POPREAL8ARRAY(zsum1, (ie-is+1)*(je-js+1))
        CALL POPREAL8ARRAY(pn2, (ie-is+1)*(km+1))
        CALL POPREAL8ARRAY(zsum0, (ie-is+1)*(je-js+1))
        CALL POPREAL8(rrg)
        CALL POPINTEGER4(ice_wat)
        CALL POPREAL8(result1)
        CALL POPINTEGER4(rainwat)
        CALL POPREAL8ARRAY(te_2d, (ie-is+1)*(je-js+1))
        CALL POPINTEGER4(cld_amt)
        CALL POPREAL8(dtmp)
        CALL POPINTEGER4(snowwat)
        CALL POPREAL8(k1k)
        CALL POPREAL8(tpe)
        CALL POPREAL8(rg)
        CALL POPINTEGER4(iep1)
        CALL POPREAL8ARRAY(dp2, (ie-is+1)*km)
        CALL POPINTEGER4(kmp)
        CALL POPREAL8ARRAY(gz, ie - is + 1)
        CALL POPREAL8ARRAY(pe3, (ie-is+2)*(km+1))
        CALL POPREAL8ARRAY(pe2, (ie-is+1)*(km+1))
        CALL POPREAL8ARRAY(pe1, (ie-is+1)*(km+1))
        CALL POPREAL8ARRAY(pe0, (ie-is+2)*(km+1))
        CALL POPREAL8ARRAY(phis, (ie-is+1)*(km+1))
        CALL POPINTEGER4(graupel)
        dtmp_ad = 0.0_8
        DO k=km,1,-1
          DO j=je,js,-1
...
&                     u_ad, is, ie, j, isd, ied, jsd, jedp1, -1, kord_mt&
&                    )
        ELSE
          gz_ad = 0.0_8
          CALL MAP1_PPM_BWD(km, pe0(is:ie, :), pe0_ad(is:ie, :), gz, &
...
&                 temp_ad13
      ...
      IF (branch .NE. 0) THEN
        ...
        IF (branch .EQ. 0) THEN
          gz_ad = 0.0_8
          CALL MAP1_PPM_BWD(km, pe1, pe1_ad, gz, gz_ad, km, pe2, pe2_ad&
...
&                     , abs_kord_tm)
        ...
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL MAP1_PPM_BWD(km, pe1, pe1_ad, ws(is:ie, j), ws_ad(is:ie, &
...
&                     , jsd, jed, -2, kord_wz_pert)
      ...
      IF (branch .LT. 2) THEN
        IF (branch .EQ. 0) THEN
          CALL MAPN_TRACER_BWD(nq, km, pe1, pe1_ad, pe2, pe2_ad, q, q_ad&
&                        , dp2, dp2_ad, kord_tr, j, is, ie, isd, ied, &
&                        jsd, jed, 0., fill)
        ELSE
          CALL MAPN_TRACER_BWD(nq, km, pe1, pe1_ad, pe2, pe2_ad, q, q_ad&
...
&                        ied, jsd, jed, 0., fill)
      ...
      ELSE IF (branch .EQ. 2) THEN
        ...
        DO iq=ad_to30,1,-1
          ...
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL MAP1_Q2_BWD(km, pe1, pe1_ad, q(isd:ied, jsd:jed, 1:km, &
&                      iq), q_ad(isd:ied, jsd:jed, 1:km, iq), km, pe2, &
&                      pe2_ad, q2, q2_ad, dp2, dp2_ad, is, ie, 0, &
&                      kord_tr(iq), j, isd, ied, jsd, jed, 0.)
          ELSE
            CALL MAP1_Q2_BWD(km, pe1, pe1_ad, q(isd:ied, jsd:jed, 1:km, &
&                      iq), q_ad(isd:ied, jsd:jed, 1:km, iq), km, pe2, &
&                      pe2_ad, q2, q2_ad, dp2, dp2_ad, is, ie, 0, &
&                      kord_tr_pert(iq), j, isd, ied, jsd, jed, 0.)
      ...
      IF (branch .LT. 3) THEN
        IF (branch .EQ. 0) THEN
          CALL MAP_SCALAR_BWD(km, peln(is:ie, 1:km+1, j), peln_ad(is:ie&
&                       , 1:km+1, j), gz, km, pn2, pn2_ad, pt, pt_ad, is&
&                       , ie, j, isd, ied, jsd, jed, 1, abs_kord_tm, &
&                       t_min)
        ELSE IF (branch .EQ. 1) THEN
          CALL MAP_SCALAR_BWD(km, peln(is:ie, 1:km+1, j), peln_ad(is:ie&
&                       , 1:km+1, j), gz, km, pn2, pn2_ad, pt, pt_ad, is&
...
&                    pe2, q2, dp2, is, ie, 0, kord_tr(iq), j, isd, ied, &
&                    jsd, jed, 0.)
            ELSE
              CALL MAP1_Q2(km, pe1, q(isd:ied, jsd:jed, 1:km, iq), km, &
...
!  Differentiation of compute_total_energy in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 
...
!n fv_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.
!mapn_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters f
!v_mapz_mod.ppm_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm fv_mapz_mod.moist_cv 
!fv_mapz_mod.moist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer2d_mod.tracer_2d fv_t
!racer2d_mod.tracer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_utils_mod.Riem_Solver
...
!e_mod.fill3_4corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.comput
!e_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mod.c
!opy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_di
...
&               )
        ...
        END IF
      END DO
      CALL PUSHREAL8ARRAY(phiz, (ie-is+1)*(km+1))
      CALL PUSHREAL8ARRAY(tv, (ie-is+1)*km)
      CALL PUSHCONTROL1B(1)
    ELSE
      CALL PUSHREAL8ARRAY(phiz, (ie-is+1)*(km+1))
      CALL PUSHREAL8ARRAY(tv, (ie-is+1)*km)
...
!  Differentiation of compute_total_energy in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4
...
!an fv_mapz_mod.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod
!.mapn_tracer fv_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters 
...
!re_mod.fill3_4corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.compu
!te_divergence_damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mod.
!copy_corners tp_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_d
!ist sw_core_mod.edge_interpolate4)):
!   gradient     of useful results: qc peln q u v w teq delp delz
!                te_2d pe pt
!   with respect to varying inputs: qc peln q u v w delp delz pe
!                pt
...
&   graupel, nwat
    ...
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREAL8ARRAY(tv, (ie-is+1)*km)
      CALL POPREAL8ARRAY(phiz, (ie-is+1)*(km+1))
    ELSE
      CALL POPREAL8ARRAY(tv, (ie-is+1)*km)
      CALL POPREAL8ARRAY(phiz, (ie-is+1)*(km+1))
      DO j=je,js,-1
...
!  Differentiation of pkez in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a2b
...
&         peln(i, k, j)))
      END DO
    END DO
    CALL PUSHREAL8ARRAY(pk2, (ilast-ifirst+1)*(km+1))
  END SUBROUTINE PKEZ_FWD
!  Differentiation of pkez in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.a2
...
&   , akap, peln, peln_ad, pkz, pkz_ad, ptop)
    ...
    REAL :: temp_ad0
    INTEGER :: branch
    CALL POPREAL8ARRAY(pk2, (ilast-ifirst+1)*(km+1))
    pk2_ad = 0.0_8
    DO k=km,1,-1
      DO i=ilast,ifirst,-1
        CALL POPREAL8(pkz(i, j, k))
...
!  Differentiation of map_scalar in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_m
...
! Compute vertical subgrid distribution
    IF (kord .GT. 7) THEN
      CALL SCALAR_PROFILE_FWD(qs, q4, dp1, km, i1, i2, iv, kord, q_min)
      CALL PUSHCONTROL1B(1)
    ELSE
...
&           +q4(4, i, m)*(1.-r23*esl)))
 ...
 120  CONTINUE
    ...
    CALL PUSHINTEGER4(l)
    CALL PUSHINTEGER4(m)
    CALL PUSHREAL8ARRAY(dp1, (i2-i1+1)*km)
    CALL PUSHREAL8(qsum)
    CALL PUSHREAL8ARRAY(q4, 4*(i2-i1+1)*km)
  END SUBROUTINE MAP_SCALAR_FWD
!  Differentiation of map_scalar in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
...
&   q2_ad, i1, i2, j, ibeg, iend, jbeg, jend, iv, kord, q_min)
    ...
    CALL POPREAL8(qsum)
    CALL POPREAL8ARRAY(dp1, (i2-i1+1)*km)
    CALL POPINTEGER4(m)
    CALL POPINTEGER4(l)
    dp1_ad = 0.0_8
    qsum_ad = 0.0_8
...
&         i, l)
 100    CALL POPINTEGER4(ad_count)
    ...
    ELSE
      CALL SCALAR_PROFILE_BWD(qs, q4, q4_ad, dp1, dp1_ad, km, i1, i2, iv&
&                       , kord, q_min)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
...
!  Differentiation of map1_ppm in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod
...
!  Differentiation of map1_ppm in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mo
...
&         i, l)
 100    CALL POPINTEGER4(ad_count)
    ...
    IF (branch .EQ. 0) THEN
      CALL PPM_PROFILE_BWD(q4, q4_ad, dp1, dp1_ad, km, i1, i2, iv, kord)
    ELSE
      CALL CS_PROFILE_BWD(qs, qs_ad, q4, q4_ad, dp1, dp1_ad, km, i1, i2&
&                   , iv, kord)
    END IF
    DO k=km,1,-1
      DO i=i2,i1,-1
...
!  Differentiation of mapn_tracer in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
!   with respect to varying inputs: pe1 pe2 dp2 q1
  SUBROUTINE MAPN_TRACER_FWD(nq, km, pe1, pe2, q1, dp2, kord, j, i1, i2&
...
! Field output
    ...
    DO iq=1,nq
      DO k=1,km
        ...
        END DO
      END DO
      CALL SCALAR_PROFILE_FWD(qs, q4(1:4, i1:i2, 1:km, iq), dp1, km, i1&
&                       , i2, 0, kord(iq), q_min)
    END DO
! Mapping
    DO i=i1,i2
...
!    if (fill) call fillz(i2-i1+1, km, 1, q2(i1,1,iq), dp2)
    ...
    CALL PUSHREAL8(fac1)
    CALL PUSHREAL8(fac2)
    CALL PUSHREAL8ARRAY(qs, i2 - i1 + 1)
    CALL PUSHREAL8ARRAY(dp1, (i2-i1+1)*km)
    CALL PUSHREAL8ARRAY(qsum, nq)
    CALL PUSHREAL8ARRAY(q4, 4*(i2-i1+1)*km*nq)
  END SUBROUTINE MAPN_TRACER_FWD
!  Differentiation of mapn_tracer in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
...
!   gradient     of useful results: pe1 pe2 dp2 q1
!   with respect to varying inputs: pe1 pe2 dp2 q1
  SUBROUTINE MAPN_TRACER_BWD(nq, km, pe1, pe1_ad, pe2, pe2_ad, q1, q1_ad&
&   , dp2, dp2_ad, kord, j, i1, i2, isd, ied, jsd, jed, q_min, fill)
    ...
    INTEGER :: ad_count0
    INTEGER :: i3
    CALL POPREAL8ARRAY(q4, 4*(i2-i1+1)*km*nq)
    CALL POPREAL8ARRAY(qsum, nq)
    CALL POPREAL8ARRAY(dp1, (i2-i1+1)*km)
    CALL POPREAL8ARRAY(qs, i2 - i1 + 1)
    CALL POPREAL8(fac2)
    CALL POPREAL8(fac1)
    CALL POPREAL8(dp)
    CALL POPINTEGER4(m)
    ...
    DO iq=nq,1,-1
      DO k=km,1,-1
        DO i=i2,i1,-1
          CALL POPREAL8(q1(i, j, k, iq))
          q2_ad(i, k, iq) = q2_ad(i, k, iq) + q1_ad(i, j, k, iq)
          q1_ad(i, j, k, iq) = 0.0_8
        END DO
      END DO
    END DO
    dp1_ad = 0.0_8
...
!  Differentiation of map1_q2 in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod.
...
!per fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_gri
!d_utils_mod.cubed_to_latlon fv_grid_utils_mod.c2l_ord4 fv_grid_utils_mod.c2l_ord2 fv_mapz_mod.Lagrangian_to_Eulerian fv_mapz_mod
!.compute_total_energy fv_mapz_mod.pkez fv_mapz_mod.remap_z fv_mapz_mod.map_scalar fv_mapz_mod.map1_ppm fv_mapz_mod.mapn_tracer f
!v_mapz_mod.map1_q2 fv_mapz_mod.remap_2d fv_mapz_mod.scalar_profile fv_mapz_mod.cs_profile fv_mapz_mod.cs_limiters fv_mapz_mod.pp
!m_profile fv_mapz_mod.ppm_limiters fv_mapz_mod.steepz fv_mapz_mod.rst_remap fv_mapz_mod.mappm fv_mapz_mod.moist_cv fv_mapz_mod.m
!oist_cp fv_mapz_mod.map1_cubic fv_restart_mod.d2c_setup fv_tracer2d_mod.tracer_2d_1L fv_tracer2d_mod.tracer_2d fv_tracer2d_mod.t
!racer_2d_nested nh_core_mod.Riem_Solver3 nh_utils_mod.update_dz_c nh_utils_mod.update_dz_d nh_utils_mod.Riem_Solver_c nh_utils_m
!od.Riem_Solver3test nh_utils_mod.imp_diff_w nh_utils_mod.RIM_2D nh_utils_mod.SIM3_solver nh_utils_mod.SIM3p0_solver nh_utils_mod
!.SIM1_solver nh_utils_mod.SIM_solver nh_utils_mod.edge_scalar nh_utils_mod.edge_profile nh_utils_mod.nest_halo_nh sw_core_mod.c_
!sw sw_core_mod.d_sw  sw_core_mod.divergence_corner sw_core_mod.divergence_corner_nest sw_core_mod.d2a2c_vect sw_core_mod.fill3_4
!corners sw_core_mod.fill2_4corners sw_core_mod.fill_4corners sw_core_mod.xtp_u sw_core_mod.ytp_v sw_core_mod.compute_divergence_
!damping sw_core_mod.smag_corner sw_core_mod.del6_vt_flux tp_core_mod.mp_ghost_ew tp_core_mod.fv_tp_2d tp_core_mod.copy_corners t
!p_core_mod.xppm tp_core_mod.yppm tp_core_mod.deln_flux a2b_edge_mod.extrap_corner fv_grid_utils_mod.great_circle_dist sw_core_mo
!d.edge_interpolate4)):
!   gradient     of useful results: pe1 pe2 dp2 q1 q2
!   with respect to varying inputs: pe1 pe2 dp2 q1 q2
  SUBROUTINE MAP1_Q2_FWD(km, pe1, q1, kn, pe2, q2, dp2, i1, i2, iv, kord&
&   , j, ibeg, iend, jbeg, jend, q_min)
    IMPLICIT NONE
! !INPUT PARAMETERS:
    INTEGER, INTENT(IN) :: j
...
!  Differentiation of map1_q2 in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_mod
!.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_mo
!d.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.mix
!_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh_S
!uper fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_gr
...
!   gradient     of useful results: pe1 pe2 dp2 q1 q2
!   with respect to varying inputs: pe1 pe2 dp2 q1 q2
  SUBROUTINE MAP1_Q2_BWD(km, pe1, pe1_ad, q1, q1_ad, kn, pe2, pe2_ad, q2&
...
!  Differentiation of scalar_profile in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_ed
...
!----- Perfectly linear scheme --------------------------------
    IF (abs0 .GT. 16) THEN
      DO k=1,km
        DO i=i1,i2
          CALL PUSHREAL8(a4(2, i, k))
          a4(2, i, k) = q(i, k)
          CALL PUSHREAL8(a4(3, i, k))
          a4(3, i, k) = q(i, k+1)
          CALL PUSHREAL8(a4(4, i, k))
          a4(4, i, k) = 3.*(2.*a4(1, i, k)-(a4(2, i, k)+a4(3, i, k)))
        END DO
      END DO
      CALL PUSHREAL8ARRAY(q, (i2-i1+1)*(km+1))
      CALL PUSHREAL8(bet)
      CALL PUSHREAL8ARRAY(d4, i2 - i1 + 1)
      CALL PUSHREAL8ARRAY(gam, (i2-i1+1)*km)
      CALL PUSHCONTROL1B(0)
    ELSE
!----- Perfectly linear scheme --------------------------------
!------------------
...
! Top 2 and bottom 2 layers always use monotonic mapping
      ...
      ELSE IF (iv .EQ. 2) THEN
        DO i=i1,i2
          CALL PUSHREAL8(a4(2, i, 1))
          a4(2, i, 1) = a4(1, i, 1)
          CALL PUSHREAL8(a4(3, i, 1))
          a4(3, i, 1) = a4(1, i, 1)
          CALL PUSHREAL8(a4(4, i, 1))
          a4(4, i, 1) = 0.
        END DO
        CALL PUSHCONTROL2B(2)
      ELSE
...
!  Differentiation of scalar_profile in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_e
...
!   gradient     of useful results: delp a4
!   with respect to varying inputs: delp a4
  SUBROUTINE SCALAR_PROFILE_BWD(qs, a4, a4_ad, delp, delp_ad, km, i1, i2&
&   , iv, kord, qmin)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i1, i2
    INTEGER, INTENT(IN) :: km
    ...
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREAL8ARRAY(gam, (i2-i1+1)*km)
      CALL POPREAL8ARRAY(d4, i2 - i1 + 1)
      CALL POPREAL8(bet)
      CALL POPREAL8ARRAY(q, (i2-i1+1)*(km+1))
      q_ad = 0.0_8
      DO k=km,1,-1
        DO i=i2,i1,-1
          CALL POPREAL8(a4(4, i, k))
          ...
          a4_ad(4, i, k) = 0.0_8
          CALL POPREAL8(a4(3, i, k))
          q_ad(i, k+1) = q_ad(i, k+1) + a4_ad(3, i, k)
          a4_ad(3, i, k) = 0.0_8
          CALL POPREAL8(a4(2, i, k))
...
!  Differentiation of cs_profile in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_m
!od.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_
!mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.m
!ix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh
!_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_
...
!  Differentiation of cs_profile in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
!   gradient     of useful results: qs delp a4
!   with respect to varying inputs: qs delp a4
  SUBROUTINE CS_PROFILE_BWD(qs, qs_ad, a4, a4_ad, delp, delp_ad, km, i1&
&   , i2, iv, kord)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i1, i2
    INTEGER, INTENT(IN) :: km
...
!  Differentiation of cs_limiters in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
! Positive definite constraint
      DO i=1,im
        IF (a4(1, i) .LE. 0.) THEN
          CALL PUSHREAL8(a4(2, i))
          a4(2, i) = a4(1, i)
          CALL PUSHREAL8(a4(3, i))
          a4(3, i) = a4(1, i)
          CALL PUSHREAL8(a4(4, i))
          a4(4, i) = 0.
          CALL PUSHCONTROL3B(5)
        ELSE
...
!  Differentiation of cs_limiters in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
...
!  Differentiation of ppm_profile in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
! Additional constraint to ensure positivity when kord=7
    ...
    END DO
    CALL PUSHREAL8(qm)
    CALL PUSHREAL8ARRAY(df2, (i2-i1+1)*km)
    CALL PUSHREAL8(dq)
    CALL PUSHREAL8(c1)
    CALL PUSHREAL8ARRAY(delq, (i2-i1+1)*km)
    CALL PUSHREAL8(fac)
    CALL PUSHREAL8ARRAY(d4, (i2-i1+1)*km)
    CALL PUSHINTEGER4(km1)
    CALL PUSHINTEGER4(it)
    CALL PUSHREAL8(min1)
    CALL PUSHREAL8ARRAY(dc, (i2-i1+1)*km)
  END SUBROUTINE PPM_PROFILE_FWD
!  Differentiation of ppm_profile in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
...
&   kord)
    ...
    REAL :: y2
    REAL :: y1
    CALL POPREAL8ARRAY(dc, (i2-i1+1)*km)
    CALL POPREAL8(min1)
    CALL POPINTEGER4(it)
    CALL POPINTEGER4(km1)
    CALL POPREAL8ARRAY(d4, (i2-i1+1)*km)
    CALL POPREAL8(fac)
    CALL POPREAL8ARRAY(delq, (i2-i1+1)*km)
    CALL POPREAL8(c1)
    CALL POPREAL8(dq)
    CALL POPREAL8ARRAY(df2, (i2-i1+1)*km)
    CALL POPREAL8(qm)
    dc_ad = 0.0_8
    DO k=km,km1,-1
...
!  Differentiation of ppm_limiters in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge
!_mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_cor
!e_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod
!.mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Raylei
!gh_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos f
...
!  Differentiation of ppm_limiters in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edg
...
!  Differentiation of map1_cubic in reverse (adjoint) mode, forward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_m
!od.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core_
!mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.m
!ix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleigh
!_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv_
...
! ---------------------------
      ...
      CALL PUSHINTEGER4(lm2)
      CALL PUSHREAL8(dlm2)
      CALL PUSHREAL8ARRAY(qx, (i2-i1+1)*km)
      CALL PUSHREAL8(ap0)
      CALL PUSHREAL8(ap1)
      CALL PUSHREAL8(am1)
      CALL PUSHREAL8ARRAY(logpl1, (i2-i1+1)*km)
      CALL PUSHREAL8(am2)
      CALL PUSHREAL8ARRAY(logpl2, (i2-i1+1)*kn)
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHINTEGER4(lp0)
      CALL PUSHREAL8(dlp0)
...
!  Differentiation of map1_cubic in reverse (adjoint) mode, backward sweep (with options r8 split(a2b_edge_mod.a2b_ord4 a2b_edge_
!mod.a2b_ord2 dyn_core_mod.dyn_core dyn_core_mod.pk3_halo dyn_core_mod.pln_halo dyn_core_mod.pe_halo dyn_core_mod.adv_pe dyn_core
!_mod.p_grad_c dyn_core_mod.nh_p_grad dyn_core_mod.split_p_grad dyn_core_mod.one_grad_p dyn_core_mod.grad1_p_update dyn_core_mod.
!mix_dp dyn_core_mod.geopk dyn_core_mod.del2_cubed dyn_core_mod.Rayleigh_fast fv_dynamics_mod.fv_dynamics fv_dynamics_mod.Rayleig
!h_Super fv_dynamics_mod.Rayleigh_Friction fv_dynamics_mod.compute_aam fv_dynamics_mod.geos_to_fv3 fv_dynamics_mod.fv3_to_geos fv
...
&   plm1_ad, plm2_ad, dlp0_ad, dlm1_ad, dlm2_ad
    ...
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
      CALL POPREAL8ARRAY(logpl2, (i2-i1+1)*kn)
      CALL POPREAL8(am2)
      CALL POPREAL8ARRAY(logpl1, (i2-i1+1)*km)
      CALL POPREAL8(am1)
      CALL POPREAL8(ap1)
      CALL POPREAL8(ap0)
      CALL POPREAL8ARRAY(qx, (i2-i1+1)*km)
      CALL POPREAL8(dlm2)
