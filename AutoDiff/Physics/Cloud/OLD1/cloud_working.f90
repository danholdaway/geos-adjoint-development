subroutine cloud ( DT, IM, JM, LM, th, q, ple, Kh,              &
...
!Kh              - Eddy heat diffusivity (m^-2 s^-1)
...
real(4), intent(in), dimension(IM,JM,LM) :: CNV_DQLDT, CNV_MFD, CNV_UPDF

real(4), intent(in) :: ESTBLX(:)

real(4), intent(in), dimension(IM,JM,0:LM) :: ple, Kh
...
!PROGNOSTICS
real(4), intent(inout), dimension(IM,JM,LM) :: th, q
real(4), intent(inout), dimension(IM,JM,LM) :: QI_ls, QL_ls, QI_con, QL_con, CF_con, CF_ls
real(4), intent(inout), dimension(IM,JM,LM) :: CNV_PRC3
...
real(4), dimension (IM,JM,LM) :: ph, pih, MASS, iMASS, T, DZET, QDDF3, RH, DP, DM
...
real(4) :: CF_tot !Precip amounts and fall rate

real(4) :: ALPHA, ALHX3, RHCRIT
real(4) :: AA, BB !Microphyiscal constants

real(4), parameter :: PI_0  = 4.*atan(1.)
real(4), parameter :: RHO_W = 1.0e3  !Density of liquid water in kg/m^3
real(4)            :: T_ICE_MAX

!PHYSPARAMS constants
real(4) :: CNV_BETA,ANV_BETA,LS_BETA,RH00,C_00,LWCRIT,C_ACC,C_EV_R,C_EV_S,CLDVOL2FRC
real(4) :: RHSUP_ICE,SHR_EVAP_FAC,MIN_CLD_WATER,CLD_EVP_EFF,LS_SDQV2,LS_SDQV3,LS_SDQVT1
real(4) :: ANV_SDQV2,ANV_SDQV3,ANV_SDQVT1,ANV_TO_LS,N_WARM,N_ICE,N_ANVIL,N_PBL
real(4) :: ANV_ICEFALL_C,LS_ICEFALL_C,REVAP_OFF_P,CNVENVFC,WRHODEP,T_ICE_ALL,CNVICEPARAM
real(4) :: CNVDDRFC,ANVDDRFC,LSDDRFC,minrhcrit,maxrhcrit,turnrhcrit,maxrhcritland
...
real(4) :: EVAP_DD_CU_above_new, EVAP_DD_AN_above_new, EVAP_DD_LS_above_new
real(4) :: EVAP_DD_CU_above_old, EVAP_DD_AN_above_old, EVAP_DD_LS_above_old
real(4) :: SUBL_DD_CU_above_new, SUBL_DD_AN_above_new, SUBL_DD_LS_above_new
real(4) :: SUBL_DD_CU_above_old, SUBL_DD_AN_above_old, SUBL_DD_LS_above_old

real(4) :: AREA_LS_PRC1, AREA_UPD_PRC1, AREA_ANV_PRC1
real(4) :: TOT_PREC_UPD, TOT_PREC_ANV, TOT_PREC_LS, AREA_UPD_PRC, AREA_ANV_PRC, AREA_LS_PRC
real(4) :: QTMP2

real(4) :: RHEXCESS, TPW, NEGTPW

TH = TH*(CONS_P00**CONS_KAPPA)
...
DO i = 1,IM
   DO j = 1,JM
      DO k = 1,LM
         ...
         if ( (QL_ls(i,j,k) + QL_con(i,j,k)) > 10e-4 ) then
         ...
         if ( (QI_LS(i,j,k) + QI_con(i,j,k)) > 10e-4 ) then
         ...
         CNV_PRC3(i,j,k) = QRN_CU_1D  !Ras Rain
...
!Locals
real(4), parameter :: minrhx = 0.001 !Minimum allowed env RH
real(4) :: TEND, QVx, fQi
...
subroutine pdf_width (PP,FRLAND,maxrhcrit,maxrhcritland,turnrhcrit,minrhcrit,pi_0,ALPHA )  
 ...
 !Inputs
 real(4), intent(in) :: PP, FRLAND

 real(4), intent(in) :: maxrhcrit, maxrhcritland, turnrhcrit, minrhcrit, pi_0
...
integer, intent(in) :: ICEFRPWR
real*4 , intent(in) :: CONS_ALHL, CONS_ALHF, CONS_ALHS, CONS_CP, CONS_H2OMW, CONS_AIRMW
real(4), intent(in) :: ESTBLX(:)
...
real(4) :: QCx, QVx, CFx, QAx, QC, QA, fQi, fQi_A, dQAi, dQAl, dQCi, dQCl
...
real*4 , intent(in) :: CONS_H2OMW, CONS_AIRMW, CONS_ALHL, CONS_RVAP, CONS_RGAS, CONS_PI, CONS_CP
...
!INPUTS
real(4), intent(in) :: DT, RHCR, PL, XF, QS, RHO_W, CLD_EVP_EFF
real*4 , intent(in) :: CONS_H2OMW, CONS_AIRMW, CONS_ALHL, CONS_RVAP, CONS_RGAS, CONS_PI, CONS_CP, CONS_ALHS

!PROGNOSTIC
real(4), intent(inout) :: TE, QV, QL, QI, F

!LOCALS
real(4) :: ES, RADIUS, K1, K2, TEFF, QCm, SUBL, RHx, QC, A_eff, NN, EPSILON
...
!Inputs
real(4), intent(in) :: DT, TE, PL, DZET, SUNDQV2, SUNDQV3, SUNDQT1, C_00, LWCRIT
...
!Inputs
real(4), intent(in ) :: RAIN, PR     ! in kg m^-2 s^-1, mbar

!Outputs
real(4), intent(out) :: DIAM3, NTOTAL, W, VE

!Locals
integer :: IQD
real(4), parameter  :: N0 = 0.08  !cm^-3
...
subroutine ice_settlefall_cnv( WXR, QI, PL, TE, F, CONS_RGAS, Kh, KHu, KHl, k, DT, DZ, QP, ANV_ICEFALL_C )
...
subroutine ice_settlefall_ls( WXR, QI, PL, TE, F, CONS_RGAS, Kh, KHu, KHl, k, DT, DZ, QP, LS_ICEFALL_C )
...
real(4), intent(in) :: DT, mass, imass, PL, AA, BB, RHCR3, dZE, QDDF3, AREA, FRLAND, ENVFC, DDRFC
real*4 , intent(in) :: CONS_ALHF, CONS_ALHS, CONS_ALHL, CONS_CP, CONS_TICE, CONS_H2OMW, CONS_AIRMW
...
real(4), intent(inout) :: PFl_above_in, Pfl_above_out, PFi_above_in, PFi_above_out
real(4), intent(inout) :: EVAP_DD_above_in, EVAP_DD_above_out, SUBL_DD_above_in, SUBL_DD_above_out

!Locals
integer :: NS, NSMX, itr,L

real(4) :: PFi, PFl, QS, dQS, ENVFRAC, TKo, QKo, QSTKo, DQSTKo, RH_BOX, T_ED, QPlKo, QPiKo
real(4) :: Ifactor, RAINRAT0, SNOWRAT0, FALLRN, FALLSN, VEsn, VErn, NRAIN, NSNOW, Efactor
real(4) :: TinLAYERrn, DIAMrn, DROPRAD, TinLAYERsn, DIAMsn, FLAKRAD
real(4) :: EVAP, SUBL, ACCR, MLTFRZ, EVAPx, SUBLx, EVAP_DD,SUBL_DD,DDFRACT, LANDSEAF
real(4) :: TAU_FRZ, TAU_MLT

real(4), parameter :: TRMV_L = 1.0   !m/s
