...
!            IRC (I,ICMIN:K-1) = RC(ICMIN:K-1)
  ...
  END SUBROUTINE RASE
!  Differentiation of rase in reverse (adjoint) mode (with options r8 split(GEOS_MoistGridComp.PRE_PROGNO_CLOUD)):
...
!         CNV_NICE,                                        & 
!         CNV_NDROP,                                    &   !DONIF
  SUBROUTINE RASE_ADM(idim, irun, k0, icmin, dt, doras, cpo, alhlo, &
...
!         rc(icmin) = 0
          ...
          IF (k .GT. 0) THEN
            ...
            CALL PUSHREAL8ARRAY(qho, idim*k0)
...
!#endif
      ...
      DO i=irun,1,-1
        ...
        IF (branch .LT. 2) THEN
          IF (branch .NE. 0) THEN
            ...
            CALL POPREAL8ARRAY(qho, idim*k0)
  ...
  CONTAINS
!  Differentiation of cloude in reverse (adjoint) mode (with options r8 split(GEOS_MoistGridComp.PRE_PROGNO_CLOUD)):
...
!   CUMULUS FRICTION
                  ...
                  ELSE
                    ...
                    vht_ad = temp_ad35
                    voi_ad(ic) = voi_ad(ic) + temp_ad36 - (eta(ic)-eta(&
&                     ic+1))*temp_ad35
                    eta_ad(ic) = eta_ad(ic) - voi(ic)*temp_ad35
                    eta_ad(ic+1) = eta_ad(ic+1) + voi(ic)*temp_ad35 - (&
&                     voi(ic)+voi(ic+1))*temp_ad34
...
&                     eta(ic+1))*vcu_ad(ic)
                    ...
                    uht_ad = temp_ad38
                    uoi_ad(ic) = uoi_ad(ic) + temp_ad39 - (eta(ic)-eta(&
&                     ic+1))*temp_ad38
                    eta_ad(ic) = eta_ad(ic) - uoi(ic)*temp_ad38
                    eta_ad(ic+1) = eta_ad(ic+1) + uoi(ic)*temp_ad38 - (&
&                     uoi(ic)+uoi(ic+1))*temp_ad37
...
&                       l)+(voi(l)-voi(l+1))*eta(l+1))*vcu_ad(l)
                      ...
                      voi_ad(l) = voi_ad(l) + temp_ad31 - eta(l)*&
&                       temp_ad30
...
&                       temp_ad30
                      ...
                      uoi_ad(l) = uoi_ad(l) + temp_ad33 - eta(l)*&
&                       temp_ad32
...
&                   *temp_ad25
                ...
                gms_ad(ic) = gms_ad(ic) + eta(ic+1)*temp_ad23 + gmh_ad(&
&                 ic)
                eta_ad(ic+1) = eta_ad(ic+1) + alhl*(qol(ic)-qht(ic+1))*&
&                 temp_ad21
...
&                     + 0.5*(hst(l)+hst(l-1))*tx3_ad
              ...
              CALL POPCONTROL1B(branch)
              IF (branch .NE. 0) THEN
                IF (min2 .LE. 0.0_8 .AND. (rasal_exp .EQ. 0.0_8 .OR. &
&                   rasal_exp .NE. INT(rasal_exp))) THEN
                  min2_ad = 0.0
                ELSE
                  min2_ad = (rasal2i-rasal1)*rasal_exp*min2**(rasal_exp-&
&                   1)*rasal_ad(ic)
                END IF
                rasal_ad(ic) = 0.0_8
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) zet_ad(ic) = zet_ad(ic) + min2_ad/&
&                   8000.
              END IF
              CALL POPCONTROL1B(branch)
...
&                 (l)-eta(l+1))*hol(l))/eta(l))*temp_ad8
...
&                 (l)
              ...
              END DO
              CALL POPCONTROL1B(branch)
              IF (branch .NE. 0) alm_ad = alm_ad + 2*alm*toki_ad/&
&                 lambda_min**2
            END IF
            temp_ad4 = alm_ad/tem
...
&       0.0553, 0.0445, 0.0633/)
      ...
      RETURN
    END SUBROUTINE ACRITN
!  Differentiation of rnevp in reverse (adjoint) mode (with options r8 split(GEOS_MoistGridComp.PRE_PROGNO_CLOUD)):
...
!! If hst is smoothed then adjusted precips may be negative
      ...
      RETURN
    END SUBROUTINE RNEVP
!  Differentiation of htest in reverse (adjoint) mode (with options r8 split(GEOS_MoistGridComp.PRE_PROGNO_CLOUD)):
...
!         else if ( RASNCL < -100.0 ) then 
      ...
      END DO
    END SUBROUTINE FINDDTLS
!  Differentiation of strap in reverse (adjoint) mode (with options r8 split(GEOS_MoistGridComp.PRE_PROGNO_CLOUD)):
...
!! SPECIFY WEIGHTS GIVEN TO EACH LAYER WITHIN SUBCLOUD "SUPERLAYER"
          ...
          END DO
          tmpt(k) = poi(k)*prh(k)
          CALL PUSHREAL8ARRAY(qst(k), k0 - k + 1)
          CALL PUSHREAL8ARRAY(dqq(k), k0 - k + 1)
          CALL DQSATPERT(dqq(k), qst(k), tmpt(k), pol(k), 1)
          CALL PUSHCONTROL1B(1)
        ELSE
          CALL PUSHCONTROL1B(0)
...
&           l)/temp**2
        ...
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) THEN
          CALL POPREAL8ARRAY(dqq(k), k0 - k + 1)
          CALL POPREAL8ARRAY(qst(k), k0 - k + 1)
          CALL DQSATPERT_ADM(dqq(k:), dqq_ad(k:), qst(k:), qst_ad(k:), &
&                      tmpt(k), tmpt_ad(k), pol(k), 1)
...
!  Differentiation of sundq3_ice in reverse (adjoint) mode (with options r8 split(GEOS_MoistGridComp.PRE_PROGNO_CLOUD)):
...
&           prs(l)))
        ...
        IF (k .LE. k0) THEN
          ...
          END DO
          arg1 = poi(k)*prh(k)
          CALL DQSATPERT(dqq(k), qst(k), arg1, pol(k), 1)
        END IF
!dh tap produces code with error
        DO jj=1,nrands
          IF (seedras(i, 1)/1000000. .LT. 1e-6) THEN
