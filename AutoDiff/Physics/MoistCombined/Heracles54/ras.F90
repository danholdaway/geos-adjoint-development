...
CONTAINS
   ...
   SUBROUTINE RASE(IDIM, IRUN, K0, ICMIN, DT, DORAS,                     &
         CPO,ALHLO,ALHL1,TICE,GRAVO,                      &
         SEEDRAS,SIGE,                          &
         KCBL,WGT0,WGT1,TPERT,QPERT,          &
         THO, QHO, UHO, VHO,                              & 
         QSS, DQS, CNV_FRACTION, RASAL2_2d,               &
         CO_AUTO,                                         &
!         pko, plo, phio, phie, qlo, qio,                  &
         PLE, PKE, CLW, FLXD, & !FLX, FLXC,                  &
         CNV_PRC3,                                        &
         CNV_UPDFRC,                                      &
!         CNV_CVW,                                         &
!         CNV_QC,                                          &
...
!         CNV_NDROP,                                    &   !DONIF
      ...
      REAL(8), DIMENSION (IDIM,K0  ), INTENT(INOUT) ::  THO, QHO, UHO, VHO!, QLO, QIO, CLAN
      REAL(8), DIMENSION (IDIM,K0+1), INTENT(IN   ) ::  PLE, PKE!,PHIE
      REAL(8), DIMENSION (IDIM,K0  ), INTENT(IN   ) ::  QSS, DQS!,PLO,PKO,PHIO
      REAL(8), DIMENSION (IDIM     ), INTENT(IN   ) ::  CNV_FRACTION, RASAL2_2d
      REAL(8), DIMENSION (     K0+1), INTENT(IN   ) ::  SIGE
      REAL(8), DIMENSION (IDIM,K0  ), INTENT(  OUT) ::  CLW , FLXD !, FLX
!      REAL(8), DIMENSION (IDIM,K0+1), INTENT(  OUT) ::  FLXC
      REAL(8), DIMENSION (IDIM,K0  ), INTENT(  OUT) ::  CNV_PRC3
      REAL(8), DIMENSION (IDIM,K0  ), INTENT(  OUT) ::  CNV_UPDFRC!, CNV_CVW, CNV_QC
...
!      INTEGER, DIMENSION(IDIM,K0), INTENT(  OUT) ::  IRC
      ...
      REAL(8),    OPTIONAL,           INTENT(IN)    :: FSCAV(ITRCR) ! Fraction scavenged per km
      ...
      REAL(8),  DIMENSION(K0) :: POI, QOI, UOI, VOI,  DQQ, BET, GAM, CLL, TMPT
      REAL(8),  DIMENSION(K0) :: POI_c, QOI_c
      REAL(8),  DIMENSION(K0) :: PRH,  PRI,  GHT, DPT, DPB, PKI, DISSK0,DISSK1, CLANTND
      ...
      REAL(8),  DIMENSION(K0) :: HOL, HST, QOL, ZOL, HCLD, CLL0,CLLX,CLLI,CLLB
...
!      INTEGER,  DIMENSION(K0-1) :: RC
      ...
      REAL(8) TX2, TX3, UHT, VHT, AKM, ACR, ALM, TTH, QQH, SHTRG, WSPBL, DQX
      REAL(8) WFN, TEM, TRG, TRGEXP, EVP, WLQ, QCC,MTKW_MAX !, BKE
      ...
      REAL(8) GRAV, CP, ALHL, CPBG, ALHI, CPI, GRAVI, DDT, LBCP, OBG, AFC 

!!!!!!!!!
      REAL(8) FRICFAC,DPTH_BL,WUPDRFT,PBLFRAC,AUTORAMPB,CO_ZDEP
      REAL(8) RASAL1, RASAL2, RASAL2i, CO_T, RASNCL,FRICLAMBDA,SDQVT1,SDQV2 
      REAL(8) LAMBDA_FAC,STRAPPING,ACRITFAC,HMINTRIGGER,LLDISAGGXP
      REAL(8) LAMBMX_FAC, DIAMMN_MIN,RDTLEXPON, CLI_CRIT,SDQV3, MAXDALLOWED_D, MAXDALLOWED_S
...
!#endif
   ...
   CONTAINS
      ...
      SUBROUTINE CLOUDE(IC)
         ...
         REAL(8) :: CLI , TE_A, C00_X, CLI_CRIT_X, PETE, TOKI, GMHx, HSTx  !, dQx
...
!               beta_e = MAPL8_ALHL*TminusTa/MAPL8_RVAP/Tparcel/Tparcel
!               RH_AMB=QOI(L)/QST(L)
...
!!! RESET PRESSURE at bottom edge of CBL 
            ...
            PRS(K+1) = PRCBL
            PRJ(K+1) = (PRS(K+1)/1000.)**(MAPL8_RGAS/MAPL8_CP)
...
!!!!! RECALCULATE PROFILE QUAN. IN LOWEST STRAPPED LAYER
            ...
            do jj = 1,nrands !dh tap produces code with error
               rndu(jj) = max( seedras(I,1)/1000000., 1e-6 )
            enddo
...
!                  nui=((alfa*W*G)**1.5)/(2.0*MAPL8_PI*980.0*beta*TPI(INDEX))
...
!           if (AER_BASE%kap(index) .gt. 0.8)    aseasalt_aux  = aseasalt_aux  + AER_BASE%num(index)*AER_BASE%dpg(index)*AER_BASE%dpg(index)*1.61*MAPL8_PI !assumes a fixed sigma = 2.0
!
!         end do
...
!
!         rho_a = PRE*100.0/MAPL8_RGAS/TEMP 
!         ninbkg=0.0
...
!               ndfaer =1.381e-23*TEMP*nslip*(1.0-exp(-nsdust_c*ahet)) /(12.*MAPL8_PI*viscosity*DDUST(ix))             
!               INDUST_C(ix) = 2.0*MAPL8_PI*ndfaer*NDUST(ix)*NL/lam_r
!
!            END DO
!
!
...
!            ndfaer =1.381e-23*TEMP*nslip*(1.0-exp(-nssoot_c*ahet)) /(12.*MAPL8_PI*viscosity*DSOOT)             
!            INSOOT_c= 2.0*MAPL8_PI*ndfaer*NSOOT*NL/lam_r
!
!         ! sea salt
...
!         DENAIR= PLo*100.0/MAPL8_RGAS/TEMP
...
!            DC=max((QICE0/(NIPRE*500.0*MAPL8_PI))**(0.333), 40.0e-6) !Assumme monodisperse size distribution about size distribution is made. 
!         else   
!            DC = 40.0e-6
!         end if
!
!         AUX=  NIPRE*DENICE*MAPL8_PI*DC*DC
...
!
!
!         AUX=  NINUC*DENICE*MAPL8_PI*20.0e-6*20.0e-6
!         TEFF = DENAIR*2.0*((K1*DIFF+K2)*DC+(K3/0.1))
!
!         if  (AUX .gt. 1.0e-12)  then 
...
!
!         lamr=min((MAPL8_PI*rho_r*NL/rho_a/qr)**(1./3.), 1.0e8)
...
!
!            lamg=min((MAPL8_PI*rho_g*NG/rho_a/qg)**(1./3.), 1.0e6)
!            aux=Dc_g*lamg
...
!         lami=(MAPL8_PI*rho_i*NI_/rho_a/qi)**(1./3.)
!         aux=Dc_i*lami
!         !fpi=exp(-aux)*(aux*aux*aux+3.0*aux*aux+6.0*aux + 6.)/6.0  !precipitated fraction
!         !fpi=min(fpi, 1.0)/DTL
!         !fni=exp(-aux)/DTL
...
!   end function ERFAPP
   ...
   SUBROUTINE RASE_FAST(IDIM, IRUN, K0, ICMIN, DT,                     &
         CPO,ALHLO,ALHL1,TICE,GRAVO,                      &
         SEEDRAS,SIGE,                          &
         KCBL,WGT0,WGT1,TPERT,QPERT,          &
         THO, QHO,                              & 
