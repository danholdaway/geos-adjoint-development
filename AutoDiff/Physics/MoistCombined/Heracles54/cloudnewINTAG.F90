...
#define GPU_MAXLEVS LM
      ...
      real :: MASS, iMASS
      real :: TOTFRC
      real :: QRN_LS, QRN_AN, QRN_CU_1D
      real :: QSN_LS, QSN_AN, QSN_CU
      ...
      real :: LSENVFC, ANVENVFC, ENVFC, DDRFC, BETA

      real :: SDQV2, SDQV3, SDQVT1
   ...
   subroutine PRECIP3(  &
      ...
      ELSE 
         ...
         IF ( (TE > MAPL_TICE ) .and.(TE <= MAPL_TICE+5. ) ) THEN
            ...
            TE  = TE  - MAPL_ALHF*MLTFRZ/MAPL_CP
            QPl = QPl + MLTFRZ
            QPi = QPi - MLTFRZ
         END IF
         ...
         MLTFRZ = 0.0
         IF ( TE > MAPL_TICE+5.  ) THEN  ! Go Ahead and melt any snow/hail left above 5 C 
            MLTFRZ= QPi 
            TE  = TE  - MAPL_ALHF*MLTFRZ/MAPL_CP
            QPl = QPl + MLTFRZ
            QPi = QPi - MLTFRZ
         ...
         MLTFRZ = 0.0
         if ( K >= LM-1 ) THEN
            IF ( TE > MAPL_TICE+0.  ) THEN ! Go Ahead and melt any snow/hail left above 0 C in lowest layers 
               MLTFRZ= QPi 
               TE  = TE  - MAPL_ALHF*MLTFRZ/MAPL_CP
               QPl = QPl + MLTFRZ
               QPi = QPi - MLTFRZ
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   subroutine RADCOUPLE(  &
      ...
      real, intent(out) :: RAD_QL,RAD_QI,RAD_QR,RAD_QS,RAD_CF,RAD_RL,RAD_RI

      real, intent(in )  :: tempor

      real :: RElAN, REiAN, RElLS, REiLS, QCm, NN, ss, RAD_RI_AN
