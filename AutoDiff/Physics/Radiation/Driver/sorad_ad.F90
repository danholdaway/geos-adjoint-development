...
!-----Fill the reff, cwc, and fcld for the column
      CALL PUSHREAL8(fcld_col(k))
      fcld_col(k) = fcld_dev(i, k)
      DO l=1,4
        CALL PUSHREAL8(reff_col(k, l))
        reff_col(k, l) = reff_dev(i, k, l)
        CALL PUSHREAL8(cwc_col(k, l))
        cwc_col(k, l) = cwc_dev(i, k, l)
      END DO
    END DO
!-----Initialize temporary arrays to zero to avoid UMR
    CALL PUSHREAL8ARRAY(rr, (np+2)*2)
    rr = 0.0
    CALL PUSHREAL8ARRAY(tt, (np+2)*2)
    tt = 0.0
    CALL PUSHREAL8ARRAY(td, (np+2)*2)
    td = 0.0
    CALL PUSHREAL8ARRAY(rs, (np+2)*2)
    rs = 0.0
    CALL PUSHREAL8ARRAY(ts, (np+2)*2)
    ts = 0.0
    CALL PUSHREAL8ARRAY(rra, (np+2)*2**2)
    rra = 0.0
    CALL PUSHREAL8ARRAY(rxa, (np+2)*2**2)
    rxa = 0.0
!if( OVERCAST == .false. ) then
    CALL PUSHREAL8ARRAY(tta, (np+1)*2**2)
    tta = 0.0
    CALL PUSHREAL8ARRAY(tda, (np+1)*2**2)
    tda = 0.0
    CALL PUSHREAL8ARRAY(rsa, (np+1)*2**2)
    rsa = 0.0
!endif
...
&               awg_nir, arg_nir, caib, caif, cons_grav)
    ...
    DO k=np+1,1,-1
      flx_devb(i, k) = 0.0_8
    END DO
    CALL POPREAL8ARRAY(rsa, (np+1)*2**2)
    CALL POPREAL8ARRAY(tda, (np+1)*2**2)
    CALL POPREAL8ARRAY(tta, (np+1)*2**2)
    CALL POPREAL8ARRAY(rxa, (np+2)*2**2)
    CALL POPREAL8ARRAY(rra, (np+2)*2**2)
    CALL POPREAL8ARRAY(ts, (np+2)*2)
    CALL POPREAL8ARRAY(rs, (np+2)*2)
    CALL POPREAL8ARRAY(td, (np+2)*2)
    CALL POPREAL8ARRAY(tt, (np+2)*2)
    CALL POPREAL8ARRAY(rr, (np+2)*2)
    DO k=np,1,-1
      DO l=4,1,-1
        CALL POPREAL8(cwc_col(k, l))
        cwc_devb(i, k, l) = cwc_devb(i, k, l) + cwc_colb(k, l)
        cwc_colb(k, l) = 0.0_8
        CALL POPREAL8(reff_col(k, l))
        reff_devb(i, k, l) = reff_devb(i, k, l) + reff_colb(k, l)
        reff_colb(k, l) = 0.0_8
      END DO
      CALL POPREAL8(fcld_col(k))
      fcld_devb(i, k) = fcld_devb(i, k) + fcld_colb(k)
      fcld_colb(k) = 0.0_8
      oa_devb(i, k) = oa_devb(i, k) + dp(k)*466.7*1.02*ohb(k)
      ohb(k) = 0.0_8
      CALL POPREAL8(swh(k+1))
...
&       tempb0
      ...
      whb(k) = 0.0_8
      CALL POPREAL8(scal(k))
      CALL POPREAL8(dp_pa(k))
      CALL POPREAL8(dp(k))
    END DO
    CALL POPREAL8(swh(1))
