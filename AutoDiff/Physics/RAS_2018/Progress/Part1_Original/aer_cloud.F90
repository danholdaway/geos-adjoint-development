...
!Global aux variables
      ...
      real*8  ::  base_mass_so4_polluted, base_mass_so4_clean, base_mass_ss, &
                    frac_dust(5), frac_bc, frac_org, ahet_dust(5), ahet_bc, aseasalt, nseasalt_ice
...
!=================================================================
	  ...
	  sc_ice, Nhet_dep, Nhet_dhf, fdust_imm, fdust_dep, fdust_dhf, &
	 ...
	 integer :: nmodes  !number of lognormal aerosol modes
	  ...
	  !Barahona(2011) spec parameters
       ...
       real*8 :: To_ice, kB_ice, hP_ice, Na_ice, vs_ice, sigIV_ice, sigIW_ice, &
		 ...
		 dtnuc_dhf, Dact_imm, vw_ice, rsc_ice, fdrop_dust, fdrop_bc, fcoa_dust, &
...
!===================================================================================
  ...
  subroutine aerosol_activate(tparc_in, pparc_in, sigwparc_in, wparc_ls,  Aer_Props, &                                           
					   ...
					   INimmr8, dINimmr8, Ncdepr8, Ncdhfr8, sc_icer8, &
					   fdust_immr8, fdust_depr8, fdust_dhfr8, nlimr8, use_average_v, CCN_param, IN_param, fd_dust, fd_soot, &
					   pfrz_inc_r8, sigma_nuc, rhi_cell)
      ...
      real(r8), intent(in)   :: tparc_in, pparc_in, sigwparc_in, wparc_ls,   &
					   npre_in, dpre_in, Ndropr8, qc, fd_soot, fd_dust, sigma_nuc, rhi_cell
...
!get dust from input structure
 ...
 call getINsubset(1, Aer_Props, Aeraux) 
  ...
  allocate(ddust_ice(nbindust_ice))
     ...
     ndust_ice=DBLE(Aeraux%num(1:nbindust_ice))*air_den
...
if (antot .gt. 1.0e2) then !only if aer is present  
	...
	! the distribution of relative humidity is assumed normal centered around the RH mean%
		 ...
		 aux =  (sc_ice - rhi_cell)/(sq2_par*sigma_nuc) ! uses grid cell RH
		 
		 !aux =  (sc_ice-1d0)/(sq2_par*sigma_nuc) !assumes mean rh =1 when nucleation is occuring 
...
subroutine vertical_vel_variance(omeg, lc_turb, tm_gw, pm_gw, rad_cool, uwind_gw, tausurf_gw, nm_gw, LCCIRRUS, Nct, Wct, &
						 ...
						 wparc_ls, wparc_gw, wparc_cgw, wparc_turb, EIS)
...
!************************************************************ 
	  ...
	  DSH =0.d0
	  FDS=1.d0
      ! here we need to decide what the supersaturation level inside an ice cloud is.      
        sc_ice = 1.d0
...
!************************************************************  
      ...
      real*8 ::  Nd, Nbc, aux, Si_, SW, del0, ddel0, &
		 ...
		 frac, aux2, Dx2, fdep, Ndep, Ndhf, dNdep, dNdhf, si, &
		 dfrac, Ncdep_, Ncdhf_ , fglassy, Nglassy, dNglassy, & 
...
!========================================
  ...
  ! sea salt
	...
	if (is_gocart) then 
		   ...
		   DO index =1,nbindust_ice                
        	       !ahet= ddust_ice(index)*ddust_ice(index)*pi_ice*4.0 !Assume spheres by now
  		       ahet= ddust_ice(index)*ddust_ice(index)*ddust_ice(index)*0.52*acorr_dust* &
	                	exp(4.5*sigdust_ice(index)*sigdust_ice(index)) !Assume spheres by now	       			
		       Naux=(1.0-exp(-nsdust*ahet))*ndust_ice(index)+Naux
		       dNaux = exp(-nsdust*ahet)*ndust_ice(index)*dnsd*coolr*ahet+dNaux	       
		   END DO 
		   !ahet=10.0e-10
		   ahet=dbc_ice*dbc_ice*dbc_ice*0.52*acorr_bc* &
            	                        	  exp(4.5*sigbc_ice*sigbc_ice)     
         end if 
...
!				
				...
				if(temperature_K < 273.15 .and. temperature_K >= 273.15 - 35.) then
					...
					do ij = 1, ijstop_soot
						...
						mu = n_in*ALPHA_SOOT*H_frac_soot*ahet_bc/BASE_SOOT_PHILIC_OMEGA
					        ...
					        ahet_aux =  PIE*D_grid_soot(ij)*D_grid_soot(ij)*D_grid_soot(ij)*16.4*acorr_bc/6.0
				...
				if(temperature_K < 273.15 - 35.) then
					...
					do ij = 1, ijstop_dust
					   ...
					   if (is_gocart) then 					     	   
						 mu = n_in_ultra*ALPHA_DUST*H_frac_dust*ahet_dust(ij)/BASE_DUST_OMEGA							
					      else
					         ahet_aux =  PIE*D_grid_dust(ij)*D_grid_dust(ij)*D_grid_dust(ij)*4.73*acorr_dust/6.0
					...
					do ij = 1, ijstop_soot
					     ...
					     if (is_gocart) then 
						mu = n_in_ultra*ALPHA_SOOT*H_frac_soot*ahet_bc/BASE_SOOT_PHILIC_OMEGA
					      else
					        ahet_aux =  PIE*D_grid_soot(ij)*D_grid_soot(ij)*D_grid_soot(ij)*16.4*acorr_bc/6.0
