...
def main_comparison_map(args):
    ...
    filename = '%s/H21-TLM-GQ%d-PH%d/%s.fvpert.eta.%s.nc4' % (root_dir,1,args.phys,args.expid,ftime_str)
    TLM_gq1 = read_state_2D(args,filename)

    ftime_str = '%sz+%sz' % (adate.format(format='%Y%m%d_%H'),vdate.format(format='%Y%m%d_%H'))
    filename = '%s/H21-TLM-GQ%d-PH%d/%s.fvpert.eta.%s.nc4' % (root_dir,2,args.phys,args.expid,ftime_str)
...
def parse_args():
    ...
    parser.add_argument('--root_tmpl',help='location of the experiment',type=str,default=os.getenv('ARCHIVE'),required=False)
    ...
    parser.add_argument('--norm',help='norm to process',type=str,choices=['txe','twe'],default='txe',required=False)
    parser.add_argument('--phys',help='physics option to process',type=int,choices=range(0,2),default='0',required=False)
...
def get_region_ij(args,grid):
    ...
    if   ( args.region == 'global'  ):
        ...
        levels     = np.arange(29,Nz)
        latitudes  = np.concatenate([np.arange(0,135),np.arange(227,Ny)])
        longitudes = np.arange(0,Nx)
...
def show_NLMTLM_correlation_profile(args,grid,gq1,gq2,sgq1=None,sgq2=None):
    ...
    suptitle_str = '%s, fhr = %02d\nregion = %s, Physics option = %d' % (date_str,args.fhr,args.region,args.phys)
...
def savecorrelationprofilefigure(args,var):
    ...
    fname = '%s-f%02d_%s_%d_%s' % (date_str,args.fhr,args.region,args.phys,var)
