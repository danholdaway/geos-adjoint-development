...
def parse_args():
    ...
    parser.add_argument('--norm',help='norm to process',type=str,choices=['txe','twe','PH0','PH1','PH2'],default='PH2',required=False)
    parser.add_argument('--variable',help='variable to plot',type=str,choices=['u','v','tv','delp','sphu','qitot','qltot','ozone'],default='tv',required=False)
    parser.add_argument('--zaxis',help='y axis for profile',type=str,choices=['semilog','linear'],default='semilog',required=False)
...
def read_state_3D(args,filename):
    print 'reading %s from %s' % (args.variable, filename)
    state = {}
    nc = Dataset(filename,'r')
    if args.variable not in nc.variables:
        print 'variable %s is not present in the file %s' % (args.variable, filename)
        sys.exit(1)
    else:
        state[args.variable] = np.squeeze(nc.variables[args.variable][:])
    nc.close()
    return state
...
def read_state_2D(args,filename):
    ...
    if args.variable not in nc.variables:
        ...
        state[args.variable] = np.squeeze(nc.variables[args.variable][:,args.level])
    nc.close()
    return state
...
def read_NLM_TLM_data(args, adate):
    ...
    #ftime_str = '%sz+%sz' % (rdate.format(format='%Y%m%d_%H'),vdate.format(format='%Y%m%d_%H'))
    ftime_str = '%sz+%sz' % (adate.format(format='%Y%m%d_%H'),vdate.format(format='%Y%m%d_%H'))
    filename = '%s/H21-TLM-GQ2-PH1/%s.fvpert.eta.%s.nc4' % (root_dir,args.expid,ftime_str)
    TLM_gq1 = read_state_3D(args,filename)

    #ftime_str = '%sz+%sz' % (rdate.format(format='%Y%m%d_%H'),vdate.format(format='%Y%m%d_%H'))
    ftime_str = '%sz+%sz' % (adate.format(format='%Y%m%d_%H'),vdate.format(format='%Y%m%d_%H'))
    filename = '%s/H21-TLM-GQ2-PH2/%s.fvpert.eta.%s.nc4' % (root_dir,args.expid,ftime_str)
...
def show_NLMTLM_correlation_profile(args,grid,gq1,gq2,sgq1=None,sgq2=None):
    ...
    for var in gq1:
        ...
        mysubplot(args,var,xvec,zvec,gq1[var][:],gq2[var][:],sgq1=sgq1[var][:],sgq2=sgq2[var][:])
...
def mysubplot(args,var,xvec,zvec,gq1,gq2,sgq1=None,sgq2=None):
    if ( args.zaxis == 'semilog' ):
    ...
    elif ( args.zaxis == 'linear' ):
        l1 = pyplot.plot(gq1,zvec,'m',linewidth=2.0)
        l2 = pyplot.plot(gq2,zvec,'c',linewidth=2.0)
        l1 = pyplot.plot(gq1,zvec,'o',mfc='m',mec='m')
        l2 = pyplot.plot(gq2,zvec,'o',mfc='c',mec='c')
    if ( sgq1 != None ):
    ...
    if ( args.zaxis == 'semilog' ):
        ...
        pyplot.plot(mgq1,1000.0,'*',mfc='m',mec='m',ms=12,clip_on=False)
        pyplot.plot(mgq2,1000.0,'*',mfc='c',mec='c',ms=12,clip_on=False)
