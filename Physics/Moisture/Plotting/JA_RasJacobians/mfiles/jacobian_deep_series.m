...
Jac = ncread('JACOBIAN_DEEP_SERIES.nc4','J');
...
for i = 1:imax
    ...
    sub_p = sub_p + 1;
    subplot(imax,4,sub_p)
    [C,h] = contour(Nk_vec(top-4:end),Nk_vec(top-4:end),J_Ht(top-4:end,top-4:end));
    gcapos = get(gca,'position');
    if sub_p <= 4; 
        title({'(a) \partial H /\partial \theta';'Output Index'},'FontSize',fontsize,'FontName','TimesNewRoman')
        set(gca,'position',[gcapos(1) gcapos(2)+yoffset1 gcapos(3) 0.95*gcapos(4)])
    end
    caxis([-climmax1 climmax1]);
    set(h,'LineWidth',line_wid);
    set(gca,'YDir','reverse')
    ylabel(['\delta = ',num2str(pert_size(i))],'FontSize',fontsize,'FontName','TimesNewRoman')
    colormap jet
    if sub_p > 4
        colorbar('location','southoutside')
        xlabel('Perturbation Index (\theta)','FontSize',fontsize,'FontName','TimesNewRoman')
        set(gca,'position',[gcapos(1) gcapos(2)+yoffset gcapos(3) 0.95*gcapos(4)])
    ...
    sub_p = sub_p + 1;
    subplot(imax,4,sub_p)
    [C,h] = contour(Nk_vec(top-4:end),Nk_vec(top-4:end),J_Hq(top-4:end,top-4:end));
    gcapos = get(gca,'position');
    if sub_p <= 4; 
        title({'(b) \partial H /\partial q';'Output Index'},'FontSize',fontsize,'FontName','TimesNewRoman')
        set(gca,'position',[gcapos(1) gcapos(2)+yoffset1 gcapos(3) 0.95*gcapos(4)])
    end
    ...
    sub_p = sub_p + 1;
    subplot(imax,4,sub_p)
    [C,h] = contour(Nk_vec(top-4:end),Nk_vec(top-4:end),J_Mt(top-4:end,top-4:end));
    gcapos = get(gca,'position');
    if sub_p <= 4; 
        title({'(c) \partial Q /\partial \theta';'Output Index'},'FontSize',fontsize,'FontName','TimesNewRoman')
        set(gca,'position',[gcapos(1) gcapos(2)+yoffset1 gcapos(3) 0.95*gcapos(4)])
    end
    ...
    sub_p = sub_p + 1;
    subplot(imax,4,sub_p)
    [C,h] = contour(Nk_vec(top-4:end),Nk_vec(top-4:end),J_Mq(top-4:end,top-4:end));
    gcapos = get(gca,'position');
    if sub_p <= 4; 
        title({'(d) \partial Q /\partial q';'Output Index'},'FontSize',fontsize,'FontName','TimesNewRoman')
        set(gca,'position',[gcapos(1) gcapos(2)+yoffset1 gcapos(3) 0.95*gcapos(4)])
    end
...
saveas(gcf,'jacobian_deep_series_hq_1.eps', 'psc2')
