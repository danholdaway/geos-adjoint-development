...
real, dimension(7,5*kmax,5*kmax) :: JACOBIAN
...
!DO pert_point = 2011,2011!num_active_points
   ...
   DO ii = 1,7 !Perturb different amounts (1 - 7)
...
! Write Jacobian to nc4 file
      IF (WRITE_JAC == .true.) THEN
         output_file = 'JACOBIAN_SHALLOW_SERIES.nc4'
         rcode = nf90_create(trim(adjustl(output_file)), NF90_CLOBBER, ncid)
         rcode = nf90_def_dim(ncid, "z", 7, z_dimid) 
