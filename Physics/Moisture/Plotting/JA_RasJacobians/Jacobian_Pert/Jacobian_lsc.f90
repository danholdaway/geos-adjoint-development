...
REAL :: active_points(imax*jmax,3)
...
REAL, DIMENSION(kmax) :: PRC3_LSC_pert
...
DO icol = 1,imax
   DO jcol = 1,jmax
      ...
      call largescalemoist_driver(kmax,DT,TH(icol,jcol,:),Q(icol,jcol,:),P(icol,jcol,:),PRC3_LSC(icol,jcol,:))
   ...
   DO j = 1,jmax
      DO k = 1,kmax
         ...
         IF (DTHDt(i,j,k) .ne. 0) THEN
            ...
            active_points(num_active_points,3) = minval(DQDt(i,j,:)*DT)
            exit
...
DO i = 1,num_active_points
   print*, active_points(i,1), active_points(i,2), active_points(i,3)
endDO
...
DO pert_point = 1,num_active_points

    icol = nint(active_points(pert_point,1))
    jcol = nint(active_points(pert_point,2))
    
    DO ii = 4,4 !Perturb different amounts (1 - 7)
      ...
      DO jj = 1,1  !Negative then positive versions of the perturbation.
         ...
         DO kk = 1,4 !PERTURB EACH VARIABLE
            ...
            DO ll = 1,kmax !Perturb level by level
               ...
               !RECALL THE NONLINEAR CODE
               call largescalemoist_driver(kmax,DT,TH_pert,Q_pert,P_pert,PRC3_LSC_pert)
