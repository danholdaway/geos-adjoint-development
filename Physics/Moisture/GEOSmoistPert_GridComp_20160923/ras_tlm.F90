...
!WMP     RASAL(IC) = RASAL1 + (RASAL2i-RASAL1)*(ZET(IC) - 2000.)/8000.
                pwr1 = min2**rasal_exp
                rasal(ic) = rasal1 + (rasal2i-rasal1)*pwr1
              END IF
...
!   to redistribute convective tendencies within CBL
      ...
      INTRINSIC PRESENT
      INTRINSIC ALLOCATED
      REAL*8, DIMENSION(k-icmin+1) :: arg1
      REAL*8 :: pwx1
      REAL :: pwy1
      REAL*8 :: pwr1
      REAL*8 :: abs1
...
!         CNV_NICE,                                        & 
!         CNV_NDROP,                                    &   !DONIF
  SUBROUTINE RASE_TLM(idim, irun, k0, icmin, dt, cpo, alhlo, &
&   alhl1, tice, gravo, seedras, sige, kcbl, wgt0, wgt1, tpert, tpert_tl&
&   , qpert, tho, tho_tl, qho, qho_tl, uho, uho_tl, vho, vho_tl, qss, &
&   qss_tl, dqs, dqs_tl, cnv_fraction, rasal2_2d, co_auto, ple, pke, clw&
&   , clw_tl, flxd, flxd_tl, cnv_prc3, cnv_prc3_tl, cnv_updfrc, &
&   cnv_updfrc_tl, rasparams, itrcr, xho, triedlev_diag, fscav, disske)
    IMPLICIT NONE
!!!!!!!!!!!!!!!======================================     
!!!!!!!!!!   Subroutine ARGact: finds the activated droplet number from Abdul_Razzak and Ghan 2000.
!      ! Tailored for GOCART AEROSOL and works with AERO input 
...
!, QLO, QIO, CLAN
    REAL*8, DIMENSION(idim, k0), INTENT(INOUT) :: tho, qho, uho, vho
    REAL*8, DIMENSION(idim, k0), INTENT(INOUT) :: tho_tl, qho_tl, uho_tl&
&   , vho_tl
!,PHIE
    REAL*8, DIMENSION(idim, k0 + 1), INTENT(IN) :: ple, pke
!,PLO,PKO,PHIO
    REAL*8, DIMENSION(idim, k0), INTENT(IN) :: qss, dqs
    REAL*8, DIMENSION(idim, k0), INTENT(IN) :: qss_tl, dqs_tl
    REAL*8, DIMENSION(idim), INTENT(IN) :: cnv_fraction, rasal2_2d
    REAL*8, DIMENSION(k0 + 1), INTENT(IN) :: sige
!, FLX
    REAL*8, DIMENSION(idim, k0), INTENT(OUT) :: clw, flxd
    REAL*8, DIMENSION(idim, k0), INTENT(OUT) :: clw_tl, flxd_tl
!      REAL(8), DIMENSION (IDIM,K0+1), INTENT(  OUT) ::  FLXC
    REAL*8, DIMENSION(idim, k0), INTENT(OUT) :: cnv_prc3
    REAL*8, DIMENSION(idim, k0), INTENT(OUT) :: cnv_prc3_tl
!, CNV_CVW, CNV_QC
    REAL*8, DIMENSION(idim, k0), INTENT(OUT) :: cnv_updfrc
    REAL*8, DIMENSION(idim, k0), INTENT(OUT) :: cnv_updfrc_tl
!      REAL(8), DIMENSION (IDIM,K0  ), INTENT(  OUT) ::  ENTLAM
!      REAL(8), DIMENSION (IDIM,K0  ), INTENT(  OUT) ::  HHO, HSO
    REAL*8, INTENT(IN) :: dt, cpo, alhlo, gravo
    REAL*8, INTENT(IN) :: alhl1, tice
    ...
    INTEGER, DIMENSION(idim), INTENT(IN) :: kcbl
    REAL*8, DIMENSION(idim), INTENT(IN) :: tpert, qpert
    REAL*8, DIMENSION(idim), INTENT(IN) :: tpert_tl
    REAL*8, DIMENSION(idim), INTENT(IN) :: co_auto
    REAL*8, DIMENSION(idim) :: mxdiam
!      REAL(8), DIMENSION (IDIM     ), INTENT(  OUT) ::  PRECU
    REAL*8, DIMENSION(idim, k0), INTENT(IN) :: wgt0, wgt1
...
!  LOCALS
    REAL*8, DIMENSION(k0) :: poi_sv, qoi_sv, uoi_sv, voi_sv
    REAL*8, DIMENSION(k0) :: poi_sv_tl, qoi_sv_tl, uoi_sv_tl, voi_sv_tl
    REAL*8, DIMENSION(k0) :: poi, qoi, uoi, voi, dqq, bet, gam, cll, &
&   tmpt
    REAL*8, DIMENSION(k0) :: poi_tl, qoi_tl, uoi_tl, voi_tl, dqq_tl, &
&   bet_tl, gam_tl, cll_tl, tmpt_tl
    REAL*8, DIMENSION(k0) :: poi_c, qoi_c
    REAL*8, DIMENSION(k0) :: poi_c_tl, qoi_c_tl
    REAL*8, DIMENSION(k0) :: prh, pri, ght, dpt, dpb, pki, dissk0, &
&   dissk1, clantnd
    REAL*8, DIMENSION(k0) :: ght_tl
    REAL*8, DIMENSION(k0) :: tcu, qcu, ucu, vcu, cln, rns, pol, dm
    REAL*8, DIMENSION(k0) :: ucu_tl, vcu_tl, rns_tl
    REAL*8, DIMENSION(k0) :: qst, ssl, rmf, rnn, rn1, rmfc, rmfp
    REAL*8, DIMENSION(k0) :: qst_tl, ssl_tl, rnn_tl, rmfp_tl
    REAL*8, DIMENSION(k0) :: gms, eta, gmh, eht, gm1, hcc, rmfd
    REAL*8, DIMENSION(k0) :: gms_tl, eta_tl, gmh_tl, eht_tl, gm1_tl, &
&   hcc_tl, rmfd_tl
    REAL*8, DIMENSION(k0) :: hol, hst, qol, zol, hcld, cll0, cllx, clli&
&   , cllb
    REAL*8, DIMENSION(k0) :: hol_tl, hst_tl, qol_tl, zol_tl, hcld_tl, &
&   cll0_tl
    REAL*8, DIMENSION(k0) :: wsp, lambdsv, bke, cvw, updfrc
    REAL*8, DIMENSION(k0) :: cvw_tl, updfrc_tl
    REAL*8, DIMENSION(k0) :: rasal, mtkwi, updfrp, bk2, bk3, dll0, dllx
    REAL*8, DIMENSION(k0) :: rasal_tl, updfrp_tl, bk2_tl
    REAL*8, DIMENSION(itrcr) :: xht
    REAL*8, DIMENSION(k0, itrcr) :: xoi, xcu, xoi_sv
    REAL*8, DIMENSION(k0 + 1) :: prj, prs, qht, sht, zet, xyd, xyd0
    REAL*8, DIMENSION(k0+1) :: qht_tl, sht_tl, zet_tl
!      INTEGER,  DIMENSION(K0-1) :: RC
    INTEGER :: k, my_pe
    REAL*8, DIMENSION(idim, k0) :: lambdsv2
    REAL*8 :: tx2, tx3, uht, vht, akm, acr, alm, tth, qqh, shtrg, wspbl&
&   , dqx
    REAL*8 :: tx2_tl, tx3_tl, uht_tl, vht_tl, akm_tl, alm_tl, dqx_tl
!, BKE
    REAL*8 :: wfn, tem, trg, trgexp, evp, wlq, qcc, mtkw_max
    REAL*8 :: wfn_tl, tem_tl, trg_tl, wlq_tl, qcc_tl
    REAL*8 :: shtrg_fac, sige_minhol, wfnog
    INTEGER :: i, ic, l, icl, itr, icl_c, n_dtl
    INTEGER :: ndtlexpon
    INTEGER, DIMENSION(:), ALLOCATABLE :: icl_v
...
!   variations   of useful results: eht hcc cvw ucu hst qht qoi
!                qol voi bk2 rasal hcld eta sht gmh rnn qst gms
!                updfrc poi rns uoi vcu rmfd ssl zet updfrp zol
!                cll0 hol cll rmfp
...
!, dQx
      REAL*8 :: cli, te_a, c00_x, cli_crit_x, pete, toki, gmhx, hstx
      REAL*8 :: cli_tl, te_a_tl, c00_x_tl, cli_crit_x_tl, toki_tl, &
&     gmhx_tl, hstx_tl
      REAL*8 :: dt_lyr, rate, cvw_x, closs, f2, f3, f4, f5
      REAL*8 :: dt_lyr_tl, rate_tl, cvw_x_tl, closs_tl, f2_tl
      INTEGER :: k700
      INTRINSIC MIN
      INTRINSIC MAX
      INTRINSIC SQRT
...
!  RECOMPUTE SOUNDING UP TO DETRAINMENT LEVEL
        ...
        qoi_c_tl = qoi_tl
        qoi_c = qoi
        poi_c_tl(k) = poi_c_tl(k) + tpert_tl(i)
        poi_c(k) = poi_c(k) + tpert(i)
        qoi_c(k) = qoi_c(k) + qpert(i)
        zet_tl(k+1) = 0.0_8
        zet(k+1) = 0.
...
!   ALPHA CALCULATION 
              ...
              IF (zet(ic) .GE. 2000.) THEN
                ...
                END IF
!WMP     RASAL(IC) = RASAL1 + (RASAL2i-RASAL1)*(ZET(IC) - 2000.)/8000.
                IF (min2 .GT. 0.0_8 .OR. (min2 .LT. 0.0_8 .AND. &
&                   rasal_exp .EQ. INT(rasal_exp))) THEN
                  pwr1_tl = rasal_exp*min2**(rasal_exp-1)*min2_tl
                ELSE IF (min2 .EQ. 0.0_8 .AND. rasal_exp .EQ. 1.0) THEN
                  pwr1_tl = min2_tl
                ELSE
                  pwr1_tl = 0.0_8
                END IF
                pwr1 = min2**rasal_exp
                rasal_tl(ic) = (rasal2i-rasal1)*pwr1_tl
                rasal(ic) = rasal1 + (rasal2i-rasal1)*pwr1
              END IF
!WMP  RASAL(IC) = MIN( RASAL(IC) , 1.0e5 )
...
&                     cli_crit_x*cli_crit_x_tl)/(cli_crit_x**2)**2)
                    arg1 = -(cli**2/cli_crit_x**2)
                    rate_tl = c00_x_tl*(1.0-EXP(arg1)) - c00_x*arg1_tl*&
&                     EXP(arg1)
                    rate = c00_x*(1.0-EXP(arg1))
                  END IF
                  IF (cvw(l) .LT. 1.00) THEN
...
! MS-A39 MASS-FLUX IN Pa/step
                  ...
                  wfn = -((wfn-acr)/akm)
                  x3_tl = (rasal_tl(ic)*wfn+rasal(ic)*wfn_tl)*trg*toki +&
&                   rasal(ic)*wfn*(trg_tl*toki+trg*toki_tl)
                  x3 = rasal(ic)*trg*toki*wfn
                  IF (x3 .GT. (prs(k+1)-prs(k))*(100.*pblfrac)) THEN
                    wfn = (prs(k+1)-prs(k))*(100.*pblfrac)
                    wfn_tl = 0.0_8
...
! Do not believe it actually makes any difference.
        ...
        arg1(:) = (uoi(icmin:k)-uoi(k))**2 + (voi(icmin:k)-voi(k))**2
        wsp(icmin:k) = SQRT(arg1(:))
        qst_tl(icmin:k) = qss_tl(i, icmin:k)
        qst(icmin:k) = qss(i, icmin:k)
        dqq_tl(icmin:k) = dqs_tl(i, icmin:k)
        dqq(icmin:k) = dqs(i, icmin:k)
        IF (do_tracers) THEN
          DO itr=1,itrcr
            xoi(icmin:k, itr) = xho(i, icmin:k, itr)
...
!! SPECIFY WEIGHTS GIVEN TO EACH LAYER WITHIN SUBCLOUD "SUPERLAYER"
          ...
          IF (do_tracers) THEN
            ...
            END DO
          END IF
          tmpt_tl(k) = prh(k)*poi_tl(k)
          tmpt(k) = poi(k)*prh(k)
          CALL DQSATPERT_TLM(dqq(k), dqq_tl(k), qst(k), qst_tl(k), tmpt(&
&                      k), tmpt_tl(k), pol(k), 1)
        END IF
!!DPTH_BL = CPBG*POI(K)*( PRJ(K+1)-PRJ(K) )
! seedras(1,2) are both integers passed from
...
!      REAL(8),    intent(inout) :: ran(npoints)
!
...
!
!      !This calculations are needed since we don't have explicit microphysics in RAS. All Fs are tendencies
!
!      REAL(8), INTENT( IN) ::  FICE_,  TE, PRE, QC, VCNx, NI, NL, DTL, COAUTO
!      REAL(8), INTENT (INOUT) :: RIM
!      REAL(8), INTENT(OUT) :: RATE_Q, F_NL, F_NI, FPICE_
!
!      REAL(8) :: Dc_r, Dc_g, Dc_i, lamr, lamg, lami, fqi, &
...
!      end if
!
!      !COAUTO tuning constant since we don't consider accretion by precip
! fng=min(fng, 1.0/DTL)
! fpg=min(fpg, 1.0/DTL)
! fni=min(fni, 1.0/DTL)
! fpi=min(fpi, 1.0/DTL)
...
!      REAL(8)           :: ICEFRCT, T_ICE_ALL, T_ICE_MAX
!
!      T_ICE_ALL=238.0
!      T_ICE_MAX=273.0
!
...
!
!   end function ERFAPP
  SUBROUTINE RASE_FAST(idim, irun, k0, icmin, dt, cpo, alhlo, alhl1, &
&   tice, gravo, seedras, sige, kcbl, wgt0, wgt1, tpert, qpert, tho, qho&
&   , qss, dqs, cnv_fraction, rasal2_2d, co_auto, ple, pke, rasparams)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: idim, irun, k0, icmin
...
!  LOCALS
    ...
    REAL*8, DIMENSION(k0) :: tcu, qcu, ucu, vcu, cln, rns, pol, dm
    REAL*8, DIMENSION(k0) :: qst, ssl, rmf
    REAL*8, DIMENSION(k0) :: gms, eta, gmh, eht, gm1, hcc
    REAL*8, DIMENSION(k0) :: hol, hst, qol, zol, hcld, cll0
    REAL*8, DIMENSION(k0) :: bke, cvw
    REAL*8, DIMENSION(k0) :: rasal, mtkwi, bk2, bk3, dll0
    REAL*8, DIMENSION(k0 + 1) :: prj, prs, qht, sht, zet, xyd, xyd0
    INTEGER :: k, my_pe
    REAL*8, DIMENSION(idim, k0) :: lambdsv2
    REAL*8 :: tx2, tx3, akm, acr, alm, tth, qqh, shtrg, wspbl, dqx
    REAL*8 :: wfn, tem, trg, trgexp, evp, wlq, qcc, mtkw_max
    REAL*8 :: shtrg_fac, sige_minhol, wfnog
    INTEGER :: i, ic, l, icl, itr, icl_c, n_dtl
    INTEGER, DIMENSION(:), ALLOCATABLE :: icl_v
...
&           prs(l)))
        ...
        IF (k .LE. k0) THEN
          ...
          DO l=k,k0
            ...
            qoi(k) = qoi(k) + wght(l)*qho(i, l)
          END DO
          CALL DQSATPERT(dqq(k), qst(k), poi(k)*prh(k), pol(k), 1)
        END IF
!dh tap produces code with error
        DO jj=1,nrands
          IF (seedras(i, 1)/1000000. .LT. 1e-6) THEN
...
&             )/(alhl*(1.0+lbcp*dqq(l+1))))
      ...
      RETURN
    END SUBROUTINE STRAP
  END SUBROUTINE RASE_FAST
!  Differentiation of rase_fast in forward (tangent) mode (with options r8):
!   variations   of useful results: tho qho
!   with respect to varying inputs: tho dqs qho tpert qss
!   subroutine congvec( npoints , seed, ran )
!
!      INTEGER, intent(in   ) :: npoints
!      INTEGER, intent(inout) :: seed(npoints)
...
!
!   end function ERFAPP
  SUBROUTINE RASE_FAST_TLM(idim, irun, k0, icmin, dt, cpo, alhlo, alhl1&
&   , tice, gravo, seedras, sige, kcbl, wgt0, wgt1, tpert, tpert_tl, &
&   qpert, tho, tho_tl, qho, qho_tl, qss, qss_tl, dqs, dqs_tl, &
&   cnv_fraction, rasal2_2d, co_auto, ple, pke, rasparams)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: idim, irun, k0, icmin
    REAL*8, DIMENSION(idim, k0), INTENT(INOUT) :: tho, qho
    REAL*8, DIMENSION(idim, k0), INTENT(INOUT) :: tho_tl, qho_tl
    REAL*8, DIMENSION(idim, k0 + 1), INTENT(IN) :: ple, pke
    REAL*8, DIMENSION(idim, k0), INTENT(IN) :: qss, dqs
    REAL*8, DIMENSION(idim, k0), INTENT(IN) :: qss_tl, dqs_tl
    REAL*8, DIMENSION(idim), INTENT(IN) :: cnv_fraction, rasal2_2d
...
!  LOCALS
    REAL*8, DIMENSION(k0) :: poi_sv, qoi_sv
    REAL*8, DIMENSION(k0) :: poi_sv_tl, qoi_sv_tl
    REAL*8, DIMENSION(k0) :: poi, qoi, dqq, bet, gam, cll
    REAL*8, DIMENSION(k0) :: poi_tl, qoi_tl, dqq_tl, bet_tl, gam_tl
    REAL*8, DIMENSION(k0) :: poi_c, qoi_c
    REAL*8, DIMENSION(k0) :: poi_c_tl, qoi_c_tl
    REAL*8, DIMENSION(k0) :: prh, pri, ght, dpt, dpb, pki
    REAL*8, DIMENSION(k0) :: ght_tl
    REAL*8, DIMENSION(k0) :: tcu, qcu, ucu, vcu, cln, rns, pol, dm
    REAL*8, DIMENSION(k0) :: qst, ssl, rmf
    REAL*8, DIMENSION(k0) :: qst_tl, ssl_tl
    REAL*8, DIMENSION(k0) :: gms, eta, gmh, eht, gm1, hcc
    REAL*8, DIMENSION(k0) :: gms_tl, eta_tl, gmh_tl, eht_tl, hcc_tl
    REAL*8, DIMENSION(k0) :: hol, hst, qol, zol, hcld, cll0
    REAL*8, DIMENSION(k0) :: hol_tl, hst_tl, qol_tl, zol_tl
    REAL*8, DIMENSION(k0) :: bke, cvw
    REAL*8, DIMENSION(k0) :: rasal, mtkwi, bk2, bk3, dll0
    REAL*8, DIMENSION(k0) :: rasal_tl
    REAL*8, DIMENSION(k0 + 1) :: prj, prs, qht, sht, zet, xyd, xyd0
    REAL*8, DIMENSION(k0+1) :: qht_tl, sht_tl, zet_tl
    INTEGER :: k, my_pe
    REAL*8, DIMENSION(idim, k0) :: lambdsv2
    REAL*8 :: tx2, tx3, akm, acr, alm, tth, qqh, shtrg, wspbl, dqx
    REAL*8 :: tx2_tl, akm_tl, alm_tl
    REAL*8 :: wfn, tem, trg, trgexp, evp, wlq, qcc, mtkw_max
    REAL*8 :: wfn_tl, tem_tl, trg_tl
    REAL*8 :: shtrg_fac, sige_minhol, wfnog
    INTEGER :: i, ic, l, icl, itr, icl_c, n_dtl
    INTEGER, DIMENSION(:), ALLOCATABLE :: icl_v
    REAL*8 :: grav, cp, alhl, cpbg, alhi, cpi, gravi, ddt, lbcp, obg, &
...
! correct.
  ...
  CONTAINS
!  Differentiation of cloude in forward (tangent) mode (with options r8):
!   variations   of useful results: eht hcc hst qht qoi qol rasal
!                eta sht gmh qst gms poi ssl zet zol hol
!   with respect to varying inputs: tpert eht hcc ght hst bet qht
!                qoi qol rasal eta sht gmh qst gms poi ssl zet
!                zol hol gam
    SUBROUTINE CLOUDE_TLM(ic)
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: ic
...
! MS-A39 MASS-FLUX IN Pa/step
                  wfn_tl = -((wfn_tl*akm-(wfn-acr)*akm_tl)/akm**2)
                  wfn = -((wfn-acr)/akm)
                  x1_tl = (rasal_tl(ic)*wfn+rasal(ic)*wfn_tl)*trg*toki +&
&                   rasal(ic)*wfn*(trg_tl*toki+trg*toki_tl)
                  x1 = rasal(ic)*trg*toki*wfn
                  IF (x1 .GT. (prs(k+1)-prs(k))*(100.*pblfrac)) THEN
                    wfn = (prs(k+1)-prs(k))*(100.*pblfrac)
...
&       0.0553, 0.0445, 0.0633/)
    ...
    END SUBROUTINE ACRITN
!  Differentiation of rnevp in forward (tangent) mode (with options r8):
!   variations   of useful results: zet
!   with respect to varying inputs: poi zet
    SUBROUTINE RNEVP_TLM()
      IMPLICIT NONE
      zet_tl(k+1) = 0.0_8
      zet(k+1) = 0
...
!            N_DTL = min( int( RASNCL ) , K-ICMIN )
    ...
    END SUBROUTINE FINDDTLS
!  Differentiation of strap in forward (tangent) mode (with options r8):
!   variations   of useful results: tho qho dqq ght bet qoi_sv
!                qoi qst poi poi_sv gam
!   with respect to varying inputs: tho dqs qho qss dqq ght bet
!                qoi_sv qoi qst poi poi_sv gam
    SUBROUTINE STRAP_TLM(final)
      IMPLICIT NONE
      INTEGER :: final
      REAL*8, DIMENSION(k0) :: wght, massf
...
&           prs(l)))
        ...
        IF (k .LE. k0) THEN
          ...
          DO l=k,k0
            ...
            qoi(k) = qoi(k) + wght(l)*qho(i, l)
          END DO
          CALL DQSATPERT_TLM(dqq(k), dqq_tl(k), qst(k), qst_tl(k), poi(k&
&                      )*prh(k), prh(k)*poi_tl(k), pol(k), 1)
        END IF
!dh tap produces code with error
        DO jj=1,nrands
          IF (seedras(i, 1)/1000000. .LT. 1e-6) THEN
