...
!  dT_out_ad	- temperature perturbation
!  dQ_out_ad	- moisture perturbation
!  dCWM_out_ad  - cloud condesate mixing ratio perturbation
!  adjoint	- logical flag (.false.=forward model only, .true.=forward and ajoint)

!OUTUT ARGUMENT DESCRIPTIONS, THE FOLLOWING COME IN AS ZEROS
...
!  dT_in_ad	- change in condensation with respect to temperature
!  dQ_in_ad	- change in condensation with respect to moisture
!  dCWM_in_ad	- change in condensation with respect to cloud condensation mixing ratio
!  dadvT_ad	- temperature advection perturbation
!  dadvQ_ad	- moisture advection perturbation
!  dadvP_ad	- pressure advection perturbation

!DEFINE CONSTANTS WITHOUT ACCESSING THE NCEP/GSI CONSTANTS MODULE
...
real, parameter :: EPSM1   =  rd/rv-one
real, parameter :: epsq    =  2.e-12
real, parameter :: cpr     =  cp*rd
real, parameter :: rcp     =  one/cp
real, parameter :: el2orc  =  hvap*hvap/(rv*cp)
real, parameter :: climit  =  1.e-20
real, parameter :: cclimit =  0.001
real, parameter :: hsub    =  hvap+hfus
real, parameter :: ttp     =  2.7316e+2
real, parameter :: elocp   =  hvap/cp
real, parameter :: ELIV    =  HVAP + HFUS 
real, parameter :: ELWV    =  HVAP 
real, parameter :: ELIW    =  ELIV - ELWV

real, parameter :: tiny_r_kind = tiny(zero)
...
REAL         :: TIK, TMT0, X, Y, QIK, CWMIK, esk, QW, &
...
REAL         :: dCONE0_ad,dCOND_ad, dCOND1_ad,dCOND2_ad, dCOND3_ad,dE0_ad,dE1_ad, dE2_ad,dE3_ad,dE4_ad, &
                dQIK_ad, dQC_ad, &
...
REAL         :: des1_ad, des2_ad, dqs2_des2, des3_ad, dqs3_des3, dqs1_des1, desk_ad

real         :: rfac, fac

REAL, dimension(IX,KM) :: T_in, Q_in, CWM_in, PRSL, SL, &
                  IW,xIW,  RHC, advT, advQ, advP, T_out, Q_out, CWM_out, dIW

REAL, dimension(IX,KM) :: dT_out_ad, dT_in_ad, &
                                  dQ_out_ad, dQ_in_ad,  &
                                  dCWM_out_ad, dCWM_in_ad, dIW_ad, &
                                  dT_out_ad0, dQ_out_ad0, dCWM_out_ad0
!REAL, dimension(IX,KM) :: dT_tend, dQ_tend, dCWM_tend
REAL, dimension(IX,KM) :: dadvT_ad, dadvQ_ad, dadvP_ad
REAL :: dAT_ad, dAQ_ad, dAP_ad
REAL :: qw_tmp1,ccrik_tmp1

REAL, dimension(IX) :: PS, QI, QINT
REAL :: dum, CWM_out_save1
!-------------------------------
RDT=ONE/DT
DO I = 1, IM
 IW(I,KM)=ZERO
