...
!-----------------------------------------------------------------------
! dyn_aux:
    real(p_precision), intent(inout) :: ps  (isd:ied  ,jsd:jed)           ! Surface pressure (pascal)
    real(p_precision), intent(inout) :: pe  (is-1:ie+1, npz+1,js-1:je+1)  ! edge pressure (pascal)
    real(p_precision), intent(inout) :: pk  (is:ie,js:je, npz+1)          ! pe**cappa
    real(p_precision), intent(inout) :: peln(is:ie,npz+1,js:je)           ! ln(pe)
    real(p_precision), intent(inout) :: pkz (is:ie,js:je,npz)             ! finite-volume mean pk
...
!     real te_den
      ...
      real :: q_dh(   isd:ied  ,jsd:jed  ,npz, ncnst)
...
!q_dh = q
         ...
         call Lagrangian_to_Eulerian(last_step, consv_te, ps, pe, delp,  &
                      ...
                      nq, sphum, u,  v, w, delz, pt, q, phis, zvir, cp_air, akap,  &
#ifdef MAPL_MODE
                     pi, radius, grav, &
#endif
                     kord_mt, kord_tr, kord_tm, peln, te_2d,  &
                     ng, ua, va, omga, dp1, pem, fill, reproduce_sum, &
                     ak, bk, ks, ze0, te_method, remap_t, hydrostatic, hybrid_z, last_step, k_top, ncnst, &
                     mfx=mfx, mfy=mfy)
