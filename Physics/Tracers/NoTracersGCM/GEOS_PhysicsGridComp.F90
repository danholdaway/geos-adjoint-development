! $Id: GEOS_PhysicsGridComp.F90,v 1.118 2014-03-02 15:32:01 ltakacs Exp $
...
! Ozone and Oxygen volume mixing ratio in ppmv (not mol/mol)
! ----------------------------------------------------------
    call MAPL_AddExportSpec ( GC   ,                               &
         SHORT_NAME = 'O3PPMV',                                    &
         CHILD_ID   = CHEM,                                        &
...
! Local derived type aliases
   ...
   real, pointer, dimension(:,:,:)     :: QV, QLLS, QLCN, QILS, QICN
...
#ifdef _CUDA
   integer, parameter :: REAL_DE = GPU_PRECISION
#else
   integer, parameter :: REAL_DE = MAPL_R8
#endif
   real(kind=REAL_DE), allocatable, dimension(:,:) :: sumdq
   real(kind=REAL_DE), allocatable, dimension(:,:) ::  dwdt
...
!---------------------------------------------------------------
    ...
    if( DPEDT_PHYS ) then
       ...
       do l=1,lm
              dwdt =        DQIDTTRB(:,:,L)
              dwdt = dwdt + DQIDTMST(:,:,L)
              dwdt = dwdt + DQLDTTRB(:,:,L)
              dwdt = dwdt + DQLDTMST(:,:,L)
              dwdt = dwdt + DQVDTCHM(:,:,L)
              dwdt = dwdt + DQVDTTRB(:,:,L)
              dwdt = dwdt + DQVDTMST(:,:,L)

             sumdq = sumdq + dwdt*dm(:,:,L)
       dpdt(:,:,L) = sumdq

         dwdt = 1.0_8/( 1.0_8 + DT*dwdt )

         ! Modify Specific Masses of Advected Mass Tracers with Updated Total Pressures
         ! ----------------------------------------------------------------------------
         do N=1,NQ
...
!            PTR3D(:,:,L) = PTR3d(:,:,L)*dwdt
    ...
    if(associated(DMDT)) DMDT(:,:) = DPDT(:,:,LM)
                         DPDT      = DPDT*MAPL_GRAV
