...
!Compute the FV variables internally, for checkpointing purposes
    CALL PERTSTATE2FVSTATE_TLM(delp, delp_tl, pe, pe_tl, pk, pkz, pkz_tl&
&                        , peln, peln_tl, ptop, pt, pt_tl, isd, ied, jsd&
...
!           endif
!#endif
      IF (tau .GT. 0.) CALL RAYLEIGH_FRICTION_TLM(bdt, npx, npy, npz, ks&
&                                           , pfull, tau, rf_center, u, &
&                                           u_tl, v, v_tl, w, w_tl, pt, &
&                                           pt_tl, ua, ua_tl, va, va_tl&
&                                           , delz, delz_tl, cp_air, rg&
&                                           , hydrostatic, .true.)
...
&               pm(k))*(1.-1./(1.+dt*rf(k)*result1)**2)
  ...
  SUBROUTINE PERTSTATE2FVSTATE_TLM(delp, delp_tl, pe, pe_tl, pk, pkz, &
&   pkz_tl, peln, peln_tl, ptop, pt, pt_tl, isd, ied, jsd, jed, is, ie, &
&   js, je, km, kappa)
    IMPLICIT NONE
...
!  Differentiation of fvstate2pertstate in forward (tangent) mode (with options r8):
