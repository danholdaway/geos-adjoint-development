...
!Compute the FV variables internally, for checkpointing purposes
    CALL PUSHREAL4ARRAY(pt, (ied-isd+1)*(jed-jsd+1)*npz)
...
!#endif
      ...
      IF (tau .GT. 0.) THEN
        ...
        CALL PUSHREAL4ARRAY(delz, (ie-is+1)*(je-js+1)*npz)
        CALL PUSHREAL4ARRAY(va, (ied-isd+1)*(jed-jsd+1)*npz)
        CALL PUSHREAL4ARRAY(ua, (ied-isd+1)*(jed-jsd+1)*npz)
        CALL PUSHREAL4ARRAY(pt, (ied-isd+1)*(jed-jsd+1)*npz)
        CALL PUSHREAL4ARRAY(w, (ied-isd+1)*(jed-jsd+1)*npz)
        CALL PUSHREAL4ARRAY(v, (ied-isd+2)*(jed-jsd+1)*npz)
        CALL PUSHREAL4ARRAY(u, (ied-isd+1)*(jed-jsd+2)*npz)
...
!$omp parallel do default(shared) private(i, j, k)
    ...
    CALL PUSHREAL8ARRAY(peln, (ie-is+1)*(npz+1)*(je-js+1))
    CALL PUSHREAL8ARRAY(pkz, (ie-is+1)*(je-js+1)*npz)
    CALL PUSHREAL4ARRAY(pem, (ie-is+3)*(npz+1)*(je-js+3))
    CALL PUSHREAL4ARRAY(vc, (ied-isd+1)*(jed-jsd+2)*npz)
    CALL PUSHREAL4ARRAY(uc, (ied-isd+2)*(jed-jsd+1)*npz)
    CALL PUSHREAL4ARRAY(va, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL PUSHREAL4ARRAY(ua, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL PUSHREAL4ARRAY(omga, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL PUSHREAL8ARRAY(pe, (ie-is+3)*(npz+1)*(je-js+3))
    CALL PUSHREAL4ARRAY(delp, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL PUSHREAL4ARRAY(q, (ied-isd+1)*(jed-jsd+1)*npz*ncnst)
    CALL PUSHREAL4ARRAY(pt, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL PUSHREAL4ARRAY(w, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL PUSHREAL4ARRAY(vm, (ied-isd+2)*(jed-jsd+1)*npz)
    CALL PUSHREAL4ARRAY(um, (ied-isd+1)*(jed-jsd+2)*npz)
    CALL PUSHREAL4ARRAY(v, (ied-isd+2)*(jed-jsd+1)*npz)
    CALL PUSHREAL4ARRAY(u, (ied-isd+1)*(jed-jsd+2)*npz)
...
! re-dimensioned as q(i,j,nq,k)
            CALL PUSHREAL4ARRAY(dp1(is, js, k), ie - is + 1)
            CALL PUSHREAL4ARRAY(q2, (ied-isd+1)*(jed-jsd+1)*nq)
            CALL TRACER_2D_1L(q2, dp1(is, js, k), mfx(is, js, k), mfy(is&
&                       , js, k), cx(is, jsd, k), cy(isd, js, k), npx, &
&                       npy, npz, nq, hord_tr_pert, q_split, k, q, mdt, &
&                       id_divg)
          END DO
...
!   enddo
! endif
          CALL PUSHREAL4ARRAY(cy, (ied-isd+1)*(je-js+2)*npz)
          CALL PUSHREAL4ARRAY(cx, (ie-is+2)*(jed-jsd+1)*npz)
          CALL PUSHREAL4ARRAY(mfy, (ie-is+1)*(je-js+2)*npz)
          CALL PUSHREAL4ARRAY(mfx, (ie-is+2)*(je-js+1)*npz)
          CALL PUSHREAL4ARRAY(dp1, (ie-is+1)*(je-js+1)*npz)
          CALL PUSHREAL4ARRAY(q, (ied-isd+1)*(jed-jsd+1)*npz*ncnst)
...
!#endif
        ...
        CALL PUSHREAL4ARRAY(v, (ied-isd+2)*(jed-jsd+1)*npz)
        CALL PUSHREAL4ARRAY(u, (ied-isd+1)*(jed-jsd+2)*npz)
        CALL PUSHREAL8ARRAY(pk, (ie-is+1)*(je-js+1)*(npz+1))
        CALL PUSHREAL8ARRAY(pkz, (ie-is+1)*(je-js+1)*npz)
        CALL PUSHREAL4ARRAY(delp, (ied-isd+1)*(jed-jsd+1)*npz)
        CALL PUSHREAL8ARRAY(pe, (ie-is+3)*(npz+1)*(je-js+3))
        CALL LAGRANGIAN_TO_EULERIAN(last_step, consv_te, pe, delp, pkz, &
&                             pk, bdt, npz, is, ie, js, je, isd, ied, &
&                             jsd, jed, nq, sphum, u, v, pt, q, phis, &
&                             zvir, cp_air, akap, pi, radius, grav, &
&                             kord_mt, kord_tr, kord_tm, peln, te_2d, ng&
&                             , ua, dp1, pem, fill, reproduce_sum, ak, &
&                             bk, ks, te_method, remap_t, ncnst)
!      if ( fv_debug ) then
...
&             , k)*pkz(i, j, k)*zvir*temp_ad0/temp1
    ...
    ELSE
      IF (branch .EQ. 1) THEN
        ...
        CALL POPREAL4ARRAY(delp, (ied-isd+1)*(jed-jsd+1)*npz)
        CALL POPREAL8ARRAY(pkz, (ie-is+1)*(je-js+1)*npz)
        CALL POPREAL8ARRAY(pk, (ie-is+1)*(je-js+1)*(npz+1))
        CALL POPREAL4ARRAY(u, (ied-isd+1)*(jed-jsd+2)*npz)
        CALL POPREAL4ARRAY(v, (ied-isd+2)*(jed-jsd+1)*npz)
        CALL POPREAL4ARRAY(pt, (ied-isd+1)*(jed-jsd+1)*npz)
        CALL POPREAL4ARRAY(q, (ied-isd+1)*(jed-jsd+1)*npz*ncnst)
        CALL POPREAL8ARRAY(peln, (ie-is+1)*(npz+1)*(je-js+1))
        CALL POPREAL4ARRAY(ua, (ied-isd+1)*(jed-jsd+1)*npz)
        CALL POPREAL4ARRAY(dp1, (ie-is+1)*(je-js+1)*npz)
!        CALL LAGRANGIAN_TO_EULERIAN_ADM(last_step, consv_te, pe, pe_ad, &
!&                                 delp, delp_ad, pkz, pkz_ad, pk, pk_ad&
!&                                 , bdt, npz, is, ie, js, je, isd, ied, &
!&                                 jsd, jed, nq, sphum, u, u_ad, v, v_ad&
!&                                 , pt, pt_ad, q, q_ad, phis, zvir, &
!&                                 cp_air, akap, pi, radius, grav, &
!&                                 kord_mt, kord_tr, kord_tm, peln, &
!&                                 peln_ad, te_2d, te_2d_ad, ng, ua, &
!&                                 ua_ad, dp1, dp1_ad, pem, fill, &
!&                                 reproduce_sum, ak, bk, ks, te_method, &
!&                                 remap_t, ncnst)
      ELSE
        ua_ad = 0.0
        pk_ad = 0.0_8
      ...
      IF (branch .LT. 2) THEN
        IF (branch .NE. 0) THEN
          q2_ad = 0.0
          DO k=npz,1,-1
            CALL POPREAL4ARRAY(q2, (ied-isd+1)*(jed-jsd+1)*nq)
            CALL POPREAL4ARRAY(dp1(is, js, k), ie - is + 1)
!            CALL TRACER_2D_1L_ADM(q2, q2_ad, dp1(is:, js, k), dp1_ad(is:&
!&                           , js, k), mfx(is, js, k), mfx_ad(is, js, k)&
!&                           , mfy(is, js, k), mfy_ad(is, js, k), cx(is, &
!&                           jsd, k), cx_ad(is, jsd, k), cy(isd, js, k), &
!&                           cy_ad(isd, js, k), npx, npy, npz, nq, &
!&                           hord_tr_pert, q_split, k, q, q_ad, mdt, id_divg)
            DO iq=nq,1,-1
              DO j=je,js,-1
                DO i=ie,is,-1
...
&                   )
      ...
      ELSE IF (branch .EQ. 2) THEN
        CALL POPREAL4ARRAY(q, (ied-isd+1)*(jed-jsd+1)*npz*ncnst)
        CALL POPREAL4ARRAY(dp1, (ie-is+1)*(je-js+1)*npz)
        CALL POPREAL4ARRAY(mfx, (ie-is+2)*(je-js+1)*npz)
        CALL POPREAL4ARRAY(mfy, (ie-is+1)*(je-js+2)*npz)
        CALL POPREAL4ARRAY(cx, (ie-is+2)*(jed-jsd+1)*npz)
        CALL POPREAL4ARRAY(cy, (ied-isd+1)*(je-js+2)*npz)
!        CALL TRACER_2D_ADM(q, q_ad, dp1, dp1_ad, mfx, mfx_ad, mfy, &
!&                    mfy_ad, cx, cx_ad, cy, cy_ad, npx, npy, npz, nq, &
!&                    hord_tr_pert, q_split, mdt, id_divg)
      END IF
    END IF
    CALL POPREAL4ARRAY(u, (ied-isd+1)*(jed-jsd+2)*npz)
    CALL POPREAL4ARRAY(v, (ied-isd+2)*(jed-jsd+1)*npz)
    CALL POPREAL4ARRAY(um, (ied-isd+1)*(jed-jsd+2)*npz)
    CALL POPREAL4ARRAY(vm, (ied-isd+2)*(jed-jsd+1)*npz)
    CALL POPREAL4ARRAY(w, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL POPREAL4ARRAY(pt, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL POPREAL4ARRAY(q, (ied-isd+1)*(jed-jsd+1)*npz*ncnst)
    CALL POPREAL4ARRAY(delp, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL POPREAL8ARRAY(pe, (ie-is+3)*(npz+1)*(je-js+3))
    CALL POPREAL4ARRAY(omga, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL POPREAL4ARRAY(ua, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL POPREAL4ARRAY(va, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL POPREAL4ARRAY(uc, (ied-isd+2)*(jed-jsd+1)*npz)
    CALL POPREAL4ARRAY(vc, (ied-isd+1)*(jed-jsd+2)*npz)
    CALL POPREAL4ARRAY(pem, (ie-is+3)*(npz+1)*(je-js+3))
    CALL POPREAL8ARRAY(pkz, (ie-is+1)*(je-js+1)*npz)
    CALL POPREAL8ARRAY(peln, (ie-is+1)*(npz+1)*(je-js+1))
!    CALL DYN_CORE_ADM(npx, npy, npz, ng, sphum, nq, mdt, n_split, zvir, &
!&               cp_air, akap, rdgas, grav, hydrostatic, u, u_ad, v, v_ad&
!&               , um, um_ad, vm, vm_ad, w, w_ad, delz, pt, pt_ad, q, &
!&               q_ad, delp, delp_ad, pe, pe_ad, pk, pk_ad, phis, omga, &
!&               omga_ad, ptop, pfull, ua, ua_ad, va, va_ad, uc, uc_ad, &
!&               vc, vc_ad, mfx, mfx_ad, mfy, mfy_ad, cx, cx_ad, cy, &
!&               cy_ad, pem, pkz, pkz_ad, peln, peln_ad, ak, bk)
    DO k=npz,1,-1
      DO j=je,js,-1
        DO i=ie,is,-1
...
&             zvir*pt_ad(i, j, k)
      ...
      IF (branch .NE. 0) THEN
        ...
        CALL POPREAL4ARRAY(ua, (ied-isd+1)*(jed-jsd+1)*npz)
        CALL POPREAL4ARRAY(va, (ied-isd+1)*(jed-jsd+1)*npz)
        CALL POPREAL4ARRAY(delz, (ie-is+1)*(je-js+1)*npz)
...
!&                            , cp_air, rg, hydrostatic, .true.)
      END IF
      CALL POPCONTROL1B(branch)
!      IF (branch .EQ. 0) CALL COMPUTE_TOTAL_ENERGY_ADM(is, ie, js, je, &
!&                                                isd, ied, jsd, jed, npz&
!&                                                , u, u_ad, v, v_ad, w, &
!&                                                delz, delz_ad, pt, &
!&                                                pt_ad, delp, delp_ad, q&
!&                                                , q_ad, pe, pe_ad, peln&
!&                                                , peln_ad, phis, grav, &
!&                                                zvir, cp_air, rg, hlv, &
!&                                                te_2d, te_2d_ad, ua, &
...
!&                                                , hydrostatic, id_te)
    ...
    CALL POPREAL8ARRAY(peln, (ie-is+1)*(npz+1)*(je-js+1))
    CALL POPREAL4ARRAY(pt, (ied-isd+1)*(jed-jsd+1)*npz)
    CALL PERTSTATE2FVSTATE_ADM(delp, delp_ad, pe, pe_ad, pk, pk_ad, pkz&
&                        , pkz_ad, peln, peln_ad, ptop, pt, pt_ad, isd, &
...
!  Differentiation of rayleigh_friction in reverse (adjoint) mode:
...
&               pm(k))*(1.-1./(1.+dt*rf(k)*SQRT(u2f(i, j, k)/u000))**2)
    ...
    END DO
  END SUBROUTINE RAYLEIGH_FRICTION
!  Differentiation of pertstate2fvstate in reverse (adjoint) mode:
!   gradient     of useful results: peln delp pkz pe pk pt
!   with respect to varying inputs: peln delp pkz pe pt
!#endif
  SUBROUTINE PERTSTATE2FVSTATE_ADM(delp, delp_ad, pe, pe_ad, pk, pk_ad, &
&   pkz, pkz_ad, peln, peln_ad, ptop, pt, pt_ad, isd, ied, jsd, jed, is&
&   , ie, js, je, km, kappa)
    IMPLICIT NONE
...
!  Differentiation of fvstate2pertstate in reverse (adjoint) mode:
