module fv_mapz_mod
  ...
  public compute_total_energy, Lagrangian_to_Eulerian,    &
         rst_remap, mappm, E_Flux, ana_remap
...
! !INPUT PARAMETERS:
   ...
   real, intent(in):: hs(isd:ied,jsd:jed)  ! surface geopotential
   real(p_precision), intent(in)::   pe(is-1:ie+1,km+1,js-1:je+1) ! pressure at layer edges
   real(p_precision), intent(in):: peln(is:ie,km+1,js:je)  ! log(pe)
...
! !INPUT PARAMETERS:
      ...
      real(p_precision), intent(in) ::  pe2(i1:i2,kn+1)     ! hieght at layer edges 
                                               ! (from model top to bottom surface)
...
!
! !INTERFACE:
  subroutine map1_cubic_te ( km,   pe1,    q1,                 &
                             kn,   pe2,    q2,   i1, i2,       &
                             j,    ibeg, iend, jbeg, jend, iv,  kord)
      implicit none
...
! !LOCAL VARIABLES:
      ...
      real(p_precision)   logpl2(i1:i2,kn)
      real(p_precision)   dlogp1(i1:i2,km)
      real(p_precision)    vsum1(i1:i2)
      real(p_precision)    vsum2(i1:i2)
      real(p_precision)   am2,am1,ap0,ap1,P,PLP1,PLP0,PLM1,PLM2,DLP0,DLM1,DLM2
...
555   continue
 ...
 subroutine remap_2dr8(km,   pe1,   q1,        &
                       kn,   pe2,   q2,        &
                       i1,   i2,    iv,   kord )
   integer, intent(in):: i1, i2
   integer, intent(in):: iv               ! Mode: 0 ==  constituents 1 ==others
   ...
   integer, intent(in):: km               ! Original vertical dimension
   integer, intent(in):: kn               ! Target vertical dimension
   real(8), intent(in):: pe1(i1:i2,km+1)     ! pressure at layer edges 
                                          ! (from model top to bottom surface)
                                          ! in the original vertical coordinate
   real(8), intent(in):: pe2(i1:i2,kn+1)     ! pressure at layer edges 
                                          ! (from model top to bottom surface)
                                          ! in the new vertical coordinate
   real(8), intent(in) :: q1(i1:i2,km) ! Field input
   real(8), intent(out):: q2(i1:i2,kn) ! Field output
! !LOCAL VARIABLES:
   real(p_precision)   dp1(i1:i2,km)
   real(p_precision)   q4(4,i1:i2,km)
   real(p_precision)   pl, pr, qsum, dp, esl
...
!-----------------------------------------------------------------------
  real(p_precision) r_vir
  real(p_precision) ps(isd:ied,jsd:jed)  ! surface pressure
  real(p_precision)  pe1(is:ie,km+1)
  real(p_precision)  pe2(is:ie,kn+1)
  real(p_precision)  pv1(is:ie+1,km+1)
  real(p_precision)  pv2(is:ie+1,kn+1)
...
! !INPUT PARAMETERS:
  ...
  integer, intent(in):: is,ie,isd,ied         ! starting & ending X-Dir index
  integer, intent(in):: js,je,jsd,jed         ! starting & ending Y-Dir index
  real(8), intent(in) :: ak(kn+1)
  real(8), intent(in) :: bk(kn+1)
  real(8), intent(in), optional:: phis_r(is:ie,js:je)   ! topography
  real(8), intent(in):: delp_r(is:ie  ,js:je  ,km)      ! pressure thickness
  real(8), intent(in)::    u_r(is:ie  ,js:je+1,km)      ! u-wind (m/s)
  real(8), intent(in)::    v_r(is:ie+1,js:je  ,km)      ! v-wind (m/s)
  real(8), intent(in)::  ptv_r(is:ie  ,js:je  ,km)      ! virtual-potential temperature  
  real(8), intent(in)::    q_r(is:ie  ,js:je  ,km,nq)
! Input related to desired remapped output:
  real(8), intent(in), optional:: phis(is:ie,js:je)       ! topography (unghosted)
! Output:
  integer, intent(out) :: istatus
  real(8), intent(inout):: delp(isd:ied  ,jsd:jed  ,kn)   ! pressure thickness
  real(8), intent(out)::      u(isd:ied  ,jsd:jed+1,kn)   ! u-wind (m/s)
  real(8), intent(out)::      v(isd:ied+1,jsd:jed  ,kn)   ! v-wind (m/s)
  real(8), intent(out)::    ptv(isd:ied  ,jsd:jed  ,kn)   ! virtual-potential temperature
  real(8), intent(out)::      q(isd:ied  ,jsd:jed  ,kn,nq)
!-----------------------------------------------------------------------
  real(p_precision) r_vir,cp,kappa
  real(p_precision) ps_r(isd:ied,jsd:jed)  ! surface pressure
  real(p_precision) ps  (isd:ied,jsd:jed)  ! surface pressure
  real(p_precision)  pe1(is:ie,km+1)
  real(p_precision)  pe2(is:ie,kn+1)
  real(p_precision)  pv1(is:ie+1,km+1)
...
!------
     ...
     call remap_2dr8(km, pe1, u_r(is:ie,j:j,1:km),       &
                     kn, pe2,   u(is:ie,j:j,1:kn),       &
                     is, ie, -1, kord)
...
!----------------
      if( nq /= 0 ) then
          do iq=1,nq
             call remap_2dr8(km, pe1, q_r(is:ie,j:j,1:km,iq:iq),  &
                             kn, pe2,   q(is:ie,j:j,1:kn,iq:iq),  &
                             is, ie, 0, kord)
          enddo
      endif
...
! Geopotential conserving remap of virtual temperature:
       ...
       call remap_2dr8(km, pe1,ptv_r(is:ie,j:j,1:km),       &
...
!------
       ...
       call remap_2dr8(km, pv1, v_r(is:ie+1,j:j,1:km),       &
                       kn, pv2,   v(is:ie+1,j:j,1:kn),       &
                       is, ie+1, -1, kord)
...
1000  continue
 ...
 subroutine mappm_r8(km, pe1, q1, kn, pe2, q2, i1, i2, iv, kord)
...
!      in the new vertical coordinate
 ...
 real*8, intent(in ):: pe1(i1:i2,km+1), pe2(i1:i2,kn+1)
 real*8, intent(in )::  q1(i1:i2,km)
 real*8, intent(out)::  q2(i1:i2,kn)
! local
      real(p_precision) dp1(i1:i2,km)
      real(p_precision) a4(4,i1:i2,km)
      integer i, k, l
      integer k0, k1
      real(p_precision) pl, pr, tt, delp, qsum, dpsum, esl
...
5555  continue
 ...
 end subroutine mappm_r8

 subroutine mappm_r4(km, pe1, q1, kn, pe2, q2, i1, i2, iv, kord)
...
!      in the new vertical coordinate
 ...
 integer, intent(in):: i1, i2, km, kn, kord, iv
 real*4, intent(in ):: pe1(i1:i2,km+1), pe2(i1:i2,kn+1)
 real*4, intent(in )::  q1(i1:i2,km)
 real*4, intent(out)::  q2(i1:i2,kn)
! local
      real(p_precision) dp1(i1:i2,km)
      real(p_precision) a4(4,i1:i2,km)
      integer i, k, l
