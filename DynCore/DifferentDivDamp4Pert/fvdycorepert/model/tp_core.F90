...
!
 ...
 implicit none
      ...
      INTERFACE pert_ppm
        MODULE PROCEDURE pert_ppm_r4
        MODULE PROCEDURE pert_ppm_r8
        MODULE PROCEDURE pert_ppm_r4r8
      END INTERFACE
...
CONTAINS
 ...
 subroutine fv_tp_2d_r8(q, crx, cry, npx, npy, hord, fx, fy, &
   ...
   real*8, intent(inout):: q(isd:ied,jsd:jed)  ! transported scalar
   real*8, intent(out)::fx(is:ie+1 ,js:je)    ! Flux in x ( E )
   real*8, intent(out)::fy(is:ie,   js:je+1 )    ! Flux in y ( N )
! optional Arguments:
   real*8, OPTIONAL, intent(in):: mfx(is:ie+1,js:je  )  ! Mass Flux X-Dir
   real*8, OPTIONAL, intent(in):: mfy(is:ie  ,js:je+1)  ! Mass Flux Y-Dir
   real*8, OPTIONAL, intent(in):: ppm_fac               ! for ord=4 option
...
! Single-to-Double Copies of Outputs  
   ...
   call fv_tp_2d_compute(q, crx, cry, npx, npy, hord, fx, fy, &
                     xfx, yfx, area, ra_x, ra_y, mfx, mfy, ppm_fac, nord, damp_c)
 ...
 subroutine fv_tp_2d_r4(q, crx, cry, npx, npy, hord, fx, fy, &
                     xfx, yfx, area, ra_x, ra_y, mfx, mfy, ppm_fac, nord, damp_c)
   integer, intent(in):: npx, npy
   integer, intent(in)::hord

   real*4, intent(in)::  crx(is:ie+1,jsd:jed)  !
   real*4, intent(in)::  xfx(is:ie+1,jsd:jed)  !
   real*4, intent(in)::  cry(isd:ied,js:je+1 )  !
   real*4, intent(in)::  yfx(isd:ied,js:je+1 )  !
   real(g_precision), intent(in):: area(isd:ied,jsd:jed)
   real*4, intent(in):: ra_x(is:ie,jsd:jed)
   real*4, intent(in):: ra_y(isd:ied,js:je)
   real*4, intent(inout):: q(isd:ied,jsd:jed)  ! transported scalar
   real*4, intent(out)::fx(is:ie+1 ,js:je)    ! Flux in x ( E )
   real*4, intent(out)::fy(is:ie,   js:je+1 )    ! Flux in y ( N )
! optional Arguments:
   real*4, OPTIONAL, intent(in):: mfx(is:ie+1,js:je  )  ! Mass Flux X-Dir
   real*4, OPTIONAL, intent(in):: mfy(is:ie  ,js:je+1)  ! Mass Flux Y-Dir
   real*4, OPTIONAL, intent(in):: ppm_fac               ! for ord=4 option
   real*4, OPTIONAL, intent(in):: damp_c
   integer, OPTIONAL, intent(in):: nord

#ifndef SINGLE_FV
! Local Single-Precision Copies
   real ::  crx_r8(is:ie+1,jsd:jed)
   real ::  xfx_r8(is:ie+1,jsd:jed)
   real ::  cry_r8(isd:ied,js:je+1 )
   real ::  yfx_r8(isd:ied,js:je+1 )
   real :: ra_x_r8(is:ie,jsd:jed)
   real :: ra_y_r8(isd:ied,js:je)
   real ::    q_r8(isd:ied,jsd:jed)
   real ::  mfx_r8(is:ie+1,js:je  )
   real ::  mfy_r8(is:ie  ,js:je+1)
   real ::   fx_r8(is:ie+1 ,js:je)
   real ::   fy_r8(is:ie,   js:je+1 )
   real :: ppm_fac_r8
   real :: damp_c_r8
...
! Double-to-Single Copies of Inputs  
        ppm_fac_r8 = ppm_fac
         damp_c_r8 = damp_c
           ...
           ra_y_r8 = ra_y
              q_r8 = q
   if (present(mfx))  mfx_r8 = mfx
   if (present(mfy))  mfy_r8 = mfy

   call fv_tp_2d_compute(q_r8, crx_r8, cry_r8, npx, npy, hord, fx_r8, fy_r8, &
                     xfx_r8, yfx_r8, area, ra_x_r8, ra_y_r8, mfx_r8, mfy_r8, ppm_fac_r8, nord, damp_c_r8)
...
! Single-to-Double Copies of Outputs  
 ...
 subroutine fv_tp_2d_compute(q, crx, cry, npx, npy, hord, fx, fy, &
                     xfx, yfx, area, ra_x, ra_y, mfx, mfy, ppm_fac, nord, damp_c)
   integer, intent(in):: npx, npy
   integer, intent(in)::hord
...
!      Assume Periodicity in X-dir and not overlapping
 ...
 subroutine pert_ppm_r8(im, a0, al, ar, iv)
...
! effect of dm=0 included here
 ...
 end subroutine pert_ppm_r8

 subroutine pert_ppm_r4(im, a0, al, ar, iv)
 integer, intent(in):: im
 integer, intent(in):: iv
...
! effect of dm=0 included here
 ...
 end subroutine pert_ppm_r4

 subroutine pert_ppm_r4r8(im, a0, al, ar, iv)
 integer, intent(in):: im
 integer, intent(in):: iv
 real*4, intent(in)   :: a0(im)
 real*8, intent(inout):: al(im), ar(im)
