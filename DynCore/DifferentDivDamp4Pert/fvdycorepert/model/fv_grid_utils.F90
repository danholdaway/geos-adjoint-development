 ...
 use fv_arrays_mod,   only: fv_atmos_type, g_precision, m_precision, i_precision, p_precision
...
#endif
 real(i_precision), parameter::  big_number=1.E+35
 real(i_precision), parameter:: tiny_number=1.E-35
...
! latlon_2_cubed:
 ...
 real(i_precision), allocatable :: cosa_u(:,:)
 real(i_precision), allocatable :: cosa_v(:,:)
 real(i_precision), allocatable :: cosa_s(:,:)
 real(i_precision), allocatable :: sina_s(:,:)
 real(i_precision), allocatable :: sina_u(:,:)
 real(i_precision), allocatable :: sina_v(:,:)
 real(i_precision), allocatable :: rsin_u(:,:)
 real(i_precision), allocatable :: rsin_v(:,:)
 real(i_precision), allocatable ::  rsina(:,:)
 real(i_precision), allocatable ::  rsin2(:,:)
 real(i_precision), allocatable :: ec1(:,:,:)
 real(i_precision), allocatable :: ec2(:,:,:)
 real(i_precision), allocatable :: ew(:,:,:,:)
 real(i_precision), allocatable :: es(:,:,:,:)
...
! Unit vectors for lat-lon grid
 ...
 real(p_precision), parameter:: ptop_min=1.E-8
 real(p_precision)  :: ptop
 integer :: ks
 integer :: g_type, npxx, npyy
 integer :: c2l_ord
 integer :: i_sst, j_sst
 real(m_precision), allocatable:: sst_ncep(:,:)

 public fv_eta
 public ptop, ks, ptop_min, fC, f0, deglat, tiny_number, big_number, ew, es, eww, ess, ec1, ec2
 ...
 public normalize_vect, a11, a12, a21, a22
 ...
 public i_sst, j_sst, sst_ncep
      ...
      INTERFACE g_sum
        MODULE PROCEDURE g_sum_r4
        MODULE PROCEDURE g_sum_r8
      END INTERFACE

      INTERFACE global_mx
        MODULE PROCEDURE global_mx_r4
        MODULE PROCEDURE global_mx_r8
      END INTERFACE

      INTERFACE cubed_to_latlon
        MODULE PROCEDURE cubed_to_latlon_r4
        MODULE PROCEDURE cubed_to_latlon_r8
      END INTERFACE

      INTERFACE c2l_ord2
        MODULE PROCEDURE c2l_ord2_r4
        MODULE PROCEDURE c2l_ord2_r8
      END INTERFACE

      INTERFACE c2l_ord4
        MODULE PROCEDURE c2l_ord4_r4
        MODULE PROCEDURE c2l_ord4_r8
      END INTERFACE

      INTERFACE get_center_vect 
        MODULE PROCEDURE get_center_vect_r4
        MODULE PROCEDURE get_center_vect_r8
      END INTERFACE

      INTERFACE normalize_vect
        MODULE PROCEDURE normalize_vect_r4
        MODULE PROCEDURE normalize_vect_r8
      END INTERFACE

      INTERFACE vect_cross
        MODULE PROCEDURE vect_cross_r4
        MODULE PROCEDURE vect_cross_r8
      END INTERFACE

      INTERFACE v_prod
        MODULE PROCEDURE v_prod_r4
        MODULE PROCEDURE v_prod_r8
      END INTERFACE

      INTERFACE inner_prod
        MODULE PROCEDURE inner_prod_r4
        MODULE PROCEDURE inner_prod_r8
      END INTERFACE
 ...
 contains
   ...
   subroutine grid_utils_init(npx, npy, npz, grid, agrid, area, area_c,  &
...
!
      ...
      real(i_precision) sin2, tmp1, tmp2
      integer i, j, k, n
...
!            (i>npx .and. j>npy) .or. (i<1   .and. j>npy) ) then
           ...
           call vect_cross(ew(:,i,j,1), p2, pp)
           call normalize_vect(ew(:,i,j,1))
!---
           call vect_cross(p1, grid3(:,i,j), grid3(:,i,j+1))
           call vect_cross(ew(:,i,j,2), p1, pp)
           call normalize_vect(ew(:,i,j,2))
        endif
        enddo
     enddo
     ...
     do j=jsd+1,jed
        ...
        if ( (i<1   .and. j<1  ) .or. (i>(npx-1) .and. j<1  ) .or.  &
           ...
           call vect_cross(es(:,i,j,2), p2, pp)
           call normalize_vect(es(:,i,j,2))
!---
           call vect_cross(p3, grid3(:,i,j), grid3(:,i+1,j))
           call vect_cross(es(:,i,j,1), p3, pp)
           call normalize_vect(es(:,i,j,1))
        endif
        enddo
     enddo
  else
...
! unit vect in X-dir: ee1
              if (i==1) then
                  call vect_cross(pp, grid3(:,i,  j), grid3(:,i+1,j))
              elseif(i==npx) then
                  call vect_cross(pp, grid3(:,i-1,j), grid3(:,i,  j))
              else
                  call vect_cross(pp, grid3(:,i-1,j), grid3(:,i+1,j))
              endif
              call vect_cross(ee1(:,i,j), pp, grid3(:,i,j))
              call normalize_vect( ee1(:,i,j) )

! unit vect in Y-dir: ee2
              if (j==1) then
                  call vect_cross(pp, grid3(:,i,j  ), grid3(:,i,j+1))
              elseif(j==npy) then
                  call vect_cross(pp, grid3(:,i,j-1), grid3(:,i,j  ))
              else
                  call vect_cross(pp, grid3(:,i,j-1), grid3(:,i,j+1))
              endif
              call vect_cross(ee2(:,i,j), pp, grid3(:,i,j))
              call normalize_vect( ee2(:,i,j) )

              tmp1 = inner_prod(ee1(:,i,j), ee2(:,i,j))
              cosa(i,j) = sign(min(1.d0, abs(tmp1)), tmp1)
              sina(i,j) = sqrt(max(0.,1. -cosa(i,j)**2))
           enddo
        enddo
#endif
...
! The above does not work because cosa at edges should have two values (left and right)
      ...
      do j=jsd,jed
         do i=isd+1,ied
            if ( (ew(1,i,j,1) /= big_number) .and. (ew(1,i,j,2) /= big_number) ) then
                     tmp1 = inner_prod(ew(:,i,j,1), ew(:,i,j,2))
              cosa_u(i,j) = sign( min(1.d0, abs(tmp1)), tmp1 )
              sin2 = 1. - cosa_u(i,j)**2
              sin2 = min(1.d0, sin2)
              ...
              cosa_u(i,j) = big_number
              sina_u(i,j) = big_number
              rsin_u(i,j) = tiny_number
            endif
         enddo
      enddo
      ...
      do j=jsd+1,jed
         do i=isd,ied
            if ( (es(1,i,j,1) /= big_number) .and. (es(1,i,j,2) /= big_number) ) then
                     tmp1 = inner_prod(es(:,i,j,1), es(:,i,j,2))
              cosa_v(i,j) = sign( min(1.d0, abs(tmp1)), tmp1 )
              sin2 = 1. - cosa_v(i,j)**2
              sin2 = min(1.d0, sin2)
              ...
              cosa_v(i,j) = big_number
              sina_v(i,j) = big_number             
              rsin_v(i,j) = tiny_number
            endif
         enddo
      enddo
      ...
      do j=jsd,jed
         do i=isd,ied
            if ( (ec1(1,i,j) /= big_number) .and. (ec2(1,i,j) /= big_number) ) then
                    tmp1  = inner_prod(ec1(:,i,j), ec2(:,i,j))
              cosa_s(i,j) = sign(min(1.d0, abs(tmp1)), tmp1 )
              sin2 = 1. - cosa_s(i,j)**2
              sin2 = min(1.d0, sin2)
              sin2 = max(tiny_number, sin2)
              ...
              sina_s(i,j) = big_number
              sina_s(i,j) = big_number
               rsin2(i,j) = tiny_number
            endif
         enddo
      enddo
    !call mpp_update_domains(cosa_s, domain)
    !call mpp_update_domains(sina_s, domain)
    !call mpp_update_domains(rsin2 , domain)
! Force the model to fail if incorrect corner values are to be used:
    ...
    !call fill_ghost(rsin2 , npx, npy, tiny_number)
...
! for old d2a2c_vect routines
     do j=js-1,je+1
        if ( is==1 ) then
             i=1
             call vect_cross(ew(:,i,j,1), grid3(:,i,j+1), grid3(:,i,j)) 
             call normalize_vect( ew(:,i,j,1) )
        endif
        if ( (ie+1)==npx ) then
             i=npx
     ...
     if ( js==1 ) then
        j=1
        do i=is-1,ie+1
             call vect_cross(es(:,i,j,2), grid3(:,i,j),grid3(:,i+1,j)) 
             call normalize_vect( es(:,i,j,2) )
        enddo
     endif
     if ( (je+1)==npy ) then
...
! Unit vectors:
     do j=js,je+1
        do i=is,ie
           call vect_cross(en1(:,i,j), grid3(:,i,j), grid3(:,i+1,j))
           call normalize_vect( en1(:,i,j) )
        enddo
     enddo
     do j=js,je
        do i=is,ie+1
           call vect_cross(en2(:,i,j), grid3(:,i,j+1), grid3(:,i,j)) 
           call normalize_vect( en2(:,i,j) )
        enddo
     enddo
!-------------------------------------------------------------
...
!     endif
  ...
  real function inner_prod_r8(v1, v2)
       ...
       integer k
         ...
         inner_prod_r8 = prod16

  end function inner_prod_r8

  real function inner_prod_r4(v1, v2)
       real*4 ,intent(in):: v1(3), v2(3)
       real (f_p) :: vp1(3), vp2(3), prod16
       integer k
         ...
         enddo
         prod16 = vp1(1)*vp2(1) + vp1(2)*vp2(2) + vp1(3)*vp2(3)
         inner_prod_r4 = prod16
        
  end function inner_prod_r4
...
! q Normalized:
 ...
 subroutine vect_cross_r8(e, p1, p2)
...
!
 ...
 end subroutine vect_cross_r8

 subroutine vect_cross_r4(e, p1, p2)
 real, intent(in) :: p1(3), p2(3)
 real*4, intent(out):: e(3)
!            
! Perform cross products of 3D vectors: e = P1 X P2
!  
      e(1) = p1(2)*p2(3) - p1(3)*p2(2)
 ...
 end subroutine vect_cross_r4


 subroutine get_center_vect_r8( npx, npy, pp, u1, u2 )
    integer, intent(in):: npx, npy
    real, intent(in) :: pp(3,isd:ied+1,jsd:jed+1)
    real*8, intent(out):: u1(3,isd:ied,  jsd:jed)
    real*8, intent(out):: u2(3,isd:ied,  jsd:jed)
! Local:
    integer i,j,k
...
! e1:
          ...
          call vect_cross(u1(:,i,j), pc, p3)
          call normalize_vect( u1(:,i,j) )
! e2:
          call mid_pt3_cart(pp(1,i,j),   pp(1,i+1,j),   p1)
          call mid_pt3_cart(pp(1,i,j+1), pp(1,i+1,j+1), p2)
          call vect_cross(p3, p2, p1)
          call vect_cross(u2(:,i,j), pc, p3)
          call normalize_vect( u2(:,i,j) )
#else
          do k=1,3
 ...
 end subroutine get_center_vect_r8


 subroutine get_center_vect_r4( npx, npy, pp, u1, u2 )
    integer, intent(in):: npx, npy
    real, intent(in) :: pp(3,isd:ied+1,jsd:jed+1)
    real*4, intent(out):: u1(3,isd:ied,  jsd:jed)
    real*4, intent(out):: u2(3,isd:ied,  jsd:jed)
! Local:
    integer i,j,k
    real p1(3), p2(3), pc(3), p3(3)
...
! e2:
 ...
 end subroutine get_center_vect_r4


 subroutine normalize_vect_r8(e)
...
!endif
 ...
 end subroutine normalize_vect_r8

 subroutine normalize_vect_r4(e)
!                              Make e an unit vector
 real*4, intent(inout):: e(3) 
 real(f_p):: pdot
 integer k
...
!endif
 ...
 end subroutine normalize_vect_r4
...
!-------------------------------------
 ...
 real function great_circle_dist( q1, q2, radius )
      ...
      real(g_precision), intent(IN), optional :: radius
 ...
 subroutine intersect(a1,a2,b1,b2,radius,x_inter,local_a,local_b)
  ...
  real*8 function v_prod_r8(v1, v2)
  real*8 v1(3), v2(3)

       v_prod_r8 = v1(1)*v2(1) + v1(2)*v2(2) + v1(3)*v2(3)

  end function v_prod_r8

  real*8 function v_prod_r4(v1, v2)
  real*4 v1(3)
  real*8 v2(3)

       v_prod_r4 = v1(1)*v2(1) + v1(2)*v2(2) + v1(3)*v2(3)
        
  end function v_prod_r4           
...
!       do i=is-1,ie+1
     ...
     do j=js-1,je+1
        do i=is-1,ie+1
           z11(i,j) =  v_prod(ec1(:,i,j), vlon(i,j,1:3))
           z12(i,j) =  v_prod(ec1(:,i,j), vlat(i,j,1:3))
           z21(i,j) =  v_prod(ec2(:,i,j), vlon(i,j,1:3))
           z22(i,j) =  v_prod(ec2(:,i,j), vlat(i,j,1:3))
!-------------------------------------------------------------------------
           a11(i,j) =  0.5*v_prod(ec2(:,i,j), vlat(i,j,1:3)) / sina_s(i,j)
           a12(i,j) = -0.5*v_prod(ec1(:,i,j), vlat(i,j,1:3)) / sina_s(i,j)
           a21(i,j) = -0.5*v_prod(ec2(:,i,j), vlon(i,j,1:3)) / sina_s(i,j)
           a22(i,j) =  0.5*v_prod(ec1(:,i,j), vlon(i,j,1:3)) / sina_s(i,j)
        enddo
     enddo
  endif
 ...
 subroutine cubed_to_latlon_r8(u, v, ua, va, dx, dy, rdxa, rdya, km, mode)
 ...
 real*8, intent(inout):: u(isd:ied,jsd:jed+1,km)
 real*8, intent(inout):: v(isd:ied+1,jsd:jed,km)
 ...
 end subroutine cubed_to_latlon_r8

 subroutine cubed_to_latlon_r4(u, v, ua, va, dx, dy, rdxa, rdya, km, mode)
 integer, intent(in) :: km
 integer, intent(in), optional:: mode   ! update if present
 real, intent(in) :: dx(isd:ied,jsd:jed+1)
 real, intent(in) :: dy(isd:ied+1,jsd:jed)
 real, intent(in) ::rdxa(isd:ied,  jsd:jed)
 real, intent(in) ::rdya(isd:ied,  jsd:jed)
 real*4, intent(inout):: u(isd:ied,jsd:jed+1,km)
 real*4, intent(inout):: v(isd:ied+1,jsd:jed,km)
 real*4, intent(out):: ua(isd:ied, jsd:jed,km)
 real*4, intent(out):: va(isd:ied, jsd:jed,km)
 ...
 end subroutine cubed_to_latlon_r4



 subroutine c2l_ord4_r8(u, v, ua, va, dx, dy, rdxa, rdya, km, mode)
...
! Simple Cartesian Geometry:
 ...
 enddo
 end subroutine c2l_ord4_r8

 subroutine c2l_ord2_r8(u, v, ua, va, dx, dy, rdxa, rdya, km)
  integer, intent(in) :: km
...
! 2nd order:
 ...
 end subroutine c2l_ord2_r8

 subroutine c2l_ord4_r4(u, v, ua, va, dx, dy, rdxa, rdya, km, mode)
...
! Simple Cartesian Geometry:
 ...
 enddo
 end subroutine c2l_ord4_r4

 subroutine c2l_ord2_r4(u, v, ua, va, dx, dy, rdxa, rdya, km)
  integer, intent(in) :: km
  real*4, intent(in) ::  u(isd:ied,jsd:jed+1,km)
  real*4, intent(in) ::  v(isd:ied+1,jsd:jed,km)
  real, intent(in) :: dx(isd:ied,jsd:jed+1)
  real, intent(in) :: dy(isd:ied+1,jsd:jed)
  real, intent(in) ::rdxa(isd:ied,  jsd:jed)
  real, intent(in) ::rdya(isd:ied,  jsd:jed)
...
! Vector Q:
 ...
 real function g_sum_r4(p, ifirst, ilast, jfirst, jlast, ngc, area, mode, reproduce)
! Fast version of globalsum 
      ...
      real*4, intent(IN) :: p(ifirst:ilast,jfirst:jlast)      ! field to be summed
      real*8, intent(IN) :: area(ifirst-ngc:ilast+ngc,jfirst-ngc:jlast+ngc)
      integer :: i,j
      real*4 gsum
...
!-------------------------
      ...
      if ( mode==1 ) then
           g_sum_r4 = gsum / global_area
      else
           g_sum_r4 = gsum
      endif

 end function g_sum_r4

 real function g_sum_r8(p, ifirst, ilast, jfirst, jlast, ngc, area, mode, reproduce)
! Fast version of globalsum      
      integer, intent(IN) :: ifirst, ilast
      integer, intent(IN) :: jfirst, jlast, ngc
      integer, intent(IN) :: mode  ! if ==1 divided by area
      logical, intent(in), optional :: reproduce
      real*8, intent(IN) :: p(ifirst:ilast,jfirst:jlast)      ! field to be summed
      real*8, intent(IN) :: area(ifirst-ngc:ilast+ngc,jfirst-ngc:jlast+ngc)
      integer :: i,j
      real*8 gsum
...
! quick global sum without area weighting
 ...
 subroutine global_mx_r8(q, n_g, qmin, qmax)
     integer, intent(in):: n_g
     real*8, intent(in)::q(is-n_g:ie+n_g, js-n_g:je+n_g)
 ...
 end subroutine global_mx_r8

 subroutine global_mx_r4(q, n_g, qmin, qmax)
     integer, intent(in):: n_g
     real*4, intent(in)::q(is-n_g:ie+n_g, js-n_g:je+n_g)
     real*4, intent(out):: qmin, qmax
     integer i,j
...
! For computing global max/min at cell Corners
  ...
  subroutine fill_ghost_r8(q, npx, npy, value)
  real*8, intent(inout):: q(isd:ied,jsd:jed)
  integer, intent(in):: npx, npy
  real(i_precision), intent(in):: value
  integer i,j
  ...
  subroutine fill_ghost_r4(q, npx, npy, value)
 ...
 subroutine make_eta_level(km, pe, area, kks, ak, bk)
  ...
  real(m_precision), intent(out):: ak(km+1), bk(km+1)
! local:
  real ph(km+1)
...
! Make it the same acroos all PEs
        ph(k) = g_sum(pem, is, ie, js, je, ng, area, 1)
     enddo
