...
! scheme 2.1: perturbation form
  ...
  private
  public :: c_sw, d_sw, d2a2c, d2a2c_vect, divergence_corner, &
            xtp_u, ytp_v
...
! Local:
      ...
      real(i_precision) :: r1b4 = 1.0/4.0
      real(i_precision) :: r1b16 = 1.0/16.0

      real(ke_precision), dimension(is:ie+1,js:je+1):: ub, vb
      real(ke_precision) :: ke(isd:ied+1,jsd:jed+1) !  needs this for corner_comm
      ...
      real ::   px(is:ie+1,js:je  )  ! 1-D X-direction Fluxes
      real ::   py(is:ie  ,js:je+1)  ! 1-D Y-direction Fluxes
...
! Interior:
        do j=jsd,jed
           if(j/=0 .and. j/=1 .and. j/=(npy-1) .and. j/=npy) then
             do i=is-1,ie+2
                ut(i,j) = ( uci(i,j) - r1b4 * cosa_u(i,j) *     &
                    (vci(i-1,j)+vci(i,j)+vci(i-1,j+1)+vci(i,j+1)))*rsin_u(i,j)
             enddo
           endif
        enddo
        ...
        do j=js-1,je+2
           if( j/=1 .and. j/=npy ) then
              do i=isd,ied
                 vt(i,j) = ( vci(i,j) - r1b4 * cosa_v(i,j) *     &
                    (uci(i,j-1)+uci(i+1,j-1)+uci(i,j)+uci(i+1,j)))*rsin_v(i,j)
              enddo
           endif
        enddo
...
!             vt(0,j) = vci(0,j) - r1b4*cosa_v(0,j)*   &
              vt(0,j) = vci(0,j) + r1b4*cosa_v(1,j)*   &
                       (ut(0,j-1)+ut(1,j-1)+ut(0,j)+ut(1,j))
              vt(1,j) = vci(1,j) - r1b4*cosa_v(1,j)*   &
...
! East edge:
        if ( (ie+1)==npx ) then
           ...
           enddo
           do j=max(3,js), min(npy-2,je+1)
              vt(npx-1,j) = vci(npx-1,j) - r1b4*cosa_v(npx-1,j)*   &
                           (ut(npx-1,j-1)+ut(npx,j-1)+ut(npx-1,j)+ut(npx,j))
!             vt(npx,j) = vci(npx,j) - r1b4*cosa_v(npx,j)*   &
              vt(npx,j) = vci(npx,j) + r1b4*cosa_v(npx-1,j)*   &
...
!             ut(i,0) = uci(i,0) - r1b4*cosa_u(i,0)*   &
              ut(i,0) = uci(i,0) + r1b4*cosa_u(i,1)*   &
                       (vt(i-1,0)+vt(i,0)+vt(i-1,1)+vt(i,1))
              ut(i,1) = uci(i,1) - r1b4*cosa_u(i,1)*   &
...
! North edge:
        if ( (je+1)==npy ) then
           ...
           enddo
           do i=max(3,is),min(npx-2,ie+1)
              ut(i,npy-1) = uci(i,npy-1) - r1b4*cosa_u(i,npy-1)*   &
                           (vt(i-1,npy-1)+vt(i,npy-1)+vt(i-1,npy)+vt(i,npy))
!             ut(i,npy) = uci(i,npy) - r1b4*cosa_u(i,npy)*   &
              ut(i,npy) = uci(i,npy) + r1b4*cosa_u(i,npy-1)*   &
        ...
        if( sw_corner ) then
            ...
            ut(2,0) = (uci(2,0)-r1b4*cosa_u(2,0)*(vt(1,1)+vt(2,1)+vt(2,0)+vci(1,0) -   &
                      r1b4*cosa_v(1,0)*(ut(1,0)+ut(1,-1)+ut(2,-1))) ) * damp
            ut(2,1) = (uci(2,1)-r1b4*cosa_u(2,1)*(vt(1,1)+vt(2,1)+vt(2,2)+vci(1,2) -   &
                      r1b4*cosa_v(1,2)*(ut(1,1)+ut(1,2)+ut(2,2))) ) * damp
            vt(1,2) = (vci(1,2)-r1b4*cosa_v(1,2)*(ut(1,1)+ut(1,2)+ut(2,2)+uci(2,1) -   &
                      r1b4*cosa_u(2,1)*(vt(1,1)+vt(2,1)+vt(2,2))) ) * damp
            vt(0,2) = (vci(0,2)-r1b4*cosa_v(0,2)*(ut(1,1)+ut(1,2)+ut(0,2)+uci(0,1) -   &
                      r1b4*cosa_u(0,1)*(vt(0,1)+vt(-1,1)+vt(-1,2))) ) * damp
        endif

        if( se_corner ) then
            damp = 1.0 / (1.0 - r1b16*cosa_u(npx-1,1)*cosa_v(npx-1,2))
            ut(npx-1,0) = ( uci(npx-1,0)+r1b4*cosa_u(npx-1,1)*(   &
                            vt(npx-1,1)+vt(npx-2,1)+vt(npx-2,0)+vci(npx-1,0) +   &
                      r1b4*cosa_v(npx-1,2)*(ut(npx,0)+ut(npx,-1)+ut(npx-1,-1))) ) * damp
            ut(npx-1,1) = ( uci(npx-1,1)-r1b4*cosa_u(npx-1,1)*(  &
                            vt(npx-1,1)+vt(npx-2,1)+vt(npx-2,2)+vci(npx-1,2) -   &
                      r1b4*cosa_v(npx-1,2)*(ut(npx,1)+ut(npx,2)+ut(npx-1,2))) ) * damp
            vt(npx-1,2) = ( vci(npx-1,2)-r1b4*cosa_v(npx-1,2)*(  &
                            ut(npx,1)+ut(npx,2)+ut(npx-1,2)+uci(npx-1,1) -   &
                      r1b4*cosa_u(npx-1,1)*(vt(npx-1,1)+vt(npx-2,1)+vt(npx-2,2))) ) * damp
            vt(npx,  2) = ( vci(npx,2)+r1b4*cosa_v(npx-1,2)*(  &
                            ut(npx,1)+ut(npx,2)+ut(npx+1,2)+uci(npx+1,1) +   &
                      r1b4*cosa_u(npx-1,1)*(vt(npx,1)+vt(npx+1,1)+vt(npx+1,2))) ) * damp
        endif

        if( ne_corner ) then
            damp = 1.0 / (1.0 - r1b16*cosa_u(npx-1,npy-1)*cosa_v(npx-1,npy-1))
            ut(npx-1,npy) = ( uci(npx-1,npy)+r1b4*cosa_u(npx-1,npy-1)*(   &
                              vt(npx-1,npy)+vt(npx-2,npy)+vt(npx-2,npy+1)+vci(npx-1,npy+1) +   &
                r1b4*cosa_v(npx-1,npy-1)*(ut(npx,npy)+ut(npx,npy+1)+ut(npx-1,npy+1))) ) * damp
            ut(npx-1,npy-1) = ( uci(npx-1,npy-1)-r1b4*cosa_u(npx-1,npy-1)*(  &
                                vt(npx-1,npy)+vt(npx-2,npy)+vt(npx-2,npy-1)+vci(npx-1,npy-1) -  &
                r1b4*cosa_v(npx-1,npy-1)*(ut(npx,npy-1)+ut(npx,npy-2)+ut(npx-1,npy-2))) ) * damp
            vt(npx-1,npy-1) = ( vci(npx-1,npy-1)-r1b4*cosa_v(npx-1,npy-1)*(  &
                                ut(npx,npy-1)+ut(npx,npy-2)+ut(npx-1,npy-2)+uci(npx-1,npy-1) -  &
                r1b4*cosa_u(npx-1,npy-1)*(vt(npx-1,npy)+vt(npx-2,npy)+vt(npx-2,npy-1))) ) * damp
            vt(npx,  npy-1) = ( vci(npx,npy-1)+r1b4*cosa_v(npx-1,npy-1)*(   &
                                ut(npx,npy-1)+ut(npx,npy-2)+ut(npx+1,npy-2)+uci(npx+1,npy-1) +   &
                r1b4*cosa_u(npx-1,npy-1)*(vt(npx,npy)+vt(npx+1,npy)+vt(npx+1,npy-1))) ) * damp
        endif

        if( nw_corner ) then
            damp = 1.0 / (1.0 - r1b16*cosa_u(2,npy-1)*cosa_v(1,npy-1))
            ut(2,npy) = ( uci(2,npy)+r1b4*cosa_u(2,npy-1)*(   &
                          vt(1,npy)+vt(2,npy)+vt(2,npy+1)+vci(1,npy+1) +   &
                      r1b4*cosa_v(1,npy-1)*(ut(1,npy)+ut(1,npy+1)+ut(2,npy+1))) ) * damp
            ut(2,npy-1) = ( uci(2,npy-1)-r1b4*cosa_u(2,npy-1)*(  &
                            vt(1,npy)+vt(2,npy)+vt(2,npy-1)+vci(1,npy-1) -   &
                      r1b4*cosa_v(1,npy-1)*(ut(1,npy-1)+ut(1,npy-2)+ut(2,npy-2))) ) * damp
            vt(1,npy-1) = ( vci(1,npy-1)-r1b4*cosa_v(1,npy-1)*(  &
                            ut(1,npy-1)+ut(1,npy-2)+ut(2,npy-2)+uci(2,npy-1) -   &
                      r1b4*cosa_u(2,npy-1)*(vt(1,npy)+vt(2,npy)+vt(2,npy-1))) ) * damp
            vt(0,npy-1) = ( vci(0,npy-1)+r1b4*cosa_v(1,npy-1)*(  &
                            ut(1,npy-1)+ut(1,npy-2)+ut(0,npy-2)+uci(0,npy-1) +   &
                      r1b4*cosa_u(2,npy-1)*(vt(0,npy)+vt(-1,npy)+vt(-1,npy-1))) ) * damp
        endif
            ...
            do j=js,je
               do i=is,ie
                  w(i,j) = w(i,j)*delp(i,j) +             &
                           (px(i,j)-px(i+1,j)+py(i,j)-py(i,j+1))*rarea(i,j)
               enddo
            enddo
        endif
        ...
        do j=js,je
           do i=is,ie
              ...
              pt(i,j) = (pt(i,j)*wk(i,j) +               &
                        (px(i,j)-px(i+1,j)+py(i,j)-py(i,j+1))*rarea(i,j))/delp(i,j)
           enddo
        enddo
        do iq=1,nq
           call fv_tp_2d(q(isd:,jsd:,k,iq), crx_adv,cry_adv, npx, npy, hord_tr, px, py,  &
                         xfx_adv,yfx_adv, area, ra_x, ra_y, mfx=fx, mfy=fy)
           do j=js,je
              do i=is,ie
...
!   endif
    ...
    endif ! shallow_water test_case 1
...
!---------------------------------------------------------------
           ...
           if ( (ie+1)==npx ) then
              ...
              if( j==1 .or. j==npy ) then
                ...
                br(npx-1) = xt+xt - u(npx-1,j)
                xt = t14*u(npx,j) + t12*u(npx+1,j) + t15*u(npx+2,j)
                bl(npx  ) = xt+xt - u(npx  ,j)
#else
                xt = 0.5*( (2.*dx(npx-1,j)+dx(npx-2,j))*(u(npx-1,j)+u(npx,j))  &
                   - dx(npx-1,j)*(u(npx-2,j)+u(npx+1,j)))/(dx(npx-1,j)+dx(npx-2,j))
                br(npx-1) = xt - u(npx-1,j)
...
!               bl(npx  ) = xt - 0.5*(v(npx,j-1)+v(npx,j))*cosa(npx,j) - u(npx  ,j)
 ...
 real, INTENT(IN)  ::   u(isd:ied,jsd:jed+1)
 real, INTENT(IN)  ::   v(isd:ied+1,jsd:jed)
 real(ke_precision), INTENT(IN) ::    c(is:ie+1,js:je+1)   !  Courant   N (like FLUX)
 real(ke_precision), INTENT(OUT):: flux(is:ie+1,js:je+1)
! Local:
...
!           bl(i,1) = xt + 0.5*(u(i-1,1)+u(i,1))*cosa(i,1) - v(i,1)
      ...
      if( (je+1)==npy ) then
         do i=is,ie+1
            ...
            br(i,npy-1) = xt+xt - v(i,npy-1)
            xt = t14*v(i,npy) + t12*v(i,npy+1) + t15*v(i,npy+2)
            bl(i,npy  ) = xt+xt - v(i,npy)
#else
            xt = 0.5*((2.*dy(i,npy-1)+dy(i,npy-2))*(v(i,npy-1)+v(i,npy)) -  &
                 dy(i,npy-1)*(v(i,npy-2)+v(i,npy+1)))/(dy(i,npy-1)+dy(i,npy-2))
            br(i,npy-1) = xt - v(i,npy-1)
