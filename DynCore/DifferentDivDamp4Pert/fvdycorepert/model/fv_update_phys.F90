...
#endif
  use nwp_nudge_mod,      only: do_nwp_nudge
...
! All fields will be updated excdept winds; wind tendencies added
    if ( nudge ) then
...
!$omp parallel do default(shared) private(i, j, k)
     ...
     call do_nwp_nudge ( Time, dt, npz,  ps_dt, u_dt, v_dt, t_dt, q_dt,  & 
...
! Edges:
    if ( js==1 ) then
       do i=is,ie
          ...
          u(i,1,k) = u(i,1,k) + dt5*((2.+gratio)*(u_dt(i,0,k)+u_dt(i,1,k))-(u_dt(i,-1,k)+u_dt(i,2,k)))/(1.+gratio)
       enddo
    endif
...
! Interior
    ...
    if ( (je+1)==npy ) then
       do i=is,ie
          ...
          u(i,npy,k) = u(i,npy,k) + dt5*((2.+gratio)*(u_dt(i,npy-1,k)+u_dt(i,npy,k))-(u_dt(i,npy-2,k)+u_dt(i,npy+1,k)))/(1.+gratio)
       enddo
    endif
...
! West Edges:
    if ( is==1 ) then
       do j=js,je
          ...
          v(1,j,k) = v(1,j,k) + dt5*((2.+gratio)*(v_dt(0,j,k)+v_dt(1,j,k))-(v_dt(-1,j,k)+v_dt(2,j,k)))/(1.+gratio)
       enddo
    endif
...
! East Edges:
    if ( (ie+1)==npx ) then
       do j=js,je
          ...
          v(npx,j,k) = v(npx,j,k) + dt5*((2.+gratio)*(v_dt(npx-1,j,k)+v_dt(npx,j,k))-(v_dt(npx-2,j,k)+v_dt(npx+1,j,k)))/(1.+gratio)
       enddo
    endif
